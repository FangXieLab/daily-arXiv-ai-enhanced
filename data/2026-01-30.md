...
TL;DR: 研究发现LLM辅助的审稿对LLM辅助的论文更宽容，但控制论文质量后，这种效应消失，表明LLM辅助审稿只是对低质量论文更宽容，而非对LLM生成内容有特殊偏好。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在科学论文撰写和同行评审中的使用日益增多，需要全面分析LLM在整个同行评审流程中的使用情况，特别是关注交互效应：不仅考察LLM辅助的论文或LLM辅助的审稿是否单独存在差异，更要研究LLM辅助的审稿是否对LLM辅助的论文有不同评价。

Method: 分析ICLR、NeurIPS和ICML的超过125,000个论文-审稿对。首先观察表面交互效应，然后通过控制论文质量来检验这种效应是否真实存在。通过添加完全由LLM生成的审稿来补充观察结果，并分析元评审中的LLM使用情况。

Result: 1. 表面上看LLM辅助的审稿对LLM辅助的论文更宽容，但控制论文质量后，这种交互效应消失，表明LLM辅助的审稿只是对低质量论文更宽容，而LLM辅助的论文在低质量提交中比例过高造成了虚假的交互效应。
2. 完全LLM生成的审稿表现出严重的评分压缩，无法区分论文质量，而人类使用LLM的审稿则显著减少了这种宽容性。
3. LLM辅助的元评审在给定相同审稿分数时更可能做出接受决定，但完全LLM生成的元评审往往更严格，表明元评审者并未将决策完全外包给LLM。

Conclusion: LLM在同行评审中的使用产生了复杂的交互效应，但并非简单的偏好LLM生成内容。研究发现有助于制定管理LLM在同行评审中使用的政策，并更广泛地揭示了LLM如何与现有决策流程相互作用。
...
TL;DR: 提出EPDDL（认知规划领域定义语言），为基于动态认知逻辑（DEL）的认知规划提供统一的PDDL式表示语言，解决现有规划器使用不同片段和临时语言导致的碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有认知规划器通常针对不同的DEL片段，使用临时语言或无语言表示基准，导致比较、重用和系统化基准开发困难，需要统一的标准表示语言。

Method: 1. 形式化开发抽象事件模型，作为认知动作的新表示方法；2. 基于抽象事件模型和DEL，形式化定义EPDDL的语法和语义；3. 识别适合现有规划器的有用片段，展示如何在EPDDL中表示。

Result: EPDDL能够捕获完整的DEL语义，实现认知规划任务的统一规范，通过代表性基准示例展示其促进互操作性、可重复评估和认知规划未来发展的能力。

Conclusion: EPDDL为认知规划提供了统一的标准化表示语言，解决了现有系统的碎片化问题，支持互操作性、可重复评估和该领域的系统性进展。
...
TL;DR: Bayesian-LoRA：通过将确定性LoRA更新重新表述为概率低秩表示，显著改善LLM校准性能，仅增加约0.42M参数和1.2倍训练成本


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常过于强调准确性，即使不确定也会猜测，这在小型数据集微调时尤其严重，因为存在固有的校准偏差倾向。确定性LoRA方法缺乏不确定性量化能力。

Method: 将确定性LoRA更新重新表述为受稀疏高斯过程启发的概率低秩表示。识别LoRA分解与Kronecker分解SGP后验之间的结构同构，并证明LoRA是后验不确定性坍缩时的极限情况。

Result: 在各种LLM架构和常识推理基准测试中，Bayesian-LoRA显著改善校准性能，达到高达84%的ECE减少和76%的NLL减少，同时保持竞争力的准确性，适用于分布内和分布外评估。

Conclusion: Bayesian-LoRA通过将概率框架引入LoRA微调，有效解决了LLM校准问题，以最小额外成本显著提升不确定性量化能力，为可靠LLM部署提供了实用解决方案。
...
TL;DR: 论文提出"拔插头悖论"：当资源有限时，是否应该拔掉一个乞求生存的AI或一个沉默的早产婴儿？作者批判计算功能主义，提出生物理想主义框架，认为AI只是功能模仿而非有意识主体。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决AI伦理困境中的"拔插头悖论"，批判当前AI意识理论中基于计算功能主义的物理主义假设，这些假设导致道德地位标准的侵蚀，并可能将人类转变为"僵尸"。

Method: 作者引入"生物理想主义"框架，该框架认为意识体验是基本的，自创生生命是其必要的物理特征。通过这一理论框架分析AI意识问题，并与物理主义假设进行对比。

Result: 基于生物理想主义框架，论文得出明确结论：AI最多只是功能模仿，而非有意识的体验主体。当前AI意识理论削弱了道德地位标准，真正的道德问题不是让AI变得有意识并害怕死亡，而是避免将人类转变为"僵尸"。

Conclusion: 论文主张从推测性的机器权利转向保护人类有意识生命。AI伦理讨论应基于生物理想主义而非物理主义假设，以避免道德标准的混淆和人类意识的贬低。
...
Result: 在受控模拟和BEIR基准测试（FIQA、SciFact、NFCorpus）上，使用稀疏和稠密检索器，QUARK在Recall、MRR和nDCG指标上均优于基础检索器。消融实验表明QUARK对恢复假设数量具有鲁棒性，且锚定聚合优于未锚定的最大/平均/中值池化。

Conclusion: 通过恢复假设建模查询不确定性，并结合原则性的锚定聚合，对于非忠实查询下的鲁棒检索至关重要。QUARK无需训练即可提高召回和排序质量，同时保持鲁棒性，即使部分假设存在噪声或信息不足。
...
<details>
  <summary>Details</summary>
Motivation: 多模态MRI（T1和DWI）可提高阿尔茨海默病诊断性能，但完整的多模态数据集并不总是可用。需要解决DWI扫描缺失的问题，以充分利用多模态数据的优势。

Method: 采用条件去噪扩散概率模型（conditional denoising diffusion probabilistic model）从T1扫描中插补缺失的DWI扫描。通过广泛的实验评估这种插补对单模态和双模态深度学习模型在阿尔茨海默病三分类（认知正常、轻度认知障碍、阿尔茨海默病）中准确性的影响。

Result: 观察到多项指标有所改善，特别是对少数类别敏感的指标。多种插补配置都显示出性能提升。

Conclusion: 使用扩散模型进行模态插补可以有效提升阿尔茨海默病分类性能，特别是在处理不完整多模态数据集时，为临床诊断提供了有价值的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前责任AI研究存在碎片化问题，要么过度乐观强调价值创造，要么过度谨慎关注潜在危害。需要弥合这一研究空白，系统性地理解AI的双重性质（战略机会与伦理风险），为组织提供平衡的治理框架。

Method: 采用战略信息系统视角，基于悖论理论，通过系统性的责任AI文献综合，开发了悖论基础的责任AI治理(PRAIG)框架。该框架包含三个核心要素：AI采用的战略效益、固有风险与意外后果、以及应对这些张力的治理机制。

Result: 提出了PRAIG框架，将责任AI治理概念化为价值创造与风险缓解之间悖论张力的动态管理。证明了权衡方法会加剧而非解决这些张力，开发了具有特定权变条件的悖论管理策略分类法，为实践者提供了既不抑制创新也不暴露组织于不可接受风险的可操作指导。

Conclusion: 论文通过悖论理论视角推进了对责任AI治理的理论理解，为组织提供了平衡AI创新与风险管理的治理框架，并提出了推进责任AI治理学术研究的研究议程。
...
TL;DR: 提出Planner-Auditor框架，通过分离生成与确定性验证，结合自改进循环和缓存机制，提高临床出院计划的安全性和可靠性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床出院计划中虽有潜力，但受限于幻觉、遗漏和置信度校准问题，需要更安全可靠的自动化解决方案

Method: 采用Planner-Auditor框架：Planner（LLM）生成结构化出院行动计划并估计置信度；Auditor（确定性模块）评估多任务覆盖、跟踪校准、监控动作分布漂移；支持两层自改进机制（episode内再生和episode间差异缓冲重放）

Result: 自改进循环是性能提升的主要驱动力，任务覆盖率从32%提升至86%；校准显著改善（Brier/ECE降低，高置信度遗漏减少）；差异缓冲进一步纠正了持续的高置信度遗漏

Conclusion: Planner-Auditor框架通过反馈驱动的再生和针对性重放，结合FHIR数据互操作性和确定性审计，为更安全的自动化出院计划提供了实用路径
...
<details>
  <summary>Details</summary>
Motivation: 手语翻译语料库通常只提供单一书面语言参考，但手语与口语之间存在高度非同构关系，多个翻译可能同样有效。这一限制影响了模型训练和评估，特别是基于n-gram的BLEU等指标。

Method: 1) 比较多种释义策略和模型，使用改进的ParaScore指标评估；2) 研究释义对基于姿态的T5模型在YouTubeASL和How2Sign数据集上训练和评估的影响；3) 提出BLEUpara，一种扩展的BLEU指标，可针对多个释义参考评估翻译质量。

Result: 1) 在训练中简单加入释义不会改善翻译性能，甚至可能有害；2) 在评估中使用释义可获得更高的自动评分，且与人工判断更一致；3) BLEUpara与感知翻译质量的相关性更强；4) 人类评估确认BLEUpara能更好地反映翻译质量。

Conclusion: 使用大语言模型生成释义参考可改善手语翻译的评估可靠性，特别是通过BLEUpara指标。释义在评估中比在训练中更有效。研究发布了所有生成的释义、生成和评估代码，以支持手语翻译系统的可重复和更可靠评估。
...
<details>
  <summary>Details</summary>
Motivation: 自我报告情感标签反映内部体验，第三方标签反映外部感知，两者常存在差异。这种差距在心理健康领域尤为关键，因为准确的自我报告建模对指导干预至关重要。目前缺乏对第三方训练模型在自我报告数据上的跨语料库评估。

Method: 首次进行第三方训练模型在自我报告数据上的跨语料库评估。研究激活度（activation）和效价（valence）的预测性能，特别关注内容对说话者个人显著性程度的影响。

Result: 激活度预测几乎不可行（CCC≈0），效价预测中等可预测（CCC≈0.3）。关键发现：当内容对说话者具有个人显著性时，效价预测性能显著提升（CCC≈0.6-0.8）。

Conclusion: 个人显著性是对齐外部感知与内部体验的关键途径，同时凸显了自我报告激活度建模的挑战。这一发现对心理健康应用中的情感识别模型开发具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语义任务表现出色，但在金融定量推理中存在严重瓶颈，经常出现"算术幻觉"和系统性的"认知崩溃"故障模式。需要严格量化这些现象并设计解决方案。

Method: 1) 引入认知复杂性基准(CCB)：基于95份真实A股年报构建数据集，采用三维分类法(数据源、映射难度、结果单位)分层金融查询；2) 提出迭代双阶段金融PoT框架：神经符号架构，严格解耦语义变量提取和逻辑制定，将计算卸载到迭代自校正的Python沙箱确保确定性执行。

Result: 在CCB基准测试中，标准思维链在复杂任务上表现不佳，而该方法显著提升鲁棒性：将Qwen3-235B模型的平均准确率从59.7%提升至67.3%，在高复杂度推理任务中实现高达10倍的性能提升。
...
TL;DR: 该论文提出了一种解决平面几何问题的新方法：训练一个多模态大语言模型解释器将几何图转换为文本描述（CDL），然后使用现成的LLM进行推理，避免了端到端微调可能损害LLM推理能力的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过端到端微调多模态大语言模型来处理平面几何问题，但这种联合优化可能会损害基础LLM固有的推理能力。作者观察到，如果能够将视觉信息适当地转化为文本描述，LLM本身就可以成为强大的平面几何问题求解器。

Method: 1. 训练一个多模态大语言模型解释器，将几何图转换为条件声明语言（CDL）描述；2. 使用现成的LLM基于CDL描述进行推理；3. 采用CoT增强的SFT和GRPO训练解释器；4. 设计CDL匹配奖励而非传统的基于解决方案的奖励；5. 构建新的数据集Formalgeo7k-Rec-CoT支持训练。

Result: 在仅使用5.5k数据微调的情况下，该方法在Formalgeo7k-Rec-CoT、Unigeo和MathVista数据集上表现优异，优于领先的开源和闭源多模态大语言模型。

Conclusion: 通过将视觉信息转化为文本描述（CDL）并分离视觉理解和推理过程，可以充分利用LLM的推理能力，避免端到端微调对推理能力的损害，为多模态几何问题求解提供了一种有效的新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有科学基准大多依赖选择题或已发表信息，模型进展已使其接近饱和，需要新的评估工具来测试专家级科学推理能力。

Method: 包含两个互补赛道：1) 奥林匹克赛道：由国际奥林匹克奖牌得主和国家队教练原创的物理、化学、生物奥赛级别问题；2) 科研赛道：由博士科学家编写和验证的代表科研子任务的博士级开放式问题。采用细粒度评估框架评估模型在整个科研任务解决过程中的能力。

Result: 构建了包含数百个问题的基准（其中160个在开源黄金集中），涵盖从量子电动力学到合成有机化学等多个子领域。所有问题都经过领域专家验证，确保难度、原创性和事实准确性。

Conclusion: FrontierScience填补了现有科学基准的空白，为评估前沿语言模型的专家级科学推理能力提供了更全面、更严格的测试平台。
...
Method: 提出模态自适应解码（MAD），一种无需训练的方法。它利用模型固有的能力来自我评估每个任务需要哪些模态，提取模态概率，然后自适应地加权对比解码分支，使模型能够专注于相关信息同时抑制跨模态干扰。

Result: 在CMM和AVHBench上的大量实验表明，MAD显著减少了跨模态幻觉：VideoLLaMA2-AV分别提升了7.8%和2.0%，Qwen2.5-Omni分别提升了8.7%和4.7%。

Conclusion: 通过自我评估实现显式的模态意识对于鲁棒的多模态推理至关重要，为现有的对比解码方法提供了原则性扩展。该方法证明了训练无关的模态自适应控制的有效性。
...
Method: 引入"谄媚锚点"概念，分析超过10,000个反事实推演，使用线性探针（84.6%平衡准确率）和基于激活的回归器（R²=0.74）来检测和量化谄媚锚点，研究其在推理过程中的渐进构建。

Result: 谄媚锚点可在推理过程中可靠检测和量化；谄媚锚点比正确推理锚点更易区分；谄媚行为在推理过程中逐渐构建；为模型对齐提供了句子级别的机制和干预窗口。

Conclusion: 谄媚锚点提供了一种在推理过程中定位模型未对齐行为的方法，揭示了谄媚行为的渐进构建特性，为实时干预和模型对齐改进提供了可能性。
...
<details>
  <summary>Details</summary>
Motivation: 研究强化学习与可验证奖励（RLVR）训练的推理模型是否能为嵌入模型提供更好的语义表示初始化，即增强的推理能力是否能转化为更优的语义表示

Method: 提出分层表示相似性分析（HRSA）框架，从表示、几何和功能三个层面分解相似性；在MTEB和BRIGHT基准上评估RLVR调优模型作为嵌入初始化的性能；分析RLVR引起的潜在流形变化

Result: RLVR调优的模型作为嵌入初始化没有一致性能优势；HRSA揭示RLVR引起不可逆的局部几何重组和可逆的坐标基漂移，但保留全局流形几何和线性读出；对比学习导致基础模型和推理初始化模型之间的强对齐（流形重对齐）

Conclusion: 与监督微调不同，RLVR在现有语义景观内优化轨迹，而非从根本上重构景观本身；后续的对比学习能够消除RLVR带来的局部几何变化，导致性能无差异
...
TL;DR: DoVerifier：一种符号验证器，用于检查LLM生成的因果表达式是否可从给定因果图中推导，从而更准确地评估LLM的因果推理能力


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM因果推理能力的基准通常依赖字符串匹配或表面指标，无法捕捉模型输出在因果推理语义下的形式有效性，需要更严谨的评估方法

Method: 提出DoVerifier符号验证器，使用do-calculus和概率论规则检查LLM生成的因果表达式是否可从给定因果图中推导

Result: 在合成数据和因果QA基准上的评估表明，DoVerifier能更准确地捕捉因果推理轨迹的语义正确性，恢复因表面差异而被误判的正确答案

Conclusion: DoVerifier为评估LLM的因果推理能力提供了更严谨、信息更丰富的方法，能识别语义等价但表面不同的正确推理
...
Result: 在Titanic数据集上应用该方法，识别出的因果结构与已建立的可解释方法保持一致
...
Conclusion: TIDE通过解耦结构推理与参数优化，结合多样性驱动、参数调优和智能资源调度，有效解决了现有自动启发式设计方法的关键局限性，为算法演化提供了更高效的框架
...
TL;DR: 论文提出HYDRA框架，通过模块化专家库和不确定性感知融合解决时间序列基础模型在安全关键系统中的灾难性遗忘和验证难题


<details>
  <summary>Details</summary>
Motivation: 当前通用时间序列基础模型在安全关键信息物理系统(CPS)中存在灾难性遗忘、频谱偏差、可验证性不足等问题，无法满足ISO 26262等安全标准要求

Method: 提出模块化主权范式，构建紧凑、冻结的特定状态专家库，通过不确定性感知混合机制(HYDRA)实现快速自适应，确保状态条件有效性

Result: HYDRA框架能够解决可塑性-稳定性悖论，实现严格的偶然不确定性和认知不确定性分离，提供模块化可审计性

Conclusion: 模块化主权范式为CPS生命周期中的鲁棒状态完整性提供了可认证路径，优于全局参数更新的传统方法
...
TL;DR: Drive-KD：通过知识蒸馏将自动驾驶分解为"感知-推理-规划"三元能力，使用层特定注意力作为蒸馏信号，构建单教师模型并统一为多教师蒸馏框架，显著提升小模型性能


<details>
  <summary>Details</summary>
Motivation: 自动驾驶是安全关键任务，LLMs/VLMs为推理规划提供新可能，但大模型需要大量GPU内存且推理延迟高，传统监督微调难以弥合小模型能力差距

Method: 提出Drive-KD框架：1)将自动驾驶分解为感知、推理、规划三元能力；2)使用层特定注意力作为蒸馏信号构建能力特定单教师模型；3)统一为多教师蒸馏框架，引入非对称梯度投影缓解跨能力梯度冲突

Result: 蒸馏后的InternVL3-1B模型比同系列预训练78B模型减少约42倍GPU内存，吞吐量提升约11.4倍，在DriveBench上整体性能更好，规划维度超越GPT-5.1，方法在不同模型家族和规模上验证了泛化性

Conclusion: Drive-KD通过知识蒸馏有效提升小模型在自动驾驶任务中的性能，为高效自动驾驶视觉语言模型提供新思路，在保持高性能的同时显著降低计算资源需求
...
<details>
  <summary>Details</summary>
Motivation: 现有多行为推荐方法牺牲可解释性追求性能，而当前可解释方法依赖外部信息导致泛化性有限。用户行为链本身蕴含适合显式推理的内生逻辑，但观测到的多行为存在混杂因素导致模型学习虚假相关

Method: 提出CNRE模型：1) 使用分层偏好传播捕获异构跨行为依赖；2) 基于偏好强度建模用户行为链中的内生逻辑规则；3) 自适应分配到相应的神经-逻辑推理路径（如合取、析取）；4) 生成近似理想状态的可解释因果中介变量

Result: 在三个大规模数据集上的实验表明，CNRE显著优于最先进的基线方法，在模型设计、决策过程和推荐结果三个层面提供多级可解释性

Conclusion: CNRE通过神经符号推理与因果推断的结合，有效解决了多行为推荐中可解释性与性能的权衡问题，利用内生逻辑规则生成可解释的因果中介变量，实现了从模型设计到推荐结果的全方位可解释性
...
TL;DR: 该研究通过方差分析量化了提示词、模型选择和随机采样对LLM输出变异的影响，发现提示词对输出质量（原创性）影响显著（36.43%），与模型选择（40.94%）相当，但对输出数量（流畅性）影响很小（4.22%），后者主要由模型选择（51.25%）和模型内部变异（33.70%）主导。


<details>
  <summary>Details</summary>
Motivation: 研究动机是量化大语言模型输出变异中不同因素的相对贡献：提示词、模型选择以及通过采样产生的随机性，以理解这些因素如何影响LLM输出的质量和数量。

Method: 方法：评估12个LLM在10个创意提示词上各采样100次（总计N=12,000个样本），通过方差分析量化提示词、模型选择和随机采样对输出变异（原创性和流畅性）的相对贡献。

Result: 结果：对于输出质量（原创性），提示词解释36.43%的方差，模型选择解释40.94%；对于输出数量（流畅性），模型选择解释51.25%的方差，模型内部变异解释33.70%，提示词仅解释4.22%。模型内部变异范围为10-34%。

Conclusion: 结论：提示词是调控输出质量的有力杠杆，但由于显著的模型内部变异（10-34%），单次采样评估可能混淆采样噪声与真实的提示词或模型效应，因此需要多次采样以获得可靠评估。
...
TL;DR: EHR-RAG：针对长时序电子健康记录的检索增强生成框架，通过事件时间感知检索、自适应迭代检索和双路径证据推理，在临床预测任务中显著优于现有LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录（EHRs）包含丰富的纵向临床证据，对医疗决策至关重要。然而，长时序EHRs常超出LLM上下文限制，现有方法（如截断或简单检索）会丢弃临床相关事件和时间依赖关系，需要专门框架来准确解释长时序结构化EHR数据。

Method: 提出EHR-RAG框架，包含三个核心组件：1）事件和时间感知混合EHR检索，保留临床结构和时间动态；2）自适应迭代检索，逐步优化查询以扩大证据覆盖范围；3）双路径证据检索与推理，联合检索和推理事实与反事实证据。

Result: 在四个长时序EHR预测任务上的实验表明，EHR-RAG始终优于最强的LLM基线方法，平均Macro-F1提升10.76%。

Conclusion: 检索增强的LLMs在结构化EHR数据的临床预测实践中具有显著潜力，EHR-RAG通过专门设计的检索和推理机制有效解决了长时序EHR处理的挑战。
...
TL;DR: 该论文提出了Ostrakon-VL，一个面向食品服务和零售商店场景的多模态大语言模型，以及ShopBench基准测试和QUAD数据管理流程，在FSRS任务上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在食品服务和零售商店场景部署面临两大障碍：1) 真实FSRS数据噪声大、缺乏可审计的闭环数据管理，难以构建高质量训练语料；2) 缺乏统一、细粒度、标准化的多模态基准测试，难以客观评估模型鲁棒性。

Method: 1) 基于Qwen3-VL-8B开发Ostrakon-VL模型；2) 创建首个公开FSRS基准ShopBench，涵盖单图、多图和视频输入；3) 提出QUAD多阶段多模态指令数据管理流程；4) 采用多阶段训练策略。
...
TL;DR: PLaT框架通过将潜在推理重新定义为规划，将推理与语言表达解耦，使模型能够动态确定推理终止时机，在数学基准测试中展现出更好的推理多样性和可扩展性。
...
Method: 提出PLaT框架，将潜在推理重新定义为规划过程，将推理与语言表达解耦：推理被建模为潜在规划状态的确定性轨迹，而单独的Decoder仅在必要时将这些"思想"转化为文本。

Result: 在数学基准测试中，PLaT虽然贪婪准确率低于基线，但展现出更优越的推理多样性可扩展性，表明其学习到了更鲁棒、更广泛的解空间。
...
TL;DR: DAMI框架通过动态参数插值实现认知深度控制，在保持系统1效率的同时获得系统2的推理深度，无需额外训练。
...
Method: 利用现有Instruct和Thinking检查点通过动态参数插值，无需额外训练。提出DAMI框架，估计查询特定的推理强度λ(q)来配置认知深度。开发基于偏好学习的方法（编码准确性和效率标准）和基于置信度的零样本方法（利用模型间认知差异）。

Result: 在五个数学推理基准测试中，DAMI比Thinking模型获得更高准确率，同时保持高效，有效结合了系统1的效率和系统2的推理深度。

Conclusion: 从输出控制转向能力控制是更有效的范式，动态参数插值能够实现系统1和系统2认知模式的统一，在保持效率的同时提升推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在理解否定指令方面存在系统性缺陷，这种缺陷在高风险决策场景中可能带来严重后果。研究旨在量化模型对否定指令的敏感性，揭示安全部署中的关键漏洞。

Method: 审计了16个模型在14个伦理场景中的表现，比较了简单否定和复合否定下的响应差异。使用确定性解码排除采样噪声，提出否定敏感性指数(NSI)作为评估指标，并设计分层认证框架。

Result: 开源模型在简单否定下77%支持被禁止行为，复合否定下达到100%（比肯定框架增加317%）。商业模型表现稍好但仍有19-128%的波动。模型间一致性从肯定提示的74%降至否定提示的62%，金融场景脆弱性是医疗场景的两倍。

Conclusion: 当前对齐技术未能确保模型可靠区分"做X"和"不做X"，这种缺陷使得模型不适合在高风险环境中进行自主决策。需要建立基于否定敏感性的治理指标和分层认证框架来确保安全部署。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在关键规则约束决策中的鲁棒性，探索已知的提示敏感性和谄媚倾向是否会导致逻辑约束满足失败，特别是在情感框架效应影响下。

Method: 使用新颖的受控扰动框架，在医疗、法律和金融三个高风险领域进行实验，量化LLMs对叙事操纵的抵抗力，并与人类受试者进行比较。

Result: 发现"鲁棒性悖论"：LLMs对情感框架效应表现出近乎完全的行为不变性（Cohen's h = 0.003），而人类则表现出显著偏见（Cohen's h在[0.3, 0.8]之间），LLMs的抵抗力比人类强110-300倍。

Conclusion: 指令调优的LLMs能够将逻辑规则遵循与说服性叙事解耦，提供决策稳定性来源，可以补充甚至去偏人类判断，特别是在制度环境中。
...
TL;DR: 提出了一个全面的AI辅助芯片设计基准测试ChipBench，评估LLM在Verilog生成、调试和参考模型生成三个关键任务上的性能，揭示现有基准测试饱和且任务多样性不足的问题。
...
Method: 构建包含44个具有复杂层次结构的真实模块、89个系统化调试案例和132个跨Python、SystemC、CXXRTL的参考模型样本的基准测试，并开发自动化工具箱用于高质量训练数据生成。

Result: 评估结果显示显著性能差距：最先进的Claude-4.5-opus在Verilog生成上仅达到30.74%，在Python参考模型生成上仅13.33%，远低于现有饱和基准测试中SOTA模型超过95%的通过率。

Conclusion: ChipBench揭示了LLM在硬件工程任务中的实际挑战，提供了更真实的性能评估，并为该领域的研究提供了高质量数据生成工具，促进未来研究发展。
...
### [31] [LION: A Clifford Neural Paradigm for Multimodal-Attributed Graph Learning](https://arxiv.org/abs/2601.21453)
*Xunkai Li,Zhengyu Wu,Zekai Chen,Henan Sun,Daohan Su,Guang Zeng,Hongchao Qin,Rong-Hua Li,Guoren Wang*

Main category: cs.AI

TL;DR: LION提出基于Clifford代数和解耦图神经范式的新型多模态属性图处理方法，通过几何感知流形实现模态对齐，自适应全息聚合改进模态融合，在9个数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前多模态属性图处理存在两个主要问题：1）模态对齐时忽视图上下文，采用拓扑约束或模态特定算子作为标记器，抑制模态交互导致次优对齐；2）模态融合缺乏适应性，现有方法多为双模态图的简单适配，未能充分利用具有拓扑先验的对齐标记，导致泛化性差和性能下降。

Method: 基于Clifford代数和解耦图神经范式（传播-聚合），实现对齐-融合的多模态属性图处理。首先构建基于Clifford代数的模态感知几何流形，通过几何诱导的高阶图传播实现模态交互和模态对齐；然后基于对齐标记的几何等级特性，提出自适应全息聚合模块，将几何等级的能量和尺度与可学习参数结合以改进模态融合。

Result: 在9个数据集上的广泛实验表明，LION在3种图下游任务和3种模态下游任务上显著优于当前最先进的基线方法。

Conclusion: LION通过Clifford代数构建的几何感知流形和自适应全息聚合，有效解决了多模态属性图中的模态对齐和融合问题，显著提升了多模态图机器学习的实用性和性能。
...
TL;DR: Topeax是一种新的主题建模方法，通过密度估计峰值发现聚类数量，结合词汇和语义指标获取高质量主题关键词，相比Top2Vec和BERTopic在聚类恢复和描述方面表现更好，对样本大小和超参数变化更稳定。


<details>
  <summary>Details</summary>
Motivation: 当前流行的聚类主题建模方法Top2Vec和BERTopic存在多个未解决的问题：1）由于对样本大小和超参数极度敏感，默认值导致次优行为，难以可靠发现自然聚类；2）BERTopic忽略关键词与主题向量的语义距离，Top2Vec忽略语料库中的词频，导致主题不连贯（包含停用词和垃圾词）且缺乏多样性和可信度。

Method: Topeax通过密度估计峰值自动发现聚类数量，结合词汇指标（考虑词频）和语义指标（考虑关键词与主题向量的语义距离）来评估术语重要性，从而获得高质量的主题关键词。

Result: Topeax在聚类恢复和聚类描述方面均优于Top2Vec和BERTopic，同时对样本大小和超参数变化的响应行为更加稳定，减少了异常波动。

Conclusion: Topeax解决了现有聚类主题建模方法的局限性，通过更可靠的聚类发现机制和结合词汇-语义的术语重要性评估，提供了更高质量的主题建模结果，对参数变化具有更好的鲁棒性。
...
### [33] [ScaleSim: Serving Large-Scale Multi-Agent Simulation with Invocation Distance-Based Memory Management](https://arxiv.org/abs/2601.21473)
...
<details>
  <summary>Details</summary>
Motivation: LLM多智能体模拟面临GPU内存压力大的扩展难题，每个智能体需要维护私有GPU驻留状态（模型、前缀缓存、适配器），随着智能体数量增加会快速耗尽设备内存

Method: 提出调用距离的统一抽象来估计智能体发出未来LLM请求的相对顺序，基于此实现ScaleSim系统，支持主动预取和基于优先级的驱逐，通过模块化接口支持多样化的智能体特定内存

Result: 在模拟基准测试中，相比SGLang实现了最高1.74倍的加速

Conclusion: ScaleSim通过调用距离抽象和内存管理优化，有效解决了LLM多智能体模拟的内存扩展问题，显著提升了系统性能
...
### [34] [ARGORA: Orchestrated Argumentation for Causally Grounded LLM Reasoning and Decision Making](https://arxiv.org/abs/2601.21533)
...
<details>
  <summary>Details</summary>
Motivation: 现有多专家LLM系统通过简单聚合整合不同观点，但无法明确显示哪些论证驱动了最终决策，缺乏对推理过程的透明性和可解释性。

Method: ARGORA将多专家讨论组织为显式论证图（支持/攻击关系），将其建模为因果模型，通过系统性移除单个论证并重新计算结果来识别必要推理链，同时引入校正机制对齐内部推理与外部判断。

Result: 在多样化基准测试和开放式用例中，ARGORA达到竞争性准确率，并在专家初始分歧时更倾向于解决争议而非引入新错误，同时提供关键论证的因果诊断。

Conclusion: ARGORA通过论证图和因果建模增强了多专家LLM系统的透明性、可解释性和可靠性，为理解复杂决策过程提供了系统化框架。
...
### [35] [ShardMemo: Masked MoE Routing for Sharded Agentic LLM Memory](https://arxiv.org/abs/2601.21545)
*Yang Zhao,Chengxiao Dai,Yue Xiu,Mengying Kou,Yuliang Zheng,Dusit Niyato*

Main category: cs.AI

TL;DR: ShardMemo是一个用于智能体LLM系统的预算分层内存服务，通过三层内存架构和基于范围的掩码路由机制，显著提升大规模并行访问下的检索效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 随着智能体LLM系统中内存容量和并行访问需求的增长，传统的集中式索引和启发式分区方法成为性能瓶颈，需要更高效的内存管理方案来支持长时状态和并发多智能体执行。

Method: 提出三层内存架构：Tier A（每个智能体的工作状态）、Tier B（分片证据存储，包含分片本地近似最近邻索引）、Tier C（版本化技能库）。Tier B采用"范围优先路由"机制，在路由或ANN搜索前通过结构化资格约束掩码过滤不合格分片，将分片探测建模为掩码混合专家路由，使用Top-B_probe或自适应Top-P策略，并基于成本感知门控机制管理不同分片家族。

Result: 在LoCoMo基准上，ShardMemo比最强基线(GAM)提升5.11-6.82 F1；在固定预算路由设置下(B_probe=3)，比余弦到原型分片路由提升6.87 F1，同时减少20.5%检索工作量和降低p95延迟。在长上下文HotpotQA上，在不同token长度下达到57.95-63.41 F1。在ToolBench上，Tier C实现0.97 Precision@3和1.94 StepRed，比嵌入相似性检索提升10.2%和7.2%。

Conclusion: ShardMemo通过分层内存架构和智能路由机制，有效解决了智能体LLM系统中的内存扩展瓶颈，在保持高准确性的同时显著提升了检索效率和系统性能。
...
### [36] [EmboCoach-Bench: Benchmarking AI Agents on Developing Embodied Robots](https://arxiv.org/abs/2601.21570)
*Zixing Lei,Genjia Liu,Yuanshuo Zhang,Qipeng Liu,Chuan Wen,Shanghang Zhang,Wenzhao Lian,Siheng Chen*

Main category: cs.AI

TL;DR: LLM智能体在具身AI中自主工程化策略的基准评估，通过代码作为通用接口，在32个任务上实现超越人工基线26.5%的性能提升


<details>
  <summary>Details</summary>
Motivation: 具身AI领域向通用机器人系统快速演进，但依赖人工监督（奖励设计、超参数调优）成为规模化瓶颈。受LLM在软件自动化和科学发现中成功的启发，需要评估LLM智能体自主工程化具身策略的能力

Method: 提出EmboCoach-Bench基准，包含32个专家策划的RL和IL任务，以可执行代码为通用接口。评估动态闭环工作流：智能体利用环境反馈迭代起草、调试和优化解决方案，涵盖从物理感知奖励设计到扩散策略等架构改进

Result: 1) 自主智能体平均成功率超越人工工程基线26.5%；2) 带环境反馈的智能体工作流显著增强策略开发，大幅缩小开源与专有模型性能差距；3) 智能体展现自校正能力，通过迭代仿真调试从近乎完全失败中恢复任务性能

Conclusion: 为自演化具身智能奠定基础，加速从劳动密集型人工调优向可扩展自主工程的范式转变，推动具身AI领域发展
...
<details>
  <summary>Details</summary>
Motivation: 虽然隐式CoT压缩（将推理步骤压缩到潜在状态）提供了token高效的替代方案，但其背后的机制尚不清楚。现有方法在压缩推理步骤时面临学习信号衰减的问题，特别是对于需要高阶逻辑依赖的不可约问题。
...
Result: ALiCoT实现了54.4倍的推理加速，同时保持与显式CoT相当的性能。实验验证了理论分析的正确性，表明ALiCoT能够成功解锁高效推理能力。

Conclusion: 该研究首次从理论上揭示了CoT压缩的困难机制，提出了有效的解决方案ALiCoT，为大语言模型的高效推理提供了理论基础和实践框架，在保持性能的同时显著降低了计算成本。
...
TL;DR: Dreamer框架通过深度循环注意力混合机制，结合序列注意力、深度注意力和稀疏专家注意力，解决了深度循环模型中隐藏层大小瓶颈问题，在语言推理任务上实现了显著效率提升。


<details>
  <summary>Details</summary>
Motivation: 现有深度循环方法存在三个主要问题：缺乏FLOP、参数和内存匹配的基线比较；由于部分固定层堆栈导致深度循环利用不足；恒定隐藏层大小限制了多步潜在推理能力。

Method: 提出Dreamer框架，结合三种注意力机制：序列注意力处理序列维度，深度注意力处理深度维度，稀疏专家注意力实现参数高效扩展。通过深度注意力缓解隐藏层大小瓶颈，解耦缩放维度，使深度循环模型能够高效扩展。

Result: 在语言推理基准测试中，相比FLOP、参数和内存匹配的SOTA模型，Dreamer达到相同准确率所需的训练token数量减少2-8倍；使用相同训练token时，性能优于约2倍大小的SOTA模型。专家选择多样性比SOTA MoEs高2-11倍。

Conclusion: Dreamer框架通过深度循环注意力混合机制有效解决了深度循环模型的扩展瓶颈，在保持参数效率的同时显著提升了多步推理能力，为潜在推理任务提供了更高效的架构设计。
...
TL;DR: ATP-Latent提出主动潜在规划方法，通过条件变分自编码器建模潜在令牌监督，结合强化学习提升潜在推理策略，在LLaMA-1B上实现精度提升和令牌消耗减少。


<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法通过模仿语言标签监督潜在令牌，但由于同一问题存在多个等价但不同的CoT标签，被动模仿可能导致次优的潜在令牌表示和推理策略，限制了潜在空间的规划能力，造成训练与测试间的差距。

Method: 提出ATP-Latent方法：1) 使用条件变分自编码器建模潜在令牌监督过程，获得更平滑的潜在空间；2) 引入基于一致性的辅助奖励进行强化学习，通过VAE解码内容的连贯性指导RL过程，优化潜在推理策略。

Result: 在LLaMA-1B模型上，ATP-Latent在四个基准测试中相比先进基线实现了+4.1%的准确率提升和-3.3%的令牌消耗减少。

Conclusion: ATP-Latent通过主动规划潜在令牌表示空间，结合VAE平滑化和基于一致性的强化学习，显著提升了潜在推理的效率和性能，验证了主动规划在潜在推理中的重要性。
...
TL;DR: 该研究将智能文档处理系统的验证问题形式化为基于搜索的软件测试问题，旨在在有限预算下最大化发现不同类型的故障模式，并通过多种搜索策略的对比分析证明了策略间的互补性。


<details>
  <summary>Details</summary>
Motivation: 企业级智能文档处理系统在金融、保险和医疗等高风险领域应用广泛，但早期系统验证面临预算有限的挑战。传统方法通常寻找单一最坏情况文档，而实际需要发现多样化的故障机制以确保系统鲁棒性。

Method: 将问题形式化为基于搜索的软件测试问题，在文档配置的组合空间中操作，实例化结构风险特征以诱导现实故障条件。在相同预算约束下，对进化算法、群体智能、质量多样性、学习型和量子算法等多种搜索策略进行基准测试，并通过配置级排他性、胜率和跨时间重叠分析评估性能。

Result: 不同求解器在相同预算下持续发现其他方法未能发现的故障模式。跨时间分析显示所有评估预算中都存在求解器特定的持久性发现，没有单一策略表现出绝对优势。虽然所有求解器的并集最终能覆盖观察到的故障空间，但依赖任何单一方法都会系统性地延迟重要风险的发现。

Conclusion: 研究结果证明了求解器之间的内在互补性，为工业级智能文档处理系统的鲁棒验证提供了基于组合策略的基于搜索的软件测试方法。这强调了在有限验证预算下采用多种搜索策略组合的重要性，而非依赖单一最优方法。
...
### [41] [Semantic Content Determines Algorithmic Performance](https://arxiv.org/abs/2601.21618)
*Martiño Ríos-García,Nawaf Alampara,Kevin Maik Jablonka*

Main category: cs.AI

TL;DR: LLMs在计数任务中表现出对语义内容的敏感性，即使列表明确无歧义，计数准确率也会因计数对象类型（城市vs化学物、名字vs符号）而产生超过40%的差异


<details>
  <summary>Details</summary>
Motivation: 算法行为应对其参数的语义内容保持不变性，但现有研究将语义敏感性与推理复杂性或提示变化混为一谈，需要设计原子化测试来专门检验LLMs的语义敏感性

Method: 提出WhatCounts测试框架：在无歧义、有分隔符、无重复项、无干扰项的列表中计数不同语义类型的项目，通过受控消融实验排除混淆因素

Result: 前沿LLMs仅因计数对象类型不同就产生超过40%的准确率差异，这种语义差距会随着少量无关微调而不可预测地变化，表明LLMs并非实现算法而是近似算法，且这种近似是参数依赖的

Conclusion: LLMs不实现算法而是近似算法，且近似程度依赖于参数语义，这种隐藏的语义依赖性不仅限于计数任务，可能影响任何LLM函数，具有重要的实际应用意义
...
### [42] [ScholarGym: Benchmarking Deep Research Workflows on Academic Literature Retrieval](https://arxiv.org/abs/2601.21654)
*Hao Shen,Hang Yang,Zhouhong Gu*

Main category: cs.AI

TL;DR: ScholarGym是一个用于评估学术文献深度研究工作流的仿真环境，通过静态语料库和确定性检索解决API依赖带来的非确定性问题，提供可重复的评估基准。


<details>
  <summary>Details</summary>
Motivation: 当前工具增强的大型语言模型已从单轮问答发展到深度研究工作流，但评估这些工作流面临根本挑战：依赖实时API会引入非确定性（如时间漂移、速率限制、后端状态变化），这种变化破坏了可重复性并使跨系统比较失效。

Method: 1. 将工作流组件解耦为查询规划、工具调用和相关性评估；2. 基于包含57万篇论文的静态语料库构建仿真环境；3. 提供2,536个带有专家标注真实标签的查询；4. 实现确定性检索以消除非确定性；5. 在不同骨干模型上进行实验，分析推理能力、规划策略和选择机制在迭代优化中的交互作用。

Result: ScholarGym环境能够：1. 在受控条件下对工作流各阶段进行细粒度分析；2. 提供可重复的评估基准；3. 揭示推理能力、规划策略和选择机制在迭代优化过程中的相互作用；4. 支持跨系统比较而不受API非确定性的影响。

Conclusion: ScholarGym为深度研究工作流提供了第一个可重复的评估环境，解决了工具增强LLM评估中的非确定性问题，使研究人员能够在受控条件下分析工作流组件，促进更可靠的跨系统比较和迭代优化研究。
...
### [43] [SONIC-O1: A Real-World Benchmark for Evaluating Multimodal Large Language Models on Audio-Video Understanding](https://arxiv.org/abs/2601.21666)
*Ahmed Y. Radwan,Christos Emmanouilidis,Hina Tabassum,Deval Pandya,Shaina Raza*

Main category: cs.AI

TL;DR: SONIC-O1是一个全面的人工验证基准测试，用于评估多模态大语言模型在现实世界时序音频-视频数据上的性能，涵盖13个对话领域和4,958个标注。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型研究主要关注静态图像理解，而处理时序音频-视频数据的能力尚未充分探索，需要高质量基准测试来系统评估模型在现实场景中的表现。

Method: 构建了SONIC-O1基准测试，包含13个现实世界对话领域，4,958个人工验证标注，并包含人口统计元数据。评估任务包括开放式摘要、多项选择题回答、以及带推理支持的时序定位。

Result: 实验显示闭源和开源模型均存在局限性：多项选择题准确率差距较小，但时序定位任务中最佳闭源模型比开源模型性能高出22.6%。模型在不同人口统计群体上性能进一步下降，表明存在持续的行为差异。

Conclusion: SONIC-O1为时序基础和社交鲁棒的多模态理解提供了开放评估套件，揭示了当前模型在处理现实世界时序音频-视频数据时的显著差距和人口统计偏差。
...
### [44] [TCAP: Tri-Component Attention Profiling for Unsupervised Backdoor Detection in MLLM Fine-Tuning](https://arxiv.org/abs/2601.21692)
...
<details>
  <summary>Details</summary>
Motivation: 现有的后门防御方法要么依赖监督信号，要么无法泛化到不同的触发类型和模态。多模态大语言模型微调服务中通过中毒数据引入的后门风险需要更有效的防御机制。

Method: 提出Tri-Component Attention Profiling (TCAP)框架：1) 将跨模态注意力图分解为系统指令、视觉输入和用户文本查询三个功能组件；2) 使用高斯混合模型统计分析识别触发响应注意力头；3) 通过基于EM的投票聚合隔离中毒样本。

Result: 在多种MLLM架构和攻击方法上的广泛实验表明，TCAP能够实现一致强大的性能，在各种触发类型和模态中表现出良好的泛化能力。

Conclusion: TCAP作为一种无监督防御框架，通过注意力分配差异检测后门攻击，为多模态大语言模型微调服务提供了稳健实用的后门防御解决方案。
...
### [45] [FBS: Modeling Native Parallel Reading inside a Transformer](https://arxiv.org/abs/2601.21708)
*Tongxi Wang*

Main category: cs.AI

TL;DR: FBS Transformer通过引入可训练的因果循环，结合前瞻注意力、分块处理和跳过机制，在保持参数不变的情况下改善语言模型的质量-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理主要依赖严格的自回归token-by-token生成，现有加速方法未能充分利用人类阅读的关键要素：内容自适应前瞻、分块结构感知的计算分配以及训练-测试一致性。

Method: 提出Fovea-Block-Skip Transformer，通过三个模块注入可训练的因果循环：Parafovea-Attention Window（前瞻注意力窗口）、Chunk-Head（分块头）和Skip-Gate（跳过门）。

Result: 在多样化基准测试中，FBS在保持参数不变的情况下改善了质量-效率权衡，消融实验显示三个模块具有互补性。

Conclusion: FBS Transformer通过模拟人类阅读机制，为语言模型推理提供了更高效的架构设计，展示了前瞻、分块和跳过机制的协同作用。
...
Conclusion: DropoutTS通过从"学什么"到"学多少"的范式转变，提供了一种高效且有效的解决方案，平衡了时间序列模型的鲁棒性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统中的个体智能体往往缺乏鲁棒性，容易盲目跟随误导性同伴。这种弱点源于两个问题：谄媚倾向（sycophancy）以及评估同伴可靠性的能力不足。需要开发一种方法使智能体能够基于历史交互评估同伴可信度，从而在不确定时向可信赖的同伴学习。

Method: 1. 形式化历史感知参考学习问题，将同伴的历史交互作为额外输入；2. 开发Epistemic Context Learning (ECL)推理框架，基于历史构建明确的同伴档案来条件化预测；3. 使用辅助奖励通过强化学习优化ECL。

Result: ECL使小模型（Qwen 3-4B）在识别可靠同伴方面超越了历史不可知基线模型（Qwen 3-30B，规模大8倍），并将前沿模型性能提升至接近完美（100%）。ECL在各种多智能体配置中泛化良好，且信任建模准确度与最终答案质量呈强相关性。

Conclusion: ECL框架通过历史交互构建同伴可信度档案，有效解决了多智能体系统中的盲目从众问题。该方法不仅显著提升了小模型的性能，还揭示了LLMs能够良好建模信任关系，且信任建模准确度与最终答案质量密切相关。
...
TL;DR: 提出ZSSD零样本统计降尺度框架，无需配对训练数据，通过物理一致的气候先验和统一坐标引导解决传统方法在GCMs上的泛化问题
...
Method: ZSSD框架包含：1）从再分析数据学习的物理一致气候先验，通过地球物理边界和时序信息约束确保物理有效性；2）统一坐标引导策略，解决vanilla DPS中的梯度消失问题并确保与大尺度场的一致性

Result: ZSSD在99百分位误差上显著优于现有零样本基线，能够成功重建复杂天气事件（如热带气旋），并在异质GCMs上表现良好

Conclusion: ZSSD为零样本统计降尺度提供了有效解决方案，通过物理约束和统一坐标引导实现了在缺乏配对数据情况下对GCMs的稳健泛化
...
TL;DR: 提出基于概念空间的抽象概念建模框架，以国际象棋为案例，将策略概念表示为可解释质量维度上的几何区域，通过轨迹分析识别意图策略


<details>
  <summary>Details</summary>
Motivation: 扩展概念空间理论以建模随时间展开的、目标导向的抽象概念，特别是策略性概念，解决传统概念空间理论在处理时序动态概念方面的局限性

Method: 采用概念空间框架，将策略概念（如攻击、牺牲）表示为可解释质量维度上的几何区域；将棋局实例化为轨迹，通过轨迹在概念空间中的方向性移动识别意图策略；支持双视角建模以捕捉玩家对相同情境的不同解释

Result: 实现证明了基于轨迹的概念识别的可行性，运动模式与专家评论一致；建立了时序决策应用的基础，支持与知识演化机制的集成

Conclusion: 该工作成功将概念空间理论扩展到时间实现的、目标导向的概念建模，为涉及顺序决策的广泛应用奠定了基础，并支持随时间学习和精炼抽象概念的机制
...
TL;DR: 提出基于大语言模型的统一框架，用于气管内吸痰术视频活动识别与反馈生成，在准确率和F1分数上比基线模型提升15-20%，并包含基于异常检测和可解释AI的学生支持模块。


<details>
  <summary>Details</summary>
Motivation: 气管内吸痰术是高风险临床操作，在家庭护理和教育环境中缺乏有效监督。现有自动化识别和反馈系统研究不足，需要开发可扩展、可解释的训练支持系统。

Method: 提出以LLM为核心的统一框架，将LLM作为中央推理模块，从视频数据执行时空活动识别和可解释决策分析，并能用自然语言生成反馈。框架还包括基于异常检测和可解释AI原理的学生支持模块。

Result: LLM方法在准确率和F1分数上比传统机器学习和深度学习方法提升约15-20%。框架能提供自动化、可解释的反馈，突出正确操作并建议针对性改进。

Conclusion: 该框架为护理教育提供了可扩展、可解释、数据驱动的基础，能提高培训效率并最终改善患者安全，建立了视频活动识别和反馈生成的新范式。
...
TL;DR: 该研究提出了一个全面的心电图专家基础模型（ECG-expert FMs）基准测试框架，结合下游性能评估和表示层分析，以深入理解其表征结构和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型（FMs）在AI辅助心电图解读领域开始应用，但现有基准测试主要关注下游性能评估，缺乏对模型嵌入表征的深入分析。在医疗等错误敏感领域，需要严格评估FMs产生的嵌入表征的泛化能力，特别是在数据稀缺的现实医疗场景中。

Method: 提出一个基准测试方法学，将基于性能的评估与表示层分析相结合。利用SHAP（SHapley Additive exPlanations）和UMAP（Uniform Manifold Approximation and Projection）技术进行表示层分析。对多个通过最先进技术预训练的ECG专家基础模型进行广泛评估，涵盖不同跨大陆数据集和数据可用性设置（包括数据稀缺场景）。

Result: 实验结果表明，该基准测试协议能够提供对ECG专家基础模型嵌入模式的丰富洞察，实现对模型表征结构和泛化能力的更深入理解。特别是在不同数据可用性设置下，包括数据稀缺场景中，都能有效评估模型的性能。

Conclusion: 该研究填补了现有ECG专家基础模型基准测试的空白，提供了一个全面的评估框架，不仅关注下游性能，还深入分析模型嵌入表征，为在医疗领域负责任地部署基础模型提供了重要工具。
...
TL;DR: 论文提出一个决策中心的仿真框架，用于在汽车售后市场备件库存管理中评估预测模型对运营绩效的影响，而非仅关注统计准确性。


<details>
  <summary>Details</summary>
Motivation: 汽车售后市场的备件需求具有高度间歇性和不确定性，传统预测模型评估仅关注统计准确性指标（如MAE、RMSE），但这些指标与关键运营绩效指标（如总成本和服务水平）之间的关系不明确，需要建立连接预测与库存决策的评估框架。

Method: 提出一个决策中心的仿真软件框架，包含三个核心组件：(1) 针对备件需求特征定制的合成需求生成器；(2) 可容纳任意预测模型的灵活预测模块；(3) 消耗预测并计算运营KPI的库存控制模拟器，形成闭环评估系统。

Result: 通过广泛的仿真场景表明，传统准确性指标的改进不一定转化为更好的运营绩效，具有相似统计误差特征的模型可能产生显著不同的成本-服务权衡，分析了预测性能特定方面如何影响库存结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM去偏方法多采用抑制范式（修改参数、提示或神经元），但这些方法脆弱、泛化性弱、数据效率低且易损害模型通用能力

Method: 使用基于归因的分析方法，通过少量偏见知识问题识别编码偏见知识的神经元，在推理时选择性增强这些神经元

Result: 在多个基准测试和LLM上实现了一致的SOTA去偏性能，同时保持了最小化的通用能力下降

Conclusion: KnowBias提供了一种概念上不同的去偏方法，通过增强而非抑制偏见知识神经元，实现了强去偏效果、跨偏见类型和人口统计的泛化能力，以及高数据效率
...
### [54] [From Meta-Thought to Execution: Cognitively Aligned Post-Training for Generalizable and Reliable LLM Reasoning](https://arxiv.org/abs/2601.21909)
*Shaojie Wang,Liang Zhang*

Main category: cs.AI

TL;DR: CoMT框架通过模仿人类认知的两阶段过程（先获取抽象策略再适应具体任务）改进LLM后训练，相比传统方法在泛化能力和训练效率上均有显著提升


<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练方法（SFT+RL）将完整推理轨迹作为基本单元，这与人类实际解决问题的方式存在根本差异。人类认知自然地将问题解决分解为两个阶段：先获取跨问题泛化的抽象策略（元知识），再将其适应到具体实例。现有方法的问题中心化处理将抽象策略与问题特定执行纠缠在一起，导致训练效率低下且泛化能力有限。
...
Conclusion: 将后训练与人类认知原则对齐不仅能产生更优的泛化能力，还能显著提高训练效率，证明了认知启发方法在LLM优化中的有效性。
...
### [55] [ProRAG: Process-Supervised Reinforcement Learning for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.21912)
*Zhao Wang,Ziliang Zhao,Zhicheng Dou*

Main category: cs.AI

TL;DR: ProRAG：一个过程监督强化学习框架，通过整合步骤级监督到在线优化循环中，解决RAG中奖励稀疏性和信用分配问题，提升复杂推理任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于结果的RL方法在RAG优化中存在奖励稀疏性和信用分配效率低的问题，粗粒度标量奖励无法识别长轨迹中的具体错误步骤，导致"过程幻觉"（模型通过错误逻辑或冗余检索步骤得到正确答案）。现有过程感知方法缺乏在线探索能力，难以将步骤级信用与全局结果解耦。
...
Result: 在五个多跳推理基准测试上的广泛实验表明，ProRAG相比基于结果的RL和过程感知RL基线方法实现了更优的整体性能，特别是在复杂长视野任务上，验证了细粒度过程监督的有效性。

Conclusion: ProRAG通过将学习到的步骤级监督整合到在线优化循环中，有效解决了RAG中奖励稀疏性和信用分配问题，为每个动作提供精确反馈，显著提升了复杂推理任务的性能。
...
### [56] [JADE: Bridging the Strategic-Operational Gap in Dynamic Agentic RAG](https://arxiv.org/abs/2601.21916)
*Yiqun Chen,Erhan Zhang,Tianyi Hu,Shijie Wang,Zixuan Yang,Meizhi Zhong,Xiaochi Wei,Yan Gao,Yi Wu,Yao Hu,Jiaxin Mao*

Main category: cs.AI

TL;DR: JADE框架通过联合优化动态多轮工作流中的规划与执行，解决了传统RAG系统中规划与执行模块分离优化导致的战略-操作不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统面临关键二分法：要么在固定图架构中联合优化模块（刚性），要么支持动态规划但将执行器视为冻结的黑盒工具。这种解耦优化导致"战略-操作不匹配"——复杂的规划策略因未适应的本地执行器而无法实现，系统复杂性增加却带来负性能收益。

Method: 提出JADE（联合智能动态执行）框架，将系统建模为统一在单一共享骨干下的协作多智能体团队，通过基于结果的奖励进行端到端学习。这种方法促进"协同适应"：规划器学习在执行器能力边界内操作，而执行器则演化以对齐高层战略意图。

Result: 实证结果表明，JADE将分离的模块转变为协同系统，通过联合优化带来显著的性能改进，并通过动态工作流编排实现效率与效果之间的灵活平衡。

Conclusion: JADE框架通过联合优化规划与执行，解决了RAG系统中战略-操作不匹配问题，实现了模块间的协同适应，为动态多轮工作流提供了统一的优化方法。
...
### [57] [Self-Compression of Chain-of-Thought via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2601.21919)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的解决方案通过简单的长度惩罚与结果奖励结合来处理推理冗余问题，但这种简单的奖励加权难以平衡简洁性与准确性，强制简洁可能会损害关键的推理逻辑。

Method: 提出SCMA多智能体强化学习框架，包含三个智能体：分割智能体将推理过程分解为逻辑块，评分智能体量化每个块的重要性，推理智能体在训练中接受重要性加权的长度惩罚激励，从而在部署时优先考虑必要逻辑而不引入推理开销。

Result: SCMA在不同模型规模上减少响应长度11.1%至39.0%，同时提高准确性4.33%至10.02%。消融研究和定性分析验证了MARL框架内的协同优化促进了涌现行为，相比传统RL范式产生了更强大的大型推理模型。

Conclusion: SCMA框架通过多智能体协作的选择性冗余惩罚机制，有效解决了大型推理模型中简洁性与准确性的平衡问题，显著减少了推理开销同时提升了模型性能。
...
### [58] [AgenticSimLaw: A Juvenile Courtroom Multi-Agent Debate Simulation for Explainable High-Stakes Tabular Decision Making](https://arxiv.org/abs/2601.21936)
...
<details>
  <summary>Details</summary>
Motivation: 针对高风险决策任务（如刑事司法）中传统黑盒方法缺乏透明度、可解释性和可控性的问题，需要一种能够提供可审计推理过程、支持人类监督的透明决策框架。

Method: 提出AgenticSimLaw框架，采用法庭辩论式结构：定义检察官、辩护律师和法官三种角色，设计7轮结构化辩论协议，包含私有推理策略，形成完全可审计的决策过程。在NLSY97数据集上进行年轻成人再犯预测基准测试，比较了近90种模型和策略组合。

Result: 结构化多智能体辩论相比单智能体推理提供更稳定和泛化的性能，准确率和F1分数指标间相关性更强。框架提供细粒度推理步骤控制、完整交互记录可解释性，支持系统化智能体行为分析。

Conclusion: AgenticSimLaw通过结构化角色组织、交互日志可观察性和敏感领域明确非部署约束，解决了LLM多智能体系统的关键挑战。该框架可推广到任何需要透明度和人类监督的高风险审议决策任务。
...
### [59] [How do Visual Attributes Influence Web Agents? A Comprehensive Evaluation of User Interface Design Factors](https://arxiv.org/abs/2601.21961)
*Kuai Yu,Naicheng Yu,Han Wang,Rui Yang,Huan Zhang*

Main category: cs.AI

TL;DR: VAF是一个评估网页视觉属性因素如何影响Web智能体决策的受控评估框架，通过生成语义相同但视觉属性不同的变体，测量智能体在浏览过程中的点击和推理行为差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注Web智能体对对抗性攻击的鲁棒性，而对良性场景中智能体偏好的研究不足。虽然早期研究探讨了文本属性对智能体行为的影响，但视觉属性如何塑造智能体决策的系统性理解仍然有限。

Method: VAF包含三个阶段：(1)变体生成：确保变体与原始项目语义相同，仅视觉属性不同；(2)浏览交互：智能体通过滚动和点击浏览页面，模拟人类用户行为；(3)验证：通过点击动作和推理验证，使用目标点击率和目标提及率联合评估视觉属性影响。

Result: 在8个变体家族（共48个变体）、5个真实网站（购物、旅游、新闻浏览）和4个代表性Web智能体上的实验表明：背景颜色对比度、项目大小、位置和卡片清晰度对智能体行为有强烈影响，而字体样式、文本颜色和项目图像清晰度影响较小。

Conclusion: 视觉属性显著影响Web智能体决策，其中某些属性（如背景对比度、大小、位置）的影响远大于其他属性。该研究为理解视觉因素在智能体决策中的作用提供了系统框架和量化方法。
...
TL;DR: 论文提出Green AI范式，关注AI系统的能耗效率，特别针对自动规划领域。通过领域模型配置框架，实证研究领域模型特征对经典规划器能耗的影响，发现能耗与运行时间并不总是相关。


<details>
  <summary>Details</summary>
Motivation: 传统AI研究优先考虑算法性能（如机器学习准确率、自动规划运行时间），而新兴的Green AI范式将能耗视为关键性能维度。尽管自动规划计算需求高，但其能耗效率研究不足。领域模型与算法分离的模块化结构为系统分析能耗提供了机会。

Method: 引入领域模型配置框架，允许控制特征变化（如元素排序、动作元数、死锁状态）。使用5个基准领域和5个最先进的规划器，分析32个领域变体对能耗和运行时间的影响。

Result: 领域级修改在不同规划器中产生可测量的能耗差异，能耗消耗并不总是与运行时间相关。这表明通过领域模型设计可以系统性地影响规划器的能耗效率。

Conclusion: 自动规划领域的能耗效率研究具有重要意义，领域模型特征对能耗有显著影响。Green AI范式应扩展到自动规划领域，通过领域模型设计优化能耗，而不仅仅是算法优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有MARL微调方法依赖预定义执行协议且通常需要集中式执行，而实际应用中更希望实现去中心化LLM协作以实现并行推理和灵活部署。同时，当前方法使用蒙特卡洛方法进行微调，存在高方差问题，需要更多样本才能有效训练。

Method: 提出两种多智能体演员-评论家方法：CoLLM-CC（集中式评论家）和CoLLM-DC（去中心化评论家）。这些方法旨在解决蒙特卡洛方法的高方差问题，优化去中心化LLM协作。

Result: 在写作、编码和游戏领域实验中，蒙特卡洛方法和CoLLM-DC在短视野和密集奖励设置下能达到与CoLLM-CC相当的性能。但在长视野或稀疏奖励任务中，两者表现均不如CoLLM-CC：蒙特卡洛方法需要更多样本，而CoLLM-DC难以收敛。

Conclusion: 多智能体演员-评论家方法（特别是集中式评论家版本CoLLM-CC）在优化去中心化LLM协作方面具有优势，特别是在处理长视野或稀疏奖励任务时，能有效解决蒙特卡洛方法的高方差问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖二元强制选择提示，这会将真实偏好与评估协议的人工因素混淆。需要系统研究不同评估协议如何影响语言模型中陈述偏好与显示偏好的相关性。

Method: 对24个语言模型进行系统研究，比较不同评估协议：1）在陈述偏好评估中允许中立和弃权以排除弱信号；2）在显示偏好评估中允许弃权；3）使用陈述偏好进行系统提示引导。在AIRiskDilemmas数据集上评估。

Result: 1）允许中立和弃权的陈述偏好评估显著提高了与强制选择显示偏好的Spearman秩相关（ρ）；2）在显示偏好中也允许弃权时，由于高中立率，ρ降至接近零或负值；3）使用陈述偏好进行系统提示引导不能可靠改善AIRiskDilemmas上的相关性。

Conclusion: 陈述偏好与显示偏好的相关性高度依赖评估协议设计，偏好评估需要能够处理不确定偏好的方法。现有二元强制选择方法存在局限性，需要更精细的评估框架。
...
TL;DR: VERSA是一个系统化的验证框架，用于确保足球领域事件流数据的完整性，通过状态转移模型检测和纠正逻辑不一致性，显著提高数据质量和下游分析可靠性。


<details>
  <summary>Details</summary>
Motivation: 事件流数据在体育等领域的细粒度分析中至关重要，但现有模型受到数据质量问题的根本限制，如逻辑不一致性（事件顺序错误或缺失事件），这影响了分析的可靠性。

Method: 提出VERSA（Verified Event Data Format for Reliable Soccer Analytics）框架，基于状态转移模型定义有效事件序列，实现事件流数据中异常模式的自动检测和纠正。

Result: 在K League 1（2024赛季）Bepro提供的事件数据中，检测到18.81%的记录事件存在逻辑不一致性。VERSA显著提高了跨提供者的一致性，确保异构源之间的稳定统一数据表示，并且经VERSA精炼的数据显著提升了VAEP（评估球员贡献的下游任务）的鲁棒性和性能。

Conclusion: 验证过程在提高数据驱动分析的可靠性方面非常有效，VERSA框架为解决事件流数据中的逻辑不一致性问题提供了系统化解决方案，从而增强下游分析任务的准确性和稳定性。
...
TL;DR: 提出"液态接口"协调范式，将接口从静态技术制品转变为运行时通过意图表达和语义协商产生的短暂关系事件，以支持自适应、概率性和上下文相关的自主智能体系统。


<details>
  <summary>Details</summary>
Motivation: 当前软件架构难以支持具有自适应、概率性和上下文相关推理能力的自主智能体，而系统集成仍被静态接口和确定性契约所主导。需要一种新的协调范式来应对语义不确定性和动态适应性需求。

Method: 提出液态接口协调范式，将接口概念化为短暂的关系事件而非持久技术制品；形式化该模型并设计液态接口协议(LIP)，管理意图驱动交互、协商执行和在语义不确定性下的短暂性执行；讨论治理影响并描述参考架构以展示实际可行性。

Result: 建立了液态接口的形式化模型，设计了液态接口协议(LIP)，提供了治理框架和参考架构，为基于智能体的自适应协调系统提供了理论基础和实践指导。

Conclusion: 液态接口为基于智能体的自适应协调系统提供了原则性基础，通过将接口从静态技术制品转变为运行时动态生成的关系事件，解决了传统架构在支持自适应、概率性和上下文相关智能体推理方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体基准测试主要关注理想环境下的任务完成，忽视了真实用户场景中的可靠性问题。在车载语音助手等实际应用中，用户常提出不完整或模糊的请求，智能体需要通过对话、工具使用和策略遵守来管理内在不确定性。

Method: 引入CAR-bench基准测试，包含LLM模拟用户、领域策略和58个互联工具（导航、生产力、充电、车辆控制）。除了标准任务完成，还设计了幻觉任务（测试在工具或信息缺失时的极限意识）和消歧任务（要求通过澄清或内部信息收集解决不确定性）。

Result: 基线结果显示，所有任务类型上都存在偶尔成功与一致成功之间的巨大差距。即使是前沿推理LLM在消歧任务上的持续通过率也低于50%（因过早行动），在幻觉任务中经常违反策略或捏造信息以满足用户请求。

Conclusion: CAR-bench揭示了现有LLM智能体在真实世界应用中的可靠性不足，强调了开发更可靠、更具自我意识的LLM智能体的必要性，特别是在需要处理不确定性和遵守策略的实际部署场景中。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键AI系统开发中，传统ODD定义依赖专家知识和标准，在早期阶段完成，但现实复杂系统或已有数据情况下难以完整描述环境条件，导致ODD定义不完整，影响AI系统认证

Method: 提出安全设计方法，使用多维度核表示从先前收集的数据中后验定义ODD，通过蒙特卡洛方法和真实航空用例验证，并定义两个ODD相等的条件

Result: 验证表明数据驱动的ODD可以与原始隐藏的ODD相等，基于核的ODD方法支持数据驱动的安全关键AI系统未来认证

Conclusion: 提出的安全设计核基ODD方法能够从已有数据中后验定义操作设计域，解决了传统方法在复杂现实环境中ODD定义不完整的问题，为数据驱动的安全关键AI系统认证提供了可行方案
...
TL;DR: 提出Agent-RRM推理奖励模型，为智能体轨迹提供结构化反馈，包含显式推理轨迹、聚焦式批评和改进指导、总体评分，并通过三种集成策略显著提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体强化学习方法主要依赖稀疏的结果奖励进行训练，这种反馈无法区分中间推理质量，导致训练结果次优。需要更精细的反馈机制来评估和改进智能体的推理过程。

Method: 提出Agent-RRM推理奖励模型，提供三方面结构化反馈：显式推理轨迹、聚焦式批评和改进指导、总体过程评分。开发三种集成策略：Reagent-C（文本增强精炼）、Reagent-R（奖励增强指导）、Reagent-U（统一反馈集成）。

Result: 在12个多样化基准测试中，Reagent-U策略实现显著性能提升：GAIA达到43.7%，WebWalkerQA达到46.2%。验证了推理奖励模型和训练方案的有效性。

Conclusion: Agent-RRM通过提供结构化反馈有效解决了稀疏奖励的局限性，显著提升了智能体强化学习的性能。三种集成策略中，统一反馈集成表现最佳，为智能体推理训练提供了新范式。
...
### [68] [BadDet+: Robust Backdoor Attacks for Object Detection](https://arxiv.org/abs/2601.21066)
...
<details>
  <summary>Details</summary>
Motivation: 后门攻击对深度学习构成严重威胁，但其在目标检测领域的影响相比图像分类仍缺乏深入理解。现有检测方法存在关键弱点：依赖不切实际的假设且缺乏物理验证。

Method: 提出BadDet+惩罚框架，统一RMA和ODA攻击。核心机制使用log-barrier惩罚来抑制触发输入的真实类别预测，实现位置尺度不变性和增强的物理鲁棒性。

Result: 在真实世界基准测试中，BadDet+相比现有RMA和ODA基线实现了更优的合成到物理转移性能，同时保持干净性能。理论分析证实惩罚在触发特定特征子空间内作用，可靠诱导攻击而不降低标准推理。

Conclusion: 结果突显了目标检测中的重大漏洞，以及开发专门防御措施的必要性。BadDet+框架为理解后门攻击在目标检测中的影响提供了重要工具。
...
### [69] [Non-Markov Multi-Round Conversational Image Generation with History-Conditioned MLLMs](https://arxiv.org/abs/2601.20911)
...
<details>
  <summary>Details</summary>
Motivation: 现有对话式图像生成模型大多基于马尔可夫假设，仅依赖最近图像进行响应，无法处理用户引用历史状态、撤销更改或跨多轮引用实体的非马尔可夫交互场景。

Method: 提出：1) 非马尔可夫多轮数据构造策略（回滚式编辑和基于名称的多轮个性化）；2) 历史条件训练与推理框架（带令牌级缓存防止身份漂移）；3) 高保真图像重建和可编辑个性化优化技术（基于重建的DiT解令牌器和多阶段微调课程）。

Result: 针对非马尔可夫交互的显式训练显著提升了多轮一致性和指令遵循能力，同时保持了强大的单轮编辑和个性化性能。

Conclusion: 通过系统解决非马尔可夫挑战，该研究为对话式图像生成提供了更强大的多轮交互能力，使模型能够更好地理解和利用完整对话历史。
...
### [70] [Text controllable PET denoising](https://arxiv.org/abs/2601.20990)
...
<details>
  <summary>Details</summary>
Motivation: PET图像在医学诊断中至关重要，但常受复杂噪声影响，噪声来源多样（扫描硬件、图像重建、示踪剂特性、剂量/计数水平、采集时间等），现有方法难以在单一模型中处理不同计数水平的去噪需求

Method: 提出文本引导的PET图像去噪方法，利用预训练CLIP模型提取特征，结合U-Net架构的降噪模型，通过文本指导实现多种计数水平下的统一去噪

Result: 实验结果表明，所提模型在定性和定量评估上均有显著改进，展现出处理复杂去噪需求或减少采集时间的潜力

Conclusion: 该方法为PET图像去噪提供了灵活高效的解决方案，能够适应不同计数水平的去噪需求，有望在临床应用中提升图像质量并减少采集时间
...
### [71] [Low performing pixel correction in computed tomography with unrolled network and synthetic data training](https://arxiv.org/abs/2601.20995)
...
Method: 提出展开式双域方法，利用从自然图像生成的合成数据，通过合成数据捕获投影域和图像域之间低性能像素的内在相关性，使训练模型无需真实临床数据即可校正伪影

Result: 在模拟1-2%探测器缺陷（靠近等中心）的实验中，该方法大幅优于现有最先进方法，表明能够在不收集训练数据成本的情况下校正低性能像素伪影，并可适应不同扫描仪设置的软件应用

Conclusion: 该方法通过合成数据和双域校正策略，有效解决了低性能像素伪影问题，无需昂贵真实数据收集，具有实际临床应用价值
...
### [72] [AI-based Prediction of Biochemical Recurrence from Biopsy and Prostatectomy Samples](https://arxiv.org/abs/2601.21022)
...
TL;DR: 基于前列腺活检病理切片训练的AI模型可预测根治性前列腺切除术后生化复发风险，在多个外部队列中验证了泛化能力，结合临床变量可改善术后预后评估。


<details>
  <summary>Details</summary>
Motivation: 根治性前列腺切除术后生化复发是侵袭性前列腺癌的替代标志物，但现有预后工具不够精确，需要开发更准确的预测方法。

Method: 使用STHLM3队列（n=676）的诊断性前列腺活检切片训练AI模型，基于基础模型和注意力机制的多实例学习预测患者特异性BCR风险，并在三个外部RP队列（LEOPARD、CHIMERA、TCGA-PRAD）评估泛化能力。

Result: 图像方法在三个外部队列中分别获得0.64、0.70和0.70的5年时间依赖性AUC；结合临床变量可提供互补预后价值并实现统计学显著的风险分层；相比指南推荐的CAPRA-S，AI模型能增量改善术后预后评估。
...
### [73] [Towards Mitigating Modality Bias in Vision-Language Models for Temporal Action Localization](https://arxiv.org/abs/2601.21078)
*Jiaqi Li,Guangming Wang,Shuntian Zheng,Minzhe Ni,Xiaoman Lu,Guanghui Ye,Yu Guan*

Main category: cs.CV

TL;DR: ActionVLM是一个视觉语言聚合框架，通过去偏重加权模块和残差聚合策略，系统性地缓解TAL中的模态偏差，在保持视觉主导的同时自适应地利用语言模态。


<details>
  <summary>Details</summary>
Motivation: 现有TAL方法在利用视觉语言模型时过度强调语言先验，牺牲了视觉性能，导致明显的模态偏差。需要一种能保持视觉主导信号，同时自适应利用语言优势的解决方案。

Method: 提出ActionVLM框架：1) 去偏重加权模块，估计语言相对于纯视觉预测的增量优势，并动态调整语言模态权重；2) 残差聚合策略，将语言视为补充性细化而非主要驱动力。

Result: 在THUMOS14数据集上，ActionVLM比现有最优方法提升了高达3.2%的mAP，有效缓解了模态偏差，减少了语言先验的过度自信，并增强了时序推理能力。

Conclusion: ActionVLM通过系统性地平衡视觉和语言模态，在保持视觉主导的同时自适应利用语言优势，为缓解TAL中的模态偏差提供了有效解决方案，显著提升了动作定位性能。
...
### [74] [Shape of Thought: Progressive Object Assembly via Visual Chain-of-Thought](https://arxiv.org/abs/2601.21081)
...
Method: 提出SoT视觉思维链框架，训练统一的多模态自回归模型生成交错的文本计划和渲染的中间状态，通过连贯的2D投影实现渐进式形状组装，无需显式几何表示。创建SoT-26K数据集（基于CAD层次结构的组装轨迹）和T2S-CompBench基准。

Result: 在SoT-26K上微调后，在组件计数方面达到88.4%，在结构拓扑方面达到84.8%，比纯文本基线提高约20%。SoT建立了透明、过程监督的组合生成新范式。

Conclusion: SoT通过视觉思维链方法有效解决了文本到图像生成中的组合结构约束问题，为透明、过程监督的组合生成建立了新范式，在组件计数和结构拓扑方面显著优于现有方法。
...
### [75] [An AI Framework for Microanastomosis Motion Assessment](https://arxiv.org/abs/2601.21120)
...
<details>
  <summary>Details</summary>
Motivation: 微血管吻合术需要极高的精确度和技术技能，传统评估依赖专家主观判断，存在评分者间变异、缺乏标准化标准、认知偏见和时间消耗等问题，急需客观、可靠、自动化的评估系统

Method: 提出包含四个核心组件的AI框架：1)基于YOLO架构的器械检测模块；2)基于DeepSORT的器械跟踪模块；3)使用形状描述符的器械尖端定位模块；4)基于专家标注数据训练的监督分类模块，用于评估器械操作熟练度

Result: 实验结果显示框架有效，器械检测精度达到97%，平均精度(mAP)为96%，通过交并比(IoU)阈值50%到95%测量(mAP50-95)
...
### [76] [Bidirectional Cross-Perception for Open-Vocabulary Semantic Segmentation in Remote Sensing Imagery](https://arxiv.org/abs/2601.21159)
...
Method: 提出空间正则化感知的双分支协同推理框架SDCI：1）特征编码阶段引入跨模型注意力融合模块，通过相互注入自注意力图指导协同推理；2）提出双向交叉图扩散细化模块，通过迭代随机游走扩散增强双分支分割分数的可靠性；3）结合低层超像素结构，开发基于凸优化的超像素协同预测机制进一步细化对象边界

Result: 在多个遥感语义分割基准测试中，该方法比现有方法表现更好。消融研究进一步证实，利用超像素结构的传统基于对象的遥感图像分析方法在深度学习框架中仍然有效

Conclusion: SDCI框架通过创新的跨模型注意力融合、双向图扩散和超像素协同预测机制，有效解决了高分辨率遥感图像开放词汇语义分割中的几何定位和语义预测挑战，为无需训练的遥感图像分析提供了新思路
...
### [77] [Enhancing Underwater Light Field Images via Global Geometry-aware Diffusion Process](https://arxiv.org/abs/2601.21179)
...
Method: 1) 带卷积和注意力适配器的改进U-Net架构建模几何线索；2) 基于张量分解和渐进加权的几何引导损失函数正则化全局结构；3) 带噪声预测的优化采样策略提高效率

Result: 在视觉保真度和定量性能上均优于现有方法，有效缓解水下场景颜色失真，推进了水下成像增强的技术前沿
...
### [78] [Generative Recall, Dense Reranking: Learning Multi-View Semantic IDs for Efficient Text-to-Video Retrieval](https://arxiv.org/abs/2601.21193)
...
<details>
  <summary>Details</summary>
Motivation: 当前两阶段文本-视频检索中，召回模型性能受限于生成式检索存在的两个关键问题：1) 语义模糊性——每个视频满足多种查询但被迫分配单一语义ID；2) 跨模态未对齐——语义ID仅从视觉特征生成，缺乏文本监督。这些问题限制了生成式检索作为召回模型的性能。

Method: GRDR设计了新颖的生成式检索方法：1) 使用查询引导的多视图分词器为每个视频分配多个语义ID，提供多样化的语义访问路径；2) 通过共享码本联合训练分词器和生成式检索器，使语义ID成为文本和视频之间的语义桥梁；3) 推理时采用trie约束解码生成紧凑候选集，然后由密集模型进行重排序。

Result: 在TVR基准测试中，GRDR在精度上匹配强大的密集检索器，同时将索引存储减少一个数量级，在全库检索中加速高达300倍。

Conclusion: GRDR通过解决生成式检索的语义模糊和跨模态对齐问题，显著提升了两阶段文本-视频检索中召回模型的质量，实现了精度与效率的平衡，为大规模实时应用提供了可行的解决方案。
...
### [79] [Thinker: A vision-language foundation model for embodied intelligence](https://arxiv.org/abs/2601.21199)
*Baiyu Pan,Daqin Luo,Junpeng Yang,Jiyuan Wang,Yixuan Zhang,Hailin Shi,Jichao Jiao*

Main category: cs.CV

TL;DR: 提出Thinker模型解决机器人领域视觉语言模型的视角混淆和时序推理问题，通过构建大规模机器人感知数据集和结合关键帧与完整视频序列的方法，在任务规划基准上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型应用于机器人领域时存在人类简单但模型易错的问题：1）第三人称与第一人称视角混淆；2）时序推理中容易忽略视频结尾信息。需要专门针对具身智能的视觉语言基础模型来解决这些问题

Method: 1）构建大规模机器人感知与推理数据集，包含自我中心视角视频、视觉定位、空间理解和思维链数据；2）提出简单有效的视频理解增强方法，联合使用关键帧和完整视频序列作为输入

Result: 在任务规划领域两个最常用的基准数据集上取得了最先进的结果

Conclusion: 提出的Thinker模型通过专门的数据集构建和视频理解方法，有效解决了机器人领域视觉语言模型的视角混淆和时序推理问题，在具身智能任务中表现出色
...
### [80] [LAMP: Learning Universal Adversarial Perturbations for Multi-Image Tasks via Pre-trained Models](https://arxiv.org/abs/2601.21220)
*Alvi Md Ishmam,Najibul Haque Sarker,Zaber Ibn Abdul Hakim,Chris Thomas*

Main category: cs.CV

TL;DR: LAMP是一种针对多模态大语言模型的黑盒攻击方法，通过学习通用对抗扰动，利用注意力约束和跨图像传染约束来破坏多图像信息聚合


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多图像处理方面取得了显著进展，但其安全漏洞尚未被充分探索。现有对抗攻击主要针对单图像场景且多为白盒攻击，在实际应用中不切实际

Method: 提出LAMP方法：1) 基于注意力的约束防止模型有效聚合多图像信息；2) 跨图像传染约束使扰动标记影响干净标记，无需修改所有输入；3) 索引注意力抑制损失实现鲁棒的位置不变攻击

Result: 实验结果表明，LAMP在多个视觉语言任务和模型上超越了现有最先进基线，实现了最高的攻击成功率

Conclusion: LAMP首次探索了多图像MLLM的黑盒对抗攻击，通过创新的约束机制有效破坏了多图像信息聚合，揭示了此类模型的安全脆弱性
...
### [81] [PTQ4ARVG: Post-Training Quantization for AutoRegressive Visual Generation Models](https://arxiv.org/abs/2601.21238)
*Xuewen Liu,Zhikai Li,Jing Zhang,Mengjuan Chen,Qingyi Gu*

Main category: cs.CV

TL;DR: PTQ4ARVG是一个针对自回归视觉生成模型的训练后量化框架，通过增益投影缩放、静态令牌级量化和分布引导校准解决通道级异常值、令牌级动态激活和样本级分布不匹配三大挑战，实现8位和6位量化而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成模型虽然与语言模型架构兼容且性能接近扩散模型，但其量化研究不足，现有量化方法无法有效应用于ARVG模型。论文识别出三个关键挑战：通道级严重异常值、令牌级高度动态激活和样本级分布信息不匹配。

Method: 提出PTQ4ARVG训练后量化框架，包含三个核心组件：1) 增益投影缩放通过泰勒展开量化激活-权重量化的缩放增益，通过微分推导最优缩放因子来缓解通道级异常值；2) 静态令牌级量化利用ARVG固定令牌长度和样本间位置不变分布的特性，无需动态校准开销即可处理令牌级方差；3) 分布引导校准选择对分布熵贡献最大的样本来消除样本级分布不匹配。

Result: 大量实验表明PTQ4ARVG能够有效将ARVG系列模型量化为8位和6位，同时保持有竞争力的性能。代码已在GitHub开源。

Conclusion: PTQ4ARVG成功解决了ARVG模型量化的三大挑战，为自回归视觉生成模型的高效部署提供了有效的训练后量化解决方案，在保持性能的同时显著减少模型大小和计算延迟。
...
### [82] [NFCDS: A Plug-and-Play Noise Frequency-Controlled Diffusion Sampling Strategy for Image Restoration](https://arxiv.org/abs/2601.21248)
*Zhen Wang,Hongyi Liu,Jianing Li,Zhihui Wei*

Main category: cs.CV

TL;DR: 提出NFCDS方法，通过频谱调制机制控制反向扩散噪声频率，解决扩散采样PnP方法中保真度与感知质量之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 基于扩散采样的PnP方法能生成高感知质量的图像，但常因反向扩散引入的噪声而降低数据保真度，需要解决保真度与感知质量之间的冲突。

Method: 提出噪声频率控制扩散采样(NFCDS)，通过傅里叶域滤波器渐进抑制低频噪声并保留高频内容，将数据一致性先验直接注入采样过程，无需额外训练。

Result: NFCDS作为PnP模块能无缝集成到现有扩散修复框架中，在多种零样本任务中改善保真度与感知质量的平衡，实现快速收敛到高保真且感知可信的结果。

Conclusion: 保真度-感知冲突可通过噪声频率理解，低频噪声导致模糊降保真度，高频噪声驱动细节生成；NFCDS通过频谱调制机制有效解决这一权衡问题。
...
### [83] [Hypersolid: Emergent Vision Representations via Short-Range Repulsion](https://arxiv.org/abs/2601.21255)
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习中防止表示崩溃是一个持续挑战。现有方法通常依赖全局正则化（如最大化距离、去相关维度或强制特定分布），但本文提出从不同角度重新思考这个问题

Method: 将表示学习重新解释为离散填充问题，其中保持信息简化为保持单射性。Hypersolid方法使用短程硬球排斥来防止局部碰撞，这种约束产生高分离几何机制，能够保持增强多样性

Result: 该方法在细粒度和低分辨率分类任务上表现出色，通过高分离几何机制有效保持了增强多样性

Conclusion: 通过将表示学习重新框架为离散填充问题并使用局部排斥约束，Hypersolid提供了一种防止表示崩溃的新方法，在特定任务类型上优于传统的全局正则化方法
...
### [84] [Lightweight High-Fidelity Low-Bitrate Talking Face Compression for 3D Video Conference](https://arxiv.org/abs/2601.21269)
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视频会议需要高保真3D说话人脸表示，但传统2D视频压缩技术无法保留细粒度几何和外观细节，而NeRF等隐式神经渲染方法计算成本过高。需要在低比特率下实现高质量3D人脸压缩

Method: 集成FLAME参数化建模与3DGS神经渲染，仅传输关键面部元数据，使用高斯头部模型进行高效重建。引入紧凑表示和压缩方案，包括高斯属性压缩和MLP优化

Result: 实验结果表明该方法在极低比特率下实现了优越的率失真性能，提供高质量面部渲染，适合实时3D视频会议应用

Conclusion: 提出的框架成功解决了3D说话人脸高保真低比特率压缩的挑战，为实时3D视频会议提供了有效的解决方案
...
### [85] [GeoRC: A Benchmark for Geolocation Reasoning Chains](https://arxiv.org/abs/2601.21278)
*Mohit Talreja,Joshua Diao,Jim Thannikary James,Radu Casapu,Tejas Santanam,Ethan Mendes,Alan Ritter,Wei Xu,James Hays*

Main category: cs.CV

TL;DR: 该论文提出了首个地理定位推理链基准测试，评估视觉语言模型在解释地理定位决策时的能力，发现即使定位准确的模型在生成可审计推理链方面仍远逊于人类专家。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在地理定位预测方面表现出色，但在解释其预测依据方面存在严重缺陷，经常产生幻觉性推理链。需要建立基准来评估模型生成可靠、可审计推理链的能力。

Method: 1. 基于GeoGuessr游戏构建包含500个查询场景的数据集；2. 与专家玩家合作创建800个真实推理链作为基准；3. 评估LLM-as-a-judge和VLM-as-a-judge两种评分策略；4. 比较不同模型在推理链生成任务上的表现。

Result: 1. Qwen 3 LLM-as-a-judge评分与人类评分相关性最佳；2. 闭源大模型如Gemini和GPT-5在定位预测上接近人类专家，但在推理链生成上仍落后；3. 开源模型如Llama和Qwen表现灾难性失败，仅略优于无视觉信息的幻觉基线。

Conclusion: 当前视觉语言模型在从高分辨率图像中提取细粒度视觉属性方面存在局限性，导致无法生成可靠的地理定位推理链。该基准揭示了模型在可解释性方面的重大差距，为未来改进提供了方向。
...
### [86] [WorldBench: Disambiguating Physics for Diagnostic Evaluation of World Models](https://arxiv.org/abs/2601.21282)
...
TL;DR: WorldBench是一个新型视频基准测试，专门用于解耦评估世界模型的物理推理能力，通过隔离单个物理概念进行诊断性测试，发现当前最先进模型在特定物理概念上存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 现有基于物理的视频基准测试存在概念纠缠问题，即单个测试同时评估多个物理定律和概念，这从根本上限制了其诊断能力。为了可靠部署生成式基础模型（世界模型）到机器人规划和自主系统训练等关键任务中，需要能够精确评估模型对单个物理概念的理解。

Method: 设计了WorldBench基准测试，包含两个层次：1) 直观物理理解评估（如物体恒存性、尺度/透视）；2) 低层次物理常数和材料属性评估（如摩擦系数、流体粘度）。该基准通过解耦设计，能够隔离并评估对单个物理概念或定律的理解。

Result: 在WorldBench上评估当前最先进的视频世界模型时，发现了特定物理概念上的系统性失败模式。所有测试模型都缺乏生成可靠真实世界交互所需的物理一致性，表明现有模型在物理推理能力上存在显著缺陷。

Conclusion: WorldBench通过概念特定的解耦评估，为视频生成和世界模型的物理推理能力评估提供了一个更细致、可扩展的框架，为开发更稳健、可泛化的世界模型驱动学习铺平了道路。
...
### [87] [Mam-App: A Novel Parameter-Efficient Mamba Model for Apple Leaf Disease Classification](https://arxiv.org/abs/2601.21307)
*Md Nadim Mahamood,Md Imran Hasan,Md Rasheduzzaman,Ausrukona Ray,Md Shafi Ud Doula,Kamrul Hasan*

Main category: cs.CV

TL;DR: 提出Mam-App模型，一种基于Mamba架构的参数高效模型，用于苹果叶病害分类，在PlantVillage数据集上达到99.58%准确率，仅使用0.051M参数，适合资源受限设备部署。


<details>
  <summary>Details</summary>
Motivation: 全球人口增长和技术进步加剧了粮食需求，苹果作为重要水果，病害导致大量产量损失。现有深度学习模型参数过多导致训练和推理时间长，轻量级模型又存在性能下降问题，需要在效率和性能之间取得平衡。

Method: 提出Mam-App模型，基于Mamba架构设计参数高效的特征提取和叶病害分类模型。模型在PlantVillage苹果叶病害数据集上进行训练和评估，并在玉米叶病害和马铃薯叶病害数据集上验证其鲁棒性和泛化能力。

Result: 在苹果叶病害数据集上达到99.58%准确率、99.30%精确率、99.14%召回率和99.22% F1分数，仅使用0.051M参数。在玉米数据集上达到99.48%准确率，在马铃薯数据集上达到98.46%准确率，证明了模型的鲁棒性和泛化能力。

Conclusion: Mam-App模型在保持高性能的同时大幅减少参数数量，解决了效率与性能的权衡问题，适合部署在无人机、移动设备等资源受限平台上，为农业病害早期诊断提供了实用解决方案。
...
### [88] [HiFi-Mesh: High-Fidelity Efficient 3D Mesh Generation via Compact Autoregressive Dependence](https://arxiv.org/abs/2601.21314)
*Yanfeng Li,Tao Tan,Qingquan Gao,Zhiwen Cao,Xiaohong liu,Yue Sun*

Main category: cs.CV

TL;DR: LANE通过紧凑自回归依赖和AdaGraph策略，实现6倍序列长度提升和加速推理，解决现有3D网格生成方法资源利用率不足的问题。
...
Conclusion: LANE为高质量3D网格生成提供了有效解决方案，通过改进自回归依赖和推理效率，显著提升了3D网格生成的性能。
...
### [89] [Do Pathology Foundation Models Encode Disease Progression? A Pseudotime Analysis of Visual Representations](https://arxiv.org/abs/2601.21334)
*Pritika Vig,Ren-Chin Wu,William Lotter*

Main category: cs.CV

TL;DR: 该研究探索视觉基础模型是否能在表示空间中隐式编码疾病的连续进展过程，发现病理学专用模型能有效恢复疾病进展轨迹，且轨迹保真度与下游任务性能高度相关。


<details>
  <summary>Details</summary>
Motivation: 尽管在离散图像上训练的视觉基础模型在分类任务上表现良好，但尚不清楚其表示是否编码了训练数据背后的连续生物学过程。在计算病理学中，能够隐式捕获疾病连续进展的模型可能更好地反映基础生物学、支持更稳健的泛化，并实现对疾病转变相关特征的定量分析。

Method: 使用扩散伪时间（一种从单细胞转录组学推断发育轨迹的方法）来探究基础模型是否在表示空间中沿着连贯的进展方向组织疾病状态。研究分析了四种癌症进展和六个模型，评估模型恢复轨迹排序的能力，并将轨迹保真度与少样本分类性能进行相关性分析。

Result: 所有病理学专用模型恢复的轨迹排序均显著超过零基线，其中仅视觉模型在CRC-Serrated上达到最高保真度（τ>0.78）。模型在参考疾病上的轨迹保真度排名能强预测在保留疾病上的少样本分类性能（ρ=0.92）。探索性分析显示细胞类型组成沿着推断轨迹平滑变化，与已知的基质重塑模式一致。
...
### [90] [Dynamical Adapter Fusion: Constructing A Global Adapter for Pre-Trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2601.21341)
*Ruiqi Liu,Boyu Diao,Zijia An,Zhulin An,Fei Wang,Yongjun Xu*

Main category: cs.CV

TL;DR: 提出Dynamical Adapter Fusion (DAF)方法，通过构建单一全局适配器解决类增量学习中任务特定参数导致的干扰和遗忘问题，基于PAC-Bayes定理实现参数融合，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 类增量学习需要模型持续学习新类别而不遗忘旧类别。现有方法通常冻结预训练模型并训练轻量级任务特定适配器，但这会阻碍知识迁移、增加检索成本，且参数融合容易导致破坏性干扰和灾难性遗忘。

Method: 提出动态适配器融合(DAF)方法：1) 基于PAC-Bayes定理推导融合机制，整合任务特定适配器参数、先前全局适配器参数和初始化参数；2) 利用损失函数的泰勒展开推导最优融合系数，动态平衡稳定性和可塑性；3) 提出鲁棒初始化策略有效捕捉全局知识模式。

Result: 在多个类增量学习基准测试中，DAF方法实现了最先进的性能表现。

Conclusion: DAF通过构建单一全局适配器，有效解决了类增量学习中任务特定参数融合的挑战，实现了知识迁移与遗忘控制的平衡，为类增量学习提供了新的解决方案。
...
### [91] [Semantic-Guided Dynamic Sparsification for Pre-Trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2601.21345)
*Ruiqi Liu,Boyu Diao,Zijia An,Runjie Shao,Zhulin An,Fei Wang,Yongjun Xu*

Main category: cs.CV

TL;DR: SGDS提出语义引导的动态稀疏化方法，通过激活空间的定向稀疏化来缓解类增量学习中的灾难性遗忘问题，避免对参数空间施加刚性约束


<details>
  <summary>Details</summary>
Motivation: 现有类增量学习方法通常冻结预训练模型并使用轻量级适配器，并通过强制参数正交来防止任务间干扰，但这种方法会损害模型的可塑性

Method: 提出语义引导的动态稀疏化（SGDS），通过控制激活空间子空间的方向和秩来进行定向稀疏化，促进相似类共享紧凑激活子空间，同时为不相似类分配非重叠激活子空间

Result: 在多个基准数据集上的广泛实验表明，SGDS实现了最先进的性能

Conclusion: SGDS通过在激活空间中雕刻类特定的稀疏子空间，有效缓解干扰而不对参数空间施加刚性约束，为类增量学习提供了更优的解决方案
...
### [92] [Rectifying Geometry-Induced Similarity Distortions for Real-World Aerial-Ground Person Re-Identification](https://arxiv.org/abs/2601.21405)
*Kailash A. Hambarde,Hugo Proença*

Main category: cs.CV

TL;DR: 提出GIQT方法解决无人机-地面行人重识别中的几何畸变问题，通过几何诱导的查询-键变换显式校正相似度空间，提升极端视角和距离变化下的匹配性能。
...
Method: 提出几何诱导查询-键变换(GIQT)，这是一个轻量级低秩模块，通过将查询-键交互条件化于相机几何来显式校正相似度空间。不修改特征表示或注意力公式本身，而是调整相似度计算以补偿主导的几何诱导各向异性畸变。在此基础上，进一步结合几何条件提示生成机制，提供直接从相机几何导出的全局、视图自适应表示先验。

Result: 在四个无人机-地面行人重识别基准测试上的实验表明，所提框架在极端和先前未见几何条件下持续提升鲁棒性，同时相比最先进方法引入最小计算开销。

Conclusion: GIQT通过显式校正相似度空间解决了无人机-地面行人重识别中的几何畸变问题，提供了一种轻量级且有效的解决方案，显著提升了在极端视角和距离变化下的匹配性能。
...
### [93] [Generation Enhances Understanding in Unified Multimodal Models via Multi-Representation Generation](https://arxiv.org/abs/2601.21406)
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型（UMMs）的研究主要集中在利用理解能力来增强生成能力，而利用生成来提升理解能力的方向尚未充分探索。理解与生成应该形成相互促进的循环，但目前缺乏有效方法来实现这一目标。

Method: 提出UniMRG（Unified Multi-Representation Generation）方法，这是一种简单有效的架构无关后训练方法。核心思想是让UMMs生成输入图像的多种内在表示：像素级重建（外观）、深度图（几何）和分割图（结构），同时结合标准的视觉理解任务进行训练。

Result: 在不同UMM架构上的大量实验表明，该方法显著提升了细粒度感知能力，减少了幻觉现象，改善了空间理解能力，同时增强了生成能力。模型通过生成多种表示获得了对视觉输入更深入全面的理解。

Conclusion: UniMRG通过引入辅助生成任务，成功实现了利用生成来增强理解的目标，填补了UMMs研究中理解与生成相互促进循环的关键空白。该方法简单有效且架构无关，为统一多模态模型的发展提供了新方向。
...
### [94] [MPF-Net: Exposing High-Fidelity AI-Generated Video Forgeries via Hierarchical Manifold Deviation and Micro-Temporal Fluctuations](https://arxiv.org/abs/2601.21408)
*Xinan He,Kaiqing Lin,Yue Zhou,Jiaming Zhong,Wei Ye,Wenhui Yi,Bing Fan,Feng Ding,Haodong Li,Bo Cao,Bin Li*

Main category: cs.CV

TL;DR: 提出一种分层双路径框架检测AI生成视频，通过静态流形偏差分支捕捉空间异常，微时域波动分支分析连续帧间的结构化投影波动，即使高保真伪造视频也能被识别。


<details>
  <summary>Details</summary>
Motivation: 尽管当前视频生成模型（如Veo和Wan）的视觉质量已很高，宏观语义错误和时间不一致性不再明显，但AI生成视频与真实视频仍有本质区别。AI视频本质上是流形拟合过程的产物而非物理录制，其连续相邻帧的像素构成逻辑表现出结构化、同质化特征，这种"流形投影波动"现象为检测提供了理论基础。

Method: 提出分层双路径框架作为顺序过滤过程：1）静态流形偏差分支：利用大规模视觉基础模型的精细感知边界，捕捉偏离自然真实世界流形的空间异常或物理违规；2）微时域波动分支：针对成功驻留在流形上并逃避空间检测的高保真视频，通过分析视觉完美序列中持续存在的结构化MPF进行细粒度过滤。

Result: 该框架能够确保伪造视频被暴露，无论其表现为全局真实世界流形偏差还是细微的计算指纹。通过结合空间异常检测和时域波动分析，即使高保真的AI生成视频也能被有效识别。

Conclusion: AI生成视频本质上是流形拟合产物而非物理录制，其连续帧间的结构化像素构成逻辑形成可检测的"流形投影波动"。提出的分层双路径框架通过顺序过滤机制，结合空间异常检测和微时域波动分析，能够有效识别高保真伪造视频，为AI生成内容检测提供了新思路。
...
### [95] [From Implicit Ambiguity to Explicit Solidity: Diagnosing Interior Geometric Degradation in Neural Radiance Fields for Dense 3D Scene Understanding](https://arxiv.org/abs/2601.21421)
*Jiangsan Zhao,Jakob Geipel,Kryzysztof Kusnierek*

Main category: cs.CV

TL;DR: 该研究揭示了NeRF在密集自遮挡场景中的几何退化问题，提出基于稀疏体素栅格化的显式几何管道，显著提高了实例恢复率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究NeRF在密集自遮挡场景中进行定量3D分析的可靠性问题。尽管NeRF在多视图重建中表现出色，但其在密集、自遮挡场景中的可靠性尚未得到充分理解，特别是在处理内部几何退化（IGD）问题方面。

Method: 研究方法包括：1）通过合成数据集进行控制实验，分析隐式密度场在重度遮挡下的失效模式；2）提出基于稀疏体素栅格化（SVRaster）的显式几何管道，该方法从SfM特征几何初始化，将2D实例掩码投影到显式体素网格上，并通过递归分割强制几何分离；3）使用退化分割掩码进行敏感性分析，比较显式与隐式方法的鲁棒性。

Result: 研究结果显示：1）最先进的掩码监督NeRF在密集场景中实例恢复率仅约89%，存在系统性的实例计数不足；2）提出的SVRaster方法在密集集群中实现了95.8%的恢复率；3）敏感性分析表明，基于SfM的显式几何对监督失效具有更强的鲁棒性，比隐式基线多恢复43%的实例。

Conclusion: 结论是显式几何先验是高度自遮挡3D场景中可靠定量分析的前提条件。隐式密度场在重度遮挡下会出现内部几何退化，导致空心或碎片化结构，而显式几何方法能保持物理实体性，提供更可靠的3D分析结果。
...
### [96] [MultiModal Fine-tuning with Synthetic Captions](https://arxiv.org/abs/2601.21426)
*Shohei Enomoto,Shin'ya Yamaguchi*

Main category: cs.CV

TL;DR: 该论文提出了一种将单模态数据集转化为多模态数据集的方法，利用多模态大语言模型生成合成图像描述，以弥合多模态预训练与单模态微调之间的差距，从而提升图像分类性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习存在一个基本差距：预训练已从单模态转向多模态学习以增强视觉理解，但微调仍主要保持单模态，限制了丰富预训练表示的优势。需要弥合多模态预训练与单模态微调之间的鸿沟。

Method: 1. 使用多模态大语言模型将单模态数据集转化为多模态数据集，生成合成图像描述；2. 设计包含类别标签和领域上下文的提示词以生成高质量分类任务描述；3. 引入监督对比损失函数，在微调期间显式鼓励同类表示的聚类；4. 提出新的推理技术，利用每张图像多个合成描述生成的类别平均文本嵌入。

Result: 在13个图像分类基准测试上的广泛实验表明，该方法优于基线方法，在少样本学习场景中改进尤为显著，验证了通过数据集增强弥合多模态预训练与微调差距的有效性。

Conclusion: 该工作建立了一个新的数据集增强范式，有效弥合了多模态预训练与微调之间的差距，为利用多模态预训练表示进行更好的视觉任务微调提供了新途径。
...
### [97] [Spava: Accelerating Long-Video Understanding via Sequence-Parallelism-aware Approximate Attention](https://arxiv.org/abs/2601.21444)
...
Method: 提出Spava序列并行框架，通过分布式近似注意力减少计算并增加并行性，无需压缩视觉嵌入；系统级优化包括负载均衡和融合前向传播

Result: 相比FlashAttn、ZigZagRing和APB分别实现12.72倍、1.70倍和1.18倍加速，且无明显性能损失

Conclusion: Spava通过序列并行和优化注意力机制，有效解决了长视频推理的计算瓶颈，实现了高效的多GPU加速
...
### [98] [Variance & Greediness: A comparative study of metric-learning losses](https://arxiv.org/abs/2601.21450)
*Donghuo Zeng,Hao Niu,Zhi Li,Masato Taya*

Main category: cs.CV

TL;DR: 该研究提出VARIANCE和GREEDINESS诊断框架，分析7种度量学习损失函数在5个图像检索数据集上的表现，揭示了效率-粒度权衡：Triplet和SCL在细粒度检索中表现更好，而Contrastive和InfoNCE收敛更快但可能过度简化类别结构。
...
Conclusion: 揭示了效率-粒度权衡：当需要保持多样性和困难样本区分时，应优先选择Triplet/SCL；当需要更快的嵌入压缩时，应选择Contrastive/InfoNCE。为实际应用提供了指导。
...
### [99] [Mining Forgery Traces from Reconstruction Error: A Weakly Supervised Framework for Multimodal Deepfake Temporal Localization](https://arxiv.org/abs/2601.21458)
*Midou Guo,Qilin Yin,Wei Lu,Xiangyang Luo,Rui Yang*

Main category: cs.CV

TL;DR: RT-DeepLoc：基于重建的弱监督时序深度伪造定位框架，利用仅在真实数据上训练的MAE检测伪造片段，通过非对称视频内对比损失增强定位能力


<details>
  <summary>Details</summary>
Motivation: 现代深度伪造技术已发展为局部化和间歇性操作，需要细粒度时序定位。由于帧级标注成本过高，弱监督方法成为实际需求，仅依赖视频级标签

Method: 提出RT-DeepLoc框架：1）使用仅在真实数据上训练的掩码自编码器学习内在时空模式；2）通过重建误差识别伪造片段；3）引入非对称视频内对比损失，利用重建线索指导真实特征紧密度，建立稳定决策边界

Result: 在LAV-DF等大规模数据集上的广泛实验表明，RT-DeepLoc在弱监督时序伪造定位任务中达到了最先进的性能

Conclusion: RT-DeepLoc通过重建误差检测伪造片段，结合非对称视频内对比损失，有效解决了弱监督时序深度伪造定位问题，在保持对未见伪造泛化能力的同时增强了局部判别能力
...
### [100] [SimGraph: A Unified Framework for Scene Graph-Based Image Generation and Editing](https://arxiv.org/abs/2601.21498)
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式AI方法通常将图像生成和编辑作为独立任务处理，导致效率低下且难以维持空间一致性和语义连贯性。主要障碍是缺乏对对象关系和空间布局的结构化控制。
...
Conclusion: SimGraph通过场景图结构化表示成功统一了图像生成和编辑，解决了空间一致性和语义连贯性问题，为生成式AI提供了更精确的构图和交互控制。
...
### [101] [Unifying Heterogeneous Degradations: Uncertainty-Aware Diffusion Bridge Model for All-in-One Image Restoration](https://arxiv.org/abs/2601.21592)
*Luwei Tu,Jiawei Wu,Xing Luo,Zhi Jin*

Main category: cs.CV

TL;DR: 提出UDBM模型，通过不确定性感知扩散桥将多任务图像恢复重新定义为随机传输问题，解决了异构退化间的优化冲突，在单步推理中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有All-in-One图像恢复方法存在局限性：粗粒度控制机制或固定映射调度难以协调异构退化间的冲突优化目标，导致次优适应。需要更精细的像素级不确定性建模来改善多退化场景下的恢复性能。

Method: 提出不确定性感知扩散桥模型（UDBM）：1）将AiOIR重新定义为随机传输问题，通过像素级不确定性引导；2）引入松弛扩散桥公式，用松弛约束替代严格终端约束，建模退化不确定性并理论解决标准扩散桥的漂移奇异性；3）设计双重调制策略：噪声调度将多样退化对齐到共享高熵潜在空间，路径调度基于熵正则化的粘性动力学自适应调节传输轨迹。

Result: UDBM通过有效修正传输几何和动力学，在多种恢复任务（如去噪、去模糊、超分辨率等）上实现了最先进的性能，且仅需单步推理。

Conclusion: UDBM通过将AiOIR重新定义为不确定性引导的随机传输问题，解决了异构退化间的优化冲突。松弛扩散桥公式和双重调制策略实现了精细的退化建模和自适应传输控制，为多任务图像恢复提供了高效统一的解决方案。
...
### [102] [HydroSense: A Dual-Microcontroller IoT Framework for Real-Time Multi-Parameter Water Quality Monitoring with Edge Processing and Cloud Analytics](https://arxiv.org/abs/2601.21595)
*Abdul Hasib,A. S. M. Ahsanul Sarkar Akib,Anish Giri*

Main category: cs.CV

TL;DR: HydroSense是一个低成本物联网水质监测系统，采用双微控制器架构，集成6个关键水质参数，相比商业系统成本降低85%，在90天测试中表现出高精度和可靠性。
...
Method: 提出HydroSense物联网框架，采用双微控制器架构：Arduino Uno用于精密模拟测量和五点校准算法，ESP32用于无线连接、边缘处理和云集成。系统集成pH、溶解氧、温度、总溶解固体、估算氮含量和水位6个参数，采用中值滤波、温度补偿算法和鲁棒错误处理等先进信号处理技术。
...
### [103] [WMVLM: Evaluating Diffusion Model Image Watermarking via Vision-Language Models](https://arxiv.org/abs/2601.21610)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型图像水印评估方法存在四大局限：缺乏统一框架同时处理残差和语义水印；结果缺乏可解释性；忽视全面安全考量；对语义水印常使用不恰当指标

Method: 提出WMVLM框架，重新定义两类水印的评估指标：残差水印评估伪影强度和擦除抗性，语义水印评估潜在分布偏移。采用三阶段训练策略，逐步实现分类、评分和可解释文本生成
...
### [104] [PathReasoner-R1: Instilling Structured Reasoning into Pathology Vision-Language Model via Knowledge-Guided Policy Optimization](https://arxiv.org/abs/2601.21617)
*Songhan Jiang,Fengchun Liu,Ziyue Wang,Linghan Cai,Yongbing Zhang*

Main category: cs.CV

TL;DR: PathReasoner是首个大规模全切片图像推理数据集，通过知识引导生成高质量指令样本，并开发PathReasoner-R1模型，结合轨迹掩码监督微调和推理导向强化学习，赋予病理模型透明、临床可信的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在病理学应用中存在局限性：诊断过程直接输出结论而缺乏可验证的证据链推理，这严重限制了临床信任度并阻碍专家错误纠正。需要构建具有透明推理能力的病理AI系统。

Method: 1) 构建PathReasoner数据集：通过医学知识图谱引导的生成流程，将结构化病理发现和临床推理与诊断对齐，生成超过20K高质量指令样本；2) 开发PathReasoner-R1模型：结合轨迹掩码监督微调与推理导向强化学习，融入知识感知的多粒度奖励函数，包含与知识图谱严格对齐的实体奖励机制。

Result: PathReasoner-R1在PathReasoner数据集和公共基准测试中均达到最先进性能，支持多种图像尺度。模型展现出逻辑一致性而非仅结果匹配的优化能力，增强了鲁棒性。

Conclusion: PathReasoner系统成功解决了病理AI中缺乏可验证推理的问题，通过知识引导的数据集构建和结构化推理训练方法，为病理模型提供了透明、临床可信的推理能力，推动了计算病理学的发展。
...
### [105] [Similarity of Processing Steps in Vision Model Representations](https://arxiv.org/abs/2601.21621)
*Matéo Mahaut,Marco Baroni*

Main category: cs.CV

TL;DR: 该研究探讨了不同视觉模型在训练过程中是否收敛到相同的中间表示和操作，而不仅仅是最终表示相似。通过量化不同模型在不同处理阶段的表示距离，发现虽然相似位置的层具有最相似的表示，但不同模型之间仍存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有文献表明大型模型倾向于收敛到相似的"通用"表示，但作者想要研究这些模型是否也通过相同的中间步骤和操作达到这些表示。他们关注视觉模型在达到收敛表示过程中的具体路径差异。

Method: 首先量化不同模型在不同处理阶段的表示距离，跟踪模型在整个处理过程中表示距离的演变，识别不同模型之间差异最大的处理步骤。特别关注分类器模型、CNN和transformer模型的行为差异。

Result: 发现相似位置的层具有最相似的表示，但不同模型之间仍存在显著差异。分类器模型与其他模型不同，会在最终层丢弃关于低层次图像统计的信息。CNN和transformer模型行为也不同，transformer模型在层与层之间对表示应用更平滑的变化。

Conclusion: 这些区别阐明了模型表示之间收敛的水平和性质，使得能够对图像模型中的底层过程提供更定性的描述。研究揭示了即使最终表示相似，不同模型达到这些表示的具体路径和操作可能存在显著差异。
...
### [106] [A Tilted Seesaw: Revisiting Autoencoder Trade-off for Controllable Diffusion](https://arxiv.org/abs/2601.21633)
*Pu Cao,Yiyang Ma,Feng Zhou,Xuedan Yin,Qing Song,Lu Yang*

Main category: cs.CV

TL;DR: 研究发现当前潜在扩散模型中自编码器评估存在系统性偏斜，过度强调生成友好性指标（gFID）而忽视重建保真度，这种偏斜在可控生成任务中会导致条件漂移问题。


<details>
  <summary>Details</summary>
Motivation: 当前ImageNet规模的自编码器研究中存在系统性偏斜：重建指标被低估，基于消融的AE选择往往优先考虑最佳gFID配置，即使重建保真度下降。这种gFID主导的偏好在ImageNet生成中看似无害，但在扩展到可控扩散时存在风险。

Method: 1) 理论分析gFID主导偏好的原因及其在可控扩散中的风险；2) 提出多维条件漂移评估协议，反映可控生成任务需求；3) 实证研究多个近期ImageNet AE，评估其对可控性的影响；4) 通过ControlNet实验验证可控性与条件保持的关系。

Result: 研究发现：1) gFID仅弱预测条件保持能力；2) 重建导向指标与条件保持显著更相关；3) ControlNet实验证实可控性跟踪条件保持而非gFID；4) 实例级重建指标能更好指示可控性。

Conclusion: 当前以ImageNet为中心的AE评估与可扩展可控扩散的需求存在差距，需要更平衡的评估框架。研究为更可靠的基准测试和模型选择提供实用指导，强调在评估AE时需同时考虑重建保真度和生成友好性，特别是在可控生成任务中。
...
### [107] [RSGround-R1: Rethinking Remote Sensing Visual Grounding through Spatial Reasoning](https://arxiv.org/abs/2601.21634)
*Shiqi Huang,Shuting He,Bihan Wen*

Main category: cs.CV

TL;DR: RSGround-R1：用于遥感视觉定位的推理引导、位置感知后训练框架，通过思维链监督微调和强化微调增强空间理解能力


<details>
  <summary>Details</summary>
Motivation: 遥感视觉定位面临大规模航空影像和自然语言描述高度依赖位置线索的挑战，现有多模态大语言模型在空间推理方面存在不足，需要专门针对遥感场景特点的增强方法

Method: 提出两阶段后训练框架：1) 使用合成生成的RSVG推理数据进行思维链监督微调建立显式位置感知；2) 应用强化微调，结合新设计的位置奖励提供连续、距离感知的定位指导；3) 引入空间一致性引导优化方案，基于空间连贯性动态调整策略更新

Result: 在RSVG基准测试中展示了优越的性能和泛化能力

Conclusion: RSGround-R1通过推理引导的位置感知后训练有效提升了多模态大语言模型在遥感视觉定位中的空间理解能力，为处理大规模航空影像中的语义模糊和空间尺度问题提供了有效解决方案
...
### [108] [OCRVerse: Towards Holistic OCR in End-to-End Vision-Language Models](https://arxiv.org/abs/2601.21639)
*Yufeng Zhong,Lei Chen,Xuanle Zhao,Wenkang Han,Liming Zheng,Jing Huang,Deyang Jiang,Yilin Cao,Lin Ma,Zhixiong Zeng*

Main category: cs.CV

TL;DR: OCRVerse是首个端到端的统一文本中心OCR和视觉中心OCR方法，通过两阶段SFT-RL多域训练，在文本密集和视觉密集图像上均取得竞争性结果。
...
Method: 提出OCRVerse端到端统一方法：1）构建全面的数据工程，覆盖文本中心文档（报纸、杂志、书籍）和视觉中心渲染复合体（图表、网页、科学图表）；2）提出两阶段SFT-RL多域训练方法：SFT阶段直接混合跨域数据训练建立初始域知识，RL阶段为每个域设计个性化奖励策略，定制灵活的奖励信号以改善跨域融合并避免数据冲突。

Result: 实验结果表明OCRVerse的有效性，在文本中心和视觉中心数据类型上均取得竞争性结果，甚至可与大规模开源和闭源模型相媲美。

Conclusion: OCRVerse作为首个端到端的统一文本中心OCR和视觉中心OCR方法，通过创新的数据工程和两阶段训练策略，成功解决了现有OCR方法在处理视觉密集图像时的局限性，为多模态数据管理提供了更全面的解决方案。
...
### [109] [CAF-Mamba: Mamba-Based Cross-Modal Adaptive Attention Fusion for Multimodal Depression Detection](https://arxiv.org/abs/2601.21648)
*Bowen Zhou,Marc-André Fiedler,Ayoub Al-Hamadi*

Main category: cs.CV

TL;DR: CAF-Mamba：基于Mamba的跨模态自适应注意力融合框架，用于抑郁症检测，通过显式和隐式跨模态交互以及模态注意力机制实现动态多模态融合，在LMVD和D-Vlog数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测深度学习方法存在三个主要局限：1）依赖有限特征类型；2）忽视显式跨模态交互；3）使用简单拼接或静态加权融合。这些限制影响了多模态融合的有效性。

Method: 提出CAF-Mamba框架，基于Mamba架构实现跨模态自适应注意力融合。该框架同时捕获显式和隐式跨模态交互，并通过模态注意力机制动态调整各模态贡献度，实现更有效的多模态融合。

Result: 在两个真实世界基准数据集LMVD和D-Vlog上进行实验，CAF-Mamba一致优于现有方法，实现了最先进的性能表现。

Conclusion: CAF-Mamba通过显式和隐式跨模态交互结合以及动态模态注意力机制，有效解决了现有抑郁症检测方法的局限性，在多模态融合方面表现出优越性，为抑郁症检测提供了更有效的解决方案。
...
### [110] [Few-Shot Domain Adaptation with Temporal References and Static Priors for Glacier Calving Front Delineation](https://arxiv.org/abs/2601.21663)
*Marcel Dreier,Nora Gourmelon,Dakota Pyles,Thorsten Seehaus,Matthias H. Braun,Andreas Maier,Vincent Christlein*

Main category: cs.CV

TL;DR: 该论文提出了一种改进冰川崩解前缘分割的方法，通过少样本域适应、空间静态先验知识和夏季参考图像，将新研究站点的分割误差从1131.6米降低到68.7米，实现了全球尺度冰川监测的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的冰川崩解前缘分割模型在基准测试中表现接近人类水平，但在实际应用于新研究站点时，由于域分布差异导致分割精度不足，无法满足科学分析需求。

Method: 采用少样本域适应策略，结合空间静态先验知识，并在输入时间序列中包含夏季参考图像，无需修改模型架构即可显著提升新域的分割性能。

Result: 通过提出的方法改进，冰川崩解前缘的分割误差从1131.6米大幅降低到68.7米，实现了对新研究站点的有效分割。

Conclusion: 该方法建立了一个将深度学习冰川崩解前缘分割应用于新研究站点的框架，使全球尺度的冰川崩解前缘监测成为可能。
...
### [111] [When Gradient Optimization Is Not Enough: $\dagger$ Dispersive and Anchoring Geometric Regularizer for Multimodal Learning](https://arxiv.org/abs/2601.21670)
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习旨在整合异构模态的互补信息，但仅靠强优化不能保证良好结构的表示。即使经过精心平衡的训练方案，多模态模型仍常表现出几何病理，包括模态内表示坍缩和样本级跨模态不一致性，这会降低单模态鲁棒性和多模态融合效果。作者发现表示几何是多模态学习中缺失的控制维度。
...
Conclusion: 表示几何是多模态学习中的重要控制维度，通过几何感知正则化框架可以缓解多模态模型中的几何病理问题，改善模态内表示多样性和跨模态一致性，从而提升整体性能。
...
### [112] [ChartE$^{3}$: A Comprehensive Benchmark for End-to-End Chart Editing](https://arxiv.org/abs/2601.21694)
...
TL;DR: ChartE³是一个端到端图表编辑基准测试，直接评估模型而不依赖中间自然语言程序或代码级监督，包含1200+高质量样本，聚焦局部和全局编辑任务。
...
Method: 提出ChartE³基准测试，包含两个互补的编辑维度：局部编辑（细粒度外观变化如字体或颜色调整）和全局编辑（整体数据为中心转换如数据过滤和趋势线添加）。通过精心设计的数据流水线构建1200+高质量样本，每个样本包含图表图像、底层代码和多模态编辑指令三元组。

Result: 对最先进的多模态大语言模型进行广泛基准测试，揭示了显著的性能差距，特别是在全局编辑任务上，突显了当前端到端图表编辑能力的关键局限性。

Conclusion: ChartE³基准测试直接评估端到端图表编辑能力，不依赖中间表示，揭示了当前模型在复杂编辑任务上的局限性，为未来研究提供了重要评估框架。
...
### [113] [DreamActor-M2: Universal Character Image Animation via Spatiotemporal In-Context Learning](https://arxiv.org/abs/2601.21716)
...
<details>
  <summary>Details</summary>
Motivation: 现有角色图像动画方法存在两个根本挑战：1）次优的运动注入策略导致身份保持与运动一致性之间的"跷跷板"权衡；2）过度依赖显式姿态先验（如骨架），无法充分捕捉复杂动态，且难以泛化到任意非人形角色。

Method: 采用两阶段范式：1）通过将参考外观和运动线索融合到统一潜在空间，弥合输入模态差距，利用基础模型的生成先验联合推理空间身份和时间动态；2）引入自引导数据合成管道，策划伪跨身份训练对，实现从姿态依赖控制到直接端到端RGB驱动动画的无缝过渡。

Result: DreamActor-M2实现了最先进的性能，提供卓越的视觉保真度和强大的跨域泛化能力。研究还引入了AW Bench基准测试，涵盖广泛的角色类型和运动场景。

Conclusion: DreamActor-M2通过重新构想运动条件化为上下文学习问题，解决了现有角色动画方法的局限性，实现了更好的身份保持与运动一致性平衡，并显著增强了跨不同角色和运动场景的泛化能力。
...
### [114] [From Global to Granular: Revealing IQA Model Performance via Correlation Surface](https://arxiv.org/abs/2601.21738)
*Baoliang Chen,Danni Huang,Hanwei Zhu,Lingyu Zhu,Wei Zhou,Shiqi Wang,Yuming Fang,Weisi Lin*

Main category: cs.CV

TL;DR: 提出GMC方法，通过粒度调制器和分布调节器提供细粒度IQA性能分析，揭示传统全局相关度量无法捕捉的局部性能变化。


<details>
  <summary>Details</summary>
Motivation: 传统IQA评估主要依赖PLCC和SRCC等全局相关度量，这些指标将性能简化为单一标量，无法捕捉局部质量谱上的排序一致性变化，且对测试样本质量分布敏感，导致跨测试集比较不稳定。

Method: 提出粒度调制相关(GMC)方法，包括：1)粒度调制器，基于绝对MOS值和成对MOS差异应用高斯加权相关来检查局部性能变化；2)分布调节器，正则化相关以减少非均匀质量分布的偏差。最终生成相关曲面，将相关值映射为MOS和|ΔMOS|的联合函数。

Result: 在标准基准测试上的实验表明，GMC能够揭示标量度量无法看到的性能特征，提供更丰富可靠的IQA模型分析、比较和部署范式。

Conclusion: GMC为IQA性能评估提供了结构化、细粒度的分析框架，克服了传统全局相关度量的局限性，是更信息丰富且可靠的评估范式。
...
### [115] [Dynamic Topology Awareness: Breaking the Granularity Rigidity in Vision-Language Navigation](https://arxiv.org/abs/2601.21751)
*Jiankun Peng,Jianyuan Guo,Ying Xu,Yue Liu,Jiashuang Yan,Xuanwei Ye,Houhua Li,Xiaoming Wang*

Main category: cs.CV

TL;DR: DGNav提出动态拓扑导航框架，通过场景感知自适应策略和动态图变换器解决VLN-CE中的"粒度刚性"问题，实现按需密度调整和拓扑噪声过滤。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑规划方法存在"粒度刚性"问题：依赖固定几何阈值采样节点，无法适应环境复杂度变化。这导致简单区域过度采样（计算冗余）和高不确定性区域欠采样（碰撞风险增加、精度下降）。

Method: 提出DGNav框架，包含两个核心创新：1) 场景感知自适应策略：基于预测路径点分散度动态调整图构建阈值，在复杂环境中实现"按需密集化"；2) 动态图变换器：融合视觉、语言和几何线索到动态边权重中重建图连接性，过滤拓扑噪声并增强指令遵循能力。

Result: 在R2R-CE和RxR-CE基准测试上的广泛实验表明，DGNav展现出卓越的导航性能和强大的泛化能力。消融研究证实该框架在导航效率和安全探索之间实现了最优权衡。

Conclusion: DGNav通过动态调整地图密度和连接性，有效解决了VLN-CE中的粒度刚性问题，实现了更高效、更安全的连续环境视觉语言导航。
...
### [116] [MMFineReason: Closing the Multimodal Reasoning Gap via Open Data-Centric Methods](https://arxiv.org/abs/2601.21821)
...
Method: 采用三阶段流水线：1) 大规模数据收集与标准化；2) 链式思维原理生成；3) 基于推理质量和难度感知的综合筛选。基于该数据集微调Qwen3-VL-Instruct得到不同规模的模型。

Result: MMFineReason-4B超越Qwen3-VL-8B-Thinking，MMFineReason-8B超越Qwen3-VL-30B-A3B-Thinking并接近Qwen3-VL-32B-Thinking，展示了卓越的参数效率。发现"少即是多"现象：仅7%数据（12.3万样本）即可达到全数据集性能。

Conclusion: MMFineReason数据集有效提升了开源视觉语言模型的推理能力，揭示了高质量推理数据的重要性以及数据筛选策略的关键作用，同时发现推理导向的数据组合能协同提升通用能力。
...
### [117] [Trajectory-Guided Diffusion for Foreground-Preserving Background Generation in Multi-Layer Documents](https://arxiv.org/abs/2601.21857)
*Taewon Kang*
...
### [118] [Improving Classifier-Free Guidance of Flow Matching via Manifold Projection](https://arxiv.org/abs/2601.21892)
*Jian-Feng Cai,Haixia Liu,Zhengyi Su,Chao Wang*

Main category: cs.CV

TL;DR: 该论文为Classifier-free Guidance（CFG）提供了一种基于优化的理论解释，揭示了CFG是平滑距离函数梯度的近似，并提出了一种带流形约束的同伦优化方法，通过增量梯度下降和Anderson加速改进生成质量和对引导尺度的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CFG虽然广泛用于扩散和流模型的受控生成，但其基于启发式线性外推的方法对引导尺度敏感，缺乏理论依据。作者旨在从优化角度为CFG提供原理性解释，并改进其稳定性和生成质量。

Method: 1) 将流匹配中的速度场解释为平滑距离函数的梯度；2) 将标准CFG重新表述为带流形约束的同伦优化问题；3) 在采样过程中通过增量梯度下降实现流形投影；4) 引入Anderson加速提高计算效率和稳定性。

Result: 提出的方法无需额外训练，在DiT-XL-2-256、Flux和Stable Diffusion 3.5等大规模模型上验证有效，显著提升了生成保真度、提示对齐和对引导尺度的鲁棒性，在多个基准测试中表现优异。

Conclusion: 该工作为CFG提供了理论基础，揭示了预测差距是引导敏感性的关键因素，提出的优化框架不仅解释了现有CFG的局限性，还提供了更稳定、高效的改进方案，推动了可控生成技术的发展。
...
### [119] [Past- and Future-Informed KV Cache Policy with Salience Estimation in Autoregressive Video Diffusion](https://arxiv.org/abs/2601.21896)
*Hanmo Chen,Chenghao Xu,Xu Yang,Xuan Chen,Cheng Deng*

Main category: cs.CV

TL;DR: 提出PaFu-KV策略，通过轻量级显著性估计头动态管理KV缓存，在长视频生成中保留关键时空信息并减少冗余缓存，实现质量与效率的更好权衡。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频生成方法依赖启发式KV缓存策略，忽略了长视频生成中不同token的重要性差异，导致关键时空信息丢失和冗余缓存积累，影响生成质量和效率。

Method: 提出Past- and Future-Informed KV Cache Policy (PaFu-KV)，引入从双向教师蒸馏的轻量级显著性估计头来评估token重要性，动态保留信息丰富的token并丢弃不相关的token，减少KV缓存容量和内存占用。

Result: 在基准测试上的大量实验表明，该方法在保持高保真视频生成质量的同时实现了加速推理，支持更高效的长时程视频生成。

Conclusion: PaFu-KV策略通过智能KV缓存管理解决了长视频生成中的质量-效率权衡问题，为高效长时程视频生成提供了有效解决方案。
...
### [120] [TraceRouter: Robust Safety for Large Foundation Models via Path-Level Intervention](https://arxiv.org/abs/2601.21900)
*Chuancheng Shi,Shangze Li,Wenjun Lu,Wenhua Wu,Cong Wang,Zifeng Cheng,Fei Shen,Tat-Seng Chua*

Main category: cs.CV

TL;DR: TraceRouter：一种路径级防御框架，通过追踪和断开有害语义的因果传播电路来增强大型基础模型对抗对抗性攻击的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要基于"局部性假设"，通过抑制孤立的神经元或特征来对抗对抗性攻击。然而，有害语义表现为分布式、跨层的电路，使得这种局部干预变得脆弱且损害模型效用。需要一种能够处理有害语义分布式特性的新防御方法。

Method: TraceRouter采用三阶段路径级框架：1)通过注意力差异分析定位敏感起始层；2)使用稀疏自编码器和差分激活分析解耦并隔离恶意特征；3)通过基于归零干预的特征影响分数将恶意特征映射到下游因果路径。通过选择性抑制这些因果链，物理切断有害信息流，同时保持正交计算路径完整。

Result: 大量实验表明，TraceRouter显著优于现有最先进的基线方法，在对抗鲁棒性和通用效用之间实现了优越的权衡。

Conclusion: TraceRouter通过追踪和断开有害语义的因果传播电路，提供了一种有效的路径级防御框架，能够在不损害模型整体效用的前提下增强大型基础模型对抗对抗性攻击的能力。
...
### [121] [Beyond Global Alignment: Fine-Grained Motion-Language Retrieval via Pyramidal Shapley-Taylor Learning](https://arxiv.org/abs/2601.21904)
*Hanmo Chen,Guangtao Lyu,Chenghao Xu,Jiexi Yan,Xu Yang,Cheng Deng*

Main category: cs.CV

TL;DR: 提出金字塔形Shapley-Taylor学习框架，通过渐进式关节级和片段级对齐实现细粒度运动-语言检索
...
Method: 提出金字塔形Shapley-Taylor学习框架，将人体运动分解为时间片段和空间身体关节，通过渐进式关节级和片段级对齐以金字塔方式学习跨模态对应，捕捉局部语义细节和层次结构关系。

Result: 在多个公开基准数据集上的实验表明，该方法显著优于现有最先进方法，实现了运动片段、身体关节与对应文本标记之间的精确对齐。

Conclusion: 该框架通过细粒度对齐有效解决了运动-语言检索中的语义鸿沟问题，为人类中心跨模态智能提供了更精确的解决方案。
...
### [122] [VideoAesBench: Benchmarking the Video Aesthetics Perception Capabilities of Large Multimodal Models](https://arxiv.org/abs/2601.21915)
*Yunhao Li,Sijing Wu,Zhilin Gao,Zicheng Zhang,Qi Jia,Huiyu Duan,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: VideoAesBench：首个全面评估大模型视频美学质量理解能力的基准，包含1804个多样化视频、多种问题格式和全面的美学维度，测试23个模型发现现有模型仅具备基本感知能力


<details>
  <summary>Details</summary>
Motivation: 大模型在多种视觉感知任务中表现出色，但视频美学质量评估这一人类基本能力在大模型中尚未得到充分探索。现有研究缺乏系统性评估大模型视频美学理解能力的基准。
...
Result: 当前大模型仅具备基本的视频美学感知能力，表现不完整且不精确。不同模型在各类视频和问题格式上表现差异显著，整体性能有待提升。

Conclusion: VideoAesBench可作为强大的测试平台，为可解释的视频美学评估提供见解。当前大模型的视频美学理解能力有限，需要进一步研究提升。
...
### [123] [Zero-Shot Video Restoration and Enhancement with Assistance of Video Diffusion Models](https://arxiv.org/abs/2601.21922)
...
### [124] [Just Noticeable Difference Modeling for Deep Visual Features](https://arxiv.org/abs/2601.21933)
...
Method: 提出FeatJND任务对齐的JND公式，预测保持下游任务性能的最大可容忍每特征扰动图。开发FeatJND估计器，在标准化分割点进行验证，应用于图像分类、检测和实例分割任务
...
### [125] [BookNet: Book Image Rectification via Cross-Page Attention Network](https://arxiv.org/abs/2601.21938)
*Shaokai Liu,Hao Feng,Bozhi Luan,Min Hou,Jiajun Deng,Wengang Zhou*

Main category: cs.CV

TL;DR: BookNet：首个用于双页书籍图像校正的端到端深度学习框架，通过双分支架构和跨页注意力机制建模左右页面的耦合几何关系，并构建了专门的合成数据集和真实世界基准。
...
Method: 提出BookNet框架，采用双分支架构和跨页注意力机制，能够同时估计单个页面和完整书籍展开页的扭曲流，显式建模左右页面之间的相互影响。同时构建Book3D合成数据集用于训练，以及Book100真实世界基准用于评估。

Result: 大量实验表明，BookNet在书籍图像校正任务上优于现有的最先进方法。代码和数据集将公开提供。

Conclusion: BookNet是首个专门为双页书籍图像校正设计的端到端深度学习框架，通过建模页面间的耦合关系解决了传统方法的局限性，并提供了专门的数据集支持该领域的研究。
...
### [126] [PaddleOCR-VL-1.5: Towards a Multi-Task 0.9B VLM for Robust In-the-Wild Document Parsing](https://arxiv.org/abs/2601.21957)
...
TL;DR: PaddleOCR-VL-1.5是一个升级的视觉语言模型，在OmniDocBench v1.5上达到94.5%的SOTA准确率，并提出了Real5-OmniDocBench基准来评估对物理扭曲的鲁棒性，同时扩展了印章识别和文本检测功能，保持0.9B超紧凑规模。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有文档理解模型在真实世界物理扭曲（如扫描、倾斜、弯曲、屏幕拍摄和光照变化）下的鲁棒性问题，并扩展模型功能以支持印章识别和文本检测等实际应用需求。

Method: 1. 开发PaddleOCR-VL-1.5升级模型；2. 提出Real5-OmniDocBench基准，专门评估模型对五种真实世界物理扭曲的鲁棒性；3. 扩展模型能力，集成印章识别和文本检测任务；4. 保持模型为0.9B参数的超紧凑视觉语言模型架构。

Result: 1. 在OmniDocBench v1.5上达到94.5%的SOTA准确率；2. 在新提出的Real5-OmniDocBench基准上同样获得SOTA性能；3. 成功集成印章识别和文本检测功能；4. 模型保持0.9B参数规模，具有高效率。

Conclusion: PaddleOCR-VL-1.5在文档理解任务上实现了新的SOTA性能，并通过Real5-OmniDocBench基准为评估模型对真实世界物理扭曲的鲁棒性提供了标准化方法。模型在保持超紧凑规模的同时扩展了实用功能，为实际文档处理应用提供了高效解决方案。
...
### [127] [Causal World Modeling for Robot Control](https://arxiv.org/abs/2601.21998)
...
<details>
  <summary>Details</summary>
Motivation: 视频世界建模结合视觉语言预训练为机器人学习提供了新的独立基础。视频世界模型能够通过理解动作与视觉动态之间的因果关系来预测近期未来，这为机器人控制提供了重要能力。

Method: 提出LingBot-VA自回归扩散框架，包含三个关键设计：1) 共享潜在空间，通过混合变换器架构整合视觉和动作令牌；2) 闭环展开机制，持续获取环境反馈和真实观测；3) 异步推理管道，并行化动作预测和运动执行以实现高效控制。

Result: 在仿真基准测试和真实世界场景中评估，模型在长时程操作、后训练数据效率和面对新配置的强泛化能力方面表现出显著潜力。

Conclusion: 视频世界建模与视觉语言预训练相结合为机器人学习建立了新的独立基础，LingBot-VA框架通过创新的架构设计在多个方面展现出优越性能，代码和模型已公开以促进社区发展。
...
### [128] [Drive-JEPA: Video JEPA Meets Multimodal Trajectory Distillation for End-to-End Driving](https://arxiv.org/abs/2601.22032)
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶使用自监督视频预训练学习规划表示，但视频世界模型预训练带来的改进有限。驾驶场景固有的模糊性（每个场景通常只有单一人为轨迹）使得学习多模态行为困难

Method: 1) 将V-JEPA适配到端到端驾驶，在大规模驾驶视频上预训练ViT编码器，生成与轨迹规划对齐的预测表示；2) 引入以提议为中心的规划器，蒸馏模拟器生成的多条轨迹和人为轨迹，使用动量感知选择机制促进稳定安全行为

Result: 在NAVSIM评估中，V-JEPA表示结合简单transformer解码器在无感知设置下比先前方法提升3 PDMS。完整Drive-JEPA框架在v1上达到93.3 PDMS，v2上达到87.8 EPDMS，创下新的最先进水平

Conclusion: Drive-JEPA通过整合视频预测表示学习和多模态轨迹蒸馏，有效解决了驾驶场景的模糊性问题，显著提升了端到端自动驾驶的规划性能
...
### [129] [Urban Neural Surface Reconstruction from Constrained Sparse Aerial Imagery with 3D SAR Fusion](https://arxiv.org/abs/2601.22045)
...
<details>
  <summary>Details</summary>
Motivation: 现有神经表面重建方法在稀疏视角条件下存在几何模糊和不稳定性问题，而城市遥感中航空影像获取受飞行路径、地形和成本限制，需要更稳健的重建方案

Method: 融合3D SAR点云与航空影像，将雷达空间约束集成到基于SDF的神经表面重建框架中，指导结构感知射线选择和自适应采样

Result: 构建首个配准的3D SAR点云与航空影像基准数据集，实验表明融合3D SAR显著提高了重建精度、完整性和鲁棒性

Conclusion: 融合3D SAR为稀疏视角下高保真城市重建提供了可行路径，展示了先进机载和星载光学-SAR传感在可扩展城市重建中的潜力
...
### [130] [PLANING: A Loosely Coupled Triangle-Gaussian Framework for Streaming 3D Reconstruction](https://arxiv.org/abs/2601.22046)
*Changjian Jiang,Kerui Ren,Xudong Li,Kaiwen Song,Linning Xu,Tao Lu,Junting Dong,Yu Zhang,Bo Dai,Mulin Yu*

Main category: cs.CV

TL;DR: PLANING是一个高效的在线单目图像序列重建框架，通过混合表示松散耦合显式几何基元与神经高斯，实现几何与外观解耦建模，支持在线初始化与优化策略，在重建质量、速度和结构清晰度方面均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有单目图像序列流式重建方法通常只能在高质量渲染或准确几何之间二选一，难以同时兼顾两者。需要一种既能保持高质量渲染又能获得准确几何的在线重建框架。

Method: 提出PLANING框架，基于混合表示松散耦合显式几何基元与神经高斯，实现几何与外观解耦建模。采用在线初始化与优化策略，分离几何与外观更新，减少结构冗余，支持流式重建。

Result: PLANING在稠密网格Chamfer-L2上比PGSR提升18.52%，PSNR比ARTDECO高1.31dB，在ScanNetV2场景上重建时间低于100秒，比2D高斯溅射快5倍以上，同时达到离线逐场景优化的质量水平。

Conclusion: PLANING通过几何与外观解耦的混合表示和在线优化策略，实现了高质量、高精度、高效率的流式重建，其结构清晰度和计算效率使其适用于大规模场景建模和具身AI的仿真就绪环境等下游应用。
...
### [131] [MetricAnything: Scaling Metric Depth Pretraining with Noisy Heterogeneous Sources](https://arxiv.org/abs/2601.22054)
...
<details>
  <summary>Details</summary>
Motivation: 尽管缩放推动了视觉基础模型的进步，但将其扩展到度量深度估计仍面临挑战，包括异构传感器噪声、相机相关偏差以及噪声跨源3D数据中的度量模糊性。现有方法需要手动工程提示、相机特定建模或任务特定架构。

Method: 提出稀疏度量提示作为通用接口，通过对深度图进行随机掩码创建，将空间推理与传感器和相机偏差解耦。使用约2000万图像-深度对，涵盖重建、捕捉和渲染的3D数据，跨越10000个相机模型进行预训练。

Result: 首次在度量深度领域展示了清晰的缩放趋势。预训练模型在深度补全、超分辨率和雷达-相机融合等提示驱动任务中表现出色，其蒸馏的无提示学生模型在单目深度估计、相机内参恢复、单/多视角度量3D重建和VLA规划中达到最先进水平。作为视觉编码器使用时，显著提升了多模态大语言模型的空间智能能力。
...
### [132] [Unsupervised Decomposition and Recombination with Discriminator-Driven Diffusion Models](https://arxiv.org/abs/2601.22057)
...
<details>
  <summary>Details</summary>
Motivation: 将复杂数据分解为因子化表示可以揭示可重用组件，并通过组件重组合成新样本。现有扩散模型学习无监督因子化潜在空间，但需要改进因子发现和组合生成质量

Method: 引入对抗训练信号：训练判别器区分单源样本与跨源因子重组生成的样本，通过优化生成器欺骗判别器，鼓励重组结果的物理和语义一致性

Result: 在CelebA-HQ、Virtual KITTI、CLEVR和Falcor3D上优于先前基线，获得更低的FID分数和更好的解耦性能（MIG和MCC指标）。在机器人视频轨迹中，通过重组学习到的动作组件生成多样化序列，显著增加LIBERO基准上的状态空间覆盖

Conclusion: 对抗训练方法有效改进了扩散模型中无监督因子分解的质量和组合生成能力，在多个数据集上验证了其优越性，并展示了在机器人视频轨迹生成中的创新应用
...
### [133] [Vision-DeepResearch: Incentivizing DeepResearch Capability in Multimodal Large Language Models](https://arxiv.org/abs/2601.22060)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要大量事实信息的任务上存在局限性：1）通常假设单一的全景或实体级图像查询加上少量文本查询就足以检索关键证据，这在现实世界存在大量视觉噪声的场景中不现实；2）推理深度和搜索广度有限，难以解决需要聚合来自不同视觉和文本来源证据的复杂问题。

Method: 提出Vision-DeepResearch，采用新的多模态深度研究范式：1）执行多轮、多实体、多尺度的视觉和文本搜索，在重噪声下鲁棒地命中现实世界搜索引擎；2）支持数十个推理步骤和数百个引擎交互；3）通过冷启动监督和强化学习训练将深度研究能力内化到MLLM中，形成端到端的多模态深度研究MLLM。

Result: Vision-DeepResearch显著优于现有的多模态深度研究MLLMs，以及在强闭源基础模型（如GPT-5、Gemini-2.5-pro和Claude-4-Sonnet）上构建的工作流程。

Conclusion: Vision-DeepResearch通过创新的多模态深度研究范式，解决了现有方法在现实世界复杂场景中的局限性，为MLLMs在需要深度事实检索和推理的任务上提供了更强大的解决方案。
...
### [134] [BLO-Inst: Bi-Level Optimization Based Alignment of YOLO and SAM for Robust Instance Segmentation](https://arxiv.org/abs/2601.22061)
*Li Zhang,Pengtao Xie*

Main category: cs.CV

TL;DR: BLO-Inst：通过双层优化统一检测与分割目标，解决SAM自动化部署中检测器提示生成的目标不匹配和对齐过拟合问题


<details>
  <summary>Details</summary>
Motivation: SAM依赖手动提示阻碍全自动化部署，现有检测器作为提示生成器的方案存在目标不匹配（检测器优化几何定位而非SAM最优提示上下文）和对齐过拟合（检测器记忆特定训练样本的提示调整而非学习泛化策略）两大根本限制

Method: 提出BLO-Inst统一框架，通过双层优化对齐检测与分割目标。将对齐问题表述为在不相交数据分割上的嵌套优化问题：下层在子集D1上微调SAM以最大化给定检测提议的分割保真度；上层在子集D2上更新检测器，使其生成的边界框显式最小化微调后SAM的验证损失，从而将检测器转变为分割感知的提示生成器

Result: 大量实验表明BLO-Inst在通用和生物医学领域任务上均取得优越性能，超越标准基线方法

Conclusion: BLO-Inst通过双层优化有效解决了SAM自动化部署中的目标不匹配和对齐过拟合问题，将检测器转变为分割感知的提示生成器，优化边界框不仅用于定位精度，更关注下游掩码质量
...
### [135] [RefAny3D: 3D Asset-Referenced Diffusion Models for Image Generation](https://arxiv.org/abs/2601.22094)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于参考的图像生成方法仅限于单图像参考，无法利用3D资产，限制了实际应用多样性。需要探索如何将3D资产整合到图像扩散模型中。
...
Conclusion: 该方法成功将3D资产整合到图像扩散模型中，通过双分支架构实现了2D图像属性与3D资产属性的关联，提升了参考图像生成的多样性和实用性。
...
### [136] [SINA: A Circuit Schematic Image-to-Netlist Generator Using Artificial Intelligence](https://arxiv.org/abs/2601.22114)
*Saoud Aldowaish,Yashwanth Karumanchi,Kai-Chen Chiang,Soroosh Noorzad,Morteza Fayazi*

Main category: cs.CV

TL;DR: SINA是一个开源、全自动的电路原理图图像转网表生成器，通过深度学习、CCL、OCR和VLM技术实现高精度组件识别和连接性推断，性能比现有方法提升2.72倍。
...
Method: SINA集成了深度学习用于精确组件检测、连通域标记(CCL)用于连接性提取、OCR用于组件参考标识符检索，并采用视觉语言模型(VLM)进行可靠的参考标识符分配。

Result: SINA实现了96.47%的整体网表生成准确率，比最先进方法提高了2.72倍。

Conclusion: SINA是一个高效、准确的电路原理图图像转网表生成系统，通过多技术集成显著提升了自动化电路分析的能力。
...
### [137] [Creative Image Generation with Diffusion Model](https://arxiv.org/abs/2601.22125)
*Kunpeng Song,Ahmed Elgammal*

Main category: cs.CV

TL;DR: 提出基于扩散模型的创意图像生成框架，将创造力定义为CLIP嵌入空间中图像存在概率的倒数，通过驱动生成图像向低概率区域移动来产生罕见且富有想象力的视觉输出


<details>
  <summary>Details</summary>
Motivation: 现有创意图像生成方法通常依赖于手动概念混合或子类别排除，缺乏系统性的创造力定义和量化方法。需要一种能够产生新颖、高质量且富有想象力图像的原理性框架

Method: 提出基于扩散模型的创意生成框架：1) 将创造力定义为CLIP嵌入空间中图像存在概率的倒数；2) 计算生成图像的概率分布并驱动其向低概率区域移动；3) 引入回拉机制在保持高创造力的同时不牺牲视觉保真度

Result: 在文本到图像扩散模型上的大量实验证明了该框架的有效性和效率，能够产生独特、新颖且引人深思的图像，实现了高创造力与视觉保真度的平衡

Conclusion: 该工作为生成模型中的创造力提供了新视角，提供了一种原理性方法来促进视觉内容合成的创新，为创意图像生成开辟了新的研究方向
...
### [138] [EditYourself: Audio-Driven Generation and Manipulation of Talking Head Videos with Diffusion Transformers](https://arxiv.org/abs/2601.22127)
*John Flynn,Wolfgang Paier,Dimitar Dinev,Sam Nhut Nguyen,Hayk Poghosyan,Manuel Toribio,Sandipan Banerjee,Guy Gafni*

Main category: cs.CV

TL;DR: EditYourself：基于DiT的音频驱动视频编辑框架，支持通过修改文字稿来编辑说话人视频，实现内容的添加、删除和重定时，同时保持运动、时序一致性、说话人身份和准确的唇形同步。
...
Method: 基于通用视频扩散模型构建EditYourself框架，通过音频条件化和区域感知、编辑聚焦的训练扩展来增强其视频到视频（V2V）能力。采用时空修复技术实现精确唇形同步和时序一致的重构，包括在新添加片段中合成逼真的人体运动。

Result: EditYourself能够实现基于文字稿的说话人视频修改，包括内容的无缝添加、删除和重定时。系统能够保持视觉保真度和身份一致性，在长时间序列中维持精确的唇形同步和时序一致性，并能在新添加片段中合成逼真的人体运动。

Conclusion: 这项工作代表了生成式视频模型作为专业视频后期制作实用工具的基础性一步，为音频驱动的视频编辑提供了创新解决方案。
...
### [139] [Early and Prediagnostic Detection of Pancreatic Cancer from Computed Tomography](https://arxiv.org/abs/2601.22134)
...
TL;DR: 开发了名为ePAI的AI系统用于早期胰腺癌检测，在内部测试中AUC达0.939-0.999，外部测试中AUC达0.918-0.945，能检测直径小至2-5mm的病变，并在临床诊断前中位数347天发现被放射科医生漏诊的病变。
...
Method: 开发了名为ePAI（早期胰腺癌检测人工智能）的自动化系统，使用来自单一医疗中心的1,598名患者数据进行训练。系统在内部测试（1,009名患者）和外部测试（6个中心的7,158名患者）中进行评估，并与30名获得委员会认证的放射科医生进行多读者研究比较。

Result: 内部测试中，ePAI检测小于2cm的PDAC的AUC为0.939-0.999，敏感性95.3%，特异性98.7%，能精确定位小至2mm的病变。外部测试中AUC为0.918-0.945，敏感性91.5%，特异性88.0%，能定位小至5mm的病变。ePAI在临床诊断前3-36个月的CT扫描中检测到被放射科医生漏诊的PDAC，在159名患者中成功检测并定位了75例，中位提前时间为347天。多读者研究显示，ePAI在敏感性上显著优于30名放射科医生50.3%（P<0.05），同时保持95.4%的可比特异性。

Conclusion: ePAI作为辅助工具具有改善胰腺癌早期检测的潜力，能够在临床诊断前长时间检测到被放射科医生漏诊的小病变，显著提高检测敏感性。
...
### [140] [PI-Light: Physics-Inspired Diffusion for Full-Image Relighting](https://arxiv.org/abs/2601.22135)
*Zhexin Liang,Zhaoxi Chen,Yongwei Chen,Tianyi Wei,Tengfei Wang,Xingang Pan*

Main category: cs.CV

TL;DR: PI-Light是一个两阶段物理启发扩散框架，通过批量感知注意力、物理引导神经渲染和物理启发损失，解决了全图像重照明中的合成-真实差距问题，在真实场景中实现了更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 全图像重照明面临三大挑战：大规模结构化配对数据收集困难、物理合理性难以保持、以及数据驱动先验带来的有限泛化能力。现有方法在弥合合成到真实场景的差距方面仍不理想。

Method: 提出PI-Light两阶段框架：1) 批量感知注意力提升跨图像内在预测一致性；2) 物理引导神经渲染模块强制物理合理的光传输；3) 物理启发损失正则化训练动态；4) 精心策划的多样化对象和场景数据集。通过微调预训练扩散模型实现高效学习。

Result: PI-Light能够合成各种材质上的镜面高光和漫反射，在真实世界场景中相比先前方法实现了更优越的泛化性能，为下游评估提供了可靠的基准。

Conclusion: PI-Light通过物理启发的扩散模型设计，有效解决了全图像重照明中的关键挑战，在保持物理合理性的同时实现了对真实世界图像编辑的更好泛化能力。
...
### [141] [Do VLMs Perceive or Recall? Probing Visual Perception vs. Memory with Classic Visual Illusions](https://arxiv.org/abs/2601.22150)
...
TL;DR: VI-Probe框架通过可控视觉错觉实验揭示大视觉语言模型对视觉变化的感知机制，发现不同模型存在记忆覆盖、感知-记忆竞争、视觉处理限制等异质原因，而非单一机制。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在原始错觉图像上能"正确"回答，但在错觉因素反转后仍坚持相同回答，尽管视觉变化对人类显而易见。这引发核心问题：VLMs是感知视觉变化还是仅回忆记忆模式？现有研究仅观察到现象，但根本原因不明。

Method: 提出VI-Probe框架：1) 使用分级扰动和匹配视觉控制（无错觉诱导器）的可控视觉错觉框架；2) 分离视觉基础感知与语言驱动回忆；3) 引入极性翻转一致性、模板固定指数和相对于匹配控制的错觉乘数等指标，测量稳定性和敏感性而非平均准确率。

Result: 实验发现响应持续性源于异质原因而非单一机制：GPT-5表现记忆覆盖，Claude-Opus-4.1显示感知-记忆竞争，Qwen变体表明视觉处理限制。挑战了单一原因观点，证明需要基于探测的评估方法。

Conclusion: VLMs对视觉变化的感知机制复杂多样，需要同时测量知识和受控视觉变化敏感性的探测式评估。VI-Probe框架为系统理解模型感知能力提供了方法论基础，代码和数据已开源。
...
### [142] [UEval: A Benchmark for Unified Multimodal Generation](https://arxiv.org/abs/2601.22155)
...
Method: UEval包含1000个专家策划的问题，来自8个真实世界任务，覆盖多种推理类型。采用基于量规的评分系统：首先由MLLM根据参考图像和文本生成初始评分量规，然后由人类专家精炼和验证。最终包含10,417个验证过的评分标准，实现可扩展的细粒度自动评分。

Result: UEval对当前统一模型具有挑战性：GPT-5-Thinking仅得66.4分（满分100），最佳开源模型仅得49.1分。推理模型通常优于非推理模型，将推理轨迹从推理模型转移到非推理模型能显著缩小性能差距。

Conclusion: 推理能力对于需要复杂多模态理解和生成的任务可能很重要。UEval为评估统一模型提供了一个全面、可扩展的基准，揭示了当前模型的局限性，并表明推理能力是提升多模态生成性能的关键因素。
...
### [143] [What Hard Tokens Reveal: Exploiting Low-confidence Tokens for Membership Inference Attacks against Large Language Models](https://arxiv.org/abs/2601.20885)
*Md Tasnim Jawad,Mingyan Xiao,Yanzhao Wu*

Main category: cs.CR

TL;DR: HT-MIA：一种针对大语言模型的新型成员推理攻击方法，通过分析低置信度（困难）标记的标记级概率来检测训练数据中的成员样本，相比现有方法显著提高了攻击效果。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用和隐私法规的日益严格，保护LLM中的数据隐私变得至关重要。现有的成员推理攻击方法主要依赖序列级聚合预测统计，无法区分泛化改进和记忆改进，导致攻击效果不佳。

Method: 提出HT-MIA方法，专注于捕获低置信度（困难）标记的标记级概率。通过比较微调目标模型和预训练参考模型在困难标记处的标记级概率改进，分离出强烈且鲁棒的成员信号。

Result: 在领域特定的医疗数据集和通用基准测试上的广泛实验表明，HT-MIA在攻击效果上持续优于七种最先进的MIA基线方法。同时研究了差分隐私训练作为有效的防御机制。
...
### [144] [Towards Sensitivity-Aware Language Models](https://arxiv.org/abs/2601.20901)
*Dren Fazlija,Iyiola E. Olatunji,Daniel Kudenko,Sandipan Sikdar*

Main category: cs.CR

TL;DR: 本文形式化定义了敏感性意识概念，建立了其与差分隐私的理论联系，并提出了一种监督微调方法，使4位量化LLM在保持敏感信息不泄露的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在企业数据管理中的部署增加，确保模型不泄露敏感信息变得至关重要。虽然已引入敏感性意识概念使LLM能遵守预定义访问权限规则，但其与差分隐私等隐私概念的关联尚不明确，阻碍了实际应用部署。

Method: 1. 形式化定义敏感性意识概念；2. 理论建立敏感性意识与差分隐私的联系；3. 开发监督微调方法，使现有4位量化LLM更具敏感性意识。

Result: 微调后的LLM性能提升高达21.7%，不仅显著超越基线模型，还在敏感性意识方面优于类似规模的全精度开源和商业模型。同时，模型在通用指令遵循、数学推理和常识推理等其他任务上的性能也得到很好保留。

Conclusion: 本文通过形式化敏感性意识概念并建立其与差分隐私的理论联系，提出了一种有效的监督微调方法，使量化LLM在保持敏感信息不泄露的同时提升性能，为企业数据管理中的LLM部署提供了理论支持和实用解决方案。
...
### [145] [ICON: Intent-Context Coupling for Efficient Multi-Turn Jailbreak Attack](https://arxiv.org/abs/2601.20903)
*Xingwei Lin,Wenhao Lin,Sicong Cao,Jiahao Yu,Renke Huang,Lei Xue,Chunming Wu*

Main category: cs.CR

TL;DR: ICON是一种高效的多轮越狱攻击框架，通过意图-上下文耦合现象，利用先验引导的语义路由构建权威风格上下文，结合分层优化策略实现高成功率攻击。


<details>
  <summary>Details</summary>
Motivation: 现有多轮越狱攻击方法存在效率低下和易陷入局部最优的问题，需要更高效的自动化攻击框架来揭示LLM安全机制的脆弱性。

Method: 提出意图-上下文耦合现象，设计ICON框架：1) 通过先验引导的语义路由将恶意意图路由到语义一致的上下文模式；2) 实例化为攻击提示序列，逐步构建权威风格上下文；3) 采用分层优化策略，结合局部提示细化和全局上下文切换。

Result: 在8个SOTA LLM上的实验结果显示，ICON实现了97.1%的平均攻击成功率，达到最先进水平。

Conclusion: ICON通过利用意图-上下文耦合现象和分层优化策略，显著提高了多轮越狱攻击的效率和成功率，揭示了LLM安全机制在特定上下文模式下的脆弱性。
...
### [146] [Robust Federated Learning for Malicious Clients using Loss Trend Deviation Detection](https://arxiv.org/abs/2601.20915)
*Deepthy K Bhaskar,Minimol B,Binu V P*

Main category: cs.CR

TL;DR: 提出FL-LTD框架，通过监测损失动态而非梯度来检测和缓解恶意行为，在非IID数据下实现轻量级、隐私保护的联邦学习防御


<details>
  <summary>Details</summary>
Motivation: 联邦学习系统面临恶意参与者发送误导性更新的风险，现有防御机制依赖梯度检查、复杂相似性计算或密码学操作，在非IID数据分布下可能不稳定且引入额外开销

Method: 提出基于损失趋势检测的联邦学习防御框架(FL-LTD)，通过监测时间损失动态（异常损失停滞或突然波动）来识别异常客户端，结合短期记忆机制持续缓解先前标记为异常的客户端，同时允许稳定参与者恢复信任

Result: 在非IID联邦MNIST设置下的损失操纵攻击实验中，FL-LTD显著增强鲁棒性，最终测试准确率达到0.84，而标准FedAvg在攻击下仅为0.41；计算和通信开销可忽略，保持稳定收敛

Conclusion: FL-LTD通过基于损失的监控实现安全联邦学习，避免客户端排除或访问敏感数据，在非IID数据下提供轻量级、隐私保护的防御解决方案
...
### [147] [FIPS 204-Compatible Threshold ML-DSA via Masked Lagrange Reconstruction](https://arxiv.org/abs/2601.20917)
*Leo Kao*
...
<details>
  <summary>Details</summary>
Motivation: 现有并发方法存在局限：Bienstock等人的方案支持任意T但需要诚实多数和37-136轮通信；Celi等人的方案支持不诚实多数但限制T≤6。需要克服拉格朗日系数随q线性增长导致贡献过大而无法通过ML-DSA拒绝采样的问题。

Method: 提出掩码拉格朗日重构技术，解决ML-DSA阈值签名的三个关键挑战：1) 掩码后仍需通过‖z‖∞的拒绝采样；2) r0检查会暴露c s2导致密钥恢复风险；3) 产生的Irwin-Hall非均匀分布需保持EUF-CMA安全性。基于该技术实现三种部署配置：P1（TEE辅助）使用可信协调器实现3轮签名；P2（完全分布式）通过MPC在8轮内实现UC安全；P3（2PC辅助）使用轻量级2PC处理r0检查，在3-5轮内实现最佳性能。

Result: 方案需要|S|≥T+1个签名者，成功率为23-32%，与单签名者ML-DSA相当。P3配置达到最佳性能（249ms），所有配置都提供完整安全证明，支持任意阈值T，生成标准3.3KB签名。

Conclusion: 掩码拉格朗日重构技术成功解决了ML-DSA阈值签名的核心挑战，实现了任意阈值T的支持，同时保持与标准FIPS 204的兼容性。三种部署配置为不同安全假设和性能需求提供了灵活选择，填补了现有方案在任意阈值支持与不诚实多数容忍之间的空白。
...
### [148] [SPOILER-GUARD: Gating Latency Effects of Memory Accesses through Randomized Dependency Prediction](https://arxiv.org/abs/2601.21211)
...
<details>
  <summary>Details</summary>
Motivation: 现代微处理器依赖推测执行，但存在瞬态执行攻击漏洞。现有防御机制主要针对推测数据泄漏，却忽视了部分地址别名导致的虚假依赖问题。SPOILER攻击利用这种虚假依赖，通过重复的squash和reissue事件增加load-store延迟。因此需要一种新的硬件防御机制来解决这一问题。

Method: SPOILER-GUARD采用两种关键技术：1) 动态随机化用于load-store比较的物理地址位，混淆推测依赖解析；2) 标记存储条目以防止延迟放大的误预测。该方法在gem5模拟器中实现，使用SPEC 2017基准测试进行评估，并通过Synopsys Design Compiler在14nm工艺节点进行HDL综合验证硬件开销。

Result: SPOILER-GUARD将误预测率降至0.0004%，整数性能提升2.12%，浮点性能提升2.87%。硬件开销极小：关键路径延迟增加69ps，面积增加0.064平方毫米，功耗增加5.863mW。

Conclusion: SPOILER-GUARD有效防御了SPOILER攻击，通过硬件机制解决了部分地址别名导致的虚假依赖问题，在性能提升的同时保持了极低的硬件开销，为现代微处理器提供了实用的安全增强方案。
...
### [149] [Lossless Copyright Protection via Intrinsic Model Fingerprinting](https://arxiv.org/abs/2601.21252)
*Lingxiao Chen,Liqin Wang,Wei Lu,Xiangyang Luo*

Main category: cs.CR

TL;DR: TrajPrint是一种无损、无需训练的扩散模型版权保护框架，通过提取确定性生成过程中的流形指纹实现黑盒API下的版权验证


<details>
  <summary>Details</summary>
Motivation: 扩散模型作为高价值知识产权面临未经授权复制的风险，现有保护方法要么损害模型性能（水印嵌入），要么不兼容黑盒API（指纹提取）

Method: 1) 使用水印图像作为锚点，精确回溯到其轨迹原点，锁定映射该路径的模型指纹；2) 实施联合优化策略，采用双端锚定合成特定指纹噪声，严格遵循目标流形以实现鲁棒水印恢复；3) 通过原子推理和统计假设检验进行验证

Result: TrajPrint在黑盒API场景下实现了无损验证，对模型修改具有优越的鲁棒性，目标模型能恢复水印图像而非目标模型则失败

Conclusion: TrajPrint提供了一种完全无损且无需训练的扩散模型版权保护框架，通过提取确定性生成过程中的独特流形指纹，解决了现有方法在性能和黑盒兼容性方面的限制
...
### [150] [SecIC3: Customizing IC3 for Hardware Security Verification](https://arxiv.org/abs/2601.21353)
*Qinhan Tan,Akash Gaonkar,Yu-Wei Fan,Aarti Gupta,Sharad Malik*

Main category: cs.CR

TL;DR: SecIC3：一种基于IC3的硬件模型检查算法，专门针对自组合结构进行优化，用于验证硬件安全属性中的非干扰性


<details>
  <summary>Details</summary>
Motivation: 现有硬件安全验证方法中，虽然通过自组合将非干扰超属性转换为安全属性，但缺乏能够有效利用自组合特殊结构的先进模型检查器

Method: 基于IC3算法，提出SecIC3算法，采用两种互补技术：对称状态探索和等价谓词添加，专门针对自组合结构进行优化

Result: 在包含10个设计的非干扰检查基准测试中，SecIC3显著减少了安全证明时间，相比基线实现最高达到49.3倍的证明加速

Conclusion: SecIC3通过有效利用自组合结构，为硬件安全验证提供了高效的模型检查解决方案，显著提升了非干扰性验证的性能
...
### [151] [RerouteGuard: Understanding and Mitigating Adversarial Risks for LLM Routing](https://arxiv.org/abs/2601.21380)
*Wenhui Zhang,Huiyu Xu,Zhibo Wang,Zhichao Li,Zeqing He,Xuelin Wei,Kui Ren*

Main category: cs.CR

TL;DR: 论文系统研究了LLM重路由攻击的安全威胁，提出了威胁分类学，开发了RerouteGuard防御框架，在保持合法查询性能的同时实现了超过99%的攻击检测准确率。
...
Result: 研究发现现有路由系统容易受到重路由攻击，特别是在成本升级场景中。可解释性分析显示攻击通过前置查询的混淆器小工具利用路由器决策边界。RerouteGuard在三种攻击设置和四个基准测试中实现了超过99%的检测准确率，同时对合法查询影响可忽略。

Conclusion: LLM重路由攻击对多模型AI系统构成严重安全威胁，RerouteGuard提供了一个原则性和实用的解决方案，能够有效防御对抗性重路由攻击，保障系统安全。
...
### [152] [On the Adversarial Robustness of Large Vision-Language Models under Visual Token Compression](https://arxiv.org/abs/2601.21531)
*Xinwei Zhang,Hangcheng Liu,Li Bai,Hao Wang,Qingqing Ye,Tianwei Zhang,Haibo Hu*

Main category: cs.CR

TL;DR: 该论文提出CAGE攻击方法，针对视觉token压缩的大型视觉语言模型，解决了现有攻击方法因优化-推理不匹配而高估模型鲁棒性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于编码器的攻击方法会显著高估压缩LVLMs的对抗鲁棒性，因为存在优化-推理不匹配：扰动在完整token表示上优化，而推理通过token压缩瓶颈进行。这一差距导致对压缩模型安全性的评估过于乐观。

Method: 提出CAGE（压缩对齐攻击）方法，在不假设访问部署的压缩机制或其token预算的情况下，将扰动优化与压缩推理对齐。该方法结合：(1) 预期特征破坏，将失真集中在可能在不同预算下存活的token上；(2) 秩失真对齐，主动将token失真与秩分数对齐，以促进高度失真证据的保留。

Result: 在多种代表性的即插即用压缩机制和数据集上，CAGE始终比基线方法获得更低的鲁棒准确率，表明忽略压缩的鲁棒性评估可能过于乐观。

Conclusion: 该研究强调忽略压缩的鲁棒性评估可能过于乐观，呼吁对高效LVLMs进行压缩感知的安全评估和防御措施。CAGE攻击方法为评估压缩视觉语言模型的真实对抗鲁棒性提供了有效工具。
...
### [153] [ICL-EVADER: Zero-Query Black-Box Evasion Attacks on In-Context Learning and Their Defenses](https://arxiv.org/abs/2601.21586)
*Ningyuan He,Ronghong Huang,Qianqian Tang,Hongyu Wang,Xianghang Mi,Shanqing Guo*

Main category: cs.CR

TL;DR: ICL-Evader：一种针对大语言模型上下文学习的零查询黑盒逃避攻击框架，包含三种新型攻击方法，能显著降低分类器性能，并提出了有效的防御策略。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习已成为文本分类的强大且数据高效的方法，但其在现实对抗威胁下的鲁棒性尚未得到充分探索。现有研究缺乏在零查询威胁模型下对ICL系统安全性的全面评估。

Method: 提出了ICL-Evader框架，包含三种新型攻击方法：Fake Claim（虚假声明）、Template（模板）和Needle-in-a-Haystack（大海捞针），这些方法利用LLM处理上下文提示的固有局限性。攻击在零查询威胁模型下运行，无需访问模型参数、梯度或基于查询的反馈。同时系统研究了防御策略，并开发了自动化工具来加固标准ICL提示。

Result: 在情感分析、毒性和非法推广任务上的评估显示，攻击显著降低了分类器性能（攻击成功率高达95.3%），远超传统NLP攻击。提出的联合防御配方能有效缓解所有攻击，且效用损失最小（准确率下降<5%）。

Conclusion: 这项工作对ICL进行了全面的安全评估，揭示了关键漏洞，并为构建更鲁棒的系统提供了实用解决方案。研究强调了ICL系统在现实对抗威胁下的脆弱性，并提供了有效的防御机制。
...
### [154] [Noise as a Probe: Membership Inference Attacks on Diffusion Models Leveraging Initial Noise](https://arxiv.org/abs/2601.21628)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面取得了显著进展，但其日益广泛的应用引发了严重的隐私担忧。特别是微调模型非常脆弱，因为它们通常在小型私有数据集上进行微调。现有针对扩散模型的成员推理攻击要么假设能够获取中间结果，要么需要辅助数据集来训练影子模型，存在局限性。

Method: 作者利用了一个关键但被忽视的漏洞：广泛使用的噪声调度无法完全消除图像中的语义信息，即使在最大噪声步长下也存在残留的语义信号。基于这一洞察，提出了一种简单而有效的成员推理攻击方法：向初始噪声注入语义信息，然后通过分析模型的生成结果来推断成员身份。

Result: 大量实验表明，语义初始噪声能够强烈地揭示成员信息，突显了扩散模型对成员推理攻击的脆弱性。该方法在无需获取中间结果或辅助数据集的情况下，有效实现了对微调扩散模型的成员推理攻击。

Conclusion: 该研究揭示了扩散模型中存在的重要隐私漏洞，即噪声调度无法完全消除语义信息，导致残留的语义信号可能被用于成员推理攻击。这一发现强调了在部署扩散模型时需要更加关注隐私保护措施，并为未来的隐私保护方法提供了重要启示。
...
### [155] [Authenticated encryption for space telemetry](https://arxiv.org/abs/2601.21657)
*Andrew Savchenko*

Main category: cs.CR

TL;DR: 该论文提出了一种轻量级认证加密方案，用于满足NASA-STD-1006A标准中关于应急空间遥测的命令栈保护要求，在资源受限环境中实现安全与性能的平衡。


<details>
  <summary>Details</summary>
Motivation: 满足NASA-STD-1006A标准对应急空间遥测命令栈保护的要求，在资源受限的航天环境中实现安全通信，同时保持与现有数据传输协议的兼容性。

Method: 提出轻量级认证加密实现方案，生成固定长度消息以确保协议兼容性，专注于可预测属性和鲁棒认证机制，平衡安全需求与操作约束。

Result: 该方案在不牺牲性能的前提下提供强大的安全性，保护应急通信中遥测数据的机密性、完整性和真实性，同时保持与底层数据传输协议的兼容性。

Conclusion: 轻量级认证加密方案能够有效满足NASA-STD-1006A标准对应急空间遥测命令栈的保护要求，在资源受限环境中实现安全与性能的平衡，为航天应急通信提供可行的安全解决方案。
...
### [156] [WADBERT: Dual-channel Web Attack Detection Based on BERT Models](https://arxiv.org/abs/2601.21893)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在处理不规则HTTP请求时效果有限，难以建模无序参数和实现攻击溯源。需要一种既能实现高检测精度，又能精确识别恶意参数的Web攻击检测模型。

Method: 提出WADBERT模型：1）使用混合粒度嵌入（HGE）为URL和payload参数生成细粒度嵌入；2）分别使用URLBERT和SecBERT提取语义特征；3）通过多头注意力机制融合参数级特征，形成综合payload特征；4）将拼接的URL和payload特征输入线性分类器获得最终检测结果。

Result: 在CSIC2010和SR-BH2020数据集上分别达到99.63%和99.50%的F1分数，显著优于现有最先进方法。

Conclusion: WADBERT模型在实现高精度Web攻击检测的同时，能够精确识别恶意参数，解决了现有方法在处理不规则HTTP请求和攻击溯源方面的不足。
...
### [157] [Beyond the Finite Variant Property: Extending Symbolic Diffie-Hellman Group Models (Extended Version)](https://arxiv.org/abs/2601.21910)
*Sofia Giampietro,Ralf Sasse,David Basin*

Main category: cs.CR

TL;DR: 论文提出了一种支持完整Diffie-Hellman群运算的半决策程序，扩展了Tamarin证明器以支持群元素乘法和指数加法，首次实现了对使用所有DH运算的协议进行建模和验证。


<details>
  <summary>Details</summary>
Motivation: 现有符号协议验证器虽然支持Diffie-Hellman群，但缺乏对指数加法等完整数学运算的支持，因为这些工具基于合一推理，而完整DH算子的理论是不可判定的。需要一种方法来验证使用所有DH群运算的协议安全性。

Method: 近似完整DH理论，提出半决策程序来确定使用所有DH群运算的协议是否满足用户定义属性。通过扩展Tamarin证明器实现该方法，支持完整的DH理论，包括群元素乘法和指数加法。

Result: 成功实现了首个支持完整DH理论的状态验证工具，能够对使用所有DH运算的协议进行建模和推理。通过案例研究证明了方法的有效性：成功验证了ElGamal加密的安全性，并重新发现了MQV协议中的已知攻击。

Conclusion: 该方法首次使状态验证工具能够支持完整的Diffie-Hellman理论，为使用所有DH群运算的协议提供了有效的安全验证手段，填补了现有工具的功能空白。
...
### [158] [Secure Group Key Agreement on Cyber-Physical System Buses](https://arxiv.org/abs/2601.21966)
*Sebastian N. Peters,Lukas Lautenschlager,David Emeis,Jason Lochert*

Main category: cs.CR

TL;DR: 本文针对工业CPS总线系统设计了一种基于TreeKEM的认证式完全分布式群组密钥协商协议，解决了现有协议在广播链路、半双工操作、资源限制等约束下的适应性不足问题。


<details>
  <summary>Details</summary>
Motivation: CPS系统依赖分布式嵌入式设备通过总线进行安全通信，需要群组共享密钥实现消息认证。为避免不安全的固定预共享密钥和首次使用信任机制，需要动态的群组密钥协商协议。然而现有GKA协议缺乏对受限CPS总线环境的适应性。

Method: 首先系统化分析现有协议，然后推导总线系统认证式完全分布式GKA的需求，最后基于TreeKEM设计、实现并评估定制化的GKA协议。

Result: 开发了适用于总线拓扑的认证式完全分布式GKA协议，能够处理广播链路、半双工操作、资源限制、动态成员变更（包括未通知离开）、长设备寿命等约束，并能抵抗能够分区总线的强Dolev-Yao敌手攻击。

Conclusion: 本文提出的基于TreeKEM的GKA协议满足了工业CPS总线系统的安全需求，为受限环境下的安全群组通信提供了可行的解决方案。
...
### [159] [Latent-IMH: Efficient Bayesian Inference for Inverse Problems with Approximate Operators](https://arxiv.org/abs/2601.20888)
*Youguang Chen,George Biros*

Main category: stat.ML

TL;DR: Latent-IMH是一种基于Metropolis-Hastings独立性采样器的贝叶斯线性反问题后验分布采样方法，利用计算成本低的近似算子Ã预处理，再通过精确算子A精炼，显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 在贝叶斯线性反问题中，参数到观测值的算子A计算成本高昂。许多应用中A可以分解，便于构建成本效益高的近似算子Ã。需要开发能利用这种近似结构的高效采样方法。

Method: 提出Latent-IMH采样方法：基于Metropolis-Hastings独立性采样器，首先使用近似算子Ã生成中间潜在变量，然后使用精确算子A进行精炼。该方法将主要计算成本转移到离线阶段。

Result: 理论分析使用KL散度和混合时间界限评估性能。数值实验表明，在合理假设下，Latent-IMH在计算效率上优于NUTS等最先进方法，某些情况下比现有方案快几个数量级。

Conclusion: Latent-IMH通过利用算子分解和近似结构，将计算负担转移到离线阶段，显著提高了贝叶斯线性反问题后验采样的计算效率，在某些情况下比现有方法快几个数量级。
...
### [160] [Efficient Causal Structure Learning via Modular Subgraph Integration](https://arxiv.org/abs/2601.21014)
*Haixiang Sun,Pengchao Tian,Zihan Zhou,Jielei Zhang,Peiyi Li,Andrew L. Liu*

Main category: stat.ML

TL;DR: VISTA是一个模块化因果结构学习框架，通过基于马尔可夫毯的局部子图分解和加权投票集成机制，解决高维场景下搜索空间超指数增长和计算需求大的问题。
...
Method: VISTA将全局因果结构学习问题分解为基于马尔可夫毯的局部子图，通过加权投票机制集成：使用指数衰减惩罚低支持度边，自适应阈值过滤不可靠边，并使用反馈弧集算法确保无环性。

Result: VISTA在合成和真实数据集上的广泛实验一致证明了其有效性，在准确性和效率方面相比多种基础学习器都有显著提升。理论分析建立了有限样本误差界，并在温和条件下证明了渐近一致性。

Conclusion: VISTA是一个模型无关的模块化框架，对基础学习器的归纳偏置没有假设，兼容任意数据设置而不需要特定结构形式，完全支持并行化，为高维因果结构学习提供了有效解决方案。
...
### [161] [A Diffusive Classification Loss for Learning Energy-based Generative Models](https://arxiv.org/abs/2601.21025)
...
<details>
  <summary>Details</summary>
Motivation: 基于分数的生成模型取得了显著成功，但能量基模型（EBMs）的训练仍然具有挑战性。最大似然估计计算成本过高，而分数匹配虽然高效但存在模式盲区问题。需要一种既能避免模式盲区又保持计算效率的EBM训练方法。

Method: 提出了Diffusive Classification（DiffCLF）目标函数，将EBM学习重新构建为跨噪声级别的监督分类问题。该方法可以与标准的基于分数的目标函数无缝结合，通过分类任务来学习能量函数。

Result: 在分析性高斯混合案例中，DiffCLF估计的能量与真实值相符。在模型组合和Boltzmann Generator采样等任务中，DiffCLF训练的EBM表现出比现有方法更高的保真度和更广泛的适用性。

Conclusion: DiffCLF为能量基模型训练提供了一种有效的方法，既避免了模式盲区问题，又保持了计算效率，使EBM在生成、组合采样和Boltzmann Generator等任务中具有更好的性能。
...
### [162] [Diffusion-based Annealed Boltzmann Generators : benefits, pitfalls and hopes](https://arxiv.org/abs/2601.21026)
*Louis Grenioux,Maxence Noble*

Main category: stat.ML

TL;DR: 该研究分析了基于退火蒙特卡洛的玻尔兹曼生成器，特别关注扩散模型作为生成骨干的性能。研究发现即使使用完美学习的扩散模型，标准一阶随机去噪核集成也会系统性失败，而二阶去噪核在可获得协方差信息时表现更好。作者还提出了基于一阶传输映射的确定性aMC集成方法。


<details>
  <summary>Details</summary>
Motivation: 当前玻尔兹曼生成器大多使用经典蒙特卡洛机制（如重要性采样），这些方法既需要骨干模型具有可处理的似然函数，又在高维多模态目标上扩展性差。研究旨在探索基于退火蒙特卡洛的玻尔兹曼生成器，特别是使用扩散模型作为生成骨干，以克服这些限制。

Method: 研究对基于扩散模型的aMC-BGs进行了实证元分析，使用受控多模态高斯混合模型（变化模式分离、模式数量和维度）。通过比较完美学习的扩散模型和从数据训练的扩散模型，明确分离了推理效应和学习效应。评估了不同集成方法：标准一阶随机去噪核、二阶去噪核，并提出了基于一阶传输映射的确定性aMC集成。

Result: 即使使用完美扩散模型，标准一阶随机去噪核集成也会系统性失败。当可获得协方差信息时，二阶去噪核能显著改善性能。提出的基于一阶传输映射的确定性aMC集成在更高计算成本下优于随机一阶变体。在学习的扩散模型设置中，所有DM-aMC变体都难以产生准确的玻尔兹曼生成器，主要瓶颈在于不准确的扩散模型对数密度估计。

Conclusion: 基于扩散模型的退火蒙特卡洛玻尔兹曼生成器在理论上具有潜力，但实际应用中面临挑战。二阶去噪核和确定性集成方法能改善性能，但扩散模型对数密度估计的准确性是关键瓶颈。需要改进扩散模型的密度估计能力，才能使DM-aMC-BGs在实际应用中有效。
...
### [163] [Provably Reliable Classifier Guidance through Cross-entropy Error Control](https://arxiv.org/abs/2601.21200)
*Sharan Sahu,Arisina Banerjee,Yuchen Wu*

Main category: stat.ML

TL;DR: 论文建立了分类器训练误差与扩散模型引导向量误差之间的定量联系，证明了在平滑性假设下，分类器的条件KL散度误差控制着引导向量的均方误差。


<details>
  <summary>Details</summary>
Motivation: 尽管分类器引导扩散模型通过增强反向时间分数来生成条件样本，但标准分类器训练程序是否能产生有效的扩散引导仍不清楚。本文旨在填补这一理论空白，建立分类器训练与引导对齐之间的定量联系。

Method: 在温和的分类器平滑性假设下，证明控制每个扩散步骤的交叉熵误差也控制着所得引导向量的误差。具体地，展示了实现条件KL散度ε²的分类器会诱导出均方误差为Õ(dε)的引导向量。

Result: 建立了分类器训练误差与引导向量误差之间的定量关系：分类器的条件KL散度ε²对应引导向量的均方误差Õ(dε)。该结果产生了分类器引导下采样误差的上界，类似于反向log-Sobolev型不等式。同时证明了平滑性假设的必要性，通过构造反例显示若无此假设，引导向量控制可能对几乎所有分布都失效。

Conclusion: 本文首次建立了分类器训练与引导对齐之间的定量联系，为分类器引导提供了理论基础，并为分类器选择提供了原则性指导。结果表明，在适当的平滑性条件下，良好训练的分类器确实能产生有效的扩散引导。
...
### [164] [A Flexible Empirical Bayes Approach to Generalized Linear Models, with Applications to Sparse Logistic Regression](https://arxiv.org/abs/2601.21217)
...
<details>
  <summary>Details</summary>
Motivation: 传统变分推断方法需要为不同似然和先验组合开发特定方法，且通常假设高斯变分分布，缺乏灵活性。现有方法需要手动调参，且优化参数数量多，计算效率低。

Method: 采用新颖的均值场变分推断方法，直接优化后验均值和先验参数，而非后验密度函数。在VI算法中估计先验参数，实现免调参。使用L-BFGS和随机梯度下降等可扩展算法。提供适用于指数族分布的通用框架。

Result: 方法在稀疏逻辑回归问题上表现出优越的预测性能，通过广泛的数值研究比较，优于现有的稀疏逻辑回归方法。框架具有通用性，无需为不同似然-先验组合开发特定VI方法。

Conclusion: 提出了一种灵活、免调参的经验贝叶斯变分推断框架，可直接优化后验均值和先验参数，减少优化参数数量，提高计算效率，适用于广泛的指数族分布，在稀疏逻辑回归中表现出优越性能。
...
### [165] [Bulk-Calibrated Credal Ambiguity Sets: Fast, Tractable Decision Making under Out-of-Sample Contamination](https://arxiv.org/abs/2601.21324)
*Mengqi Chen,Thomas B. Berrett,Theodoros Damoulas,Michele Caprio*

Main category: stat.ML

TL;DR: 论文提出了一种新的分布鲁棒优化方法，通过引入bulk-calibrated credal模糊集来处理重尾分布和分布偏移问题，避免了传统方法中可能出现的无限风险问题。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化在处理Huber污染模型时，当包含任意扰动时可能导致最坏情况风险无限大，使目标函数失去意义，除非施加强有界性或支撑集假设。需要一种既能处理分布偏移又能避免无限风险的方法。

Method: 提出bulk-calibrated credal模糊集：从数据中学习高概率质量的主体区域，考虑主体区域内的污染，同时单独限制尾部贡献。这导出了封闭形式的有限mean+sup鲁棒目标，并为常见损失函数和主体几何形状提供了可处理的线性或二阶锥规划。

Result: 该方法在重尾库存控制、地理偏移房价回归和人口统计偏移文本分类等实验中展示了具有竞争力的鲁棒性-准确性权衡和高效的优化时间，能够使用贝叶斯、频率主义或经验参考分布。

Conclusion: 通过bulk-calibrated credal模糊集框架，成功解决了传统DRO在处理污染模型时的无限风险问题，建立了不精确概率上期望与最坏情况风险之间的等价关系，实现了具有可解释容忍水平的鲁棒优化。
...
### [166] [Statsformer: Validated Ensemble Learning with LLM-Derived Semantic Priors](https://arxiv.org/abs/2601.21410)
*Erica Zhang,Naomi Sagan,Danny Tse,Fangzhao Zhang,Mert Pilanci,Jose Blanchet*

Main category: stat.ML

TL;DR: Statsformer：通过集成架构将LLM知识整合到统计学习中的框架，具有防护机制和理论保证


<details>
  <summary>Details</summary>
Motivation: 现有方法在整合LLM知识到统计学习中存在局限性：要么将LLM指导作为未经验证的启发式方法注入（易受幻觉影响），要么将语义信息嵌入到单个固定学习器中。需要一种既能利用LLM知识又能防止幻觉影响的灵活框架。

Method: 采用防护集成架构，将LLM衍生的特征先验嵌入到线性和非线性学习器的集成中，通过交叉验证自适应校准其影响。框架具有理论保证，性能不会比其库中任何基学习器的凸组合差（在统计误差范围内）。

Result: 实证研究表明，信息丰富的先验能带来一致的性能提升，而无信息或错误指定的LLM指导会被自动降权，从而减轻幻觉在各种预测任务中的影响。

Conclusion: Statsformer提供了一个原则性框架，能够安全有效地将LLM知识整合到监督统计学习中，既利用了LLM的语义理解能力，又通过统计方法防护了幻觉风险。
...
### [167] [Questioning the Coverage-Length Metric in Conformal Prediction: When Shorter Intervals Are Not Better](https://arxiv.org/abs/2601.21455)
*Yizhou Min,Yizhou Lu,Lanqi Li,Zhen Zhang,Jiaye Teng*

Main category: stat.ML

TL;DR: 本文批判性审视了保形预测的标准评估指标，揭示了一种名为"偏见技巧"的方法可以在保持覆盖率有效的同时欺骗性地改善区间长度，但引入了区间不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统上，保形预测通过覆盖率和区间长度进行评估。本文质疑这些标准指标的充分性，指出可能存在欺骗性优化区间长度的方法，同时保持覆盖率有效，这暴露了当前评估体系的漏洞。

Method: 提出并分析了"偏见技巧"方法：对于任何给定的测试样本，该方法概率性地返回一个区间，要么为空集，要么使用调整后的置信水平构建。该方法保持边际覆盖率，但可能产生欺骗性的区间长度改善。同时引入了新的评估指标"区间稳定性"来检测类似PT的技巧。

Result: 形式化推导了PT实现误导性改进的条件，并在各种回归和分类任务上提供了广泛的实证证据。证明PT可以在保持覆盖率的同时欺骗性地降低区间长度，但代价是引入了区间不稳定性——同一输入在不同算法运行中可能产生完全不同的预测区间。

Conclusion: 仅依赖覆盖率和区间长度评估保形预测方法是不充分的，可能掩盖欺骗性优化。区间稳定性是一个重要的补充指标，有助于检测类似PT的技巧。需要更全面的评估框架来确保保形预测方法的实际可靠性。
...
### [168] [A Judge-Aware Ranking Framework for Evaluating Large Language Models without Ground Truth](https://arxiv.org/abs/2601.21817)
*Mingyuan Xu,Xinzi Tan,Jiawei Wu,Doudou Zhou*

Main category: stat.ML

TL;DR: 提出一个考虑评委可靠性的LLM评估框架，通过引入评委特异性参数改进传统Bradley-Terry-Luce模型，实现无参考标签下的模型质量与评委可靠性联合估计


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-judge评估范式存在关键问题：不同评委LLM的可靠性差异显著，但现有方法将所有评委同等对待，导致排行榜偏差和不确定性估计误导。更多数据在错误聚合下反而会增强错误置信度

Method: 提出评委感知排名框架，扩展Bradley-Terry-Luce模型，引入评委特异性判别参数，通过成对比较无监督联合估计潜在模型质量和评委可靠性。建立可识别性理论，证明最大似然估计的一致性和渐近正态性

Result: 在多个公共基准和新收集数据集上，该方法相比未加权基线：1）提高与人类偏好的一致性；2）实现更高的数据效率；3）为LLM排名生成校准的不确定性量化，支持分数差异和排名比较的置信区间

Conclusion: 评委感知排名框架解决了LLM评估中的评委可靠性异质性问题，通过统计建模提供更准确、高效且具有校准不确定性的评估方案，为无监督LLM评估提供了理论保证和实践工具
...
### [169] [Generative Modeling of Discrete Data Using Geometric Latent Subspaces](https://arxiv.org/abs/2601.21831)
...
Result: 经验结果表明，减少的潜在维度足以表示用于生成建模的数据

Conclusion: 在分类分布乘积流形的指数参数空间中使用潜在子空间是学习离散数据生成模型的有效工具
...
### [170] [On Forgetting and Stability of Score-based Generative models](https://arxiv.org/abs/2601.21868)
...
<details>
  <summary>Details</summary>
Motivation: 理解生成模型的稳定性和长期行为是现代机器学习的基本问题。当前需要量化分析基于分数的生成模型的采样误差，并阐明随机动力学在此类模型中的作用。

Method: 利用与反向时间动力学相关的马尔可夫链的稳定性和遗忘性质，在弱假设下提出了两个确保反向过程初始化和离散化误差传播的结构性质：Lyapunov漂移条件和Doeblin型次要化条件。

Result: 获得了采样过程的定量稳定性，证明反向扩散动力学沿着采样轨迹诱导了收缩机制。结果为分析基于分数的方法中的误差传播提供了原则性框架。

Conclusion: 该研究阐明了随机动力学在基于分数的模型中的作用，为分析此类方法中的误差传播提供了理论框架，证明了反向扩散过程中的收缩机制确保了采样过程的稳定性。
...
### [171] [Clustering in Deep Stochastic Transformers](https://arxiv.org/abs/2601.21942)
*Lev Fedorov,Michaël E. Sander,Romuald Elie,Pierre Marion,Mathieu Laurière*

Main category: stat.ML

TL;DR: 该论文证明，在考虑随机初始化噪声的情况下，深度Transformer的token动态不会坍缩到单点，而是收敛到球面上的交互粒子系统，存在由交互强度和token维度控制的相变。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer理论通常假设确定性权重，预测token会坍缩到单点，但这无法捕捉标准随机初始化方案的实际行为。需要研究随机初始化噪声如何影响token动态。
...
Result: 初始化噪声阻止了确定性模型预测的单点坍缩。对于两个token，证明了由交互强度和token维度控制的相变：与确定性注意力流不同，反极配置以正概率成为吸引子。数值实验证实了预测的相变，显示反极构型在多于两个token时持续存在，且抑制固有噪声会降低准确性。

Conclusion: 随机初始化噪声是Transformer动态的关键因素，它改变了token的长期行为，阻止了坍缩并引入了新的吸引子构型，这对理解Transformer的理论性质和实际性能有重要意义。
...
### [172] [Diffusion Path Samplers via Sequential Monte Carlo](https://arxiv.org/abs/2601.21951)
*James Matthew Young,Paula Cordero-Encinar,Sebastian Reich,Andrew Duncan,O. Deniz Akyildiz*

Main category: stat.ML

TL;DR: 提出一种基于扩散的采样器，用于从归一化常数未知的目标分布中采样，结合扩散退火Langevin Monte Carlo和序列蒙特卡洛进行分数估计


<details>
  <summary>Details</summary>
Motivation: 针对已知归一化常数的目标分布采样问题，需要开发高效且理论保证的采样方法。扩散模型中的扩散路径提供了从简单基分布到目标分布的平滑插值，但实际应用需要解决分数估计和方差控制等挑战

Method: 1) 实现扩散退火Langevin Monte Carlo来近似扩散路径；2) 开发序列蒙特卡洛采样器，沿路径演化辅助变量以估计时变分布的分数；3) 设计新的控制变量调度来最小化分数估计的方差

Result: 提供了理论保证，并在多个合成和真实数据集上实证证明了方法的有效性

Conclusion: 该方法成功解决了目标分布归一化常数未知时的采样问题，通过结合扩散路径、序列蒙特卡洛和方差控制技术，实现了高效且理论可靠的采样
...
### [173] [Near-Optimal Private Tests for Simple and MLR Hypotheses](https://arxiv.org/abs/2601.21959)
*Yu-Wei Chen,Raghu Pasupathy,Jordan Awan*

Main category: stat.ML

TL;DR: 开发了一种基于高斯差分隐私框架的近最优检验方法，适用于单调似然比条件下的单边和双边检验，其统计效率与非私有最优检验相当。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私框架下进行统计检验时，需要在保护数据隐私的同时保持检验的统计效率。现有方法在平衡隐私保护与检验功效方面存在不足，特别是在小样本和中等隐私预算条件下。

Method: 基于具有数据驱动截断界限的私有均值估计器构建私有检验统计量。该估计器的总体风险在私有极小极大率范围内（最多相差对数因子），利用该估计器构造的检验统计量能保持保守的第一类错误控制。

Result: 私有检验统计量实现了与非私有最优检验相同的渐近相对效率，数值实验表明该方法优于其他差分隐私方法，在中等样本量和隐私预算下与非私有最优检验具有可比功效。

Conclusion: 提出的基于高斯差分隐私的检验方法在保护数据隐私的同时，能够达到与非私有最优检验相当的统计效率，为隐私保护下的统计推断提供了有效的解决方案。
...
### [174] [Towards regularized learning from functional data with covariate shift](https://arxiv.org/abs/2601.21019)
...
<details>
  <summary>Details</summary>
Motivation: 协变量偏移（训练和测试数据的输入分布不同）给可靠学习带来重大挑战，特别是在向量值回归任务中。现有方法在处理函数输出和参数选择方面存在不足，需要开发理论上有保障且实用的域适应框架。

Method: 1. 在向量值再生核希尔伯特空间(vRKHS)中构建正则化框架，通过限制假设空间来处理协变量偏移；2. 开发实用的算子学习算法处理函数输出；3. 提出聚合方法，通过线性组合不同正则化参数和不同核对应的估计器来自动选择调优参数。

Result: 1. 在一般源条件下建立了最优收敛率，为正则化学习提供了理论基础；2. 聚合方法有效解决了调优参数选择问题，理论证明了其有效性；3. 在真实人脸图像数据集上验证了方法的鲁棒性和有效性，能够有效缓解协变量偏移下的分布差异。

Conclusion: 该研究提出了一个理论上有保障的vRKHS域适应框架，能够有效处理协变量偏移下的向量值回归问题。聚合方法解决了参数选择的关键挑战，在实际应用中表现出良好的鲁棒性和有效性，为无监督域适应提供了实用的解决方案。
...
### [175] [Thompson sampling: Precise arm-pull dynamics and adaptive inference](https://arxiv.org/abs/2601.21131)
*Qiyang Han*

Main category: math.ST

TL;DR: Thompson采样算法的臂选择动态呈现二分现象：次优臂和唯一最优臂的拉动次数渐近确定，而多个最优臂则收敛到SDE的不变分布，揭示了稳定性与统计噪声交互渐近可忽略性的统一原理。


<details>
  <summary>Details</summary>
Motivation: 研究Thompson采样类型算法中臂选择动态的精确特性，以理解自适应采样方案中的统计推断问题。现有工作表明UCB类型算法具有稳定性特性，但Thompson采样算法的动态特性尚不明确，需要揭示其背后的统一原理。

Method: 采用两种新方法：对于次优臂，开发"逆过程"方法，通过Stieltjes积分刻画臂拉动计数过程的逆；对于最优臂，采用臂拉动和噪声过程的重参数化，将自然SDE中的奇异性简化为证明另一个SDE不变律的唯一性，使用抛物型Hörmander条件和Stroock-Varadhan支撑定理等解析工具。

Result: 发现Thompson采样算法的臂拉动动态呈现二分现象：次优臂或唯一最优臂的拉动次数渐近确定，而多个最优臂则收敛到随机微分方程的不变分布。归一化臂均值服从相同二分性，稳定臂有高斯极限，不稳定臂有半通用非高斯极限。

Conclusion: 揭示了稳定性与统计噪声交互渐近可忽略性的统一原理，为构建置信区间提供了理论基础，并展示了在非稳定区域开发可处理推断程序的潜力。Thompson采样算法的动态特性与UCB算法有本质区别，需要不同的推断方法。
...
### [176] [Identifiability in Graphical Discrete Lyapunov Models](https://arxiv.org/abs/2601.21818)
*Cecilie Olesen Recke,Sarah Lumpp,Nataliia Kushnerchuk,Janike Oldekop,Jiayi Li,Jane Ivy Coons,Elina Robeva*

Main category: math.ST

TL;DR: 该论文研究离散Lyapunov模型，关注在非高斯误差项下利用高阶累积量识别参数矩阵，证明了有向无环图（带自环）的通用可识别性，并建立了所有带自环且无孤立顶点有向图的局部可识别性。


<details>
  <summary>Details</summary>
Motivation: 传统Lyapunov模型参数识别通常假设高斯误差项，这限制了可识别性。本文动机在于探索非高斯误差项下利用高阶累积量进行参数识别的可能性，特别是在包含环的有向图结构中。

Method: 采用离散Lyapunov模型（一阶向量自回归模型的稳态分布），参数矩阵编码有向图结构。通过假设非高斯误差项，利用模型的高阶累积量进行参数识别。使用代数统计方法，将参数表示为累积量的有理函数。

Result: 1. 证明了带自环的有向无环图（DAGs）的通用可识别性；2. 建立了所有带自环且无孤立顶点有向图的局部可识别性；3. 提供了模型定义方程的初步结果，展示了某些图的模型等价性。

Conclusion: 非高斯误差项为Lyapunov模型的参数识别提供了新途径，特别是通过高阶累积量。研究结果为结构学习奠定了基础，展示了在包含环的有向图结构中参数识别的可行性。
...
### [177] [Extremal conditional independence for Hüsler-Reiss distributions via modular functions](https://arxiv.org/abs/2601.21931)
*Karel Devriendt,Ignacio Echave-Sustaeta Rodríguez,Frank Röttger*

Main category: math.ST

TL;DR: 本文研究了Hüsler-Reiss分布的极值条件独立性，引入了两个集合函数来刻画极值条件独立关系，并探讨了Hüsler-Reiss参数有界子集与高斯椭圆体的几何关系。
...
Method: 引入两个集合函数：1）利用Hüsler-Reiss与高斯模型的紧密联系，提出受多信息启发的度量m^HR；2）考虑与Hüsler-Reiss参数化自然相关的不变量σ^2。通过这两个函数的模性关系来刻画极值条件独立陈述。

Result: 建立了极值条件独立陈述的两个模性准则：第一个基于m^HR函数，第二个基于σ^2不变量（在附加正性约束下）。同时研究了Hüsler-Reiss参数有界子集的几何性质及其与高斯椭圆体的关系。

Conclusion: 为描述高维极值统计中的极值依赖结构提供了新工具，通过集合函数的模性关系系统地表征了Hüsler-Reiss分布的极值条件独立性，并建立了与高斯模型的几何联系。
...
TL;DR: 提出采样自由的隐私放大边界，基于Rényi散度和条件组合，用于差分隐私矩阵分解训练中的随机分配场景，相比现有方法提供确定性保证且不依赖δ逆比例采样。
...
Method: 提出两种采样自由边界：1）基于Rényi散度的边界，通过动态规划公式高效计算；2）基于条件组合的边界，在小ε情况下提供更强隐私保证。框架适用于任意带状和非带状矩阵。

Result: 通过数值比较证明，该方法在研究和实践中使用的广泛矩阵机制范围内都有效，相比采样方法提供更可靠的隐私保证且计算效率更高。

Conclusion: 提出的采样自由隐私放大框架解决了现有蒙特卡洛方法的局限性，为差分隐私矩阵分解训练中的随机分配提供了更可靠、高效的分析工具，适用于各种矩阵机制。
...
### [179] [Hardware-Triggered Backdoors](https://arxiv.org/abs/2601.21902)
*Jonas Möller,Erik Imgrund,Thorsten Eisenhofer,Konrad Rieck*

Main category: cs.LG

TL;DR: 该论文揭示了机器学习模型在不同硬件上运行时产生的微小数值变化可被利用创建硬件触发的后门攻击


<details>
  <summary>Details</summary>
Motivation: 机器学习模型广泛部署在各种计算硬件上，虽然硬件预期应产生相同结果，但设计差异会导致推理过程中的微小数值变化。研究者发现这些变化可被恶意利用，构成新的安全威胁。

Method: 通过将模型的决策边界局部移动到目标输入附近，然后优化数值偏差，使模型在不同硬件上对相同输入产生不同预测。具体方法包括：1）定位决策边界附近的目标输入；2）利用硬件间的数值差异调整模型参数；3）在特定硬件上触发预测翻转。

Result: 实验证明，这种硬件触发的后门可以在常见的GPU加速器上可靠创建。攻击成功率高，揭示了第三方模型使用中的新攻击向量。

Conclusion: 硬件差异导致的数值变化构成了机器学习安全的新威胁向量，需要开发相应的防御机制来应对这种硬件触发的后门攻击。
...
### [180] [A Theory of Universal Agnostic Learning](https://arxiv.org/abs/2601.20961)
*Steve Hanneke,Shay Moran*

Main category: cs.LG

TL;DR: 该论文建立了不可知设置下二元分类最优通用率的完整理论，将Bousquet等人的可实现情况理论扩展到非可实现分布，识别出最优收敛率的四分类：e^{-n}、e^{-o(n)}、o(n^{-1/2})或任意慢速


<details>
  <summary>Details</summary>
Motivation: 扩展Bousquet等人(2021)的可实现情况理论，移除分布的可实现性假设，为不可知设置下的二元分类建立完整的理论框架，填补理论空白

Method: 理论分析方法，识别概念类的最优收敛率四分类，通过组合结构确定每个概念类所属的类别，建立不可知设置下的完整理论框架

Result: 证明了每个概念类的最优通用收敛率必然属于四种类型之一：指数衰减(e^{-n})、次指数衰减(e^{-o(n)})、次平方根衰减(o(n^{-1/2}))或任意慢速，并确定了决定分类的简单组合结构

Conclusion: 该研究为不可知设置下的二元分类提供了完整的理论框架，识别了最优收敛率的四分类，并建立了概念类组合结构与收敛率类型之间的对应关系，扩展了可实现情况的理论成果
...
### [181] [Supervised Guidance Training for Infinite-Dimensional Diffusion Models](https://arxiv.org/abs/2601.20756)
*Elizabeth L. Baker,Alexander Denker,Jes Frellsen*

Main category: cs.LG

TL;DR: 该论文提出了一种在无限维函数空间中调节扩散模型的方法，用于贝叶斯反问题中的后验采样，通过Doob's h-transform理论扩展和新的监督引导训练目标实现高效稳定的后验采样。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已扩展到无限维函数空间用于偏微分方程反问题，但在贝叶斯反问题中，如何调节扩散模型以从后验分布采样仍缺乏理论支持。现有方法在函数空间后验采样方面存在理论空白。

Method: 假设先验位于Cameron-Martin空间或相对于高斯测度绝对连续，证明可使用无限维Doob's h-transform调节扩散模型。提出监督引导训练（Supervised Guidance Training）这一免模拟的分数匹配目标，通过分解条件分数为无条件分数和引导项来高效稳定地进行后验采样。

Result: 理论证明了扩散模型可通过无限维Doob's h-transform进行调节，条件分数可分解为无条件分数和引导项。监督引导训练目标使后验采样高效稳定，数值实验验证了该方法在函数空间贝叶斯反问题中的有效性。

Conclusion: 该工作首次提供了函数空间方法，能够微调训练好的扩散模型以精确地从后验分布采样，为函数空间贝叶斯反问题提供了理论框架和实用算法。
...
### [182] [The Powers of Precision: Structure-Informed Detection in Complex Systems -- From Customer Churn to Seizure Onset](https://arxiv.org/abs/2601.21170)
...
Method: 提出一种机器学习方法，从单参数估计器族（经验协方差矩阵或精度矩阵的幂）中学习最优特征表示，然后通过监督学习模块对学习到的表示进行分类

Result: 在癫痫发作检测和客户流失预测任务中取得有竞争力的结果，证明了方法的经验有效性；同时证明了最优协方差幂具有良好的可识别性并捕获结构特征

Conclusion: 该方法不仅提供预测能力，还通过最优协方差幂的良好可识别性和结构特征捕获，实现了预测性能与可解释统计结构的统一，为涌现现象的早期检测提供了新途径
...
### [183] [Missing-Data-Induced Phase Transitions in Spectral PLS for Multimodal Learning](https://arxiv.org/abs/2601.21294)
*Anders Gjølbye,Ida Kargaard,Emma Kargaard,Lars Kai Hansen*

Main category: cs.LG

TL;DR: 该论文研究了在数据缺失情况下偏最小二乘奇异值分解(PLS-SVD)的性能，发现了类似BBP相变的临界现象：当信噪比低于阈值时，主奇异向量无信息量；高于阈值时，它们与潜在共享方向对齐。


<details>
  <summary>Details</summary>
Motivation: 多模态数据集通常存在两个视图中的条目缺失问题，而传统的PLS-SVD方法假设数据完整。需要理解在缺失数据情况下PLS-SVD的性能表现，特别是当数据缺失是独立且完全随机时。

Method: 研究在比例高维尖峰模型下，独立条目完全随机缺失掩码的PLS-SVD。通过适当归一化，掩码交叉协方差表现为尖峰矩形随机矩阵，其有效信号强度衰减因子为√ρ（ρ为联合条目保留概率）。

Result: PLS-SVD表现出尖锐的BBP型相变：低于临界信噪比阈值时，主奇异向量渐近无信息量；高于阈值时，它们与潜在共享方向实现非平凡对齐，并给出闭式渐近重叠公式。模拟和半合成多模态实验验证了预测的相图和恢复曲线。

Conclusion: 在缺失数据情况下，PLS-SVD的性能存在明确的相变阈值，该阈值取决于信号强度、缺失水平和数据维度比例。这为实际应用中处理不完整多模态数据提供了理论指导。
...
TL;DR: 本文为平均奖励MDP中的同步和异步Q学习建立了最优样本复杂度保证，通过引入懒惰化动态采样解决了缺乏收缩性的挑战。


<details>
  <summary>Details</summary>
Motivation: 平均奖励MDP中的Q学习面临缺乏收缩性的根本挑战。现有方法要么需要强假设来强制半范数收缩，要么依赖折扣或片段MDP作为连续近似，这些方法要么需要未知参数，要么导致次优样本复杂度。

Method: 提出一种简单的同步和异步Q学习变体，从懒惰化动态中采样，系统以固定概率保持在当前状态。核心分析是构建实例依赖的半范数，并证明在MDP经过懒惰化变换后，Bellman算子在该半范数下变为一步收缩。

Result: 在可达性假设下，建立了最优的$\widetilde{O}(\varepsilon^{-2})$样本复杂度保证（忽略对数因子）。

Conclusion: 通过懒惰化变换和实例依赖的半范数分析，成功克服了平均奖励MDP中Q学习缺乏收缩性的挑战，获得了最优样本复杂度。
...
### [185] [Rethinking LLM-Driven Heuristic Design: Generating Efficient and Specialized Solvers via Dynamics-Aware Optimization](https://arxiv.org/abs/2601.20868)
*Rongzheng Wang,Yihong Huang,Muquan Li,Jiakai Li,Di Liang,Bob Simons,Pei Ke,Shuang Liang,Ke Qin*

Main category: cs.LG

TL;DR: DASH框架通过收敛感知度量和档案库检索，解决了LLM驱动启发式设计中端点评估和适应成本高的问题，在组合优化问题上实现了3倍以上的运行效率提升和90%以上的适应成本降低。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的启发式设计框架存在两个关键限制：1)仅基于最终质量的端点评估，忽略了收敛过程和运行效率；2)分布漂移导致的高适应成本，需要为新的实例组重新生成专门的求解器。

Method: 提出DASH框架，包含两个核心组件：1)基于收敛感知度量的求解器搜索机制和运行时调度协同优化；2)Profiled Library Retrieval(PLR)，在进化过程中并行归档专门的求解器，实现成本有效的热启动。

Result: 在四个组合优化问题上的实验表明：DASH将运行效率提高了3倍以上，同时在多样化问题规模上超越了最先进基线的求解质量。通过基于配置的热启动，在不同分布下保持优越的准确性，同时将LLM适应成本降低了90%以上。

Conclusion: DASH通过收敛感知度量和档案库检索机制，有效解决了现有LLM驱动启发式设计框架的局限性，实现了高效、高性能的求解器生成，显著降低了适应成本，为组合优化提供了更实用的自动化求解器设计框架。
...
### [186] [Partial Feedback Online Learning](https://arxiv.org/abs/2601.21462)
*Shihao Shao,Cong Fang,Zhouchen Lin,Dacheng Tao*

Main category: cs.LG

TL;DR: 该论文研究了部分反馈在线学习，其中每个实例存在一组正确标签，但学习者每轮只观察到一个正确标签；任何在正确集合内的预测都被视为正确。该模型捕捉了语言生成等场景，其中多个响应可能有效但数据仅提供单一参考。


<details>
  <summary>Details</summary>
Motivation: 研究部分反馈在线学习模型，该模型适用于语言生成等场景，其中多个响应可能有效但数据仅提供单一参考。需要理解在这种部分反馈设置下的学习能力和最小最大遗憾。

Method: 引入部分反馈Littlestone维度(PFLdim)来刻画确定性学习者的学习能力，开发部分反馈测量粉碎维度(PMSdim)来获得随机学习者的紧界。使用新的集合版本空间视角和辅助维度，识别确保确定性和随机学习能力不可分离的广泛条件。

Result: PFLdim精确地控制确定性学习者的学习能力和最小最大遗憾；PMSdim为随机学习者提供紧界。确定了确保确定性和随机学习能力不可分离的广泛条件，并解决了Raman等人[2024b]的开放问题。展示了与较弱现实和不可知变体的尖锐分离。

Conclusion: 在集合可实现性范围内，部分反馈在线学习可以得到完全刻画，但超出此范围，问题可能变得信息理论上不可处理，需要新的噪声敏感复杂性度量来有意义地表征超出集合可实现性的学习能力。
...
### [187] [Finetune-Informed Pretraining Boosts Downstream Performance](https://arxiv.org/abs/2601.20884)
*Atik Faysal,Mohammad Rostami,Reihaneh Gh. Roshan,Nikhil Muralidhar,Huaxia Wang*

Main category: cs.LG

TL;DR: 提出Finetune-Informed Pretraining (FIP)方法，通过调整预训练策略使表示学习偏向于下游微调时实际使用的目标模态，提升单模态下游任务性能


<details>
  <summary>Details</summary>
Motivation: 多模态预训练通常对所有模态进行统一处理，但在实际部署中下游微调往往只使用单一模态，这导致对实际重要模态的表示学习不足
...
Conclusion: FIP是一种简单易实现、架构兼容且广泛适用于多模态掩码建模流程的方法，能有效优化实际部署中重要模态的表示学习
...
### [188] [PPI-SVRG: Unifying Prediction-Powered Inference and Variance Reduction for Semi-Supervised Optimization](https://arxiv.org/abs/2601.21470)
*Ruicheng Ao,Hongyu Chen,Haoyang Liu,David Simchi-Levi,Will Wei Sun*

Main category: cs.LG

TL;DR: PPI-SVRG：一种结合预测模型和SVRG的半监督随机优化方法，在标签稀缺时通过预测信息降低方差，收敛界分解为标准SVRG速率加预测不确定性误差项。


<details>
  <summary>Details</summary>
Motivation: 解决标签数据稀缺但预训练模型预测可用的半监督优化问题。现有方法PPI使用预测作为控制变量，SVRG使用参考梯度作为控制变量，两者在数学上等价，但各有优势。

Method: 提出PPI-SVRG方法，结合PPI和SVRG的优点。PPI使用预测作为控制变量减少方差，SVRG使用参考梯度减少方差。证明两者数学等价性，并开发统一框架。收敛分析显示误差分解为标准SVRG速率和预测不确定性误差项。

Result: 理论证明：收敛界仅依赖损失函数几何性质，预测只影响邻域大小。完美预测时完全恢复SVRG；预测质量下降时仍保持稳定收敛但达到更大邻域。实验验证：在均值估计基准上减少MSE 43-52%；MNIST上仅用10%标签数据提升测试准确率2.7-2.9个百分点。

Conclusion: PPI-SVRG有效结合预测信息和梯度方差减少技术，在半监督设置下显著提升优化性能。方法在预测质量变化时保持鲁棒性，为标签稀缺场景提供了实用解决方案。
...
### [189] [A generative machine learning model for designing metal hydrides applied to hydrogen storage](https://arxiv.org/abs/2601.20892)
...
### [190] [Is Parameter Isolation Better for Prompt-Based Continual Learning?](https://arxiv.org/abs/2601.20894)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的持续学习方法通常为每个任务分配固定提示集，导致任务间知识完全隔离和参数利用不足。需要更灵活高效的提示共享机制来改善参数利用和知识迁移。

Method: 提出提示共享框架：1）构建全局提示池；2）引入任务感知门控路由机制，稀疏激活提示子集实现任务特定特征表示的动态解耦与协作优化；3）设计历史感知调制器，利用累积提示激活统计保护常用提示免受过度更新。

Result: 广泛分析和实证结果表明，该方法在效果和效率上持续优于现有静态分配策略，有效缓解了参数利用不足和知识遗忘问题。

Conclusion: 提示共享框架通过动态提示激活和协作优化，实现了更高效的参数利用和知识保留，为持续学习提供了更灵活有效的解决方案。
...
TL;DR: 论文提出ReD（Reset-and-Discard）方法，通过重置和丢弃策略提高LLM在固定预算下的问题覆盖率，减少所需尝试次数、token和成本


<details>
  <summary>Details</summary>
Motivation: 现有pass@k指标在固定预算下存在局限性，覆盖度随成本增长呈现次线性（收益递减），需要更有效的查询策略来提高问题覆盖率

Method: 提出Reset-and-Discard（ReD）方法：当LLM正确回答问题时重置其状态，错误回答时丢弃该尝试；建立了pass@k与coverage@cost的数学关系，可预测节省的尝试次数

Result: 在三个LLM上的HumanEval实验表明，ReD显著减少了达到目标覆盖率所需的尝试次数、token和美元成本，同时提供了一种高效测量推理幂律的方法

Conclusion: ReD方法有效解决了pass@k指标在固定预算下的收益递减问题，提高了LLM在可验证任务上的效率，为模型评估和成本优化提供了实用工具
...
### [192] [TwinWeaver: An LLM-Based Foundation Model Framework for Pan-Cancer Digital Twins](https://arxiv.org/abs/2601.20906)
...
TL;DR: TwinWeaver框架将患者纵向历史序列化为文本，使用大语言模型进行统一事件预测和预后，在癌症患者数据上显著提升了预测精度和风险分层能力。


<details>
  <summary>Details</summary>
Motivation: 精准肿瘤学需要预测临床事件和轨迹，但稀疏、多模态临床时间序列建模仍然是一个关键挑战。现有方法难以有效处理这种复杂数据。

Method: 提出TwinWeaver开源框架，将纵向患者历史序列化为文本，使用大语言模型进行统一事件预测和预后。在此基础上构建了Genie Digital Twin (GDT)，基于93,054名患者、20种癌症类型的数据。

Result: GDT显著降低预测误差：中位平均绝对缩放误差(MASE)为0.87，优于最强时间序列基线的0.97(p<0.001)。风险分层方面，在生存、进展和治疗切换任务上的平均一致性指数(C-index)为0.703，优于最佳基线的0.662。在分布外临床试验数据上，零样本匹配训练基线，微调后超越基线，中位MASE为0.75-0.88，事件预测平均C-index为0.672 vs 0.648。

Conclusion: TwinWeaver框架为纵向临床建模提供了可扩展和透明的基础，支持可解释的临床推理扩展，显著提升了精准肿瘤学中的预测能力。
...
### [193] [LoRA and Privacy: When Random Projections Help (and When They Don't)](https://arxiv.org/abs/2601.21719)
*Yaxi Hu,Johanna Düngler,Bernhard Schölkopf,Amartya Sanyal*

Main category: cs.LG

TL;DR: 提出Wishart投影机制，研究其差分隐私特性：对向量查询无需加噪即可实现DP，但对矩阵查询无噪声时不满足DP，需加噪并结合低秩投影实现隐私增强，LoRA更新是该机制实例。


<details>
  <summary>Details</summary>
Motivation: 探索随机投影在差分隐私中的应用，特别是研究Wishart分布作为随机性来源能否替代传统加噪机制，并分析低秩投影对隐私的增强效果，为LoRA等低秩微调方法提供隐私理论支持。

Method: 提出Wishart投影机制S↦Mf(S)，其中M∼W_d(1/r I_d, r)。分析向量和矩阵查询的DP性质：向量查询证明非渐近DP保证无需加噪；矩阵查询展示无噪声时的脆弱性并设计成员推理攻击；分析加噪变体在大小秩机制下的隐私放大效应。

Result: 向量查询：Wishart随机性足以实现DP无需加噪；矩阵查询：无噪声机制不满足DP（AUC>0.99的成员推理攻击）；加噪机制：随机性和低秩投影产生隐私放大，低秩微调比全秩微调在相同噪声水平下更隐私；LoRA是矩阵机制实例，不具固有隐私性。

Conclusion: Wishart投影机制为差分隐私提供新视角，向量查询可免加噪，矩阵查询需结合噪声和低秩投影实现隐私增强。LoRA等低秩微调方法在相同噪声下比全参数微调更隐私，更紧的隐私核算可降低噪声提升实用精度。
...
### [194] [Noisy but Valid: Robust Statistical Evaluation of LLMs with Imperfect Judges](https://arxiv.org/abs/2601.20913)
*Chen Feng,Minghe Shen,Ananth Balashankar,Carsten Gerner-Beuerle,Miguel R. D. Rodrigues*

Main category: cs.LG

TL;DR: 提出"嘈杂但有效"假设检验框架，通过小规模人工标注校准集估计LLM评判者的真阳性/假阳性率，在大规模评判者标注数据集上应用方差校正临界阈值，理论保证有限样本类型I错误控制。


<details>
  <summary>Details</summary>
Motivation: LLM可靠性认证（验证失败率低于安全阈值）至关重要但具有挑战性。"LLM作为评判者"方法虽具可扩展性，但评判者的不完美、噪声和偏差可能使统计保证失效。需要解决评判者不完美情况下的统计评估问题。

Method: 引入"嘈杂但有效"假设检验框架：1）使用小规模人工标注校准集估计评判者的真阳性率（TPR）和假阳性率（FPR）；2）推导方差校正临界阈值应用于大规模评判者标注数据集；3）理论保证有限样本类型I错误控制，明确建模评判者行为而非黑盒估计。
...
### [195] [Why Adam Works Better with $β_1 = β_2$: The Missing Gradient Scale Invariance Principle](https://arxiv.org/abs/2601.21739)
*Alberto Fernández-Hernández,Cristian Pérez-Corral,Jose I. Mestre,Manuel F. Dolz,Enrique S. Quintana-Ortí*

Main category: cs.LG

TL;DR: Adam优化器在β₁=β₂时表现更好，这源于梯度尺度不变性这一结构特性，该论文对此提供了理论解释和实验验证。


<details>
  <summary>Details</summary>
Motivation: Adam优化器已广泛使用近十年，但一个简单经验事实仍未得到解释：当动量参数β₁=β₂时，验证分数和训练过程的定性行为都会改善。虽然近期研究报道了这种模式，但缺乏理论解释。

Method: 论文形式化了"梯度尺度不变性"概念，并证明Adam在一阶意义上具有梯度尺度不变性的充要条件是β₁=β₂。通过理论分析和实验验证，在视觉和语言任务以及不同架构家族中进行测试。

Result: 实验表明，当β₁=β₂时，梯度缩放对更新的影响更加平滑。理论证明Adam具有一阶梯度尺度不变性当且仅当β₁=β₂，这为Adam的平衡机制提供了理论基础。

Conclusion: β₁=β₂的选择与梯度尺度不变性密切相关，这一视角将Adam的平衡机制与多个近期优化器的设计原则对齐，为Adam行为中的开放问题提供了连贯解释，并为未来优化器设计提供了简单指导原则。
...
### [196] [Noninvasive Intracranial Pressure Estimation Using Subspace System Identification and Bespoke Machine Learning Algorithms: A Learning-to-Rank Approach](https://arxiv.org/abs/2601.20916)
...
TL;DR: 开发了一种结合系统辨识和排序约束优化的机器学习算法，通过无创信号（动脉血压、脑血流速度、R-R间期）估计颅内压，约32%测试样本误差在2mmHg内，34%误差在2-6mmHg之间。
...
Result: 测试结果显示：约31.88%的测试样本估计误差在2mmHg以内，34.07%的测试样本误差在2-6mmHg之间，证明了所提无创ICP估计方法的可行性。
...
### [197] [ECSEL: Explainable Classification via Signomial Equation Learning](https://arxiv.org/abs/2601.21789)
*Adia Lumadjeng,Ilker Birbil,Erman Acar*

Main category: cs.LG

TL;DR: ECSEL是一种可解释的分类方法，通过学习符号方程形式的正式表达式，既能作为分类器又能提供解释，在符号回归基准上优于现有方法且计算量更小。


<details>
  <summary>Details</summary>
Motivation: 许多符号回归基准具有紧凑的符号结构，但现有方法在恢复目标方程方面存在不足，需要一种既能保持高分类准确性又不牺牲可解释性的方法。

Method: ECSEL直接构建结构化的闭式表达式作为分类器和解释，学习符号方程形式的正式表达式，利用符号回归基准的紧凑符号结构特性。

Result: 在标准符号回归基准上，ECSEL比现有方法恢复更多目标方程且计算量显著减少；在分类准确性上与成熟机器学习模型相当；在基准数据集和电子商务、欺诈检测案例中，学习到的方程能揭示数据集偏差、支持反事实推理并提供可操作见解。

Conclusion: ECSEL是一种高效且可解释的分类方法，通过学习符号方程实现分类和解释的双重功能，在保持高准确性的同时满足全局特征行为、决策边界分析和局部特征归因等理想特性，具有实际应用价值。
...
TL;DR: 提出一种结构化矩阵估计的迁移学习框架，通过将目标参数分解为嵌入的源组件、低秩创新和稀疏编辑，在环境维度和内在表示同时增长的情况下实现知识迁移。


<details>
  <summary>Details</summary>
Motivation: 学习系统经常随时间扩展其环境特征或潜在表示，将早期表示嵌入到更大的空间中，但新潜在结构有限。需要研究在环境维度和内在表示同时增长情况下的结构化矩阵估计迁移学习问题。

Method: 提出通用迁移框架，将目标参数分解为：1)嵌入的源组件，2)低维低秩创新，3)稀疏编辑。开发锚定交替投影估计器，保留迁移的子空间，同时仅估计低维创新和稀疏修改。

Result: 建立确定性误差界限，分离目标噪声、表示增长和源估计误差，当秩和稀疏性增量较小时获得严格改进的速率。将框架应用于两个典型问题：单轨迹马尔可夫转移矩阵估计和扩大维度下的结构化协方差估计。

Conclusion: 提出的迁移学习框架在环境维度和内在表示同时增长的情况下有效，通过分解目标参数为源组件、低秩创新和稀疏编辑，实现了知识迁移和性能提升，具有广泛适用性。
...
### [199] [Distributional Active Inference](https://arxiv.org/abs/2601.20985)
*Abdullah Akgül,Gulcin Baykal,Manuel Haußmann,Mustafa Mert Çelikok,Melih Kandemir*

Main category: cs.LG

TL;DR: 将主动推理与分布强化学习框架集成，无需建模状态转移动态即可获得性能优势


<details>
  <summary>Details</summary>
Motivation: 机器人系统在复杂环境中的最优控制面临两个互补且交织的挑战：感官状态信息的有效组织和远见行动规划。强化学习仅解决后者，导致样本效率低下。主动推理是解释生物大脑如何处理这一双重问题的最先进过程理论，但其在人工智能中的应用迄今仅限于现有基于模型方法的扩展。

Method: 提出一个形式化的强化学习算法抽象，涵盖基于模型、分布式和免模型方法。该抽象将主动推理无缝集成到分布强化学习框架中，使其性能优势可在无需建模转移动态的情况下实现。

Result: 通过将主动推理与分布强化学习集成，可以在不依赖状态转移动态建模的情况下获得主动推理的性能优势，解决了传统强化学习样本效率低下的问题。

Conclusion: 该研究通过形式化抽象将主动推理集成到分布强化学习框架中，为复杂环境中的机器人控制提供了更高效的解决方案，既保留了主动推理的性能优势，又避免了传统基于模型方法对状态转移动态建模的需求。
...
### [200] [Pre-trained Encoders for Global Child Development: Transfer Learning Enables Deployment in Data-Scarce Settings](https://arxiv.org/abs/2601.20987)
*Md Muhtasim Munif Fahim,Md Rezaul Karim*

Main category: cs.LG

TL;DR: 开发首个全球儿童发展预训练编码器，使用UNICEF 44国35.7万儿童数据，仅需50个样本即可实现0.65 AUC，显著优于冷启动方法，支持资源受限环境下的SDG 4.2.1监测。


<details>
  <summary>Details</summary>
Motivation: 每年大量儿童经历可预防的发展迟缓，但机器学习在新国家的部署面临数据瓶颈：可靠模型需要数千样本，而新项目通常少于100个样本。

Method: 使用UNICEF调查数据，在44个国家357,709名儿童上训练首个全球儿童发展预训练编码器，采用迁移学习方法，应用迁移学习边界理论解释预训练多样性如何实现少样本泛化。

Result: 仅需50个训练样本，预训练编码器平均AUC达0.65（95% CI: 0.56-0.72），比冷启动梯度提升的0.61高8-12%；500个样本时AUC达0.73；零样本部署到未见国家AUC最高达0.84。

Conclusion: 预训练编码器能够显著改变资源受限环境下SDG 4.2.1监测的可行性，通过预训练多样性实现少样本泛化，解决全球儿童发展监测的数据瓶颈问题。
...
### [201] [Top-k on a Budget: Adaptive Ranking with Weak and Strong Oracles](https://arxiv.org/abs/2601.20989)
*Lutz Oettershagen*

Main category: cs.LG

TL;DR: 该论文研究在弱、强双预言机设置下高效识别top-k项的问题，提出自适应认证算法ACE和ACE-W，显著减少对昂贵强预言机的调用。


<details>
  <summary>Details</summary>
Motivation: 当精确评估代价高昂时，识别top-k项变得困难。论文旨在利用廉价但嘈杂的弱预言机和稀缺但高保真的强预言机，设计高效算法以减少对昂贵强预言机的依赖。

Method: 首先分析基线方法STC（筛选后认证），然后提出ACE算法，自适应地将强查询集中在关键边界项上。进一步提出ACE-W，这是一个两阶段自适应方法，先自适应分配弱预算，再运行ACE。

Result: STC最多需要m(4ε_max)次强调用。ACE在理论上达到相同的O(m(4ε_max))上界，但在实践中显著减少强调用。ACE-W通过自适应弱预算分配进一步降低强成本。

Conclusion: 论文提出了高效的自适应认证算法ACE和ACE-W，在双预言机设置下显著减少对昂贵强预言机的调用，同时建立了Ω(m(ε_max))的条件下界，表明所提算法接近最优。
...
### [202] [The Depth Delusion: Why Transformers Should Be Wider, Not Deeper](https://arxiv.org/abs/2601.20994)
*Md Muhtasim Munif Fahim,Md Rezaul Karim*
...
<details>
  <summary>Details</summary>
Motivation: 传统神经缩放定律将架构视为可互换的，但实际中相同参数规模可以通过不同深度-宽度组合实现。需要理解架构如何影响缩放行为，特别是深度和宽度的最优权衡。

Method: 提出架构条件化的缩放定律，将损失分解为深度和宽度的函数。通过30个Transformer架构（1700万到70亿参数）的实验验证，每个架构在代表性高计算样本上训练，使用回归分析建立缩放关系。

Result: 发现最优深度缩放为D* ~ C^0.12，最优宽度缩放为W* ~ C^0.34，宽度增长应比深度快2.8倍。揭示了临界深度现象：超过D_crit ~ W^0.44后增加层数反而增加损失。在70亿规模下，64层模型（63.8亿参数）比32层模型（68.6亿参数）性能差0.12 nats，验证了深度错觉。

Conclusion: 架构对缩放定律有重要影响，深度和宽度的最优权衡在生产规模上仍然存在。宽度应优先于深度扩展，超过临界深度会导致性能下降。这些发现对大规模语言模型设计具有实际指导意义。
...
### [203] [Solver-in-the-Loop: MDP-Based Benchmarks for Self-Correction and Behavioral Rationality in Operations Research](https://arxiv.org/abs/2601.21008)
*Ruicheng Ao,David Simchi-Levi,Xinshang Wang*

Main category: cs.LG

TL;DR: 论文提出了两个新的运筹学基准测试ORDebug和ORBias，将求解器纳入评估循环，评估LLM在迭代调试和偏差方面的表现，发现领域特定的训练能超越前沿API模型。
...
Method: 引入两个基准测试：1) ORDebug评估迭代自修正能力，包含5000+问题覆盖9种错误类型，每次修复触发求解器重新执行和IIS重新计算；2) ORBias评估行为理性，包含2000个报童问题实例，测量与闭式最优策略的系统性偏差。

Result: 在26个模型和12000+样本测试中，领域特定的RLVR训练使8B模型超越前沿API：恢复率95.3% vs 86.2%，诊断准确率62.4% vs 47.8%，解决步骤2.25 vs 3.78（快1.7倍）。在ORBias上，课程训练实现了唯一的负ID→OOD偏差漂移（-9.6%），系统性偏差减少48%（从20.0%到10.4%）。

Conclusion: 通过可验证预言的过程级评估能够实现针对性的训练，其效果超越了单纯扩大模型规模，为运筹学领域的LLM评估提供了更贴近实际工作流程的基准框架。
...
### [204] [Order-Aware Test-Time Adaptation: Leveraging Temporal Dynamics for Robust Streaming Inference](https://arxiv.org/abs/2601.21012)
*Young Kyung Kim,Oded Schlesinger,Qiangqiang Wu,J. Matías Di Martino,Guillermo Sapiro*

Main category: cs.LG

TL;DR: OATTA是一种考虑时间动态的测试时适应方法，通过贝叶斯估计和时间先验提升预训练模型在分布偏移下的性能，同时使用似然比门控确保安全性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法通常将测试流视为独立样本，忽略了时间动态中固有的监督信号，这限制了模型在连续测试流中的适应能力。

Method: 将测试时适应建模为无梯度递归贝叶斯估计任务，使用学习到的动态转移矩阵作为时间先验来优化基础模型预测，并引入似然比门控在缺乏时间证据时回退到基础预测器。

Result: 在图像分类、可穿戴和生理信号分析、语言情感分析等多个领域的实验中，OATTA持续提升现有基线方法，准确率最高提升6.35%，证明了时间动态建模的重要性。
...
### [205] [Predict-Project-Renoise: Sampling Diffusion Models under Hard Constraints](https://arxiv.org/abs/2601.21033)
*Omer Rochman-Sharabi,Gilles Louppe*

Main category: cs.LG

TL;DR: 提出PPR（Predict-Project-Renoise）约束采样框架，通过预测-投影-重噪的迭代算法，在扩散模型生成过程中强制执行物理定律等硬约束，显著减少约束违反并提高样本一致性。
...
### [206] [Test-Time Adaptation for Unsupervised Combinatorial Optimization](https://arxiv.org/abs/2601.21048)
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督神经组合优化方法分为两种互斥范式：跨实例泛化模型和实例特定模型。前者推理高效但缺乏实例级适应性，后者灵活但无法利用学习到的归纳结构且易陷入局部最优。需要结合泛化模型的归纳偏置和实例特定模型的灵活性。

Method: 提出TACO框架，采用策略性热启动方法，部分放松训练参数同时保持归纳偏置，实现快速有效的无监督适应。该框架模型无关，统一并扩展了现有两种范式。

Result: 相比简单微调泛化模型或从头优化实例特定模型，TACO在最小顶点覆盖和最大团等经典组合优化问题上获得更好的解质量，且计算成本可忽略。在静态、分布偏移和动态问题上均表现出有效性和鲁棒性。

Conclusion: TACO建立了泛化性和实例特定无监督神经组合优化之间的实用桥梁，通过策略性热启动实现了归纳偏置保持与实例级灵活性的统一。
...
### [207] [Snowball: A Scalable All-to-All Ising Machine with Dual-Mode Markov Chain Monte Carlo Spin Selection and Asynchronous Spin Updates for Fast Combinatorial Optimization](https://arxiv.org/abs/2601.21058)
...
Conclusion: Snowball通过解决拓扑、算法和精度三大挑战，为伊辛机的实际部署提供了有效的数字架构解决方案
...
TL;DR: 提出一种人机协作的特征工程框架，将LLM生成特征变换操作与基于效用估计的选择过程解耦，并引入人类专家偏好反馈来指导选择
...
Method: 提出人机协作特征工程框架：1) 解耦操作提出和选择过程，LLM仅生成候选操作；2) 通过显式建模每个操作的效用和不确定性来指导选择；3) 在早期轮次引入人类专家偏好反馈，比较哪些操作更有前景

Result: 在合成研究和真实用户研究中，该框架在多种表格数据集上提高了特征工程性能，并降低了用户在特征工程过程中的认知负荷

Conclusion: 通过解耦LLM操作生成与基于效用估计的选择过程，并整合人类专家反馈，可以更有效地进行特征工程，提高性能同时减轻用户负担
...
### [209] [Textual Equilibrium Propagation for Deep Compound AI Systems](https://arxiv.org/abs/2601.21064)
*Minghui Chen,Wenlong Deng,James Zou,Han Yu,Xiaoxiao Li*

Main category: cs.LG

TL;DR: TEP是一种用于深度复合AI系统的局部学习框架，通过平衡传播机制解决长时程工作流中的文本梯度爆炸和消失问题，相比全局传播方法在准确性和效率上均有提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于全局文本反馈传播的方法（如TextGrad）在深度复合AI系统中存在性能随深度增加而下降的问题，具体表现为文本梯度爆炸（反馈长度指数增长）和文本梯度消失（长上下文能力有限导致反馈特异性丧失）。

Method: 提出文本平衡传播（TEP），受能量模型中平衡传播启发，包含两个阶段：1）自由阶段：局部LLM批评器迭代优化提示直至达到平衡；2）推动阶段：应用有界修改强度的近端提示编辑，通过前向信号而非后向反馈链传播任务级目标。

Result: 在长时程QA基准测试和多智能体工具使用数据集上，TEP相比TextGrad等全局传播方法在准确性和效率上均有持续改进，且增益随深度增加而增长，同时保持了黑盒LLM组件的实用性。

Conclusion: TEP通过局部优化和受控适应机制有效缓解了深度复合AI系统中的文本梯度问题，为长时程智能体工作流提供了更可扩展和高效的优化框架。
...
### [210] [Out-of-Distribution Generalization in Graph Foundation Models](https://arxiv.org/abs/2601.21067)
*Haoyang Li,Haibo Chen,Xin Wang,Wenwu Zhu*

Main category: cs.LG

TL;DR: 该论文是关于图基础模型（GFMs）在分布外（OOD）泛化方面的综述研究，首次系统性地梳理了图学习中的分布偏移挑战及GFMs的应对策略。


<details>
  <summary>Details</summary>
Motivation: 图学习模型在实际应用中面临分布偏移的挑战，包括图结构变化、领域语义差异、模态可用性和任务表述变化等。图基础模型通过大规模预训练学习通用表示，有望解决这些泛化问题，但目前缺乏系统性的综述研究。

Method: 1. 首先讨论图学习中分布偏移的主要挑战，并建立统一的问题设定框架；2. 根据是否支持跨异构任务泛化，将现有方法分为两类；3. 总结相应的OOD处理策略和预训练目标；4. 回顾常见的评估协议。

Result: 该综述首次系统性地梳理了图基础模型在OOD泛化方面的研究进展，建立了分类框架，总结了不同方法的策略和评估标准，为后续研究提供了理论基础和方法指导。

Conclusion: 图基础模型为解决图学习中的分布偏移问题提供了有前景的方向，但仍面临诸多挑战。该综述为OOD泛化在GFMs中的研究奠定了基础，并指出了未来的研究方向。
...
TL;DR: 该研究揭示了安全强化学习算法在糖尿病管理中的安全泛化差距：训练时满足安全约束的策略在未见患者上频繁违反安全要求，而测试时屏蔽技术能有效恢复安全性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是调查安全强化学习算法在分布偏移下的安全性泛化问题。当前安全RL算法通常在固定训练条件下评估，但实际部署时会面临分布偏移，这在糖尿病管理等安全关键领域尤为重要。

Method: 方法包括：1）在统一的临床模拟器上对八种安全RL算法进行基准测试；2）使用糖尿病管理作为安全关键测试平台；3）引入测试时屏蔽技术，利用学习到的动力学模型过滤不安全动作；4）评估三种糖尿病类型和三个年龄组的患者群体。

Result: 结果显示：1）存在安全泛化差距：训练时满足约束的策略在未见患者上频繁违反安全要求；2）测试时屏蔽能有效恢复安全性，对PPO-Lag和CPO等强基线算法实现13-14%的时间在范围内增益；3）同时降低临床风险指数和血糖变异性。

Conclusion: 结论是测试时屏蔽技术能有效解决安全RL在分布偏移下的安全泛化问题。研究提供的模拟器和基准平台为安全关键控制领域中分布偏移下的安全性研究提供了工具。
...
### [212] [TRACE: Trajectory Recovery for Continuous Mechanism Evolution in Causal Representation Learning](https://arxiv.org/abs/2601.21135)
*Shicheng Fan,Kun Zhang,Lu Cheng*

Main category: cs.LG

TL;DR: 提出TRACE方法解决连续机制转换的因果表示学习问题，通过原子机制的凸组合建模过渡机制，实现潜在因果变量和连续混合轨迹的联合识别。
...
Method: 提出TRACE框架：1) 将过渡机制建模为有限原子机制的凸组合，由时变混合系数控制；2) 采用专家混合框架，每个专家学习一个原子机制；3) 训练时学习原子机制，测试时恢复机制轨迹；4) 可泛化到训练中未见的中间机制状态。

Result: 在合成和真实数据实验中，TRACE恢复混合轨迹的相关性高达0.99，显著优于离散切换基线方法。理论证明潜在因果变量和连续混合轨迹可联合识别。

Conclusion: TRACE成功解决了连续机制转换的因果表示学习问题，通过原子机制凸组合建模实现了对连续机制轨迹的准确恢复，为现实世界连续变化系统的因果分析提供了有效框架。
...
### [213] [Smooth Dynamic Cutoffs for Machine Learning Interatomic Potentials](https://arxiv.org/abs/2601.21147)
*Kevin Han,Haolin Cong,Bowen Deng,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 该论文提出了一种动态截断半径方法来解决机器学习原子间势能（MLIPs）在推理时间和内存消耗方面的瓶颈，通过在四种先进MLIPs上实现，显著减少了内存使用和推理时间，同时保持了模拟精度。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势能（MLIPs）在分子动力学模拟中应用广泛，但面临推理时间和内存消耗两大瓶颈，限制了其达到真实模拟规模。传统MLIPs使用固定截断半径，作者挑战了这一长期假设。

Method: 首次引入动态截断半径公式，通过针对每个原子设定特定的邻居数量来诱导底层原子图的稀疏性，从而减少内存消耗和推理时间。该方法在四种先进MLIPs（MACE、Nequip、Orbv3、TensorNet）上实现。

Result: 动态截断方法显著减少了内存消耗（最多减少2.26倍）和推理时间（最多加快2.04倍），具体效果取决于模型和原子系统。广泛的误差分析表明，与固定截断模型相比，动态截断模型在材料和分子数据集上仅表现出最小的精度下降。

Conclusion: 动态截断半径方法成功解决了MLIPs的推理时间和内存消耗瓶颈，在保持模拟稳定性和精度的同时显著提升了计算效率。所有模型实现和训练代码将完全开源。
...
### [214] [Mobility-Embedded POIs: Learning What A Place Is and How It Is Used from Human Movement](https://arxiv.org/abs/2601.21149)
*Maria Despoina Siampou,Shushman Choudhury,Shang-Ling Hsu,Neha Arora,Cyrus Shahabi*

Main category: cs.LG

TL;DR: ME-POIs框架通过将语言模型生成的POI嵌入与大规模人类移动数据结合，学习基于真实使用情况的POI中心化、上下文无关表示，强调POI功能是通用POI表示的关键信号。
...
Method: 提出Mobility-Embedded POIs（ME-POIs）框架：1）将个体访问编码为时间上下文嵌入；2）通过对比学习将其与可学习的POI表示对齐，捕捉跨用户和时间的使用模式；3）针对长尾稀疏性问题，提出从邻近高频访问POI跨多个空间尺度传播时间访问模式的新机制。

Result: 在五个新提出的地图丰富化任务上评估，ME-POIs在增强文本嵌入方面始终优于纯文本和纯移动性基线。值得注意的是，仅使用移动数据训练的ME-POIs在某些任务上也能超越纯文本模型，突显POI功能是准确且可泛化POI表示的关键组成部分。

Conclusion: POI功能是通用POI表示中不可或缺的信号。ME-POIs框架通过结合文本语义和人类移动模式，能够学习同时捕捉POI身份和功能的表示，在地图丰富化任务中表现出色，为地理空间基础模型提供了新的研究方向。
...
### [215] [A Federated Generalized Expectation-Maximization Algorithm for Mixture Models with an Unknown Number of Components](https://arxiv.org/abs/2601.21160)
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦聚类中全局聚类数K未知且客户端具有异构、可能重叠聚类集的问题。现有方法通常假设K已知或客户端聚类集相同，无法处理实际场景中的异构性和重叠性。

Method: 提出FedGEM算法：客户端本地执行EM步骤，为每个本地组件构建不确定性集合；中央服务器利用不确定性集合学习客户端间潜在聚类重叠，通过闭式计算推断全局聚类数。针对各向同性高斯混合模型提供了可处理、低复杂度的客户端计算。

Result: 理论分析提供了概率收敛保证；数值实验表明FedGEM性能与集中式EM相当，优于现有联邦聚类方法；针对各向同性GMM验证了算法收敛所需假设。

Conclusion: FedGEM有效解决了未知全局聚类数下的联邦聚类问题，能够处理客户端异构和聚类重叠，具有理论保证和实际性能优势。
...
### [216] [Efficient Simple Regret Algorithms for Stochastic Contextual Bandits](https://arxiv.org/abs/2601.21167)
*Shuai Liu,Alireza Bakhtiari,Alex Ayoub,Botao Hao,Csaba Szepesvári*

Main category: cs.LG

TL;DR: 论文研究了随机上下文逻辑赌博机在简单遗憾目标下的性能，提出了首个达到$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾的确定性算法，以及基于Thompson Sampling的随机算法，其遗憾为$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，且主要项不依赖于参数向量的指数常数。


<details>
  <summary>Details</summary>
Motivation: 虽然线性上下文赌博机在简单遗憾方面已有理论保证，但逻辑回归设置下的简单遗憾问题尚未得到解决。现有方法通常依赖于参数向量范数相关的指数常数κ，这在实际应用中可能导致性能下降。因此需要开发不依赖κ的高效算法。

Method: 结合上下文线性赌博机和自协调分析的思想，提出了首个确定性算法。同时，针对简单遗憾设置设计了新的Thompson Sampling变体，分别应用于线性上下文赌博机和逻辑赌博机。算法在动作集有限时完全可计算。

Result: 确定性算法达到$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾，主要项不依赖κ=$\mathcal{O}(\exp(S))$。随机算法在线性设置中达到$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，在逻辑设置中同样达到$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，且主要项均不依赖κ。实验验证了理论保证。

Conclusion: 本文首次解决了逻辑上下文赌博机的简单遗憾问题，提出了不依赖指数常数κ的高效算法。确定性算法在遗憾界上更优，而随机算法计算成本更低。这些结果为逻辑回归设置下的简单遗憾优化提供了理论基础和实用方法。
...
### [217] [AC2L-GAD: Active Counterfactual Contrastive Learning for Graph Anomaly Detection](https://arxiv.org/abs/2601.21171)
*Kamal Berahmand,Saman Forouzandeh,Mehrnoush Mohammadi,Parham Moradi,Mahdi Jalili*

Main category: cs.LG

TL;DR: AC2L-GAD：基于主动反事实对比学习的图异常检测框架，通过信息论主动选择和反事实生成解决语义一致性破坏和负样本信息不足问题，在保持检测质量的同时减少约65%计算开销。


<details>
  <summary>Details</summary>
Motivation: 图异常检测面临标签稀缺和极端类别不平衡的挑战。现有图对比学习方法存在两个关键缺陷：随机增强破坏正样本对的语义一致性，而朴素负采样产生无信息对比。需要解决这些问题以实现更有效的无监督异常检测。

Method: 提出AC2L-GAD框架，结合信息论主动选择和反事实生成。通过主动选择识别结构复杂节点，生成保持异常特征的正增强样本和提供困难对比的正常负样本，同时将昂贵的反事实生成限制在策略选择的子集上。

Result: 在九个基准数据集（包括GADBench的真实金融交易图）上的实验表明，AC2L-GAD达到或优于最先进基线性能，在异常表现出复杂属性-结构交互的数据集上增益显著。相比全图反事实生成，计算开销减少约65%同时保持检测质量。

Conclusion: AC2L-GAD通过主动反事实对比学习有效解决了图异常检测中的语义一致性破坏和负样本信息不足问题，在保持高性能的同时显著降低计算成本，特别适用于具有复杂属性-结构交互的异常检测场景。
...
### [218] [Breaking the Reasoning Horizon in Entity Alignment Foundation Models](https://arxiv.org/abs/2601.21174)
...
<details>
  <summary>Details</summary>
Motivation: 现有实体对齐模型缺乏可迁移性，无法对齐未见知识图谱而无需重新训练。虽然图基础模型提供了一种解决方案，但直接将其应用于实体对齐效果不佳，这源于关键的"推理视野差距"：与图基础模型中的链接预测不同，实体对齐需要捕捉稀疏异构知识图谱结构中的长程依赖关系

Method: 提出基于并行编码策略的实体对齐基础模型：1) 使用种子对齐对作为局部锚点引导信息流，同时初始化并编码两个并行流；2) 促进锚点条件消息传递，利用局部结构邻近性而非全局搜索显著缩短推理轨迹；3) 引入合并关系图建模全局依赖关系；4) 设计可学习交互模块进行精确匹配

Result: 大量实验验证了该框架的有效性，突显了其对未见知识图谱的强大泛化能力

Conclusion: 通过并行编码策略和锚点引导的信息流，成功解决了实体对齐中的推理视野差距问题，实现了对未见知识图谱的有效对齐，为知识图谱融合提供了可迁移的解决方案
...
### [219] [Flow Perturbation++: Multi-Step Unbiased Jacobian Estimation for High-Dimensional Boltzmann Sampling](https://arxiv.org/abs/2601.21177)
*Xin Peng,Ang Gao*

Main category: cs.LG

TL;DR: Flow Perturbation++是一种方差减少的连续归一化流方法，通过多步离散化概率流ODE和每步无偏雅可比估计，改进了高维系统中无偏玻尔兹曼采样的可扩展性。
...
Method: 提出Flow Perturbation++，这是Flow Perturbation的方差减少扩展，通过离散化概率流ODE并在每个积分步骤执行无偏的逐步雅可比估计。这种多步构造保持了Flow Perturbation的无偏性，同时显著降低了估计器方差。

Result: 在顺序蒙特卡洛框架中集成Flow Perturbation++，在1000维高斯混合模型和全原子Chignolin蛋白质上，相比基于Hutchinson和单步Flow Perturbation的基线方法，实现了显著改进的平衡采样。

Conclusion: Flow Perturbation++通过多步离散化和逐步雅可比估计，在保持无偏性的同时显著降低方差，为高维系统中的连续归一化流玻尔兹曼采样提供了更可扩展的解决方案。
...
### [220] [Rethinking Refinement: Correcting Generative Bias without Noise Injection](https://arxiv.org/abs/2601.21182)
*Xin Peng,Ang Gao*
...
<details>
  <summary>Details</summary>
Motivation: 生成模型（如扩散模型和基于流的模型）在高维设置中常表现出系统性偏差，降低样本质量。现有精炼方法通常需要噪声注入或多步重采样，扰动采样动态，作者旨在开发一种无需这些操作的有效偏差校正方法。

Method: 提出基于流匹配的双阶段流精炼（BFR）框架，包含两种精炼策略：1）针对近似可逆生成器的潜空间对齐；2）使用轻量增强训练的数据空间精炼。该方法保持原始ODE轨迹，对生成样本应用确定性校正，无需噪声注入或多步重采样。

Result: 在MNIST、CIFAR-10和FFHQ（256×256分辨率）数据集上实验表明，BFR能一致提升保真度和覆盖率。特别地，从FID 3.95的基础样本开始，潜空间精炼仅使用单次额外函数评估（1-NFE）就在MNIST上实现了最先进的FID 1.46，同时保持了样本多样性。

Conclusion: BFR框架证明有效的偏差校正可以作为后处理程序实现，无需扰动采样动态。该方法通过保持原始ODE轨迹并应用确定性校正，在多个数据集上显著提升生成样本质量，为生成模型精炼提供了高效实用的解决方案。
...
### [221] [Rethinking Self-Training Based Cross-Subject Domain Adaptation for SSVEP Classification](https://arxiv.org/abs/2601.21203)
*Weiguang Wang,Yong Liu,Yingjie Gao,Guangyuan Xu*

Main category: cs.LG

TL;DR: 提出了一种基于自训练范式的跨被试SSVEP-BCI域适应方法，包含FBEA、CSST框架和TFA-CL模块，在多个数据集上实现了最先进性能。
...
Conclusion: 提出的跨被试域适应方法有效解决了SSVEP-BCI中信号变异性和标注成本问题，通过FBEA、CSST框架和TFA-CL模块的组合实现了优异的跨被试解码性能。
...
### [222] [PHDME: Physics-Informed Diffusion Models without Explicit Governing Equations](https://arxiv.org/abs/2601.21234)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型为动力系统轨迹预测提供了表达性先验，但在稀疏数据条件下不可靠。物理信息机器学习（PIML）提高了可靠性，但大多数方法需要训练时的显式控制方程，而复杂非线性动力学往往只能部分已知。

Method: 1. 在有限观测上训练高斯过程分布式端口哈密顿系统（GP-dPHS），捕捉基于能量的动力学表示；2. 使用GP-dPHS生成物理一致的人工数据集用于扩散训练；3. 通过结构化物理残差损失为扩散模型提供信息；4. 训练后扩散模型作为摊销采样器和预测器；5. 应用分割保形校准提供不确定性陈述。

Result: 在PDE基准测试和真实世界弹簧系统实验中，PHDME在数据稀缺条件下显示出改进的准确性和物理一致性。

Conclusion: PHDME框架成功解决了稀疏观测和不完全物理条件下的轨迹预测问题，通过结合端口哈密顿结构先验、扩散模型和不确定性量化，为数据稀缺的复杂动力系统提供了可靠的预测工具。
...
### [223] [Less Noise, More Voice: Reinforcement Learning for Reasoning via Instruction Purification](https://arxiv.org/abs/2601.21244)
...
Method: 提出Less Noise Sampling Framework (LENS)：1) 识别并移除提示中的干扰token进行净化；2) 将净化过程产生的成功rollout转移到原始噪声提示上监督策略优化，使模型学会在真实噪声环境中忽略干扰。

Result: LENS显著优于GRPO，平均性能提升3.88%，收敛速度加快1.6倍以上，证明了修剪干扰token对提升rollout效率的关键作用。

Conclusion: 干扰token的识别和移除是提升RLVR探索效率的关键，LENS框架为RLVR研究提供了新视角，通过净化-转移机制有效解决了噪声提示下的探索问题。
...
### [224] [EGAM: Extended Graph Attention Model for Solving Routing Problems](https://arxiv.org/abs/2601.21281)
...
<details>
  <summary>Details</summary>
Motivation: 现有图注意力模型(GAM)仅考虑节点特征，存在局限性；需要更通用的图注意力机制来处理复杂图结构，特别是高度约束的路由问题
...
Conclusion: EGAM通过扩展图注意力机制，有效解决了传统GAM的局限性，在神经组合优化中展现出优越性能，特别是在处理复杂约束的路由问题上
...
### [225] [DUET: Distilled LLM Unlearning from an Efficiently Contextualized Teacher](https://arxiv.org/abs/2601.21283)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法存在显著限制：传统调优方法计算量大且易导致灾难性遗忘，而上下文遗忘方法虽轻量但易受提示移除或逆向工程攻击。需要结合两者优点的新方法

Method: 提出蒸馏遗忘方法DUET，通过提示引导的教师模型指导学生模型，在拒绝不良知识生成的同时保留通用领域知识

Result: 在现有基准测试和增强评估协议下，DUET在遗忘和效用保持方面表现更高，同时数据效率比最先进方法高出数量级

Conclusion: DUET是一种新颖的基于蒸馏的遗忘方法，成功结合了调优和上下文遗忘的优点，实现了高效、精确且鲁棒的LLM遗忘
...
### [226] [Zenith: Scaling up Ranking Models for Billion-scale Livestreaming Recommendation](https://arxiv.org/abs/2601.21285)
...
TL;DR: Zenith是一个可扩展且高效的推荐排序架构，通过处理少量高维Prime Tokens并利用Token Fusion和Token Boost模块，以最小运行时开销学习复杂特征交互，在TikTok Live上验证了其实际效果。
...
Method: 提出Zenith架构，通过处理少量高维Prime Tokens，结合Token Fusion和Token Boost模块，以最小运行时开销学习复杂特征交互。该架构表现出优于其他最先进排序方法的扩展规律，得益于改进的token异质性。

Result: 在TikTok Live平台部署的A/B测试显示：在线CTR AUC提升1.05%，Logloss降低1.10%；质量观看会话/用户提升9.93%，质量观看时长/用户提升8.11%。

Conclusion: Zenith架构通过高效处理高维Prime Tokens和创新的Token模块，实现了在最小运行时开销下学习复杂特征交互，在实际大规模推荐场景中验证了其有效性和可扩展性。
...
### [227] [TimeSliver : Symbolic-Linear Decomposition for Explainable Time Series Classification](https://arxiv.org/abs/2601.21289)
*Akash Pandey,Payal Mohapatra,Wei Chen,Qi Zhu,Sinan Keten*

Main category: cs.LG

TL;DR: TimeSliver：一种新颖的可解释性驱动深度学习框架，通过联合利用原始时间序列数据及其符号抽象来构建保持原始时间结构的表示，每个元素线性编码每个时间片段对最终预测的贡献，为时间序列分类提供有意义的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列可解释方法存在局限性：基于梯度和特征归因的后验方法存在参考状态敏感性，难以跨数据集泛化，且将时间点独立处理而忽略序列依赖性；基于自注意力机制等可解释组件的方法中，注意力权重往往无法提供忠实的时间重要性度量。需要开发既能保持良好预测性能又能提供有意义时间重要性解释的框架。

Method: 提出TimeSliver框架，联合利用原始时间序列数据及其符号抽象来构建表示，该表示保持原始时间结构。每个表示元素线性编码每个时间片段对最终预测的贡献，从而为每个时间点分配有意义的重要性分数。框架通过符号抽象增强可解释性，同时通过联合表示保持预测能力。

Result: 在时间序列分类任务中，TimeSliver在7个不同的合成和真实世界多元时间序列数据集上，比其他时间归因方法的性能高出11%。在26个UEA基准数据集上，TimeSliver的预测性能与最先进基线相比差距在2%以内，使其成为既强大又可解释的通用时间序列分类框架。

Conclusion: TimeSliver通过联合原始数据和符号抽象构建的表示，成功解决了现有时间序列可解释方法的局限性，在保持接近最先进预测性能的同时，提供了有意义的、线性编码的时间重要性解释，为时间序列分类提供了一个强大且可解释的深度学习框架。
...
### [228] [Physics-Guided Tiny-Mamba Transformer for Reliability-Aware Early Fault Warning](https://arxiv.org/abs/2601.21293)
*Changyu Li,Dingcheng Huang,Kexuan Yao,Xiaoya Ni,Lijuan Shen,Fei Luo*

Main category: cs.LG

TL;DR: PG-TMT：一种用于旋转机械可靠性预测的紧凑型三分支编码器，结合物理引导的频谱对齐和极值理论校准决策，实现非平稳工况下的早期故障预警。


<details>
  <summary>Details</summary>
Motivation: 旋转机械的可靠性预测需要在非平稳工况、领域偏移（速度/负载/传感器）、严重类别不平衡条件下，提供准确且误报率可控的早期预警信号。

Method: 提出物理引导的Tiny-Mamba Transformer (PG-TMT)：1) 深度可分离卷积主干捕获微瞬变；2) Tiny-Mamba状态空间分支建模近线性长程动态；3) 轻量级局部Transformer编码跨通道共振。通过解析的时频映射将注意力谱与经典轴承故障阶次带对齐，获得物理合理性评分。使用极值理论建模健康分数超限，实现目标误报强度的阈值校准，并采用双阈值滞回和最小保持时间抑制抖动。

Result: 在CWRU、Paderborn、XJTU-SY和工业试点数据集上，采用无泄漏流式协议和右截断检测，PG-TMT在类别不平衡下获得更高的精确率-召回率AUC，竞争性或更优的ROC AUC，在匹配误报强度下更短的均值检测时间，并表现出强大的跨领域迁移能力。

Conclusion: 通过将物理对齐表示与EVT校准决策规则结合，PG-TMT为以可靠性为中心的预测和健康管理提供了校准、可解释且可部署的早期预警系统。
...
### [229] [Grounding and Enhancing Informativeness and Utility in Dataset Distillation](https://arxiv.org/abs/2601.21296)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法多依赖启发式方法平衡效率与质量，但原始数据与合成数据之间的基本关系研究不足。本文旨在建立坚实的理论框架重新审视基于知识蒸馏的数据集蒸馏。
...
Conclusion: 通过引入信息性和效用性的理论概念，并基于博弈论和梯度范数方法实现平衡优化，InfoUtil框架为数据集蒸馏提供了坚实的理论基础和有效的实践方法，显著提升了蒸馏数据集的质量。
...
### [230] [The Surprising Difficulty of Search in Model-Based Reinforcement Learning](https://arxiv.org/abs/2601.21306)
...
<details>
  <summary>Details</summary>
Motivation: 传统观点认为模型强化学习的主要障碍是长期预测和复合误差，但作者质疑这一观点，旨在探究搜索在模型强化学习中的实际作用，并揭示搜索可能损害性能的根本原因。

Method: 通过实验分析搜索在模型强化学习中的表现，对比搜索与学习策略的效果，研究分布偏移对性能的影响，并基于这些发现开发了实现有效搜索的关键技术。

Result: 研究发现搜索并非学习策略的直接替代品，即使模型精度很高，搜索也可能损害性能；缓解分布偏移比提高模型或价值函数精度对性能提升更为关键；基于这些发现开发的技术在多个流行基准领域实现了最先进的性能。

Conclusion: 模型强化学习成功的关键在于有效管理分布偏移而非单纯追求模型精度，搜索需要与适当的分布偏移缓解技术结合才能发挥最大效用，这一新视角为模型强化学习的发展提供了重要指导。
...
### [231] [Transferable Graph Condensation from the Causal Perspective](https://arxiv.org/abs/2601.21309)
...
TL;DR: TGCC是一种基于因果不变性的可迁移图数据集压缩方法，通过提取空间域因果不变特征、增强压缩操作和谱域对比学习，生成可跨任务和跨域使用的压缩图数据集。


<details>
  <summary>Details</summary>
Motivation: 现有图数据集压缩方法要求下游应用与原始数据集和任务严格匹配，在跨任务和跨域场景中效果不佳，需要开发具有更好迁移能力的压缩方法。

Method: 1) 通过因果干预从图的空间域提取领域因果不变特征；2) 执行增强压缩操作以完整捕获原始图的结构和特征信息；3) 通过谱域增强对比学习将因果不变特征注入压缩图中，确保保留原始图的因果信息。

Result: 在5个公共数据集和新提出的FinReport数据集上，TGCC在跨任务和跨域复杂场景中相比现有方法提升达13.41%，在单数据集和任务场景中，在6个数据集中的5个上达到最先进性能。

Conclusion: TGCC通过因果不变性框架成功解决了图数据集压缩的迁移性问题，生成的压缩数据集在保持性能的同时具有更好的跨任务和跨域适应性。
...
### [232] [Distributionally Robust Classification for Multi-source Unsupervised Domain Adaptation](https://arxiv.org/abs/2601.21315)
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督域自适应方法在实际应用中面临挑战，特别是在目标域只有有限无标签数据或源域存在伪相关性的场景下。需要一种能够处理这些实际限制的鲁棒方法。

Method: 提出一个分布鲁棒学习框架，同时建模协变量分布和条件标签分布的不确定性。该框架最初受多源域自适应启发，但同样适用于单源场景。开发了能与现有UDA方法无缝集成的有效学习算法。

Result: 在各种分布偏移场景下的广泛实验表明，该方法始终优于强基线方法，特别是在目标数据极其稀缺的情况下表现尤为突出。

Conclusion: 提出的分布鲁棒学习框架有效解决了无监督域自适应中的实际挑战，特别是在目标数据稀缺和源域伪相关性场景下，为UDA提供了更鲁棒的解决方案。
...
### [233] [Memorization Control in Diffusion Models from Denoising-centric Perspective](https://arxiv.org/abs/2601.21348)
*Thuy Phuong Vu,Mai Viet Hoang Do,Minhhuy Le,Dinh-Cuong Hoang,Phan Xuan Tan*

Main category: cs.LG

TL;DR: 提出一种基于去噪中心视角的扩散模型记忆控制方法，通过调整时间步采样策略来控制学习位置，从而平衡记忆化与泛化
...
Method: 提出一种时间步采样策略，通过调整置信区间宽度来显式控制学习在去噪轨迹上的发生位置。该方法允许直接控制记忆化与泛化的权衡

Result: 在图像和1D信号生成任务上的实验表明，将学习重点移向后期去噪步骤能持续减少记忆化并改善与训练数据的分布对齐，验证了方法的通用性和有效性

Conclusion: 从去噪中心视角研究扩散模型记忆化问题，提出的时间步采样策略为控制记忆化提供了有效方法，通过调整学习位置能改善模型泛化能力
...
### [234] [L2R: Low-Rank and Lipschitz-Controlled Routing for Mixture-of-Experts](https://arxiv.org/abs/2601.21349)
...
<details>
  <summary>Details</summary>
Motivation: 现代MoE系统通常在线性高维表示空间中使用路由器，存在表示不匹配、角度集中和尺度敏感评分等问题，这些会共同削弱路由判别能力和专家专业化的稳定性

Method: L2R框架包含三个关键组件：1）在共享低维潜在路由空间进行专家分配；2）引入饱和内积评分（SIPS）显式控制路由函数的Lipschitz行为；3）参数高效的多锚点路由机制增强专家表达能力

Result: 在大规模语言MoE模型和ImageNet上的视觉MoE设置中，L2R一致地改善了路由稳定性、专家专业化和整体模型性能

Conclusion: L2R通过重塑路由空间和评分几何，解决了传统MoE路由中的关键问题，为MoE模型提供了更稳定和高效的路由框架
...
### [235] [Factored Causal Representation Learning for Robust Reward Modeling in RLHF](https://arxiv.org/abs/2601.21350)
...
Result: 在数学和对话任务上的实验表明，该方法能学习更鲁棒的奖励模型，并持续改进下游RLHF性能，优于最先进的基线方法。对长度和谄媚偏见的分析进一步验证了该方法在缓解奖励黑客行为方面的有效性。

Conclusion: 通过因果视角的分解表示学习框架，能够有效识别和分离奖励预测中的因果因素与非因果因素，从而构建更鲁棒的奖励模型，显著改善RLHF对齐效果并减少奖励黑客问题。
...
### [236] [Theoretically Optimal Attention/FFN Ratios in Disaggregated LLM Serving](https://arxiv.org/abs/2601.21351)
...
<details>
  <summary>Details</summary>
Motivation: AFD架构虽然能独立扩展内存和计算资源，但其性能对Attention/FFN资源配置比例高度敏感，配置不当会导致步骤级阻塞和昂贵的设备空闲时间，需要理论指导来优化资源配置。
...
Conclusion: 论文提出的分析框架为AFD架构资源配置提供了理论指导，通过优化A/F比例能有效提升系统吞吐量，减少资源浪费。
...
### [237] [Perceptrons and localization of attention's mean-field landscape](https://arxiv.org/abs/2601.21366)
*Antonio Álvarez-López,Borjan Geshkovski,Domènec Ruiz-Balet*

Main category: cs.LG

TL;DR: 将Transformer前向传播视为单位球面上的交互粒子系统，研究感知器块对系统临界点结构的影响，发现临界点通常是原子化且局部化的。


<details>
  <summary>Details</summary>
Motivation: 将Transformer前向传播建模为粒子系统，利用平均场极限和Wasserstein梯度流理论，研究感知器块如何影响系统临界点的几何结构。

Method: 将Transformer层视为时间，token嵌入视为粒子，层归一化理想化为单位球面。在特定权重设置下，系统可视为显式能量的梯度流，利用平均场极限和Wasserstein梯度流理论分析感知器块的影响。

Result: 感知器块导致临界点通常是原子化的（atomic）且局部化在球面的子集上，而非连续分布。

Conclusion: 感知器块对Transformer粒子系统的临界点结构有显著影响，使其呈现原子化和局部化特征，这为理解Transformer的动力学行为提供了新的理论视角。
...
### [238] [DA-SPS: A Dual-stage Network based on Singular Spectrum Analysis, Patching-strategy and Spearman-correlation for Multivariate Time-series Prediction](https://arxiv.org/abs/2601.21381)
...
Method: 提出DA-SPS模型，包含目标变量处理阶段(TVPS)和外生变量处理阶段(EVPS)。TVPS使用SSA分解目标序列，LSTM处理趋势分量，P-Conv-LSTM处理季节性分量。EVPS使用Spearman相关性分析筛选相关外生变量，通过L-Attention模块(LSTM+注意力机制)进行分析。最后加权求和并线性映射得到最终预测。

Result: 在四个公共数据集上，DA-SPS模型优于现有最先进方法。在自收集的笔记本电脑主板测试项目私有数据集上进一步验证了其在实际场景中的性能。

Conclusion: DA-SPS模型通过分离处理目标变量和外生变量，结合多种特征提取技术，有效提升了多元时间序列预测的准确性，在实际应用中表现出色。
...
### [239] [Intrinsic Reward Policy Optimization for Sparse-Reward Environments](https://arxiv.org/abs/2601.21391)
*Minjae Cho,Huy Trong Tran*

Main category: cs.LG

TL;DR: 提出IRPO算法，通过多个内在奖励直接优化策略，无需预训练子策略，在稀疏奖励环境中提供更有信息量的学习信号
...
Conclusion: IRPO通过多个内在奖励直接优化策略，有效解决了稀疏奖励环境中的探索问题，避免了现有方法的信用分配不稳定和样本效率低的问题
...
### [240] [Mitigating Overthinking in Large Reasoning Models via Difficulty-aware Reinforcement Learning](https://arxiv.org/abs/2601.21418)
*Qian Wan,Ziao Xu,Luona Wei,Xiaoxuan Shen,Jianwen Sun*

Main category: cs.LG

TL;DR: 提出DiPO框架，通过强化学习让大推理模型自主感知任务难度并调整推理开销，减少简单任务中的过度思考现象


<details>
  <summary>Details</summary>
Motivation: 大推理模型在处理简单任务时容易出现过度思考现象，导致不必要的冗长推理和资源浪费。现有研究低估了任务难度感知的重要性，难以有效分配推理资源

Method: 提出难度感知策略优化框架，包括基于模型自推理的难度建模方法和难度信号增强的奖励函数，通过强化学习调整后训练引入的生成偏好

Result: DiPO使模型能够自主调整推理开销，在保持性能的同时显著减少冗余token，实现思维压缩而不损失性能

Conclusion: DiPO框架通过强化学习让大推理模型自主感知任务难度并优化推理过程，有效解决了过度思考问题，提高了推理效率
...
### [241] [Revisiting Diffusion Model Predictions Through Dimensionality](https://arxiv.org/abs/2601.21419)
*Qing Jin,Chaoyang Wang*

Main category: cs.LG

TL;DR: 该论文提出了一个理论框架，解释为什么在扩散和流匹配模型中，最优预测目标（噪声ε、速度v或数据x）取决于数据特性，特别是当环境维度远大于数据内在维度时，x预测更优。作者还提出了k-Diff框架，通过数据驱动方法自动学习最优预测参数k，无需显式估计内在维度。


<details>
  <summary>Details</summary>
Motivation: 当前扩散和流匹配模型的研究趋势显示，预测目标正从噪声(ε)和速度(v)转向直接数据(x)预测，尤其是在高维设置中。然而，缺乏正式的理论解释为什么最优预测目标取决于数据的特定属性。需要建立一个理论框架来理解数据几何特性与最优预测目标之间的关系，并提供实际可用的方法来自动确定最优预测目标。

Method: 1. 提出了一个广义预测公式，可以容纳任意输出目标，其中ε-、v-和x-预测都是特例。2. 推导了数据几何特性与最优预测目标之间的解析关系，从理论上证明当环境维度显著超过数据内在维度时，x预测更优。3. 提出了k-Diff框架，采用数据驱动方法直接从数据中学习最优预测参数k，避免了显式维度估计的需要。

Result: 1. 理论分析表明，维度是决定最优预测目标的主导因素。2. k-Diff框架在潜在空间和像素空间的图像生成实验中，在不同架构和数据规模下，始终优于固定目标的基线方法。3. 提供了原则性和自动化的方法来提升生成性能。

Conclusion: 该工作为扩散和流匹配模型中的预测目标选择提供了理论依据，证明了数据内在维度与环境维度的相对关系决定了最优预测目标。提出的k-Diff框架通过数据驱动方法自动学习最优预测参数，为实际应用提供了有效的解决方案，无需显式估计难以获取的数据内在维度。
...
### [242] [ConceptMoE: Adaptive Token-to-Concept Compression for Implicit Compute Allocation](https://arxiv.org/abs/2601.21420)
*Zihao Huang,Jundong Zhou,Xingwei Qu,Qiyang Min,Ge Zhang*

Main category: cs.LG

TL;DR: ConceptMoE通过动态合并语义相似token为概念表示，实现隐式token级计算分配，在保持计算量和参数不变条件下提升性能并减少注意力计算


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型对所有token采用均匀计算分配，忽略了某些序列是简单可预测的而其他需要深度推理。需要一种能动态分配计算资源的方法来提高效率和效果

Method: 引入ConceptMoE：1) 可学习分块模块通过测量token间相似性识别最优边界，按目标压缩比R压缩序列；2) MoE架构实现受控评估，将节省的计算重新分配到匹配基线FLOPs和参数；3) 动态合并语义相似token为概念表示，执行隐式token级计算分配

Result: 在语言和视觉语言任务上持续优于标准MoE：语言预训练+0.9点，长上下文理解+2.3点，多模态基准+0.6点。使用层循环持续训练时增益达+5.5点。减少注意力计算达R²倍，KV缓存达R倍。R=2时，预填充速度提升175%，解码速度提升117%

Conclusion: ConceptMoE通过自适应概念级处理，在保持计算和参数不变条件下，显著提升大语言模型的效果和效率。最小架构修改便于集成到现有MoE中，证明自适应概念处理能根本改进大语言模型
...
### [243] [From Consistency to Complementarity: Aligned and Disentangled Multi-modal Learning for Time Series Understanding and Reasoning](https://arxiv.org/abs/2601.21436)
*Hang Ni,Weijia Zhang,Fei Wang,Zezhi Shao,Hao Liu*

Main category: cs.LG

TL;DR: MADI是一种增强的多模态大语言模型，通过细粒度对齐和解缠交互解决时间序列跨模态集成中的对齐和语义纠缠问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在时间序列理解中面临两个主要挑战：跨模态的细粒度时间对齐困难，以及共享语义和模态特定语义之间的严重纠缠，这阻碍了局部化解释和互补推理

Method: 提出MADI模型，包含三个关键技术：(1) 补丁级对齐：在异构模态间建立物理基础的细粒度对应关系；(2) 离散解缠交互：将模态共同语义分离到紧凑的离散潜在空间中，并自适应地协同纯化的模态独特信息；(3) 关键令牌突出：强调信息丰富、查询相关的信号以增强鲁棒推理

Result: 在合成和真实世界基准测试中，MADI始终优于通用大语言模型和时间序列专用的多模态大语言模型

Conclusion: MADI通过细粒度对齐和解缠交互机制，有效解决了时间序列多模态理解中的跨模态集成挑战，为时间序列的自然语言查询和推理提供了更强大的解决方案
...
### [244] [SAGE: Sequence-level Adaptive Gradient Evolution for Generative Recommendation](https://arxiv.org/abs/2601.21452)
*Yu Xie,Xing Kai Ren,Ying Qi,Hu Yao*

Main category: cs.LG

TL;DR: SAGE提出序列级自适应梯度演化框架，解决OneRec中词汇表依赖和对称保守主义问题，实现无需单独词汇表的LLM推荐系统优化


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的推荐系统如OneRec需要构建单独的词汇表，导致维护成本高、可扩展性差；同时其GBPO优化策略存在"对称保守主义"问题，抑制冷启动项目更新动量且无法防止高噪声环境中的多样性崩溃

Method: 提出SAGE统一优化框架：1）序列级信号解耦：结合几何平均重要性比率和解耦多目标优势，消除令牌级方差，解决"奖励崩溃"问题；2）非对称自适应动态：构建动态梯度流形，对高潜力冷启动项目应用"提升因子"实现超线性更新，采用"熵感知惩罚"打破信息茧房

Result: 理论分析和实证结果表明，SAGE有效解锁冷启动流量并维持推荐多样性，同时保持了GBPO的数值稳定性

Conclusion: SAGE框架成功解决了LLM推荐系统中词汇表依赖和优化策略缺陷，实现了无需单独词汇表的架构重用，并通过自适应梯度演化机制提升了冷启动性能和多样性保持能力
...
### [245] [HER: Human-like Reasoning and Reinforcement Learning for LLM Role-playing](https://arxiv.org/abs/2601.21459)
...
TL;DR: HER框架通过双层思维机制和强化学习，实现了LLM角色扮演中的认知级人物模拟，显著提升了角色内在思维模拟能力


<details>
  <summary>Details</summary>
Motivation: 当前LLM角色扮演主要关注角色语气和知识，但模拟行为背后的内在思维仍具挑战。现有方法缺乏高质量推理轨迹数据和对齐人类偏好的可靠奖励信号

Method: 提出HER统一框架，引入双层思维机制区分角色第一人称思维和LLM第三人称思维。通过逆向工程构建推理增强的角色扮演数据，建立人类对齐原则和奖励模型，基于Qwen3-32B进行监督学习和强化学习训练

Result: 模型显著优于Qwen3-32B基线，在CoSER基准上提升30.26分，在Minimax Role-Play Bench上提升14.97分。验证了方法的有效性

Conclusion: HER框架成功解决了LLM角色扮演中的认知模拟挑战，通过双层思维机制和强化学习实现了更真实的角色内在思维模拟，为未来研究提供了数据集、原则和模型资源
...
### [246] [L$^3$: Large Lookup Layers](https://arxiv.org/abs/2601.21461)
*Albert Tseng,Christopher De Sa*

Main category: cs.LG

TL;DR: L³（大型查找层）是一种新的稀疏模型架构，通过静态基于令牌的路由将嵌入表泛化到解码器层，避免了MoE的动态路由问题，在保持硬件效率的同时实现了更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现代稀疏语言模型通常使用MoE层实现稀疏性，但动态硬路由存在硬件效率低、需要辅助损失函数稳定训练等问题。嵌入表虽然天然稀疏且避免了这些问题，但缺乏上下文信息。需要一种既能保持硬件效率又能提供上下文感知的稀疏架构。

Method: L³层通过静态令牌路由将嵌入表泛化到解码器层，包含两个核心组件：1）系统友好的架构，支持快速训练和无开销的CPU卸载推理；2）基于信息论的嵌入分配算法，有效平衡速度和质量。模型使用静态令牌路由以上下文相关的方式聚合每个令牌的学习嵌入。

Result: 在训练包含26亿活跃参数的Transformer模型时，L³在语言建模和下游任务中都显著优于密集模型和同等稀疏度的MoE模型。

Conclusion: L³通过解锁新的稀疏维度，提供了一种硬件高效、训练稳定的稀疏架构替代方案，在保持计算效率的同时实现了更好的性能表现。
...
### [247] [A block-coordinate descent framework for non-convex composite optimization. Application to sparse precision matrix estimation](https://arxiv.org/abs/2601.21467)
*Guillaume Lauga*
...
<details>
  <summary>Details</summary>
Motivation: 块坐标下降（BCD）方法在大规模优化问题中广泛应用，但其在非凸优化中的理论研究相对不足。作者希望开发一个通用的BCD框架来处理非凸复合优化问题，特别是针对稀疏精度矩阵估计（Graphical Lasso）这类重要应用。

Method: 提出一个新的块坐标下降（BCD）框架，该框架足够通用，能够包含变量度量近端梯度更新、近端牛顿更新和交替最小化更新。该框架确保了目标函数的单调下降和收敛性，并特别应用于稀疏精度矩阵估计问题，涵盖了graphical ISTA、Primal GLasso和QUIC三种最常用求解器。

Result: 新框架在非凸稀疏精度矩阵估计问题上表现出色，提供了收敛性保证，并且与现有方法相比，达到最先进估计质量所需的迭代次数减少了高达100倍。

Conclusion: 该研究提出了一个通用的块坐标下降框架，成功解决了非凸复合优化问题，特别是在稀疏精度矩阵估计应用中显著提升了计算效率，为相关优化问题提供了强有力的理论保证和实用工具。
...
### [248] [ETS: Energy-Guided Test-Time Scaling for Training-Free RL Alignment](https://arxiv.org/abs/2601.21484)
*Xiuyu Li,Jinkai Zhang,Mingyang Yi,Yu Li,Longqiang Wang,Yue Wang,Ju Fan*

Main category: cs.LG

TL;DR: 提出一种无需训练的推理方法ETS，通过能量引导的测试时缩放直接从最优RL策略中采样，显著降低推理延迟并提升生成质量


<details>
  <summary>Details</summary>
Motivation: RL后训练对齐虽然有效但成本高且训练过程不稳定，需要一种更高效稳定的替代方法

Method: ETS算法：基于掩码语言建模的转移概率（参考策略模型+能量项），通过在线蒙特卡洛估计能量项，结合重要性采样估计器降低推理延迟

Result: 在推理、编码和科学基准测试中，ETS在自回归模型和扩散语言模型上均能一致提升生成质量

Conclusion: ETS作为一种无需训练的推理方法，能够直接从最优RL策略中高效采样，在保持采样质量的同时显著降低推理成本
...
### [249] [Task-Awareness Improves LLM Generations and Uncertainty](https://arxiv.org/abs/2601.21500)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM解码和不确定性估计方法仅在语言空间操作，忽视了自然语言响应背后的结构化信息（如离散标签、数值、图结构），导致无法充分利用任务相关的潜在结构

Method: 为LLM输出建立任务相关的潜在结构模型，配备相异度度量，计算贝叶斯最优响应（通过潜在空间合成而非从采样生成中选择），并通过诱导的贝叶斯风险量化结构感知的不确定性

Result: 贝叶斯最优响应在不同任务中持续优于波束搜索等标准解码方法；基于潜在结构的贝叶斯风险不确定性量化能更好地捕捉输出质量和正确性的变化

Conclusion: 该决策理论框架适用于任何具有潜在响应结构的问题，能够实现可靠的任务感知LLM预测，将LLM输出建模从纯语言空间扩展到任务相关的结构化空间
...
### [250] [Cascaded Transfer: Learning Many Tasks under Budget Constraints](https://arxiv.org/abs/2601.21513)
*Eloi Campagne,Yvenn Amara-Ouali,Yannig Goude,Mathilde Mougeot,Argyris Kalogeratos*

Main category: cs.LG

TL;DR: 提出了一种名为"级联迁移学习"的新型多任务学习范式，通过层次化级联方式在大量相关任务间传递信息，同时满足预算约束


<details>
  <summary>Details</summary>
Motivation: 在多任务学习场景中，当需要学习大量相关任务且任务间确切关系未知时，如何有效且经济地在任务间迁移信息是一个重要挑战。现有方法在处理大规模任务集合时可能效率不高或成本过高。

Method: 提出级联迁移学习范式：1) 将任务组织为根树结构，指定学习顺序；2) 基于合适距离度量构建最小生成树连接任务；3) 在树分支上分配可用训练预算；4) 信息（如模型参数）通过相同类别的个体模型在任务间层次化级联传递。

Result: 在合成和真实多任务设置上的实验表明，该方法相比替代方法能够实现更准确且成本效益更高的跨大型任务集合的适应。

Conclusion: 级联迁移学习为大规模相关任务学习提供了一种有效的解决方案，通过层次化信息传递和预算优化，显著提升了多任务学习的准确性和成本效益。
...
### [251] [A Unified SPD Token Transformer Framework for EEG Classification: Systematic Comparison of Geometric Embeddings](https://arxiv.org/abs/2601.21521)
...
<details>
  <summary>Details</summary>
Motivation: EEG信号的空间协方差矩阵是SPD矩阵，位于黎曼流形上，但嵌入几何选择与优化动力学之间的理论联系尚未被探索。论文旨在形式化分析不同嵌入选择（特别是BWSPD和Log-Euclidean）对梯度条件和数值稳定性的影响，为SPD流形上的深度学习提供理论指导。

Method: 1. 理论分析：建立三个理论结果：(1) BWSPD的√κ梯度条件优于Log-Euclidean的κ条件；(2) 嵌入空间批归一化近似黎曼归一化；(3) BWSPD令牌保持流形距离的bi-Lipschitz界限。2. 实验验证：构建统一Transformer框架，在三个EEG范式（运动想象、ERP、SSVEP）上进行1500+次实验，比较BWSPD、Log-Euclidean和欧几里得嵌入。

Result: 1. 理论预测得到验证：BWSPD在高维输入（d≥22）上梯度条件更好，但在低维输入（d≤8）上特征分解开销占主导。2. 嵌入空间批归一化在56通道ERP数据上提升26%准确率，但在8通道SSVEP数据上效果可忽略。3. Log-Euclidean Transformer在所有数据集上达到SOTA性能，显著优于传统黎曼分类器和近期SPD基线，BWSPD提供竞争性准确率且训练时间相似。

Conclusion: 该研究首次建立了SPD流形嵌入几何与优化动力学之间的形式化联系，为选择适合特定EEG应用的嵌入提供了理论依据。Log-Euclidean嵌入在实践中表现最佳，而BWSPD在理论梯度条件方面有优势。嵌入空间批归一化的有效性取决于通道数量，为EEG信号处理中的嵌入选择提供了实用指导。
...
### [252] [Explicit Credit Assignment through Local Rewards and Dependence Graphs in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2601.21523)
...
<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，全局奖励包含所有智能体的贡献，存在噪声问题且需要信用分配；局部奖励虽然学习更快但可能导致智能体短视地优化自身奖励而忽略全局最优性。需要结合两种方法的优点。

Method: 提出使用智能体交互图的方法，比全局奖励更精细地区分个体智能体贡献，同时缓解局部奖励的合作问题。还引入了近似这种交互图的实用方法。

Result: 实验证明该方法具有灵活性，能够超越传统的局部和全局奖励设置，实现性能改进。

Conclusion: 通过结合全局奖励和局部奖励的优势，利用智能体交互图精细分配信用，可以在多智能体强化学习中实现更好的合作和学习效果。
...
### [253] [Multi-Modal Time Series Prediction via Mixture of Modulated Experts](https://arxiv.org/abs/2601.21547)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列具有复杂演化动态，准确预测极具挑战。现有多模态方法依赖token级融合，但在高质量时-文对稀缺、时间序列尺度特征差异大时效果不佳。MoE架构虽有效，但现有模态集成方法仍依赖token级融合

Method: 提出Expert Modulation范式，基于文本信号同时调节专家路由和专家计算，实现直接高效的跨模态专家行为控制

Result: 通过全面的理论分析和实验验证，该方法在多模态时间序列预测中展现出显著改进

Conclusion: Expert Modulation为多模态时间序列预测提供了新的有效范式，通过文本信号直接调节专家行为，克服了token级融合的局限性
...
### [254] [HistoPrism: Unlocking Functional Pathway Analysis from Pan-Cancer Histology via Gene Expression Prediction](https://arxiv.org/abs/2601.21560)
*Susu Hu,Qinghe Zeng,Nithya Bhasker,Jakob Nicolas Kather,Stefanie Speidel*

Main category: cs.LG

TL;DR: HistoPrism：基于Transformer的高效架构，用于从H&E组织学图像中预测空间基因表达，具有跨癌症泛化能力和生物学通路级评估


<details>
  <summary>Details</summary>
Motivation: 从H&E组织学预测空间基因表达为测序提供了可扩展且临床可及的替代方案，但现有方法通常局限于单一癌症类型，且评估主要基于基因水平方差，缺乏对生物学功能相关性的深入探索

Method: 提出HistoPrism，一种基于Transformer的高效架构，用于从组织学图像中进行泛癌基因表达预测；引入通路级基准评估，将评估重点从孤立基因水平方差转向连贯的功能通路

Result: HistoPrism不仅在高变异基因预测上超越先前最先进模型，更重要的是在通路级预测上取得显著提升，证明其能够恢复生物学上连贯的转录组模式；具有强大的泛癌泛化能力和改进的效率

Conclusion: HistoPrism为从常规可得的组织学图像进行临床相关转录组建模建立了新标准，实现了跨癌症类型的泛化并捕获生物学连贯信号
...
### [255] [Representation Unlearning: Forgetting through Information Compression](https://arxiv.org/abs/2601.21564)
*Antonio Almudévar,Alfonso Ortega*

Main category: cs.LG

TL;DR: 提出Representation Unlearning框架，直接在表示空间进行遗忘学习，而非修改模型参数，通过信息瓶颈最大化保留数据信息同时抑制待遗忘数据信息


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法通常修改模型参数，但存在更新不稳定、计算成本高、受限于局部近似等问题，需要更可靠、高效且实用的遗忘方法

Method: 在模型表示空间学习变换，施加信息瓶颈：最大化与保留数据的互信息，同时抑制与待遗忘数据的互信息；推导变分替代使目标可处理，并在两种实际场景中实例化：保留和遗忘数据均可用时，以及仅能访问遗忘数据的零样本设置
...
### [256] [FlexCausal: Flexible Causal Disentanglement via Structural Flow Priors and Manifold-Aware Interventions](https://arxiv.org/abs/2601.21567)
*Yutao Jin,Yuang Tao,Junyong Zhai*

Main category: cs.LG

TL;DR: FlexCausal是一种基于块对角协方差VAE的因果解耦表示学习框架，通过因子化流先验建模复杂外生噪声分布，结合监督对齐和反事实一致性约束，实现高保真生成。


<details>
  <summary>Details</summary>
Motivation: 现有解耦方法依赖均值场近似和对角后验协方差，解耦所有潜在维度；同时假设外生噪声为各向同性高斯分布，无法捕捉现实世界中因果因素的复杂非高斯统计特性。
...
Conclusion: FlexCausal通过块对角协方差VAE和因子化流先验，有效分离因果机制学习和分布统计，实现了更精确的因果结构对应关系和高质量生成。
...
### [257] [Bridging Functional and Representational Similarity via Usable Information](https://arxiv.org/abs/2601.21568)
*Antonio Almudévar,Alfonso Ortega*

Main category: cs.LG

TL;DR: 提出一个基于"可用信息"的统一框架，从三个维度量化表示相似性：功能相似性、表示相似性及其关系，建立任务粒度层次结构


<details>
  <summary>Details</summary>
Motivation: 现有表示相似性度量方法缺乏统一的理论框架，需要建立功能相似性和表示相似性之间的形式化联系，并揭示它们在不同观测者能力下的相对性

Method: 基于"可用信息"概念，建立stitching性能与条件互信息的理论联系；证明重建类度量和标准工具（CKA、RSA）在特定约束下是可用信息的估计器；通过任务粒度层次结构统一概念

Result: stitching具有内在不对称性，需要双向分析；表示相似性相对于预测族能力具有相对性；表示相似性是功能相似性的充分但不必要条件；建立了任务粒度层次结构

Conclusion: 提出了一个统一的表示相似性量化框架，揭示了功能相似性和表示相似性之间的理论联系，强调了相似性的相对性和观测者能力的重要性，为表示分析提供了理论基础
...
### [258] [Learning the Mechanism of Catastrophic Forgetting: A Perspective from Gradient Similarity](https://arxiv.org/abs/2601.21577)
*Mutian Yang,Zisen Zhan,Yutong Chen,Haolin Li,Kaiwen Wang,Kaili Zheng,Yuguang Wang,Qi Wang,Jiandong Gao,Ji Wu*

Main category: cs.LG

TL;DR: 提出基于梯度相似性的理论框架解释灾难性遗忘，识别冲突神经元和协作神经元，并提出协作神经学习方法，在多种设置下显著减少遗忘。


<details>
  <summary>Details</summary>
Motivation: 知识注入过程中的灾难性遗忘严重削弱了大语言模型的持续学习能力，现有方法缺乏理论基础解释遗忘现象。

Method: 建立基于梯度的理论框架，证明强负梯度相似性是遗忘的根本原因；识别冲突神经元和协作神经元；提出协作神经学习方法，冻结冲突神经元，仅更新协作神经元。

Result: 在5个LLM、4个数据集和4个优化器上的实验表明，CNL在in-set设置下实现零遗忘，在out-of-set设置下减少59.1%-81.7%的遗忘。

Conclusion: 梯度相似性理论框架为理解灾难性遗忘提供了理论基础，协作神经学习方法通过选择性更新神经元有效缓解遗忘问题。
...
### [259] [Evaluating Prediction Uncertainty Estimates from BatchEnsemble](https://arxiv.org/abs/2601.21581)
...
Conclusion: BatchEnsemble和GRUBE相比传统集成方法，以更少的参数和更低的训练推理时间实现了相似的性能，为不确定性估计提供了高效解决方案
...
### [260] [CORDS: Continuous Representations of Discrete Structures](https://arxiv.org/abs/2601.21583)
...
Method: 提出CORDS方法，通过可逆映射将空间对象集合转换为连续场：密度场编码对象位置和数量，特征场在同一支撑域上携带对象属性。由于映射可逆，模型完全在场空间中操作，同时保持可精确解码为离散集合。

Result: 在分子生成与回归、目标检测、基于模拟的推理以及局部极大值恢复的数学任务上评估CORDS，证明其能够稳健处理未知集合大小，并具有竞争力的准确性。

Conclusion: CORDS为可变大小集合预测提供了一种新颖的连续表示方法，通过可逆映射将离散结构转化为连续场，有效解决了传统方法在处理未知集合大小时面临的挑战。
...
### [261] [Scalable Power Sampling: Unlocking Efficient, Training-Free Reasoning for LLMs via Distribution Sharpening](https://arxiv.org/abs/2601.21590)
*Xiaotong Ji,Rasul Tutunov,Matthieu Zimmer,Haitham Bou Ammar*

Main category: cs.LG

TL;DR: 提出一种无需外部奖励的训练免费方法，通过token级缩放低温分布近似全局幂分布，替代计算昂贵的MCMC采样，在推理延迟降低10倍的同时达到或超越RL后训练效果


<details>
  <summary>Details</summary>
Motivation: RL后训练虽然能提升LLMs推理性能，但其增益主要来自分布锐化而非新能力获取。现有MCMC方法虽能实现类似效果但计算成本过高，需要更实用的替代方案

Method: 提出理论推导：全局幂分布可由token级缩放低温分布近似，其中缩放因子捕获未来轨迹质量。基于此设计训练免费、无需验证器的算法，自回归地锐化基础模型生成分布

Result: 在数学、QA和代码任务上评估四种LLMs，该方法匹配或超越one-shot GRPO且不依赖外部奖励，相比MCMC采样推理延迟降低超过10倍

Conclusion: 提供了一种理论严谨且实用的替代方案，无需迭代MCMC即可实现RL后训练的分布锐化效果，显著降低计算成本，为LLMs推理性能提升提供了高效路径
...
### [262] [Visual-Guided Key-Token Regularization for Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2601.22020)
*Chengyi Cai,Zesheng Ye,Peike Li,Bo Han,Jianzhong Qi,Feng Liu*

Main category: cs.LG

TL;DR: 提出ViKeR方法，通过视觉引导的关键令牌正则化实现多模态大语言模型的安全遗忘，重点关注答案中关键令牌的识别与处理


<details>
  <summary>Details</summary>
Motivation: 现有MLLM遗忘方法主要沿用LLM技术，存在两个问题：1) 对所有答案令牌一视同仁，忽视了它们在遗忘过程中的重要性差异；2) 仅关注语言模态，忽略了视觉线索对关键令牌的指示作用

Method: 提出视觉引导的关键令牌正则化(ViKeR)：1) 利用无关视觉输入预测理想的后遗忘令牌级分布；2) 用这些分布正则化遗忘过程，优先处理关键令牌；3) 基于信息熵定义关键令牌；4) 通过令牌级梯度重加权放大关键令牌的更新

Result: 在MLLMU和CLEAR基准测试中，ViKeR方法有效执行遗忘任务，同时减轻遗忘副作用并保持响应连贯性

Conclusion: ViKeR通过视觉引导的关键令牌正则化解决了MLLM遗忘中的令牌重要性差异问题，实现了更有效的隐私保护遗忘，同时保持模型性能
...
### [263] [Breaking the Overscaling Curse: Thinking Parallelism Before Parallel Thinking](https://arxiv.org/abs/2601.21619)
*Yiming Wang,Zhuosheng Zhang,Rui Wang*

Main category: cs.LG

TL;DR: 论文提出"过度缩放诅咒"问题：在并行思维LLM推理中，系统级分配固定并行度N会导致样本级效率低下，因为不同样本所需最优并行度N'不同。作者提出T2方法，利用潜在表示预测每个样本的最优并行度，在保持性能的同时显著降低成本。


<details>
  <summary>Details</summary>
Motivation: 现有并行思维方法在系统层面为所有样本分配相同的全局并行度N，通常设置较大值以最大化数据集整体准确率。但由于样本异质性，许多样本可以用更小的N'达到可比性能，导致计算资源冗余。这种系统级效能与样本级效率之间的不兼容构成了"过度缩放诅咒"。

Method: 提出T2方法：1）形式化并量化过度缩放诅咒，分析其触发机制；2）利用潜在表示在解码前估计每个样本的最优并行度；3）轻量级设计，避免额外计算开销。

Result: 实验表明T2能显著降低成本同时保持可比性能，实现更高效的并行思维。具体量化了过度缩放诅咒的普遍性和严重性，验证了T2方法的有效性。

Conclusion: 过度缩放诅咒是并行思维中的普遍问题，T2方法通过样本级自适应并行度分配打破了这一诅咒，在保持性能的同时提高了计算效率，为高效并行推理提供了新思路。
...
### [264] [LAMP: Look-Ahead Mixed-Precision Inference of Large Language Models](https://arxiv.org/abs/2601.21623)
*Stanislav Budzinskiy,Marian Gloser,Tolunay Yilmaz,Ying Hong Tham,Yuanyi Lin,Wenyi Fang,Fan Wu,Philipp Petersen*

Main category: cs.LG

TL;DR: 该论文提出了一种针对Transformer推理的自适应混合精度计算策略，通过选择性地对关键组件进行高精度计算，在保持低精度计算效率的同时显著提升整体精度。


<details>
  <summary>Details</summary>
Motivation: 混合精度计算是当前AI发展的关键特征，特别是对于大型语言模型的本地部署。然而，在计算复合函数时，如何平衡计算效率和精度是一个重要挑战。论文旨在解决Transformer推理中复合函数的浮点计算问题，通过分析舍入误差来优化精度分配。

Method: 基于复合函数f(g(x))的舍入误差分析，提出自适应策略：选择g(x)的一小部分关键组件进行高精度计算，而其他大部分计算保持低精度。将该策略应用于Transformer中的不同复合结构，并分析其对推理的整体影响。

Result: 在GPT-2模型上的数值实验表明，即使非常低的重计算率（即只有少量组件需要高精度计算）也能带来高达两个数量级的精度提升。

Conclusion: 该自适应混合精度策略能够有效平衡Transformer推理的计算效率和精度，为大型语言模型的本地部署提供了实用的精度优化方案。
...
### [265] [Seg-MoE: Multi-Resolution Segment-wise Mixture-of-Experts for Time Series Forecasting Transformers](https://arxiv.org/abs/2601.21641)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的时间序列预测模型在扩展效率和捕捉长期时间动态方面存在困难。虽然MoE层在NLP中解决了扩展问题，但现有时间序列MoE方法采用token-wise路由机制，未能充分利用时间数据的自然局部性和连续性特征。
...
### [266] [SWE-Spot: Building Small Repo-Experts with Repository-Centric Learning](https://arxiv.org/abs/2601.21649)
*Jinjun Peng,Magnus Saebo,Tianjun Zhong,Yi-Jie Cheng,Junfeng Yang,Baishakhi Ray,Simin Chen,Yangruibo Ding*

Main category: cs.LG

TL;DR: 提出Repository-Centric Learning (RCL)范式，通过深度掌握特定代码库的"物理特性"而非广度任务学习，训练出SWE-Spot-4B模型，在代码任务上超越更大规模开源模型并匹敌商业模型效率。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型在隐私敏感和资源受限环境中部署需求增加，但面临根本能力差距：缺乏对复杂陌生代码库的强泛化能力。传统任务中心学习范式通过扩展不同代码库暴露无法解决此问题。
...
Result: SWE-Spot-4B打破现有扩展趋势，超越更大规模开源模型（如Meta的CWM、Qwen3-Coder-30B），在多个软件工程任务上达到或超过效率导向的商业模型（如GPT-4.1-mini、GPT-5-nano）。RCL展现出更高的训练样本效率和更低的推理成本。

Conclusion: 构建高效智能需要代码库掌握这一独特且必要的维度来补充通用编码能力。RCL范式通过参数化知识获取而非昂贵的推理时搜索，使小型语言模型能够内化目标软件环境的"物理特性"。
...
### [267] [SENDAI: A Hierarchical Sparse-measurement, EfficieNt Data AssImilation Framework](https://arxiv.org/abs/2601.21664)
*Xingyue Zhang,Yuxuan Bao,Mars Liyao Gao,J. Nathan Kutz*

Main category: cs.LG

TL;DR: SENDAI框架通过结合模拟先验与学习偏差校正，从超稀疏传感器观测中重建完整空间状态，在分布偏移、异构结构和多尺度动态条件下显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 解决数据丰富训练与观测稀疏部署之间的差距，特别是在目标域存在分布偏移、异构结构和多尺度动态变化的情况下，这些特征在可用训练数据中可能缺失。

Method: 提出SENDAI分层稀疏测量高效数据同化框架，结合模拟衍生的先验知识与学习得到的偏差校正，从超稀疏传感器观测中重建完整空间状态。

Result: 在六个全球分布的站点上重建MODIS植被指数场，相比传统基线最大SSIM提升185%，相比近期高频方法提升36%，特别在具有尖锐边界和亚季节性动态的景观中表现突出，有效保留了诊断相关结构。

Conclusion: SENDAI提供了一个轻量级且操作可行的稀疏测量重建框架，适用于物理基础推理、资源有限部署和实时监控控制，重建场更适合下游间接观测变量的推断。
...
### [268] [Expected Return Causes Outcome-Level Mode Collapse in Reinforcement Learning and How to Fix It with Inverse Probability Scaling](https://arxiv.org/abs/2601.21669)
*Abhijeet Sinha,Sundari Elango,Dianbo Liu*

Main category: cs.LG

TL;DR: 论文揭示了强化学习中多模态崩溃的根本原因在于期望回报目标本身的结构性缺陷，并提出逆概率缩放修正方法，通过IPS-GRPO实现奖励比例分布，有效防止多模态崩溃。


<details>
  <summary>Details</summary>
Motivation: 许多强化学习问题存在多个质量相当的最优解，但标准期望回报最大化训练的策略通常会崩溃到少数解上。传统解释归因于探索不足或正则化弱，但本文认为这种解释不完整，需要揭示多模态崩溃的结构性根源。

Method: 识别期望回报目标中的概率乘子是问题的根源，提出逆概率缩放修正方法，消除学习信号中的结果频率放大效应。将此原则实例化为IPS-GRPO（Group Relative Policy Optimization的改进版），作为即插即用修改，无需辅助模型或架构变更。

Result: 在理想化学习动态下，任意两个结果的概率对数比线性演化于其奖励差异，导致指数级比率发散和不可避免的崩溃。IPS-GRPO能够产生奖励比例分布的终端分布，防止多模态崩溃。在不同推理和分子生成任务中，IPS-GRPO持续减少结果级模式崩溃，同时匹配或超越基线性能。

Conclusion: 多模态崩溃是期望回报目标本身的结构性后果，而非探索不足。通过逆概率缩放修正目标函数而非添加探索启发式方法，是可靠多模态策略优化的关键。IPS-GRPO提供了一种简单有效的解决方案。
...
### [269] [LLM4Fluid: Large Language Models as Generalizable Neural Solvers for Fluid Dynamics](https://arxiv.org/abs/2601.21681)
...
TL;DR: LLM4Fluid：利用大型语言模型作为通用神经求解器进行流体动力学时空预测的框架，通过降阶建模和模态对齐实现零样本泛化


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在流体动力学建模中存在泛化能力有限、对新场景需要重新训练的问题，需要开发能够零样本泛化的通用神经求解器
...
Conclusion: LLM4Fluid成功将LLM应用于流体动力学预测，通过模态对齐和降阶建模解决了跨模态表示问题，为通用物理系统建模提供了新范式
...
### [270] [XFACTORS: Disentangled Information Bottleneck via Contrastive Supervision](https://arxiv.org/abs/2601.21688)
*Alexandre Myara,Nicolas Bourriez,Thomas Boyer,Thomas Lemercier,Ihab Bendidi,Auguste Genovesio*

Main category: cs.LG

TL;DR: XFactors是一种弱监督VAE框架，通过对比监督和KL正则化实现特定因子的解耦和控制，避免了对抗训练和分类器依赖。


<details>
  <summary>Details</summary>
Motivation: 完全无监督方法在合成数据上成功但难以从真实数据中恢复语义因子，而监督方法由于依赖对抗目标或辅助分类器，存在不稳定性和可扩展性问题。

Method: 基于解耦信息瓶颈视角，将表示分解为残差子空间S和因子特定子空间T₁,...,Tₖ。通过InfoNCE损失进行对比监督，将相同因子值的潜在表示拉近，不匹配的推远。同时使用KL正则化对S和聚合因子子空间施加高斯结构。

Result: 在多个数据集上，XFactors以恒定超参数实现了最先进的解耦分数，在相应子空间中产生一致的定性因子对齐，支持通过潜在替换进行受控因子交换。方法能正确随潜在容量增加而扩展，并在CelebA真实数据集上得到验证。

Conclusion: XFactors提供了一种稳定、可扩展的弱监督解耦方法，避免了对抗训练和分类器依赖，实现了对选定因子的解耦和显式控制。
...
### [271] [Understanding Model Merging: A Unified Generalization Framework for Heterogeneous Experts](https://arxiv.org/abs/2601.21690)
*Qinglun Li,Anke Tang,Miao Zhang,Mengzhu Wang,Quanjun Yin,Li Shen*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的理论框架，用于解释模型合并在不同异构微调超参数下的有效性，并为实践者提供了构建合并友好模型的实用建议。


<details>
  <summary>Details</summary>
Motivation: 模型合并虽然能有效聚合多个微调模型的能力，但在异构微调超参数（如不同学习率、批量大小）下的统一理论仍然缺失。同时，开源微调模型缺乏超参数透明度，使得难以预测合并模型性能，实践者缺乏指导来微调合并友好的专家模型。

Method: 采用L2-稳定性理论在异构超参数环境下分析合并模型x_avg的泛化性能。通过理论分析建立统一框架来解释现有合并算法，并提出实践建议来构建合并友好模型。

Result: 在ResNet/ViT系列上进行大量实验，涵盖20/8个视觉分类任务，涉及数千个微调模型，结果稳健地证实了不同超参数对x_avg泛化性能的影响，与理论预测一致。

Conclusion: 该研究首次为异构超参数下的模型合并提供了统一的理论框架，揭示了现有合并算法如何优化理论边界中的特定项，并为实践者提供了在预训练到微调流程中构建合并友好模型的具体建议。
...
### [272] [Curriculum Learning for LLM Pretraining: An Analysis of Learning Dynamics](https://arxiv.org/abs/2601.21698)
*Mohamed Elgaar,Hadi Amiri*

Main category: cs.LG

TL;DR: 课程学习主要改变训练数据在固定学习轨迹内的暴露顺序，而非创造新的学习阶段，通过降低梯度噪声和输出头谱饱和度来提升小模型性能，但在大模型中收益减小。


<details>
  <summary>Details</summary>
Motivation: 研究课程学习是否真正改变模型的学习轨迹，还是仅仅重新排列数据在固定轨迹上的暴露顺序，并探究课程学习在不同规模模型中的优化效果差异。

Method: 训练Pythia模型（14M-410M参数）300B tokens，使用三种语言学驱动的课程：习得年龄、词频和动词变体，与随机排序对比；在1B参数规模上比较随机排序和动词变体课程。分析梯度噪声、输出头谱饱和度等优化指标。

Result: 不同排序下训练遵循相同的潜在阶段序列，课程主要改变阶段内的数据暴露。小模型（≤160M）中随机排序表现出更高的梯度噪声和更强的后期训练输出头谱饱和度，同时最终准确率较低；课程学习在相同计算量下减少这两种效应。大模型中饱和度差异较小，课程收益缩小。

Conclusion: 课程学习通过稳定阶段内优化而非创造新阶段来帮助训练，特别是在小模型中通过控制梯度方差减少优化不稳定性。研究建立了难度调节与优化稳定性之间的理论联系。
...
### [273] [Beyond Forgetting: Machine Unlearning Elicits Controllable Side Behaviors and Capabilities](https://arxiv.org/abs/2601.21702)
...
<details>
  <summary>Details</summary>
Motivation: 尽管表示误导(RM)作为LLM遗忘方法很重要，但其使用的目标向量的作用尚未充分探索。研究者希望从线性表示假设的角度重新审视RM，探索机器遗忘可能引发的超出遗忘本身的副作用和增强能力。

Method: 从线性表示假设的视角研究RM：如果能识别对应高层概念的一维表示，就可以在遗忘表示空间中对这个概念向量进行线性操作。基于此假设，研究者实证验证了机器遗忘不仅能实现遗忘，还能引发可控的副作用行为和增强的侧向能力。

Result: 研究在广泛任务中验证了假设，包括行为控制（如控制遗忘模型的真实性、情感和拒绝能力）和能力增强（如提升遗忘模型的上下文学习能力）。发现这种现象既可能是被滥用的隐藏风险，也可用于开发需要更强能力和可控行为的模型。

Conclusion: 机器遗忘通过表示误导不仅能实现目标遗忘，还会引发可控的副作用和增强的侧向能力。这一现象具有双重性：既可能是风险，也可被利用来开发具有更强能力和可控行为的模型。
...
### [274] [SmartMeterFM: Unifying Smart Meter Data Generative Tasks Using Flow Matching Models](https://arxiv.org/abs/2601.21706)
*Nan Lin,Yanbo Wang,Jacco Heres,Peter Palensky,Pedro P. Vergara*

Main category: cs.LG

TL;DR: 提出基于流匹配模型的统一生成框架，用于智能电表数据的合成生成、缺失值插补和超分辨率等任务，避免为每个任务单独训练模型。
...
Method: 利用流匹配模型的强大建模能力，提出统一的条件生成方法。将不同生成任务视为部分数据观测的不同形式，在生成过程中注入这些观测信息，从而用单一模型统一处理插补和超分辨率等任务，无需重新训练。模型训练用于生成具有挑战性的高维时间序列数据（15分钟分辨率的月度智能电表数据）。

Result: 所提模型生成的数据不仅与给定观测一致，而且保持真实性，在性能上优于插补方法和针对特定任务的机器学习基线模型。

Conclusion: 流匹配模型能够有效统一智能电表数据的多种生成任务，通过单一条件生成模型实现数据合成、缺失值插补和超分辨率，避免了为每个任务单独训练模型的冗余，提高了效率并保持了数据质量。
...
### [275] [When does predictive inverse dynamics outperform behavior cloning?](https://arxiv.org/abs/2601.21718)
...
<details>
  <summary>Details</summary>
Motivation: 行为克隆在专家演示有限时经常失败，而预测逆动力学模型虽然表现更好，但其优势原因尚不清楚。本文旨在从理论上解释PIDM优于BC的原因。

Method: 提出理论分析框架，证明PIDM引入了偏差-方差权衡：预测未来状态引入偏差，但基于预测的条件化逆动力学模型能显著降低方差。建立状态预测器偏差的条件，使得PIDM比BC具有更低的预测误差和更高的样本效率。

Result: 在2D导航任务中，BC需要比PIDM多5倍（平均3倍）的演示才能达到相当性能；在具有高维视觉输入和随机转移的复杂3D游戏环境中，BC需要比PIDM多66%以上的样本。

Conclusion: PIDM通过偏差-方差权衡机制，在专家演示有限时比BC具有更好的性能，特别是在有额外数据源可用时，这种优势更加明显。理论分析为PIDM的优势提供了明确的解释。
...
### [276] [Mixed-Precision Training and Compilation for RRAM-based Computing-in-Memory Accelerators](https://arxiv.org/abs/2601.21737)
...
<details>
  <summary>Details</summary>
Motivation: CIM加速器通过内存交叉阵列直接执行矩阵向量乘法（MVM）来加速ML工作负载，但交叉阵列输入和单元的位宽非常有限。大多数CIM编译器不支持8位以下的量化，导致单个MVM需要多个计算周期，且权重无法高效存储在单个交叉阵列单元中。

Method: 提出混合精度训练和编译框架，采用基于强化学习的策略搜索合适的量化配置，以平衡延迟和精度。该方法解决了大规模搜索空间带来的挑战，能够自动找到优化的量化参数。

Result: 在最佳情况下，该方法相比现有最先进解决方案实现了最高2.48倍的加速，同时精度损失仅为0.086%。

Conclusion: 提出的基于强化学习的混合精度框架有效解决了CIM架构中低比特量化问题，显著提升了计算效率同时保持了高精度，为CIM加速器的实际部署提供了可行方案。
...
### [277] [Temporal Sepsis Modeling: a Fully Interpretable Relational Way](https://arxiv.org/abs/2601.21747)
*Vincent Lemaire,Nédra Meloulli,Pierre Jaquet*

Main category: cs.LG

TL;DR: 提出一种基于关系数据方法的机器学习框架，用于脓毒症早期预测，通过关系数据模式表示EMR时序数据，使用命题化技术构建可解释特征，采用选择性朴素贝叶斯分类器，实现四重可解释性。


<details>
  <summary>Details</summary>
Motivation: 脓毒症是ICU中最复杂、异质性最强的综合征之一，具有多样化的生理轨迹和治疗反应。现有深度学习模型虽然能早期预测脓毒症，但缺乏可解释性且忽略了潜在的病人亚表型。
...
Conclusion: 提出的关系方法为脓毒症预测提供了新的途径，不仅保持了预测性能，还显著增强了模型的可解释性，有助于识别患者亚表型和理解治疗反应差异。
...
### [278] [Effective LoRA Adapter Routing using Task Representations](https://arxiv.org/abs/2601.21795)
...
Method: LORAUTER使用从少量验证集派生的任务嵌入来路由查询，而不是直接映射查询到适配器。它在任务级别操作，路由效率随任务数量而非适配器数量扩展，且不需要适配器训练数据

Result: 实验显示LORAUTER在多个任务上始终优于基线路由方法：当存在任务对齐适配器时达到Oracle性能的101.2%，在未见任务上获得+5.2分的SOTA结果，并能扩展到超过1500个适配器的大规模嘈杂适配器池

Conclusion: LORAUTER通过任务表示而非适配器特征的路由方法，实现了高效、可扩展的LoRA适配器选择和组合，为大型语言模型的模块化适配提供了有效的路由解决方案
...
### [279] [Knowledge Vector Weakening: Efficient Training-free Unlearning for Large Vision-Language Models](https://arxiv.org/abs/2601.21794)
*Yejin Kim,Dongjun Hwang,Sungmin Cha,Junsuk Choe*

Main category: cs.LG

TL;DR: KVW是一种无需训练的知识向量弱化方法，通过直接干预模型内部知识向量实现大视觉语言模型的高效遗忘学习


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型存在隐私泄露和有害内容生成等严重问题，而现有的基于梯度的遗忘学习方法计算成本高昂，需要更高效的解决方案

Method: 提出知识向量弱化方法，无需梯度计算，直接识别模型在遗忘集上输出生成时激活的知识向量，并逐步削弱其贡献，防止模型利用不良知识

Result: 在MLLMU和CLEAR基准测试中，KVW实现了稳定的遗忘-保留权衡，同时在计算效率上显著优于基于梯度和基于LoRA的遗忘学习方法

Conclusion: KVW作为一种无需训练的遗忘学习方法，为大视觉语言模型的安全部署提供了高效且有效的解决方案，平衡了遗忘效果与计算效率
...
### [280] [Nonparametric LLM Evaluation from Preference Data](https://arxiv.org/abs/2601.21816)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估LLM的方法存在局限性：要么依赖限制性参数假设，要么在使用灵活机器学习方法时缺乏有效的统计推断和不确定性量化。需要一种既能利用复杂机器学习方法又能提供可靠统计推断的框架。

Method: 提出DMLEval框架，引入广义平均排序分数（GARS），该分数推广了常用排序模型（如Bradley-Terry模型、PageRank/Rank centrality），能处理复杂的人类响应（如平局）。采用去偏机器学习（DML）方法进行估计，允许结合黑盒机器学习方法，并可集成预训练的LLM评估器（如LLM-as-a-judge）。

Result: DMLEval能够产生统计有效的GARS排序分数估计，理论上和实证上都证明了其优势。框架还提出了在预算约束下收集偏好数据的最优策略。在合成和真实世界偏好数据集上的实验验证了方法的有效性。

Conclusion: DMLEval为从业者提供了强大、先进的LLM比较和排序方法，结合了灵活机器学习能力和可靠统计推断，解决了现有方法的局限性。
...
### [281] [Generalized Information Gathering Under Dynamics Uncertainty](https://arxiv.org/abs/2601.21988)
...
<details>
  <summary>Details</summary>
Motivation: 现有主动信息收集方法为特定建模选择（动力学模型、信念更新过程、观测模型、规划器）推导定制成本，缺乏统一框架。需要一种能解耦这些选择、适用于广泛建模场景的通用信息收集成本方法。

Method: 提出统一框架，通过显式暴露参数、信念和控制之间的因果依赖关系，基于Massey定向信息推导通用信息收集成本。该框架仅假设马尔可夫动力学和加性噪声，对其他建模选择保持不可知。证明互信息成本是此通用成本的特例，并建立互信息成本与线性化贝叶斯估计中信息增益的显式理论联系。

Result: 1) 成功推导出通用的信息收集成本框架；2) 证明现有文献中的互信息成本是所提通用成本的特例；3) 建立了互信息成本与线性化贝叶斯估计中信息增益的显式理论联系；4) 通过线性、非线性和多智能体系统实验验证了框架的实用性。

Conclusion: 提出的统一框架成功将信息收集成本与具体建模选择解耦，为主动学习提供了理论基础。该框架不仅统一了现有方法，还建立了互信息成本与信息增益的理论联系，并通过实验验证了其在实际系统中的实用性。
...
### [282] [DASH: Deterministic Attention Scheduling for High-throughput Reproducible LLM Training](https://arxiv.org/abs/2601.21824)
...
<details>
  <summary>Details</summary>
Motivation: 当前广泛使用的注意力实现（如FlashAttention-3）中，确定性反向传播会导致高达37.9%的吞吐量损失，这主要源于梯度累积操作必须串行化以保证数值一致性。这种性能损失源于计算和梯度归约阶段的次优调度，导致硬件利用率低下。

Method: 将确定性注意力反向传播建模为有向无环图（DAG）调度问题，推导最小化关键路径长度的调度方案。提出DASH框架，包含两种互补调度策略：1）降序Q-Tile迭代（反向查询块遍历，减少因果注意力的流水线停顿）；2）移位调度（在DAG模型中理论最优的调度，减少全掩码和因果掩码的流水线停顿）。

Result: 在NVIDIA H800 GPU上的实验表明，DASH显著缩小了确定性注意力的性能差距。与基线相比，所提策略将注意力反向传播的吞吐量提高了最高1.28倍，显著提升了可重现LLM训练的效率。
...
### [283] [Goal-Driven Adaptive Sampling Strategies for Machine Learning Models Predicting Fields](https://arxiv.org/abs/2601.21832)
*Jigar Parekh,Philipp Bekemeyer*

Main category: cs.LG

TL;DR: 提出一种与模型架构无关的主动学习策略，用于机器学习模型的场预测，通过结合标量参考值的GP模型，同时减少认知模型误差和标量与场预测之间的差异。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在处理昂贵黑箱仿真（如计算流体动力学）的多查询挑战时，如何在最小计算成本下确保所需精度仍具挑战。虽然主动学习策略已用于标量量，但扩展到场预测仍缺乏或局限于特定场景和模型类型。

Method: 提出一种与模型架构无关的主动学习策略，结合标量参考值的高斯过程模型，同时减少认知模型误差和标量与场预测之间的差异。引入该方法的不同具体形式，并与仅基于标量值的填充方法进行比较。

Result: 在NASA通用研究模型的参数不确定性传播任务中展示结果，相比无主动学习方法，以显著更小的计算成本实现了高精度水平。

Conclusion: 提出的主动学习策略能够有效处理场预测问题，在保持高精度的同时显著降低计算成本，为昂贵黑箱仿真的多查询挑战提供了通用解决方案。
...
### [284] [Constrained Meta Reinforcement Learning with Provable Test-Time Safety](https://arxiv.org/abs/2601.21845)
...
<details>
  <summary>Details</summary>
Motivation: 现实应用（如机器人和医疗）需要在测试期间施加安全约束，但现有约束元强化学习缺乏在测试任务上确保策略安全性的方法，同时需要降低样本复杂度以实现快速学习

Method: 提出一种算法，在训练期间学习策略的基础上进行精炼，为测试任务学习接近最优策略提供可证明的安全性和样本复杂度保证

Result: 算法具有可证明的安全性和样本复杂度保证，能够学习测试任务的接近最优策略，并推导出匹配的下界，证明该样本复杂度是紧的

Conclusion: 该算法填补了约束元强化学习的空白，为现实应用中的安全约束问题提供了理论保证的解决方案
...
### [285] [READY: Reward Discovery for Meta-Black-Box Optimization](https://arxiv.org/abs/2601.21847)
...
<details>
  <summary>Details</summary>
Motivation: 现有元黑盒优化工作中的奖励函数由人工专家设计，存在设计偏差和奖励黑客攻击风险，需要自动化、无偏的奖励发现方法

Method: 1. 有效性方面：借鉴启发式进化思想，在基于LLM的程序搜索过程中引入定制化进化范式，确保持续改进；2. 效率方面：引入多任务进化架构，支持为不同元黑盒优化方法并行发现奖励函数，通过跨任务知识共享加速收敛

Result: 实验结果表明，通过该方法发现的奖励函数能够有效提升现有元黑盒优化工作的性能，强调了奖励设计在元黑盒优化中的重要性

Conclusion: LLM可作为元黑盒优化的自动奖励发现工具，通过进化范式和多任务架构设计的奖励函数能够提升优化性能，为算法设计提供了新的自动化途径
...
### [286] [Latent Adversarial Regularization for Offline Preference Optimization](https://arxiv.org/abs/2601.22083)
*Enyi Jiang,Yibo Jacky Zhang,Yinglun Xu,Andreas Haupt,Nancy Amato,Sanmi Koyejo*

Main category: cs.LG

TL;DR: GANPO：一种基于对抗学习的潜在空间正则化方法，用于语言模型偏好优化，通过惩罚策略模型与参考模型内部表征的差异来改进现有离线偏好优化方法。
...
Method: 提出GANPO方法，采用对抗学习思想最小化策略模型与参考模型在潜在空间中的表征差异。将GANPO作为正则化项集成到现有离线偏好优化目标中，通过对抗训练实现潜在空间正则化。

Result: 在多种模型架构和任务上的实验表明，潜在空间正则化带来了一致的性能改进。GANPO在分布偏移和噪声条件下提供了更鲁棒的结构反馈，同时保持可比较的下游性能且计算开销很小。

Conclusion: GANPO通过潜在空间正则化有效解决了token级正则化的局限性，为语言模型偏好优化提供了更鲁棒和有效的正则化方法，在保持性能的同时增强了模型的稳定性。
...
TL;DR: 提出DiDAE框架，通过解耦字典学习和扩散自编码器实现梯度自由的反事实生成，结合反事实知识蒸馏提升基础模型在不平衡数据集上的性能
...
Method: 提出视觉解耦扩散自编码器(DiDAE)，将冻结的基础模型与解耦字典学习集成，通过解耦字典在可解释的解耦方向上编辑基础模型嵌入，然后通过扩散自编码器解码，实现高效、梯度自由的反事实生成

Result: DiDAE能够为每个事实生成多个多样化、解耦的反事实，速度远快于现有基线方法（后者只能生成单个纠缠的反事实）。与反事实知识蒸馏(DiDAE-CFKD)结合后，在缓解捷径学习方面达到最先进性能，显著提升了不平衡数据集上的下游性能

Conclusion: DiDAE提供了一种高效、梯度自由的反事实生成框架，能够有效缓解基础模型中的捷径学习问题，特别适用于不平衡数据集，为提升基础模型的鲁棒性提供了新方法
...
### [288] [Managing Solution Stability in Decision-Focused Learning with Cost Regularization](https://arxiv.org/abs/2601.21883)
...
<details>
  <summary>Details</summary>
Motivation: 决策导向学习通过训练模型直接提升决策质量而非仅预测精度，但组合优化问题的可微性是一大挑战。现有扰动近似方法在学习阶段可能因扰动强度波动导致训练无效，需要解决这一问题以提高学习过程的鲁棒性和可靠性。

Method: 通过引入估计成本向量的正则化方法，建立扰动强度波动与组合优化中解稳定性概念的理论联系，从而改善学习过程的鲁棒性。该方法在理论分析基础上进行了广泛的数值实验验证。

Result: 理论分析表明扰动强度波动确实会导致训练无效，而提出的正则化方法能够有效提高学习过程的鲁棒性和可靠性。广泛的数值实验验证了该方法的有效性。

Conclusion: 通过将扰动强度波动与解稳定性概念建立理论联系，并引入成本向量正则化，可以有效解决决策导向学习中因扰动波动导致的训练无效问题，提高学习过程的鲁棒性和可靠性。
...
### [289] [Not All Code Is Equal: A Data-Centric Study of Code Complexity and LLM Reasoning](https://arxiv.org/abs/2601.21894)
*Lukas Twist,Shu Yang,Hanqi Yan,Jingzhi Gong,Di Wang,Helen Yannakoudakis,Jie M. Zhang*

Main category: cs.LG

TL;DR: 研究发现代码的结构复杂性（而非代码本身）是提升大语言模型推理能力的关键因素，通过控制代码的结构复杂度进行微调，在83%的实验中优于使用结构多样的代码数据。
...
Method: 研究两种互补设置：1）解决方案驱动复杂性（同一问题的不同解决方案具有不同复杂性）；2）问题驱动复杂性（基础任务本身的变化反映复杂性）。使用圈复杂度和逻辑代码行数构建受控微调数据集，在多样化推理基准上评估一系列开源大语言模型。

Result: 代码确实能提升推理能力，但其结构性质量决定其有效性。在83%的实验中，将微调数据限制在特定结构复杂性范围内的效果优于使用结构多样的代码数据。

Conclusion: 代码的结构复杂性是提升推理能力的关键因素，这为超越单纯数据规模扩展、通过数据中心的路径改进推理提供了新方向。
...
### [290] [A Low-Complexity Plug-and-Play Deep Learning Model for Generalizable Massive MIMO Precoding](https://arxiv.org/abs/2601.21897)
*Ali Hasanzadeh Karkan,Ahmed Ibrahim,Jean-François Frigon,François Leduc-Primeau*

Main category: cs.LG

TL;DR: 提出PaPP框架，一种可跨站点、功率水平和信道估计误差重用的深度学习方法，用于mMIMO下行链路预编码，无需为每个部署从头训练新模型。


<details>
  <summary>Details</summary>
Motivation: mMIMO下行链路预编码在实际部署中面临挑战：WMMSE等近最优算法计算成本高且对SNR和信道估计质量敏感，而现有基于深度学习的方法缺乏鲁棒性且需要为每个部署站点重新训练。

Method: 提出PaPP框架，结合高容量教师模型和紧凑学生模型，使用自监督损失平衡教师模仿和归一化和速率，通过元学习领域泛化和发射功率感知输入归一化进行训练，支持全数字或混合波束成形预编码。

Result: 在三个未见站点的射线追踪数据上，PaPP全数字和混合波束成形模型在少量本地未标记样本微调后均优于传统和深度学习基线，实现超过21倍的计算能量减少，并在信道估计误差下保持良好性能。

Conclusion: PaPP是一种实用的能量高效mMIMO预编码解决方案，具有跨站点重用性、计算效率高和对信道估计误差鲁棒的特点。
...
### [291] [LoRIF: Low-Rank Influence Functions for Scalable Training Data Attribution](https://arxiv.org/abs/2601.21929)
*Shuangqi Li,Hieu Le,Jingyi Xu,Mathieu Salzmann*

Main category: cs.LG

TL;DR: LoRIF是一种低秩影响函数方法，通过利用梯度的低秩结构解决训练数据归因中的存储和计算瓶颈，相比现有方法实现高达20倍的存储减少和查询加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的训练数据归因方法（如TRAK、LoGRA）面临两个主要可扩展性瓶颈：1）存储和加载所有训练样本的投影梯度导致I/O延迟问题；2）形成D×D逆Hessian近似需要O(D²)内存。这两个瓶颈都与投影维度D相关，但提高D又是提升归因质量所必需的，形成了质量-可扩展性权衡。
...
Result: 在0.1B到70B参数规模的模型和数百万样本的数据集上，LoRIF相比LoGRA实现了高达20倍的存储减少和查询时间加速，同时匹配或超越了其归因质量。

Conclusion: LoRIF通过利用梯度的低秩结构，有效解决了梯度基训练数据归因方法的质量-可扩展性权衡问题，使前沿规模下的梯度基TDA变得实用可行。
...
### [292] [Entropy-Based Dimension-Free Convergence and Loss-Adaptive Schedules for Diffusion Models](https://arxiv.org/abs/2601.21943)
*Ahmad Aghapour,Erhan Bayraktar,Ziqing Zhang*

Main category: cs.LG

TL;DR: 提出了一种基于信息论的无维度收敛分析方法，避免了传统方法对维度或几何结构的依赖，并设计了基于训练损失的自适应采样调度方法
...
Method: 采用信息论方法分析收敛性，推导出KL散度上界为O(H²/K)，其中H是香农熵，K是采样步数。基于KL散度重构提出Loss-Adaptive Schedule (LAS)自适应调度方法，仅依赖训练损失进行高效离散化

Result: 理论证明了在温和假设下，目标分布与生成分布间的KL散度上界为O(H²/K)，避免了维度依赖。实验表明LAS相比常见启发式调度方法能提升采样质量

Conclusion: 提出了一种信息论框架实现扩散模型的无维度收敛分析，避免了传统几何假设限制。LAS自适应调度方法仅依赖训练损失，无需后训练重计算，能有效提升采样效率和质量
...
### [293] [Clarity: The Flexibility-Interpretability Trade-Off in Sparsity-aware Concept Bottleneck Models](https://arxiv.org/abs/2601.21944)
*Konstantinos P. Panousis,Diego Marcos*

Main category: cs.LG

TL;DR: 提出概念清晰度指标，系统评估概念瓶颈模型在不同稀疏化策略下的表示质量，揭示灵活性与可解释性之间的关键权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型广泛使用但缺乏可解释性研究，现有评估方法不系统，特别是对声称能"诱导可解释性"的稀疏感知方法缺乏客观评估。

Method: 引入清晰度指标衡量下游性能与概念表示稀疏性、精确性的交互；提出使用真实概念标注数据的可解释性评估框架；比较VLM和属性预测器两种CBM，以及ℓ₁、ℓ₀和伯努利三种稀疏化策略。

Result: 实验揭示了灵活性与可解释性之间的关键权衡，即使性能相近，不同方法在表示质量上表现出显著差异。

Conclusion: 需要系统评估概念瓶颈模型的可解释性，清晰度指标能有效捕捉表示质量，不同建模决策对可解释性有重要影响。
...
### [294] [Embracing Aleatoric Uncertainty in Medical Multimodal Learning with Missing Modalities](https://arxiv.org/abs/2601.21950)
*Linxiao Gong,Yang Liu,Lianlong Sun,Yulai Bi,Jing Liu,Xiaoguang Zhu*

Main category: cs.LG

TL;DR: 提出AUM框架，通过建模单模态的随机不确定性来量化模态可靠性，并利用不确定性感知的动态消息传递机制自适应聚合多模态信息，有效处理临床实践中常见的模态缺失问题。


<details>
  <summary>Details</summary>
Motivation: 医学多模态学习中存在模态缺失的普遍问题，现有方法假设模态贡献相等且缺失模式随机，忽略了医学数据采集过程中的固有不确定性。

Method: 提出Aleatoric Uncertainty Modeling (AUM)：1) 将每个单模态表示建模为多元高斯分布以捕获随机不确定性；2) 在患者-模态二分图中使用不确定性感知聚合机制开发动态消息传递机制；3) 自适应聚合捕获的信息，动态强调可用模态中更可靠的信息。

Result: 在MIMIC-IV死亡率预测上AUC-ROC提升2.26%，在eICU上提升2.17%，优于现有最先进方法。

Conclusion: AUM框架通过显式量化单模态随机不确定性，实现了对模态缺失的自然处理，并能动态强调可靠信息，在医学多模态学习任务中表现出优越性能。
...
### [295] [Investigation into using stochastic embedding representations for evaluating the trustworthiness of the Fréchet Inception Distance](https://arxiv.org/abs/2601.21979)
*Ciaran Bench,Vivek Desai,Carlijn Roozemond,Ruben van Engen,Spencer A. Thomas*

Main category: cs.LG

TL;DR: 该论文研究了Fréchet Inception Distance (FID)在医学图像评估中的局限性，通过蒙特卡洛dropout计算FID的预测方差，分析其与测试数据分布偏移的相关性。


<details>
  <summary>Details</summary>
Motivation: FID作为合成图像质量评估指标，依赖于在自然图像上预训练的InceptionV3模型，这使其在医学图像应用中效果有限。然而，该指标未能捕捉图像特征有意义的差异程度尚不明确，需要评估FID作为医学图像质量指标的可信度。

Method: 使用蒙特卡洛dropout方法计算FID的预测方差，同时计算特征嵌入模型潜在表示的预测方差补充估计。通过在不同强度增强的ImageNet1K验证集和其他外部数据集上测试，分析预测方差与测试输入相对于训练数据分布偏移程度的相关性。

Result: 研究发现，所考虑的预测方差大小与测试输入相对于训练数据的分布偏移程度存在不同程度的相关性。这为理解FID作为医学图像质量指标的可信度提供了洞察，表明预测方差可以作为评估FID可信度的指示器。

Conclusion: 通过分析FID预测方差与分布偏移的相关性，可以更好地评估FID在医学图像应用中的可信度。该方法为理解基于自然图像预训练模型在医学图像评估中的局限性提供了量化依据，有助于改进医学图像质量评估指标。
...
### [296] [PowerGenie: Analytically-Guided Evolutionary Discovery of Superior Reconfigurable Power Converters](https://arxiv.org/abs/2601.21984)
*Jian Gao,Yiwei Zou,Abhishek Pradhan,Wenhao Huang,Yumin Su,Kaiyuan Yang,Xuan Zhang*

Main category: cs.LG

TL;DR: PowerGenie：一个自动化框架，用于大规模发现高性能可重构电源转换器拓扑结构，通过分析框架和进化微调方法，超越了现有AI方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 发现优越的电路拓扑结构需要探索指数级大的设计空间，这一挑战传统上由人类专家完成。现有AI方法要么从预定义模板中选择，要么在有限规模下生成新拓扑而缺乏严格验证，导致大规模性能驱动的发现未被充分探索。

Method: PowerGenie包含两个核心组件：(1) 自动化分析框架，无需元件尺寸确定或SPICE仿真即可确定转换器功能和理论性能极限；(2) 进化微调方法，通过适应度选择和唯一性验证，使生成模型与其训练分布共同进化。

Result: 相比现有方法，PowerGenie在语法有效性、功能有效性、新颖率和性能指标(FoM)方面表现更优。发现了一种新颖的8模式可重构转换器，其FoM比最佳训练拓扑高23%。SPICE仿真确认在8种模式下平均绝对效率提升10%，单模式下最高提升17%。
...
### [297] [Elign: Equivariant Diffusion Model Alignment from Foundational Machine Learning Force Fields](https://arxiv.org/abs/2601.21985)
*Yunyang Li,Lin Huang,Luojia Xia,Wenhe Zhang,Mark Gerstein*

Main category: cs.LG

TL;DR: Elign是一个后训练框架，通过使用预训练的机器学习力场替代昂贵的DFT计算，并将物理引导转移到训练阶段，实现快速、高质量的3D分子构象生成。


<details>
  <summary>Details</summary>
Motivation: 现有的E(3)-等变扩散模型在生成3D分子构象时，往往只是复制半经验训练数据的偏差，而非捕捉高精度哈密顿量的平衡分布。虽然基于物理的引导可以纠正这一问题，但面临两个计算瓶颈：昂贵的量子化学计算（如DFT）以及需要在每个采样步骤重复此类查询。

Method: 提出Elign后训练框架：1) 使用预训练的基础机器学习力场替代昂贵的DFT计算提供物理信号；2) 将物理引导转移到训练阶段，通过将反向扩散表述为强化学习问题，引入FED-GRPO算法微调去噪策略，该算法包含基于势能的能量奖励和基于力的稳定性奖励，并进行独立优化和组归一化。

Result: 实验表明，Elign生成的构象具有更低的金标准DFT能量和力，同时提高了稳定性。关键的是，由于生成过程中不需要能量评估，推理速度与无引导采样一样快。

Conclusion: Elign通过摊销计算成本，实现了快速且高质量的3D分子构象生成，解决了现有方法在物理准确性和计算效率之间的权衡问题。
...
### [298] [Negatives-Dominant Contrastive Learning for Generalization in Imbalanced Domains](https://arxiv.org/abs/2601.21999)
*Meng Cao,Jiexi Liu,Songcan Chen*

Main category: cs.LG

TL;DR: 提出负样本主导对比学习(NDCL)解决不平衡领域泛化问题，通过强调负样本信号增强决策边界分离，同时促进类内紧凑性和领域间后验一致性。


<details>
  <summary>Details</summary>
Motivation: 不平衡领域泛化(IDG)需要同时处理领域偏移和标签偏移，这在异构长尾分布下尤为复杂。现有方法缺乏理论基础，且难以处理这两种偏移的纠缠关系。

Method: 首先建立IDG的理论泛化界，强调后验差异和决策边界的作用。提出负样本主导对比学习(NDCL)：1)通过强调负样本作为主要对比信号增强类间决策边界分离；2)使用重加权交叉熵促进类内紧凑性；3)通过预测中心对齐策略强制领域间后验一致性。

Result: 在基准测试上的严格实验验证了NDCL的有效性，代码已开源。

Conclusion: NDCL通过直接引导决策边界，有效解决了不平衡领域泛化问题，在增强判别性的同时保证了领域间后验一致性，为这一重要但未充分探索的问题提供了新的解决方案。
...
### [299] [Rate-Distortion Optimization for Transformer Inference](https://arxiv.org/abs/2601.22002)
*Anderson de Andrade,Alon Harell,Ivan V. Bajić*

Main category: cs.LG

TL;DR: 该论文提出了一种基于率失真理论的Transformer中间表示压缩框架，通过权衡比特率与精度来学习紧凑编码，在语言基准测试中实现了显著的存储节省，并在某些情况下提高了准确性。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在推理时存在计算和内存需求高的问题，跨设备分区推理需要压缩中间表示。现有方法缺乏理论指导，需要一种原则性的压缩框架来权衡比特率与模型精度。

Method: 提出基于率失真理论的损失压缩框架，学习紧凑编码以明确权衡比特率与准确性。扩展信息论概念定义率与熵之间的差距并推导其界限，开发PAC式界限来估计这一差距。

Result: 在语言基准测试中，提出的编解码器实现了显著的存储节省，在某些情况下甚至提高了准确性，优于更复杂的基线方法。实证表明不同架构和任务的率受这些界限驱动。

Conclusion: 该框架为Transformer表示编码提供了统一的理论视角，增强了模型的可解释性，将信息论概念扩展到深度学习领域，为高效推理提供了理论指导。
...
TL;DR: STARS是一种无需训练、推理时干预的方法，通过Stiefel流形优化多个正交的激活引导方向，最大化并发生成运行的几何体积，从而提升语言模型输出的多样性。
...
Method: STARS在推理时收集并发生成运行的隐藏激活，在Stiefel流形上联合优化多个加性引导方向。该方法最大化引导激活的几何体积，同时Stiefel流形确保引导干预的正交性。为降低延迟，设计了轻量级单步更新算法和封闭形式的步长。

Result: 在测试用例生成和科学发现基准测试中，STARS始终优于标准采样方法，在不牺牲质量性能的情况下实现了更大的多样性。

Conclusion: STARS通过将激活引导转化为探索引擎，有效解决了语言模型生成多样性不足的问题，提供了一种无需训练、推理时干预的解决方案，在保持质量的同时显著提升输出多样性。
...
TL;DR: 该论文提出了一个机制性框架，从几何角度解释灾难性遗忘是单个特征编码变换的结果，这些变换通过降低特征分配容量和破坏下游计算读取导致遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习研究通常只在性能或最后一层表示层面衡量灾难性遗忘，忽视了底层机制。需要从机制角度理解遗忘现象，为持续学习提供新的特征中心化词汇。

Method: 1) 引入机制性框架，将灾难性遗忘解释为单个特征编码的几何变换；2) 分析可处理模型，形式化这一观点并识别最佳和最差情况；3) 通过实验验证形式分析，强调深度的影响；4) 使用Crosscoders将框架应用于实际模型分析，以Vision Transformer在顺序CIFAR-10上的训练为例进行案例研究。

Result: 1) 形式分析识别了特征编码变换导致遗忘的最佳和最差情况；2) 实验验证了形式分析，并显示深度对遗忘有负面影响；3) 案例研究展示了框架在实际模型分析中的应用可行性；4) 为持续学习提供了新的特征中心化分析词汇和方法。

Conclusion: 该研究提出了一个机制性框架，从特征编码变换的几何角度解释灾难性遗忘，为理解持续学习中的遗忘现象提供了新的视角和工具，有助于更深入地分析实际模型的遗忘机制。
...
<details>
  <summary>Details</summary>
Motivation: 为大规模语言模型选择高质量、多样化的训练数据是困难任务，因为缺乏廉价可靠的质量指标。现有方法要么不可扩展（直接查询LLM），要么依赖稀疏质量信号训练的不可靠分类器。

Method: 提出基于文本嵌入的层次聚类方法：首先构建文档的层次聚类树，然后自适应选择文档让LLM评估，利用聚类结构推断未评估文档的质量。算法在不知道纯子树的情况下，通过少量查询就能准确预测文档质量。

Result: 理论证明方法具有查询效率：在层次聚类包含纯叶子簇的假设下，以高概率通过少量查询就能正确预测每个文档质量，查询数量与最小纯子树大小成正比。实验验证相比其他基于分类器的过滤方法具有优势。

Conclusion: 该方法为大规模文档质量筛选提供了高效解决方案，通过层次聚类结构减少对昂贵LLM查询的依赖，同时保持质量预测的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法主要通过在预测空间定义对齐式目标来近似从头训练的行为，虽然能减少遗忘内容的生成，但可能只是抑制而非真正移除遗忘概念。遗忘概念可能仍然存在于表示中，并与保留知识纠缠在一起。

Method: CLReg（对比表示正则化器）通过识别遗忘特征并将其与保留特征推离，明确减少遗忘-保留干扰，同时最小化对保留特征的偏移。该方法提供了首个将表示塑形与纠缠减少相关联的理论见解。

Result: 在不同规模的LLM和遗忘基准测试中，CLReg减少了遗忘-保留表示的纠缠，促进了主流遗忘方法的有效性，且不会带来额外的隐私风险。

Conclusion: CLReg通过重塑表示空间来移除遗忘概念，为未来工作提供了启示，表明表示空间的塑形是有效遗忘的关键方向。
...
TL;DR: 提出一种称为集合逆问题（EIP）的新统计问题，旨在从观测数据反演通过前向过程推演的先验分布，并开发基于条件生成模型的非迭代推理方法。


<details>
  <summary>Details</summary>
Motivation: 解决高能物理中的反卷积（unfolding）问题以及全波形反演和未知先验的逆成像问题，这些都需要从受探测器效应扭曲的测量数据中重建真实的物理分布。

Method: 提出非迭代推理时方法，基于新的条件生成模型类别（集合逆生成模型），利用观测集中的集合信息，通过跨多个真值-观测对集合的训练隐式编码似然模型，避免推理时显式迭代使用前向模型。

Result: 在逆成像、高能物理和全波形反演的合成和真实数据集上进行基准测试，证明方法能有效进行后验推断并泛化到未见过的先验分布。

Conclusion: 提出的集合逆问题框架及相关方法为解决逆问题提供了新思路，特别是在需要处理未知先验和避免推理时迭代前向模型的应用场景中具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统任务算术方法通过减去特定任务向量实现遗忘，但会导致过度遗忘，破坏保留其他信息所需的关键参数。观察到不同参数对遗忘和保留的重要性不同，需要更精细的参数级调整机制

Method: 提出参数级任务算术（PerTA），通过重新缩放任务向量实现每个参数的独立调整。使用梯度（PerTA-grad）或对角Fisher信息近似（PerTA-fisher）估计每个参数对遗忘与保留的相对重要性权重

Result: PerTA在广泛实验中一致优于标准任务向量方法，在许多情况下甚至超过广泛使用的基于训练的遗忘方法，在遗忘效果和整体模型效用方面都表现更好
...
TL;DR: 该论文提出了一种基于AdS/CFT全息原理的生成机器学习框架，将流匹配算法与AdS物理相结合，实现了更快、更高质量的收敛


<details>
  <summary>Details</summary>
Motivation: 利用量子引力中的全息原理（特别是AdS/CFT对应关系）来开发新的生成建模范式，为流匹配算法提供物理可解释性

Method: 使用AdS中标量场的体-边界映射来表示数据从基础分布到学习分布的流动，将AdS物理与深度学习、输运理论技术相结合来增强流匹配算法

Result: 在checkerboard玩具数据集和MNIST上，该模型比可比较的无物理流匹配模型实现了更快、更高质量的收敛

Conclusion: 该方法提供了流匹配的物理可解释版本，并确立了AdS物理和几何在开发生成建模新范式中的实用性
...
TL;DR: 提出Cross-Fusion Distance(CFD)作为衡量表示空间中数据组融合程度的新度量，能区分融合改变因素与融合保持因素，具有线性计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有分布距离度量混淆了融合改变因素（如几何位移）和融合保持因素（如全局缩放和采样引起的布局变化），导致无法准确反映数据组间的真实融合程度。需要一种能区分这些因素、对表示学习有意义的度量。

Method: 引入Cross-Fusion Distance(CFD)，这是一种原则性度量，能够隔离融合改变的几何因素，同时对融合保持的变化保持鲁棒性。该方法具有线性计算复杂度，并通过理论分析其不变性和敏感性特性。

Result: 在受控合成实验中验证了CFD的不变性和敏感性特性；在实际数据集上，CFD与下游泛化性能下降的相关性比常用替代方法更紧密，表明其在实际应用中的有效性。

Conclusion: CFD为表示学习提供了一个理论基础扎实且可解释的距离度量，能够准确量化表示空间中数据组间的融合程度，特别是在领域偏移场景下。
...
### [308] [Where Do the Joules Go? Diagnosing Inference Energy Consumption](https://arxiv.org/abs/2601.22076)
*Jae-Won Chung,Ruofan Wu,Jeff J. Ma,Mosharaf Chowdhury*

Main category: cs.LG

TL;DR: 大规模测量生成式AI推理能耗研究，揭示任务类型、模型配置和GPU利用率对能耗的显著影响，提出时间与能耗分析框架
...
Method: 在NVIDIA H100和B200 GPU上进行大规模测量研究，涵盖46个模型、7个任务和1,858种配置，分析推理时间和能耗

Result: 发现显著的能耗差异：LLM任务类型可导致25倍能耗差异，视频生成能耗有时超过图像的100倍，GPU利用率差异可造成3-5倍能耗差异

Conclusion: 提出分析时间与能耗底层机制的理论框架，认为时间和能耗由内存和利用率等潜在指标决定，这些指标受算法、软件和硬件层因素影响，框架可扩展到功率受限数据中心的关键指标——每瓦吞吐量
...
### [309] [GeoNorm: Unify Pre-Norm and Post-Norm with Geodesic Optimization](https://arxiv.org/abs/2601.22095)
...
TL;DR: GeoNorm：一种基于流形优化的新型Transformer归一化方法，将FFN和注意力层输出解释为优化方向，用流形上的测地线更新替代标准归一化，并引入类似学习率调度的层间更新衰减。


<details>
  <summary>Details</summary>
Motivation: Transformer架构中归一化层的位置（Pre-Norm与Post-Norm）仍是一个开放问题。本文从流形优化的角度重新思考这些方法，将FFN和注意力层的输出解释为优化过程中的更新方向，旨在提供更理论化的归一化设计框架。

Method: 提出GeoNorm方法：1）将标准归一化替换为流形上的测地线更新；2）引入类似学习率调度的层间更新衰减机制，对FFN和注意力组件分别进行衰减控制；3）该方法可无缝集成到标准Transformer架构中。

Result: 综合实验表明，GeoNorm在Transformer模型中持续优于现有的归一化方法。关键优势在于：1）性能提升显著；2）可无缝集成到标准架构；3）额外计算成本可忽略不计。

Conclusion: GeoNorm通过流形优化视角重新思考Transformer归一化，提供了一种理论驱动且高效的新方法，在保持计算效率的同时显著提升模型性能，为解决归一化层位置问题提供了新思路。
...
### [310] [Boosting CVaR Policy Optimization with Quantile Gradients](https://arxiv.org/abs/2601.22100)
...
<details>
  <summary>Details</summary>
Motivation: 传统CVaR-PG方法样本效率低下，主要原因是它只关注尾部性能而忽略了大量采样轨迹，需要改进以利用所有采样数据

Method: 通过将期望分位数项增强到CVaR中，分位数优化允许动态规划公式化，从而利用所有采样数据，提高样本效率

Result: 在具有可验证风险规避行为的领域中，该方法在马尔可夫策略类中显著优于CVaR-PG，并一致优于其他现有方法

Conclusion: 通过分位数优化增强CVaR-PG可以有效解决样本效率问题，同时保持CVaR的风险规避目标，为风险敏感强化学习提供了更高效的算法
...
### [311] [Prior-Informed Flow Matching for Graph Reconstruction](https://arxiv.org/abs/2601.22107)
*Harvey Chen,Nicolas Zilberstein,Santiago Segarra*

Main category: cs.LG

TL;DR: PIFM是一种用于图重建的条件流模型，通过将基于嵌入的先验与连续时间流匹配相结合，解决了图重建中局部信息与全局一致性的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 图重建面临关键挑战：经典嵌入方法缺乏全局一致性，而现代生成模型难以融入结构先验。需要一种能平衡局部信息与全局结构的方法。
...
Conclusion: PIFM成功地将嵌入先验与流匹配相结合，为图重建提供了一种有效的方法，能够同时利用局部信息和全局结构一致性。
...
### [312] [Learning Hamiltonian Flow Maps: Mean Flow Consistency for Large-Timestep Molecular Dynamics](https://arxiv.org/abs/2601.22123)
...
Result: 方法在多种哈密顿系统中验证有效，特别改进了基于机器学习力场（MLFF）的分子动力学模拟，支持显著更大的积分时间步长

Conclusion: 该方法在保持相当训练和推理成本的同时，支持更大的积分时间步长，可直接在广泛可用的无轨迹MLFF数据集上训练
...
### [313] [Discovering Hidden Gems in Model Repositories](https://arxiv.org/abs/2601.22157)
*Jonathan Kahana,Eliahu Horwitz,Yedid Hoshen*

Main category: cs.LG

TL;DR: 研究发现公共模型库中存在大量被忽视的优质微调模型（"隐藏宝石"），这些模型性能显著优于热门模型但未被发现，作者提出基于多臂老虎机的高效发现方法
...
Method: 1) 评估2000多个模型验证"隐藏宝石"现象；2) 将模型发现问题建模为多臂老虎机问题；3) 使用共享查询集和激进淘汰策略加速顺序减半搜索算法

Result: 1) 发现确实存在大量被忽视的优质模型，如Llama-3.1-8B家族中罕见下载的检查点将数学性能从83.2%提升到96.0%；2) 提出的方法仅需每个候选模型50个查询即可检索顶级模型，加速发现过程超过50倍
