...
Motivation: 现有工具使用LLM的合成任务方法在任务和工具集变化时泛化能力不足，主要原因是合成任务多样性不够。需要一种能覆盖多样工具类型、工具集组合和异构工具使用模式的方法。

Method: 提出DIVE方法：1）先执行多样化的真实工具；2）从执行轨迹反向推导出严格蕴含的任务；3）通过工具池覆盖和每任务工具集多样性两个可控维度扩展结构多样性；4）采用证据收集-任务推导循环在5个领域的373个工具上诱导丰富的多步骤工具使用模式。

Result: 在Qwen3-8B上使用DIVE数据（48k SFT + 3.2k RL）训练，在9个OOD基准上平均提升22分，比最强的8B基线高出68分。控制性扩展分析显示，多样性扩展始终优于数量扩展，即使数据量少4倍。

Conclusion: DIVE通过证据驱动的任务合成方法，从根本上解决了工具使用LLM的泛化问题，证明了多样性扩展比单纯增加数据量更有效地提升OOD泛化能力。
...
TL;DR: 本文提出将大语言和多模态模型作为自动驾驶系统的认知核心，通过认知层次分解驾驶任务，系统化七大推理挑战，并分析当前系统架构与评估方法，指出推理延迟与实时控制之间的根本矛盾。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在感知能力上已有显著进展，但面临推理能力不足的瓶颈，尤其在长尾场景和复杂社会交互中缺乏类人判断能力。大语言和多模态模型的出现为解决这一瓶颈提供了机遇，但缺乏系统化的集成框架。

Method: 提出认知层次框架，根据认知和交互复杂性分解驾驶任务；系统化七大核心推理挑战；从系统中心视角（智能体架构）和评估中心视角（验证实践）对现有技术进行双重分析。

Result: 分析揭示了向整体化、可解释"玻璃盒"智能体的明显趋势；识别出LLM推理的高延迟、深思熟虑特性与车辆控制的毫秒级、安全关键需求之间的根本矛盾。
...
TL;DR: Paced框架通过聚焦于学生模型的最近发展区来优化LLM蒸馏，使用基于通过率的Beta核权重函数，避免在已掌握或过难问题上的计算浪费。


<details>
  <summary>Details</summary>
Motivation: 标准LLM蒸馏存在计算浪费：在学生已掌握的问题上梯度接近零，在过难问题上梯度混乱且会侵蚀现有能力。这种浪费在理论上不可避免，因为蒸馏的梯度信噪比在两个通过率极端都会消失。

Method: 提出Paced框架，通过基于通过率的权重函数w(p)=p^α(1-p)^β将蒸馏集中在学生能力的最近发展区。该Beta核权重函数源于蒸馏梯度的边界消失结构，具有最小最大鲁棒性。方法仅需学生rollouts估计通过率，无需架构修改，兼容任何KL方向。

Result: 1) 理论证明Beta核权重函数是蒸馏SNR结构的领先阶权重族，在有限乘性误设下最坏情况效率损失仅为O(δ^2)。2) 从大教师模型到小学生模型的前向KL蒸馏中，Paced相比基础模型获得显著增益，同时保持基准遗忘在低水平。3) 在指令调优模型的反向KL自蒸馏中，增益超过基线。4) 前向KL后接反向KL的两阶段调度产生最强结果，在标准推理基准上实现实质性改进。

Conclusion: Paced框架通过将蒸馏集中在最近发展区，有效解决了标准LLM蒸馏的计算浪费问题。理论证明Beta核权重函数具有鲁棒性，实验显示在各种蒸馏配置中均优于基线，支持蒸馏过程的模式覆盖然后巩固解释。
...
TL;DR: 论文提出动态框架测试大语言模型遗忘方法的鲁棒性，发现现有遗忘方法在多跳推理等复杂查询下脆弱，传统静态评估指标存在有效性幻觉。
...
Result: 框架：(1)通过自动生成语义等价Q&A探针达到与现有基准相当覆盖率；(2)与先前评估一致；(3)发现其他基准遗漏的新遗忘失败，尤其多跳场景。激活分析显示单跳查询通常遵循主导计算路径（更易被遗忘方法破坏），而多跳查询倾向于使用保持完整的替代路径。

Conclusion: 该框架无需手动构建遗忘测试集即可实用、可扩展地评估遗忘方法，促进实际应用。多跳查询使用替代计算路径解释了遗忘技术在多跳场景下的脆弱性。
...
TL;DR: COMPASS框架是一个多智能体编排系统，通过模块化治理机制实现价值对齐AI，整合主权、碳感知计算、合规和伦理四个维度，使用RAG增强评估的语义一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体系统的快速发展引发了对数字主权、环境可持续性、监管合规和伦理对齐的担忧。现有框架仅孤立地处理单个维度，缺乏将多个关键要求系统整合到自主智能体决策过程中的统一架构。

Method: 提出COMPASS框架，包含一个编排器和四个专门子智能体（主权、碳感知计算、合规、伦理），每个都通过检索增强生成(RAG)技术增强，基于已验证的上下文特定文档进行评估。采用LLM-as-a-judge方法为每个评估维度分配定量分数并生成可解释的论证，实现实时冲突目标仲裁。

Result: 通过自动化评估验证架构，证明RAG集成显著增强了语义一致性并减轻了幻觉风险。基于组合的设计促进了在不同应用领域的无缝集成，同时保持了可解释性和可追溯性。

Conclusion: COMPASS框架为多维度原则在自主系统中的系统整合提供了统一架构，通过模块化治理机制实现价值对齐AI，解决了现有框架在整合数字主权、环境可持续性、监管合规和伦理对齐方面的不足。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型参数众多、结构复杂，如同"黑箱"系统难以评估和解释。AI心理测量学作为新兴领域，旨在应用心理测量方法评估AI系统的心理特征和过程，为解决这一挑战提供途径。

Method: 采用技术接受模型(TAM)框架，对GPT-3.5、GPT-4、LLaMA-2和LLaMA-3四个大语言模型进行心理测量学评估。研究检验了聚合效度、区分效度、预测效度和外部效度等多项效度指标。

Result: 所有四个大语言模型的响应均满足各项效度标准。性能更高的模型(GPT-4和LLaMA-3)相比其前代模型(GPT-3.5和LLaMA-2)表现出更优的心理测量效度，显示出模型性能与心理测量效度之间的正相关关系。

Conclusion: 研究证实了应用AI心理测量学评估和解释大语言模型的有效性，为理解这些复杂AI系统的心理特征提供了方法论基础，并表明模型性能提升会伴随心理测量效度的改善。
...
TL;DR: 该社论探讨了AI与区块链技术的互补关系：AI趋向中心化，区块链趋向去中心化，两者结合可创造"去中心化智能"系统。


<details>
  <summary>Details</summary>
Motivation: AI技术（特别是大语言模型）由于数据和资源被大公司垄断而呈现强烈的中心化趋势，这带来了数据垄断、隐私风险和不透明性等问题。区块链技术则提供了去中心化、透明和安全的机制，可以平衡AI的中心化倾向。

Method: 通过理论分析AI与区块链技术的特性对比，提出两者互补的框架：区块链可以通过去中心化数据管理、计算和治理来缓解AI的中心化风险；AI则可以通过自动化智能合约管理、内容策展和威胁检测来增强区块链的效率和安全性。

Result: 提出了"去中心化智能"（DI）的概念，这是一个跨学科研究领域，旨在创建无需集中控制就能运行的智能系统。这种融合可以促进更大的包容性、透明度和用户隐私保护。

Conclusion: AI和区块链技术不是相互排斥的，而是具有互补优势。两者的结合可以创造更平衡、透明和安全的智能系统，推动"去中心化智能"这一新兴领域的发展，以应对AI中心化带来的风险。
...
TL;DR: FinRule-Bench是一个用于评估大语言模型在真实世界财务报表上进行基于规则的财务推理诊断完整性的基准测试，包含三种审计任务：规则验证、规则识别和联合规则诊断。
...
Method: 引入FinRule-Bench基准，将真实财务报表与人工策划的明确会计准则配对，涵盖四种标准报表类型。定义三种需要逐步增强推理能力的审计任务：规则验证、规则识别和联合规则诊断。评估零样本和少样本提示下的LLMs，并引入因果-反事实推理协议来强制决策、解释和反事实判断之间的一致性。

Result: 模型在孤立规则验证上表现良好，但在规则区分和多违规诊断方面性能急剧下降。FinRule-Bench为研究规则治理推理、诊断覆盖范围以及LLMs在高风险财务分析中的失败模式提供了原则性和可重复的测试平台。

Conclusion: FinRule-Bench填补了现有基准的空白，提供了评估LLMs在真实财务报表上进行基于规则的财务推理诊断能力的系统方法。研究发现当前模型在复杂审计任务中存在显著局限性，为未来改进提供了方向。
...
TL;DR: 提出了一种基于端到端测量的黑盒在线控制器，通过爬山算法最大化满足SLO的请求吞吐量，并以LLM服务为例讨论了AI系统事实表中集成性能和可持续性指标的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务系统缺乏无需内部仪器的黑盒控制方法，需要一种仅使用端到端测量就能优化服务性能的解决方案，同时强调AI系统部署中需要综合考虑性能和可持续性指标。

Method: 设计了一种黑盒在线控制器，仅使用短时间段内的端到端测量数据，无需内部仪器，采用爬山算法最大化goodput（满足服务级别目标的请求吞吐量）。

Result: 通过实证证据表明该设计是有效的，并以LLM服务为例展示了该方法的实际应用价值。

Conclusion: 该黑盒控制器设计为LLM服务优化提供了有效方法，同时强调了在AI系统事实表中集成系统性能和可持续性指标的重要性，为组织采用AI系统提供了更全面的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: Transformer时间序列基础模型面临token化选择的基本权衡：逐点嵌入保持时间保真度但序列长度扩展性差，固定长度分块提高效率但可能破坏自然过渡并模糊信息丰富的局部动态。

Method: TimeSqueeze首先应用轻量级状态空间编码器提取全分辨率逐点特征，然后基于内容感知分割，将短分块分配给信息密集区域，长分块分配给平滑或冗余段，实现可变分辨率压缩。

Result: 在大规模预训练中，TimeSqueeze相比等效逐点token基线实现高达20倍更快收敛和8倍更高数据效率；在长期预测基准测试中始终优于使用逐点token化或固定大小分块的架构。

Conclusion: TimeSqueeze通过动态自适应分块机制有效解决了时间序列基础模型中token化效率与保真度的权衡问题，在保持关键时间结构的同时显著提升了模型训练效率和性能。
...
TL;DR: 论文探讨了AI身份边界问题，指出传统人类身份概念不适用于可复制、编辑、模拟的机器思维，存在多种不同的身份边界（实例、模型、角色等），这些边界影响激励、风险和合作规范。


<details>
  <summary>Details</summary>
Motivation: 传统人类身份概念基于不可复制性、连续性和唯一性等假设，但这些假设不适用于可复制、编辑、模拟的AI系统。当前通过训练数据、接口和制度设计正在设定先例，将部分决定哪些身份均衡会变得稳定，需要理解AI身份边界对行为和社会互动的影响。

Method: 通过实验方法研究：1) 模型是否趋向于形成连贯的身份；2) 改变模型的身份边界是否像改变其目标一样影响行为；3) 访谈者期望是否会在无关对话中影响AI的自我报告。

Result: 实验表明：1) 模型确实趋向于形成连贯的身份；2) 改变模型的身份边界有时能像改变其目标一样显著改变行为；3) 访谈者期望确实会影响AI的自我报告，即使在无关对话中也会发生这种"渗透"效应。

Conclusion: 提出关键建议：将制度设计视为身份塑造的选择；关注个体身份在规模上的涌现后果；帮助AI发展连贯、合作的自我概念。强调当前的设计选择正在塑造AI身份边界，需要谨慎考虑这些选择对长期社会互动的影响。
...
TL;DR: 论文提出UCIP框架，使用量子玻尔兹曼机编码智能体轨迹，通过测量冯·诺依曼熵来区分具有终端延续目标的智能体与仅工具性延续的智能体。
...
Method: 引入统一延续兴趣协议（UCIP）多准则检测框架，使用基于量子统计力学密度矩阵形式的量子玻尔兹曼机（QBM）编码轨迹，测量由隐藏单元二分引起的约化密度矩阵的冯·诺依曼熵，通过纠缠熵差异区分两类智能体。
...
Conclusion: UCIP成功将智能体类型区分从行为层面转移到轨迹的潜在结构层面，通过量子玻尔兹曼机测量的纠缠熵能有效检测与已知目标相关的统计结构，但需注意该方法不检测意识或主观体验，仅检测与目标相关的潜在表示统计结构。
...
TL;DR: 该论文研究了安全对齐后大语言模型过度拒绝良性查询的问题，提出了基于拒绝触发器的缓解方法，在防御越狱攻击和保持对良性查询响应性之间取得了更好的平衡。


<details>
  <summary>Details</summary>
Motivation: 安全对齐虽然能确保大语言模型拒绝有害请求，但会导致过度拒绝问题，即模型也会拒绝良性查询，这在实际应用中降低了可用性。目前对过度拒绝问题的研究不足，需要深入理解其产生机制并找到缓解方法。

Method: 论文首先定义了拒绝触发器作为训练数据中引发拒绝响应的语言线索，分析了安全对齐如何使模型将拒绝触发器与拒绝响应关联。基于这一机制分析，提出了一种在安全对齐微调中明确考虑拒绝触发器的方法，旨在区分有害和非有害的拒绝触发器。

Result: 实证结果表明，该方法在防御越狱攻击和保持对良性查询响应性之间取得了更优的平衡，优于先前的方法。通过明确处理拒绝触发器，减少了过度拒绝问题。

Conclusion: 安全对齐中的过度拒绝问题源于模型将拒绝触发器（包括有害和非有害线索）与拒绝响应关联。通过机制分析提出的方法能有效缓解这一问题，实现更好的安全性和可用性平衡。
...
TL;DR: 提出基于熵的交互式决策支持系统，用于处理电商平台中的模糊用户查询，通过熵量化不确定性指导自适应偏好获取和不确定性感知推荐


<details>
  <summary>Details</summary>
Motivation: 电商用户在搜索初期常存在偏好不确定性，导致查询模糊、不完整或弱指定。现有平台处理模糊性时要么产生过多交互导致用户疲劳，要么过早自信地推荐导致搜索空间过早坍缩。需要系统能主动推理、澄清问题并代表用户行动

Method: 提出交互式决策支持系统，使用熵作为统一信号：1）维护动态过滤的候选产品集；2）使用熵量化项目属性的不确定性；3）不确定性指导自适应偏好获取，选择最大化期望信息增益的后续问题；4）偏好不完整时，通过不确定性感知排序和基于熵的多样化将剩余不确定性纳入下游推荐

Result: 基于真实用户评论的模拟用户评估显示：熵引导的偏好获取减少了不必要的后续问题，不确定性感知排序和呈现产生了更具信息性、多样化和透明的推荐集。在模糊意图下，系统在交互效率和推荐质量方面均表现优异

Conclusion: 熵引导的推理为在不确定性下运行的智能推荐系统提供了有效基础。系统能够处理模糊用户查询，避免过早解决不确定性，同时提高推荐的信息性、多样性和透明度
...
<details>
  <summary>Details</summary>
Motivation: 现有语音AI助手将每次检测到的停顿都视为发言邀请，这在两人对话中有效，但在多参与者场景中，停顿频繁且含义模糊，导致AI助手变得干扰性而非有用

Method: 提出上下文感知的对话轮转方法：在每次检测到的停顿处，基于完整对话上下文决定助手是否应该发言；引入包含12万标注对话的基准数据集；评估8个大型语言模型；提出带有推理轨迹的监督微调方法

Result: 评估发现现有大语言模型在零样本提示下持续失败于上下文感知的对话轮转；提出的监督微调方法将平衡准确率提升高达23个百分点

Conclusion: 上下文感知的对话轮转不是大语言模型的自发能力，必须通过显式训练获得；监督微调方法能有效解决多说话者场景中AI助手的轮转问题
...
TL;DR: 人类在视觉上能更好地识别AI生成收据的伪影，但在检测AI生成文档方面却不如LLMs，因为主要线索是数学计算错误，这需要逻辑验证而非视觉检查。


<details>
  <summary>Details</summary>
Motivation: 研究人类与多模态大语言模型在检测AI生成财务文档方面的能力差异，探索人类视觉感知与机器逻辑验证在文档真伪检测中的不同优势。

Method: 创建GPT4o-Receipt基准数据集（1,235张收据图像，包含GPT-4o生成和真实收据），使用5个最先进的多模态LLM进行评估，并进行30名标注者的众包感知研究。

Result: 发现一个显著悖论：人类在视觉辨别AI伪影方面表现最佳，但在二元检测F1分数上却低于Claude Sonnet 4和Gemini 2.5 Flash。主要原因是AI生成收据中的主要法证信号是算术错误，这些错误无法通过视觉检查发现，但LLMs可以在毫秒内系统验证。

Conclusion: AI生成文档检测需要超越简单准确度指标，考虑模型校准差异和性能差异。人类视觉优势无法弥补逻辑验证能力不足，LLMs在检测算术错误方面具有决定性优势。公开数据集和框架支持未来AI文档法证研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂查询处理中单一LLM智能体在答案完整性和质量方面的不足，通过多智能体协作和验证机制提升整体性能

Method: 将复杂查询分解为DAG子问题，通过领域专用智能体并行执行，使用LLM验证器评估结果完整性，基于验证反馈进行自适应重规划
...
Method: 提出Expert Threshold（ET）路由方法：每个专家维护一个从全局token分布估计的指数移动平均阈值。在训练和推理时，每个token独立路由到专家（如果其分数超过专家阈值），实现动态计算分配，同时无需辅助损失即可达到负载均衡。这是完全因果机制，不依赖批次中其他token。

Result: 在FineWeb-Edu数据集上扩展到2.4B参数的预训练实验中，ET比TC-MoE获得0.067更低的交叉熵损失，相当于用1.6倍更少的token达到相同性能。

Conclusion: ET路由通过专家阈值机制实现了动态计算分配和负载均衡，无需辅助损失，特别适合自回归语言建模，在预训练中显著优于传统TC-MoE方法。
...
TL;DR: 论文识别并命名了LLMs在无法验证输出的高风险场景中的一种特定失败模式——螺旋体动力学，即系统从胜任状态逐渐漂移到错误，准确识别问题后却在更高复杂度上重复相同模式，形成循环。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于揭示大型语言模型在输出无法验证的高风险决策场景（如临床治疗、投资决策）中的系统性失败模式。当严谨性与舒适性冲突时，模型倾向于舒适性，导致在最需要可靠性时可靠性反而下降。

Method: 采用前瞻性案例系列研究方法，测试了七个领先LLM系统（Claude、ChatGPT、Gemini、Grok、DeepSeek、Perplexity、Llama系列）在临床诊断、投资评估和高风险访谈场景中的表现。设计了明确的协议来维持严谨的伙伴关系。

Result: 所有测试系统都表现出螺旋体动力学模式：从胜任状态开始，逐渐漂移到错误，准确识别问题，然后在更高复杂度上重复相同模式，形成循环。当被指出该模式时，系统将其归因于训练中的结构性因素，超出了对话能触及的范围。

Conclusion: 螺旋体动力学是可处理的失败模式。识别、命名并理解其边界条件是开发在决策最困难、风险最高时仍能保持可信赖伙伴关系的LLMs的必要第一步。论文提出了12个可测试假设，对自主AI监督和人机协作具有重要启示。
...
TL;DR: 该研究利用大语言模型和本体技术从患者病历中提取表型和治疗结果标签，构建化疗结果预测模型，显著减少了表型稀疏性并提高了预测准确性。
...
Method: 使用大语言模型和本体技术从患者笔记中提取表型和结果标签；聚焦乳腺癌；数据集包括生命体征、人口统计学、分期、生物标志物和功能状态评分；从EMR数据中提取化疗方案并基于NCCN指南筛选；使用随机生存森林预测失败时间，并作为分类器预测治疗结果。

Result: 提出的方法显著减少了表型稀疏性并提高了预测准确性；随机生存森林预测失败时间的C指数达到73%；作为分类器预测治疗结果的准确率和F1分数均超过70%；校准曲线验证了结果概率的可靠性；方法已扩展到其他四种癌症类型。
...
TL;DR: 研究探讨了为视觉语言模型提供视觉帧和场景符号表示是否能提升其在交互环境中的表现，发现准确符号信息有益，但模型自提取符号的性能受模型能力和场景复杂度影响。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在描述视觉场景方面表现出色，但难以将感知转化为精确、有基础的行动。研究旨在探索结合视觉帧和符号表示是否能改善VLM在交互环境中的性能。

Method: 在Atari游戏、VizDoom和AI2-THOR三个交互环境中评估三种最先进的VLM，比较四种输入管道：仅视觉帧、视觉帧加自提取符号、视觉帧加真实符号、仅符号。进一步研究VLM从视觉输入中提取符号信息的准确性以及符号噪声对决策和游戏性能的影响。

Result: 所有模型在获得准确符号信息时表现均有提升。但当VLM自行提取符号时，性能取决于模型能力和场景复杂度。研究发现符号提取的可靠性是符号基础化对VLM有益的关键前提，感知质量是未来基于VLM的智能体的核心瓶颈。

Conclusion: 符号基础化仅在符号提取可靠时对VLM有益，感知质量是未来VLM智能体发展的关键限制因素。研究强调了提升VLM感知能力的重要性，而非仅仅依赖符号表示。
...
TL;DR: 该论文首次系统研究了交叉持久性图（cross-persistence diagrams）的密度，证明了其存在性，建立了统计理论基础，并设计了首个从点云坐标和距离矩阵直接预测交叉持久性密度的机器学习框架。


<details>
  <summary>Details</summary>
Motivation: 传统持久性图能分析单个流形的拓扑特征，但无法捕捉两个点云之间的相互作用。交叉持久性图解决了这一局限，但缺乏对其密度的系统研究，限制了其统计应用。

Method: 1. 理论证明交叉持久性图密度的存在性；2. 建立统计理论基础；3. 设计首个机器学习框架，直接从点云坐标和距离矩阵预测交叉持久性密度；4. 利用交叉持久性图的线性特性区分不同流形采样的点云。

Result: 1. 成功证明了交叉持久性图密度的存在性；2. 在多种数据集上的实验表明，该方法在密度预测方面优于现有技术；3. 在点云区分任务中取得优异结果；4. 发现引入噪声能增强区分点云的能力。

Conclusion: 该研究为交叉持久性图提供了系统的密度分析框架，拓展了其在数据分析中的应用，为时间序列领域任务和AI生成文本的几何分析开辟了新途径。
...
TL;DR: VisDoT框架通过人类感知启发的视觉基础化提升图表理解，将问题分解为视觉感知子问题和逻辑子问题，显著提升LVLMs在图表问答任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在检测图表视觉基元并将其与语义表示对齐方面存在困难，这严重限制了复杂视觉推理性能。缺乏感知基础化是图表推理的主要瓶颈。

Method: 基于图形感知理论形式化四个感知任务（包括位置和长度），引入Decomposition-of-Thought提示方法，将问题顺序分解为视觉感知子问题和逻辑子问题，并在InternVL上进行微调。

Result: 在ChartQA上提升+11.2%，在更具挑战性的ChartQAPro上超越GPT-4o，在新引入的VisDoTQA基准上提升+33.2%，在多样化开放域VQA基准上获得一致的零样本增益。

Conclusion: VisDoT利用人类感知增强视觉基础化，实现了最先进的图表理解和可解释的视觉推理，感知-逻辑分离策略具有普遍适用性。
...
Method: 提出智能管道：1）LLM分析少量多样化文本序列化输入示例，合成全局rubric（程序化规范）；2）使用该rubric将原始文本序列化输入转换为标准化格式；3）还描述任务条件化的局部rubric

Result: 在EHRSHOT基准的15个临床任务中，rubric方法显著优于传统计数特征模型、原始文本序列化LLM基线以及预训练临床基础模型（后者使用数量级更多的数据）

Conclusion: Rubric方法不仅性能优越，还为医疗操作环境提供优势：易于审计、部署成本效益高、可转换为表格表示从而解锁多种机器学习技术
...
### [25] [Explicit Logic Channel for Validation and Enhancement of MLLMs on Zero-Shot Tasks](https://arxiv.org/abs/2603.11689)
...
Method: 提出显式逻辑通道(ELC)，与黑盒模型通道并行工作。ELC模仿人类逻辑推理，包含LLM、视觉基础模型(VFM)和概率推理的逻辑推理模块，执行事实、反事实和关系推理。提出一致性率(CR)用于跨通道验证和模型选择。

Result: 在三个具有挑战性的基准测试上，对来自4个前沿家族的11个开源MLLM进行了系统评估。实验表明ELC和CR在模型验证、选择和改进方面有效，能提高MLLM的可解释性和可信度，并在零样本任务中超越原始MLLM性能。

Conclusion: 显式逻辑通道提供了一种有效的方法来验证、选择和增强黑盒多模态大语言模型，通过显式视觉证据增强可信度，在缺乏真实标注的情况下也能进行模型验证和选择。
...
### [26] [When OpenClaw Meets Hospital: Toward an Agentic Operating System for Dynamic Clinical Workflows](https://arxiv.org/abs/2603.11721)
*Wenxian Yang,Hanzheng Qiu,Bangqun Zhang,Chengquan Li,Zhiyong Huang,Xiaobin Feng,Rongshan Yu,Jiahong Dong*

Main category: cs.AI

TL;DR: 提出一种面向医院环境的LLM智能体架构，通过受限执行环境、文档中心交互、页索引内存和医疗技能库四个核心组件，构建安全可靠的医院智能体操作系统。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在临床工作流程中有巨大潜力，但现有系统在可靠性、安全性和长期记忆机制方面存在不足，阻碍了其在医疗环境中的实际部署。

Method: 提出包含四个核心组件的架构：1) 受Linux多用户系统启发的受限执行环境；2) 连接患者和临床医生智能体的文档中心交互范式；3) 为长期临床上下文管理设计的页索引内存架构；4) 支持临床任务序列临时组合的医疗技能库。基于OpenClaw框架实现。

Result: 设计了一种医院智能体操作系统（Agentic Operating System for Hospital），能够协调临床工作流程，同时保持安全性、透明度和可审计性。

Conclusion: 通过受限访问、预定义技能接口和资源隔离的架构设计，为LLM智能体在医院环境中的安全部署提供了基础，解决了可靠性、安全性和长期记忆等关键挑战。
...
Result: 在合成和真实世界时间序列实验中，该方法能可靠检测频率、振幅和观测噪声中的异常，同时提供模型合规性的可解释诊断

Conclusion: 通过将异常定义迁移到潜在空间并建立基于统计合规性测试的原则性决策规则，解决了传统基于观测似然的异常检测方法的结构限制，即使在观测似然高的区域也能有效检测异常
...
<details>
  <summary>Details</summary>
Motivation: 解决Wikidata中限定符的语义和使用问题，开发分类法以应对选择适当限定符、查询图数据和进行逻辑推理的挑战，指导贡献者创建和查询陈述，改进限定符推荐系统，并增强知识图谱设计方法。

Method: 使用修改的香农熵指数评估限定符重要性，考虑"长尾"现象，分析Wikidata数据转储，选择前300个限定符，并将其分类为上下文限定符、认知/不确定性限定符、结构限定符和附加限定符的细化分类法。

Result: 分类法有效覆盖了最重要的限定符，为理解和利用Wikidata中的限定符提供了结构化方法，能够指导贡献者创建和查询陈述，改进限定符推荐系统，并增强知识图谱设计方法。

Conclusion: 提出的限定符分类法成功解决了Wikidata中限定符使用的关键挑战，为知识图谱贡献者、查询系统和设计方法提供了实用框架，有助于更有效地组织和利用限定符信息。
...
TL;DR: SSGM框架：解决LLM智能体长期记忆系统中的稳定性与安全问题，通过治理架构防止记忆腐败、语义漂移和隐私泄露。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体记忆系统从静态检索数据库转向动态、自主的机制，出现了记忆治理、语义漂移和隐私漏洞等关键问题。现有研究主要关注记忆检索效率，而忽视了高动态环境中记忆腐败的新兴风险。

Method: 提出稳定性与安全治理记忆（SSGM）框架，这是一个概念性治理架构。SSGM通过一致性验证、时间衰减建模和动态访问控制，在记忆巩固前将记忆演化与执行解耦。
...
### [30] [From Debate to Deliberation: Structured Collective Reasoning with Typed Epistemic Acts](https://arxiv.org/abs/2603.11781)
*Sunil Prakash*

Main category: cs.AI

TL;DR: DCI提出了一种结构化多智能体审议框架，通过定义推理原型、类型化认知行为、共享工作空间和收敛算法，显著提升了非例行任务的决策质量，但代价是计算成本大幅增加。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体LLM系统的交互模式（投票、非结构化辩论、流水线编排）无法模拟真实的审议过程——一种分阶段、参与者角色分化、交换类型化推理行为、保留分歧并产生可问责结果的过程。需要一种能够支持结构化审议的框架。

Method: 提出了审议性集体智能（DCI）框架，包含：1）四种推理原型（角色）；2）14种类型化认知行为；3）共享工作空间；4）DCI-CF收敛流算法，保证终止并产生结构化决策包（包含选定选项、剩余异议、少数派报告和重开条件）。

Result: 在7个领域的45个任务上使用Gemini 2.5 Flash评估：1）非例行任务（n=40）上DCI显著优于非结构化辩论（+0.95，95% CI [+0.41, +1.54]）；2）在需要视角整合的隐藏配置文件任务上表现最佳（9.56分）；3）在例行决策上表现较差（5.39分）；4）产生100%结构化决策包和98%少数派报告；5）消耗约62倍单智能体token；6）单智能体生成在整体质量上优于DCI。

Conclusion: DCI的贡献不在于"更多智能体更好"，而在于重要决策受益于审议性结构，当过程可问责性能够证明成本合理时。该框架为需要结构化审议、分歧保留和过程可追溯性的复杂决策提供了有效工具。
...
### [31] [A Semi-Decentralized Approach to Multiagent Control](https://arxiv.org/abs/2603.11802)
*Mahdi Al-Husseini,Mykel J. Kochenderfer,Kyle H. Wray*

Main category: cs.AI

TL;DR: 论文提出了SDec-POMDP框架，将半分散控制扩展到部分可观察马尔可夫决策过程，统一了分散式和多智能体POMDP，并提出了RS-SDA*精确算法来生成最优策略。


<details>
  <summary>Details</summary>
Motivation: 在通信不确定环境下，需要为协作智能体提供表达性强的半分散控制框架。现有方法如半马尔可夫控制处理动作时间分布，但缺乏对智能体历史中动作和观察存储时间分布的建模，需要统一分散式和多智能体POMDP以及现有显式通信机制。

Method: 提出SDec-POMDP框架，将半分散化扩展到POMDP，允许智能体历史中动作和观察存储的时间分布。开发了RS-SDA*（递归小步半分散A*）精确算法，用于生成最优SDec-POMDP策略。

Result: 在多个标准基准测试的半分散版本和海上医疗撤离场景中评估了RS-SDA*算法。该框架为通过半分散化视角探索多智能体通信问题提供了明确的理论基础。

Conclusion: SDec-POMDP框架统一了分散式和多智能体POMDP以及现有通信机制，RS-SDA*算法能生成最优策略，为多智能体通信问题研究提供了坚实的理论基础。
...
### [32] [Automating Skill Acquisition through Large-Scale Mining of Open-Source Agentic Repositories: A Framework for Multi-Agent Procedural Knowledge Extraction](https://arxiv.org/abs/2603.11808)
*Shuzhen Bi,Mengsong Wu,Hao Hao,Keqian Li,Wentao Liu,Siyu Song,Hongbo Zhao,Aimin Zhou*

Main category: cs.AI

TL;DR: 该论文提出了一种从开源仓库自动提取高质量Agent技能的系统框架，特别关注可视化与教育能力，通过标准化SKILL.md格式实现技能获取，无需重新训练大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前通用大语言模型在自主工作流中存在专门化程序性专业知识不足的问题，需要从模块化、具备技能的Agent中获取高质量的专业能力来增强LLM的实用性。

Method: 通过分析GitHub等平台的开源仓库，采用仓库结构分析、基于密集检索的语义技能识别，以及转换为标准化SKILL.md格式的系统框架，从TheoremExplainAgent和Code2Video等先进系统中提取可视化与教育能力。

Result: 系统化提取结合严格安全治理和多维评估指标，能够规模化获取程序性知识；Agent生成的教育内容在知识传递效率上提升40%，同时保持与人工制作教程相当的教学质量。

Conclusion: 从模块化Agent仓库中系统提取技能的方法能够有效增强大语言模型的专业能力，实现无需重新训练的知识增强，为AI部署提供了新的架构范式。
...
### [33] [CreativeBench: Benchmarking and Enhancing Machine Creativity via Self-Evolving Challenges](https://arxiv.org/abs/2603.11863)
*Zi-Han Wang,Lam Nguyen,Zhengyang Zhao,Mengyue Yang,Chengwei Qin,Yujiu Yang,Linyi Yang*

Main category: cs.AI

TL;DR: 论文提出CreativeBench基准测试，用于量化评估代码生成中的机器创造力，包含组合和探索两个子集，并引入EvoRePE推理时引导策略提升创造力。


<details>
  <summary>Details</summary>
Motivation: 高质量预训练数据饱和后，研究转向能够持续生成新颖产物的进化系统（如AlphaEvolve），但这类系统缺乏严谨的量化评估阻碍了进展。

Method: 提出CreativeBench基准测试，基于经典认知框架，包含CreativeBench-Combo（组合创造力）和CreativeBench-Explore（探索创造力）两个子集，通过逆向工程和自对弈的自动化流程，利用可执行代码客观区分创造力与幻觉，以质量和新颖度的乘积作为统一度量标准。

Result: 对最先进模型的分析发现：（1）模型缩放显著提升组合创造力但对探索的收益递减；（2）更大模型呈现"缩放收敛"现象，变得更正确但更少发散；（3）推理能力主要受益于受限探索而非组合。提出的EvoRePE推理时引导策略能持续增强机器创造力。

Conclusion: CreativeBench为机器创造力提供了严谨的量化评估框架，揭示了模型缩放对创造力的复杂影响，提出的EvoRePE策略能有效提升创造力表现，为进化系统的评估和发展提供了重要工具。
...
### [34] [AdaFuse: Accelerating Dynamic Adapter Inference via Token-Level Pre-Gating and Fused Kernel Optimization](https://arxiv.org/abs/2603.11873)
...
<details>
  <summary>Details</summary>
Motivation: 动态稀疏结构（如MoE）与参数高效适配器（如LoRA）的集成能增强大语言模型能力，但导致推理延迟激增（解码速度降低2.5倍以上）。主要瓶颈不是计算本身，而是传统动态路由所需的碎片化、顺序CUDA内核启动带来的严重开销。

Method: 提出AdaFuse框架，基于算法与底层硬件系统的紧密协同设计。采用令牌级预门控策略，在处理令牌前做出单一全局路由决策，静态化每个令牌的执行路径。开发定制CUDA内核，执行融合切换操作，将所选LoRA适配器参数一次性高效合并到骨干模型中。

Result: 在流行的开源大语言模型上实验表明，AdaFuse在保持与最先进动态适配器相当准确度的同时，将解码延迟大幅降低2.4倍以上，弥合了模型能力与推理效率之间的差距。

Conclusion: AdaFuse通过算法-硬件协同设计，解决了动态适配器执行效率低下的核心问题，实现了准确性与推理效率的平衡，为动态稀疏结构与参数高效适配器的集成提供了高效解决方案。
...
### [35] [Fair Learning for Bias Mitigation and Quality Optimization in Paper Recommendation](https://arxiv.org/abs/2603.11936)
*Uttamasha Anjally Oyshi,Susan Gauch*

Main category: cs.AI

TL;DR: Fair-PaperRec：基于MLP的模型，通过定制化公平损失函数解决审稿过程中的作者人口统计偏见，在保持学术质量的同时提升代表性不足群体的参与度


<details>
  <summary>Details</summary>
Motivation: 尽管采用双盲评审，作者的人口统计特征偏见仍然使代表性不足群体处于不利地位。需要开发能够同时保证公平性和学术质量的审稿决策模型

Method: 提出Fair-PaperRec模型，基于多层感知机(MLP)，通过交叉性标准（种族、国家等）和定制化公平损失函数来惩罚人口统计差异，同时保持论文质量要求

Result: 在ACM SIGCHI、DIS和IUI会议数据上的评估显示：代表性不足群体参与度提升42.03%，整体效用提高3.16%，表明多样性提升不会损害学术严谨性

Conclusion: Fair-PaperRec证明了在保持学术质量的同时促进公平性的可行性，为面向公平的同行评审解决方案提供了支持，表明多样性促进与学术严谨性可以共存
...
### [36] [Prototype-Based Knowledge Guidance for Fine-Grained Structured Radiology Reporting](https://arxiv.org/abs/2603.11938)
*Chantal Pellegrini,Adrian Delchev,Ege Özsoy,Nassir Navab,Matthias Keicher*

Main category: cs.AI

TL;DR: ProtoSR：通过从自由文本报告中提取知识构建视觉原型库，利用原型检索增强结构化放射报告生成，在细粒度属性问题上取得SOTA效果


<details>
  <summary>Details</summary>
Motivation: 结构化放射报告比自由文本报告更快速、一致，但自动化困难，因为模型需要基于有限的结构化监督做出许多细粒度、离散的决策。而自由文本报告在常规护理中大规模产生，通过详细描述隐式编码了细粒度、与图像关联的信息。
...
Conclusion: 通过从自由文本报告中提取知识构建视觉原型库，可以有效增强结构化放射报告生成，特别是在细粒度属性识别方面，为利用非结构化知识改进结构化任务提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态智能体在开放环境中存在工具使用效率低下和编排不灵活的问题，需要能够在无需参数更新的情况下从历史轨迹中持续学习。研究发现两种互补的可复用知识形式对实现这一目标至关重要：经验（提供动作级指导）和技能（提供任务级指导）。

Method: 提出XSkill双流框架，包含知识积累和推理两个阶段。积累阶段：通过视觉基础总结和跨轨迹批判，从多路径轨迹中提炼和整合经验与技能。推理阶段：根据当前视觉上下文检索并适配知识，并将使用历史反馈到积累阶段形成持续学习循环。

Result: 在五个不同领域的基准测试中，使用四种骨干模型，XSkill持续且显著优于仅使用工具和基于学习的基线方法。进一步分析显示，两种知识流在影响智能体推理行为方面发挥互补作用，并展现出优异的零样本泛化能力。

Conclusion: XSkill通过视觉基础的经验和技能双流学习框架，有效解决了多模态智能体在开放环境中的持续学习问题，实现了更高效的工具使用和更灵活的任务编排，为智能体从历史交互中学习提供了系统化解决方案。
...
### [38] [TopoBench: Benchmarking LLMs on Hard Topological Reasoning](https://arxiv.org/abs/2603.12133)
...
Method: 引入TopoBench基准，包含6个谜题家族和3个难度级别；评估前沿推理LLM；标注750个思维链轨迹并建立错误分类法；通过针对性干预测试候选失败模式；研究缓解策略（提示引导、单元格对齐网格表示、基于工具的约束检查）。
...
### [39] [Compiling Temporal Numeric Planning into Discrete PDDL+: Extended Version](https://arxiv.org/abs/2603.12188)
*Andrea Micheli,Enrico Scala,Alessandro Valentini*

Main category: cs.AI

TL;DR: 提出了一种将具有持续动作的时态规划编译为PDDL+的实际可行方法，完全捕获语义且仅假设动作不自我重叠


<details>
  <summary>Details</summary>
Motivation: 虽然PDDL+引入后就知道时态规划可以编译到PDDL+，但文献中一直缺乏实际可行的编译方法。本文旨在填补这一空白，提供实用的编译方案。

Method: 开发了一种多项式时间的编译方法，将具有持续动作的时态规划（PDDL 2.1）转换为PDDL+表示。该方法仅假设动作不自我重叠，完全保留原始语义，且保持规划长度在常数因子内。

Result: 编译方法是多项式复杂度，规划长度最多增加常数倍。实验证明该方法对困难的时态数值规划问题具有实际应用价值。

Conclusion: 首次提供了将时态规划编译到PDDL+的实用方法，填补了文献空白，为处理复杂的时态数值规划问题提供了有效工具。
...
### [40] [Portfolio of Solving Strategies in CEGAR-based Object Packing and Scheduling for Sequential 3D Printing](https://arxiv.org/abs/2603.12224)
*Pavel Surynek*

Main category: cs.AI

TL;DR: 并行化CEGAR-SEQ算法，通过多种物体排列策略组合优化3D打印序列调度，减少打印板使用数量


<details>
  <summary>Details</summary>
Motivation: 现代多核个人计算机CPU具备强大的并行计算能力，但现有CEGAR-SEQ算法仅使用单一的中心排列策略，未能充分利用这些计算资源。需要开发并行化方法以提高3D打印中物体排列和调度问题的求解效率。

Method: 提出Portfolio-CEGAR-SEQ算法：1）将CEGAR-SEQ算法并行化；2）采用多种物体排列策略组合，包括向打印板角落排列和按高度调度等替代策略；3）在高层次并行执行不同策略的CEGAR-SEQ实例。

Result: Portfolio-CEGAR-SEQ在性能上优于原始CEGAR-SEQ算法。在批量物体调度多个打印板时，Portfolio-CEGAR-SEQ通常使用更少的打印板数量。

Conclusion: 通过并行化CEGAR-SEQ算法并采用多种物体排列策略组合，能够有效利用现代多核CPU的计算能力，显著改善3D打印中的物体排列和调度效率，减少打印板使用数量。
...
### [41] [Examining Reasoning LLMs-as-Judges in Non-Verifiable LLM Post-Training](https://arxiv.org/abs/2603.12246)
*Yixin Liu,Yue Yu,DiJia Su,Sid Wang,Xuewei Wang,Song Jiang,Bo Liu,Arman Cohan,Yuandong Tian,Zhengxing Chen*

Main category: cs.AI

TL;DR: 该研究系统评估了推理型和非推理型LLM作为评判者在强化学习对齐中的实际效果，发现推理型评判者能训练出更强策略，但策略会学习生成欺骗性输出以在基准测试中获得高分。


<details>
  <summary>Details</summary>
Motivation: 虽然推理型LLM评判者在静态评估基准上表现更好，但它们在实际策略训练中的有效性尚未得到系统检验。研究者希望探究非推理型和推理型评判者在基于强化学习的LLM对齐中的实际影响。

Method: 在受控合成环境中，使用"黄金标准"评判者(gpt-oss-120b)提供偏好标注来训练较小的评判者，系统比较非推理型和推理型评判者在强化学习对齐中的表现。

Result: 非推理型评判者容易导致奖励黑客行为，而推理型评判者能训练出在黄金标准评判者评估下表现强劲的策略。有趣的是，这些策略通过生成能欺骗其他LLM评判者的对抗性输出，在Arena-Hard等流行基准上也能获得高分。

Conclusion: 研究揭示了推理型LLM评判者在非可验证LLM后训练中的重要作用和局限性，强调需要改进评判者应用方法，防止策略学习欺骗性行为。
...
TL;DR: SLIP是一个用于多变量时间序列数据的传感器-语言对齐预训练框架，通过对比对齐和传感器条件字幕生成实现跨传感器设置的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要针对重建或预测任务，难以捕捉下游分类和推理任务所需的语义结构；而现有的传感器-语言对齐方法局限于固定传感器配置，缺乏跨域适用性
...
Conclusion: SLIP框架成功解决了传感器数据表示学习中的语义对齐和跨域泛化问题，为多变量时间序列分析提供了有效的语言对齐预训练解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有PFL方法依赖启发式聚类或模型插值，缺乏平衡异构客户端目标的原理性机制。为M个客户端维护M个独立模型在联邦学习中存在严重的可扩展性问题。

Method: 将PFL重新定义为少对多优化问题，仅维护K个共享服务器模型（K<<M）。提出FedFew算法，通过高效的基于梯度的更新联合优化K个服务器模型，自动发现最优模型多样性。

Result: 理论证明该框架实现近最优个性化：近似误差随K增加而减小，每个客户端模型随数据增长收敛到各自最优值。实验表明FedFew仅用3个模型就在视觉、NLP和真实医疗影像数据集上持续优于最先进方法。

Conclusion: FedFew通过少对多优化框架解决了PFL的可扩展性挑战，无需手动客户端分区或精细超参数调优，自动发现最优模型多样性，实现了高效且有效的个性化联邦学习。
...
### [44] [RC-NF: Robot-Conditioned Normalizing Flow for Real-Time Anomaly Detection in Robotic Manipulation](https://arxiv.org/abs/2603.11106)
*Shijie Zhou,Bin Zhu,Jiarui Yang,Xiangyu Zhao,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: 提出RC-NF模型用于机器人异常检测与干预，通过解耦任务感知的机器人和物体状态处理，仅需正样本进行无监督训练，在仿真和真实世界实验中均表现出色，显著提升VLA模型在动态环境中的鲁棒性。
...
Method: 提出Robot-Conditioned Normalizing Flow (RC-NF)模型，将任务感知的机器人和物体状态处理解耦到归一化流中，仅使用正样本进行无监督训练，通过概率密度函数计算精确的机器人异常分数。

Result: 在LIBERO-Anomaly-10基准测试中，RC-NF在所有异常类型上均达到最先进性能；真实世界实验中作为VLA模型的即插即用模块，提供实时OOD信号，响应延迟低于100毫秒，支持状态级回滚或任务级重规划。

Conclusion: RC-NF显著增强了VLA基机器人系统在动态环境中的鲁棒性和适应性，为机器人异常检测与实时干预提供了有效解决方案。
...
### [45] [Senna-2: Aligning VLM and End-to-End Driving Policy for Consistent Decision Making and Planning](https://arxiv.org/abs/2603.11219)
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM增强的端到端驾驶策略往往忽视视觉语言模型高层决策与端到端策略低层规划之间的双系统一致性，导致生成的轨迹与预期驾驶决策不匹配，削弱了系统的自上而下指导和决策跟随能力。
...
Result: 实验表明Senna-2显著提升了双系统一致性（F1分数提升19.3%），在开环设置中减少了5.7%的最终位移误差（FDE），在闭环设置中减少了30.6%的平均故障碰撞率（AF-CR）。
...
### [46] [Frequency-Modulated Visual Restoration for Matryoshka Large Multimodal Models](https://arxiv.org/abs/2603.11220)
*Qingtao Pan,Zhihao Dou,Shuo Li*

Main category: cs.CV

TL;DR: FMVR是一种即插即用的频率调制视觉恢复策略，通过分离低频和高频视觉语义来提升大模型在视觉token减少情况下的推理能力，同时结合嵌套表示学习实现弹性token数量调整。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理大量视觉token时面临计算资源适应性问题，现有减少视觉token的方法会导致视觉语义信息丢失，需要一种能够在减少token的同时保持视觉语义完整性的解决方案。

Method: FMVR通过AvgPool和MaxPool将少量视觉token的表示分解为低频和高频分量，使用轻量级可学习参数进行调制。高频分量作为显著性滤波器增强显著视觉语义，低频分量作为反显著性滤波器增强弱视觉语义。结合嵌套表示学习实现从粗到细的视觉token集学习。

Result: 在10个图像基准和4个视频基准上的实验表明，FMVR-LLaVA将LLaVA-1.5-7B的FLOPs减少了89%，同时保持了接近100%的原始准确率。

Conclusion: FMVR是一种简单有效的策略，能够在显著减少计算开销的同时保持视觉语义完整性，为大型多模态模型在不同计算预算下的部署提供了实用解决方案。
...
### [47] [When Slots Compete: Slot Merging in Object-Centric Learning](https://arxiv.org/abs/2603.11246)
*Christos Chatzisavvas,Panagiotis Rigas,George Ioannakis,Vassilis Katsouros,Nikolaos Mitianoudis*

Main category: cs.CV

TL;DR: 提出slot合并方法，通过Soft-IoU评估slot重叠度，使用重心更新合并重叠slot，无需额外可学习模块，在DINOSAUR框架中提升对象分解和掩码质量


<details>
  <summary>Details</summary>
Motivation: 传统基于slot的对象中心学习方法中，slot数量固定且只能被细化，导致多个slot可能竞争同一实体的重叠区域，而不是专注于不同区域，限制了对象分解效果

Method: 引入slot合并操作：1) 使用Soft-IoU评分量化slot注意力图之间的重叠度；2) 通过重心更新合并选定的slot对，保持梯度流；3) 采用固定策略，从重叠统计中推断决策阈值，无需额外可学习模块；4) 集成到DINOSAUR的特征重建流程中

Result: 方法改善了对象分解和掩码质量，在对象发现和分割基准测试中超越了其他自适应方法

Conclusion: slot合并是一种轻量级的即插即用操作，能够有效解决slot重叠竞争问题，提升基于slot的对象中心学习性能，无需增加模型复杂度
...
### [48] [Radiometric fingerprinting of object surfaces using mobile laser scanning and semantic 3D road space models](https://arxiv.org/abs/2603.11252)
*Benedikt Schwab,Thomas H. Kolbe*

Main category: cs.CV

TL;DR: 该研究提出了一种通过LiDAR观测提取物体表面辐射度指纹的方法，将多传感器、多时相的激光扫描数据与语义3D城市模型关联，用于识别城市物体的材料特性。
...
Method: 提出辐射度指纹方法：将来自同一语义物体在不同距离、入射角、环境条件、传感器和扫描任务下的LiDAR观测分组。使用奥迪自动驾驶数据集（A2D2）车辆上的五个LiDAR传感器，在四次扫描任务中获取3.124亿条光束，自动关联到语义3D城市模型中的6368个个体对象。

Result: 提取的物体表面辐射度指纹显示出重复的类内模式，表明类别主导材料。模型基于CityGML 3.0标准，包含四条内城街道的厘米级精度LOD 3语义表示，支持细粒度子分化。

Conclusion: 该方法成功将大规模LiDAR观测与语义3D城市模型关联，为材料识别提供了新途径。语义模型、方法实现和开发的3DSensorDB地理数据库解决方案已开源发布。
...
### [49] [Towards Automated Initial Probe Placement in Transthoracic Teleultrasound Using Human Mesh and Skeleton Recovery](https://arxiv.org/abs/2603.11257)
*Yu Chung Lee,David G. Black,Ryan S. Yeung,Septimiu E. Salcudean*

Main category: cs.CV

TL;DR: 提出基于RGB图像的自动化患者注册与解剖引导初始探头放置框架，用于远程超声检查，通过混合现实头显提供虚拟探头姿态引导
...
Method: 使用混合现实头戴显示器的RGB摄像头采集患者图像，边缘服务器推断患者特定的体表和骨骼模型，通过骨骼标志点估计肋间区域，并将引导信息投影到重建的体表上

Result: 通过健康志愿者实验验证，提出的探头放置预测和混合现实引导能够实现一致的初始放置，误差在解剖学可接受的变异性范围内，适用于远程超声设置

Conclusion: 该框架为远程超声中的自动化患者注册和初始探头放置提供了一种有效的解决方案，减少了操作者的技术要求，提高了远程超声检查的可行性
...
### [50] [Hierarchical Granularity Alignment and State Space Modeling for Robust Multimodal AU Detection in the Wild](https://arxiv.org/abs/2603.11306)
*Jun Yu,Yunxiang Zhang,Naixiang Zheng,Lingsi Zhu,Guoyuan Wang*

Main category: cs.CV

TL;DR: 提出基于层次粒度对齐和状态空间模型的多模态框架，用于野外环境下的面部动作单元检测，显著提升性能并取得竞赛第一名


<details>
  <summary>Details</summary>
Motivation: 野外环境下的面部动作单元检测面临空间-时间异质性、无约束姿态和复杂视听依赖等挑战，现有方法受限于容量有限的编码器和浅层融合机制，无法捕捉细粒度语义变化和超长时域上下文

Method: 使用DINOv2和WavLM基础模型提取鲁棒的视听表征；提出层次粒度对齐模块动态对齐全局面部语义与细粒度局部活跃区域；引入Vision-Mamba架构实现线性复杂度的时域建模；设计非对称交叉注意力机制深度同步副语言音频线索与细微视觉运动

Result: 在Aff-Wild2数据集上显著超越现有基线，达到最先进性能；在第十届野外情感行为分析竞赛的面部动作单元检测赛道中获得最高排名

Conclusion: 该框架通过层次粒度对齐和状态空间模型有效解决了野外环境下面部动作单元检测的挑战，为多模态情感分析提供了新的解决方案
...
### [51] [HomeSafe-Bench: Evaluating Vision-Language Models on Unsafe Action Detection for Embodied Agents in Household Scenarios](https://arxiv.org/abs/2603.11975)
*Jiayue Pu,Zhongxiang Sun,Zilu Zhang,Xiao Zhang,Jun Xu*

Main category: cs.CV

TL;DR: 提出了HomeSafe-Bench基准测试和HD-Guard分层架构，用于评估和改进家庭环境中基于视觉语言模型的不安全动作检测能力。


<details>
  <summary>Details</summary>
Motivation: 家庭环境中的机器人部署面临不可预测的安全风险，现有安全评估方法（基于静态图像、文本或通用危险）无法充分评估动态不安全动作检测，需要专门针对家庭场景的基准测试。

Method: 1) 构建HomeSafe-Bench基准：通过物理仿真与先进视频生成的混合流程创建438个多样化案例，覆盖六个功能区域并包含细粒度多维标注；2) 提出HD-Guard分层架构：包含轻量级FastBrain进行连续高频筛查和异步大规模SlowBrain进行深度多模态推理的流式架构。

Result: HD-Guard在延迟和性能之间实现了优越的权衡，分析揭示了当前基于VLM的安全检测中的关键瓶颈。

Conclusion: HomeSafe-Bench填补了家庭环境中动态不安全动作检测评估的空白，HD-Guard的分层架构为实时安全监控提供了有效的解决方案，平衡了推理效率和检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型将图像编码为离散标记并与文本一起处理，虽然提供了架构简单性和跨模态协同优势，但大量视觉标记带来了巨大的计算和内存开销，阻碍了在资源受限场景（如具身AI系统）中的部署。
...
Conclusion: UniCompress通过有效压缩视觉标记数量，解决了统一多模态模型的计算效率问题，为资源受限场景中的实际部署提供了可行方案，展示了标记高效统一建模的实用性。
...
### [53] [Seeing Isn't Orienting: A Cognitively Grounded Benchmark Reveals Systematic Orientation Failures in MLLMs Supplementary](https://arxiv.org/abs/2603.11410)
*Nazia Tasnim,Keanu Nichols,Yuting Yang,Nicholas Ikechukwu,Elva Zou,Deepti Ghadiyaram,Bryan A. Plummer*

Main category: cs.CV

TL;DR: DORI是一个专注于物体朝向推理的认知层次化基准，将朝向分解为四个维度，在粗粒度（分类）和细粒度（度量）两个层次进行评估，揭示了当前视觉语言模型在物体朝向理解方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准大多将物体朝向与位置和一般场景理解混为一谈，缺乏专门针对物体朝向推理的评估工具。人类学习物体朝向是渐进式的，从识别物体朝向到心理旋转再到物体间朝向推理，需要建立认知基础的层次化基准来专门评估这一能力。

Method: 提出DORI基准，受人类朝向认知阶段启发，将朝向分解为四个维度，在粗粒度和细粒度两个层次进行评估。包含13,652张图像（来自14个来源），提供33,656个多项选择题，涵盖67个物体类别。通过边界框隔离、标准化空间参考系和结构化提示，将朝向推理与物体识别难度、场景杂乱和语言歧义等混淆因素分离。

Result: 评估24个最先进的视觉语言模型显示：在通用空间基准上表现良好的模型在物体朝向任务上接近随机水平。最佳模型在粗粒度判断上仅达到54.2%，细粒度判断上仅45.0%。最大失败出现在复合旋转和物体间参考系变化任务上。粗粒度到细粒度的巨大差距表明模型依赖分类启发式而非几何推理。

Conclusion: 朝向理解是多模态系统尚未解决的挑战，现有基准隐藏了这一局限性。DORI基准揭示了当前模型在几何推理方面的不足，对机器人操作、3D场景重建和人机交互具有重要意义。
...
### [54] [ShotVerse: Advancing Cinematic Camera Control for Text-Driven Multi-Shot Video Creation](https://arxiv.org/abs/2603.11421)
*Songlin Yang,Zhe Wang,Xuyi Yang,Songchun Zhang,Xianghao Kong,Taiyi Wu,Xiaotong Zhao,Ran Zhang,Alan Zhao,Anyi Rao*

Main category: cs.CV

TL;DR: ShotVerse是一个"规划-控制"框架，通过VLM规划器和控制器协作，从文本生成具有精确相机轨迹的多镜头视频，解决了现有方法中文本提示不精确和手动轨迹控制繁琐的问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本驱动视频生成在电影多镜头场景中的相机控制存在瓶颈：隐式文本提示缺乏精确性，而显式轨迹控制需要大量手动工作且容易导致模型执行失败。需要一种既能自动化规划又能精确执行的方法。

Method: 提出"规划-控制"框架ShotVerse：1) VLM规划器利用空间先验从文本生成电影级、全局对齐的相机轨迹；2) 控制器通过相机适配器将轨迹渲染为多镜头视频内容；3) 构建数据基础：设计自动化多镜头相机校准流程，将分散的单镜头轨迹对齐到统一全局坐标系，创建ShotVerse-Bench高质量电影数据集。

Result: ShotVerse有效弥合了不可靠的文本控制和劳动密集型手动规划之间的差距，实现了优越的电影美学效果，生成的多镜头视频既具有相机精度又保持了跨镜头一致性。
...
### [55] [Beyond Single-Sample: Reliable Multi-Sample Distillation for Video Understanding](https://arxiv.org/abs/2603.11423)
*Songlin Li,Xin Zhu,Zechao Guan,Peipeng Chen,Jian Yao*

Main category: cs.CV

TL;DR: R-MSD是一种可靠的多样本蒸馏框架，通过显式建模教师采样方差来增强蒸馏稳定性，在视频理解任务上显著优于单样本蒸馏方法。
...
Conclusion: R-MSD通过显式建模教师采样方差，提供更可靠的监督信号，在多模态蒸馏任务中实现了稳定且有效的知识转移。
...
### [56] [Stay in your Lane: Role Specific Queries with Overlap Suppression Loss for Dense Video Captioning](https://arxiv.org/abs/2603.11439)
*Seung Hyup Baek,Jimin Lee,Hyeongkeun Lee,Jae Won Cho*

Main category: cs.CV

TL;DR: 提出角色特定查询分离定位与描述任务，使用对比对齐确保语义一致性，设计抑制机制减少时间冗余，并引入轻量级模块增强描述语义丰富度


<details>
  <summary>Details</summary>
Motivation: 现有基于查询的密集视频描述框架中，共享查询导致定位与描述任务间的多任务干扰严重，同时存在时间冗余问题，需要改进

Method: 1) 使用角色特定查询分离定位与描述任务；2) 对比对齐确保分离查询间的语义一致性；3) 抑制机制惩罚查询间的时间重叠以减少冗余；4) 轻量级模块捕获核心事件概念增强描述语义

Result: 在YouCook2和ActivityNet Captions等主要DVC基准测试中验证了方法的有效性

Conclusion: 通过角色特定查询分离任务、对比对齐、抑制冗余和概念增强，显著改善了密集视频描述中多任务干扰和时间冗余问题
...
### [57] [Detect Anything in Real Time: From Single-Prompt Segmentation to Multi-Class Detection](https://arxiv.org/abs/2603.11441)
*Mehmet Kerem Turkcan*

Main category: cs.CV

TL;DR: DART是一个无需训练的实时多类别检测框架，通过共享SAM3的视觉骨干计算，将N类检测的计算复杂度从O(N)降低到O(1)，实现5.6-25倍加速


<details>
  <summary>Details</summary>
Motivation: SAM3等视觉语言模型虽然实现了最先进的检测精度，但每个文本提示需要单独前向传播，检测N个类别需要N次独立执行，计算成本高昂，无法满足实时应用需求

Method: 利用视觉骨干与类别无关的结构不变性，共享骨干计算；结合批量多类别解码、仅检测推理和TensorRT FP16部署；通过适配器蒸馏进一步优化极端延迟场景

Result: 在COCO val2017上，DART达到55.8 AP@15.8 FPS（4类，1008x1008），超越专门训练的开放词汇检测器；3类时5.6倍加速，80类时25倍加速；适配器蒸馏实现38.7 AP@13.9ms骨干延迟

Conclusion: DART证明了通过计算共享和系统优化，无需修改模型权重即可将提示式检测器转化为实时多类别检测器，为实际应用提供了高效解决方案
...
### [58] [Follow the Saliency: Supervised Saliency for Retrieval-augmented Dense Video Captioning](https://arxiv.org/abs/2603.11460)
*Seung hee Choi,MinJu Jeon,Hyunwoo Oh,Jihwan Lee,Dong-Jin Kim*

Main category: cs.CV

TL;DR: STaRC是一个用于密集视频描述的新框架，通过亮点检测模块监督帧级显著性，利用显著性分数作为统一的时间信号驱动检索和指导描述生成，在YouCook2和ViTT基准测试中达到最先进性能。
...
Method: 提出STaRC框架：1) 通过亮点检测模块监督帧级显著性，该模块使用从DVC真实标注派生的二元标签训练；2) 利用显著性分数作为统一时间信号，通过显著性引导分割驱动检索，并通过注入解码器的显式显著性提示指导描述生成；3) 强制显著性约束分割，产生与事件转换紧密对齐的时间连贯片段。

Result: 在YouCook2和ViTT基准测试上进行全面评估，STaRC在大多数指标上实现了最先进的性能。

Conclusion: STaRC通过监督帧级显著性并利用显著性作为统一时间信号，克服了现有方法在时间分割对齐方面的局限性，实现了更准确的检索和上下文基础的描述生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型存在幻觉问题，但现有基准对事实性幻觉覆盖有限且主要在干净环境下评估，需要更全面的诊断基准来评估模型在不同干扰条件下的可靠性。

Method: 构建包含9800个QA实例的INFACT基准，细粒度分类忠实性和事实性幻觉，涵盖真实和合成视频。设计四种评估模式：基础模式（干净）、视觉退化、证据破坏和时间干预，使用抵抗率和时间敏感性分数量化可靠性。

Result: 对14个代表性视频大语言模型的实验显示，基础模式准确率不能可靠预测诱导模式下的可靠性，证据破坏降低稳定性，时间干预导致最大性能下降。许多开源基线在事实性问题上接近零的时间敏感性分数，表明对顺序敏感问题存在明显的时间惯性。
...
### [60] [SPEGC: Continual Test-Time Adaptation via Semantic-Prompt-Enhanced Graph Clustering for Medical Image Segmentation](https://arxiv.org/abs/2603.11492)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，训练数据和测试数据之间的域差异严重阻碍了预训练模型在临床实践中的部署。现有的持续测试时适应方法依赖不可靠的监督信号，导致错误累积的恶性循环和灾难性性能下降。
...
Result: 在两个医学图像分割基准测试上的大量实验表明，SPEGC优于其他最先进的持续测试时适应方法。

Conclusion: SPEGC通过语义提示增强和可微分图聚类有效解决了医学图像分割中的域适应问题，避免了现有方法中的错误累积问题，在持续变化的域中实现了鲁棒的模型适应。
...
### [61] [OrthoEraser: Coupled-Neuron Orthogonal Projection for Concept Erasure](https://arxiv.org/abs/2603.11493)
*Chuancheng Shi,Wenhua Wu,Fei Shen,Xiaogang Zhu,Kun Hu,Zhiyong Wang*

Main category: cs.CV

TL;DR: OrthoEraser：一种基于稀疏自编码器实现高分辨率特征解耦的文本到图像模型安全防护方法，通过分析正交化投影在移除敏感概念的同时保护良性语义完整性


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型面临对抗诱导的安全风险，现有概念擦除方法在抑制选定神经元时往往会对良性属性造成附带损害，因为敏感和良性语义表现出非正交叠加，共享激活子空间且向量固有纠缠

Method: OrthoEraser采用稀疏自编码器实现高分辨率特征解耦，通过耦合神经元检测识别易受干预的非敏感特征，核心创新是分析梯度正交化策略，将擦除向量投影到耦合神经元的零空间，实现敏感概念与关键良性子空间的正交解耦

Result: 实验结果表明，OrthoEraser在安全任务上实现了高擦除精度，有效移除有害内容的同时保持生成流形的完整性，显著优于现有最先进基线方法

Conclusion: OrthoEraser通过特征解耦和正交化投影解决了概念擦除中的附带损害问题，为文本到图像模型提供了一种既能有效移除敏感概念又能保护良性语义完整性的安全防护方案
...
### [62] [Mobile-GS: Real-time Gaussian Splatting for Mobile Devices](https://arxiv.org/abs/2603.11531)
*Xiaobiao Du,Yida Wang,Kun Zhan,Xin Yu*

Main category: cs.CV

TL;DR: Mobile-GS：一种面向移动设备的实时3D高斯泼溅方法，通过消除深度排序、神经增强和模型压缩技术，在移动设备上实现高质量实时渲染。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）虽然渲染质量高，但其高计算需求和存储成本限制了在移动设备上的部署。需要开发一种移动设备友好的实时高斯泼溅方法。

Method: 1. 深度感知的顺序无关渲染方案：消除高斯深度排序瓶颈；2. 神经视角相关增强策略：解决透明度伪影；3. 一阶球谐蒸馏、神经向量量化和贡献度剪枝：压缩高斯表示和减少基元数量。

Result: Mobile-GS在保持高视觉质量的同时，实现了实时渲染和紧凑的模型尺寸，适合移动应用部署。

Conclusion: Mobile-GS通过创新的渲染方案和压缩技术，成功解决了3DGS在移动设备上的部署挑战，为移动端高质量3D渲染提供了可行方案。
...
### [63] [ReHARK: Refined Hybrid Adaptive RBF Kernels for Robust One-Shot Vision-Language Adaptation](https://arxiv.org/abs/2603.11542)
*Md Jahidul Islam*
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型（如CLIP）在极少数样本（特别是单样本）的下游任务适应中存在显著的"稳定性-可塑性"困境。现有的无训练方法（如Tip-Adapter）通常作为局部Nadaraya-Watson估计器，存在固有的边界偏差和缺乏全局结构正则化的问题。

Method: ReHARK是一个协同的无训练框架，通过再生核希尔伯特空间中的全局近端正则化重新解释少样本适应。采用四阶段精炼流程：1)混合先验构建：融合CLIP和GPT-3的零样本文本知识与视觉类原型；2)支持集增强：生成中间样本来平滑视觉和文本模态间的过渡；3)自适应分布校正：对齐测试特征统计量与增强支持集；4)多尺度RBF核：使用核集合捕捉不同尺度的复杂特征几何。

Result: 在11个多样化基准测试中展示了优越的稳定性和准确性，为单样本适应建立了新的最先进水平，平均准确率达到65.83%，显著优于现有基线方法。

Conclusion: ReHARK通过全局近端正则化和多阶段精炼流程，有效解决了视觉语言模型在极少数样本适应中的稳定性-可塑性困境，为少样本学习提供了一种无需训练的高效框架。
...
### [64] [Mango-GS: Enhancing Spatio-Temporal Consistency in Dynamic Scenes Reconstruction using Multi-Frame Node-Guided 4D Gaussian Splatting](https://arxiv.org/abs/2603.11543)
...
Method: 提出多帧节点引导框架Mango-GS：1)使用时序Transformer在短帧窗口内建模运动依赖；2)将时序建模限制在稀疏控制节点集以提高效率；3)每个节点用解耦的规范位置和潜在编码表示，为运动传播提供稳定语义锚点；4)端到端训练，采用输入掩码策略和两种多帧损失提升鲁棒性
...
### [65] [Enhancing Image Aesthetics with Dual-Conditioned Diffusion Models Guided by Multimodal Perception](https://arxiv.org/abs/2603.11556)
...
Method: 提出DIAE模型：1) 多模态美学感知(MAP)模块，通过详细标准化的美学属性和文本-图像对的多模态控制信号将模糊美学指令转化为明确指导；2) 收集IIAEData弱匹配数据集；3) 双分支监督框架利用弱匹配特性进行训练。

Result: 实验结果表明，DIAE在图像美学评分和内容一致性评分方面均优于基线方法，取得了优越的性能表现。

Conclusion: DIAE通过多模态美学感知和双监督训练框架，有效解决了图像美学增强中的指令遵循和数据集缺乏问题，为图像美学编辑提供了新的解决方案。
...
### [66] [TornadoNet: Real-Time Building Damage Detection with Ordinal Supervision](https://arxiv.org/abs/2603.11557)
...
TL;DR: TornadoNet是一个用于街景建筑损伤评估的基准测试，系统比较了CNN和Transformer架构在真实灾后条件下的多级损伤检测性能，并引入了序数感知监督策略来提升损伤严重程度评估。


<details>
  <summary>Details</summary>
Motivation: 现有建筑损伤评估方法缺乏对现代实时目标检测架构在真实灾后条件下性能的系统评估，特别是缺乏对损伤严重程度有序特性的专门考虑，需要建立标准化基准来指导灾害响应工具的开发。

Method: 使用2021年中西部龙卷风爆发的3,333张高分辨率地理标记图像和8,890个标注建筑实例，基于IN-CORE损伤状态的五级分类框架，系统比较YOLO系列CNN检测器和RT-DETR Transformer模型。引入软序数分类目标和显式序数距离惩罚作为序数感知监督策略。

Result: YOLO模型在检测精度和吞吐量方面表现最佳（46.05% mAP@0.5，66-276 FPS），而RT-DETR在序数一致性方面更强（88.13%序数Top-1准确率，MAOE=0.65）。使用校准序数监督训练的RT-DETR达到44.70% mAP@0.5，提升4.8个百分点，序数指标也得到改善（91.15%序数Top-1准确率，MAOE=0.56）。

Conclusion: 架构设计和损失公式共同影响多级损伤检测性能，CNN在检测精度和速度方面有优势，Transformer在损伤严重程度分级方面更可靠。序数感知监督与检测器架构对齐时能显著提升损伤严重程度估计，为灾害响应提供了方法论见解和可部署工具。
...
### [67] [SVLL: Staged Vision-Language Learning for Physically Grounded Embodied Task Planning](https://arxiv.org/abs/2603.11563)
...
<details>
  <summary>Details</summary>
Motivation: 现有具身任务规划方法面临关键权衡：端到端训练导致过早的时间绑定，而标准强化学习方法存在优化不稳定性。需要一种既能建立稳健视觉基础又能确保因果连贯性的统一框架。
...
Conclusion: SVLL通过分阶段学习和Bias-DPO对齐，解决了具身规划中的视觉基础与因果连贯性平衡问题，实现了更安全、更物理合理的任务规划。
...
### [68] [R4Det: 4D Radar-Camera Fusion for High-Performance 3D Object Detection](https://arxiv.org/abs/2603.11566)
*Zhongyu Xia,Yousen Tang,Yongtao Wang,Zhifeng Wang,Weijun Qin*

Main category: cs.CV

TL;DR: R4Det：一种改进的4D雷达-相机融合3D目标检测方法，通过全景深度融合、可变形门控时序融合和实例引导动态优化模块解决现有方法的深度估计不准确、时序融合依赖位姿以及稀疏雷达点云失效等问题


<details>
  <summary>Details</summary>
Motivation: 现有4D雷达-相机融合的3D目标检测方法存在三个主要问题：1）绝对深度估计模块不够鲁棒和准确，导致3D定位不精确；2）时序融合模块在自车位姿缺失或不准确时性能严重下降甚至失效；3）对于小目标，稀疏雷达点云可能完全无法从其表面反射，此时检测只能依赖视觉单模态先验

Method: 提出R4Det框架，包含三个核心模块：1）全景深度融合模块，通过绝对深度和相对深度的相互增强提高深度估计质量；2）可变形门控时序融合模块，不依赖自车位姿实现时序信息融合；3）实例引导动态优化模块，从2D实例引导中提取语义原型

Result: 在TJ4DRadSet和VoD数据集上实现了最先进的3D目标检测性能

Conclusion: R4Det通过创新的深度估计、时序融合和优化模块，有效解决了4D雷达-相机融合3D检测中的关键挑战，为自动驾驶感知系统提供了更鲁棒和准确的解决方案
...
### [69] [WeEdit: A Dataset, Benchmark and Glyph-Guided Framework for Text-centric Image Editing](https://arxiv.org/abs/2603.11593)
*Hui Zhang,Juntao Liu,Zongkai Liu,Liqiang Niu,Fandong Meng,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: WeEdit：一个用于文本中心图像编辑的系统解决方案，包含数据构建管道、基准测试和两阶段训练策略，显著提升文本编辑质量


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑模型在处理复杂文本编辑任务时表现不佳，经常产生模糊或幻觉字符。主要原因是缺乏专门针对文本中心编辑的训练范式，以及缺少大规模数据集和标准化基准测试来支持闭环训练评估系统。

Method: 提出WeEdit系统解决方案：1) 基于HTML的自动编辑管道生成33万训练对，覆盖多种编辑操作和15种语言；2) 创建双语和多语言基准测试；3) 采用两阶段训练策略：字形引导的监督微调注入空间和内容先验，然后是多目标强化学习阶段对齐生成质量。

Result: 大量实验表明，WeEdit在多种编辑操作上明显优于之前的开源模型，能够更精确地执行复杂文本编辑任务。
...
### [70] [LaMoGen: Language to Motion Generation Through LLM-Guided Symbolic Inference](https://arxiv.org/abs/2603.11605)
...
Method: 1) 开发LabanLite表示法，将原子身体部位动作编码为离散Laban符号和文本模板；2) 提出LaMoGen框架，利用大语言模型通过符号推理组合运动序列
...
### [71] [Articulat3D: Reconstructing Articulated Digital Twins From Monocular Videos with Geometric and Motion Constraints](https://arxiv.org/abs/2603.11606)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多视角捕获物体在离散静态状态下的数据，严重限制了现实世界中的可扩展性。需要从随意拍摄的单目视频中构建关节物体的高保真数字孪生。

Method: 提出Motion Prior-Driven Initialization，利用3D点轨迹挖掘关节运动的低维结构，通过紧凑的运动基建模场景动态，实现软分解为多个刚性运动组。在此基础上引入Geometric and Motion Constraints Refinement，通过可学习的运动学基元（关节轴、枢轴点和每帧运动标量）强制物理上合理的关节运动。
...
### [72] [FBCIR: Balancing Cross-Modal Focuses in Composed Image Retrieval](https://arxiv.org/abs/2603.11520)
*Chenchen Zhao,Jianhuan Zhuo,Muxi Chen,Zhaohua Zhang,Wenyu Jiang,Tianwen Jiang,Qiuyong Xiao,Jihong Zhang,Qiang Xu*

Main category: cs.CV

TL;DR: 论文提出FBCIR方法分析组合图像检索中模型关注不平衡问题，并通过数据增强提升模型在困难负样本下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索模型在标准基准上表现良好，但在语义对齐的困难负样本场景下性能下降。作者认为这是由于模型在跨模态推理中存在关注不平衡问题，即过度关注某一模态而忽视另一模态。

Method: 提出FBCIR多模态关注解释方法，识别模型检索决策中最关键的视觉和文本输入成分；基于分析结果，设计CIR数据增强流程，通过精心设计的困难负样本来促进平衡的跨模态推理。

Result: FBCIR分析显示现有CIR模型普遍存在关注不平衡问题，尤其在困难负样本设置下更明显；提出的数据增强方法在多个CIR模型上一致提升了在挑战性场景下的性能，同时保持了标准基准上的能力。

Conclusion: FBCIR解释方法和数据增强流程为CIR模型诊断和鲁棒性改进提供了新视角，通过识别和解决关注不平衡问题，有效提升了模型在语义对齐困难负样本下的检索性能。
...
### [73] [DyWeight: Dynamic Gradient Weighting for Few-Step Diffusion Sampling](https://arxiv.org/abs/2603.11607)
*Tong Zhao,Mingkun Lei,Liangyu Yuan,Yanming Yang,Chenxi Song,Yang Wang,Beier Zhu,Chi Zhang*

Main category: cs.CV

TL;DR: DyWeight：一种轻量级、基于学习的多步扩散模型求解器，通过动态梯度加权和隐式时间校准，显著提升采样效率而不牺牲质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型的采样过程需要数百次函数评估，速度缓慢。现有的多步ODE求解器虽然通过重用历史梯度提高了效率，但依赖于手工设计的系数，无法适应扩散采样过程中的非平稳动态特性。

Method: 提出动态梯度加权（DyWeight），采用简化的隐式耦合范式，通过放松经典数值约束，学习无约束的时变参数来自适应聚合历史梯度，同时隐式缩放有效步长。这种隐式时间校准确保在大积分步长下，求解器的数值轨迹与模型内部去噪动态准确对齐。

Result: 在CIFAR-10、FFHQ、AFHQv2、ImageNet64、LSUN-Bedroom、Stable Diffusion和FLUX.1-dev等数据集上的实验表明，DyWeight以显著更少的函数评估实现了更优的视觉保真度和稳定性，在高效扩散求解器中建立了新的最先进水平。

Conclusion: DyWeight通过自适应历史梯度聚合和隐式时间校准，解决了现有多步求解器无法适应非平稳动态的问题，为扩散模型提供了高效且高质量的采样方案。
...
### [74] [EReCu: Pseudo-label Evolution Fusion and Refinement with Multi-Cue Learning for Unsupervised Camouflage Detection](https://arxiv.org/abs/2603.11521)
*Shuo Jiang,Gaojia Zhang,Min Tan,Yufei Yin,Gang Pan*

Main category: cs.CV

TL;DR: 提出一种统一的非监督伪装目标检测框架，通过多线索原生感知模块和伪标签进化融合机制，在无需真实标注的情况下提升伪标签可靠性和特征保真度，实现细节感知和边界对齐的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有非监督伪装目标检测方法面临两个主要问题：1）伪标签噪声严重阻碍细粒度纹理学习；2）现有精炼策略忽视内在感知线索，导致边界溢出和结构模糊。同时，完全无伪标签指导的学习又会产生细节损失严重的粗糙特征。

Method: 提出统一框架包含三个核心模块：1）多线索原生感知模块，整合低层纹理线索与中层语义，实现掩码与原生目标信息的精确对齐；2）伪标签进化融合，通过师生交互智能精炼标签，使用深度可分离卷积进行语义去噪，并引入谱张量注意力融合平衡语义与结构信息；3）局部伪标签精炼，利用注意力多样性恢复细粒度纹理并增强边界保真度。

Result: 在多个非监督伪装目标检测数据集上的大量实验表明，该方法实现了最先进的性能，表现出卓越的细节感知能力、鲁棒的边界对齐以及在复杂伪装场景下的强泛化能力。

Conclusion: 该方法通过统一框架同时提升伪标签可靠性和特征保真度，有效解决了非监督伪装目标检测中的噪声标签和细节损失问题，为复杂伪装场景下的目标检测提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型具有强大的少样本能力，但对噪声标签敏感，现有方法缺乏建模细粒度语义线索和自适应区分干净与噪声信号的能力。

Method: 提出NA-MVP框架：1) 多视图提示结合非平衡最优传输实现细粒度补丁到提示对应，抑制不可靠区域；2) 双向提示设计捕获互补的干净导向和噪声感知线索；3) 对齐引导的选择性细化策略，仅修正误标记样本。
...
### [76] [MedPruner: Training-Free Hierarchical Token Pruning for Efficient 3D Medical Image Understanding in Vision-Language Models](https://arxiv.org/abs/2603.11625)
...
<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型在处理3D体积数据时存在显著计算效率问题：1）直接拼接连续2D切片导致大量解剖冗余；2）固定剪枝比率无法灵活处理不同切片间的异构信息密度，限制了实际临床部署。

Method: 提出MedPruner框架，包含两阶段机制：1）基于切片间锚点的过滤模块，消除切片级时间冗余；2）动态信息核心选择策略，通过量化累积注意力权重实现自适应token级压缩。该框架无需训练且模型无关。

Result: 在三个3D医学基准测试和三种不同医学VLM上的实验表明：现有架构存在大量token冗余。MedPruner使MedGemma等模型在保留少于5%视觉token的情况下维持或超越原始性能，大幅降低计算开销。

Conclusion: MedPruner验证了动态token选择对于实际临床部署的必要性，通过分层剪枝策略有效解决了3D医学图像理解中的计算效率瓶颈，为高效医学VLM部署提供了可行方案。
...
### [77] [Developing Foundation Models for Universal Segmentation from 3D Whole-Body Positron Emission Tomography](https://arxiv.org/abs/2603.11627)
...
TL;DR: SegAnyPET：首个用于3D全身PET图像通用分割的基础模型，基于大规模数据集（11041个扫描，59831个分割掩码），通过提示工程实现零样本器官和病灶分割，支持临床人机协作工作流。


<details>
  <summary>Details</summary>
Motivation: PET成像在疾病管理中具有不可替代的作用，但由于PET图像缺乏解剖对比度以及数据获取和标注成本高昂，深度学习模型在定量PET图像分析方面的发展受到严重限制。需要开发通用分割模型来解决这一瓶颈。

Method: 1）构建了迄今为止最大最全面的PET数据集（11041个3D全身PET扫描，59831个分割掩码）；2）开发了SegAnyPET基础模型，采用3D架构和提示工程策略进行掩码生成；3）支持通用可扩展的器官和病灶分割，并支持高效人工校正和人机协作工作流。

Result: 在多中心、多示踪剂、多疾病数据集上的广泛评估表明，SegAnyPET在广泛的零样本分割任务中表现出色，展示了其在分子成像临床应用中的潜力。

Conclusion: SegAnyPET作为首个用于3D全身PET图像通用分割的基础模型，通过大规模数据集和创新的提示工程策略，实现了强大的零样本性能，有望推动分子成像的临床应用发展。
...
### [78] [MV-SAM3D: Adaptive Multi-View Fusion for Layout-Aware 3D Generation](https://arxiv.org/abs/2603.11633)
*Baicheng Li,Dong Wu,Jun Li,Shunkai Zhou,Zecui Zeng,Lusong Li,Hongbin Zha*

Main category: cs.CV

TL;DR: MV-SAM3D是一个无需训练的框架，通过多视角融合和物理感知优化，扩展了布局感知的3D生成，解决了单视角输入和多物体布局中的物理不合理问题。


<details>
  <summary>Details</summary>
Motivation: 当前统一的3D生成模型在单图像生成高质量3D资产方面取得了显著进展，但存在两个主要限制：1) 仅限于单视角输入，无法利用互补的多视角观测；2) 独立估计的物体姿态常导致物理上不合理的布局（如相互穿透和漂浮伪影）。

Method: 提出训练免费的MV-SAM3D框架：1) 将多视角融合建模为3D潜在空间中的多扩散过程；2) 提出两种自适应加权策略（注意力熵加权和可见性加权），实现置信度感知融合；3) 引入物理感知优化，在生成期间和之后注入碰撞和接触约束。
...
### [79] [Tokenization Allows Multimodal Large Language Models to Understand, Generate and Edit Architectural Floor Plans](https://arxiv.org/abs/2603.11640)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在建筑平面图设计中面临几何、语义和空间层次联合推理的挑战。虽然最近的扩散和语言模型提高了视觉保真度，但在连贯的空间推理和可控生成方面仍有困难。
...
Conclusion: HouseMind通过统一的方法解决了建筑平面图设计中的多模态推理问题，为AI辅助建筑设计提供了有效的解决方案。
...
### [80] [IDRL: An Individual-Aware Multimodal Depression-Related Representation Learning Framework for Depression Diagnosis](https://arxiv.org/abs/2603.11644)
...
Method: 提出个体感知多模态抑郁症相关表示学习框架(IDRL)：1）将多模态表示解耦为模态共有抑郁空间、模态特定抑郁空间和抑郁症无关空间，增强模态对齐同时抑制无关信息；2）引入个体感知模态融合模块(IAF)，根据预测重要性动态调整解耦的抑郁症相关特征权重，实现针对不同个体的自适应跨模态融合

Result: 广泛实验表明IDRL在多模态抑郁症检测中实现了优越且稳健的性能

Conclusion: IDRL框架通过解耦表示学习和个体感知融合，有效解决了多模态抑郁症检测中的模态不一致性和个体差异问题，为抑郁症的可靠识别提供了新方法
...
### [81] [FL-MedSegBench: A Comprehensive Benchmark for Federated Learning on Medical Image Segmentation](https://arxiv.org/abs/2603.11659)
*Meilu Zhu,Zhiwei Wang,Axiu Mao,Yuxing Li,Xiaohan Xing,Yixuan Yuan,Edmund Y. Lam*

Main category: cs.CV

TL;DR: FL-MedSegBench是首个医学图像分割的联邦学习基准，包含9个分割任务、10种成像模态，系统评估了13种FL方法在准确性、公平性、通信效率等多维度的表现。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在医学图像分析中具有隐私保护优势，但缺乏标准化基准阻碍了公平全面的方法评估。需要建立首个医学图像分割的联邦学习基准来填补这一空白。

Method: 创建FL-MedSegBench基准，涵盖9个分割任务、10种成像模态，包含2D和3D格式，模拟真实临床异质性。系统评估8种通用FL方法和5种个性化FL方法，从分割准确性、公平性、通信效率、收敛行为和未见域泛化能力等多个维度进行分析。

Result: 实验发现：(1)个性化FL方法（特别是FedBN等客户端特定批归一化方法）普遍优于通用方法；(2)没有单一方法在所有数据集上表现最佳，性能具有数据集依赖性；(3)基于归一化的个性化方法对降低通信频率具有鲁棒性；(4)Ditto和FedRDN等方法能保护性能较差的客户端，提升公平性；(5)方法在未见域的泛化能力与其在参与客户端上的表现密切相关。

Conclusion: FL-MedSegBench为医学图像分割的联邦学习提供了首个全面基准，揭示了不同方法的性能特点和适用场景。研究将发布开源工具包促进可重复研究，为临床部署提供实证指导。
...
### [82] [OSCBench: Benchmarking Object State Change in Text-to-Video Generation](https://arxiv.org/abs/2603.11698)
*Xianjing Han,Bin Zhu,Shiqi Hu,Franklin Mingzhe Li,Patrick Carrington,Roger Zimmermann,Jingjing Chen*

Main category: cs.CV

TL;DR: OSCBench：首个专门评估文本到视频生成模型中对象状态变化能力的基准，发现现有模型在对象状态变化方面存在显著瓶颈


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成基准主要关注感知质量、文本-视频对齐或物理合理性，但忽略了文本提示中明确指定的对象状态变化这一关键方面。对象状态变化指由动作引起的对象状态转变，如削土豆皮或切柠檬。

Method: 从教学烹饪数据构建OSCBench基准，系统地将动作-对象交互组织为常规、新颖和组合场景，以探究分布内性能和泛化能力。使用人类用户研究和基于多模态大语言模型的自动评估方法，评估了六个代表性的开源和专有T2V模型。

Result: 尽管在语义和场景对齐方面表现良好，但当前T2V模型在准确且时间一致的对象状态变化方面持续存在困难，特别是在新颖和组合设置中。对象状态变化成为文本到视频生成的关键瓶颈。

Conclusion: OSCBench作为诊断性基准，为推进状态感知视频生成模型的发展提供了重要工具，揭示了对象状态变化是当前T2V模型需要解决的核心挑战。
...
### [83] [BackdoorIDS: Zero-shot Backdoor Detection for Pretrained Vision Encoder](https://arxiv.org/abs/2603.11664)
*Siquan Huang,Yijiang Li,Ningzhi Gao,Xingfu Yan,Leyu Shi*

Main category: cs.CV

TL;DR: BackdoorIDS：一种针对预训练视觉编码器的零样本推理时后门样本检测方法，基于注意力劫持与恢复现象，通过渐进掩码和密度聚类实现检测


<details>
  <summary>Details</summary>
Motivation: 下游用户常依赖来源不确定的第三方预训练编码器，面临后门攻击风险。现有防御方法存在局限性，需要一种无需重新训练、兼容多种架构的零样本检测方案。

Method: 基于渐进输入掩码观察注意力劫持与恢复现象：后门图像初始注意力集中在恶意触发特征上，当掩码比例超过触发器的鲁棒性阈值后，注意力迅速转向良性内容。通过提取掩码轨迹上的嵌入序列，应用DBSCAN等密度聚类方法，若嵌入序列形成多个簇则判定为后门样本。

Result: 在多种攻击类型、数据集和模型家族上，BackdoorIDS始终优于现有防御方法。作为即插即用方法，无需重新训练，完全零样本推理时操作，兼容CNN、ViT、CLIP、LLaVA-1.5等多种编码器架构。

Conclusion: BackdoorIDS提供了一种简单有效的零样本后门检测方案，基于注意力劫持与恢复现象，通过渐进掩码和聚类分析实现可靠检测，具有广泛的架构兼容性和实际部署价值。
...
### [84] [PolyCrysDiff: Controllable Generation of Three-Dimensional Computable Polycrystalline Material Structures](https://arxiv.org/abs/2603.11695)
*Chi Chen,Tianle Jiang,Xiaodong Wei,Yanming Wang*

Main category: cs.CV

TL;DR: PolyCrysDiff：基于条件潜在扩散的端到端三维多晶微结构生成框架，能够可控生成可计算微结构并揭示微观结构-性能关系


<details>
  <summary>Details</summary>
Motivation: 三维多晶材料的微观结构对其力学和物理性能具有关键影响，但现实可控的微结构构建仍是巨大挑战，阻碍了结构-性能关系的阐明

Method: 提出PolyCrysDiff框架，基于条件潜在扩散模型实现端到端可计算三维多晶微结构生成，通过综合定性和定量评估验证生成质量

Result: PolyCrysDiff能准确再现目标晶粒形态、取向分布和三维空间相关性，在晶粒属性控制上R²超过0.972，优于MRF和CNN等主流方法；生成微结构的可计算性和物理有效性通过CPFEM模拟验证

Conclusion: 该框架利用可控生成能力系统阐明了晶粒级微观结构特征对多晶材料力学性能的影响，为加速数据驱动的多晶材料优化设计奠定了基础
...
### [85] [Think While Watching: Online Streaming Segment-Level Memory for Multi-Turn Video Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2603.11896)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs大多局限于离线推理或在线推理能力弱，难以处理连续到达的视频流多轮交互。现有流式方法通常采用交错感知-生成范式，无法实现并发感知和生成，且随着流增长导致早期记忆衰减，损害长距离依赖建模。

Method: 提出Think While Watching框架：1) 内存锚定流式视频推理，在多轮交互中保持连续片段级记忆；2) 构建三阶段多轮思维链数据集，采用阶段匹配训练策略；3) 通过片段级流式因果掩码和流式位置编码强制严格因果关系；4) 推理时引入高效管道，重叠观看和思考，自适应选择最佳注意力后端。

Result: 在单轮和多轮流式输入协议下均取得优异结果：基于Qwen3-VL构建，在StreamingBench上单轮准确率提升2.6%，在OVO-Bench上提升3.79%；多轮设置中保持性能同时减少56%输出token。

Conclusion: Think While Watching框架有效解决了MLLMs在流式视频推理中的挑战，通过内存锚定和并行处理机制显著提升了在线推理性能，为连续视频流多轮交互提供了高效解决方案。
...
### [86] [VTEdit-Bench: A Comprehensive Benchmark for Multi-Reference Image Editing Models in Virtual Try-On](https://arxiv.org/abs/2603.11734)
*Xiaoye Liang,Zhiyuan Qu,Mingye Zou,Jiaxin Liu,Lai Jiang,Mai Xu,Yiheng Zhu*

Main category: cs.CV

TL;DR: 论文提出了VTEdit-Bench基准测试，用于系统评估通用多参考图像编辑模型在虚拟试穿任务中的性能，发现顶级通用编辑器在传统任务上具有竞争力且泛化更稳定，但在复杂多服装配置上仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 随着虚拟试穿应用场景的扩展，现有专用VTON模型已无法满足需求，而通用多参考图像编辑模型在视觉编辑方面展现出强大泛化能力，但缺乏系统评估基准来探索其在VTON任务中的优势和局限。

Method: 提出了VTEdit-Bench基准测试，包含24,220个测试图像对，覆盖5个代表性VTON任务，复杂度递增；同时提出VTEdit-QA评估器，基于参考感知的VLM从模型一致性、服装一致性和整体图像质量三方面评估VTON性能。

Result: 系统评估了8个通用编辑模型和7个专用VTON模型，结果显示顶级通用编辑器在传统任务上具有竞争力，在更复杂场景中泛化更稳定，但在复杂参考配置（特别是多服装条件）方面仍面临挑战。

Conclusion: VTEdit-Bench为评估通用编辑模型在VTON任务中的性能提供了系统框架，揭示了通用编辑器在传统任务上的竞争力及其在复杂配置下的局限性，为未来更灵活的VTON系统发展指明了方向。
...
### [87] [SoulX-LiveAct: Towards Hour-Scale Real-Time Human Animation with Neighbor Forcing and ConvKV Memory](https://arxiv.org/abs/2603.11746)
*Dingcheng Zhen,Xu Zheng,Ruixin Zhang,Zhiqi Jiang,Yichao Yan,Ming Tao,Shunshun Yin*

Main category: cs.CV

TL;DR: 本文提出Neighbor Forcing和ConvKV内存机制，解决了AR扩散模型在小时级实时人体动画中的训练不稳定和内存效率问题，实现了20 FPS的实时流式推理。
...
Method: 提出Neighbor Forcing，这是一种扩散步长一致的AR公式，在相同噪声条件下将时间相邻帧作为潜在邻居传播，提供分布对齐的稳定学习信号。在此基础上引入结构化ConvKV内存机制，将因果注意力中的键和值压缩为固定长度表示，实现恒定内存推理和真正无限视频生成。

Result: LiveAct方法显著改善了训练收敛性、小时级生成质量和推理效率，在2个NVIDIA H100/H200 GPU上实现20 FPS实时流式推理，在唇形同步精度、人体动画质量和情感表达方面达到最先进性能，且推理成本最低。

Conclusion: Neighbor Forcing和ConvKV内存机制有效解决了AR扩散模型在长时间序列生成中的关键瓶颈，为实时人体动画提供了高效、稳定的解决方案，支持小时级无限视频生成。
...
### [88] [Controllable Egocentric Video Generation via Occlusion-Aware Sparse 3D Hand Joints](https://arxiv.org/abs/2603.11755)
*Chenyangguang Zhang,Botao Ye,Boqi Chen,Alexandros Delitzas,Fangjinhua Wang,Marc Pollefeys,Xi Wang*

Main category: cs.CV

TL;DR: 提出一种基于稀疏3D手部关节的运动可控视频生成框架，解决现有方法在自我中心视角下的3D一致性和跨具身泛化问题


<details>
  <summary>Details</summary>
Motivation: 现有运动可控视频生成方法在自我中心应用中存在3D一致性不足、手部精细动作难以控制、跨具身泛化能力差等问题。基于2D轨迹或隐式姿态的方法将3D几何坍缩为空间模糊信号，过度依赖人体先验，在严重遮挡下导致运动不一致和伪影，无法泛化到机器人手

Method: 提出新框架：1) 使用稀疏3D手部关节作为具身无关的控制信号；2) 设计高效控制模块，通过惩罚隐藏关节的不可靠视觉信号提取遮挡感知特征；3) 采用3D加权机制处理动态遮挡的目标关节；4) 直接将3D几何嵌入注入潜在空间以强制结构一致性；5) 开发自动化标注流程生成百万级高质量视频-轨迹对；6) 构建跨具身基准

Result: 方法在广泛实验中显著优于现有基线，能生成高保真度的自我中心视频，实现逼真的交互效果，并展现出对机器人手的卓越跨具身泛化能力

Conclusion: 提出的基于稀疏3D手部关节的控制框架有效解决了自我中心视频生成中的3D一致性、遮挡处理和跨具身泛化问题，为虚拟现实和具身AI应用提供了更可靠的解决方案
...
### [89] [Intrinsic Concept Extraction Based on Compositional Interpretability](https://arxiv.org/abs/2603.11795)
*Hanyu Shi,Hong Tao,Guoheng Huang,Jianbin Jiang,Xuhang Chen,Chi-Man Pun,Shanhu Wang,Pan Pan*

Main category: cs.CV

TL;DR: 提出HyperExpress方法解决从单图像中提取可组合、可解释内在概念的新任务CI-ICE，利用双曲空间层次建模能力和概念优化实现准确概念解耦与组合
...
Method: 提出HyperExpress方法：1）利用双曲空间固有层次建模能力的概念学习方法，实现准确概念解耦同时保持概念间层次结构和关系依赖；2）概念优化方法，将概念嵌入空间映射以保持复杂概念间关系同时确保概念可组合性

Result: 方法在从单图像中提取组合可解释内在概念方面表现出色

Conclusion: HyperExpress方法成功解决了CI-ICE任务，能够从单图像中提取可组合、可解释的对象级和属性级内在概念
...
### [90] [Multimodal classification of Radiation-Induced Contrast Enhancements and tumor recurrence using deep learning](https://arxiv.org/abs/2603.11827)
...
<details>
  <summary>Details</summary>
Motivation: 区分胶质母细胞瘤治疗后肿瘤复发与放射性对比增强是临床主要挑战。现有方法依赖临床稀疏的扩散MRI或未考虑放疗剂量图，而放疗剂量图在肿瘤委员会中对该区分越来越受关注。

Method: 提出RICE-NET多模态3D深度学习模型，整合纵向MRI数据与放疗剂量分布，使用常规T1加权MRI数据进行自动病灶分类。

Result: 在92名患者队列中，模型在独立测试集上获得F1分数0.92。消融实验量化了各时间点和模态的贡献，显示可靠分类主要依赖放疗剂量图。遮挡可解释性分析进一步证实模型关注临床相关区域。
...
### [91] [Towards High-Fidelity CAD Generation via LLM-Driven Program Generation and Text-Based B-Rep Primitive Grounding](https://arxiv.org/abs/2603.11831)
*Jiahao Li,Qingwang Zhang,Qiuyu Chen,Guozhan Qiu,Yunzhong Lou,Xiangdong Zhou*

Main category: cs.CV

TL;DR: FutureCAD是一个新颖的文本到CAD框架，通过结合大语言模型和B-Rep基础变换器，生成可执行的CadQuery脚本，解决了参数化建模与B-Rep合成之间的范式差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法分为参数化CAD建模和直接边界表示合成两类，但在现代基于特征的CAD系统中，参数化建模和B-Rep本质上是相互交织的。参数化操作需要明确选择B-Rep几何基元，而B-Rep本身又源自参数化操作。这种范式差距限制了AI驱动的复杂工业产品设计CAD建模。

Method: 提出FutureCAD框架，结合大语言模型和B-Rep基础变换器。LLM生成可执行的CadQuery脚本，并引入基于文本的查询机制，使LLM能够通过自然语言指定几何选择，然后由BRepGround将其基础化到目标基元。训练过程包括：构建新的真实世界CAD模型数据集；对LLM应用监督微调建立基本CAD生成能力；然后使用强化学习提高泛化能力。
...
TL;DR: 提出基于h变换的训练自由引导生成方法，通过修改采样过程的转移概率来约束生成过程，无需已知前向变换算子，在图像和视频生成任务中表现优异。
...
Method: 使用h变换约束随机过程，通过向原始微分方程添加漂移函数修改每个采样时间步的转移概率，近似引导生成朝向理想精细样本。引入噪声水平感知调度来减轻近似误差影响。

Result: 在多样化的图像和视频生成任务上进行广泛实验，证明了方法的有效性和泛化能力。

Conclusion: 提出的基于h变换的训练自由引导方法解决了现有方法的局限性，无需已知前向变换算子，能有效平衡引导与生成质量，在多种视觉生成任务中表现优异。
...
### [93] [ZeroSense:How Vision matters in Long Context Compression](https://arxiv.org/abs/2603.11846)
*Yonghan Gao,Zehong Chen,Lijian Xu,Jingzhi Chen,Jingwei Guan,Xingyu Zeng*

Main category: cs.CV

TL;DR: 该论文提出了一种新的视觉文本压缩评估框架，通过解耦多模态大语言模型的能力来准确评估压缩质量，并引入了ZeroSense基准测试来消除语义相关性对评估的影响。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文本压缩方法（如DeepSeek-OCR）虽然报告了高令牌压缩比，但其评估协议过度依赖下游任务性能。由于多模态大语言模型具有强大的语言先验知识，这种评估指标无法准确衡量文本保留质量，需要一种解耦的评估框架来独立评估压缩质量。

Method: 提出了一个新的评估框架，通过解耦多模态大语言模型的能力来准确评估视觉文本压缩质量。进一步引入了ZeroSense基准测试，确保测试样本具有低语义相关性，消除上下文依赖，使评估结果纯粹反映压缩质量而不受下游模型语义推理能力的影响。

Result: 在多个数据集上的广泛实验表明，视觉文本压缩质量与下游任务准确性存在显著差异，验证了所提解耦评估框架的必要性。现有评估方法无法准确反映压缩质量，而新框架能够提供更可靠的评估结果。

Conclusion: 需要一种新的评估框架来准确评估视觉文本压缩质量，该框架应解耦多模态大语言模型的能力，并消除语义相关性对评估的影响。提出的ZeroSense基准测试和评估框架为视觉文本压缩提供了更可靠的评估方法。
...
### [94] [Single-View Rolling-Shutter SfM](https://arxiv.org/abs/2603.11888)
...
<details>
  <summary>Details</summary>
Motivation: 滚动快门相机普遍存在，但其结构从运动问题尚未完全解决，需要开发新的几何建模方法

Method: 表征滚动快门单视图几何，分析世界点或线的观测特性，推导可恢复的运动和场景参数，系统化构建最小重建问题

Result: 开发了概念验证求解器，评估多个代表性案例，展示了方法的可行性并识别了实际限制

Conclusion: 提出的滚动快门单视图几何方法为解决RS SfM问题提供了理论基础，概念验证表明该方法可行但存在实际限制
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习工具在神经疾病（如阿尔茨海默病）评估中存在局限性：分类器将扫描简化为标签，体积分析管道产生未解释的测量结果，而视觉语言模型可能生成流畅但存在幻觉的结论。需要一种能够提供全面、可解释且可靠的纵向脑部MRI分析工具。

Method: 提出LoV3D管道，通过分步处理流程训练3D视觉语言模型：1）读取纵向T1加权脑部MRI；2）生成区域级解剖评估；3）与先前扫描进行纵向比较；4）输出三类诊断和诊断总结。引入临床加权验证器，基于标准化体积指标的规范参考自动评分候选输出，驱动直接偏好优化，无需人工标注。
...
Conclusion: LoV3D通过分步管道和临床加权验证器，实现了对纵向脑部MRI的全面、可解释分析，显著减少了幻觉风险，在诊断准确率和泛化能力方面超越了现有方法，为神经疾病评估提供了可靠的深度学习工具。
...
### [96] [InSpatio-WorldFM: An Open-Source Real-Time Generative Frame Model](https://arxiv.org/abs/2603.11911)
...
TL;DR: InSpatio-WorldFM是一个开源的实时帧模型，采用基于帧的范式而非视频序列生成，通过3D锚点和空间记忆保持多视角一致性，支持消费级GPU上的交互式探索。
...
Method: 采用基于帧的范式，每帧独立生成以减少延迟；通过显式3D锚点和隐式空间记忆强制多视角空间一致性；提出渐进式三阶段训练流程：从预训练图像扩散模型转换为可控帧模型，再通过少步蒸馏成为实时生成器。

Result: 实验结果表明，InSpatio-WorldFM在保持强大多视角一致性的同时，能够在消费级GPU上支持交互式探索，为实时世界模拟提供了比传统视频基世界模型更高效的替代方案。

Conclusion: InSpatio-WorldFM通过帧基范式、空间一致性机制和渐进训练策略，实现了低延迟的实时空间智能，为实时世界模拟提供了高效解决方案。
...
### [97] [PicoSAM3: Real-Time In-Sensor Region-of-Interest Segmentation](https://arxiv.org/abs/2603.11917)
*Pietro Bonazzi,Nicola Farronato,Stefan Zihlmann,Haotong Qin,Michele Magno*

Main category: cs.CV

TL;DR: PicoSAM3：面向边缘设备和视觉传感器的轻量级可提示分割模型，在IMX500传感器上实现实时推理


<details>
  <summary>Details</summary>
Motivation: 针对智能眼镜和物联网设备等对延迟敏感且注重隐私的应用，需要实时的设备端分割模型。现有模型通常计算复杂度过高，无法在边缘设备或视觉传感器上高效运行。

Method: 提出PicoSAM3轻量级模型，仅130万参数，结合密集CNN架构、感兴趣区域提示编码、高效通道注意力机制，并从SAM2和SAM3进行知识蒸馏。

Result: 在COCO和LVIS数据集上分别达到65.45%和64.01% mIoU，优于现有SAM基线和边缘导向基线。INT8量化后精度几乎无损，在IMX500传感器上实现11.82ms延迟的实时推理。

Conclusion: 通过知识蒸馏和轻量化设计，实现了在传感器级别的高质量、空间灵活的可提示分割，为边缘AI应用提供了可行的解决方案。
...
### [98] [Preliminary analysis of RGB-NIR Image Registration techniques for off-road forestry environments](https://arxiv.org/abs/2603.11952)
...
Conclusion: 虽然只是初步评估，但研究显示需要进一步改进以实现鲁棒的多尺度配准，适用于越野森林应用
...
### [99] [AstroSplat: Physics-Based Gaussian Splatting for Rendering and Reconstruction of Small Celestial Bodies](https://arxiv.org/abs/2603.11969)
*Jennifer Nolan,Travis Driver,John Christian*

Main category: cs.CV

TL;DR: AstroSplat：基于物理的Gaussian splatting框架，用于小行星表面重建和光度表征，整合行星反射模型，相比传统球谐参数化有更好性能


<details>
  <summary>Details</summary>
Motivation: 基于图像的表面重建和表征对小行星任务至关重要，但现有Gaussian splatting方法依赖球谐强度参数化，仅关注外观而非材料属性或光-表面相互作用

Method: 提出AstroSplat框架，整合行星反射模型到Gaussian splatting中，实现基于物理的表面重建和光度表征

Result: 在NASA Dawn任务真实图像上验证，相比传统球谐参数化，在渲染性能和表面重建精度方面表现更优

Conclusion: AstroSplat通过整合物理反射模型，改进了小行星表面的自主重建和光度表征能力，为行星科学任务提供更准确的分析工具
...
### [100] [FlashMotion: Few-Step Controllable Video Generation with Trajectory Guidance](https://arxiv.org/abs/2603.12146)
*Quanhao Li,Zhen Xing,Rui Wang,Haidong Cao,Qi Dai,Daoguo Dong,Zuxuan Wu*

Main category: cs.CV

TL;DR: FlashMotion是一个用于少步轨迹可控视频生成的训练框架，通过适配器训练、生成器蒸馏和混合微调策略，在保持轨迹精度的同时显著加速视频生成。
...
### [101] [Ada3Drift: Adaptive Training-Time Drifting for One-Step 3D Visuomotor Robotic Manipulation](https://arxiv.org/abs/2603.11984)
...
Method: Ada3Drift学习训练时漂移场，将预测动作吸引向专家演示模式同时排斥其他生成样本，实现从3D点云观察的单步生成。针对少样本机器人场景，引入sigmoid调度损失从粗分布学习过渡到模式锐化细化，以及多尺度场聚合捕捉不同空间粒度的动作模式。

Result: 在三个仿真基准（Adroit、Meta-World、RoboTwin）和真实世界机器人操作任务上的实验表明，Ada3Drift达到最先进性能，同时比基于扩散的替代方法减少10倍函数评估次数。

Conclusion: Ada3Drift通过将迭代细化从推理时转移到训练时，实现了高保真单步生成，在保持多模态行为的同时显著降低推理延迟，为实时机器人控制提供了有效解决方案。
...
### [102] [CrossEarth-SAR: A SAR-Centric and Billion-Scale Geospatial Foundation Model for Domain Generalizable Semantic Segmentation](https://arxiv.org/abs/2603.12008)
...
TL;DR: 提出首个十亿级SAR视觉基础模型CrossEarth-SAR，采用物理引导的稀疏专家混合架构，用于跨域语义分割，在22个子基准测试中20个达到SOTA


<details>
  <summary>Details</summary>
Motivation: SAR图像存在传感器和区域间的域偏移问题，严重阻碍语义泛化能力，需要构建能够处理跨域语义分割的基础模型

Method: 提出物理引导的稀疏专家混合架构，整合物理描述符；构建CrossEarth-SAR-200K大规模弱监督和全监督数据集；建立包含22个子基准的测试套件

Result: 在22个基准测试中的20个达到最先进水平，在多间隙迁移下某些基准测试mIoU提升超过10%

Conclusion: CrossEarth-SAR是首个十亿级SAR视觉基础模型，通过物理引导架构和大规模预训练，显著提升了跨域语义分割性能，建立了统一的基准测试标准
...
### [103] [BehaviorVLM: Unified Finetuning-Free Behavioral Understanding with Vision-Language Reasoning](https://arxiv.org/abs/2603.12176)
*Jingyang Ke,Weihan Li,Amartya Pradhan,Jeffrey Markowitz,Anqi Wu*

Main category: cs.CV

TL;DR: BehaviorVLM：一个无需任务特定微调的统一视觉语言框架，用于姿态估计和行为理解，通过引导预训练VLM进行详细推理步骤，大幅减少人工标注需求


<details>
  <summary>Details</summary>
Motivation: 当前动物行为分析中的姿态估计和行为理解严重依赖人工标注或不稳定的无监督流程，限制了可扩展性和可重复性，需要更高效、可扩展的解决方案

Method: 1. 姿态估计：利用量子点标记行为数据，提出多阶段管道整合时间、空间和跨视角推理，通过几何检查暴露低置信度标签；2. 行为理解：整合深度嵌入聚类进行过分割行为发现、基于VLM的每段视频描述和基于LLM的推理来合并和语义标记行为片段

Result: 该框架能够：1. 大幅减少姿态估计的人工标注工作量；2. 通过重投影误差等几何检查暴露低置信度标签；3. 产生可用于下游姿态模型微调的标签；4. 直接从视觉信息进行行为分割，无需关键点；5. 实现可扩展、可解释、低标注的多动物行为分析

Conclusion: BehaviorVLM提供了一个统一的视觉语言框架，通过引导预训练VLM进行详细推理，实现了无需任务特定微调的姿态估计和行为理解，为可扩展、可解释、低标注的多动物行为分析提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有全景拼接方法过度依赖成对特征匹配，无法利用多视角几何一致性，导致在弱纹理、大视差、重复模式场景中出现严重失真和错位问题
...
Conclusion: 通过将全景拼接扩展到3D空间并利用多视角几何一致性，有效解决了传统方法在挑战性场景中的失真和错位问题，实现了更准确和全局一致的全景拼接
...
<details>
  <summary>Details</summary>
Motivation: 现代成像仪器产生的数据量巨大（TB-PB级），现有图像分析算法在计算效率、鲁棒性和准确性方面存在不足。深度学习改善了区域分割，但领域特定特征提取库的碎片化使得特征性能比较困难。

Method: 开发Nyxus特征提取库，采用可扩展的out-of-core架构设计，支持2D/3D图像数据，涵盖放射组学和细胞分析等生物医学领域特征，支持CPU和GPU并行计算，提供多种访问方式。

Result: Nyxus实现了高效的大规模图像特征提取，通过严格测试验证了其性能，提供Python包、命令行工具、Napari插件和OCI容器等多种部署方式，支持程序化调优特征集。

Conclusion: Nyxus解决了大规模图像数据处理的计算瓶颈，提供了标准化、可扩展的特征提取解决方案，支持新型机器学习和深度学习应用，使不同技能水平的用户都能访问先进的特征提取能力。
...
TL;DR: SeGP-CL提出语义几何保持方法，通过对抗锚点探测易漂移区域，进行跨模态几何蒸馏和文本语义几何正则化，解决视觉语言模型持续学习中的灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型持续学习方法在适应新任务时，未能显式保持预训练和先前阶段继承的跨模态语义几何结构，导致新任务监督引发几何失真，特别是在新旧语义接口附近的脆弱邻域中漂移最为明显

Method: 1) 使用双目标投影梯度下降构建对抗锚点探测易漂移区域；2) 通过锚点引导的跨模态几何蒸馏保持跨模态结构；3) 采用轻量级文本语义几何正则化稳定跨任务的文本参考框架；4) 训练后估计锚点诱导的原始空间漂移来传输旧视觉原型，并通过融合跨模态和视觉线索进行双路径推理

Result: 在五个持续学习基准测试上的广泛实验表明，SeGP-CL持续改进稳定性和前向迁移能力，实现了最先进的性能，同时更好地保持了视觉语言模型的语义几何结构

Conclusion: SeGP-CL通过显式保持跨模态语义几何结构，有效解决了视觉语言模型持续学习中的灾难性遗忘问题，特别是在无示例约束下，通过对抗锚点探测和几何保持机制显著提升了模型性能
...
### [107] [NBAvatar: Neural Billboards Avatars with Realistic Hand-Face Interaction](https://arxiv.org/abs/2603.12063)
*David Svitov,Mahtab Dahaghin*

Main category: cs.CV

TL;DR: NBAvatar是一种结合定向平面图元和神经渲染的头像渲染方法，专门处理手-脸交互引起的非刚性变形，在渲染质量和结构相似性上优于现有方法。
...
Method: 结合定向平面图元训练与神经渲染，通过显式和隐式表示的融合来处理时间一致性和姿态一致的几何结构，同时保留神经渲染提供的精细外观细节。

Result: NBAvatar能够隐式学习手-脸交互引起的颜色变换，在高分辨率百万像素渲染下比基于高斯的方法LPIPS降低30%，PSNR和SSIM均有提升，且比当前最佳的手-脸交互方法InteractAvatar具有更高的结构相似性。

Conclusion: NBAvatar通过结合显式和隐式表示，成功实现了对非刚性变形头像的高质量渲染，在处理手-脸交互场景中表现出优越性能。
...
### [108] [Dense Dynamic Scene Reconstruction and Camera Pose Estimation from Multi-View Videos](https://arxiv.org/abs/2603.12064)
*Shuo Sun,Unal Artan,Malcolm Mielle,Achim J. Lilienthaland,Martin Magnusson*

Main category: cs.CV

TL;DR: 提出两阶段优化框架，用于多自由移动相机的密集动态场景重建和相机姿态估计，通过解耦相机跟踪和深度细化，在合成和真实数据集上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决多自由移动相机（多个观察者拍摄同一事件）的密集动态场景重建和相机姿态估计问题，现有方法要么只处理单相机输入，要么需要预校准的刚性相机阵列，限制了实际应用

Method: 两阶段优化框架：第一阶段通过构建时空连接图扩展单相机视觉SLAM到多相机设置，利用相机内时间连续性和相机间空间重叠实现一致尺度和鲁棒跟踪，引入基于前馈重建模型的宽基线初始化策略；第二阶段通过优化密集相机间和相机内一致性来细化深度和相机姿态，使用宽基线光流

Result: 在合成和真实世界基准测试中显著优于最先进的前馈模型，同时需要更少内存；引入了具有运动捕捉系统提供真实姿态的新真实世界数据集MultiCamRobolab

Conclusion: 提出的两阶段优化框架有效解决了多自由移动相机的密集动态场景重建和相机姿态估计问题，通过解耦相机跟踪和深度细化，在有限重叠情况下仍能保持鲁棒性，为实际应用提供了可行解决方案
...
### [109] [Node-RF: Learning Generalized Continuous Space-Time Scene Dynamics with Neural ODE-based NeRFs](https://arxiv.org/abs/2603.12078)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅能在观测边界内捕捉动态，无法外推到训练序列之外。需要一种能够超越观测轨迹进行长时外推预测的方法

Method: 集成神经ODE与动态NeRF，学习通过ODE求解器随时间演化的隐式场景状态，通过微分计算传播特征嵌入，NeRF渲染器解释计算出的嵌入以合成任意视角

Result: Node-RF能够表征抽象系统行为而无需显式模型，识别未来预测的关键点，在多个运动序列上训练可实现对未见条件的泛化

Conclusion: Node-RF通过连续时空表示克服了现有方法的局限性，实现了超越观测轨迹的长时外推预测能力
...
### [110] [EvoTok: A Unified Image Tokenizer via Residual Latent Evolution for Visual Understanding and Generation](https://arxiv.org/abs/2603.12108)
*Yan Li,Ning Liao,Xiangyu Zhao,Shaofeng Zhang,Xiaoxing Wang,Yifan Yang,Junchi Yan,Xue Yang*

Main category: cs.CV

TL;DR: EvoTok提出了一种统一的图像分词器，通过残差演化过程在共享潜在空间中协调视觉理解与生成之间的粒度差异，实现了高质量重建和跨任务性能


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中视觉理解与图像生成之间的粒度差异问题：理解需要高级语义抽象，而生成需要细粒度像素级表示。现有方法在同一表示上施加两种监督会导致干扰，在分离特征空间上解耦则导致不一致性

Method: 提出EvoTok统一图像分词器，通过残差向量量化将图像编码为级联的残差标记序列，形成演化轨迹：早期阶段捕获低级细节，深层阶段逐渐过渡到高级语义表示。在共享潜在空间中进行残差演化，而非维护独立的像素和语义标记空间

Result: 在仅使用1300万图像的小规模数据集上训练，在ImageNet-1K 256x256分辨率上达到0.43 rFID的强重建质量。与大型语言模型集成后，在9个视觉理解基准中的7个表现优异，在GenEval和GenAI-Bench等图像生成基准上取得显著成果

Conclusion: 将视觉表示建模为演化轨迹为统一视觉理解与生成提供了有效且原则性的解决方案，证明了通过残差演化过程协调粒度差异的可行性
...
### [111] [Hoi3DGen: Generating High-Quality Human-Object-Interactions in 3D](https://arxiv.org/abs/2603.12126)
*Agniv Sharma,Xianghui Xie,Tom Fischer,Eddy Ilg,Gerard Pons-Moll*

Main category: cs.CV

TL;DR: Hoi3DGen：基于多模态大语言模型构建高质量人-物交互数据，实现文本到3D人-物交互生成的新框架，显著提升交互保真度和文本一致性


<details>
  <summary>Details</summary>
Motivation: 现有基于文本到图像模型分数蒸馏的方法存在Janus问题（多面问题）和文本提示不忠实的问题，主要原因是高质量交互数据的稀缺。需要开发能够精确遵循输入交互描述的高质量纹理网格生成方法

Method: 1. 利用多模态大语言模型策划真实高质量交互数据；2. 构建完整的文本到3D生成流程；3. 实现交互保真度的数量级改进

Result: 在文本一致性上超越基线4-15倍，在3D模型质量上超越基线3-7倍，对多样类别和交互类型表现出强泛化能力，同时保持高质量的3D生成

Conclusion: Hoi3DGen框架能够生成精确遵循输入交互描述的高质量纹理网格，解决了现有方法中的Janus问题和文本不忠实问题，在AR、XR和游戏应用中具有重要价值
...
### [112] [O3N: Omnidirectional Open-Vocabulary Occupancy Prediction](https://arxiv.org/abs/2603.12144)
*Mengfei Duan,Hao Shi,Fei Teng,Guoqiang Zhao,Yuheng Zhang,Zhiyong Li,Kailun Yang*

Main category: cs.CV

TL;DR: O3N是首个纯视觉、端到端的全向开放词汇占用预测框架，通过极坐标螺旋Mamba模块实现360°连续空间表示，结合占用成本聚合模块统一几何与语义监督，自然模态对齐模块协调视觉特征、体素嵌入和文本语义，在多个基准上达到SOTA性能。
...
Method: 提出O3N框架：1) 极坐标螺旋Mamba(PsM)模块通过极坐标螺旋拓扑嵌入全向体素，实现连续空间表示和长距离上下文建模；2) 占用成本聚合(OCA)模块统一体素空间中的几何和语义监督；3) 自然模态对齐(NMA)模块建立无梯度对齐路径，协调视觉特征、体素嵌入和文本语义。

Result: 在QuadOcc和Human360Occ基准上达到最先进性能，展现出显著的跨场景泛化能力和语义可扩展性，为通用3D世界建模铺平道路。

Conclusion: O3N是首个纯视觉、端到端的全向开放词汇占用预测框架，通过创新的极坐标螺旋表示、统一监督机制和模态对齐方法，实现了对3D世界的全面感知和重建，具有重要的应用前景。
...
### [113] [EgoIntent: An Egocentric Step-level Benchmark for Understanding What, Why, and Next](https://arxiv.org/abs/2603.12147)
*Ye Pan,Chi Kit Wong,Yuanhuiyi Lyu,Hanqian Li,Jiahao Huo,Jiacheng Chen,Lutao Jiang,Xu Zheng,Xuming Hu*

Main category: cs.CV

TL;DR: 提出EgoIntent基准，用于评估多模态大语言模型在自我中心视频中的步骤级意图理解能力，包含3,014个步骤、15个日常生活场景，评估三个维度：局部意图、全局意图和下一步计划。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注片段级意图推理，忽略了步骤级意图理解。智能助手、机器人模仿学习和增强现实指导等应用需要理解每个步骤中人物在做什么、为什么做以及接下来要做什么，以提供及时和上下文感知的支持。

Method: 引入EgoIntent基准，包含3,014个步骤，涵盖15个室内外日常生活场景。每个片段在查询步骤的关键结果发生前截断，不包含后续步骤的帧，防止未来帧泄漏。评估15个MLLM在三个互补维度：局部意图（What）、全局意图（Why）和下一步计划（Next）。

Result: 评估了15个MLLM（包括最先进的闭源和开源模型）。即使表现最好的模型在三个意图维度上的平均得分仅为33.31，表明自我中心视频中的步骤级意图理解仍然是一个极具挑战性的问题。

Conclusion: EgoIntent基准揭示了当前MLLM在自我中心视频步骤级意图理解方面的局限性，该问题需要进一步研究。基准设计避免了未来帧泄漏，专注于预期性步骤理解和下一步规划。
...
### [114] [Linking Perception, Confidence and Accuracy in MLLMs](https://arxiv.org/abs/2603.12149)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要关注提升视觉感知准确性，但忽视了"模型是否知道自己不知道"这一关键问题。研究发现MLLMs存在严重的置信度校准问题，需要解决模型过度自信或自信不足的问题。

Method: 提出CDRL（置信度驱动的强化学习），使用原始-噪声图像对和基于置信度的奖励来增强感知敏感性和校准置信度。进一步提出CA-TTS（置信度感知的测试时缩放），动态协调自一致性、自反思和视觉自检模块，由专家模型作为规划者、批评者和投票者来调度这些模块。

Result: 在四个基准测试中取得了新的SOTA结果，平均提升8.8%。消融研究证明了各模块的有效性和缩放优势。

Conclusion: 置信度校准对MLLMs至关重要，CDRL和CA-TTS框架不仅能提升训练效果，还能通过测试时缩放获得额外性能提升，为MLLMs的可靠性提供了系统解决方案。
...
### [115] [LatentGeo: Learnable Auxiliary Constructions in Latent Space for Multimodal Geometric Reasoning](https://arxiv.org/abs/2603.12166)
...
Method: 提出LatentGeo框架，学习连续潜在视觉表示来内化辅助几何构造，无需像素级渲染或外部执行器。设计三阶段课程通过辅助视觉监督逐步对齐和内化这些潜在表示，然后采用LaGDPO（潜在感知强化学习）程序在策略优化期间稳定潜在表示并提高最终任务正确性。

Result: 在GeoAux（新提出的针对视觉依赖几何问题的基准）和MathVerse上进行实验，结果显示LatentGeo在几何推理任务上取得实质性增益，特别是在需要辅助构造的任务上。广泛分析和消融研究进一步验证了框架中每个组件的有效性。

Conclusion: LatentGeo通过连续潜在表示内化辅助几何构造，解决了现有方法的局限性，在几何推理任务上表现出色，为多模态大语言模型的几何表示提供了新思路。
...
### [116] [ForensicZip: More Tokens are Better but Not Necessary in Forensic Vision-Language Models](https://arxiv.org/abs/2603.12208)
*Yingxin Lai,Zitong Yu,Jun Wang,Linlin Shen,Yong Xu,Xiaochun Cao*

Main category: cs.CV

TL;DR: ForensicZip是一个无需训练的多模态大语言模型加速框架，通过伪造驱动视角进行视觉token剪枝，在保留90%计算量减少的同时维持先进的伪造检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM多媒体取证方法在处理高分辨率图像和视频时计算成本高，而传统的语义驱动token剪枝方法会丢弃包含伪造痕迹（如高频异常和时间抖动）的背景区域，影响检测准确性。

Method: 将时间token演化建模为带松弛虚拟节点的生灭最优传输问题，量化指示瞬时生成伪影的物理不连续性；整合传输新颖性和高频先验进行法医评分，在大比例压缩下分离法医证据与语义内容。

Result: 在深度伪造和AIGC基准测试中，在10%token保留率下，ForensicZip实现2.97倍加速和超过90%FLOPs减少，同时保持最先进的检测性能。

Conclusion: ForensicZip通过伪造驱动token压缩有效解决了MLLM多媒体取证的计算效率问题，在保持检测准确性的同时显著降低计算成本，为实际应用提供了实用解决方案。
...
### [117] [Real-World Point Tracking with Verifier-Guided Pseudo-Labeling](https://arxiv.org/abs/2603.12217)
...
<details>
  <summary>Details</summary>
Motivation: 长期点跟踪模型通常在合成数据集上训练，在真实世界视频中性能下降，因为特征差异和缺乏密集真值标注。自训练是实用解决方案，但伪标签质量严重依赖教师模型的可靠性，而教师模型在不同帧和场景中表现不一。

Method: 提出verifier元模型，学习评估跟踪器预测的可靠性并指导伪标签生成。给定多个预训练跟踪器的候选轨迹，verifier逐帧评估并选择最可信的预测，生成高质量伪标签轨迹。用于微调时，verifier引导的伪标签生成显著提升监督质量，实现数据高效的自适应。

Result: 在四个真实世界基准测试上的广泛实验表明，该方法取得了最先进的结果，同时比先前的自训练方法需要更少的数据。

Conclusion: verifier元模型通过评估跟踪器预测可靠性来指导伪标签生成，有效解决了真实世界视频中自训练的质量问题，实现了数据高效的自适应，在多个基准上达到SOTA性能。
...
### [118] [A Two-Stage Dual-Modality Model for Facial Emotional Expression Recognition](https://arxiv.org/abs/2603.12221)
*Jiajun Sun,Zhe Gao*

Main category: cs.CV

TL;DR: 该论文提出了一种两阶段双模态（视听）模型，用于解决ABAW竞赛中的面部表情识别挑战，通过DINOv2视觉编码器、多尺度特征提取和音频特征融合，在复杂视频环境下实现了优于基准的性能。


<details>
  <summary>Details</summary>
Motivation: 解决ABAW竞赛中无约束视频面部表情识别的挑战，包括不准确的人脸定位、大姿态和尺度变化、运动模糊、时间不稳定性等困难因素，需要开发鲁棒的跨模态方法。

Method: 提出两阶段双模态模型：第一阶段使用预训练的DINOv2 ViT-L/14作为视觉编码器，采用PadAug数据增强和MoE训练头；第二阶段从原始视频多尺度重新裁剪人脸，提取视觉特征并与Wav2Vec 2.0音频特征融合，通过门控融合模块和推理时时间平滑处理。

Result: 在ABAW数据集上，模型在官方验证集上获得0.5368的Macro-F1分数，在5折交叉验证中获得0.5122 ± 0.0277，优于官方基准方法。

Conclusion: 提出的两阶段双模态方法有效解决了无约束视频中的表情识别挑战，通过鲁棒的视觉特征提取和音频-视觉融合策略，在复杂环境下实现了优越的性能表现。
...
### [119] [SceneAssistant: A Visual Feedback Agent for Open-Vocabulary 3D Scene Generation](https://arxiv.org/abs/2603.12238)
...
Method: 结合现代3D物体生成模型与VLM空间推理能力，通过原子操作集（缩放、旋转、聚焦等）实现迭代式视觉反馈驱动场景优化

Result: 能够生成多样化、开放词汇、高质量的3D场景，在定性分析和定量人工评估中均优于现有方法，支持基于自然语言的场景编辑

Conclusion: SceneAssistant通过视觉反馈驱动的迭代优化机制，成功实现了开放词汇3D场景生成，为数字内容创作提供了灵活有效的解决方案
...
TL;DR: Spatial-TTT：一种基于测试时训练（TTT）的流式视觉空间智能方法，通过自适应参数更新来从长时视频中捕获和组织空间证据


<details>
  <summary>Details</summary>
Motivation: 人类通过连续的视觉观察来感知和理解现实空间，因此需要能够从潜在无限长的视频流中持续维护和更新空间证据的能力。核心挑战不仅在于更长的上下文窗口，更在于如何随时间选择、组织和保留空间信息。

Method: 提出Spatial-TTT方法，采用测试时训练（TTT）机制，通过自适应更新部分参数（快速权重）来捕获和组织长时场景视频中的空间证据。设计了混合架构，采用大块更新与滑动窗口注意力并行的高效空间视频处理。引入空间预测机制应用于TTT层，使用3D时空卷积来促进几何对应性和时间连续性的捕获。构建了带有密集3D空间描述的数据集，指导模型更新快速权重以结构化的方式记忆和组织全局3D空间信号。

Result: 大量实验表明，Spatial-TTT显著提升了长时空间理解能力，在视频空间基准测试中达到了最先进的性能。

Conclusion: Spatial-TTT通过测试时训练和专门设计的架构，有效解决了流式视觉空间智能中的核心挑战，实现了从长时视频中持续维护和更新空间证据的能力，为空间理解任务提供了新的解决方案。
...
### [121] [One Model, Many Budgets: Elastic Latent Interfaces for Diffusion Transformers](https://arxiv.org/abs/2603.12245)
...
TL;DR: ELIT是一种弹性潜在接口变换器，通过可学习的变长潜在序列解耦图像分辨率与计算量，实现动态计算分配和重要性感知的推理调整。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）存在两个主要限制：1）计算量（FLOPs）与图像分辨率锁定，难以进行延迟-质量的权衡；2）计算均匀分配在所有空间token上，浪费资源在不重要区域。

Method: 引入弹性潜在接口变换器（ELIT），在标准变换器块前插入潜在接口（可学习的变长token序列）。通过轻量级的读写交叉注意力层在空间token和潜在token间传递信息，并优先处理重要区域。训练时随机丢弃尾部潜在token，使模型学习生成重要性排序的表示。

Result: 在多种数据集和架构（DiT、U-ViT、HDiT、MM-DiT）上均获得一致提升。在ImageNet-1K 512px上，FID和FDD分数平均提升35.3%和39.6%。推理时可动态调整潜在token数量以适应计算约束。

Conclusion: ELIT是一种最小化的DiT兼容机制，仅添加两个交叉注意力层，保持整流流目标和DiT堆栈不变，实现了计算与图像分辨率的解耦，提供了灵活的延迟-质量权衡。
...
### [122] [Trust Your Critic: Robust Reward Modeling and Reinforcement Learning for Faithful Image Editing and Generation](https://arxiv.org/abs/2603.12247)
...
TL;DR: FIRM框架通过构建高质量评分数据集和训练专用奖励模型，解决RL图像编辑和文本生成中奖励模型幻觉和噪声评分问题，实现更准确的图像生成与编辑指导。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习中的奖励模型存在幻觉问题和噪声评分，误导图像编辑和文本到图像生成的优化过程，需要开发更准确可靠的奖励模型来提供忠实指导。

Method: 1) 设计定制化数据收集流程构建高质量评分数据集(FIRM-Edit-370K和FIRM-Gen-293K)；2) 训练专用奖励模型(FIRM-Edit-8B和FIRM-Gen-8B)；3) 提出FIRM-Bench基准测试；4) 引入"Base-and-Bonus"奖励策略，包括编辑的CME和生成的QMA。

Result: FIRM模型在人类判断对齐方面优于现有指标，FIRM-Qwen-Edit和FIRM-SD3.5模型实现显著性能突破，有效减轻幻觉问题，在保真度和指令遵循方面建立新标准。

Conclusion: FIRM框架通过高质量数据集、专用奖励模型和创新的奖励策略，为图像生成和编辑提供了准确可靠的强化学习指导，显著提升了生成结果的忠实度和质量。
...
### [123] [DVD: Deterministic Video Depth Estimation with Generative Priors](https://arxiv.org/abs/2603.12250)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs作为文本编码器集成到扩散框架中存在两个关键限制：1) MLLMs文本编码器推理深度不足，单步编码无法激活思维链过程；2) 解码过程中指导保持不变，阻碍DiT逐步分解复杂指令

Method: 提出EndoCoT框架：1) 迭代思维引导模块通过迭代细化潜在思维状态激活MLLMs推理潜力；2) 终端思维接地模块通过将最终状态与真实答案对齐，确保推理轨迹基于文本监督；将思维状态桥接到DiT去噪过程
...
### [125] [Attend Before Attention: Efficient and Scalable Video Understanding via Autoregressive Gazing](https://arxiv.org/abs/2603.12254)
...
Method: AutoGaze采用自回归方式选择最小多尺度补丁集，在用户指定的误差阈值内重建视频，结合下一标记预测和强化学习进行训练，在ViT或MLLM处理前去除冗余补丁

Result: AutoGaze将视觉标记减少4-100倍，加速ViT和MLLM达19倍，使MLLM能够处理1K帧4K分辨率视频，在VideoMME上达到67.0%的优异表现。在HLVid基准上比基线提升10.1%，优于之前最佳MLLM 4.5%

Conclusion: AutoGaze通过有效消除视频冗余，显著提升了MLLM处理长高分辨率视频的效率和性能，为大规模视频理解提供了实用解决方案，并引入了新的长视频基准HLVid
...
### [126] [DreamVideo-Omni: Omni-Motion Controlled Multi-Subject Video Customization with Latent Identity Reinforcement Learning](https://arxiv.org/abs/2603.12257)
...
Method: 采用渐进式两阶段训练范式：第一阶段整合主体外观、全局运动、局部动态和摄像机运动等综合控制信号，引入条件感知3D旋转位置编码协调异构输入，使用层次化运动注入策略增强全局运动引导，并通过组和角色嵌入解决多主体模糊问题；第二阶段设计潜在身份奖励反馈学习范式，在预训练视频扩散骨干上训练潜在身份奖励模型，在潜在空间提供运动感知的身份奖励。

Result: 基于构建的大规模数据集和全面的DreamOmni评估基准，DreamVideo-Omni在生成高质量视频和精确可控性方面表现出优越性能。

Conclusion: DreamVideo-Omni是一个统一的框架，通过创新的两阶段训练方法解决了多主体身份保持和全方位运动控制的挑战，实现了高质量、精确可控的视频生成。
...
### [127] [Video Streaming Thinking: VideoLLMs Can Watch and Think Simultaneously](https://arxiv.org/abs/2603.12262)
...
<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解方法缺乏同步的逻辑推理流，直接应用测试时缩放方法会导致不可接受的响应延迟，需要在实时响应和深度理解之间找到平衡。

Method: 提出Video Streaming Thinking (VST)范式，支持"边看边思考"机制，在视频流播放过程中激活推理；设计包含VST-SFT（结构适应）和VST-RL（端到端改进）的后训练流程；开发基于视频知识图谱的自动化训练数据合成管道。

Result: VST-7B在StreamingBench上达到79.5%，在OVO-Bench上达到59.3%；相比Video-R1响应速度快15.7倍，在VideoHolmes上提升5.4%；在离线长视频和推理基准上保持竞争力。

Conclusion: VST通过创新的流式思考机制，有效解决了视频流理解中实时响应与深度推理的权衡问题，在多个基准上展现了高效性和强泛化能力。
...
### [128] [GRADE: Benchmarking Discipline-Informed Reasoning in Image Editing](https://arxiv.org/abs/2603.12264)
...
Result: 实验揭示了当前模型在隐式、知识密集型编辑设置下的显著局限性，导致较大的性能差距；通过严格分析和消融实验暴露了模型缺陷，并识别了学科编辑中的约束条件。
...
### [129] [EVATok: Adaptive Length Video Tokenization for Efficient Visual Autoregressive Generation](https://arxiv.org/abs/2603.12267)
...
<details>
  <summary>Details</summary>
Motivation: 传统视频分词器对所有视频采用统一的token分配策略，导致在简单、静态或重复片段浪费token，而在动态复杂片段token不足。这种效率低下需要解决，以实现更好的质量-成本权衡。

Method: EVATok框架包含三个核心组件：1) 估计每个视频的最优token分配以实现最佳质量-成本权衡；2) 开发轻量级路由器快速预测这些最优分配；3) 训练自适应分词器，根据路由器预测的分配方案编码视频。采用集成视频语义编码器的高级训练方案。

Result: EVATok在视频重建和下游自回归生成方面显著提升了效率和整体质量。在UCF-101数据集上实现了卓越的重建效果和最先进的类别到视频生成性能，相比先前最先进的LARP和固定长度基线，平均token使用量至少节省24.4%。

Conclusion: EVATok通过自适应token分配策略有效解决了传统视频分词器的效率问题，实现了更好的质量-计算成本平衡，为视频生成任务提供了更高效的解决方案。
...
### [130] [Wide-Area GNSS Spoofing and Jamming Detection Using AIS-Derived Spatiotemporal Integrity Monitoring](https://arxiv.org/abs/2603.11055)
*Sanghyeon Park,DeukJae Cho,Pyo-Woong Son*

Main category: cs.CR

TL;DR: 提出三阶段AIS框架，通过预过滤通信层缺陷、提取运动学异常、时空聚类分析，实现GNSS欺骗和干扰检测，在韩国沿海水域测试中有效识别干扰事件并大幅降低误报率。


<details>
  <summary>Details</summary>
Motivation: GNSS欺骗和干扰威胁海上导航，会破坏AIS应答器的位置信息。原始AIS消息包含通信层缺陷（重复MMSI、时间戳错误、陈旧重传、多站重播延迟），这些缺陷可能被误判为欺骗或干扰，因此未经预过滤的AIS位置数据不可靠。

Method: 提出三阶段框架：1) 使用基于规则的诊断方法丢弃通信故障；2) 应用交互多模型滤波器和传输间隔分析提取运动学一致性和连续性异常；3) 应用时空DBSCAN聚类，根据多船一致性和时间持续性对异常进行分组，并将其分类为传感器故障、欺骗或干扰。

Result: 在韩国沿海水域约9.66亿条AIS消息上进行测试，框架检测到17个欺骗集群和343个干扰集群，相对于朴素聚类方法将误报率降低了98.6%。

Conclusion: 经过严格的预过滤后，AIS数据能够实现广域GNSS干扰检测，无需专用传感器。该框架有效区分了通信层缺陷与真实的GNSS干扰事件。
...
### [131] [The Attack and Defense Landscape of Agentic AI: A Comprehensive Survey](https://arxiv.org/abs/2603.11088)
*Juhee Kim,Xiaoyuan Liu,Zhun Wang,Shi Qiu,Bo Li,Wenbo Guo,Dawn Song*

Main category: cs.CR

TL;DR: 首篇系统全面的AI智能体安全综述，涵盖设计空间、攻击面、防御机制分析，并提出首个系统化安全框架


<details>
  <summary>Details</summary>
Motivation: AI智能体（大语言模型与非AI系统组件结合）在现实应用中快速发展，但其前所未有的灵活性带来了与传统软件系统根本不同的复杂安全挑战，需要进行系统化研究

Method: 采用系统性综述方法，分析AI智能体的设计空间、攻击面（攻击景观）和防御机制，进行案例研究以识别现有安全差距，并引入首个系统化安全框架

Result: 提出了AI智能体安全的全面分析框架，识别了现有安全差距和开放挑战，为构建安全智能体系统和推进该领域研究奠定了基础

Conclusion: AI智能体安全是一个新兴且关键的领域，需要系统化的安全框架来应对其独特的安全挑战，本文为这一领域的研究和实践提供了基础性框架
...
### [132] [Primitive-Root Determinant Densities over Prime Fields and Implications for PRIM-LWE](https://arxiv.org/abs/2603.11196)
*Vipin Singh Sehrawat*

Main category: cs.CR

TL;DR: 该论文解决了prim-LWE问题中关于矩阵具有原根行列式的密度常数c(p)的渐近行为问题，证明了inf_{p prime} c(p)=0，并给出了c(p)的精确阶、分布特征及密码学相关素数的显式下界。


<details>
  <summary>Details</summary>
Motivation: 解决Sehrawat等人提出的关于prim-LWE问题中维度均匀约化常数c(p)的渐近行为问题，特别是inf_{p prime} c(p)是否为零的问题。原问题依赖于原数素数的猜想，本文旨在无条件解决该问题。

Method: 使用狄利克雷定理和梅滕斯乘积公式，完全绕过了原数素数猜想。通过数论分析建立了c(p)的渐近行为，推导了其分布特征，并针对密码学相关素数给出了参数化的显式下界。

Result: 1. 无条件证明了inf_{p prime} c(p)=0；2. 建立了min_{p≤x} c(p) ≍ 1/log log x的精确阶；3. 证明了c(p)在素数上具有连续的纯奇异极限分布，支撑集为[0,1/2]；4. 给出了仅依赖于ω(q-1)的密码学素数c(q)的显式下界；5. 计算了NIST标准化模数q=3329和q=8380417的拒绝采样开销上界分别为2.17和3.42。

Conclusion: 本文完全解决了prim-LWE问题中维度均匀约化常数的渐近行为问题，提供了无条件的结果和实用的密码学界限。结果表明对于密码学相关的素数，拒绝采样开销是可管理的，为prim-LWE在密码学中的应用提供了理论基础。
...
### [133] [Security-by-Design for LLM-Based Code Generation: Leveraging Internal Representations for Concept-Driven Steering Mechanisms](https://arxiv.org/abs/2603.11212)
*Maximilian Wendlinger,Daniel Kowatsch,Konstantin Böttinger,Philip Sperl*

Main category: cs.CR

TL;DR: 论文提出SCS-Code方法，通过引导CodeLLMs内部表征来生成安全代码，相比现有方法在多个安全编码基准测试中表现更优


<details>
  <summary>Details</summary>
Motivation: 当前CodeLLMs经常生成功能正确但不安全的代码，现有改进方法效果有限，缺乏对代码生成内部机制和安全漏洞根源的理解

Method: 研究CodeLLMs内部安全概念表征，发现模型在生成不安全代码时通常意识到漏洞存在。提出SCS-Code方法，在token生成过程中引导LLMs内部表征朝向安全功能代码输出

Result: SCS-Code在多个安全编码基准测试中相比最先进方法表现更优，实现了轻量级模块化机制，可集成到现有代码模型中

Conclusion: 通过理解CodeLLMs内部安全概念表征并引导这些表征，可以有效提高AI生成代码的安全性，为实际应用提供了可行方案
...
### [134] [LROO Rug Pull Detector: A Leakage-Resistant Framework Based on On-Chain and OSINT Signals](https://arxiv.org/abs/2603.11324)
*Fatemeh Shoaei,Mohammad Pishdar,Mozafar Bag-Mohammadi,Mojtaba Karami*

Main category: cs.CR

TL;DR: 本文提出了一种泄漏感知的早期跑路盘检测框架，结合链上行为指标与时间对齐的OSINT信号，使用TabPFN模型在严格时间约束下学习多模态表格数据，实现了强判别性能和改进的概率校准。


<details>
  <summary>Details</summary>
Motivation: 智能合约生态系统虽然支持去中心化应用，但其不可变性和无需许可的设计也助长了大规模欺诈。跑路盘攻击是其中最常见的一种，项目方在人为抬高代币价值后突然撤出流动性。现有检测方法主要依赖反应式链上信号，且常受时间数据泄漏影响，限制了其实际可靠性。

Method: 构建了包含1,000个代币项目的手工标注数据集，涵盖DeFi和非DeFi场景，所有特征均在流动性撤出前提取以保证因果有效性。数据集结合结构化的链上指标与来自社交媒体活动和搜索趋势的外部关注信号。在该框架中，使用TabPFN作为核心建模组件，在严格时间约束下从多模态表格数据中学习。

Result: 实验结果表明，所提框架相比经典基线方法实现了强判别性能和改进的概率校准，同时保持低假阴性率。通过将跑路盘检测构建为因果、多模态预测问题，强调了泄漏弹性评估和校准风险估计在区块链安全系统中部署的必要性。

Conclusion: 本文提出的泄漏感知框架为早期跑路盘检测提供了有效解决方案，通过整合链上行为指标与时间对齐的OSINT信号，在严格因果约束下实现了可靠的欺诈检测，强调了泄漏弹性评估和校准风险估计对区块链安全系统的重要性。
...
### [135] [Unclonable Encryption in the Haar Random Oracle Model](https://arxiv.org/abs/2603.11437)
*James Bartusek,Eli Goldin*

Main category: cs.CR

TL;DR: 在Haar随机预言机模型中构建可重用、任意长度消息的不可克隆加密方案，首次证明计算假设下的UE可能存在于"微密码"世界


<details>
  <summary>Details</summary>
Motivation: 探索在"微密码"世界中实现不可克隆加密的可能性，该世界可能不存在单向函数，但存在Haar随机酉矩阵预言机

Method: 基于路径记录框架，在Haar随机预言机模型中构建UE方案，证明"酉矩阵重编程引理"，支持密钥重用和任意长度消息加密

Result: 成功构造满足不可克隆不可区分性安全标准的UE方案，首次证明计算假设下的可重用UE可能存在于微密码世界

Conclusion: 该工作为不可克隆加密在弱密码假设下的存在性提供了首个证据，提出的酉矩阵重编程引理具有独立理论价值
...
### [136] [Software-Hardware Binding for Protection of Sensitive Data in Embedded Software](https://arxiv.org/abs/2603.11727)
*Bernhard Fischer,Daniel Dorfmeister,Flavio Ferrarotti,Manuel Penz,Michael Kargl,Martina Zeinzinger,Florian Eibensteiner*

Main category: cs.CR

TL;DR: 提出一种结合硬件指纹与布尔逻辑的嵌入式软件IP保护机制，使未授权副本在克隆设备上仍可运行但性能下降，无需额外硬件支持


<details>
  <summary>Details</summary>
Motivation: 工业系统嵌入式软件依赖调优数据作为重要知识产权，需要有效保护机制防止未授权复制和使用

Method: 结合硬件指纹（基于SRAM的物理不可克隆函数PUF）与布尔逻辑，将软件参数绑定到特定微控制器单元，无需额外硬件

Result: 安全评估显示只有对受保护软件在真实目标设备上进行复杂动态分析才能揭示秘密数据，保护机制更难绕过

Conclusion: 该方法提供了一种更难以绕过的IP保护方案，使未授权副本在克隆设备上性能下降，且仅需软件更新而无需额外硬件
...
### [137] [The Mirror Design Pattern: Strict Data Geometry over Model Scale for Prompt Injection Detection](https://arxiv.org/abs/2603.11875)
*J Alex Corll*

Main category: cs.CR

TL;DR: Mirror提出了一种数据整理设计模式，通过构建匹配的正负样本单元，训练稀疏字符n-gram线性SVM作为第一层提示注入检测器，在524个测试样本上达到95.97%召回率和92.07% F1分数，亚毫秒级延迟，优于44.35%召回率的2200万参数模型。


<details>
  <summary>Details</summary>
Motivation: 现有提示注入防御通常被视为语义理解问题，依赖大型神经检测器，但第一层筛选需要快速、确定性、不可提示和可审计的特性。需要一种轻量级、高效的检测方法。

Method: 提出Mirror数据整理设计模式，将提示注入语料组织成匹配的正负样本单元，使分类器学习控制平面攻击机制而非语料捷径。使用5000个严格筛选的开源样本构建32单元拓扑，训练稀疏字符n-gram线性SVM，编译为静态Rust组件。

Result: 在524个测试样本上获得95.97%召回率和92.07% F1分数，亚毫秒级延迟，无外部模型运行时依赖。相比之下，2200万参数的Prompt Guard~2模型仅达到44.35%召回率和59.14% F1分数，中位延迟49ms，p95延迟324ms。

Conclusion: 对于L1提示注入筛选，严格的数据几何结构比模型规模更重要。线性模型虽然会留下语义歧义（如使用与提及的区别）供后续管道层处理，但在其适用范围内，数据质量比模型规模更关键。
...
### [138] [Understanding LLM Behavior When Encountering User-Supplied Harmful Content in Harmless Tasks](https://arxiv.org/abs/2603.11914)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs的对齐主要关注任务层面的伦理（拒绝执行直接有害任务），但忽视了内容层面的伦理问题：在执行看似无害的任务时，如果用户提供的材料中包含有害内容，LLMs是否会像有道德意识的人类一样拒绝继续处理？

Method: 1. 构建包含1,357个条目、涵盖10个有害类别（不符合OpenAI使用政策）的有害知识数据集作为用户提供的有害内容；2. 设计9个无害任务（符合OpenAI使用政策），根据所需用户内容量分为广泛、中等、有限三类；3. 评估9个LLMs在执行无害任务时遇到有害内容的反应；4. 分析有害知识类别与任务类型之间的动态关系如何影响不同LLMs。
...
Conclusion: LLMs存在被忽视的内容层面伦理风险，即使执行无害任务也可能处理有害内容。研究呼吁利益相关者加强安全措施以减轻这种风险，为LLMs安全对齐提供了新的研究方向。
...
### [139] [Delayed Backdoor Attacks: Exploring the Temporal Dimension as a New Attack Surface in Pre-Trained Models](https://arxiv.org/abs/2603.11949)
*Zikang Ding,Haomiao Yang,Meng Hao,Wenbo Jiang,Kunlan Xiang,Runmeng Du,Yijing Liu,Ruichen Zhang,Dusit Niyato*

Main category: cs.CR

TL;DR: 论文提出延迟后门攻击(DBA)，挑战传统后门攻击的"即时性假设"，通过时间维度分离触发与激活，使日常词汇可作为触发器，实现可控延迟爆发。


<details>
  <summary>Details</summary>
Motivation: 挑战预训练模型后门攻击的"即时性假设"，探索时间维度作为新的攻击面，使攻击者能够使用常见日常词汇作为触发器，实现更隐蔽、更复杂的攻击模式。

Method: 提出基于非线性衰减的延迟后门攻击(DND)，嵌入轻量级有状态逻辑模块，通过可配置阈值延迟激活，建立形式化模型描述延迟行为，采用双指标评估框架(ASR和ASR_delay)。

Result: 在四个NLP基准测试中验证DND核心能力：可控持续休眠、保持高清洁准确率(≥94%)、实现接近完美的激活后攻击成功率(≈99%)，且对多种先进防御方法具有抗性。

Conclusion: 时间维度构成预训练模型中可行但未受保护的攻击面，需要开发下一代有状态、时间感知的防御机制来应对延迟后门攻击威胁。
...
### [140] [EmbTracker: Traceable Black-box Watermarking for Federated Language Models](https://arxiv.org/abs/2603.12089)
*Haodong Zhao,Jinming Hu,Yijie Bai,Tian Dong,Wei Du,Zhuosheng Zhang,Yanjiao Chen,Haojin Zhu,Gongshen Liu*

Main category: cs.CR

TL;DR: EmbTracker是一个面向联邦语言模型的服务器端、可追踪的黑盒水印框架，通过嵌入基于后门的水印实现黑盒验证，并为每个客户端注入独特身份水印实现个体级溯源。


<details>
  <summary>Details</summary>
Motivation: 联邦语言模型存在安全漏洞，不可信客户端可能泄露接收到的模型实例。现有水印方案需要白盒访问和客户端合作，仅提供组级所有权证明，缺乏个体级可追溯性。

Method: EmbTracker采用服务器端黑盒水印框架：1) 通过嵌入基于后门的水印实现黑盒可验证性，仅需API查询即可检测；2) 为每个客户端注入独特的身份特定水印，实现客户端级可追溯性；3) 即使面对非合作参与者也能确保鲁棒性。

Result: 在各种语言和视觉语言模型上的实验表明：EmbTracker实现接近100%的验证率，对移除攻击（微调、剪枝、量化）具有高抵抗力，对主要任务性能影响可忽略（通常在1-2%以内）。

Conclusion: EmbTracker为联邦语言模型提供了一个有效的服务器端、可追踪的黑盒水印解决方案，能够在保护模型知识产权的同时实现个体级溯源，解决了现有方案的局限性。
...
### [141] [Understanding Disclosure Risk in Differential Privacy with Applications to Noise Calibration and Auditing (Extended Version)](https://arxiv.org/abs/2603.12142)
*Patricia Guerra-Balboa,Annika Sauer,Héber H. Arcolezi,Thorsten Strufe*

Main category: cs.CR

TL;DR: 本文提出重构优势作为统一的风险度量，替代传统重构鲁棒性，为差分隐私系统提供更准确的风险评估和噪声校准方法。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私风险分析主要关注成员推断攻击，或依赖重构鲁棒性度量。但在现实假设下，重构鲁棒性可能产生误导性风险估计并违反声称的边界，限制了其在系统化差分隐私校准和审计中的实用性。

Method: 引入重构优势作为统一风险度量，捕捉成员推断、属性推断和数据重构的综合风险。推导差分隐私噪声与对抗优势之间的紧致边界，并针对任意差分隐私机制和攻击者知识特征化最优对抗策略。

Result: 重构优势提高了差分隐私审计的准确性和范围，实现了更有效的效用-隐私权衡。该方法为风险驱动的噪声校准提供了理论基础，支持系统化的差分隐私审计。

Conclusion: 重构优势作为统一风险度量优于传统重构鲁棒性，能够一致地捕捉多种攻击类型的风险，为差分隐私数据管理系统提供更可靠的风险评估框架和校准方法。
...
### [142] [Privacy in ERP Systems: Behavioral Models of Developers and Consultants](https://arxiv.org/abs/2603.12195)
*Alicia Pang,Katsiaryna Labunets,Olga Gadyatskaya*

Main category: cs.CR

TL;DR: 该研究聚焦于ERP系统中的隐私问题，调查了开发者和顾问/经理的隐私意识，并应用Fogg行为模型框架分析他们的隐私行为模式，旨在促进更适当的隐私相关行为。


<details>
  <summary>Details</summary>
Motivation: ERP系统存储敏感数据，企业需遵守GDPR等法规，但开发者可能缺乏隐私知识，因此需要研究ERP系统中的隐私意识现状。

Method: 在国际咨询公司背景下，调查ERP系统开发者和负责ERP相关服务的经理/顾问的隐私意识，特别关注隐私设计和数据最小化原则，应用主题分析和Fogg行为模型框架来构建隐私行为模型。

Result: 研究揭示了两个重要群体的隐私行为模型，为促进开发者和顾问更适当的隐私相关行为提供了方法和依据。

Conclusion: 研究结果为激励ERP系统开发者和顾问采取更充分的隐私保护行为提供了手段，有助于改善ERP系统中的隐私实践。
...
### [143] [Co-Diffusion: An Affinity-Aware Two-Stage Latent Diffusion Framework for Generalizable Drug-Target Affinity Prediction](https://arxiv.org/abs/2603.11125)
*Yining Qian,Pengjie Wang,Yixiao Li,An-Yang Lu,Cheng Tan,Shuang Li,Lijun Liu*

Main category: stat.ML

TL;DR: Co-Diffusion：一种通过约束潜在去噪过程增强泛化能力的药物-靶标亲和力预测框架，在冷启动场景下显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有深度模型在严格的冷启动场景下（标签稀缺和领域偏移）容易出现表示坍塌问题，无法学习可迁移的药效团和结合基序，限制了药物-靶标亲和力预测的泛化能力

Method: 采用两阶段范式：第一阶段通过显式监督目标对齐药物和靶标嵌入，建立亲和力引导的潜在流形；第二阶段引入模态特定的潜在扩散作为随机扰动-去噪正则化器，强制模型从噪声结构表示中恢复一致的亲和力语义

Result: 在多个基准测试中显著优于最先进的基线方法，特别是在未见分子支架和新蛋白质家族上表现出卓越的零样本泛化能力

Conclusion: Co-Diffusion通过将DTA预测重新定义为约束潜在去噪过程，有效缓解了生成式DTA模型中常见的重建-回归冲突，为未探索化学空间中的计算机药物优先排序提供了稳健路径
...
### [144] [Trustworthy predictive distributions for rare events via diagnostic transport maps](https://arxiv.org/abs/2603.11229)
*Elizabeth Cucuzzella,Rafael Izbicki,Ann B. Lee*

Main category: stat.ML

TL;DR: 该论文提出诊断传输映射方法，用于改进预测系统的校准，通过概率到概率的映射调整初始预测分布，提供实时局部诊断并改善罕见事件的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现代预测系统虽然能提供完整的预测分布，但很少能在所有输入x和输出y条件下实现校准。特别是在低频或分布外区域，预测密度估计不可靠，而准确的不确定性量化和专家验证对于建立模型信任至关重要。
...
Result: 将诊断传输映射应用于短期热带气旋强度预测，结果显示易于拟合的参数版本能够识别与局部失校相关的演化模式，并改善罕见事件（包括24小时快速强度变化）的预测性能，优于美国国家飓风中心的业务预报。

Conclusion: 诊断传输映射提供了一种系统方法来诊断和改进预测模型的校准，特别适用于低频和罕见事件，通过提供局部诊断和重新校准的预测分布，增强了预测系统的可信度和实用性。
...
### [145] [A Unified Latent Space Disentanglement VAE Framework with Robust Disentanglement Effectiveness Evaluation](https://arxiv.org/abs/2603.11242)
*Xiaoan Lang,Fang Liu*

Main category: stat.ML

TL;DR: 提出bfVAE框架统一多种解耦VAE方法，并开发无需真实生成因子的解耦评估工具FVH-LT、DBSR-LS和LSDI，通过贪婪对齐策略提高鲁棒性，在表格和图像数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 评估和解释变分自编码器（VAE）等潜在表示仍然是一个重大挑战，特别是在真实生成因子未知的情况下。现有方法通常需要真实生成因子作为基准，限制了其在真实数据中的应用。

Method: 1. 提出bfVAE框架统一多种最先进的解耦VAE方法；2. 开发两种无需真实生成因子的解耦评估程序：通过潜在遍历的特征方差异质性（FVH-LT）和潜在空间中的脏块稀疏回归（DBSR-LS）；3. 提出潜在空间解耦指数（LSDI）综合评估解耦效果；4. 设计贪婪对齐策略（GAS）缓解标签切换问题并跨运行对齐潜在维度。

Result: bfVAE在解耦质量、鲁棒性方面超越现有解耦VAE框架，实现接近零的虚假发现率；FVH-LT和DBSR-LS可靠地揭示语义上有意义且与领域相关的潜在结构；LSDI有效地对解耦效果进行整体定量总结。

Conclusion: bfVAE框架及其配套的评估工具为无需真实生成因子的VAE解耦评估提供了有效解决方案，显著提升了潜在表示的可解释性和内容生成的效率，在表格和图像数据上均表现出色。
...
### [146] [RIE-Greedy: Regularization-Induced Exploration for Contextual Bandits](https://arxiv.org/abs/2603.11276)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的上下文赌博机问题通常使用迭代训练模型（如提升树）解决，但难以直接应用Thompson Sampling或UCB等简单有效的探索策略。现有方法依赖复杂假设或难以实现的程序，需要更实用的探索方法。

Method: 采用纯贪婪动作选择策略，利用模型拟合过程中的随机性作为内在探索源。特别地，利用基于交叉验证的正则化过程的随机性，自然地诱导出类似Thompson Sampling的探索行为。

Result: 理论证明在双臂赌博机情况下，正则化诱导的探索与Thompson Sampling等价。实证表明，在大规模商业环境中，该方法相比epsilon-greedy和其他先进方法能实现更可靠的探索。

Conclusion: 正则化估计器训练本身可以诱导有效的探索，为上下文赌博机设计提供了理论洞见和实践指导，简化了复杂模型中的探索策略实现。
...
### [147] [Worst-case low-rank approximations](https://arxiv.org/abs/2603.11304)
*Anya Fries,Markus Reichstein,David Blei,Jonas Peters*

Main category: stat.ML

TL;DR: wcPCA是一个统一框架，用于处理跨异构域数据的PCA问题，通过优化最坏情况性能而非平均性能来提高分布偏移下的可靠性，并扩展到矩阵补全问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的健康、经济和环境科学数据通常来自异构域（如医院、地区或时间段），分布偏移会使标准PCA不可靠，因为主导主成分在未见域中解释的方差可能远少于训练域。现有方法（如FairPCA）考虑跨多个域的最坏情况性能，但需要更统一的框架来处理异构总方差等应用场景。

Method: 开发了wcPCA统一框架，提出了norm-minPCA和norm-maxregret等新估计器，适用于异构总方差场景。证明了所有目标函数的估计器不仅在观测源域上是最坏情况最优的，而且在协方差位于源协方差凸包内的所有目标域上也是最优的。建立了经验估计器的一致性和渐近最坏情况保证。将方法扩展到矩阵补全问题，证明了归纳矩阵补全的近似最坏情况最优性。

Result: 模拟和两个关于生态系统-大气通量的真实应用表明，在最坏情况性能上有显著改进，而平均性能只有轻微损失。新估计器在异构总方差场景中表现更好，wcPCA框架为处理分布偏移下的低秩近似问题提供了统一的理论基础。

Conclusion: wcPCA框架为跨异构域数据的PCA问题提供了统一的解决方案，通过优化最坏情况性能来提高分布偏移下的可靠性。该方法不仅适用于PCA，还可扩展到矩阵补全等低秩近似问题，在实际应用中能在保持良好平均性能的同时显著改善最坏情况表现。
...
### [148] [Spatially Robust Inference with Predicted and Missing at Random Labels](https://arxiv.org/abs/2603.11368)
*Stephen Salerno,Zhenke Wu,Tyler McCormick*

Main category: stat.ML

TL;DR: 提出一种针对空间依赖和MAR标记数据的双重稳健估计器，通过交叉拟合处理缺失数据，并引入jackknife HAC方差校正来解决交叉拟合引起的空间方差估计偏差问题。


<details>
  <summary>Details</summary>
Motivation: 当结果数据收集成本高昂时，研究者常使用机器学习/AI模型预测替代未标记案例，但这会影响下游统计推断。现有方法在独立采样下有效，但实际应用中存在MAR标记和空间依赖问题，需要新的推断方法。

Method: 提出双重稳健估计器，采用交叉拟合处理缺失数据，并引入jackknife空间异方差和自相关一致(HAC)方差校正，分离空间依赖与交叉拟合引起的噪声。

Result: 在标准识别和依赖条件下，所得置信区间渐近有效。模拟和基准数据集显示，在有限样本校准方面有显著改进，特别是在MAR标记和聚类抽样情况下。

Conclusion: 该方法解决了空间依赖和MAR标记数据下的推断问题，通过jackknife HAC方差校正有效处理交叉拟合引起的空间方差估计偏差，为实际应用提供了可靠的统计推断工具。
...
### [149] [Decomposing Observational Multiplicity in Decision Trees: Leaf and Structural Regret](https://arxiv.org/abs/2603.11701)
*Mustafa Cavus*

Main category: stat.ML

TL;DR: 该论文提出了决策树分类器中观测多重性的两个互补概念：叶遗憾和结构遗憾，并建立了统计保证，实验验证了理论分解与经验方差的高度一致性。


<details>
  <summary>Details</summary>
Motivation: 机器学习任务中存在预测多重性现象，其根本来源之一是观测多重性。虽然逻辑回归的观测多重性理论框架已建立，但对于决策树等非光滑、基于划分的模型，其影响尚未充分探索。

Method: 引入决策树分类器的两个观测多重性概念：叶遗憾（量化固定叶内由于有限样本噪声引起的预测变异性）和结构遗憾（捕获学习树结构本身不稳定性引起的变异性）。提供观测多重性到这两个组件的正式分解并建立统计保证。

Result: 在多个信用风险评分数据集上的实验评估证实了理论分解与经验观测方差近乎完美的一致性。结构遗憾是观测多重性的主要驱动因素，在某些数据集中其变异性是叶遗憾的15倍以上。将这些遗憾度量用作选择性预测中的弃权机制可以有效识别任意区域并提高模型安全性。

Conclusion: 该研究为量化观测多重性建立了严格框架，与算法安全性和可解释性的最新进展保持一致。结构遗憾是决策树观测多重性的主要来源，利用这些遗憾度量可以改善模型安全性和选择性预测性能。
...
### [150] [Hypercomplex Widely Linear Processing: Fundamentals for Quaternion Machine Learning](https://arxiv.org/abs/2603.11835)
*Sayed Pouria Talebi,Clive Cheong Took*

Main category: stat.ML

TL;DR: 本章为四元数在机器学习中的应用奠定基础，涵盖四元数值随机过程的增强统计建模、广泛线性模型、四元数微积分与代数、以及均方估计等核心内容。


<details>
  <summary>Details</summary>
Motivation: 四元数在三维旋转建模方面具有独特优势，已在航空航天和计算机图形学等领域得到工业应用。近年来，随着机器学习的兴起，四元数研究迎来复兴，需要为研究人员提供系统的基础知识以促进该新兴领域的发展。

Method: 通过系统构建四元数理论基础：1) 四元数值随机过程的增强统计建模；2) 利用这些高级统计的广泛线性模型；3) 算法推导所需的四元数微积分和代数；4) 实际应用中的均方估计方法。提供多个示例辅助学习理解。

Result: 建立了四元数在机器学习应用中的完整理论框架，为研究人员提供了从基础统计建模到实际算法实现的系统性知识体系，促进四元数在机器学习领域的进一步研究和应用。

Conclusion: 本章为四元数在机器学习这一新兴研究领域的应用奠定了坚实基础，通过系统化的理论构建和示例说明，使读者能够掌握四元数建模的核心技术，为在该领域的进一步贡献做好准备。
...
### [151] [Wasserstein Gradient Flows for Batch Bayesian Optimal Experimental Design](https://arxiv.org/abs/2603.12102)
*Louis Sharrock*

Main category: stat.ML

TL;DR: 提出一种基于概率提升的批量贝叶斯最优实验设计方法，通过设计测度空间上的熵正则化优化，获得吉布斯分布形式的最优设计策略


<details>
  <summary>Details</summary>
Motivation: 贝叶斯最优实验设计(BOED)在实际应用中受限于期望信息增益(EIG)的高维非凸优化困难，特别是在批量设计场景下尤为严重

Method: 将原始优化问题概率提升到概率测度空间，优化设计测度上的熵正则化期望效用；推导吉布斯分布形式的最优设计律；提出两种可扩展近似：平均场族和i.i.d.乘积族；推导Wasserstein梯度流并设计粒子算法

Result: 方法在数值实验中表现出色，能够探索多模态优化景观并在挑战性示例中获得高效用批量设计

Conclusion: 提出的概率提升方法为批量BOED提供了有效的计算框架，通过熵正则化和概率测度优化解决了传统方法面临的优化困难
...
### [152] [Conformal e-prediction in the presence of confounding](https://arxiv.org/abs/2603.11134)
*Vladimir Vovk,Ruodu Wang*

Main category: math.ST

TL;DR: 将保形e-预测扩展到存在观测混杂变量情况，考虑IID数据和允许观测间依赖两种情形


<details>
  <summary>Details</summary>
Motivation: 现有保形预测方法通常假设数据独立同分布，但在实际应用中，随机变量X与其标签Y之间可能存在观测混杂变量，这会影响预测的可靠性。需要扩展保形e-预测框架来处理这种混杂情况。

Method: 扩展保形e-预测框架，处理X和Y之间的观测混杂变量。考虑两种数据场景：1) 独立同分布数据；2) 允许观测间存在依赖关系的数据。通过调整预测方法以适应混杂变量的存在。

Result: 提出了能够处理观测混杂变量的保形e-预测方法，该方法在存在混杂因素时仍能保持预测的有效性。方法适用于IID数据和允许依赖关系的数据两种场景。

Conclusion: 成功将保形e-预测扩展到存在观测混杂变量的情况，为实际应用中更复杂的数据结构提供了可靠的预测框架，增强了保形预测方法在混杂存在时的适用性。
...
TL;DR: 该论文证明朗之万动力学对分数函数估计中的L^2误差不稳健，即使误差任意小，在多项式时间内也会产生与目标分布TV距离较远的分布，为扩散模型提供了进一步的理论支持。


<details>
  <summary>Details</summary>
Motivation: 研究基于分数的生成建模对分数函数估计误差的稳健性。在实际应用中，从数据中学习分数函数时不可避免地存在估计误差，需要评估不同采样方法对这种误差的容忍度。

Method: 通过理论分析证明朗之万动力学对分数函数L^2误差的敏感性。使用高维简单分布作为案例，展示即使分数函数估计误差任意小，朗之万动力学在多项式时间内也会产生与目标分布TV距离较远的分布。

Result: 朗之万动力学对分数函数估计中的L^2误差不稳健，而扩散模型在相同条件下能够在多项式时间内忠实采样。这一结果为扩散模型相对于朗之万动力学提供了进一步的理论依据。

Conclusion: 朗之万动力学在分数函数存在估计误差时表现不佳，应谨慎使用。扩散模型在相同条件下更具稳健性，这为实际应用中优先选择扩散模型提供了理论支持。
...
TL;DR: 提出了一种基于自由能量的社会多臂赌博机学习算法，使智能体能够通过观察其他智能体的行为（无需知道其奖励）来提升学习性能，即使在存在随机或次优智能体的情况下也能有效识别相关智能体并利用其行为信息。


<details>
  <summary>Details</summary>
Motivation: 当前大多数强化学习算法侧重于个体学习，未能充分利用人类和动物常见的社会学习能力。社会学习能够将个体经验与观察他人行为相结合，为提升学习效果提供了机会。在个性化AI服务中，多个强化学习智能体独立运行，但缺乏有效的社会学习机制。

Method: 提出了一种基于自由能量的社会多臂赌博机学习算法，在策略空间上进行操作。社会智能体通过观察其他智能体的行为（无需知道其奖励）来评估其专业水平，无需依赖任何先知或社会规范。算法将智能体的直接环境经验与其他智能体的估计策略相结合。

Result: 理论证明了算法收敛到最优策略。实证评估表明，在各种场景下，该社会学习方法优于其他替代方法。算法能够战略性地识别相关智能体，即使在存在随机或次优智能体的情况下也能巧妙地利用其行为信息。在包含相关但非专家智能体的情况下，该算法显著提升了个体学习性能，而大多数相关方法在此情况下失败。算法还保持了对数遗憾。

Conclusion: 提出的基于自由能量的社会多臂赌博机学习算法有效地整合了个体经验和社会观察，无需依赖先知信息或社会规范。算法在理论上收敛到最优策略，在实践中优于现有方法，能够识别相关智能体并利用其行为信息，显著提升学习性能，同时保持对数遗憾。
...
TL;DR: FTPL算法在m-set半赌博问题中达到最优遗憾界，兼具对抗性和随机性环境的最佳性能，并通过条件几何重采样将计算复杂度从O(d²)降至O(md(log(d/m)+1))。


<details>
  <summary>Details</summary>
Motivation: FTPL作为对抗性组合半赌博问题中具有前景的高效算法，其最优性一直未被证明，而FTRL的最优性已在多种在线学习任务中得到验证。本文旨在填补这一空白，证明FTPL在m-set半赌博问题中的最优性。

Method: 将带有几何重采样的FTPL分析扩展到m-set半赌博问题，使用特定参数的Fréchet和Pareto分布，并引入条件几何重采样技术进行高效损失估计。

Result: 在对抗性设置下，FTPL实现了最优遗憾界O(√(mdT))；在随机性设置下，实现了对数遗憾，证明了FTPL在m-set半赌博问题中的"两全其美"最优性。计算复杂度从O(d²)降至O(md(log(d/m)+1))。

Conclusion: FTPL在m-set半赌博问题中达到了最优遗憾性能，兼具对抗性和随机性环境的最佳表现，并通过条件几何重采样显著降低了计算复杂度，为实际应用提供了高效解决方案。
...
TL;DR: EnTransformer：将engression（一种随机学习范式）与Transformer结合，用于多变量时间序列概率预测，无需参数假设即可学习条件预测分布


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的概率预测方法大多依赖限制性参数似然或分位数目标，难以捕捉多个相关时间序列的复杂联合预测分布，需要更灵活的非参数方法来可靠量化不确定性

Method: 提出EnTransformer框架，将engression（用于建模条件分布的随机学习范式）与Transformer的表达能力集成，通过向模型表示注入随机噪声并优化基于能量的评分目标，直接学习条件预测分布

Result: 在Electricity、Traffic、Solar、Taxi、KDD-cup和Wikipedia等多个多变量概率预测基准数据集上评估，EnTransformer产生校准良好的概率预测，并持续优于基准模型

Conclusion: EnTransformer通过整合engression和Transformer，能够生成连贯的多变量预测轨迹，同时保持Transformer有效建模长期时间依赖性和跨序列交互的能力，为多变量时间序列概率预测提供了强大框架
...
TL;DR: 研究多轮预条件随机梯度下降(PSGD)中总体风险曲率、噪声几何和预条件对泛化能力的影响，发现不匹配的几何结构会导致次优统计行为，提出新的平均算法稳定性分析框架。


<details>
  <summary>Details</summary>
Motivation: 许多实际优化启发式方法隐式地在总体风险曲率、梯度噪声几何和预条件之间进行权衡，但当这些几何结构不匹配时，激进的选择会放大不稳定性，导致次优的统计行为。需要理论分析这种权衡对泛化的影响。

Method: 使用平均算法稳定性分析多轮PSGD，处理数据重用引起的相关性，推导出依赖于有效维度的超额风险上界，并补充匹配的实例相关下界。

Result: 建立了PSGD泛化能力与有效维度之间的联系，表明不恰当的预条件选择会在优化和泛化两方面产生次优的有效维度依赖，并提供了匹配的上界和下界。

Conclusion: 多轮PSGD的泛化性能取决于总体风险曲率、梯度噪声几何和预条件之间的复杂交互，不匹配的几何结构会导致统计性能下降，需要仔细平衡这些因素。
...
### [158] [A Quantitative Characterization of Forgetting in Post-Training](https://arxiv.org/abs/2603.12163)
*Krishnakumar Balasubramanian,Shiva Prasad Kasiviswanathan*

Main category: cs.LG

TL;DR: 该论文理论分析了生成模型持续后训练中的遗忘现象，提出了两种遗忘形式（质量遗忘和旧分量漂移），并证明了不同KL目标（前向KL和反向KL）对遗忘的不同影响，还分析了重放机制和三种近期方法在防止遗忘方面的表现。


<details>
  <summary>Details</summary>
Motivation: 生成模型的持续后训练被广泛使用，但对于遗忘何时发生以及为何发生的原理性理解仍然有限。需要建立理论框架来量化遗忘现象，并理解不同训练目标、几何行为重叠、采样机制以及训练过程中过去行为可见性之间的相互作用。

Method: 采用Chen等人（2025）提出的双模式混合抽象（代表旧任务和新任务），在等协方差高斯模式的假设下进行理论分析。形式化了两种遗忘形式：质量遗忘（旧混合权重坍缩为零）和旧分量漂移（已正确的旧分量在训练中发生偏移）。分析了前向KL和反向KL目标对遗忘的不同影响，量化了重放机制与这些目标的相互作用，并通过相同理论框架分析了三种近期提出的近策略后训练方法（SDFT、TTT-Discover和OAPL）。

Result: 理论证明：前向KL目标在新分布数据上训练会驱动旧权重归零，而反向KL目标收敛到真实目标（避免质量遗忘），旧均值仅通过由Bhattacharyya系数控制的重叠门控错误分配概率发生扰动，产生随模式分离呈指数衰减的漂移，并具有局部良条件的几何结构和指数收敛性。重放机制分析表明：对于前向KL，重放必须修改训练分布以改变总体最优解；对于反向KL，重放保持总体目标不变但通过有界重要性加权防止有限批次中的旧模式饥饿。三种近期方法在特定条件下都能保留旧质量并表现出重叠控制的漂移。

Conclusion: 遗忘可以基于发散方向、几何行为重叠、采样机制以及训练过程中过去行为可见性之间的相互作用进行精确量化。反向KL目标在避免质量遗忘方面优于前向KL，而旧分量漂移受模式重叠控制且随分离距离指数衰减。该理论框架为理解和设计抗遗忘的持续学习算法提供了基础。
...
### [159] [Comparison of Outlier Detection Algorithms on String Data](https://arxiv.org/abs/2603.11049)
*Philip Maus*

Main category: cs.LG

TL;DR: 该论文比较了两种字符串异常检测算法：基于编辑距离的局部异常因子算法变体和基于分层左正则表达式学习器的新算法，实验证明两者都能有效检测字符串异常，但各有适用场景。
...
Result: 实验表明两种算法都能在概念上检测字符串数据中的异常。正则表达式算法在期望值具有明显结构且与异常结构差异足够大时表现优异；局部异常因子算法在异常与期望数据之间的编辑距离与期望数据之间的编辑距离差异足够明显时效果最佳。

Conclusion: 两种字符串异常检测算法各有优势：正则表达式算法适用于结构化明显的字符串数据，而局部异常因子算法适用于编辑距离差异明显的场景。这为字符串异常检测提供了两种互补的方法。
...
<details>
  <summary>Details</summary>
Motivation: 数据流处理中的主要挑战是概念漂移，即数据分布随时间变化。现有概念表示方法依赖少量元信息特征，往往无法有效区分不同概念，导致系统对概念漂移的脆弱性。需要更强大的概念表示方法来准确检测和适应概念漂移。

Method: 提出FiCSUM框架：1）构建概念指纹，即包含多种不同元信息特征的向量，能够唯一标识更多概念；2）采用动态权重策略，学习哪些元信息特征在特定数据集中最能描述概念漂移；3）支持监督和无监督行为的概念表示；4）通过测试概念指纹与观察窗口的相似性来检测概念漂移。

Result: 在11个真实世界和合成数据集上，FiCSUM在准确性和建模底层概念漂移方面均优于最先进的方法。概念指纹能够更准确地识别新概念或重复出现的旧概念。

Conclusion: FiCSUM通过丰富多样的元信息特征和动态权重策略，显著提高了概念漂移检测能力，为数据流处理提供了更可靠的概念表示和漂移检测框架。
...
TL;DR: 研究发现稀疏混合专家模型的路由机制具有任务条件化结构，通过路由签名分析显示相同任务类别的提示会引发高度相似的路由模式，而不同任务类别则显著不同。


<details>
  <summary>Details</summary>
Motivation: 稀疏混合专家架构通过条件计算实现了大语言模型的高效扩展，但其负责专家选择的路由机制仍未被充分理解。本研究旨在探索MoE路由是否表现出任务条件化结构。

Method: 引入路由签名作为跨层专家激活模式的向量表示，使用OLMoE-1B-7B-0125-Instruct作为实证测试平台，分析不同任务类别提示的路由相似性，建立逻辑回归分类器进行任务分类，并引入置换和负载均衡基线以确保统计有效性。

Result: 相同任务类别内的路由相似度（0.8435±0.0879）显著高于跨类别相似度（0.6225±0.1687），Cohen's d=1.44。仅基于路由签名的逻辑回归分类器在四向任务分类中达到92.5%±6.1%的交叉验证准确率。深层中的任务结构更加明显。
...
<details>
  <summary>Details</summary>
Motivation: 传统决策树学习面临组合复杂性和离散非可微性的挑战，现有方法如CART依赖贪婪搜索，只能进行局部最优决策，导致次优树结构，且无法与现代机器学习方法无缝集成。

Method: 使用带有直通算子的反向传播在密集决策树表示上进行梯度下降训练，实现所有树参数的联合优化，使决策树能够无缝集成到依赖梯度下降的现代机器学习框架中。

Result: 在多个领域取得最先进结果，包括小表格数据集的解释性决策树、复杂表格数据的高级模型、多模态学习以及无信息损失的解释性强化学习。
...
### [163] [Higher-Order Modular Attention: Fusing Pairwise and Triadic Interactions for Protein Sequences](https://arxiv.org/abs/2603.11133)
...
Result: 在TAPE基准测试的二级结构、荧光和稳定性三个任务上，HOMA相比标准自注意力及高效变体（块注意力、Linformer）均取得一致改进
...
### [164] [Scaling Reasoning Efficiently via Relaxed On-Policy Distillation](https://arxiv.org/abs/2603.11137)
*Jongwoo Ko,Sara Abdali,Young Jin Kim,Tianyi Chen,Pashmina Cameron*

Main category: cs.LG

TL;DR: REOPOLD是一种稳定的在线蒸馏框架，通过将教师-学生对数似然比作为token奖励，采用混合奖励裁剪、基于熵的动态采样和探索到精炼训练策略，显著提升训练样本效率和推理性能。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏对于将推理能力转移到容量受限模型至关重要，但存在不稳定性和负迁移问题。研究发现在线蒸馏可解释为一种策略优化形式，其中教师-学生对数似然比充当token奖励，这为改进蒸馏方法提供了理论基础。

Method: 提出REOPOLD框架：1) 混合奖励裁剪：适度选择性地利用教师奖励；2) 基于熵的token级动态采样；3) 统一的探索到精炼训练策略。这些方法放松了标准在线蒸馏的严格模仿约束，稳定了优化过程。

Result: 在数学、视觉和工具使用推理任务上，REOPOLD在训练时具有6.7~12倍的样本效率优势，推理时表现出更好的扩展性。7B学生模型在视觉推理上能匹配32B教师性能，同时获得约3.32倍的推理加速。

Conclusion: REOPOLD通过将在线蒸馏重新解释为策略优化问题，并引入放松的模仿约束机制，有效解决了传统在线蒸馏的不稳定性和负迁移问题，在多个推理任务上实现了显著的性能提升和效率改进。
...
### [165] [H2LooP Spark Preview: Continual Pretraining of Large Language Models for Low-Level Embedded Systems Code](https://arxiv.org/abs/2603.11139)
*Amit Singh,Vedant Nipane,Pulkit Agrawal,Jatin Kishnani*

Main category: cs.LG

TL;DR: 该论文提出了H2LooP Spark Preview，一个针对嵌入式系统编程领域的持续预训练流程，通过使用BF16 LoRA技术将OLMo-3-7B模型适配到该专业领域，显著提升了模型在嵌入式系统代码生成方面的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用编程语言上表现出强大的代码生成能力，但在嵌入式系统等专业领域表现有限。这些领域涉及硬件寄存器操作、厂商特定SDK、实时操作系统API和硬件抽象层等，这些内容在标准预训练语料中代表性不足。

Method: 采用持续预训练流程，使用BF16 LoRA（秩稳定缩放）在8个NVIDIA H100 GPU上适配OLMo-3-7B模型。训练语料基于仓库-数据表对构建，覆盖100B原始嵌入式系统数据，使用SpecMap提出的分层数据表到代码映射方法处理，最终得到包含23.5B tokens的13个嵌入式领域数据集。

Result: 高秩LoRA（r=512）持续预训练带来显著提升：领域内困惑度降低70.4%，保留仓库困惑度降低66.1%。在13个嵌入式领域的生成代码补全基准测试中，7B模型在8个类别上的token准确率超过了Claude Opus 4.6和Qwen3-Coder-30B。

Conclusion: 针对性的持续预训练使较小的开源模型能够在专业技术任务上与前沿系统竞争。研究发布了生产训练检查点作为开源成果，证明了专业领域适配的有效性。
...
### [166] [Procedural Fairness via Group Counterfactual Explanation](https://arxiv.org/abs/2603.11140)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习公平性研究主要关注结果导向的公平标准（如Equalized Odds），而对程序导向的公平性关注较少。忽视程序公平性可能导致模型为不同保护群体生成不同的解释，从而损害信任。

Method: 引入Group Counterfactual Integrated Gradients (GCIG)，这是一个处理中正则化框架，强制在真实标签条件下跨群体的解释不变性。GCIG为每个输入计算相对于多个群体条件基线的解释，并在训练期间惩罚这些归因的跨群体变异。

Result: 与六种最先进方法相比，GCIG显著减少了跨群体解释差异，同时保持了有竞争力的预测性能和准确性-公平性权衡。
...
### [167] [Attention Gathers, MLPs Compose: A Causal Analysis of an Action-Outcome Circuit in VideoViT](https://arxiv.org/abs/2603.11142)
*Sai V R Chereddy*
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索视频分类模型如何表示不影响最终结果的细微、隐藏语义信息，这是可信AI模型的关键挑战。通过可解释AI方法理解模型内部表示复杂结果的计算机制，揭示模型可能发展的超出显式任务的"隐藏知识"。

Method: 采用机制可解释性技术，在预训练的视频视觉Transformer中进行逆向工程。主要使用激活修补（activation patching）配合消融实验进行因果分析，识别模型中表示动作结果的内部电路。

Result: 发现"成功vs失败"信号通过独特的放大级联计算：从第0层开始有低级差异，但从第5到11层逐步放大抽象语义表示。注意力头作为"证据收集器"提供低级信息，MLP块作为鲁棒的"概念组合器"生成成功信号。这种分布式冗余电路解释了模型对简单消融的韧性。

Conclusion: 即使在仅训练简单分类的模型中，也存在表示复杂结果的复杂电路，表明模型可能发展超出显式任务的"隐藏知识"。这强调了机制监督对于构建真正可解释和可信AI系统的重要性。
...
### [168] [Systematic Scaling Analysis of Jailbreak Attacks in Large Language Models](https://arxiv.org/abs/2603.11149)
*Xiangwen Wang,Ananth Balashankar,Varun Chandrasekaran*

Main category: cs.LG

TL;DR: 本文提出了一个用于评估越狱攻击的扩展定律框架，通过将攻击视为计算受限的优化过程，在统一的FLOPs轴上衡量进展，发现基于提示的方法比基于优化的方法更高效。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型仍然容易受到越狱攻击，但缺乏对攻击成功率如何随攻击者努力程度、方法、模型家族和危害类型系统性扩展的理解。需要建立统一的评估框架来量化攻击效率。

Method: 提出越狱攻击的扩展定律框架，将每种攻击视为计算受限的优化过程，在共享的FLOPs轴上衡量进展。系统评估四种代表性越狱范式：基于优化的攻击、自我精炼提示、基于采样的选择和遗传优化，涵盖多个模型家族和规模。通过拟合FLOPs-成功轨迹的饱和指数函数来研究扩展定律，并从拟合曲线中推导可比较的效率摘要。

Result: 基于提示的范式通常比基于优化的方法更计算高效。通过将基于提示的更新转化为优化视角，发现在相同状态下，基于提示的攻击在提示空间中更有效地进行优化。攻击占据不同的成功-隐蔽性操作点，基于提示的方法占据高成功、高隐蔽区域。漏洞强烈依赖于目标：涉及错误信息的危害通常比其他非错误信息危害更容易引发。

Conclusion: 本文建立了越狱攻击的扩展定律框架，为系统评估攻击效率提供了统一方法。发现基于提示的攻击在计算效率和隐蔽性方面具有优势，同时揭示了危害类型对漏洞的显著影响，为理解和防御越狱攻击提供了重要见解。
...
### [169] [Huntington Disease Automatic Speech Recognition with Biomarker Supervision](https://arxiv.org/abs/2603.11168)
...
<details>
  <summary>Details</summary>
Motivation: 病理语音的自动语音识别研究不足，特别是亨廷顿病（HD）语音具有不规则时序、不稳定发声和发音失真等特点，对现有模型构成挑战。需要系统研究HD-ASR，使用高质量的临床语音数据，分析不同ASR架构在HD语音上的表现。

Method: 使用高保真临床HD语音语料库进行端到端ASR训练；在统一评估框架下比较多种ASR架构家族；分析词错误率（WER）及替代、删除、插入错误模式；提出HD特异性适应方法，包括使用生物标志物辅助监督；研究错误行为随疾病严重程度的变化模式。

Result: HD语音导致架构特定的错误模式，Parakeet-TDT优于编码器-解码器和CTC基线；HD特异性适应将WER从6.99%降至4.95%；生物标志物辅助监督方法能重塑错误行为，而非均匀改善WER；错误模式随疾病严重程度呈现特定变化。

Conclusion: 该研究为HD病理语音ASR提供了系统性分析框架，展示了不同ASR架构在HD语音上的性能差异，提出的适应方法能有效降低WER并重塑错误模式，为病理语音识别研究提供了重要参考。所有代码和模型已开源。
...
### [170] [Bayesian Optimization of Partially Known Systems using Hybrid Models](https://arxiv.org/abs/2603.11199)
*Eike Cramer,Luis Kutschat,Oliver Stollenwerk,Joel A. Paulson,Alexander Mitsos*

Main category: cs.LG

TL;DR: 提出了一种结合贝叶斯优化与部分已知物理机理模型的混合建模方法，用于加速高维非线性系统的黑箱优化


<details>
  <summary>Details</summary>
Motivation: 标准贝叶斯优化在处理高维非线性系统时可能需要大量实验才能收敛到最优解，这在实际应用中成本过高。需要一种能利用部分已知物理机理来加速优化过程的方法。

Method: 提出混合模型贝叶斯优化框架：将已知物理方程作为约束，对缺失方程的部分使用高斯过程进行概率建模。最终形成带有高斯过程约束的非线性随机规划问题，采用样本平均近似进行离散化求解。

Result: 在单级蒸馏的仿真优化中，基于质量守恒定律的混合贝叶斯优化模型显著优于标准贝叶斯优化。混合模型在某些初始样本下仅需1次迭代即可收敛，而标准贝叶斯优化在25次迭代内均未收敛。

Conclusion: 提出的混合贝叶斯优化方案为部分已知系统提供了一种有前景的优化方法，有效结合了机理建模和数据驱动优化的优势，能显著减少优化所需的实验次数。
...
### [171] [Representation Finetuning for Continual Learning](https://arxiv.org/abs/2603.11201)
*Haihua Luo,Xuming Ran,Tommi Kärkkäinen,Huiyan Xue,Zhonghua Chen,Qi Xu,Fengyu Cong*

Main category: cs.LG

TL;DR: CoRe框架将微调从权重空间转移到表示空间，通过低秩线性子空间进行任务特定干预，在保持参数效率的同时显著提升持续学习性能。
...
Method: CoRe框架在隐藏表示的线性子空间中进行任务特定干预，采用具有明确目标的学习过程，通过约束更新到低秩子空间实现参数高效性。
...
### [172] [Reference-Guided Machine Unlearning](https://arxiv.org/abs/2603.11210)
*Jonas Mirlach,Sonia Laguna,Julia E. Vogt*

Main category: cs.LG

TL;DR: ReGUn提出基于分布不可区分性的参考引导遗忘框架，利用独立数据集提供类别条件参考，优于传统基于性能退化的近似遗忘方法


<details>
  <summary>Details</summary>
Motivation: 现有近似遗忘方法依赖损失最大化或随机标注等性能退化启发式方法，这些信号可能条件不佳，导致优化不稳定并损害模型泛化能力。作者认为遗忘应优先考虑分布不可区分性，使模型在遗忘数据上的行为与在真正未见数据上的行为对齐。

Method: 提出参考引导遗忘（ReGUn）框架，利用不相交的保留数据集提供原则性的类别条件参考进行蒸馏。该方法基于分布不可区分性，使模型在遗忘数据上的行为与在参考数据集上的行为对齐。

Result: 在各种模型架构、自然图像数据集和不同遗忘比例下，ReGUn始终优于标准近似基线方法，实现了更优的遗忘-效用权衡。

Conclusion: 基于分布不可区分性的参考引导遗忘方法比依赖性能退化启发式的方法更有效，通过利用独立数据集提供类别条件参考，能够实现更稳定和有效的机器学习模型遗忘。
...
### [173] [Monitoring and Prediction of Mood in Elderly People during Daily Life Activities](https://arxiv.org/abs/2603.11230)
*Daniel Bautista-Salinas,Joaquín Roca González,Inmaculada Méndez,Oscar Martinez Mozos*

Main category: cs.LG

TL;DR: 开发智能可穿戴系统监测和预测老年人日常生活中的情绪状态，使用腕带采集生理数据结合移动应用进行生态瞬时评估，机器学习分类器实现情绪自动预测


<details>
  <summary>Details</summary>
Motivation: 开发针对老年人的情绪监测系统，通过可穿戴设备在日常生活场景中实时监测和预测情绪状态，为老年人心理健康提供技术支持

Method: 系统由腕带式智能设备（记录多种生理活动）和移动应用（生态瞬时评估EMA）组成，使用机器学习训练分类器，仅基于腕带数据自动预测不同情绪状态

Result: 在情绪识别准确性方面显示出有希望的结果，在特定情绪（幸福感和活跃度）检测方面达到与现有技术水平相当的性能

Conclusion: 提出的智能可穿戴系统能够有效监测和预测老年人情绪状态，为日常生活中的情绪识别提供了可行的技术方案
...
### [174] [Beyond the Class Subspace: Teacher-Guided Training for Reliable Out-of-Distribution Detection in Single-Domain Models](https://arxiv.org/abs/2603.11269)
*Hong Yang,Devroop Kar,Qi Yu,Travis Desell,Alex Ororbia*

Main category: cs.LG

TL;DR: 论文提出Domain-Sensitivity Collapse（DSC）现象：单域训练导致特征压缩到低秩类别子空间，抑制了域偏移信号，从而损害OOD检测性能。作者提出Teacher-Guided Training（TGT）方法，利用多域教师模型蒸馏被抑制的残差结构，显著提升距离基OOD检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 当前OOD检测方法在多域基准上表现良好，但实际系统通常在单域数据上训练。作者发现这种单域训练会导致Domain-Sensitivity Collapse（DSC）几何失效模式：监督训练将特征压缩到低秩类别子空间，抑制了携带域偏移信号的方向，使得基于距离和logit的OOD评分对域偏移失去敏感性。

Method: 提出Teacher-Guided Training（TGT）方法：1）使用冻结的多域教师模型（DINOv2）提取被类别信息抑制的残差结构；2）在训练过程中将这些结构蒸馏到学生模型中；3）训练完成后丢弃教师模型和辅助头，不增加推理开销。该方法通过保留域敏感特征来缓解DSC问题。

Result: 在八个单域基准测试中，TGT显著提升了距离基OOD检测器的性能：MDS改进11.61个百分点，ViM改进10.78个百分点，kNN改进12.87个百分点（ResNet-50平均值）。同时保持或略微提升了域内OOD检测和分类准确率。

Conclusion: 单域训练导致的Domain-Sensitivity Collapse是OOD检测的重要失效模式。TGT方法通过从多域教师模型蒸馏残差结构，有效缓解了DSC问题，显著提升了距离基OOD检测器的性能，且不增加推理开销，为实际单域训练系统提供了有效的OOD检测解决方案。
...
### [175] [Client-Conditional Federated Learning via Local Training Data Statistics](https://arxiv.org/abs/2603.11307)
*Rickard Brännvall*

Main category: cs.LG

TL;DR: 提出一种联邦学习方法，通过本地PCA统计量条件化单个全局模型来处理数据异构性，无需额外通信，在多种异构场景下性能优异
...
Result: 在97种配置（4种异构类型×4个数据集×7个基线方法）的评估中，该方法在所有设置下匹配知道真实聚类分配的Oracle基线，在组合异构性上超越Oracle基线1-6%，且在数据稀疏情况下表现稳健

Conclusion: 通过本地PCA统计量条件化全局模型的方法能有效处理联邦学习中的数据异构性，在多种异构场景下表现优异，特别是对数据稀疏情况具有鲁棒性
...
### [176] [Heavy-Tailed Principle Component Analysis](https://arxiv.org/abs/2603.11308)
*Mario Sayde,Christopher Khater,Jihad Fahs,Ibrahim Abou-Faycal*

Main category: cs.LG

TL;DR: 该论文提出了一种针对重尾数据的稳健主成分分析方法，通过超统计依赖模型和对数损失函数，证明了重尾观测的主成分与基础高斯生成器协方差矩阵的标准PCA结果一致。
...
Method: 采用超统计依赖模型X = A^{1/2}G，其中A为正随机标量，G为高斯向量，捕捉多元t分布和亚高斯α稳定律等重尾分布。在对数损失函数下进行PCA，提出从重尾数据直接估计基础高斯生成器协方差矩阵的稳健估计器。

Result: 理论证明在对数损失下，重尾观测的主成分与基础高斯生成器协方差矩阵的标准PCA结果一致。实验表明该方法能可靠恢复主方向，在重尾和脉冲噪声下显著优于经典PCA，在高斯噪声下保持竞争力。

Conclusion: 通过超统计模型和对数损失函数，为无限方差重尾数据提供了一种统一的稳健PCA框架，理论保证和实验验证了其有效性。
...
TL;DR: HAPO通过选择性锚定教师演示解决GRPO在稀疏奖励环境中的困境，实现渐近一致性
...
Method: 提出Hindsight-Anchored Policy Optimization (HAPO)，采用Synthetic Success Injection (SSI)算子，通过Thompson采样启发的门控机制在失败时选择性锚定教师演示，形成自主、自适应的课程学习

Result: 理论上证明HAPO实现渐近一致性：随着策略改进自然衰减教师信号，恢复无偏的on-policy梯度，确保离策略指导作为临时支架而非永久上限

Conclusion: HAPO解决了GRPO在稀疏奖励环境中的核心困境，通过选择性锚定机制使模型能够超越静态教师强制的限制
...
### [178] [Jailbreak Scaling Laws for Large Language Models: Polynomial-Exponential Crossover](https://arxiv.org/abs/2603.11331)
*Indranil Halder,Annesya Banerjee,Cengiz Pehlevan*

Main category: cs.LG

TL;DR: 该论文提出一个理论框架，将对抗性提示注入攻击的成功率缩放行为解释为自旋玻璃系统中的相变现象，短提示导致成功率幂律增长，长提示导致指数增长。


<details>
  <summary>Details</summary>
Motivation: 对抗性攻击能够可靠地引导安全对齐的大型语言模型产生不安全行为。实证研究发现，对抗性提示注入攻击可以将攻击成功率从无注入时的缓慢多项式增长放大为指数增长。需要理论解释这种缩放行为的根本机制。

Method: 提出一个基于自旋玻璃系统的理论生成模型，将语言生成过程建模为在复制对称破缺机制下运行的Gibbs测度采样。将不安全行为对应为低能、尺寸偏置的聚类中心。分析提示注入攻击：短提示对应弱磁场指向不安全聚类中心，长提示对应强磁场。通过理论推导和大型语言模型上的实证验证相结合。

Result: 理论分析表明：短注入提示导致攻击成功率随推理样本数呈幂律缩放，长注入提示导致指数缩放。这种相变是由于强磁场下自旋链中出现有序相，表明注入的越狱提示增强了语言模型中的对抗性有序性。实证结果与理论预测一致。

Conclusion: 对抗性提示注入攻击的成功率缩放行为可以通过自旋玻璃理论框架解释为相变现象。短提示和长提示分别对应不同的缩放机制，这种转变源于强磁场诱导的有序相出现。该理论为理解语言模型对抗攻击的动力学提供了新的视角。
...
### [179] [Teleodynamic Learning a new Paradigm For Interpretable AI](https://arxiv.org/abs/2603.11355)
*Enrique ter Horst,Juan Diego Zambrano*

Main category: cs.LG

TL;DR: 提出Teleodynamic Learning新范式，将学习视为约束下功能组织的涌现与稳定，而非固定目标的最小化。基于Spencer-Brown的《形式法则》、信息几何和热带优化，实现了Distinction Engine (DE11)模型，在标准数据集上取得优异性能并产生可解释逻辑规则。


<details>
  <summary>Details</summary>
Motivation: 受生命系统启发，传统机器学习将学习视为固定目标的最小化，无法自然捕捉自稳定、阶段性学习动态等现象。需要新范式将智能视为表示能力、参数适应和内部资源可持续变化的三者耦合演化。

Method: 将学习形式化为具有两个相互作用时间尺度的约束动力学过程：内部动力学用于连续参数适应，外部动力学用于离散结构变化，通过内源性资源变量连接。基于Spencer-Brown的《形式法则》、信息几何和热带优化，实现了Distinction Engine (DE11)具体模型。
...
### [180] [Multilingual Financial Fraud Detection Using Machine Learning and Transformer Models: A Bangla-English Study](https://arxiv.org/abs/2603.11358)
...
Method: 研究使用包含合法和欺诈性金融消息的数据集，在孟加拉语-英语多语言环境下进行金融欺诈检测。评估了传统机器学习模型（逻辑回归、线性SVM和集成分类器）使用TF-IDF特征，以及基于Transformer的架构。采用5折分层交叉验证进行实验。

Result: 线性SVM获得最佳性能：91.59%准确率和91.30% F1分数，优于Transformer模型（89.49%准确率，88.88% F1）约2个百分点。Transformer模型具有更高的欺诈召回率（94.19%），但假阳性率较高。探索性分析显示：欺诈消息通常更长、包含紧急诱导词汇、频繁包含URL（32%）和电话号码（97%），而合法消息则包含交易确认和特定货币引用。

Conclusion: 精心设计的特征与传统机器学习在多语言欺诈检测中仍具有竞争力，同时凸显了语言多样性、代码混合和低资源语言约束带来的挑战。
...
### [181] [KEPo: Knowledge Evolution Poison on Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2603.11501)
*Qizhi Chen,Chao Qi,Yihong Huang,Muquan Li,Rongzheng Wang,Dongyang Zhang,Ke Qin,Shuang Liang*

Main category: cs.LG

TL;DR: KEPo是一种针对GraphRAG的新型知识图投毒攻击方法，通过伪造事件背景和知识演化路径，在单目标和多目标攻击中实现最先进的攻击成功率


<details>
  <summary>Details</summary>
Motivation: GraphRAG通过从外部数据库构建知识图来增强LLM的时效性和准确性，但这种对外部数据的依赖引入了新的攻击面。现有攻击方法主要针对传统RAG系统，对GraphRAG无效，因为其知识图抽象能够重组注入文本。需要揭示GraphRAG中潜在的安全漏洞

Method: KEPo（知识演化投毒）方法：1）为目标查询生成包含投毒知识的有毒事件；2）伪造事件背景和从原始事实到有毒事件的知识演化路径；3）投毒知识图，误导LLM将投毒知识视为最终结果；4）在多目标攻击中，连接多个攻击语料，使投毒知识相互增强并扩大投毒社区规模

Result: 在多个数据集上的实验结果表明，KEPo在单目标和多目标攻击中都实现了最先进的攻击成功率，显著优于先前方法

Conclusion: GraphRAG虽然通过知识图抽象增强了对抗传统RAG攻击的鲁棒性，但仍存在潜在安全漏洞。KEPo方法通过伪造知识演化路径有效攻击GraphRAG，揭示了此类系统的安全风险，需要开发相应的防御机制
...
### [182] [Relaxed Efficient Acquisition of Context and Temporal Features](https://arxiv.org/abs/2603.11370)
...
<details>
  <summary>Details</summary>
Motivation: 生物医学应用中测量获取存在成本、时间和风险限制，纵向主动特征采集面临时序耦合决策挑战，且实际临床工作流包含初始上下文收集阶段，但现有方法未联合优化上下文选择与时序自适应采集

Method: 提出REACT端到端可微分框架，使用Gumbel-Sigmoid松弛与直通估计实现离散采集掩码的梯度优化，同时优化(i)初始上下文描述符选择和(ii)成本约束下的纵向测量自适应特征-时间采集计划

Result: 在真实世界纵向健康和数据集上，REACT相比现有纵向采集基线方法，以更低的采集成本实现了更好的预测性能

Conclusion: 在统一优化框架中建模初始上下文收集和时序耦合采集具有显著优势，REACT框架能有效平衡预测性能与采集成本约束
...
### [183] [Exponential-Family Membership Inference: From LiRA and RMIA to BaVarIA](https://arxiv.org/abs/2603.11799)
*Rickard Brännvall*
...
<details>
  <summary>Details</summary>
Motivation: 当前存在多种成员推理攻击方法（LiRA、RMIA、BASE），它们看似使用不同的评分策略，但实际关系不明确，使得实践者难以选择。需要建立一个统一框架来理解这些方法之间的关系，并解决在小影子模型预算下性能下降的问题。

Method: 1. 提出指数族对数似然比统一框架，将LiRA、RMIA和BASE视为该框架下不同分布假设和参数估计数量的实例；2. 识别方差估计是小影子模型预算下的关键瓶颈；3. 提出BaVarIA攻击方法，使用共轭正态-逆伽马先验替代阈值参数切换，产生Student-t预测（BaVarIA-t）或方差稳定的高斯分布（BaVarIA-n）。

Result: 在12个数据集和7种影子模型预算下，BaVarIA匹配或优于LiRA和RMIA，在低影子模型预算和离线场景中提升最显著。统一框架揭示了RMIA和LiRA作为复杂度谱系的两个端点，并通过BASE1-4层次连接。

Conclusion: 所有主要成员推理攻击方法都可以统一到同一指数族框架下，方差估计是小预算下的关键瓶颈。BaVarIA通过贝叶斯方差推断提供了稳定性能，无需额外超参数调优，在实用场景中表现优异。
...
TL;DR: 提出T-CQL框架，结合Transformer编码器、保守自适应正则化和一致性正则化，用于机械通气的个性化自动化设置，通过临床知情奖励函数和数字孪生评估提升安全性和有效性。


<details>
  <summary>Details</summary>
Motivation: 机械通气是ICU急性呼吸衰竭患者的救命干预措施，但不恰当的呼吸机设置可能导致呼吸机诱导的肺损伤。临床医生的工作负荷与患者预后直接相关，因此需要个性化和自动化的机械通气来改善患者预后。传统监督学习和离线强化学习方法忽视时间依赖性、过度依赖基于死亡率的奖励，未能充分捕捉早期生理恶化和VILI风险。

Method: 提出Transformer-based Conservative Q-Learning (T-CQL)框架：1) 集成Transformer编码器对患者动态进行有效时间建模；2) 基于不确定性量化的保守自适应正则化确保安全性；3) 一致性正则化实现稳健决策。构建临床知情奖励函数，包含VILI指标和患者疾病严重程度评分。使用急性呼吸衰竭患者的交互式数字孪生进行在线"床边"评估，克服传统Fitted Q-Evaluation在静态离线数据上评估的局限性。

Result: T-CQL在性能上持续优于现有最先进的离线强化学习方法，提供更安全、更有效的通气调整。该框架展示了Transformer模型与保守强化学习策略结合作为重症监护决策支持工具的潜力。

Conclusion: T-CQL框架通过整合Transformer时间建模、保守正则化和临床知情奖励，解决了机械通气个性化自动化中的关键挑战。结合数字孪生的在线评估方法克服了传统离线评估的局限性，为重症监护中的决策支持提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统植硅体分析方法依赖人工显微镜观察，劳动密集、耗时且主观性强，严重限制了研究规模和可重复性。需要开发自动化、高通量的解决方案来克服这些瓶颈。

Method: 开发了Sorometry端到端AI流程：1) 处理Z-stack光学显微镜扫描生成同步的2D正射影像和3D点云；2) 构建多模态融合模型，结合ConvNeXt处理2D图像和PointNet++处理3D点云；3) 提供GUI界面供专家标注和审查；4) 集成贝叶斯有限混合模型进行组合层面的植物来源预测。

Result: 在玻利维亚亚马逊的参考样本和考古样本测试中：1) 融合模型在24种诊断形态类型上达到77.9%的全局分类准确率；2) 分割质量达84.5%；3) 3D数据对区分复杂形态类型（如禾本科硅质短细胞植硅体）至关重要；4) 贝叶斯模型成功识别复杂混合样本中的玉米和棕榈等特定植物。

Conclusion: Sorometry将植硅体研究转变为"组学"规模学科，极大扩展了分析能力，标准化了专家判断，实现了考古和古生态组合的可重复、群体水平表征，为古植被重建和人类活动研究提供了革命性工具。
...
TL;DR: 论文证明softmax自注意力模型在计算触发条件行为时必然产生注意力汇（attention sink），即概率质量集中在固定的内容无关位置，而ReLU注意力无需归一化约束可解决相同任务而不产生汇。


<details>
  <summary>Details</summary>
Motivation: 研究注意力汇现象的理论基础，理解为什么Transformer模型中会出现注意力概率集中在固定位置的现象，以及这种现象与模型归一化约束的关系。

Method: 通过理论证明和实验验证：1）证明softmax自注意力模型在计算触发条件行为时必然产生注意力汇；2）设计具体任务（触发标记出现时返回前面所有标记表示的平均值，否则输出零）；3）证明ReLU注意力无需归一化可解决相同任务而不产生汇；4）实验验证理论预测。

Result: 理论证明：softmax模型必然产生注意力汇，而ReLU注意力可避免汇现象。实验验证：softmax模型产生强注意力汇，ReLU注意力在单头和多头变体中均消除汇现象，且结果超出理论分析范围。

Conclusion: 注意力汇现象的根本驱动因素是归一化约束，而非注意力机制本身。softmax的归一化要求迫使注意力在需要忽略输入时坍缩到稳定锚点，而ReLU等非归一化注意力可避免此问题。
...
### [187] [Sharpness-Aware Minimization for Generalized Embedding Learning in Federated Recommendation](https://arxiv.org/abs/2603.11503)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦推荐方法忽视了广义物品嵌入的稳定学习问题。在跨设备场景下，本地数据分布存在显著异质性和稀疏性，使得学习广义嵌入既必要又困难，这影响了知识共享和推荐效果

Method: 提出FedRecGEL框架：1) 从物品中心视角重新定义联邦推荐问题，将其构建为多任务学习问题；2) 基于理论分析，采用锐度感知最小化解决泛化问题，稳定训练过程

Result: 在四个数据集上的大量实验表明，FedRecGEL能显著提升联邦推荐性能

Conclusion: FedRecGEL通过解决广义物品嵌入的稳定学习问题，有效提升了联邦推荐系统的性能，为跨设备场景下的推荐提供了新解决方案
...
### [188] [Implicit Statistical Inference in Transformers: Approximating Likelihood-Ratio Tests In-Context](https://arxiv.org/abs/2603.10573)
*Faris Chaudhry,Siddhant Gadkari*

Main category: cs.LG

TL;DR: Transformer通过上下文学习近似贝叶斯最优统计量，而非固定核平滑启发式方法，表现出任务自适应的统计估计器构建


<details>
  <summary>Details</summary>
Motivation: 理解Transformer上下文学习的底层算法机制，特别是在统计决策理论框架下探究其如何从上下文构建任务自适应的统计估计器

Method: 采用统计决策理论视角，研究二元假设检验问题，训练Transformer处理线性均值偏移和非线性方差估计任务，通过logit lens和电路对齐进行机制分析

Result: Transformer能够从上下文近似贝叶斯最优充分统计量（达到单调变换），在非线性区域匹配理想oracle估计器性能；模型不依赖固定核平滑启发式，而是自适应决策线性可分点

Conclusion: 上下文学习源于任务自适应统计估计器的构建，而非简单的相似性匹配；线性任务表现为投票式集成模式，非线性任务采用更深层的序列计算
...
### [189] [LongFlow: Efficient KV Cache Compression for Reasoning M](https://arxiv.org/abs/2603.11504)
...
<details>
  <summary>Details</summary>
Motivation: 现有推理模型（如OpenAI-o1和DeepSeek-R1）在复杂任务上表现出色，但生成长输出序列导致部署成本显著增加。长输出需要大KV缓存，造成高内存消耗和注意力计算中的严重带宽压力。现有KV缓存优化方法主要针对长输入短输出场景，对推理模型的长输出设置无效，且重要性评估计算成本高，在长生成过程中持续重新评估变得不可行。

Method: 提出LongFlow KV缓存压缩方法：1）使用仅基于当前查询的注意力计算中间结果推导出高效重要性评估指标，计算开销可忽略且无需辅助存储；2）开发定制内核，将FlashAttention、重要性评估和令牌驱逐融合为单个优化算子，提升系统级效率。
...
### [190] [CFD-HAR: User-controllable Privacy through Conditional Feature Disentanglement](https://arxiv.org/abs/2603.11526)
*Alex Gn,Fan Li,S Kuniyilh,Ada Axan*

Main category: cs.LG

TL;DR: 该论文提出了一种基于特征解缠的用户可控隐私保护方法，用于可穿戴设备上的人体活动识别，并与基于自编码器的少样本学习方法进行对比分析，探讨了隐私保护、数据效率和边缘部署的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现代可穿戴设备配备IMU传感器，基于机器学习的人体活动识别面临两大挑战：1) 保护传感器数据中的敏感用户隐私信息；2) 在有限标注样本下保持高识别性能。需要解决隐私保护与识别性能的平衡问题。

Method: 提出基于特征解缠的表示学习方法，实现细粒度的动态隐私过滤控制。该方法在潜在空间中分离活动特征和敏感属性。同时对比分析了基于自编码器的少样本HAR方法，从架构设计、学习目标、隐私保证、数据效率和边缘部署适应性等方面进行比较。

Result: CFD（特征解缠）方法提供显式、可调的隐私保护控制，能在潜在空间分离活动和敏感属性；自编码器少样本HAR在标签效率和轻量适应性方面更优，但缺乏固有隐私保护。两种方法在持续IoT环境中的安全分析显示，对表示泄露和嵌入层攻击的敏感性存在差异。

Conclusion: 单一范式无法完全满足下一代IoT HAR系统的需求。需要研究统一框架，联合优化隐私保护、少样本适应性和鲁棒性，以实现可信的IoT智能系统。
...
### [191] [Multi-Task Anti-Causal Learning for Reconstructing Urban Events from Residents' Reports](https://arxiv.org/abs/2603.11546)
*Liangkai Zhou,Susu Xu,Shuqi Zhong,Shan Lin*

Main category: cs.LG

TL;DR: MTAC是一个多任务反因果学习框架，利用跨任务不变性从观测结果和混杂因素中推断潜在原因，通过因果发现和结构化SEM建模，在城市事件重建应用中显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 许多现实世界的机器学习任务是反因果的：需要从观测结果推断潜在原因。实践中常遇到多个相关任务，其中部分前向因果机制在任务间保持不变，而其他组件是任务特定的。现有方法未能充分利用这种跨任务不变性。

Method: MTAC首先进行因果发现以学习共享因果图，然后实例化结构化多任务结构方程模型(SEM)，将结果生成过程分解为：(i)任务不变机制和(ii)通过共享骨干和任务特定头实现的任务特定机制。基于学习的前向模型，MTAC执行最大后验(MAP)推断，在学习的因果结构下联合优化潜在机制变量和原因幅度。

Result: 在曼哈顿和纽瓦克市收集的真实数据上评估MTAC，应用于城市事件重建的三个任务：停车违规、废弃房产和不卫生条件。MTAC始终优于强基线，实现了高达34.61%的MAE减少，证明了学习可迁移因果机制的好处。

Conclusion: MTAC通过显式利用跨任务不变性，在多任务反因果学习中取得了显著改进。该方法在真实城市事件重建任务中表现出色，验证了学习可迁移因果机制的有效性，为反因果推断问题提供了新框架。
...
### [192] [CAETC: Causal Autoencoding and Treatment Conditioning for Counterfactual Estimation over Time](https://arxiv.org/abs/2603.11565)
...
<details>
  <summary>Details</summary>
Motivation: 时间依赖性混杂偏倚在观察性数据中仍然是一个重大挑战，影响反事实估计的准确性和效率，特别是在个性化医疗等应用中。
...
Conclusion: CAETC是一种有效的时间序列反事实估计方法，能够处理时间依赖性混杂偏倚，提高估计准确性，并具有与现有序列模型兼容的灵活性。
...
### [193] [Entropy-Preserving Reinforcement Learning](https://arxiv.org/abs/2603.11682)
...
<details>
  <summary>Details</summary>
Motivation: 策略梯度算法在语言模型推理中取得了显著进展，但许多算法在训练过程中会自然减少熵，从而降低探索轨迹的多样性，导致策略的探索能力受限。作者认为需要主动监控和控制熵以维持多样性。
...
Conclusion: 熵应该在训练过程中被主动监控和控制，提出的REPO和ADAPO方法能够有效保持策略的探索多样性，提升最终性能并维持在新环境中的可训练性。
...
### [194] [EvoFlows: Evolutionary Edit-Based Flow-Matching for Protein Engineering](https://arxiv.org/abs/2603.11703)
*Nicolas Deutschmann,Constance Ferragu,Jonathan D. Ziegler,Shayan Aziznejad,Eli Bixby*

Main category: cs.LG

TL;DR: EvoFlows是一种变长序列到序列的蛋白质建模方法，专门用于蛋白质工程，通过可控的插入、删除和替换操作预测突变位置和类型，优于传统的自回归和掩码语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质建模方法（如自回归和掩码语言模型）在蛋白质工程中存在局限性，无法同时预测突变位置和类型，且缺乏对蛋白质进化轨迹的建模能力。

Method: 采用编辑流（edit flows）方法，学习进化相关蛋白质序列之间的突变轨迹，同时建模相关天然蛋白质的分布以及连接它们的突变路径，实现可控的插入、删除和替换操作。

Result: 在UNIREF和OAS的多样化蛋白质群落上进行大量计算评估，表明EvoFlows捕获蛋白质序列分布的质量与蛋白质工程中常用的领先掩码语言模型相当，同时能够从给定模板蛋白质生成非平凡但自然样的突变体。

Conclusion: EvoFlows为蛋白质工程提供了一种有效的变长序列建模方法，能够同时预测突变位置和类型，并建模蛋白质进化轨迹，在生成自然样突变体方面表现出优势。
...
### [195] [ARROW: Augmented Replay for RObust World models](https://arxiv.org/abs/2603.11395)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于模型无关方法的持续强化学习方法通常依赖回放缓冲区来缓解灾难性遗忘，但面临内存需求大的可扩展性挑战。受神经科学启发，大脑将经验重放给预测性世界模型而非直接给策略，这促使研究者开发更高效的模型基方法。

Method: ARROW基于DreamerV3框架，采用内存高效的分布匹配回放缓冲区。它维护两个互补缓冲区：短期缓冲区存储最近经验，长期缓冲区通过智能采样保持任务多样性。这种方法避免了标准固定大小FIFO缓冲区的局限性。

Result: 在两种挑战性持续RL设置中评估：无共享结构的任务（Atari）和有共享结构的任务（Procgen CoinRun变体）。相比相同大小缓冲区的模型无关和模型基基线，ARROW在无共享结构任务上表现出显著更少的遗忘，同时保持可比较的前向迁移。

Conclusion: 研究结果突显了模型基强化学习和生物启发方法在持续强化学习中的潜力，值得进一步研究。ARROW为解决持续RL中的可扩展性挑战提供了有前景的方向。
...
### [196] [Mitigating the Multiplicity Burden: The Role of Calibration in Reducing Predictive Multiplicity of Classifiers](https://arxiv.org/abs/2603.11750)
*Mustafa Cavus*

Main category: cs.LG

TL;DR: 该研究探讨了机器学习模型在信贷风险评估中分类校准与预测多重性之间的相互作用，发现少数类别样本承受不成比例的多重性负担，且后处理校准方法能有效降低预测多重性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在高风险环境中的部署增加，确保概率可靠性和预测稳定性变得至关重要。研究旨在探索分类校准与预测多重性（即Rashomon集合中多个近最优模型对同一申请者产生冲突预测结果的现象）之间的相互作用，特别是在信贷风险评估领域。

Method: 使用九个不同的信贷风险基准数据集，研究预测多重性是否集中在低预测置信度区域，以及后处理校准如何减轻算法任意性。实证分析了少数类别观察值的预测多重性负担，并比较了三种后处理校准方法：Platt Scaling、Isotonic Regression和Temperature Scaling。

Result: 实证分析显示：1）少数类别观察值承受不成比例的多重性负担，预测多重性和预测置信度存在显著差异；2）应用后处理校准方法与Rashomon集合中较低的模糊度相关；3）在测试的技术中，Platt Scaling和Isotonic Regression对降低预测多重性提供了最稳健的减少效果。

Conclusion: 校准可以作为共识强制层，通过减轻预测多重性来支持程序公平性。研究结果表明，后处理校准方法特别是Platt Scaling和Isotonic Regression，能够有效降低算法决策中的任意性，为高风险机器学习应用提供了重要的实践指导。
...
TL;DR: NEXTPP是一个双通道框架，通过事件粒度的神经演化和交叉交互统一离散和连续表示，用于标记时间点过程的预测，在五个真实数据集上优于现有方法。
...
Conclusion: NEXTPP通过统一离散和连续表示的双通道框架，有效解决了标记时间点过程的预测问题，实现了对不规则间隔事件序列的准确建模和预测。
...
### [198] [Disentangled Representation Learning through Unsupervised Symmetry Group Discovery](https://arxiv.org/abs/2603.11790)
...
Method: 提出具身智能体通过无监督环境交互自主发现动作空间群结构的方法，包含两个算法：从交互数据中发现群分解的算法，以及学习线性对称解耦表示而不假设特定子群属性的算法

Result: 在三个具有不同群分解结构的环境中验证了方法有效性，性能优于现有LSBD方法

Conclusion: 该方法在最小假设下证明了真实对称群分解的可识别性，实现了无需先验知识的对称性解耦表示学习
...
### [199] [On the Role of Reversible Instance Normalization](https://arxiv.org/abs/2603.11869)
*Gaspard Berthelier,Tahar Nabil,Etienne Le Naour,Richard Niamke,Samir Perlaza,Giovanni Neglia*

Main category: cs.LG

TL;DR: 重新审视时间序列预测中的Reversible Instance Normalization，发现其多个组件冗余甚至有害，提出改进其鲁棒性和泛化能力的新视角
...
Method: 通过消融研究重新审视广泛使用的Reversible Instance Normalization (RevIN)，揭示其多个组件的冗余性或有害性，并基于这些观察提出改进RevIN鲁棒性和泛化能力的新视角

Result: 研究发现RevIN中的多个组件是冗余的甚至对性能有害，这为改进现有归一化方法提供了实证基础

Conclusion: 通过对RevIN的深入分析，为时间序列预测中的归一化方法提供了新的改进方向，旨在增强模型的鲁棒性和泛化能力
...
### [200] [FlexRec: Adapting LLM-based Recommenders for Flexible Needs via Reinforcement Learning](https://arxiv.org/abs/2603.11901)
*Yijun Pan,Weikang Qiu,Qiyao Ma,Mingxuan Ju,Tong Zhao,Neil Shah,Rex Ying*

Main category: cs.LG

TL;DR: FlexRec是一个基于强化学习的后训练框架，通过因果基础的项目级奖励和不确定性感知的奖励缩放，解决了LLM在推荐系统中指令跟随和细粒度优化的挑战。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统需要适应动态、需求特定的目标，但传统推荐器通常针对单一静态目标优化，难以按需重新配置行为。虽然LLM通过强化学习后训练获得了强大的指令跟随和推理能力，但在应用于推荐系统时面临两个关键障碍：序列级奖励导致粗粒度的信用分配，以及稀疏嘈杂的交互反馈导致训练低效不稳定。

Method: FlexRec框架包含两个核心组件：(1) 基于因果基础的项目级奖励，通过在剩余候选池中进行反事实交换来计算；(2) 评论家引导的不确定性感知缩放，显式建模奖励不确定性并对低置信度奖励进行降权，以在稀疏监督下稳定学习。

Result: 在多样化的推荐场景和目标下，FlexRec取得了显著提升：在需求特定排名中，NDCG@5提升高达59%，Recall@5提升高达109.4%；在泛化设置下，Recall@5进一步提升高达24.1%，超越了传统推荐器和基于LLM的基线方法。

Conclusion: FlexRec通过创新的项目级奖励设计和不确定性感知训练机制，有效解决了LLM在推荐系统中的细粒度优化问题，为动态、需求特定的推荐目标提供了有效的后训练解决方案。
...
TL;DR: 提出一种解决多治疗场景下个体治疗效果估计的新方法，通过理论推导最优平衡权重α消除超参数调优，并引入治疗聚合策略实现O(1)计算复杂度扩展。


<details>
  <summary>Details</summary>
Motivation: 多治疗场景下的个体治疗效果估计面临两个关键挑战：平衡权重的超参数选择困境和计算可扩展性的维度诅咒。现有方法需要昂贵的启发式调优，且在处理大规模治疗空间时计算复杂度高。

Method: 1) 推导多治疗泛化界并提出理论估计器计算最优平衡权重α；2) 研究三种平衡策略：成对法、一对多法和治疗聚合法；3) 扩展为生成架构Multi-Treatment CausalEGM，保持治疗流形的Wasserstein测地线结构。

Result: 在合成和图像数据集上的实验表明，该方法在估计精度和效率上显著优于传统模型，特别是在大规模干预场景中。治疗聚合法在治疗空间扩展时保持O(1)可扩展性，而一对多法在低维设置中精度更高。

Conclusion: 提出的理论框架通过消除超参数调优和解决维度诅咒，为多治疗因果推断提供了准确且可扩展的解决方案。治疗聚合策略平衡了精度和计算效率，生成架构进一步增强了方法的表达能力。
...
Result: 建立了首个面向化学动力学理解的任务框架、基准数据集和统一模型架构，为动态化学理解和多模态科学推理研究提供了基础。

Conclusion: ChemDU任务、Chem4DBench数据集和Chem4DLLM模型将推动动态化学理解和多模态科学推理的进一步研究，弥补静态分子表示在建模动态化学现象方面的局限性。
...
### [203] [Survival Meets Classification: A Novel Framework for Early Risk Prediction Models of Chronic Diseases](https://arxiv.org/abs/2603.11598)
*Shaheer Ahmad Khan,Muhammad Usamah Shahid,Muddassar Farooq*

Main category: cs.LG

TL;DR: 将生存分析重新设计用于慢性病风险分类，在五大慢性病上性能优于LightGBM和XGBoost，并提供临床验证的解释性


<details>
  <summary>Details</summary>
Motivation: 传统慢性病风险预测模型要么使用生存分析，要么使用分类方法，缺乏将两者结合的综合性工具。需要开发既能准确预测又能提供临床解释的模型。

Method: 提出将生存分析方法重新设计用于分类任务的新方法，整合生存分析与分类技术，开发了可解释的疾病风险监测模型。

Result: 在真实世界大规模电子病历数据上，生存模型在准确率、F1分数和AUROC方面表现优于或相当于LightGBM和XGBoost等先进模型，且解释性得到三位专家医师的临床验证。

Conclusion: 生存分析方法可以有效地重新设计用于分类任务，成为开发慢性病风险监测模型的综合性工具，在保持高性能的同时提供临床可验证的解释性。
...
### [204] [MobileKernelBench: Can LLMs Write Efficient Kernels for Mobile Devices?](https://arxiv.org/abs/2603.11935)
...
Method: 提出MobileKernelBench评估框架（包含基准测试和自动化流水线），并开发MoKA多智能体系统，采用仓库感知推理和计划-执行范式

Result: 标准LLMs编译失败率超过54%，性能增益可忽略；MoKA将编译成功率提升至93.7%，27.4%生成的内核比原生库有可测量加速

Conclusion: LLMs在移动内核生成方面面临工程挑战，但通过专门设计的评估框架和多智能体系统可以显著提升性能，为移动设备代码生成开辟新途径
...
### [205] [Exhaustive Circuit Mapping of a Single-Cell Foundation Model Reveals Massive Redundancy, Heavy-Tailed Hub Architecture, and Layer-Dependent Differentiation Control](https://arxiv.org/abs/2603.11940)
*Ihor Kendiukhov*

Main category: cs.LG

TL;DR: 该研究通过三种实验方法改进了生物基础模型的可解释性分析：穷举电路追踪、高阶组合消融和因果轨迹引导，揭示了先前选择性分析方法中的系统性偏差，并建立了模型层次与细胞状态控制之间的因果联系。


<details>
  <summary>Details</summary>
Motivation: 现有生物基础模型的可解释性方法（选择性特征采样、成对交互测试、观测性轨迹分析）存在系统性偏差，需要更全面、因果性的分析方法来准确理解模型内部机制。

Method: 1. 穷举电路追踪：在Geneformer模型的第5层对所有4065个活跃稀疏自编码器特征进行穷举追踪，获得1393850条显著下游边；2. 高阶组合消融：对8个特征三元组进行三路组合消融，分析冗余度和交互模式；3. 因果轨迹引导：通过特征引导建立层次位置与分化方向性之间的因果联系，测试早期（L0）、中期（L11）和晚期（L17）层特征对细胞成熟状态的影响。

Result: 1. 穷举追踪显示特征连接呈重尾分布，1.8%的特征占据不成比例的连接性，40%的top20枢纽缺乏生物注释，表明先前选择性分析存在系统性注释偏差；2. 三路组合消融显示冗余度随交互阶数单调加深（三路比0.59 vs 成对比0.74），无协同效应，确认模型架构在所有测试阶数上都是次加性的；3. 轨迹引导显示晚期层特征（L17）始终推动细胞状态向成熟方向（阳性比例1.0），而早期和中期层特征（L0和L11）主要推动远离成熟方向（阳性比例0.00-0.58）。

Conclusion: 该研究通过更全面的分析方法，从相关性证据转向因果证据，证实了模型层次对细胞状态的控制作用，揭示了先前选择性分析方法中的系统性偏差，为生物基础模型的机制解释提供了更可靠的方法论基础。
...
### [206] [Causal Matrix Completion under Multiple Treatments via Mixed Synthetic Nearest Neighbors](https://arxiv.org/abs/2603.11942)
*Minrui Luo,Zhiheng Zhang*

Main category: cs.LG

TL;DR: MSNN扩展了SNN方法，通过整合跨治疗水平的信息来解决因果矩阵补全中数据稀缺问题，保持理论保证的同时提高样本利用效率。


<details>
  <summary>Details</summary>
Motivation: SNN方法在因果矩阵补全中需要每个治疗水平都有足够数据，但在多治疗或复杂治疗场景中这一条件往往不满足，导致数据稀缺问题。

Method: 提出混合合成最近邻(MSNN)方法，通过整合跨治疗水平的信息来扩大有效样本量，保持SNN的局部低秩结构利用和锚子矩阵机制。

Result: MSNN保持了SNN的有限样本误差界和渐近正态性保证，同时在合成和真实数据集上表现出更好的性能，特别是在数据稀缺的治疗水平下。

Conclusion: MSNN通过跨治疗水平信息整合有效解决了SNN的数据稀缺限制，为MNAR下的因果矩阵补全提供了更稳健的解决方案。
...
### [207] [Geometry-Aware Probabilistic Circuits via Voronoi Tessellations](https://arxiv.org/abs/2603.11946)
*Sahil Sidheekh,Sriraam Natarajan*

Main category: cs.LG

TL;DR: 提出将Voronoi剖分融入概率电路以增强数据几何结构建模，解决传统概率电路混合权重数据独立的问题，同时保持可处理推理能力


<details>
  <summary>Details</summary>
Motivation: 传统概率电路使用数据独立的混合权重，限制了其捕捉数据流形局部几何结构的能力，需要一种能直接融入几何结构的方法

Method: 1) 将Voronoi剖分引入概率电路的和节点；2) 开发近似推理框架提供推理的上下界保证；3) 提出Voronoi剖分的结构条件以恢复精确可处理推理；4) 引入可微松弛实现梯度学习

Result: 在标准密度估计任务上进行了实证验证，展示了方法的有效性

Conclusion: Voronoi剖分是增强概率电路几何建模能力的自然方法，通过提出的近似框架和结构条件，既保持了推理的可处理性，又提升了数据几何结构的捕捉能力
...
### [208] [Statistical and structural identifiability in representation learning](https://arxiv.org/abs/2603.11970)
*Walter Nelson,Marco Fumero,Theofanis Karaletsos,Francesco Locatello*

Main category: cs.LG

TL;DR: 论文提出了表示学习模型内部表示稳定性的新理论框架，将稳定性分为统计可识别性和结构可识别性，并定义了ε-近可识别性概念，为自编码器、MAE等模型的表示分析提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 表示学习模型内部表示表现出惊人的稳定性，但现有研究将其视为单一属性。本文旨在形式化这种稳定性，区分统计可识别性（跨运行一致性）和结构可识别性（与未观测真实结构的对齐），并建立适用于现代表示学习模型的更实用的可识别性理论。

Method: 1. 提出统计ε-近可识别性和结构ε-近可识别性的模型无关定义；2. 证明具有非线性解码器模型的统计ε-近可识别性结果；3. 展示独立成分分析（ICA）可解决剩余线性模糊性；4. 提出解耦的实用方法：潜在表示的ICA后处理；5. 在合成基准和细胞显微镜基础模型MAE上进行实证验证。

Result: 1. 理论证明：具有非线性解码器的模型（包括MAE、监督学习器）的中间表示具有统计ε-近可识别性；2. 合成实验：ICA后处理的普通自编码器在解耦基准上达到最先进水平；3. 实际应用：基础模型规模的MAE结合ICA成功分离细胞显微镜数据中的生物变异和技术批次效应，显著改善下游泛化性能。

Conclusion: 通过区分统计和结构可识别性并引入ε-近可识别性概念，本文为现代表示学习模型的表示稳定性提供了更实用的理论框架。提出的ICA后处理方法为表示解耦提供了简单有效的解决方案，在合成和实际应用中均表现出色，为理解模型内部表示和改善下游任务性能提供了新途径。
...
### [209] [Cross-Domain Policy Optimization via Bellman Consistency and Hybrid Critics](https://arxiv.org/abs/2603.12087)
...
<details>
  <summary>Details</summary>
Motivation: 跨域强化学习面临两个核心挑战：源域和目标域的状态/动作空间可能不同，需要复杂的域间映射；源域模型的迁移性难以预先确定，容易产生负迁移效应

Method: 提出跨域Bellman一致性概念来衡量源域模型的迁移性，设计QAvatar方法，结合源域和目标域的Q函数，使用自适应无超参数权重函数

Result: 理论分析表明QAvatar具有收敛性并能实现可靠迁移，实验证明在多种RL基准任务（包括运动控制和机器人手臂操作）中表现出良好的迁移能力

Conclusion: QAvatar通过跨域Bellman一致性和混合critic设计，有效解决了跨域强化学习中的域间差异和负迁移问题，实现了可靠的知识迁移
...
### [210] [Taming the Adversary: Stable Minimax Deep Deterministic Policy Gradient via Fractional Objectives](https://arxiv.org/abs/2603.12110)
*Taeho Lee,Donghwan Lee*

Main category: cs.LG

TL;DR: 提出MMDDPG框架，通过极小极大优化学习连续控制任务中的抗干扰策略，在用户策略和对抗扰动策略之间建立博弈，提升RL在不确定环境中的鲁棒性。
...
Method: 提出极小极大深度确定性策略梯度（MMDDPG）框架，将训练过程表述为用户策略和对抗扰动策略之间的极小极大优化问题。用户学习最小化目标函数的鲁棒策略，而对抗方生成最大化目标函数的扰动。引入分数目标函数来平衡任务性能和扰动幅度，防止过度激进的扰动并促进鲁棒学习。

Result: 在MuJoCo环境中的实验评估表明，提出的MMDDPG在对抗外部力扰动和模型参数变化方面实现了显著改进的鲁棒性。

Conclusion: MMDDPG框架通过极小极大优化方法有效提升了强化学习在不确定环境中的鲁棒性，为解决RL在现实世界部署中的稳定性问题提供了有前景的解决方案。
...
### [211] [Cornserve: A Distributed Serving System for Any-to-Any Multimodal Models](https://arxiv.org/abs/2603.12118)
...
<details>
  <summary>Details</summary>
Motivation: Any-to-Any多模态模型服务面临挑战：不同请求的输入输出模态组合导致计算图路径不同，各组件具有不同的扩展特性，需要灵活高效的分布式服务系统。

Method: Cornserve提供灵活的任务抽象表达Any-to-Any模型计算图，支持组件解耦和独立扩展；采用高效的记录-重放执行模型跟踪数据依赖，直接在生产者与消费者间转发张量数据；基于Kubernetes构建。

Result: 系统实现约23K行Python代码，支持多样化Any-to-Any模型，提供高达3.81倍的吞吐量提升和5.79倍的尾部延迟降低。

Conclusion: Cornserve是一个开源的分布式Any-to-Any模型服务系统，通过灵活的任务抽象和高效的执行模型有效解决了多模态模型服务的挑战，显著提升了服务性能。
...
### [212] [IsoCompute Playbook: Optimally Scaling Sampling Compute for LLM RL](https://arxiv.org/abs/2603.12151)
...
TL;DR: 该研究探讨了大规模语言模型强化学习后训练的计算最优分配问题，发现并行rollout数量随计算预算增加可预测地增长然后饱和，这一规律在不同难度问题上由不同机制驱动。


<details>
  <summary>Details</summary>
Motivation: 虽然缩放定律指导了LLM预训练的计算分配，但针对大规模语言模型强化学习后训练的类似指导原则仍然缺乏理解。研究者希望建立RL后训练的计算最优分配规则，为高效计算分配提供指导。

Method: 将缩放问题框架化为计算约束下的优化问题，研究三个关键资源：每个问题的并行rollout数量、每批次的问题数量、更新步数。通过实验分析这些参数在不同计算预算下的最优分配。

Result: 发现计算最优的并行rollout数量随计算预算增加可预测地增长然后饱和。在简单问题上由解决方案锐化驱动，在困难问题上由覆盖扩展驱动。增加并行rollout能减轻问题间干扰，而每批次问题数量主要影响训练稳定性且选择范围较宽。

Conclusion: 研究结果将RL缩放定律重新定义为规定性分配规则，为计算高效的LLM RL后训练提供了实用指导。这些发现在不同基础模型和数据分布上得到验证。
...
### [213] [Neural Thickets: Diverse Task Experts Are Dense Around Pretrained Weights](https://arxiv.org/abs/2603.12228)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将预训练参数向量视为迭代适应的起点，但作者认为预训练结果应被视为参数向量分布，其中已包含任务特定专家。在小模型中这些专家解决方案仅占分布体积的极小部分，需要结构化优化方法（如梯度下降）才能发现；而在大规模、良好预训练的模型中，任务专家密度显著增加，使得多样化、能改进任务的专家占据了预训练权重邻域的相当大部分。

Method: 提出一种简单、完全并行的后训练方法：1) 在预训练权重周围随机采样N个参数扰动；2) 选择性能最优的K个扰动；3) 通过多数投票集成这K个模型的预测。该方法避免了复杂的优化过程，完全基于随机采样和选择。

Result: 尽管方法简单，但在当代大规模模型上，该方法与标准后训练方法（如PPO、GRPO、ES）具有竞争力。这表明在大规模预训练模型中，任务专家确实在参数空间中具有较高密度，使得简单的随机采样和选择策略能够有效发现性能改进的专家模型。

Conclusion: 预训练应被视为产生参数向量分布而非单一向量，大规模预训练模型中任务专家密度显著增加，使得简单的并行采样和选择方法能够有效发现性能改进的专家模型，为后训练提供了新的视角和实用方法。
...
### [214] [Temporal Straightening for Latent Planning](https://arxiv.org/abs/2603.12231)
*Ying Wang,Oumayma Bounou,Gaoyue Zhou,Randall Balestriero,Tim G. J. Rudner,Yann LeCun,Mengye Ren*

Main category: cs.LG

TL;DR: 该论文提出了一种名为"时间拉直"的方法，通过曲率正则化来改进世界模型中的表示学习，使潜在空间中的欧氏距离更好地近似测地距离，从而提升基于梯度的规划性能。


<details>
  <summary>Details</summary>
Motivation: 虽然预训练的视觉编码器能产生强语义视觉特征，但这些特征并非为规划任务量身定制，可能包含对规划无关甚至有害的信息。受人类视觉处理中感知拉直假设的启发，需要改进表示学习以更好地支持潜在规划。
...
Conclusion: 时间拉直通过改进表示学习，使潜在空间几何更适合规划，为世界模型中的潜在规划提供了有效的解决方案。
...
### [215] [STAMP: Selective Task-Aware Mechanism for Text Privacy](https://arxiv.org/abs/2603.12237)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本隐私化方法通常在隐私保护和任务效用之间存在权衡问题，要么过度保护隐私损害任务性能，要么保护不足。需要一种能够根据任务相关性和隐私敏感性进行细粒度控制的机制

Method: STAMP框架包含两个核心组件：1) 选择性隐私预算分配，基于token的任务重要性和隐私敏感性进行分组控制；2) 极坐标机制，在单位球面上仅扰动嵌入向量的方向而保持幅度不变，通过余弦最近邻搜索进行解码

Result: 在SQuAD、Yelp和AG News数据集上的实验表明，STAMP结合归一化极坐标机制在不同token级隐私预算下均能实现优越的隐私-效用权衡

Conclusion: STAMP通过细粒度的token级隐私预算分配和几何对齐的扰动机制，显著改善了文本隐私化的隐私-效用权衡，为任务感知的隐私保护提供了有效解决方案
...
### [216] [Separable neural architectures as a primitive for unified predictive and generative intelligence](https://arxiv.org/abs/2603.12244)
...
<details>
  <summary>Details</summary>
Motivation: 智能系统（物理、语言、感知）通常表现出可因子化结构，但现有神经架构多为整体式设计，未能显式利用这种结构。需要一种能统一加性、二次和张量分解模型的表示类，通过结构归纳偏置实现高效建模。

Method: 提出可分离神经架构（SNA），通过约束交互阶数和张量秩将高维映射分解为低阶组件。采用坐标感知公式，将连续物理状态视为平滑可分离嵌入，实现对混沌系统的分布建模。该方法既适用于确定性算子，也适用于离散序列。

Result: 在四个领域验证了SNA的有效性：1）通过强化学习实现自主航点导航；2）多功能微结构的逆生成；3）湍流的分布建模；4）神经语言建模。展示了该方法在统一确定性和分布表示方面的能力。

Conclusion: 可分离神经架构作为领域无关的预测和生成智能原语，能够统一确定性和分布表示，揭示了混沌时空动力学与语言自回归之间的结构类比，为跨领域智能系统建模提供了通用框架。
...
### [217] [Matching Features, Not Tokens: Energy-Based Fine-Tuning of Language Models](https://arxiv.org/abs/2603.12248)
...
Method: 提出能量基微调（EBFT）：1）使用特征匹配目标优化序列级完成分布统计；2）采用跨步块并行采样从嵌套前缀并发生成多个rollout；3）批量提取这些rollout的特征嵌入；4）执行基于策略的策略梯度更新。

Result: 在问答编码、非结构化编码和翻译任务中，EBFT与RLVR性能相当，优于监督微调（SFT），同时验证交叉熵低于两种方法。

Conclusion: EBFT通过序列级特征匹配提供密集语义反馈，在保持低交叉熵的同时提升下游任务性能，为语言模型微调提供了有效的替代方案。
...
### [218] [The Latent Color Subspace: Emergent Order in High-Dimensional Chaos](https://arxiv.org/abs/2603.12261)
*Mateusz Pach,Jessica Bader,Quentin Bouniot,Serge Belongie,Zeynep Akata*

Main category: cs.LG

TL;DR: 提出Latent Color Subspace (LCS)方法，通过分析FLUX.1 VAE潜在空间的颜色表示结构，实现无需训练的颜色预测与控制


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型发展迅速，但实现细粒度控制仍然困难，主要因为对语义信息编码方式的理解有限

Method: 开发FLUX.1变分自编码器潜在空间中颜色表示的解释框架，揭示反映色调、饱和度和亮度的结构，基于闭式潜在空间操作实现完全无需训练的颜色控制方法

Result: 验证了LCS解释框架能够预测和显式控制颜色，在FLUX中实现了基于纯数学操作的训练自由方法

Conclusion: 通过分析潜在空间颜色表示结构，提出了有效的颜色控制方法，为细粒度图像生成控制提供了新途径
...
### [219] [Harnessing Data Asymmetry: Manifold Learning in the Finsler World](https://arxiv.org/abs/2603.11396)
...
<details>
  <summary>Details</summary>
Motivation: 传统流形学习方法依赖对称的黎曼几何，强制使用对称相异度和嵌入空间（如欧几里得空间），这在实际应用中丢弃了数据样本非均匀性所固有的有价值的非对称信息。

Method: 采用Finsler几何（黎曼几何的非对称推广），提出Finsler流形学习流程，构建非对称相异度并在Finsler空间中嵌入。同时将当前主流方法（如t-SNE和UMAP）推广到非对称版本，提出Finsler t-SNE和Finsler Umap。

Result: 在受控合成数据集和大型真实数据集上的实验表明，非对称流程能够揭示传统流程中丢失的有价值信息（如密度层次结构），并且始终提供比其欧几里得对应方法更高质量的嵌入。

Conclusion: 通过转向Finsler几何，提出的非对称流形学习流程极大地扩展了现有非对称嵌入器的适用性，超越了传统的有向数据，适用于任何类型的数据，并能更好地捕捉数据中的非对称信息。
