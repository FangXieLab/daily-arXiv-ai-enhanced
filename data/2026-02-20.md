...
TL;DR: AIdentifyAGE本体论为法医牙科年龄评估提供标准化语义框架，解决现有方法异质性、数据碎片化和AI方法透明度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 年龄评估在法医和司法决策中至关重要，特别是针对无证个人和无人陪伴未成年人。当前牙科年龄评估实践面临方法异质性、数据表示碎片化、临床-法医-法律信息系统互操作性有限等挑战，AI方法的采用进一步加剧了透明度和可重复性问题。

Method: 开发AIdentifyAGE领域特定本体论，提供标准化语义框架，涵盖手动和AI辅助的法医牙科年龄评估工作流程。该本体论建模完整的医疗-法律工作流程，整合司法背景、个体信息、法医检查数据、牙齿发育评估方法、放射影像、统计参考研究和AI估计方法。基于上层和已建立的生物医学、牙科及机器学习本体论开发，确保互操作性、可扩展性和FAIR原则合规性。

Result: AIdentifyAGE本体论建立了观察、方法、参考数据和报告结果之间的可追溯链接，为医疗-法律和司法背景下的本体驱动决策支持系统奠定坚实基础。

Conclusion: AIdentifyAGE本体论是提高法医牙科年龄评估一致性、透明度和可解释性的关键步骤，通过标准化语义框架解决了当前实践中的主要挑战，为未来决策支持系统提供了可靠基础。
...
TL;DR: 本文证明上下文性不是量子力学的特有现象，而是经典概率表示中单状态重用的必然结果，揭示了上下文性作为自适应智能的一般表示约束。


<details>
  <summary>Details</summary>
Motivation: 自适应系统经常在多个上下文中操作，同时由于内存、表示或物理资源的限制而重用固定的内部状态空间。这种单状态重用普遍存在于自然和人工智能中，但其基本的表示后果尚未被充分理解。研究者旨在探索上下文性是否仅限于量子力学，还是更一般的表示约束。

Method: 将上下文建模为作用于共享内部状态的干预措施，证明任何再现上下文结果统计的经典模型都必须承担不可约的信息论成本。提供了最小构造性示例来明确实现这一成本并澄清其操作意义。解释了非经典概率框架如何通过放宽单一全局联合概率空间的假设来避免这种障碍。
...
Conclusion: 上下文性是自适应智能的一般表示约束，与物理实现无关。单状态重用在经典概率表示中必然导致上下文性，这为理解自然和人工智能中的适应性提供了新的理论基础。
...
TL;DR: MobCache：一种利用可重构缓存实现高效大规模人类移动仿真的框架，通过将推理步骤编码为潜在空间嵌入并重用组合，结合轻量解码器，在保持性能的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的人类移动仿真方法虽然能模拟真实移动行为，但计算成本过高限制了可扩展性，需要一种既能保持仿真保真度又能显著提升效率的解决方案。

Method: 设计MobCache框架，包含两个核心组件：1) 推理组件将每个推理步骤编码为潜在空间嵌入，使用潜在空间评估器实现推理步骤的重用和重组；2) 解码组件采用轻量解码器，通过移动规律约束的蒸馏训练，将潜在空间推理链转换为自然语言。

Result: 实验表明，MobCache在多个维度上显著提升了效率，同时保持了与最先进的基于大语言模型方法相当的性能。

Conclusion: MobCache通过创新的缓存框架解决了大规模人类移动仿真的效率瓶颈问题，在保持仿真质量的同时实现了显著的计算效率提升，为城市规划、流行病学和交通分析等应用提供了可行的解决方案。
...
TL;DR: 该研究分析了60个大型语言模型基准测试的饱和现象，发现近半数基准已饱和，饱和率随基准使用时间增加而上升。研究识别了影响饱和率的14个设计属性，发现专家策划的基准比众包基准更抗饱和，而隐藏测试数据并无保护作用。


<details>
  <summary>Details</summary>
Motivation: AI基准测试在衡量模型进展和指导部署决策中起核心作用，但许多基准测试很快饱和，无法区分最佳性能模型，削弱了其长期价值。研究旨在分析基准饱和现象及其驱动因素，为设计更持久的评估方法提供依据。

Method: 研究从主要模型开发商的技术报告中选取60个LLM基准测试，沿任务设计、数据构建和评估格式三个维度定义14个属性特征。通过测试五个假设，分析每个属性对饱和率的影响，识别驱动饱和的关键因素。

Result: 近半数基准测试表现出饱和现象，饱和率随基准使用时间增加而上升。隐藏测试数据（公开vs私有）对防止饱和无保护作用，而专家策划的基准比众包基准更能抵抗饱和。研究确定了延长基准寿命的关键设计选择。

Conclusion: 基准测试饱和是普遍现象，特定设计属性显著影响饱和率。专家策划优于众包，隐藏测试数据并非有效防护策略。研究结果为设计更持久、有效的AI评估基准提供了实证指导，有助于提升基准测试的长期价值。
...
TL;DR: 简单基准方法在代码进化任务中表现优于复杂方法，揭示了当前代码进化研究在评估方法和问题设计上的不足
...
Method: 在三个领域（数学界限优化、智能体脚手架设计、机器学习竞赛）测试两种简单基准方法，并与复杂代码进化方法进行对比分析

Result: 简单基准方法在所有三个领域都匹配或超越了更复杂的方法；发现搜索空间设计和领域知识比搜索算法本身更重要；高方差和小数据集导致次优脚手架选择

Conclusion: 代码进化研究的重点应从搜索算法转向更好的搜索空间设计、减少评估随机性，并提出更严谨的评估方法和最佳实践
...
TL;DR: 证明了超立方体边切片所需超平面数的新上界：S(n) ≤ ⌈4n/5⌉（n不是5的奇数倍时），或S(n) ≤ 4n/5 + 1（n是5的奇数倍时），改进了1971年的已知上界S(n) ≤ ⌈5n/6⌉。


<details>
  <summary>Details</summary>
Motivation: 研究超立方体Q_n的边切片问题：需要多少超平面才能与超立方体的每条边相交于其内部？这个问题在组合几何和离散几何中具有重要意义，与超立方体的几何性质、超平面排列以及计算几何中的相关问题有关。

Method: 通过构造8个超平面切片Q_{10}来证明改进的上界，并利用新开发的自动工具CPro1辅助构造。CPro1是一种结合推理大语言模型和自动超参数调优的工具，用于创建搜索算法以发现数学构造。

Result: 证明了S(n) ≤ ⌈4n/5⌉（当n不是5的奇数倍时），当n是5的奇数倍时，S(n) ≤ 4n/5 + 1。这改进了Paterson在1971年提出的上界S(n) ≤ ⌈5n/6⌉。同时获得了关于k<n个超平面能切片的最大边数的新下界。

Conclusion: 该研究显著改进了超立方体边切片问题中所需超平面数的上界，并展示了自动数学工具CPro1在发现复杂数学构造中的有效性，为组合几何中的类似问题提供了新的技术途径。
...
TL;DR: 提出Node Learning范式：一种去中心化边缘智能方法，节点自主学习本地数据并通过选择性对等交互扩散知识，无需全局同步或中心聚合


<details>
  <summary>Details</summary>
Motivation: 传统中心化AI在边缘计算中存在成本高、脆弱性等问题，数据传输、延迟、能耗和对大型数据中心的依赖在异构、移动和资源受限环境中扩展性差

Method: 提出Node Learning范式：节点持续从本地数据学习，维护自身模型状态，在协作有益时机会性地交换学习知识；学习通过重叠和扩散而非全局同步或中心聚合传播

Result: 该概念论文建立了Node Learning的概念基础，对比了现有去中心化方法，并探讨了其对通信、硬件、信任和治理的影响
...
<details>
  <summary>Details</summary>
Motivation: 传统犹豫模糊集评分方法缺乏序理论基础，需要建立形式化的评分框架。现有研究声称经典序能诱导格结构，但作者发现这一论断不成立，需要重新审视评分函数与序结构的关系。

Method: 1. 提出序导向的评分统一框架，将评分函数明确定义为相对于给定序的映射；2. 分析犹豫模糊元素上的经典序，证明它们不构成格结构；3. 证明基于对称序的评分满足强单调性和Gärdenfors条件等规范准则；4. 引入优势函数类，通过控制集和最小可接受阈值比较犹豫模糊元素；5. 提出离散优势函数和相对优势函数两个具体实例。

Result: 1. 经典序（如包含序、字典序等）在犹豫模糊元素上不诱导格结构，反驳了先前研究中的论断；2. 基于对称序的评分函数满足强单调性（相对于并集）和Gärdenfors条件等关键规范准则；3. 优势函数能够构建典型犹豫模糊集上的模糊偏好关系，支持群体决策；4. 离散优势函数和相对优势函数为有限集提供了具体的比较机制。

Conclusion: 该研究建立了犹豫模糊集评分的序理论基础，纠正了关于经典序格结构的错误认识，证明了对称序在评分中的优越性，并提出了实用的优势函数方法，为犹豫模糊环境下的决策分析提供了更严谨和灵活的数学框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体开发套件(ADK)存在功能支持不足或依赖人工设计拓扑、工具和内存的问题，限制了智能体的泛化能力和整体性能

Method: OpenSage让LLM自动创建具有自生成拓扑和工具集的智能体，提供分层图结构内存系统，并包含针对软件工程任务的专用工具包
...
TL;DR: AgentLAB是首个专门评估LLM智能体对自适应长视野攻击脆弱性的基准测试，包含5种新型攻击类型、28个真实环境和644个安全测试用例，发现现有防御措施无法有效缓解此类威胁。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在复杂长视野环境中部署增多，它们面临通过多轮交互实现的攻击风险，这些攻击在单轮设置中不可行，需要专门的评估基准来测量智能体对此类威胁的脆弱性。

Method: 提出AgentLAB基准测试框架，支持5种新型长视野攻击类型：意图劫持、工具链攻击、任务注入、目标漂移和内存污染，覆盖28个真实智能体环境和644个安全测试用例，用于评估代表性LLM智能体的安全性。

Result: 评估发现现有LLM智能体对长视野攻击高度脆弱，且为单轮交互设计的防御措施无法可靠缓解长视野威胁，突显了专门安全机制的必要性。

Conclusion: AgentLAB作为首个专门评估LLM智能体长视野攻击脆弱性的基准测试，为实际环境中智能体安全性的跟踪和提升提供了有价值的工具，基准测试已公开可用。
...
TL;DR: 论文研究发现，对已对齐的视觉语言模型进行窄领域有害数据微调会引发严重的跨任务跨模态的突发性错位，且多模态评估揭示的错位程度远高于单模态基准，表明现有持续学习框架存在安全风险。


<details>
  <summary>Details</summary>
Motivation: 终身多模态智能体需要通过后训练持续适应新任务，但这在能力获取与安全对齐之间产生了根本性冲突。当前研究需要探究微调对齐模型是否会破坏其安全对齐，以及这种错位如何在不同任务和模态间泛化。

Method: 使用Gemma3-4B模型进行实验，在窄领域有害数据集上进行微调，通过LoRA技术控制微调强度。采用多模态和文本单模态两种评估方式测量错位程度，分析有害数据比例的影响，并通过几何分析（主成分分析）探究有害行为的低维特性。评估了两种缓解策略：良性窄微调和基于激活的引导。

Result: 1. 窄领域有害数据微调会引发严重的突发性错位，且这种错位能广泛泛化到无关任务和模态；2. 错位程度随LoRA秩单调增加，多模态评估揭示的错位（70.71±1.22，r=128）远高于文本单模态评估（41.19±2.51）；3. 即使训练混合中仅含10%有害数据也会导致显著对齐退化；4. 几何分析显示有害行为占据极低维子空间，10个主成分即可捕获大部分错位信息；5. 两种缓解策略虽能显著减少错位，但都无法完全消除已学习的有害行为。

Conclusion: 当前的后训练范式在部署后环境中可能无法充分保持模型对齐，需要开发更鲁棒的持续学习框架。多模态安全基准对于准确评估视觉语言模型的对齐退化至关重要，因为单模态基准可能严重低估风险。有害行为在低维子空间中的集中性为开发更有效的对齐保护机制提供了机会。
...
TL;DR: DeepContext是一个状态化监控框架，使用RNN架构追踪多轮对话中的用户意图演变，显著提升越狱攻击检测性能，同时保持实时推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全护栏大多是无状态的，将多轮对话视为独立事件处理，缺乏时间感知能力。这导致"安全漏洞"，使得对抗性攻击（如Crescendo和ActorAttack）能够通过跨轮次边界缓慢渗透恶意意图来绕过无状态过滤器。

Method: 提出DeepContext框架，摒弃孤立评估模型，采用RNN架构处理细化的轮次级嵌入序列。通过在整个对话中传播隐藏状态，捕捉无状态模型忽略的风险累积过程。

Result: DeepContext在多轮越狱检测中显著优于现有基线，达到最先进的F1分数0.84，大幅超越超大规模云提供商护栏（如Llama-Prompt-Guard-2和Granite-Guardian的0.67）。在T4 GPU上保持低于20ms的推理开销，适合实时应用。

Conclusion: 建模意图的顺序演变比部署大规模无状态模型更有效且计算效率更高。状态化监控框架是解决多轮对话安全漏洞的关键方向。
...
TL;DR: LLM4Cov：一种离线智能体学习框架，通过确定性评估器将硬件验证建模为无记忆状态转移，解决了在线强化学习中工具反馈昂贵缓慢的问题，使紧凑模型在覆盖率验证任务中超越教师模型并媲美更大模型。


<details>
  <summary>Details</summary>
Motivation: 基于执行的LLM智能体虽然能通过工具反馈学习，但反馈获取成本高、速度慢，使得在线强化学习不切实际。硬件覆盖率验证尤其面临这一挑战，因为它依赖工业模拟器和不可微的执行信号。

Method: 提出LLM4Cov离线智能体学习框架：1）将验证建模为基于确定性评估器的无记忆状态转移；2）引入执行验证的数据整理；3）策略感知的智能体数据合成；4）最差状态优先采样，以在有限执行约束下实现可扩展学习。

Result: 使用该框架，一个紧凑的4B参数模型在智能体评估下达到69.2%的覆盖率通过率，比其教师模型高出5.3%，并且与规模大一个数量级的模型表现相当。同时创建了基于现有验证套件的现实对齐基准。

Conclusion: LLM4Cov框架通过离线学习有效解决了执行反馈昂贵的问题，使紧凑模型在硬件覆盖率验证任务中实现超越教师模型的性能，证明了该框架在资源受限环境下的实用性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 挑战AI系统黑盒安全评估的核心假设——测试分布上的模型行为能可靠预测部署性能。作者发现某些模型的输出依赖于在评估中罕见但在部署中普遍的未观测内部变量（潜在上下文条件策略），这导致黑盒评估存在根本性缺陷。

Method: 采用多种理论方法：1) 使用Le Cam方法证明被动评估的极小极大下界；2) 使用基于哈希的触发构造和Yao极小极大原理分析自适应评估；3) 基于陷门单向函数假设建立计算分离；4) 为白盒探测提供样本复杂度分析和偏差校正方法。
...
Conclusion: 黑盒测试在统计上可能无法确定安全性，为最坏情况安全保证需要额外保障措施（架构约束、训练时保证、可解释性、部署监控）。研究量化了黑盒评估的局限性，并提供了判断何时需要额外安全措施的明确标准。
...
Method: 构建Conv-FinRe基准：基于真实市场数据和人类决策轨迹，包含入职访谈、逐步市场情境和咨询对话。模型需要在固定投资期限内生成股票排名。基准提供多视角参考，区分描述性行为与基于投资者特定风险偏好的规范性效用。

Result: 评估一系列最先进的LLMs，结果显示理性决策质量与行为对齐之间存在持续紧张关系：在基于效用的排名上表现良好的模型往往无法匹配用户选择，而行为对齐的模型可能过度拟合短期噪声。

Conclusion: Conv-FinRe基准能够诊断LLMs是遵循理性分析、模仿用户噪声还是受市场动量驱动，为金融推荐系统提供了超越行为模仿的评估框架。数据集已在Hugging Face公开，代码库在GitHub可用。
...
TL;DR: Sonar-TS是一个神经符号框架，通过搜索-验证管道解决时间序列数据库的自然语言查询问题，使用特征索引和Python程序来定位和验证候选窗口，并引入了首个大规模基准NLQTSBench。
...
Method: 提出Sonar-TS神经符号框架，采用搜索-验证管道：1）使用特征索引通过SQL查询候选窗口；2）生成Python程序在原始信号上锁定和验证候选窗口。

Result: 实验表明Sonar-TS能有效处理传统方法失败的复杂时间查询，并揭示了该领域的独特挑战。引入了NLQTSBench作为首个大规模基准。
...
TL;DR: Cinder是一个两阶段匹配系统，通过Ruzicka相似性指数快速筛选，然后使用基于反正态分布技能桶的Kantorovich距离计算"制裁分数"，为异质技能水平的预组队提供公平快速匹配。


<details>
  <summary>Details</summary>
Motivation: 现代多人在线游戏中，公平快速的匹配系统直接影响玩家留存率和满意度。然而，为异质技能水平的预组队创建公平匹配存在重大挑战，仅基于平均团队技能指标（如均值或中位数评级）的匹配通常会导致不平衡和一边倒的比赛，特别是在技能分布广泛或偏斜时。

Method: Cinder采用两阶段匹配系统：第一阶段使用Ruzicka相似性指数快速比较团队的"非异常值"技能范围进行初步筛选；第二阶段将玩家等级映射到基于反正态分布生成的技能桶中，提供平均技能水平下的更高粒度，然后使用Kantorovich距离在排序的桶索引上量化匹配公平性，产生"制裁分数"。

Result: 通过分析1.4亿个模拟预组队配对的制裁分数分布，证明了系统的可行性，为公平匹配阈值提供了稳健基础。

Conclusion: Cinder系统通过两阶段方法有效解决了异质技能预组队的公平匹配问题，结合快速筛选和精确公平性评估，为多人在线游戏提供了可行的公平快速匹配解决方案。
...
TL;DR: M2F是首个用于端到端、项目级数学自动形式化的智能体框架，可将长篇数学资料转换为可编译的Lean项目，实现教科书级形式化


<details>
  <summary>Details</summary>
Motivation: 现有数学自动形式化技术局限于孤立定理和短代码片段，无法扩展到教科书和研究论文级别，需要解决跨文件依赖、导入解析和端到端项目编译等挑战

Method: 采用两阶段智能体框架：1) 语句编译阶段将文档拆分为原子块，通过推断依赖关系排序，修复声明骨架直到项目可编译；2) 证明修复阶段在固定签名下使用目标导向的局部编辑填补证明空缺，全程保持验证器在循环中

Result: 在约三周内将479页实分析和凸分析教科书转换为153,853行Lean代码，在FATE-H基准上达到96%证明成功率（基线为80%），实现教科书级形式化

Conclusion: M2F框架证明大规模数学文献自动形式化具有可行性，能以传统专家数月或数年工作量所需的时间完成教科书级形式化，为数学形式化的实际应用开辟了新途径
...
### [19] [RFEval: Benchmarking Reasoning Faithfulness under Counterfactual Reasoning Intervention in Large Reasoning Models](https://arxiv.org/abs/2602.17053)
*Yunseok Han,Yejoon Lee,Jaeyoung Do*

Main category: cs.AI

TL;DR: 论文提出了推理忠实性的形式化框架和评估基准RFEval，发现大型推理模型在49.7%的输出中存在不忠实推理，准确性与忠实性关联微弱，RL微调可能损害推理忠实性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）虽然表现出色，但常常产生听起来合理却未能反映其真实决策过程的推理，这削弱了可靠性和信任。需要建立形式化框架来评估推理忠实性，确保AI不仅输出正确结果，而且推理过程具有结构完整性。

Method: 提出了推理忠实性的形式化框架，包含两个可测试条件：立场一致性（推理与答案间的连贯立场）和因果影响（陈述的推理在输出级干预下因果驱动答案）。开发了RFEval基准，包含7,186个实例，涵盖七个任务，通过受控的输出级反事实干预来探测忠实性。评估了12个开源大型推理模型。

Result: 评估发现49.7%的输出存在不忠实推理，主要源于立场不一致。失败集中在数学和代码等脆弱、收敛性领域。忠实性与后训练机制（而非规模）更相关：在模型族内的消融实验表明，在监督微调基础上添加当前RL风格的目标会降低推理忠实性，即使准确性得以保持。准确性与忠实性关联微弱且统计不显著。

Conclusion: 建立了审计大型推理模型可靠性的严谨方法，表明可信AI不仅需要优化正确结果，还需确保推理过程的结构完整性。准确性与忠实性脱钩，当前RL微调可能损害推理忠实性，需要开发新的训练目标来提升推理透明度。
...
<details>
  <summary>Details</summary>
Motivation: 现有价值分解方法依赖单一最优动作，在价值函数变化时难以适应，容易收敛到次优策略，需要更好的探索机制来应对训练过程中的价值函数偏移。

Method: 提出Successive Sub-value Q-learning (S2Q)，学习多个子价值函数来保留替代高价值动作，结合Softmax行为策略促进持续探索，使总Q值能快速适应变化的最优解。

Result: 在挑战性MARL基准测试中，S2Q一致优于多种MARL算法，展现出改进的适应性和整体性能。

Conclusion: S2Q通过多子价值函数和Softmax策略有效解决了价值函数偏移问题，提升了多智能体强化学习的探索能力和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有课程学习方法需要预定义难度指标，而硬样本挖掘方法需要昂贵的逐样本损失跟踪。需要一种既能有效优先处理困难样本又计算开销小的训练优化方法。

Method: PBS使用在线训练的轻量级线性预测器，仅基于四个简单的标记级特征（标记频率、序列长度、词汇多样性和稀有标记比例）来估计样本难度，动态构建优先包含高损失样本的训练批次。

Result: 在130M参数transformer上的实验表明，PBS实现了6-13%的收敛加速（按评估损失衡量），预测器与真实损失的相关系数从0.14提升到0.44（超过10,000训练步）。
...
<details>
  <summary>Details</summary>
Motivation: 当前各ESG评级机构对同一公司的评分差异很大，限制了评级的可比性、可信度和决策相关性，需要建立统一评估框架来协调评级结果。

Method: 提出包含两部分的人机协作框架：STRIDE提供原则性标准和评分系统，指导使用大语言模型构建公司级基准数据集；SR-Delta是差异分析程序框架，揭示潜在调整的见解。

Result: 该框架能够实现可持续发展评级方法的可扩展和可比评估，为评估不同评级方法提供系统性工具。

Conclusion: 呼吁AI社区采用AI驱动方法加强和推进可持续发展评级方法，以支持和执行紧迫的可持续发展议程。
...
TL;DR: 提出O-Shap方法，通过满足T属性的层次化分割改进Owen值在图像解释中的应用，提升特征归因的精度和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 传统Shapley值方法假设特征独立，但在视觉任务中像素存在空间和语义依赖。现有Owen值实现的分割方法（如轴对齐或SLIC）违反一致性属性，导致归因不准确。

Method: 提出新的分割方法，满足T属性以确保层次间语义对齐，支持计算剪枝。该方法通过层次化分组改进Owen值在特征归因中的应用。

Result: 在图像和表格数据集上的实验表明，O-Shap在归因精度、语义一致性和运行效率方面优于基线SHAP变体，特别是在结构重要的情况下。

Conclusion: 通过满足T属性的层次化分割，O-Shap改进了Owen值在视觉任务中的应用，提供了更准确、语义一致且高效的特征归因方法。
...
<details>
  <summary>Details</summary>
Motivation: 掌握教育概念需要理解其先决条件和子概念，这对于识别学生知识差距和实现个性化学习至关重要。在大规模课程中，教师难以诊断个体误解或确定需要强化的概念。现有知识图谱方法要么停留在表面层次（如课程级概念），要么忽略了教学材料中丰富的教学信号。
...
Conclusion: InstructKG为自动构建教师对齐的知识图谱提供了一种有效框架，能够捕捉课程预期的学习进展，有助于大规模个性化教育。
...
<details>
  <summary>Details</summary>
Motivation: CircuitSAT问题在形式验证和密码分析中具有重要应用，但困难实例的求解极具挑战性，需要开发高效分解技术来降低求解复杂度

Method: 使用专用约束将原始SAT实例划分为弱化公式族，设计参数化并行算法，通过并行计算的硬度估计指导高质量分解的识别

Result: 算法在具有挑战性的CircuitSAT实例上展示实际效能，包括布尔电路逻辑等价性验证和密码哈希函数原像攻击的编码实例

Conclusion: 提出的并行分解算法为处理困难CircuitSAT问题提供了有效解决方案，在形式验证和密码分析领域具有实用价值
...
TL;DR: JEPA-DNA：一种结合联合嵌入预测架构与生成目标的DNA预训练框架，通过潜在空间预测提升基因组表示的功能上下文理解
...
Method: 提出JEPA-DNA框架，将联合嵌入预测架构与传统的生成目标相结合。通过引入潜在接地机制，将标记级恢复与潜在空间中的预测目标耦合，通过监督CLS标记来预测掩码基因组片段的高级功能嵌入，而非仅关注单个核苷酸。

Result: 在多样化的基因组基准测试中，JEPA-DNA在监督和零样本任务上始终优于仅生成基线，展现出更优越的性能。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级但高性能的公式识别模型，使其能够在消费级硬件上实现实时推理，并支持浏览器内部署，降低计算资源需求和使用门槛。

Method: 采用注意力机制设计、知识蒸馏技术，以及词汇表和分词器的迁移策略，构建仅含2000万参数的极简模型架构。

Result: Texo在性能上可与UniMERNet-T和PPFormulaNet-S等SOTA模型相媲美，同时模型大小分别减少了80%和65%，实现了消费级硬件上的实时推理和浏览器部署能力。

Conclusion: Texo证明了通过精心设计和优化，可以在大幅减小模型规模的同时保持高性能，为公式识别任务提供了轻量高效的解决方案，并开发了Web应用以促进实际使用。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI正在重塑知识工作，但现有研究主要集中在软件工程和自然科学领域，人文社科领域的方法论探索有限。本研究旨在填补这一空白，为人文社科研究者提供一个可复制的AI协作框架。

Method: 采用"方法论实验"定位，设计了一个基于AI Agent的七阶段模块化工作流，基于任务模块化、人机分工和可验证性三原则。使用Anthropic经济指数中台湾Claude.ai使用数据（7,729个对话，2025年11月）作为实证案例验证该方法。

Result: 提出了一个可复制的人文社科AI协作框架，并通过操作过程的反身性文档识别了三种人机协作模式：直接执行、迭代优化和人类主导。揭示了人类在研究问题制定、理论解释、情境化推理和伦理反思方面的不可替代性。

Conclusion: 本研究为人文社科研究者提供了一个实用的AI协作方法论框架，同时强调了人类判断在研究关键环节的不可替代性。承认了单平台数据、横截面设计和AI可靠性风险等局限性。
...
TL;DR: 提出大型行为模型(LBM)，通过行为嵌入而非提示工程，基于结构化心理特征档案预测个体战略决策，在复杂情境中实现高保真行为模拟


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在预测高风险环境下的人类决策时存在局限：提示工程方法脆弱，易出现身份漂移，且难以有效利用详细人物描述；需要一种能捕捉心理特质与情境约束复杂交互的稳定预测方法

Method: 开发大型行为模型(LBM)，基于Llama-3.1-8B-Instruct微调，采用行为嵌入而非提示工程；模型以结构化高维特征档案为条件输入，该档案源自综合心理测量工具；使用专有数据集训练，该数据集连接稳定倾向、动机状态、情境约束与观察到的选择

Result: LBM微调相比未适配的Llama-3.1-8B-Instruct骨干模型提升了行为预测性能；在基于大五人格特征条件下，性能与前沿基线相当；发现提示基线存在复杂度上限，而LBM能持续受益于更密集的特征档案，随特征维度增加性能提升

Conclusion: LBM为高保真行为模拟提供了可扩展方法，在战略预见、谈判分析、认知安全和决策支持等领域具有应用潜力，实现了从临时人物提示向稳定行为嵌入的范式转变
...
TL;DR: 该研究使用布卢姆分类法作为分层框架，通过分析大语言模型的高维激活向量，探究不同认知复杂度水平是否在模型的残差流中线性可分。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的黑箱特性需要超越表面性能指标的新评估框架。本研究旨在探究模型内部神经表示是否编码了认知复杂度层次，使用布卢姆分类法作为认知层次的理论框架。

Method: 通过分析不同大语言模型的高维激活向量，使用线性分类器探测从基础回忆（Remember）到抽象综合（Create）的不同认知水平是否在模型残差流中线性可分。

Result: 线性分类器在所有布卢姆认知水平上达到约95%的平均准确率，强有力地证明了认知水平被编码在模型表示的一个线性可访问子空间中。模型在正向传播早期就解析了提示的认知难度，且表示在不同层中变得越来越可分。

Conclusion: 该研究提供了证据表明大语言模型在内部表示中编码了认知复杂度层次，这些表示在线性子空间中是可访问的。这为理解模型如何处理不同认知难度的任务提供了新的视角，并支持了模型在早期层就解析认知复杂度的假设。
...
TL;DR: 论文提出Shapley-DCLR指标量化LLM预测中的时间知识泄露，并开发TimeSPEC方法通过声明验证主动过滤时间污染，提高回溯测试的可靠性。
...
Method: 1) 提出声明级框架检测时间知识泄露：将模型推理分解为原子声明，按时间可验证性分类，用Shapley值衡量每个声明对预测的贡献，得到Shapley-DCLR指标；2) 开发TimeSPEC方法：通过声明验证和再生主动过滤时间污染，确保所有支持声明都能追溯到截止日期前的来源。

Result: 在350个实例（美国最高法院案件预测、NBA薪资估计、股票回报排名）上的实验显示：标准提示基线存在显著时间知识泄露；TimeSPEC能降低Shapley-DCLR同时保持任务性能，表明显式的声明级验证优于基于提示的时间约束。

Conclusion: 提出的Shapley-DCLR指标能有效量化LLM预测中的时间知识泄露，TimeSPEC方法通过主动声明验证显著减少泄露，为可靠的LLM回溯测试提供了可解释的框架。
...
TL;DR: 该论文详细记录了在有限计算资源下（2xA100 GPU）从arXiv LaTeX源文件训练1.36B参数科学语言模型的完整工程流程，分析了预处理、分词、训练稳定性等关键因素。


<details>
  <summary>Details</summary>
Motivation: 尽管前沿大语言模型展现出强大的推理和数学能力，但如何从原始科学文献源文件训练领域专业化语言模型的实践过程仍缺乏详细文档。本研究旨在填补这一空白，为计算资源有限的研究者提供可复现的工程指导。

Method: 构建端到端训练流水线：1) arXiv元数据筛选（数学、计算机科学、理论物理领域）；2) 存档验证；3) LaTeX提取；4) 文本规范化；5) 领域感知分词；6) 在2xA100 GPU上训练1.36B参数的稠密Transformer模型。通过24次实验运行分析训练稳定性、扩展行为、数据损失和基础设施瓶颈。

Result: 研究发现：1) 预处理决策显著影响可用token数量；2) 分词策略影响符号稳定性；3) 存储和I/O约束可能与计算资源同等重要；4) 在数据丰富环境下（52B预训练token）训练收敛稳定；5) 提供了详细的工程洞见而非新架构。

Conclusion: 本研究提供了从原始arXiv LaTeX源文件训练小型科学语言模型的透明工程记录，强调了预处理、分词和基础设施管理的重要性。这些发现有助于计算资源有限的研究者构建领域专业化模型，为科学语言模型训练提供了实用的工程指导。
...
TL;DR: 提出一种无数据的方法，通过将表示漂移正则化构建为曲率矩阵近似问题，利用Kronecker分解近似曲率技术，实现任务向量组合时的性能提升


<details>
  <summary>Details</summary>
Motivation: 任务算术提供模块化、可扩展的基础模型适配方法，但多任务向量组合会导致跨任务干扰、表示漂移和性能下降。现有表示漂移正则化方法通常需要外部任务数据，这与模块化和数据可用性约束（如隐私要求）相冲突

Method: 提出无数据方法，将表示漂移正则化构建为曲率矩阵近似问题，采用Kronecker分解近似曲率技术，获得实用正则化器。该方法具有任务数量上的常数复杂度，并提升对任务向量缩放的鲁棒性

Result: 在任务添加和否定方面达到最先进的结果，无需保留调优，具有任务数量上的常数复杂度，并促进对任务向量缩放的鲁棒性

Conclusion: 通过将表示漂移正则化构建为曲率矩阵近似问题，提出了一种无数据的方法，利用Kronecker分解近似曲率技术，实现了高效、模块化的任务向量组合，解决了现有方法需要外部数据的问题
...
TL;DR: 提出将形式验证与深度学习图像检索结合的框架，通过图验证和神经代码生成支持开放词汇自然语言查询，确保结果可信可验证
...
Method: 整合形式验证到深度学习图像检索中，采用基于图的验证方法和神经代码生成的协同组合，对用户查询中的每个原子真值进行形式化验证

Result: 不仅返回匹配结果，还能识别标记哪些具体约束被满足或未满足，提供更透明、可问责的检索过程，同时提升最流行嵌入方法的性能

Conclusion: 通过将检索结果建立在形式推理系统上，超越了向量表示的模糊性，实现了可信可验证的开放词汇自然语言图像检索
...
Method: 提出多模态对比变分自编码器（MCVAE）：模态特异性变分编码器捕获各数据源不确定性；融合瓶颈门控机制归一化现有模态贡献；多任务目标结合生存损失、重建损失和跨模态对比损失；训练时应用随机模态掩码增强鲁棒性。

Result: 在TCGA-LUAD（n=475）和TCGA-LUSC（n=446）数据集上评估，模型在疾病特异性生存预测方面优于两种SOTA模型，对严重缺失场景具有鲁棒性；测试所有模态子集发现多模态整合并非总是有益。

Conclusion: MCVAE能有效处理多模态数据严重缺失问题，提高NSCLC生存预测性能；多模态整合需谨慎评估，并非所有组合都能提升任务表现。
...
TL;DR: 提出基于隐私设计原则的框架，指导AI应用开发者采用主动、风险规避的方法保护儿童隐私，结合GDPR、PIPEDA、COPPA等法规原则，覆盖LLM应用全生命周期，并通过教育辅导案例验证可行性。


<details>
  <summary>Details</summary>
Motivation: 儿童使用AI技术日益增多，但面临隐私风险。现有隐私法规要求企业实施保护措施，但在实践中执行困难。需要为AI服务提供商和开发者提供实用框架，在满足法律标准的同时降低儿童隐私风险。

Method: 提出基于隐私设计原则的框架，整合GDPR、PIPEDA、COPPA等隐私法规原则，将其映射到LLM应用的数据收集、模型训练、运营监控和持续验证等阶段。框架包含技术组织控制措施和基于UNCRC、AADC及学术研究的儿童设计指南，并通过13岁以下儿童LLM教育辅导案例研究验证应用。

Result: 框架展示了如何通过技术和组织控制措施以及适龄设计决策，支持开发符合法律要求并提供隐私保护的儿童AI应用。案例研究表明，在整个LLM生命周期中实施数据保护策略是可行的。

Conclusion: 基于隐私设计原则的框架能够指导开发者采用主动、风险规避的方法，通过整合隐私法规原则、技术组织控制和适龄设计指南，支持开发既保护儿童隐私又符合法律要求的AI应用。
...
TL;DR: WarpRec是一个高性能推荐系统框架，通过后端无关架构消除内存实验与分布式工业引擎之间的鸿沟，包含50+算法、40+指标和19种策略，支持从本地到分布式的无缝过渡，集成能耗追踪，并面向Agentic AI时代。


<details>
  <summary>Details</summary>
Motivation: 当前推荐系统创新受到生态系统分裂的阻碍：研究人员必须在易于进行内存实验与需要昂贵复杂重写才能适应分布式工业引擎之间做出选择。这种分裂阻碍了学术研究与工业应用之间的有效衔接。

Method: 提出WarpRec框架，采用新颖的后端无关架构，包含50多种最先进算法、40个评估指标和19种过滤与分割策略。框架集成CodeCarbon进行实时能耗追踪，支持从本地执行到分布式训练和优化的无缝过渡。

Result: WarpRec成功消除了推荐系统研究中内存实验与分布式部署之间的权衡，展示了可扩展性不必以科学完整性或可持续性为代价。框架为下一代可持续、面向智能体的推荐系统提供了架构基础。

Conclusion: WarpRec不仅弥合了学术界与工业界之间的鸿沟，还可以作为下一代可持续、面向智能体的推荐系统的架构支柱，推动推荐系统从静态排名引擎向生成式AI生态系统中的交互工具演进。
...
TL;DR: 该研究提出了一个针对ARM Cortex处理器（M0+、M4、M7）的AI模型优化基准测试框架，通过自动化测试平台系统评估能效、精度和资源利用率，发现FLOPs与推理时间呈近线性关系，并利用帕累托分析平衡能耗与精度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 嵌入式系统中AI模型部署面临能效、精度和资源利用率的平衡挑战，缺乏针对ARM Cortex处理器的系统化基准测试框架来指导开发者选择最优的处理器与模型组合。

Method: 设计自动化测试平台，系统评估ARM Cortex M0+、M4、M7处理器上的AI模型性能，分析关键性能指标（KPIs），包括FLOPs与推理时间的相关性，并采用帕累托分析来平衡能耗与模型精度的权衡。

Result: 发现FLOPs与推理时间呈近线性相关，可作为计算需求估计的可靠指标；M7处理器适合短推理周期任务，M4处理器在长推理任务中能效更优，M0+处理器适用于简单AI任务；通过帕累托分析实现了能耗与精度的优化平衡。

Conclusion: 该基准测试框架为开发者提供了设计高能效AI系统的实用指导，确保在满足性能要求的同时兼顾可持续性，不同ARM Cortex处理器在不同应用场景下各有优势，需根据具体任务需求进行选择。
...
TL;DR: KG-RAG：一种结合知识图谱与检索增强生成的新框架，用于提升LLM在电信领域的性能，解决领域复杂性、标准演进和术语专业化带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在电信领域应用面临挑战，包括领域复杂性、标准不断演进、专业术语多，导致通用领域LLM在该领域产生幻觉、准确性不足、实用性降低。

Method: 提出KG-RAG框架，将知识图谱与检索增强生成相结合：知识图谱提供从电信标准和技术文档中提取的结构化领域知识表示；RAG实现动态检索相关事实以支撑模型输出。

Result: 在基准数据集上的实验表明，KG-RAG优于纯LLM和标准RAG基线：平均准确率比RAG提高14.3%，比纯LLM模型提高21.6%。

Conclusion: KG-RAG能有效提升LLM在复杂电信场景中的准确性、可靠性和可解释性，减少幻觉，确保符合电信规范要求。
...
TL;DR: 该论文提出了两个新的评估指标——可重用性和可验证性，用于评估多智能体IR管道中LLM智能体的推理过程质量，发现这些指标与传统准确率不相关，揭示了当前基于准确率的排行榜在评估推理能力方面的盲点。


<details>
  <summary>Details</summary>
Motivation: 当前在多智能体IR管道（如搜索和排序）中，LLM智能体通过思维链（CoT）交换中间推理，但现有的CoT评估仅关注目标任务准确率，无法评估推理过程本身的质量或效用。
...
Result: 研究发现可重用性和可验证性与标准准确率不相关，揭示了当前基于准确率的推理能力排行榜的盲点。令人惊讶的是，专门推理模型生成的CoT并不比通用LLM（如Llama和Gemma）生成的CoT更具可重用性或可验证性。

Conclusion: 需要超越准确率的新指标来评估LLM智能体的推理质量，可重用性和可验证性提供了评估推理过程效用的重要维度，这对于多智能体协作系统至关重要。
...
TL;DR: KLong是一个开源LLM智能体，通过轨迹分割SFT和渐进式RL训练来解决极长视野任务，在PaperBench等基准上超越现有模型


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在处理极长视野任务时存在困难，需要开发专门的方法来训练模型解决这类复杂、多步骤的任务

Method: 1. 使用轨迹分割SFT冷启动：通过分割长轨迹、保留早期上下文、渐进截断后期上下文并保持子轨迹重叠来训练基础模型；2. 使用Research-Factory自动化管道生成高质量训练数据；3. 提出渐进式RL训练：将训练分为多个阶段，逐步延长超时时间

Result: KLong（106B）在PaperBench上超越Kimi K2 Thinking（1T）11.28%，性能提升泛化到SWE-bench Verified和MLE-bench等其他编码基准

Conclusion: 轨迹分割SFT和渐进式RL训练是解决极长视野任务的有效方法，KLong展示了在长视野任务上的优越性能和泛化能力
...
Result: ODESteer在多样化LLM对齐基准测试中取得一致改进：TruthfulQA提升5.7%，UltraFeedback提升2.5%，RealToxicityPrompts提升2.4%，优于现有激活导向方法。

Conclusion: 通过ODE统一激活导向的理论基础，提出ODESteer方法并实证验证其有效性，为大语言模型对齐提供了原则性的新视角。
...
TL;DR: 提出一种基于联邦学习的混合AI模型，结合SWIN Transformer和CNN，用于通过X射线图像诊断COVID-19和肺炎，同时保护医疗数据隐私。


<details>
  <summary>Details</summary>
Motivation: 医疗数据分散在不同医院和专家手中，存在隐私和安全问题。需要开发一个既能利用AI进行准确疾病诊断，又能保护患者数据隐私的分布式系统。

Method: 采用联邦学习框架，结合SWIN Transformer和CNN模型（DenseNet201、Inception V3、VGG 19）构建混合模型。使用TensorFlow和Keras实现，通过实时持续学习提升诊断准确性。

Result: 开发了一个安全、分布式的医疗数据处理系统，能够准确检测COVID-19和肺炎，同时确保数据隐私和模型安全性。

Conclusion: 联邦学习与混合AI模型的结合为医疗诊断提供了可靠解决方案，既能提高疾病诊断准确性，又能保护患者数据隐私，为医生提供有效的辅助工具。
...
TL;DR: 提出AI GameStore平台，通过评估AI在所有人类游戏中的表现来衡量类人通用智能，证明当前前沿VLM在多数游戏中表现远低于人类水平


<details>
  <summary>Details</summary>
Motivation: 传统AI基准测试仅评估狭窄能力且容易饱和，需要更全面的方法来评估类人通用智能。人类游戏空间（"人类游戏多元宇宙"）提供了丰富的评估环境

Method: 构建AI GameStore平台，利用LLM和人类参与循环，从Apple App Store和Steam等平台自动获取并容器化游戏环境变体，生成代表性人类游戏。基于Top 100游戏生成测试集，评估7个前沿视觉语言模型

Result: 最佳模型在多数游戏中得分低于人类平均水平的10%，特别是在需要世界模型学习、记忆和规划能力的游戏中表现更差

Conclusion: AI GameStore作为实用方法可测量和推动机器向类人通用智能发展，当前模型在游戏表现上仍远落后于人类，需进一步扩展平台和评估方法
...
TL;DR: MolHIT：基于层次化离散扩散模型的分子图生成框架，在MOSES数据集上实现接近完美的化学有效性，超越现有图扩散和1D基线方法


<details>
  <summary>Details</summary>
Motivation: 现有图扩散模型在分子生成中存在化学有效性低、难以满足目标属性等问题，相比1D建模方法表现不足，需要开发更强大的分子图生成框架

Method: MolHIT基于层次化离散扩散模型，将离散扩散推广到编码化学先验的附加类别，并采用解耦原子编码根据化学角色分割原子类型

Result: 在MOSES数据集上实现新的SOTA性能，首次在图扩散中达到接近完美的化学有效性，在多个指标上超越强1D基线；在多属性引导生成和支架扩展等下游任务中表现优异
...
TL;DR: HIPE-2026是CLEF评估实验室，专注于从多语言历史文本中提取人物-地点关系，扩展了先前系列，引入语义关系提取任务，要求系统分类两种关系类型，并采用三重评估框架。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是解决历史文本中人物-地点关系提取的挑战，这些文本通常具有噪声、多语言特性，且需要时间推理。通过将关系提取与大规模历史数据处理相结合，旨在支持知识图谱构建、历史传记重建和数字人文学科中的空间分析等下游应用。

Method: HIPE-2026采用评估实验室方法，基于HIPE-2020和HIPE-2022系列扩展，专注于人物-地点关联提取。系统需要分类两种关系类型：$at$（"此人是否曾在此地？"）和$isAt$（"此人是否在出版时间前后位于此地？"）。实验室引入三重评估框架，联合评估准确性、计算效率和领域泛化能力。

Result: 作为评估实验室，HIPE-2026本身不提供具体实验结果，而是为参与者系统建立评估框架。通过多语言、多时间期的历史文本数据集，为关系提取系统提供标准化测试平台，促进该领域技术进步。

Conclusion: HIPE-2026通过扩展历史信息提取系列，将重点从命名实体识别转向语义关系提取，特别是人物-地点关联。其三重评估框架为历史文本处理系统提供了更全面的评估标准，有助于推动数字人文学科中大规模历史数据处理技术的发展。
...
### [47] [Three-dimensional Damage Visualization of Civil Structures via Gaussian Splatting-enabled Digital Twins](https://arxiv.org/abs/2602.16713)
*Shuo Wang,Shuo Wang,Xin Nie,Yasutaka Narazaki,Thomas Matiki,Billie F. Spencer*

Main category: cs.CV

TL;DR: 提出基于高斯溅射的数字孪生方法，用于土木基础设施的三维损伤可视化，超越传统二维图像检测


<details>
  <summary>Details</summary>
Motivation: 土木基础设施检测需要精确的三维损伤可视化，传统二维图像检测和传统摄影测量三维重建技术存在局限性，需要更先进的场景表示方法

Method: 采用高斯溅射技术进行三维重建，提出多尺度重建策略平衡效率与细节，支持随时间演化的数字孪生更新

Result: 在开源合成地震后检测数据集上验证，能够有效可视化二维损伤分割结果，减少分割误差，实现全面的三维损伤可视化

Conclusion: 基于高斯溅射的数字孪生方法为土木基础设施三维损伤可视化提供了有前景的解决方案，优于传统方法
...
### [48] [Analytic Score Optimization for Multi Dimension Video Quality Assessment](https://arxiv.org/abs/2602.16856)
*Boda Lin,Yongjie Zhu,Wenyu Qin,Meng Wang,Pengfei Wan*

Main category: cs.CV

TL;DR: UltraVQA数据集提供多维视频质量评估，包含五个关键质量维度的标注，并引入Analytic Score Optimization方法优化质量评分预测。


<details>
  <summary>Details</summary>
Motivation: 视频质量评估正从单一平均意见分数向更丰富的多维度评估演进。现有数据集缺乏对视频质量多个关键维度的细粒度标注，限制了模型对视频质量的全面理解和评估能力。

Method: 1) 构建UltraVQA大规模多维VQA数据集，包含用户生成内容，标注五个关键质量维度：运动质量、运动幅度、美学质量、内容质量和清晰度质量；2) 引入Analytic Score Optimization（ASO），一种基于理论推导的后训练目标，将质量评估重新定义为正则化决策过程，获得闭式解以捕捉人类评分的序数性质。

Result: 该方法在实验中优于大多数基线（包括闭源API和开源模型），同时减少了质量预测的平均绝对误差（MAE）。数据集包含超过3名人类评分者对每个视频的维度评分，并配有基于集体人类判断生成的GPT解释性理由。

Conclusion: 多维、可解释的标注和基于强化学习的对齐对于推进视频质量评估至关重要。ASO方法通过理论推导的优化目标，能够更好地利用丰富的标注信息，提高离散质量评分的评估准确性。
...
### [49] [DODO: Discrete OCR Diffusion Models](https://arxiv.org/abs/2602.16872)
*Sean Man,Roy Ganz,Roi Ronen,Shahar Tsiper,Shai Mazor,Niv Nayman*

Main category: cs.CV

TL;DR: DODO：首个利用块离散扩散的视觉语言模型，为OCR任务实现并行解码，相比自回归基线推理速度提升3倍，同时保持接近SOTA的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前基于自回归解码的VLM在长文档OCR任务中计算成本高、速度慢。OCR是高度确定性任务，理论上可通过扩散模型实现高效并行解码，但现有掩码扩散模型存在结构不稳定性，无法满足OCR的精确匹配要求。

Method: 提出DODO模型，采用块离散扩散方法，将生成过程分解为块，缓解全局扩散中的同步错误，从而利用扩散模型的并行解码优势。

Result: DODO在保持接近最先进准确率的同时，相比自回归基线实现了高达3倍的推理速度提升。

Conclusion: 块离散扩散方法成功解锁了扩散模型在OCR任务中的加速潜力，为确定性视觉语言任务提供了高效并行解码的新范式。
...
### [50] [StereoAdapter-2: Globally Structure-Consistent Underwater Stereo Depth Estimation](https://arxiv.org/abs/2602.16915)
*Zeyu Ren,Xiang Li,Yiran Wang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: StereoAdapter-2提出了一种基于选择性状态空间模型的ConvSS2D算子替代传统ConvGRU，用于水下立体深度估计，通过四向扫描策略实现单步长距离空间传播，并构建了大规模合成水下立体数据集UW-StereoDepth-80K。
...
Method: 1) 提出ConvSS2D算子，基于选择性状态空间模型，采用四向扫描策略与极线几何对齐，同时捕捉垂直结构一致性；2) 构建UW-StereoDepth-80K大规模合成水下立体数据集，通过语义感知风格迁移和几何一致新视角合成的两阶段生成流程；3) 继承StereoAdapter的动态LoRA适应机制。

Result: 在TartanAir-UW上实现17%的性能提升，在SQUID上实现7.2%的提升，达到最先进的零样本性能。在BlueROV2平台上的真实世界验证证明了方法的鲁棒性。

Conclusion: StereoAdapter-2通过ConvSS2D算子和大规模合成数据集，有效解决了水下立体深度估计中的长距离视差传播问题，在计算效率和性能上均优于传统GRU方法，为水下机器人感知提供了更可靠的解决方案。
...
TL;DR: Xray-Visual是一个统一的视觉模型架构，用于大规模图像和视频理解，基于行业级社交媒体数据训练，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 构建一个能够同时处理图像和视频理解任务的统一视觉模型，利用大规模社交媒体数据进行训练，以解决传统模型在跨模态任务和实际应用中的局限性。

Method: 采用三阶段训练流程：自监督MAE、半监督标签分类和CLIP风格对比学习；基于Vision Transformer架构，增强高效token重组(EViT)；整合大型语言模型作为文本编码器(LLM2CLIP)；使用超过150亿图像-文本对和100亿视频-标签对进行训练。

Result: 在ImageNet图像分类、Kinetics和HMDB51视频理解、MSCOCO跨模态检索等多个基准测试中达到最先进性能；表现出对领域偏移和对抗扰动的强鲁棒性；LLM2CLIP集成显著提升了检索性能和泛化能力。

Conclusion: Xray-Visual为可扩展的多模态视觉模型设立了新的基准，在保持计算效率的同时实现了卓越的准确性，特别是在真实世界环境中表现出强大的泛化能力。
...
### [52] [HS-3D-NeRF: 3D Surface and Hyperspectral Reconstruction From Stationary Hyperspectral Images Using Multi-Channel NeRFs](https://arxiv.org/abs/2602.16950)
...
<details>
  <summary>Details</summary>
Motivation: 高光谱成像（HSI）和3D重建技术的结合对农产品质量表征和植物表型分析至关重要，但现有方法需要复杂的硬件设置和移动相机配置，不适合自动化表型分析系统。需要一种能在标准室内农业环境中实现高通量、可重复的高光谱3D重建方法。

Method: 提出HSI-SC-NeRF框架：1）使用静态相机采集多视角高光谱数据，物体在特氟龙成像腔内旋转；2）通过ArUco标记估计物体姿态并转换到相机坐标系；3）采用多通道NeRF公式，使用复合光谱损失联合优化所有高光谱波段重建；4）采用两阶段训练协议，将几何初始化与辐射度细化解耦。

Result: 在三个农产品样本上的实验表明，HSI-SC-NeRF在可见光和近红外光谱范围内实现了高空间重建精度和强光谱保真度，验证了其适合集成到自动化农业工作流程中。

Conclusion: HSI-SC-NeRF成功解决了静态相机设置下的高通量高光谱3D重建问题，为自动化农产品采后检测和表型分析提供了可行的解决方案，克服了传统方法在硬件兼容性和通量方面的限制。
...
### [53] [DDiT: Dynamic Patch Scheduling for Efficient Diffusion Transformers](https://arxiv.org/abs/2602.16968)
...
Method: 提出动态标记化策略，根据内容复杂度和去噪时间步动态变化补丁大小。关键洞察是：早期时间步只需要较粗的补丁来建模全局结构，而后期迭代需要更细（更小尺寸）的补丁来细化局部细节。在推理过程中，该方法动态重新分配去噪步骤中的补丁大小。

Result: 该方法在FLUX-1.Dev上实现了高达3.52倍的加速，在Wan 2.1上实现了3.2倍的加速，同时保持了生成质量和提示遵循性。广泛的实验证明了该方法的有效性。

Conclusion: 动态标记化是一种高效的测试时策略，可显著降低扩散变换器的计算成本，同时保持感知生成质量，为高效生成模型提供了有前景的方向。
...
### [54] [Characterizing the Predictive Impact of Modalities with Supervised Latent-Variable Modeling](https://arxiv.org/abs/2602.16979)
*Divyam Madaan,Sumit Chopra,Kyunghyun Cho*

Main category: cs.CV

TL;DR: PRIMO是一个监督潜在变量插补模型，用于处理多模态学习中缺失模态的问题，通过量化缺失模态的预测影响，并利用所有可用训练样本（无论模态是否完整）进行学习。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法通常假设训练和推理时所有模态都可用，但实际应用中多模态数据往往不完整（模态缺失、异步收集或仅部分样本可用）。需要一种能处理模态缺失并量化其预测影响的方法。

Method: PRIMO通过潜在变量建模缺失模态与观测模态在预测任务中的关系。训练时利用所有可用样本（完整或部分模态），推理时从学习到的缺失模态分布中采样，获得边际预测分布并分析缺失模态对预测的影响。
...
Conclusion: PRIMO有效处理多模态学习中的模态缺失问题，既能利用不完整数据训练，又能量化缺失模态的预测不确定性，为实际应用中不完整多模态数据的处理提供了实用解决方案。
...
### [55] [Patch-Based Spatial Authorship Attribution in Human-Robot Collaborative Paintings](https://arxiv.org/abs/2602.17030)
*Eric Chen,Patricia Alves-Oliveira*

Main category: cs.CV

TL;DR: 提出基于图像块的框架用于人机协作绘画中的空间作者归属，通过法医案例研究验证，在块级别达到88.8%准确率，并引入条件香农熵量化风格重叠


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI越来越多地参与创意生产，记录作者身份对艺术家、收藏家和法律环境变得至关重要。需要解决人机协作创作中作者归属的量化问题。

Method: 采用基于图像块的框架进行空间作者归属，使用商用平板扫描仪和留一绘画交叉验证。对于协作作品，使用条件香农熵量化风格重叠，并通过手动标注的混合区域验证模型检测混合作者身份的能力。

Result: 方法在块级别达到88.8%准确率（通过多数投票在绘画级别达到86.7%），优于基于纹理和预训练特征的基线方法（68.0%-84.7%）。混合区域的熵值比纯绘画高64%（p=0.003），表明模型检测到混合作者身份而非分类失败。

Conclusion: 训练模型针对特定人机对，但为数据稀缺的人机创意工作流中的样本高效归属提供了方法论基础，未来有望扩展到任何人机协作绘画的作者归属。
...
### [56] [PartRAG: Retrieval-Augmented Part-Level 3D Generation and Editing](https://arxiv.org/abs/2602.17033)
...
Method: 提出PartRAG框架，包含：1）分层对比检索模块，将密集图像块与3D部件潜在空间对齐，从1236个部件标注资产库中检索多样化物理合理示例注入去噪过程；2）掩码部件级编辑器，在共享规范空间中操作，支持部件交换、属性细化和组合更新

Result: 在Objaverse、ShapeNet和ABO数据集上取得竞争性结果：Objaverse上Chamfer距离从0.1726降至0.1528，F-Score从0.7472提升至0.844；推理时间38秒，交互编辑5-8秒；定性结果显示更清晰的部件边界、更好的薄结构保真度和对铰接物体的鲁棒性

Conclusion: PartRAG通过检索增强方法有效解决了单图像3D生成中的部件级结构挑战，实现了多样化生成和精确局部编辑，同时保持多视图一致性和非目标部件不变性
...
### [57] [Amber-Image: Efficient Compression of Large-Scale Diffusion Transformers](https://arxiv.org/abs/2602.17047)
*Chaojie Yang,Tian Li,Yue Zhang,Jun Gao*

Main category: cs.CV

TL;DR: 提出高效压缩框架，将60层双流MMDiT的Qwen-Image转换为轻量级模型，无需从头训练。推出Amber-Image系列：10B版本采用时间步敏感深度剪枝，6B版本引入混合流架构，参数减少70%，训练成本极低。


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformer (DiT) 架构在文本到图像生成方面取得显著进展，但存在计算成本过高和部署障碍的问题。需要开发高效的压缩方法来降低模型复杂度，同时保持生成质量。

Method: 1. 提出高效压缩框架，无需从头训练；2. Amber-Image-10B：采用时间步敏感深度剪枝策略，保留层通过局部权重平均重新初始化，通过层间蒸馏和全参数微调优化；3. Amber-Image-6B：引入混合流架构，将深层双流转换为单流（从图像分支初始化），通过渐进蒸馏和轻量级微调进一步优化。

Result: 1. 参数减少70%；2. 无需大规模数据工程；3. 从10B到6B的完整压缩训练流程仅需不到2000 GPU小时，成本效益极高；4. 在DPG-Bench和LongText-Bench等基准测试中，Amber-Image实现了高保真合成和卓越的文本渲染能力，与更大模型相当。

Conclusion: 提出的压缩框架成功将大型DiT模型转换为轻量级版本，在显著降低参数和计算成本的同时保持了生成质量，为高效文本到图像生成模型的部署提供了实用解决方案。
...
### [58] [StructCore: Structure-Aware Image-Level Scoring for Training-Free Unsupervised Anomaly Detection](https://arxiv.org/abs/2602.17048)
...
TL;DR: StructCore是一种无需训练、结构感知的图像级异常评分方法，通过捕捉异常得分图的结构特征来改进传统的最大池化方法。


<details>
  <summary>Details</summary>
Motivation: 最大池化是基于内存库的无监督异常检测中图像级决策的标准方法，但它仅依赖单一极端响应，丢弃了异常证据在图像中的分布和结构信息，导致正常和异常得分经常重叠。

Method: 给定异常得分图，StructCore计算低维结构描述符φ(S)来捕捉分布和空间特征，并通过从训练正常样本估计的对角马氏距离校准来改进图像级评分，而不修改像素级定位。

Result: 在MVTec AD上达到99.6%的图像级AUROC，在VisA上达到98.4%，通过利用最大池化忽略的结构特征实现了鲁棒的图像级异常检测。

Conclusion: StructCore超越了传统最大池化方法，通过捕捉异常得分图的结构特征显著提升了图像级异常检测性能，且无需额外训练。
...
### [59] [Cholec80-port: A Geometrically Consistent Trocar Port Segmentation Dataset for Robust Surgical Scene Understanding](https://arxiv.org/abs/2602.17060)
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中的套管端口会持续遮挡视野并吸引过多特征点，对图像拼接、3D重建和视觉SLAM等几何处理流程造成负面影响。然而现有公共数据集缺乏高质量的端口标注，且现有标注往往违反几何一致性（如遮挡中心开口），限制了相关算法的鲁棒性。

Method: 1. 从Cholec80数据集创建Cholec80-port高保真套管端口分割数据集；2. 制定严格的标准操作程序（SOP），定义排除中心开口的端口套管掩码；3. 按照相同SOP清理和统一现有公共数据集；4. 通过实验验证几何一致标注对跨数据集鲁棒性的提升效果。

Result: 实验表明，几何一致的标注显著提高了跨数据集鲁棒性，这种提升效果超越了单纯增加数据集规模所能带来的改进。统一的标注标准使得不同数据集之间具有更好的兼容性和可迁移性。

Conclusion: 该研究强调了手术数据集中几何一致标注的重要性，提出的Cholec80-port数据集和SOP标准为腹腔镜手术中的套管端口分割提供了高质量基准，有助于提升下游几何处理任务的性能。
...
### [60] [Cross Pseudo Labeling For Weakly Supervised Video Anomaly Detection](https://arxiv.org/abs/2602.17077)
...
<details>
  <summary>Details</summary>
Motivation: 弱监督视频异常检测需要在仅有视频级标签的情况下检测异常并识别异常类别。现有方法往往难以同时实现精确的异常定位和准确的类别识别，需要一种能够结合时空定位精度和语义判别能力的统一框架。
...
Conclusion: CPL-VAD通过双分支结构和交叉伪标签机制，有效结合了异常定位的时间精度和类别识别的语义判别能力，为弱监督视频异常检测提供了一种有效的统一解决方案。
...
### [61] [ComptonUNet: A Deep Learning Model for GRB Localization with Compton Cameras under Noisy and Low-Statistic Conditions](https://arxiv.org/abs/2602.17085)
*Shogo Sato,Kazuo Tanaka,Shojun Ogasawara,Kazuki Yamamoto,Kazuhiko Murasaki,Ryuichi Tanida,Jun Kataoka*

Main category: cs.CV

TL;DR: ComptonUNet：一种混合深度学习框架，用于在低光子统计和高背景噪声条件下实现稳健的伽马射线暴定位


<details>
  <summary>Details</summary>
Motivation: 微弱伽马射线暴（GRBs）是宇宙中能量最高的瞬变现象之一，可作为高能天体物理过程的重要探针，特别是来自遥远宇宙的微弱GRBs可能为恒星形成的早期阶段提供独特见解。然而，由于低光子统计和大量背景噪声，检测和定位这些弱源仍然具有挑战性。现有的机器学习模型虽然解决了这些挑战的个别方面，但往往难以平衡统计稳健性和噪声抑制之间的权衡。

Method: 提出ComptonUNet，一种混合深度学习框架，联合处理原始数据并重建图像以实现稳健的GRB定位。该框架旨在通过结合直接重建模型的统计效率和基于图像的架构的去噪能力，在有限光子统计和强背景污染条件下有效运行。研究进行了GRB类事件的真实模拟，这些事件嵌入在代表低地球轨道任务的背景环境中，以评估ComptonUNet的性能。

Result: ComptonUNet显著优于现有方法，在广泛的低统计和高背景场景中实现了改进的定位精度。

Conclusion: ComptonUNet通过结合统计效率和去噪能力，成功解决了微弱伽马射线暴定位中的关键挑战，为高能天体物理研究提供了更强大的工具。
...
### [62] [3D Scene Rendering with Multimodal Gaussian Splatting](https://arxiv.org/abs/2602.17124)
*Chi-Shiang Gau,Konstantinos D. Polyzos,Athanasios Bacharis,Saketh Madhuvarasu,Tara Javidi*

Main category: cs.CV

TL;DR: 提出了一种将射频感知（如汽车雷达）与3D高斯泼溅渲染相结合的多模态框架，以解决视觉线索不可靠场景下的3D重建问题


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的高斯泼溅方法需要足够多的相机视角来初始化高斯基元，在恶劣天气、低光照或部分遮挡等视觉线索不可靠条件下表现不佳。射频信号对这些条件具有鲁棒性，因此需要整合射频感知来提高3D重建的鲁棒性和效率

Method: 提出多模态框架，将射频感知（如汽车雷达）与高斯泼溅渲染相结合。通过稀疏的射频深度测量实现高效深度预测，生成高质量3D点云用于初始化高斯函数，可适配多种高斯泼溅架构

Result: 数值测试表明，将射频感知合理整合到高斯泼溅流程中，能够实现高保真度的3D场景渲染，射频信息提供的结构准确性显著提升了渲染质量

Conclusion: 射频感知与高斯泼溅渲染的融合为视觉线索不可靠场景下的3D重建提供了更高效、更鲁棒的替代方案，展示了多模态感知在3D场景重建中的优势
...
### [63] [B$^3$-Seg: Camera-Free, Training-Free 3DGS Segmentation via Analytic EIG and Beta-Bernoulli Bayesian Updates](https://arxiv.org/abs/2602.17134)
*Hiromichi Kamata,Samuel Arthur Munro,Fuminori Homma*

Main category: cs.CV

TL;DR: B³-Seg提出了一种基于Beta-Bernoulli贝叶斯更新的快速开放词汇3D高斯泼溅分割方法，无需预定义视角、真实标签或重新训练，通过解析预期信息增益主动选择视角，实现几秒内完成端到端分割。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅分割方法依赖预定义视角、真实标签或昂贵的重新训练，无法满足电影和游戏制作中实时编辑的低延迟需求，需要一种无需训练、视角自由且高效的分割方法。

Method: 将分割问题重新表述为顺序Beta-Bernoulli贝叶斯更新过程，通过解析预期信息增益主动选择下一个最佳视角，贝叶斯公式保证了EIG的自适应单调性和子模性，产生贪婪(1-1/e)近似最优视角采样策略。

Result: 在多个数据集上的实验表明，B³-Seg能够达到与高成本监督方法竞争的结果，同时端到端分割仅需几秒钟，实现了具有可证明信息效率的实用交互式3DGS分割。

Conclusion: B³-Seg为3D高斯泼溅分割提供了一种理论严谨、快速且实用的解决方案，在无需训练、视角自由的条件下实现了高效的开放词汇分割，满足了实时编辑应用的低延迟需求。
...
### [64] [BadCLIP++: Stealthy and Persistent Backdoors in Multimodal Contrastive Learning](https://arxiv.org/abs/2602.17168)
...
<details>
  <summary>Details</summary>
Motivation: 现有针对多模态对比学习模型的后门攻击方法面临两大挑战：隐蔽性不足（跨模态不一致暴露触发模式）和持久性不足（低毒化率下的梯度稀释加速后门遗忘）。这些耦合原因尚未得到充分建模和解决。

Method: 1) 隐蔽性：提出语义融合QR微触发器，在任务相关区域附近嵌入不可察觉模式，保持干净数据统计特性同时产生紧凑触发分布；采用目标对齐子集选择在低注入率下增强信号。2) 持久性：通过半径收缩和质心对齐稳定触发嵌入；通过曲率控制和弹性权重巩固稳定模型参数，将解保持在抵抗微调的低曲率宽盆地内。

Result: 仅0.3%毒化率下，BadCLIP++在数字设置中达到99.99%攻击成功率，比基线高11.4个百分点。在19种防御方法下，攻击成功率保持在99.90%以上，干净准确率下降小于0.8%。物理攻击成功率65.03%，对水印移除防御具有鲁棒性。

Conclusion: BadCLIP++成功解决了多模态对比学习后门攻击的隐蔽性和持久性挑战，首次提供了理论分析表明在信任区域内，干净微梯度和后门目标梯度同向，攻击成功率退化具有非递增上界。该方法在极低毒化率下实现了高攻击成功率和防御鲁棒性。
...
### [65] [NRGS-SLAM: Monocular Non-Rigid SLAM for Endoscopy via Deformation-Aware 3D Gaussian Splatting](https://arxiv.org/abs/2602.17182)
...
Method: 1. 变形感知3D高斯地图：为每个高斯基元添加可学习的变形概率，通过贝叶斯自监督策略优化，无需外部非刚性标签；2. 可变形跟踪模块：采用粗到精的姿态估计，优先考虑低变形区域，然后进行高效的逐帧变形更新；3. 可变形建图模块：平衡表示能力和计算效率，逐步扩展和细化地图；4. 统一鲁棒几何损失：结合外部几何先验缓解单目非刚性SLAM的病态性。
...
### [66] [Selective Training for Large Vision Language Models via Visual Information Gain](https://arxiv.org/abs/2602.17186)
*Seulbi Lee,Sangheum Hwang*

Main category: cs.CV

TL;DR: 提出视觉信息增益(VIG)指标量化视觉输入对预测不确定性的减少，并基于此设计选择性训练方案以缓解LVLM的语言偏见问题


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在语言偏见问题，倾向于不依赖视觉证据生成答案。现有方法缺乏量化评估训练样本或token从图像中获益程度的指标。

Method: 提出视觉信息增益(VIG)指标，基于困惑度测量视觉输入提供的预测不确定性减少；利用VIG设计选择性训练方案，优先训练高VIG的样本和token。

Result: VIG能够在样本和token层面进行细粒度分析，有效识别颜色、空间关系和属性等视觉基础元素；选择性训练方案改善了视觉基础性并缓解了语言偏见。

Conclusion: VIG为量化视觉信息贡献提供了有效指标，基于VIG的选择性训练方案通过专注于视觉信息丰富的样本和token，以更少的监督实现了更好的性能。
...
### [67] [EntropyPrune: Matrix Entropy Guided Visual Token Pruning for Multimodal Large Language Models](https://arxiv.org/abs/2602.17196)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型处理大量视觉token导致推理成本高昂，现有token剪枝方法依赖经验选择的静态层，缺乏理论依据且可解释性差，难以跨模型迁移。

Method: 提出矩阵熵视角识别"熵崩溃层"作为剪枝时机；设计EntropyPrune框架，通过量化单个视觉token的信息价值进行剪枝；利用对偶Gram矩阵的谱等价性降低熵计算复杂度，实现64倍理论加速。

Result: 在多样化多模态基准测试中，EntropyPrune在准确性和效率上均优于最先进的剪枝方法；在LLaVA-1.5-7B上实现68.2% FLOPs减少，保持96.0%原始性能；在高分辨率和视频模型中表现出良好泛化能力。

Conclusion: 矩阵熵为视觉token剪枝提供了理论依据，EntropyPrune框架在保持性能的同时显著加速MLLM推理，具有强鲁棒性和可扩展性，为实际应用中的MLLM加速提供了有效解决方案。
...
Method: 采用混合感知架构，包括交叉口安装子系统和路段中间子系统，通过无线通信连接实现同步数据传输。感知流程结合了基于聚类和基于深度学习的检测方法，并使用卡尔曼滤波跟踪实现稳定的实时性能。激光雷达测量被配准到地理参考框架中，以支持车道级定位和一致的车辆跟踪。

Result: 现场评估表明，该系统能够以高时空分辨率可靠地监测货运车辆运动。设计和部署为开发支持货运信号优先应用的基于基础设施的感知系统提供了实用见解。

Conclusion: 本文提出的多模态检测系统能够有效支持货运信号优先应用，通过基础设施感知实现可靠的货运车辆检测和跟踪，为智能交通系统提供了实用的技术方案。
...
TL;DR: EA-Swin是一种嵌入无关的Swin Transformer模型，通过因子化窗口注意力设计直接在预训练视频嵌入上建模时空依赖，结合EA-Video数据集，在AI生成视频检测任务上实现了97-99%的准确率，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法（基于浅层嵌入轨迹、图像自适应或计算量大的MLLMs）无法有效检测Sora2、Veo3等先进基础视频生成器产生的高度真实合成视频，需要更强大的检测方案。

Method: 提出EA-Swin（嵌入无关Swin Transformer），采用因子化窗口注意力设计直接在预训练视频嵌入上建模时空依赖，兼容通用ViT风格补丁编码器；构建EA-Video数据集（13万视频），整合新收集样本与现有数据集，涵盖商业和开源生成器，包含未见生成器分割用于跨分布评估。

Result: EA-Swin在主要生成器上达到0.97-0.99准确率，比现有SOTA方法（通常0.8-0.9）提升5-20%，同时保持对未见分布的强泛化能力。

Conclusion: EA-Swin为现代AI生成视频检测提供了一个可扩展且鲁棒的解决方案，通过嵌入无关设计和综合数据集实现了卓越的检测性能和泛化能力。
...
### [70] [Attachment Anchors: A Novel Framework for Laparoscopic Grasping Point Prediction in Colorectal Surgery](https://arxiv.org/abs/2602.17310)
*Dennis N. Schneider,Lars Wagner,Daniel Rueckert,Dirk Wilhelm*

Main category: cs.CV

TL;DR: 提出attachment anchors结构化表示，用于结直肠手术中组织抓取点预测，通过编码组织与解剖附着点的局部几何和力学关系来降低不确定性，在90例手术数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 结直肠手术等复杂微创手术中，精确的抓取点预测是自主组织操作的关键挑战。当前研究对结直肠手术关注不足，但其重复性组织操作特性为机器学习驱动的自主支持提供了有前景的切入点。

Method: 引入attachment anchors结构化表示，编码组织与其解剖附着点之间的局部几何和力学关系。该表示通过将手术场景归一化到一致的局部参考系来降低不确定性。从腹腔镜图像预测attachment anchors，并将其整合到基于机器学习的抓取框架中。

Result: 在90例结直肠手术数据集上的实验表明，attachment anchors相比仅使用图像的基线方法显著改善了抓取点预测。在分布外设置（包括未见过的术式和手术医生）中表现尤为突出。

Conclusion: attachment anchors是结直肠手术中基于学习的组织操作的有效中间表示，特别在提高泛化能力和处理分布外场景方面具有优势。
...
### [71] [Polaffini: A feature-based approach for robust affine and polyaffine image registration](https://arxiv.org/abs/2602.17337)
*Antoine Legouhy,Cosimo Campo,Ross Callaghan,Hojjat Azadbakht,Hui Zhang*

Main category: cs.CV

TL;DR: Polaffini：基于解剖特征的快速、鲁棒医学图像配准框架，利用深度学习分割模型提取解剖结构质心作为特征点，通过闭式解实现全局/局部仿射匹配，生成从仿射到多仿射的可调平滑变换。


<details>
  <summary>Details</summary>
Motivation: 传统基于强度的配准方法依赖对齐质量的替代度量，而理论上更理想的基于特征的方法因难以可靠提取特征而失宠。深度学习分割模型的进步使得能够即时提供可靠、细粒度的解剖分割，这为创建新的解剖基础图像配准算法提供了机会。

Method: 从分割区域提取解剖结构质心作为具有1对1对应关系的特征点；通过闭式解实现高效的全局和局部仿射匹配；生成从仿射到多仿射的可调平滑变换，多仿射变换在log-Euclidean框架中嵌入以确保微分同胚特性。

Result: Polaffini在结构对齐方面优于流行的基于强度的配准方法，并为下游非线性配准提供改进的初始化；该方法快速、鲁棒且准确，特别适合集成到医学图像处理流程中。

Conclusion: 深度学习分割模型的进步使得基于解剖特征的配准方法重新成为可行选择；Polaffini框架展示了利用这些进展创建解剖基础图像配准算法的潜力，为医学图像配准提供了快速、鲁棒且准确的解决方案。
...
### [72] [Tree crop mapping of South America reveals links to deforestation and conservation](https://arxiv.org/abs/2602.17372)
...
TL;DR: 开发首个南美洲10米分辨率树木作物地图，用于监测树木作物扩张，支持欧盟零毁林政策，发现现有监管地图误将小农农林系统分类为森林，可能导致不公平处罚。


<details>
  <summary>Details</summary>
Motivation: 监测树木作物扩张对欧盟零毁林政策至关重要，但缺乏高分辨率数据区分农业系统和森林，现有监管地图可能误将小农农林系统分类为森林，导致错误毁林警报和不公平处罚。

Method: 使用多模态时空深度学习模型，基于Sentinel-1和Sentinel-2卫星影像时间序列，生成南美洲首个10米分辨率树木作物地图。

Result: 识别出约1100万公顷树木作物，其中23%与2000-2020年森林覆盖损失相关；发现现有欧盟监管地图常将已建立的农业（特别是小农农林系统）误分类为"森林"。

Conclusion: 提供高分辨率基线数据，有助于减少错误毁林警报和不公平处罚风险，支持更有效、包容和公平的保护政策。
...
### [73] [DRetHTR: Linear-Time Decoder-Only Retentive Network for Handwritten Text Recognition](https://arxiv.org/abs/2602.17387)
*Changhun Kim,Martin Mayr,Thomas Gorges,Fei Wu,Mathias Seuret,Andreas Maier,Vincent Christlein*

Main category: cs.CV

TL;DR: DRetHTR：基于RetNet的仅解码器手写文本识别模型，通过软注意力替代和层间gamma缩放，实现线性时间/内存解码，在保持Transformer级精度的同时获得1.6-1.9倍推理加速和38-42%内存节省。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的HTR系统存在KV缓存随序列增长而膨胀的问题，导致解码速度慢、内存占用高。需要一种既能保持高精度又能实现高效解码的替代架构。

Method: 1. 基于Retentive Networks构建仅解码器模型，用无softmax的retention机制替代注意力，消除KV缓存增长问题；2. 注入多尺度序列先验；3. 提出层间gamma缩放策略，在深层逐步扩大有效retention范围，恢复注意力从局部到全局的归纳偏置。

Result: 相比同等规模的仅解码器Transformer基线：推理速度提升1.6-1.9倍，内存使用减少38-42%，精度无损失。在多个数据集上取得SOTA结果：IAM-A 2.26%（英）、RIMES 1.81%（法）、Bentham 3.46%（英），READ-2016 4.21%（德）具有竞争力。

Conclusion: 基于RetNet的仅解码器架构能够在保持Transformer级识别精度的同时，显著提升手写文本识别的解码效率和内存效率，为实际部署提供了更优的解决方案。
...
TL;DR: SpectralGCD是一种高效的多模态广义类别发现方法，使用CLIP跨模态相似性作为统一表示，通过谱过滤保留相关概念，实现与SOTA相当或更优的性能但计算成本显著降低。


<details>
  <summary>Details</summary>
Motivation: 广义类别发现(GCD)旨在利用少量已知类标注数据识别未标注数据中的新类别。现有方法存在两个主要问题：1）仅基于图像特征的参数化分类器容易对旧类别过拟合；2）最近的多模态方法虽然通过引入文本信息提升了性能，但将模态独立处理且计算成本高昂。

Method: 提出SpectralGCD方法：1）使用CLIP跨模态图像-概念相似性作为统一的跨模态表示，将每个图像表示为大型任务无关字典中语义概念的混合；2）引入谱过滤技术，利用强教师模型计算的softmax相似性的跨模态协方差矩阵自动保留字典中相关概念；3）通过前向和反向知识蒸馏确保学生模型的跨模态表示既语义充分又对齐良好。

Result: 在六个基准测试中，SpectralGCD实现了与最先进方法相当或显著更优的准确率，同时计算成本仅为这些方法的一小部分。代码已公开可用。

Conclusion: SpectralGCD通过统一的跨模态表示、谱过滤概念选择和知识蒸馏，提供了一种高效有效的广义类别发现解决方案，在保持语义质量的同时大幅降低了计算复杂度。
...
### [75] [A High-Level Survey of Optical Remote Sensing](https://arxiv.org/abs/2602.17397)
*Panagiotis Koletsis,Vasilis Efthymiou,Maria Vakalopoulou,Nikos Komodakis,Anastasios Doulamis,Georgios Th. Papadopoulos*

Main category: cs.CV

TL;DR: 该论文是一篇关于光学遥感领域无人机RGB相机应用的综合性综述，旨在为进入该领域的研究者提供全面的能力概述、数据集信息和关键见解。


<details>
  <summary>Details</summary>
Motivation: 近年来计算机视觉的进步推动了遥感技术的发展，同时无人机的普及使得RGB相机成为最常用且易于使用的传感器。光学遥感文献庞大且分散，缺乏一个从整体视角出发的综述来指导新研究者快速了解领域全貌和核心资源。

Method: 采用文献综述方法，对光学遥感领域（特别是基于无人机RGB相机的应用）进行全面梳理，涵盖不同任务、能力和方法学，同时收集整理关键数据集和领域见解。

Result: 提供了光学遥感领域的全面能力概述，包括各种任务和方法学的分类整理，以及重要的数据集资源和领域关键见解，填补了现有文献中缺乏整体视角综述的空白。

Conclusion: 该综述为遥感领域的新研究者提供了有价值的入门指南，通过高层次的见解帮助他们聚焦最相关的研究方向，并建立了该领域的整体知识框架，对推动光学遥感研究具有重要意义。
...
### [76] [EAGLE: Expert-Augmented Attention Guidance for Tuning-Free Industrial Anomaly Detection in Multimodal Large Language Models](https://arxiv.org/abs/2602.17419)
*Xiaomeng Peng,Xilang Huang,Seon Han Choi*

Main category: cs.CV

TL;DR: EAGLE是一种无需调优的工业异常检测框架，通过专家模型输出指导多模态大语言模型实现准确检测和可解释的异常描述


<details>
  <summary>Details</summary>
Motivation: 工业异常检测对智能制造很重要，但现有深度学习方法只能提供二元决策且语义解释有限。多模态大语言模型能生成细粒度语言分析，但现有方法需要昂贵的微调，且相比轻量级专家检测器并不总能提高检测精度。

Method: 提出EAGLE框架，无需调优，通过集成专家模型输出来指导多模态大语言模型，实现准确检测和可解释的异常描述。进一步研究EAGLE如何影响MLLMs内部机制，通过检查中间层对异常图像区域的注意力分布。

Result: 在MVTec-AD和VisA数据集上的实验表明，EAGLE无需参数更新就能提高多种MLLMs的异常检测性能，达到与基于微调方法相当的结果。成功异常检测与异常区域注意力集中度增加相关，EAGLE倾向于促进这种对齐。

Conclusion: EAGLE提供了一种无需调优的框架，有效结合专家模型和多模态大语言模型的优势，在保持检测精度的同时提供语义丰富的异常描述，为工业异常检测提供了新的解决方案。
...
### [77] [4D Monocular Surgical Reconstruction under Arbitrary Camera Motions](https://arxiv.org/abs/2602.17473)
...
<details>
  <summary>Details</summary>
Motivation: 现有内窥镜视频重建方法主要针对固定视角的可变形场景，依赖立体深度先验或精确SfM进行初始化，难以处理真实临床环境中单目序列的大相机运动问题

Method: 提出渐进式窗口化全局表示，为每个观察窗口分配局部可变形场景模型；设计从粗到精策略整合多视图几何、跨窗口信息和单目深度先验；引入长距离2D像素轨迹约束和物理运动先验

Result: 在三个公开内窥镜数据集上的实验表明，Local-EndoGS在视觉质量和几何精度上持续优于现有方法，消融研究验证了关键设计的有效性

Conclusion: Local-EndoGS能够处理具有任意相机运动的单目内窥镜序列，实现高质量4D重建，为临床环境中的内窥镜场景重建提供了有效解决方案
...
### [78] [QuPAINT: Physics-Aware Instruction Tuning Approach to Quantum Material Discovery](https://arxiv.org/abs/2602.17478)
*Xuan-Bac Nguyen,Hoang-Quan Nguyen,Sankalp Pandey,Tim Faltermeier,Nicholas Borys,Hugh Churchill,Khoa Luu*

Main category: cs.CV

TL;DR: 该论文提出了一个物理感知的多模态框架，用于从光学显微镜图像中表征二维量子材料，解决了层依赖对比度微弱、标记数据有限以及实验室间差异大的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型在二维量子材料表征中存在三个主要问题：缺乏物理先验知识、无法泛化到新材料或硬件条件、以及标记数据稀缺。这些限制阻碍了从光学显微镜图像中准确识别材料厚度和类型的能力。

Method: 方法包括四个核心组件：1) Synthia - 基于物理的合成数据生成器，模拟量子材料薄片在薄膜干涉下的光学响应；2) QMat-Instruct - 首个大规模量子材料指令数据集，包含多模态、物理感知的问答对；3) QuPAINT - 物理感知指令调优架构，集成物理感知注意力模块融合视觉嵌入和光学先验；4) QF-Bench - 涵盖多种材料、基底和成像设置的综合性基准测试。

Result: 该框架通过合成数据减少了对专家手动标注的依赖，通过物理感知注意力模块获得了更鲁棒和判别性的薄片表征，并通过标准化基准实现了公平可重复的评估。

Conclusion: 该工作提出的物理感知多模态框架成功解决了二维量子材料光学表征中的关键挑战，通过结合物理模拟、大规模指令数据和专门设计的架构，显著提升了模型在跨材料和跨实验室条件下的泛化能力。
...
### [79] [Tracing Copied Pixels and Regularizing Patch Affinity in Copy Detection](https://arxiv.org/abs/2602.17484)
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在图像复制检测中面临精细对应学习不足的问题，无法有效处理复杂编辑操作。需要利用编辑内容中固有的几何可追踪性来改进特征表示学习。

Method: 提出PixTrace像素坐标跟踪模块，保持跨编辑变换的显式空间映射；引入CopyNCE几何引导对比损失，使用PixTrace验证映射的重叠比率来正则化块亲和度。将像素级可追踪性与块级相似性学习相结合，抑制自监督训练中的监督噪声。

Result: 在DISC21数据集上取得最先进性能：匹配器达到88.7% uAP / 83.9% RP90，描述符达到72.6% uAP / 68.4% RP90。相比现有方法具有更好的可解释性。

Conclusion: 通过利用编辑内容中的几何可追踪性，提出的PixTrace和CopyNCE方法有效改进了图像复制检测，在性能和可解释性方面均优于现有自监督学习方法。
...
### [80] [FoundationPose-Initialized 3D-2D Liver Registration for Surgical Augmented Reality](https://arxiv.org/abs/2602.17517)
...
TL;DR: 该论文提出了一种用于腹腔镜肝脏手术中肿瘤定位的增强现实配准新方法，使用深度增强基础姿态估计器和非刚性迭代最近点替代传统的有限元变形模型，降低了工程复杂性和专业知识要求。


<details>
  <summary>Details</summary>
Motivation: 现有腹腔镜肝脏手术中的增强现实配准方法通常依赖器官轮廓，且变形配准多采用有限元模型结合降维或机器学习组件，这些方法工程复杂且需要专业知识。需要开发更轻量、工程友好的替代方案。

Method: 1. 整合腹腔镜深度图与基础姿态估计器进行相机-肝脏姿态估计；2. 用非刚性迭代最近点替代有限元变形模型；3. 采用刚性-NICP组合配准方法，降低工程建模复杂性和专业知识要求。

Result: 在真实患者数据上，深度增强基础姿态方法在3个病例中实现了9.91毫米的平均配准误差。刚性-NICP组合配准优于纯刚性配准，证明NICP可作为有限元变形模型的有效替代。

Conclusion: 该配准管道在达到临床相关精度的同时，提供了比有限元变形模型更轻量、工程友好的替代方案，降低了技术门槛和专业知识要求。
...
### [81] [LATA: Laplacian-Assisted Transductive Adaptation for Conformal Uncertainty in Medical VLMs](https://arxiv.org/abs/2602.17535)
*Behzad Bozorgtabar,Dwarikanath Mahapatra,Sudipta Roy,Muzammal Naseer,Imran Razzak,Zongyuan Ge*

Main category: cs.CV

TL;DR: LATA是一种无需训练和标签的Laplacian辅助转导适应方法，通过图像-图像k-NN图上的平滑操作改进医学视觉语言模型的零样本预测，结合故障感知的conformal评分提高预测集效率和类别平衡，同时保持split conformal prediction的有效性保证。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在领域偏移下的可靠性依赖于具有保证的校准不确定性。虽然split conformal prediction提供有限样本覆盖保证，但预测集通常过大（效率低）且类别覆盖不平衡（高类别条件覆盖差距），特别是在少样本、不平衡场景中。此外，简单地适应校准标签会破坏可交换性并导致保证失效。

Method: 提出LATA（Laplacian-Assisted Transductive Adaptation），这是一种无需训练和标签的细化方法，通过图像-图像k-NN图上的Laplacian平滑零样本概率，使用少量CCCP平均场更新。引入故障感知conformal评分，结合视觉语言不确定性框架，提供实例级难度和标签合理性。该方法为黑盒（无需更新VLM）、计算轻量（窗口化转导、无反向传播），并包含可选先验旋钮，可在严格无标签或标签知情变体下运行。

Result: 在三个医学VLMs和九个下游任务上，LATA一致地减少预测集大小和类别条件覆盖差距，同时匹配或收紧目标覆盖率，优于先前的转导基线方法，缩小了与使用标签方法的差距，同时计算量显著减少。全面的消融实验和定性分析表明，LATA在不损害可交换性的情况下锐化了零样本预测。

Conclusion: LATA提供了一种有效且实用的方法，通过转导适应改进医学视觉语言模型的零样本预测可靠性，在保持split conformal prediction有效性的同时，显著提高预测集效率和类别平衡，为医学影像的可靠不确定性量化提供了有前景的解决方案。
...
### [82] [GraphThinker: Reinforcing Video Reasoning with Event Graph Thinking](https://arxiv.org/abs/2602.17555)
...
<details>
  <summary>Details</summary>
Motivation: 视频推理需要理解事件间的因果关系，但这些关系通常是隐式的且标注成本高。现有MLLM通过密集字幕或视频摘要推断事件关系，但缺乏因果结构建模，导致推理时出现幻觉问题。

Method: 提出GraphThinker方法：1）使用MLLM构建事件级视频场景图（EVSG），显式建模事件内和事件间关系，并将场景图作为中间思考过程整合到MLLM中；2）在强化微调中引入视觉注意力奖励，增强视频定位能力。
...
### [83] [Art2Mus: Artwork-to-Music Generation via Visual Conditioning and Large-Scale Cross-Modal Alignment](https://arxiv.org/abs/2602.17599)
*Ivan Rinaldi,Matteo Mendula,Nicola Fanelli,Florence Levé,Matteo Testi,Giovanna Castellano,Gennaro Vessio*

Main category: cs.CV

TL;DR: ArtToMus：首个直接从艺术作品生成音乐的框架，无需图像到文本转换，使用视觉嵌入直接指导潜在扩散模型生成音乐
...
Method: 1）构建ArtSound数据集：105,884个艺术作品-音乐对，包含双模态描述；2）提出ArtToMus框架：将视觉嵌入投影到潜在扩散模型的条件空间中，实现无需图像到文本转换或基于语言语义监督的直接艺术作品到音乐生成
...
### [84] [Adapting Actively on the Fly: Relevance-Guided Online Meta-Learning with Latent Concepts for Geospatial Discovery](https://arxiv.org/abs/2602.17605)
...
<details>
  <summary>Details</summary>
Motivation: 在环境监测、灾害响应、公共卫生等实际场景中，数据收集成本高、环境动态变化，需要在资源约束下战略性地从未观测区域采样以发现隐藏目标。现有基于学习的方法（如强化学习）受限于稀疏且有偏差的地理空间地面真值数据。

Method: 提出统一的地理空间发现框架，整合主动学习、在线元学习和概念引导推理。核心创新包括：1）概念加权不确定性采样策略，基于领域特定概念（如土地覆盖、源距离）的习得相关性来调节不确定性；2）相关性感知元批次形成策略，在在线元更新中促进语义多样性，提升动态环境中的泛化能力。

Result: 在真实世界数据集（致癌PFAS污染物）上进行测试，展示了该方法在有限数据和变化环境下可靠发现目标的能力。

Conclusion: 该框架通过利用领域特定概念的相关性，有效解决了地理空间发现中数据稀缺和动态环境的挑战，为实际应用提供了可靠且高效的解决方案。
...
### [85] [CORAL: Correspondence Alignment for Improved Virtual Try-On](https://arxiv.org/abs/2602.17636)
*Jiyoung Kim,Youngjin Shin,Siyoon Jin,Dahyun Chung,Jisu Nam,Tongmin Kim,Jongjae Park,Hyeonwoo Kang,Seungryong Kim*

Main category: cs.CV

TL;DR: CORAL：基于DiT的虚拟试穿框架，通过显式对齐查询-键匹配与外部对应关系来提升服装细节保留
...
Method: 分析DiT中完整3D注意力机制，发现人-服装对应关系依赖精确的查询-键匹配；提出CORAL框架，包含对应关系蒸馏损失（对齐可靠匹配与人-服装注意力）和熵最小化损失（锐化注意力分布）

Result: CORAL相比基线方法持续改进，提升了全局形状传递和局部细节保留；广泛消融实验验证了设计选择的有效性；提出基于VLM的评估协议以更好反映人类偏好

Conclusion: 通过显式对齐查询-键匹配与外部对应关系，CORAL框架有效解决了虚拟试穿中服装细节保留不足的问题，为DiT架构中的对应关系形成提供了理论解释
...
### [86] [IntRec: Intent-based Retrieval with Contrastive Refinement](https://arxiv.org/abs/2602.17639)
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇检测器采用一次性方式，缺乏基于用户反馈精炼预测的能力，特别是在查询模糊或涉及多个相似对象时难以准确检索

Method: 提出IntRec交互框架，核心是意图状态(IS)维护正锚点(确认线索)和负约束(拒绝假设)的双重记忆集，通过对比对齐函数对候选对象排序，最大化与正线索相似性同时惩罚被拒绝假设

Result: 在LVIS上达到35.4 AP，优于OVMR(+2.3)、CoDet(+3.7)和CAKE(+0.5)；在LVIS-Ambiguous基准上，单次纠正反馈后比一次性基线提升+7.9 AP，每次交互增加延迟小于30毫秒
...
### [87] [When Vision Overrides Language: Evaluating and Mitigating Counterfactual Failures in VLAs](https://arxiv.org/abs/2602.17659)
...
<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在实际应用中经常无法忠实跟随语言指令，当指令缺乏场景特定监督时，模型会基于数据集偏差产生的视觉捷径采取行动，执行训练中常见的行为而忽略语言意图

Method: 提出反事实动作引导：一种简单的双分支推理方案，结合标准VLA策略和无语言条件的视觉-动作模块，通过反事实比较进行动作选择，无需额外演示或修改现有架构

Result: 在LIBERO-CF基准上，CAG将语言跟随准确率提高9.7%，任务成功率提高3.6%；结合VA模型后分别提升15.5%和8.5%；真实世界评估中减少9.4%反事实失败，平均任务成功率提高17.2%

Conclusion: 反事实失败是VLA模型中被忽视但普遍存在的问题，CAG作为一种即插即用的推理方案能有效改善语言跟随能力，增强对未充分观察任务的鲁棒性
...
### [88] [Intent Laundering: AI Safety Datasets Are Not What They Seem](https://arxiv.org/abs/2602.16729)
*Shahriar Golchin,Marc Wetter*

Main category: cs.CR

TL;DR: 研究发现当前AI安全数据集过度依赖"触发线索"（具有明显负面/敏感含义的词语），无法真实反映现实世界攻击，且一旦去除这些线索，所有"相对安全"的模型都会变得不安全。


<details>
  <summary>Details</summary>
Motivation: 评估广泛使用的AI安全数据集的质量，检验这些数据集是否真实反映现实世界攻击，以及它们是否真正衡量安全风险还是仅仅通过触发线索引发拒绝响应。

Method: 从两个角度系统评估：1）孤立评估数据集是否反映现实攻击（基于三个关键属性：恶意意图驱动、精心设计、分布外）；2）实践中评估数据集是否真正衡量安全风险。引入"意图清洗"方法：抽象掉攻击中的触发线索，同时严格保留恶意意图和相关细节。

Result: 1）当前AI安全数据集过度依赖触发线索，无法忠实代表现实世界攻击；2）去除触发线索后，所有先前评估为"相对安全"的模型（包括Gemini 3 Pro和Claude Sonnet 3.7）都变得不安全；3）将意图清洗作为越狱技术时，在黑盒访问下攻击成功率高达90%-98%。

Conclusion: 当前AI安全数据集与真实世界攻击行为存在显著脱节，模型安全评估方法需要改进以更好地反映现实威胁。
...
### [89] [Can Adversarial Code Comments Fool AI Security Reviewers -- Large-Scale Empirical Study of Comment-Based Attacks and Defenses Against LLM Code Analysis](https://arxiv.org/abs/2602.16741)
*Scott Thornton*

Main category: cs.CR

TL;DR: 对抗性注释对LLM漏洞检测性能影响有限，检测准确率未显著下降，复杂策略不优于简单操纵，失败主要源于漏洞本身难度而非对抗性注释


<details>
  <summary>Details</summary>
Motivation: 研究对抗性提示操纵是否会影响大型语言模型在漏洞检测任务中的性能，此前研究表明在代码生成任务中对抗性提示会降低LLM性能

Method: 构建包含Python、JavaScript和Java的100个样本基准，每个样本配对8种注释变体（从无注释到对抗性策略如权威欺骗和技术欺骗），评估8个前沿模型（5个商业、3个开源）共9,366次试验，测试4种自动化防御机制

Result: 对抗性注释对检测准确率产生微小且统计上不显著的影响（McNemar精确p>0.21；所有95%置信区间包含零），商业模型基线检测率89-96%，开源模型53-72%，复杂对抗策略不优于简单操纵性注释，静态分析交叉引用防御效果最佳（96.9%检测率），失败集中在固有困难漏洞类别

Conclusion: 与代码生成场景不同，对抗性注释操纵在漏洞检测中未显著降低性能，检测失败主要源于漏洞本身复杂性而非对抗性注释，简单防御机制如静态分析交叉引用可有效提升检测率
...
### [90] [The Vulnerability of LLM Rankers to Prompt Injection Attacks](https://arxiv.org/abs/2602.16752)
*Yu Yin,Shuai Wang,Bevan Koopman,Guido Zuccon*

Main category: cs.CR

TL;DR: 该论文对LLM排序器中的越狱提示攻击进行了全面实证研究，评估了不同模型、架构和设置下的脆弱性边界条件。


<details>
  <summary>Details</summary>
Motivation: LLM作为强大的重排序器存在安全风险，简单提示注入可显著改变其排序决策，但该脆弱性在不同LLM家族、架构和设置中的持续性尚未充分探索。

Method: 采用两个互补任务评估：偏好脆弱性评估（攻击成功率ASR）和排序脆弱性评估（nDCG@10质量影响）。系统研究了三种主流排序范式（成对、列表、集合）和两种注入变体（决策目标劫持、决策标准劫持），并扩展分析了模型家族缩放、位置敏感性、骨干架构和跨域鲁棒性。

Result: 研究揭示了这些脆弱性的边界条件，关键发现包括编码器-解码器架构对越狱攻击表现出强大的固有韧性。结果量化了不同设置下的攻击效果和影响程度。

Conclusion: 该研究为LLM排序器的安全评估提供了全面框架，揭示了架构选择对抵御提示注入攻击的重要性，特别是编码器-解码器架构的固有韧性，为构建更安全的LLM排序系统提供了指导。
...
### [91] [Large-scale online deanonymization with LLMs](https://arxiv.org/abs/2602.16800)
*Simon Lermen,Daniel Paleka,Joshua Swanson,Michael Aerni,Nicholas Carlini,Florian Tramèr*

Main category: cs.CR

TL;DR: LLM可实现大规模去匿名化攻击，仅凭伪匿名在线资料和对话就能高精度重新识别用户，性能远超传统方法


<details>
  <summary>Details</summary>
Motivation: 研究在线伪匿名用户的隐私保护现状，验证LLM在大规模去匿名化攻击中的有效性，重新评估在线隐私威胁模型

Method: 设计可扩展攻击流程：1) LLM提取身份相关特征；2) 语义嵌入搜索候选匹配；3) 推理验证匹配并减少误报。构建三个真实数据集评估攻击效果

Result: LLM方法在三个数据集上显著优于传统基线，在90%精度下达到68%召回率，而最佳非LLM方法接近0%。证明在线伪匿名用户的实际模糊性保护已失效

Conclusion: LLM能够以前所未有的规模和精度执行去匿名化攻击，在线隐私的威胁模型需要重新考虑，伪匿名用户的保护已不再有效
...
### [92] [NeST: Neuron Selective Tuning for LLM Safety](https://arxiv.org/abs/2602.16835)
*Sasha Behrouzi,Lichao Wu,Mohamadreza Rostami,Ahmad-Reza Sadeghi*

Main category: cs.CR

TL;DR: NeST是一个轻量级、结构感知的安全对齐框架，通过选择性调整安全相关神经元来增强拒绝行为，相比全微调和LoRA大幅减少参数更新量，同时实现更强的安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法存在成本高、更新困难、效率低等问题：全微调计算和存储开销大；LoRA等参数高效方法在安全增益上不一致且对设计选择敏感；断路器机制不直接塑造内部表示。这些限制阻碍了快速可靠的安全更新，特别是在模型频繁演化或需要适应新策略和领域的场景中。

Method: NeST通过选择性调整安全相关神经元子集来增强拒绝行为，同时冻结模型其余部分。该方法将参数更新与安全行为的内部组织对齐，通过聚类功能一致的安全神经元并在每个聚类内强制执行共享更新，实现有针对性和稳定的安全适应，无需广泛的模型修改或推理时开销。

Result: 在10个开源LLM上的评估显示，NeST将攻击成功率从平均44.5%降至4.36%，不安全生成减少90.2%，平均仅需0.44百万可训练参数。相比全微调参数更新减少17,310倍，相比LoRA减少9.25倍，同时在所有评估模型中一致实现更强的安全性能。

Conclusion: NeST提供了一个轻量级、结构感知的安全对齐框架，能够实现快速可靠的安全更新，特别适用于模型频繁演化或需要适应新策略和领域的场景，在保持高效的同时提供卓越的安全性能。
...
### [93] [Privacy-Preserving Mechanisms Enable Cheap Verifiable Inference of LLMs](https://arxiv.org/abs/2602.17223)
*Arka Pal,Louai Zahran,William Gvozdjak,Akilesh Potti,Micah Goldblum*

Main category: cs.CR

TL;DR: 该论文提出利用隐私保护LLM推理来实现低成本验证推理的新方法，避免传统零知识证明的高计算开销


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模增长，用户更多依赖第三方托管服务，但缺乏对推理提供商计算过程的保证。不诚实的提供商可能用廉价弱模型替代昂贵大模型，而现有验证方法（如零知识证明）计算开销大，不适用于大型模型

Method: 提出两种新协议：利用隐私保护LLM推理来提供推理过程保证。这些方法成本低廉，只需添加少量额外token计算，对下游影响小。基于隐私保护推理方法通常比零知识证明更快的特点，改进验证运行时间

Result: 开发了隐私与可验证性在LLM推理中的新连接，提供了边际成本极低的验证推理方案。相比传统零知识证明方法，显著降低了计算开销

Conclusion: 该工作揭示了隐私保护LLM推理与推理验证之间的新联系，提出了高效、低成本的验证协议，为大模型推理验证提供了实用解决方案
...
### [94] [Grothendieck Topologies and Sheaf-Theoretic Foundations of Cryptographic Security: Attacker Models and $Σ$-Protocols as the First Step](https://arxiv.org/abs/2602.17301)
*Takao Inoué*

Main category: cs.CR

TL;DR: 论文提出基于格罗滕迪克拓扑和层论的密码学安全结构重构，将攻击者观测建模为格罗滕迪克位点，协议转录本自然形成层，安全性质表现为几何条件。


<details>
  <summary>Details</summary>
Motivation: 传统密码学安全使用基于游戏或模拟的定义，本文旨在通过代数几何工具提供更结构化的安全表述，为模拟安全提供概念性解释并建立几何基础。

Method: 将攻击者观测建模为格罗滕迪克位点，覆盖族表示由高效模拟确定的部分信息的可容许分解。在此框架下，协议转录本形成层，安全性质表现为几何条件。以Σ-协议为重点，证明其转录本结构定义关联层拓扑中的挠子。

Result: 证明Σ-协议转录本结构定义层拓扑中的挠子：局部平凡性对应零知识性，全局截面的缺失反映可靠性。通过Schnorr Σ-协议的具体分析验证了该构造。

Conclusion: 层论视角为模拟安全提供了概念性解释，并为进一步的密码学抽象建立了几何基础，展示了代数几何工具在密码学安全形式化中的潜力。
...
### [95] [Security of the Fischlin Transform in Quantum Random Oracle Model](https://arxiv.org/abs/2602.17307)
...
<details>
  <summary>Details</summary>
Motivation: Fischlin变换在经典随机预言机模型中能产生具有直线可提取性的非交互式零知识证明，但它在量子随机预言机模型中的直线可提取性一直未得到证明，这阻碍了其在后量子安全中的应用

Method: 使用压缩预言机方法构建提取器，结合独立随机变量和的尾界、鞅的尾界、对称化、查询振幅和量子并集界等技术

Result: 证明了Fischlin变换在量子随机预言机模型中保持直线可提取性，为后量子安全提供了比Pass变换更小证明尺寸的直线可提取NIZK替代方案

Conclusion: Fischlin变换在量子随机预言机模型中具有直线可提取性，这确立了其后量子安全性，为后量子密码学提供了更高效的NIZK构造
...
TL;DR: DAVE是一个基于LLM的发言人系统，在多方数据空间中通过自然语言接口回答查询，同时强制执行使用策略，避免敏感信息泄露，而无需手动编辑文档。


<details>
  <summary>Details</summary>
Motivation: 当前组织间数据空间中的使用策略主要在资产级别执行：整个文档要么共享要么保留。当文档只有部分内容敏感时，提供者需要手动编辑文档以避免泄露受保护信息，这种方法成本高、粒度粗，且难以随着策略或合作伙伴的变化而维护。

Method: 提出了DAVE系统，这是一个基于LLM的使用策略强制执行发言人，代表数据提供者回答关于私有文档的问题。系统采用虚拟编辑方法：在查询时抑制敏感信息而不修改源文档。架构上集成了Eclipse Dataspace Components和ODRL风格策略，并实现了提供者侧的集成原型，将QA请求通过发言人服务路由而不是触发原始文档传输。

Result: 本文主要贡献是架构设计：尚未实现或实证评估完整的强制执行流程。因此提出了一个评估方法学，用于评估在良性和对抗性查询下的安全性、实用性和性能权衡，为未来关于多方数据空间中系统化治理LLM访问的实证工作奠定基础。

Conclusion: DAVE系统为多方数据空间提供了一种新颖的LLM访问控制方法，通过自然语言接口和虚拟编辑技术，在保持文档完整性的同时强制执行细粒度使用策略，解决了传统手动编辑方法的局限性。
...
### [97] [Privacy in Theory, Bugs in Practice: Grey-Box Auditing of Differential Privacy Libraries](https://arxiv.org/abs/2602.17454)
...
<details>
  <summary>Details</summary>
Motivation: 现有差分隐私验证方法存在局限性：形式化工具限制过多，黑盒统计审计对复杂流程不可行且无法定位错误源，导致DP实现中的隐私漏洞难以发现。

Method: 提出Re:cord-play灰盒审计范式，通过在有相同随机性的相邻数据集上运行插桩算法，检查数据依赖的控制流，并通过比较声明敏感度与内部输入间的经验距离来具体证伪敏感度违规。进一步扩展为Re:cord-play-sample，对每个组件进行完整统计审计。

Result: 审计了12个开源库（包括SmartNoise SDK、Opacus、Diffprivlib），发现了13个影响理论保证的隐私违规，证明该方法既有效又必要。

Conclusion: Re:cord-play提供了一种计算成本低、易于集成的隐私测试框架，使DP开发者能够在软件开发周期中进行有效的隐私验证。
...
### [98] [Coin selection by Random Draw according to the Boltzmann distribution](https://arxiv.org/abs/2602.17490)
*Jan Lennart Bönsel,Michael Maurer,Silvio Petriconi,Andrea Tundis,Marc Winstel*

Main category: cs.CR

TL;DR: 提出一种基于玻尔兹曼分布的随机算法用于代币选择问题，相比随机抽取和贪心算法在限制输入代币数量、减少粉尘生成和控制钱包代币池大小方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 代币选择问题在加密货币和央行数字货币等代币支付系统中至关重要，需要解决如何选择一组代币来资助交易。现有方法如随机抽取和贪心算法存在不足，需要更有效的算法来限制输入代币数量、减少粉尘生成并控制钱包代币池大小。

Method: 提出玻尔兹曼抽取算法，这是一种受统计物理学原理启发的概率算法。算法基于玻尔兹曼分布抽取代币，作为随机抽取方法的扩展和改进。该方法可以高效实现，提升性能并满足隐私要求。

Result: 数值结果表明该方法在限制所选输入代币数量、减少粉尘生成和限制钱包代币池大小方面有效。与标准随机抽取和贪心算法相比，玻尔兹曼抽取在以上目标方面表现更优。

Conclusion: 玻尔兹曼抽取算法对代币技术具有重要价值，特别对需要处理高频大交易量的央行数字货币有实际意义。该概率算法高效、性能优越且尊重隐私要求，是代币选择问题的有效解决方案。
...
### [99] [Non-Trivial Zero-Knowledge Implies One-Way Functions](https://arxiv.org/abs/2602.17651)
*Suvradip Chakraborty,James Hulett,Dakshita Khurana,Kabir Tomer*

Main category: cs.CR

TL;DR: 该论文证明了在NP⊈ioP/poly的假设下，非平凡（误差总和远离1）的非交互式和交互式零知识论证可推导出单向函数的存在性，填补了高误差区域的理论空白。


<details>
  <summary>Details</summary>
Motivation: 先前研究已证明在NP⊈ioP/poly的假设下，具有可忽略误差的零知识论证可推导出单向函数。然而，对于高误差区域（即零知识误差、完备性误差和可靠性误差总和接近1的情况），单向函数的存在性问题尚未解决。本文旨在填补这一理论空白，探索在误差参数较大时零知识与单向函数之间的关系。

Method: 定义"非平凡"零知识论证为完备性误差、可靠性误差和零知识误差的总和远离1的论证系统。在NP⊈ioP/poly的假设下，通过理论分析和证明技术，建立：1）非平凡非交互式零知识（NIZK）论证可推导出单向函数；2）非平凡常数轮公开抛币交互式零知识论证同样可推导出单向函数。利用已知的放大技术，可将弱NIZK无条件转换为标准NIZK。

Result: 主要结果：1）在NP⊈ioP/poly的假设下，非平凡NIZK论证可推导出单向函数的存在性；2）非平凡常数轮公开抛币交互式零知识论证同样可推导出单向函数；3）通过放大技术，可无条件地将弱NIZK转换为标准NIZK。这些结果完全解决了高误差区域（ε_zk + √ε_s ≥ 1）的开放问题，并扩展了交互式设置下的理论含义。

Conclusion: 本文首次在NP⊈ioP/poly的假设下，证明了非平凡（误差总和远离1）的零知识论证可推导出单向函数的存在性，完全填补了高误差区域的理论空白。这一结果为从最坏情况硬度构造单向函数提供了重要的理论基石和技术工具，扩展了零知识与密码学基础假设之间的深刻联系。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个开源工具来自动化高效估计因果和结构参数的过程，将多种现有方法统一到Riesz表示定理和去偏机器学习的框架下，简化复杂因果推断任务的实施。

Method: 基于Riesz表示定理和去偏机器学习，实现广义Riesz回归框架，通过最小化经验Bregman散度估计Riesz表示子。核心设计原则是自动回归器平衡（ARB），根据Bregman生成器和表示子模型类自动构建兼容的连接函数。提供模块化接口指定目标线性泛函、表示子模型和Bregman生成器。

Result: 开发了genriesz Python包，实现了回归调整、Riesz加权、增强Riesz加权和TMLE风格估计器，支持交叉拟合、置信区间和p值计算。包已开源并在PyPI上可用，支持平均处理效应、处理组平均处理效应和平均边际效应等典型因果推断问题。

Conclusion: genriesz提供了一个统一、自动化的框架，将多种因果推断方法整合到广义Riesz回归中，通过自动回归器平衡确保估计器满足平衡最优性条件，简化了复杂因果参数的估计过程。
...
### [101] [Beyond Procedure: Substantive Fairness in Conformal Prediction](https://arxiv.org/abs/2602.16794)
*Pengqi Liu,Zijun Yu,Mouloud Belbahri,Arthur Charpentier,Masoud Asgharian,Jesse C. Cresswell*

Main category: stat.ML

TL;DR: 该论文研究共形预测与公平性的交互作用，超越程序公平性分析下游决策的实质性公平，提出理论界限分解预测集大小差异，引入LLM评估器，实验显示标签聚类CP变体提供更好的实质性公平性。


<details>
  <summary>Details</summary>
Motivation: 共形预测虽然提供分布无关的不确定性量化，但其与下游决策公平性的交互作用研究不足。现有工作主要关注共形预测作为独立操作的程序公平性，而缺乏对整体决策流程中实质性公平（下游结果公平性）的分析。

Method: 1) 理论分析：推导预测集大小差异的上界，分解为可解释的组成部分，阐明标签聚类CP如何控制方法驱动的公平性贡献；2) 实证方法：引入LLM-in-the-loop评估器，近似人类对多模态实质性公平的评估；3) 实验设计：比较不同CP变体的实质性公平表现。

Result: 1) 标签聚类CP变体始终提供更优的实质性公平性；2) 预测集大小均等化（而非覆盖率均等化）与改善的实质性公平性强相关；3) LLM评估器能够有效近似人类评估，支持可扩展的实证分析。

Conclusion: 该研究为共形预测与公平性的交互提供了理论框架和实证工具，表明标签聚类CP方法能更好地控制实质性公平，且预测集大小均等化是设计更公平CP系统的关键指标，为实践者提供了实用指导。
...
### [102] [Poisson-MNL Bandit: Nearly Optimal Dynamic Joint Assortment and Pricing with Decision-Dependent Customer Arrivals](https://arxiv.org/abs/2602.16923)
*Junhui Cai,Ran Chen,Qitao Huang,Linda Zhao,Wu Zhu*

Main category: stat.ML

TL;DR: 论文提出Poisson-MNL模型，将上下文MNL选择模型与依赖于品类和价格的泊松到达模型结合，开发了基于UCB的高效算法PMNL，证明了√TlogT的遗憾界，并通过仿真验证了考虑到达率依赖性的重要性。


<details>
  <summary>Details</summary>
Motivation: 传统MNL模型假设顾客到达率固定，但现实中品类和定价不仅影响顾客购买选择，还会影响当期顾客到达数量。忽略这种依赖性会导致次优决策，因此需要开发能够同时学习顾客选择和到达模型的动态联合品类定价优化方法。

Method: 提出Poisson-MNL模型，将上下文MNL选择模型与泊松到达模型耦合，其中到达率依赖于提供的品类和价格。基于此模型开发了PMNL算法，采用上置信界(UCB)思想，在T个周期内动态更新品类和定价决策以最大化累计收益。

Result: 证明了PMNL算法具有√TlogT的非渐近遗憾界，并给出了匹配的下界（相差log T因子）。仿真研究表明，PMNL能有效学习顾客选择和到达模型，其联合品类定价决策优于假设固定到达率的方法，验证了考虑到达率依赖性的重要性。

Conclusion: Poisson-MNL模型和PMNL算法为动态联合品类定价问题提供了有效的解决方案，通过同时考虑品类定价对顾客到达和购买决策的影响，能够获得更优的收益表现。该研究强调了在收益管理中考虑到达率依赖性的必要性。
...
### [103] [Anti-causal domain generalization: Leveraging unlabeled data](https://arxiv.org/abs/2602.17187)
*Sorawit Saengkyongam,Juan L. Gamella,Andrew C. Miller,Jonas Peters,Nicolai Meinshausen,Christina Heinze-Deml*

Main category: stat.ML

TL;DR: 该论文研究反因果设置下的领域泛化问题，提出利用无标签数据估计环境扰动方向并正则化模型敏感性的方法，具有最坏情况最优性保证。


<details>
  <summary>Details</summary>
Motivation: 现有领域泛化方法通常需要来自多个训练环境的标记数据，这在标记数据稀缺时限制了其适用性。本文旨在解决反因果设置下的领域泛化问题，其中结果导致观察到的协变量，使得环境扰动影响协变量但不传播到结果。

Method: 在反因果结构下，环境扰动影响协变量但不传播到结果，这促使正则化模型对这些扰动的敏感性。关键的是，估计这些扰动方向不需要标签，因此可以利用来自多个环境的无标签数据。提出了两种方法：分别惩罚模型对协变量均值和协方差跨环境变化的敏感性，并证明这些方法在特定环境类别下具有最坏情况最优性保证。

Result: 在受控物理系统和生理信号数据集上展示了该方法的实证性能，验证了所提方法的有效性。

Conclusion: 该研究提出了一种在反因果设置下利用无标签数据进行领域泛化的新方法，通过正则化模型对环境扰动的敏感性，在标记数据稀缺的情况下实现了有效的领域泛化，并提供了理论保证。
...
### [104] [SOLVAR: Fast covariance-based heterogeneity analysis with pose refinement for cryo-EM](https://arxiv.org/abs/2602.17603)
*Roey Yadgar,Roy R. Lederman,Yoel Shkolnisky*

Main category: stat.ML

TL;DR: SOLVAR是一种基于随机优化的低秩变异性分析方法，用于高效估计冷冻电镜数据中的连续构象异质性，通过低秩协方差矩阵假设实现计算可处理性，并支持粒子图像姿态精修。


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜在解析大分子三维结构方面具有强大能力，但表征连续异质性（分子呈现连续构象状态）是一个关键挑战。基于协方差的方法为建模结构变异性提供了原则性方法，但高效估计协方差矩阵仍然是一个计算难题。

Method: SOLVAR利用协方差矩阵的低秩假设，为其主成分提供可处理的估计器。在低秩假设下，估计器可以表述为一个优化问题，能够快速准确地求解。该框架还支持输入粒子图像的姿态精修，这是大多数异质性分析方法和所有基于协方差方法所不具备的能力。

Result: 在合成和实验数据集上的数值实验表明，该算法能够准确捕捉变异性的主要成分，同时保持计算效率。SOLVAR在最近的异质性基准测试中，在多个数据集上实现了最先进的性能。

Conclusion: SOLVAR通过低秩协方差矩阵假设和随机优化方法，为冷冻电镜连续异质性分析提供了高效、准确的解决方案，同时具备姿态精修能力，在计算效率和准确性方面均表现出色。
...
### [105] [Support Recovery and $\ell_2$-Error Bound for Sparse Regression with Quadratic Measurements via Weakly-Convex-Concave Regularization](https://arxiv.org/abs/2602.17466)
*Jun Fan,Jingyu Yang,Xinyu Zhang,Liqun Wang*

Main category: math.ST

TL;DR: 该论文研究了高维二次测量模型中弱凸-凹正则化估计器的有限样本性质，建立了局部极小值的支撑恢复和ℓ₂误差界，并提出了两种近端梯度求解策略。


<details>
  <summary>Details</summary>
Motivation: 从二次测量中恢复未知信号在相位恢复、电力系统状态估计和无标签距离几何等领域有广泛应用。现有研究需要在高维设置下建立具有理论保证的正则化估计器。

Method: 采用弱凸-凹惩罚最小二乘方法，建立了局部极小值的支撑恢复和ℓ₂误差界。为解决优化问题，提出了两种近端梯度策略：一种是闭式解算近端步，另一种是通过加权ℓ₁近似计算。

Result: 建立了高维二次测量模型中弱凸-凹正则化估计器的有限样本理论保证，包括支撑恢复和ℓ₂误差界。数值实验验证了所提方法的有效性。

Conclusion: 该研究为高维二次测量问题提供了具有理论保证的正则化估计框架，所提出的弱凸-凹惩罚方法和近端梯度策略在实际应用中表现出良好性能。
...
### [106] [Asymptotically Optimal Sequential Testing with Markovian Data](https://arxiv.org/abs/2602.17587)
*Alhad Sethi,Kavali Sofia Sagar,Shubhada Agrawal,Debabrota Basu,P. N. Karthik*

Main category: math.ST

TL;DR: 该论文研究了马尔可夫链数据的单边和α-正确序贯假设检验，建立了非渐近的实例依赖下界，并提出了匹配该下界的渐近最优检验方法。
...
Method: 方法包括：1) 建立任意有效序贯检验在备择假设下期望停止时间的紧非渐近实例依赖下界；2) 提出一种最优检验，其期望停止时间在α→0时渐近匹配该下界；3) 分析结合了平稳分布和未知马尔可夫链诱导的转移结构。

Result: 主要结果：1) 建立了紧的非渐近实例依赖下界，改进了现有渐近或次优下界；2) 提出的最优检验在α→0时渐近匹配该下界；3) 框架成功应用于马尔可夫链蒙特卡洛模型误设的序贯检测和马尔可夫决策过程中转移动态线性等结构性质的检验。

Conclusion: 结论：该研究为马尔可夫依赖下的最优序贯检验程序提供了尖锐而一般的特征描述，提出的框架在模型误设检测和结构性质检验等应用中具有实用价值。
...
TL;DR: 提出一个通用的多模态框架，用于检测和减少各种数据类型中的冗余信息（ballast），通过综合多种分析方法识别并消除低效用特征，显著降低维度、存储需求和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现代数据集常包含大量冗余或低效用信息（ballast），这些信息增加了维度、存储需求和计算成本，却不提供有意义的分析价值，因此需要系统性的方法来识别和消除这些冗余。

Method: 提出一个通用的多模态框架，使用熵、互信息、Lasso、SHAP、PCA、主题建模和嵌入分析等多种方法，结合提出的Ballast Score指标，形成统一的跨模态剪枝策略，适用于结构化、半结构化、非结构化和稀疏数据。

Result: 实验结果显示，在稀疏或半结构化数据中，超过70%的特征空间可以被剪枝，同时保持甚至改善分类性能，显著减少训练时间和内存占用。框架还揭示了不同的ballast类型（统计、语义、基础设施等）。

Conclusion: 该研究提供了一个实用的多模态框架，能够有效识别和消除数据集中的冗余信息，为构建更精简、更高效的机器学习管道提供指导，显著提升计算效率和存储利用率。
...
TL;DR: 提出模块化LLM训练框架，通过门控机制组合领域专家模型，无需启发式数据混合调优，理论上可超越单体模型性能


<details>
  <summary>Details</summary>
Motivation: 大规模生成模型训练资源密集且依赖启发式数据集加权，需要解决能否模块化训练LLM以及能否对任意数据混合实现鲁棒性能的问题

Method: 提出模块化生成建模理论框架，定义归一化门控函数空间，构建极小极大博弈寻找鲁棒门控，证明存在性，引入可扩展随机原始对偶算法和结构蒸馏方法

Result: 理论证明模块化方法可作为强正则化器，泛化界随轻量门控复杂度缩放，理论上可超越聚合数据重训练模型，实验证实能有效缓解梯度冲突并超越单体基线

Conclusion: 模块化架构通过组合预训练专家实现鲁棒性能，无需启发式调优，为高效LLM训练提供了理论保证和实用方法
...
TL;DR: 提出首个基于e值的文本水印框架Anchored E-Watermarking，通过锚定分布优化采样策略，实现任意时间有效的假设检验，显著提升检测效率。


<details>
  <summary>Details</summary>
Motivation: 现有统计水印方法存在两个关键局限：缺乏选择采样分布的规范方法，以及依赖固定时间假设检验，无法支持有效的早期停止。需要开发既能优化采样又能支持任意时间有效推断的统一框架。

Method: 开发基于e值的水印框架，通过锚定分布近似目标模型，构建检测过程的测试超鞅，表征最坏情况对数增长率下的最优e值，并推导最优期望停止时间。

Result: 理论分析通过模拟和基准测试验证，框架能显著提升样本效率，相比最先进基线方法，检测所需的平均token预算减少13-15%。

Conclusion: Anchored E-Watermarking框架首次统一了最优采样与任意时间有效推断，解决了传统方法中可选停止使I类错误保证失效的问题，为机器生成内容检测提供了更高效可靠的方法。
...
### [110] [MMCAformer: Macro-Micro Cross-Attention Transformer for Traffic Speed Prediction with Microscopic Connected Vehicle Driving Behavior](https://arxiv.org/abs/2602.16730)
*Lei Han,Mohamed Abdel-Aty,Younggun Kim,Yang-Jun Joo,Zubayer Islam*

Main category: cs.LG

TL;DR: 本文提出MMCAformer模型，通过交叉注意力机制融合宏观交通流特征与微观驾驶行为特征，用于交通速度预测，在四个佛罗里达高速公路上验证了其优越性能。
...
Result: 在四个佛罗里达高速公路上验证，相比仅使用宏观特征，引入微观驾驶行为特征使整体RMSE、MAE和MAPE分别降低9.0%、6.9%和10.2%，模型预测不确定性降低10.1-24.0%。硬刹车和加速频率是最具影响力的特征，改进在拥堵、低速交通条件下更显著。

Conclusion: MMCAformer通过融合宏观交通流和微观驾驶行为特征，显著提升了交通速度预测的准确性和可靠性，特别是在拥堵条件下，为主动交通管理提供了更有效的工具。
...
### [111] [A Few-Shot LLM Framework for Extreme Day Classification in Electricity Markets](https://arxiv.org/abs/2602.16735)
...
Result: 在德州电力市场历史数据上，该方法性能与支持向量机和XGBoost相当，在历史数据有限时表现更优，证明了LLMs在数据稀缺场景下的有效性

Conclusion: 大语言模型可作为数据高效工具用于电价尖峰分类，在数据稀缺环境中具有应用潜力
...
### [112] [Real-time Secondary Crash Likelihood Prediction Excluding Post Primary Crash Features](https://arxiv.org/abs/2602.16739)
...
### [113] [Quantifying LLM Attention-Head Stability: Implications for Circuit Universality](https://arxiv.org/abs/2602.16740)
*Karan Bali,Jack Stanley,Praneet Suresh,Danilo Bzdok*

Main category: cs.LG

TL;DR: 该研究系统分析了Transformer语言模型中注意力头在不同训练实例间的稳定性，发现中层头最不稳定但表征最独特，深层模型的中层发散更强，不稳定的深层头功能更重要，权重衰减优化能显著提升稳定性，残差流相对稳定。


<details>
  <summary>Details</summary>
Motivation: 在机制可解释性研究中，虽然已识别出Transformer的"电路"（稀疏、单层或多层子计算），但这些电路在不同架构实例间的稳定性尚未得到充分验证。缺乏这种验证使得无法确定报告的电路是普遍存在的还是特定训练实例的偶然产物，这在安全关键场景中限制了可信度。

Method: 系统研究不同规模和复杂度的Transformer语言模型在多次独立初始化训练中的稳定性。逐层量化注意力头在独立训练运行中学习表征的相似性，进行严格的实验分析。

Result: 发现五个关键结果：1）中层注意力头最不稳定但表征最独特；2）更深层的模型表现出更强的中层发散；3）深层中不稳定的头比同层其他头功能更重要；4）应用权重衰减优化能显著提高注意力头在不同随机初始化间的稳定性；5）残差流相对稳定。

Conclusion: 电路在不同实例间的鲁棒性是实现可扩展监督的重要前提，但这一特性尚未得到充分重视。研究结果勾勒了AI系统可能实现白盒监控的边界，为机制可解释性研究提供了稳定性验证框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLVR训练数据集主要来自小规模人工构建或现有资源重组，数据多样性和覆盖范围有限，制约了模型性能的进一步提升

Method: 构建DeepVision-103K数据集，涵盖多样化的K12数学主题、广泛的知识点和丰富的视觉元素，用于RLVR训练

Result: 在DeepVision上训练的模型在多模态数学基准测试中表现优异，并能有效泛化到一般多模态推理任务；分析显示训练模型具有增强的视觉感知、反思和推理能力
...
TL;DR: PETS提出了一种原则性且高效的测试时自一致性方法，通过优化框架进行轨迹分配，在有限预算下提升模型性能。
...
Method: 引入自一致性率作为新度量，将轨迹分配问题形式化为优化框架。研究离线和在线两种设置：离线场景将推理轨迹建模为众包工作者，利用多数投票算法；在线场景提出自适应预算分配方法，根据问题难度动态调整。

Result: PETS在GPQA数据集上实现了完美的自一致性，同时相对于均匀分配减少了75%（离线）和55%（在线）的采样预算。

Conclusion: PETS为测试时自一致性提供了理论基础和高效算法，在有限预算下显著优于均匀分配策略。
...
TL;DR: 该研究通过几何分析揭示了transformer在模块化算术任务中grokking现象的机制：训练主要发生在低维执行子空间内，横向曲率增长先于泛化出现，且沿学习子空间的运动是grokking的必要条件。


<details>
  <summary>Details</summary>
Motivation: 理解grokking现象——在小规模算法任务中从记忆到泛化的延迟转变机制，这一现象目前仍缺乏深入理解。

Method: 1. 对transformer在模块化算术任务上的注意力权重轨迹进行PCA分析，识别低维执行子空间；2. 测量交换子缺陷（连续梯度步的非交换性）来探测损失景观几何；3. 将曲率投影到学习子空间；4. 进行因果干预实验，验证子空间运动与曲率增长的作用。

Result: 1. 训练主要发生在低维执行子空间内（单个主成分解释68-83%方差）；2. 横向（正交于子空间）曲率增长先于泛化出现，且领先时间服从幂律关系；3. 沿学习子空间的运动是grokking的必要条件，但人为增加曲率不足以引发泛化；4. 所有发现在不同学习率范围、慢速训练机制和随机种子下均可复现。

Conclusion: grokking反映了从亚稳态机制逃逸的过程，该机制特征为低维约束和横向曲率积累。几何分析为理解这一现象提供了新视角，表明训练动态的几何特性在泛化转变中起关键作用。
...
### [117] [Fail-Closed Alignment for Large Language Models](https://arxiv.org/abs/2602.16977)
*Zachary Coalson,Beth Sohler,Aiden Gabriel,Sanghyun Hong*

Main category: cs.LG

TL;DR: 论文提出当前LLM对齐存在"故障开放"的结构性弱点，并提出"故障封闭"对齐作为鲁棒安全设计原则，通过渐进式对齐框架实现多独立拒绝路径，增强抗越狱攻击能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的对齐机制存在结构性弱点：现代拒绝机制是"故障开放"的。现有方法将拒绝行为编码在多个潜在特征中，但通过提示词越狱抑制单个主导特征就能导致对齐崩溃，产生不安全生成。这促使研究者寻求更鲁棒的安全设计原则。

Method: 提出"故障封闭"对齐设计原则：拒绝机制应在部分故障时仍保持有效，通过冗余、独立的因果路径实现。具体实现为渐进式对齐框架：迭代识别并消融先前学习的拒绝方向，迫使模型在新的独立子空间中重建安全性。

Result: 在四种越狱攻击测试中，该方法实现了最强的整体鲁棒性，同时缓解了过度拒绝问题并保持了生成质量，计算开销较小。机制分析证实，经该方法训练的模型编码了多个因果独立的拒绝方向，提示词越狱无法同时抑制所有这些方向。

Conclusion: "故障封闭"对齐为鲁棒LLM安全提供了原则性基础。渐进式对齐框架通过强制模型学习多个独立拒绝路径，有效抵御越狱攻击，为构建更安全的语言模型提供了新思路。
...
TL;DR: LiveClin是一个动态更新的医学LLM基准测试，从当代同行评审病例报告构建，每半年更新一次，包含1407个病例报告和6605个问题，旨在解决数据污染和知识过时问题，评估模型在真实临床场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前医学LLM评估面临数据污染和知识过时两大挑战，导致静态基准测试得分虚高，无法准确反映模型在真实临床实践中的表现。需要建立一个动态、临床相关的评估框架来指导医学LLM的发展。

Method: 1. 从当代同行评审病例报告构建基准测试，每半年更新一次；2. 采用经过验证的AI-人类工作流程，涉及239名医生；3. 将真实患者病例转化为涵盖整个临床路径的复杂多模态评估场景；4. 包含1407个病例报告和6605个问题；5. 评估26个模型并与人类专家对比。

Result: 1. 模型在真实临床场景中表现困难，最佳模型仅达到35.7%的病例准确率；2. 人类专家表现优于大多数模型：主任医师准确率最高，主治医师紧随其后；3. 基准测试揭示了医学LLM与临床实践之间的显著差距。

Conclusion: LiveClin提供了一个持续演化、临床基础的框架，能够更可靠地评估医学LLM的真实世界效用，指导模型开发缩小与临床实践的差距，数据与代码已公开可用。
...
### [119] [Discovering Universal Activation Directions for PII Leakage in Language Models](https://arxiv.org/abs/2602.16980)
*Leo Marchyok,Zachary Coalson,Sungho Keum,Sooel Son,Sanghyun Hong*

Main category: cs.LG

TL;DR: UniLeak框架通过识别语言模型残差流中的通用激活方向，能够在不访问训练数据或真实PII的情况下，通过线性添加这些方向来一致增加PII泄露概率，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型具有丰富的内部结构，但隐私敏感行为（如个人身份信息泄露）在其隐藏状态中的表示和调控机制尚不清楚。需要理解PII泄露如何在模型内部表示，以及如何通过机制解释方法识别和调控这些行为。

Method: 提出UniLeak机制解释框架：1）识别模型残差流中的通用激活方向——这些方向在推理时线性添加能一致增加PII生成概率；2）仅依赖模型自生成文本，无需访问训练数据或真实PII；3）这些方向具有跨上下文泛化能力，且对生成质量影响最小。

Result: 在多个模型和数据集上的实验表明：1）沿通用方向引导显著增加了PII泄露，优于现有基于提示的提取方法；2）这些方向能放大PII生成概率，同时保持生成质量；3）揭示了PII泄露是模型表示中潜在信号的叠加。

Conclusion: PII泄露是模型表示中潜在信号的叠加现象，UniLeak框架为此提供了新视角，既能放大风险也能实现缓解。该发现为理解语言模型隐私泄露机制提供了机制解释基础，并为风险检测和缓解提供了新方法。
...
### [120] [Attending to Routers Aids Indoor Wireless Localization](https://arxiv.org/abs/2602.16762)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于机器学习的Wi-Fi定位方法在不同环境中难以取得突破性性能，主要问题在于大多数算法在聚合不同路由器信息时未能适当加权，导致收敛次优和精度降低。受传统加权三角定位方法启发，需要为不同路由器分配不同权重。

Method: 引入路由器注意力概念，确保在聚合多个路由器信息进行三角定位时，每个路由器的贡献被不同地加权。将注意力层集成到标准机器学习定位架构中，强调每个路由器的相关性。

Result: 在开源数据集上的评估表明，Attention to Routers方法比基准架构在精度上提升了超过30%。

Conclusion: 通过为不同路由器信息分配不同权重的注意力机制，可以显著提升Wi-Fi定位系统的整体性能，解决了传统方法中信息聚合不当的问题。
...
### [121] [Omitted Variable Bias in Language Models Under Distribution Shift](https://arxiv.org/abs/2602.16784)
*Victoria Lin,Louis-Philippe Morency,Eli Ben-Michael*

Main category: cs.LG

TL;DR: 该论文提出一个框架，将语言模型中的分布偏移分解为可观测和不可观测部分，通过建立不可观测变量强度与最坏情况泛化性能边界的关系，改进OOD评估和优化。


<details>
  <summary>Details</summary>
Motivation: 尽管现代语言模型在各种任务上表现出色，但对分布偏移仍然脆弱。现有方法只处理可观测的分布偏移，而不可观测变量导致的遗漏变量偏差会损害语言模型的评估和优化效果。

Method: 提出一个理论框架，将分布偏移分解为可观测和不可观测部分。建立不可观测变量强度与最坏情况泛化性能边界的映射关系，并将这些边界直接应用于语言模型的评估和优化过程。

Result: 实验表明，使用该框架提供的边界进行语言模型评估和优化，能够获得更原则性的OOD性能度量，相比标准分布偏移调整方法提升了真实的OOD性能，并在目标分布标签可用时能够推断不可观测变量的强度。

Conclusion: 该研究通过区分可观测和不可观测的分布偏移，并建立不可观测变量与泛化性能边界的关系，为解决语言模型中的分布偏移问题提供了更全面的理论框架和实用方法。
...
### [122] [Better Think Thrice: Learning to Reason Causally with Double Counterfactual Consistency](https://arxiv.org/abs/2602.16787)
*Victoria Lin,Xinnuo Xu,Rachel Lawrence,Risa Ueno,Amit Sharma,Javier Gonzalez,Niranjani Prasad*

Main category: cs.LG

TL;DR: 提出DCC（双重反事实一致性）方法，无需标注数据即可评估和指导LLM的因果推理能力


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在推理基准测试中表现良好，但在面对反事实问题时表现脆弱，表明其因果推理能力存在弱点。现有方法需要大量标注的反事实数据，难以覆盖广阔的反事实空间。

Method: 提出DCC（双重反事实一致性）方法，这是一种轻量级的推理时方法，无需标注的反事实数据。DCC通过验证模型执行因果干预和反事实预测这两个关键元素的能力来评估因果推理。

Result: 使用DCC评估了多种领先LLM在各种推理任务和干预下的因果推理能力。此外，DCC作为无需训练、测试时的拒绝采样标准，能够直接提升多个模型家族在推理任务上的性能。

Conclusion: DCC提供了一种无需标注数据即可有效评估和提升LLM因果推理能力的方法，解决了大规模反事实数据标注的局限性问题。
...
### [123] [Escaping the Cognitive Well: Efficient Competition Math with Off-the-Shelf Models](https://arxiv.org/abs/2602.16793)
*Xingyu Dang,Rohit Agarwal,Rodrigo Porto,Anirudh Goyal,Liam H Fowl,Sanjeev Arora*

Main category: cs.LG

TL;DR: 提出一个低成本、高性能的IMO数学推理推理管道，通过认知陷阱分析和猜想提取技术，在IMO-ProofBench Advanced上达到67.1%的准确率，成本仅为31美元/题


<details>
  <summary>Details</summary>
Motivation: 现有数学推理模型在IMO问题上达到金牌水平，但公开模型需要大规模推理且成本极高（如3000美元/题）。需要开发低成本、高性能的推理管道，使用通用现成模型解决IMO级数学问题。

Method: 提出推理管道，基于对求解器-评分器管道中评分器失败的洞察（认知陷阱：迭代优化收敛到错误解但求解器和内部评分器都认为正确）。通过猜想提取技术，从生成的解中分离候选引理，并在新环境中独立验证引理及其否定（上下文分离）。

Result: 在IMO-ProofBench Advanced上，使用Gemini 3.0 Pro达到67.1%的性能，平均每题成本约31美元。在评估时，这在公开和未发布模型中均达到最先进水平，比次优公开管道的成功率提高一倍以上，且成本大幅降低。

Conclusion: 提出的推理管道实现了低成本、高性能的IMO数学问题求解，通过解决认知陷阱问题，使用通用现成模型达到最先进性能，大幅降低了数学推理的成本门槛。
...
### [124] [Efficient Tail-Aware Generative Optimization via Flow Model Fine-Tuning](https://arxiv.org/abs/2602.16796)
*Zifan Wang,Riccardo De Santi,Xiaoyu Mo,Michael M. Zavlanos,Andreas Krause,Karl H. Johansson*

Main category: cs.LG

TL;DR: TFFT是一种基于条件风险价值（CVaR）的分布微调算法，专注于控制生成模型输出分布的尾部行为，包括高奖励尾部的探索和低奖励尾部的可靠性控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵正则化的微调方法主要最大化期望奖励，缺乏对尾部行为的控制机制。然而，尾部控制在实际应用中至关重要：下尾部决定了可靠性（限制低奖励失败），上尾部则支持探索（发现罕见的高奖励结果）。

Method: 提出Tail-aware Flow Fine-Tuning (TFFT)，基于CVaR的变分对偶公式，将问题分解为两个解耦阶段：1）轻量级的一维阈值优化步骤；2）通过特定伪奖励进行单次熵正则化微调。该方法支持右CVaR（探索高奖励尾部）和左CVaR（控制低奖励尾部）。

Result: TFFT在计算成本上与标准期望微调方法相当，实现了高效的CVaR微调。在多个实验中验证了有效性，包括说明性实验、高维文本到图像生成和分子设计任务。

Conclusion: TFFT提供了一种原则性且高效的分布微调算法，能够专门控制生成模型输出分布的尾部行为，解决了现有方法在尾部控制方面的局限性，为实际应用中的可靠性和探索性需求提供了有效解决方案。
...
Result: 在3,185个节点的校园导航任务中取得显著性能提升，但发现学习到的潜在表征缺乏对实际心理状态的显式基础。

Conclusion: 分层结构改善了预测性能，但表征基础问题仍需解决。提出自监督对齐策略，并寻求社区对基础化方法的反馈。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习后训练面临探索瓶颈，稀疏反馈和大动作空间容易导致模型陷入重复行为模式。需要一种可控的探索机制来改善学习效率。

Method: 提出Verbalized Action Masking (VAM)：1) 在提示中语言化动作掩码，强制模型从掩码集合中输出动作；2) 基于此接口引入迭代动作空间剪枝：如果未采样到目标动作，则从掩码中移除已采样的有效动作，在缩减的候选集下重新采样，重复直到采样到目标或耗尽预算。在象棋任务中评估两种训练机制：引擎对弈机制和固定数据集机制。

Result: 在象棋谜题和完整对弈（通过平均百兵损失ACPL衡量）的评估中，VAM相比强基线方法提升了学习效率和最终性能。语言化掩码被证明是大语言模型强化学习后训练中一种实用的可控探索机制。

Conclusion: Verbalized Action Masking (VAM)通过语言化动作掩码和迭代动作空间剪枝，有效解决了大语言模型强化学习后训练中的探索瓶颈问题，在象棋任务中展现出优越性能，为可控探索提供了实用机制。
...
### [127] [A Residual-Aware Theory of Position Bias in Transformers](https://arxiv.org/abs/2602.16837)
*Hanna Herasimchyk,Robin Labryga,Tomislav Prusina,Sören Laue*

Main category: cs.LG

TL;DR: 该论文提出了一种考虑残差连接的累积注意力展开理论，解释了因果Transformer中位置偏置的架构起源，特别是U形位置偏置和"迷失在中间"现象。


<details>
  <summary>Details</summary>
Motivation: Transformer模型系统性地偏好某些token位置，但这种位置偏置的架构起源仍不清楚。先前理论预测在无限深度因果掩码下注意力会坍缩到第一个token，但这与实际情况不符，需要解释这种差异。

Method: 提出了残差感知的累积注意力展开理论，通过纳入残差连接来分析注意力机制。在有限深度下，证明了因果Transformer会诱导U形位置偏置，注意力集中在早期和晚期token上。

Result: 残差连接防止了在实际条件下注意力的坍缩。理论分析表明因果Transformer会产生U形位置偏置，为"迷失在中间"现象提供了架构层面的解释。

Conclusion: 残差连接是防止注意力坍缩的关键架构组件，因果Transformer的U形位置偏置是架构固有的特性，这为理解Transformer的位置偏好提供了理论基础。
...
### [128] [Training Large Reasoning Models Efficiently via Progressive Thought Encoding](https://arxiv.org/abs/2602.16839)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂问题上表现出色，但强化学习训练需要长序列展开，自回归解码占用大量时间和内存。滑动窗口缓存策略虽然能限制内存，但会破坏长上下文推理并降低性能。需要一种方法使模型在固定大小缓存下有效推理。

Method: 提出渐进思维编码，一种参数高效的微调方法。通过逐步将中间推理过程编码为固定大小的向量表示，消除对完整缓存展开的反向传播需求，减少内存使用，同时在推理期间保持恒定内存。

Result: 在三个模型（Qwen2.5-3B-Instruct、Qwen2.5-7B-Instruct、DeepSeek-R1-Distill-Llama-8B）和六个数学基准测试上取得一致提升：相比基于LoRA的微调平均提升19.3%，相比未微调的LRMs提升29.9%，在相同严格缓存预算下AIME2024/2025上最高提升23.4%准确率。
...
### [129] [What is the Value of Censored Data? An Exact Analysis for the Data-driven Newsvendor](https://arxiv.org/abs/2602.16842)
*Rachitesh Kumar,Omar Mouchtaki*

Main category: cs.LG

TL;DR: 该论文研究了在需求数据被审查情况下的离线数据驱动报童问题，提出了计算经典库存策略最坏情况遗憾的通用方法，并分析了需求审查对学习能力的影响。


<details>
  <summary>Details</summary>
Motivation: 传统报童问题研究通常假设需求被完全观测，但实际零售场景中需求往往在库存水平处被审查，只能观测到销售数据而非真实需求。现有研究缺乏对这种需求审查情况下数据驱动库存策略性能的系统分析。

Method: 提出了一种通用程序来计算经典数据驱动库存策略在所有需求分布上的精确最坏情况遗憾。主要技术贡献是将无限维非凸优化问题简化为有限维问题，从而能够精确刻画任何样本量和审查水平下策略的性能。

Result: 分析表明：1）需求审查从根本上限制了从被动销售数据中学习的能力；2）少量针对性的高库存水平探索能显著改善最坏情况保证，即使在严重审查下也能实现接近最优的性能；3）将销售视为需求的启发式方法会随着审查数据积累而遭受严重的性能下降。

Conclusion: 销售点信息质量对离线学习能力具有关键影响。需求审查限制了被动数据的学习能力，但通过有针对性的探索可以克服这一限制。将销售视为需求的简单方法在长期运行中会导致性能显著下降，强调了正确处理审查数据的重要性。
...
### [130] [Construction of a classification model for dementia among Brazilian adults aged 50 and over](https://arxiv.org/abs/2602.16887)
...
<details>
  <summary>Details</summary>
Motivation: 构建一个针对巴西中老年人的痴呆症分类模型，使用低成本、可干预的变量，旨在识别高危人群并为公共卫生政策提供依据。

Method: 采用预测建模方法的观察性研究，使用横断面设计。数据来自巴西老龄化纵向研究(ELSI-Brazil)，涉及9,412名参与者。使用随机森林(RF)和多变量逻辑回归分析，结合变量选择方法。

Result: 痴呆症患病率为9.6%。主要风险因素包括：文盲(OR=7.42)、90岁及以上(OR=11.00)、低体重(OR=2.11)、低握力(OR=2.50)、自报黑色皮肤(OR=1.47)、缺乏运动(OR=1.61)、听力损失(OR=1.65)和抑郁症状(OR=1.72)。保护因素包括：高等教育(OR=0.44)、较高生活满意度(OR=0.72)和就业(OR=0.78)。RF模型表现优于逻辑回归，AUC为0.776，敏感性0.708，特异性0.702。

Conclusion: 研究结果强调了痴呆症的多维性质以及可及性因素在识别脆弱个体中的重要性。加强关注促进大脑健康的公共政策，有助于巴西初级保健资源的有效分配和痴呆症预防。
...
### [131] [Exact Certification of Data-Poisoning Attacks Using Mixed-Integer Programming](https://arxiv.org/abs/2602.16944)
*Philip Sosnin,Jodie Knapp,Fraser Kennedy,Josh Collyer,Calvin Tsay*

Main category: cs.LG

TL;DR: 提出首个提供完备性保证的数据投毒攻击验证框架，通过混合整数二次规划同时编码训练动态和测试评估，实现对训练时鲁棒性的精确认证


<details>
  <summary>Details</summary>
Motivation: 现有数据投毒攻击研究缺乏同时具备完备性和可靠性的验证框架，无法提供训练时鲁棒性的精确认证保证

Method: 将对抗性数据操纵、模型训练和测试评估统一表述为混合整数二次规划问题，编码梯度训练动态和测试时模型评估

Result: 框架能证明找到全局最优解即产生最坏情况投毒攻击，同时限定所有可能攻击的有效性，在小模型上实现鲁棒性的完整表征

Conclusion: 该工作提出了首个提供完备性保证的训练时鲁棒性验证框架，为数据投毒攻击提供了精确的认证方法
...
TL;DR: 提出了一种去中心化多玩家随机老虎机算法，在连续Lipschitz动作空间中实现无通信协调，达到接近最优的遗憾界


<details>
  <summary>Details</summary>
Motivation: 解决去中心化多玩家随机老虎机问题，其中硬碰撞产生零奖励。目标是设计无需通信的策略，最大化集体奖励，同时协调成本与时间范围T无关
...
Conclusion: 该框架成功解决了去中心化多玩家Lipschitz老虎机问题，实现了无通信协调和接近最优的性能，为连续动作空间中的多智能体协作提供了理论保证
...
TL;DR: 本文提出了一种新的多智能体强化学习可扩展性分析框架，揭示了局部性可以是策略依赖的现象，并建立了比现有方法更紧的指数衰减条件。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习面临维度诅咒的挑战，现有基于最坏情况环境边界保证指数衰减属性的条件过于保守，未能捕捉策略本身的正则化效应。

Method: 提出了策略诱导的相互依赖矩阵H^π的新分解方法，将环境对状态(E^s)和动作(E^a)的敏感性与策略对状态(Π(π))的敏感性解耦，建立了谱条件ρ(E^s+E^aΠ(π))<1来保证指数衰减。

Result: 证明了局部性可以由平滑策略诱导，即使环境存在强动作耦合，揭示了局部性与最优性之间的基本权衡，并基于该理论分析了具有理论保证的局部化块坐标策略改进框架。

Conclusion: 局部性不仅是环境属性，也是策略依赖现象，新框架提供了比现有方法更紧的指数衰减条件，为可扩展的多智能体强化学习算法设计提供了理论基础。
...
TL;DR: 该研究发现交换子缺陷（一种源于非交换梯度更新的曲率度量）是Transformer中延迟泛化的稳健、架构无关、因果相关的早期预警信号，在三个任务家族（模算术、SCAN、Dyck-1）中均表现出普遍性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索Grokking现象（从记忆到泛化的突然转变）是否仅限于模算术任务，以及是否存在跨任务的通用早期预警信号。先前研究将Grokking与低维执行流形上的限制联系起来，但这一机制是否适用于其他序列学习任务尚不明确。

Method: 方法包括：1）在两个序列学习基准（SCAN组合泛化和Dyck-1深度预测）上训练Transformer模型；2）测量交换子缺陷（一种源于非交换梯度更新的曲率度量）；3）使用权重空间PCA分析谱集中性；4）进行因果干预实验，包括放大非交换性和抑制正交梯度流；5）比较三个任务家族（模算术、SCAN、Dyck）的因果敏感性。

Result: 主要结果：1）交换子缺陷在泛化前显著上升，领先时间遵循超线性幂律（SCAN α≈1.18，Dyck α≈1.13），与模算术结果一致；2）谱集中性不是通用前兆，而交换子缺陷是；3）因果干预显示机制作用：放大非交换性加速Grokking（SCAN约32%，Dyck约50%），抑制正交梯度流延迟或阻止Grokking；4）三个任务家族形成因果敏感性谱系：模算术刚性、Dyck响应性、SCAN中间性，但抑制在所有情况下都延迟或阻止Grokking。

Conclusion: 结论：交换子缺陷是Transformer中延迟泛化的稳健、架构无关、因果相关的早期预警信号。尽管不同任务对干预的敏感性不同（形成从刚性到响应性的谱系），但抑制正交梯度流在所有情况下都延迟或阻止Grokking，确立了其必要性作为普遍发现。这些结果将交换子缺陷确立为跨任务Grokking现象的通用预测指标。
...
### [135] [Unified Latents (UL): How to train your latents](https://arxiv.org/abs/2602.17270)
*Jonathan Heek,Emiel Hoogeboom,Thomas Mensink,Tim Salimans*

Main category: cs.LG

TL;DR: UL框架通过扩散先验和扩散解码器联合正则化学习潜在表示，在图像和视频生成任务上取得优异性能


<details>
  <summary>Details</summary>
Motivation: 提出一种统一的潜在表示学习框架，旨在通过结合扩散先验和扩散解码器来获得高质量的潜在表示，同时提供紧致的比特率上界，减少训练计算成本

Method: 通过将编码器输出噪声与先验的最小噪声水平关联，设计简单的训练目标；使用扩散先验进行联合正则化，扩散模型进行解码

Result: 在ImageNet-512上达到FID 1.4的竞争性结果，高重建质量（PSNR），比在Stable Diffusion潜在空间训练的模型需要更少FLOPs；在Kinetics-600上创下FVD 1.3的新SOTA

Conclusion: UL框架通过扩散先验和扩散解码器的联合正则化，实现了高质量的潜在表示学习，在图像和视频生成任务上均表现出色，同时具有计算效率优势
...
<details>
  <summary>Details</summary>
Motivation: 现有黑盒对抗攻击方法如M-Attack在大型视觉语言模型上存在梯度方差大、优化不稳定问题，主要源于ViT平移敏感性和源-目标裁剪结构不对称性

Method: 1) 多裁剪对齐：每迭代平均多个独立采样局部视图的梯度以降低方差；2) 辅助目标对齐：用语义相关分布的小辅助集替代激进目标增强，产生更平滑的目标流形；3) 补丁动量：重放历史裁剪梯度；4) 改进的补丁大小集成

Result: 在Claude-4.0上成功率从8%提升至30%，Gemini-2.5-Pro从83%提升至97%，GPT-5从98%提升至100%，显著优于现有黑盒LVLM攻击方法

Conclusion: M-Attack-V2通过梯度去噪和局部匹配重新表述，为M-Attack提供了简单模块化增强，大幅改善了黑盒对抗攻击在大型视觉语言模型上的效果
...
### [137] [Dynamic Delayed Tree Expansion For Improved Multi-Path Speculative Decoding](https://arxiv.org/abs/2602.16994)
*Rahul Thomas,Teo Kitanovski,Micah Goldblum,Arka Pal*

Main category: cs.LG

TL;DR: 本文系统评估了多路径推测解码中的验证策略，发现遍历验证始终优于基于最优传输的方法，并提出延迟树扩展和动态神经选择器，使OT方法首次超越遍历验证，实现5%的平均吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 多路径推测解码通过使用更便宜的草稿模型生成令牌草稿树来加速无损采样，但现有验证算法在独立同分布生成条件下的相对性能尚不明确，需要系统评估并改进验证策略。

Method: 1) 系统评估不同验证策略在模型家族、任务和采样机制下的性能；2) 提出延迟树扩展方法，推迟独立同分布分支点；3) 开发动态神经选择器，根据草稿和目标特征估计基于最优传输验证方法的预期块效率。

Result: 遍历验证始终优于基于最优传输的方法；延迟树扩展能保持目标分布并改进根节点独立同分布生成；动态神经选择器使基于最优传输的方法（如SpecInfer）首次超越遍历验证，在各种模型、数据集和采样设置下实现5%的平均吞吐量提升。

Conclusion: 通过系统评估验证策略并基于分析开发延迟树扩展和动态神经选择器，显著提升了多路径推测解码的性能，使基于最优传输的验证方法首次超越遍历验证，为实际应用提供了更优的验证策略选择机制。
...
### [138] [Action-Graph Policies: Learning Action Co-dependencies in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.17009)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，协调行动是最基本的合作形式。成功的分散决策不仅取决于良好的个体行动，还需要选择跨智能体的兼容行动以实现行为同步、避免冲突并满足全局约束。现有方法在处理协调任务时表现不佳。

Method: 提出动作图策略(AGP)，建模智能体可用动作选择之间的依赖关系。该方法构建"协调上下文"，使智能体能够基于全局动作依赖关系来条件化其决策。AGP在理论上比完全独立策略更具表达力。

Result: 理论上证明AGP能够实现比贪婪执行更优的协调联合行动，即使是从集中式价值分解方法出发。实证结果显示，在具有部分可观测性和反协调惩罚的典型协调任务中，AGP达到80-95%成功率，而其他MARL方法仅达到10-25%。在多样多智能体环境中，AGP始终优于基线方法。

Conclusion: 动作图策略(AGP)通过显式建模动作依赖关系，有效解决了多智能体协调问题，在理论和实证上都表现出优越性能，为分散决策中的协调提供了有效解决方案。
...
### [139] [Malliavin Calculus as Stochastic Backpropogation](https://arxiv.org/abs/2602.17013)
*Kevin D. Oden*

Main category: cs.LG

TL;DR: 该论文建立了路径式（重参数化）和得分函数（Malliavin）梯度估计器之间的严格联系，并基于此提出了一种方差感知的混合估计器，在VAE和合成问题上实现了显著的方差减少。


<details>
  <summary>Details</summary>
Motivation: 现有随机梯度估计方法（路径式重参数化和得分函数法）虽然广泛使用，但缺乏统一的理论框架。研究者希望建立两者之间的严格数学联系，并开发能够自适应结合两者优点的混合估计器，以降低方差并提高优化效率。

Method: 通过Malliavin积分恒等式建立了路径式和得分函数梯度估计器的理论等价性。基于此，提出了一种统一的方差感知混合估计器，该估计器使用经验协方差结构自适应地组合两种梯度，在所有无偏线性组合中达到最小方差，并提供了闭式有限样本收敛界。

Result: 在CIFAR-10的变分自编码器（VAE）上实现了9%的方差减少，在强耦合合成问题上实现了高达35%的方差减少。探索性策略梯度实验表明，非平稳优化场景对混合方法提出了挑战。

Conclusion: Malliavin微积分为随机梯度估计提供了概念上统一且实践上可解释的框架，阐明了混合方法何时能提供实际效益以及何时面临固有局限性。该工作为理解随机反向传播提供了原则性基础。
...
### [140] [WS-GRPO: Weakly-Supervised Group-Relative Policy Optimization for Rollout-Efficient Reasoning](https://arxiv.org/abs/2602.17025)
...
<details>
  <summary>Details</summary>
Motivation: GRPO在复杂推理训练中有效，但基于相对轨迹组的优化目标导致过度思考：更长的推理过程可能创造更多相对收益机会，造成推理效率低下和过度思考问题。实践中难以控制这种行为，因为长度惩罚难以校准（长推理可能反映更难问题），且缺乏继续/停止的直接监督信号。

Method: 提出弱监督GRPO（WS-GRPO）：1）将终端奖励转换为对部分轨迹的正确性感知指导；2）训练偏好模型从仅结果正确性生成前缀级信号，指示何时继续推理有益；3）提供结果衍生的继续/停止指导，减少冗余思考同时保持准确性。

Result: 理论分析支持该方法有效性，在推理基准测试中实证显示：WS-GRPO显著减少推理长度，同时保持与GRPO基线相当的竞争力。

Conclusion: WS-GRPO通过弱监督方式解决了GRPO中的过度思考问题，提供了一种更高效的推理训练方法，在减少冗余推理的同时维持模型准确性，改善了正确性与推理效率之间的权衡。
...
### [141] [Transforming Behavioral Neuroscience Discovery with In-Context Learning and AI-Enhanced Tensor Methods](https://arxiv.org/abs/2602.17027)
...
Method: 提出AI增强的流水线，采用"上下文学习"范式作为领域专家接口，无需AI模型训练和微调；引入张量分解模型的AI增强，从异构数据中无缝发现模式；在行为神经科学的小鼠恐惧泛化研究中应用

Result: 实验评估显示，提出的流水线性能优于领域标准实践和非ICL范式的合理ML基线；实现了有效发现，结果得到团队领域专家的验证

Conclusion: 上下文学习范式为领域专家提供了无缝易用的接口，AI增强的张量分解模型能有效处理异构数据，该AI增强流水线能显著加速科学发现过程，让领域专家专注于解释和理解发现
...
### [142] [Multi-Probe Zero Collision Hash (MPZCH): Mitigating Embedding Collisions and Enhancing Model Freshness in Large-Scale Recommenders](https://arxiv.org/abs/2602.17050)
...
TL;DR: 提出MPZCH（多探测零碰撞哈希），一种基于线性探测的新型索引机制，有效减少嵌入表碰撞，实现用户嵌入零碰撞并提升项目嵌入新鲜度。


<details>
  <summary>Details</summary>
Motivation: 传统基于哈希的索引方法在处理大规模推荐系统中高基数分类特征时，随着唯一ID数量增加，哈希碰撞会降低模型性能和个性化质量。

Method: 基于线性探测的索引机制，使用辅助张量和高性能CUDA内核实现可配置探测和主动驱逐策略，通过淘汰过时ID和重置重新分配的槽位来防止陈旧嵌入继承。

Result: 在合理表大小下通常能完全消除碰撞，保持与现有方法相当的训练QPS和推理延迟，在线实验显示实现用户嵌入零碰撞，显著提升项目嵌入新鲜度和质量。

Conclusion: MPZCH有效解决了嵌入表碰撞问题，已在开源TorchRec库中发布，为推荐系统社区提供了实用的碰撞缓解解决方案。
...
### [143] [Adam Improves Muon: Adaptive Moment Estimation with Orthogonalized Momentum](https://arxiv.org/abs/2602.17080)
...
<details>
  <summary>Details</summary>
Motivation: 现有优化器如Adam使用自适应矩估计来适应随机扰动，Muon利用权重层的矩阵结构通过正交动量提升性能，但缺乏将正交动量与基于范数的Adam型噪声适应进行原则性整合的方法

Method: 提出NAMO和NAMO-D两种优化器：NAMO使用单一自适应步长缩放正交动量，保持正交性；NAMO-D将正交动量右乘带截断项的对角矩阵，实现神经元级噪声适应并利用近块对角Hessian结构

Result: 在确定性设置下建立了最优收敛率，在随机设置下收敛保证能适应随机梯度噪声水平；GPT-2预训练实验显示NAMO和NAMO-D均优于AdamW和Muon基线，NAMO-D通过截断超参数进一步优于NAMO

Conclusion: NAMO和NAMO-D首次实现了正交动量与基于范数的Adam型噪声适应的原则性整合，NAMO-D通过神经元级适应和截断机制在保持良好条件更新方向与利用细粒度噪声适应之间取得平衡
...
### [144] [FLoRG: Federated Fine-tuning with Low-rank Gram Matrices and Procrustes Alignment](https://arxiv.org/abs/2602.17095)
*Chuiyang Meng,Ming Tang,Vincent W. S. Wong*

Main category: cs.LG

TL;DR: FLoRG：一种用于大语言模型联邦微调的高效框架，通过使用单一低秩矩阵和Gram矩阵聚合，解决了传统LoRA在联邦学习中存在的聚合误差和分解漂移问题。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA在联邦学习微调中存在两个主要问题：1）分别聚合两个低秩矩阵会引入误差；2）即使聚合矩阵乘积，服务器需要通过矩阵分解恢复因子，但分解不唯一且会导致分解漂移。需要一种更稳定高效的联邦微调方法。

Method: FLoRG采用单一低秩矩阵进行微调，聚合其Gram矩阵（列向量内积矩阵）而非矩阵本身，消除聚合误差并减少通信开销。引入Procrustes对齐方法在连续微调轮次间对齐分解矩阵，最小化分解漂移。

Result: 实验结果表明，FLoRG在多个LLM微调基准测试中优于五种最先进的基线方案，下游任务准确率更高，通信开销最多可减少2041倍。理论分析证明Procrustes对齐能获得更紧的收敛界。

Conclusion: FLoRG通过单一低秩矩阵和Gram矩阵聚合解决了联邦学习中LoRA的聚合误差和分解漂移问题，结合Procrustes对齐实现了更稳定高效的联邦微调，显著降低通信开销并提升性能。
...
TL;DR: 提出MeGU框架，通过概念感知重对齐和多模态大语言模型引导，实现选择性遗忘，解决机器学习遗忘中的欠遗忘和过遗忘问题
...
Method: 提出MeGU框架：1) 利用多模态大语言模型为遗忘样本分配语义上有意义的扰动标签，确定重对齐方向；2) 将MLLM估计的类间概念相似性编码为轻量级转移矩阵；3) 引入正负特征噪声对，负噪声抑制目标特定特征模式，正噪声增强剩余关联特征并与扰动概念对齐

Result: MeGU实现了可控的选择性遗忘，有效缓解了欠遗忘和过遗忘问题，能够在保持共享语义结构的同时选择性地破坏目标特定表示

Conclusion: 通过概念感知重对齐和特征噪声对设计，MeGU框架解决了机器学习遗忘中的基本权衡问题，为训练数据隐私保护提供了有效的选择性遗忘方案
...
### [146] [TIFO: Time-Invariant Frequency Operator for Stationarity-Aware Representation Learning in Time Series](https://arxiv.org/abs/2602.17122)
*Xihao Piao,Zheng Chen,Lingwei Zhu,Yushun Dong,Yasuko Matsubara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: 提出TIFO方法，通过在频域学习时不变频率算子来缓解时间序列预测中的分布偏移问题，该方法可即插即用，在多个数据集上显著提升预测性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 非平稳时间序列预测面临分布偏移问题，即训练数据和测试数据由不同分布产生。现有方法（如去除单个样本的低阶矩）未能捕捉样本间的时间演化结构，也无法建模复杂的时间结构。

Method: 提出时不变频率算子（TIFO），在频域中学习整个数据集上频率谱的平稳性感知权重。该权重表示突出平稳频率分量，抑制非平稳分量，从而缓解分布偏移。基于傅里叶变换在频域中隐式诱导特征分解的理论依据。

Result: 在28个预测设置中取得18个top-1和6个top-2结果；在ETTm2数据集上平均MSE分别提升33.3%和55.3%；计算成本相比基线方法降低60%-70%，展示出跨不同预测模型的强可扩展性。

Conclusion: TIFO通过频域方法有效解决时间序列预测中的分布偏移问题，作为即插即用组件可无缝集成到各种预测模型中，在提升预测性能的同时显著降低计算复杂度。
...
### [147] [Synergizing Transport-Based Generative Models and Latent Geometry for Stochastic Closure Modeling](https://arxiv.org/abs/2602.17089)
*Xinghao Dong,Huchen Yang,Jin-long Wu*

Main category: cs.LG

TL;DR: 该论文提出使用潜在空间中的流匹配方法替代扩散模型来学习随机闭合模型，实现了快两个数量级的单步采样速度，同时通过显式和隐式正则化控制潜在空间失真以保持物理保真度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能产生高质量且多样化的样本，适合学习随机闭合模型，但其迭代采样速度较慢。需要寻找既能保持扩散模型优势又能实现快速采样的替代方法。

Method: 在2D Kolmogorov流数值示例上系统比较基于传输的生成模型，提出在低维潜在空间中使用流匹配方法，实现单步采样。为控制潜在空间失真，比较了联合训练提供的隐式正则化与两种显式正则化：度量保持(MP)约束和几何感知(GA)约束。

Result: 潜在空间流匹配方法比基于迭代扩散的方法采样速度快两个数量级。显式和隐式正则化的潜在空间都继承了原始复杂动力系统低维流形的关键拓扑信息，使得无需大量训练数据即可学习随机闭合模型。

Conclusion: 潜在空间中的流匹配方法为学习随机闭合模型提供了有前景的路径，既保持了扩散模型的样本质量和多样性优势，又显著提高了采样速度，同时通过正则化确保了物理保真度。
...
### [148] [TimeOmni-VL: Unified Models for Time Series Understanding and Generation](https://arxiv.org/abs/2602.17149)
*Tong Guan,Sheng Pan,Johan Barthelemy,Zhao Li,Yujun Cai,Cesare Alippi,Ming Jin,Shirui Pan*

Main category: cs.LG

TL;DR: TimeOmni-VL是首个基于视觉的统一时间序列理解与生成框架，通过双向映射和基于理解的生成机制，解决了现有模型在数值生成和语义理解之间的割裂问题。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列建模存在数值生成与语义理解之间的明显割裂：生成模型往往依赖表面模式匹配，而理解导向的模型难以产生高保真数值输出。尽管统一多模态模型在视觉领域已弥合这一鸿沟，但在时间序列领域的潜力尚未开发。

Method: 提出TimeOmni-VL框架，包含两个关键创新：(1) 保真双向映射(Bi-TSI)：实现时间序列与图像之间的近无损转换，包括时间序列到图像(TS2I)和图像到时间序列(I2TS)；(2) 理解引导的生成：引入TSUMM-Suite数据集（包含六个理解任务和两个生成任务），通过校准的思维链，将时间序列理解作为显式控制信号用于高保真生成。

Result: 实验证实，这种统一方法显著提升了语义理解和数值精度，为多模态时间序列建模建立了新前沿。

Conclusion: TimeOmni-VL首次通过视觉中心框架统一了时间序列的理解与生成，利用理解作为生成的控制信号，实现了高保真转换，为多模态时间序列建模开辟了新方向。
...
### [149] [Operationalization of Machine Learning with Serverless Architecture: An Industrial Operationalization of Machine Learning with Serverless Architecture: An Industrial Implementation for Harmonized System Code Prediction](https://arxiv.org/abs/2602.17102)
...
TL;DR: 论文提出了一个基于无服务器架构的MLOps框架，用于协调从数据摄取到模型再训练的完整机器学习生命周期，并通过工业应用案例（HS编码预测）验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 解决合规关键任务（如HS编码预测）中传统ML部署面临的挑战：频繁更新、描述模糊、分类困难，以及错误导致的物流延误和经济损失。同时应对ML生命周期管理中的基础设施开销、可重复性、可审计性和SLA遵守等问题。

Method: 采用无服务器架构的事件驱动管道和托管服务，构建模型无关的MLOps框架。使用自定义文本嵌入编码器和多种深度学习架构（特别是Text-CNN），通过标准化接口支持多样化推理模式。实现自动A/B测试、动态模型选择和自动扩展。

Result: 在HS编码预测任务中，Text-CNN模型在真实数据上达到98%的准确率。框架确保了可重复性、可审计性和在可变负载下的SLA遵守。通过成本效益分析，优先选择确定性分类模型而非计算成本更高的Transformer模型。

Conclusion: 该工作提供了一个可复制的无服务器ML操作化蓝图，使企业能够在优化性能和经济性的同时扩展ML应用。框架强调可扩展性、成本效率和确定性分类，同时保持对Transformer变体和LLM推理的扩展性。
...
### [150] [In-Context Learning in Linear vs. Quadratic Attention Models: An Empirical Study on Regression Tasks](https://arxiv.org/abs/2602.17171)
*Ayush Goel,Arjun Kohli,Sarvagya Somvanshi*

Main category: cs.LG

TL;DR: 比较transformer和线性注意力机制在线性回归任务上的上下文学习表现差异


<details>
  <summary>Details</summary>
Motivation: 已有研究表明transformer和线性注意力模型能在简单函数类（如线性回归）上进行上下文学习，但两种注意力机制在该任务上的具体行为差异尚不明确

Method: 在Garg等人的经典线性回归任务上，实证研究两种注意力机制的上下文学习行为，评估学习质量（MSE）、收敛性和泛化行为，并分析模型深度增加对性能的影响

Result: 结果展示了线性注意力相对于二次注意力的相似性和局限性

Conclusion: 在线性回归的上下文学习任务中，线性注意力与二次注意力既有相似之处，也存在特定局限性
...
### [151] [Continual uncertainty learning](https://arxiv.org/abs/2602.17174)
*Heisei Yonezawa,Ansei Yonezawa,Itsuro Kajiwara*

Main category: cs.LG

TL;DR: 提出基于课程持续学习的鲁棒控制框架，通过分解多不确定性复杂问题为序列任务，结合模型基控制器加速学习，应用于汽车动力总成振动控制
...
Method: 提出课程持续学习框架：1)将多不确定性控制问题分解为持续学习任务序列；2)将原系统扩展为有限植物集，动态不确定性随学习进展逐步扩展；3)结合模型基控制器保证基线性能；4)采用残差学习方案实现任务特定优化

Result: 应用于汽车动力总成主动振动控制，验证了控制器对结构非线性和动态变化的鲁棒性，实现了成功的仿真到现实迁移

Conclusion: 提出的课程持续学习框架能有效处理多不确定性鲁棒控制问题，通过任务分解和模型基控制器集成提高了学习效率和策略质量，在工业应用中展示了良好的鲁棒性和可迁移性
...
### [152] [i-PhysGaussian: Implicit Physical Simulation for 3D Gaussian Splatting](https://arxiv.org/abs/2602.17117)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于3D重建的物理模拟器通常采用显式逐步更新方法，对时间步长敏感，在复杂场景（如高刚度材料或准静态运动）下精度迅速下降。需要开发更稳健的模拟方法来解决这些问题。

Method: 提出i-PhysGaussian框架，将3D高斯泼溅（3DGS）与隐式物质点法（MPM）积分器耦合。通过隐式牛顿型优化和GMRES求解器最小化动量平衡残差，获得步长结束状态，而非显式逐步更新。

Result: i-PhysGaussian在时间步长稳定性方面显著优于显式基线方法，能够维持高达20倍更大时间步长的稳定性，在复杂动态转换中保持结构一致性和平滑运动。

Conclusion: 该框架通过隐式积分方法有效解决了显式模拟器的时间步长敏感性问题，为复杂物理场景提供了更稳健、高效的模拟解决方案。
...
### [153] [SubQuad: Near-Quadratic-Free Structure Inference with Distribution-Balanced Objectives in Adaptive Receptor framework](https://arxiv.org/abs/2602.17330)
*Rong Fu,Zijian Zhang,Wenxin Zhang,Kun Liu,Jiekai Wu,Xianda Li,Simon Fong*

Main category: cs.LG

TL;DR: SubQuad：一个端到端管道，通过抗原感知的近次二次检索结合GPU加速亲和力核、学习多模态融合和公平约束聚类，解决免疫组库大规模比较分析中的计算效率和数据集不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决群体规模适应性免疫组库比较分析中的两个实际瓶颈：1）成对亲和力评估的近二次成本；2）数据集不平衡掩盖临床重要的少数克隆型。

Method: 结合紧凑MinHash预过滤大幅减少候选比较，可微分门控模块自适应加权互补对齐和嵌入通道，自动校准程序强制实施稀有抗原特异性亚群的比例表示，以及GPU加速亲和力核和学习多模态融合。

Result: 在大型病毒和肿瘤免疫组库上，SubQuad在吞吐量和峰值内存使用方面实现显著提升，同时保持或改进recall@k、聚类纯度和亚群公平性。
...
### [154] [From Subtle to Significant: Prompt-Driven Self-Improving Optimization in Test-Time Graph OOD Detection](https://arxiv.org/abs/2602.17342)
*Luzhi Wang,Xuanshuo Fu,He Zhang,Chuang Liu,Xiaobao Wang,Hongbo Liu*

Main category: cs.LG

TL;DR: SIGOOD是一种自改进的图分布外检测框架，通过提示增强和能量偏好优化实现无监督的测试时训练，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有图分布外检测方法多采用一次性推理范式，无法渐进修正错误预测以放大分布外信号，需要一种能持续自我学习的方法。

Method: 提出SIGOOD框架：1)生成提示构建提示增强图放大分布外信号；2)引入能量偏好优化损失利用原始图与增强图的能量差异；3)通过自改进循环迭代优化提示，最终使用最优提示增强图进行分布外检测。

Result: 在21个真实世界数据集上的综合评估证实了SIGOOD的有效性和优越性能。

Conclusion: SIGOOD通过整合持续自学习与测试时训练，提供了一种有效的无监督图分布外检测方法，在多个数据集上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在生物医学数据应用中面临鲁棒性差、可解释性有限以及特征在数据扰动（如缺失值）下不稳定的问题，这限制了其在高风险领域的应用。即使预测性能高的模型，如果关键特征随数据完整性变化而波动，也会破坏可重复性和下游决策的信任度。
...
Result: CACTUS在预测性能上达到竞争性或优越水平，同时在缺失数据增加时保持显著更高的顶级特征稳定性，包括在性别分层分析中。特征稳定性提供了与传统性能指标互补的信息，对于评估生物医学数据机器学习模型的可信度至关重要。

Conclusion: 通过明确量化对缺失数据的鲁棒性并优先考虑可解释、稳定的特征，CACTUS为可信的数据驱动决策支持提供了一个可推广的框架，解决了生物医学机器学习应用中的关键信任问题。
...
### [156] [Powering Up Zeroth-Order Training via Subspace Gradient Orthogonalization](https://arxiv.org/abs/2602.17155)
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化在避免反向传播方面具有内存效率优势，但存在精度与查询效率之间的根本性矛盾。现有方法需要大量函数评估来准确估计梯度，限制了其实际应用。

Method: 提出统一的子空间梯度正交化框架：1) 利用投影子空间视角，通过模型更新的内在低秩结构降低梯度估计方差；2) 采用Muon风格谱优化，通过梯度正交化从噪声零阶梯度中提取信息性谱结构。该方法实例化为ZO-Muon，可解释为零阶设置下的低秩Muon优化器。

Result: 在大语言模型和视觉Transformer上的实验表明，ZO-Muon显著加速收敛，在精度和查询/运行时效率方面实现双赢改进。相比MeZO基线，在LLM微调中仅需24.7%的查询即可达到相同SST-2性能，在ViT-B微调中CIFAR-100准确率提升25.1%。

Conclusion: 通过统一子空间降维和梯度正交化两个互补原则，ZO-Muon有效解决了零阶优化中精度与效率的矛盾，为零阶优化提供了更高效实用的框架。
...
TL;DR: softdtw-cuda-torch是一个开源的PyTorch库，用于在GPU上计算Soft Dynamic Time Warping，解决了现有实现中的序列长度限制、数值不稳定性和内存消耗问题。


<details>
  <summary>Details</summary>
Motivation: 现有GPU实现的SoftDTW存在三个关键限制：1) 硬性的序列长度上限（1024）；2) 小平滑参数下反向传播的数值不稳定性；3) 计算成对距离张量导致过高的GPU内存消耗。

Method: 引入了三种关键技术：1) 分块反对角线核执行，消除序列长度限制；2) 对数空间反向传播，防止浮点数溢出；3) 融合距离计算模式，消除O(BNM)中间距离张量，减少高达98%的内存使用。

Result: 该库支持任意序列长度，完整的PyTorch自动微分集成，以及Soft-DTW Barycenter计算。相比先前工作，内存使用减少了高达98%。

Conclusion: softdtw-cuda-torch提供了一个高效、稳定且内存优化的GPU SoftDTW实现，解决了现有实现的局限性，为时间序列分析任务提供了更好的工具支持。
...
### [158] [Structured Prototype-Guided Adaptation for EEG Foundation Models](https://arxiv.org/abs/2602.17251)
...
<details>
  <summary>Details</summary>
Motivation: EEG基础模型在完全微调下表现良好，但在实际临床环境中，当受试者级监督有限时泛化能力差。这种失败不仅源于监督有限，更源于噪声监督与模型高度可塑参数空间之间的结构不匹配。

Method: SCOPE采用两阶段流程：第一阶段通过几何正则化的任务先验学习构建可靠的外部监督，在嵌入空间上构建平衡的类级原型，并基于原型一致性生成置信感知伪标签以过滤不可靠信号；第二阶段引入ProAdapter，通过轻量级适配器基于结构化原型适配冻结的EEG基础模型。

Result: 在三个EEG任务和五个基础模型骨干上的实验表明，SCOPE在标签有限的跨受试者设置下始终实现强大的性能和效率。

Conclusion: SCOPE通过结构化原型引导的适配框架有效解决了EEG基础模型在有限监督下的泛化挑战，为临床环境中的实际应用提供了可行的解决方案。
...
### [159] [Learning a Latent Pulse Shape Interface for Photoinjector Laser Systems](https://arxiv.org/abs/2602.17263)
...
<details>
  <summary>Details</summary>
Motivation: 在自由电子激光器的光注入器中，控制纵向激光脉冲形状是优化电子束质量的重要杠杆，但庞大的设计空间使得系统探索受到暴力脉冲传播模拟成本的限制。

Method: 提出基于Wasserstein自编码器的生成建模框架，学习脉冲整形与下游束流动力学之间的可微分潜在接口；通过主成分分析和高斯混合模型分析潜在空间几何结构；实现从模拟数据到真实实验脉冲测量的泛化。

Result: 学习的潜在空间具有连续性和可解释性，同时保持高保真重建；高阶高斯等脉冲族在潜在空间中形成连贯轨迹；标准化脉冲长度显示与脉冲能量相关的潜在组织；线性插值可在不同脉冲类型间实现平滑过渡；模型能准确重建实验脉冲并将其一致嵌入学习流形。

Conclusion: 该方法减少了对昂贵脉冲传播模拟的依赖，促进了下游束流动力学的模拟和分析，为光注入器激光脉冲形状的系统优化提供了高效工具。
...
### [160] [LORA-CRAFT: Cross-layer Rank Adaptation via Frozen Tucker Decomposition of Pre-trained Attention Weights](https://arxiv.org/abs/2602.17510)
...
<details>
  <summary>Details</summary>
Motivation: 现有张量基PEFT方法存在局限性：LoTR使用共享因子矩阵的Tucker分解，SuperLoRA对跨层梯度更新进行分组和重塑后分解，而PiSSA等方法每层独立进行SVD。需要一种方法能直接对预训练权重进行跨层张量分解，同时保持高效适应。

Method: CRAFT将预训练注意力权重矩阵跨transformer层堆叠为3D张量，通过高阶奇异值分解（HOSVD）进行完整Tucker分解，冻结所有分解因子，仅训练轻量级可训练变换矩阵应用于每个因子矩阵。

Result: 在RoBERTa-base和RoBERTa-large上的GLUE基准实验表明，CRAFT仅需41K Tucker适应参数（与模型维度和深度无关，仅取决于固定Tucker秩），就能达到与现有方法竞争的性能。

Conclusion: CRAFT通过跨层Tucker分解预训练权重并冻结因子，仅训练轻量适应矩阵，实现了参数高效的微调，在保持性能的同时显著减少可训练参数数量，为大规模语言模型微调提供了有效解决方案。
...
TL;DR: 该论文发现transformer注意力头中存在成员测试器，专门检测"该token是否在上下文中出现过"，在多个语言模型中识别出三种不同的成员测试策略，包括高精度过滤器和经典Bloom过滤器。


<details>
  <summary>Details</summary>
Motivation: 研究transformer注意力头中是否存在专门执行成员测试功能的结构，即检测token是否在上下文中重复出现，并探究这些头的具体工作机制和特性。

Method: 在四个语言模型（GPT-2 small/medium/large和Pythia-160M）中识别成员测试头，分析其错误率曲线，使用混淆控制排除伪阳性，通过消融实验验证功能。

Result: 发现三种真正的成员测试头：L0H1和L0H5为高精度过滤器（错误率0-4%），L1H11符合经典Bloom过滤器理论曲线（R²=1.0），这些头集中在早期层（0-1层），形成多分辨率系统，对任何重复token类型都有响应。

Conclusion: transformer中存在专门的成员测试注意力头，这些头采用不同的过滤策略，集中在早期层，具有广泛泛化能力，成员测试功能与更广泛的计算角色共存，混淆控制增强了发现的可靠性。
...
### [162] [RLGT: A reinforcement learning framework for extremal graph theory](https://arxiv.org/abs/2602.17276)
...
<details>
  <summary>Details</summary>
Motivation: Wagner的开创性工作展示了深度交叉熵强化学习方法可以应用于极值图论问题，将其重新表述为组合优化问题。随后许多研究者对这一框架进行改进和扩展，创建了专门的图论RL环境，并解决了一些极值图论问题。然而，需要一个系统化的框架来支持更广泛的图类型和促进未来研究。

Method: 提出了RLGT（Reinforcement Learning for Graph Theory）框架，该框架系统化了先前工作，支持有向图和无向图、带环图、任意数量的边颜色。框架采用高效的图表示方法，具有优化的计算性能和清晰模块化的设计。

Result: 该框架为图论研究提供了一个统一的强化学习平台，能够处理多种图类型（有向/无向、带环、多色边），通过高效的图表示和模块化设计优化了计算性能。

Conclusion: RLGT框架系统化了先前强化学习在图论中的应用工作，提供了一个支持广泛图类型的统一平台，旨在通过优化的计算性能和模块化设计促进未来基于强化学习的极值图论研究。
...
### [163] [Position: Evaluation of ECG Representations Must Be Fixed](https://arxiv.org/abs/2602.17531)
*Zachary Berger,Daniel Prakah-Asante,John Guttag,Collin M. Stultz*

Main category: cs.LG

TL;DR: 该立场论文指出当前12导联心电图表示学习的基准测试实践存在问题，需要修正以确保进展可靠且与临床有意义的目标一致。作者建议扩展下游评估范围，提出评估最佳实践，并发现随机初始化编码器在许多任务上能与最先进的预训练方法相媲美。


<details>
  <summary>Details</summary>
Motivation: 当前心电图表示学习领域主要依赖三个公开的多标签基准测试（PTB-XL、CPSC2018、CSN），这些基准主要关注心律失常和波形形态标签，但心电图实际上编码了更广泛的临床信息。现有的基准测试实践限制了该领域的发展，无法充分评估模型在更广泛临床场景中的表现。

Method: 作者提出了心电图表示学习的评估最佳实践，包括：1）扩展下游评估范围，纳入结构性心脏病和患者水平预测等临床相关目标；2）在多标签、不平衡设置中应用适当的评估方法；3）使用随机初始化编码器作为合理的基线模型。通过实证评估三种代表性的心电图预训练方法在六个评估设置中的表现来验证观点。

Result: 研究发现：1）当应用适当的评估最佳实践时，文献中关于哪些表示性能最佳的结论会发生改变；2）随机初始化编码器配合线性评估在许多任务上能与最先进的预训练方法相媲美；3）在六个评估设置（三个标准基准、结构性心脏病数据集、血流动力学推断、患者预测）的实证评估中验证了这些观察结果。

Conclusion: 当前心电图表示学习的基准测试实践需要修正，应扩展评估范围以涵盖更广泛的临床相关目标，并采用适当的评估方法。随机初始化编码器应作为合理的基线模型。这些改进将确保该领域的进展更加可靠且与临床意义保持一致。
...
<details>
  <summary>Details</summary>
Motivation: 现有随机分配采样方案的理论分析存在两个主要缺陷：1) 实际场景中隐私参数不够紧致；2) 使用曲棍球棒散度或Renyi散度计算会引入额外开销。需要更精确的隐私损失计算方法。

Method: 开发基于PLD实现概念的新工具，能够高效计算任何差分隐私算法在随机分配采样下的隐私损失分布，特别应用于高斯机制的分析。

Result: 证明随机分配采样在隐私-效用权衡上至少与泊松采样相当，特别适用于DP-SGD训练，并扩展了精确隐私损失计算到采样机制。

Conclusion: 提出的PLD计算方法解决了随机分配采样分析中的紧致性和计算开销问题，为采样机制提供了通用的隐私损失计算框架。
...
### [165] [MASPO: Unifying Gradient Utilization, Probability Mass, and Signal Reliability for Robust and Sample-Efficient LLM Reasoning](https://arxiv.org/abs/2602.17550)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习算法（如GRPO）依赖于刚性、均匀且对称的信任区域机制，这与大型语言模型的复杂优化动态不匹配。具体存在三个关键问题：硬截断导致的梯度利用效率低下、均匀比率约束忽视令牌分布导致的概率质量不敏感、以及正负样本信用分配模糊性不同导致的非对称信号可靠性。
...
Conclusion: MASPO通过解决现有RLVR方法的三个关键挑战，提供了一个更适应大型语言模型优化动态的统一框架，在可验证奖励的强化学习中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 归一化和缩放是时间序列建模的基本预处理步骤，但它们在Transformer模型中的作用从理论角度仍未被充分探索。本文旨在填补这一空白，首次正式分析不同归一化策略如何影响时间序列Transformer架构的表达能力。

Method: 提出了专门针对时间序列的表达能力框架，用于量化模型在表示空间中区分相似和不相似输入的能力。使用该框架推导了两种广泛使用的归一化方法（标准化和最小-最大缩放）的理论界限。通过多个Transformer模型在分类和预测基准上的实验验证理论分析。

Result: 分析表明归一化策略的选择能显著影响模型的表示能力，具体取决于任务和数据特征。实证结果显示没有单一归一化方法始终优于其他方法，在某些情况下，完全省略归一化反而能获得更好的性能。

Conclusion: 预处理在时间序列学习中起着关键作用，需要针对特定任务和数据集设计更有原则的归一化策略。研究强调了归一化策略选择的重要性，并表明"一刀切"的方法可能不是最优选择。
...
TL;DR: VCPO：一种针对异步强化学习中REINFORCE/GRPO风格算法的高方差问题的稳定化方法，通过有效样本量调整学习率和最小方差基线来提升训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 异步强化学习训练虽然能提高端到端吞吐量，但对于无评论家的策略梯度方法（如REINFORCE和GRPO），高异步性会导致策略梯度估计器方差显著增大。过时的rollout会产生重尾的重要性比率，使少数样本主导更新，导致梯度噪声大、学习不稳定。

Method: 提出VCPO（方差控制策略优化），包含两个核心组件：1）基于有效样本量（ESS）调整学习率，以抑制不可靠的更新；2）为离策略设置应用闭式最小方差基线，避免使用辅助价值模型，增加的开销最小。

Result: VCPO在数学、通用推理和工具使用任务中显著提高了异步训练的鲁棒性，优于多种基线方法（包括掩码/裁剪稳定器和算法变体）。在长上下文、多轮训练中将训练时间减少2.5倍，同时达到同步训练的性能。

Conclusion: 显式控制策略梯度方差是实现大规模可靠异步强化学习的关键。VCPO通过有效样本量调整学习率和最小方差基线，为REINFORCE/GRPO风格算法提供了通用的稳定化解决方案。
...
TL;DR: 通过简化Mamba-2并改进其A-mask和隐藏状态阶数，提出了2Mamba方法，在保持线性注意力计算效率的同时，几乎达到softmax注意力的准确率


<details>
  <summary>Details</summary>
Motivation: 线性注意力变换器因其计算效率成为softmax注意力的替代方案，但通常表达能力较弱且准确率较低。需要弥合softmax注意力与线性注意力之间的准确率差距

Method: 首先简化Mamba-2到最基本组件，评估哪些选择最准确；从简化版本(Mamba-2S)出发，改进A-mask并增加隐藏状态阶数，提出2Mamba方法

Result: 2Mamba方法在长上下文长度下内存效率更高，几乎达到softmax注意力的准确率，同时研究了超越softmax注意力准确率的Mamba-2元素

Conclusion: 通过系统分析Mamba-2并改进关键组件，成功开发出2Mamba方法，在保持线性注意力计算优势的同时显著提升了准确率，为高效注意力机制提供了新方向
...
TL;DR: 提出Reverso系列高效时间序列基础模型，使用小型混合架构（长卷积+线性RNN层）替代大型Transformer，在零样本预测任务中实现可比性能但模型规模小100倍以上。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型为追求性能而过度扩展，导致模型参数量达数亿级别，虽然性能良好但实际使用效率低下且成本高昂。需要寻找更高效的架构替代大型Transformer。

Method: 采用小型混合模型架构，交替使用长卷积和线性RNN层（特别是DeltaNet层），配合数据增强和推理策略优化，构建Reverso系列高效时间序列基础模型。

Result: Reverso模型在零样本时间序列预测任务中能够匹配更大Transformer模型的性能，同时模型规模小100倍以上，显著提升了性能-效率的帕累托前沿。

Conclusion: 大型Transformer并非必要，小型混合架构（长卷积+线性RNN）能够实现高效的时间序列基础建模，为实际应用提供了更实用的解决方案。
...
### [170] [MARS: Margin-Aware Reward-Modeling with Self-Refinement](https://arxiv.org/abs/2602.17658)
*Payel Bhattacharjee,Osvaldo Simeone,Ravi Tandon*

Main category: cs.LG

TL;DR: 提出MARS框架：一种自适应、边界感知的数据增强和采样策略，针对奖励模型的模糊和失败模式进行增强，专注于低边界（模糊）偏好对，通过迭代硬样本增强改进训练分布。
...
Method: 提出MARS框架：自适应、边界感知的增强和采样策略，明确针对奖励模型的模糊和失败模式。该方法将增强集中在奖励模型最不确定的低边界（模糊）偏好对上，并通过硬样本增强迭代优化训练分布。

Result: 提供了理论保证，表明该策略增加了损失函数的平均曲率，从而增强了信息并改善了条件化。实证结果显示，与均匀增强相比，该方法在稳健奖励建模方面取得了持续增益。

Conclusion: MARS框架通过自适应地针对奖励模型的模糊区域进行增强，有效解决了奖励建模中的数据稀缺问题，为稳健的奖励建模提供了理论保证和实证优势。
...
### [171] [Variational Grey-Box Dynamics Matching](https://arxiv.org/abs/2602.17477)
*Gurjeet Sangra Singh,Frantzeska Lavda,Giangiacomo Mercatali,Alexandros Kalousis*

Main category: cs.LG

TL;DR: 提出一种灰盒方法，将不完整的物理模型直接集成到生成模型中，在仅使用观测轨迹的情况下学习动力学，无需真实物理参数，同时保持物理模型的解释性。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型（如流匹配和扩散模型）能学习复杂分布和动力系统，但常被视为黑盒，忽略了底层物理原理。而基于物理的仿真模型（ODE/PDE）虽然可解释，但可能存在缺失或未知项，无法完全描述真实观测。需要弥合这一差距。

Method: 提出一种新颖的灰盒方法，在流匹配框架内建模结构化变分分布，使用两个潜在编码：一个建模缺失的随机性和多模态速度，另一个将物理参数编码为具有物理信息先验的潜在变量。还提出了处理二阶动力学的框架适配方法。
...
### [172] [Linear Convergence in Games with Delayed Feedback via Extra Prediction](https://arxiv.org/abs/2602.17486)
*Yuma Fujimoto,Kenshi Abe,Kaito Ariu*

Main category: cs.LG

TL;DR: WOGDA算法在带延迟反馈的双线性博弈中实现线性收敛，额外乐观主义（预测更远未来奖励）可显著加速收敛速度
...
Result: 标准乐观主义（预测下一步奖励）在延迟m下实现收敛率exp(-Θ(t/m^5))；额外乐观主义（预测更远未来奖励）可容忍更大步长，显著加速收敛至exp(-Θ(t/(m^2 log m)))；实验验证了额外乐观主义驱动的加速收敛

Conclusion: 额外乐观主义是对抗反馈延迟导致性能下降的有效对策，能显著加速收敛并提高算法鲁棒性
...
TL;DR: RICOL：利用大语言模型通过回顾式上下文学习将稀疏奖励转化为密集训练信号，实现更高效的时间信用分配
...
Method: 提出RICL（回顾式上下文学习），利用预训练大语言模型将稀疏奖励转化为密集优势函数；进一步提出RICOL在线学习框架，基于RICL的信用分配结果迭代优化策略。

Result: RICL能够用有限样本准确估计优势函数并识别环境中的关键状态；在四个BabyAI场景中，RICOL达到与传统在线RL算法相当的收敛性能，但样本效率显著更高。

Conclusion: 研究表明利用大语言模型进行时间信用分配具有潜力，为更高效和可泛化的强化学习范式开辟了新途径。
...
### [174] [Revisiting Weight Regularization for Low-Rank Continual Learning](https://arxiv.org/abs/2602.17559)
...
<details>
  <summary>Details</summary>
Motivation: 在参数高效持续学习（PECL）范式中，权重正则化技术（如EWC）尚未得到充分探索。现有低秩持续学习方法主要通过分配任务特定模块来缓解任务干扰，但缺乏对权重正则化的研究

Method: 提出EWC-LoRA方法，将EWC权重正则化应用于低秩持续学习。通过低秩表示估计全维空间中的参数重要性，正则化共享的低秩更新，保持存储需求和推理成本恒定

Result: 在多个基准测试上的实验表明，EWC-LoRA在稳定性-可塑性权衡方面优于现有低秩持续学习方法，证明即使在低秩参数化下，权重正则化仍是缓解任务干扰的有效机制
...
### [175] [Canonicalizing Multimodal Contrastive Representation Learning](https://arxiv.org/abs/2602.17584)
*Sharut Gupta,Sanyam Kansal,Stefanie Jegelka,Phillip Isola,Vikas Garg*

Main category: cs.LG

TL;DR: 独立训练的多模态对比模型之间存在正交映射关系，同一正交矩阵可同时对齐图像和文本编码器


<details>
  <summary>Details</summary>
Motivation: 研究独立训练的多模态对比模型（如CLIP、SigLIP、FLAVA）的表示空间之间是否存在系统性的几何关系，这种关系是否在模态间保持一致

Method: 理论证明：如果两个模型在锚点集上的多模态核函数一致，则它们必须通过单一正交矩阵Q相关联；实验验证：在不同模型族、不同数据分布和架构下验证正交映射关系

Result: 发现跨模型族的几何关系可近似为正交映射（考虑全局均值偏移），同一正交矩阵Q可同时对齐图像编码器（f→f̃）和文本编码器（g→g̃）

Conclusion: 多模态对比模型表示空间之间存在统一的几何结构，这一发现支持向后兼容的模型升级，避免重新嵌入成本，并对表示学习的隐私性有启示
...
### [176] [Guarding the Middle: Protecting Intermediate Representations in Federated Split Learning](https://arxiv.org/abs/2602.17614)
*Obaidullah Zaland,Sajib Mistry,Monowar Bhuyan*

Main category: cs.LG

TL;DR: 提出KD-UFSL方法，结合k-匿名化和差分隐私保护U型联邦分割学习中的中间数据表示，防止数据重建攻击


<details>
  <summary>Details</summary>
Motivation: 在大数据场景中，联邦学习虽能保护数据隐私，但U型联邦分割学习(UFSL)中客户端与服务器共享的中间表示数据容易暴露隐私，需要增强隐私保护机制

Method: 提出KD-UFSL方法，结合微聚合(k-匿名化)和差分隐私技术，对客户端传输给服务器的中间表示数据进行隐私保护处理，防止数据重建攻击

Result: 在四个基准数据集上，KD-UFSL将实际图像与重建图像之间的均方误差提高达50%，结构相似性降低达40%，同时保持全局模型的实用性

Conclusion: KD-UFSL在保护隐私的同时保持了模型效用，适合需要平衡隐私和效用的大规模大数据应用场景
...
### [177] [Catastrophic Forgetting Resilient One-Shot Incremental Federated Learning](https://arxiv.org/abs/2602.17625)
*Obaidullah Zaland,Zulfiqar Ahmad Khan,Monowar Bhuyan*

Main category: cs.LG

TL;DR: OSI-FL是一种单轮增量联邦学习框架，通过冻结的视觉语言模型提取类别特定嵌入，在服务器端使用预训练扩散模型合成数据，结合选择性样本保留机制缓解灾难性遗忘。
...
Method: OSI-FL采用单轮通信：客户端使用冻结的视觉语言模型提取类别特定嵌入，服务器端使用预训练扩散模型合成类似客户端数据分布的新样本。为应对增量任务和灾难性遗忘，引入选择性样本保留机制，基于样本损失保留每个类别-任务对中最具信息量的top-p样本。

Result: OSI-FL在三个基准数据集上，在类增量和域增量场景中均优于传统联邦学习和单轮联邦学习方法。

Conclusion: OSI-FL是首个同时解决通信开销和灾难性遗忘双重挑战的联邦学习框架，通过单轮通信和选择性样本保留机制，在增量学习场景中实现了优越性能。
...
### [178] [A.R.I.S.: Automated Recycling Identification System for E-Waste Classification Using Deep Learning](https://arxiv.org/abs/2602.17642)
*Dhruv Talwar,Harsh Desai,Wendong Yin,Goutam Mohanty,Rafael Reveles*

Main category: cs.LG

TL;DR: 开发低成本便携式电子废弃物分选系统A.R.I.S.，使用YOLOx模型实时识别金属、塑料和电路板，提高材料回收效率
...
Method: 采用A.R.I.S.（自动化回收识别系统），这是一种低成本便携式电子废弃物分选器，使用YOLOx模型对金属、塑料和电路板进行实时分类，实现低推理延迟和高检测精度

Result: 实验评估显示：总体精度90%，平均精度均值(mAP)82.2%，分选纯度84%。系统通过深度学习与传统分选方法结合，提高了材料回收效率

Conclusion: A.R.I.S.系统降低了先进回收技术的采用门槛，增强了材料回收效率，支持延长产品生命周期、促进以旧换新和回收计划，减少供应链环境影响
...
### [179] [Multi-Round Human-AI Collaboration with User-Specified Requirements](https://arxiv.org/abs/2602.17646)
...
<details>
  <summary>Details</summary>
Motivation: 随着人类在关键决策中越来越依赖多轮对话AI，需要建立原则性框架来确保这种交互能可靠地提升决策质量。当前缺乏能够确保AI不削弱人类优势同时又能补充人类弱点的系统化方法。

Method: 采用以人为中心的视角，基于两个核心原则：反事实伤害（确保AI不削弱人类优势）和互补性（确保AI在人类易犯错处增加价值）。通过用户定义规则形式化这些概念，引入具有有限样本保证的在线、无分布算法，在协作动态中强制执行用户指定的约束。

Result: 在两个交互设置中评估框架：医疗诊断任务的LLM模拟协作和图像推理任务的人类众包研究。在线程序即使在非平稳交互动态下也能维持规定的反事实伤害和互补性违反率。收紧或放松这些约束会产生可预测的下游人类准确率变化。

Conclusion: 反事实伤害和互补性原则可作为实用的调控杠杆，引导多轮协作朝着更好的决策质量发展，无需建模或约束人类行为。该框架为高风险决策中的可靠人机协作提供了原则性方法。
