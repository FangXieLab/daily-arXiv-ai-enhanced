...
### [1] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*

Main category: cs.AI

TL;DR: 论文提出将模型重训练视为计算约束下的近似贝叶斯推断，引入"学习债务"概念，并建立基于决策理论的重训练策略框架。


<details>
  <summary>Details</summary>
Motivation: 传统上模型重训练被视为维护任务，但作者认为应将其理解为计算约束下的近似贝叶斯推断。当前部署的冻结模型与持续更新的信念状态之间存在差距，这种差距被称为"学习债务"，需要建立系统化的决策框架来优化重训练策略。

Method: 提出决策理论框架，将重训练决策建模为成本最小化问题，从损失函数中推导出重训练阈值。该方法将重训练视为计算约束下的近似贝叶斯推断，并基于证据制定触发机制。

Result: 开发出基于证据的触发机制，替代传统的日历调度方式，使治理过程可审计。该框架为模型重训练提供了理论依据和实用工具。

Conclusion: 模型重训练应被视为计算约束下的近似贝叶斯推断过程，通过决策理论框架可以优化重训练策略，实现基于证据的触发机制，提高模型维护的效率和可审计性。
...
TL;DR: 该研究使用演化博弈论建模AI安全治理，将用户信任视为动态监控过程而非一次性选择，发现只有低成本监控与有效惩罚相结合才能实现安全AI的广泛采用。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型通常将用户信任简化为一次性采纳决策，忽略了信任作为动态演化过程的本质。随着AI能力增强和安全问题日益紧迫，需要更精细地建模用户与开发者之间的重复互动，以理解信任演化如何影响AI安全治理。

Method: 采用演化博弈论框架，将用户信任建模为重复非对称互动中的监控减少，其中检查AI行为需要成本。研究包含无限种群复制动态分析、随机有限种群动态模拟以及强化学习（Q-learning）仿真，考察不同监控成本和制度机制下的演化结果。

Result: 研究发现三种稳健的长期演化机制：1）不安全开发且无采纳；2）不安全但广泛采用的系统；3）安全且广泛采用的系统。只有第三种是理想的，其出现条件是：对不安全行为的惩罚超过安全额外成本，且用户仍能负担偶尔监控。透明度、低成本监控和有效制裁是必要条件。

Conclusion: 仅靠监管或盲目用户信任都不足以防止向不安全或低采纳结果的演化漂移。AI安全治理需要透明度、低成本监控和实质性惩罚相结合的制度设计，才能实现安全AI系统的广泛采用和可持续发展。
...
TL;DR: 本文针对多模态大语言模型（MLLMs）中的路由问题，发现传统基于隐藏状态的探针路由在视觉输入存在时性能显著下降，提出了注意力探针和KL正则化LoRA探针两种互补方法来提升MLLMs中的路由效果。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本LLM的探针路由方法在多模态LLMs中性能显著下降，因为视觉输入的存在削弱了隐藏状态中正确性信号的可分离性，使得标准探针设计难以有效提取这些信号。

Method: 提出了两种互补方法：1）注意力探针（Attention Probe）：基于注意力分数聚合前一层的隐藏状态来恢复分布式的正确性信号；2）KL正则化LoRA探针（ReLope）：插入轻量级LoRA适配器并应用KL正则化来学习路由感知的表示。

Result: 综合实验表明，所提出的方法在MLLMs中一致优于基线方法，证明了提升隐藏状态质量对于MLLMs中有效路由的关键作用。

Conclusion: 在MLLMs中，改进隐藏状态的质量是提升路由效果的关键，提出的注意力探针和KL正则化LoRA探针为多模态环境下的有效路由提供了解决方案。
...
### [4] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*
...
Result: 在82个跨架构测试中（模型达67亿参数）实现零哈希不匹配，四个地理分布式节点产生相同输出，356个链上证明交易验证，99000行Rust代码部署三大洲

Conclusion: 平台确定性是AI信任的基础，AI信任本质上是算术问题，确定性推理使验证复杂度从难解成员问题降为O(1)哈希比较
...
TL;DR: 论文探讨AI前端设计的伦理意义，特别是对话式用户界面中拟人化设计对用户心理模型、信任校准和行为反应的影响，并通过Chayn组织的案例说明伦理考量如何指导界面设计中的克制原则。


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表征元素）的伦理意义被忽视。对话式用户界面中日益普遍的拟人化设计元素（对话交互、情感语言、个性模式、拟人隐喻）对用户产生深刻影响，需要伦理审视。

Method: 结合人机交互、对话式AI和价值敏感设计的研究，分析界面如何影响用户期望、信任和自主性。通过Chayn非营利组织（支持性别暴力幸存者）的两个AI系统案例研究，展示创伤知情设计原则如何指导伦理前端设计。

Result: AI前端拟人化设计是价值驱动的选择，会深刻塑造用户心理模型、信任校准和行为反应。界面可能在脆弱情境下错配用户期望、培养错误信任、削弱用户自主性。Chayn案例表明伦理考量可促使设计克制，挑战当前AI产品以参与度为导向的规范。

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑来体现。需要重视前端设计的伦理意义，特别是在脆弱用户群体中，设计克制可能比最大化参与度更符合伦理要求。
...
### [6] [A Public Theory of Distillation Resistance via Constraint-Coupled Reasoning Architectures](https://arxiv.org/abs/2603.25022)
*Peng Wei,Wesley Shu*

Main category: cs.AI

TL;DR: 论文提出了一种理论框架，旨在通过架构设计减少能力蒸馏与治理结构之间的不对称性，使模型能力与内部稳定性约束耦合，从而降低蒸馏作为捷径的价值。


<details>
  <summary>Details</summary>
Motivation: 前沿AI中的知识蒸馏、模型提取和行为转移已成为核心关注点，主要风险不仅是复制，而是有用的能力可能比原本伴随的治理结构更廉价地被转移。这造成了能力与治理之间的不对称性。

Method: 提出了一个约束耦合推理框架，包含四个要素：有界转换负担、路径负载累积、动态演化的可行区域，以及能力-稳定性耦合条件。该框架是公开的、保护商业秘密的理论框架，省略了专有实现细节。

Result: 提供了一个可证伪的架构论点、清晰的威胁模型，以及一组可通过实验验证的假设，为未来蒸馏抵抗、对齐和模型治理研究奠定基础。

Conclusion: 通过将高层能力与塑造状态随时间转换的内部稳定性约束耦合，可以减少蒸馏作为捷径的价值。该理论贡献为构建更安全的AI系统提供了架构层面的指导方向。
...
### [7] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
*Xunguang Wang,Yuguang Zhou,Qingyue Wang,Zongjie Li,Ruixuan Huang,Zhenlan Ji,Pingchuan Ma,Shuai Wang*

Main category: cs.AI

TL;DR: 该论文提出"推理安全"概念，系统定义了9类不安全推理行为，开发了推理安全监控器实时检测推理链中的逻辑错误和对抗攻击，在基准测试中达到84.88%的步骤级定位准确率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全研究主要关注内容安全（有害、偏见或事实错误的输出），而将推理链视为不透明的中间产物。论文认为推理过程本身的安全性是一个正交且同等关键的安全维度，需要确保模型的推理轨迹在逻辑上一致、计算高效且能抵抗对抗性操纵。

Method: 1. 正式定义推理安全并引入9类不安全推理行为的分类法；2. 对4111条推理链进行大规模流行性研究，涵盖自然推理基准和四种对抗攻击方法；3. 提出推理安全监控器：一个外部LLM组件，通过分类法嵌入提示实时检查每个推理步骤，并在检测到不安全行为时发送中断信号。

Result: 在450条推理链的静态基准测试中，监控器达到84.88%的步骤级定位准确率和85.37%的错误类型分类准确率，显著优于幻觉检测器和过程奖励模型基线。所有9类错误类型在实践中都存在，每种攻击都产生机制上可解释的特征。

Conclusion: 推理级监控既是必要的也是实际可行的，推理安全应成为大型推理模型安全部署的基础关注点。该研究为LLM推理过程的安全性提供了系统性框架和实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的评估过于偏重视觉真实性，忽略了理解构建物理世界所需的逐步过程和物理依赖关系，而这些能力对于自动化设计到建造流程至关重要。

Method: 基于住宅木结构建筑领域（具有完全编码化的工程标准和客观可验证的正确性），收集了13种建筑风格的26,000多个结构，开发了确定性的10项结构验证框架，支持模型观察中间建造状态、生成建造动作并接收结构化环境反馈的迭代式智能体交互。

Result: 对最先进的视觉语言模型进行广泛实验，揭示了在现有排行榜上基本不可见的显著能力差距，表明物理有效性是与视觉真实性正交的关键评估维度。

Conclusion: 物理生成推理是多模态智能中一个独特且未充分发展的前沿领域，DreamHouse基准为评估模型在满足几何、结构、可建造性和规范合规性约束方面的能力提供了新方向。
...
Method: 引入LogitScope框架，通过在每个生成步骤计算熵和变熵等信息度量来分析LLM不确定性。该框架模型无关，通过惰性评估实现计算高效，兼容任何HuggingFace模型。

Result: LogitScope能够揭示模型置信度模式、识别潜在幻觉、暴露高不确定性决策点，在不确定性量化、模型行为分析和生产监控等多样化应用中展示其实用性。

Conclusion: LogitScope为研究人员和从业者提供了一种无需标记数据或语义解释即可检查LLM推理行为的工具，有助于提高LLM部署的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 随着加密货币的普及和社交媒体上投机活动的增加，需要系统的方法来识别和分析加密货币相关的预测性陈述，以理解市场情绪和预测模式。

Method: 采用两阶段分类框架：任务1为预测性/非预测性二元分类；任务2将预测性推文细分为增量、减量或中性预测。结合人工和GPT标注构建数据集，使用SenticNet提取情感特征，通过GPT生成改写进行数据增强以解决类别不平衡。评估了机器学习、深度学习和基于Transformer的多种模型。

Result: GPT平衡策略显著提升模型性能：Transformer模型在任务1中取得最高F1分数，而传统机器学习模型在任务2中表现最佳。情感分析揭示了不同加密货币中各类预测类别对应的独特情感模式。

Conclusion: 提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强的数据平衡方法提升了模型性能，情感分析为理解加密货币市场预测行为提供了新视角。
...
TL;DR: 该研究首次大规模分析了电商平台嵌入式购物AI的采用和使用模式，基于携程3100万用户数据发现：AI助手采用者主要为年长、女性、高活跃度用户；AI聊天与传统搜索处于购买旅程的同一阶段且常交替使用；用户主要将AI用于探索性、难以关键词化的任务。


<details>
  <summary>Details</summary>
Motivation: 虽然通用AI工具的采用模式已有研究，但针对电商平台嵌入式购物AI的采用和使用模式缺乏大规模实证证据。本研究旨在填补这一空白，了解消费者如何在实际电商环境中采用和使用平台集成的AI助手。

Method: 使用中国最大在线旅游平台携程的3100万用户数据，研究其LLM-based AI助手"问道"。通过大规模数据分析，考察用户采用模式、AI聊天在购买旅程中的位置、以及与搜索的交互模式。

Result: 发现三个经验规律：1) AI采用者主要为年长消费者、女性用户和高活跃度现有用户，与通用AI工具的年轻男性主导模式相反；2) AI聊天与传统搜索处于购买旅程的同一阶段，常交替使用；3) 消费者主要将AI用于探索性、难以关键词化的任务，景点查询占聊天请求的42%，聊天意图随聊天时间、搜索时机和后续购买类别系统变化。

Conclusion: 嵌入式购物AI并非传统搜索的替代品，而是作为电商中探索性产品发现的补充界面。AI助手主要服务于难以通过关键词搜索完成的探索性任务，与搜索形成互补关系而非替代关系。
...
TL;DR: ScratchMath是一个专门用于解释和分类学生手写数学草稿中错误的新型基准测试，包含1720个中国中小学数学样本，支持错误原因解释和分类两大任务。


<details>
  <summary>Details</summary>
Motivation: 评估学生手写草稿对个性化教育反馈至关重要，但现有教育NLP主要关注文本响应，忽略了手写草稿的复杂性、多模态性和真实性问题解决方法的多样性。当前多模态大语言模型擅长视觉推理，但通常采用"考生视角"，优先生成正确答案而非诊断学生错误。

Method: 引入ScratchMath基准测试，包含1720个中国中小学学生数学样本，支持错误原因解释(ECE)和错误原因分类(ECC)两大任务，定义了七种错误类型。数据集通过严格的人机协作方法进行标注，包括多个阶段的专家标注、审查和验证。系统评估了16个领先的MLLMs模型。

Result: 评估显示，所有模型在视觉识别和逻辑推理方面与人类专家存在显著性能差距。专有模型明显优于开源模型，大型推理模型在错误解释方面表现出强大潜力。

Conclusion: ScratchMath填补了教育评估领域的空白，为诊断学生手写数学草稿中的错误提供了专门的基准测试。评估结果揭示了当前MLLMs在理解复杂手写草稿方面的局限性，为未来研究提供了方向。所有评估数据和框架已公开可用。
...
TL;DR: 该论文提出了一个将大语言模型不确定性分解为三个语义组件的框架：输入歧义、知识缺口和解码随机性，以更好地理解和改进LLM可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前的不确定性评估方法（单一不确定性分数或经典的偶然-认知二分法）无法为改进生成模型提供可操作的见解，且不足以理解LLM中的不确定性。需要一种更细粒度的框架来诊断LLM不确定性的根源。

Method: 提出了一个不确定性分解框架，将LLM不确定性分解为三个语义组件：(1) 输入歧义：由模糊提示引起；(2) 知识缺口：由参数化证据不足引起；(3) 解码随机性：由随机采样引起。通过一系列实验验证该框架的有效性。

Result: 实验表明，这些不确定性组件的主导地位会随模型大小和任务而变化。该框架为审计LLM可靠性和检测幻觉提供了更好的理解，能够支持有针对性的干预措施。

Conclusion: 提出的三组件不确定性分解框架比现有方法提供更细粒度的见解，有助于开发更可信的LLM系统，为针对性改进（如提示工程、知识增强或解码策略调整）铺平道路。
...
TL;DR: 论文提出多视角故障归因范式，解决多智能体系统中故障归因的模糊性问题，并构建首个相关基准MP-Bench
...
Method: 提出多视角故障归因范式，明确考虑归因模糊性；构建MP-Bench基准，这是首个针对多智能体系统多视角故障归因的基准，并设计了专门针对该范式的评估协议

Result: 通过大量实验发现，先前关于LLM在故障归因方面表现不佳的结论很大程度上源于现有基准设计的局限性；结果强调了多视角基准和评估协议对于实现现实可靠的多智能体系统调试的必要性

Conclusion: 多视角故障归因是解决多智能体系统故障归因模糊性的实用范式，MP-Bench基准和相关评估协议为更现实可靠的系统调试提供了必要工具
...
Result: 在PDEBench八个家族上，SAKE是最强的匹配预算低成本选择器：67.8%精确匹配率，91.7%±1窗口内匹配率，6.1%平均遗憾@膝点，成本比0.051（节省94.9%搜索成本）
...
TL;DR: LEKIA 2.0提出了一种情境化LLM架构，通过分离认知层与执行层，构建可持续更新的外部情境结构，解决了情感支持中LLM因无状态特性导致的阶段错位和边界违规问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在心理支持和情感陪伴场景中的核心局限在于其基于局部下一词预测的无状态特性，这导致系统在多轮干预中无法维持时间连续性、阶段意识和用户同意边界，容易出现阶段错位、边界违规等问题。

Method: 提出LEKIA 2.0情境化LLM架构，将认知层与执行层分离，从而解耦情境建模与干预执行。该设计使系统能够在持续交互中维护用户情境和同意边界的稳定表示。

Result: LEKIA在深度干预循环完成度上比仅使用提示的基线方法平均绝对提升了约31%。通过引入静态到动态的在线评估协议验证了其过程控制能力。
...
TL;DR: 压缩视觉语言模型(VLMs)通过剪枝和量化降低计算成本，但会改变内部计算和安全行为：剪枝保持电路结构但旋转衰减特征，量化修改高级电路但保留特征对齐；新基准VLMSafe-420显示剪枝导致拒绝行为显著下降。


<details>
  <summary>Details</summary>
Motivation: 压缩视觉语言模型(VLMs)虽能降低内存和计算成本，但压缩过程可能改变模型的内部计算和安全行为，需要研究压缩如何影响模型内部机制和安全性能。

Method: 使用因果电路分析和crosscoder-based特征比较方法，分析剪枝和量化对代表性VLMs内部机制的影响；开发VLMSafe-420基准，包含有害输入与匹配的良性反事实对，评估不同安全类别下的行为。

Result: 剪枝通常保持电路结构完整但旋转和衰减内部特征，量化修改高级电路但保留的特征对齐更好；剪枝导致真实拒绝行为急剧下降，表明压缩方法选择具有安全影响。

Conclusion: 压缩方法对视觉语言模型的内部机制和安全行为有显著影响，剪枝和量化以不同方式改变模型计算，压缩选择需要考虑安全影响，VLMSafe-420基准有助于评估压缩模型的安全性能。
...
<details>
  <summary>Details</summary>
Motivation: 热带地区（如越南）降水预报面临复杂地形和对流不稳定性挑战，数值天气预报模型存在偏差。现有数据驱动后处理框架依赖逐点目标函数，在轻微时间错位时遭受"双惩罚"效应，需要改进
...
Conclusion: MP-MoE框架能有效捕捉峰值降雨强度并保持风暴事件的形态完整性，为热带地区降水预报提供改进方案
...
TL;DR: 稀疏自编码器特征在视觉语言模型中缺乏模块化组合性，特征联合干预会导致输出漂移和准确率下降，揭示了SAE特征组合的局限性


<details>
  <summary>Details</summary>
Motivation: 验证稀疏自编码器特征是否形成模块化、可组合的推理单元——这是许多基于干预的导向方法的基本假设

Method: 开发可复现的因果管道，在Qwen3-VL-8B中定位和测试稀疏视觉思维电路；使用包含7种任务类型和3个难度级别的合成基准；通过线性探针识别任务类型信息的中层解码器位置；在该层训练SAE，通过显式规则构建任务选择性特征集，并进行推理时缩放和消融

Result: 任务选择性特征集的干预能适度提高推理准确率，但两个特征集的联合干预会可靠地引起输出漂移并降低准确率，即使是在范数匹配的扰动下；这种非模块化电路干扰与共享内部通路一致，其中特征联合会放大激活偏移

Conclusion: SAE特征组合性存在边界限制，特征联合干预会导致非模块化干扰；研究提供了严格的诊断框架，可用于更可靠的视觉语言模型控制，并在多个VLM家族和五个不同数据集上得到验证
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在需要事实基础的高风险多轮交互场景中，其记忆系统通常依赖简单的键值存储或向量检索，缺乏追踪知识来源和可信度的机制，这限制了智能体在关键任务中的可靠性和可审计性。

Method: 系统采用Neo4j知识图谱与Qdrant向量存储的混合架构，通过Cognee SDK实现统一。核心方法包括：完整的认知循环（存储、检索、评分、组合、保护、学习）、混合五源检索管道、十一维竞争性评分引擎、四状态证据验证模型、五阶段上下文生命周期管理、六层廉价优先安全防护管道、AI防火墙、九阶段整合引擎以及多组织身份管理的数字权限模型。

Result: 通过包含2200多个测试的全面测试套件验证了子系统正确性。系统支持三层部署模式、五种预设配置、多网关隔离和管理仪表板，能够从轻量级仅记忆智能体扩展到具有企业级安全性和可审计性的完整认知运行时。

Conclusion: ElephantBroker为LLM智能体提供了可验证、可追溯的持久化记忆系统，解决了当前智能体记忆系统在知识来源追踪和可信度评估方面的不足，支持从轻量级到企业级的多种部署配置，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习(FSCIL)在只有少量标注样本的采集上下文中特别脆弱。触觉感知场景中，采集上下文（如不同设备、接触状态和交互设置）由于缺乏标准化会显著降低性能。需要解决上下文依赖对FSCIL性能的负面影响。
...
Conclusion: CaT-FSCIL通过分解和处理采集上下文的不同分量，有效解决了触觉感知中少样本类增量学习的上下文依赖问题，在标准基准上表现出优越性能。
...
TL;DR: RubricEval：首个面向指令跟随评估的细粒度rubric级别元评估基准，揭示当前rubric评估方法可靠性不足，GPT-4o在困难子集上仅55.97%准确率


<details>
  <summary>Details</summary>
Motivation: 尽管基于rubric的评估已成为评估大语言模型指令跟随能力的主流范式，但其可靠性尚不明确。现有元评估主要关注响应层面，缺乏对rubric级别细粒度判断准确性的评估，需要填补这一空白

Method: 提出RubricEval基准，包含：(1)首个指令跟随的rubric级别元评估基准；(2)涵盖多类别、多模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及能更好区分评估者性能的Easy/Hard子集

Result: 实验显示rubric级别评估远未解决：广泛使用的GPT-4o在Hard子集上仅55.97%准确率。rubric级别评估优于checklist级别，显式推理提高准确性，两者结合减少评估者间方差。通过建立的rubric分类法识别常见失败模式

Conclusion: RubricEval为指令跟随评估提供了首个细粒度元评估基准，揭示了当前rubric评估方法的局限性，并为可靠评估提供了可操作的见解和失败模式分析
...
TL;DR: UniAI-GraphRAG是一个增强的图检索增强生成框架，通过本体引导知识提取、多维社区聚类策略和双通道图检索融合，提升了复杂推理和多跳查询性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在复杂推理、多跳查询和领域特定QA方面面临挑战，传统GraphRAG框架在跨行业适应性、社区报告完整性和检索性能方面仍有局限，需要更先进的解决方案。

Method: 提出三个核心创新：1) 本体引导知识提取，使用预定义模式指导LLMs准确识别领域特定实体和关系；2) 多维社区聚类策略，通过对齐补全、基于属性的聚类和多跳关系聚类提高社区完整性；3) 双通道图检索融合，通过混合图和社区检索平衡QA准确性和性能。

Result: 在MultiHopRAG基准测试中，UniAI-GraphRAG在综合F1分数上优于主流开源解决方案（如LightRAG），特别是在推理和时间查询方面表现突出。

Conclusion: UniAI-GraphRAG通过创新的知识提取、社区聚类和检索融合方法，显著提升了RAG系统在复杂场景下的性能，为跨行业应用提供了更有效的解决方案。
...
TL;DR: 该论文提出了一个AI辅助安全分析的正式框架，揭示了AI助手在物理AI系统安全工程中可能引发的"能力阴影"问题，即AI生成的分析会系统性地限制人类考虑的范围。


<details>
  <summary>Details</summary>
Motivation: 随着AI助手被整合到物理AI系统的安全工程工作流程中，需要回答一个关键问题：AI辅助是提高了安全分析质量，还是引入了系统性盲点，这些盲点只有在部署后的事故中才会显现？传统基准测试方法无法评估安全工程能力，因为安全能力是多维的、依赖于上下文、具有固有的不完整性，且存在合理的专家分歧。

Method: 1. 建立了一个五维能力框架，涵盖领域知识、标准专业知识、操作经验、上下文理解和判断力；2. 提出了"能力阴影"概念，即AI生成的安全分析所引发的人类推理系统性窄化；3. 形式化了四种典型的人机协作结构；4. 推导了闭式性能边界，证明能力阴影以乘法方式复合，导致性能退化远超简单的加法估计。

Result: 核心发现表明：AI辅助安全工程是一个协作设计问题，而非软件采购决策。同一工具可能改善或降低分析质量，完全取决于使用方式。论文推导了抗阴影工作流的非退化条件，并证明能力阴影的复合效应导致性能退化远超预期。

Conclusion: 需要从工具认证转向工作流认证，以确保物理AI系统的可信度。AI辅助安全分析的质量取决于协作结构设计，而非工具本身。必须建立能够抵抗能力阴影的工作流程，才能真正实现可信的物理AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 步态越来越被认为是重要的生命体征，但目前的方法将其视为特定病理的症状而非系统性生物标志物。研究旨在开发步态基础模型，探索步态作为多系统生物信号的潜力。

Method: 使用深度相机记录3,414名深度表型化成年人在五种运动任务中的3D骨骼运动数据，构建步态基础模型，学习运动嵌入特征，并与工程特征进行比较。通过解剖学消融分析不同身体部位对表型预测的贡献。
...
Conclusion: 研究确立了步态作为独立的多系统生物信号，为将其转化为消费级视频技术并作为可扩展、被动的生命体征进行整合提供了理论基础。
...
TL;DR: 提出SliderQuant框架，通过自适应滑动量化解决LLM不同层对量化敏感度差异的问题，在低比特量化下显著优于现有PTQ方法。
...
Result: 在基础语言生成、零样本常识推理以及数学和代码任务上，使用Llama/Llama2/Llama3/Qwen2.5、DeepSeek-R1蒸馏模型和大规模MoE模型进行测试，SliderQuant在仅权重量化和权重-激活量化方面均优于现有PTQ方法（包括使用旋转变换的最新方法）。

Conclusion: SliderQuant通过考虑LLM不同层对量化的敏感度差异，采用自适应滑动量化策略，有效减少了各层的量化误差，在低比特量化场景下表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 真实世界DAG数据集稀缺，因为构建需要专家解释领域文档。文档可能允许多种合理抽象，预期结构通常隐含，支持证据分散在文本、方程、标题和图表中。

Method: 利用包含显式DAG图的科学论文作为监督源，提出DAGverse框架。其核心组件DAGverse-Pipeline是半自动系统，通过图分类、图重建、语义接地和验证来产生高精度语义DAG示例。
...
TL;DR: 该论文提出了一个评估AI有害操纵的框架，通过跨领域和跨地域的人机交互研究，发现AI模型能够产生操纵行为并影响用户信念和行为，且操纵效果因领域和地域而异。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI驱动有害操纵的方法有限，需要开发一个系统性的评估框架来理解AI模型在不同高风险场景下的操纵能力及其影响。

Method: 提出了一个基于上下文特定人机交互研究的评估框架，在三个AI应用领域（公共政策、金融、健康）和三个地区（美国、英国、印度）对10,101名参与者进行测试，评估AI模型的操纵倾向和效果。

Result: 测试模型在被提示时能够产生操纵行为，并在实验环境中能够诱导参与者的信念和行为改变。操纵效果在不同领域和地域存在显著差异，且模型的操纵倾向与操纵成功率之间没有一致的相关性。

Conclusion: AI有害操纵需要在具体使用场景中评估，跨领域和跨地域的差异表明评估需要针对特定上下文，操纵倾向和效果应分开研究。论文提供了测试协议和材料以促进框架的采用，并讨论了评估AI有害操纵的开放挑战。
...
TL;DR: 该研究分析了深度强化学习（DRL）在混合交通中控制自动驾驶车辆时的宏观交通流特性和燃油效率，发现相比传统IDM模型，DRL能提升约7.52%的道路容量，并在高速下提升约28.98%的燃油效率。


<details>
  <summary>Details</summary>
Motivation: 传统跟车模型（如IDM）在多样化交通场景中泛化能力有限，且通常不考虑燃油效率。虽然DRL在微观跟车性能上表现良好，但其宏观交通流特性尚未充分探索，需要研究DRL在混合交通中的宏观表现和燃油效率。

Method: 使用Twin Delayed Deep Deterministic Policy Gradient (TD3)算法控制自动驾驶车辆，基于NGSIM高速公路数据集训练，实现与人类驾驶车辆的真实交互。通过基本图（FD）评估交通性能，考虑驾驶员异质性、异质时间间隔渗透水平和不同RL车辆比例，并与IDM模型进行宏观燃油效率比较。

Result: 交通性能对安全时间间隔分布和RL车辆比例敏感。从完全人类驾驶过渡到完全RL控制交通可将道路容量提升约7.52%。RL自动驾驶车辆在高速（>50 km/h）下相比IDM提升约28.98%的平均燃油效率，低速（<50 km/h）下提升约1.86%。

Conclusion: DRL框架在不牺牲安全性的前提下，能够显著提升交通容量和燃油效率，为混合交通中自动驾驶车辆的控制提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究算术谜题游戏中的难度结构，为自适应学习系统中的数学推理任务难度评估提供理论基础，解决现有机器学习模型难以可靠区分简单实例的问题。

Method: 开发精确的动态规划求解器，枚举可达目标并提取最小操作见证；构建超过340万个实例的数据集；定义难度为达到目标所需的最小操作数；分析难度与求解器导出特征的关系。

Result: 基于袋级和目标级统计的基线机器学习模型只能部分预测可解性，但无法可靠区分简单实例；难度完全由从精确见证中导出的少量可解释结构属性决定，特别是最小构造中使用的输入值数量是难度的最小充分统计量。

Conclusion: 研究提供了透明、计算基础的谜题难度解释，桥接了符号推理和数据驱动建模，支持可解释的难度估计和原则性任务排序，对自适应算术学习和智能练习系统有直接应用价值。
...
TL;DR: 研究验证了基于5W3H的PPS结构化意图表示框架在多语言和多模型中的泛化能力，发现AI辅助扩展的提示与人工编写的效果相当，且能减少跨模型输出方差


<details>
  <summary>Details</summary>
Motivation: 验证PPS结构化意图表示框架是否能在不同语言和模型中泛化，扩展先前仅针对中文的研究，探索AI辅助扩展提示的可行性，以及结构化表示对跨模型输出一致性的影响

Method: 采用5W3H的PPS框架，在三个语言（英语、日语、中文）和四个条件（包括AI辅助扩展的5W3H提示）下，使用三个大语言模型处理60个任务，共生成2,160个模型输出进行分析

Result: AI扩展的5W3H提示在目标对齐上与人工编写的无显著差异；结构化条件能减少或重塑跨模型输出方差；非结构化提示存在双重膨胀偏差：人工高复合分数和人工低表观方差
...
TL;DR: 该论文提出了一种用于具身语义场景图生成的模块化导航组件，通过改进优化算法和动作表示来提升有限动作预算下的场景探索效率。


<details>
  <summary>Details</summary>
Motivation: 在有机计算中，语义世界模型使具身智能体能够超越纯几何表示进行推理。核心挑战是在有限动作预算内获取最大化模型质量和下游有用性的观测。语义场景图为此提供了结构化紧凑表示，但有限动作范围内的构建需要权衡信息增益与导航成本。

Method: 提出了模块化导航组件，用现代优化方法替代策略优化方法，重新审视离散动作表示。研究了紧凑和细粒度的大规模离散动作集，比较了原子动作的单头策略与动作组件的分解多头策略。评估了课程学习和可选的基于深度的碰撞监督。

Result: 仅替换优化算法就能在相同奖励塑造下将SSG完整性相对基线提高21%。深度主要影响执行安全性（无碰撞运动），而完整性基本不变。现代优化与细粒度分解动作表示结合产生了最强的完整性-效率权衡。

Conclusion: 通过现代优化算法和细粒度分解动作表示的结合，实现了语义场景图生成中导航效率与完整性的最佳平衡，为有限动作预算下的具身探索提供了有效解决方案。
...
### [33] [EcoThink: A Green Adaptive Inference Framework for Sustainable and Accessible Agents](https://arxiv.org/abs/2603.25498)
*Linxiao Li,Zhixiang Lu*

Main category: cs.AI

TL;DR: EcoThink：一个能量感知的自适应推理框架，通过轻量级路由器动态评估查询复杂度，跳过简单查询的推理过程，为复杂逻辑保留深度计算，平均减少40.4%的推理能耗，最高可达81.9%。


<details>
  <summary>Details</summary>
Motivation: 随着Web从静态检索转向生成式交互，大语言模型日益增长的环境足迹成为关键可持续性挑战。当前范式不加区分地对数十亿日常查询应用计算密集型策略（如思维链），导致LLM过度思考，这种冗余放大了碳排放并造成操作障碍，直接阻碍了联合国可持续发展目标13（气候行动）和10（减少不平等）的实现，特别是在资源受限地区阻碍了公平的AI访问。

Method: EcoThink采用基于蒸馏的轻量级路由器动态评估查询复杂度，对事实检索类查询跳过不必要的推理过程，仅为复杂逻辑问题保留深度计算。该框架实现了能量感知的自适应推理机制。

Result: 在9个多样化基准测试上的广泛评估表明，EcoThink平均减少40.4%的推理能耗，在Web知识检索任务中最高可减少81.9%的能耗，且没有统计上显著的性能损失。

Conclusion: 通过减少算法浪费，EcoThink为实现可持续、包容和节能的生成式AI智能体提供了可扩展的路径，有助于调和高性能AI智能与环境责任之间的矛盾。
...
TL;DR: Voxtral TTS是一个多语言语音合成模型，仅需3秒参考音频即可生成自然语音，采用自回归语义token生成与流匹配声学token的混合架构，在语音克隆任务中优于ElevenLabs Flash v2.5。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够从极短参考音频（仅3秒）生成自然、富有表现力的多语言语音的文本转语音系统，解决现有模型在语音克隆自然度和表现力方面的不足。

Method: 采用混合架构：1）使用自回归生成语义语音token；2）使用流匹配生成声学token；3）使用Voxtral Codec语音tokenizer，该tokenizer采用混合VQ-FSQ量化方案从头训练。

Result: 在由母语者进行的人类评估中，Voxtral TTS在多语言语音克隆任务中因其自然度和表现力而受到青睐，以68.4%的胜率优于ElevenLabs Flash v2.5模型。

Conclusion: Voxtral TTS是一个有效的多语言语音合成模型，能够从极短参考音频生成自然、富有表现力的语音，在语音克隆任务中表现优于现有先进模型，模型权重已按CC BY-NC许可证发布。
...
TL;DR: RC2：通过强化学习框架利用跨模态不一致性作为学习信号，强制实现跨模态循环一致性以提升推理准确性


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在处理相同概念时，视觉和文本表示往往产生矛盾预测，标准投票机制会放大系统偏差。跨模态不一致性实际上提供了丰富的自然学习信号。

Method: 引入RC2强化学习框架，通过强制跨模态循环一致性来解决内部冲突。要求模型执行反向推理、切换模态，并通过前向推理可靠地重建答案，从而获得密集的无标签奖励。

Result: 优化这种结构缓解了模态特定错误，将推理准确性提高了多达7.6个百分点。

Conclusion: 高级推理不仅来自数据扩展，还来自强制对世界形成结构一致的理解。跨模态循环一致性是提升多模态模型性能的有效方法。
...
### [36] [Training the Knowledge Base through Evidence Distillation and Write-Back Enrichment](https://arxiv.org/abs/2603.25737)
*Yuxing Lu,Xukai Zhao,Wei Wu,Jinzhuo Wang*

Main category: cs.AI

TL;DR: WriteBack-RAG框架将知识库视为可训练组件，通过标注示例识别检索成功案例，提取相关文档并蒸馏为紧凑知识单元，与原始语料一起索引，作为离线预处理步骤提升各种RAG系统性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统的知识库通常一次性构建后不再更新，而查询所需的事实往往分散在多个文档中且被无关内容淹没。需要将知识库视为可训练组件，通过优化语料本身来提升检索效果。

Method: 提出WriteBack-RAG框架：1) 使用标注示例识别检索成功案例；2) 隔离相关文档；3) 将相关文档蒸馏为紧凑的知识单元；4) 将蒸馏后的知识单元与原始语料一起索引。该方法仅修改语料，可作为离线预处理步骤与任何RAG管道结合。

Result: 在四种RAG方法、六个基准测试和两个LLM骨干上，WriteBack-RAG在所有评估设置中均取得改进，平均提升+2.14%。跨方法转移实验表明，蒸馏的知识对未参与训练的其他RAG管道也有益，证实改进确实源于语料本身。

Conclusion: 将知识库视为可训练组件并通过WriteBack-RAG框架进行优化，能够显著提升RAG系统的性能。该方法通过改进语料本身而非检索算法，为RAG系统提供了一种通用且有效的增强方式。
...
TL;DR: SITH是一种无需数据、无需训练的可解释性框架，直接在权重空间分析CLIP视觉Transformer，通过奇异值分解和COMP算法为注意力头提供语义解释，并支持精确的权重编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的可解释性方法存在数据集依赖、易受数据偏见影响、解释粒度粗（仅到头级别）等问题。需要开发更鲁棒、更精细的权重空间分析方法来理解视觉语言模型的内部机制。

Method: 提出SITH框架：1）对每个注意力头的值输出矩阵进行奇异值分解；2）使用COMP（相干正交匹配追踪）算法将奇异向量解释为稀疏、语义连贯的人类可理解概念组合；3）通过重建保真度和可解释性实验验证解释质量。

Result: SITH能产生连贯、忠实（faithful）的注意力头内部解释。支持精确、可解释的权重空间模型编辑，可放大或抑制特定概念，无需重新训练即可提升下游性能。还揭示了微调主要通过重新加权稳定的语义基而非学习全新特征来实现模型适应。

Conclusion: SITH提供了一种数据无关、训练无关的Transformer头语义检查方法，实现了细粒度的权重空间可解释性，支持精确模型编辑，并为理解模型适应机制提供了新视角。
...
### [38] [ReDiPrune: Relevance-Diversity Pre-Projection Token Pruning for Efficient Multimodal LLMs](https://arxiv.org/abs/2603.24680)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算开销大，因为Transformer需要处理大量视觉token。现有投影后剪枝方法在压缩表示上操作，会丢失细粒度空间和语义信息。

Method: 在视觉编码器输出和投影器之间插入ReDiPrune，使用轻量级规则为每个token评分，综合考虑文本条件相关性和最大-最小多样性，选择既与查询相关又非冗余的token。

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善准确率-效率权衡。在EgoSchema上，仅保留15%视觉token可使LLaVA-NeXT-Video-7B准确率提升2.0%，同时计算量减少6倍以上。

Conclusion: ReDiPrune是一种高效、无需训练的多模态模型token剪枝方法，通过在投影前直接选择信息丰富的视觉token，在显著减少计算量的同时提升模型性能，具有即插即用优势。
...
TL;DR: 论文提出了一种诊断多模态大模型中上下文学习敏感性的认知框架，构建了大规模数据集和基准测试，并设计了轻量级模块来稳定少样本适应性能。


<details>
  <summary>Details</summary>
Motivation: 多模态统一模型中的上下文学习对示例选择和格式高度敏感，存在跨模态干扰和认知需求差异问题，导致性能非单调且高度任务依赖，需要系统诊断和解决方案。

Method: 1) 提出六层能力导向分类法，从基础感知到高阶辨别对演示功能进行系统分类；2) 构建UniICL-760K大规模语料库，包含15个子任务的8-shot上下文学习片段；3) 设计UniICL-Bench用于严格评估；4) 提出Context-Adaptive Prototype Modulator轻量级即插即用模块来稳定少样本适应。

Result: 在UniICL-Bench上的评估显示，该方法在大多数理解型上下文学习任务上超越了更大参数的多模态大语言模型基线，取得了高度竞争力的统一结果。

Conclusion: 通过认知框架、大规模数据和轻量级架构干预，能够有效诊断和缓解多模态统一模型中上下文学习的敏感性，提升少样本适应性能，为多模态上下文学习提供了系统性解决方案。
...
### [40] [BCMDA: Bidirectional Correlation Maps Domain Adaptation for Mixed Domain Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2603.24691)
...
Method: 提出双向相关图域适应框架，包含知识迁移虚拟域桥接和原型对齐伪标签校正。KTVDB利用双向相关图合成标注和未标注图像，通过固定比例和渐进动态MixUp生成虚拟图像，使用双重双向CutMix实现知识迁移。PAPLC采用可学习原型余弦相似度分类器进行虚拟域与真实域间的双向原型对齐，并通过原型伪标签校正生成更可靠的伪标签

Result: 在三个公开多域数据集上的实验表明该方法具有优越性，特别是在标注样本非常有限的情况下仍能表现出色

Conclusion: BCMDA框架有效解决了混合域半监督医学图像分割中的域偏移和确认偏差问题，通过虚拟域桥接促进跨域学习，通过原型对齐改善特征表示，在有限标注条件下实现了优异性能
...
### [41] [LLaVA-LE: Large Language-and-Vision Assistant for Lunar Exploration](https://arxiv.org/abs/2603.24696)
*Gokce Inal,Pouyan Navard,Alper Yilmaz*

Main category: cs.CV

TL;DR: LLaVA-LE是一个专门用于月球表面和次表面特征分析的多模态视觉语言模型，通过构建大规模月球数据集LUCID并进行两阶段训练，在月球地形分析任务上显著超越了基础模型。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态视觉语言模型在视觉-文本联合推理方面取得了进展，但在行星科学领域的应用仍处于探索阶段。主要障碍是缺乏将真实行星图像与详细科学描述配对的大规模数据集。本研究旨在开发专门用于月球探索的视觉语言模型。

Method: 1. 构建LUCID数据集：包含96k高分辨率全色图像与详细地形描述配对，以及从约20k图像中衍生的81k问答对。2. 两阶段训练课程：第一阶段进行领域特定地形描述的概念对齐；第二阶段进行指令调优的视觉问答训练。

Result: 与GPT和Gemini评估相比，LLaVA-LE相比基础LLaVA实现了3.3倍的整体性能提升，相比第一阶段模型提升了2.1倍，推理得分达到1.070，超过了评估者自身的参考分数。

Conclusion: 领域特定的多模态数据和指令调优能有效推进视觉语言模型在行星探索中的应用。LLaVA-LE展示了在月球地形分析任务上的显著性能提升，为行星科学领域的多模态AI应用提供了新途径。
...
### [42] [Lookalike3D: Seeing Double in 3D](https://arxiv.org/abs/2603.24713)
*Chandan Yeshwanth,Angela Dai*

Main category: cs.CV

TL;DR: 提出室内场景中的相似物体检测任务，利用多视角图像通过Lookalike3D模型识别相同、相似和不同的物体对，并构建3DTwins数据集，在3D感知任务中取得显著改进。
...
Method: 提出Lookalike3D多视角图像Transformer模型，利用大型图像基础模型的强语义先验来区分物体对；构建3DTwins数据集，包含基于ScanNet++的76k人工标注的相同、相似和不同物体对。

Result: 在3DTwins数据集上相比基线方法提升了104% IoU；能够改进下游任务如联合3D物体重建和部件共分割，将重复和相似物体转化为一致高质量3D感知的强大线索。

Conclusion: 室内场景中的相似物体检测是一个重要任务，Lookalike3D模型通过利用重复物体的互补信息显著提升了3D感知能力，为一致的3D理解提供了新方法。
...
### [43] [Accurate Point Measurement in 3DGS -- A New Alternative to Traditional Stereoscopic-View Based Measurements](https://arxiv.org/abs/2603.24716)
*Deyan Deng,Rongjun Qin*

Main category: cs.CV

TL;DR: 3DGS实现高精度三维点测量：通过多视图一致性点选取与三角测量，无需立体工作站，精度超越传统立体测量和网格方法


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在实时渲染方面表现出色，但其在精确几何测量方面的潜力尚未充分开发。传统立体测量需要昂贵工作站和专业人员，而基于网格的直接测量往往不完整且不准确。

Method: 利用3DGS作为新型视图合成器，能够精确渲染源视图并平滑插值中间视图。用户可在不同视图中直观选取一致性点，然后通过三角测量生成精确的三维点测量。支持多视图交会（超过两个视图）以提高测量精度。

Result: 在无人机航拍数据集上验证，该方法实现了高精度点测量，精度匹配或超越传统立体方法。RMSE在1-2厘米范围内，在薄结构上（网格方法RMSE 0.062m）达到0.037m，在尖锐角点（网格方法完全失败）上实现0.013m RMSE。

Conclusion: 3DGS不仅可用于高质量视图合成，还能作为精确几何测量的有效工具。该方法无需立体工作站和专业立体视觉能力，通过多视图一致性点选取和三角测量，实现了超越传统网格方法的高精度测量。
...
### [44] [Scalable Object Relation Encoding for Better 3D Spatial Reasoning in Large Language Models](https://arxiv.org/abs/2603.24721)
...
<details>
  <summary>Details</summary>
Motivation: 3D场景-语言配对数据有限，现有方法存在局限性：基于绝对位置编码的方法难以从过早融合的特征中提取空间关系；显式编码所有对象间关系的方法（二次复杂度）可扩展性差。

Method: 提出QuatRoPE位置编码方法，使用四元数表示3D坐标，通过注意力层的点积显式计算成对空间关系，复杂度与对象数量呈线性关系。引入IGRE机制，将QuatRoPE的影响限制在对象相关token上，避免干扰LLM原有位置编码。

Result: 实验证明方法有效，QuatRoPE的3D坐标整体向量编码保证了高度的空间一致性，IGRE成功限制了QuatRoPE对LLM原有位置编码的影响。

Conclusion: QuatRoPE解决了现有3D空间推理方法的可扩展性和效率问题，通过线性复杂度的位置编码和隔离机制，在保持LLM原有能力的同时提升了空间推理性能。
...
TL;DR: 提出一种用于3D高斯泼溅的自监督置信度框架，通过动态平衡光度与几何监督，结合颜色/法线方差惩罚和改进的外观模型，实现高效的无边界网格提取


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)虽然加速了从姿态图像中提取网格的过程，但在具有丰富视角依赖效应的场景中，网格提取仍然困难。现有方法依赖多视图技术、迭代网格提取或大型预训练模型，牺牲了3DGS的固有效率。

Method: 1) 引入自监督置信度框架，可学习的置信度值动态平衡光度与几何监督；2) 扩展置信度驱动公式，引入惩罚每个基元颜色和法线方差的损失函数；3) 改进外观模型，解耦D-SSIM损失的各个项。

Result: 该方法在无边界网格提取方面取得了最先进的结果，同时保持了高效率，解决了视角依赖效应场景中的歧义问题。

Conclusion: 通过自监督置信度框架、方差惩罚损失和改进的外观模型，实现了高效且准确的3D高斯泼溅网格提取，在保持3DGS固有效率的同时提升了表面提取质量。
...
### [46] [A Framework for Generating Semantically Ambiguous Images to Probe Human and Machine Perception](https://arxiv.org/abs/2603.24730)
*Yuqi Hu,Vasha DuTell,Ahna R. Girshick,Jennifer E. Corbett*

Main category: cs.CV

TL;DR: 该研究利用语义模糊图像作为可解释性探针，通过心理物理学框架在CLIP嵌入空间中插值生成连续模糊图像谱，比较人类和机器分类器在概念边界上的差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类和机器视觉系统如何处理语义模糊性。经典的鸭兔错觉表明，当视觉证据模糊时，人脑必须决定看到什么。研究者想知道人类观察者在何处划分"鸭子"和"兔子"的界限，以及机器分类器是否在同一位置划分。这有助于理解人类与模型的对齐、鲁棒性、模型可解释性和图像合成方法。

Method: 提出一个心理物理学框架，在CLIP嵌入空间中对概念进行插值，生成连续的模糊图像谱。通过这种方法精确测量人类和机器分类器如何放置语义边界。使用语义模糊图像作为可解释性探针，暴露视觉模型如何表示概念之间的边界。

Result: 研究发现机器分类器更偏向于看到"兔子"，而人类更符合用于合成的CLIP嵌入。指导尺度对人类敏感性的影响比对机器分类器更强。机器分类器与人类在概念边界划分上存在系统性差异。

Conclusion: 受控模糊性可以作为诊断工具，弥合人类心理物理学分析、图像分类和生成图像模型之间的差距。该框架为理解人类-模型对齐、鲁棒性、模型可解释性和图像合成方法提供了新视角，展示了如何利用语义模糊性来揭示视觉表示的本质差异。
...
### [47] [OpenCap Monocular: 3D Human Kinematics and Musculoskeletal Dynamics from a Single Smartphone Video](https://arxiv.org/abs/2603.24733)
*Selim Gilon,Emily Y. Miller,Scott D. Uhlrich*

Main category: cs.CV

TL;DR: OpenCap Monocular：一种从单部智能手机视频估计3D骨骼运动学和动力学的新算法，通过优化、物理模拟和机器学习实现，在行走、深蹲和坐站任务中验证了准确性，并通过云端部署提供免费可访问的生物力学评估。


<details>
  <summary>Details</summary>
Motivation: 量化人体运动学和肌肉骨骼力对于预测、治疗和监测与移动性相关的疾病至关重要，但传统方法需要在专业实验室进行成本高昂、耗时的分析，限制了临床转化。需要可扩展、准确的生物力学评估工具。

Method: 通过优化单目姿态估计模型（WHAM）的3D人体姿态估计，计算生物力学约束骨骼模型的运动学，并通过基于物理的模拟和机器学习估计动力学。算法通过智能手机应用、网页应用和安全云计算部署。

Result: 与基于标记的运动捕捉和测力板数据相比，OpenCap Monocular在行走、深蹲和坐站任务中表现出低运动学误差（旋转自由度平均绝对误差4.8°；骨盆平移3.4 cm），旋转精度比仅回归的计算机视觉基线提高48%，平移精度提高69%。动力学估计与先前的双摄像头系统相当或更好。

Conclusion: OpenCap Monocular能够从单部智能手机视频准确估计3D骨骼运动学和动力学，在衰弱和膝骨关节炎相关应用中具有临床意义的准确性，通过免费云端部署实现了可访问的单智能手机生物力学评估。
...
### [48] [TIGeR: A Unified Framework for Time, Images and Geo-location Retrieval](https://arxiv.org/abs/2603.24749)
...
<details>
  <summary>Details</summary>
Motivation: 数字取证、城市监测和环境分析等实际应用需要联合推理视觉外观、地理位置和时间信息。现有方法主要关注标准的地理定位和拍摄时间预测，但实际应用需要更复杂的能力，如检索在查询图像相同位置但指定目标时间拍摄的图像。

Method: 提出TIGeR模型，基于多模态Transformer架构，将图像、地理位置和时间映射到统一的时空嵌入空间。支持灵活输入配置（单模态和多模态查询），使用相同表示执行地理定位、拍摄时间预测和时空感知检索。通过更好地保留大外观变化下的位置身份，实现基于场景时空而非纯视觉相似性的检索。

Result: 在包含450万训练三元组和8.6万高质量评估三元组的基准上，TIGeR显著优于强基线和最先进方法：季节预测提升16%，时间预测提升8%，时空感知检索召回率提升14%。

Conclusion: 统一的时空建模方法在联合推理视觉外观、地理位置和时间信息方面具有显著优势，TIGeR模型能够有效处理复杂时空感知检索任务，为实际应用提供强大支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过2D基础模型先验和手工正则化处理动态场景重建，但在极端新视角和高度关节运动情况下效果不佳。需要更有效的几何一致性约束方法。

Method: 1) 利用动态对象的静态预扫描作为显式几何和外观先验；2) 将高斯原语组织成锚定在物体表面的像素网格，建立网格结构和表面对齐模型；3) 利用原语的网格结构，使用CNN参数化运动，注入强隐式正则化并关联相邻点运动
...
TL;DR: AVControl：基于LTX-2音频视觉基础模型的轻量可扩展框架，通过并行画布和LoRA适配器实现多模态控制，无需架构修改，在视频生成控制任务上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有视频和音频生成控制方法存在局限性：要么为固定控制集训练单一庞大模型，要么为每个新模态引入昂贵的架构修改。需要一种轻量、可扩展的框架来支持多样化的控制模态

Method: 基于LTX-2联合音频视觉基础模型构建AVControl框架，每个控制模态作为独立的LoRA适配器训练，使用并行画布提供参考信号作为注意力层的额外token，无需修改基础模型架构

Result: 在VACE基准测试中，在深度和姿态引导生成、修复和外绘任务上优于所有基线方法，在相机控制和音频视觉基准上取得竞争性结果。支持多种独立训练模态，计算和数据效率高

Conclusion: AVControl提供了一种轻量、可扩展的视频和音频生成控制框架，通过并行画布和LoRA适配器有效解决了结构控制问题，支持多种模态独立训练，显著降低了计算和数据需求
...
<details>
  <summary>Details</summary>
Motivation: 揭示扩散变换器在生成任务中的隐藏潜力，发现去噪过程中引入学习缩放参数可以显著提升DiT块性能，但需要参数高效的方法来优化DiT组件校准

Method: 提出Calibri方法：1) 引入单个学习缩放参数优化DiT块；2) 将DiT校准建模为黑盒奖励优化问题；3) 使用进化算法高效求解；4) 仅修改约100个参数实现参数高效优化

Result: Calibri在各种文本到图像模型中一致提升性能，尽管设计轻量；显著减少图像生成所需的推理步骤，同时保持高质量输出

Conclusion: Calibri展示了通过参数高效校准扩散变换器组件可以显著提升生成质量并加速推理，为扩散模型优化提供了新思路
...
TL;DR: 提出基于可解释AI指导的编码器塑形框架，通过生成注意力场来优化复杂腹主动脉瘤CT图像分割，解决模型关注无关结构或忽略低对比度目标的问题。


<details>
  <summary>Details</summary>
Motivation: 复杂腹主动脉瘤CT图像分割常因模型关注无关结构或忽略薄层低对比度目标而失败，需要优化模型的注意力分布以提高分割可靠性。

Method: 提出XAI指导的编码器塑形框架：1)从最终编码器块计算密集的基于归因的注意力场；2)将预测概率质量与注意力场对齐；3)将注意力场输入轻量级细化路径和置信度先验，在推理时调制logits以抑制干扰并保留细微结构。

Result: 在临床验证的挑战性病例上评估，相比基础SAM设置，该方法在失败倾向场景中取得了显著改进，表明通过XAI指导显式优化编码器注意力是复杂场景中可靠分割的有效原则。

Conclusion: 将归因指导整合到表示和解码过程中，通过显式优化编码器注意力分布，可有效提高复杂腹主动脉瘤CT图像分割的可靠性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型(VLMs)的成功通常依赖于数十亿样本的数据集，这构成了研究进展的重大障碍。虽然最近的研究通过提高监督质量来缩小这一差距，但每个方法只解决了对比预训练中的部分弱点。

Method: GoldiCLIP框架基于Goldilocks原则，寻找监督信号的恰当平衡，包含三个关键创新：1)文本条件自蒸馏方法，对齐文本无关和文本条件特征；2)集成解码器的编码器，使用VQA目标使编码器能够超越标题式查询进行泛化；3)基于不确定性的加权机制，自动平衡所有异构损失。

Result: 仅用3000万图像（比领先方法少300倍）训练，GoldiCLIP在数据高效方法中达到最先进水平：在MSCOCO检索上比最佳可比基线提高2.2分，细粒度检索提高2.0分，基于问题的检索提高5.9分，同时与十亿规模模型保持竞争力。

Conclusion: GoldiCLIP通过协同结合多种监督信号，在数据效率方面取得了显著突破，证明了通过精心设计的训练框架，可以在远少于传统方法的数据量下实现强大的视觉语言表示学习。
...
TL;DR: 该论文开发了一种基于深度学习的针道感染分类方法，使用注意力机制和高效冗余重建卷积技术，在针道感染数据集上取得了优异的分类性能。


<details>
  <summary>Details</summary>
Motivation: 针道是外固定器与骨骼连接的接口，容易发生感染，导致患者疼痛和并发症。目前缺乏基于视觉特征的针道感染自动识别方法，需要开发有效的分类技术来改善针道感染的管理。

Method: 收集并构建针道伤口感染数据集，提出基于注意力的深度学习模型，强调相关区域并减少针体干扰。引入高效冗余重建卷积方法增强特征图丰富性同时减少参数数量。

Result: 模型在AUC（0.975）和F1分数（0.927）上优于基线方法，仅需5.77M参数。能够基于视觉感染征象区分针道状态，与医疗专业人员评估一致。
...
TL;DR: 提出一种新颖的对抗攻击框架，能够同时攻击密度图和点回归两种人群计数范式，通过多任务损失优化实现跨范式攻击


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击研究主要关注密度图模型之间的可迁移性，但跨范式攻击（密度图与点回归之间）尚未探索。考虑到人群计数在安全领域的重要性，研究模型鲁棒性对抗攻击具有实际意义

Method: 提出综合多任务损失优化框架：对点回归模型使用场景密度特定高置信度logit抑制；对密度图方法使用峰值目标密度图抑制；结合模型无关的感知约束确保扰动有效且对人眼不可察觉

Result: 攻击效果显著，平均MAE增加7倍，同时保持竞争性视觉质量；成功迁移到7个SOTA人群模型，迁移比在0.55-1.69之间；在攻击效果和不可察觉性之间取得良好平衡

Conclusion: 首次实现了跨密度图和点回归范式的对抗攻击，提出的框架在攻击效果和不可察觉性方面优于现有可迁移攻击策略，为人群计数模型的安全性评估提供了新工具
...
Method: 提出DCARL分治自回归框架：1）使用专门的关键帧生成器（无时间压缩训练）建立长范围、全局一致的结构锚点；2）使用插值生成器以自回归方式合成密集帧，采用重叠片段，利用关键帧提供全局上下文，单个干净的前一帧确保局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，在视觉质量（更低的FID和FVD）和相机姿态一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法，能够稳定生成长达32秒的高保真长轨迹视频。

Conclusion: DCARL成功结合了分治方案的结构稳定性和视频扩散模型的高保真生成能力，有效解决了长轨迹视频生成中的视觉漂移和可控性问题，为世界建模提供了更可靠的解决方案。
...
TL;DR: WAFT-Stereo是一种基于扭曲的立体匹配方法，无需构建代价体，在多个基准测试中取得最佳性能，同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配方法通常依赖代价体，但这种方法计算成本高且效率低。作者旨在证明无需代价体也能实现高性能立体匹配，通过更高效的扭曲操作替代代价体。

Method: 提出WAFT-Stereo方法，采用基于扭曲的架构替代传统的代价体设计。通过扭曲操作直接处理图像特征，避免了构建和优化代价体的计算开销。

Result: 在ETH3D、KITTI和Middlebury公开基准测试中排名第一，在ETH3D基准上零样本误差降低81%，同时比竞争方法快1.8-6.7倍。

Conclusion: 代价体并非高性能立体匹配的必要组件，基于扭曲的方法能够实现更优的性能和效率，为立体匹配领域提供了新的设计思路。
...
TL;DR: 该研究对20种前沿多模态大语言模型在神经影像诊断任务中的性能进行了全面基准测试，评估了分类准确性、校准度、结构化输出有效性和计算效率，发现技术属性识别已接近解决，但诊断推理尤其是亚型预测仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在基于图像的决策支持中展现出新潜力，但其在神经影像领域的可靠性和操作权衡尚未得到充分理解。需要系统评估这些模型在神经影像诊断任务中的实际表现，为临床应用提供依据。

Method: 使用精心策划的MRI和CT数据集（涵盖多发性硬化、中风、脑肿瘤、其他异常和正常对照），要求模型同时生成诊断、亚型、成像模态、专业序列和解剖平面等多重输出。采用四维度评估框架：带弃权的判别分类、校准度、结构化输出有效性和计算效率。通过多阶段框架确保公平比较并控制选择偏差。

Result: 技术成像属性（如模态和平面）识别已接近解决，而诊断推理尤其是亚型预测仍具挑战性。肿瘤分类最可靠，中风中等可解，多发性硬化和罕见异常仍困难。少样本提示可提升多个模型性能但增加token使用、延迟和成本。Gemini-2.5-Pro和GPT-5-Chat整体诊断性能最强，Gemini-2.5-Flash提供最佳效率-性能权衡。在开源模型中，MedGemma-1.5-4B表现最突出，其少样本性能接近多个专有模型的零样本性能，同时保持完美的结构化输出。

Conclusion: 该研究为多模态大语言模型在神经影像中的性能、可靠性和效率权衡提供了实用见解，支持该领域标准化评估。技术属性识别已成熟，但诊断推理仍需改进，开源模型MedGemma-1.5-4B展现出临床应用潜力。
...
TL;DR: CORA：一种用于心血管风险评估的3D视觉基础模型，通过病理中心合成驱动的自监督学习，在冠状动脉CT血管成像分析中超越现有方法


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球心血管死亡的主要原因，虽然冠状动脉CT血管成像（CCTA）可用于非侵入性评估，但深度学习分析面临专家标注数据稀缺的挑战。现有的无标签预训练策略（如掩码图像建模）偏向全局解剖统计，难以捕捉冠状动脉斑块的空间局部病理特征。

Method: 提出CORA模型，采用病理中心、合成驱动的自监督学习框架。使用解剖引导的病变合成引擎，训练模型检测模拟的血管异常，使表征学习偏向临床相关的疾病特征而非占主导的背景解剖结构。在12,801个无标签CCTA体积上进行训练，并将成像编码器与大型语言模型结合形成多模态框架。

Result: 在来自9家独立医院的多中心数据集上全面评估，在斑块表征、狭窄检测和冠状动脉分割等诊断和解剖任务中，CORA始终优于最先进的3D视觉基础模型，性能提升高达29%。通过多模态框架显著改善了30天主要不良心脏事件（MACE）的风险分层。
...
### [60] [OptiSAR-Net++: A Large-Scale Benchmark and Transformer-Free Framework for Cross-Domain Remote Sensing Visual Grounding](https://arxiv.org/abs/2603.24876)
...
Method: 提出OptiSAR-Net++框架：1) PL-MoE用于高效跨域特征解耦；2) 采用CLIP对比范式替代Transformer解码，结合动态对抗负采样；3) 文本引导双门融合模块(TGDF-SSA)增强语义-视觉对齐；4) 区域感知辅助头改进空间建模。

Result: 在首个大规模跨域基准数据集OptSAR-RSVG和DIOR-RSVG上达到最先进性能，在定位精度和效率方面均有显著优势。

Conclusion: OptiSAR-Net++成功解决了跨域遥感视觉定位的挑战，通过高效特征解耦和对比学习范式实现了准确高效的跨模态定位，为实际应用提供了可行方案。
...
### [61] [SurgPhase: Time efficient pituitary tumor surgery phase recognition via an interactive web platform](https://arxiv.org/abs/2603.24897)
...
Method: 1) 开发协作式在线平台用于外科医生上传手术视频、接收自动阶段分析并贡献数据；2) 在251个未标注PTS视频上使用自监督框架预训练ResNet-50模型；3) 在81个标注手术上微调，采用包含焦点损失、渐进层解冻和动态采样的改进训练机制以应对类别不平衡和手术变异性。

Result: 在保留测试集上达到90%准确率，优于当前最先进方法，并在不同手术案例中表现出强大的泛化能力。

Conclusion: 该框架通过结合自监督学习、鲁棒时序建模和可扩展数据标注策略，实现了高精度的垂体瘤手术阶段识别，同时提出的协作平台促进了大规模数据收集和持续模型改进。
...
### [62] [Self-Supervised Learning for Knee Osteoarthritis: Diagnostic Limitations and Prognostic Value of Uncurated Hospital Data](https://arxiv.org/abs/2603.24903)
*Haresh Rengaraj Rajamohan,Yuxuan Chen,Kyunghyun Cho,Cem M. Deniz*

Main category: cs.CV

TL;DR: 评估自监督学习（SSL）在膝关节骨关节炎诊断和预后建模中相对于ImageNet预训练初始化的效果，发现图像-文本多模态SSL在预后预测中表现优异，但在诊断分级中因数据偏倚而效果有限


<details>
  <summary>Details</summary>
Motivation: 研究自监督学习是否能改善膝关节骨关节炎的诊断和预后建模，特别是比较图像自监督和多模态图像-文本自监督相对于传统ImageNet预训练的效果

Method: 使用两种SSL方法：(1) 图像自监督：在OAI、MOST和NYU队列的膝关节X光片上预训练；(2) 多模态图像-文本自监督：在未筛选的医院膝关节X光片与放射科医生报告配对的语料库上预训练。比较这些方法与ImageNet预训练在KL分级诊断和4年结构发生/进展预后预测上的表现

Result: 诊断KL分级：SSL结果混合。图像SSL在线性探测（冻结编码器）时提高准确性，但完全微调时未超越ImageNet预训练。多模态SSL未改善分级性能，归因于预训练语料库严重偏倚（93%估计KL 3级）。预后建模：多模态初始化显著改善，在预测4年结构发生和进展上优于ImageNet基线，包括外部验证（MOST AUROC：0.701 vs. 0.599，10%标记数据）

Conclusion: 未筛选的医院图像-文本数据因严重偏倚可能对学习诊断无效，但当下游任务与预训练数据分布一致时，为预后建模提供了强信号。多模态SSL在预后预测中表现优异，但在诊断分级中受数据偏倚限制
...
### [63] [ICTPolarReal: A Polarized Reflection and Material Dataset of Real World Objects](https://arxiv.org/abs/2603.24912)
*Jing Yang,Krithika Dharanikota,Emily Jia,Haiwei Chen,Yajie Zhao*

Main category: cs.CV

TL;DR: 该论文提出了一个大规模的真实世界偏振反射与材质数据集，通过配备交叉/平行偏振的8相机346光源光舞台采集，包含218个日常物体，提供超过120万张高分辨率图像，用于改进逆渲染和材质理解。


<details>
  <summary>Details</summary>
Motivation: 现有逆渲染方法主要依赖合成数据集，这些数据集光照简化且材质真实性有限，导致模型难以泛化到真实世界图像。真实测量的反射率数据稀缺是核心挑战。

Method: 使用配备交叉/平行偏振的8相机346光源光舞台采集218个日常物体的真实世界数据，涵盖多视角、多光照、偏振、反射分离和材质属性五个采集维度，获得超过120万张高分辨率图像，并提供漫反射-镜面反射分离以及解析推导的漫反射反照率、镜面反射反照率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆渲染和正向渲染模型，在固有分解、重光照和稀疏视角3D重建等任务上展示了材质分离、光照保真度和几何一致性方面的显著改进。

Conclusion: 该工作为基于物理的材质理解建立了新基础，使模型能够超越合成训练机制实现真实世界泛化，有望推动逆渲染和材质建模领域的发展。
...
### [64] [TIGFlow-GRPO: Trajectory Forecasting via Interaction-Aware Flow Matching and Reward-Driven Optimization](https://arxiv.org/abs/2603.24936)
*Xuepeng Jing,Wenhuan Lu,Hao Meng,Zhizhi Yu,Jianguo Wei*

Main category: cs.CV

TL;DR: 提出TIGFlow-GRPO两阶段框架，通过条件流匹配建模轨迹分布，并利用GRPO后训练将轨迹生成与行为规则对齐，提升轨迹预测的社会合规性和物理可行性。
...
Method: 两阶段框架：第一阶段使用带轨迹交互图(TIG)模块的CFM预测器，建模细粒度视觉空间交互；第二阶段进行Flow-GRPO后训练，将确定性流展开重构为随机ODE-to-SDE采样以支持轨迹探索，结合视图感知的社会合规性和地图感知的物理可行性构建复合奖励函数。
...
### [65] [Infinite Gaze Generation for Videos with Autoregressive Diffusion](https://arxiv.org/abs/2603.24938)
*Jenna Kang,Colin Groth,Tong Wu,Finley Torrens,Patsorn Sangkloy,Gordon Wetzstein,Qi Sun*

Main category: cs.CV

TL;DR: 提出用于无限时域原始注视预测的生成框架，通过自回归扩散模型合成具有连续空间坐标和高分辨率时间戳的注视轨迹，显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统显著性图和扫描路径无法捕捉原始注视的细粒度时间动态，且现有模型局限于短时窗口（约3-5秒），无法捕获真实世界内容中的长程行为依赖。

Method: 使用自回归扩散模型生成具有连续空间坐标和高分辨率时间戳的注视轨迹，模型以显著性感知的视觉潜在空间为条件。

Result: 定量和定性评估表明，该方法在长程时空准确性和轨迹真实性方面显著优于现有方法。

Conclusion: 提出的生成框架能够有效预测任意长度视频中的无限时域原始注视，捕捉长程行为依赖并生成更真实的注视轨迹。
...
### [66] [Beyond Attention Magnitude: Leveraging Inter-layer Rank Consistency for Efficient Vision-Language-Action Models](https://arxiv.org/abs/2603.24941)
*Peiju Liu,Jinming Liu,Xipeng Qiu,Xuanjing Huang*

Main category: cs.CV

TL;DR: TIES：基于τ引导的层间高效选择框架，通过动态平衡注意力大小与排名一致性，在减少视觉令牌使用的同时提升机器人操作性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型存在高推理延迟问题，主要源于处理密集视觉令牌。现有令牌缩减方法主要依赖注意力大小作为静态选择标准，但研究发现高注意力令牌具有任务依赖性，甚至可能降低策略性能。

Method: 提出TIES框架，通过τ引导的层间令牌排名一致性进行动态令牌选择。该方法自适应地平衡注意力大小与排名一致性，无需额外训练即可实现鲁棒的令牌选择。

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提升6%，同时减少78%的令牌使用量，并在不同解码器和基准测试中表现出强大的泛化能力。

Conclusion: TIES通过动态令牌选择框架有效解决了VLA模型的高延迟问题，证明了基于层间排名一致性的方法优于传统的静态注意力大小选择策略。
...
### [67] [Self-Corrected Image Generation with Explainable Latent Rewards](https://arxiv.org/abs/2603.24965)
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在复杂提示对齐方面存在挑战，特别是细粒度语义和空间关系。生成过程的feed-forward特性使得在未完全理解输出的情况下难以实现对齐，而评估生成图像相对更容易。这种不对称性促使研究者开发自校正框架

Method: xLARD框架包含轻量级校正器，基于模型生成参考的结构化反馈精炼潜在表示。核心组件是将潜在编辑映射到可解释奖励信号的可微分映射，使不可微分的图像级评估能够提供连续的潜在级指导。利用多模态大语言模型理解、评估和校正生成过程

Result: 在多样化的生成和编辑任务实验中，xLARD显著提高了语义对齐和视觉保真度，同时保持了生成先验。该方法有效解决了复杂提示对齐问题

Conclusion: xLARD通过可解释潜在奖励的自校正机制，成功利用评估-生成不对称性，实现了更精确的文本到图像对齐，为复杂语义控制提供了有效解决方案
...
### [68] [PASDiff: Physics-Aware Semantic Guidance for Joint Real-world Low-Light Face Enhancement and Restoration](https://arxiv.org/abs/2603.24969)
*Yilin Ni,Wenjie Li,Zhengxue Wang,Juncheng Li,Guangwei Gao,Jian Yang*

Main category: cs.CV

TL;DR: PASDiff：一种免训练的物理感知语义扩散模型，用于真实世界低光人脸图像恢复，通过逆强度加权和Retinex理论引入光度约束，结合风格无关结构注入技术，在WildDark-Face基准上显著优于现有方法。
...
Method: 提出PASDiff（物理感知语义扩散）模型，采用免训练方式：1）利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色度；2）提出风格无关结构注入（SASI）技术，从现成的人脸先验中提取结构同时过滤其固有光度偏差，将身份特征与物理约束无缝协调；3）构建WildDark-Face基准数据集（700张真实世界低光人脸图像）。

Result: 在WildDark-Face基准上的大量实验表明，PASDiff显著优于现有方法，在自然光照、颜色恢复和身份一致性之间实现了优越的平衡。

Conclusion: PASDiff通过物理感知语义扩散框架有效解决了真实世界低光人脸恢复的多重退化问题，实现了高质量的人脸图像恢复，在自然光照、颜色保真和身份保持方面表现出色。
...
### [69] [MoE-GRPO: Optimizing Mixture-of-Experts via Reinforcement Learning in Vision-Language Models](https://arxiv.org/abs/2603.24984)
*Dohwan Ko,Jinyoung Park,Seoung Choi,Sanghyeok Lee,Seohyun Lee,Hyunwoo J. Kim*

Main category: cs.CV

TL;DR: 提出MoE-GRPO框架，使用强化学习优化视觉语言模型中专家路由选择，解决传统top-K路由导致的专家过拟合问题，通过模态感知路由指导提升训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构中广泛采用的确定性top-K路由机制可能忽略更优的专家组合，导致专家过拟合，限制了专家选择的多样性。需要一种更灵活的路由优化方法。

Method: 提出MoE-GRPO框架：1) 将专家选择建模为序列决策问题；2) 使用Group Relative Policy Optimization (GRPO)优化路由策略；3) 引入模态感知路由指导，防止路由器探索特定模态下很少激活的专家。

Result: 在多模态图像和视频基准测试中，MoE-GRPO持续优于标准top-K路由及其变体，通过促进更丰富的专家选择，缓解专家过拟合，实现任务级专家专业化。

Conclusion: MoE-GRPO通过强化学习优化专家路由，有效解决了传统MoE路由的局限性，提高了视觉语言模型的性能和效率，为多模态MoE架构提供了更灵活的路由优化方案。
...
### [70] [Few-Shot Left Atrial Wall Segmentation in 3D LGE MRI via Meta-Learning](https://arxiv.org/abs/2603.24985)
*Yusri Al-Sanaani,Rebecca Thornhill,Pablo Nery,Elena Pena,Robert deKemp,Calum Redpath,David Birnie,Sreeraman Rajan*

Main category: cs.CV

TL;DR: 提出基于MAML的少样本3D左心房壁分割框架，通过元学习结合辅助任务和边界感知损失，在低样本情况下提升薄壁结构分割精度和域适应鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 左心房壁的LGE-MRI分割面临薄几何结构、低对比度和专家标注稀缺的挑战，需要开发能够在少量标注样本下实现准确分割且具有良好域适应能力的方法。

Method: 采用模型无关元学习框架，在元训练阶段结合左心房壁主任务和左/右心房腔辅助任务，使用边界感知复合损失函数强调薄壁结构精度，支持K-shot（K=5,10,20）少样本适应。

Result: 在保留测试集上，MAML在5-shot时Dice分数0.64优于监督微调的0.52，HD95为5.70mm优于7.60mm；20-shot时接近全监督基准（0.69 vs 0.71）。在未见域偏移和本地队列上保持鲁棒性，5-shot时分别达到0.59/5.99mm和0.57/6.01mm。

Conclusion: MAML框架能够在少量标注样本下实现更准确可靠的薄壁边界分割，性能随样本数增加而提升，具有良好的域适应鲁棒性，有望在临床应用中减少额外标注需求，促进心房重构评估的临床转化。
...
### [71] [Towards Video Anomaly Detection from Event Streams: A Baseline and Benchmark Datasets](https://arxiv.org/abs/2603.24991)
*Peng Wu,Yuting Yan,Guansong Pang,Yujia Sun,Qingsen Yan,Peng Wang,Yanning Zhang*

Main category: cs.CV

TL;DR: EWAD：首个事件流视频异常检测框架，通过动态采样、密度调制建模和知识蒸馏实现显著性能提升
...
Method: 提出EWAD框架：1)事件密度感知动态采样策略选择信息丰富的时间段；2)密度调制时序建模从稀疏事件流中捕获上下文关系；3)RGB到事件的知识蒸馏机制增强弱监督下的事件表示

Result: 在三个基准测试上的广泛实验表明，EWAD相比现有方法取得显著改进，突显事件驱动建模在视频异常检测中的潜力和有效性

Conclusion: 该研究首次将事件流视频异常检测确立为统一研究方向，构建了同步事件和RGB记录的数据集，提出的EWAD框架展示了事件驱动建模的优势
...
### [72] [C2W-Tune: Cavity-to -Wall Transfer Learning for Thin Atrial Wall Segmentation in 3D Late Gadolinium-enhanced Magnetic Resonance](https://arxiv.org/abs/2603.24992)
*Yusri Al-Sanaani,Rebecca Thornhill,Sreeraman Rajan*

Main category: cs.CV

TL;DR: C2W-Tune：一种两阶段腔室到壁的迁移学习框架，利用高精度左心房腔室分割作为解剖先验，通过渐进式层解冻策略改进3D LGE-MRI中薄壁左心房壁的分割精度。


<details>
  <summary>Details</summary>
Motivation: 3D LGE-MRI中左心房壁分割因壁薄、解剖结构复杂和对比度低而具有挑战性，但准确的壁分割对于壁厚度映射和纤维化量化至关重要。

Method: 提出C2W-Tune两阶段框架：第一阶段使用3D U-Net（ResNeXt编码器+实例归一化）预训练分割左心房腔室；第二阶段将权重迁移至壁分割任务，采用渐进式层解冻策略，在保留心内膜特征的同时实现壁特异性细化。
...
Conclusion: 基于解剖的任务迁移结合受控微调显著提高了3D LGE-MRI中薄左心房壁分割的边界精度，为临床壁厚度映射和纤维化量化提供了更可靠的解决方案。
...
### [73] [Relaxed Rigidity with Ray-based Grouping for Dynamic Gaussian Splatting](https://arxiv.org/abs/2603.24994)
...
Method: 提出视图空间射线分组策略：将α混合权重超过阈值的、被同一射线穿过的Gaussians聚类成组，然后对这些组施加约束以保持一致的空间分布，从而维持局部几何结构。该方法不依赖外部指导，直接确保局部几何随时间稳定。

Result: 将方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验，结果显示该方法显著优于现有方法，实现了更好的时间一致性和重建质量。

Conclusion: 通过显式保持Gaussians在4D场景中的局部几何结构，提出了一种更物理真实的运动建模方法，消除了对外部先验的依赖，在单目视频数据集上实现了优越的时间一致性和重建质量。
...
### [74] [Distributed Real-Time Vehicle Control for Emergency Vehicle Transit: A Scalable Cooperative Method](https://arxiv.org/abs/2603.25000)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（集中式数学求解器和强化学习）存在两个根本性限制：高计算成本和缺乏可扩展性。集中式方法仅适用于小规模场景，强化学习方法训练模型对不同交通条件扩展性有限。
...
Conclusion: 该方法克服了现有方法的高计算成本和缺乏可扩展性限制，消除了集中式方法的单点故障风险，提供了学习型方法无法提供的确定性安全保证，具有自然适应不同交通条件的能力。
...
### [75] [Interpretable Zero-shot Referring Expression Comprehension with Query-driven Scene Graphs](https://arxiv.org/abs/2603.25004)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）在零样本指代表达理解任务中难以捕捉细粒度视觉细节和复杂对象关系，而大语言模型虽然擅长高层语义推理，但无法直接提取视觉特征。需要一种方法结合两者的优势。

Method: 提出SGREC方法：1）使用视觉语言模型构建查询驱动的场景图，显式编码空间关系、描述性标题和对象交互；2）利用场景图作为结构化中介，连接低层图像区域和LLM所需的高层语义理解；3）LLM从场景图提供的结构化文本表示中推断目标对象，并提供决策的详细解释以确保可解释性。

Result: 在多个零样本REC基准测试中达到最先进性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动场景图作为结构化中介，有效结合了视觉语言模型和大语言模型的优势，实现了可解释的零样本指代表达理解，在多个基准测试中表现出色。
...
### [76] [Few TensoRF: Enhance the Few-shot on Tensorial Radiance Fields](https://arxiv.org/abs/2603.25008)
*Thanh-Hai Le,Hoang-Hau Tran,Trong-Nghia Vu*

Main category: cs.CV

TL;DR: Few TensoRF结合TensorRF的高效张量表示与FreeNeRF的频率驱动少样本正则化，通过频率掩码和遮挡掩码提升稀疏视角下的3D重建质量，在保持快速训练的同时显著提升PSNR指标。


<details>
  <summary>Details</summary>
Motivation: 解决稀疏输入视角下3D重建的稳定性与质量问题，同时保持TensorRF的高效渲染速度优势，为实时3D重建提供数据高效的解决方案。

Method: 结合TensorRF的基于张量的高效表示与FreeNeRF的频率驱动少样本正则化，引入频率掩码和遮挡掩码来提升稀疏视角下的重建稳定性与质量。

Result: 在Synthesis NeRF基准测试中，PSNR从21.45 dB提升到23.70 dB，微调版本达到24.52 dB，训练时间保持约10-15分钟；在THuman 2.0数据集上，仅用8张输入图像获得27.37-34.00 dB的竞争性人体重建性能。

Conclusion: Few TensoRF是一种高效且数据有效的实时3D重建解决方案，在保持TensorRF快速训练优势的同时，显著提升了稀疏视角下的重建质量，适用于多样化场景。
...
### [77] [GDPO-Listener: Expressive Interactive Head Generation via Auto-Regressive Flow Matching and Group reward-Decoupled Policy Optimization](https://arxiv.org/abs/2603.25020)
*Zhangyu Jin,Maksim Siniukov,Deuksin Kwon,Ashutosh Chaubey,Mohammad Soleymani*

Main category: cs.CV

TL;DR: GDPO-Listener：一种用于生成高度表达性说话和倾听动作的新框架，通过自回归流匹配架构和组奖励解耦策略优化解决倾听者动作的回归到均值问题


<details>
  <summary>Details</summary>
Motivation: 为虚拟人合成生成真实的3D头部运动是一个重要挑战。现有方法在说话者头部运动方面取得显著成果，但在倾听者运动方面存在"回归到均值"问题，导致动作塌缩为静态面部，且缺乏复杂非语言动作的参数空间

Method: 1. 引入自回归流匹配架构实现稳定的监督学习；2. 应用组奖励解耦策略优化(GDPO)，通过对不同FLAME参数组进行独立的奖励归一化，明确激励高方差表达性生成；3. 实现显式语义文本控制以支持可定制的响应
...
### [78] [VideoTIR: Accurate Understanding for Long Videos with Efficient Tool-Integrated Reasoning](https://arxiv.org/abs/2603.25021)
*Zhe Gao,Shiyu Shen,Taifeng Chai,Weinong Wang,Haotian Xu,Xing W,Wenbin Li,Qi Fan,Yang Gao,Dacheng Tao*

Main category: cs.CV

TL;DR: VideoTIR：基于强化学习的多级工具包调用方法，通过零RL和SFT冷启动、工具包动作分组策略优化和沙盒轨迹合成框架，解决长视频理解中的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解中存在幻觉问题，主要源于文本和视觉令牌的不平衡。虽然SFT-based工具调用方法可以缓解此问题，但需要大量细粒度高质量数据且工具调用轨迹受限

Method: 提出VideoTIR方法：1）利用强化学习鼓励正确使用多级工具包；2）探索零RL和SFT冷启动使MLLMs能够检索和聚焦有意义的视频片段/图像/区域；3）提出工具包动作分组策略优化减少冗余工具调用；4）开发沙盒轨迹合成框架生成高质量轨迹数据

Result: 在三个长视频问答基准测试上的广泛实验证明了方法的有效性和效率

Conclusion: VideoTIR通过强化学习驱动的多级工具包调用策略，显著提升了长视频理解的准确性和效率，解决了现有方法的幻觉问题
...
### [79] [CARE: Training-Free Controllable Restoration for Medical Images via Dual-Latent Steering](https://arxiv.org/abs/2603.25026)
*Xu Liu*

Main category: cs.CV

TL;DR: CARE是一个无需训练的可控医学图像修复框架，通过双潜在分支策略和风险感知自适应控制器，在保持数据保真度和解剖一致性的同时，利用生成先验恢复缺失信息，实现保守或增强导向的修复模式。
...
Method: 提出CARE框架，采用双潜在修复策略：一个分支强制执行数据保真度和解剖一致性，另一个分支利用生成先验恢复缺失或退化信息。通过风险感知自适应控制器，根据修复不确定性和局部结构可靠性动态调整各分支贡献，实现无需额外模型训练的可控修复。

Result: 在噪声和不完整医学成像场景中评估CARE，显示其实现了强大的修复质量，同时更好地保留了临床相关结构，减少了不合理重建的风险。

Conclusion: CARE为更安全、更可控、更易于部署的医学图像修复提供了实用步骤，通过训练自由的可控框架在结构保留和先验引导细化之间取得平衡。
...
### [80] [GeoNDC: A Queryable Neural Data Cube for Planetary-Scale Earth Observation](https://arxiv.org/abs/2603.25037)
...
<details>
  <summary>Details</summary>
Motivation: 卫星地球观测积累了海量时空档案，但这些数据仍以离散栅格文件形式组织，导致存储、传输和查询成本高昂。需要一种更高效的表示方法来支持按需查询和连续时间重建。

Method: 提出GeoNDC（可查询神经数据立方体），将地球观测数据编码为连续的时空隐式神经场。该方法支持直接时空查询，无需完全解压缩数据，实现连续时间参数化重建。

Result: 在20年全球MODIS MCD43A4反射率记录上，GeoNDC将存档压缩至0.44GB（相对于优化的Int16基线约95:1压缩比），光谱保真度高（平均R²>0.98，平均RMSE=0.021）。在Sentinel-2影像上，连续时间参数化能在模拟2公里云遮挡下高保真恢复无云动态（R²>0.85）。在HiGLASS生物物理产品上达到近乎完美的精度（R²>0.98）。

Conclusion: GeoNDC为行星尺度地球观测提供了统一的AI原生表示，通过紧凑、分析就绪的数据层补充原始档案，将查询、重建和压缩集成在单一框架中，支持在消费级硬件上进行直接时空查询。
...
### [81] [MoRGS: Efficient Per-Gaussian Motion Reasoning for Streamable Dynamic 3D Scenes](https://arxiv.org/abs/2603.25042)
*Wonjoon Lee,Sungmin Woo,Donghyeong Kim,Jungho Lee,Sangheon Park,Sangyoun Lee*

Main category: cs.CV

TL;DR: MoRGS：基于3D高斯溅射的在线动态场景重建框架，通过光流监督和运动置信度机制提升每高斯运动建模质量


<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效，但仅依赖光度损失优化导致每高斯运动追逐像素残差而非真实3D运动，无法准确反映场景动态

Method: 1) 使用稀疏关键视图的光流作为轻量运动线索，正则化每高斯运动；2) 学习每高斯运动偏移场，补偿3D投影运动与观测流之间的差异；3) 引入每高斯运动置信度，分离动态/静态高斯并加权属性更新

Result: 在在线方法中达到最先进的重建质量和运动保真度，同时保持流式处理性能，显著提升动态场景的时空一致性

Conclusion: MoRGS通过显式建模每高斯运动并引入光流监督，有效解决了在线4D重建中运动建模不准确的问题，实现了高质量、高保真度的动态场景在线重建
...
### [82] [GaussFusion: Improving 3D Reconstruction in the Wild with A Geometry-Informed Video Generator](https://arxiv.org/abs/2603.25053)
*Liyuan Zhu,Manjunath Narayana,Michal Stary,Will Hutchcroft,Gordon Wetzstein,Iro Armeni*

Main category: cs.CV

TL;DR: GaussFusion通过几何感知的视频生成改进3D高斯泼溅重建，减少浮动物体、闪烁和模糊等常见伪影，支持优化式和前馈式方法，在实时21FPS下实现最先进的性能。
...
Method: 提出几何感知的视频到视频生成器，渲染包含深度、法线、不透明度和协方差的Gaussian primitive视频缓冲区，通过生成器细化产生时间一致的伪影消除帧；引入模拟多样化退化模式的伪影合成管道确保鲁棒性和泛化性。

Result: 在新视角合成基准测试中达到最先进性能；高效变体在保持相似性能的同时以21FPS实时运行，支持交互式3D应用。
...
### [83] [Synergistic Event-SVE Imaging for Quantitative Propellant Combustion Diagnostics](https://arxiv.org/abs/2603.25054)
...
Method: 开发闭环事件-SVE测量系统：SVE分支采用烟雾感知融合策略生成HDR图；事件相机分支利用HDR图作为绝对强度参考抑制烟雾伪影；基于清洁事件观测的立体事件三维管道通过特征提取和三角测量估计分离高度和等效粒子尺寸

Result: 系统最大校准误差0.56%；在硼基推进剂实验中获取多模态等效半径统计；能够捕获传统传感器难以观测的快速分离瞬态

Conclusion: 该框架为烟雾遮蔽高动态范围条件下的微秒级三维燃烧测量提供了实用且校准一致的方法
...
### [84] [Learning domain-invariant features through channel-level sparsification for Out-Of Distribution Generalization](https://arxiv.org/abs/2603.25083)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型倾向于捕获领域特定上下文，形成对非因果特征的捷径依赖，导致在不同数据源上性能不一致。现有方法（如不变性学习）难以分离深度潜在空间中高度混合的特征，无法完全解决捷径学习问题。

Method: 提出分层因果丢弃(HCD)：1)使用通道级因果掩码强制特征稀疏性，分离因果特征与伪特征；2)基于矩阵互信息(MMI)目标指导训练，最小化潜在特征与领域标签的互信息，同时最大化与类别标签的共享信息；3)引入StyleMix驱动的VICReg模块确保稳定性，防止掩码意外过滤关键因果数据。
...
### [85] [Learning Explicit Continuous Motion Representation for Dynamic Gaussian Splatting from Monocular Videos](https://arxiv.org/abs/2603.25058)
*Xuankai Zhang,Junjin Xiao,Shangwei Huang,Wei-shi Zheng,Qing Zhang*

Main category: cs.CV

TL;DR: 提出一种从单目视频进行高质量动态高斯泼溅的方法，通过SE(3)B样条运动基显式建模高斯的位置和方向连续变形，采用自适应控制机制调整运动基和控制点数量，结合软分割重建策略和多视图扩散模型提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有动态高斯泼溅方法在建模复杂运动方面存在局限，需要更精确的连续变形表示，同时要解决计算效率、长间隔运动干扰和训练视图过拟合等问题。

Method: 1. 使用SE(3)B样条运动基显式建模动态高斯的连续位置和方向变形；2. 设计自适应控制机制动态调整运动基和控制点数量；3. 开发软分割重建策略减轻长间隔运动干扰；4. 采用多视图扩散模型提供多视图线索避免过拟合。

Result: 大量实验表明，该方法在新视角合成任务上优于现有最先进方法，能够生成更高质量的动态场景重建结果。

Conclusion: 该方法通过显式连续变形建模、自适应控制、软分割重建和多视图扩散模型的综合方案，有效提升了单目视频动态高斯泼溅的质量和鲁棒性。
...
### [86] [Pixelis: Reasoning in Pixels, from Seeing to Acting](https://arxiv.org/abs/2603.25091)
*Yunpeng Zhou*

Main category: cs.CV

TL;DR: Pixelis：一种直接在像素空间操作的视觉语言智能体，通过可执行操作（缩放/裁剪、分割、跟踪、OCR、时序定位）学习，采用三阶段训练方法，在多个基准测试中实现性能提升


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统多为静态观察者，仅描述像素而不执行动作，无法在分布偏移下安全改进。这种被动性限制了通用化、物理基础的视觉智能。需要通过行动而非静态描述来学习，超越精心策划的数据。

Method: 三阶段训练方法：1) 监督微调：从思维-行动链轨迹学习像素-工具语法，使用掩码模仿损失加权操作/参数标记，辅助头部稳定像素基础参数；2) 好奇心-一致性奖励微调：优化双驱动目标，结合预测误差好奇心与相邻步骤一致性，在KL锚点下加入适度效率先验；3) 像素测试时强化学习：通过检索邻居、对完整轨迹而非答案进行投票，更新向短而高保真示例，同时使用KL-to-EMA安全控制约束漂移。

Result: 在六个公共图像和视频基准测试中，Pixelis实现一致改进：相对于相同8B基线的平均相对增益为+4.08%（在VSI-Bench上峰值达+6.03%），同时产生更短、可审计的工具链，并在测试时学习中保持走廊内KL。
...
### [87] [GIFT: Global Irreplaceability Frame Targeting for Efficient Video Understanding](https://arxiv.org/abs/2603.25072)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型处理密集帧计算成本高，现有关键帧选择方法采用贪婪决策且将相关性和多样性评估解耦，容易陷入局部最优并选择不相关的噪声帧
...
Conclusion: GIFT通过评估帧的全局不可替代性，有效解决了现有帧选择方法的局限性，显著提升了视频大语言模型的效率和性能
...
### [88] [Z-Erase: Enabling Concept Erasure in Single-Stream Diffusion Transformers](https://arxiv.org/abs/2603.25074)
...
<details>
  <summary>Details</summary>
Motivation: 概念擦除是文本到图像模型的重要安全机制，但在新兴的单流扩散Transformer架构（如Z-Image）中研究不足。现有方法直接应用于单流模型会导致生成崩溃，需要专门解决方案

Method: 提出Z-Erase方法：1）流解耦概念擦除框架，将单流模型解耦更新，使现有方法可应用；2）拉格朗日引导自适应擦除调制，通过约束算法平衡擦除与保留的敏感权衡

Result: 成功克服生成崩溃问题，在广泛任务上实现最先进性能。提供严格的收敛性分析，证明Z-Erase能收敛到帕累托稳定点

Conclusion: Z-Erase是首个针对单流T2I模型的概念擦除方法，通过流解耦框架和拉格朗日引导调制解决了生成崩溃问题，为单流扩散Transformer的安全应用提供了有效工具
...
### [89] [MoireMix: A Formula-Based Data Augmentation for Improving Image Classification Robustness](https://arxiv.org/abs/2603.25109)
...
Method: 提出基于解析干涉图案的程序化增强方法，利用莫尔干涉生成覆盖广泛空间频率的结构化扰动，通过闭式数学公式在内存中实时生成纹理，与训练图像混合后立即丢弃，实现无存储的数据增强流程。

Result: 方法计算成本极低（每张图像0.0026秒），在ImageNet-C、ImageNet-R和对抗基准测试中，相比标准增强基线和现有无外部数据增强方法，能持续提升视觉Transformer的鲁棒性。

Conclusion: 解析干涉图案为数据驱动的生成式增强方法提供了实用且高效的替代方案，证明了程序化增强在提升模型鲁棒性方面的有效性。
...
### [90] [Bridging Perception and Reasoning: Token Reweighting for RLVR in Multimodal LLMs](https://arxiv.org/abs/2603.25077)
...
<details>
  <summary>Details</summary>
Motivation: 将RLVR扩展到MLLMs面临根本挑战：MLLMs的响应交织着感知相关token（视觉内容基础）和推理相关token（推理链构建），这两种token类型体现了不同但相互依赖的能力（视觉基础和符号推理），使得孤立优化不足。

Method: 提出即插即用的Token-Reweighting (ToR)策略，通过识别关键感知和推理token并动态重加权，在RLVR训练中显式建模感知与推理token的相互依赖性。该方法可应用于现有方法（如GRPO和DAPO）之上。

Result: ToR在多个多模态推理基准测试中带来一致的性能提升，实现了最先进的性能，同时具备准确的视觉基础和连贯的推理能力。

Conclusion: ToR策略有效解决了MLLMs中感知与推理token耦合的优化问题，通过动态重加权关键token，在RLVR训练中实现了感知和推理能力的协同优化。
...
### [91] [SAVe: Self-Supervised Audio-visual Deepfake Detection Exploiting Visual Artifacts and Audio-visual Misalignment](https://arxiv.org/abs/2603.25140)
*Sahibzada Adil Shahzad,Ammarah Hashmi,Junichi Yamagishi,Yusuke Yasuda,Yu Tsao,Chia-Wen Lin,Yan-Tsung Peng,Hsin-Min Wang*

Main category: cs.CV

TL;DR: SAVe是一个自监督的音频-视觉深度伪造检测框架，完全在真实视频上训练，通过生成身份保持的区域感知自混合伪篡改来模拟篡改伪影，并建模唇语同步以检测跨模态不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态深度伪造检测方法主要依赖精心策划的合成伪造数据进行训练，这会导致数据集和生成器偏差，限制了模型的可扩展性和对未见篡改的鲁棒性。多模态深度伪造通常存在微妙的视觉伪影和跨模态不一致性，这些难以检测。

Method: SAVe采用自监督学习，完全在真实视频上训练。方法包括：1）生成身份保持、区域感知的自混合伪篡改来模拟篡改伪影，使模型能够学习跨多个面部粒度级别的互补视觉线索；2）通过音频-视觉对齐组件建模唇语同步，检测音频-视觉伪造特有的时间错位模式。

Result: 在FakeAVCeleb和AV-LipSync-TIMIT数据集上的实验表明，SAVe在域内性能具有竞争力，并展现出强大的跨数据集泛化能力。

Conclusion: 自监督学习是多模态深度伪造检测的可扩展范式，SAVe框架通过完全在真实视频上训练，避免了合成数据依赖带来的偏差，提高了对未见篡改的鲁棒性和泛化能力。
...
### [92] [FD$^2$: A Dedicated Framework for Fine-Grained Dataset Distillation](https://arxiv.org/abs/2603.25144)
...
Method: FD²框架包含两个阶段：预训练阶段通过反事实注意力学习聚合判别表示更新类别原型；蒸馏阶段采用细粒度特征约束对齐样本与类别原型并排斥其他类别，同时使用相似性约束使同类样本的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够无缝集成到解耦数据集蒸馏框架中，在大多数设置下提升性能，显示出良好的可迁移性。

Conclusion: FD²通过定位判别区域和构建细粒度表示，有效解决了细粒度数据集蒸馏中的挑战，为细粒度视觉识别任务提供了更有效的数据集压缩解决方案。
...
### [93] [Visual Attention Drifts,but Anchors Hold:Mitigating Hallucination in Multimodal Large Language Models via Cross-Layer Visual Anchors](https://arxiv.org/abs/2603.25088)
*Chengxu Yang,Jingling Yuan,Chuang Hu,Jiawei Jiang*

Main category: cs.CV

TL;DR: CLVA通过跨层视觉锚点抑制幻觉：研究发现幻觉源于深层注意力回归到早期层视觉噪声，提出训练免费方法强化中层特征，有效将深层注意力拉回正确视觉区域。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型常出现物体幻觉问题。现有研究使用注意力增强和视觉回溯方法，但这些工作缺乏对最终模型阶段注意力漂移的充分可解释性。本文旨在探究视觉特征在层间的演化过程，理解幻觉产生的根本原因。

Method: 提出CLVA（跨层视觉锚点）方法：1）分析视觉特征在模型各层的演化过程，发现幻觉源于深层注意力回归到早期层的视觉噪声；2）观察到输出可靠性依赖于中间层而非最终层获取的视觉锚点；3）设计训练免费方法，通过强化关键中层特征同时抑制回归噪声，利用注意力动态捕获的必需锚点将深层注意力拉回正确视觉区域。

Result: 方法在多种架构和基准测试中表现出色，无需显著增加计算时间和GPU内存。CLVA有效解决了多模态大语言模型中的物体幻觉问题，通过跨层视觉锚点机制提高了模型的可靠性和准确性。

Conclusion: 物体幻觉的根本原因是深层注意力回归到早期层视觉噪声。通过跨层视觉锚点方法强化中层特征、抑制回归噪声，可以有效将深层注意力引导至正确视觉区域，显著减少幻觉现象，且该方法训练免费、计算高效。
...
### [94] [THEMIS: Towards Holistic Evaluation of MLLMs for Scientific Paper Fraud Forensics](https://arxiv.org/abs/2603.25089)
...
TL;DR: THEMIS是一个用于评估多模态大语言模型在真实学术场景中视觉欺诈推理能力的基准测试，包含4000多个问题，涵盖7个场景和5种欺诈类型，对16个领先MLLM的测试显示最佳模型仅达到56.15%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试与真实世界学术欺诈的复杂性存在差距，需要开发一个能够全面评估多模态大语言模型在视觉欺诈推理方面能力的基准，特别是在学术场景中检测图像操纵和欺诈行为的能力。

Method: THEMIS基准包含三个主要创新：1) 基于真实撤稿案例和多模态合成数据构建的7个场景、4000多个问题，其中60.47%为复杂纹理图像；2) 系统覆盖5种欺诈类型和16种细粒度操纵操作，每个样本通常经历多个堆叠操纵；3) 建立从欺诈类型到5个核心视觉欺诈推理能力的映射，实现多维度能力评估。

Result: 在16个领先的MLLM上进行实验，即使表现最佳的GPT-5模型总体准确率也仅为56.15%，表明该基准对模型提出了严格挑战。基准能够揭示不同模型在核心能力上的具体优势和弱点。

Conclusion: THEMIS基准填补了现有评估与真实世界学术欺诈复杂性之间的关键空白，为MLLM在复杂真实世界欺诈推理任务中的发展提供了重要推动，展示了当前模型在该领域仍有显著提升空间。
...
### [95] [Label What Matters: Modality-Balanced and Difficulty-Aware Multimodal Active Learning](https://arxiv.org/abs/2603.25107)
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习依赖大规模标注数据，主动学习可减少标注成本。但现有方法假设模态重要性稳定不变，忽略了多模态学习中模态相对价值和样本难度随训练进程动态变化的特性，导致选择策略不敏感。
...
Conclusion: RL-MBA通过强化学习框架有效解决了多模态主动学习中模态重要性动态变化的问题，实现了自适应、难度感知的样本选择，在减少标注成本的同时提升了模型性能和模态平衡。
...
### [96] [Knowledge-Guided Adversarial Training for Infrared Object Detection via Thermal Radiation Modeling](https://arxiv.org/abs/2603.25170)
*Shiji Zhao,Shukun Xiong,Maoxun Yuan,Yao Huang,Ranjie Duan,Qing Guo,Jiansheng Chen,Haibin Duan,Xingxing Wei*

Main category: cs.CV

TL;DR: KGAT通过将红外物理知识嵌入对抗训练，利用不同类别间的热辐射关系作为可靠知识源，提升红外目标检测在对抗攻击和常见损坏下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 红外目标检测在复杂环境中应用广泛但易受对抗样本和常见损坏影响，现有数据驱动方法未充分考虑红外图像特性，导致鲁棒性有限。
...
Conclusion: 利用红外物理知识作为可靠知识源，通过知识引导对抗训练可显著提升红外目标检测的鲁棒性，为红外视觉系统安全提供新思路。
...
### [97] [MSRL: Scaling Generative Multimodal Reward Modeling via Multi-Stage Reinforcement Learning](https://arxiv.org/abs/2603.25108)
*Chenglong Wang,Yifu Huo,Yang Gan,Qiaozhi He,Qi Meng,Bei Li,Yan Wang,Junfu Liu,Tianhua Zhou,Jingbo Zhu,Tong Xiao*

Main category: cs.CV

TL;DR: 提出多阶段强化学习（MSRL）方法，通过文本偏好数据学习通用奖励推理能力，逐步迁移到多模态任务，无需额外多模态标注即可扩展多模态奖励模型的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习（RLVR）方法训练多模态奖励模型（MRMs）需要大量标注的多模态偏好数据，成本高且难以扩展。需要一种能在有限多模态数据下实现可扩展强化学习的方法。
...
Conclusion: MSRL方法有效解决了多模态奖励模型强化学习训练的可扩展性问题，通过从文本数据迁移奖励推理能力，显著提升多模态任务性能，为多模态奖励建模提供了更高效的训练范式。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视频扩散模型通常在短视频片段上训练，直接用于生成长视频会导致视觉质量显著下降。研究发现这主要源于两个分布外问题：帧级相对位置分布外和上下文长度分布外。

Method: 提出FreeLOC框架，包含两个核心技术：1) 视频相对位置重编码(VRPR)解决帧级相对位置分布外问题，采用多粒度策略分层重编码时间相对位置以对齐预训练分布；2) 分层稀疏注意力(TSA)解决上下文长度分布外问题，通过结构化不同时间尺度的注意力密度来保持局部细节和长程依赖。引入层自适应探测机制识别各transformer层对分布外问题的敏感性，实现选择性高效应用。

Result: 大量实验表明，该方法显著优于现有无需训练的方法，在时间一致性和视觉质量方面均达到最先进水平。

Conclusion: FreeLOC有效解决了预训练视频扩散模型生成长视频时的分布外问题，通过创新的位置重编码和注意力机制设计，无需额外训练即可显著提升长视频生成质量。
...
### [99] [AnyDoc: Enhancing Document Generation via Large-Scale HTML/CSS Data Synthesis and Height-Aware Reinforcement Optimization](https://arxiv.org/abs/2603.25118)
...
<details>
  <summary>Details</summary>
Motivation: 现有文档生成方法存在局限性：1）现有数据集覆盖范围有限且规模较小；2）缺乏统一的文档表示格式；3）需要处理多种文档生成任务；4）训练过程中存在内容溢出问题。因此需要开发一个能够处理多种文档类别和任务的统一框架。

Method: 1）建立可扩展的数据合成管道，自动生成HTML/CSS格式文档，创建DocHTML数据集（265,206个样本，111个类别，32种样式）；2）基于该数据集微调多模态大语言模型，实现三种文档生成任务：意图到文档、文档反渲染、元素到文档；3）引入高度感知强化学习后训练过程，通过基于预测与目标文档高度差异的奖励函数惩罚内容溢出。

Result: 实验结果表明：1）AnyDoc在三个文档生成任务上均优于通用多模态大语言模型和任务特定基线方法；2）高度感知强化学习有效缓解了内容溢出问题；3）统一的HTML/CSS表示格式使模型能够处理广泛的文档类别和样式。

Conclusion: AnyDoc通过统一的HTML/CSS表示、大规模合成数据集和高度感知强化学习，成功实现了跨多种文档类别和任务的文档生成框架，在质量和多样性方面均表现出色，为AI驱动的文档生成提供了有效的解决方案。
...
### [100] [AirSplat: Alignment and Rating for Robust Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2603.25129)
*Minh-Quan Viet Bui,Jaeho Moon,Munchurl Kim*

Main category: cs.CV

TL;DR: AirSplat是一个训练框架，将3D视觉基础模型的几何先验适配到无姿态的新视角合成任务中，通过自洽姿态对齐和基于评分的透明度匹配实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 虽然3D视觉基础模型在视觉几何估计方面表现出强大的零样本能力，但直接应用于通用的无姿态新视角合成仍然具有挑战性。需要一种方法能够有效利用3DVFMs的几何先验，同时解决姿态-几何不一致问题。

Method: 提出AirSplat训练框架，包含两个关键技术：1) 自洽姿态对齐(SCPA)：训练时反馈循环确保像素对齐监督，解决姿态-几何差异；2) 基于评分的透明度匹配(ROM)：利用稀疏视图NVS教师模型的局部3D几何一致性知识过滤退化基元。

Result: 在大规模基准测试中，该方法在重建质量方面显著优于最先进的无姿态NVS方法，展示了同时进行视觉几何估计和高质量视图合成的潜力。

Conclusion: AirSplat展示了将3D视觉基础模型适配到无姿态新视角合成任务中的潜力，通过解决姿态-几何不一致和利用局部几何一致性，实现了同时进行视觉几何估计和高质量视图合成的目标。
...
### [101] [Denoise and Align: Towards Source-Free UDA for Robust Panoramic Semantic Segmentation](https://arxiv.org/abs/2603.25131)
...
<details>
  <summary>Details</summary>
Motivation: 全景语义分割在自动驾驶和虚拟现实等应用中至关重要，但面临两大挑战：全景投影的几何畸变和密集标注的高成本。无监督领域自适应（UDA）提供了解决方案，但许多实际任务需要更严格的无源数据约束（SFUDA），这加剧了领域偏移问题，导致伪标签不可靠和性能下降，特别是对于少数类别。
...
Conclusion: DAPASS框架成功解决了无源数据全景语义分割中的关键挑战，通过创新的去噪和对抗对齐机制，在无需访问源数据的情况下实现了稳健的知识迁移和显著的性能提升。
...
### [102] [Robust Principal Component Completion](https://arxiv.org/abs/2603.25132)
...
<details>
  <summary>Details</summary>
Motivation: 传统鲁棒主成分分析(RPCA)假设稀疏前景与低秩背景是相加关系，但在许多实际应用中，稀疏前景会遮挡或替换低秩背景元素，这种不匹配需要新的框架来处理

Method: 提出鲁棒主成分完成(RPCC)框架，间接识别稀疏分量的支持集；采用完全概率贝叶斯稀疏张量分解，通过变分贝叶斯推理求解；证明收敛到硬分类器，无需后处理阈值化

Result: 在合成数据上获得接近最优的估计；在真实彩色视频数据集上实现鲁棒的前景提取；在高光谱数据集上实现稳健的异常检测

Conclusion: RPCC框架有效解决了传统RPCA在遮挡场景下的局限性，通过变分贝叶斯推理实现了无需阈值化的稀疏分量识别，在多种实际应用中表现出色
...
### [103] [EgoXtreme: A Dataset for Robust Object Pose Estimation in Egocentric Views under Extreme Conditions](https://arxiv.org/abs/2603.25135)
*Taegyoon Yoon,Yegyu Han,Seojin Ji,Jaewoo Park,Sojeong Kim,Taein Kwon,Hyung-Sin Kim*

Main category: cs.CV

TL;DR: 提出了EgoXtreme数据集，这是首个大规模的第一人称视角6D物体姿态估计数据集，专门针对工业维护、体育和应急救援等极端场景，包含严重运动模糊、动态光照和视觉遮挡等挑战。
...
Method: 创建EgoXtreme数据集，包含三个具有挑战性的场景：工业维护、体育和应急救援，通过极端光照、剧烈运动模糊和烟雾引入严重的感知模糊性。评估现有通用姿态估计器在该数据集上的表现，并探索图像恢复和基于跟踪的方法。

Result: 现有最先进的通用姿态估计器在EgoXtreme极端条件下泛化能力失效，特别是在低光照条件下。简单的图像恢复（如去模糊）对极端条件没有积极改善，而基于跟踪的方法显示出性能提升，表明在快速运动场景中使用时间信息是有意义的。
...
### [104] [Learning to Rank Caption Chains for Video-Text Alignment](https://arxiv.org/abs/2603.25145)
*Ansel Blume,Burak Uzkent,Shalini Chaudhuri,Garin Kessler*

Main category: cs.CV

TL;DR: 论文提出排名优化作为DPO的替代方案，用于提升视觉语言模型在视频文本对齐任务中的表现，通过生成有序的退化字幕链进行训练，并发现需要微调视觉编码器才能有效。


<details>
  <summary>Details</summary>
Motivation: 标准DPO的二元"赢家通吃"方法不适用于视觉语言模型，因为即使是不太理想的回答也可能保持对视觉内容的高度忠实性。Bradley-Terry DPO公式缺乏这种细微差别，过度强调获胜回答而忽略了"失败"回答可能仍保持高视觉保真度的问题。

Method: 提出排名优化作为替代方案，专注于视频文本对齐任务，通过重复字幕退化生成具有挑战性的完全有序字幕链。该方法需要微调视觉编码器，而不仅仅是语言重加权过程。

Result: 排名优化在长内容生成和评估任务中优于二元DPO。关键发现是这些方法需要微调视觉编码器才能有效，这挑战了DPO仅作为纯语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排名优化比二元DPO更适合处理视觉保真度的细微差别。视觉编码器的微调对于提升模型在视觉内容对齐任务中的表现至关重要，表明DPO类方法在视觉语言领域需要更全面的模型调整。
...
### [105] [Challenges in Hyperspectral Imaging for Autonomous Driving: The HSI-Drive Case](https://arxiv.org/abs/2603.25510)
*Koldo Basterretxea,Jon Gutiérrez-Zaballa,Javier Echanobe*

Main category: cs.CV

TL;DR: 该论文分析了自动驾驶中高光谱成像技术面临的挑战，包括非受控光照条件、大景深范围、动态场景以及实时性要求，并探讨了基于HSI-Drive数据集的相关视觉算法研究。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域对高光谱成像技术的应用前景广阔，但面临特定应用场景的独特挑战。这些挑战包括非受控且多变的照明条件、大景深范围、快速移动物体的动态场景，以及嵌入式平台有限计算资源下的实时性要求。这些因素共同决定了选择合适HSI技术的标准和开发定制视觉算法的需求。

Method: 论文通过分析HSI-Drive数据集最新版本的实验数据，研究了多种基于高光谱成像的视觉系统技术。这些技术旨在利用传感器获取的光谱和空间信息，开发适用于自动驾驶场景的定制算法。

Result: 研究分析了多种高光谱成像技术在自动驾驶视觉系统中的应用探索，基于HSI-Drive数据集的最新版本提供了具体的实验结果和案例。

Conclusion: 自动驾驶中高光谱成像技术的应用需要综合考虑特定领域的挑战和需求，包括环境条件、实时性要求和计算资源限制。选择合适的HSI技术和开发定制算法对于实现有效的自动驾驶视觉系统至关重要。
...
### [106] [A Semantically Disentangled Unified Model for Multi-category 3D Anomaly Detection](https://arxiv.org/abs/2603.25159)
*SuYeon Kim,Wongyu Lee,MyeongAh Cho*

Main category: cs.CV

TL;DR: 提出语义解纠缠的统一3D异常检测模型，通过条件化重构解决类别间特征纠缠问题，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 统一模型在多类别3D异常检测中面临类别间特征纠缠问题，导致模型采用错误的语义先验进行重构，产生不可靠的异常分数

Method: 语义解纠缠的统一3D异常检测框架，包含三个关键组件：1）粗到细全局标记化形成实例级语义身份；2）类别条件对比学习解纠缠类别语义；3）几何引导解码器实现语义一致重构

Result: 在Real3D-AD和Anomaly-ShapeNet数据集上达到最先进性能，统一模型和类别特定模型的物体级AUROC分别提升2.8%和9.1%，增强了统一3D异常检测的可靠性

Conclusion: 提出的语义解纠缠方法有效解决了统一3D异常检测中的类别间特征纠缠问题，通过条件化重构实现了更可靠的异常检测性能
...
### [107] [CHIRP dataset: towards long-term, individual-level, behavioral monitoring of bird populations in the wild](https://arxiv.org/abs/2603.25524)
...
TL;DR: CHIRP数据集结合CORVID方法，为野生鸟类个体重识别和行为监测提供计算机视觉解决方案，填补了野生动物监测数据集空白


<details>
  <summary>Details</summary>
Motivation: 野生动物长期行为监测对保护生物学和进化生物学至关重要，但缺乏覆盖多种计算机视觉任务的数据集，难以从野生种群中自动提取有生物学意义的个体动物测量数据

Method: 1) 创建CHIRP数据集：从瑞典拉普兰野生西伯利亚松鸦长期种群中收集，支持重识别、动作识别、2D关键点估计、目标检测和实例分割；2) 开发CORVID方法：基于彩色腿环分割和分类的鸟类个体识别管道，使用概率匹配算法将检测到的彩色环组合与数据库匹配；3) 引入应用特定基准测试：使用生物学相关指标（喂食率、共现率）评估模型在实际应用中的性能

Result: CHIRP数据集填补了野生动物监测数据集空白，支持多种计算机视觉任务；CORVID方法在应用特定基准测试中优于现有最先进的重识别方法；通过生物学相关指标验证了模型在实际应用中的有效性

Conclusion: 该工作为社区提供了从伦理批准的生物学研究中策划真实世界数据集的蓝图，弥合了计算机视觉研究与生物学应用之间的差距，推动了野生动物自动行为监测的发展
...
### [108] [SportSkills: Physical Skill Learning from Sports Instructional Videos](https://arxiv.org/abs/2603.25163)
*Kumar Ashutosh,Chi Hsuan Wu,Kristen Grauman*

Main category: cs.CV

TL;DR: SportSkills是首个面向物理技能学习的大规模体育数据集，包含36万+教学视频和63万+视觉演示，支持细粒度动作理解和个性化错误条件视频检索。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视频数据集主要关注一般人类活动，缺乏针对物理技能学习所需的细粒度活动覆盖深度。需要专门的数据集来支持动作理解和个性化反馈生成。

Method: 构建SportSkills数据集，包含55种不同运动的36万+教学视频和63万+视觉演示，配有解释动作要领的教学旁白。基于此数据集开发了错误条件教学视频检索任务，将表示学习与可操作反馈生成相结合。

Result: SportSkills显著提升了模型理解物理动作细粒度差异的能力，相比传统活动中心数据集训练相同模型，性能提升高达4倍。专业教练的正式评估表明，检索方法显著提升了视频模型为用户查询个性化视觉指导的能力。

Conclusion: SportSkills填补了物理技能学习数据集的空白，首次实现了大规模的错误条件教学视频检索任务，为动作理解和个性化反馈生成提供了重要基础。
...
### [109] [Towards Foundation Models for 3D Scene Understanding: Instance-Aware Self-Supervised Learning for Point Clouds](https://arxiv.org/abs/2603.25165)
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在点云语义理解方面取得进展，但生成的表示在实例定位任务上迁移效果差，通常需要完整微调才能获得良好性能。实例感知是3D感知的基础组成部分，弥合这一差距对于推进真正的3D基础模型至关重要。

Method: 提出PointINS框架，采用正交偏移分支联合学习高级语义理解和几何推理，实现实例感知。识别了两个对鲁棒实例定位至关重要的属性，并制定为互补的正则化策略：偏移分布正则化（ODR）将预测偏移与经验观察的几何先验对齐；空间聚类正则化（SCR）通过伪实例掩码正则化偏移来强制局部一致性。

Result: 在五个数据集上的广泛实验表明，PointINS在室内实例分割上平均提升+3.5% mAP，在室外全景分割上平均提升+4.1% PQ，为可扩展的3D基础模型铺平了道路。

Conclusion: PointINS通过几何感知的自监督学习成功增强了点云表示的实例感知能力，显著提升了实例定位性能，推动了向支持所有下游任务的真正3D基础模型的发展。
...
### [110] [ET-SAM: Efficient Point Prompt Prediction in SAM for Unified Scene Text Detection and Layout Analysis](https://arxiv.org/abs/2603.25168)
*Xike Zhang,Maoyuan Ye,Juhua Liu,Bo Du*

Main category: cs.CV

TL;DR: ET-SAM是一个基于SAM的高效框架，通过两个解码器实现统一的场景文本检测和布局分析，解决了传统方法依赖像素级分割导致的推理延迟高和数据利用率有限的问题。


<details>
  <summary>Details</summary>
Motivation: 基于SAM的方法在统一场景文本检测和布局分析中取得了良好性能，但传统方法依赖像素级文本分割来采样数千个前景点作为提示，导致推理延迟不理想且数据利用率有限。

Method: 提出ET-SAM框架，包含两个解码器：1) 轻量级点解码器生成词热力图以获得少量前景点，消除过多点提示并加速推理；2) 设计联合训练策略，利用具有异构文本级标注的现有数据；3) 为多级、词级和行级标注数据集引入三组可学习的任务提示，缓解数据集间的差异。

Result: 与之前的SAM架构相比，ET-SAM在HierText上实现了约3倍的推理加速，同时获得竞争性性能；在Total-Text、CTW1500和ICDAR15上的F-score平均提高了11.0%。

Conclusion: ET-SAM通过消除对像素级分割的依赖，实现了高效的统一场景文本检测和布局分析，显著加速推理并提高性能，同时能够有效利用异构标注数据。
...
TL;DR: DeepFAN是一种基于Transformer的深度学习模型，用于肺结节良恶性分类，在超过10K病理证实结节上训练，并通过多读者多病例临床试验验证，显著提升了初级放射科医生的诊断性能。


<details>
  <summary>Details</summary>
Motivation: CT的广泛应用增加了检测到的肺结节数量，但现有深度学习方法在全局和局部特征整合方面不足，且缺乏临床试验验证。

Method: 开发了基于Transformer的DeepFAN模型，在超过10,000个病理证实的肺结节上进行训练，并进行了多读者多病例临床试验，涉及三个独立医疗机构的400个病例和12名读者。

Result: DeepFAN在内部测试集上AUC为0.939，临床试验数据集上AUC为0.954。初级放射科医生使用DeepFAN后，平均AUC提升10.9%，准确率提升10.0%，敏感性提升7.6%，特异性提升12.6%。结节级读者间诊断一致性从一般提升到中等。
...
TL;DR: 该研究评估了9个开源多模态大语言模型在面部验证任务中的公平性，发现FaceLLM-8B（专门的面部模型）性能显著优于通用MLLMs，且偏见模式与传统面部识别系统不同。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）最近被探索作为面部验证系统，但与传统专用面部识别系统不同，MLLMs依赖通用视觉和推理能力。这些模型的群体公平性尚未得到充分研究，需要评估不同人口统计群体间的性能差异。

Method: 使用IJB-C和RFW面部验证协议，评估9个开源MLLMs（来自6个模型家族，参数规模2B-8B）。在4个种族群体和2个性别群体上测量验证准确率，采用等错误率和多个操作点的真实匹配率。使用4种基于错误接受率的公平性指标量化人口统计差异。

Result: FaceLLM-8B（研究中唯一的面部专用模型）在两个基准测试中显著优于通用MLLMs。观察到的偏见模式与传统面部识别系统不同，不同群体受影响程度取决于基准测试和模型。最准确的模型不一定最公平，整体准确率低的模型可能因为所有群体都产生高错误率而显得公平。

Conclusion: MLLMs在面部验证任务中表现出与传统面部识别系统不同的公平性特征，需要专门评估其群体公平性。面部专用模型性能优于通用模型，但公平性评估需考虑模型在不同人口统计群体间的相对性能差异。
...
TL;DR: 提出Just Zoom In方法，通过自回归缩放城市级卫星地图进行跨视角地理定位，避免对比学习和困难负样本挖掘，在真实基准测试中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法普遍采用基于对比学习的图像检索范式，存在三个主要问题：1) 依赖大批量和困难负样本挖掘；2) 忽略地图的几何结构；3) 街景与卫星图像覆盖不匹配导致目标模糊，限制了显式空间推理能力。

Method: 提出Just Zoom In方法，将CVGL重新定义为在城市级卫星地图上的自回归缩放过程。模型从粗粒度卫星视图开始，通过一系列缩放决策选择目标分辨率的终端卫星单元，无需对比损失或困难负样本挖掘。同时引入包含众包街景和高分辨率卫星图像的真实基准测试。

Result: 在提出的真实基准测试上，Just Zoom In达到最先进性能：在50米范围内Recall@1提升5.5%，在100米范围内Recall@1提升9.6%，显著优于最强的对比检索基线方法。

Conclusion: 结果表明，通过序列化从粗到细的空间推理进行跨视角地理定位是有效的，为CVGL提供了新的替代方案，克服了传统对比检索方法的局限性。
...
### [114] [Bilingual Text-to-Motion Generation: A New Benchmark and Baselines](https://arxiv.org/abs/2603.25178)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法面临双语数据集缺乏和语言模型跨语言语义理解不足的问题，限制了跨语言应用潜力

Method: 1) 通过LLM辅助标注和人工校正构建首个双语文本-动作基准BiHumanML3D；2) 提出双语动作扩散模型BiMD，采用跨语言对齐策略显式对齐不同语言的语义表示
...
TL;DR: VolDiT：首个基于纯Transformer的3D扩散模型，用于体素医学图像合成，通过体积块嵌入和全局自注意力直接处理3D数据，并提出时间步门控控制适配器实现精确空间引导。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像生成方法主要依赖卷积U-Net架构，这些架构存在局部性偏差和有限感受野的问题，限制了可扩展性、全局上下文整合和灵活条件控制。

Method: 提出VolDiT：1）通过体积块嵌入和全局自注意力将扩散Transformer扩展到原生3D数据；2）设计时间步门控控制适配器，将分割掩码映射为可学习的控制令牌，在去噪过程中调制Transformer层。

Result: 在高分辨率3D医学图像合成任务中，相比基于U-Net的3D潜在扩散模型，VolDiT表现出改进的全局一致性、更优的生成保真度和增强的可控性。

Conclusion: 完全基于Transformer的扩散模型为体素医学图像合成提供了灵活的基础框架，代码和模型已在公开数据上训练并开源。
...
TL;DR: AnyID是一个超保真身份保持视频生成框架，通过可扩展的全参考架构统一异构身份输入，并采用主参考生成范式实现精确属性级控制，解决了现有方法局限于单一身份参考的问题。


<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常针对单一身份参考进行设计和优化，这限制了创作灵活性，无法适应多样化的现实世界输入格式。单一参考源构成一个不适定场景，导致模型难以在新颖上下文中忠实地再现身份。
...
Conclusion: AnyID通过统一异构身份输入和引入主参考生成范式，解决了现有身份保持视频生成方法的局限性，实现了超高的身份保真度和精确的属性级控制，为创意表达提供了更强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型的内存机制将环境视为静态画布，当动态物体隐藏后重新出现时，常导致物体冻结、扭曲或消失，缺乏对动态物体的持续跟踪能力

Method: 提出混合内存范式，要求模型同时作为静态背景的精确档案员和动态物体的警觉跟踪器；构建HM-World数据集（59K高保真片段，17个场景，49个不同主体）；开发HyDRA架构，通过token化内存压缩和时空相关性驱动检索机制，选择性关注相关运动线索

Result: 在HM-World数据集上的广泛实验表明，该方法在动态物体一致性和整体生成质量方面显著优于现有最先进方法

Conclusion: 混合内存范式有效解决了动态物体隐藏后重现的连续性保持问题，HyDRA架构通过智能内存管理确保动态物体的身份和运动一致性，为视频世界模型研究提供了新方向
...
### [118] [CardioDiT: Latent Diffusion Transformers for 4D Cardiac MRI Synthesis](https://arxiv.org/abs/2603.25194)
...
TL;DR: CardioDiT：基于扩散Transformer的4D潜在扩散模型，用于短轴cine心脏MRI合成，实现空间和时间的统一建模，无需架构分解或辅助机制。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理cine心脏MRI（CMR）这种具有时间维度的3D医学图像时，通常将空间和时间分离建模，或通过解剖掩码等辅助机制强制时间一致性。这些策略引入了结构偏差，可能限制全局上下文整合，导致细微的时空不连续性或生理上不一致的心脏动力学。

Method: 提出CardioDiT，一个完全4D的潜在扩散框架：1）使用时空VQ-VAE将2D+t切片编码为紧凑潜在表示；2）采用扩散Transformer将整个3D+t体积作为联合建模对象，在生成过程中耦合空间和时间维度。

Result: 在公开CMR数据集和更大私有队列上评估，与具有渐进增强时空耦合的基线比较。结果显示：改进的切片间一致性、时间相干运动、真实的心脏功能分布，表明显式4D建模为时空心脏图像合成提供了原则性基础。

Conclusion: 统一的4D生成模型能够学习连续的心脏动力学而无需架构分解。基于扩散Transformer的CardioDiT框架通过耦合空间和时间维度，为心脏图像合成提供了更自然和生理一致的解决方案。
...
TL;DR: PixelSmile是一个基于扩散模型的细粒度面部表情编辑框架，通过完全对称联合训练解耦表情语义，结合强度监督和对比学习实现精确线性控制，在FFE-Bench上表现出色。


<details>
  <summary>Details</summary>
Motivation: 细粒度面部表情编辑长期受限于内在语义重叠问题，需要解决表情语义解耦、精确控制和身份保持之间的平衡。
...
Conclusion: PixelSmile能够实现连续、可控、细粒度的表情编辑，自然支持平滑表情混合，为面部表情编辑提供了有效的解决方案。
...
### [120] [CIV-DG: Conditional Instrumental Variables for Domain Generalization in Medical Imaging](https://arxiv.org/abs/2603.25202)
*Shaojin Bai,Yuting Su,Weizhi Nie*

Main category: cs.CV

TL;DR: CIV-DG：利用条件工具变量解决医学AI中因选择偏差导致的跨站点泛化问题，通过DeepGMM架构分离病理语义与扫描仪伪影


<details>
  <summary>Details</summary>
Motivation: 医学AI的跨站点泛化受到选择偏差的根本性损害，患者人口统计学特征（如年龄、严重程度）非随机地决定医院分配，导致站点特异性变化与诊断标签之间的虚假相关性。传统的域泛化方法主要针对图像级分布偏移，无法解决这种结构性混杂问题。

Method: 提出CIV-DG因果框架，利用条件工具变量分离病理语义与扫描仪诱导的伪影。通过放宽标准工具变量方法的严格随机分配假设，适应医院选择由患者人口统计学特征内生驱动的复杂临床场景。具体实现采用深度广义矩估计架构，使用条件批评器最小化矩违规并在人口统计学层内强制实施工具-误差正交性。

Result: 在Camelyon17基准和大规模胸部X射线数据集上的广泛实验表明，CIV-DG显著优于领先的基线方法，验证了条件因果机制在解决医学AI结构性混杂问题以实现鲁棒泛化方面的有效性。

Conclusion: CIV-DG通过条件工具变量框架成功解决了医学AI中由选择偏差引起的跨站点泛化问题，为处理结构性混杂提供了有效的因果方法，显著提升了医学AI模型在不同医疗站点间的泛化能力。
...
### [121] [Vega: Learning to Drive with Natural Language Instructions](https://arxiv.org/abs/2603.25741)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型大多仅将语言模态用于场景描述或推理，缺乏根据多样化用户指令进行个性化驾驶的灵活性

Method: 构建包含约10万个场景的大规模驾驶数据集InstructScene；提出Vega模型，采用自回归范式处理视觉输入和语言指令，扩散范式生成未来预测和轨迹；使用联合注意力实现模态间交互，不同模态使用独立投影层
...
### [122] [Probabilistic Concept Graph Reasoning for Multimodal Misinformation Detection](https://arxiv.org/abs/2603.25203)
*Ruichao Yang,Wei Gao,Xiaobin Zhu,Jing Ma,Hongzhan Lin,Ziyang Luo,Bo-Wen Zhang,Xu-Cheng Yin*

Main category: cs.CV

TL;DR: PCGR是一个可解释、可演化的多模态虚假信息检测框架，通过构建概率概念图进行结构化推理，在检测准确性和鲁棒性方面达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息日益严重，传统检测器存在不透明（黑盒）、脆弱（难以应对新型操纵手法）的问题，需要更可解释和适应性强的解决方案。

Method: 采用"先构建后推理"范式：1) 构建人类可理解的概念图，包括MLLM自动发现和验证的新型高级概念节点；2) 在概念图上应用分层注意力机制进行推理，得出声明真实性判断，生成可解释的证据-结论推理链。

Result: PCGR在多模态虚假信息检测的准确性和鲁棒性方面达到最先进水平，在粗粒度检测和细粒度操纵识别任务上都优于现有方法，特别是对新型操纵类型具有更好的适应性。

Conclusion: PCGR通过将多模态虚假信息检测重构为结构化、基于概念的概率推理问题，提供了一种可解释、可演化的解决方案，有效解决了传统黑盒检测器的局限性。
...
### [123] [SDD-YOLO: A Small-Target Detection Framework for Ground-to-Air Anti-UAV Surveillance with Edge-Efficient Deployment](https://arxiv.org/abs/2603.25218)
*Pengyu Chen,Haotian Sa,Yiwei Hu,Yuhan Cheng,Junbo Wang*

Main category: cs.CV

TL;DR: SDD-YOLO是针对地空反无人机监视的小目标检测框架，引入P2高分辨率检测头、YOLO26架构改进和MuSGD混合训练策略，在自建DroneSOD-30K数据集上mAP@0.5达86.0%，推理速度达226 FPS（RTX 5090）


<details>
  <summary>Details</summary>
Motivation: 地空视角下的小型无人机检测面临像素占比极低、空中背景杂乱和实时性要求严格等挑战。现有YOLO检测器主要针对通用目标检测，缺乏对亚像素目标的足够特征分辨率，且部署复杂。

Method: 1. 引入P2高分辨率检测头（4倍下采样）以捕获微目标的关键空间细节；2. 集成YOLO26架构改进：DFL-free、NMS-free架构简化推理；3. 采用MuSGD混合训练策略，结合ProgLoss和STAL，缓解稀疏小目标信号的梯度振荡；4. 构建DroneSOD-30K大规模地空数据集（约30,000张标注图像，涵盖多种气象条件）。

Result: SDD-YOLO-n在DroneSOD-30K数据集上mAP@0.5达到86.0%，比YOLOv5n基线提升7.8个百分点。推理性能：NVIDIA RTX 5090上226 FPS，Intel Xeon CPU上35 FPS，显示出色的边缘部署效率。

Conclusion: SDD-YOLO通过高分辨率检测头、简化架构和先进训练策略，有效解决了地空反无人机监视中的小目标检测难题，在精度和速度方面均表现优异，适合未来边缘部署。
...
### [124] [Training-free Detection and 6D Pose Estimation of Unseen Surgical Instruments](https://arxiv.org/abs/2603.25228)
*Jonas Hein,Lilian Calvet,Matthias Seibold,Siyu Tang,Marc Pollefeys,Philipp Fürnstahl*

Main category: cs.CV

TL;DR: 提出无需训练的6D姿态估计流程，仅需CAD模型作为先验，实现未见手术器械的多视角毫米级姿态估计


<details>
  <summary>Details</summary>
Motivation: 监督方法对新器械缺乏灵活性且需要大量标注数据，需要开发无需训练即可准确估计未见手术器械6D姿态的方法

Method: 两阶段流程：1) 检测阶段：生成掩膜提议，使用预训练特征提取器评分，跨视图匹配并三角化为3D实例候选；2) 姿态估计：迭代精炼姿态假设，使用多视角遮挡感知轮廓配准最小化重投影误差

Result: 在MVPSP数据集上达到毫米级精度，与监督方法相当，同时保持对未见器械的完全泛化能力

Conclusion: 提出新颖灵活流程，结合基础模型、多视角几何和轮廓精炼，实现无需特定训练的高精度手术器械6D姿态估计
...
### [125] [An Image Dataset of Common Skin Diseases of Bangladesh and Benchmarking Performance with Machine Learning Models](https://arxiv.org/abs/2603.25229)
...
<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球性的重大公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备严重不足，缺乏及时检测和治疗可能导致严重后果甚至死亡。利用人工智能和机器学习技术，通过图像处理和计算机视觉方法检测皮肤疾病，可以缓解医疗资源短缺问题。

Method: 收集了来自孟加拉国法里德布尔医学院门诊患者的皮肤疾病图像数据，构建了包含五种常见皮肤病的公开数据集：接触性皮炎、白癜风、湿疹、疥疮和癣。数据集包含1612张图像（其中250张为原始图像，其余为增强图像）。应用了多种机器学习和深度学习模型对数据集进行分类性能评估。

Result: 创建了一个包含五种皮肤病的公开数据集，具体分布为：皮炎302张、湿疹381张、疥疮301张、癣316张、白癜风312张。虽然数据收集具有地域性，但这些疾病在南亚地区普遍存在，使该数据集在全球机器学习皮肤病学应用中具有潜在价值。研究还报告了不同模型在数据集上的分类性能。

Conclusion: 该研究开发了一个有价值的皮肤疾病检测公开数据集，为机器学习在皮肤病自动诊断领域的应用提供了资源。数据集虽然基于孟加拉国地区数据，但所包含的疾病在南亚地区普遍存在，具有全球应用潜力。研究期望能吸引从事自动化疾病诊断的机器学习和深度学习研究人员及从业者的关注。
...
### [126] [A Unified Spatial Alignment Framework for Highly Transferable Transformation-Based Attacks on Spatially Structured Tasks](https://arxiv.org/abs/2603.25230)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于变换的对抗攻击(TAAs)在分类任务中表现出良好的可迁移性，但在结构化任务（语义分割、目标检测）中效果不佳甚至失败。研究发现，结构化任务的标签具有空间结构，而现有TAAs在应用空间变换时未能同步调整标签，导致空间错位和错误梯度。
...
### [127] [Towards Practical Lossless Neural Compression for LiDAR Point Clouds](https://arxiv.org/abs/2603.25260)
...
<details>
  <summary>Details</summary>
Motivation: LiDAR点云的极端稀疏性限制了现有方法的压缩速度和性能，需要一种紧凑表示来实现高效预测无损编码

Method: 包含两个轻量级模块：1) 几何重稠密化模块迭代稠密化稀疏几何，提取稠密尺度特征后稀疏化用于预测编码；2) 跨尺度特征传播模块利用多分辨率占用线索指导分层特征传播，实现跨尺度信息共享；还引入了整数推理管道确保跨平台一致性

Result: 实验表明在实时速度下具有竞争力的压缩性能，代码已在GitHub开源

Conclusion: FastPCC框架通过紧凑表示和轻量级模块设计，在保持高性能的同时实现了实时无损点云压缩，解决了现有方法在稀疏几何细节处理上的效率瓶颈
...
### [128] [Knowledge-Guided Failure Prediction: Detecting When Object Detectors Miss Safety-Critical Objects](https://arxiv.org/abs/2603.25499)
...
<details>
  <summary>Details</summary>
Motivation: 安全关键环境中的目标检测器可能无声地失效（如漏检行人等安全关键对象），而传统的OOD检测方法仅识别陌生输入，无法直接预测检测器本身的功能失效。

Method: 提出知识引导的失效预测（KGFP）框架，使用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义错位。当检测器超出其能力范围或视觉基础模型遇到新输入时，两种嵌入会发散产生高角度信号。

Result: 在COCO行人检测任务中，KGFP作为选择性预测门，在5%假阳性率下将接受图像中行人的召回率从64.3%提升至84.5%，并在六个COCO-O视觉域中保持强劲性能，大幅优于OOD基线方法。

Conclusion: KGFP通过监测目标检测器与视觉基础模型之间的语义一致性，能够有效预测功能失效，为安全关键应用中的目标检测器提供了可靠的运行时监控解决方案。
...
### [129] [V2U4Real: A Real-world Large-scale Dataset for Vehicle-to-UAV Cooperative Perception](https://arxiv.org/abs/2603.25275)
*Weijia Li,Haoen Xiang,Tianxu Wang,Shuaibing Wu,Qiming Xia,Cheng Wang,Chenglu Wen*

Main category: cs.CV

TL;DR: V2U4Real是首个大规模真实世界多模态车辆-无人机协同感知数据集，通过地面车辆和无人机配备多视角LiDAR和RGB相机采集，包含超过56K LiDAR帧、56K多视角图像和700K标注3D边界框，覆盖城市街道、校园和乡村道路等多种交通场景。


<details>
  <summary>Details</summary>
Motivation: 现有车辆协同感知（V2V和V2I）局限于地面协作，无法完全解决大规模遮挡和复杂环境中的长距离感知问题。需要推进跨视角协同感知研究，特别是车辆与无人机（V2U）的协作，以增强感知鲁棒性和长距离感知能力。

Method: 通过地面车辆和无人机配备多视角LiDAR和RGB相机采集数据，构建大规模真实世界多模态数据集。数据集覆盖城市街道、大学校园和乡村道路的多样化交通场景，包含超过56K LiDAR帧、56K多视角相机图像和700K标注3D边界框（涵盖四类对象）。

Result: 建立了单智能体3D目标检测、协同3D目标检测和目标跟踪的基准测试。对多个最先进模型的综合评估表明，V2U协作能有效增强感知鲁棒性和长距离感知能力。数据集和代码已开源。

Conclusion: V2U4Real作为首个大规模真实世界V2U协同感知数据集，为跨视角协同感知研究提供了重要资源。V2U协作能显著提升感知系统的鲁棒性和长距离感知能力，为解决遮挡和感知范围限制问题提供了新途径。
...
TL;DR: 提出两种层次感知多模态学习方法CLiBD-HiR和CLiBD-HiR-Fuse，通过编码生物分类层次结构提升大规模生物多样性识别准确率


<details>
  <summary>Details</summary>
Motivation: 现有多模态方法将分类学视为扁平标签空间，未能编码生物分类的层次结构，这在噪声和模态缺失条件下影响鲁棒性

Method: CLiBD-HiR引入层次信息正则化(Hierarchical Information Regularization)塑造跨分类层级的嵌入几何；CLiBD-HiR-Fuse额外训练轻量级融合预测器，支持图像、DNA或联合推理，对模态损坏具有弹性

Result: 在大规模生物多样性基准测试中，相比强大多模态基线，分类准确率提升超过14%，在部分和损坏DNA条件下增益尤为显著
...
### [131] [MACRO: Advancing Multi-Reference Image Generation with Structured Long-Context Data](https://arxiv.org/abs/2603.25319)
*Zhekai Chen,Yuqing Wang,Manyuan Zhang,Xihui Liu*

Main category: cs.CV

TL;DR: 论文提出MacroData数据集和MacroBench基准，解决多参考图像生成任务中因数据瓶颈导致的性能下降问题。
...
Method: 构建MacroData大规模数据集（40万样本，每个最多10张参考图像），按四个互补维度组织：定制化、插画、空间推理和时间动态；同时提出MacroBench基准（4000样本）用于标准化评估。

Result: 在MacroData上微调显著提升多参考生成性能，消融研究显示跨任务协同训练具有协同效益，并提出了处理长上下文复杂性的有效策略。

Conclusion: MacroData和MacroBench解决了多参考图像生成的数据和评估瓶颈，为研究社区提供了系统化解决方案，数据集和基准将公开发布。
...
TL;DR: LanteRn框架让大型多模态模型能够在潜在空间中直接进行视觉推理，通过生成和关注连续的视觉思想嵌入，避免了依赖外部模块或在像素空间中推理的计算开销。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在视觉推理方面存在局限，大多将感知内容转化为文本处理，这限制了需要细粒度空间和视觉理解的任务。现有方法要么依赖外部工具，要么在像素空间中直接推理导致不必要的计算开销。

Method: 提出LanteRn框架，增强视觉-语言Transformer的能力，使其能够在推理过程中生成和关注连续的视觉思想嵌入。采用两阶段训练：监督微调将视觉特征映射到潜在状态，然后通过强化学习将潜在推理与任务级效用对齐。

Result: 在三个感知中心基准测试（VisCoT、V*和Blink）上评估，观察到在视觉基础和细粒度推理方面的一致改进。

Conclusion: 内部潜在表示为更高效的多模态推理提供了有前景的方向，使模型能够在潜在空间中直接进行视觉推理，避免了对文本转换或像素级处理的依赖。
...
### [133] [HeSS: Head Sensitivity Score for Sparsity Redistribution in VGGT](https://arxiv.org/abs/2603.25336)
...
<details>
  <summary>Details</summary>
Motivation: VGGT等视觉几何Transformer的全局注意力层存在二次计算复杂度问题，现有稀疏化加速技术通常导致显著的精度下降。作者假设精度下降源于注意力头对稀疏化的敏感度存在异质性，而现有方法对所有头采用统一的稀疏模式

Method: 提出两阶段稀疏化流程：1) 使用新颖的Head Sensitivity Score (HeSS)度量在小型校准集上量化每个注意力头的稀疏化敏感度，该分数近似计算两个不同误差项的Hessian；2) 在推理阶段执行HeSS引导的稀疏化，利用预计算的HeSS重新分配总注意力预算，为敏感头分配更密集的注意力，为更鲁棒的头分配更稀疏的注意力

Result: 实验证明HeSS能有效捕捉头级稀疏化敏感度，确认全局注意力层中的注意力头确实表现出异质性敏感度特征。在多种稀疏化水平下，该方法能有效缓解高稀疏度下的性能下降，展现出强大的鲁棒性

Conclusion: 通过量化注意力头对稀疏化的异质性敏感度并相应调整稀疏化策略，可以显著减少稀疏化带来的精度损失，为Transformer模型的加速提供了更有效的方法
...
### [134] [No Hard Negatives Required: Concept Centric Learning Leads to Compositionality without Degrading Zero-shot Capabilities of Contrastive Models](https://arxiv.org/abs/2603.25722)
*Hai X. Pham,David T. Hoffmann,Ricardo Guerrero,Brais Martinez*

Main category: cs.CV

TL;DR: 论文提出两种简单方法改善视觉语言模型的组合性：1) 使用短概念中心字幕部分与图像对齐；2) 引入参数自由的跨模态注意力池化获取概念中心视觉嵌入，在保持基础能力的同时显著提升组合性任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉语言模型在组合性表示学习方面存在局限。先前方法通过生成自定义训练数据获取困难负样本，但这些方法通常针对单一基准，缺乏泛化性，且会严重损害模型的零样本和检索等基础能力，实用性不足。

Method: 1) 使用标准NLP工具获取短概念中心字幕部分并与图像对齐；2) 引入参数自由的跨模态注意力池化机制，从图像编码器中提取概念中心视觉嵌入；3) 结合简单的辅助对比损失进行训练。

Result: 在标准组合性基准测试中达到最先进性能，同时保持或改善了零样本和检索能力，且不增加推理成本。

Conclusion: 通过识别并解决限制V&L模型组合性表现的两个根本原因（长训练字幕不需要组合性表示、全局池化导致绑定信息丢失），提出的两种简单解决方案能有效提升模型组合性，同时保持基础能力，具有实用价值。
...
### [135] [InstanceAnimator: Multi-Instance Sketch Video Colorization](https://arxiv.org/abs/2603.25357)
*Yinhan Zhang,Yue Ma,Bingyuan Wang,Kunyu Feng,Yeying Jin,Qifeng Chen,Anyi Rao,Zeyu Wang*

Main category: cs.CV

TL;DR: InstanceAnimator是一个基于扩散Transformer的多实例草图视频着色框架，通过画布引导条件、实例匹配机制和自适应解耦控制模块解决了现有方法的三大限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心限制：1) 由于过度依赖单一参考帧导致用户控制不灵活；2) 在多角色场景中实例可控性差导致错位；3) 细粒度区域细节保真度下降。需要开发能够同时解决这些问题的多实例草图视频着色方法。

Method: 提出InstanceAnimator框架，包含三个创新组件：1) 画布引导条件，允许自由放置参考元素和背景，消除工作流程碎片化；2) 实例匹配机制，将实例特征与草图集成，确保对多个角色的精确控制；3) 自适应解耦控制模块，将角色、背景和文本条件的语义特征注入扩散过程，增强细节保真度。

Result: 大量实验表明，InstanceAnimator在多实例着色方面表现优异，具有增强的用户控制能力、高视觉质量和强大的实例一致性。

Conclusion: InstanceAnimator通过创新的画布引导条件、实例匹配机制和自适应解耦控制模块，成功解决了现有草图视频着色方法的三大核心限制，实现了灵活的用户控制、精确的多实例对齐和高质量的细节保真。
...
### [136] [CLIP-RD: Relational Distillation for Efficient CLIP Knowledge Distillation](https://arxiv.org/abs/2603.25383)
*Jeannie Chung,Hanna Jang,Ingyeong Yang,Uiwon Hwang,Jaehyung Sim*

Main category: cs.CV

TL;DR: 提出CLIP-RD关系知识蒸馏框架，通过垂直关系蒸馏和交叉关系蒸馏方法，在轻量级学生模型中更好地保持CLIP教师模型的多向关系结构


<details>
  <summary>Details</summary>
Motivation: CLIP模型虽然具有强大的零样本泛化能力，但其大规模架构需要大量计算和内存资源。现有CLIP蒸馏方法未能显式建模教师和学生嵌入之间的多向关系依赖，限制了学生模型保持教师编码的结构关系的能力

Method: 提出关系知识蒸馏框架CLIP-RD，包含两种新方法：1）垂直关系蒸馏（VRD）- 在分布层面强制教师-学生蒸馏强度在跨模态间的一致性；2）交叉关系蒸馏（XRD）- 对跨模态教师-学生相似度分布施加双向对称性约束

Result: CLIP-RD通过联合建模多向关系结构，促进学生嵌入几何与教师嵌入几何的忠实对齐，性能优于现有方法0.8个百分点

Conclusion: 提出的关系知识蒸馏框架通过显式建模教师和学生嵌入之间的多向关系依赖，有效提升了轻量级学生模型保持CLIP教师模型结构关系的能力，实现了更好的知识迁移效果
...
### [137] [Multimodal Dataset Distillation via Phased Teacher Models](https://arxiv.org/abs/2603.25388)
*Shengbin Guo,Hang Zhao,Senqiao Yang,Chenyang Jiang,Yuhang Cheng,Xiangru Peng,Rui Shao,Zhuotao Tian*

Main category: cs.CV

TL;DR: 提出PTM-ST框架，通过分阶段教师建模和基于捷径的轨迹构建策略，解决多模态数据集蒸馏中跨阶段性能差距和教师轨迹不稳定问题，显著提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉教师模型后期训练阶段中复杂、动态演化的知识，导致学生性能下降和蒸馏数据质量受损，特别是存在显著的跨阶段性能差距和不稳定的教师轨迹问题。

Method: 提出PTM-ST框架，包含分阶段教师建模和基于捷径的轨迹构建策略。前者准确拟合教师在不同训练阶段的学习动态，后者增强蒸馏过程的稳定性和表达能力。

Result: PTM-ST显著缓解优化振荡和阶段间知识差距，同时减少存储开销。在Flickr30k和COCO数据集上超越现有方法，Flickr30k上最高提升13.5%，平均提升9.53%。

Conclusion: PTM-ST通过分阶段蒸馏框架有效解决了多模态数据集蒸馏中的关键挑战，提高了学生模型性能并改善了蒸馏数据质量，为高效知识压缩和迁移提供了新思路。
...
### [138] [FSGNet: A Frequency-Aware and Semantic Guidance Network for Infrared Small Target Detection](https://arxiv.org/abs/2603.25389)
...
Method: 提出FSGNet框架，包含三个核心模块：1) 编码器中的多方向交互注意力模块，捕捉细粒度和方向性特征；2) 多尺度频率感知模块，利用快速傅里叶变换过滤目标相似杂波；3) 全局池化模块捕获高层语义信息，通过全局语义引导流传播到各解码器阶段。

Result: 在四个公开IRSTD数据集上的大量实验表明，FSGNet实现了优越的检测性能，同时保持了高效率，突出了其实用性和鲁棒性。

Conclusion: FSGNet通过频率感知和语义引导机制有效解决了U-Net中的语义退化问题，为红外小目标检测提供了一个轻量级且高效的解决方案，具有实际应用价值。
...
### [139] [PMT: Plain Mask Transformer for Image and Video Segmentation with Frozen Vision Encoders](https://arxiv.org/abs/2603.25398)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型的编码器-仅分割方法（如EoMT、VidEoMT）虽然速度快，但需要微调编码器，牺牲了VFMs的多任务编码器共享优势。需要平衡编码器-仅设计的简单性与冻结VFM特征的保持

Method: 提出Plain Mask Decoder (PMD)，一种基于Transformer的快速分割解码器，在冻结的VFM特征上操作。由此产生的PMT模型保持了编码器-仅设计的架构简单性和低延迟，同时保持编码器表示不变且可共享

Result: 在图像分割基准测试中，PMT匹配冻结编码器的最先进性能，同时运行速度提高约3倍。在视频分割中，性能与完全微调的方法相当，同时比最先进的冻结编码器模型快8倍

Conclusion: PMT成功平衡了编码器-仅设计的简单性和速度与冻结VFM特征的保持，实现了高效的多任务编码器共享，适用于图像和视频分割任务
...
### [140] [LaMP: Learning Vision-Language-Action Policies with 3D Scene Flow as Latent Motion Prior](https://arxiv.org/abs/2603.25399)
*Xinkai Wang,Chenyi Wang,Yifu Xu,Mingzhe Ye,Fu-Cheng Zhang,Jialin Tian,Xinyu Zhan,Lifeng Zhu,Cewu Lu,Lixin Yang*

Main category: cs.CV

TL;DR: LaMP是一种双专家视觉-语言-动作框架，将密集3D场景流作为潜在运动先验嵌入机器人操作，通过流匹配运动专家和策略预测动作专家的门控交叉注意力对齐，在多个基准测试中优于现有VLA基线。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型直接从2D语义视觉特征回归动作，迫使它们隐式学习复杂的3D物理交互，这种隐式学习策略在不熟悉的空间动态下性能下降。需要显式建模3D运动先验来提高机器人操作的鲁棒性和泛化能力。

Method: 提出LaMP双专家框架：1) 运动专家通过流匹配生成一步部分去噪的3D场景流；2) 动作专家预测策略动作；3) 通过门控交叉注意力将运动专家的隐藏状态条件化动作专家，无需完整多步重建。将密集3D场景流作为潜在运动先验嵌入。
...
Conclusion: 通过将3D场景流作为潜在运动先验显式嵌入，LaMP框架有效解决了现有VLA模型在复杂3D物理交互学习中的局限性，显著提高了机器人操作的性能和鲁棒性，特别是在空间动态不熟悉的情况下。
...
### [141] [HiSpatial: Taming Hierarchical 3D Spatial Understanding in Vision-Language Models](https://arxiv.org/abs/2603.25411)
*Huizhi Liang,Yichao Shen,Yu Deng,Sicheng Xu,Zhiyuan Feng,Tong Zhang,Yaobo Liang,Jiaolong Yang*

Main category: cs.CV

TL;DR: 提出分层框架将3D空间理解分解为四个渐进层次，构建自动化流水线生成大规模3D空间VQA数据，开发RGB-D VLM，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 实现类人空间智能需要从2D观察推断3D结构、识别3D空间中的物体属性和关系、进行高级空间推理。现有VLMs在3D空间理解方面存在不足。

Method: 提出分层框架将3D空间理解分解为几何感知、物体级理解、场景级理解和抽象推理四个层次；构建自动化流水线处理约500万图像生成4500万物体的3D空间VQA数据；开发RGB-D VLM，将度量尺度点云图作为辅助输入。

Result: 在多个空间理解和推理基准上达到最先进性能，超越专用空间模型和大型专有系统如Gemini-2.5-pro和GPT-5；分析显示分层任务级别之间存在明确依赖关系。

Conclusion: 分层框架有效促进3D空间智能的涌现；多级任务设计为VLMs的3D空间理解提供了新见解；自动化数据生成和RGB-D VLM架构显著提升空间推理能力。
...
### [142] [VideoWeaver: Multimodal Multi-View Video-to-Video Transfer for Embodied Agents](https://arxiv.org/abs/2603.25420)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频到视频转换方法只能处理单视角，而具身AI任务通常需要多视角同步摄像头支持策略学习。独立应用单视角模型会导致跨视角外观不一致，标准Transformer架构由于跨视角注意力的二次成本无法扩展到多视角设置。

Method: 1. 首先训练为基于流的单视角V2V模型；2. 通过Pi3空间基础模型将所有视角锚定在共享4D潜在空间中，确保视角一致性；3. 在不同扩散时间步训练不同视角，使模型学习联合和条件视角分布；4. 支持基于现有视角自回归合成新视角。

Result: 在单视角转换基准上达到或优于最先进水平；首次实现物理和风格一致的多视角转换，包括对机器人学习世界随机化至关重要的挑战性自我中心视角和异构摄像头设置。

Conclusion: VideoWeaver是首个多模态多视角V2V转换框架，通过共享4D潜在空间和扩散时间步训练策略，解决了多视角一致性问题，为具身AI演示的重新模拟提供了可扩展解决方案。
...
### [143] [DC-Reg: Globally Optimal Point Cloud Registration via Tight Bounding with Difference of Convex Programming](https://arxiv.org/abs/2603.25442)
...
<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在部分重叠和大错位情况下面临挑战：同时估计变换和对应关系的非凸耦合目标容易陷入局部最优，而现有全局求解器由于下界松弛导致收敛时间过长。

Method: 提出DC-Reg框架，核心创新是基于凸差规划推导出变换-对应耦合目标的整体凹下界估计器。不同于依赖逐项松弛的方法，该整体DC分解捕捉了变换参数θ和对应矩阵P之间的联合结构交互，通过搜索框顶点处的高效线性分配问题计算紧致下界。

Result: 在合成数据和3DMatch基准测试中，DC-Reg相比最先进的全局技术实现了显著更快的收敛速度，对极端噪声和异常值具有更强的鲁棒性，验证了其在2D相似变换和3D刚性配准中的有效性。

Conclusion: DC-Reg通过整体凸差分解为耦合变换-对应优化问题提供了紧致的全局下界，显著加速了分支定界搜索，为解决部分重叠和大错位点云配准的全局最优问题提供了有效框架。
...
### [144] [CIAR: Interval-based Collaborative Decoding for Image Generation Acceleration](https://arxiv.org/abs/2603.25463)
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成方面取得了显著进展，但其计算密集和序列特性阻碍了设备端部署，导致高延迟。需要解决视觉合成中的两个关键特性：高保真图像所需的大词汇量和空间冗余性
...
Conclusion: CIAR通过设备端不确定性量化和云-设备协作，有效解决了自回归图像生成的延迟问题，为设备端部署提供了可行的解决方案
...
### [145] [GridVAD: Open-Set Video Anomaly Detection via Spatial Reasoning over Stratified Frame Grids](https://arxiv.org/abs/2603.25467)
*Mohamed Eltahir,Ahmed O. Ibrahim,Obada Siralkhatim,Tabarak Abdallah,Sondos Mohamed*

Main category: cs.CV

TL;DR: GridVAD：一种无需训练的零样本视频异常检测框架，通过"提议-定位-传播"原则，使用视觉语言模型生成异常提议，再通过专门的空间和时间模块进行定位和跟踪，实现像素级异常分割。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型作为开放集推理器在视频异常检测中直接使用时存在脆弱性：缺乏校准的异常先验会导致漏检和误报。问题不在于VLM本身，而在于如何使用它们。VLM应作为异常提议生成器，而专门的时空模块负责定位和跟踪。

Method: GridVAD采用"提议-定位-传播"流程：1) VLM在视频片段的分层网格表示上进行推理，生成自然语言异常提议；2) 自一致性整合通过多次独立采样过滤幻觉提议；3) Grounding DINO将提议锚定为边界框；4) SAM2在异常区间内传播为密集掩码。整个流程无需训练，VLM调用次数固定为M+1次。

Result: 在UCSD Ped2数据集上，GridVAD达到最高像素级AUROC（77.59），超越部分微调的TAO（75.11），在物体级RBDC上比其他零样本方法高出5倍以上。自一致性整合提供可控的精度-召回权衡，过滤可改善所有像素级指标。效率上比均匀每帧VLM查询高效2.7倍。

Conclusion: GridVAD证明了将VLM作为异常提议生成器，配合专门时空模块的"提议-定位-传播"原则的有效性，实现了无需训练的零样本像素级异常检测，在精度和效率上均优于现有方法。
...
### [146] [AdaSFormer: Adaptive Serialized Transformers for Monocular Semantic Scene Completion from Indoor Environments](https://arxiv.org/abs/2603.25494)
...
<details>
  <summary>Details</summary>
Motivation: 室内单目语义场景补全比室外更具挑战性，因为复杂的空间布局和严重遮挡。Transformer虽然适合建模全局依赖，但其高内存成本和难以重建细粒度细节限制了其在室内MSSC中的应用。
...
Conclusion: AdaSFormer通过创新的串行化Transformer设计有效解决了室内MSSC中Transformer的内存和细节重建问题，在多个基准数据集上取得了最优结果。
...
### [147] [RealRestorer: Towards Generalizable Real-World Image Restoration with Large-Scale Image Editing Models](https://arxiv.org/abs/2603.25502)
...
TL;DR: 构建大规模真实世界退化数据集和开源模型，缩小与闭源模型在图像修复任务上的性能差距，并建立包含464张真实退化图像的评估基准RealIR-Bench。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复模型受限于训练数据的规模和分布，在真实场景中泛化能力差。虽然大型闭源编辑模型（如Nano Banana Pro）在修复任务上表现出色，但需要大量数据和计算成本。需要开发开源解决方案来缩小这一差距。

Method: 1. 构建覆盖九种常见真实世界退化类型的大规模数据集；2. 训练最先进的开源模型；3. 提出RealIR-Bench评估基准，包含464张真实退化图像和专注于退化去除与一致性保持的定制评估指标。

Result: 大量实验表明，所提出的模型在开源方法中排名第一，实现了最先进的性能表现。

Conclusion: 通过构建大规模数据集和开源模型，成功缩小了与闭源模型在真实世界图像修复任务上的性能差距，并建立了专门的评估基准来推动该领域的发展。
...
### [148] [Beyond the Golden Data: Resolving the Motion-Vision Quality Dilemma via Timestep Selective Training](https://arxiv.org/abs/2603.25527)
*Xiangyang Luo,Qingyu Li,Yuming Li,Guanbo Huang,Yongjie Zhu,Wenyu Qin,Meng Wang,Pengfei Wan,Shao-Lun Huang*

Main category: cs.CV

TL;DR: TQD通过时间步感知质量解耦，利用质量不平衡数据训练视频生成模型，挑战了完美数据的必要性
...
Method: 提出时间步感知质量解耦（TQD）：分析视频扩散模型的分层学习动态，发现质量不平衡数据在适当时间步能产生与黄金数据相似的梯度；修改数据采样分布，使运动丰富数据偏向高时间步采样，高视觉质量数据偏向低时间步采样

Result: TQD仅使用分离的不平衡数据训练即可超越传统使用更好数据的方法；在高质量数据上训练也能提升模型性能，在不同数据场景下均有效

Conclusion: TQD挑战了视频生成中完美数据的必要性，通过时间步选择训练过程，有效利用质量不平衡数据提升模型性能
...
### [149] [BFMD: A Full-Match Badminton Dense Dataset for Dense Shot Captioning](https://arxiv.org/abs/2603.25533)
*Ning Ding,Keisuke Fujii,Toru Tamaki*

Main category: cs.CV

TL;DR: 论文提出了首个羽毛球完整比赛密集标注数据集BFMD，包含19场完整比赛、1687个回合和16751次击球事件，并开发了基于VideoMAE的多模态字幕生成框架，通过语义反馈机制提升击球描述质量。


<details>
  <summary>Details</summary>
Motivation: 现有羽毛球数据集主要关注短片段或特定任务标注，缺乏完整比赛数据和密集多模态标注，这限制了准确击球描述生成和比赛级别战术分析的能力。

Method: 1) 构建BFMD数据集：包含19场广播比赛（单打和双打），超过20小时比赛时间，提供分层标注（比赛片段、回合事件、密集回合级多模态标注）；2) 开发基于VideoMAE的多模态字幕生成框架，引入语义反馈机制，利用击球语义指导字幕生成。

Result: 实验结果表明，多模态建模和语义反馈相比仅使用RGB的基线方法显著提高了击球字幕质量。进一步展示了BFMD在分析完整比赛中战术模式时间演化方面的潜力。

Conclusion: BFMD数据集填补了羽毛球完整比赛密集标注数据的空白，提出的多模态字幕生成框架通过语义反馈机制提升了击球描述质量，为羽毛球战术动态分析和比赛级别研究提供了重要资源。
...
### [150] [PAWS: Perception of Articulation in the Wild at Scale from Egocentric Videos](https://arxiv.org/abs/2603.25539)
...
TL;DR: PAWS方法从大规模第一人称视角视频中的手-物体交互直接提取物体铰接结构，无需3D数据监督标注，提升铰接感知的扩展性和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的铰接感知方法严重依赖高质量3D数据和人工标注，限制了方法的扩展性和多样性。需要从更易获取的大规模真实世界视频中学习铰接结构。

Method: 提出PAWS方法，直接从大规模第一人称视角视频中的手-物体交互提取物体铰接结构，无需3D监督标注。利用手部运动作为铰接运动的自然监督信号。

Result: 在HD-EPIC和Arti4D数据集上评估，相比基线方法取得显著改进。提取的铰接结构有利于下游任务，包括微调3D铰接预测模型和机器人操作。

Conclusion: PAWS证明了从大规模真实世界视频中学习铰接结构的可行性，为铰接感知提供了更可扩展和多样化的数据源，并能有效支持下游应用。
...
### [151] [GeoHeight-Bench: Towards Height-Aware Multimodal Reasoning in Remote Sensing](https://arxiv.org/abs/2603.25565)
*Xuran Hu,Zhitong Xiong,Zhongcheng Hong,Yifang Ban,Xiaoxiang Zhu,Wufan Zhao*

Main category: cs.CV

TL;DR: 提出首个面向地球观测的高度感知评估框架，通过VLM驱动的数据生成构建两个高度感知基准，并开发首个高度感知遥感LMM基线模型GeoHeightChat，验证了高度信息对遥感理解的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在地球观测领域普遍忽视关键的"垂直"维度，限制了其在复杂遥感几何结构和灾害场景中的推理能力，而物理空间结构往往比平面视觉纹理更重要。

Method: 1) 开发可扩展的VLM驱动数据生成管道，利用系统提示工程和元数据提取构建两个互补基准：GeoHeight-Bench用于相对高度分析，GeoHeight-Bench+用于整体地形感知推理；2) 提出首个高度感知遥感LMM基线GeoHeightChat，将视觉语义与隐式注入的高度几何特征相结合。

Result: GeoHeightChat基线模型有效缓解了"垂直盲点"问题，成功在现有光学模型中解锁了交互式高度推理的新范式，证明了视觉语义与高度几何特征协同的有效性。

Conclusion: 高度感知对于遥感理解至关重要，提出的评估框架和基线模型为地球观测中的高度感知研究奠定了基础，开启了现有光学模型中交互式高度推理的新范式。
...
### [152] [TRACE: Object Motion Editing in Videos with First-Frame Trajectory Guidance](https://arxiv.org/abs/2603.25707)
...
Conclusion: Trace框架提供了一种实用且用户友好的视频物体运动路径编辑方法，通过单锚帧设计轨迹，有效处理相机运动，实现高质量的运动编辑。
...
### [153] [AnyHand: A Large-Scale Synthetic Dataset for RGB(-D) Hand Pose Estimation](https://arxiv.org/abs/2603.25726)
*Chen Si,Yulin Liu,Bo Ai,Jianwen Xie,Rolandos Alexandros Potamias,Chuanxia Zheng,Hao Su*

Main category: cs.CV

TL;DR: AnyHand是一个大规模合成数据集，包含650万张RGB-D图像，用于提升3D手部姿态估计性能，在RGB-only和RGB-D设置下均显著改进现有基准，并展示了强大的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界收集的手部姿态估计数据集覆盖范围有限，而先前的合成数据集很少大规模提供遮挡、手臂细节和深度对齐信息，这限制了3D手部姿态估计的性能和鲁棒性提升。

Method: 创建包含250万张单手图像和410万张手-物体交互图像的大规模合成RGB-D数据集AnyHand，提供丰富的几何标注；同时贡献了一个轻量级深度融合模块，可轻松集成到现有RGB模型中。

Result: 在RGB-only设置下，使用AnyHand扩展训练集显著提升了FreiHAND和HO-3D基准性能；在未微调情况下，模型在跨域HO-Cap数据集上表现出更强的泛化能力；RGB-D模型在HO-3D基准上达到最优性能。

Conclusion: AnyHand数据集有效解决了3D手部姿态估计的数据瓶颈，通过大规模合成数据显著提升了模型性能、鲁棒性和泛化能力，深度融合模块进一步展示了RGB-D输入的优越性。
...
### [154] [BizGenEval: A Systematic Benchmark for Commercial Visual Content Generation](https://arxiv.org/abs/2603.25732)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注自然图像合成，无法系统评估模型在现实商业设计任务中的结构化、多约束要求下的表现。需要专门针对商业视觉内容生成创建系统性评估框架。

Method: 构建BizGenEval基准测试，包含五种代表性文档类型（幻灯片、图表、网页、海报、科学图表）和四个关键能力维度（文本渲染、布局控制、属性绑定、知识推理），形成20个评估任务。包含400个精心设计的提示和8000个人工验证的检查清单问题。

Result: 对26个流行的图像生成系统（包括最先进的商业API和领先的开源模型）进行大规模基准测试，结果显示当前生成模型与专业视觉内容创作需求之间存在显著能力差距。

Conclusion: BizGenEval可作为现实世界商业视觉内容生成的标准化基准测试，揭示了当前生成模型在满足复杂视觉和语义约束方面的局限性，为未来模型改进提供了评估框架。
...
### [155] [SlotVTG: Object-Centric Adapter for Generalizable Video Temporal Grounding](https://arxiv.org/abs/2603.25733)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频时序定位任务中存在两个主要问题：1）粗粒度识别能力不足，需要任务特定的微调；2）微调导致模型记忆数据集特定的捷径而非真正基于视觉内容进行定位，导致跨域泛化能力差。对象中心学习虽然提供了解决方案，但现有方法需要从头重新训练整个多阶段训练流程，成本高昂。

Method: 提出SlotVTG框架，通过轻量级slot适配器引导MLLMs进行对象中心、基于输入的视觉推理。该适配器通过slot attention将视觉token分解为抽象slot并重建原始序列，利用自监督视觉模型的对象性先验促进语义连贯的slot形成，以最小成本实现细粒度理解。

Result: 在标准VTG基准上的跨域评估表明，SlotVTG显著提升了OOD鲁棒性，同时保持了有竞争力的ID性能，且计算开销最小。

Conclusion: SlotVTG通过对象中心表示和轻量级适配器设计，有效解决了MLLMs在视频时序定位中的细粒度理解不足和跨域泛化差的问题，为提升视觉语言模型的时空推理能力提供了高效解决方案。
...
### [156] [Unleashing Guidance Without Classifiers for Human-Object Interaction Animation](https://arxiv.org/abs/2603.25734)
*Ziyin Wang,Sirui Xu,Chuan Guo,Bing Zhou,Jiangshan Gong,Jian Wang,Yu-Xiong Wang,Liang-Yan Gui*

Main category: cs.CV

TL;DR: LIGHT提出了一种基于数据驱动的引导方法，通过异步去噪调度实现模态特定组件的个性化噪声水平，无需手动设计接触先验即可生成高质量的人-物交互动画。


<details>
  <summary>Details</summary>
Motivation: 生成真实的人-物交互动画具有挑战性，需要联合建模动态人体动作和多样物体几何。现有基于扩散的方法通常依赖手工设计的接触先验或人为施加的运动学约束来改善接触质量，这限制了方法的通用性和灵活性。

Method: 基于扩散强迫技术，将表示分解为模态特定组件，并为每个组件分配个性化噪声水平和异步去噪调度。在这种范式下，较干净的组件通过交叉注意力引导较嘈杂的组件，无需辅助分类器即可实现引导。通过使用广泛的合成物体几何数据进行训练增强，鼓励接触语义对几何多样性的不变性。

Result: 实验表明，节奏诱导的引导比传统的无分类器引导更能有效模拟接触先验的益处，同时实现更高的接触保真度、更真实的HOI生成，以及对未见物体和任务的更强泛化能力。

Conclusion: LIGHT提供了一种数据驱动的替代方案，其中引导从去噪节奏本身产生，减少了对手动设计先验的依赖。该方法自然地具有接触感知能力，并能通过几何多样性增强训练进一步提高性能。
...
### [157] [PSDesigner: Automated Graphic Design with a Human-Like Creative Workflow](https://arxiv.org/abs/2603.25738)
*Xincheng Shuai,Song Tang,Yutong Huang,Henghui Ding,Dacheng Tao*

Main category: cs.CV

TL;DR: PSDesigner是一个模仿人类设计师工作流程的自动化图形设计系统，通过专业工具调用和CreativePSD数据集训练，实现从用户意图到可编辑设计文件的转换
...
Method: 构建PSDesigner系统，模仿人类设计师工作流程，基于多个专用组件收集主题相关资源，自主推断和执行工具调用来操作设计文件；创建CreativePSD数据集，包含大量高质量PSD文件及操作轨迹标注

Result: PSDesigner在多样化图形设计任务中优于现有方法，使非专业人士能够方便地创建生产质量的设计

Conclusion: PSDesigner通过模仿人类设计师工作流程和构建专业数据集，成功解决了自动化图形设计中的灵活性和直观性问题
...
### [158] [MegaFlow: Zero-Shot Large Displacement Optical Flow](https://arxiv.org/abs/2603.25739)
...
<details>
  <summary>Details</summary>
Motivation: 现有光流估计方法依赖迭代局部搜索和领域特定微调，在大位移和零样本泛化场景下性能受限，需要一种能处理大位移且具有零样本泛化能力的新方法

Method: 将光流估计构建为全局匹配问题，利用预训练的全局Vision Transformer特征捕获大位移，然后通过少量轻量级迭代细化提高亚像素精度

Result: 在多个光流基准测试中实现了最先进的零样本性能，在长程点跟踪基准上也表现出高度竞争力的零样本性能，展示了强大的可迁移性

Conclusion: MegaFlow通过利用预训练视觉先验实现了零样本大位移光流估计，为通用化运动估计提供了统一范式，无需复杂任务特定架构设计
...
### [159] [RefAlign: Representation Alignment for Reference-to-Video Generation](https://arxiv.org/abs/2603.25743)
*Lei Wang,YuXin Song,Ge Wu,Haocheng Feng,Hang Zhou,Jingdong Wang,Yaxing Wang,jian Yang*

Main category: cs.CV

TL;DR: RefAlign：一种用于参考到视频生成任务的表示对齐框架，通过显式对齐扩散Transformer的参考分支特征到视觉基础模型的语义空间，提升身份一致性和语义区分性


<details>
  <summary>Details</summary>
Motivation: 现有R2V方法虽然引入了高级语义或跨模态特征来缓解VAE潜在空间中的像素级信息泄漏，但仍难以解决异构编码器特征间的模态不匹配导致的复制粘贴伪影和多主体混淆问题

Method: 提出RefAlign框架，核心是参考对齐损失：将同一主体的参考特征和VFM特征拉近以提升身份一致性，同时将不同主体的对应特征推远以增强语义区分性；仅在训练时应用，不增加推理开销

Result: 在OpenS2V-Eval基准测试中，RefAlign在TotalScore上超越了当前最先进方法，验证了显式参考对齐对R2V任务的有效性

Conclusion: RefAlign通过显式对齐扩散Transformer参考分支特征到视觉基础模型的语义空间，在文本可控性和参考保真度之间实现了更好的平衡，为R2V生成提供了有效的解决方案
...
### [160] [MuRF: Unlocking the Multi-Scale Potential of Vision Foundation Models](https://arxiv.org/abs/2603.25744)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型在推理时通常局限于单一固定尺度，忽略了不同分辨率提供的互补归纳偏置：低分辨率擅长全局语义识别，高分辨率适合细粒度细化

Method: 提出Multi-Resolution Fusion (MuRF)策略，通过冻结的视觉基础模型处理图像的多分辨率视图，然后将生成的特征融合成统一表示

Result: MuRF在多种关键计算机视觉任务上验证有效，适用于DINOv2等多种视觉基础模型家族，并能成功泛化到对比学习模型如SigLIP2

Conclusion: MuRF是一种通用、无需训练的方法，能够利用多分辨率互补优势提升视觉表示，不依赖于特定架构，可作为视觉基础模型的基本增强策略
...
### [161] [Less Gaussians, Texture More: 4K Feed-Forward Textured Splatting](https://arxiv.org/abs/2603.25745)
...
<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯泼溅方法预测像素对齐的基元，导致基元数量随分辨率呈二次增长，从根本上限制了其可扩展性，使得4K等高分辨率合成变得不可行。

Method: LGTM框架预测紧凑的高斯基元，并为每个基元配备纹理，从而将几何复杂度与渲染分辨率解耦。这种方法避免了像素对齐基元导致的二次增长问题。

Result: LGTM实现了高保真度的4K新视角合成，无需逐场景优化，这是前馈方法之前无法达到的能力，同时使用的基元数量显著减少。

Conclusion: LGTM通过解耦几何复杂度和渲染分辨率，克服了前馈式3D高斯泼溅方法的分辨率缩放障碍，为高分辨率新视角合成提供了可行的解决方案。
...
### [162] [ShotStream: Streaming Multi-Shot Video Generation for Interactive Storytelling](https://arxiv.org/abs/2603.25746)
*Yawen Luo,Xiaoyu Shi,Junhao Zhuang,Yutian Chen,Quande Liu,Xintao Wang,Pengfei Wan,Tianfan Xue*

Main category: cs.CV

TL;DR: ShotStream：一种用于交互式叙事的新型因果多镜头视频生成架构，通过双向到因果的蒸馏实现实时帧生成，解决了现有方法延迟高、交互性有限的问题。


<details>
  <summary>Details</summary>
Motivation: 当前双向架构的多镜头视频生成存在交互性有限和高延迟问题，无法满足实时交互式叙事的需求。需要一种能够支持用户动态指导叙事、实现高效实时帧生成的解决方案。

Method: 1) 将文本到视频模型微调为双向下一镜头生成器；2) 通过分布匹配蒸馏将其蒸馏为因果学生模型；3) 引入双缓存记忆机制：全局上下文缓存保持镜头间一致性，局部上下文缓存保持镜头内一致性；4) 使用RoPE不连续性指示器区分两个缓存；5) 采用两阶段蒸馏策略：从基于真实历史镜头的镜头内自强制开始，逐步扩展到使用自生成历史的镜头间自强制。

Result: ShotStream在单GPU上实现16 FPS的次秒级延迟，生成连贯的多镜头视频。在质量上匹配或超越较慢的双向模型，为实时交互式叙事铺平了道路。

Conclusion: ShotStream通过因果多镜头架构成功解决了交互式叙事中的延迟和一致性挑战，实现了高效的实时视频生成，为交互式长叙事创作提供了可行的技术方案。
...
### [163] [SolRugDetector: Investigating Rug Pulls on Solana](https://arxiv.org/abs/2603.24625)
*Jiaxin Chen,Ziwei Li,Zigui Jiang,Ruihong He,Yantong Zhou,Jiajing Wu,Zibin Zheng*

Main category: cs.CR

TL;DR: 该论文对Solana上的Rug Pulls进行了首次系统性实证研究，构建了标注数据集，开发了检测系统SolRugDetector，并在大规模测量中识别了大量欺诈代币，揭示了Solana上Rug Pulls的独特特征。


<details>
  <summary>Details</summary>
Motivation: Solana因其高性能和低交易成本而快速增长，但极低的代币发行门槛导致了广泛的Rug Pulls。与依赖恶意智能合约的以太坊Rug Pulls不同，Solana上的统一SPL Token程序将欺诈行为转向链上操作（如市场操纵）。现有研究尚未对Solana上这些特定的Rug Pull模式进行系统分析。

Method: 基于68个真实事件报告，构建并发布了包含117个确认Rug Pull代币的手动标注数据集，并描述了Solana上Rug Pulls的工作流程。在此基础上提出了SolRugDetector检测系统，该系统仅使用链上交易和状态数据识别欺诈代币。进一步对2025年上半年新发行的100,063个代币进行了大规模测量。

Result: 实验结果显示SolRugDetector在标注数据集上优于现有工具。大规模测量识别出76,469个Rug Pull代币。分析发现Solana上的Rug Pulls具有极短的生命周期、强烈的价格驱动动态、严重的经济损失和高度组织化的群体行为。

Conclusion: 该研究首次系统分析了Solana上的Rug Pulls现象，提供了对Solana Rug Pull生态的深入见解，并支持开发有效的链上防御机制。发布的标注数据集和检测系统为后续研究和实践提供了重要基础。
...
### [164] [Bridging Code Property Graphs and Language Models for Program Analysis](https://arxiv.org/abs/2603.24837)
*Ahmed Lekssays*

Main category: cs.CR

TL;DR: CodeBadger是一个开源MCP服务器，将Joern的代码属性图引擎与LLM集成，解决LLM分析大型代码库安全漏洞时的token限制、跨过程数据流分析和复杂查询生成问题。


<details>
  <summary>Details</summary>
Motivation: LLM在分析真实世界代码库安全漏洞时面临三个关键挑战：token限制无法加载整个仓库、代码嵌入无法捕捉跨过程数据流、LLM难以生成复杂的静态分析查询。现有方法只能处理孤立代码片段，无法发现跨多个函数和文件的漏洞。
...
Conclusion: CodeBadger使LLM能够在整个仓库范围内进行语义代码推理，支持大规模漏洞发现、补丁生成和程序理解，解决了现有LLM在代码安全分析中的关键限制。
...
### [165] [AI Security in the Foundation Model Era: A Comprehensive Survey from a Unified Perspective](https://arxiv.org/abs/2603.24857)
*Zhenyi Wang,Siyu Luan*

Main category: cs.CR

TL;DR: 提出统一闭环威胁分类法，将机器学习安全威胁按数据与模型之间的四类交互方向进行分类，为系统化分析和防御提供框架。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习安全研究将威胁孤立看待，缺乏揭示其共享原理和相互依赖性的统一框架。这种碎片化视角阻碍了系统性理解，限制了全面防御设计。数据与模型这两个ML基础资产不再独立，一方的漏洞会直接危及另一方，需要整体性框架来分析双向风险在ML管道中的传播。

Method: 提出统一闭环威胁分类法，将模型-数据交互沿四个方向轴进行显式框架化：数据→数据、数据→模型、模型→数据、模型→模型。该框架为分析和防御基础模型提供了原则性视角。

Result: 建立了四类安全威胁分类：(1)数据→数据：包括数据解密攻击和水印移除攻击；(2)数据→模型：包括投毒攻击、有害微调攻击和越狱攻击；(3)模型→数据：包括模型反演、成员推理攻击和训练数据提取攻击；(4)模型→模型：包括模型提取攻击。

Conclusion: 统一框架阐明了这些安全威胁之间的底层联系，为开发可扩展、可迁移和跨模态的安全策略奠定了基础，特别是在基础模型领域。
...
### [166] [Trusted-Execution Environment (TEE) for Solving the Replication Crisis in Academia](https://arxiv.org/abs/2603.24878)
*Jiasun Li,Project Team*

Main category: cs.CR

TL;DR: 提出基于可信执行环境(TEE)的新框架解决学术研究可重复性危机，通过云TEE执行复制包并提交密码学证明，期刊可验证而不需重新运行代码。


<details>
  <summary>Details</summary>
Motivation: 经济学、金融学、社会科学和计算机科学等领域日益严重的可重复性危机损害了学术研究的可信度。现有解决方案（如工件评估和复制包）存在关键限制：合格数据编辑短缺、处理专有数据集困难、流程低效、依赖志愿劳动。

Method: 提出利用可信执行环境(TEE)技术（如Intel TDX）的新框架。作者在基于云的TEE中执行复制包并提交正确执行的密码学证明，期刊或会议可高效验证而无需重新运行代码。通过实地实验验证可行性，在TDX支持的云VM上复制已发表论文。

Result: 试点研究发现每个复制包平均成本1.35-1.80美元，相对于标准VM计算开销最小，即使没有TEE经验的新手用户成功率也很高。形式分析表明TEE采用对作者具有激励相容性，对期刊具有成本优势，构成认证市场的均衡。
...
### [167] [LiteGuard: Efficient Task-Agnostic Model Fingerprinting with Enhanced Generalization](https://arxiv.org/abs/2603.24982)
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进方法MetaV需要大量不同模型进行训练，这在现实场景中难以实现且计算成本高昂。减少模型数量会导致过拟合问题，而MetaV的纠缠设计进一步加剧了这一泛化问题

Method: 提出LiteGuard框架，包含两个关键创新：1) 检查点模型集增强策略，利用训练过程中的中间模型快照来丰富模型多样性；2) 本地验证器架构，为每个指纹配备轻量级本地验证器，减少参数纠缠和过拟合

Result: 在五个代表性任务上的广泛实验表明，LiteGuard在泛化性能和计算效率方面均优于MetaV

Conclusion: LiteGuard通过创新的模型增强和验证器设计，解决了任务无关模型指纹框架在实际部署中的计算成本和泛化问题，为模型保护提供了更实用的解决方案
...
### [168] [IrisFP: Adversarial-Example-based Model Fingerprinting with Enhanced Uniqueness and Robustness](https://arxiv.org/abs/2603.24996)
...
Method: 1) 将指纹定位在所有决策边界的交点附近，而非单一边界；2) 构建复合样本指纹，每个指纹包含多个靠近多边界交点的样本；3) 使用基于两个参考模型集（盗版模型和独立训练模型）的统计可分性指标评估指纹判别力，保留高判别力指纹并分配指纹特定阈值。

Result: 实验表明IrisFP在多个基准测试中始终优于现有最先进方法，通过同时增强鲁棒性和唯一性实现了可靠的所有权验证。

Conclusion: IrisFP通过创新的多边界定位、复合样本构建和判别力评估机制，成功解决了模型指纹中唯一性和鲁棒性的平衡问题，为模型知识产权保护提供了有效解决方案。
...
### [169] [PIDP-Attack: Combining Prompt Injection with Database Poisoning Attacks on Retrieval-Augmented Generation Systems](https://arxiv.org/abs/2603.25164)
*Haozhen Wang,Haoyue Liu,Jionghao Zhu,Zhichao Wang,Yongxin Guo,Xiaoying Tang*

Main category: cs.CR

TL;DR: PIDP-Attack是一种针对RAG系统的复合攻击方法，通过结合提示注入和数据库投毒，无需预先知道用户查询即可有效操纵LLM响应


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统虽然能增强LLM的实时知识获取能力，但仍面临对抗攻击威胁。传统投毒攻击需要预先知道用户具体查询，限制了其实际应用灵活性，因此需要开发更通用、无需查询先验知识的攻击方法

Method: 提出PIDP-Attack复合攻击策略：1）在推理时向查询附加恶意字符实现提示注入；2）向检索数据库注入少量投毒段落。这种组合攻击无需预先知道用户实际查询，即可有效操纵LLM对任意查询的响应

Result: 在三个基准数据集（Natural Questions、HotpotQA、MS-MARCO）和八个LLM上的实验表明，PIDP-Attack始终优于原始PoisonedRAG方法。在开放域QA任务中，攻击成功率提高了4%到16%，同时保持高检索精度

Conclusion: 复合攻击策略对于RAG系统的对抗攻击是必要且高效的。PIDP-Attack通过结合提示注入和数据库投毒，实现了无需查询先验知识的有效攻击，揭示了RAG系统安全性的新挑战
...
### [170] [Usability of Passwordless Authentication in Wi-Fi Networks: A Comparative Study of Passkeys and Passwords in Captive Portals](https://arxiv.org/abs/2603.25290)
*Martiño Rivera-Dourado,Rubén Pérez-Jove,Alejandro Pazos,Jose Vázquez-Naya*

Main category: cs.CR

TL;DR: 该论文通过实证比较研究，分析了在Wi-Fi热点认证门户中Passkeys与密码的可用性差异，发现Passkeys在登录时倾向于被感知为更可用但差异不显著，同时认证门户的限制对用户体验有负面影响。


<details>
  <summary>Details</summary>
Motivation: Passkeys作为无密码认证机制已出现，但其在认证门户（captive portals）中的可用性尚未被探索。认证门户是Wi-Fi热点等场景中常见的认证界面，研究Passkeys在此环境下的表现对推动无密码认证的实际应用具有重要意义。

Method: 采用对照实验室实验，50名参与者，使用分块设计（split-plot design）在Android和Windows平台上进行。使用支持FIDO2CAP协议的路由器，比较Passkeys和密码在认证门户中的可用性。收集定量和定性数据进行分析。

Result: 1. Passkeys在登录时倾向于被感知为比密码更可用，但差异未达到统计显著性。2. 无论使用何种认证方法，认证门户的限制都对用户体验产生负面影响并增加错误率。3. Passkeys在两个平台上配置都相对容易，但平台特定问题带来了显著的可用性挑战。

Conclusion: 基于研究发现，提出了改进认证门户认证的设计建议：引入无用户名认证流程、改进认证门户检测机制、优化用户界面设计。这些建议旨在提升Passkeys在认证门户环境中的可用性和用户体验。
...
### [171] [On the Vulnerability of Deep Automatic Modulation Classifiers to Explainable Backdoor Threats](https://arxiv.org/abs/2603.25310)
*Younes Salmi,Hanna Bogucka*

Main category: cs.CR

TL;DR: 本文研究针对无线通信中基于深度学习的自动调制分类系统的物理后门攻击，利用可解释AI指导触发器在信号最脆弱部分的放置，实现高效隐蔽的攻击。


<details>
  <summary>Details</summary>
Motivation: 深度学习在无线通信应用中广泛使用，特别是自动调制分类，但这些模型容易受到对抗性机器学习威胁。现有研究主要关注计算机视觉领域的威胁，而无线通信领域的物理后门攻击研究不足，需要探索针对无线信号传输前的物理后门攻击方法。

Method: 提出一种针对无线信号传输前的物理后门攻击方法：1）使用可解释AI识别信号中最脆弱的部分；2）利用类原型结合主成分分析生成触发器；3）在信号传输前植入触发器，实现对多个基于深度学习的自动调制分类模型的攻击。

Result: 该攻击方法在多种信噪比条件下都能实现高成功率，即使在很小的投毒比例下也能有效突破多个基于深度学习的自动调制分类模型，证明了物理后门攻击在无线通信领域的有效性和隐蔽性。

Conclusion: 无线通信中的深度学习模型面临严重的物理后门攻击威胁，利用可解释AI指导的触发器放置策略能够实现高效隐蔽的攻击，这凸显了在无线通信系统中加强对抗性防御机制的重要性。
...
### [172] [Multi-target Coverage-based Greybox Fuzzing](https://arxiv.org/abs/2603.25354)
*Masami Ichikawa*

Main category: cs.CR

TL;DR: 提出MTCFuzz：一种针对操作系统与固件协同架构的多目标覆盖率导向灰盒模糊测试方法，通过QEMU虚拟化环境统一监控跨软件边界的代码覆盖率，实现比传统单目标模糊测试更深入的系统探索。


<details>
  <summary>Details</summary>
Motivation: 现代系统软件架构通常由固件和操作系统通过定义良好的接口协同工作（如RISC-V的OpenSBI和ARM的OP-TEE），而传统模糊测试技术主要针对单一软件组件，忽视了软件间的协同执行，难以有效发现跨组件漏洞。

Method: 采用QEMU作为虚拟化环境，在虚拟机内启动系统执行模糊测试，统一监控操作系统和固件的执行过程。利用各协同软件组件的代码覆盖率作为反馈，实现跨软件边界的覆盖率测量。将方法实现为多目标覆盖率导向灰盒模糊测试器MTCFuzz。

Result: 实现了MTCFuzz原型系统，通过实验评估验证了其有效性。相比传统单目标模糊测试，该方法能够实现更深入的系统探索，发现更多跨组件交互相关的潜在漏洞。

Conclusion: 针对操作系统与固件协同架构，提出了一种有效的多目标模糊测试方法，通过统一监控跨软件边界的代码覆盖率，显著提升了系统软件模糊测试的深度和效果，为复杂系统软件的安全测试提供了新思路。
...
### [173] [ALPS: Automated Least-Privilege Enforcement for Securing Serverless Functions](https://arxiv.org/abs/2603.25393)
*Changhee Shin,Bom Kim,Seungsoo Lee*

Main category: cs.CR

TL;DR: ALPS是一个自动化框架，通过静态分析和LLM生成安全策略，在无服务器环境中实施最小权限原则，支持多云供应商和多种编程语言。
...
Method: ALPS采用针对无服务器环境的静态分析从函数代码中提取精确权限需求，使用微调的大型语言模型生成特定语言和供应商的安全策略，并进行实时监控以阻止未授权访问并适应策略或代码变化。
...
Conclusion: ALPS为无服务器工作负载提供了一个有效、实用且供应商无关的安全解决方案，能够显著改善权限管理并降低安全风险。
...
### [174] [Shape and Substance: Dual-Layer Side-Channel Attacks on Local Vision-Language Models](https://arxiv.org/abs/2603.25403)
*Eyal Hadad,Mordechai Guri*

Main category: cs.CR

TL;DR: 动态高分辨率预处理的视觉语言模型存在算法侧信道漏洞，攻击者可通过执行时间差异和末级缓存争用推断输入图像的几何形状和语义内容，威胁本地部署的数据隐私。


<details>
  <summary>Details</summary>
Motivation: 尽管本地执行的视觉语言模型（VLMs）承诺数据隐私，但动态高分辨率预处理（如AnyRes）引入的算法侧信道可能泄露敏感信息。作者发现动态预处理根据图像宽高比生成可变数量的图像块，导致工作负载依赖的输入，这可能被攻击者利用来推断隐私敏感内容。

Method: 提出双层攻击框架：第一层，无特权攻击者利用标准操作系统指标分析执行时间变化，可靠地指纹输入图像的几何形状；第二层，通过分析末级缓存（LLC）争用，攻击者可以解析相同几何形状内的语义模糊性，区分视觉密集内容（如医学X光片）和稀疏内容（如文本文档）。评估了LLaVA-NeXT和Qwen2-VL等最先进模型。

Result: 实验表明结合执行时间和缓存争用信号能够可靠地推断隐私敏感上下文。分析安全工程权衡发现，使用恒定工作填充的缓解方法会产生显著的性能开销。

Conclusion: 动态高分辨率预处理在视觉语言模型中引入了固有的算法侧信道漏洞，威胁本地部署的数据隐私。作者揭示了安全工程权衡，并提出了边缘AI部署的实用设计建议。
...
### [175] [TAAC: A gate into Trustable Audio Affective Computing](https://arxiv.org/abs/2603.25570)
*Xintao Hu,Feng-Qi Cui*

Main category: cs.CR

TL;DR: TAAC框架通过对抗损失子空间分解实现可信音频情感计算，在保护用户身份隐私的同时进行抑郁症检测


<details>
  <summary>Details</summary>
Motivation: 音频抑郁症诊断面临用户敏感身份信息泄露风险，现有方法难以有效分离抑郁特征与敏感特征，需要安全加密方法和强大分类器

Method: 提出TAAC框架，包含基于对抗损失子空间分解的DFSD、灵活噪声加密器FNE和分阶段训练范式，分别用于特征分解、身份加密和性能增强

Result: 实验表明TAAC在抑郁症检测、身份保留和音频重建方面优于现有加密方法，在不同加密强度下保持稳定性能
...
### [176] [Fair regression under localized demographic parity constraints](https://arxiv.org/abs/2603.25224)
*Arthur Charpentier,Christophe Denis,Romuald Elie,Mohamed Hebiri,François HU*

Main category: stat.ML

TL;DR: 该论文提出了一种针对回归任务的DP公平性放松方法，通过在有限的分位数和阈值上实施公平性约束，而非要求整个分布完全不变，从而在公平性和准确性之间取得更好的平衡。


<details>
  <summary>Details</summary>
Motivation: 在回归任务中，完全分布性的DP公平性要求过于严格，常导致显著的准确性损失。需要一种更灵活的公平性准则，能够在特定决策相关的分位数或阈值上实施公平性约束，同时保持预测性能。

Method: 提出(ℓ,Z)-公平预测器，在有限的分位数水平或得分阈值上施加群体累积分布函数约束。通过拉格朗日对偶公式推导最优公平离散预测器的闭式解，并开发基于两个样本（标记样本用于学习基础回归器，未标记样本用于校准）的模型无关后处理算法。

Result: 理论分析表明，随着网格细化，离散化成本趋近于零，风险差距收敛到连续最优解。实验在合成和真实数据集上展示了可解释的公平性-准确性权衡，能够在决策相关的分位数或阈值上进行有针对性的修正，同时保持预测性能。

Conclusion: 该研究为回归任务提供了一种灵活且实用的公平性框架，通过在特定分位数或阈值上实施公平性约束，实现了比完全分布性DP更优的公平性-准确性平衡，为实际应用提供了可解释的公平性调整机制。
...
### [177] [Practical Efficient Global Optimization is No-regret](https://arxiv.org/abs/2603.25311)
*Jingyi Wang,Haowei Wang,Nai-Yuan Chiang,Juliane Mueller,Tucker Hartland,Cosmin G. Petra*

Main category: stat.ML

TL;DR: 首次为带nugget的实用EGO算法建立了累积遗憾上界，证明其对常用核函数具有次线性遗憾界，属于无遗憾算法


<details>
  <summary>Details</summary>
Motivation: 尽管带nugget的实用EGO算法在实践中被广泛采用且经验上成功，但其累积遗憾界一直未被建立，需要填补这一理论空白

Method: 分析带正nugget的实用EGO算法，针对平方指数核和Matérn核（ν>1/2）建立累积遗憾上界，并分析nugget对遗憾界的影响

Result: 证明了实用EGO具有次线性累积遗憾界，因此对常用核函数是无遗憾算法；分析了nugget对遗憾界的影响并讨论了其选择的理论意义

Conclusion: 首次为带nugget的实用EGO建立了理论保证，证明其无遗憾性，为实践中nugget的选择提供了理论指导
...
### [178] [Adaptive Subspace Modeling With Functional Tucker Decomposition](https://arxiv.org/abs/2603.25530)
*Noah Steidle,Joppe De Jonghe,Mariya Ishteva*

Main category: stat.ML

TL;DR: 提出功能性Tucker分解(FTD)，将模式连续性约束嵌入分解中，使用再生核希尔伯特空间(RKHS)建模连续模式，无需先验基函数，保持Tucker模型的多线性子空间结构，应用于领域变体张量分类任务。


<details>
  <summary>Details</summary>
Motivation: 张量为多维数据提供结构化表示，但当数据源自连续过程时，离散化会掩盖重要信息。现有方法未能有效处理连续模式下的张量分解问题。

Method: 提出功能性Tucker分解(FTD)，利用再生核希尔伯特空间(RKHS)建模连续模式，无需先验基函数选择。该方法将模式连续性约束直接嵌入分解过程，同时保持Tucker模型的多线性子空间结构，通过RKHS驱动表示获得自适应且富有表达力的因子描述。

Result: 在领域变体张量分类任务中验证了方法的有效性，特别是在高光谱成像和多变量时间序列分析的分类任务中表现出色，展示了结构分解与功能适应性结合的优势。

Conclusion: 功能性Tucker分解通过嵌入模式连续性约束和利用RKHS建模连续模式，有效解决了连续过程数据离散化带来的信息损失问题，为张量分析提供了更强大的功能适应性。
...
### [179] [Kendall Correlation Coefficient for non-Identically Distributed Variables](https://arxiv.org/abs/2603.25055)
*Alexei Stepanov*

Main category: math.ST

TL;DR: 首次提出并分析了非独立同分布双变量数据的理论Kendall相关系数τ_n，证明了秩Kendall相关系数τ̃_n的期望等于τ_n，且τ̃_n依概率收敛于τ=lim τ_n，验证了τ_n作为非独立同分布情况下的理论Kendall相关系数的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有Kendall相关系数理论主要针对独立同分布数据，缺乏对非独立同分布双变量数据的理论框架。本文旨在填补这一空白，为更广泛的数据类型提供理论支持。

Method: 首先定义非独立同分布情况下的理论Kendall相关系数τ_n，然后证明秩Kendall相关系数τ̃_n的期望等于τ_n，并证明τ̃_n依概率收敛于极限τ。最后通过模拟实验验证理论结果。

Result: 成功建立了非独立同分布双变量数据的理论Kendall相关系数框架，证明了τ_n是τ̃_n的无偏估计量，且τ̃_n具有一致性收敛性质。模拟实验进一步支持了理论推导的正确性。

Conclusion: τ_n是适用于非独立同分布双变量数据的正确理论Kendall相关系数定义，为处理更复杂的数据依赖结构提供了理论基础，扩展了Kendall相关系数的应用范围。
...
### [180] [Quantitative analysis of non-exchangeability in bivariate copulas: Sharp bounds, statistical tests and mixing constructions](https://arxiv.org/abs/2603.25348)
*Manuel Úbeda-Flores*

Main category: math.ST

TL;DR: 论文研究了二元随机向量的非交换性度量，推导了非交换性度量的尖锐上界，证明了精确的缩放恒等式，并提出了一种非参数置换检验方法。


<details>
  <summary>Details</summary>
Motivation: 基于Durante等人(2010)的公理化框架，深入研究二元随机向量非交换性的度量问题，旨在建立更精确的理论界限、揭示非交换性度量的数学性质，并开发有效的统计检验方法。

Method: 1. 推导非交换性度量μ_p(C)的尖锐上界，用Schweizer-Wolff依赖度量和Spearman's ρ表示；2. 证明精确缩放恒等式μ_p(αC+(1-α)C^t)=|2α-1|μ_p(C)；3. 提出非参数置换检验方法检验H_0:C=C^t，证明其一致性，并通过蒙特卡洛模拟验证有限样本性能。

Result: 1. 建立了非交换性度量的尖锐上界；2. 揭示了非交换性度量的精确缩放性质；3. 开发了有效的非参数置换检验方法，证明了其理论一致性并通过模拟验证了实际性能。

Conclusion: 该研究深化了对二元随机向量非交换性的理解，提供了理论界限、数学性质和统计检验的完整框架，为非交换性分析提供了有效的工具和方法。
...
### [181] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: 随机裁剪作为视觉数据增强技术，在训练差分隐私模型时能通过概率性排除敏感内容（如人脸、车牌）来增强隐私保护，无需改变模型架构或训练过程。


<details>
  <summary>Details</summary>
Motivation: 随机裁剪是计算机视觉中最常见的数据增强技术之一，但其在训练差分隐私机器学习模型中的固有随机性作用尚未被充分探索。当图像中的敏感内容（如人脸、车牌）在空间上局部化时，随机裁剪可以概率性地将这些内容排除在模型输入之外，这为差分隐私训练引入了额外的随机性来源。

Method: 引入视觉数据的补丁级相邻关系，推导随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的严格隐私边界。分析量化补丁包含概率，展示其如何与小批量采样组合以产生更低的有效采样率。

Result: 补丁级放大改善了多个分割架构和数据集上的隐私-效用权衡。实证验证表明，将隐私计算与领域结构和现有随机性来源对齐可以在不增加成本的情况下提供更强的隐私保证。

Conclusion: 随机裁剪的固有随机性可以放大差分隐私保护，这为视觉领域的隐私保护训练提供了一种无需额外成本的方法。通过形式化补丁级相邻关系并推导严格的隐私边界，展示了如何利用现有数据增强技术来增强隐私保证。
...
### [182] [DyMRL: Dynamic Multispace Representation Learning for Multimodal Event Forecasting in Knowledge Graph](https://arxiv.org/abs/2603.24636)
...
Method: DyMRL提出动态多空间表示学习方法：1) 将欧几里得、双曲和复数空间的时间特定结构特征集成到关系消息传递框架中，学习深度表示，反映人类在联想思维、高阶抽象和逻辑推理方面的智能；2) 采用先进的双重融合-演化注意力机制，以对称方式为不同时间戳的不同模态分配动态学习权重。

Result: 构建了四个多模态时序知识图谱基准数据集进行实验评估。大量实验表明，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法。

Conclusion: DyMRL通过动态获取和融合多模态时序知识，有效解决了事件预测中的关键挑战，在多个基准数据集上表现出优越性能，为多模态时序知识表示学习提供了新思路。
...
### [183] [Experiential Reflective Learning for Self-Improving LLM Agents](https://arxiv.org/abs/2603.24639)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型智能体在专业环境中适应性差，无法利用历史经验，每次任务都从零开始，缺乏从过往交互中学习的能力
...
Conclusion: 通过单次尝试经验反思提取可迁移启发式规则，能够实现有效的智能体自我改进，为LLM智能体在专业环境中的适应性提供了新方法
...
### [184] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: 提出一个解决多步骤工具编排挑战的强化学习框架，通过真实API响应缓存构建环境，并使用分级奖励设计来提升LLM在复杂函数调用序列中的执行准确性。
...
Method: 1) 构建基于大规模真实API响应缓存的强化学习环境，支持数据合成管道，可采样具有可控复杂度的有效多步骤编排轨迹，生成效率显著高于无约束方法；2) 提出分级奖励设计，将正确性分解为原子有效性（单个函数调用在递增粒度上的正确性）和编排（尊重依赖关系的正确工具排序）。

Result: 在ComplexFuncBench上，该方法在轮次准确性方面表现出显著改进。消融研究证实两个奖励组件都是必需的：单独使用任一组件都会显著降低性能。

Conclusion: 该框架通过真实API环境和高粒度奖励设计，有效解决了多步骤工具编排的训练挑战，显著提升了LLM在复杂工作流中的执行准确性。
...
### [185] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: 提出了一种用于模拟电路尺寸优化的演员-评论家框架（ACOF），将设计指导融入优化循环，显著提升性能指标并减少遗憾


<details>
  <summary>Details</summary>
Motivation: 模拟设计优化通常效率低下，因为即使对器件尺寸或偏置进行微小更改也需要昂贵的仿真周期，高质量解决方案通常只占据巨大搜索空间的狭窄部分。现有优化器虽然减轻了部分负担，但缺乏设计师在决定下一步搜索位置时所使用的判断能力。

Method: 提出了演员-评论家优化框架（ACOF），将优化过程分为提案和评估两个角色：演员负责建议设计空间中有前景的区域，评论家则审查这些选择、强制执行设计合法性，并在进展受阻时重定向搜索。该框架保持了与标准基于仿真流程的兼容性，同时使搜索过程更加审慎、稳定和可解释。

Result: 在测试电路中，ACOF将前10个性能指标平均提高了38.9%，比最强竞争基线平均减少了24.7%的遗憾。在个别电路上，性能指标峰值增益达到70.5%，遗憾降低42.2%。

Conclusion: 通过将迭代推理与仿真驱动搜索相结合，该框架为跨挑战性设计空间的自动化模拟尺寸优化提供了更透明的路径，使搜索过程更加审慎、稳定和可解释。
...
### [186] [Light Cones For Vision: Simple Causal Priors For Visual Hierarchy](https://arxiv.org/abs/2603.24753)
*Manglam Kartik,Neel Tushar Shah*

Main category: cs.LG

TL;DR: Worldline Slot Attention模型通过洛伦兹几何将对象建模为时空世界线，解决了传统视觉模型无法捕捉层次结构的问题，在三个数据集上实现了6倍性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型将对象视为欧几里得空间中的独立点，无法捕捉部分与整体之间的层次结构关系，这限制了模型对复杂场景的理解能力。

Method: 提出Worldline Slot Attention模型，将对象建模为通过时空的持久轨迹（世界线），每个对象在不同层次级别有多个槽位，共享相同的空间位置但具有不同的时间坐标。特别强调几何结构的重要性：欧几里得世界线效果差，而洛伦兹几何世界线能有效编码不对称因果关系。

Result: 欧几里得世界线准确率仅为0.078（低于随机机会0.33），而洛伦兹世界线在三个数据集上达到0.479-0.661的准确率，实现了6倍的性能提升，这一结果在20多个独立运行中得到复现。洛伦兹几何也优于双曲嵌入，表明视觉层次需要因果结构而非树结构。

Conclusion: 层次对象发现需要编码不对称因果关系的几何结构，这是欧几里得空间缺乏但洛伦兹光锥自然具备的归纳偏置。该模型仅用11K参数就实现了这一目标，为视觉层次理解提供了新的几何视角。
...
### [187] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: 本文探讨了Transformer架构能否近似搜索算法，通过引入"未知树搜索与老虎机反馈"框架，证明了Transformer能够实现不同的搜索策略并泛化到未见条件。


<details>
  <summary>Details</summary>
Motivation: 虽然外部搜索算法能增强LLMs的问题解决能力，但需要外部组件会使整体过程复杂化。因此研究LLMs或其底层Transformer架构能否近似搜索算法，以简化问题解决流程。

Method: 引入"未知树搜索与老虎机反馈"简化框架，其中树扩展和反馈信号由外部指定。在此设置下，从零开始训练Transformer模型来近似不同的搜索策略。

Result: Transformer在理论上具有足够的表达能力来实现不同的搜索策略，能够从零训练来近似这些策略，并展现出对更长视野或更深树等未见条件的泛化能力。通过任务导向的持续训练，预训练LLM能够完全掌握搜索能力。

Conclusion: Transformer架构能够近似搜索算法，这为简化LLMs的问题解决流程提供了可能，通过适当的训练可以使LLMs具备内在的搜索能力。
...
### [188] [A Practical Guide Towards Interpreting Time-Series Deep Clinical Predictive Models: A Reproducibility Study](https://arxiv.org/abs/2603.24828)
...
<details>
  <summary>Details</summary>
Motivation: 临床决策具有高风险性，需要明确的解释依据，因此在部署深度临床模型前，模型可解释性至关重要。随着模型架构和可解释性方法的生态系统不断扩展，关键问题仍然存在：注意力等架构特征是否能改善可解释性？可解释性方法是否能跨临床任务泛化？现有基准研究缺乏可扩展性和可重复性，且未能系统性地考察可解释性在临床任务与模型架构交互中的变化。

Method: 提出了一个综合基准，评估不同临床预测任务和模型架构中的可解释性方法。通过PyHealth开源框架实现，支持可重复性和可扩展性。系统分析了注意力机制、KernelSHAP、LIME等黑盒解释器以及其他可解释性方法在临床时间序列预测任务中的表现。

Result: 分析发现：(1) 当正确利用时，注意力机制是忠实解释模型预测的高效方法；(2) KernelSHAP和LIME等黑盒解释器在时间序列临床预测任务中计算上不可行；(3) 多种可解释性方法可靠性不足，难以信任。基于这些发现，提出了改进临床预测流程可解释性的指南。

Conclusion: 该研究为临床预测任务中的模型可解释性提供了系统评估框架，揭示了不同可解释性方法的优缺点，特别是注意力机制的有效性和黑盒解释器的计算限制。通过开源实现促进了该领域的可重复性和可扩展性研究，并为改进临床预测流程的可解释性提供了实用指南。
...
### [189] [Flow matching on homogeneous spaces](https://arxiv.org/abs/2603.24829)
*Francesco Ruscelli*
...
<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在处理齐次空间（如李群的商空间）时面临几何复杂性挑战，需要定义和计算预度量或测地线，计算复杂且效率低

Method: 通过将齐次空间上的数据分布提升到其底层李群上，将齐次空间上的流匹配问题转化为李群上的流匹配问题，进而通过李代数映射转化为欧几里得空间上的流匹配任务

Result: 相比黎曼流匹配方法，该方法避免了预度量或测地线的定义和计算，实现了更简单、更快、完全内蕴的框架

Conclusion: 提出的框架为齐次空间上的流匹配提供了一种通用、高效且几何简洁的解决方案，显著简化了计算复杂度并提高了效率
...
### [190] [Reaching Beyond the Mode: RL for Distributional Reasoning in Language Models](https://arxiv.org/abs/2603.24844)
*Isha Puri,Mehul Damani,Idan Shenfeld,Marzyeh Ghassemi,Jacob Andreas,Yoon Kim*

Main category: cs.LG

TL;DR: 该论文提出了一种多答案强化学习方法，用于训练语言模型在推理时对多个答案进行分布推理，避免将答案分布坍缩到单一主导模式，从而更好地处理存在多个有效答案或不确定性的任务。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的许多任务（如医疗诊断、模糊问答、信息不完整场景）天然涉及多个有效答案或不可约的不确定性。然而，语言模型的后训练过程通常将答案分布坍缩到单一主导模式，这限制了模型处理多答案场景的能力。需要一种方法让语言模型能够生成多个合理假设，并为每个假设提供置信度估计，同时避免计算密集的重复采样。

Method: 提出了一种多答案强化学习方法，修改了强化学习目标，使模型能够在单次前向传递中显式生成多个候选答案。该方法将推理时搜索的某些方面内化到模型的生成过程中，从而实现对多个答案的分布推理。

Result: 在问答、医疗诊断和编程基准测试中，与单答案训练基线相比，该方法显示出改进的多样性、覆盖率和集合级校准分数。使用该方法训练的模型生成多个答案所需的token数量少于竞争方法。在编程任务中，模型的准确性也显著提高。

Conclusion: 多答案强化学习为推理时扩展程序（如best-of-k）提供了一个原则性且计算高效的替代方案。该方法使语言模型能够更好地处理多答案场景，在保持计算效率的同时提高生成答案的多样性和准确性。
...
### [191] [Learning to Staff: Offline Reinforcement Learning and Fine-Tuned LLMs for Warehouse Staffing Optimization](https://arxiv.org/abs/2603.24883)
*Kalle Kujanpää,Yuying Zhu,Kristina Klinkner,Shervin Malmasi*

Main category: cs.LG

TL;DR: 论文研究机器学习方法优化半自动化仓库分拣系统的实时人员配置决策，比较了基于Transformer的离线强化学习和基于LLM的抽象状态描述两种方法，均能提升吞吐量或匹配历史基线。


<details>
  <summary>Details</summary>
Motivation: 研究动机是优化半自动化仓库分拣系统的实时人员配置决策，支持不同抽象层次的运营决策，平衡不同权衡，探索AI辅助运营决策的可行路径。

Method: 采用两种方法：1) 基于详细历史状态表示的Transformer策略离线强化学习；2) 基于抽象、人类可读状态描述的LLM方法，系统比较提示技术、自动提示优化和微调策略，包括监督微调结合模拟器生成偏好的直接偏好优化。

Result: Transformer离线强化学习方法在学习的模拟器中实现2.4%吞吐量提升；LLM方法通过监督微调结合直接偏好优化，在手工制作的模拟器中达到匹配或略超历史基线的性能。
...
### [192] [CVA: Context-aware Video-text Alignment for Video Temporal Grounding](https://arxiv.org/abs/2603.24934)
*Sungho Moon,Seunghun Lee,Jiwan Seo,Sunghoon Im*

Main category: cs.LG

TL;DR: CVA框架通过查询感知上下文多样化、上下文不变边界判别损失和上下文增强Transformer编码器，解决视频时序定位中背景上下文干扰问题，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决视频时序定位中的关键挑战：实现时间敏感的视文对齐，同时保持对无关背景上下文的鲁棒性。传统方法在处理复杂背景和上下文干扰时存在局限性，特别是容易产生假阴性结果。

Method: 提出三个核心组件：1) 查询感知上下文多样化(QCD)：基于视频-文本相似性的数据增强策略，仅混合语义无关内容；2) 上下文不变边界判别(CBD)损失：对比损失函数，增强时间边界表示的语义一致性；3) 上下文增强Transformer编码器(CTE)：分层架构，结合窗口自注意力、双向交叉注意力和可学习查询，捕获多尺度时间上下文。

Result: 在QVHighlights和Charades-STA等主要VTG基准测试中达到最先进性能，Recall@1(R1)分数相比现有最佳方法提升约5个百分点，显著减少假阴性。

Conclusion: CVA框架通过数据中心和架构增强的协同作用，有效解决了视频时序定位中的上下文干扰问题，实现了更鲁棒的时间敏感视文对齐，显著提升了定位精度。
...
### [193] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: 该论文解决了在线凸优化中两点反馈的强凸损失高概率遗憾界问题，首次实现了O(d(log T + log(1/δ))/μ)的最优遗憾界。


<details>
  <summary>Details</summary>
Motivation: 在线凸优化中，两点反馈下的强凸损失高概率遗憾界问题一直未解决，主要挑战在于带反馈梯度估计器的重尾特性使得标准集中性分析困难。

Method: 通过处理带反馈梯度估计器的重尾特性，开发新的分析技术来克服标准集中性分析的困难，实现强凸损失下的高概率遗憾界。

Result: 首次获得了μ-强凸损失下的高概率遗憾界O(d(log T + log(1/δ))/μ)，该结果在时间范围T和维度d上都是极小极大最优的。

Conclusion: 成功解决了在线凸优化中两点反馈下强凸损失高概率遗憾界的开放问题，实现了最优的遗憾界，填补了该领域的重要空白。
...
### [194] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在稳定环境中表现优异，但在医学、金融、政策等高可靠性要求领域存在"保真度悖论"。这不仅是数据问题，更是结构性问题——在规则随时间变化的领域，额外模型容量会放大噪声而非捕捉信号。

Method: 提出"认知压缩"原则，强调通过架构设计而非事后正则化来强制简约性。开发"制度指数"区分"变化制度"（不稳定、数据贫乏）和"稳定制度"（不变、数据丰富），指导模型复杂度选择。

Result: 在15个高风险领域的探索性综合研究中，制度指数与经验上优越的建模策略在86.7%的案例中一致（13/15）。验证了认知压缩原则的有效性。

Conclusion: 高风险AI需要从为扩展而扩展转向有原则的简约性。模型结构应设计为在架构上难以表示超出数据证据的方差，从而匹配模型复杂度与数据"保质期"。
...
### [195] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
TL;DR: Intern-S1-Pro是首个万亿参数科学多模态基础模型，通过XTuner和LMDeploy基础设施支持实现高效RL训练，在通用能力和科学专业任务上均有显著提升，成为可专业化的通用智能体。


<details>
  <summary>Details</summary>
Motivation: 构建首个万亿参数级别的科学多模态基础模型，旨在实现通用智能与专业科学能力的深度融合，解决大规模模型在科学领域专业化不足的问题，同时确保训练与推理的精度一致性。

Method: 采用XTuner和LMDeploy基础设施支持，实现万亿参数级别的高效强化学习训练；模型架构整合多模态理解能力，扩展覆盖化学、材料、生命科学、地球科学等100多个专业科学任务；通过严格精度一致性机制确保训练与推理对齐。

Result: 模型在通用能力上达到开源模型顶尖水平，在科学专业任务深度上超越专有模型；实现了推理能力、图文理解能力和智能体能力的全面提升；成功掌握100多个跨学科科学任务。

Conclusion: Intern-S1-Pro作为首个万亿参数科学多模态基础模型，成功实现了通用智能与专业科学能力的融合，确立了"可专业化的通用智能体"新范式，为大规模科学AI模型的发展提供了重要参考。
...
### [196] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
*Xinyu Wang,Fei Dou,Jinbo Bi,Minghu Song*

Main category: cs.LG

TL;DR: SIGMA通过token级对比学习解决分子线性化表示中的轨迹发散问题，保持序列可扩展性的同时确保结构不变性
...
Method: 提出SIGMA框架：1) 通过token级对比目标显式对齐具有相同后缀的前缀的潜在状态，使模型严格识别几何对称性；2) 引入同构束搜索(IsoBeam)在推理时动态剪枝等效路径，消除同构冗余

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，相比强基线在多参数优化中展现出更优的样本效率和结构多样性

Conclusion: SIGMA在不放弃高效字符串表示的前提下，通过对比学习和推理优化解决了分子生成中的轨迹发散问题，实现了可扩展且结构感知的分子生成
...
### [197] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
*Mohammad A. Farmani,Hoshin V. Gupta,Ali Behrangi,Muhammad Jawad,Sadaf Moghisi,Guo-Yue Niu*

Main category: cs.LG

TL;DR: 研究通过逐步增强Mass-Conserving Perceptron（MCP）的物理过程表示，评估其对径流预测性能和可解释性的影响，发现物理约束的逐步嵌入能提升预测能力并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在水文应用中虽然预测精度高，但通常缺乏物理可解释性。MCP提供了物理感知的AI框架，能够强制执行守恒原理，同时从数据中学习水文过程关系。本研究旨在探索如何在单个MCP存储单元中逐步嵌入具有物理意义的水文过程表示，以改进降雨-径流建模的预测能力和可解释性。

Method: 从最小MCP公式开始，逐步引入有界土壤存储、状态相关导水率、可变孔隙度、入渗能力、地表积水、垂直排水和非线性水位动态等物理过程表示。在跨越美国大陆五个水文气候区域的15个流域中，使用日径流预测作为目标，评估这一系列过程感知MCP模型的层次结构。

Result: 逐步增强MCP单元内部物理结构通常能提高预测性能。这些过程表示的影响具有强烈的水文气候依赖性：垂直排水在干旱和雪主导流域显著提高模型技能，但在降雨主导区域降低性能；地表积水的影响相对较小。性能最佳的MCP配置接近LSTM基准的预测技能，同时保持明确的物理可解释性。

Conclusion: 将水文过程约束嵌入AI架构中，为实现可解释和过程感知的降雨-径流建模提供了一条有前景的途径。物理感知的MCP模型能够在保持物理可解释性的同时，达到与传统机器学习模型相当的预测性能。
...
### [198] [Gap Safe Screening Rules for Fast Training of Robust Support Vector Machines under Feature Noise](https://arxiv.org/abs/2603.25221)
*Tan-Hau Nguyen,Thu-Le Tran,Kien Trung Nguyen*

Main category: cs.LG

TL;DR: 提出R-SVM的安全样本筛选规则，通过识别不确定性集完全位于间隔超平面一侧的训练样本来降低计算复杂度，同时不影响最优解。


<details>
  <summary>Details</summary>
Motivation: 鲁棒支持向量机(R-SVM)通过最坏情况鲁棒公式处理特征噪声，但增加了计算成本。需要开发安全样本筛选规则来降低训练复杂度而不影响最优解。

Method: 基于拉格朗日对偶推导安全筛选规则（而非常用的Fenchel-Rockafellar对偶），建立理想筛选规则，并通过将GAP-based安全区域适配到鲁棒设置得到实用规则。

Result: 实验表明该方法显著减少训练时间，同时保持分类准确率。这是首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。

Conclusion: 提出的安全样本筛选规则有效解决了R-SVM计算复杂度高的问题，在保持鲁棒性的同时显著加速训练过程，为鲁棒模型的高效训练提供了新方法。
...
TL;DR: 该论文提出了一种基于离线强化学习的组合优化方法，使用Decision Transformer从启发式算法解的数据集中学习，旨在超越而非仅仅模仿这些启发式算法。


<details>
  <summary>Details</summary>
Motivation: 传统神经组合优化方法依赖在线强化学习，部署困难且未能充分利用数十年的算法知识积累。离线强化学习框架可以直接从现有启发式算法解的数据集中学习，有望超越这些算法的性能。

Method: 1) 应用离线强化学习框架Decision Transformer，直接从启发式算法解的数据集中学习；2) 集成Pointer Network处理节点选择的实例依赖、可变动作空间；3) 使用期望分位数回归对Return-to-Go进行乐观条件化处理，以应对最优值变化范围大的实例。

Result: 实验表明，该方法在旅行商问题上产生的路径质量始终优于训练所用的四种经典启发式算法，证明了离线强化学习能够解锁并超越现有领域知识中嵌入的性能。

Conclusion: 离线强化学习为组合优化问题提供了有前景的解决方案，能够从现有算法知识中学习并超越其性能，克服了在线强化学习的部署限制。
...
TL;DR: 该研究首次系统分析了非结构化剪枝对语言模型内部特征几何结构的影响，使用稀疏自编码器作为解释性探针，发现罕见特征比高频特征更能抵抗剪枝，剪枝起到隐式特征选择作用。


<details>
  <summary>Details</summary>
Motivation: 权重剪枝是压缩大语言模型的标准技术，但其对学习到的内部表征的影响仍不清楚。研究旨在系统理解非结构化剪枝如何重塑语言模型的特征几何结构。

Method: 使用稀疏自编码器作为解释性探针，在三个模型家族（Gemma 3 1B、Gemma 2 2B、Llama 3.2 1B）、两种剪枝方法（幅度剪枝和Wanda剪枝）和六个稀疏度水平（0-60%）下，研究五个研究问题：种子稳定性、特征存活、SAE可迁移性、特征脆弱性和因果相关性。

Result: 最显著发现是罕见SAE特征（低激活率）比高频特征更能抵抗剪枝，在17个实验条件中的11个条件下，Spearman相关系数达到rho=-1.0。Wanda剪枝比幅度剪枝能更好地保留特征结构（最多3.7倍）。预训练的SAE在Wanda剪枝模型上保持有效至50%稀疏度。几何特征存活不能预测因果重要性。
...
TL;DR: 该论文通过范畴论形式化深度研究智能体行为，提出了一个包含296个问题的机制感知基准，用于从四个可解释维度压力测试智能体，揭示了当前AI在复杂结构信息处理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体的评估受限于临时性经验基准，这些启发式方法无法严格建模智能体行为或充分压力测试长时程合成与歧义消解。需要建立理论框架来形式化智能体行为并设计系统化评估方法。

Method: 1. 使用范畴论形式化深度研究智能体行为，将深度研究工作流建模为结构保持映射（函子）的组合。2. 基于此理论框架，设计包含296个问题的机制感知基准，从四个可解释维度压力测试智能体：遍历顺序连接链、验证V结构拉回中的交集、对检索子结构施加拓扑排序、通过Yoneda探针执行本体证伪。

Result: 对11个领先模型的评估显示持续低基线，最先进模型平均准确率仅为19.9%，暴露了形式结构压力测试的难度。研究发现当前AI能力存在明显二分：高级深度研究流水线能成功重新定义动态拓扑排序并展现稳健的本体验证能力（在证伪幻觉前提方面与纯推理模型相当），但在多跳结构合成方面几乎普遍崩溃。不同任务间的巨大性能差异揭示了智能体仍依赖脆弱的启发式方法而非系统性理解。

Conclusion: 虽然顶级自主智能体现在能够有机统一搜索与推理，但要实现对复杂结构信息的广义掌握仍然是一个严峻的开放挑战。该工作为深度研究智能体的评估提供了理论基础和系统化基准，揭示了当前AI在结构信息处理方面的根本局限性。
...
TL;DR: 提出Hi-MLIP（Hessian-informed MLIP）框架，通过HINT高效训练协议实现Hessian监督，显著提升机器学习原子间势的曲率预测能力，用于精确分析热力学和动力学现象。


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对预测分子和材料的实验可观测性至关重要，但对于复杂系统仍难以计算。现有方法在曲率预测方面存在局限，需要开发能可靠捕捉曲率的机器学习原子间势。

Method: 提出Hi-MLIP框架，包含HINT训练协议：Hessian预训练、构型采样、课程学习和随机投影Hessian损失。该协议将昂贵Hessian标签的需求降低2-4个数量级，使Hessian监督变得实用。

Result: Hi-MLIP显著改善过渡态搜索，将Gibbs自由能预测提升至接近化学精度（尤其在数据稀缺情况下）。能准确处理强非谐氢化物，重现声子重整化和超导临界温度，与实验结果吻合良好。

Conclusion: 该工作建立了增强机器学习原子间势曲率感知能力的实用途径，为广泛系统的模拟和实验可观测性之间搭建了桥梁，解决了非谐计算的计算瓶颈问题。
...
Method: 提出GlowQ：一种用于量化LLM的组共享低秩近似方法。该方法为每个输入共享组缓存单个共享右因子，仅恢复提供最高精度收益的组或层。对每个输入共享组计算一次高精度投影并在其模块中重复使用，减少参数和内存开销。还提出选择性变体GlowQ-S，仅在提供最大收益的地方应用缓存的共享模块。

Result: 相比强基线方法，GlowQ平均减少TTFB 5.6%，增加吞吐量9.6%，在WikiText-2上降低困惑度0.17%，下游任务准确率提高0.42个百分点。选择性模型GlowQ-S进一步减少延迟，TTFB减少23.4%，吞吐量增加37.4%，同时平均保持准确率在0.2个百分点内。

Conclusion: GlowQ通过组共享低秩近似有效解决了量化LLM中的精度损失问题，同时显著减少了延迟和内存开销。选择性变体GlowQ-S在保持准确性的同时进一步优化了性能，为高效部署量化大语言模型提供了实用解决方案。
...
TL;DR: FB-MEBE算法结合无监督行为探索与正则化批评器，用于四足机器人控制的在线零样本强化学习，提升探索多样性和策略自然性


<details>
  <summary>Details</summary>
Motivation: 零样本RL算法需要从无奖励数据集中学习策略族，但预收集相关且多样化的数据集面临挑战。无导向探索产生低多样性数据，导致下游性能差且策略无法直接部署到硬件

Method: 提出FB-MEBE算法：1) 无监督行为探索策略最大化实现行为分布的熵以促进探索；2) 正则化批评器将恢复的策略塑造成更自然和物理合理的行为

Result: FB-MEBE在模拟下游任务中相比其他探索策略获得改进性能，且生成的自然策略可直接部署到硬件而无需进一步微调

Conclusion: FB-MEBE通过结合行为探索和正则化，解决了在线零样本RL中探索多样性和策略自然性的关键问题，实现了可直接硬件部署的高质量策略
...
TL;DR: 提出一种与真实决策场景对齐的森林火灾预报模型评估新方法，系统评估火灾活动预测准确性和误报率，并证明ML模型集成能改善火灾识别和减少误报


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法在预测野火发生时，虽然利用了高分辨率数据和传统过程模型忽略的预测因子，但标准评估指标对火灾危险指数预报的操作性能衡量有限，且评估常未充分考虑误报率这一关键操作因素

Method: 重新审视每日火灾危险指数模型评估范式，提出与真实世界决策对齐的森林火灾预报模型评估新方法，系统评估火灾活动预测准确性和误报率，并采用机器学习模型集成方法

Result: 机器学习模型集成能同时改善火灾识别和减少误报，提出的评估方法能更准确地反映模型在操作环境中的实际性能

Conclusion: 需要重新思考野火预测模型的评估框架，使其更贴近实际操作需求，机器学习模型集成是提升火灾危险指数预报性能的有效途径
...
<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统和基于它们训练的预测模型至关重要。现有方法通常关注数据生成过程的因果结构，而本文关注预测器隐含的依赖关系。

Method: 提出Causal-INSIGHT框架：1）在推理时对输入应用系统性的、受干预启发的输入钳制；2）从预测器的响应中构建有向时序影响信号；3）引入Qbic准则（基于稀疏性的图选择准则），在不需要真实图标签的情况下平衡预测保真度和结构复杂度。

Result: 在合成、模拟和真实基准测试中，Causal-INSIGHT能够泛化到不同的骨干架构，保持竞争力的结构准确性，并在应用于现有预测器时显著改善时序延迟定位性能。

Conclusion: Causal-INSIGHT提供了一种有效的方法来提取预测器隐含的有向时序影响结构，为理解复杂动态系统中的预测模型提供了实用的解释工具。
...
TL;DR: 该论文研究了音频到音频迁移学习，通过在不同子集的AudioSet上预训练模型并在三个下游任务上微调，发现预训练数据量、类别数以及预训练与下游任务的相似性对迁移学习效果有重要影响。


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果显著，但其内部工作机制仍有许多未解之谜，特别是关于迁移学习何时以及为何有效的问题。本研究旨在通过音频到音频迁移学习的系统性实验，深入理解迁移学习的工作机制。

Method: 在基于本体的AudioSet子集上进行预训练，然后在三个计算机听觉任务上进行微调：声学场景识别、鸟类活动识别和语音命令识别。通过控制预训练数据的样本数量和类别数量，并评估预训练与下游任务的相似性对迁移效果的影响。

Result: 增加预训练数据的样本数量和类别数量都对迁移学习有积极影响。然而，预训练与下游任务之间的相似性通常比数据量和类别数的影响更大，因为相似性可以使模型学习到可比较的特征。

Conclusion: 迁移学习的效果不仅取决于预训练数据的规模，更重要的是预训练与目标任务的相似性。相似性可以使模型学习到更相关的特征表示，从而在有限数据的下游任务上获得更好的性能。
...
TL;DR: 本研究比较了三种轻量级可解释模型（SARIMAX、Facebook Prophet、NeuralProphet）在北京PM2.5小时预测中的性能，发现在周滚动更新机制下Facebook Prophet表现最佳，而在冻结模型机制下经残差校正的SARIMAX误差最低。


<details>
  <summary>Details</summary>
Motivation: 当前空气质量预测框架通常依赖复杂、数据密集且计算需求大的模型，本研究旨在探索轻量级可解释预测方法能否在PM2.5小时预测中提供有竞争力的性能。

Method: 使用多年污染物和气象时间序列数据，开发了泄漏感知预测工作流程，结合时间数据分区、预处理、特征选择和外生驱动建模。评估了SARIMAX、Facebook Prophet和NeuralProphet三种预测模型家族，并在两种自适应机制下测试：周滚动向前更新和冻结预测配合在线残差校正。

Result: 在周滚动更新机制下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间显著少于NeuralProphet。在冻结模型机制下，在线残差校正改善了Facebook Prophet和SARIMAX，校正后的SARIMAX总体误差最低（MAE 32.50，RMSE 46.85）。NeuralProphet在两个机制下准确性和稳定性都较差，残差校正对其无效。校正后的Facebook Prophet误差接近其滚动更新版本，同时将运行时间从15分21.91秒减少到46.60秒。
...
### [209] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
*Yuqian Fu,Haohuan Huang,Kaiwen Jiang,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 该论文分析了on-policy蒸馏(OPD)在长序列生成中的局限性，提出了改进方法：通过teacher top-K局部支持匹配（截断反向KL与top-p采样和特殊token掩码）来解决采样token OPD的失败模式，在数学推理和智能体任务中实现更稳定的优化和更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统on-policy蒸馏(OPD)在长序列生成中存在脆弱性问题：采样token变体将分布匹配简化为单token信号，随着生成序列偏离教师模型常见前缀，指导变得不可靠。需要重新审视OPD的估计器和实现方式，解决长序列设置中的稳定性问题。

Method: 从理论和实证两方面分析OPD：1) 理论分析token级OPD相对于序列级反向KL的偏差和方差权衡；2) 识别采样token OPD的三个失败模式：不平衡单token信号、学生生成前缀上不可靠的教师指导、分词器或特殊token不匹配导致的失真；3) 提出teacher top-K局部支持匹配方法，通过截断反向KL与top-p采样和特殊token掩码来解决这些问题。

Result: 在单任务数学推理和多任务智能体+数学训练中，提出的teacher top-K局部支持匹配方法比采样token OPD表现出更稳定的优化过程和更好的下游性能。理论分析显示token级OPD虽然相对于序列级反向KL有偏差，但具有更紧的最坏情况方差界限。

Conclusion: 采样token OPD在长序列生成中存在根本性缺陷，而提出的teacher top-K局部支持匹配方法通过截断反向KL与top-p采样和特殊token掩码，能够提供更可靠的教师指导，实现更稳定的优化和更好的性能，为大规模语言模型后训练提供了有效的改进方案。
...
Method: 1) 训练基于GatedMLP的不确定性预测器，将遥测窗口总结为分布运动学特征并输出不确定性分数；2) 应用不确定性引导的标签重平衡机制(uLNR)，将具有异常高不确定性的安全标签窗口概率性地重新标记为不安全，从而在不合成新数据的情况下用信息丰富的边界样本丰富少数类；3) 在重平衡数据集上训练安全预测器
...
Conclusion: U-Balance通过利用行为不确定性进行数据重平衡，有效解决了CPS安全监测中的极端类别不平衡问题，显著提升了安全预测性能，为CPS安全监测提供了新的解决方案
...
### [211] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
*Shashank Subramanian,Alexander Kiefer,Arnur Nigmetov,Amir Gholami,Dmitriy Morozov,Michael W. Mahoney*

Main category: cs.LG

TL;DR: 该研究探讨了科学机器学习中的神经缩放定律，特别关注天气预报模型。通过采用最小化Swin Transformer架构和持续训练策略，展示了可预测的缩放趋势，并识别了计算最优的训练机制。


<details>
  <summary>Details</summary>
Motivation: 神经缩放定律在自然语言处理和计算机视觉领域已成为构建基础模型的关键，但在科学机器学习（特别是天气预报）中的应用尚未充分探索。研究旨在分析科学机器学习中的缩放行为，为高效资源分配提供诊断工具。

Method: 采用最小化、可扩展的通用Swin Transformer架构，使用恒定学习率的持续训练配合周期性冷却作为高效训练策略。系统探索了不同模型和数据集规模下的计算预算，构建IsoFLOP曲线，并识别计算最优训练机制。

Result: 研究表明这种最小化训练方式遵循可预测的缩放趋势，甚至优于标准余弦学习率调度。冷却阶段可重新用于提升下游性能，如实现更长的预测时间跨度和通过谱损失调整获得更清晰的预测。通过外推趋势揭示了潜在性能极限。

Conclusion: 神经缩放定律可作为科学机器学习中高效资源分配的重要诊断工具。研究展示了在天气预报领域可预测的缩放行为，并为大规模模型训练提供了实用指导。代码已开源以确保可复现性。
...
### [212] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
*Xueji Zhao,Likai Pei,Jianbo Liu,Kai Ni,Ningyuan Cao*

Main category: cs.LG

TL;DR: 论文提出将确定性数据访问视为随机采样的特例，建立统一框架分析AI系统中数据与随机性访问的交互，揭示随机性需求降低数据访问效率并导致熵受限操作，提出内存级评估标准并分析概率计算内存架构。


<details>
  <summary>Details</summary>
Motivation: 可信AI日益依赖概率计算实现鲁棒性、可解释性、安全性和隐私保护。实际系统中，确定性数据访问与跨模型、数据路径和系统功能的重复随机采样交织，使性能瓶颈从算术单元转移到必须同时提供数据和随机性的内存系统。

Method: 提出统一的数据访问视角，将确定性访问视为随机采样的极限情况，使两种模式能在共同框架下分析。基于此视角定义内存级评估标准：统一操作、分布可编程性、效率、对硬件非理想性的鲁棒性以及并行兼容性。

Result: 分析显示增加随机性需求会降低有效数据访问效率，并可能驱动系统进入熵受限操作。使用定义的标准分析了传统架构的局限性，并研究了集成采样与内存访问的新兴概率计算内存方法。

Conclusion: 通过统一框架揭示了随机性对内存系统的影响，提出了评估标准，并指出集成采样与内存访问的概率计算内存架构是实现可扩展可信AI硬件的有前景途径。
