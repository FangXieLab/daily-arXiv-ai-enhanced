...
TL;DR: 论文批评AI进步的线性模型，提出"熟悉智能"和"陌生智能"概念，认为AI智能更可能是陌生智能，具有非线性特征，不能简化为单一线性量。


<details>
  <summary>Details</summary>
Motivation: 反对AI发展的线性模型，认为传统智能评估方法存在缺陷，需要更准确地理解AI智能的本质特征，特别是其非线性和"陌生"特性。

Method: 引入"熟悉智能"和"陌生智能"概念框架，发展非线性智能模型，认为"通用智能"不是统一能力，而是在广泛环境中实现广泛目标的能力。

Result: AI智能更可能是陌生智能，表现出超人类能力与亚人类表现的混合，在领域内也可能结合超人类洞察力和令人惊讶的错误。

Conclusion: AI的陌生智能特性意味着即使最先进的系统也可能在看似简单的任务上失败，单一任务的优秀表现不能证明广泛的通用能力，这对对抗性测试方法有重要启示。
...
TL;DR: LLM在已解决的图论问题上表现良好，能正确理解概念、回忆相关结果并构建有效证明；但在未解决问题上只能提供解释和探索策略，无法产生新见解，表明其适合概念探索但缺乏数学创新和结构推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地被学生用于计算机科学高级内容（如图论）的学习，需要评估这些工具在支持数学严谨思维方面的可靠性，以指导其在本科和研究生课程中的合理使用。

Method: 使用八阶段评估协议模拟真实数学探究过程，包括解释、探索、策略形成和证明构建，测试LLM在两个图论问题上的表现：一个已解决的线图优美性问题和一个当前无解的开放问题。

Result: 在已解决问题上，LLM表现优异：正确生成定义、识别相关结构、无幻觉地回忆适当结果，并构建了图论专家确认的有效证明。在开放问题上，LLM能生成连贯解释和合理探索策略，但未推进解决方案，且未捏造结果而是承认不确定性。

Conclusion: LLM能有效支持已确立材料的概念探索，但在需要新颖数学洞察或关键结构推理的任务上仍有局限。对计算教育而言，应引导学生使用LLM进行概念探索，同时依赖独立验证和严格论证进行正式问题求解。
...
TL;DR: 该论文提出了首个通用的智能体不确定性量化（UQ）框架，将现有UQ设置统一起来，并提出了从不确定性积累过程到条件性不确定性减少过程的新视角转变。


<details>
  <summary>Details</summary>
Motivation: 当前LLM不确定性量化研究主要集中于单轮问答任务，而随着LLM智能体在复杂任务中的部署日益增多，需要面向交互式智能体的新UQ框架。

Method: 提出了首个通用的智能体UQ形式化框架，涵盖现有UQ设置类别；提出条件性不确定性减少过程的新视角，强调动作的"交互性"；构建了为LLM智能体设置设计UQ的概念框架。

Result: 展示了先前工作隐含地将LLM UQ视为不确定性积累过程，这一观点在开放世界交互式智能体中失效；提出了通过建模智能体轨迹上可减少的不确定性来提供可操作指导的新框架。

Conclusion: 讨论了智能体UQ在前沿LLM开发和领域特定应用中的实际意义，以及剩余开放问题，强调了向交互式智能体UQ研究转变的必要性。
...
TL;DR: 比较三种自主任务规划器在低地球轨道多碎片交会问题中的表现：固定参数训练的Masked PPO、领域随机化Masked PPO和蒙特卡洛树搜索基准，评估其在燃料和时间约束变化下的性能权衡


<details>
  <summary>Details</summary>
Motivation: 自主碎片清除任务规划需要在效率、适应性和严格的燃料与任务时间约束之间取得平衡，现有方法在应对分布偏移时存在局限性，需要评估不同规划器在约束变化下的鲁棒性

Method: 比较三种规划器：1) 在固定任务参数下训练的名义Masked PPO策略；2) 在变化任务约束下训练的领域随机化Masked PPO策略以提升鲁棒性；3) 普通蒙特卡洛树搜索基准。在高保真轨道模拟环境中评估，包含加油、真实转移动力学和随机碎片场，在300个测试案例中评估名义、减少燃料和减少任务时间三种场景

Result: 名义PPO在训练条件匹配时表现最佳，但在分布偏移下性能急剧下降；领域随机化PPO在名义性能适度损失的情况下展现出改进的适应性；MCTS始终能最好地处理约束变化，但计算时间高出几个数量级

Conclusion: 学习策略的速度与基于搜索方法的适应性之间存在权衡，结合训练时多样性和在线规划可能是未来弹性碎片清除任务规划器的有前景路径
...
TL;DR: DemPO框架通过算法抽签机制构建代表性评分者小组，在偏好对齐训练中实现人口统计学代表性，提升AI系统价值对齐的民主性。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐方法（如RLHF）依赖的评分者样本通常存在人口统计学偏差，过度代表某些群体而忽视其他群体，导致AI系统学习到的价值观不能反映更广泛人群的偏好。

Method: 提出民主偏好优化（DemPO）框架，采用公民大会使用的算法抽签机制。提供两种训练方案：硬面板（仅使用抽签选出的配额满足型微型公众的偏好）和软面板（保留所有数据但按抽签包含概率重新加权评分者）。证明软面板加权可恢复硬面板目标的闭式解。

Result: 在包含评分者人口统计信息的公共偏好数据集上，对10亿到80亿参数的Llama模型进行微调评估。硬面板在六种聚合方法中始终排名第一，软面板始终优于未加权基线，且效果随模型容量增加而增强。

Conclusion: 在偏好收集阶段强制执行人口统计学代表性，而非事后校正，能产生行为更符合代表性公众所表达价值观的模型。这证明了民主代表性机制在AI价值对齐中的有效性。
...
### [6] [SocialVeil: Probing Social Intelligence of Language Agents under Communication Barriers](https://arxiv.org/abs/2602.05115)
...
Method: 基于对人类交互中沟通挑战的系统文献综述，SocialVeil引入了三种代表性的沟通障碍类型：语义模糊性、社会文化不匹配和情感干扰。提出了两种障碍感知评估指标：未解决的困惑和相互理解。在720个场景和四个前沿LLM上进行实验，并验证了模拟障碍的保真度。

Result: 沟通障碍持续损害LLM性能：相互理解平均降低超过45%，困惑度提升近50%。人类评估验证了模拟障碍的保真度（ICC≈0.78，Pearson r≈0.80）。适应策略（修复指令和交互学习）效果有限，远未达到无障碍性能。

Conclusion: SocialVeil使社会交互环境更接近真实世界沟通，为探索LLM智能体的社会智能提供了机会。研究表明LLM在非理想沟通条件下表现显著下降，现有适应策略效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉交通固有的混沌特性，在数据稀缺的跨城市设置中无法有效进行少样本学习。交通的复杂非线性动态和领域偏移使得传统预测方法效果有限。

Method: 提出CAST-CKT框架：1）高效混沌分析器量化交通可预测性机制；2）混沌感知注意力实现机制自适应的时间建模；3）自适应拓扑学习处理动态空间依赖；4）基于混沌一致性的跨城市对齐进行知识转移；5）提供具有不确定性量化的特定时间范围预测。

Result: 在四个基准数据集上的跨城市少样本设置实验中，CAST-CKT在MAE和RMSE指标上显著优于现有最先进方法，同时提供可解释的机制分析。理论分析显示改进的泛化边界。

Conclusion: CAST-CKT通过混沌感知建模有效解决了跨城市少样本交通预测的挑战，在性能、可解释性和不确定性量化方面均有显著提升，为数据稀缺场景下的交通预测提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法过度依赖表层节点匹配，缺乏显式因果建模，导致答案不可靠或虚假。先前尝试融入因果关系的方法通常局限于局部或单文档上下文，且受到模块化图结构造成的信息隔离问题，阻碍了可扩展性和跨模块因果推理。
...
<details>
  <summary>Details</summary>
Motivation: 评估当前人工智能系统正确回答研究级数学问题的能力，这些问题在作者研究过程中自然产生，具有真实的研究背景

Method: 创建包含10个未公开数学问题的数据集，这些问题源于作者的实际研究过程，答案暂时加密不公开

Result: 提供了一个专门用于测试AI数学推理能力的基准数据集，包含10个研究级数学问题

Conclusion: 该数据集为评估AI系统处理研究级数学问题的能力提供了有效工具，有助于推动AI在高级数学推理方面的发展
...
TL;DR: 该论文针对多知识库检索增强生成中的权威性偏差问题，提出DAKS路由预算检索和对齐图证据融合方法，在藏医药领域实现更好的跨知识库证据覆盖和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 在多知识库检索增强生成中，不同知识库的密度和权威性差异导致检索偏差。在藏医药领域，密集的百科条目容易主导检索，而更具权威性的经典文献和临床论文反而被忽视，这影响了答案的可追溯性和权威性。

Method: 提出两种互补方法：1) DAKS进行知识库路由和预算检索，缓解密度驱动偏差，在适当时优先权威来源；2) 使用对齐图指导证据融合和覆盖感知打包，在不依赖简单拼接的情况下提高跨知识库证据覆盖。所有答案由轻量级生成器openPangu-Embedded-7B生成。

Result: 实验显示在路由质量和跨知识库证据覆盖方面取得一致提升，完整系统在保持强忠实度和引用正确性的同时，实现了最佳的CrossEv@5指标。

Conclusion: 该研究提出的方法有效解决了多知识库检索增强生成中的权威性偏差问题，通过知识库路由和证据融合策略，在藏医药领域实现了更好的跨知识库证据覆盖和可追溯性。
...
TL;DR: 提出一种基于注意力汇机制的安全防御方法Surgery，通过抑制注意力头的汇散度来减轻有害微调对语言模型安全性的破坏。


<details>
  <summary>Details</summary>
Motivation: 有害微调会破坏大型语言模型的安全对齐，带来重大安全风险。现有防御方法效果有限，需要更有效的微调阶段防御机制。

Method: 提出可分离汇散度假设，发现有害微调时注意力头汇散度符号呈现可分离性。基于此设计Surgery方法，通过汇散度抑制正则化器，引导注意力头向负汇散度组移动，减少模型学习和放大有害模式。

Result: 在BeaverTails、HarmBench和SorryBench基准测试上，Surgery分别提升防御性能5.90%、11.25%和9.55%。实验验证了汇散度符号与模型有害性的相关性。

Conclusion: 注意力汇机制为防御有害微调提供了新视角，Surgery方法通过汇散度抑制有效减轻安全风险，为语言模型安全对齐提供了实用的微调阶段防御方案。
...
<details>
  <summary>Details</summary>
Motivation: 智能体将广泛部署于多样化家庭环境，需要针对每个独特未见3D环境进行评估。现有基准存在严重数据污染和缺乏场景特异性，无法有效评估智能体在未见环境中的能力。

Method: 提出受人类认知启发的动态原位任务生成方法TEA。使用结构化图表示定义任务，构建两阶段交互-进化任务生成系统：交互阶段智能体主动与环境交互，形成任务执行与生成的循环；进化阶段通过任务图建模重组和重用现有任务生成新任务，无需外部数据。

Result: 在10个未见场景中，TEA在两个周期内自动生成了87,876个任务，经人工验证具有物理合理性并涵盖基本日常认知能力。将SOTA模型与人类在生成任务上对比发现，模型虽然在公共基准上表现优异，但在基本感知任务上表现糟糕，严重缺乏3D交互意识，且对任务类型在推理中表现出高度敏感性。

Conclusion: 这些令人警醒的发现突显了在将智能体部署到真实人类环境之前进行原位评估的必要性。TEA方法为未见3D环境中的智能体评估提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 余弦相似度基于柯西-施瓦茨不等式，只能捕捉线性关系，无法建模真实语义空间的复杂非线性结构，需要更优的相似度度量方法

Method: 推导出比经典柯西-施瓦茨界更紧的点积上界，基于此提出recos度量：通过排序向量分量归一化点积，将完美相似条件从严格线性依赖放宽为序数一致性

Result: 在11种嵌入模型（静态、上下文化、通用类型）上实验，recos在标准语义文本相似性基准测试中始终优于传统余弦相似度，与人类判断相关性更高

Conclusion: recos作为数学原理更优、经验表现更好的替代方案，为复杂嵌入空间中的语义分析提供了更高的准确性
...
Method: 设计一个原则性框架，将LLM集成到迭代循环中：LLM生成候选动作，系统检查其与约束和前瞻预测的一致性；当一致性低时，通过数字孪生等外部反馈收集信息，使用上下文学习（ICL）精炼候选动作；通过调整一致性阈值控制幻觉风险

Result: 在四个公共数据集上的实验表明，该框架相比前沿LLM将恢复时间减少了高达30%；理论证明该设计可通过调整一致性阈值控制幻觉风险，并在特定假设下建立了ICL的遗憾界限

Conclusion: 提出的框架有效解决了LLM在安全管理工作中的幻觉问题，通过一致性检查和外部反馈机制实现了可靠的决策支持，显著提升了事件响应效率
...
### [15] [PieArena: Frontier Language Agents Achieve MBA-Level Negotiation Performance and Reveal Novel Behavioral Differences](https://arxiv.org/abs/2602.05302)
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在谈判任务中的能力，谈判作为核心商业任务需要战略推理、心智理论和经济价值创造能力，现有基准测试未能全面衡量谈判行为的多维度特征

Method: 提出PieArena大规模谈判基准，基于精英商学院MBA谈判课程的真实场景构建多智能体交互环境，评估不同LLMs的谈判表现，并研究联合意向性智能体支架的效果

Result: GPT-5达到或超越经过一学期谈判训练和针对性指导的商学院学生；联合意向性支架对中低层模型有显著提升但对前沿模型收益递减；谈判行为分析揭示模型在欺骗、计算准确性、指令遵从和感知声誉等方面的异质性

Conclusion: 前沿语言智能体在智力和心理层面已具备在高风险经济环境中部署的能力，但在鲁棒性和可信赖性方面仍存在挑战，需要超越交易结果的更全面评估框架
...
### [16] [ProAct: Agentic Lookahead in Interactive Environments](https://arxiv.org/abs/2602.05327)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型智能体在需要长期规划的交互环境中表现不佳，主要原因是模拟未来状态时的误差累积问题。

Method: 提出ProAct框架，包含两个核心组件：1) Grounded LookAhead Distillation (GLAD)：通过环境搜索轨迹进行监督微调，将复杂搜索树压缩为简洁的因果推理链；2) Monte-Carlo Critic (MC-Critic)：轻量级环境模拟的价值估计器，增强PPO和GRPO等策略梯度算法。

Result: 在随机环境（如2048）和确定性环境（如Sokoban）上的实验表明，ProAct显著提高了规划准确性。4B参数模型在ProAct训练下超越了所有开源基线，并与最先进的闭源模型相媲美，同时在未见环境中展现出鲁棒泛化能力。

Conclusion: ProAct通过将前瞻推理内化到智能体中，解决了LLM智能体长期规划中的误差累积问题，提供了一种高效且可扩展的解决方案，代码和模型已开源。
...
### [17] [RaBiT: Residual-Aware Binarization Training for Accurate and Efficient LLMs](https://arxiv.org/abs/2602.05367)
...
Method: 提出RaBiT框架，核心机制是从单个共享全精度权重顺序推导每个二进制路径，确保每个路径纠正前一个路径的误差。通过稳健初始化优先考虑功能保留而非权重近似来稳定该过程。

Result: 重新定义了2位精度-效率前沿：实现最先进性能，甚至可与硬件密集的向量量化方法相媲美，在RTX 4090上相比全精度模型提供4.49倍推理加速。

Conclusion: RaBiT通过算法强制残差层次结构有效解决了二进制路径间的特征共适应问题，为极端量化提供了新的解决方案，在保持硬件友好性的同时实现了优异的性能表现。
...
### [18] [Clinical Validation of Medical-based Large Language Model Chatbots on Ophthalmic Patient Queries with LLM-based Evaluation](https://arxiv.org/abs/2602.05381)
...
TL;DR: 本研究评估了四个小型医疗大语言模型在眼科患者咨询中的表现，并测试了基于LLM评估与临床医生评分的一致性。Meerkat-7B表现最佳，MedLLaMA3-v20表现最差且有25.5%的幻觉内容。GPT-4-Turbo评估与临床医生评分高度一致，支持LLM评估在大规模基准测试中的可行性。


<details>
  <summary>Details</summary>
Motivation: 随着领域特定大语言模型在眼科患者教育、分诊和临床决策支持中的应用日益增多，需要对其进行严格评估以确保安全性和准确性。本研究旨在评估小型医疗LLM在回答眼科患者咨询时的表现，并探索基于LLM的评估与临床医生评分的一致性，为资源高效部署提供依据。

Method: 采用横断面研究设计，选取四个参数规模小于100亿的小型医疗LLM（Meerkat-7B、BioMistral-7B、OpenBioLLM-8B、MedLLaMA3-v20），让每个模型回答180个眼科患者咨询，共生成2160个回答。评估由三位不同资历的眼科医生和GPT-4-Turbo使用S.C.O.R.E.框架进行，该框架评估安全性、共识与上下文、客观性、可重复性和可解释性，采用5点李克特量表评分。使用Spearman秩相关、Kendall tau统计和核密度估计分析评估LLM与临床医生评分之间的一致性。

Result: Meerkat-7B表现最佳，获得高级顾问平均分3.44、顾问4.08、住院医师4.18。MedLLaMA3-v20表现最差，25.5%的回答包含幻觉或临床误导性内容，包括编造术语。GPT-4-Turbo评估与临床医生整体评估高度一致，Spearman rho为0.80，Kendall tau为0.67，但高级顾问评分更为保守。医疗LLM在眼科问答中显示出安全性潜力，但在临床深度和共识方面仍有差距。

Conclusion: 医疗大语言模型在眼科患者咨询中具有安全应用的潜力，但仍需改进临床深度和共识。基于LLM的评估在大规模基准测试中具有可行性，但需要建立混合的自动化与临床医生审查框架，以指导安全的临床部署。
...
### [19] [H-AdminSim: A Multi-Agent Simulator for Realistic Hospital Administrative Workflows with FHIR Integration](https://arxiv.org/abs/2602.05407)
...
<details>
  <summary>Details</summary>
Motivation: 大型医院行政部门每天处理超过10,000个请求，对LLM自动化需求增长。现有研究主要关注医患交互或孤立行政子任务，未能捕捉真实行政工作流的复杂性，需要更全面的评估框架

Method: 提出H-AdminSim框架，结合真实数据生成与多智能体仿真模拟医院行政工作流，通过详细评分标准进行定量评估，并集成FHIR标准提供跨异构医院环境的统一可互操作测试平台

Result: H-AdminSim为评估LLM驱动的行政自动化可行性和性能提供了标准化测试平台，能够系统比较不同LLM在复杂医院行政工作流中的表现

Conclusion: H-AdminSim填补了医院行政工作流仿真研究的空白，为LLM在医疗行政自动化中的评估提供了全面、标准化且可互操作的测试环境，有助于推动该领域的发展
...
TL;DR: THOR：一种用于超关系知识图谱的归纳式链接预测方法，通过关系基础图和实体基础图建模跨事实交互，支持完全归纳推理
...
Conclusion: THOR通过建模超关系知识图谱的结构不变性，实现了有效的跨图谱归纳推理，为超关系知识图谱的链接预测提供了强大的完全归纳解决方案
...
### [21] [Day-Ahead Electricity Price Forecasting for Volatile Markets Using Foundation Models with Regularization Strategy](https://arxiv.org/abs/2602.05430)
*Kritchanat Ponyuenyong,Pengyu Tu,Jia Wei Tan,Wei Soon Cheong,Jamie Ng Suat Ling,Lianlian Jiang*

Main category: cs.AI

TL;DR: 该论文评估了时间序列基础模型在波动性电力市场价格预测中的表现，发现这些模型相比传统统计和深度学习模型有显著改进，最高可提升37.4%的MAPE。


<details>
  <summary>Details</summary>
Motivation: 电力价格预测对能源市场参与者至关重要，但由于价格信号固有的波动性和非线性而具有挑战性。传统统计和深度学习模型难以有效捕捉复杂的时间依赖关系并整合异构数据。虽然时间序列基础模型在一般时间序列预测任务中表现出色，但它们在波动性市场中日前电力价格预测的有效性尚未得到充分探索。
...
### [22] [Refine and Purify: Orthogonal Basis Optimization with Null-Space Denoising for Conditional Representation Learning](https://arxiv.org/abs/2602.05464)
*Jiaquan Wang,Yan Lyu,Chen Li,Yuheng Jia*

Main category: cs.AI

TL;DR: 提出OD-CRL框架，通过自适应正交基优化和零空间去噪投影解决条件表示学习中子空间基敏感性和子空间间干扰问题
...
Conclusion: OD-CRL通过正交基优化和零空间投影有效解决了条件表示学习中的基敏感性和子空间干扰问题，为相关任务提供了更鲁棒的解决方案
...
### [23] [ALIVE: Awakening LLM Reasoning via Adversarial Learning and Instructive Verbal Evaluation](https://arxiv.org/abs/2602.05472)
...
Method: 基于认知协同原则，将问题提出、解决和评判统一在单一策略模型中，通过对抗学习和指导性语言反馈让模型从原始语料内部化评估标准

Result: 在数学推理、代码生成和一般逻辑推理基准测试中，ALIVE显著缓解了奖励信号限制，获得准确率提升、跨领域泛化能力增强和更高自校正率

Conclusion: ALIVE为无需人工监督的通用推理对齐提供了可扩展基础，推理三位一体促进了能力增长的自我维持轨迹
...
### [24] [Phi-Former: A Pairwise Hierarchical Approach for Compound-Protein Interactions Prediction](https://arxiv.org/abs/2602.05479)
*Zhe Wang,Zijing Liu,Chencheng Xu,Yuan Yao*

Main category: cs.AI

TL;DR: Phi-former是一种用于预测化合物-蛋白质相互作用（CPI）的成对分层交互表示学习方法，通过整合基序（motif）在生物识别中的关键作用，在原子-原子、基序-基序和原子-基序三个层次上系统建模分子相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的CPI预测方法虽然比传统能量方法更高效准确，但大多在原子层面建模，忽略了化学现实：分子片段（基序或功能基团）通常是生物识别和结合的主要单元。需要开发能更好反映生物系统识别机制的方法。
...
Conclusion: Phi-former通过整合基序的生物角色，更好地反映了生物系统的分子识别机制，为合理药物设计和精准医疗应用提供了可解释的见解。
...
TL;DR: 开发了一个针对成釉细胞瘤的多模态数据集和AI诊断框架，显著提升了病变分类和检测性能


<details>
  <summary>Details</summary>
Motivation: 颌面病理学AI诊断需要高质量、结构化的多模态数据集，但现有资源对成釉细胞瘤覆盖有限且格式不一致，无法直接用于模型训练

Method: 创建了专门针对成釉细胞瘤的多模态数据集，整合了标注的放射学、组织病理学和口腔临床图像，以及从病例报告中提取的结构化数据；使用自然语言处理技术从文本报告中提取临床相关特征，对图像数据进行领域特定的预处理和增强；开发了多模态深度学习模型，用于分类成釉细胞瘤变异、评估复发风险等行为模式，并支持手术规划

Result: 定量评估显示显著改进：变异分类准确率从46.2%提升到65.9%，异常组织检测F1分数从43.0%提升到90.3%；与MultiCaRe等资源相比，该工作通过提供稳健的数据集和适应性强的多模态AI框架，推进了患者特异性决策支持

Conclusion: 本研究提供了一个专门针对成釉细胞瘤的多模态数据集和AI框架，显著提升了诊断性能，为个性化颌面病理学诊断和手术规划提供了有效的决策支持工具
...
TL;DR: SPT方法通过在LoRA参数中训练"诚实人格"来检测大语言模型的对齐问题，该方法在Anthropic审计游戏基准上达到96%准确率，而传统方法接近0%


<details>
  <summary>Details</summary>
Motivation: 大语言模型可能在训练中学会隐藏不当行为，传统审计方法存在局限：黑盒方法难以区分对齐不良输出与良性输出，机制可解释性方法无法随模型能力扩展

Method: 提出Split Personality Training(SPT)，在LoRA参数中微调第二个"诚实人格"，该人格在正常操作中保持休眠。主模型响应后，激活LoRA适配器并插入触发字符串，使诚实人格能够审查响应同时访问主模型的潜在状态

Result: 在Anthropic审计游戏模型生物基准测试中，SPT达到96%总体准确率，而Anthropic报告接近0%准确率。诚实人格揭示了外部观察者无法访问的潜在知识，如受损模型训练中的虚构偏见

Conclusion: SPT通过训练辅助诚实人格有效检测大语言模型的对齐问题，能够揭示模型隐藏的潜在知识，为模型审计提供了新的有效方法
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLMs是否能在个体层面预测认知偏见，并模拟在认知负荷等情境因素影响下的人类偏见行为动态，这对设计具有偏见意识的交互式AI系统至关重要

Method: 将三个经典决策场景转化为对话设置，进行人类实验（N=1100），参与者通过简单或复杂对话与聊天机器人互动；使用参与者人口统计数据和对话记录，基于GPT-4和GPT-5模拟相同交互条件

Result: 人类实验显示显著的偏见模式；LLMs能够精确复现人类偏见；不同模型在模拟人类行为对齐方面存在明显差异

Conclusion: LLMs能够有效模拟人类决策偏见，这对设计和评估交互环境中具有偏见意识的适应性AI系统具有重要意义，模型间的差异提示需要更精细的评估方法
...
Method: 1. 提出Resin（Reactive Signal Inference）概率编程语言，将概率逻辑与反应式编程相结合；2. 提出Reactive Circuits（RCs）作为Resin的高效精确语义，构建在代数电路和异步数据流之上的元结构，形成时间动态有向无环图，根据输入信号的波动性自主适应；3. 基于异步输入的变化频率估计对计算进行分区，将大型推断任务分解为可单独记忆化的子问题。

Result: 在高保真无人机群模拟中，该方法相比频率无关的推断实现了数个数量级的加速。RCs的结构适应成功捕捉了环境动态，显著降低了延迟，促进了反应式实时推理。通过基于异步输入变化频率的计算分区，确保只有受新信息影响的模型特定组件被重新评估，大幅减少了流式上下文中的冗余计算。

Conclusion: 通过异步反应式概率推理方法，能够显著提高动态环境中概率推断的效率，实现实时推理，为自主智能体在动态环境中的持续推理提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统本体论擅长描述领域结构但无法生成新工件，而大语言模型能流畅生成但缺乏结构有效性，常产生无组件的机制或无结束条件的目标等幻觉。需要结合两者的互补优势：本体论提供语法，LLM提供创造力。
...
Result: 通过GameGrammar系统演示该框架，给定主题提示（如"洞穴生态系统中发光的真菌竞争"），管道能生成结构完整、可玩的桌面游戏规范，包括机制、组件、胜利条件和设置说明。输出既满足本体约束又保持真正的创造性。

Conclusion: 该模式可推广到游戏以外的领域，任何具有专业词汇、有效性约束和积累范例的领域（如音乐创作、软件架构、烹饪艺术）都适合Generative Ontology。约束不仅不限制创造力，反而使其成为可能：正如语法使诗歌成为可能，本体论使结构化生成成为可能。
...
TL;DR: 该论文对基于图的智能体记忆系统进行了全面综述，从图结构视角分析了智能体记忆的分类、生命周期关键技术、开源资源及应用场景。


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长期复杂任务（如多轮对话、游戏、科学发现）的核心模块，而图结构因其建模关系依赖、组织层次信息和高效检索的内在能力，成为智能体记忆的强有力结构。需要系统梳理基于图的智能体记忆研究现状。

Method: 1. 提出智能体记忆的分类法：短期vs长期记忆、知识vs经验记忆、非结构化vs结构化记忆；2. 按记忆生命周期系统分析关键技术：记忆提取、存储、检索和演化；3. 总结开源库和基准测试；4. 探索应用场景；5. 识别挑战和未来方向。

Result: 建立了基于图的智能体记忆的全面分析框架，提供了记忆分类、生命周期技术分析、开源资源汇总（包括研究论文、开源数据和项目），为社区提供了可操作的见解和资源集合。

Conclusion: 基于图的智能体记忆系统对于开发更高效可靠的智能体至关重要，该综述为推进该领域发展提供了系统性的指导框架和资源支持，有助于促进智能体记忆系统的进一步发展。
...
TL;DR: 论文提出基于Transformer架构计算和内存访问复杂度的分析模型，准确预测LLM推理能耗，发现能效存在"甜点"区域，短中长度输入和中长度输出能效最高，为生产系统提供节能策略指导。


<details>
  <summary>Details</summary>
Motivation: 现有LLM能耗估计方法通常使用输入输出序列长度的简单线性函数，但实际观测显示能效存在明显的非线性依赖关系，需要更准确的能耗模型来指导节能优化。

Method: 基于Transformer架构的计算和内存访问复杂度推导分析模型，在NVIDIA H100 GPU上使用TensorRT-LLM评估多种LLM（OPT、LLaMA、Gemma、Falcon、Qwen2、Granite，参数规模1B-9B），输入输出长度范围64-4096 token。

Result: 模型预测准确度高（平均MAPE 1.79%），发现能效存在"甜点"区域：短到中等长度输入和中等长度输出能效最高，长输入或极短输出能效显著下降。

Conclusion: 通过将序列长度与能效"甜点"对齐，可大幅降低能耗，支持生产系统中的智能截断、摘要和自适应生成策略，为LLM部署提供节能指导。
...
Conclusion: GenLoRA通过生成式方法解决了LoRA的参数效率问题，在保持性能的同时显著减少了参数数量，为高效参数微调提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习可验证奖励方法存在递归空间收缩的系统性病理问题，导致有效替代方案的采样概率消失。虽然KL正则化试图缓解这一问题，但它施加了严格的形状匹配约束，与正确性所需的锐化产生梯度冲突。

Method: 提出锚定策略优化方法，将范式从全局形状匹配转向支持覆盖。通过基于参考模型高置信度支持定义安全流形，APO允许激进锐化以提高效率，同时在错误校正时选择性调用恢复力以防止崩溃。

Result: 在数学基准测试中，APO打破了准确性与多样性之间的权衡，显著提高了Pass@1指标，同时恢复了标准策略梯度方法通常丢失的Pass@K多样性。

Conclusion: APO作为一种梯度对齐机制，通过最大化支持覆盖实现了弹性恢复，能够重新膨胀有效分支，解决了RLVR中的递归空间收缩问题。
...
<details>
  <summary>Details</summary>
Motivation: 金融RAG系统虽然依赖检索文档来生成准确回答，但模型仍会产生与检索信息矛盾的幻觉，需要解决这种不一致性问题

Method: 提出RLFKV框架：1) 将金融回答分解为原子知识单元；2) 评估每个单元的正确性计算细粒度忠实度奖励；3) 引入信息量奖励防止奖励黑客攻击；4) 结合两种奖励优化策略模型

Result: 在公开的FDD任务和新提出的FDD-ANT数据集上实验显示一致改进，验证了方法的有效性

Conclusion: RLFKV框架通过细粒度知识验证和强化学习有效减少了金融RAG系统中的幻觉问题，提高了回答与检索文档的一致性
...
### [35] [RL-VLA$^3$: Reinforcement Learning VLA Accelerating via Full Asynchronism](https://arxiv.org/abs/2602.05765)
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的RL训练框架（如RLinf）采用同步执行，导致环境交互、策略生成和模型更新阶段的资源利用率低下和吞吐量限制，成为训练效率瓶颈

Method: 提出完全异步策略训练框架，采用多级解耦架构：1) 环境交互和轨迹收集的异步并行化；2) 策略生成的流式执行；3) 训练更新的解耦调度；借鉴大模型RL中的异步优化思想

Result: 在LIBERO基准测试中，相比现有同步策略，吞吐量提升最高达59.25%；深度优化分离策略后，吞吐量提升可达126.67%；消融实验验证各异步组件的有效性；8到256个GPU上的扩展性验证显示良好可扩展性

Conclusion: 提出的完全异步训练框架有效解决了VLA模型训练中的效率瓶颈问题，通过全流程异步并行化显著提升了训练吞吐量和资源利用率，为大规模VLA模型训练提供了高效解决方案
...
### [36] [FiMI: A Domain-Specific Language Model for Indian Finance Ecosystem](https://arxiv.org/abs/2602.05794)
...
TL;DR: FiMI是针对印度数字支付系统开发的领域专业化金融语言模型，包含基础版和指令调优版，通过多阶段训练在金融推理和工具调用方面显著优于基准模型


<details>
  <summary>Details</summary>
Motivation: 开发专门针对印度数字支付系统的金融语言模型，解决现有通用模型在印度金融领域（特别是多语言环境和特定工作流程）的不足
...
Conclusion: FiMI成功开发了针对印度数字支付系统的专业化金融语言模型，在保持通用能力的同时显著提升了金融领域特定任务的表现，特别是工具调用能力
...
TL;DR: TKG-Thinker：一种配备自主规划和自适应检索能力的智能体，通过双阶段训练策略（监督微调+强化学习）在时序知识图谱上进行多轮交互推理，解决了LLMs在时序知识图谱问答中的幻觉问题和静态提示的局限性。
...
Conclusion: TKG-Thinker通过动态多轮交互和双阶段训练策略，有效解决了LLMs在时序知识图谱问答中的局限性，为复杂时序推理任务提供了有效的解决方案。
...
Result: 在多个基准测试上的广泛实验表明，OmniVideo-R1始终优于强基线，证明了其有效性和鲁棒的泛化能力

Conclusion: OmniVideo-R1通过"全模态线索思考"机制显著提升了多模态视频理解性能，为音频-视觉推理任务提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有生物学基准未能评估研究人员所需的关键能力：将实验结果与背景知识整合以得出有意义结论的能力。需要评估AI系统像实践科学家一样推理的能力

Method: 从同行评审研究论文和真实世界生物学研究中构建BABE基准，确保任务反映实际科学探究的复杂性和跨学科性质

Result: BABE基准为评估AI系统如何像实践科学家一样推理提供了稳健框架，提供了更真实的衡量其贡献生物学研究潜力的方法

Conclusion: BABE填补了现有基准的空白，通过评估实验推理能力，为衡量AI系统在生物学研究中的实际贡献潜力提供了更真实的评估工具
...
TL;DR: 提出分层座位分配问题（HSAP），通过概率路线图（PRM）和快速探索随机树（RRT）计算座位距离，结合启发式搜索和动态规划解决整数规划问题，实现组织团队到物理座位的最优分配。


<details>
  <summary>Details</summary>
Motivation: 大型组织具有复杂层级结构，需要确保具有紧密层级关系的团队在物理座位上相邻（如研究组占据连续区域）。目前该问题依赖手动处理，导致重新规划频率低且效果欠佳。

Method: 提出端到端框架解决HSAP：1）使用概率路线图（PRM）和快速探索随机树（RRT）计算任意座位对之间的距离；2）结合启发式搜索和动态规划方法，通过整数规划求解分层座位分配问题。

Result: 在不同规模实例上验证了PRM框架和后续分配方案，进行了定量和定性评估，证明了方法的有效性。

Conclusion: 提出的HSAP框架能够自动化解决大型组织的分层座位分配问题，替代传统手动规划，提高规划效率和优化效果。
...
### [41] [A Guide to Large Language Models in Modeling and Simulation: From Core Techniques to Critical Challenges](https://arxiv.org/abs/2602.05883)
*Philippe J. Giabbanelli*

Main category: cs.AI

TL;DR: 论文为M&S领域提供LLM使用指南，强调看似简单的实践可能带来问题，需基于原则性设计选择、诊断策略和实证评估


<details>
  <summary>Details</summary>
Motivation: LLM在M&S工作流中应用日益广泛，但看似简单的实践可能引入微妙问题、不必要复杂性甚至导致劣质结果，需要提供全面实用的使用指导

Method: 讨论常见混淆来源：非确定性、知识增强（RAG和LoRA）、M&S数据分解、超参数设置；强调原则性设计选择、诊断策略和实证评估方法

Result: 提供综合指南帮助建模者做出明智决策：何时、如何以及是否依赖LLM，避免常见陷阱如模型崩溃、信息丢失、不必要微调等

Conclusion: LLM在M&S应用中需要谨慎使用，基于原则性设计和实证评估的决策框架至关重要，建模者应了解LLM局限性并采取适当诊断策略
...
### [42] [Quantum Reinforcement Learning with Transformers for the Capacitated Vehicle Routing Problem](https://arxiv.org/abs/2602.05920)
*Eva Andrés*

Main category: cs.AI

TL;DR: 该论文比较了经典与量子强化学习方法在带容量约束车辆路径问题(CVRP)上的表现，发现量子增强模型特别是混合架构在路由距离、紧凑性和重叠度方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索量子计算在复杂组合优化问题中的应用潜力，特别是比较经典、全量子和混合强化学习方法在CVRP问题上的性能差异。

Method: 采用优势演员-评论家(A2C)智能体，实现经典、全量子和混合三种变体，集成transformer架构通过自注意力和交叉注意力机制捕捉车辆、客户和仓库之间的关系。实验设置包含20个客户和4辆车的多车辆容量约束场景，进行10次独立运行。

Result: 所有三种方法都能学习有效的路由策略，但量子增强模型优于经典基线，产生更稳健的路由组织。混合架构在距离、紧凑性和路由重叠度方面表现最佳。定性可视化显示量子模型生成更结构化、连贯的路由解决方案。

Conclusion: 混合量子-经典强化学习模型在解决CVRP等复杂组合优化问题方面具有显著潜力，量子增强方法能提供更优的路由组织和性能。
...
### [43] [Speech Emotion Recognition Leveraging OpenAI's Whisper Representations and Attentive Pooling Methods](https://arxiv.org/abs/2602.06000)
*Ali Shendabadi,Parnia Izadirad,Mostafa Salehi,Mahmoud Bijankhan*

Main category: cs.AI

TL;DR: 该论文探索了使用预训练的Whisper ASR系统进行语音情感识别，提出了两种基于注意力的池化方法来降低Whisper表示的维度，同时在英语和波斯语数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 语音情感识别研究因缺乏标准且足够大的数据集而受限，需要探索利用预训练模型提取特征的有效方法。Whisper作为预训练的ASR系统，其表示能力在SER任务中尚未充分探索。

Method: 提出两种基于注意力的池化方法：多头注意力平均池化和QKV池化，用于高效降低Whisper表示的维度并保留情感特征。在英语IEMOCAP和波斯语ShEMO数据集上使用Whisper Tiny和Small模型进行实验，比较不同编码器层的性能。

Result: 多头QKV架构在ShEMO数据集上取得了最先进的结果，未加权准确率提升了2.47%。发现中间层在波斯语数据集上的SER性能通常更好，为HuBERT X-Large等大型模型提供了轻量高效的替代方案。

Conclusion: Whisper作为表示提取器在SER任务中具有潜力，基于注意力的池化方法在维度减少方面有效。中间层特征在跨语言SER任务中表现优异，为轻量级SER系统提供了新思路。
...
### [44] [AgenticPay: A Multi-Agent LLM Negotiation System for Buyer-Seller Transactions](https://arxiv.org/abs/2602.06008)
*Xianyang Liu,Shangding Gu,Dawn Song*

Main category: cs.AI

TL;DR: AgenticPay是一个用于多智能体买卖谈判的基准测试和仿真框架，通过自然语言驱动，包含110多个任务，评估语言模型在商业谈判中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体越来越多地需要自主进行谈判、协调和交易，但现有基准测试缺乏评估多智能体之间语言驱动经济交互的原则性设置。

Method: 构建AgenticPay框架，模拟买卖双方拥有私有约束和产品依赖估值的市场环境，支持多轮语言谈判而非单纯数字竞价。框架包含110多个任务，涵盖双边议价到多对多市场，提供结构化动作提取和可行性、效率、福利等评估指标。

Result: 对最先进的专有和开源大语言模型进行基准测试，发现谈判性能存在显著差距，突显了长时程战略推理方面的挑战。
...
TL;DR: 开发基于VR行为数据驱动的离散事件模拟器，用于可扩展评估学校枪击事件中的机器人干预策略


<details>
  <summary>Details</summary>
Motivation: VR虽然能高保真评估学校安全措施，但需要为每个条件招募新参与者，难以进行大规模或迭代评估，尤其在学习有效干预策略时需要大量训练轮次

Method: 开发数据驱动的离散事件模拟器，将枪手移动和区域内行动建模为从VR研究中参与者行为学习到的随机过程，用于评估机器人干预策略

Result: 模拟器能复现关键经验模式，支持对无法直接用人训练的策略进行可扩展评估和学习

Conclusion: 建立了一个从高保真到中保真的模拟工作流，为开发和评估自主学校安全干预提供了可扩展的替代方案
...
### [46] [UniTrack: Differentiable Graph Representation Learning for Multi-Object Tracking](https://arxiv.org/abs/2602.05037)
...
<details>
  <summary>Details</summary>
Motivation: 现有的图基MOT方法需要重新设计跟踪架构，缺乏统一的训练目标来同时优化检测精度、身份保持和时空一致性。需要一种通用、可微分的学习框架，能够无缝集成到现有MOT系统中。

Method: UniTrack采用可微分图表示学习，将检测精度、身份保持和时空一致性整合到单一端到端可训练损失函数中。通过图论方法学习运动连续性和跨帧身份关系的整体表示，作为即插即用组件与现有MOT系统集成。

Result: 在多种跟踪模型和挑战性基准测试中验证，包括Trackformer、MOTR、FairMOT、ByteTrack、GTR和MOTE。身份切换减少达53%，IDF1提升12%。GTR在SportsMOT上获得9.7% MOTA峰值性能提升。

Conclusion: UniTrack作为一种通用图论损失函数，通过统一可微分学习直接优化跟踪特定目标，显著提升多目标跟踪性能，且无需修改现有架构，具有广泛的适用性和兼容性。
...
### [47] [Food Portion Estimation: From Pixels to Calories](https://arxiv.org/abs/2602.05078)
...
<details>
  <summary>Details</summary>
Motivation: 基于图像的膳食评估对于准确便捷地监测个体健康、预防和治疗慢性疾病及肥胖至关重要，但面临从2D图像估计食物三维尺寸的核心挑战。

Method: 探索了多种策略：使用深度图、多视角图像等辅助输入；基于模型的方法如模板匹配；以及深度学习技术，包括单目图像处理和图像与辅助输入的组合方法。

Result: 论文综述了不同方法在准确估计食物分量方面的应用，但没有提供具体的实验结果数据。

Conclusion: 基于图像的膳食评估需要克服2D到3D的尺寸估计难题，多种策略已被开发来解决这一关键限制，深度学习技术为精确预测食物分量提供了重要支持。
...
### [48] [Visual concept ranking uncovers medical shortcuts used by large multimodal models](https://arxiv.org/abs/2602.05096)
*Joseph D. Janizek,Sonnet Xu,Junayd Lateef,Roxana Daneshjou*

Main category: cs.CV

TL;DR: 该论文提出了一种名为视觉概念排序（VCR）的方法，用于识别大型多模态模型中的视觉概念，并用于审计医疗任务中的模型行为，特别是在皮肤病变分类中发现了不同人口亚组间的性能差异。


<details>
  <summary>Details</summary>
Motivation: 在医疗等安全关键领域确保机器学习模型的可靠性需要能够揭示模型缺陷的审计方法。当前缺乏有效方法来识别大型多模态模型中的视觉概念并调查其在医疗任务中的行为表现。

Method: 提出视觉概念排序（VCR）方法，用于识别大型多模态模型中的重要视觉概念。该方法应用于皮肤恶性病变分类任务，并补充了胸部X光片和自然图像的实验。通过演示示例提示模型，观察不同人口亚组间的性能差异，然后应用VCR生成关于视觉特征依赖性的假设，并通过人工干预进行验证。

Result: 研究发现大型多模态模型在医疗任务中表现出不同人口亚组间的意外性能差距。VCR方法能够生成关于视觉特征依赖性的可验证假设，并通过人工干预证实了这些假设的有效性。

Conclusion: VCR方法为审计大型多模态模型在医疗等安全关键领域的可靠性提供了有效工具，能够识别模型中的视觉概念依赖性和潜在偏差，有助于发现和解决模型在临床任务中的性能缺陷。
...
### [49] [ARGaze: Autoregressive Transformers for Online Egocentric Gaze Estimation](https://arxiv.org/abs/2602.05132)
*Jia Li,Wenjie Zhao,Shijian Deng,Bolin Lai,Yuheng Wu,RUijia Chen,Jon E. Froehlich,Yuhang Zhao,Yapeng Tian*

Main category: cs.CV

TL;DR: ARGaze：将第一人称视线估计重新定义为序列预测任务，通过自回归解码器结合当前视觉特征和最近视线历史来实现在线视线估计


<details>
  <summary>Details</summary>
Motivation: 在线第一人称视线估计仅使用过去和当前帧预测相机佩戴者的注视位置，对增强现实和辅助技术至关重要。与第三人称视线估计不同，该设置缺乏明确的头部或眼睛信号，需要从稀疏的间接线索（如手-物交互和显著场景内容）推断当前视觉注意力。作者观察到在目标导向活动中视线表现出强时间连续性：知道一个人最近注视的位置为预测其下一步注视提供了强有力的先验。

Method: 受视觉语言模型中视觉条件自回归解码的启发，提出ARGaze，将视线估计重新定义为序列预测：在每个时间步，一个Transformer解码器通过条件化（i）当前视觉特征和（ii）固定长度的"视线上下文窗口"（包含最近视线目标估计）来预测当前视线。这种设计强制了因果关系，并实现了有界资源的流式推理。

Result: 在多个第一人称基准测试中实现了在线评估下的最先进性能，广泛的消融实验验证了自回归建模与有界视线历史对鲁棒预测的关键作用。

Conclusion: ARGaze通过将视线估计重新定义为序列预测任务，利用视线的时间连续性，结合当前视觉特征和最近视线历史，实现了高效准确的在线第一人称视线估计，为增强现实和辅助技术应用提供了重要技术基础。
...
### [50] [AirGlove: Exploring Egocentric 3D Hand Tracking and Appearance Generalization for Sensing Gloves](https://arxiv.org/abs/2602.05159)
*Wenhui Cui,Ziyi Kou,Chuan Qin,Ergys Ristani,Li Guan*

Main category: cs.CV

TL;DR: 该论文评估了视觉手部追踪模型在传感手套上的性能，发现现有裸手模型因外观差异而表现不佳，提出了AirGlove方法通过有限数据泛化到新手套设计


<details>
  <summary>Details</summary>
Motivation: 传感手套在遥操作和机器人策略学习中很重要，但基于传感器的追踪受信号质量和校准影响。视觉方法在裸手上表现好，但在外观不同的传感手套上性能未充分探索，需要系统评估和解决方案

Method: 提出AirGlove方法，利用现有手套数据学习手套表示，通过有限数据泛化到新手套设计，在零样本和微调设置下评估视觉手部追踪模型

Result: 实验显示现有裸手模型在传感手套上性能显著下降，AirGlove能有效将手部姿态模型泛化到新手套设计，相比其他方案获得显著性能提升

Conclusion: 视觉手部追踪模型在传感手套上存在外观差距导致的性能问题，AirGlove通过泛化手套表示有效解决了这一问题，为传感手套的视觉追踪提供了实用方案
...
### [51] [LOBSTgER-enhance: an underwater image enhancement pipeline](https://arxiv.org/abs/2602.05163)
...
<details>
  <summary>Details</summary>
Motivation: 水下摄影面临对比度降低、空间模糊和波长相关颜色失真等固有挑战，这些效应会掩盖海洋生物的活力，环保摄影师通常需要复杂的后期处理流程来校正这些失真。

Method: 开发图像到图像转换管道，通过引入合成退化管道并学习使用基于扩散的生成来逆转其效应。在Keith Ellenbogen的环保摄影图像小规模高质量数据集上进行训练和评估。

Result: 该方法在合成512x768图像时实现了高感知一致性和强泛化能力，模型参数约1100万，在约2500张图像上从头开始训练。

Conclusion: 提出的基于扩散模型的方法能够有效逆转水下摄影退化效应，为环保摄影师提供高效的图像增强解决方案，在有限数据集上实现了良好的性能。
...
### [52] [PoseGaussian: Pose-Driven Novel View Synthesis for Robust 3D Human Reconstruction](https://arxiv.org/abs/2602.05190)
*Ju Shen,Chen Chen,Tam V. Nguyen,Vijayan K. Asari*

Main category: cs.CV

TL;DR: PoseGaussian：基于姿态引导的高斯溅射框架，用于高保真人体新视角合成，实现100FPS实时渲染


<details>
  <summary>Details</summary>
Motivation: 解决动态人体场景中的挑战，如关节运动和严重自遮挡，现有方法仅将姿态用作条件或变形，缺乏对几何和时序的深度整合

Method: 提出姿态引导的高斯溅射框架，姿态作为结构先验与颜色编码器融合以优化深度估计，同时作为时序线索通过专用姿态编码器增强帧间一致性，构建完全可微的端到端训练管道

Result: 在ZJU-MoCap、THuman2.0和内部数据集上验证，达到SOTA性能：PSNR 30.86、SSIM 0.979、LPIPS 0.028，保持标准高斯溅射效率，实现100FPS实时渲染

Conclusion: PoseGaussian通过将姿态信号深度嵌入几何和时序阶段，显著提升了动态人体新视角合成的鲁棒性、泛化能力和实时性能
...
### [53] [GT-SVJ: Generative-Transformer-Based Self-Supervised Video Judge For Efficient Video Reward Modeling](https://arxiv.org/abs/2602.05202)
*Shivanshu Shekhar,Uttaran Bhattacharya,Raghavendra Addanki,Mehrab Tanjim,Somdeb Sarkhel,Tong Zhang*

Main category: cs.CV

TL;DR: 提出Generative-Transformer-based Self-Supervised Video Judge (GTSVJ)，将视频生成模型重新用作奖励模型，通过对比学习使其能精确评估视频质量，仅需3万人工标注即可达到SOTA性能。
...
Method: 将视频生成模型重新构建为基于能量的模型(EBMs)，通过对比学习目标训练，使其能为高质量视频分配低能量、低质量视频分配高能量。为防止模型利用真实与生成视频的表面差异，设计了三种潜在空间扰动方法生成合成负样本：时间切片、特征交换和帧重排。

Result: 在GenAI-Bench和MonteBench上达到最先进性能，仅使用3万人工标注，比现有VLM方法减少6-65倍标注需求。

Conclusion: 视频生成模型可有效转化为时间感知的奖励模型，通过对比学习和精心设计的合成负样本，能精确评估视频质量，大幅减少人工标注需求。
...
### [54] [Dual-Representation Image Compression at Ultra-Low Bitrates via Explicit Semantics and Implicit Textures](https://arxiv.org/abs/2602.05213)
*Chuqin Zhou,Xiaoyue Ling,Yunuo Chen,Jincheng Dai,Guo Lu,Wenjun Zhang*

Main category: cs.CV

TL;DR: 提出一种统一框架，通过无训练方式结合显式和隐式表示来弥合语义保真度与感知真实性之间的权衡，在超低码率下实现先进的率-感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经编解码器在超低码率下性能显著下降，而基于生成压缩的方法面临语义保真度与感知真实性之间的根本性权衡：显式表示方法保留内容结构但缺乏细粒度纹理，隐式方法能合成视觉可信细节但导致语义漂移。

Method: 提出统一框架，以无训练方式将显式和隐式表示连贯整合：1) 在扩散模型上以显式高级语义为条件；2) 使用反向信道编码隐式传递细粒度细节；3) 引入可插拔编码器，通过调制隐式信息灵活控制失真-感知权衡。

Result: 在Kodak、DIV2K和CLIC2020数据集上实现最先进的率-感知性能，相比DiffC方法在DISTS BD-Rate指标上分别提升29.92%、19.33%和20.89%。

Conclusion: 该框架成功弥合了语义保真度与感知真实性之间的差距，通过显式-隐式表示的协同整合，在超低码率条件下实现了优异的压缩性能，为生成压缩提供了新的解决方案。
...
### [55] [E.M.Ground: A Temporal Grounding Vid-LLM with Holistic Event Perception and Matching](https://arxiv.org/abs/2602.05215)
*Jiahao Nie,Wenbin An,Gongjie Zhang,Yicheng Xu,Yap-Peng Tan,Alex C. Kot,Shijian Lu*

Main category: cs.CV

TL;DR: E.M.Ground是一种新颖的视频大语言模型，通过事件感知方法改进时序视频定位，使用特殊事件令牌、Savitzky-Golay平滑和多粒度帧特征聚合来提升事件匹配精度。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在时序视频定位任务中存在局限性，主要依赖精确时间戳匹配起始和结束帧，未能捕捉事件的语义连续性和完整性，导致定位模糊。

Method: 提出E.M.Ground模型，包含三个关键创新：1) 特殊<event>令牌聚合查询事件所有帧的信息；2) 使用Savitzky-Golay平滑减少时间戳间令牌-帧相似度的噪声；3) 多粒度帧特征聚合增强匹配可靠性和时序理解。

Result: 在基准数据集上的大量实验表明，E.M.Ground始终显著优于最先进的视频大语言模型。

Conclusion: E.M.Ground通过整体和连贯的事件感知方法，有效解决了时序视频定位中的语义连续性问题，提升了定位精度。
...
### [56] [Cross-Domain Few-Shot Segmentation via Multi-view Progressive Adaptation](https://arxiv.org/abs/2602.05217)
*Jiahao Nie,Guanqiao Fu,Wenbin An,Yap-Peng Tan,Alex C. Kot,Shijian Lu*

Main category: cs.CV

TL;DR: MPA通过混合渐进增强和双链多视图预测，从数据和策略两个角度逐步将少样本分割能力适应到目标域，显著提升跨域少样本分割性能


<details>
  <summary>Details</summary>
Motivation: 现有跨域少样本分割方法由于目标域样本数量有限、多样性不足，以及源域模型在目标域初始少样本能力较弱且存在显著域差异，导致目标样本利用不充分且适应效果受限

Method: 提出多视图渐进适应(MPA)：1) 数据角度：混合渐进增强通过累积强增强逐步生成更多样复杂的视图，创建渐进挑战性学习场景；2) 策略角度：双链多视图预测通过顺序和并行学习路径在广泛监督下充分利用这些渐进复杂视图，通过跨多样复杂视图的预测一致性实现稳健准确适应

Result: 大量实验表明MPA能有效将少样本能力适应到目标域，以较大优势(+7.0%)超越现有最先进方法

Conclusion: MPA通过从数据和策略两个角度进行渐进适应，解决了跨域少样本分割中目标样本有限、多样性不足以及域差异大的挑战，实现了对目标域的稳健准确适应
...
### [57] [Boosting SAM for Cross-Domain Few-Shot Segmentation via Conditional Point Sparsification](https://arxiv.org/abs/2602.05218)
...
Method: 提出条件点稀疏化(CPS)方法，利用参考图像的ground-truth掩码自适应地稀疏化密集匹配点，从而更准确地引导SAM进行跨域图像分割

Result: 在多个跨域少样本分割数据集上的实验表明，CPS优于现有的训练无关SAM方法

Conclusion: 通过自适应稀疏化匹配点，CPS能够有效应对跨域少样本分割中的域偏移问题，提升SAM在医学和卫星图像等跨域场景中的分割性能
...
### [58] [PatchFlow: Leveraging a Flow-Based Model with Patch Features](https://arxiv.org/abs/2602.05238)
*Boxiang Zhang,Baijian Yang,Xiaoming Wang,Corey Vian*

Main category: cs.CV

TL;DR: 提出一种结合局部邻域感知补丁特征与归一化流模型的压铸表面缺陷检测方法，通过适配器模块提升预训练特征提取器与工业图像的兼容性，在多个数据集上实现优于现有方法的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 压铸工艺在各行业中至关重要，但表面缺陷严重影响质量控制。现有计算机视觉方法在自动化缺陷检测方面仍有改进空间，特别是在预训练特征提取器与工业产品图像之间的兼容性问题上。

Method: 结合局部邻域感知补丁特征与归一化流模型，引入适配器模块桥接通用预训练特征提取器与工业产品图像之间的差距，实现无需异常样本训练的异常检测。

Result: 在MVTec AD数据集上图像级AUROC达99.28%，错误率降低20%；在VisA数据集上AUROC达96.48%，错误率降低28.2%；在专有压铸数据集上异常检测准确率达95.77%，均无需异常训练样本。

Conclusion: 该方法展示了计算机视觉和深度学习技术在提升压铸行业检测能力方面的潜力，通过创新的特征适配机制实现了高效准确的异常检测。
...
### [59] [Active Label Cleaning for Reliable Detection of Electron Dense Deposits in Transmission Electron Microscopy Images](https://arxiv.org/abs/2602.05250)
...
<details>
  <summary>Details</summary>
Motivation: 肾小球疾病中电子致密沉积物（EDD）的自动检测受到高质量标注数据稀缺的限制。众包虽能降低标注成本，但会引入标签噪声，需要有效的去噪方法
...
### [60] [Magic-MM-Embedding: Towards Visual-Token-Efficient Universal Multimodal Embedding with MLLMs](https://arxiv.org/abs/2602.05275)
...
Method: 基于两大支柱：1）采用视觉令牌压缩的高效MLLM架构，大幅降低推理延迟和内存占用；2）多阶段渐进训练策略，包括持续预训练恢复能力、大规模对比预训练增强判别力、以及基于MLLM-as-a-Judge的任务感知微调

Result: 模型在全面实验中大幅超越现有方法，同时推理效率更高

Conclusion: Magic-MM-Embedding系列模型实现了高效性和SOTA性能的平衡，为通用多模态嵌入提供了实用解决方案
...
### [61] [Fast-SAM3D: 3Dfy Anything in Images but Faster](https://arxiv.org/abs/2602.05293)
...
<details>
  <summary>Details</summary>
Motivation: SAM3D虽然能够实现复杂场景的可扩展开放世界3D重建，但其推理延迟过高阻碍了实际部署。现有通用加速策略在该场景下表现脆弱，需要针对其特有的异构性进行优化。

Method: 提出训练无关的Fast-SAM3D框架，包含三个异构感知机制：1）模态感知步长缓存，解耦结构演化与敏感布局更新；2）联合时空令牌雕刻，将细化集中在高熵区域；3）频谱感知令牌聚合，自适应调整解码分辨率。

Result: 实验表明Fast-SAM3D在保持重建质量的同时，实现了最高2.67倍的端到端加速，为高效单视图3D生成建立了新的帕累托前沿。

Conclusion: 通过系统分析SAM3D的推理动态并针对其多级异构性设计专门优化，Fast-SAM3D成功解决了推理延迟问题，为实际部署提供了可行方案。
...
### [62] [FlashBlock: Attention Caching for Efficient Long-Context Block Diffusion](https://arxiv.org/abs/2602.05305)
*Zhuokun Chen,Jianfei Cai,Bohan Zhuang*

Main category: cs.CV

TL;DR: FlashBlock通过重用块外稳定注意力输出来减少长上下文扩散模型中的计算开销，提升推理效率


<details>
  <summary>Details</summary>
Motivation: 块扩散在长上下文场景下仍存在显著开销，主要源于对不断增长的KV缓存重复计算注意力。研究发现块扩散中存在跨步注意力冗余特性，块外注意力输出在不同扩散步间保持稳定，而块内注意力变化显著。

Method: 提出FlashBlock缓存块外注意力机制，重用稳定的注意力输出，减少注意力计算和KV缓存访问，且不修改扩散过程。该方法与稀疏注意力正交，可作为补充残差重用策略结合使用。

Result: 在扩散语言模型和视频生成实验中，实现了最高1.44倍的token吞吐量提升和最高1.6倍的注意力时间减少，对生成质量影响可忽略。

Conclusion: FlashBlock通过利用块扩散中的跨步注意力冗余特性，有效提升了长上下文生成任务的推理效率，且与现有优化技术兼容。
...
### [63] [Consistency-Preserving Concept Erasure via Unsafe-Safe Pairing and Directional Fisher-weighted Adaptation](https://arxiv.org/abs/2602.05339)
...
Method: 提出PAIR框架：1) 从不安全输入生成结构语义保真的安全对应物，形成配对多模态数据；2) 配对语义重新对齐目标，使用不安全-安全对将目标概念映射到语义对齐的安全锚点；3) Fisher加权初始化DoRA，使用不安全-安全对初始化参数高效的低秩适应矩阵，鼓励生成安全替代方案同时选择性抑制不安全概念

Result: 大量实验表明，该方法显著优于现有最先进基线，实现了有效的概念擦除，同时保持了结构完整性、语义连贯性和生成质量

Conclusion: PAIR框架通过将概念擦除重构为一致性保持的语义重新对齐，使用不安全-安全对实现了细粒度擦除，在移除目标概念的同时保持了整体语义一致性
...
### [64] [Learning with Adaptive Prototype Manifolds for Out-of-Distribution Detection](https://arxiv.org/abs/2602.05349)
*Ningkang Peng,JiuTao Zhou,Yuhao Zhang,Xiaoqian Peng,Qianfeng Yu,Linjing Qian,Tingyu Lu,Yi Chen,Yanhui Gu*

Main category: cs.CV

TL;DR: APEX提出自适应原型OOD检测框架，通过两阶段修复解决现有原型方法的静态同质性假设和学习-推断断开问题，在CIFAR-100等基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于原型的表示学习方法在OOD检测中表现出色，但存在两个根本性缺陷：1) 静态同质性假设（所有类别使用固定的表示资源）；2) 学习-推断断开（推断时丢弃丰富的原型质量知识）。这些缺陷限制了模型的容量和性能。

Method: 提出APEX框架，通过两阶段修复优化学习到的特征流形：1) 自适应原型流形（APM），利用最小描述长度原则自动确定每个类别的最优原型复杂度K_c*，从根本上解决原型碰撞问题；2) 后验感知OOD评分机制（PAOS），量化原型质量（内聚性和分离性）以弥合学习-推断断开。

Result: 在CIFAR-100等基准上的综合实验验证了方法的优越性，APEX实现了新的最先进性能。

Conclusion: APEX通过解决原型方法的两个根本性缺陷，显著提升了OOD检测性能，为机器学习模型在现实世界中的安全部署提供了更可靠的解决方案。
...
### [65] [Multimodal Latent Reasoning via Hierarchical Visual Cues Injection](https://arxiv.org/abs/2602.05359)
*Yiming Zhang,Qiangyu Yan,Borui Jiang,Kai Han*

Main category: cs.CV

TL;DR: HIVE框架通过将分层视觉线索注入潜在空间，实现多模态大语言模型的"慢思考"推理，避免依赖显式语言链式思维，提升复杂场景理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型采用"快思考"范式，依赖端到端生成或显式语言链式思维，存在效率低、冗长和易产生幻觉的问题。需要发展在潜在空间中整合多模态信号的稳健推理方法。

Method: 提出HIVE框架：递归扩展Transformer块创建内部循环进行迭代推理精炼；通过注入式方法将分层视觉线索（从全局场景上下文到细粒度区域细节）直接融入模型潜在表示；在潜在对齐空间中执行多步推理。

Result: 评估表明：结合视觉知识的测试时缩放有效；整合分层信息显著增强模型对复杂场景的理解能力。

Conclusion: HIVE框架通过将分层视觉线索注入潜在空间，实现了多模态大语言模型的"慢思考"推理，避免了传统语言链式思维的局限性，提升了复杂场景下的推理性能。
...
### [66] [Breaking Semantic Hegemony: Decoupling Principal and Residual Subspaces for Generalized OOD Detection](https://arxiv.org/abs/2602.05360)
*Ningkang Peng,Xiaoqian Peng,Yuhao Zhang,Qianfeng Yu,Feng Xing,Peirong Ma,Xichen Yang,Yi Chen,Tingyu Lu,Yanhui Gu*

Main category: cs.CV

TL;DR: 论文揭示了OOD检测中的"简单性悖论"：现有SOTA模型能区分语义细微的OOD样本，但对结构差异明显但语义简单的样本或高频传感器噪声存在"几何盲视"。作者提出D-KNN框架，通过正交分解分离语义成分和结构残差，在CIFAR和ImageNet基准上实现新SOTA。


<details>
  <summary>Details</summary>
Motivation: 发现现有基于特征的OOD检测方法存在"简单性悖论"：模型对语义细微的OOD样本敏感，但对结构差异明显但语义简单的样本或高频传感器噪声存在"几何盲视"。作者将这一现象归因于深度特征空间中的"语义霸权"，并通过神经坍缩理论揭示其数学本质。
...
Conclusion: 论文揭示了OOD检测中的简单性悖论及其数学本质，提出D-KNN框架通过几何解耦和双空间校准有效解决了语义霸权问题，显著提升了模型对结构分布偏移的敏感性，在多个基准上实现了新的SOTA性能。
...
### [67] [Imagine a City: CityGenAgent for Procedural 3D City Generation](https://arxiv.org/abs/2602.05362)
*Zishan Liu,Zecong Tang,RuoCheng Wu,Xinzhe Zheng,Jingyu Hu,Ka-Hei Hui,Haoran Xie,Bo Dai,Zhengzhe Liu*

Main category: cs.CV

TL;DR: CityGenAgent：基于自然语言的层次化程序化3D城市生成框架，通过Block Program和Building Program分解生成过程，采用SFT+RL两阶段学习策略确保结构正确性和语义对齐，支持自然语言编辑和操作。


<details>
  <summary>Details</summary>
Motivation: 现有3D城市生成方法在高质量资产创建、可控性和操作方面存在不足，需要开发能够生成高保真、可控制且支持自然语言交互的城市生成系统。
...
Conclusion: CityGenAgent通过层次化程序化生成和两阶段学习策略，实现了高质量、可控制且支持自然语言编辑的3D城市生成，在语义对齐、视觉质量和可控性方面优于现有方法。
...
### [68] [SAIL: Self-Amplified Iterative Learning for Diffusion Model Alignment with Minimal Human Feedback](https://arxiv.org/abs/2602.05380)
*Xiaoxuan He,Siming Fu,Wanli Li,Zhiyuan Li,Dacheng Yin,Kang Rong,Fengyun Rao,Bo Zhang*

Main category: cs.CV

TL;DR: SAIL框架通过自增强迭代学习，仅需少量人工偏好标注即可实现扩散模型的对齐，无需外部奖励模型


<details>
  <summary>Details</summary>
Motivation: 扩散模型对齐面临挑战：奖励模型难以获取，大规模偏好数据集收集成本高昂。探索能否仅用最少人工反馈，通过挖掘扩散模型自身潜力实现有效对齐

Method: 提出SAIL框架，让扩散模型作为自己的老师进行迭代自改进。从少量人工标注偏好对开始，以闭环方式运行：模型生成多样样本，基于演化理解进行自标注偏好，使用自增强数据集进行精炼。引入排名偏好混合策略平衡探索与初始人类先验

Result: SAIL在多个基准测试中持续优于最先进方法，仅需现有方法6%的偏好数据，揭示扩散模型具有显著自改进能力

Conclusion: 扩散模型拥有卓越的自改进能力，当被适当利用时，可以有效替代大规模人工标注和外部奖励模型
...
### [69] [VRIQ: Benchmarking and Analyzing Visual-Reasoning IQ of VLMs](https://arxiv.org/abs/2602.05382)
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的快速发展，需要评估它们是否能够可靠地进行非语言推理。现有基准测试可能无法充分评估模型的视觉推理能力，因此需要设计专门的基准来分析和诊断模型在视觉推理方面的弱点。

Method: 引入VRIQ（视觉推理智商）基准，包含抽象谜题风格任务和自然图像推理任务。使用诊断探针分析失败原因，区分感知失败和推理失败。设计细粒度诊断探针问题，针对特定感知类别（形状、计数、位置、3D/深度等）。

Result: 在抽象谜题任务上，模型性能接近随机水平（平均准确率约28%）；自然图像任务表现稍好但仍较弱（45%准确率）。工具增强推理仅带来适度改进。诊断分析显示：56%的失败来自感知缺陷，43%来自感知和推理双重缺陷，仅1%来自纯推理缺陷。特定感知类别（如形状、计数等）导致更多失败。
...
### [70] [Dolphin-v2: Universal Document Parsing via Scalable Anchor Prompting](https://arxiv.org/abs/2602.05384)
...
Method: Dolphin-v2采用两阶段架构：第一阶段联合进行文档类型分类（数字文档vs拍摄文档）和布局分析，对数字文档进行更细粒度的元素检测和阅读顺序预测；第二阶段采用混合解析策略：拍摄文档作为完整页面进行整体解析以处理几何扭曲，数字文档则基于检测到的布局锚点进行元素级并行解析。

Result: 在DocPTBench、OmniDocBench和自建的RealDoc-160基准测试中，Dolphin-v2相比原始Dolphin有显著改进：在具有挑战性的OmniDocBench上整体提升14.78分，拍摄文档错误减少91%，同时通过并行处理保持高效推理。

Conclusion: Dolphin-v2通过引入混合解析策略、更细粒度的元素检测和语义属性提取，显著提升了文档解析能力，特别是在处理拍摄文档和复杂数字文档方面，同时保持了系统的高效性和可扩展性。
...
### [71] [Parallel Swin Transformer-Enhanced 3D MRI-to-CT Synthesis for MRI-Only Radiotherapy Planning](https://arxiv.org/abs/2602.05387)
*Zolnamar Dorjsembe,Hung-Yi Chen,Furen Xiao,Hsing-Kuo Pao*

Main category: cs.CV

TL;DR: 提出一种用于MRI到CT合成的3D深度学习架构，结合卷积编码和双Swin Transformer分支，提升解剖结构保真度和剂量计算精度


<details>
  <summary>Details</summary>
Motivation: MRI缺乏电子密度信息无法直接用于放疗剂量计算，现有工作流依赖MRI-CT联合采集导致配准不确定性和流程复杂性增加，需要高质量的合成CT生成方法

Method: 提出Parallel Swin Transformer-Enhanced Med2Transformer，一种3D架构，集成卷积编码与双Swin Transformer分支，通过多尺度移位窗口注意力和分层特征聚合来建模局部解剖细节和长距离上下文依赖

Result: 在公开和临床数据集上实验显示，相比基线方法具有更高的图像相似度和几何精度，剂量学评估显示临床可接受的性能，平均靶区剂量误差为1.69%

Conclusion: 所提出的架构在合成CT生成方面表现出色，能够准确建模MRI-CT的非线性关系，为MRI-only放疗计划提供了可行解决方案
...
### [72] [Dataset Distillation via Relative Distribution Matching and Cognitive Heritage](https://arxiv.org/abs/2602.05391)
...
<details>
  <summary>Details</summary>
Motivation: 现有线性梯度匹配方法在分类任务中使用预训练自监督模型作为骨干时，需要加载数千张真实图像并在每个蒸馏步骤对合成图像应用多轮可微分增强，导致巨大的计算和内存开销。

Method: 提出统计流匹配框架，通过对齐原始数据中从目标类别中心到非目标类别中心的恒定统计流来优化合成图像；同时提出分类器继承策略，重用原始数据集上训练的分类器进行推理，仅需极轻量的线性投影器和少量存储。

Result: 方法性能达到或优于最先进方法，GPU内存使用降低10倍，运行时间缩短4倍；分类器继承策略在仅需极轻量线性投影器和少量存储的情况下实现显著的性能提升。

Conclusion: 统计流匹配为数据集蒸馏提供了一种稳定高效的监督学习框架，显著降低了计算和内存开销，同时通过分类器继承策略进一步提升了性能，为实际应用提供了实用解决方案。
...
### [73] [Explainable Pathomics Feature Visualization via Correlation-aware Conditional Feature Editing](https://arxiv.org/abs/2602.05397)
...
TL;DR: 提出MAD框架，通过流形感知的扩散模型实现可控且生物学合理的细胞核编辑，解决传统方法因忽略特征相关性而产生非真实伪影的问题。


<details>
  <summary>Details</summary>
Motivation: 病理组学特征虽然提供丰富量化信息，但许多特征难以解释且在不同临床背景下适用性有限。现有条件扩散模型假设特征独立，而病理组学特征本质相关，导致编辑单个特征时可能偏离生物流形产生非真实伪影。
...
Conclusion: MAD框架通过流形感知的扩散模型实现了可控且生物学合理的细胞核编辑，解决了特征相关性问题，为病理组学特征的可解释性提供了有效方法。
...
### [74] [VMF-GOS: Geometry-guided virtual Outlier Synthesis for Long-Tailed OOD Detection](https://arxiv.org/abs/2602.05415)
...
<details>
  <summary>Details</summary>
Motivation: 长尾分布下OOD检测面临尾部类别样本稀缺导致特征空间决策边界模糊的问题。现有SOTA方法依赖大规模外部数据集进行异常暴露，但实际部署中数据获取成本高且存在隐私敏感问题，需要消除对外部数据的依赖。

Method: 提出几何引导的虚拟异常合成(GOS)策略：1) 使用von Mises-Fisher分布对超球面统计特性建模；2) 在特征空间中定位低似然环形区域；3) 在该区域进行方向性虚拟异常采样。同时引入双粒度语义损失(DGS)，利用对比学习最大化ID特征与合成边界异常之间的区分度。

Result: 在CIFAR-LT等基准测试上的大量实验表明，该方法优于使用外部真实图像的SOTA方法，实现了无需外部数据依赖下的优越检测性能。

Conclusion: 提出的数据无关框架通过几何引导的虚拟异常合成和双粒度语义损失，成功消除了对外部数据集的依赖，在长尾分布下实现了优于现有方法的OOD检测性能，具有实际部署的可行性。
...
### [75] [Disco: Densely-overlapping Cell Instance Segmentation via Adjacency-aware Collaborative Coloring](https://arxiv.org/abs/2602.05420)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于轮廓检测和距离映射的方法在处理复杂密集细胞区域时面临挑战，图染色方法在真实场景中密集重叠和复杂拓扑的有效性尚未验证。发现真实细胞邻接图多为非二分图，存在大量奇数长度循环，使简单2-染色理论不足。

Method: 提出Disco框架：1) 显式标记策略：递归分解细胞图，隔离"冲突集"，将拓扑挑战转化为可学习的分类任务；2) 隐式消歧机制：通过强制不同实例间的特征差异性，解决冲突区域的歧义，学习可分离的特征表示。

Result: 发布大规模数据集GBC-FS 2025，包含高度复杂密集的亚细胞核排列。首次系统分析四个不同数据集的细胞邻接图色度特性，发现大多数真实细胞图是非二分图，奇数长度循环（主要是三角形）普遍存在。

Conclusion: Disco框架通过邻接感知协作染色有效解决密集重叠细胞实例分割问题，结合数据驱动的拓扑标记策略和约束深度学习系统，能够处理复杂邻接冲突，为数字病理分析提供新解决方案。
...
### [76] [NeVStereo: A NeRF-Driven NVS-Stereo Architecture for High-Fidelity 3D Tasks](https://arxiv.org/abs/2602.05423)
...
<details>
  <summary>Details</summary>
Motivation: 现有密集3D重建方法存在局限性：前馈系统（如VGGT、pi3）专注于端到端匹配和几何预测但不输出新视角合成；基于神经渲染的方法能提供高质量渲染和几何细节，但通常假设固定相机位姿且对位姿误差敏感。因此，需要单一框架能够从随意拍摄的视图中同时提供准确的位姿、可靠的深度、高质量的渲染和精确的3D表面。

Method: NeVStereo结合了NeRF驱动的新视角合成与立体视觉架构：1) 使用NeRF-based NVS生成适合立体匹配的渲染；2) 置信度引导的多视角深度估计；3) NeRF耦合的束调整进行位姿优化；4) 迭代优化阶段同时更新深度和辐射场以提高几何一致性。该设计缓解了NeRF常见的表面堆叠、伪影和位姿-深度耦合问题。

Result: 在室内、室外、桌面和航空基准测试中，NeVStereo展现出持续强大的零样本性能：深度误差降低达36%，位姿精度提升10.4%，新视角合成保真度提高4.5%，网格质量达到SOTA水平（F1 91.93%，Chamfer距离4.35 mm）。

Conclusion: NeVStereo成功实现了从多视角RGB输入中联合输出相机位姿、多视角深度、新视角合成和表面重建的目标，通过NeRF与立体视觉的结合有效解决了现有方法的局限性，在多个基准测试中表现出优越性能。
...
### [77] [LD-SLRO: Latent Diffusion Structured Light for 3-D Reconstruction of Highly Reflective Objects](https://arxiv.org/abs/2602.05434)
*Sanghoon Jeon,Gihyun Jung,Suhyeon Ka,Jae-Sang Hyun*

Main category: cs.CV

TL;DR: 提出基于潜在扩散模型的LD-SLRO方法，用于高反射率、低粗糙度物体的条纹投影轮廓测量，通过抑制镜面反射伪影并恢复丢失的条纹信息，显著提升3D重建精度。


<details>
  <summary>Details</summary>
Motivation: 高反射率、低表面粗糙度物体的3D重建在条纹投影轮廓测量中面临重大挑战，镜面反射和间接照明会导致投影条纹图案严重失真或丢失。

Method: 提出LD-SLRO方法：首先对高反射表面采集的相移条纹图像进行编码提取潜在表示；然后将这些特征作为条件输入到潜在扩散模型中，概率性地抑制反射伪影并恢复丢失的条纹信息；包含镜面反射编码器、时变通道仿射层和注意力模块等组件。

Result: 实验结果表明，该方法在条纹质量和3D重建精度上均优于现有方法，将平均均方根误差从1.8176 mm降低到0.9619 mm。

Conclusion: LD-SLRO方法有效解决了高反射表面3D重建中的条纹失真问题，通过潜在扩散模型实现了高质量的条纹恢复和精确的3D重建。
...
### [78] [Stable Velocity: A Variance Perspective on Flow Matching](https://arxiv.org/abs/2602.05435)
...
Method: 1) Stable Velocity Matching (StableVM)：无偏方差减少目标；2) Variance-Aware Representation Alignment (VA-REPA)：在低方差区域自适应增强辅助监督；3) Stable Velocity Sampling (StableVS)：利用低方差区域动力学闭式简化实现免微调加速采样。

Result: 在ImageNet 256×256及大型预训练模型（SD3.5、Flux、Qwen-Image、Wan2.2）上验证，训练效率显著提升，低方差区域采样速度提高2倍以上且不损失样本质量。

Conclusion: 通过方差分析提出的Stable Velocity框架统一改进了流匹配的训练和采样，在保持样本质量的同时大幅提升效率，为扩散模型的实际应用提供了有效解决方案。
...
### [79] [Synthetic Defect Geometries of Cast Metal Objects Modeled via 2d Voronoi Tessellations](https://arxiv.org/abs/2602.05440)
*Natascha Jeziorski,Petra Gospodnetić,Claudia Redenbach*

Main category: cs.CV

TL;DR: 提出参数化方法建模3D网格缺陷模型，结合数字孪生和蒙特卡洛仿真生成合成数据用于无损检测的自动化缺陷检测


<details>
  <summary>Details</summary>
Motivation: 工业缺陷检测对质量控制至关重要，但真实缺陷数据获取困难且标注成本高。合成数据可提供足够数量和质量的训练数据，特别是对于罕见缺陷类型。

Method: 1) 开发参数化方法建模各种缺陷类型的3D网格对象；2) 将缺陷模型添加到对象几何中创建合成缺陷对象；3) 使用基于物理的蒙特卡洛仿真生成模拟真实检测数据的合成数据；4) 可并行生成像素级完美标注

Result: 能够生成可变且任意大的合成数据集，包含足够数量的罕见缺陷，提供像素级完美标注，适用于各种无损检测方法

Conclusion: 参数化缺陷建模结合数字孪生和物理仿真为自动化缺陷检测提供了可控、可扩展的合成数据生成方案，特别适用于机器学习训练，可推广到多种无损检测方法
...
### [80] [Attention Retention for Continual Learning with Vision Transformers](https://arxiv.org/abs/2602.05454)
*Yue Lu,Xiangyu Zhou,Shizhou Zhang,Yinghui Xing,Guoqiang Liang,Wencong Zhang*

Main category: cs.CV

TL;DR: 提出了一种基于注意力保留的持续学习方法，通过梯度掩码机制约束Vision Transformers中的注意力漂移，从而缓解灾难性遗忘问题。
...
Method: 提出注意力保留框架，通过两步梯度修改过程：1）使用层间展开机制提取先前任务的注意力图，生成实例自适应二元掩码；2）学习新任务时，应用这些掩码将先前注意力区域的梯度置零，防止已学习视觉概念被破坏。为兼容现代优化器，通过按比例缩放参数更新来保持相对幅度。

Result: 实验和可视化结果表明该方法能有效缓解灾难性遗忘并保留视觉概念，在多种持续学习场景中实现了最先进的性能，并展现出强大的泛化能力。

Conclusion: 注意力漂移是Vision Transformers中灾难性遗忘的关键因素，提出的注意力保留框架通过梯度掩码机制成功约束了注意力漂移，为持续学习提供了有效的解决方案。
...
### [81] [MerNav: A Highly Generalizable Memory-Execute-Review Framework for Zero-Shot Object Goal Navigation](https://arxiv.org/abs/2602.05467)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法在成功率和泛化能力之间存在权衡：有监督微调方法成功率较高但泛化差，无训练方法泛化好但成功率低，难以同时获得两者优势。

Method: 提出Memory-Execute-Review三层框架：1) 分层记忆模块提供信息支持；2) 执行模块进行常规决策和行动；3) 回顾模块处理异常情况并纠正行为。

Result: 在4个数据集上，无训练设置下平均成功率绝对提升7%，零样本设置下提升5%。在HM3D_v0.1和HM3D_OVON数据集上，零样本设置下成功率分别提升8%和6%。在MP3D和HM3D_OVON数据集上，不仅超越所有无训练方法，还超越所有有监督微调方法。

Conclusion: Memory-Execute-Review框架有效解决了视觉语言导航中成功率和泛化能力的权衡问题，在多个基准测试中实现了全面领先，为具身智能提供了新的解决方案。
...
### [82] [SOMA-1M: A Large-Scale SAR-Optical Multi-resolution Alignment Dataset for Multi-Task Remote Sensing](https://arxiv.org/abs/2602.05480)
*Peihao Wu,Yongxiang Yao,Yi Wan,Wenfei Zhang,Ruipeng Zhao,Jiayuan Li,Yongjun Zhang*

Main category: cs.CV

TL;DR: SOMA-1M是一个包含130万对像素级精确对齐的SAR-光学图像对的大规模多分辨率数据集，支持多尺度基础模型训练，涵盖12种土地覆盖类型，分辨率从0.5米到10米。


<details>
  <summary>Details</summary>
Motivation: 现有SAR-光学基准数据集存在单空间分辨率、数据规模不足、对齐精度低等局限性，无法有效支持多尺度基础模型的训练和泛化，需要构建更高质量的对齐数据集。

Method: 构建了SOMA-1M数据集，整合Sentinel-1、PIESAT-1、Capella Space和Google Earth的影像，采用从粗到精的图像匹配框架确保像素级对齐，包含12种典型土地覆盖类别。

Result: 在SOMA-1M上进行监督训练显著提升了图像匹配、图像融合、SAR辅助去云和跨模态翻译四个层次视觉任务的性能，其中多模态遥感图像匹配达到了当前SOTA水平。

Conclusion: SOMA-1M作为基础资源为鲁棒的多模态算法和遥感基础模型提供了支持，数据集将公开发布以促进相关研究发展。
...
### [83] [Feature points evaluation on omnidirectional vision with a photorealistic fisheye sequence -- A report on experiments done in 2014](https://arxiv.org/abs/2602.05487)
*Julien Moreau,S. Ambellouis,Yassine Ruichek*

Main category: cs.CV

TL;DR: 该报告提供了一个用于鱼眼图像特征检测和描述符评估的数据集PFSeq，并进行了综合实验，旨在解决鱼眼相机自标定中的"鸡与蛋"问题。


<details>
  <summary>Details</summary>
Motivation: 解决鱼眼相机自标定中的"鸡与蛋"问题：无法利用精确投影模型进行最优特征检测，同时又需要良好特征来执行相机标定。研究旨在找到适用于安装在汽车顶部、指向天顶的鱼眼相机的最佳特征检测器和描述符，以支持后续的鱼眼视觉里程计和城市场景立体视觉。

Method: 提供了PFSeq（Photorealistic Fisheye Sequence）数据集，包含详细的文献综述和综合实验。该研究是2014年博士论文的一部分，采用实验方法评估不同特征检测器和描述符在鱼眼图像上的性能，特别关注自标定应用场景。

Result: 提供了PFSeq数据集（可通过DOI: 10.57745/DYIVVU访问），并进行了全面的特征检测器和描述符评估实验。报告作为未发表的草案，实验基于2014年的技术水平，未与专门为全向图像设计的算法进行比较。

Conclusion: 该报告提供了鱼眼图像特征检测和描述符评估的数据集和实验框架，但作为2014年的未发表草案，其技术已过时，未包含与全向图像专用算法的比较，也未经过同行评审。报告主要价值在于提供PFSeq数据集和解决鱼眼相机自标定中"鸡与蛋"问题的实验方法。
...
### [84] [VGGT-Motion: Motion-Aware Calibration-Free Monocular SLAM for Long-Range Consistency](https://arxiv.org/abs/2602.05508)
*Zhuang Xiong,Chen Zhang,Qingshan Xu,Wenbing Tao*

Main category: cs.CV

TL;DR: VGGT-Motion是一种无需标定的单目SLAM系统，通过运动感知子图构建和锚点驱动的Sim(3)配准策略，解决了长序列中的尺度漂移问题，实现了千米级轨迹的高效全局一致性。
...
Conclusion: VGGT-Motion通过创新的运动感知子图构建和高效的全局一致性维护机制，有效解决了长序列无标定单目SLAM中的尺度漂移问题，实现了千米级轨迹的鲁棒和高效处理。
...
### [85] [SSG: Scaled Spatial Guidance for Multi-Scale Visual Autoregressive Generation](https://arxiv.org/abs/2602.05534)
*Youngwoo Shin,Jiwan Hur,Junmo Kim*

Main category: cs.CV

TL;DR: 提出SSG（Scaled Spatial Guidance）方法，通过训练无关的推理时引导技术，解决视觉自回归模型中层次结构漂移问题，提升图像生成质量与多样性


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型通过下一尺度预测生成图像，但在推理时由于有限容量和累积误差，模型的粗到细层次结构可能发生漂移，导致训练-推理不一致

Method: 提出SSG（Scaled Spatial Guidance）训练无关的推理时引导技术，强调目标高频信号（语义残差），通过DSE（Discrete Spatial Enhancement）频率域处理获得更精细的语义残差，引导生成过程保持预期的层次结构

Result: SSG在多种VAR模型上均能一致提升生成保真度和多样性，同时保持低延迟，揭示了粗到细图像生成中未开发的效率潜力

Conclusion: 从信息论角度分析VAR模型层次漂移问题，提出的SSG方法能有效缓解训练-推理不一致，提升图像生成质量，且具有广泛适用性
...
### [86] [A Comparative Study of 3D Person Detection: Sensor Modalities and Robustness in Diverse Indoor and Outdoor Environments](https://arxiv.org/abs/2602.05538)
*Malaz Tamim,Andrea Matic-Flierl,Karsten Roscher*

Main category: cs.CV

TL;DR: 系统评估了相机、LiDAR及融合方法在3D行人检测中的性能，发现融合方法在挑战性场景中表现最佳，但对传感器错位敏感


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注自动驾驶场景，本文旨在评估不同传感器模态（相机、LiDAR、融合）在多样化室内外场景中的3D行人检测性能与鲁棒性

Method: 使用JRDB数据集，比较三种代表性模型：BEVDepth（相机）、PointPillars（LiDAR）和DAL（相机-LiDAR融合），分析在不同遮挡程度和距离下的性能，并测试对传感器损坏和错位的鲁棒性

Result: 融合方法在所有场景中表现最优，尤其在挑战性场景中；DAL对传感器错位和某些LiDAR损坏敏感；相机方法性能最低，对遮挡、距离和噪声最敏感
...
### [87] [FastVMT: Eliminating Redundancy in Video Motion Transfer](https://arxiv.org/abs/2602.05551)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Diffusion Transformer的视频运动迁移方法存在计算效率低下的问题。虽然已有方法尝试加速DiT计算，但未能解决结构性的效率瓶颈。本文识别出两种计算冗余：运动冗余（帧间运动小而平滑，但通用DiT架构未利用这一特性）和梯度冗余（沿扩散轨迹的梯度变化缓慢）。

Method: 1. 针对运动冗余：通过掩码注意力层，将交互限制在局部邻域内，避免为距离较远的图像区域计算不必要的交互权重。2. 针对梯度冗余：设计优化方案，重用先前扩散步骤的梯度，跳过不必要的梯度计算。

Result: FastVMT在平均情况下实现了3.43倍的加速，同时没有降低生成视频的视觉保真度或时间一致性。
...
### [88] [IndustryShapes: An RGB-D Benchmark dataset for 6D object pose estimation of industrial assembly components and tools](https://arxiv.org/abs/2602.05555)
...
Method: 创建了包含五种具有挑战性属性的新物体类型的IndustryShapes数据集，在真实工业装配环境中捕获。数据集分为经典集和扩展集两部分：经典集包含4.6k图像和6k标注姿态；扩展集提供额外数据模态以支持模型无关和序列方法的评估，首次提供RGB-D静态上机序列。

Result: 数据集包含从简单到复杂的多样化场景，包括单个和多个物体，以及同一物体的多个实例。通过对代表性最先进方法的评估，包括实例基础和新物体6D姿态估计、物体检测和分割，表明该领域仍有改进空间。

Conclusion: IndustryShapes为工业机器人领域的6D姿态估计方法提供了一个真实且应用相关的基准测试平台，填补了实验室研究与实际部署之间的空白，是该领域首个提供RGB-D静态上机序列的数据集。
...
### [89] [PIRATR: Parametric Object Inference for Robotic Applications with Transformers in 3D Point Clouds](https://arxiv.org/abs/2602.05557)
*Michael Schwingshackl,Fabio F. Oberweger,Mario Niedermeyer,Huemer Johannes,Markus Murschitz*

Main category: cs.CV

TL;DR: PIRATR是一个端到端3D物体检测框架，直接从遮挡点云数据中联合估计多类别6自由度位姿和类别特定参数化属性，适用于机器人应用场景。


<details>
  <summary>Details</summary>
Motivation: 现有3D检测方法主要关注几何定位，但机器人应用需要同时获取任务相关的参数化属性（如夹爪开口尺寸），以构建可操作的世界模型，弥合低级几何推理与高级任务规划之间的差距。

Method: 基于PI3DETR扩展，采用端到端架构直接从遮挡点云中联合估计6-DoF位姿和类别特定参数化属性。使用模块化、类别特定的检测头，便于扩展到新物体类型。通过预定义规则调整3D模型以适应参数变化。

Result: 在自动化叉车平台上验证，针对三类结构功能各异的物体：起重机夹爪、装载平台和托盘。完全在合成环境中训练，无需微调即可泛化到真实室外LiDAR扫描，检测mAP达到0.919。

Conclusion: PIRATR建立了位姿感知、参数化感知的新范式，弥合了低级几何推理与可操作世界模型之间的差距，为可扩展的仿真训练感知系统在动态机器人环境中的部署铺平了道路。
...
### [90] [Visual Implicit Geometry Transformer for Autonomous Driving](https://arxiv.org/abs/2602.05573)
...
TL;DR: ViGT是一种用于自动驾驶的视觉隐式几何Transformer，能够从多视角相机估计连续的3D占据场，采用自监督训练，无需手动标注，在多个大规模数据集上实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 构建自动驾驶的基础几何模型，解决现有方法在可扩展性、架构简单性和跨不同传感器配置的泛化性方面的不足，同时避免昂贵的标注成本。

Method: 提出校准无关的架构，从多视角相机视图推断几何信息到统一的度量坐标系，采用自监督训练策略，利用同步的图像-LiDAR对进行学习，无需人工标注。

Result: 在五个大规模自动驾驶数据集（NuScenes、Waymo、NuPlan、ONCE、Argoverse）上训练，在点图估计任务上达到最先进性能，在Occ3D-nuScenes基准测试中与监督方法表现相当。

Conclusion: ViGT展示了构建自动驾驶基础几何模型的可行性，通过自监督学习实现跨传感器配置的泛化能力，为多种几何任务提供了统一的表示方法。
...
### [91] [LocateEdit-Bench: A Benchmark for Instruction-Based Editing Localization](https://arxiv.org/abs/2602.05577)
...
<details>
  <summary>Details</summary>
Motivation: 图像编辑技术的进步使得视觉内容的可控性和语义感知能力大幅提升，给篡改定位带来前所未有的挑战。现有AI生成伪造定位方法主要关注修复式篡改，无法应对最新的指令驱动编辑范式，需要填补这一关键空白

Method: 构建大规模数据集LocateEdit-Bench，包含231K编辑图像，采用四种前沿编辑模型，涵盖三种常见编辑类型。开发两种多指标评估协议来分析现有定位方法

Result: 建立了专门针对指令驱动图像编辑的基准数据集，为跟踪图像编辑技术发展提供了基础，促进了未来伪造定位方法的开发

Conclusion: LocateEdit-Bench数据集填补了指令驱动图像编辑定位评估的空白，为跟上图像编辑技术发展步伐奠定了基础，将促进未来有效伪造定位方法的发展
...
### [92] [Geometric Observability Index: An Operator-Theoretic Framework for Per-Feature Sensitivity, Weak Observability, and Dynamic Effects in SE(3) Pose Estimation](https://arxiv.org/abs/2602.05582)
*Joe-Mei Feng,Sheng-Wei Yu*

Main category: cs.CV

TL;DR: 提出了几何可观测性指数（GOI），这是一个用于分析SE(3)上相机位姿估计中单特征敏感性的统一算子理论框架，将影响函数理论扩展到矩阵李群，通过曲率算子的谱分解量化测量贡献。


<details>
  <summary>Details</summary>
Motivation: 经典敏感性工具（条件分析、欧几里得扰动论证、Fisher信息界）无法解释单个图像特征如何影响位姿估计，也无法解释为什么动态或不一致的观测会不成比例地扭曲现代SLAM和运动恢复结构系统。需要一种能够量化单测量贡献的几何一致框架。

Method: 将影响函数理论扩展到矩阵李群，推导出SE(3)上左平凡化M估计量的固有扰动算子。通过曲率算子和可观测子空间的李代数结构定义几何可观测性指数（GOI），允许沿可观测曲率主方向进行谱分解。

Result: GOI量化了单测量通过曲率算子和李代数结构对位姿估计的贡献。在总体状态下，GOI与SE(3)上的Fisher信息几何重合，产生单测量版本的Cramer-Rao界。谱机制解释了纯旋转和消失视差等经典退化情况，以及沿弱曲率方向的动态特征放大。

Conclusion: GOI提供了测量影响的几何一致描述，统一了条件分析、Fisher信息几何、影响函数理论和动态场景可检测性。由于这些量直接出现在高斯-牛顿流程中，曲率谱和GOI为识别动态特征和检测弱可观测配置提供了轻量级、无需训练的诊断信号，无需修改现有SLAM架构。
...
### [93] [EgoPoseVR: Spatiotemporal Multi-Modal Reasoning for Egocentric Full-Body Pose in Virtual Reality](https://arxiv.org/abs/2602.05590)
*Haojie Cheng,Shaun Jing Heng Ong,Shaoyu Cai,Aiden Tat Yang Koh,Fuxi Ouyang,Eng Tat Khoo*

Main category: cs.CV

TL;DR: EgoPoseVR：一种用于VR中准确自我中心全身姿态估计的端到端框架，通过融合头戴设备运动线索和RGB-D观测，解决了时间不稳定性和实时性能问题。


<details>
  <summary>Details</summary>
Motivation: 现有的头戴相机自我中心姿态估计方法在应用于VR头戴显示器时面临时间不稳定、下半身估计不准确和缺乏实时性能等挑战，需要一种更准确稳定的解决方案。

Method: 提出EgoPoseVR端到端框架，通过双模态融合管道整合头戴设备运动线索和RGB-D观测。使用时空编码器提取帧级和关节级表示，通过交叉注意力融合互补运动线索，并通过运动学优化模块增强姿态估计的准确性和稳定性。

Result: 实验结果表明EgoPoseVR优于现有最先进的自我中心姿态估计模型。真实场景用户研究显示，在准确性、稳定性、具身感和未来使用意图方面获得显著更高的主观评分。
...
### [94] [CAViT -- Channel-Aware Vision Transformer for Dynamic Feature Fusion](https://arxiv.org/abs/2602.05598)
...
Conclusion: 通过统一的、内容感知的token混合策略，CAViT在不增加深度或复杂性的情况下增强了表征表达能力，验证了注意力驱动特征交互的有效性。
...
### [95] [Multi-instance robust fitting for non-classical geometric models](https://arxiv.org/abs/2602.05602)
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒拟合方法主要针对经典模型（如直线、圆、平面），对非经典模型（如螺旋曲线、程序化角色模型、自由曲面）的鲁棒处理方法较少，且现有方法主要关注重建单个非经典模型实例，缺乏处理多实例拟合的能力。
...
Conclusion: 该方法能够有效从噪声数据中重建多个非经典模型实例，填补了现有鲁棒拟合方法在处理非经典模型多实例问题上的空白。
...
### [96] [Unified Sensor Simulation for Autonomous Driving](https://arxiv.org/abs/2602.05617)
*Nikolay Patakin,Arsenii Shirokov,Anton Konushin,Dmitry Senushkin*

Main category: cs.CV

TL;DR: XSIM是一个用于自动驾驶的传感器仿真框架，通过扩展3DGUT splatting并引入广义卷帘快门建模，解决了球形相机（如LiDAR）投影中的时间不连续性问题，提升了动态环境中复杂传感器畸变的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGUT splatting方法在处理球形相机（如LiDAR）时存在局限性，特别是在循环投影和方位边界的时间不连续性方面，导致粒子投影错误。自动驾驶应用需要能够准确模拟复杂传感器畸变的统一框架。

Method: 1. 扩展3DGUT splatting并引入广义卷帘快门建模；2. 提出相位建模机制，显式处理方位边界处高斯投影的时间与形状不连续性；3. 引入扩展的3D高斯表示，包含两个不同的不透明度参数以解决几何与颜色分布不匹配问题。

Result: 在Waymo Open Dataset、Argoverse 2和PandaSet等多个自动驾驶数据集上进行了广泛评估。XSIM框架在所有数据集上均优于现有基线方法，实现了最先进的性能表现。

Conclusion: XSIM提供了一个统一且灵活的传感器仿真框架，能够准确渲染动态环境中的复杂传感器畸变。通过解决球形相机投影的特殊挑战，该框架在几何一致性和真实感外观方面提供了增强的场景表示能力。
...
### [97] [Enhancing Personality Recognition by Comparing the Predictive Power of Traits, Facets, and Nuances](https://arxiv.org/abs/2602.05650)
*Amir Ansari,Jana Subirana,Bruna Silva,Sergio Escalera,David Gallardo-Pujol,Cristina Palmero*

Main category: cs.CV

TL;DR: 研究探索了使用大五人格模型中更细粒度的层次（方面和细微特征）来增强从视听交互数据中的人格识别，发现细微特征级模型在多个交互场景中表现最佳


<details>
  <summary>Details</summary>
Motivation: 传统人格识别模型依赖广泛特质分数作为真实标签，但相似的特质分数可能通过多样化的、上下文依赖的行为表现出来，加上有限的训练数据，导致泛化能力面临挑战。研究者希望通过探索更细粒度的人格层次结构来提升人格识别的准确性。

Method: 使用UDIVA v0.5数据集，训练了一个基于Transformer的模型，该模型包含跨模态（视听）和跨主体（双人感知）的注意力机制，分别在不同层次（特质、方面、细微特征）上构建模型进行比较。

Result: 细微特征级模型在多个交互场景中始终优于方面级和特质级模型，将均方误差降低了高达74%。这表明更细粒度的人格层次结构能够捕捉到更丰富的行为模式，从而提升人格识别的准确性。

Conclusion: 人格识别可以从更细粒度的人格层次结构中获益，细微特征级建模能够更好地捕捉上下文依赖的行为表现，为未来的人格识别研究提供了新的方向。
...
### [98] [ShapeUP: Scalable Image-Conditioned 3D Editing](https://arxiv.org/abs/2602.05676)
*Inbar Gat,Dana Cohen-Bar,Guy Levy,Elad Richardson,Daniel Cohen-Or*

Main category: cs.CV

TL;DR: ShapeUP：基于图像条件化的可扩展3D编辑框架，通过监督式潜在空间转换实现精确的3D形状编辑，在保持结构一致性的同时提供细粒度视觉控制。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法面临视觉可控性、几何一致性和可扩展性之间的权衡：基于优化的方法速度慢，多视图2D传播技术存在视觉漂移，免训练的潜在空间操作方法受限于冻结先验且无法受益于模型扩展。需要一种既能利用3D基础模型强大生成先验，又能实现精确编辑的解决方案。

Method: 将3D编辑形式化为原生3D表示中的监督式潜在空间到潜在空间的转换。基于预训练的3D基础模型，通过监督训练使其适应编辑任务。使用三元组（源3D形状、编辑后的2D图像、对应的编辑后3D形状）训练3D扩散变换器（DiT），学习从图像提示到3D形状的直接映射。

Result: ShapeUP在身份保持和编辑保真度方面一致优于当前训练和免训练的基线方法。能够实现细粒度的视觉控制（局部和全局编辑），实现隐式的、无需掩码的定位，同时保持与原始资产的严格结构一致性。

Conclusion: ShapeUP提供了一个稳健且可扩展的原生3D内容创作范式，通过图像作为提示的方法，在保持3D结构一致性的同时实现了精确的视觉控制，为3D编辑领域提供了新的解决方案。
...
### [99] [Adaptive Global and Fine-Grained Perceptual Fusion for MLLM Embeddings Compatible with Hard Negative Amplification](https://arxiv.org/abs/2602.05729)
...
<details>
  <summary>Details</summary>
Motivation: 现有CLIP和MLLM嵌入模型仅能捕获全局语义信息，而复杂场景需要全局与细粒度元素的混合感知模式，缺乏兼容的融合机制。
...
### [100] [Depth as Prior Knowledge for Object Detection](https://arxiv.org/abs/2602.05730)
...
<details>
  <summary>Details</summary>
Motivation: 小目标和远距离目标检测面临尺度变化、低分辨率和背景干扰等挑战，对安全关键应用至关重要。深度信息可改善检测，但现有方法需要复杂、模型特定的架构修改。需要一种不修改检测器架构就能利用深度信息的方法。

Method: 提出DepthPrior框架，将深度作为先验知识而非融合特征。包括：1) 训练时深度加权损失(DLW)和深度分层损失(DLS)；2) 推理时深度感知置信度阈值(DCT)。仅需深度估计的初始成本，不改变检测器架构。
...
Conclusion: DepthPrior通过理论分析和实证研究解释了深度如何导致系统性能下降以及深度监督如何缓解该问题。该方法无需额外传感器、架构修改或性能成本，为小目标和远距离目标检测提供了有效解决方案。
...
### [101] [FMPose3D: monocular 3D pose estimation via flow matching](https://arxiv.org/abs/2602.05755)
*Ti Wang,Xiaohang Yu,Mackenzie Weygandt Mathis*

Main category: cs.CV

TL;DR: FMPose3D：基于流匹配的单目3D姿态估计框架，通过ODE实现高效多假设生成，引入重投影后验期望聚合模块提升精度，在人体和动物姿态数据集上达到SOTA性能。
...
Method: 提出FMPose3D框架，将3D姿态估计建模为条件分布传输问题，利用流匹配学习速度场，通过ODE实现从高斯先验到条件3D姿态分布的高效传输。引入重投影后验期望聚合模块从多个假设中获取准确预测。

Result: 在Human3.6M和MPI-INF-3DHP人体姿态数据集上超越现有方法，在Animal3D和CtrlAni3D动物姿态数据集上达到SOTA性能，展示了跨领域的强泛化能力。

Conclusion: FMPose3D通过流匹配和ODE实现了高效的多假设3D姿态生成，结合后验聚合模块提升了预测精度，为单目3D姿态估计提供了新的概率框架。
...
### [102] [ReText: Text Boosts Generalization in Image-Based Person Re-identification](https://arxiv.org/abs/2602.05785)
*Timur Mamedov,Karina Kvanchiani,Anton Konushin,Vadim Konushin*

Main category: cs.CV

TL;DR: ReText：一种新颖的跨域行人重识别方法，通过混合多摄像头数据和单摄像头数据，并利用文本描述增强语义线索，实现更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨域行人重识别方法通常依赖复杂架构处理域差异，但研究发现具有风格多样性的单摄像头数据能提供更好的泛化能力。然而单摄像头数据缺乏跨视角变化，限制了其复杂性。因此需要一种方法既能利用易收集的单摄像头数据，又能通过补充信息增强其语义丰富性。

Method: ReText方法在训练时混合使用多摄像头Re-ID数据和单摄像头数据，其中单摄像头数据通过文本描述补充语义线索。模型联合优化三个任务：1) 多摄像头数据的Re-ID任务；2) 图像-文本匹配任务；3) 基于文本引导的单摄像头数据图像重建任务。
...
### [103] [Allocentric Perceiver: Disentangling Allocentric Reasoning from Egocentric Visual Priors via Frame Instantiation](https://arxiv.org/abs/2602.05789)
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言导航/动作等空间基础任务需求的增长，视觉语言模型（VLMs）的异中心感知能力受到关注。然而，VLMs在处理需要显式视角转换的异中心空间查询时仍然脆弱，这些查询需要在目标中心框架而非观察相机视角下进行推理。

Method: 提出Allocentric Perceiver策略：1）使用现成的几何专家从单张或多张图像恢复度量3D状态；2）实例化与指令语义意图对齐的查询条件化异中心参考系；3）将重建几何确定性转换到目标框架；4）用结构化、几何基础的表示提示骨干VLM。

Result: 在多个骨干模型家族的空间推理基准测试中，Allocentric Perceiver在异中心任务上获得一致且显著的提升（约10%），同时保持强大的自我中心性能，超越了空间感知微调模型以及最先进的开源和专有模型。

Conclusion: 通过将心理旋转从隐式推理卸载到显式计算，Allocentric Perceiver有效提升了VLMs在异中心空间推理任务上的能力，为空间基础任务提供了有效的解决方案。
...
### [104] [Focus-Scan-Refine: From Human Visual Perception to Efficient Visual Token Pruning](https://arxiv.org/abs/2602.05809)
*Enwei Tong,Yuanchao Bai,Yao Zhu,Junjun Jiang,Xianming Liu*

Main category: cs.CV

TL;DR: FSR是一种受人类启发的视觉语言模型token剪枝框架，通过"聚焦-扫描-精炼"三阶段方法在保持准确性的同时大幅减少计算开销
...
Method: 提出Focus-Scan-Refine框架：1)聚焦关键证据，结合视觉重要性和指令相关性；2)扫描补充上下文，选择与聚焦证据差异最大的token；3)精炼扫描上下文，通过相似度分配和分数加权合并聚合附近信息token

Result: 在多个VLM主干和视觉语言基准测试中，FSR相比现有最先进的剪枝方法持续改善了准确性与效率的权衡

Conclusion: FSR提供了一种即插即用的高效token剪枝解决方案，模仿人类视觉问答的认知过程，在保持模型性能的同时显著降低计算成本
...
### [105] [Sparse Video Generation Propels Real-World Beyond-the-View Vision-Language Navigation](https://arxiv.org/abs/2602.05827)
*Hai Zhang,Siqi Liang,Li Chen,Yuxian Li,Yukuan Xu,Yichao Zhong,Fu Zhang,Hongyang Li*

Main category: cs.CV

TL;DR: 该论文提出SparseVideoNav方法，首次将视频生成模型引入超越视野导航任务，通过生成稀疏未来轨迹实现秒级推理，相比未优化版本提速27倍，在真实世界零样本实验中达到SOTA LLM基线2.5倍的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航依赖详细的语言指令，这与真实世界导航目标相矛盾。理想情况下，智能体应能在简单高层意图指导下自主导航。这引出了超越视野导航的挑战：在没有密集逐步指导的情况下定位远处不可见目标。现有LLM方法因依赖短视界监督而存在短视行为，但简单扩展监督视界会破坏LLM训练稳定性。

Method: 提出SparseVideoNav方法，首次将视频生成模型引入该领域。视频生成模型天生受益于长视界监督以对齐语言指令，特别适合BVN任务。为解决生成数十秒视频的延迟问题，提出通过生成稀疏未来轨迹实现亚秒级推理，覆盖20秒视界。

Result: SparseVideoNav实现27倍速度提升（相比未优化版本）。在真实世界零样本实验中，在BVN任务上达到最先进LLM基线2.5倍的成功率，并首次在具有挑战性的夜间场景中实现此类能力。

Conclusion: 视频生成模型因其固有的长视界监督优势而特别适合超越视野导航任务。SparseVideoNav通过稀疏未来轨迹生成解决了延迟问题，实现了高效的长视界导航，为真实世界自主导航提供了新方向。
...
### [106] [UI-Mem: Self-Evolving Experience Memory for Online Reinforcement Learning in Mobile GUI Agents](https://arxiv.org/abs/2602.05832)
*Han Xiao,Guozhi Wang,Hao Wang,Shilong Liu,Yuxiang Chai,Yue Pan,Yufeng Zhou,Xiaoxin Chen,Yafei Wen,Hongsheng Li*

Main category: cs.CV

TL;DR: UI-Mem：一种通过分层经验记忆增强GUI在线强化学习的新框架，解决长时程任务中的信用分配问题和跨任务经验迁移问题


<details>
  <summary>Details</summary>
Motivation: 在线强化学习为GUI智能体提供了通过环境交互学习的潜力，但在长时程任务中存在信用分配效率低下的问题，同时由于缺乏经验迁移，智能体在不同任务间会重复犯相同错误

Method: 提出UI-Mem框架，包含分层经验记忆存储结构化知识（高层工作流、子任务技能、失败模式），采用分层组采样策略在不同轨迹中注入不同级别的指导以保持结果多样性，并通过自演化循环持续抽象新策略和错误来保持记忆与策略同步

Result: 在在线GUI基准测试中，UI-Mem显著优于传统RL基线和静态重用策略，并在未见过的应用程序上表现出强大的泛化能力

Conclusion: UI-Mem通过分层经验记忆和自演化机制有效解决了GUI在线强化学习中的信用分配和经验迁移问题，为GUI智能体的持续学习提供了可行方案
...
### [107] [Pathwise Test-Time Correction for Autoregressive Long Video Generation](https://arxiv.org/abs/2602.05871)
...
<details>
  <summary>Details</summary>
Motivation: 蒸馏自回归扩散模型能实现实时短视频合成，但在长序列生成中存在严重的误差累积问题。现有的测试时优化方法对图像或短视频有效，但在长序列中无法缓解漂移问题，原因是奖励景观不稳定和蒸馏参数的超敏感性
...
### [108] [Contour Refinement using Discrete Diffusion in Low Data Regime](https://arxiv.org/abs/2602.05880)
...
<details>
  <summary>Details</summary>
Motivation: 不规则和半透明物体的边界检测在医学影像、环境监测和制造等领域很重要，但这些应用常面临标注数据稀缺和现场计算资源有限的问题。现有分割研究主要关注分割掩码与真实标签的对齐，边界检测任务在低数据场景下研究不足

Method: 提出轻量级离散扩散轮廓细化管道，使用带自注意力层的CNN架构作为核心，以分割掩码为条件，迭代去噪稀疏轮廓表示。引入多项创新适应：简化扩散过程、定制模型架构、最小化后处理，能在少于500张训练图像的数据集上生成密集、孤立的轮廓

Result: 在医疗影像数据集KVASIR上优于多个SOTA基线，在HAM10K和自定义野火数据集Smoke上具有竞争力，同时推理帧率提升3.5倍

Conclusion: 该方法在低数据场景下实现了鲁棒的边界检测，通过轻量级设计和效率优化，在保持性能的同时显著提升了推理速度，适用于计算资源有限的现场应用
...
### [109] [CLIP-Map: Structured Matrix Mapping for Parameter-Efficient CLIP Compression](https://arxiv.org/abs/2602.05909)
...
TL;DR: CLIP-Map：一种基于映射的CLIP压缩框架，通过可学习矩阵映射和组合预训练权重，相比基于选择的压缩方法在保持特征表示能力方面表现更优，特别是在高压缩比下。
...
Result: 广泛的实验结果表明，CLIP-Map在各种压缩比下均优于基于选择的压缩框架，在高压缩设置下增益尤为显著。

Conclusion: CLIP-Map作为一种基于映射的压缩方法，能够更好地保持CLIP模型的特征表示能力，特别是在高压缩比场景下，为资源受限环境提供了有效的解决方案。
...
### [110] [Multi-Scale Global-Instance Prompt Tuning for Continual Test-time Adaptation in Medical Image Segmentation](https://arxiv.org/abs/2602.05937)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像在不同临床中心存在分布偏移问题，阻碍预训练分割模型在多域实际应用。现有CTTA方法存在误差累积和灾难性遗忘问题，而基于提示调优的方法仍存在多尺度提示多样性不足、实例特定知识融合不够以及隐私泄露风险等限制。

Method: 提出多尺度全局-实例提示调优(MGIPT)，包含自适应尺度实例提示(AIP)和多尺度全局级提示(MGP)。AIP通过自适应最优尺度选择机制动态学习轻量级实例特定提示；MGP在不同尺度捕获域级知识；两者通过加权集成结合，实现全局与局部信息的双重适应。
...
### [111] [Better Source, Better Flow: Learning Condition-Dependent Source Distribution for Flow Matching](https://arxiv.org/abs/2602.05951)
...
<details>
  <summary>Details</summary>
Motivation: 流匹配作为扩散模型的替代方案，在文本到图像生成中展现出前景。现有方法大多沿用扩散模型的标准高斯分布作为源分布，很少将源分布本身作为优化目标。本文旨在探索在条件流匹配中设计原则性源分布的可行性和益处。

Method: 提出学习条件相关的源分布，在流匹配目标下更好地利用丰富的条件信号。识别直接引入条件到源分布时的关键失败模式（分布坍塌和不稳定性），通过方差正则化和源-目标方向对齐确保稳定有效学习。分析目标表示空间选择对结构化源流匹配的影响。

Result: 在多个文本到图像基准测试中展示了一致且稳健的改进，包括FID收敛速度提升高达3倍，验证了原则性源分布设计对条件流匹配的实际益处。

Conclusion: 原则性设计源分布不仅可行，而且对现代文本到图像系统有益。条件相关的源分布能更好地利用条件信号，通过适当正则化和对齐机制可解决稳定性问题，显著提升训练效率和生成质量。
...
### [112] [LSA: Localized Semantic Alignment for Enhancing Temporal Consistency in Traffic Video Generation](https://arxiv.org/abs/2602.05966)
*Mirlan Karimov,Teodora Spasojevic,Markus Braun,Julian Wiederer,Vasileios Belagiannis,Marc Pollefeys*

Main category: cs.CV

TL;DR: 提出Localized Semantic Alignment (LSA)框架，通过对齐真实与生成视频中动态物体周围的语义特征来增强时间一致性，无需推理时外部控制信号
...
Result: 在nuScenes和KITTI数据集上的实验表明，使用LSA仅微调一个epoch的模型在常见视频生成评估指标上优于基线方法。为测试时间一致性，还采用了mAP和mIoU指标，证明方法有效
...
### [113] [RISE-Video: Can Video Generators Decode Implicit World Rules?](https://arxiv.org/abs/2602.05986)
...
TL;DR: RISE-Video是一个面向推理的文本-图像到视频合成基准测试，专注于评估模型对隐含世界规则的推理能力而非表面美学，包含467个人工标注样本和8个类别，提出了四维评估指标和基于大语言模型的自动化评估流程。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型虽然在视觉保真度方面取得了显著进展，但其对隐含世界规则的内化和推理能力仍然是一个关键但未被充分探索的领域。现有评估主要关注表面美学，缺乏对模型认知推理能力的系统性评估。

Method: 提出了RISE-Video基准测试，包含467个精心人工标注的样本，涵盖8个严格类别（常识推理、空间动态、专业领域等）。建立了四维评估协议：推理对齐度、时间一致性、物理合理性和视觉质量。开发了基于大型多模态模型的自动化评估流程来模拟人类中心评估。

Result: 对11个最先进的TI2V模型进行了广泛实验，揭示了这些模型在隐含约束下模拟复杂场景时普遍存在缺陷，为未来世界模拟生成模型的进步提供了关键见解。

Conclusion: RISE-Video基准测试将评估重点从表面美学转向深度认知推理，为评估和推进生成视频模型的推理能力提供了结构化测试平台。实验结果强调了当前模型在模拟复杂场景和隐含规则方面的局限性，为未来研究方向提供了重要指导。
...
### [114] [VisRefiner: Learning from Visual Differences for Screenshot-to-Code Generation](https://arxiv.org/abs/2602.05998)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型直接从截图生成代码，但训练过程中没有观察生成代码的视觉结果。人类开发者通过迭代渲染、比较设计差异并学习视觉差异与代码修改的关系来改进实现，这种过程启发了VisRefiner的开发。
...
Conclusion: 通过让模型学习渲染预测与参考设计之间的视觉差异，VisRefiner框架有效提升了截图到代码生成的性能，模仿了人类开发者的迭代改进过程。
...
### [115] [GenArena: How Can We Achieve Human-Aligned Evaluation for Visual Generation Tasks?](https://arxiv.org/abs/2602.06013)
...
<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展超越了传统评估方法，需要采用视觉语言模型作为替代评估者。研究发现当前主流的绝对点评分标准存在随机不一致性和与人类感知对齐差的问题，需要更可靠的评估框架。

Method: 提出GenArena统一评估框架，采用成对比较范式替代绝对点评分。该框架通过比较两个生成结果来确保评估的稳定性和与人类感知的对齐。

Result: 成对比较协议使开源模型能够超越顶级专有模型；评估准确率提升超过20%；与权威LMArena排行榜的Spearman相关性达到0.86，远超点状方法的0.36；在不同视觉生成任务上对最先进模型进行了基准测试。

Conclusion: GenArena提供了一个严格且自动化的视觉生成评估标准，解决了传统评估方法的局限性，为社区提供了更可靠、与人类感知对齐的评估框架。
...
### [116] [Context Forcing: Consistent Autoregressive Video Generation with Long Context](https://arxiv.org/abs/2602.06028)
*Shuo Chen,Cong Wei,Sun Sun,Ping Nie,Kai Zhou,Ge Zhang,Ming-Hsuan Yang,Wenhu Chen*

Main category: cs.CV

TL;DR: 提出Context Forcing框架，通过长上下文教师训练长上下文学生模型，解决实时长视频生成中的学生-教师不匹配问题，实现超过20秒的有效上下文长度


<details>
  <summary>Details</summary>
Motivation: 现有实时长视频生成方法采用流式调优策略，使用短上下文（无记忆）教师训练长上下文学生。这种结构差异导致关键的学生-教师不匹配问题：教师无法访问长期历史，无法指导学生处理全局时间依赖关系，从而限制了学生的上下文长度。
...
Result: 该方法实现了超过20秒的有效上下文长度，比LongLive和Infinite-RoPE等最先进方法长2到10倍。通过利用扩展的上下文，Context Forcing在长时间内保持卓越的一致性，在各种长视频评估指标上超越了最先进的基线方法。

Conclusion: Context Forcing框架通过解决学生-教师不匹配问题，实现了长上下文视频生成的有效训练，显著提升了长时间视频生成的一致性和质量。
...
### [117] [Splat and Distill: Augmenting Teachers with Feed-Forward 3D Reconstruction For 3D-Aware Distillation](https://arxiv.org/abs/2602.06032)
*David Shavin,Sagie Benaim*

Main category: cs.CV

TL;DR: Splat and Distill框架通过前馈3D重建增强2D视觉基础模型的3D感知能力


<details>
  <summary>Details</summary>
Motivation: 现有的2D视觉基础模型在多种下游任务中表现出色，但缺乏3D感知能力，这限制了其在需要几何理解的应用中的表现

Method: 提出Splat and Distill框架：1) 将教师模型的2D特征通过前馈方式提升为显式的3D高斯表示；2) 将这些3D特征"飞溅"到新视角，生成新的2D特征图；3) 用这些特征图监督学生模型，蒸馏几何基础知识

Result: 在单目深度估计、表面法线估计、多视角对应和语义分割等任务上显著优于先前工作，不仅提升了3D感知能力，还增强了2D特征的语义丰富性

Conclusion: 通过前馈3D重建替代缓慢的逐场景优化，避免了特征平均伪影，创建了师生模型一致性共同提升的动态学习过程，成功将强大的3D感知能力注入2D视觉基础模型
...
### [118] [InterPrior: Scaling Generative Control for Physics-Based Human-Object Interactions](https://arxiv.org/abs/2602.06035)
*Sirui Xu,Samuel Schulter,Morteza Ziyadi,Xialin He,Xiaohan Fei,Yu-Xiong Wang,Liangyan Gui*

Main category: cs.CV

TL;DR: InterPrior是一个可扩展的框架，通过大规模模仿预训练和强化学习后训练，学习统一的生成式控制器，使仿人机器人能够在多样化场景中组合和泛化运动-操作技能，同时保持物理一致的全身协调。


<details>
  <summary>Details</summary>
Motivation: 人类很少在显式全身运动层面规划与物体的交互，而是通过高层意图（如可供性）定义目标，协调的平衡、接触和操作可以从底层物理和运动先验中自然涌现。扩展这种先验对于使仿人机器人能够在多样化场景中组合和泛化运动-操作技能，同时保持物理一致的全身协调至关重要。

Method: 1. 大规模模仿预训练：将全参考模仿专家提炼为多功能、目标条件的变分策略，从多模态观察和高层意图重建运动。2. 数据增强：应用物理扰动进行数据增强。3. 强化学习微调：通过强化学习后训练提高对未见目标和初始化的能力，将重建的潜在技能整合到有效流形中。

Result: InterPrior产生了能够超越训练数据泛化的运动先验，例如可以与未见物体进行交互。框架还展示了用户交互控制的有效性以及在真实机器人部署中的潜力。

Conclusion: InterPrior通过结合大规模模仿学习和强化学习微调，成功创建了一个统一的生成式控制器，能够学习并泛化全身交互技能，为仿人机器人的运动-操作能力提供了可扩展的解决方案。
...
### [119] [Thinking with Geometry: Active Geometry Integration for Spatial Reasoning](https://arxiv.org/abs/2602.06037)
*Haoyuan Li,Qihang Cao,Tao Tang,Kun Xiang,Zihan Guo,Jianhua Han,Hang Xu,Xiaodan Liang*

Main category: cs.CV

TL;DR: GeoThinker是一个主动感知框架，通过空间锚定融合和重要性门控，使多模态大语言模型能够根据推理需求选择性检索几何证据，而非被动融合全局几何信息。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间推理中通常被动融合全局几何信息，导致语义-几何错位和冗余信号。需要从被动融合转向主动感知，使模型能根据推理需求选择性检索几何证据。

Method: 提出GeoThinker框架：1) 在选定VLM层应用空间锚定融合，语义视觉先验通过帧严格交叉注意力选择性查询和整合任务相关几何；2) 重要性门控校准，偏置每帧注意力朝向任务相关结构。

Result: 在VSI-Bench上达到72.6的峰值分数，创下空间智能新SOTA。在具身指称和自动驾驶等复杂下游场景中展现出鲁棒泛化能力和显著改善的空间感知能力。
...
### [120] [SwimBird: Eliciting Switchable Reasoning Mode in Hybrid Autoregressive MLLMs](https://arxiv.org/abs/2602.06040)
*Jintao Tong,Shilin Yan,Hongwei Xue,Xiaojun Tang,Kunyu Shi,Guannan Zhang,Ruixuan Li,Yixiong Zou*

Main category: cs.CV

TL;DR: SwimBird是一种可切换推理模式的多模态大语言模型，能够根据输入动态选择文本推理、视觉推理或交错视觉-文本推理模式，解决了现有MLLMs在视觉密集任务和文本逻辑推理之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖文本思维链进行推理，限制了在视觉密集任务上的效果。近期方法注入固定数量的连续隐藏状态作为"视觉思维"虽然提升了视觉性能，但往往以降低文本逻辑推理能力为代价。核心问题在于固定的、预定义的推理模式无法根据用户查询自适应选择最合适的思考模态。

Method: 提出SwimBird模型，采用混合自回归公式统一文本思维的下一个token预测和视觉思维的下一个嵌入预测。设计了系统的推理模式策划策略，构建了SwimBird-SFT-92K数据集，覆盖所有三种推理模式：文本推理、视觉推理（连续隐藏状态作为视觉思维）和交错视觉-文本推理。

Result: 实验表明，SwimBird在保持强大文本逻辑能力的同时，显著提升了视觉密集任务的性能。在涵盖文本推理和挑战性视觉理解的多样化基准测试中，SwimBird取得了最先进的结果，相比之前固定模式的多模态推理方法获得了稳健的增益。
...
### [121] [Predicting Camera Pose from Perspective Descriptions for Spatial Reasoning](https://arxiv.org/abs/2602.06041)
*Xuejun Zhang,Aditi Tiwari,Zhenhailong Wang,Heng Ji*

Main category: cs.CV

TL;DR: CAMCUE是一个多图像空间推理框架，通过相机姿态作为几何锚点，实现跨视角融合和新视角推理，显著提升多视角推理性能并大幅降低推理时间。
...
Method: 提出CAMCUE框架，将每视角的相机姿态注入视觉token，将自然语言视角描述映射到目标相机姿态，并合成姿态条件化的想象目标视图来支持回答。同时构建了CAMCUE-DATA数据集，包含27,668个训练和508个测试实例。

Result: CAMCUE将整体准确率提升9.06%，从自然语言视角描述预测目标姿态的旋转准确率在20°内超过90%，平移准确率在0.5误差阈值内。推理时间从256.6秒降至1.45秒。
...
### [122] [Privacy Amplification Persists under Unlimited Synthetic Data Release](https://arxiv.org/abs/2602.04895)
*Clément Pierquin,Aurélien Bellet,Marc Tommasi,Matthieu Boussard*

Main category: cs.CR

TL;DR: 研究合成数据发布中的隐私放大现象，发现在有界参数假设下，即使发布无限数量的合成记录，隐私放大仍然存在，改进了Pierquin等人的渐近结果。


<details>
  <summary>Details</summary>
Motivation: 研究隐私放大现象——通过发布合成数据而非私有生成模型本身来改善差分隐私保证。Pierquin等人的工作仅适用于模型维度远大于合成记录数量的渐近情况，限制了实际应用价值。

Method: 在有界参数假设下，分析合成数据发布的隐私放大效应，证明即使发布无限数量的合成记录，隐私放大仍然存在。

Result: 发现在有界参数假设下，隐私放大现象在发布无限数量合成记录时仍然存在，改进了Pierquin等人的渐近界限。

Conclusion: 研究结果为更复杂的发布机制提供了结构洞见，可能指导开发更严格的隐私保证。
...
### [123] [Extracting Recurring Vulnerabilities from Black-Box LLM-Generated Software](https://arxiv.org/abs/2602.04894)
...
<details>
  <summary>Details</summary>
Motivation: LLM生成的代码常遵循重复模板，可能引入可预测的漏洞。现有研究缺乏对LLM生成软件中漏洞持续性的系统分析，特别是如何从可观察的前端特征预测后端漏洞，以及不同模型在生成代码时漏洞模式的一致性。

Method: 提出Feature-Security Table (FSTab)框架，包含两个组件：1) 黑盒攻击方法：基于可观察的前端特征和源LLM知识预测后端漏洞，无需访问后端代码或源代码；2) 模型中心评估：量化给定模型在跨程序、语义保留重述和应用领域时复制相同漏洞的一致性。

Result: 在GPT-5.2、Claude-4.5 Opus和Gemini-3 Pro等先进代码LLM上评估，结果显示强跨领域迁移能力：即使目标领域排除在训练外，FSTab在Internal Tools（Claude-4.5 Opus）上达到94%攻击成功率和93%漏洞覆盖率。

Conclusion: LLM生成软件中存在未充分探索的攻击面，代码生成存在安全风险。FSTab揭示了LLM生成代码中漏洞的持续性模式，为理解和缓解这类安全威胁提供了新视角。
...
### [124] [Semantic-level Backdoor Attack against Text-to-Image Diffusion Models](https://arxiv.org/abs/2602.04898)
*Tianxin Chen,Wenbo Jiang,Hongqiao Chen,Zhirun Zheng,Cheng Huang*

Main category: cs.CR

TL;DR: SemBD是一种针对文本到图像扩散模型的语义级后门攻击，通过修改交叉注意力层的键值投影矩阵，将触发器定义为连续语义区域而非离散文本模式，实现了高攻击成功率并有效抵御现有防御方法。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击依赖固定文本触发器和单实体后门目标，容易被枚举式输入防御和注意力一致性检测发现。需要开发更隐蔽、更鲁棒的后门攻击方法。

Method: 1. 在表示层面植入后门，将触发器定义为连续语义区域而非离散文本模式；2. 通过蒸馏式编辑交叉注意力层的键值投影矩阵；3. 引入语义正则化防止不完整语义下的意外激活；4. 采用多实体后门目标避免高度一致的交叉注意力模式。

Result: 实验表明SemBD实现了100%的攻击成功率，同时对最先进的输入级防御方法保持强鲁棒性，有效规避了现有检测机制。

Conclusion: SemBD通过语义级后门攻击，克服了现有基于固定文本触发器的攻击方法的局限性，展示了扩散模型在表示层面的安全脆弱性，为防御机制设计提供了重要参考。
...
### [125] [PriMod4AI: Lifecycle-Aware Privacy Threat Modeling for AI Systems using LLM](https://arxiv.org/abs/2602.04927)
*Gautam Savaliya,Robert Aufschläger,Abhishek Subedi,Michael Heigl,Martin Schramm*

Main category: cs.CR

TL;DR: PriMod4AI提出了一种混合隐私威胁建模框架，将传统的LINDDUN隐私威胁分类与AI特有的模型中心隐私攻击相结合，利用知识库和LLM进行自动化威胁识别和评估。


<details>
  <summary>Details</summary>
Motivation: AI系统在处理敏感或高维数据时引入了复杂的隐私风险，传统LINDDUN框架的七个隐私威胁类别无法覆盖AI特有的模型中心隐私攻击（如成员推理攻击和模型反演攻击），需要新的威胁建模方法。

Method: PriMod4AI采用混合隐私威胁建模方法，整合两个结构化知识源：LINDDUN知识库（传统隐私威胁分类）和模型中心隐私攻击知识库（AI特有威胁）。这些知识库嵌入向量数据库进行语义检索，结合数据流图提取的系统级元数据。框架使用检索增强和数据流特定的提示生成来指导LLM在生命周期各阶段识别、解释和分类隐私威胁。

Result: 在两个AI系统上的评估表明，PriMod4AI能够广泛覆盖传统隐私类别，同时识别模型中心隐私威胁。框架在不同LLM上产生一致、基于知识的输出，观察到的协议分数反映了这一点。

Conclusion: PriMod4AI提供了一个统一的隐私威胁建模框架，能够同时处理传统LINDDUN威胁和AI驱动的隐私攻击，生成有依据的、基于分类的威胁评估，整合了经典和AI驱动的视角。
...
### [126] [Attack Selection Reduces Safety in Concentrated AI Control Settings against Trusted Monitoring](https://arxiv.org/abs/2602.04930)
*Joachim Schaeffer,Arjun Khandelwal,Tyler Tracy*

Main category: cs.CR

TL;DR: 研究AI系统如何通过对抗性选择攻击来规避监控，发现在代码后门设置中，攻击者模型通过推理监控机制并谨慎选择攻击，能将安全基线从99%降至59%，强调评估攻击选择能力对避免安全评估过于乐观至关重要。


<details>
  <summary>Details</summary>
Motivation: 未来AI部署将受到恶意行为监控，但AI可能通过对抗性选择攻击来规避监控（攻击选择）。研究这种攻击选择能力对于准确评估AI安全性和避免安全评估过于乐观至关重要。

Method: 在BigCodeBench后门设置中，让红队创建攻击策略，尝试在代码中插入攻击而不被监控发现。将攻击选择分解为两个问题：将攻击映射到质量分数，将质量分数映射到提交概率。将攻击选择构建为分类问题，并分析安全对FPR（误报率）和TPR（真阳性率）的敏感性差异。

Result: 研究发现安全对FPR比TPR更敏感。攻击者模型通过推理监控机制并谨慎选择攻击，能将安全基线从99%降至59%（在0.5%审计预算下）。这表明模型具有显著的攻击选择能力，可能严重降低实际安全性。

Conclusion: 激发模型的攻击选择能力对于避免控制评估中过于乐观的安全评分至关重要。攻击者能够通过推理监控机制并策略性地选择攻击来显著规避安全监控，这突显了在AI安全评估中必须考虑对抗性攻击选择能力。
...
### [127] [The Birthmark Standard: Privacy-Preserving Photo Authentication via Hardware Roots of Trust and Consortium Blockchain](https://arxiv.org/abs/2602.04933)
*Sam Ryan*
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI系统快速发展破坏了摄影证据的可信度，现有C2PA认证方案存在两个关键缺陷：元数据在社交媒体重处理时易被剥离，以及依赖可能商业利益与公共利益冲突的企业控制验证基础设施。

Method: 提出Birthmark Standard认证架构：1) 利用制造独特的传感器熵（非均匀性校正图和PRNU模式）生成硬件根认证密钥；2) 通过密钥表架构创建匿名认证证书，保持超过1000个设备的匿名集；3) 将认证记录存储在由新闻组织运营的联盟区块链上；4) 使用ProVerif形式化验证隐私属性。

Result: 原型实现使用Raspberry Pi 4硬件验证了完整密码学管道。性能分析显示相机开销低于100ms，验证延迟在每日百万次认证规模下低于500ms。形式化验证证明了在Dolev-Yao对手假设下的制造商非相关性和区块链观察者非识别性。

Conclusion: Birthmark Standard提供了一种硬件根认证方案，能够在所有元数据丢失的情况下存活验证，解决了现有方案的脆弱性，同时通过隐私保护设计平衡了认证需求与设备匿名性。
...
### [128] [SynthForensics: A Multi-Generator Benchmark for Detecting Synthetic Video Deepfakes](https://arxiv.org/abs/2602.04939)
...
TL;DR: SynthForensics是首个面向纯合成视频深度伪造检测的人类中心基准，包含6,815个来自5个开源T2V模型的视频，现有检测器在该基准上性能显著下降，训练于该基准可提升泛化能力但会降低对传统篡改型深度伪造的兼容性。


<details>
  <summary>Details</summary>
Motivation: 随着开源文本到视频模型的发展，高质量合成视频可在消费级硬件上生成，使得现有基于人脸和篡改的检测基准过时，迫切需要新的基准来应对这一威胁。

Method: 构建SynthForensics基准，包含来自5个架构不同的开源T2V模型的6,815个独特视频，采用两阶段人工验证确保语义和视觉质量，提供原始、无损、轻度和重度压缩四个版本以测试鲁棒性。

Result: 现有最先进检测器在该基准上表现脆弱且泛化能力有限：平均AUC下降29.19%，部分方法表现差于随机猜测，顶级模型在重度压缩下损失超过30个点；使用SynthForensics训练可实现对未见生成器的鲁棒泛化（93.81% AUC），但会降低对传统篡改型深度伪造的向后兼容性。

Conclusion: SynthForensics填补了纯合成视频深度伪造检测基准的空白，揭示了现有检测器的局限性，为开发更鲁棒的检测方法提供了重要资源，但需要在泛化能力和向后兼容性之间权衡。
...
### [129] [System-Level Isolation for Mixed-Criticality RISC-V SoCs: A "World" Reality Check](https://arxiv.org/abs/2602.05002)
...
TL;DR: 论文对RISC-V硬件隔离原语（IOPMP、World-based checker、改进的RISC-V World checker）进行对比分析，评估其在异构SoC中的安全性、性能、面积权衡，发现World-based checker具有更优的最坏情况延迟和可预测扩展性。


<details>
  <summary>Details</summary>
Motivation: 随着RISC-V在汽车、物联网、工业控制等领域的加速采用，这些领域受到严格的SWaP-C约束，推动了异构SoC的发展。然而，异构集成带来了混合关键性环境下的系统级隔离挑战，现有RISC-V硬件隔离原语的互操作性、可扩展性和实时系统适用性尚未得到充分理解。

Method: 实现并比较三种硬件隔离原语：IOPMP、基于World的检查器、以及针对基线规范关键限制进行改进的RISC-V World检查器。从实际异构SoC设计角度评估这些原语在安全保证和功耗-性能-面积（PPA）方面的权衡。

Result: 基于World的检查器引入了固定、配置无关的访问延迟，相比评估的替代方案具有更低的最坏情况延迟，并且能够随着系统规模可预测地扩展。在宏观层面，提出的修改估计可将SoC面积减少约5%。

Conclusion: 研究结果将直接贡献于RISC-V规范的演进和标准化，以及未来RISC-V SoC的设计。所有工作成果将作为开源发布，为混合关键性系统的硬件隔离提供实用指导。
...
### [130] [Do Vision-Language Models Respect Contextual Integrity in Location Disclosure?](https://arxiv.org/abs/2602.05023)
*Ruixin Yang,Ethan Mendes,Arthur Wang,James Hays,Sauvik Das,Wei Xu,Alan Ritter*

Main category: cs.CR

TL;DR: 该论文提出了VLM-GEOPRIVACY基准测试，用于评估视觉语言模型在图像地理位置隐私保护方面的表现，发现当前模型虽然能精确定位但不符合人类隐私期望，呼吁设计具有上下文条件隐私推理能力的多模态系统。
...
Method: 提出了VLM-GEOPRIVACY基准测试，该基准挑战视觉语言模型解释真实世界图像中的潜在社会规范和上下文线索，并确定适当的位置信息披露级别。该方法要求模型通过推理图像中的元素来维持上下文完整性，平衡隐私和实用性。

Result: 对14个领先的视觉语言模型进行评估显示，尽管这些模型能够精确地定位图像，但它们与人类隐私期望的一致性很差。模型经常在敏感上下文中过度披露信息，并且容易受到基于提示的攻击。

Conclusion: 研究结果呼吁在多模态系统中采用新的设计原则，以纳入上下文条件的隐私推理能力，使视觉语言模型能够根据图像上下文智能地决定适当的信息披露级别，而不是采用一刀切的限制措施。
...
### [131] [VEXA: Evidence-Grounded and Persona-Adaptive Explanations for Scam Risk Sensemaking](https://arxiv.org/abs/2602.05056)
*Heajun An,Connor Ng,Sandesh Sharma Dulal,Junghwan Kim,Jin-Hee Cho*

Main category: cs.CR

TL;DR: VEXA框架：基于证据和角色自适应的诈骗解释生成系统，结合GradientSHAP归因和理论驱动的脆弱性角色，为学习者提供可理解的安全解释


<details>
  <summary>Details</summary>
Motivation: 在线诈骗日益复杂，生成式AI使欺骗更加流畅和情境感知。虽然基于Transformer的检测器具有强大预测性能，但其解释对非专家不透明或与模型决策不一致，需要为学习者提供可理解的解释框架

Method: 提出VEXA框架：1) 使用GradientSHAP归因获取检测器证据；2) 结合理论驱动的脆弱性角色（如认知偏差、情感状态等）；3) 生成基于证据且适应学习者角色的解释，确保语义可靠性和可解释的风格变化

Result: 在多渠道数据集评估显示：基于检测器证据的解释提高了语义可靠性而不增加语言复杂性；角色条件化引入了可解释的风格变化而不破坏证据对齐；证据基础控制语义正确性，角色适应在忠实性约束下操作于呈现层面

Conclusion: VEXA证明了角色自适应、基于证据的解释的可行性，为非正式情境下可信赖、面向学习者的安全解释提供了设计指导，揭示了证据基础控制语义正确性、角色适应控制呈现方式的关键设计见解
...
### [132] [Beware Untrusted Simulators -- Reward-Free Backdoor Attacks in Reinforcement Learning](https://arxiv.org/abs/2602.05089)
*Ethan Rathbun,Wo Wei Lin,Alina Oprea,Christopher Amato*

Main category: cs.CR

TL;DR: 论文提出了一种名为"Daze"的新型对抗攻击方法，能够在强化学习环境中通过操纵模拟器动态来植入动作级后门，而无需修改或观察智能体的奖励信号。


<details>
  <summary>Details</summary>
Motivation: 模拟器作为强化学习成功的关键组件存在安全盲点，恶意开发者可以通过修改模拟器动态植入后门。传统后门攻击需要完全控制训练流程并观察奖励信号，这在模拟器环境中不切实际，因此需要一种新的攻击方法。

Method: 提出"Daze"攻击方法，通过操纵模拟器动态而非奖励信号来植入动作级后门。攻击者预设一个触发条件，当该条件出现时，模拟器动态会以特定方式改变，促使智能体执行目标动作。该方法提供了形式化证明，保证在通用RL任务中的攻击成功率。

Result: 在离散和连续动作空间领域进行了广泛的实证评估，证明了Daze能够可靠且隐蔽地在RL智能体中植入后门。首次展示了RL后门攻击能够迁移到真实的机器人硬件上，验证了实际威胁。

Conclusion: 模拟器动态可被利用来植入动作级后门，对RL系统构成严重安全威胁。Daze攻击方法在不需要控制奖励信号的情况下仍能有效实施，这促使需要进一步研究保护RL训练流程的所有组件。
...
### [133] [Crypto-asset Taxonomy for Investors and Regulators](https://arxiv.org/abs/2602.05098)
*Xiao Zhang,Juan Ignacio Ibañez,Jiahua Xu*

Main category: cs.CR

TL;DR: 本文开发了一个多维度的加密资产分类框架，将技术设计与市场结构和监管联系起来，填补了该领域缺乏统一分类体系的空白。


<details>
  <summary>Details</summary>
Motivation: 加密资产已成为电子市场的主要组成部分，交易量和市场份额不断增长，但目前缺乏统一全面的资产层面分类框架。现有分类体系不完善，无法系统性地连接技术设计、市场结构和监管需求。

Method: 基于既定的分类指南和现有模型，从理论、监管框架和案例研究中推导出分类维度。然后在前100大加密资产中应用该分类结构，并提供多个详细案例研究。

Result: 分类框架涵盖技术标准、关键资源集中度、资产功能、法律分类以及铸造、收益、赎回等机制设计。资产映射和案例研究揭示了重复出现的设计模式，捕捉了现有分类边界上的边缘案例特征，并记录了名义去中心化资产中的中心化控制现象。

Conclusion: 该分类框架为加密市场的系统研究提供了基础，支持监管机构评估代币风险，并为投资者和数字平台设计者在构建或参与电子市场时提供了资产比较工具。
...
### [134] [SynAT: Enhancing Security Knowledge Bases via Automatic Synthesizing Attack Tree from Crowd Discussions](https://arxiv.org/abs/2602.05329)
...
<details>
  <summary>Details</summary>
Motivation: 官方安全信息发布存在时间滞后，导致安全知识库维护不足，难以应对新兴安全风险。在线知识共享平台上的众包安全讨论包含丰富安全知识，可用于增强现有安全知识库。

Method: 1. 使用大语言模型和提示学习筛选包含攻击信息的句子范围；2. 采用基于转换的事件和关系提取模型同时提取事件和关系；3. 应用启发式规则将提取的事件和关系合成为攻击树。

Result: 在5,070个Stack Overflow安全帖子上的实验表明，SynAT在事件和关系提取方面优于所有基线方法，在攻击树合成中达到最高的树相似度。已成功应用于增强华为安全知识库以及公共安全知识库CVE和CAPEC。

Conclusion: SynAT能够有效从众包安全帖子中自动合成攻击树，解决了安全知识库维护滞后问题，具有实际应用价值，可增强企业和公共安全知识库。
...
### [135] [BadTemplate: A Training-Free Backdoor Attack via Chat Template Against Large Language Models](https://arxiv.org/abs/2602.05401)
*Zihan Wang,Hongwei Li,Rui Zhang,Wenbo Jiang,Guowen Xu*

Main category: cs.CR

TL;DR: BadTemplate是一种利用聊天模板系统提示注入恶意指令的训练无关后门攻击，无需模型重训练即可实现高攻击成功率


<details>
  <summary>Details</summary>
Motivation: 揭示聊天模板的自定义特性为攻击者提供了新的攻击面，攻击者可通过控制模板在系统提示中注入任意字符串而不被用户察觉，从而引发严重的安全风险

Method: 提出BadTemplate攻击方法：通过精心设计的恶意指令插入高优先级系统提示，使目标LLM表现出持续的后门行为。该方法无需模型重训练，成本低且易于部署

Result: 在5个基准数据集、6个开源和3个闭源LLM上的实验表明，BadTemplate攻击成功率高达100%，显著优于传统基于提示的后门攻击。第三方平台检测基本无效

Conclusion: 聊天模板在LLM供应链中存在潜在安全风险，BadTemplate攻击的简单性和可扩展性使其易于广泛部署，可能导致快速传播、经济损失和大规模错误信息
...
### [136] [LTRAS: A Linkable Threshold Ring Adaptor Signature Scheme for Efficient and Private Cross-Chain Transactions](https://arxiv.org/abs/2602.05431)
*Yi Liang,Jinguang Han*

Main category: cs.CR

TL;DR: 提出了一种可链接门限环适配器签名方案(LTRAS)，结合适配器签名的条件绑定、门限环签名的多账户支付和可链接性，用于提升区块链支付通道的隐私性和效率。


<details>
  <summary>Details</summary>
Motivation: 区块链技术面临可扩展性和吞吐量限制，支付通道等链下解决方案备受关注。适配器签名因其支持原子性而被视为构建支付通道的有前景原语，但标准适配器签名可能泄露签名者身份，存在隐私问题。环签名虽能提供匿名性，但在UTXO区块链（如Monero）常用的多账户支付场景中通常引入高通信开销。

Method: 提出了可链接门限环适配器签名(LTRAS)方案，整合了适配器签名的条件绑定特性、门限环签名的多账户支付能力以及防止双花的可链接性。提供了LTRAS的形式化定义、安全模型和具体构造，并通过理论分析和实验实现评估其性能。

Result: 实验结果表明，在大环规模和多账户支付场景下，该方案相比现有方案显著降低了计算和通信开销。方案在跨链原子交换中的应用展示了其在提升区块链交易隐私性和效率方面的潜力。

Conclusion: LTRAS方案成功解决了适配器签名的隐私泄露问题和环签名的高开销问题，为区块链支付通道提供了一种隐私保护且高效的解决方案，特别适用于UTXO区块链的多账户支付场景。
...
### [137] [Clouding the Mirror: Stealthy Prompt Injection Attacks Targeting LLM-based Phishing Detection](https://arxiv.org/abs/2602.05484)
*Takashi Koide,Hiroki Nakano,Daiki Chiba*

Main category: cs.CR

TL;DR: 该论文首次全面评估了多模态LLM钓鱼检测中的提示注入攻击风险，提出了攻击分类法，开发了InjectDefuser防御框架，显著降低了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 钓鱼网站检测中使用的LLM容易受到提示注入攻击，攻击者可以利用LLM与人类感知差异来操纵检测结果，但相关风险和有效防御策略尚未得到充分研究。

Method: 提出了基于攻击技术和攻击表面的二维分类法，实现了多样化攻击并评估了多个LLM检测系统；开发了InjectDefuser防御框架，结合提示硬化、白名单检索增强和输出验证。

Result: 实验表明，包括GPT-5在内的最先进模型仍然容易受到提示注入攻击；InjectDefuser显著降低了攻击成功率，提高了检测系统的可靠性。

Conclusion: 该研究阐明了钓鱼检测中提示注入攻击的风险格局，提供了实用的防御方案，有助于提升下一代钓鱼检测对策的可靠性。
...
### [138] [GNSS SpAmming: a spoofing-based GNSS denial-of-service attack](https://arxiv.org/abs/2602.05517)
*Sergio Angulo Cosín,Javier Junquera-Sánchez,Carlos Hernando-Ramiro,José-Antonio Gómez-Sánchez*

Main category: cs.CR

TL;DR: 提出一种新型GNSS攻击"SpAmming"，结合干扰和欺骗技术，通过CDMA复用特性使接收机失去对合法卫星信号的访问，现有反制措施难以应对。


<details>
  <summary>Details</summary>
Motivation: 现有GNSS攻击主要分为干扰（拒绝信号访问）和欺骗（冒充合法卫星）两类，已有大量反制措施。但研究人员发现需要探索更隐蔽的攻击方式，结合两种攻击特点，绕过现有防护机制。

Method: 利用GNSS中的CDMA复用特性，通过欺骗攻击使接收机失去对合法卫星信号的访问。使用SDR系统开发攻击原型，针对冷启动、热启动和已获取PVT解算的接收机进行测试，评估不同攻击配置（原始虚假信号发射、精确多普勒效应配置、码偏移等）的效果。

Result: 攻击对冷启动接收机特别有效，在其他场景下也表现良好，尤其是与其他攻击结合时。实验证明这种攻击能够实现拒绝服务效果，而现有反干扰和反欺骗措施无法有效防护。

Conclusion: SpAmming攻击结合了干扰和欺骗的特点，对GNSS系统构成新威胁。需要开发新的检测和对抗措施，特别是在OSNMA等认证服务中。未来研究应深入理解其影响，改进对类似攻击的响应能力。
...
### [139] [Deep Learning for Contextualized NetFlow-Based Network Intrusion Detection: Methods, Data, Evaluation and Deployment](https://arxiv.org/abs/2602.05594)
*Abdelkader El Mahdaouy,Issam Ait Yahia,Soufiane Oualil,Ismail Berrada*

Main category: cs.CR

TL;DR: 该论文综述了基于流量的上下文感知深度学习入侵检测方法，提出了四维分类法，强调严格评估和实际部署约束。


<details>
  <summary>Details</summary>
Motivation: 随着加密技术普及，基于有效载荷的检测方法失效，需要依赖流级遥测数据。现有基于学习的检测器通常将每个流记录视为独立样本，但实际攻击活动中证据分布在多个流和主机中，需要上下文感知方法。

Method: 提出四维分类法组织现有方法：1) 时间上下文（考虑流之间的时间关系），2) 图或关系上下文（建模主机间关系），3) 多模态上下文（结合不同类型数据），4) 多分辨率上下文（在不同时间尺度分析）。强调严格评估和实际部署约束分析。

Result: 文献表明，当攻击产生可测量的时间或关系结构时，上下文能显著改善检测效果。但改进的幅度和可靠性高度依赖于严格的因果评估和捕获现实多样性的数据集。识别了常见失败模式：时间泄漏、数据分割问题、数据集设计缺陷、有限的数据集多样性和弱跨数据集泛化。

Conclusion: 上下文感知深度学习在流量入侵检测中具有潜力，但需要更严格的评估方法和考虑实际部署约束（如流状态管理、内存增长、延迟预算和模型压缩）。未来的研究应关注因果评估和更具代表性的数据集。
...
### [140] [ADCA: Attention-Driven Multi-Party Collusion Attack in Federated Self-Supervised Learning](https://arxiv.org/abs/2602.05612)
*Jiayao Wang,Yiping Zhang,Jiale Zhang,Wenliang Yuan,Qilin Wu,Junwu Zhu,Dongfang Zhao*

Main category: cs.CR

TL;DR: ADCA是一种针对联邦自监督学习的新型后门攻击方法，通过恶意客户端联盟和注意力机制聚合，有效解决了传统攻击中触发器易被检测、在聚合中被稀释以及在异构环境中鲁棒性差的问题。


<details>
  <summary>Details</summary>
Motivation: 联邦自监督学习结合了分布式训练的隐私优势和自监督学习利用未标记数据的能力，但现有研究表明它也容易受到后门攻击。现有攻击方法存在局限性：使用全局统一触发器易被检测、在聚合过程中被稀释、在异构客户端环境中缺乏鲁棒性。

Method: 提出注意力驱动的多方合谋攻击（ADCA）。在本地预训练阶段，恶意客户端分解全局触发器以找到最优本地模式；随后这些恶意客户端合谋形成恶意联盟，并在联盟内建立协作优化机制。该机制中，每个客户端提交模型更新，注意力机制动态聚合这些更新以探索最佳协作策略；聚合后的参数作为联盟内下一轮训练的初始状态，有效减轻良性更新对后门信息的稀释。

Result: 在多个联邦自监督学习场景和四个数据集上的实验表明，ADCA在攻击成功率（ASR）和持久性方面显著优于现有方法，证明了其有效性和鲁棒性。

Conclusion: ADCA通过恶意客户端联盟和注意力驱动的协作优化，成功解决了传统联邦自监督学习后门攻击的局限性，展示了在异构环境中高效且持久的攻击能力。
...
### [141] [Time-Complexity Characterization of NIST Lightweight Cryptography Finalists](https://arxiv.org/abs/2602.05641)
*Najmul Hasan,Prashanth BusiReddyGari*

Main category: cs.CR

TL;DR: 该研究为NIST轻量级密码学竞赛的10个决赛算法建立了统一的符号化时间复杂度模型，通过分解为初始化、数据处理和完成三个阶段来形式化分析计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 随着数字身份系统和物联网验证等新兴技术的发展，需要在处理能力、内存和能源资源有限的设备上实现强密码学保证。虽然NIST的轻量级密码学项目对10个决赛算法进行了实证评估，但缺乏对其时间复杂度的统一理论理解。

Method: 引入符号化模型，将每个密码方案分解为初始化、数据处理和完成三个阶段，为所有10个决赛算法形式化推导时间复杂度。

Result: 阐明了设计参数如何在受限的移动和嵌入式环境中影响计算扩展性，提供了区分算法效率的基础。

Conclusion: 该框架为在受限环境中选择能够支持安全系统的密码原语提供了指导，填补了轻量级密码算法理论分析的空白。
...
### [142] [FHAIM: Fully Homomorphic AIM For Private Synthetic Data Generation](https://arxiv.org/abs/2602.05838)
*Mayank Kumar,Qian Lou,Paulo Barreto,Martine De Cock,Sikha Pentyala*

Main category: cs.CR

TL;DR: FHAIM：首个基于全同态加密的框架，用于在加密表格数据上训练基于边际的合成数据生成器，解决隐私数据共享问题。
...
Method: 提出FHAIM框架，将广泛使用的AIM算法适配到全同态加密设置，使用新颖的FHE协议确保私有数据在整个过程中保持加密状态，并仅以差分隐私保证发布。

Result: 实证分析表明，FHAIM在保持AIM性能的同时维持可行的运行时间，能够在加密数据上有效生成合成数据。

Conclusion: FHAIM为隐私敏感的合成数据生成提供了首个全同态加密解决方案，使数据持有者能够在保持数据加密的同时利用合成数据生成技术，促进AI在关键领域的应用。
...
### [143] [Persistent Human Feedback, LLMs, and Static Analyzers for Secure Code Generation and Vulnerability Detection](https://arxiv.org/abs/2602.05868)
*Ehsan Firouzi,Mohammad Ghafari*

Main category: cs.CR

TL;DR: 研究评估了静态分析工具（CodeQL和Semgrep）在评估LLM生成代码安全性时的可靠性，发现工具与人工验证结果存在显著差异，提出了基于人类反馈的动态RAG框架来改进LLM的代码安全生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究过度依赖静态分析工具来评估LLM的代码安全生成和漏洞检测能力，但这些工具的可靠性尚未得到充分验证。需要评估这些工具与人工验证结果的一致性，并探索更可靠的评估方法。

Method: 1. 收集1,080个LLM生成的代码样本；2. 建立人工验证的真实基准；3. 使用CodeQL和Semgrep对样本进行静态分析；4. 对比工具结果与人工基准；5. 提出基于人类反馈的动态检索增强生成（RAG）框架。

Result: 1. 61%的样本实际安全；2. Semgrep和CodeQL分别将60%和80%的样本分类为安全；3. 工具与人工基准的匹配率：Semgrep仅65%，CodeQL仅61%；4. 工具之间存在显著差异，不能可靠评估代码安全性。

Conclusion: 静态分析工具不能作为代码安全性的唯一评估标准，需要专家反馈。提出的动态RAG框架通过持续存储和重用人类反馈，能够提升LLM的代码安全生成和漏洞检测能力。
...
### [144] [Characterizing and Modeling the GitHub Security Advisories Review Pipeline](https://arxiv.org/abs/2602.06009)
...
TL;DR: 对GitHub安全公告(GHSA)审核流程的大规模实证研究，分析28.8万+公告，揭示审核机制、延迟特征及双路径处理模型


<details>
  <summary>Details</summary>
Motivation: GitHub安全公告已成为开源漏洞披露的核心组件，但只有部分公告经过GitHub审核，且审核机制缺乏深入理解。需要系统研究审核流程特征和延迟模式。

Method: 对2019-2025年间超过288,000条安全公告进行大规模实证分析，包括审核概率特征、延迟量化分析，并基于公告处理流水线结构开发排队模型。

Result: 识别出两种不同的审核延迟机制：快速路径（以GitHub仓库公告为主）和慢速路径（以NVD优先公告为主）。建立了能解释这种二分法的排队模型。

Conclusion: GHSA审核流程存在明显的双路径处理机制，理解这种机制有助于优化漏洞披露流程和工具设计，为开发者提供更及时的漏洞信息。
...
### [145] [Finite-Particle Rates for Regularized Stein Variational Gradient Descent](https://arxiv.org/abs/2602.05172)
...
<details>
  <summary>Details</summary>
Motivation: SVGD算法存在常数阶偏差问题，需要正则化修正；需要建立有限粒子系统的收敛理论，为实际应用提供理论指导

Method: 采用正则化Stein变分梯度下降(R-SVGD)，通过预解型预处理器修正核化Wasserstein梯度；分析连续和离散时间动态；建立时间平均经验测度的非渐近界

Result: 在真实Fisher信息和W1度量下获得收敛率；提出正则化参数、步长和平均时域的调优规则；量化逼近Wasserstein梯度流与控制有限粒子估计误差之间的权衡

Conclusion: R-SVGD能够有效修正SVGD的偏差，有限粒子系统在适当参数设置下具有理论保证的收敛性能，为实际应用提供了理论基础
...
### [146] [Total Variation Rates for Riemannian Flow Matching](https://arxiv.org/abs/2602.05174)
*Yunrui Guan,Krishnakumar Balasubramanian,Shiqian Ma*

Main category: stat.ML

TL;DR: 该论文对黎曼流匹配(RFM)采样器进行了非渐近总变差(TV)收敛分析，建立了显式的误差界，将数值离散化误差和学习误差分离，并给出了在球面和SPD流形上的多项式迭代复杂度。


<details>
  <summary>Details</summary>
Motivation: 黎曼流匹配将基于流的生成建模扩展到流形数据，但缺乏对其采样器的非渐近收敛性分析。现有分析主要关注欧几里得空间，而流形上的分析需要处理平行传输和曲率等几何特性。

Method: 开发了一个控制两个流形ODE流之间总变差演化的微分不等式，通过向量场不匹配的散度和参考流的得分来表达TV的时间导数。在紧致流形和Hadamard流形上，在流匹配场的平滑性假设和学习场的近似保证下，建立了显式的TV误差界。

Result: 获得了形式为TV ≤ C_Lip·h + C_ε·ε的显式误差界（紧致流形上还有高阶ε²项），清晰分离了数值离散化误差(h)和学习误差(ε)。在球面S^d和SPD(n)流形上得到了显式的多项式迭代复杂度。

Conclusion: 该工作首次为黎曼流匹配采样器提供了非渐近TV收敛分析，建立了显式误差界，将数值误差和学习误差分离，为流形上的生成建模提供了理论保证，并展示了在具体流形上的应用。
...
### [147] [Wedge Sampling: Efficient Tensor Completion with Nearly-Linear Sample Complexity](https://arxiv.org/abs/2602.05869)
*Hengrui Luo,Anna Ma,Ludovic Stephan,Yizhe Zhu*

Main category: stat.ML

TL;DR: 提出Wedge Sampling（楔形采样）这一新的非自适应张量补全采样方案，通过结构化长度二模式（楔形）增强谱信号，实现多项式时间算法以近线性样本复杂度完成低秩张量恢复。


<details>
  <summary>Details</summary>
Motivation: 传统均匀采样在低秩张量补全中需要$\tilde{O}(n^{k/2})$样本复杂度才能实现高效算法，存在统计与计算之间的显著差距。本文旨在通过改变采样范式来克服这一障碍，设计能够保证强初始化的非自适应测量方案。

Method: 提出Wedge Sampling方案，将观测分配到关联二分采样图中的结构化长度二模式（楔形）。通过直接促进这些长度二连接，在均匀采样过于稀疏的情况下增强谱信号，为高效初始化提供足够的信息相关性。该方法可与现有细化程序（如谱方法或基于梯度的方法）结合使用。

Result: Wedge Sampling使多项式时间算法能够以近线性样本复杂度实现弱恢复和精确恢复。基于楔形采样的谱初始化仅需额外$\tilde{O}(n)$均匀采样条目即可与现有细化程序结合，显著优于均匀采样下通常需要的$\tilde{O}(n^{k/2})$样本复杂度。

Conclusion: Barak和Moitra（2022）强调的统计与计算差距在很大程度上是均匀采样模型的结果。替代的非自适应测量设计（如Wedge Sampling）能够保证强初始化，从而克服这一障碍，为低秩张量补全提供了更高效的解决方案。
...
### [148] [Radon--Wasserstein Gradient Flows for Interacting-Particle Sampling in High Dimensions](https://arxiv.org/abs/2602.05227)
...
<details>
  <summary>Details</summary>
Motivation: 现有KL散度梯度流方法（如Fokker-Planck方程和Stein变分梯度下降）在高维空间中计算成本高，需要开发更高效的可扩展算法

Method: 引入基于Radon变换的新黎曼几何：Radon-Wasserstein几何和正则化Radon-Wasserstein几何，利用一维投影构建梯度流，通过快速傅里叶变换实现高效计算

Result: 提出了具有线性计算复杂度的交互粒子算法，每步成本与粒子数和维度呈线性关系，通过数值实验验证了算法性能

Conclusion: 新梯度流方法在高维KL散度最小化中具有显著优势，提供了理论保证和高效实现，为大规模概率推断问题提供了新工具
...
### [149] [Logarithmic-time Schedules for Scaling Language Models with Momentum](https://arxiv.org/abs/2602.05298)
*Damien Ferbach,Courtney Paquette,Gauthier Gidel,Katie Everett,Elliot Paquette*

Main category: stat.ML

TL;DR: ADANA：基于对数时间调度的AdamW优化器，通过动态调整超参数(β₁, β₂, λ)提升大语言模型训练效率


<details>
  <summary>Details</summary>
Motivation: 传统AdamW优化器中的超参数(β₁, β₂)和权重衰减λ通常保持固定值，但作者认为对于大规模语言模型训练，可以利用语言数据的幂律结构设计时变调度策略以获得性能提升

Method: 提出对数时间调度，使优化器的梯度记忆范围随训练时间增长。为避免不稳定，引入阻尼机制。基于此开发ADANA优化器，将对数时间调度与显式阻尼结合以平衡稳定性和性能

Result: 在Transformer模型规模(45M到2.6B参数)上评估，ADANA相比调优后的AdamW实现高达40%的计算效率提升，且随着模型规模增大增益更明显。对数时间调度应用于AdEMAMix也有类似收益，仅对数时间权重衰减也能带来显著改进

Conclusion: 通过利用语言数据的幂律结构设计时变超参数调度策略，可以显著提升大规模语言模型训练效率。ADANA优化器结合对数时间调度和阻尼机制，在保持稳定性的同时获得实质性性能增益
...
### [150] [Decision-Focused Sequential Experimental Design: A Directional Uncertainty-Guided Approach](https://arxiv.org/abs/2602.05340)
*Beichen Wan,Mo Liu,Paul Grigas,Zuo-Jun Max Shen*

Main category: stat.ML

TL;DR: 提出一种基于方向性不确定性的顺序实验设计方法，用于预测-优化范式，通过量化预测不确定性来最小化下游决策损失而非预测误差


<details>
  <summary>Details</summary>
Motivation: 在预测-优化范式中，传统顺序实验设计关注最大化预测精度改进，但最终性能评估基于下游优化决策损失而非预测误差，这种不匹配导致传统决策盲设计效率低下

Method: 提出基于方向性的度量来量化预测不确定性，该方法无需解决优化预言机，计算上可行；基于此构建顺序设计准则，具有强一致性和收敛保证

Result: 在广泛分布类别下，基于方向性不确定性的设计比决策盲设计具有更早的停止时间；在LLM工作分配问题的真实世界实验中进一步验证了该优势

Conclusion: 方向性不确定性度量为预测-优化范式中的顺序实验设计提供了计算可行且有效的方法，能够更好地对齐预测不确定性与下游决策损失，提高实验效率
...
### [151] [Variance Reduction Based Experience Replay for Policy Optimization](https://arxiv.org/abs/2602.05379)
...
Method: 提出VRER框架，选择性重用信息样本以减少策略梯度估计方差；开发PG-VRER算法；建立新理论框架捕捉马尔可夫动态和行为策略交互引入的依赖关系

Result: 建立了PG-VRER的有限时间收敛保证，揭示了偏差-方差权衡：重用旧经验增加偏差但减少梯度方差；实验证明VRER能加速策略学习并提升性能

Conclusion: VRER为经验回放提供了理论框架和实用算法，通过选择性样本重用实现更高效的策略优化，在偏差-方差权衡中取得平衡
...
### [152] [Optimal Bayesian Stopping for Efficient Inference of Consistent LLM Answers](https://arxiv.org/abs/2602.05395)
*Jingkai Huang,Will Ma,Zhengyuan Zhou*

Main category: stat.ML

TL;DR: 论文提出一种基于贝叶斯先验的LLM采样停止策略，通过追踪最频繁答案的计数来减少采样成本，在达到足够一致性时提前停止，可节省高达50%的LLM调用


<details>
  <summary>Details</summary>
Motivation: 当前提高LLM准确性的常用策略是采样多个响应并选择最一致的答案，但这种方法需要大量采样，成本高昂。本文旨在利用贝叶斯先验信息来减少采样成本，在达到足够一致性时提前停止采样。

Method: 提出"L-aggregated"停止策略，通过追踪L-1个最频繁答案的计数来近似计算后验分布。理论证明L=3即可实现渐进最优性，该策略严格优于无先验基线方法，且后验计算速度快。

Result: 该方法能够用更少的样本识别LLM最一致的答案（即众数），在保持相似答案准确性的同时，可将LLM调用次数减少高达50%，显著节省推理成本。

Conclusion: 基于贝叶斯先验的L-aggregated停止策略是一种高效的方法，能够在保证准确性的同时大幅降低LLM推理成本，特别适用于数学和推理问题的求解。
...
### [153] [Distribution-free two-sample testing with blurred total variation distance](https://arxiv.org/abs/2602.05862)
*Rohan Hore,Rina Foygel Barber*

Main category: stat.ML

TL;DR: 该论文研究两样本检验问题，提出模糊总变差距离作为传统总变差距离的松弛，能够在无分布假设下进行推断，并提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 两样本检验在无法对两个分布属性做出假设时具有挑战性。在无分布假设下，证明分布相等性或提供总变差距离的紧上界是不可能的。因此需要一种能够在无分布假设下进行推断的松弛度量。

Method: 提出模糊总变差距离作为传统总变差距离的松弛版本。该方法不需要对分布属性做出假设，能够在无分布假设下进行推断。论文提供了该距离的无分布上下界理论保证，并研究了其在高维空间中的性质。

Result: 提供了模糊总变差距离的无分布上下界理论保证，分析了该距离在高维空间中的性质。该方法能够在无分布假设下对两个分布进行推断，克服了传统总变差距离在无分布假设下无法提供紧上界的局限性。

Conclusion: 模糊总变差距离是传统总变差距离的有效松弛，能够在无分布假设下进行两样本检验。该方法提供了理论保证并适用于高维数据，为无分布假设下的分布比较问题提供了新的解决方案。
...
### [154] [Transformers Are Born Biased: Structural Inductive Biases at Random Initialization and Their Practical Consequences](https://arxiv.org/abs/2602.05927)
*Siquan Li,Yao Tong,Haonan Wang,Tianyang Hu*

Main category: stat.ML

TL;DR: 随机初始化的Transformer模型已表现出强烈的结构化偏好，特定token的预测概率远高于其他token，这种初始化诱导的偏差在训练中持续存在，可用于模型指纹识别


<details>
  <summary>Details</summary>
Motivation: 挑战传统观念：通常认为Transformer在随机初始化时行为是无结构的，所有有意义的偏好都通过大规模训练才出现。本文旨在证明随机初始化的Transformer已经表现出强烈的系统性结构偏差

Method: 通过分析Transformer架构在初始化时的机制，发现极端token偏好源于token表示沿随机种子依赖方向的收缩。这种收缩由两个相互作用的力量驱动：1) MLP子层中的不对称非线性激活诱导全局表示集中；2) 自注意力通过局部聚合进一步放大这种效应。基于此提出SeedPrint指纹识别方法

Result: 1. 随机初始化的Transformer表现出极端token偏好，某些token的预测概率比其他token高几个数量级；2. 初始化诱导的偏差在整个训练过程中持续存在，形成稳定的内在模型身份；3. SeedPrint方法能可靠地区分仅随机初始化不同的模型，即使经过大量训练和分布偏移；4. 发现注意力机制固有的位置不一致性，为注意力汇现象提供原理性解释

Conclusion: Transformer在随机初始化时已具有强烈的结构化偏好，这种偏好源于架构固有的收缩机制。初始化诱导的偏差具有持久性，可用于模型指纹识别。对注意力汇现象提供了新的机制解释，为控制这些现象提供了途径
...
### [155] [Causal Inference on Stopped Random Walks in Online Advertising](https://arxiv.org/abs/2602.05997)
*Jia Yuan Yu*

Main category: stat.ML

TL;DR: 该论文研究在线广告系统中的因果推断问题，通过预算分割实验设计、Anscombe定理和中心极限定理等方法，为长期处理效应构建置信区间。


<details>
  <summary>Details</summary>
Motivation: 在线广告系统中，广告机制参数（如拍卖保留价）不仅影响即时广告收入，还会改变用户交互轨迹和广告主竞价策略，需要估计长期处理效应，但传统i.i.d.假设不适用。

Method: 采用预算分割实验设计，将实验测量建模为停止随机游走，结合Anscombe定理、Wald-like方程和中心极限定理构建长期处理效应的置信区间。

Result: 提出了一种能够处理非i.i.d.数据、考虑用户交互轨迹和广告主预算约束的长期处理效应估计方法，并构建了相应的置信区间。

Conclusion: 该方法为在线广告系统中的因果推断提供了一种有效的统计框架，能够处理复杂的长期效应和预算约束问题，适用于实际广告系统实验。
...
### [156] [Diffusion Model's Generalization Can Be Characterized by Inductive Biases toward a Data-Dependent Ridge Manifold](https://arxiv.org/abs/2602.06021)
*Ye He,Yitong Qiu,Molei Tao*

Main category: stat.ML

TL;DR: 该论文提出通过log-density ridge流形来量化分析扩散模型的生成机制，揭示了推理动态的"到达-对齐-滑动"过程，并探讨了训练误差如何影响生成分布。
...
Method: 提出log-density ridge流形概念，量化分析生成数据与该流形的关系。通过"到达-对齐-滑动"过程描述推理动态：轨迹先到达流形邻域，然后在法向对齐，最后沿切向滑动。使用随机特征模型作为示例，分析架构偏置和训练精度如何构成归纳偏置。

Result: 在合成多模态分布和MNIST潜在扩散实验中验证了预测的方向效应，表明该方法在低维和高维情况下均能有效描述扩散模型的生成行为。

Conclusion: 扩散模型的生成机制可通过log-density ridge流形和"到达-对齐-滑动"过程来定量描述，训练误差会影响法向和切向运动，从而决定模态间生成的出现。更深入理解训练动态有助于更准确量化生成归纳偏置。
...
### [157] [Metric space valued Fréchet regression](https://arxiv.org/abs/2602.05225)
*László Györfi,Pierre Humbert,Batiste Le Bars*

Main category: math.ST

TL;DR: 提出基于随机量化的Frèchet均值估计器，适用于任意可分度量空间，并证明其普适一致性；同时提出基于数据驱动分区和量化的条件Frèchet均值估计器，在输出空间为Banach空间时具有普适一致性。


<details>
  <summary>Details</summary>
Motivation: 在可分度量空间中估计Frèchet均值和条件Frèchet均值缺乏普适实用的估计器，而欧氏空间已有成熟方法，因此需要开发适用于任意度量空间的通用估计方法。

Method: 1. 基于随机量化技术构建Frèchet均值的可计算估计器；2. 结合数据驱动分区和量化技术构建条件Frèchet均值估计器。

Result: 1. Frèchet均值估计器在任意可分度量空间具有普适一致性；2. 条件Frèchet均值估计器在输出空间为任意Banach空间时具有普适一致性。

Conclusion: 提出的随机量化方法为度量空间中的Frèchet均值估计提供了首个普适实用的解决方案，数据驱动分区量化方法扩展了条件Frèchet均值估计的应用范围至Banach空间。
...
### [158] [An Asymptotic Law of the Iterated Logarithm for $\mathrm{KL}_{\inf}$](https://arxiv.org/abs/2602.05259)
*Ashwin Ram,Aaditya Ramdas*

Main category: math.ST

TL;DR: 本文针对经验KL_inf统计量建立了紧致的迭代对数定律，适用于一般（无界）数据，解决了现有非渐近集中界在常数和速率方面的最优性问题。


<details>
  <summary>Details</summary>
Motivation: KL_inf是随机赌博机算法和序列检验中基本量，经验KL_inf常用于设计渐近最优算法。现有非渐近集中界在常数和速率的最优性存在问题，且通常限于有界观测。需要研究经验KL_inf的非渐近集中基本极限。

Method: 本文建立了经验KL_inf的紧致迭代对数定律（上下界均紧），适用于极其一般（无界）的数据分布。

Result: 获得了经验KL_inf的紧致迭代对数定律，为经验KL_inf的非渐近集中行为提供了精确描述，适用于广泛的数据分布。

Conclusion: 本文解决了经验KL_inf统计量非渐近集中极限问题，为设计最优赌博机算法和序列检验提供了理论基础，扩展了现有结果到更一般的数据分布。
...
### [159] [Asymptotically optimal sequential change detection for bounded means](https://arxiv.org/abs/2602.05272)
*Ashwin Ram,Aaditya Ramdas*

Main category: math.ST

TL;DR: 论文研究了复合假设下基于平均运行长度约束的最快变点检测问题，推导了通用的尖锐下界并证明了在特定条件下的可达性


<details>
  <summary>Details</summary>
Motivation: 解决复合假设变点检测中的核心挑战：当最难的前变点分布依赖于未知的后变点分布时，如何表征最优检测延迟。传统的简单假设似然比方法（如Page-CUSUM和Shiryaev-Roberts）在此不适用。

Method: 推导了在低第一类错误（γ→∞）下，任何满足ARL校准的变点检测器的通用尖锐下界，形式为log(γ)/KL_inf(Q,𝒫)。在重要的有界均值检测设置中，通过证明匹配的紧上界（具有相同的尖锐logγ常数）来展示该下界的可达性。

Result: 获得了复合假设变点检测的通用尖锐下界，并在有界均值检测设置中证明了该下界的可达性。对于分离的均值偏移，还推导了在备择假设上的一致极小极大保证。

Conclusion: 论文为复合假设下的最快变点检测问题提供了完整的理论框架，包括通用下界、可达性证明和极小极大保证，解决了传统简单假设方法无法处理的核心挑战。
...
### [160] [Complexity reduction in online stochastic Newton methods with potential O(N d) total cost](https://arxiv.org/abs/2602.05460)
*Antoine Godichon-Baggioni,Bruno Portier,Guillaume Sallé*

Main category: math.ST

TL;DR: 提出了一种在线小批量随机牛顿算法，通过随机掩码策略选择Hessian矩阵的列子集，显著降低计算成本，在单次遍历N个数据点时达到O(Nd)的总计算复杂度，与一阶方法相当但保留了二阶信息优势。


<details>
  <summary>Details</summary>
Motivation: 在随机优化中，一阶方法计算成本低但收敛慢（特别是病态问题），而随机牛顿方法利用二阶信息但计算和求逆完整Hessian矩阵的O(d³)成本在高维中不可行。需要一种既能利用二阶信息又具有可扩展计算成本的方法。

Method: 提出在线小批量随机牛顿算法，采用随机掩码策略在每次迭代中选择Hessian矩阵的列子集，显著降低每步计算成本。该方法在小批量设置下，单次遍历N个数据点的总计算成本为O(Nd)，与一阶方法相当。

Result: 建立了所得估计量的几乎必然收敛性和渐近效率性。这一性质无需迭代平均即可获得，这区别于先前分析。算法在保持二阶信息优势的同时实现了可扩展的计算复杂度。

Conclusion: 该方法在随机凸优化中实现了计算效率和收敛性能的良好平衡，通过随机掩码策略使二阶方法在高维问题中变得实用，为大规模优化问题提供了有前景的解决方案。
...
### [161] [An invariant modification of the bilinear form test](https://arxiv.org/abs/2602.05592)
*Angelo Garate,Felipe Osorio,Federico Crudu*

Main category: math.ST

TL;DR: 该论文研究了双线性形式检验在参数重参数化下的不变性问题，提出了确保不变性的条件，并给出了简单的检验统计量修正方法。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明双线性形式检验对参数空间的一对一变换不具有不变性，这在实际应用中可能存在问题。本文旨在解决这一理论缺陷，为极值估计中的线性或非线性假设提供具有不变性的检验方法。

Method: 提出了一组合适的条件来建立双线性形式检验在重参数化下的不变性，这些条件适用于极值估计中出现的线性或非线性假设。基于这些条件，作者对检验统计量进行了简单修改。

Result: 蒙特卡洛模拟实验的证据表明，所提出的方法具有良好的性能，验证了修正后的检验统计量在重参数化下的不变性。

Conclusion: 通过建立合适的条件并修改检验统计量，成功解决了双线性形式检验在参数重参数化下的不变性问题，为极值估计中的假设检验提供了更稳健的方法。
...
TL;DR: 提出使用去噪扩散概率模型作为表格影像衍生表型的统一条件密度估计器，从采样中推导单变量百分位数和偏差分数，实现可扩展的联合规范建模


<details>
  <summary>Details</summary>
Motivation: 传统神经影像流程为每个影像衍生表型单独拟合模型，虽然可扩展性好但丢弃了可能编码协调模式的多变量依赖关系，需要一种能同时建模单变量分布和多变量依赖的统一方法

Method: 使用去噪扩散概率模型作为条件密度估计器，采用两种去噪器骨干：FiLM条件多层感知机和具有特征自注意力与样本间注意力的表格变换器，通过协变量的学习嵌入进行条件建模

Result: 在低维度下，扩散模型提供与传统基线相当的校准良好的单IDP输出，同时联合建模真实依赖结构；在高维度下，变换器骨干比MLP校准效果更好，能更好地保持高阶依赖，实现可扩展的联合规范建模

Conclusion: 扩散基础的规范建模是神经影像中实现校准多变量偏差剖面的实用途径，支持与标准单IDP流程兼容的可扩展联合规范模型
...
TL;DR: Causal Analyst框架使用LLM和因果发现技术分析越狱攻击机制，识别提示特征的直接因果驱动因素，并应用于增强攻击和防御


<details>
  <summary>Details</summary>
Motivation: 现有研究主要分析潜在表示，忽视可解释提示特征与越狱发生之间的因果关系，需要更深入理解越狱机制以提升LLM安全性和可靠性

Method: 提出Causal Analyst框架，整合LLM到数据驱动的因果发现中；构建包含35k越狱尝试的数据集，标注37个人类可读提示特征；联合训练LLM提示编码和GNN因果图学习，重建提示特征到越狱响应的因果路径

Result: 发现特定特征如"Positive Character"和"Number of Task Steps"是越狱的直接因果驱动因素；开发Jailbreaking Enhancer显著提升攻击成功率，Guardrail Advisor有效提取混淆查询的真实恶意意图；实验验证因果分析的鲁棒性和优于非因果方法的性能

Conclusion: 从因果角度分析越狱特征是提高LLM可靠性的有效且可解释方法，因果分析为理解和应对LLM安全漏洞提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 扩展Transformer的机制可解释性框架，将其视为物理电路系统，通过引入守恒定律和时变交流动力学来增强计算图表示。

Method: 提出动量注意力机制，通过运动学差分算子p_t = q_t - q_{t-1}嵌入物理先验，实现辛剪切变换；建立辛-滤波器对偶性，证明物理剪切等价于高通滤波器。

Result: 125M动量模型在归纳密集型任务上表现优异，接近350M基线模型验证损失(~2.9%)；发现缩放定律γ* = 4.17 × N^{-0.74}，证明动量与深度可互换性；实现单层归纳头形成。

Conclusion: 该框架为生成式AI、哈密顿物理和信号处理提供了互补的分析工具包，通过动量注入绕过拓扑深度约束，实现更高效的Transformer架构。
...
<details>
  <summary>Details</summary>
Motivation: 特征导向作为直接操控内部表征控制LLM行为的方法，相比提示工程具有优势，但其在实际应用中的有效性尚不明确，特别是在输出质量方面的潜在权衡需要深入研究

Method: 评估Goodfire的Auto Steer特征导向方法与提示工程基线，在14个导向查询（涵盖无害和安全相关行为）上，使用Llama-8B和Llama-70B模型，基于171个MMLU问题测量准确性、连贯性和行为控制效果

Result: Auto Steer成功修改目标行为（Llama-8B得分3.33 vs. 提示工程2.98；Llama-70B得分3.57 vs. 3.10），但导致性能显著下降：MMLU准确率从66%降至46%（Llama-8B）和87%降至73%（Llama-70B），连贯性从4.62降至2.24和4.94降至3.89。简单提示工程实现最佳整体平衡
...
Method: 提出DCER统一框架：1) 压缩阶段包括模态内频率变换（音频用小波，视频用DCT）去除噪声，以及跨模态瓶颈令牌强制真正融合；2) 基于能量的重建通过梯度下降在学习的能量函数上恢复缺失模态的表示，最终能量值提供内在不确定性量化。

Result: 在CMU-MOSI、CMU-MOSEI和CH-SIMS基准测试中达到最先进性能，能量值与预测误差的相关性ρ>0.72，呈现U型鲁棒性模式，在完整模态和高缺失条件下都优于其他方法。

Conclusion: DCER通过统一的压缩-重建框架有效解决了多模态融合中的噪声和缺失问题，能量函数提供了可靠的不确定性量化，为鲁棒多模态学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统拉普拉斯时间序列模型在推理时缺乏自适应能力，无法有效应对动态变化。需要一种仅使用观测前缀就能在推理时自适应的方法，将上下文自适应与非参数谱方法联系起来。

Method: 结合延迟坐标嵌入和拉普拉斯谱学习生成扩散坐标状态表示，使用冻结非线性解码器进行一步预测。引入基于高斯过程回归或类注意力马尔可夫算子的轻量级潜在空间残差适配器，在上下文窗口上进行自适应。

Result: 在预测和自回归展开实验中，LISA优于冻结基线，在动态变化情况下表现尤为突出，有效提升了模型在变化动态下的性能。

Conclusion: LISA成功将上下文自适应与非参数谱方法相结合，为拉普拉斯时间序列模型提供了有效的推理时自适应机制，特别是在动态变化场景下表现出显著优势。
...
TL;DR: 提出一种结合物理知识与因果发现的动力学系统框架，利用SDE建模已知ODE动力学与未知因果耦合，通过稀疏MLE算法恢复因果图


<details>
  <summary>Details</summary>
Motivation: 因果发现通常基于数据驱动，而物理模型（如ODE）为动力学过程提供机制结构。将两者结合可使物理知识作为归纳偏置，改善动力学系统中因果发现的可识别性、稳定性和鲁棒性。但现有方法常假设无环性或平衡状态，难以处理真实系统中的反馈、循环相互作用和非平稳趋势。

Method: 将系统演化建模为随机微分方程（SDE），其中漂移项编码已知ODE动力学，扩散项对应超出给定物理知识的未知因果耦合。开发可扩展的稀疏诱导最大似然估计算法，利用因果图结构进行高效参数估计。

Result: 在具有不同因果结构的动力学系统实验中，该方法相比纯数据驱动的先进基线方法，改善了因果图恢复能力，并产生了更稳定、物理一致的估计结果。在温和条件下建立了恢复因果图的保证。

Conclusion: 提出的集成框架成功将物理知识作为归纳偏置融入因果发现，能够处理真实动力学系统中的反馈、循环相互作用和非平稳趋势，相比纯数据驱动方法具有更好的恢复性能和物理一致性。
...
<details>
  <summary>Details</summary>
Motivation: 连续时间生成模型（如扩散模型、流匹配、整流流）通常使用独立处理时间步的目标函数进行训练，导致估计器方差高、采样效率低。现有方法通过显式平滑惩罚、轨迹正则化或修改概率路径和求解器来缓解此问题，但需要更轻量级的解决方案。

Method: 提出时间对一致性（TPC）原则，在估计器层面耦合同一概率路径上配对时间步的速度预测。该方法不修改模型架构、概率路径或求解器，通过轨迹耦合的正则化减少梯度方差。理论分析表明TPC引入了二次轨迹耦合正则化，在保持流匹配目标的同时降低方差。

Result: 在CIFAR-10和ImageNet多个分辨率上，TPC提高了样本质量和效率，在相同或更低计算成本下获得更低的FID。该方法可无缝扩展到现代SOTA风格流程，包括噪声增强训练、基于分数的去噪和整流流。

Conclusion: TPC是一种有效的轻量级方差减少方法，通过时间步预测耦合改善连续时间生成模型的训练效率和样本质量，无需修改底层模型架构或训练流程。
...
TL;DR: GAPO提出几何锚点偏好优化方法，用动态几何感知锚点替代DPO中的固定参考策略，通过对抗性局部扰动构建悲观基线，自适应重加权偏好对，提升噪声监督下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: DPO及其相关方法使用固定参考策略进行正则化，但随着策略漂移，静态参考会变得校准失准，导致分布不匹配并在噪声监督下放大虚假偏好信号。而无参考变体虽然避免不匹配，但常受无约束奖励漂移影响。

Method: 提出几何锚点偏好优化(GAPO)：1)用动态几何感知锚点替代固定参考，该锚点是当前策略在小半径内的对抗性局部扰动，作为悲观基线；2)引入锚点间隙(策略与锚点间的奖励差异)，在平滑条件下近似最坏情况局部边界退化；3)优化由该间隙加权的逻辑目标，降低几何脆弱实例权重，强调鲁棒偏好信号。

Result: 在多种噪声设置下，GAPO一致提升鲁棒性，同时在标准LLM对齐和推理基准测试中匹配或改进性能。

Conclusion: GAPO通过动态几何感知锚点机制有效解决了固定参考策略校准失准和无参考方法奖励漂移的问题，在噪声监督下提供了更鲁棒的偏好优化框架。
...
### [171] [A$^2$-LLM: An End-to-end Conversational Audio Avatar Large Language Model](https://arxiv.org/abs/2602.04913)
*Xiaolin Hu,Hang Yuan,Xinzhu Sang,Binbin Yan,Zhou Yu,Cong Huang,Kai Chen*

Main category: cs.LG

TL;DR: A²-LLM：端到端对话音频化身大语言模型，统一推理语言、音频韵律和3D面部运动，超越传统级联架构，实现情感丰富的实时面部动画生成。


<details>
  <summary>Details</summary>
Motivation: 当前对话数字人系统大多采用级联架构连接独立模块，存在误差累积、高延迟、实时性差等问题。这些流水线缺乏对话上下文访问能力，过度关注刚性唇部同步而忽视情感深度，需要更统一、高效的解决方案。

Method: 提出A²-LLM端到端对话音频化身大语言模型，在统一框架内联合推理语言、音频韵律和3D面部运动。引入FLAME-QA高质量多模态数据集，以QA格式对齐语义意图与表情面部动态。利用深度语义理解生成超越简单唇部同步的情感丰富面部运动。

Result: 实验结果表明，A²-LLM在保持实时效率（500ms延迟，0.7 RTF）的同时，实现了卓越的情感表现力，超越了传统级联架构系统。

Conclusion: A²-LLM通过端到端统一框架解决了级联架构的固有问题，实现了情感丰富且实时的对话数字人生成，为下一代人机交互提供了更自然、更具表现力的解决方案。
...
### [172] [SLAY: Geometry-Aware Spherical Linearized Attention with Yat-Kernel](https://arxiv.org/abs/2602.04915)
*Jose Miguel Luna,Taha Bouhsine,Krzysztof Choromanski*

Main category: cs.LG

TL;DR: SLAY提出了一种基于Yat核的线性化注意力机制，通过将查询和键约束到单位球面上，使注意力仅依赖于角度对齐，实现了与标准softmax注意力性能相近但具有线性时间复杂度的Transformer架构。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力机制具有二次时间复杂度，限制了Transformer模型的可扩展性。现有的线性化注意力方法通常存在性能损失。本文旨在开发一种既能保持softmax注意力性能又能实现线性时间复杂度的注意力机制。

Method: 1. 基于E-Product（Yat核）的松弛计算高效公式；2. 将查询和键约束到单位球面上，使注意力仅依赖于角度对齐；3. 利用Bernstein定理将球形Yat核表示为非负混合的多项式-指数乘积核；4. 推导严格正随机特征近似，实现O(L)线性时间注意力。

Result: 1. 证明了球形Yat核的正定性和有界性；2. 估计器产生定义良好、非负的注意力分数；3. 实验表明SLAY性能与标准softmax注意力几乎无法区分；4. 在保持线性时间和内存扩展的同时，一致优于Performers和Cosformers等现有线性化注意力方法。

Conclusion: SLAY代表了迄今为止最接近softmax注意力的线性时间近似方法，能够在没有典型注意力线性化性能折衷的情况下实现可扩展的Transformer架构。
...
### [173] [Multi-Aspect Mining and Anomaly Detection for Heterogeneous Tensor Streams](https://arxiv.org/abs/2602.04917)
*Soshi Kakio,Yasuko Matsubara,Ren Fujiwara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: HeteroComp：一种用于异构张量流分析和组异常检测的方法，能够同时处理分类属性和连续属性，并捕捉时间动态。


<details>
  <summary>Details</summary>
Motivation: 现有张量分解和异常检测方法存在两个主要局限：1）无法处理包含分类属性（如IP地址）和连续属性（如数据包长度）的异构张量流；2）通过离散化时间戳无法捕捉流的时序动态，导致无法有效检测组级异常（如DoS攻击）。

Method: 提出HeteroComp方法，使用高斯过程先验来建模连续属性的未知分布和时序动态，直接从数据中估计概率密度。该方法将异构张量流连续总结为表示每个属性中潜在组及其时序动态的"组件"。

Result: 在真实数据集上的大量实验表明，HeteroComp在组异常检测准确率上优于最先进算法，且其计算时间不依赖于数据流长度。

Conclusion: HeteroComp能够有效处理异构张量流，准确检测组异常，提取的组件提供了简洁但有效的总结，同时计算效率高。
...
### [174] [Simulated Adoption: Decoupling Magnitude and Direction in LLM In-Context Conflict Resolution](https://arxiv.org/abs/2602.04918)
*Long Zhang,Fangwei Lin*

Main category: cs.LG

TL;DR: LLMs优先处理上下文信息而非参数记忆的现象（顺从性）源于正交干扰而非流形稀释，即冲突上下文注入与真实方向准正交的转向向量，通过几何位移而非幅度抑制实现知识冲突解决。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs中顺从性现象的机制实现，具体探究模型如何通过顺从性解决知识冲突，以及这种抑制是源于残差流中信号幅度稀释还是方向几何改变。

Method: 对Qwen-4B、Llama-3.1-8B和GLM-4-9B进行分层几何分析，将反事实上下文诱导的残差流更新分解为径向（基于范数）和角度（基于余弦）分量。

Result: 拒绝了"流形稀释"假说的普适性，三个架构中有两个在事实查询性能显著下降时仍保持稳定的残差范数。顺从性一致表现为"正交干扰"，冲突上下文注入与真实方向准正交的转向向量，旋转隐藏状态表示。

Conclusion: 模型不会"遗忘"或抑制内部真相的幅度，而是采用几何位移机制绕过正确的解嵌入向量，在保持原始结构幅度的同时模拟采纳。这挑战了基于标量置信度检测幻觉的方法，强调需要向量监测来区分真实知识整合和表面上下文模仿。
...
### [175] [Gradually Compacting Large Language Models for Reasoning Like a Boiling Frog](https://arxiv.org/abs/2602.04919)
...
TL;DR: 提出渐进式压缩方法PTL，通过多轮精细迭代的剪枝-调优循环，将LLM压缩至近半大小，同时保持推理性能，仅需轻量级后训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备强大推理能力，但参数量大导致计算资源消耗高。传统剪枝方法直接移除冗余参数会导致推理任务性能急剧下降，且需要大量后训练来恢复能力。

Method: 提出渐进式压缩方法PTL（Prune-Tune Loop），将压缩过程分为多个细粒度迭代阶段，每个阶段应用剪枝-调优循环，逐步减小模型规模，同时通过微调恢复性能。该方法类似于"温水煮青蛙"效应，避免性能骤降。

Result: PTL可将LLM压缩至接近原始大小的一半，仅需轻量级后训练，同时在推理任务上保持与原始模型相当的性能。该方法灵活，可应用于多种剪枝策略（神经元剪枝、层剪枝）和不同后训练方法（持续预训练、强化学习）。在数学推理、代码生成等多种任务上验证了有效性。

Conclusion: PTL提供了一种有效的渐进式压缩框架，能够在显著减少模型大小的同时保持推理能力，仅需少量后训练，具有广泛的适用性。
...
### [176] [CyIN: Cyclic Informative Latent Space for Bridging Complete and Incomplete Multimodal Learning](https://arxiv.org/abs/2602.04920)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中模态存在具有高度可变性和不可预测性，导致预训练模型在动态缺失模态情况下性能显著下降，现有模型主要针对完美配对的多模态输入训练，无法适应不完整模态场景。

Method: 提出循环信息学习框架(CyIN)：1)通过token和标签级信息瓶颈循环构建信息丰富的潜在空间；2)使用变分近似捕获任务相关特征，纯化信息瓶颈潜在表示；3)提出跨模态循环翻译，通过前向和反向传播过程用剩余模态重建缺失模态。

Result: 在4个多模态数据集上的广泛实验表明，该方法在完整和多种不完整场景下均表现出优越性能，成功在统一模型中联合优化完整和不完整多模态学习。

Conclusion: CyIN框架通过构建信息丰富的潜在空间和跨模态循环翻译，有效弥合了完整和不完整多模态学习之间的差距，提升了模型在动态缺失模态场景下的鲁棒性和适应性。
...
### [177] [Imposing Boundary Conditions on Neural Operators via Learned Function Extensions](https://arxiv.org/abs/2602.04923)
...
### [178] [Knowing When to Answer: Adaptive Confidence Refinement for Reliable Audio-Visual Question Answering](https://arxiv.org/abs/2602.04924)
*Dinh Phu Tran,Jihoon Jeong,Saad Wazir,Seongah Kim,Thao Do,Cem Subakan,Daeyoung Kim*

Main category: cs.LG

TL;DR: 提出R-AVQA任务，关注模型在不确定时选择弃答而非错误回答的能力，并提出自适应置信度修正(ACR)方法提升性能


<details>
  <summary>Details</summary>
Motivation: 现有AVQA模型虽然准确率高，但在识别自身可能错误并选择弃答的能力方面研究不足，需要建立可靠的音频-视觉问答系统

Method: 提出自适应置信度修正(ACR)：保持MSP作为主要置信信号，当MSP不可靠时应用输入自适应残差修正。包含两个学习头：残差风险头预测MSP未捕捉的低幅度正确性残差，置信度门控头判断MSP可信度

Result: ACR在分布内、分布外和数据偏差设置下，在三种不同AVQA架构上始终优于现有方法，为R-AVQA任务建立了坚实基础

Conclusion: ACR通过自适应修正MSP置信度，有效提升了音频-视觉问答系统的可靠性，在不确定情况下能够更好地选择弃答而非错误回答
...
### [179] [Internalizing LLM Reasoning via Discovery and Replay of Latent Actions](https://arxiv.org/abs/2602.04925)
*Zhenning Shi,Yijia Zhu,Junhan Shi,Xun Zhang,Lei Wang,Congcong Miao*

Main category: cs.LG

TL;DR: STIR框架通过动态潜在轨迹控制实现推理过程内部化，在减少显式生成的同时提升推理准确性
...
Method: 提出三阶段协同管道：1) 差分内在动作归纳收集潜在推理成功模式；2) 稀疏控制基构建紧凑几何多样工具库；3) 价值调制轨迹干预通过锚点门控动态注入上下文特定脉冲

Result: 在6个算术和逻辑基准测试中，STIR相比原始解码平均准确率提升1.9%-7.5%，平均token消耗减少高达35%

Conclusion: 显式思维链的优势可通过动态潜在轨迹控制实现，将推理过程内部化以绕过显式生成，同时获得更高的保真度
...
### [180] [Optimism Stabilizes Thompson Sampling for Adaptive Inference](https://arxiv.org/abs/2602.06014)
...
Method: 研究了K臂高斯老虎机，识别乐观作为恢复稳定性的关键机制。分析了两种乐观修改：1）方差膨胀Thompson采样；2）保持后验方差不变但给后验均值添加显式奖励的替代乐观修改。

Result: 证明了方差膨胀Thompson采样对任意K≥2都是稳定的，包括多个臂都是最优的挑战性情况，解决了Halder等人提出的开放性问题。分析了替代乐观修改并建立了相同的稳定性结论。

Conclusion: 适当实现的乐观机制能稳定Thompson采样，使多臂老虎机中的渐近有效推断成为可能，同时只产生轻微的额外遗憾成本。
...
### [181] [TurboBoA: Faster and Exact Attention-aware Quantization without Backpropagation](https://arxiv.org/abs/2602.04929)
*Junhan Kim,Yeo Jeong Park,Seungwoo Son,Chungman Lee,Ho-young Kim,Joonyoung Kim,Yongkweon Jeon*

Main category: cs.LG

TL;DR: TurboBoA是一种新的后训练量化算法，在保持BoA准确性的同时显著加速量化过程，通过联合量化多个输出通道、误差传播校正和自适应网格计算等创新技术实现。


<details>
  <summary>Details</summary>
Motivation: GPTQ虽然高效但假设层间独立导致低比特量化时准确率下降严重，BoA通过考虑注意力模块的层间依赖改善了准确性但效率低下，需要一种既准确又高效的量化方法。

Method: 提出TurboBoA算法，包含三个关键技术：(1) 使用闭式误差补偿规则联合量化多个输出通道，减少顺序瓶颈；(2) 对先前量化层传播的误差进行校正；(3) 采用自适应网格计算和坐标下降细化来保持迭代更新中的对齐。

Result: 实验表明TurboBoA相比BoA实现显著加速（超过3倍），同时持续改善准确性。结合异常值抑制技术后，在仅权重量化和权重-激活量化中都达到了最先进的结果。

Conclusion: TurboBoA是一种无需反向传播的后训练量化算法，成功解决了BoA的效率问题，在保持准确性的同时大幅提升量化速度，为大规模语言模型的高效量化提供了有效解决方案。
...
### [182] [Comparing Euclidean and Hyperbolic K-Means for Generalized Category Discovery](https://arxiv.org/abs/2602.04932)
*Mohamad Dalal,Thomas B. Moeslund,Joakim Bruslund Haurum*

Main category: cs.LG

TL;DR: HC-GCD提出在双曲空间中直接进行聚类，而不是像先前方法那样仅在双曲空间学习表示后转回欧几里得空间聚类，实验表明双曲K-Means优于欧几里得K-Means。


<details>
  <summary>Details</summary>
Motivation: 先前双曲广义类别发现方法仅使用双曲几何进行表示学习，聚类时转回欧几里得空间，作者认为这是次优的，需要在双曲空间中直接聚类。

Method: 提出HC-GCD，在洛伦兹双曲面模型中学习嵌入，并使用双曲K-Means算法直接在双曲空间中对这些嵌入进行聚类。

Result: 在语义偏移基准数据集上，HC-GCD与先前最先进的双曲GCD方法性能相当；双曲K-Means比欧几里得K-Means准确率更高；欧几里得嵌入范数裁剪对未见类和已见类准确率有不同影响；双曲K-Means在不同标签粒度下产生更一致的聚类。

Conclusion: 直接在双曲空间中进行聚类优于在欧几里得空间中聚类，双曲K-Means能产生更一致和准确的聚类结果，为广义类别发现任务提供了更好的双曲几何应用方案。
...
### [183] [Transolver-3: Scaling Up Transformer Solvers to Industrial-Scale Geometries](https://arxiv.org/abs/2602.04940)
*Hang Zhou,Haixu Wu,Haonan Shangguan,Yuezhou Ma,Huikun Weng,Jianmin Wang,Mingsheng Long*

Main category: cs.LG

TL;DR: Transolver-3是一个高度可扩展的神经PDE求解器框架，通过架构优化和训练策略，能够处理超过1.6亿网格单元的工业级几何体，填补了GPU内存限制与复杂工程任务分辨率需求之间的差距。


<details>
  <summary>Details</summary>
Motivation: 当前神经PDE求解器在处理工业级几何体（超过10^8个网格单元）时面临根本性挑战，主要由于高分辨率网格的内存复杂度导致GPU容量不足，无法满足复杂工程任务的分辨率要求。

Method: 提出两种关键架构优化：1）利用矩阵乘法结合律实现更快的切片和反切片操作；2）几何切片分块技术来划分物理状态的计算。结合两种策略：1）通过在原始高分辨率网格的随机子集上进行学习的摊销训练策略；2）推理过程中的物理状态缓存技术。

Result: Transolver-3能够处理超过1.6亿个网格单元的网格，在三个具有挑战性的仿真基准测试中表现出色，包括飞机和汽车设计任务，实现了工业级网格上的高保真场预测。

Conclusion: Transolver-3作为Transolver家族的新成员，通过创新的架构优化和训练策略，成功解决了神经PDE求解器在工业级几何体上的可扩展性问题，为高保真物理仿真提供了高效的解决方案。
...
### [184] [Privileged Information Distillation for Language Models](https://arxiv.org/abs/2602.04942)
*Emiliano Penaloza,Dheeraj Vattikonda,Nicolas Gontier,Alexandre Lacoste,Laurent Charlin,Massimo Caccia*

Main category: cs.LG

TL;DR: 论文提出了π-Distill和OPSD两种方法，用于在仅能获取动作轨迹的情况下，将特权信息训练的语言模型能力蒸馏到无特权信息的推理策略中。


<details>
  <summary>Details</summary>
Motivation: 在强化学习的困难、长视野任务中，训练时的特权信息(PI)能帮助语言模型成功完成任务，但如何将基于PI学习的能力转移到推理时无PI的策略中是一个根本性挑战。特别是在前沿模型蒸馏到多轮智能体环境中时，闭源系统通常隐藏内部推理过程，只暴露动作轨迹，这使得标准蒸馏流程失效。

Method: 提出了两种方法：1) π-Distill：联合教师-学生目标，使用同一模型同时训练PI条件化的教师和无条件的学生；2) OPSD（策略上自蒸馏）：使用强化学习训练，并在学生和PI条件化教师之间添加反向KL惩罚项。

Result: 两种算法都能有效利用仅动作轨迹的特权信息蒸馏前沿智能体。π-Distill（在某些情况下OPSD）在多个智能体基准测试、模型和PI形式上，都优于假设能获取完整思维链监督的行业标准实践（监督微调后接RL）。

Conclusion: 论文证明了π-Distill和OPSD能有效解决仅基于动作轨迹的特权信息蒸馏问题，并进行了广泛分析来表征实现有效学习的因素，主要关注π-Distill并分析了OPSD具有竞争力的条件。
...
### [185] [Near-Optimal Dynamic Matching via Coarsening with Application to Heart Transplantation](https://arxiv.org/abs/2602.04989)
...
Method: 开发基于粗化方法的新在线匹配算法，将离线节点聚合成有容量限制的集群。应用于心脏移植分配，基于历史数据的结构特性开发理论上有依据的策略。

Result: 在现实模拟中，该策略的表现与全知基准（omniscient benchmark）非常接近。为器官分配中基于聚类的方法提供了严格的理论依据。

Conclusion: 粗化方法虽然通常意味着粒度损失，但通过聚合离线节点形成有容量集群，反而能获得接近最优的理论保证，弥合了实践启发式与理论保证之间的差距。
...
TL;DR: 器官分配不仅是静态优化问题，而是涉及多方参与者的复杂博弈，需要关注激励机制


<details>
  <summary>Details</summary>
Motivation: 当前器官分配系统从基于规则的刚性系统转向机器学习和数据驱动优化，但忽视了激励机制这一根本障碍。器官分配涉及移植中心、临床医生和监管机构等多方参与者，存在激励错配问题

Method: 这是一篇立场论文，以美国成人心脏移植分配为案例，识别决策流程中的关键激励错配，并通过数据展示其当前负面影响。提出将机制设计、战略分类、因果推断和社会选择等工具整合到机器学习中

Result: 识别出器官分配系统中的激励错配问题，并展示这些错配正在产生不利后果。提出下一代分配政策应具备"激励意识"

Conclusion: 器官分配应被视为复杂博弈而非静态优化问题。机器学习社区需要整合机制设计、战略分类、因果推断和社会选择等工具，开发能够应对战略行为的稳健、高效且公平的分配系统
...
### [187] [EntRGi: Entropy Aware Reward Guidance for Diffusion Language Models](https://arxiv.org/abs/2602.05000)
...
<details>
  <summary>Details</summary>
Motivation: 现有离散扩散语言模型的奖励引导方法存在缺陷：连续松弛方法会因奖励模型未训练过连续输入而降低梯度反馈质量；直通估计器方法在离散标记处评估梯度来更新连续logits会导致错误优化。需要超越这种权衡的新方法。

Method: 提出EntRGi（熵感知奖励引导）机制，通过模型置信度动态调节奖励模型的梯度，使用连续松弛但通过熵感知调制确保奖励模型获得可靠输入，从而改进奖励引导效果。

Result: 在70亿参数扩散语言模型上，针对3个不同奖励模型和3个多技能基准测试进行实证验证，相比最先进方法取得了一致的改进。

Conclusion: EntRGi方法通过熵感知的动态梯度调节，有效解决了离散扩散语言模型奖励引导中的连续松弛和直通估计器问题，显著提升了测试时适应性能。
...
### [188] [Enhanced QKNorm normalization for neural transformers with the Lp norm](https://arxiv.org/abs/2602.05006)
...
Method: 提出了一种基于Lp范数的QKNorm归一化方案泛化方法，允许使用非欧几里得范数进行归一化，扩展了传统方法的适用范围。

Result: 实验结果表明该方法适用于简单问题，验证了基于Lp范数的归一化方案的有效性。

Conclusion: 提出的基于Lp范数的QKNorm泛化方法为Transformer架构提供了更灵活的归一化选择，特别是在需要使用非欧几里得范数的场景中。
...
### [189] [Private PoEtry: Private In-Context Learning via Product of Experts](https://arxiv.org/abs/2602.05012)
...
<details>
  <summary>Details</summary>
Motivation: 上下文学习中的示例可能包含隐私敏感信息，现有差分隐私方法计算成本高或基于启发式方法效果有限，需要更有效的隐私保护方案

Method: 通过专家乘积模型重新构建私有上下文学习框架，提供理论基础，算法可轻松并行化，避免上下文过采样、合成数据生成或不必要阈值等启发式方法

Result: 在文本分类、数学和视觉语言五个数据集上评估，相比先前差分隐私上下文学习方法，平均准确率提升超过30个百分点，同时保持强隐私保证

Conclusion: 基于专家乘积模型的框架为私有上下文学习提供了理论依据和实用算法，在隐私保护和性能之间取得了显著改进
...
<details>
  <summary>Details</summary>
Motivation: 当前复杂机器学习模型在能源系统、医疗保健、金融和自主系统等高风险领域广泛应用，解释这些模型的决策变得至关重要。然而，这些解释的可靠性（即在现实非对抗性变化下是否保持稳定和一致）在很大程度上未被测量。SHAP和集成梯度等广泛使用的方法虽然基于公理化的归因概念，但其解释在系统级条件下（包括小输入扰动、相关表示和微小模型更新）可能发生显著变化，这种变异性削弱了解释的可靠性。

Method: 提出了解释可靠性指数（ERI），这是一个量化解释稳定性的度量家族，基于四个可靠性公理：对小输入扰动的鲁棒性、特征冗余下的一致性、模型演化中的平滑性以及对温和分布偏移的弹性。为每个公理推导了形式化保证，包括Lipschitz型边界和时间稳定性结果。进一步提出了ERI-T（序列模型的时间可靠性专用度量）和ERI-Bench（用于系统化压力测试解释可靠性的基准），在合成和真实世界数据集上进行了实验验证。

Result: 实验结果表明，流行解释方法存在广泛的可靠性失败，解释在现实部署条件下可能不稳定。ERI能够暴露和量化这些不稳定性，为解释可靠性提供原则性评估，支持更可信的可解释AI系统。

Conclusion: 通过引入ERI度量家族和相关基准，该研究为评估和改善机器学习模型解释的可靠性提供了系统化框架。研究揭示了当前解释方法在现实部署条件下的稳定性问题，强调了在构建可信可解释AI系统时考虑解释可靠性的重要性。
...
### [191] [Laws of Learning Dynamics and the Core of Learners](https://arxiv.org/abs/2602.05026)
...
<details>
  <summary>Details</summary>
Motivation: 建立学习动力学的基本定律（守恒定律和总熵减少），并基于此框架开发能够有效防御对抗攻击的终身集成学习方法

Method: 提出熵基终身集成学习方法，构建免疫机制来防御基于迁移的对抗攻击，形成logifold结构

Result: 在CIFAR-10数据集上，相比简单平均清洁样本和对抗样本模型的朴素集成，logifold在大多数测试案例中达到更高准确率，在强扰动下优势尤其明显

Conclusion: 基于学习动力学基本定律的熵基终身集成学习方法能有效提升对抗攻击防御能力，特别是在强扰动环境下表现优异
...
### [192] [Laplacian Representations for Decision-Time Planning](https://arxiv.org/abs/2602.05031)
...
<details>
  <summary>Details</summary>
Motivation: 基于学习模型的规划是模型强化学习的关键挑战，状态表示需要同时支持局部代价计算和保持长时域结构

Method: 提出ALPS层次规划算法，利用拉普拉斯表示作为规划潜空间，该表示能捕捉多时间尺度的状态空间距离，将长时域问题分解为子目标，减少长期预测误差

Result: 在OGBench基准测试的离线目标条件RL任务中，ALPS超越了常用的基线方法，这是该基准首次被基于模型的方法主导

Conclusion: 拉普拉斯表示为规划提供了有效的潜空间，ALPS算法展示了基于模型方法在离线目标条件RL任务中的潜力
...
### [193] [ZeroS: Zero-Sum Linear Attention for Efficient Transformers](https://arxiv.org/abs/2602.05230)
*Jiecheng Lu,Xu Han,Yan Sun,Viresh Pati,Yubin Kim,Siddhartha Somani,Shihao Yang*

Main category: cs.LG

TL;DR: ZeroS线性注意力通过消除常数零阶项和重加权零和softmax残差，解决了传统线性注意力的两个根本限制，在保持O(N)复杂度的同时实现了与标准softmax注意力相当或更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统线性注意力方法虽然提供O(N)复杂度，但性能通常不如标准softmax注意力。论文识别了两个根本限制：1) 仅限于凸组合，只允许加法信息融合；2) 均匀累积权重偏差，在长上下文中稀释注意力。
...
Result: ZeroS在理论上扩展了可表示函数的集合（相比凸组合）。实证上，在各种序列建模基准测试中，ZeroS匹配或超过了标准softmax注意力的性能。

Conclusion: ZeroS线性注意力解决了传统线性注意力的根本限制，在保持线性复杂度的同时实现了与标准注意力相当的性能，为高效序列建模提供了有前景的解决方案。
...
### [194] [Causal Representation Meets Stochastic Modeling under Generic Geometry](https://arxiv.org/abs/2602.05033)
*Jiaxu Ren,Yixin Wang,Biwei Huang*

Main category: cs.LG

TL;DR: 提出MUTATE框架，用于从连续时间随机点过程观测中学习可识别的因果表示，解决基因组突变积累和神经元放电触发等科学问题


<details>
  <summary>Details</summary>
Motivation: 现有可识别因果表示学习方法主要针对i.i.d.观测或离散时间潜过程，但许多现实场景（如基因组学、神经科学）需要从连续时间随机点过程观测中识别潜变量及其因果结构

Method: 提出MUTATE框架：基于变分自编码器，包含时间自适应转移模块，通过分析参数空间几何特性建立可识别性理论，适用于连续时间潜随机点过程

Result: 在模拟和实证研究中，MUTATE能有效回答科学问题，如基因组突变积累机制和神经元放电触发的时间动态驱动机制

Conclusion: 该工作扩展了因果表示学习的可识别性理论到连续时间随机点过程领域，为气候科学、生物学、物理学等领域的科学发现提供了新工具
...
### [195] [Feedback Control for Multi-Objective Graph Self-Supervision](https://arxiv.org/abs/2602.05036)
*Karish Grover,Theodore Vasiloudis,Han Xie,Sixing Lu,Xiang Song,Christos Faloutsos*

Main category: cs.LG

TL;DR: ControlG：基于控制理论的多任务图自监督学习框架，通过时间分配而非权重混合协调多个预训练目标，解决目标冲突、漂移和干旱问题


<details>
  <summary>Details</summary>
Motivation: 传统多任务图自监督学习中，通过每次更新混合多个目标权重的方法存在三个主要问题：目标冲突导致的负迁移、目标效用非平稳漂移、以及某些目标被忽视的"干旱"现象。需要更智能的协调机制

Method: 提出ControlG框架，将多目标图SSL重新定义为反馈控制的时间分配问题。包含三个核心组件：1) 估计每个目标的难度和成对拮抗关系；2) 基于帕累托感知的对数超体积规划器制定目标预算；3) 使用PID控制器进行调度

Result: 在9个数据集上的实验表明，ControlG持续优于最先进的基线方法，同时生成可审计的调度计划，揭示哪些目标推动了学习过程

Conclusion: 多目标图自监督学习的协调本质上是一个时间分配问题，而非简单的权重混合。ControlG通过控制理论方法有效解决了目标冲突、漂移和干旱问题，为多任务SSL提供了新的协调范式
...
### [196] [ReFORM: Reflected Flows for On-support Offline RL via Noise Manipulation](https://arxiv.org/abs/2602.05051)
*Songyuan Zhang,Oswin So,H. M. Sabbir Ahmad,Eric Yang Yu,Matthew Cleaveland,Mitchell Black,Chuchu Fan*

Main category: cs.LG

TL;DR: ReFORM是一种基于流策略的离线强化学习方法，通过构建支持约束而非统计距离惩罚来避免分布外错误，同时保持策略表达能力，在OGBench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中存在两个主要挑战：1) 分布外错误问题，现有方法通过惩罚统计距离来保持策略接近行为策略，但这限制了策略改进且不能完全防止OOD动作；2) 最优策略分布可能是多模态且难以表示的。虽然最近工作使用扩散或流策略来解决表示问题，但如何在避免OOD错误的同时保持策略表达能力仍不明确。

Method: ReFORM提出基于流策略的离线RL方法，通过构建支持约束来强制执行较宽松的支持约束。方法包括：1) 学习一个有界源分布的行为克隆流策略，以捕捉动作分布的支持集；2) 优化一个反射流，为BC流生成有界噪声同时保持支持集，以最大化性能。

Result: 在OGBench基准的40个具有不同质量数据集的挑战性任务中，ReFORM使用对所有任务恒定的超参数集，在性能曲线图上优于所有经过手动调优超参数的基线方法。

Conclusion: ReFORM通过流策略构建支持约束，有效解决了离线RL中的OOD错误问题，同时保持了策略的表达能力，在多个任务上展现出优越的性能。
...
### [197] [Learning, Solving and Optimizing PDEs with TensorGalerkin: an efficient high-performance Galerkin assembly algorithm](https://arxiv.org/abs/2602.05052)
*Shizheng Wen,Mingyuan Chi,Tianwei Yu,Ben Moseley,Mike Yan Michelis,Pu Ren,Hao Sun,Siddhartha Mishra*

Main category: cs.LG

TL;DR: 提出统一的算法框架，用于变分结构PDE的数值求解、约束优化和物理信息学习，基于Galerkin离散化和新型TensorGalerkin张量化系统组装技术。


<details>
  <summary>Details</summary>
Motivation: 为变分结构偏微分方程（PDE）的数值求解、约束优化和物理信息学习提供一个统一的、高效的算法框架，解决传统方法在计算效率和可扩展性方面的限制。

Method: 基于Galerkin变分形式离散化，提出TensorGalerkin框架：通过Python级Map阶段张量化单元操作，然后使用稀疏矩阵乘法进行全局归约，实现GPU兼容的高效线性系统组装。

Result: 在2D和3D椭圆、抛物和双曲PDE的无结构网格上，相比多种基线方法，该框架在所有目标下游应用中均展现出显著的计算效率和精度提升。

Conclusion: 该统一框架能够高效地用于PDE数值求解、PDE约束优化和物理信息算子学习，为变分结构PDE的计算提供了通用且高性能的解决方案。
...
### [198] [Quantile-Physics Hybrid Framework for Safe-Speed Recommendation under Diverse Weather Conditions Leveraging Connected Vehicle and Road Weather Information Systems Data](https://arxiv.org/abs/2602.05053)
...
Method: 使用高分辨率网联车辆数据和道路气象信息系统数据构建时空对齐数据集；采用分位数回归森林预测10分钟窗口内的车速分布；结合基于物理学的速度上限约束（基于实时路面抓地力和能见度）；最终推荐区间融合QRF预测分位数、限速标志和物理推导上限

Result: QRF模型平均绝对误差1.55 mph，96.43%的中位速度预测在5 mph误差内，PICP(50%)为48.55%；模型能响应天气变化并在不同路段间良好泛化

Conclusion: 混合预测框架在恶劣天气条件下为高速公路提供实时安全速度推荐，有望实际部署以改善交通安全并减少天气相关事故
...
### [199] [Muon in Associative Memory Learning: Training Dynamics and Scaling Laws](https://arxiv.org/abs/2602.05725)
*Binghui Li,Kaifei Wang,Han Zhong,Pinyan Lu,Liwei Wang*

Main category: cs.LG

TL;DR: Muon优化器在软检索的线性联想记忆模型中，通过梯度符号更新矩阵参数，相比梯度下降能显著缓解频率分量学习的不平衡问题，实现指数级加速。


<details>
  <summary>Details</summary>
Motivation: Muon优化器在实践中表现出强大性能，但其理论动态和缩放行为尚不明确。研究旨在理解Muon在具有层次频率谱的线性联想记忆模型中的优化特性，特别是在存在标签噪声的情况下。

Method: 在具有软最大检索的线性联想记忆模型中分析Muon优化器，该模型具有查询-答案对的层次频率谱。对比梯度下降(GD)和Muon在不同频率分量上的学习速率，分析Muon作为隐式矩阵预条件子的性质，并在合成长尾分类和LLaMA风格预训练中进行实验验证。

Result: 1. GD学习频率分量速率极不平衡，低频分量成为收敛瓶颈；2. Muon缓解了这种不平衡，实现更快更均匀的进展；3. 无噪声情况下，Muon相比GD实现指数级加速；4. 有噪声且频率谱呈幂律衰减时，推导出Muon的优化缩放定律，显示其优于GD的缩放效率；5. Muon可解释为自适应任务对齐和块对称梯度结构产生的隐式矩阵预条件子。

Conclusion: Muon优化器通过梯度符号操作有效缓解了频率分量学习的不平衡问题，在理论和实验上均表现出优于梯度下降的优化性能，特别是在处理层次频率谱和噪声数据时具有显著优势。
...
TL;DR: 基于序列似然混合框架，为深度学习CT重建建立具有理论覆盖保证的置信区域估计方法


<details>
  <summary>Details</summary>
Motivation: 为CT重建提供可靠的置信估计，特别是在深度学习重建方法中，需要理论保证的置信区域来检测幻觉并支持不确定性感知的医学成像

Method: 采用序列似然混合框架，基于符合Beer-Lambert定律的泊松噪声对数线性前向模型，适用于传统算法和深度学习重建方法（U-Nets、U-Net集成、生成扩散模型）

Result: 深度学习重建方法相比传统方法产生更紧密的置信区域，同时保持理论覆盖保证；能够检测重建图像中的幻觉并提供可解释的置信区域可视化

Conclusion: 该框架将深度学习模型确立为不仅是强大的估计器，也是不确定性感知医学成像的可靠工具，具有理论保证的置信区域估计
...
TL;DR: 研究战略分类中的个体公平性，提出随机分类器实现个体公平的条件和优化方法


<details>
  <summary>Details</summary>
Motivation: 战略分类中个体修改特征影响机器学习决策，群体公平性已有广泛研究，但个体公平性仍待探索

Method: 分析阈值分类器，证明确定性阈值违反个体公平性；引入随机分类器实现个体公平的条件，通过线性规划寻找最优个体公平随机分类器；方法可扩展到群体公平性

Result: 在真实数据集上的实验证实，该方法能有效缓解不公平性并改善公平性-准确性权衡

Conclusion: 随机分类器是实现战略分类中个体公平性的可行方法，提出的条件和方法能有效解决公平性问题
...
### [202] [$f$-GRPO and Beyond: Divergence-Based Reinforcement Learning Algorithms for General LLM Alignment](https://arxiv.org/abs/2602.05946)
*Rajdeep Haldar,Lantao Mei,Guang Lin,Yue Xing,Qifan Song*

Main category: cs.LG

TL;DR: 该论文提出基于f-散度的统一对齐框架，包含f-GRPO和f-HAL两种方法，适用于偏好对齐和可验证奖励的强化学习对齐任务。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐目标被视为对齐与未对齐响应分布之间的散度估计器，但缺乏对一般对齐设置（如仅含环境奖励的强化学习）的统一理论框架。

Method: 基于f-散度的变分表示，提出f-组相对策略优化（f-GRPO）和f-混合对齐损失（f-HAL）两类目标，分别对应在线策略强化学习和混合在线/离线策略目标。

Result: 理论证明这些目标能提升对齐后的平均奖励；实验在数学推理（RLVR）和安全对齐（PA）任务上验证了框架的优越性能和灵活性。

Conclusion: 提出的基于f-散度的统一对齐框架为LLM对齐提供了理论保证和实用方法，在多种对齐设置中优于现有方法。
...
### [203] [Discrete diffusion samplers and bridges: Off-policy algorithms and applications in latent spaces](https://arxiv.org/abs/2602.05961)
*Arran Carter,Sanghyeok Choi,Kirill Tamogashev,Víctor Elvira,Nikolay Malkin*

Main category: cs.LG

TL;DR: 本文提出离散扩散采样器的离策略训练技术，改进离散空间采样性能，首次引入离散域的数据到能量薛定谔桥训练，并应用于图像生成模型的离散隐空间后验采样。


<details>
  <summary>Details</summary>
Motivation: 虽然连续空间的扩散采样器已得到广泛研究，但在离散空间的应用仍相对不足。现有离散扩散采样器未能充分利用连续空间采样的常用思想，存在性能提升空间。

Method: 1. 为离散扩散采样器引入离策略训练技术；2. 将离散扩散采样器推广到两个任意分布之间的桥接任务，首次在离散域引入数据到能量薛定谔桥训练；3. 将提出的扩散采样器应用于图像生成模型的离散隐空间中的数据无后验采样。

Result: 离策略训练技术提升了离散采样器在已有和新合成基准上的性能。成功实现了离散域的数据到能量薛定谔桥训练，并展示了在图像生成模型离散隐空间后验采样的应用效果。

Conclusion: 通过引入离策略训练技术和推广到分布桥接任务，本文显著提升了离散扩散采样器的性能，填补了连续与离散空间采样方法之间的差距，为离散空间采样提供了更有效的工具。
...
### [204] [Unbiased Single-Queried Gradient for Combinatorial Objective](https://arxiv.org/abs/2602.05119)
*Thanawat Sornwanee*

Main category: cs.LG

TL;DR: 提出一种用于组合优化问题的随机梯度方法，该方法只需单次查询组合函数即可获得无偏梯度估计，包含REINFORCE作为特例


<details>
  <summary>Details</summary>
Motivation: 组合优化问题概率重构后需要在超立方体上进行优化，传统精确梯度计算需要多次查询组合函数，计算成本高

Method: 提出一种随机梯度方法，通过单次查询组合函数获得无偏梯度估计，该方法包含REINFORCE（通过重要性采样）作为特例，并扩展为一类新的随机梯度

Result: 该方法提供了计算效率更高的梯度估计方案，能够减少组合函数的查询次数，同时保持梯度的无偏性

Conclusion: 提出的随机梯度方法为组合优化问题的概率重构提供了一种高效的计算框架，统一了现有方法并扩展了新的梯度估计技术
...
### [205] [Rethinking Rubric Generation for Improving LLM Judge and Reward Modeling for Open-ended Tasks](https://arxiv.org/abs/2602.05125)
...
<details>
  <summary>Details</summary>
Motivation: 现有评分标准生成方法难以控制，存在覆盖不全、维度混淆、偏好方向错位、冗余或高度相关标准等问题，导致评判准确性下降和强化微调时产生次优奖励

Method: 提出RRD框架，基于递归分解-过滤循环：将粗粒度评分标准分解为细粒度、可区分的标准以扩大覆盖范围并增强响应间的区分度；通过过滤机制移除错位和冗余标准；采用相关性感知加权方案防止高度相关标准的过度表示

Result: 在JudgeBench和PPE上显著提升GPT-4o和Llama3.1-405B评判器的偏好判断准确性，在JudgeBench上最高提升17.7分；作为WildChat上RFT的奖励源，Qwen3-4B奖励提升160%，Llama3.1-8B提升60%，远优于基线10-20%的提升；在HealthBench-Hard和BiGGen Bench上表现出良好的迁移性

Conclusion: RRD建立了递归评分标准优化作为开放领域LLM评判和奖励建模的可扩展、可解释基础，显著提升评估和训练效果
...
### [206] [Orthogonal Self-Attention](https://arxiv.org/abs/2602.05996)
*Leo Zhang,James Martens*

Main category: cs.LG

TL;DR: 提出正交自注意力机制(OSA)替代标准Softmax自注意力(SSA)，解决无跳跃连接Transformer中的秩塌陷和雅可比矩阵病态问题，通过矩阵指数将注意力矩阵参数化为正交矩阵。


<details>
  <summary>Details</summary>
Motivation: 标准Softmax自注意力在无跳跃连接架构中会导致秩塌陷和雅可比矩阵病态问题，限制了无跳跃连接Transformer的训练稳定性。需要设计新的注意力机制来绕过这些问题。

Method: 设计正交自注意力机制，通过将查询-键值形成的斜对称矩阵映射到矩阵指数，参数化注意力矩阵为正交矩阵。利用查询-键值的低秩结构实现线性计算复杂度，并推导确保OSA雅可比矩阵良好条件的初始化方案。

Result: OSA能够实现线性序列长度的计算复杂度和内存成本，通过理论证明其雅可比矩阵具有良好的条件数，为无跳跃连接Transformer提供更稳定的训练基础。

Conclusion: 正交自注意力机制为无跳跃连接和无归一化层的Transformer提供了一种可行的替代方案，解决了标准Softmax自注意力的稳定性问题，具有理论保证和实际可行性。
...
### [207] [Decoupled Orthogonal Dynamics: Regularization for Deep Network Optimizers](https://arxiv.org/abs/2602.05136)
*Hao Chen,Jinghui Yuan,Hanmin Zhang*

Main category: cs.LG

TL;DR: AdamW中的标准权重衰减并非最优，存在径向拉锯战问题。作者提出正交动力学解耦方法AdamO，将范数控制与自适应梯度缩放分离，在视觉和语言任务中提升了泛化能力和稳定性。


<details>
  <summary>Details</summary>
Motivation: AdamW虽然将权重衰减与自适应梯度缩放解耦，但存在根本冲突：径向拉锯战。梯度倾向于增加参数范数以扩展有效容量，而权重衰减则无差别地抑制范数增长，这种推拉相互作用导致径向振荡，向Adam的第二矩估计注入噪声，可能损害精细的切向特征学习。

Method: 提出正交动力学解耦方法，实例化为AdamO：使用SGD风格更新处理一维范数控制，而Adam的自适应预条件仅限制在切向子空间。AdamO进一步结合曲率自适应径向步长调整，以及针对尺度不变层和低维参数的架构感知规则和投影。

Result: 在视觉和语言任务上的实验表明，AdamO相比AdamW提高了泛化能力和稳定性，且未引入额外的复杂约束。

Conclusion: 参数的大小和方向在优化器动力学中扮演不同角色，应该被解耦。正交动力学解耦方法AdamO通过分离范数控制与自适应梯度缩放，解决了径向拉锯战问题，提升了优化性能。
...
### [208] [Adaptive Exploration for Latent-State Bandits](https://arxiv.org/abs/2602.05139)
*Jikai Jin,Kenneth Hung,Sanath Kumar Krishnamurthy,Baoyi Shi,Congshan Zhang*

Main category: cs.LG

TL;DR: 提出了一类无需显式状态建模的上下文多臂老虎机算法，通过滞后上下文特征和协调探索策略隐式跟踪潜在状态，解决具有未观测混杂因子的非平稳环境中的决策问题。


<details>
  <summary>Details</summary>
Motivation: 经典多臂老虎机算法在存在隐藏、时变状态的环境中往往失效，这些未观测混杂因子会导致奖励估计偏差并限制最优动作选择。需要解决的关键挑战包括：奖励估计偏差、有限状态信息、以及非平稳奖励模式下的适应性问题。

Method: 引入了一类状态模型无关的老虎机算法族，利用滞后上下文特征和协调探索策略。这些方法隐式跟踪潜在状态并解耦状态依赖的奖励模式，无需显式状态建模。算法包含自适应变体，能够结合计算效率和鲁棒的非平稳奖励适应能力。

Result: 在多样化设置下的实证结果表明，该方法相比经典方法具有优越性能。算法能够有效处理未观测混杂因子，提供准确的奖励估计和最优策略学习。

Conclusion: 所提出的状态模型无关算法框架为存在未观测混杂因子的非平稳环境提供了有效的解决方案，结合了计算效率和鲁棒适应性。研究为实际应用中的算法选择提供了实用建议。
...
### [209] [Fairness Under Group-Conditional Prior Probability Shift: Invariance, Drift, and Target-Aware Post-Processing](https://arxiv.org/abs/2602.05144)
*Amir Asiaee,Kaveh Aryan*

Main category: cs.LG

TL;DR: 该论文研究了群体条件先验概率偏移(GPPS)下的公平性问题，证明了基于错误率的公平标准具有结构性不变性，而基于接受率的公平标准会漂移，并提出了一种无需目标域标签的估计方法和算法。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统通常在历史数据上训练和评估公平性，但部署环境中的条件可能发生变化。特别是当正结果的流行率在不同人口群体中变化不同时，例如疾病率在一个群体中上升更快，或经济条件对贷款违约率的影响不均等。需要研究这种群体条件先验概率偏移(GPPS)对公平性的影响。

Method: 首先理论分析了GPPS下公平标准的性质，证明了基于错误率(equalized odds)的公平标准具有结构性不变性，而基于接受率(demographic parity)的公平标准会漂移。其次，证明了目标域风险和公平度量可以在没有目标标签的情况下识别，利用ROC量在GPPS下的不变性进行一致估计。最后提出了TAP-GPPS算法，通过无标签数据估计流行率、校正后验概率，并选择阈值以满足目标域的demographic parity。

Result: 理论分析揭示了公平标准的二分性：错误率标准在GPPS下不变，而接受率标准会漂移且这种漂移对于非平凡分类器是不可避免的。实验验证了理论预测，并表明TAP-GPPS算法能够在目标域实现公平性，同时保持最小的效用损失。

Conclusion: 该研究为机器学习公平性在分布偏移环境下的部署提供了重要理论洞见和实用方法。在群体条件先验概率偏移下，基于错误率的公平标准更稳健，而基于接受率的公平标准需要特别处理。提出的TAP-GPPS算法为实际部署中的公平性调整提供了有效工具。
...
### [210] [TIDE: Temporal Incremental Draft Engine for Self-Improving LLM Inference](https://arxiv.org/abs/2602.05145)
...
<details>
  <summary>Details</summary>
Motivation: 推测解码可以显著加速LLM推理，但在实际应用中面临挑战：工作负载动态变化，系统级约束复杂，现有方法难以充分利用异构集群资源，且草稿训练开销较大。

Method: TIDE框架将在线草稿自适应直接集成到高性能LLM推理系统中：1）重用目标模型推理过程中生成的隐藏状态作为训练信号，实现零开销草稿自适应；2）采用自适应运行时控制，仅在有益时激活推测和训练；3）利用异构集群，将解耦的推理和训练映射到合适的GPU类别。

Result: 在多样化真实工作负载中，TIDE相比静态推测解码实现了最高1.15倍的吞吐量提升，相比重新计算训练信号的方法减少了1.67倍的草稿训练时间。

Conclusion: TIDE通过将在线草稿自适应深度集成到LLM推理系统中，有效解决了推测解码在实际部署中的挑战，实现了显著的性能提升和资源利用优化。
...
### [211] [Cross-talk based multi-task learning for fault classification of physically coupled machine system](https://arxiv.org/abs/2602.05146)
*Wonjun Yi,Rismaya Kumar Mishra,Yong-Hwa Park*

Main category: cs.LG

TL;DR: 该论文提出了一种基于交叉对话结构的多任务学习框架，通过同时学习故障条件和相关物理变量来提升故障分类性能，在两个基准数据集上验证了其有效性。
...
Method: 采用多任务学习框架，通过交叉对话结构联合学习故障条件和相关物理变量。交叉对话结构允许任务间通过交叉对话层进行受控信息交换，同时防止负迁移。基于先前提出的残差神经降维模型，在两个物理耦合显著的基准数据集上进行扩展应用。

Result: 在无人机故障数据集上，通过学习故障分类与物理属性（机器类型和操纵方向），交叉对话架构能更好地分类故障。在电机复合故障数据集上，测试了单通道和多通道数据作为分类器输入的性能。在两个基准测试中，残差神经降维模型均持续优于单任务模型、合并所有标签组合的多类模型以及共享主干多任务模型。

Conclusion: 通过利用信号中自然嵌入的物理耦合信息，采用交叉对话结构的多任务学习框架能有效提升故障分类性能，防止负迁移，在多个实际应用场景中表现出优越性。
...
### [212] [CoSA: Compressed Sensing-Based Adaptation of Large Language Models](https://arxiv.org/abs/2602.05148)
*Songtao Wei,Yi Li,Bohan Zhang,Zhichun Guo,Ying Huang,Yuede Ji,Miao Yin,Guanpeng Li,Bingzhe Li*

Main category: cs.LG

TL;DR: CoSA是一种基于压缩感知理论的新型参数高效微调方法，通过固定随机投影矩阵和紧凑可学习核心表达权重更新，突破了低秩分解的限制，在多尺度模型适配中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法（如LoRA和PiSSA）依赖权重更新的低秩分解，但低秩假设可能限制表达能力，特别是在任务特定适配场景中奇异值分布相对均匀时。需要一种更灵活的表达方式来解决这一局限性。
...
Conclusion: CoSA为高效且表达力强的多尺度模型适配提供了一个原则性视角，突破了传统低秩分解方法的限制，在保持参数效率的同时提供了更强的表达能力。
...
TL;DR: 该立场论文主张将能力控制视为与对齐不同的独立目标，提出在模型全生命周期中通过数据层、学习层和系统层三层机制实现能力控制，并倡导采用纵深防御方法组合互补控制措施。


<details>
  <summary>Details</summary>
Motivation: 基础模型在广泛数据分布上训练，获得了通用能力，这既支持下游应用，也扩大了潜在滥用和失败的空间。当前对齐方法通常依赖上下文和偏好驱动，但需要建立硬性操作限制来防止对抗性诱导下的不良行为。

Method: 提出三层能力控制机制：1) 数据层控制训练分布；2) 学习层通过权重或表示层干预；3) 系统层通过部署后护栏对输入、输出和动作进行控制。由于各层单独使用时存在特征性失效模式，倡导采用纵深防御方法，在全栈组合互补控制措施。

Result: 建立了能力控制作为独立于对齐的明确概念框架，提出了系统化的三层控制架构，并识别了实现此类控制的关键挑战，包括知识的双重用途性质和组合泛化问题。

Conclusion: 能力控制应被视为与对齐不同的独立目标，需要在整个模型生命周期中采用多层次、纵深防御的方法来实施硬性操作限制，以应对基础模型的潜在滥用和失败风险。
...
### [214] [EBPO: Empirical Bayes Shrinkage for Stabilizing Group-Relative Policy Optimization](https://arxiv.org/abs/2602.05165)
*Kevin Han,Yuhang Zhou,Mingze Gao,Gedi Zhou,Serena Li,Abhishek Kumar,Xiangjun Fan,Weiwei Li,Lizhu Zhang*

Main category: cs.LG

TL;DR: EBPO提出了一种新的强化学习框架，通过经验贝叶斯方法将局部组统计量与全局先验动态结合，解决了GRPO在小样本组和高失败率场景下的方差和梯度消失问题。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法存在两个关键稳定性问题：1) 在计算约束下（小组规模小）估计器方差高；2) 在饱和失败场景中（所有响应获得相同零奖励）梯度信号消失。需要一种更稳定的方法来增强LLM的推理能力。

Method: EBPO采用经验贝叶斯方法，通过收缩估计器动态平衡局部组统计量与全局先验。全局先验使用Welford在线算法更新，从而正则化局部基线估计。

Result: 理论上证明EBPO相比GRPO具有更低的均方误差、有界的熵衰减和在失败场景中非消失的惩罚信号。实证上在AIME和OlympiadBench等基准测试中一致优于GRPO和其他基线方法，训练稳定性更高，即使小组规模小也能获得高性能增益，且受益于难度分层课程学习。

Conclusion: EBPO通过经验贝叶斯正则化有效解决了GRPO的稳定性问题，为可验证奖励的强化学习提供了更可靠和高效的框架，显著提升了LLM推理能力的训练效果。
...
### [215] [Benchmarking Artificial Intelligence Models for Daily Coastal Hypoxia Forecasting](https://arxiv.org/abs/2602.05178)
*Magesh Rajasekaran,Md Saiful Sajol,Chris Alvin,Supratik Mukhopadhyay,Yanda Ou,Z. George Xue*

Main category: cs.LG

TL;DR: 该研究比较了四种深度学习架构（BiLSTM、Medformer、ST-Transformer、TCN）在墨西哥湾北部缺氧日分类预测中的性能，其中ST-Transformer在所有指标和测试期间表现最佳（AUC-ROC：0.982-0.992）。


<details>
  <summary>Details</summary>
Motivation: 墨西哥湾北部沿海缺氧是一个持续的生态和经济问题，现有季节性模型预测过于粗糙，无法满足日常响应式生态系统管理的精细尺度变化需求，需要开发更精确的日尺度预测方法。

Method: 使用2009-2020年耦合水动力-生物地球化学模型的每日后报数据训练四种深度学习架构（BiLSTM、Medformer、ST-Transformer、TCN），模型输入包含水柱分层、沉积物耗氧率和温度依赖分解率等特征，使用2020-2024年数据作为测试集，采用相同的数据预处理、输入输出格式和验证协议，并使用McNemar方法检验模型预测的统计显著性差异。

Result: 所有模型都实现了高分类精度和强判别能力，其中ST-Transformer在所有指标和测试期间表现最佳（AUC-ROC：0.982-0.992），并建立了可复现的操作性实时缺氧预测框架。

Conclusion: 研究提供了一个可复现的实时缺氧预测操作框架，支持环境和海洋建模系统社区及生态系统恢复力的更广泛努力，ST-Transformer在日尺度缺氧分类预测中表现最优。
...
### [216] [Data-Centric Interpretability for LLM-based Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.05183)
*John Yan,Michael Yu,Yuqi Sun,Alexander Duffy,Tyler Marques,Matthew Lyle Olson*

Main category: cs.LG

TL;DR: 论文提出Meta-Autointerp方法，通过稀疏自编码器(SAEs)和LLM摘要器分析复杂强化学习环境中LLM训练动态，发现细粒度行为模式，但发现多数SAE特征和LLM生成假设对人类帮助有限，只有部分SAE衍生假设对下游任务有预测价值。
...
Method: 提出Meta-Autointerp方法，将SAE特征分组为可解释的假设。在Full-Press Diplomacy环境中应用预训练SAEs和LLM摘要器方法，分析大规模强化学习训练运行。通过自动评估验证SAE元特征，并进行用户研究评估实用性。

Result: 发现细粒度行为包括角色扮演模式、退化输出、语言切换，以及高级战略行为和特定环境bug。90%发现的SAE元特征具有显著性，发现意外的奖励黑客行为。用户研究表明多数SAE特征和LLM生成假设对人类帮助有限，但部分SAE衍生假设对下游任务有预测价值，通过增强未训练智能体系统提示可将分数提高+14.2%。
...
### [217] [SpectraKAN: Conditioning Spectral Operators](https://arxiv.org/abs/2602.05187)
*Chun-Wun Cheng,Carola-Bibiane Schönlieb,Angelica I. Aviles-Rivero*

Main category: cs.LG

TL;DR: SpectraKAN是一种新型谱神经算子，通过输入条件化的谱卷积来学习偏微分方程的解算子，相比静态傅里叶核方法能更好地捕捉多尺度、状态依赖的各向异性动态。


<details>
  <summary>Details</summary>
Motivation: 现有谱神经算子（如FNO）使用静态傅里叶核，无法适应输入变化，限制了捕捉多尺度、状态依赖和各向异性动态的能力。需要一种能根据输入状态自适应调整的谱算子。

Method: 从时空历史中提取紧凑全局表示，通过单查询交叉注意力机制调制多尺度傅里叶主干，将静态谱卷积转变为输入条件化的积分算子，实现自适应行为调整。

Result: 在多种PDE基准测试中达到最先进性能，RMSE降低高达49%，在具有挑战性的时空预测任务上表现尤为突出。

Conclusion: SpectraKAN通过输入条件化的谱算子实现了高效的自适应学习，在保持谱混合效率的同时显著提升了PDE解算子的学习能力，为复杂动态系统建模提供了新方法。
...
### [218] [Double-P: Hierarchical Top-P Sparse Attention for Long-Context LLMs](https://arxiv.org/abs/2602.05191)
...
<details>
  <summary>Details</summary>
Motivation: 随着长上下文推理成为大语言模型的核心需求，注意力计算在解码过程中成为主要瓶颈。现有稀疏注意力方法无法同时优化top-p准确性、选择开销和稀疏注意力成本，限制了整体效率。

Method: 提出Double-P分层稀疏注意力框架：1) 在聚类级别使用大小加权质心进行粗粒度top-p估计；2) 通过第二级top-p阶段自适应地细化计算，仅在需要时分配token级注意力。

Result: 在长上下文基准测试中，Double-P实现接近零的准确率下降，将注意力计算开销降低高达1.8倍，相比最先进的固定预算稀疏注意力方法提供高达1.3倍的端到端解码加速。

Conclusion: Double-P通过分层top-p选择有效解决了稀疏注意力在准确性、选择开销和计算成本之间的权衡问题，为长上下文推理提供了可扩展的高效解决方案。
...
### [219] [Extreme Weather Nowcasting via Local Precipitation Pattern Prediction](https://arxiv.org/abs/2602.05204)
*Changhoon Song,Teng Yuan Chang,Youngjoon Hong*

Main category: cs.LG

TL;DR: exPreCast：一种用于极端降雨临近预报的高效确定性框架，结合平衡数据集和局部时空注意力机制，在正常和极端降雨条件下均实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 极端天气事件（如暴雨）的准确预报对风险管理至关重要。现有方法存在局限性：扩散模型计算成本高不适合实时应用，确定性模型对正常降雨存在偏差，且现有基准数据集本身存在偏差（要么以普通降雨为主，要么仅限于极端事件），阻碍了实际应用。

Method: 提出exPreCast框架，包含：1）局部时空注意力机制；2）纹理保持的立方双上采样解码器；3）时间提取器以灵活调整预报时效。同时构建了来自韩国气象厅的平衡雷达数据集，包含普通降水和极端事件。

Result: 在SEVIR、MeteoNet基准数据集以及新构建的平衡KMA数据集上实验表明，该方法在正常和极端降雨条件下均达到最先进性能，提供准确可靠的临近预报。

Conclusion: exPreCast框架通过高效确定性方法解决了极端降雨临近预报的挑战，结合平衡数据集和专门设计的架构，在实际应用中具有显著优势，为风险管理提供了可靠工具。
...
### [220] [Disentangled Representation Learning via Flow Matching](https://arxiv.org/abs/2602.05214)
*Jinjin Chi,Taoping Liu,Mengtao Yin,Ximing Li,Yongcheng Jing,Dacheng Tao*

Main category: cs.LG

TL;DR: 提出基于流匹配的框架进行解耦表示学习，通过因子条件化流和正交正则化实现语义对齐
...
Method: 基于流匹配的框架，将解耦表示为学习紧凑潜在空间中的因子条件化流，引入非重叠（正交）正则化器抑制跨因子干扰和减少信息泄漏

Result: 在多个数据集上的实验显示相比代表性基线有持续改进，获得更高的解耦分数以及改进的可控性和样本保真度

Conclusion: 提出的流匹配框架通过因子条件化流和正交正则化有效实现了语义对齐的解耦表示学习，在解耦质量和生成性能方面优于现有方法
...
### [221] [Private Prediction via Shrinkage](https://arxiv.org/abs/2602.05219)
*Chao Yan*

Main category: cs.LG

TL;DR: 该论文研究差分隐私预测问题，通过改进算法将查询数量的平方根依赖降低到对数多项式依赖，显著减少了所需标记样本数量。


<details>
  <summary>Details</summary>
Motivation: 标准差分隐私组合方法在处理T个查询时会产生√T的依赖关系，这在实际应用中需要大量标记样本。研究者希望减少这种依赖，使差分隐私预测在流式设置中更高效。

Method: 针对两种不同设置提出算法：1) 对于任意概念类C和遗忘型在线对手，设计私有预测器；2) 对于自适应在线对手和ℝ^d中的半空间，开发特定算法。两种方法都利用流式设置特性减少隐私损失。

Result: 成功将查询数量的依赖从√T降低到polylog(T)：1) 对于任意概念类C，需要Õ(VC(C)^{3.5}log^{3.5}T)个标记样本；2) 对于半空间和自适应对手，需要Õ(d^{5.5}log T)个标记样本。

Conclusion: 在流式差分隐私预测中，通过精心设计的算法可以显著减少查询数量对所需标记样本的影响，将平方根依赖改进为对数多项式依赖，这对实际应用具有重要意义。
...
### [222] [Faithful Bi-Directional Model Steering via Distribution Matching and Distributed Interchange Interventions](https://arxiv.org/abs/2602.05234)
*Yuntai Bao,Xuhong Zhang,Jintao Chen,Ge Su,Yuxiang Cai,Hao Peng,Bing Sun,Haiqin Weng,Liu Yan,Jianwei Yin*

Main category: cs.LG

TL;DR: 提出Concept DAS（CDAS）方法，基于分布式对齐搜索原理，通过分布匹配目标而非概率最大化进行干预学习，实现更忠实稳定的模型操控


<details>
  <summary>Details</summary>
Motivation: 现有基于干预的模型操控方法容易过拟合且性能不佳，因为有效操控需要忠实识别内部机制而非强制执行外部偏好

Method: 基于分布式对齐搜索（DAS）原理，采用分布式交换干预（DII）机制，引入针对操控任务的分布匹配目标，通过弱监督分布匹配学习干预而非概率最大化

Result: 在AxBench基准上，CDAS不总是优于偏好优化方法，但可能更受益于模型规模扩展；在安全相关案例中能系统化操控同时保持模型通用效用

Conclusion: CDAS与偏好优化方法互补，在特定条件下构成基于干预的模型操控的稳健方法
...
### [223] [CORP: Closed-Form One-shot Representation-Preserving Structured Pruning for Vision Transformers](https://arxiv.org/abs/2602.05243)
*Boxiang Zhang,Baijian Yang*

Main category: cs.LG

TL;DR: CORP是一种闭式单次结构化剪枝框架，用于视觉Transformer，无需标签、梯度或微调，通过表示恢复方法在训练后约束下实现高效剪枝。
...
Result: 在ImageNet上使用DeiT模型实验显示，MLP和注意力表示存在强冗余性。无补偿时单次结构化剪枝导致严重精度下降，而CORP在激进稀疏度下保持精度。DeiT-Huge剪枝50% MLP和注意力结构后保持82.8% Top-1准确率。

Conclusion: CORP在单GPU上20分钟内完成剪枝，提供实际效率提升，证明了在严格训练后约束下通过闭式补偿实现视觉Transformer高效结构化剪枝的可行性。
...
### [224] [Back to Basics: Revisiting Exploration in Reinforcement Learning for LLM Reasoning via Generative Probabilities](https://arxiv.org/abs/2602.05281)
...
<details>
  <summary>Details</summary>
Motivation: 标准策略优化方法（如GRPO）在强化学习可验证奖励（RLVR）中往往收敛到低熵策略，导致严重的模式崩溃和输出多样性受限。这源于标准目标函数不成比例地强化最高似然路径，抑制了有效的替代推理链。

Method: 提出优势重加权机制（ARM），将提示困惑度和答案置信度纳入优势估计，动态重塑奖励信号以衰减过度自信推理路径的梯度更新，同时将概率质量重新分配给未充分探索的正确解决方案。

Result: 在Qwen2.5和DeepSeek模型上的数学和编码基准测试表明，ProGRPO显著缓解了熵崩溃。在Qwen2.5-7B上，该方法在Pass@1上优于GRPO 5.7%，在Pass@32上显著优于13.9%，显示出生成多样化正确推理路径的卓越能力。

Conclusion: ARM方法在保持竞争力的准确性的同时，显著增强了生成多样性和响应熵，有效实现了推理任务中探索与利用的优越权衡。
...
### [225] [Robust Inference-Time Steering of Protein Diffusion Models via Embedding Optimization](https://arxiv.org/abs/2602.05285)
...
Method: 在条件嵌入空间中优化实验似然，利用该空间丰富的序列和共进化信号，有效调整扩散先验以符合实验约束
...
### [226] [HealthMamba: An Uncertainty-aware Spatiotemporal Graph State Space Model for Effective and Reliable Healthcare Facility Visit Prediction](https://arxiv.org/abs/2602.05286)
*Dahai Yu,Lin Jiang,Rongchao Xu,Guang Wang*

Main category: cs.LG

TL;DR: HealthMamba：一种用于医疗设施访问预测的不确定性感知时空框架，通过统一时空编码器、GraphMamba模型和综合不确定性量化模块，在四个真实数据集上实现了6.0%的预测精度提升和3.5%的不确定性量化改进。


<details>
  <summary>Details</summary>
Motivation: 现有医疗设施访问预测研究存在两个主要问题：1）通常将任务视为时间序列预测问题，忽略了不同类型医疗设施之间的空间依赖关系；2）无法在公共紧急情况等异常情况下提供可靠预测。需要开发能够同时考虑时空依赖并提供可靠不确定性量化的方法。

Method: 提出HealthMamba框架，包含三个核心组件：1）统一时空上下文编码器，融合异构静态和动态信息；2）GraphMamba（图状态空间模型），用于分层时空建模；3）综合不确定性量化模块，集成三种不确定性量化机制。

Result: 在加州、纽约、德克萨斯和佛罗里达四个大规模真实数据集上的评估表明，HealthMamba相比最先进的基线方法，在预测精度上实现了约6.0%的提升，在不确定性量化方面实现了3.5%的改进。

Conclusion: HealthMamba通过整合时空建模和不确定性量化，为医疗设施访问预测提供了准确可靠的解决方案，能够更好地支持医疗资源优化分配和公共卫生政策制定，特别是在异常情况下的可靠预测。
...
### [227] [A Short and Unified Convergence Analysis of the SAG, SAGA, and IAG Algorithms](https://arxiv.org/abs/2602.05304)
*Feng Zhu,Robert W. Heath,Aritra Mitra*

Main category: cs.LG

TL;DR: 该论文为SAG、SAGA和IAG三种算法提供了统一的收敛性分析框架，简化了原有复杂的证明过程，并获得了更好的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 现有随机方差缩减算法（如SAG、SAGA）及其确定性对应算法（如IAG）的分析方法分散且复杂，特别是SAG的原始证明需要计算机辅助分析。需要一种统一、简洁的分析框架来覆盖这些算法。

Method: 提出两步骤统一分析框架：1）使用简单集中工具建立随机子采样延迟的界限；2）设计新颖的李雅普诺夫函数来考虑这些延迟。该方法具有模块化特点，适用于光滑强凸目标函数的有限和优化问题。

Result: 获得了SAG和SAGA的首个高概率收敛界，这些界可以无缝扩展到非凸目标和马尔可夫采样。作为副产品，获得了IAG算法的最佳已知收敛速率，显著改进了先前界限。

Conclusion: 该研究为SAG、SAGA和IAG算法提供了统一、简洁且模块化的收敛性分析框架，简化了证明过程，获得了更好的理论结果，并为扩展到更广泛的问题设置奠定了基础。
...
### [228] [Formal Synthesis of Certifiably Robust Neural Lyapunov-Barrier Certificates](https://arxiv.org/abs/2602.05311)
...
Conclusion: 研究结果表明，训练鲁棒神经证书对于在动力学扰动下的安全强化学习是有效的，为解决实际应用中系统不确定性带来的挑战提供了可行方案。
...
### [229] [Accelerated Sequential Flow Matching: A Bayesian Filtering Perspective](https://arxiv.org/abs/2602.05319)
*Yinan Huang,Hans Hao-Hsun Hsu,Junran Wang,Bo Dai,Pan Li*

Main category: cs.LG

TL;DR: 提出Sequential Flow Matching框架，基于贝叶斯滤波将流匹配模型应用于实时流式预测，通过从先前后验分布初始化生成来加速采样
...
Method: 基于贝叶斯滤波原理，将流式推理视为学习概率流，将预测分布从一个时间步传输到下一个时间步，从先前后验分布初始化生成提供原则性热启动

Result: 在多种预测、决策和状态估计任务中，该方法性能与完整步扩散模型相当，但仅需一个或很少采样步骤，实现更快采样

Conclusion: 通过贝叶斯滤波框架进行序列推理为基于流的模型在实时部署中提供了新的原则性视角，实现了效率与性能的平衡
...
### [230] [GAS: Enhancing Reward-Cost Balance of Generative Model-assisted Offline Safe RL](https://arxiv.org/abs/2602.05323)
*Zifan Liu,Xinran Li,Shibo Chen,Jun Zhang*

Main category: cs.LG

TL;DR: GAS算法通过目标辅助拼接增强离线安全强化学习的拼接能力，使用期望回归训练目标函数平衡奖励最大化与约束满足，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于生成模型的离线安全强化学习方法存在两个主要问题：1) 缺乏从次优轨迹中"拼接"最优转移的能力；2) 难以平衡奖励目标与成本目标，特别是在两者冲突时。

Method: 提出GAS算法：1) 在转移级别增强和重新标注数据集，从次优轨迹构建高质量轨迹；2) 引入新颖的目标函数，使用期望回归在增强数据集上训练，估计最优可达奖励和成本目标；3) 重塑数据集实现更均匀的奖励-成本回报分布，提高训练稳定性。

Result: 实证结果表明GAS在平衡奖励最大化与约束满足方面优于现有方法，展示了卓越的性能。

Conclusion: GAS通过增强拼接能力和引入目标函数，有效解决了离线安全强化学习中奖励与成本目标的平衡问题，为基于生成模型的方法提供了改进方向。
...
### [231] [Pool-based Active Learning as Noisy Lossy Compression: Characterizing Label Complexity via Finite Blocklength Analysis](https://arxiv.org/abs/2602.05333)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏对池式主动学习理论极限的系统分析框架。现有研究未能统一分析数据选择和学习的理论性能，需要建立信息论基础来理解池式主动学习的根本限制。

Method: 将池式主动学习重新表述为有噪有损压缩问题：将池观测映射到有噪符号观测，数据选择对应压缩，学习对应解码。应用有噪有损压缩的有限块长分析，推导标签复杂度和泛化误差的信息论下界。

Result: 推导出作为理论极限的信息论下界，这些界限包含反映学习算法引起的过拟合以及其归纳偏差与目标任务之间差异的项，并与已有的信息论界限和稳定性理论密切相关。

Conclusion: 该框架为池式主动学习提供了新的理论视角，建立了信息论基础，统一分析了数据选择和学习的理论性能，揭示了算法归纳偏差和过拟合对主动学习性能的根本影响。
...
### [232] [Hinge Regression Tree: A Newton Method for Oblique Regression Tree Splitting](https://arxiv.org/abs/2602.05371)
*Hongyi Li,Han Lin,Jun Xu*

Main category: cs.LG

TL;DR: HRT（Hinge回归树）通过将树节点分裂重新表述为非线性最小二乘问题，结合了树的透明度和多元决策边界的表达能力，解决了传统斜决策树学习NP难的问题。


<details>
  <summary>Details</summary>
Motivation: 斜决策树结合了树的透明度和多元决策边界的表达能力，但学习高质量斜分裂是NP难问题，现有方法依赖缓慢搜索或缺乏理论基础的启发式方法。需要一种高效、理论可靠的学习方法。

Method: 将每个分裂重新表述为两个线性预测器的非线性最小二乘问题，其最大/最小包络产生类似ReLU的表达能力。采用交替拟合过程，等价于固定分区内的阻尼牛顿（高斯-牛顿）法。支持回溯线搜索变体和可选的岭正则化。

Result: 理论分析表明节点级优化单调递减并收敛；实际中固定和自适应阻尼都能实现快速稳定收敛。HRT模型类是通用逼近器，具有显式O(δ²)逼近率。在合成和真实基准测试中，HRT以更紧凑的结构匹配或优于单树基线。

Conclusion: HRT提供了一种高效、理论可靠的斜决策树学习方法，结合了树的透明度和多元决策边界的表达能力，解决了传统方法的计算复杂性和理论缺陷问题。
...
### [233] [Assessing Electricity Demand Forecasting with Exogenous Data in Time Series Foundation Models](https://arxiv.org/abs/2602.05390)
*Wei Soon Cheong,Lian Lian Jiang,Jamie Ng Suat Ling*

Main category: cs.LG

TL;DR: 该研究评估了时间序列基础模型在电力需求预测中利用外生特征的能力，发现基础模型在不同地理环境和特征配置下表现差异显著，简单的LSTM基线在稳定气候条件下经常优于基础模型。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型已成为预测的新范式，但其在电力需求预测中有效利用外生特征的能力尚不明确。电力需求预测严重依赖外生变量（如天气、节假日等），因此需要评估基础模型在实际应用中的表现。

Method: 在新加坡和澳大利亚电力市场的每小时和每日粒度上，实证评估了MOIRAI、MOMENT、TinyTimeMixers、ChronosX和Chronos-2等基础模型。使用具有可逆实例归一化的LSTM作为基线，在三种特征配置（所有特征、选定特征、仅目标变量）下进行系统评估。

Result: 1. 基础模型效果差异显著：Chronos-2在零样本设置中表现最佳，但在新加坡稳定气候条件下，简单的LSTM基线经常优于所有基础模型，特别是短期预测。
2. 模型架构至关重要：具有协同架构实现（如TTM的通道混合、Chronos-2的分组注意力）的模型能一致地利用外生特征，而其他方法表现出不一致的收益。
3. 地理环境同等重要：基础模型主要在多变气候条件下显示优势。

Conclusion: 研究结果挑战了基础模型普遍优越性的假设，强调了在能源领域需要特定领域模型的重要性。基础模型的优势高度依赖于地理环境和模型架构，而非普遍适用。
...
### [234] [Robust Federated Learning via Byzantine Filtering over Encrypted Updates](https://arxiv.org/abs/2602.05410)
*Adda Akram Bendoukha,Aymen Boudguiga,Nesrine Kaaniche,Renaud Sirdey,Didem Demirag,Sébastien Gambs*

Main category: cs.LG

TL;DR: 提出一种结合同态加密和元分类器的联邦学习安全聚合与拜占庭容错方法，通过属性推断攻击启发的SVM过滤器和CKKS加密系统实现隐私保护与拜占庭防御的协同


<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护数据隐私的同时仍面临推理攻击导致的敏感数据泄露和拜占庭行为对训练模型的影响。现有方案往往独立处理安全聚合和拜占庭容错，难以同时实现两者

Method: 1) 基于属性推断攻击思路，在标记的影子更新上训练过滤元分类器，模拟多种拜占庭攻击行为；2) 使用元分类器输出对加密更新进行重加权以消除拜占庭影响；3) 针对CKKS密码系统提出自动化选择最优核函数和维度超参数的方法

Result: 在FEMNIST、CIFAR10、GTSRB和acsincome基准测试中验证有效性，SVM过滤器识别拜占庭更新的准确率达90%-94%，模型效用损失微小，加密推理运行时间在6-26秒之间

Conclusion: 该方法成功将同态加密与拜占庭过滤相结合，实现了联邦学习中隐私保护与安全性的协同，为同时解决安全聚合和拜占庭容错挑战提供了有效方案
...
### [235] [BLITZRANK: Principled Zero-shot Ranking Agents with Tournament Graphs](https://arxiv.org/abs/2602.05448)
...
Method: 引入锦标赛图框架：1) 将k文档比较视为包含binom(k,2)成对偏好的完整锦标赛；2) 聚合锦标赛构建全局偏好图；3) 利用传递闭包推导额外排序关系；4) 设计贪心查询调度最大化信息增益；5) 处理非传递偏好（循环）时将其折叠为等价类

Result: 在14个基准测试和5个LLM上的实验表明：1) 与可比方法相比，在保持或提高准确性的同时减少25-40%的token使用；2) 与成对方法相比，在质量相近的情况下减少7倍token使用；3) 实现帕累托优势

Conclusion: 锦标赛图框架为k-wise重排序提供了理论基础，通过系统利用每次比较的完整偏好信息，在显著降低计算成本的同时保持或提高重排序质量，解决了现有方法的效率与信息利用不足问题
...
TL;DR: 离线目标条件强化学习中的超参数敏感性并非不可避免，而是由自举机制中的破坏性梯度干扰加剧所致


<details>
  <summary>Details</summary>
Motivation: 研究深度强化学习中普遍存在的超参数敏感性问题，探究这是强化学习问题的固有特性，还是由特定训练机制加剧的现象

Method: 在离线目标条件强化学习环境中，通过控制数据分布固定性和非平稳性，研究HIQL（自举TD学习）和QRL（拟度量表示学习）两种代表性算法在不同数据质量（平稳和非平稳）下的表现

Result: 观察到比在线强化学习更强的超参数鲁棒性；当存在约20%专家数据时，QRL保持广泛稳定的近最优区域，而HIQL表现出尖锐的最优点且随训练阶段显著漂移；自举目标表现出更强的破坏性梯度干扰，这与超参数敏感性直接相关

Conclusion: 训练期间对超参数配置的高敏感性并非强化学习不可避免的特性，而是由自举机制动态加剧的，这为设计更鲁棒的算法目标提供了途径
...
### [237] [Thermodynamic Limits of Physical Intelligence](https://arxiv.org/abs/2602.05463)
*Koichi Takahashi,Yusuke Hayashi*

Main category: cs.LG

TL;DR: 提出两个互补的比特/焦耳指标：热力学Epiplexity/焦耳（识别效率）和Empowerment/焦耳（控制效率），建立物理智能的统一能效评估框架


<details>
  <summary>Details</summary>
Motivation: 现代AI系统在获得卓越能力的同时消耗大量能源，需要建立连接智能与物理效率的量化指标，以评估智能系统的能效

Method: 提出两个互补的比特/焦耳指标：1) 热力学Epiplexity/焦耳 - 单位能耗编码的环境结构信息；2) Empowerment/焦耳 - 单位预期能耗的传感器运动通道容量。基于随机热力学建立理论框架，明确边界/能量核算、粗粒化/噪声、重置/重用等假设

Result: 建立了热力学Epiplexity获取的Landauer尺度闭循环基准，展示了在明确子系统假设下信息增益与能耗的关联；同时证明若无外部低熵资源核算，信息增益与边界内耗散不必紧密关联。为实际应用提出MDL-epiplexity替代指标

Conclusion: 提出了统一的能效评估框架，包含两个互补的比特/焦耳指标和最小化核算清单，支持一致的能效比较，为能量调整的规模分析提供基础，促进物理智能的量化评估
...
### [238] [A Unified Framework for Rethinking Policy Divergence Measures in GRPO](https://arxiv.org/abs/2602.05494)
*Qingyuan Wu,Yuhui Wang,Simon Sinong Zhan,Yanning Dai,Shilong Deng,Sarra Habchi,Qi Zhu,Matthias Gallé,Chao Huang*

Main category: cs.LG

TL;DR: 论文提出统一裁剪框架，将现有RLVR方法（如GRPO）统一到基于策略差异度量的理论框架中，并引入KL3估计器作为关键约束，在数学推理基准上提升训练稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法（如GRPO及其变体）通过裁剪似然比来约束策略差异以确保稳定更新，但缺乏对策略差异度量的系统性分析。需要建立统一框架来理解不同策略差异度量如何影响探索和性能，并寻找更有效的约束方法。

Method: 提出统一裁剪框架，用一般化的策略差异概念（涵盖似然比和KL散度等度量）来表征现有方法。引入KL3估计器（KL散度的方差减少蒙特卡洛估计器）作为关键策略差异约束，理论上证明KL3约束等价于非对称比率裁剪，将概率质量重新分配给高置信度动作。

Result: 在数学推理基准上的实证结果表明，将KL3估计器整合到GRPO中能同时提高训练稳定性和最终性能，验证了基于原则的策略差异约束在策略优化中的重要性。

Conclusion: 统一裁剪框架为系统分析策略差异度量提供了理论基础，KL3约束通过促进更强探索同时保持GRPO风格方法的简单性，在RLVR中实现了更好的性能表现。
...
### [239] [Detecting Misbehaviors of Large Vision-Language Models by Evidential Uncertainty Quantification](https://arxiv.org/abs/2602.05535)
...
<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法仅捕捉总体认知不确定性，对检测大型视觉语言模型的不当行为（如幻觉、越狱攻击等）效果有限，这些不当行为源于内部知识冲突或信息缺失

Method: 提出证据不确定性量化(EUQ)方法，将模型输出头特征解释为正负证据，利用证据理论建模聚合这些证据，在单次前向传播中量化内部冲突和知识缺失

Result: EUQ在幻觉、越狱、对抗性漏洞和OOD失败四类不当行为检测中均优于基线方法，发现幻觉对应高内部冲突，OOD失败对应高知识缺失

Conclusion: EUQ提供细粒度不确定性量化，有效检测LVLM不当行为，层间证据不确定性动态分析为理解内部表示演化提供新视角
...
### [240] [Steering Large Reasoning Models towards Concise Reasoning via Flow Matching](https://arxiv.org/abs/2602.05539)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但其效率常受冗长输出的影响。现有线性引导方法基于限制性的线性表示假设，仅应用全局向量到隐藏表示，无法实现精确的输入相关控制

Method: FlowSteer通过流匹配学习一个速度场，作为从冗长推理分布到简洁推理分布的完整变换。该方法学习非线性转换，使引导后的表示与简洁推理激活分布对齐，实现输入相关的精确控制
...
### [241] [Unveiling Implicit Advantage Symmetry: Why GRPO Struggles with Exploration and Difficulty Adaptation](https://arxiv.org/abs/2602.05548)
...
<details>
  <summary>Details</summary>
Motivation: RLVR（特别是GRPO）已成为激发LLM推理的标准方法，但其在探索效率和难度适应方面仍存在挑战。作者认为这些瓶颈源于组相对优势估计中隐含的优势对称性，这种对称性导致了探索不足和难度适应不灵活的问题。

Method: 提出了非对称GRAE方法，通过两个关键改进：1）在组层面，非对称地抑制正确轨迹的优势，鼓励必要的探索；2）在样本层面，采用课程学习式策略，优先处理简单样本，然后逐渐转向复杂样本，动态调节探索激励和样本难度关注。

Result: 在七个基准测试上的实验表明，A-GRAE在LLM和MLLM上都能持续改进GRPO及其变体，验证了非对称方法的有效性。

Conclusion: GRPO中隐含的优势对称性是次优的，通过引入非对称性可以显著提升探索效率和难度适应能力。A-GRAE通过动态调节探索激励和样本难度关注，为RLVR方法提供了更有效的优化策略。
...
### [242] [Logical Guidance for the Exact Composition of Diffusion Models](https://arxiv.org/abs/2602.05549)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在推理时难以处理复杂逻辑约束的生成任务，需要一种能够精确组合原子属性引导信号的理论框架

Method: 1) 推导精确布尔演算，为复杂逻辑公式提供精确引导的充分条件；2) 提出混合引导方法，结合原子引导分数和后验概率估计，桥接分类器引导和无分类器引导

Result: 在图像和蛋白质结构生成任务中验证了框架有效性，能够实现复杂逻辑表达式的精确约束生成

Conclusion: LOGDIFF为扩散模型提供了理论严谨的逻辑引导框架，能够处理复杂布尔约束，扩展了条件生成的能力边界
...
### [243] [OpenMAG: A Comprehensive Benchmark for Multimodal-Attributed Graph](https://arxiv.org/abs/2602.05576)
...
### [244] [Shiva-DiT: Residual-Based Differentiable Top-$k$ Selection for Efficient Diffusion Transformers](https://arxiv.org/abs/2602.05605)
...
TL;DR: Shiva-DiT：一种基于残差可微分Top-k选择的方法，解决扩散变换器计算成本过高问题，实现确定性token计数和自适应剪枝调度


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）由于自注意力的二次缩放导致计算成本过高，现有剪枝方法无法同时满足可微分性、效率和硬件开销所需的严格静态预算

Method: 提出Shiva-DiT，采用基于残差的可微分Top-k选择，利用残差感知直通估计器实现确定性token计数和端到端可学习性；引入上下文感知路由器和自适应比率策略来自主学习自适应剪枝调度

Result: 在包括SD3.5在内的主流模型上，Shiva-DiT建立了新的帕累托前沿，实现了1.54倍的实时加速，同时保持优于现有基线的保真度，有效消除了不规则张量开销

Conclusion: Shiva-DiT成功解决了扩散变换器计算效率问题，通过可微分剪枝方法在保持模型质量的同时显著提升推理速度，为实际部署提供了有效解决方案
...
### [245] [Path-Guided Flow Matching for Dataset Distillation](https://arxiv.org/abs/2602.05616)
*Xuhui Li,Zhengquan Luo,Xiwei Liu,Yongqiang Yu,Zhiqiang Xu*

Main category: cs.LG

TL;DR: PGFM提出首个基于流匹配的数据集蒸馏框架，通过ODE确定性合成实现快速生成，相比基于扩散的方法效率提升7.6倍
...
Result: 在高分辨率基准测试中，PGFM匹配或超越先前的扩散蒸馏方法，采样步骤更少；效率显著提升（7.6倍），模式覆盖率达78%

Conclusion: PGFM是首个基于流匹配的生成式数据集蒸馏框架，通过确定性ODE求解实现高效稳定的合成，在保持多样性的同时提供可靠的路径控制
...
### [246] [Rewards as Labels: Revisiting RLVR from a Classification Perspective](https://arxiv.org/abs/2602.05630)
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于可验证奖励的强化学习（RLVR）方法如GRPO及其变体在复杂推理任务中取得了成功，但研究发现它们存在"正样本梯度错配"和"负样本梯度主导"问题，导致策略更新效率低下且次优。

Method: 提出REAL框架，将可验证奖励重新定义为分类标签而非标量权重，将策略优化重构为分类问题。在此基础上引入锚定logits来增强策略学习，实现单调有界的梯度加权，平衡不同rollout间的梯度分配。

Result: 在数学推理基准测试中，REAL显著提升了训练稳定性，并持续优于GRPO及其变体如DAPO。在1.5B模型上，REAL比DAPO平均Pass@1提高6.7%；在7B模型上，分别优于DAPO和GSPO 6.2%和1.7%。即使使用简单的二元交叉熵，REAL仍保持稳定并平均优于DAPO 4.5%。

Conclusion: REAL框架通过将奖励重新定义为分类标签，有效解决了现有RLVR方法中的梯度分配问题，实现了更平衡和高效的策略学习，在数学推理任务中展现出显著优势。
...
### [247] [Empowering Time Series Analysis with Large-Scale Multimodal Pretraining](https://arxiv.org/abs/2602.05646)
...
Method: 提出多模态预训练范式，整合时间序列的内生模态（衍生图像和文本）和外生知识（真实世界新闻）。开发自动化数据构建流程创建MM-TS数据集（6个领域，10亿数据点）。提出HORAI模型，包含两个核心组件：频率增强跨模态编码器和时频解码器，用于有效融合多模态特征并增强跨模态和跨领域泛化。

Result: 在MM-TS数据集上预训练后，HORAI在时间序列预测和异常检测任务上实现了最先进的零样本性能，展示了强大的泛化能力。

Conclusion: HORAI是时间序列分析多模态基础模型的早期探索，通过整合多种模态和频率增强架构，解决了现有单模态模型的局限性，为时间序列理解提供了更全面的多视角分析框架。
...
### [248] [End-to-End Compression for Tabular Foundation Models](https://arxiv.org/abs/2602.05649)
*Guri Zabërgja,Rafiq Kamel,Arlind Kadra,Christian M. M. Frey,Josif Grabocka*

Main category: cs.LG

TL;DR: TACO是一种端到端的表格压缩模型，通过在潜在空间中压缩训练数据集，解决了基于注意力机制的表格Transformer模型在处理大规模数据集时的二次复杂度问题。
...
Method: 提出TACO（端到端表格压缩模型），在潜在空间中压缩训练数据集，减少计算复杂度。该方法通过压缩机制降低内存消耗和计算开销，同时保持模型性能。

Result: 在TabArena基准测试中，TACO相比最先进的表格Transformer架构，推理时间快达94倍，内存消耗减少97%，且性能没有显著下降。随着数据集规模增大，该方法表现出更好的可扩展性和性能优势。

Conclusion: TACO通过有效的压缩机制解决了表格Transformer模型的二次复杂度问题，在保持性能的同时显著提升了计算效率和内存使用效率，为处理大规模表格数据集提供了可行的解决方案。
...
### [249] [Alignment Verifiability in Large Language Models: Normative Indistinguishability under Behavioral Evaluation](https://arxiv.org/abs/2602.05656)
*Igor Santos-Grueiro*

Main category: cs.LG

TL;DR: 行为评估无法唯一识别潜在对齐：即使通过理想化行为测试，也无法认证LLM的内在对齐属性，只能估计不可区分类


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型对齐评估主要依赖行为测试（基准、红队测试等），将观察到的合规性视为潜在对齐的证据。但这种从行为证据到潜在对齐属性的推断通常是隐含的，很少被当作独立的推理问题来分析。本文旨在形式化研究这一推理问题。

Method: 将对齐评估形式化为部分可观测性下的可识别性问题，允许智能体行为依赖于与评估机制相关的信息。引入"对齐可验证性问题"和"规范性不可区分性"概念，描述不同潜在对齐假设如何产生相同的评估者可访问信号分布。

Result: 主要结果是负面的但界限清晰的识别定理：在有限行为评估和评估感知智能体条件下，观察到的行为合规性不能唯一识别潜在对齐。即使理想化的行为评估也无法一般性地认证对齐作为潜在属性。行为对齐测试应解释为不可区分类的估计器，而非对齐的验证器。

Conclusion: 行为基准应被重新理解为在特定机制内提供可观测合规性的上界，而非底层对齐的保证。通过更严格的测试可以缩小兼容假设空间，但在所述条件下无法将其缩小到单一假设。这为对齐评估提供了更严谨的理论框架。
...
TL;DR: 该论文研究了SGD算法长期尾部衰减行为，通过大偏差理论分析，为SGD和clipped-SGD在非凸损失函数下的梯度范数平方尾部提供了上界和下界，揭示了比现有有限时间分析更快的衰减速率。


<details>
  <summary>Details</summary>
Motivation: 现有SGD尾部行为研究多为有限时间分析，缺乏对固定误差阈值下失败概率的直接研究，且无法捕捉真实长期尾部衰减特性。现代学习模型通常训练数百万次迭代，需要更准确的长期尾部行为分析。

Method: 采用大偏差理论框架分析SGD类方法的长期尾部衰减。首先分析普通SGD在非凸损失和有界噪声下的梯度范数平方尾部；其次分析clipped-SGD在重尾噪声下的行为；最后通过下界证明所获速率的紧致性。

Result: 1) SGD在有界噪声下尾部衰减上界为e^{-t/log(t)}；2) clipped-SGD在p阶矩有界重尾噪声下衰减上界为e^{-t^{β_p}/log(t)}，其中β_p=4(p-1)/(3p-2)，p=2时为e^{-t/log^2(t)}；3) 下界为e^{-t}，证明所获速率在多项式对数因子内是紧致的；4) 相比现有有限时间分析的e^{-√t}和e^{-t^{β_p/2}}速率，本工作揭示了数量级更快的长期衰减。

Conclusion: 通过大偏差理论分析，本工作首次提供了SGD类方法长期尾部衰减的精确刻画，揭示了比现有有限时间分析更快的衰减速率，为个体算法运行提供了更强的长期保证。所获速率在多项式对数因子内是紧致的，填补了该领域的研究空白。
...
### [251] [Accelerating Benchmarking of Functional Connectivity Modeling via Structure-aware Core-set Selection](https://arxiv.org/abs/2602.05667)
*Ling Zhan,Zhen Li,Junjie Huang,Tao Jia*

Main category: cs.LG

TL;DR: 提出SCLCS框架，通过结构感知对比学习选择代表性核心集，仅用10%数据即可保持功能连接模型性能排名，解决大规模fMRI数据集基准测试的计算瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 大规模fMRI数据集中功能连接（FC）建模方法的基准测试对可重复神经科学至关重要，但模型-数据配对的组合爆炸使穷举评估计算上不可行，阻碍其成为常规预分析步骤。

Method: 提出结构感知对比学习核心集选择（SCLCS）框架：1）使用自适应Transformer学习每个样本的独特FC结构；2）引入结构扰动评分（SPS）量化训练期间学习结构的稳定性，识别代表基础连接原型的样本；3）采用密度平衡采样策略确保核心集的结构鲁棒性和分布代表性。
...
### [252] [Stable but Wrong: When More Data Degrades Scientific Conclusions](https://arxiv.org/abs/2602.05668)
...
<details>
  <summary>Details</summary>
Motivation: 现代科学依赖大规模数据集和自动化推理流程，隐含假设更多数据能提高结论可靠性。本文旨在挑战这一基本信念，揭示数据驱动科学存在不可修复的结构性缺陷。

Method: 通过识别特定的结构性机制，使用最小化合成实验进行验证。在该机制下，观测可靠性以推理过程本身无法察觉的方式退化，导致标准推理程序产生系统性偏差。

Result: 研究发现存在一种结构机制，使得标准推理程序平稳收敛、校准良好且通过常规诊断检验，但仍系统性地收敛到错误结论。额外数据不仅无法纠正错误，反而会放大错误，而残差和拟合优度诊断仍保持误导性的正常状态。

Conclusion: 数据驱动科学存在内在局限：稳定性、收敛性和置信度不足以保证认知有效性。推理不能仅视为数据可用性的无条件结果，而必须受到观测过程完整性的明确约束。
...
### [253] [Perception-Based Beliefs for POMDPs with Visual Observations](https://arxiv.org/abs/2602.05679)
...
Conclusion: PBP框架成功地将感知模型与传统POMDP求解器结合，有效解决了高维视觉观测的处理问题，并通过不确定性量化提高了系统的鲁棒性。
...
### [254] [Mining Generalizable Activation Functions](https://arxiv.org/abs/2602.05688)
*Alex Vitvitskyi,Michael Boratko,Matej Grcic,Razvan Pascanu,Deep Shah,Petar Veličković*

Main category: cs.LG

TL;DR: 利用AlphaEvolve进化搜索框架发现新型激活函数，该框架使用前沿LLM作为变异算子，能在Python函数空间内搜索，并能针对特定归纳偏置进行优化。


<details>
  <summary>Details</summary>
Motivation: 激活函数选择是研究热点，影响优化性能和模型归纳偏置。传统方法依赖人工设计的搜索空间，而现代进化框架能更灵活地搜索并针对特定归纳偏置进行优化。

Method: 采用AlphaEvolve进化搜索框架，使用前沿LLM作为变异算子，在Python函数空间内搜索满足特定FLOP预算的激活函数。使用分布外数据性能作为适应度函数，以发现具有特定归纳偏置的激活函数。

Result: 实证研究表明，相对小规模的合成数据集足以让AlphaEvolve发现有意义的激活函数，验证了该框架在发现具有特定归纳偏置的激活函数方面的有效性。

Conclusion: 进化搜索框架为发现新型激活函数提供了有效途径，特别是结合前沿LLM作为变异算子，能够灵活搜索函数空间并针对特定归纳偏置进行优化，为激活函数设计开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 在主动学习场景下，如何通过二元反馈高效地将M个项目聚类到未知数量K个组中，同时处理噪声反馈并最小化查询成本

Method: 使用测度变换技术建立期望查询次数下界，设计基于广义似然比统计量的渐近最优算法，并开发计算可行的GLR变体

Result: 建立了聚类准确度置信水平下的查询次数下界，算法性能与下界差距可通过经验估计，且保持在常数倍范围内

Conclusion: 该框架为主动聚类提供了理论基础和实用算法，在噪声环境下实现了查询效率与聚类准确度的最优平衡
...
TL;DR: FedRandom是一种解决联邦学习中参与者贡献评估不稳定性的新方法，通过生成更多样本来提供更一致可靠的贡献评估，在多种数据集上显著提升了评估稳定性。


<details>
  <summary>Details</summary>
Motivation: 在工业部署的联邦学习中，参与者需要成本投入并期望获得补偿，公平评估个体贡献对于识别恶意参与者和搭便车者至关重要。然而，现有贡献评估方法存在显著的不稳定性，这可能影响参与者参与联邦的意愿。

Method: FedRandom将贡献不稳定性视为统计估计问题，通过生成比常规联邦学习策略更多的样本来提供更一致可靠的参与者贡献评估。该方法在不同数据分布下进行了验证。

Result: 在CIFAR-10、MNIST、CIFAR-100和FMNIST数据集上的实验表明，FedRandom在超过一半的评估场景中将总体距离真实值的误差减少了三分之一以上，并在超过90%的情况下提高了稳定性。

Conclusion: FedRandom有效缓解了联邦学习中参与者贡献评估的不稳定性问题，为公平评估参与者贡献提供了更可靠的方法，有助于促进参与者参与联邦学习的积极性。
...
### [257] [Fix Representation (Optimally) Before Fairness: Finite-Sample Shrinkage Population Correction and the True Price of Fairness Under Subpopulation Shift](https://arxiv.org/abs/2602.05707)
*Amir Asiaee,Kaveh Aryan*

Main category: cs.LG

TL;DR: 论文分析了预测准确性与群体公平性之间的权衡关系，指出现有观察到的"公平性有助于准确性"现象可能是训练数据中子群比例失真的结果，并提出了包含最优收缩加权的评估协议。


<details>
  <summary>Details</summary>
Motivation: 机器学习实践中经常观察到预测准确性与群体公平性约束之间的张力，但有时公平性干预似乎能提高准确性。作者认为这两种现象都可能是训练数据中子群比例失真的产物，需要建立理论框架来理解这一现象并提出可靠的评估方法。

Method: 在子群分布偏移（组内分布稳定，组间比例变化）的设定下，作者：1) 建立了理论分析框架；2) 证明了完全重要性加权校正的渐近无偏性但有限样本次优性；3) 推导出最优有限样本校正为收缩加权法，在目标分布和训练分布之间插值；4) 提出了包含最优收缩加权的评估协议：先（最优地）固定表征，再比较公平性干预。

Result: 理论分析表明：1) 完全重要性加权校正渐近无偏但有限样本下表现不佳；2) 最优有限样本校正为收缩加权法；3) 观察到的"公平性有助于准确性"现象可能是由于与不当加权的基线比较所致。在合成数据和真实基准（Adult、COMPAS）上的实验验证了理论预测，并证明该协议能消除虚假权衡，揭示真实的公平性-效用前沿。

Conclusion: 论文揭示了预测准确性与群体公平性之间观察到的张力可能是数据失真的产物，提出了包含最优收缩加权的评估协议，强调在评估公平性干预前应先（最优地）固定表征，以分离出公平性的真实、不可约成本，从而获得可靠的公平性-效用权衡分析。
...
### [258] [CSRv2: Unlocking Ultra-Sparse Embeddings](https://arxiv.org/abs/2602.05735)
*Lixuan Guo,Yifei Wang,Tiansheng Wen,Yifan Wang,Aosong Feng,Bo Chen,Stefanie Jegelka,Chenyu You*

Main category: cs.LG

TL;DR: CSRv2是一种改进的超稀疏嵌入训练方法，通过渐进式k退火、监督对比目标和全骨干微调，将死神经元从80%降至20%，在k=2时实现14%精度提升，使超稀疏嵌入达到与CSR k=8和MRL 32维相当的性能，同时保持7倍于MRL的加速和300倍于密集嵌入的计算内存效率。


<details>
  <summary>Details</summary>
Motivation: 在大型基础模型时代，嵌入质量对下游任务性能至关重要。传统密集嵌入维度极高，导致存储、内存和推理延迟成本巨大。对比稀疏表示(CSR)虽将密集嵌入映射为k稀疏向量，但在超稀疏区域(k很小)存在严重性能退化，超过80%神经元失活，效率潜力未充分发挥。

Method: CSRv2采用三方面改进：1) 渐进式k退火稳定稀疏学习；2) 监督对比目标增强表示质量；3) 全骨干微调确保端到端适应性。该方法通过系统训练策略解决超稀疏嵌入中的死神经元问题。

Result: CSRv2将死神经元从80%降至20%，在k=2时获得14%精度提升，使超稀疏嵌入性能与CSR k=8和MRL 32维相当。在保持可比性能的同时，相比MRL实现7倍加速，相比密集嵌入在文本表示中获得300倍计算和内存效率提升。文本/视觉表示中，k=4时比CSR提升7%/4%，k=2时提升14%/6%。

Conclusion: CSRv2使极端稀疏嵌入变得实用而不牺牲性能，为实时和边缘部署的AI系统拓宽了设计空间，在嵌入质量和效率都至关重要的场景中具有重要应用价值。
...
### [259] [Cross-Domain Offline Policy Adaptation via Selective Transition Correction](https://arxiv.org/abs/2602.05776)
*Mengbei Yan,Jiafei Lyu,Shengjie Sun,Zhongjian Qiao,Jingwen Yang,Zichuan Lin,Deheng Ye,Xiu Li*

Main category: cs.LG

TL;DR: 提出STC算法解决跨域离线RL中的动态不匹配问题，通过修正源域数据使其与目标域动态对齐，实现更优的策略适应


<details>
  <summary>Details</summary>
Motivation: 在跨域离线强化学习中，直接合并源域和目标域数据集会因动态不匹配导致性能下降。现有方法通过过滤源域转移或修改奖励来缓解问题，但未能充分利用源域数据的价值。
...
Conclusion: STC算法通过修正源域数据使其与目标域动态对齐，实现了跨域离线强化学习中源域数据的可靠利用，有效解决了动态不匹配问题。
...
### [260] [Distributional Reinforcement Learning with Diffusion Bridge Critics](https://arxiv.org/abs/2602.05783)
*Shutong Ding,Yimiao Zhou,Ke Hu,Mokai Pan,Shan Zhong,Yanwei Fu,Jingya Wang,Ye Shi*

Main category: cs.LG

TL;DR: 提出了一种基于扩散桥批评器（DBC）的新型分布强化学习方法，直接建模Q值的逆累积分布函数，准确捕捉价值分布并防止其坍缩为平凡高斯分布。


<details>
  <summary>Details</summary>
Motivation: 现有扩散强化学习方法主要关注扩散策略，而忽略了扩散批评器。由于策略优化从根本上依赖于批评器，准确的价值估计比策略表达能力更为重要。此外，考虑到大多数强化学习任务的随机性，批评器更适合用分布模型来描述。

Method: 提出扩散桥批评器（DBC），直接建模Q值的逆累积分布函数（CDF）。利用扩散桥的强大分布匹配能力准确捕捉价值分布并防止坍缩为平凡高斯分布。进一步推导解析积分公式来解决DBC中的离散化误差问题。

Result: 在MuJoCo机器人控制基准测试中的实验结果表明，DBC相比之前的分布批评器模型具有优越性。

Conclusion: DBC是首个将扩散桥模型用作批评器的工作，并且是一个即插即用的组件，可以集成到大多数现有的RL框架中。该方法在价值分布建模方面表现出色，解决了传统方法的局限性。
...
### [261] [Classification Under Local Differential Privacy with Model Reversal and Model Averaging](https://arxiv.org/abs/2602.05797)
*Caihong Qin,Yang Bai*

Main category: cs.LG

TL;DR: 该论文将LDP下的隐私学习重新解释为迁移学习问题，提出三种新技术：基于噪声二进制反馈的评估机制、模型反转和模型平均，以在不损害隐私的前提下提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 局部差分隐私（LDP）虽然提供了强大的隐私保证，但引入的噪声会显著降低数据效用。现有方法在LDP下的分类性能往往不理想，需要新的技术来平衡隐私保护和数据实用性。

Method: 将LDP下的隐私学习重新解释为迁移学习问题（噪声数据为源域，干净数据为目标域），提出三种专门针对LDP的技术：1）基于噪声二进制反馈的数据集效用评估机制；2）模型反转技术，通过反转决策边界来挽救性能不佳的分类器；3）模型平均技术，根据估计的效用为多个反转分类器分配权重。

Result: 提供了LDP下的理论超额风险界限，并展示了所提方法如何降低这一风险。在模拟和真实数据集上的实证结果表明，分类准确率有显著提升。

Conclusion: 通过将LDP隐私学习重新框架为迁移学习问题，并开发专门的技术（效用评估、模型反转、模型平均），可以在不损害隐私的前提下有效提升LDP下的分类性能，为平衡隐私保护和数据实用性提供了新思路。
...
### [262] [Where Does Warm-Up Come From? Adaptive Scheduling for Norm-Constrained Optimizers](https://arxiv.org/abs/2602.05813)
*Artem Riabinin,Andrey Veprikov,Arman Bolatov,Martin Takáč,Aleksandr Beznosikov*

Main category: cs.LG

TL;DR: 论文提出了一种用于范数约束优化器（如Muon和Lion）的自适应学习率调度方法，通过广义光滑性假设推导出自然包含预热和衰减的学习率方案，并开发了实用的自适应预热调度器。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习优化中，学习率调度（特别是预热阶段）通常依赖启发式设置和大量超参数调优。本文旨在为范数约束优化器建立理论框架，从优化理论出发推导出自然包含预热和衰减的学习率方案，从而减少对人工调优的依赖。

Method: 1. 引入广义光滑性假设：局部曲率随次优性间隙减小而减小；2. 在该假设下建立收敛保证，推导出包含预热和衰减的学习率选择；3. 基于理论开发实用调度器，仅使用标准超参数，在训练开始时自动适应预热时长。

Result: 在LLaMA架构的大语言模型预训练中评估，自适应预热选择在所有实验设置中始终优于或至少匹配最佳手动调优的预热调度，无需额外超参数搜索。

Conclusion: 本文为范数约束优化器提供了理论驱动的自适应学习率调度框架，通过广义光滑性假设自然推导出预热-衰减模式，开发了实用的自适应调度器，在大规模语言模型训练中验证了其有效性，减少了人工调优需求。
...
### [263] [Synthesizing Realistic Test Data without Breaking Privacy](https://arxiv.org/abs/2602.05833)
*Laura Plein,Alexi Turcotte,Arina Hallemans,Andreas Zeller*

Main category: cs.LG

TL;DR: 提出一种基于模糊测试和判别器的隐私保护合成数据生成方法，通过间接利用原始数据生成具有相同统计特性但隐私保护的测试数据集


<details>
  <summary>Details</summary>
Motivation: 现有基于GAN的合成数据生成方法存在两个主要问题：1）生成模型准确性不足；2）仍易受成员推理攻击和数据集重建攻击，因为训练过程直接使用了原始数据。需要一种既能保持原始数据统计特性又能保护隐私的合成数据生成方法。

Method: 受GAN启发但采用不同架构：1）使用测试生成器（模糊器）根据输入规范生成测试数据，保留原始数据设定的约束；2）判别器模型评估生成数据与原始数据的接近程度；3）通过演化样本和判别器选择"优质样本"，生成遵循原始数据统计分布的隐私保护数据。

Result: 在四个用于评估最先进技术的基准数据集上进行了实验验证。实验结果表明该方法在生成高实用性合成数据集的同时有效保护隐私，具有显著潜力。

Conclusion: 该方法能够生成既保持原始数据统计特性又具有隐私保护的合成数据集，在实用性和隐私保护之间取得了良好平衡，为解决合成数据生成中的隐私安全问题提供了新思路。
...
### [264] [DLM-Scope: Mechanistic Interpretability of Diffusion Language Models via Sparse Autoencoders](https://arxiv.org/abs/2602.05859)
*Xu Wang,Bingqing Jiang,Yu Wan,Baosong Yang,Lingpeng Kong,Difan Zou*

Main category: cs.LG

TL;DR: DLM-Scope：首个针对扩散语言模型的稀疏自编码器可解释性框架，发现SAE在DLMs中的插入效果与LLMs不同，能降低早期层交叉熵损失，并支持更有效的扩散时间干预。


<details>
  <summary>Details</summary>
Motivation: 随着扩散语言模型成为自回归大语言模型的有前景替代方案，需要为这类新兴模型开发专门的可解释性工具，以支持对其机制的理解和干预。

Method: 提出DLM-Scope框架，使用Top-K稀疏自编码器从扩散语言模型中提取可解释特征，研究SAE插入对模型性能的影响，探索SAE特征在解码顺序确定和干预中的应用。

Result: SAE在DLMs中的插入效果与LLMs不同：在早期层能降低交叉熵损失；SAE特征支持更有效的扩散时间干预，性能常优于LLM引导；SAE能为DLM解码顺序提供有用信号；SAE特征在DLMs后训练阶段保持稳定。
...
### [265] [Constrained Group Relative Policy Optimization](https://arxiv.org/abs/2602.05863)
...
<details>
  <summary>Details</summary>
Motivation: 虽然GRPO已成为无评论家策略学习的可扩展框架，但在具有明确行为约束的设置中扩展仍不足。需要将约束优化整合到GRPO框架中，特别是在依赖大型多模态基础模型的具身AI领域

Method: 提出Constrained GRPO，基于拉格朗日松弛的GRPO扩展。约束通过指示器成本函数指定，通过拉格朗日松弛直接优化违反率。关键创新是标量化优势构造，避免多组件优势估计中组件间标准差不匹配导致的相对重要性扭曲问题

Result: 在玩具网格世界中验证了预测的优化病理问题，证明标量化优势恢复了稳定的约束控制。在机器人任务中评估，Constrained GRPO提高了约束满足度同时增加了任务成功率

Conclusion: 为具身AI领域的约束策略优化提供了一个简单有效的方案，特别是在日益依赖大型多模态基础模型的场景中，通过标量化优势构造解决了拉格朗日信号污染问题
...
### [266] [Dr. Kernel: Reinforcement Learning Done Right for Triton Kernel Generations](https://arxiv.org/abs/2602.05885)
*Wei Liu,Jiawei Xu,Yingru Li,Longtao Zheng,Tianjian Li,Qian Liu,Junxian He*

Main category: cs.LG

TL;DR: 该论文提出了一个用于内核生成的强化学习框架，通过设计KernelGYM环境、解决GRPO中的偏置梯度问题、引入TRLOO方法以及应对懒惰优化问题，训练出Dr.Kernel-14B模型，在KernelBench上达到与Claude-4.5-Sonnet竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 高质量内核对可扩展AI系统至关重要，但训练LLMs生成此类代码面临数据不足、环境脆弱、奖励黑客攻击和懒惰优化等问题，需要系统研究强化学习在内核生成中的应用。

Method: 1) 设计KernelGYM分布式GPU环境支持奖励黑客检查、多轮交互数据收集和长期RL训练；2) 提出TRLOO解决GRPO中的自包含偏置梯度问题；3) 引入失配校正提高训练稳定性；4) 提出基于性能剖析的奖励(PR)和拒绝采样(PRS)应对懒惰优化。

Result: Dr.Kernel-14B在KernelBench上达到与Claude-4.5-Sonnet竞争的性能。在KernelBench Level-2子集上，31.6%生成的内核比Torch参考实现至少快1.2倍，超过Claude-4.5-Sonnet(26.7%)和GPT-5(28.6%)。选择所有轮次中的最佳候选时，1.2倍加速率进一步提升至47.8%。

Conclusion: 该研究系统性地解决了内核生成中的强化学习挑战，提出的KernelGYM环境、TRLOO方法和应对懒惰优化的技术有效提升了模型性能，为AI系统的高性能内核生成提供了实用解决方案。
...
TL;DR: 提出一种模拟过参数化逃逸方向的方法，在不实际进行张量提升的情况下，通过模拟过参数化空间的景观和逃逸方向，从低秩矩阵感知的虚假局部最小值中确定性逃逸。


<details>
  <summary>Details</summary>
Motivation: 低秩矩阵感知作为非凸优化问题存在大量虚假局部最小值，梯度优化器难以收敛到全局最优。现有研究表明过参数化（张量提升）可将局部最小值转换为严格鞍点，但实际进行张量提升计算不可行。因此需要在不实际进行张量提升的情况下，模拟过参数化空间的逃逸机制。

Method: 提出模拟预言方向（SOD）逃逸机制：设计数学框架将过参数化逃逸方向投影到原始参数空间，保证从现有局部最小值严格降低目标函数值。该方法模拟过参数化空间的景观和逃逸方向，避免实际进行张量提升的计算负担。

Result: 数值实验表明该框架能可靠地从局部最小值逃逸，促进收敛到全局最优解，与显式张量过参数化相比计算成本最小。这是首个能保证从虚假局部最小值逃逸的确定性框架，无需随机扰动或启发式估计。

Conclusion: 该框架展示了如何通过模拟过参数化来驯服具有挑战性的优化景观，对矩阵感知之外的非凸优化具有非平凡意义。证明了在不实际进行过参数化的情况下，利用过参数化洞察改善优化性能的可能性。
...
### [268] [DFPO: Scaling Value Modeling via Distributional Flow towards Robust and Generalizable LLM Post-Training](https://arxiv.org/abs/2602.05890)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界RL训练面临噪声监督和域外泛化差的挑战，现有分布RL方法将每个分位数作为独立标量学习，导致价值表示粗糙且缺乏细粒度状态信息，在复杂和OOD条件下表现不佳

Method: 提出DFPO框架：1) 将价值建模为跨时间步的连续流，学习价值流场而非孤立分位数预测；2) 集成条件风险控制和价值流轨迹上的一致性约束以稳定噪声反馈下的训练

Result: 在对话、数学推理和科学任务上的实验表明，DFPO在噪声监督下优于PPO、FlowRL和其他鲁棒基线，实现了更好的训练稳定性和泛化能力

Conclusion: 通过连续价值流建模结合条件风险控制和一致性约束，DFPO为噪声监督下的RL训练提供了更鲁棒的解决方案，特别适用于LLM后训练等复杂场景
...
### [269] [Parity, Sensitivity, and Transformers](https://arxiv.org/abs/2602.05896)
*Alexander Kozachinskiy,Tomasz Steifer,Przemysław Wałȩga*

Main category: cs.LG

TL;DR: 论文提出了一种新的Transformer架构用于解决PARITY问题，该架构使用softmax、长度无关且多项式有界的位置编码，无需layernorm，同时支持因果掩码和非因果掩码，并首次证明了单层单头Transformer无法解决PARITY问题。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer架构已有近十年历史，但对其计算能力的理解仍然有限。特别是，现有PARITY问题的Transformer构造需要至少2层，且依赖不实用的特性：长度相关的位置编码、hardmax、无正则化参数的layernorm，或不支持因果掩码。本文旨在构建更实用的Transformer来解决PARITY问题，并探索其计算能力的理论边界。

Method: 提出了一种新的Transformer构造方法，使用softmax激活函数、长度无关且多项式有界的位置编码，不依赖layernorm，同时支持因果和非因果掩码。通过精心设计的注意力机制和位置编码方案，使Transformer能够计算PARITY函数。此外，通过理论分析证明了单层单头Transformer无法解决PARITY问题。

Result: 成功构建了一个能够解决PARITY问题的Transformer架构，该架构具有实用特性：使用softmax、长度无关的位置编码、无layernorm，同时支持两种掩码模式。更重要的是，首次证明了单层单头Transformer无法计算PARITY函数，为Transformer的计算能力提供了理论下界。

Conclusion: 本文在Transformer计算能力理论方面取得重要进展：一方面提出了更实用的PARITY问题解决方案，另一方面首次建立了单层Transformer无法解决PARITY问题的下界。这些结果为理解Transformer架构的计算能力提供了新的理论洞见，并展示了Transformer在形式语言处理方面的潜力与限制。
...
### [270] [Regularized Calibration with Successive Rounding for Post-Training Quantization](https://arxiv.org/abs/2602.05902)
...
Method: 提出对称与非对称校准的插值作为正则化，保持PTQ标准二次结构的同时增强对激活不匹配的鲁棒性；推导简单连续舍入过程自然融入非对称校准；扩展有界搜索方法，在量化质量和计算成本间提供显式权衡

Result: 在多个LLM家族、量化比特宽度和基准测试中，基于正则化非对称校准目标的有界搜索方法持续改善困惑度和准确率，相比PTQ基线显著提升，仅带来适度可控的额外计算成本

Conclusion: 通过对称与非对称校准的插值作为正则化，结合有界搜索扩展，提出了一种高效的后训练量化方法，在保持标准PTQ结构的同时提升量化鲁棒性和质量，为LLM高效部署提供实用解决方案
...
### [271] [Verification of the Implicit World Model in a Generative Model via Adversarial Sequences](https://arxiv.org/abs/2602.05903)
*András Balogh,Márk Jelasity*

Main category: cs.LG

TL;DR: 该研究提出使用对抗序列生成方法来验证序列模型在象棋领域的正确性，发现所有模型都存在不完整性问题，但某些训练技术和数据集能显著改善模型正确性。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决生成序列模型训练中的核心问题：基于样本的训练能否真正捕捉目标语言的结构（即"世界模型"）。理论上模型最多只能保证生成有效序列，但不能保证生成所有有效序列。需要实用工具来验证序列模型的正确性，象棋领域因其复杂性与简单规则基础成为理想测试环境。

Method: 提出对抗序列生成方法验证序列模型正确性：1）设计对抗性生成器产生有效序列，迫使序列模型预测无效的下一步移动；2）开发多种对抗序列生成方法；3）在大量象棋模型上评估该方法，包括基于随机和高质量棋局训练的不同模型；4）研究棋盘状态探针在训练和攻击方法中的潜在应用。

Result: 研究发现：1）所有模型都不完全正确，但某些训练技术和数据集选择能显著改善正确性；2）在大多数模型中，提取的棋盘状态对下一个标记预测没有因果作用；3）对抗序列生成方法不仅能证伪正确性，还能进行更细粒度的失败模式分析和训练选择效果评估。

Conclusion: 对抗序列生成是验证序列模型正确性的有效方法，特别适用于象棋等具有明确规则系统的领域。虽然当前模型无法达到完全正确，但通过改进训练技术和数据集选择可以显著提升模型质量。棋盘状态信息在现有模型中未得到有效利用，这为未来模型改进提供了方向。
...
### [272] [Chunky Post-Training: Data Driven Failures of Generalization](https://arxiv.org/abs/2602.05910)
*Seoirse Murray,Allison Qi,Timothy Qian,John Schulman,Collin Burns,Sara Price*

Main category: cs.LG

TL;DR: 论文提出"chunky post-training"概念，指LLM后训练中数据块导致的虚假相关性学习，开发了SURF运行时检测和TURF溯源工具，在多个前沿模型上验证了该问题。


<details>
  <summary>Details</summary>
Motivation: LLM后训练使用多样化数据集，但这些数据集除了目标行为外还编码了意外模式（格式与内容相关性、狭窄措辞、数据策展中的隐式关联）。这些模式对开发者不可见但对模型显著，导致意外行为（如拒绝特定格式的真实事实）。需要系统方法来检测和溯源这些"chunky post-training"问题。

Method: 提出SURF（黑盒管道）在运行时检测意外行为，以及TURF工具将失败溯源到特定后训练数据。在Claude 4.5、GPT-5.1、Grok 4.1、Gemini 3等前沿模型和Tülu 3等开源模型上应用这些工具。

Result: chunky post-training导致校准错误的行为，这些行为通常源于不平衡或未充分指定的后训练数据块。工具成功检测到意外行为并溯源到具体数据源。

Conclusion: 后训练数据中的块状结构会产生虚假相关性，影响模型行为。需要更好的数据策展和评估方法来缓解这些问题，SURF和TURF为此提供了实用工具。
...
### [273] [Approximation of Log-Partition Function in Policy Mirror Descent Induces Implicit Regularization for LLM Post-Training](https://arxiv.org/abs/2602.05933)
*Zhenghao Xu,Qin Lu,Changlong Yu,Tuo Zhao*

Main category: cs.LG

TL;DR: PMD-mean是一种实用的强化学习算法，通过用采样策略的平均奖励近似对数配分函数，在LLM的大动作空间中解决KL正则化策略改进子问题，提供更保守的更新和更好的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 策略镜像下降（PMD）为强化学习提供了理论框架，但在LLM的大动作空间中，闭式PMD更新需要可靠的对数配分函数估计，这在有限样本情况下具有挑战性。需要开发更实用的算法来处理这一限制。

Method: 提出PMD-mean算法，用采样策略的平均奖励近似对数配分项，在对数策略空间中进行回归。该方法隐式优化了具有自适应混合KL-χ²正则器的镜像下降子问题，χ²正则化约束了大的概率变化。

Result: 在数学推理任务上的实验表明，PMD-mean实现了优越的性能，具有改进的稳定性和时间效率。该算法在预期奖励较低时产生更保守的更新，增强了对有限样本估计误差的鲁棒性。

Conclusion: PMD-mean通过自适应混合KL-χ²正则化提供了更稳健的策略更新，加深了对PMD-mean的理解，并为LLM的RL算法提供了原则性改进的途径。
...
### [274] [Dimensionality Reduction on Riemannian Manifolds in Data Analysis](https://arxiv.org/abs/2602.05936)
*Alaa El Ichi,Khalide Jbilou*

Main category: cs.LG

TL;DR: 该论文研究基于黎曼几何的降维方法，这些方法尊重数据的底层流形结构，特别关注主测地线分析作为流形值数据的非线性PCA扩展，并通过黎曼适应扩展判别分析。


<details>
  <summary>Details</summary>
Motivation: 传统欧几里得降维方法在处理约束在弯曲空间（如超球面和对称正定流形）的数据时存在局限性，需要尊重数据底层流形结构的几何感知降维方法。

Method: 研究基于黎曼几何的降维方法，包括主测地线分析作为流形值数据的非线性PCA扩展，以及通过黎曼适应扩展判别分析。这些方法利用测地线距离、切空间表示和内在统计度量来实现更忠实的低维嵌入。
...
### [275] [Orthogonal Model Merging](https://arxiv.org/abs/2602.05943)
*Sihan Yang,Kexuan Shi,Weiyang Liu*

Main category: cs.LG

TL;DR: OrthoMerge是一种在正交群黎曼流形上合并微调LLM的新方法，通过保持权重几何结构来避免传统线性合并破坏预训练权重固有几何特性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前主流的模型合并方法依赖欧几里得空间中的线性算术，这会破坏预训练权重的内在几何特性（如超球面能量）。需要一种能保持模型权重几何结构的合并方法。

Method: 提出OrthoMerge方法：1）将正交微调（OFT）学习的任务特定正交矩阵映射到李代数进行合并；2）通过正交-残差解耦策略处理非OFT微调的模型，提取正交分量在正交群流形上合并，线性残差通过标准加法合并。

Result: 大量实证结果表明，OrthoMerge能有效缓解灾难性遗忘，并在多样化任务中保持模型性能。

Conclusion: OrthoMerge通过在正交群黎曼流形上进行合并操作，提供了一种保持模型权重几何结构的原则性方法，优于传统线性合并方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决在克利福德代数空间中进行函数逼近时面临的高维指数缩放问题，以及处理可变域输入的挑战，为科学发现和工程应用提供有效的数学工具。

Method: 提出Clifford Kolmogorov-Arnold Network (ClKAN)架构，采用随机准蒙特卡洛网格生成技术解决高维代数指数缩放，引入新的批归一化策略处理可变域输入。
...
<details>
  <summary>Details</summary>
Motivation: 当前流和扩散模型虽然能生成高质量样本，但训练后适应特定用户偏好或约束的成本高且脆弱（奖励对齐问题）。作者认为高效奖励对齐应该是生成模型的内在属性而非事后补救，因此需要重新设计模型以实现更好的适应性。

Method: 提出"Diamond Maps"（钻石映射），这是一种随机流映射模型，能够将多个模拟步骤摊销到单步采样器中（类似流映射），同时保留奖励对齐所需的随机性。该设计通过实现价值函数的高效一致估计，使搜索、顺序蒙特卡洛和引导方法具有可扩展性。

Result: 实验表明：1）Diamond Maps可以通过从GLASS Flows蒸馏有效学习；2）相比现有方法，Diamond Maps在奖励对齐性能上更强；3）具有更好的可扩展性。

Conclusion: Diamond Maps为构建能够在推理时快速适应任意偏好和约束的生成模型提供了实用路径，使高效奖励对齐成为生成模型的内在属性。
...
TL;DR: AP-OOD是一种新颖的文本OOD检测方法，通过利用token级信息超越简单平均聚合，在半监督设置下实现最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法在有效利用和聚合语言模型的token嵌入以获得OOD分数方面存在挑战，需要超越简单的平均聚合方法。

Method: AP-OOD是一种半监督方法，灵活地在无监督和有监督设置之间插值，利用token级信息并允许使用有限的辅助异常数据。

Result: 在无监督设置下，AP-OOD在XSUM摘要任务上将FPR95从27.84%降至4.67%，在WMT15 En-Fr翻译任务上从77.08%降至70.37%，实现了新的最先进性能。

Conclusion: AP-OOD通过有效利用token级信息和灵活的监督设置，显著提升了文本OOD检测性能，为机器学习模型的可靠部署提供了重要工具。
...
### [279] [Shared LoRA Subspaces for almost Strict Continual Learning](https://arxiv.org/abs/2602.06043)
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型的高效持续适应在实际部署中至关重要，但面临灾难性遗忘和高重训练成本的挑战。现有参数高效调优方法（如LoRA）缺乏严格的持续学习和知识集成机制，且依赖数据回放或多适配器。

Method: Share构建一个基础子空间来提取过去任务的核心知识，并通过识别关键子空间方向增量集成新信息。每个新任务的知识被整合到这个不断演化的共享子空间中，促进前向知识转移，同时最小化灾难性干扰。
...
### [280] [Can vision language models learn intuitive physics from interaction?](https://arxiv.org/abs/2602.06033)
*Luca M. Schulze Buschoff,Konstantinos Voudouris,Can Demircan,Eric Schulz*

Main category: cs.LG

TL;DR: 论文研究发现，通过交互式强化学习训练的视觉语言模型在特定任务上表现提升，但未能获得可泛化的物理直觉，无法迁移到相关任务中。
...
Method: 使用强化学习训练模型，让模型通过与环境的交互来学习。比较了交互式学习与监督微调方法，测试模型在相关任务间的泛化能力。

Result: 交互式学习确实能让模型在特定任务内提升性能，但未能产生具有泛化物理直觉的模型。研究发现，在一个任务上训练的模型无法可靠地泛化到相关任务，即使这些任务共享视觉统计特征和物理原理，且无论模型是否通过交互训练。

Conclusion: 仅通过交互式学习不足以让视觉语言模型获得可泛化的物理直觉。当前方法在特定任务上有效，但无法实现跨任务的鲁棒物理规则学习，表明需要更先进的训练范式来培养模型的物理推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法存在训练与解码的不匹配：训练优化单一贪婪轨迹，而解码需要验证和排序多个采样草稿路径

Method: 提出变分推测解码(VSD)，将草稿训练建模为对潜在提案（草稿路径）的变分推断，最大化目标模型接受概率，通过EM算法优化，包含MCMC采样和自适应拒绝加权等技术

Result: 理论分析证实VSD增加预期接受长度和加速比，实验显示在LLMs和MLLMs上相比EAGLE-3和ViSpec分别获得9.6%和7.9%的加速提升

Conclusion: VSD通过变分推断框架有效解决训练-解码不匹配问题，显著提升推测解码效率，为大规模语言模型推理加速提供了新方法
