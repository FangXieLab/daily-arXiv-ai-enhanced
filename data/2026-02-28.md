...
TL;DR: GYWI系统通过结合作者知识图谱与检索增强生成，为LLMs提供可控的学术背景和可追溯的灵感路径，以生成新颖的科学想法。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在科学想法生成方面存在局限性：生成的成果缺乏可控的学术背景和可追溯的灵感路径，这限制了其在科学研究中的实际应用价值。

Method: 1) 提出作者中心的知识图谱构建方法和灵感源采样算法构建外部知识库；2) 设计结合RAG和GraphRAG的混合检索机制获取深度和广度知识；3) 采用基于强化学习原理的Prompt优化策略自动指导LLMs优化结果。

Result: 在基于arXiv（2018-2023）构建的数据集上，GYWI在GPT-4o、DeepSeek-V3、Qwen3-8B和Gemini 2.5等多个LLMs上显著优于主流方法，在创新性、可靠性和相关性等多个指标上表现优异。
...
Method: 1) 理论评估：从广泛认可的金融资格考试中收集多样化考题；2) 实践评估：提出系统性评估矩阵，分类复杂金融领域并确保覆盖关键子领域和业务活动；3) 收集3000个金融场景问题，包括有参考答案的封闭式决策问题和按预定标准评估的开放式问题。

Result: 对包括轩辕4.0在内的最先进LLMs在FIRE基准上进行了全面评估，轩辕4.0作为金融领域模型提供了强大的领域内基线，结果能够系统分析当前LLMs在金融应用中的能力边界。

Conclusion: FIRE基准为评估LLMs在金融领域的能力提供了全面框架，通过理论知识和实践场景的双重评估，有助于推动金融AI研究的未来发展，基准问题和评估代码已公开。
...
TL;DR: 该论文提出了因果嵌入框架，将多个详细模型映射到粗粒度因果模型的子系统中，作为抽象化的推广，并应用于多分辨率边际问题
...
Result: 展示了因果嵌入在统计边际问题和因果边际问题中的相关性，并证明其在合并不同表示模型的数据集方面的实际应用价值

Conclusion: 因果嵌入框架为处理多模型映射和数据集合并提供了理论基础，扩展了因果抽象的应用范围，为解决多分辨率因果推理问题提供了新工具
...
<details>
  <summary>Details</summary>
Motivation: 传统软件依赖API、类型系统和断言等契约来确保正确行为，而AI智能体基于提示和自然语言指令运行，缺乏形式化的行为规范，这导致智能体AI部署中出现漂移、治理失败和项目失败等问题。

Method: 引入ABC框架，定义契约C = (P, I, G, R)，包含前置条件、不变量、治理策略和恢复机制。提出(p, delta, k)-满足度的概率性契约遵从概念，考虑LLM非确定性和恢复机制。证明漂移界限定理，推导多智能体链的安全组合条件和概率退化界限。实现AgentAssert运行时强制执行库。

Result: 在AgentContract-Bench基准测试中，包含7个模型、6个供应商的200个场景，1980次会话结果显示：契约化智能体检测到5.2-6.8个软违规（基线完全未检测到，p<0.0001，Cohen's d=6.7-33.8），实现88-100%硬约束遵从，将行为漂移限制在D*<0.27，前沿模型恢复率100%，所有模型恢复率17-100%，每个动作开销<10ms。

Conclusion: ABC框架为AI智能体提供了形式化的行为契约规范，能够有效检测违规、限制行为漂移并实现恢复，为可靠、可治理的自主AI系统部署提供了理论基础和实用工具。
...
TL;DR: CogARC研究通过人类在抽象视觉推理任务中的行为数据，揭示了人类抽象推理的灵活性和策略多样性，为理解人类如何从稀疏示例中学习规则提供了丰富的行为环境。


<details>
  <summary>Details</summary>
Motivation: 研究人类在抽象推理中表现出的灵活性，探究人类如何从少量示例中快速学习和应用规则，以及理解支撑这种能力的认知策略。

Method: 使用CogARC（Cognitive Abstraction and Reasoning Corpus），这是从ARC（Abstraction and Reasoning Corpus）中选取的人类适应子集。在两个实验中，共260名参与者自由生成75个抽象视觉推理问题的解决方案。记录高时间分辨率的行为数据，包括示例查看、编辑序列和多尝试提交。

Result: 参与者总体成功率较高（实验1平均准确率约90%，实验2约80%），但不同问题和参与者间差异显著。较难问题引发更长的思考时间和更大的策略分歧。随着任务进行，参与者响应更快但准确率略有下降。即使错误解决方案也常高度收敛，问题解决轨迹在长度和平滑度上存在差异。

Conclusion: CogARC为研究人类抽象推理提供了丰富的行为环境，揭示了人类如何在不确定性下进行泛化、错误泛化和策略调整，展示了人类抽象推理的灵活性和策略多样性。
...
TL;DR: 该论文研究了异质智能体通过校准自身可靠性并选择性弃权来提升集体决策准确性的框架，将孔多塞陪审团定理扩展到包含置信度门控的序列设置。


<details>
  <summary>Details</summary>
Motivation: 传统孔多塞陪审团定理假设固定参与，但现实世界聚合通常允许智能体说"我不知道"。需要研究智能体学习自身可靠性并选择性弃权如何影响集体准确性，特别是在AI安全领域应对大语言模型幻觉问题。

Method: 提出概率框架：智能体经历校准阶段更新对自身固定能力的信念，然后面对最终置信度门控决定投票或弃权。推导群体成功概率的非渐近下界，证明选择性参与将CJT渐近保证推广到序列置信度门控设置。通过蒙特卡洛模拟验证边界。

Result: 证明了选择性参与框架能够推广孔多塞陪审团定理的渐近保证，为群体成功概率提供了非渐近下界。经验验证表明该框架在模拟中有效运作。

Conclusion: 选择性参与框架扩展了传统投票理论，允许智能体基于学习到的可靠性进行弃权，在保持渐近保证的同时提升集体准确性。该框架对AI安全有潜在应用价值，可缓解集体LLM决策中的幻觉问题。
...
Method: 对具有不同监督水平的潜在推理方法进行综合分析，识别了两个关键问题：1）观察普遍存在的捷径行为，即模型在不依赖潜在推理的情况下达到高准确率；2）检验潜在推理支持广度优先搜索式探索的假设，分析潜在表示如何编码多种可能性以及推理过程是否忠实实现结构化搜索。
...
Conclusion: 本研究揭示了潜在推理方法的内部机制，指出了当前方法在实现真正多步推理方面的局限性。研究发现强调了在监督强度与潜在表示多样性之间需要平衡，为未来改进潜在推理方法提供了重要见解，特别是需要设计更忠实实现结构化搜索的机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用监督微调（SFT）训练动作评分器，但SFT独立处理每个候选动作，没有明确教导模型区分物理上正确和微妙错误的动作。可靠的行动可行性评分器是具身智能体流程中的关键瓶颈。

Method: 提出对比世界模型（CWM），使用InfoNCE对比目标对大型语言模型进行微调，通过挖掘困难负样本（语义相似但物理不兼容的候选动作）来训练动作评分器，使有效动作在评分空间中远离无效动作。

Result: 在ScienceWorld基准测试中：1）内在可供性评估显示，CWM在最小编辑负样本上的Precision@1比SFT高6.76个百分点，AUC-ROC更高（0.929 vs 0.906）；2）实时过滤器特性研究表明，在分布外压力条件下，CWM保持更好的安全边际（-2.39 vs -3.96），表明黄金动作排名更靠前。

Conclusion: 对比训练比单独使用监督微调更能诱导出忠实捕捉物理可行性的表示，支持了对比学习能更好地区分物理上可执行与不可执行动作的假设。
...
TL;DR: ConstraintBench：评估LLM直接解决约束优化问题的基准，发现可行性而非最优性是主要瓶颈，最佳模型仅65%约束满足率。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLM能否将优化问题表述为求解器代码，但缺少对LLM能否直接产生正确解决方案的评估。需要研究LLM在完全指定的约束优化问题中，不依赖求解器直接生成可行且最优解的能力。

Method: 引入ConstraintBench基准，涵盖10个运筹学领域，包含200个任务。每个任务提供自然语言场景描述（实体、约束、优化目标），模型需返回结构化解决方案。通过确定性验证器检查每个约束，并与Gurobi求解器验证的最优解对比。

Result: 评估6个前沿模型发现：可行性是主要瓶颈，最佳模型仅65.0%约束满足率；可行解平均达到Gurobi最优目标的89-96%；没有模型在可行性和最优性（与求解器参考值相差0.1%以内）联合指标上超过30.5%。领域间差异大：生产组合领域平均可行性83.3%，机组分配领域仅0.8%。系统失败模式包括持续时间约束误解、实体幻觉、设施选址和车辆路径中的可行性-最优性解耦现象。

Conclusion: LLM在直接约束优化方面面临显著挑战，可行性是核心瓶颈而非最优性。ConstraintBench为评估LLM的约束优化能力提供了标准化基准，揭示了领域特异性差异和系统失败模式，有助于指导未来模型改进。
...
TL;DR: VERO框架为编码智能体的优化任务提供系统评估工具，通过版本控制、奖励机制和观察记录来支持智能体的迭代改进


<details>
  <summary>Details</summary>
Motivation: 编码智能体优化作为新兴应用领域缺乏系统性的评估方法，传统软件工程方法不适用于包含随机LLM生成的智能体改进过程

Method: 提出VERO框架，包含版本化智能体快照、预算控制评估和结构化执行轨迹的可复现评估工具，以及包含目标智能体和任务的基准测试套件

Result: 使用VERO进行实证研究，比较不同优化器配置在任务中的表现，分析哪些修改能可靠提升目标智能体性能

Conclusion: VERO框架的发布将支持编码智能体优化作为核心能力的研究，填补了该领域系统性评估工具的空白
...
TL;DR: 使用大语言模型进行AI-LCA研究文献综述，识别趋势、主题和未来方向


<details>
  <summary>Details</summary>
Motivation: 尽管AI在生命周期评估中的应用加速发展，但缺乏全面综合的AI-LCA研究综述，需要填补这一空白

Method: 结合LLM文本挖掘与传统文献综述技术，建立动态有效框架，分析AI与LCA交叉领域的研究

Result: 发现AI技术在LCA中应用显著增长，向LLM驱动方法转变，ML应用持续增加，AI方法与LCA阶段存在显著相关性

Conclusion: LLM辅助方法支持大规模可重复综述，为计算高效的LCA提供路径，帮助从业者整合先进工具提升可持续决策质量
...
TL;DR: 该论文提出从认知模型和AI算法中获取灵感来设计模块化语言智能体，通过定义智能体模板来规范单个LLM的角色和功能组合方式，并展示了现有语言智能体如何基于这些模板构建。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLM）单独使用时能力有限，许多复杂问题超出单个LLM的能力范围。如何将多个LLM作为组件组合成一个更强大的整体仍存在不确定性，需要系统化的设计方法。

Method: 提出"智能体模板"的概念，用于规范单个LLM的角色和功能组合方式。通过文献调研，分析现有语言智能体，揭示它们如何直接从认知模型或AI算法中衍生出底层模板。

Result: 展示了多种现有语言智能体如何基于认知科学和AI启发的模板构建，验证了这种设计方法的有效性。这些模板为开发模块化语言智能体提供了系统化的设计蓝图。

Conclusion: 认知科学和AI算法启发的智能体模板是开发有效、可解释语言智能体的强大工具。这种方法为模块化语言智能体设计提供了系统化的理论基础和实践指导。
...
TL;DR: AHCE框架通过主动学习策略将人类专家作为交互式推理工具，解决LLM智能体在专业领域因缺乏长尾知识而失败的问题，显著提升任务成功率


<details>
  <summary>Details</summary>
Motivation: LLM智能体在通用推理方面表现出色，但在需要特定领域长尾知识的专业任务中经常失败。虽然人类专家可以提供这些缺失知识，但他们的指导通常是非结构化和不可靠的，直接集成到智能体计划中存在困难。
...
Conclusion: 成功增强智能体需要学习如何请求专家推理，而不仅仅是简单的求助请求。AHCE框架通过主动学习策略有效整合人类专业知识，解决了LLM在专业领域的知识局限性问题。
...
TL;DR: CourtGuard：基于检索增强的多智能体框架，将安全评估重构为证据辩论，无需微调即可实现SOTA安全性能，具有零样本适应性和自动数据审计能力


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全机制依赖静态微调分类器，存在适应刚性，无法在不昂贵重训练的情况下实施新治理规则

Method: 引入CourtGuard检索增强多智能体框架，将安全评估重构为基于外部政策文档的证据辩论，通过对抗性辩论实现安全评估

Result: 在7个安全基准上实现SOTA性能，超越专用政策遵循基线；零样本适应维基百科破坏检测任务达90%准确率；自动审计并策划9个新颖对抗攻击数据集

Conclusion: 将安全逻辑与模型权重解耦为AI治理提供了鲁棒、可解释且适应性强的方法，能满足当前和未来的监管要求
...
Method: 通过对人类编写和模型生成解决方案的对照分析，识别使用与可执行性之间的系统性分离。基于此诊断，提出选择性策略检索（SSR）框架，该框架通过经验性、多路径、源感知信号来显式建模可执行性，选择性检索和组合策略。
...
TL;DR: 该论文提出将心理测量学评分者模型整合到AI评估流程中，以解决人类评分中的系统性误差问题，提高AI模型评估的可靠性和有效性。


<details>
  <summary>Details</summary>
Motivation: 人类评估在AI模型训练和评估中占据核心地位，但这些数据很少被视为存在系统性误差的测量结果。当前AI评估中的人类评分存在评分者效应（如严格性和中心性偏差），这些偏差会扭曲观察到的评分，影响对AI输出质量的准确判断。

Method: 将心理测量学评分者模型（特别是项目反应理论评分者模型）整合到AI评估流程中，重点使用多面Rasch模型来分离真实输出质量与评分者行为。以OpenAI摘要数据集为实证案例，展示如何通过调整评分者严格性来校正摘要质量估计，并提供评分者表现的诊断性见解。

Result: 通过调整评分者严格性，获得了校正后的摘要质量估计，并提供了对评分者表现的诊断性洞察。研究表明，心理测量学建模能够产生更准确、可靠的AI输出质量评估，减少评分者偏差的影响。

Conclusion: 将心理测量学建模整合到人机协同评估中，能够实现更原则化、透明化的人类数据使用，使开发者能够基于校正后的分数而非原始、易错的评分做出决策。这一视角为AI开发和评估提供了更稳健、可解释且与构念对齐的实践路径。
...
<details>
  <summary>Details</summary>
Motivation: 在线广告环境复杂且动态变化，多渠道场景下预算和约束的有效分配对投资回报优化至关重要。现有方法存在局限性：基于优化的方法缺乏动态适应性，强化学习方法难以在MDP框架内捕捉历史依赖性和观测模式。

Method: 提出AHBid自适应分层出价框架，整合生成式规划和实时控制。高层使用基于扩散模型的生成式规划器动态分配预算和约束，捕捉历史上下文和时间模式；引入约束执行机制确保符合指定约束，以及轨迹细化机制利用历史数据增强环境适应性；系统还包含基于控制的出价算法，协同结合历史知识和实时信息。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在复杂任务中的广泛应用，其需要在长期交互中适应用户个性化需求并保持行为连续性。当前研究缺乏对个性化LLM智能体系统性的框架梳理，需要从能力导向的角度整合相关文献，为构建更用户对齐、自适应、可部署的智能体系统提供结构化指导。

Method: 采用能力导向的综述方法，将文献组织为四个相互依赖的组件：1) 用户画像建模（profile modeling）- 如何表示和建模用户特征；2) 记忆（memory）- 如何存储和检索用户相关信息；3) 规划（planning）- 如何基于用户特征制定决策；4) 行动执行（action execution）- 如何将个性化决策转化为具体行动。通过这一分类框架分析用户信号在不同组件间的表示、传播和利用机制。

Result: 提出了一个结构化的个性化LLM智能体分析框架，系统梳理了代表性方法，揭示了跨组件交互模式和常见设计权衡。总结了针对个性化智能体的评估指标和基准测试，涵盖从通用助手到专业领域的应用场景，为从原型个性化到可扩展实际助手的发展提供了路线图。

Conclusion: 个性化已渗透到LLM智能体的整个决策流程中，而不仅仅是表层生成。通过提供结构化框架，本综述为理解和设计个性化LLM智能体奠定了基础，将加速从原型个性化到可扩展实际助手系统的进展，推动构建更用户对齐、自适应、鲁棒和可部署的智能体系统。
...
<details>
  <summary>Details</summary>
Motivation: RLHF在LLM后训练中被广泛应用以对齐人类偏好，但现有RLHF框架依赖服务器基础设施，难以应对细粒度资源变化，导致同步训练中组件间/内的空闲时间造成开销和资源浪费

Method: 基于无服务器计算环境构建同步RLHF训练框架，采用动态资源需求适配、共享前缀预计算避免重复计算、成本感知的actor缩放策略考虑响应长度变化寻找最优平衡点，以及高效分配工作负载减少函数内不平衡和空闲时间

Result: 在物理测试平台和大规模模拟集群上的实验表明，RLHFless相比最先进的基线方法实现了最高1.35倍加速和44.8%的成本降低

Conclusion: RLHFless是首个基于无服务器计算的可扩展同步RLHF训练框架，通过动态资源适配和优化策略有效解决了传统服务器基础设施在RLHF训练中的效率问题，显著提升了训练速度和成本效益
...
<details>
  <summary>Details</summary>
Motivation: 现有跨域推荐主要采用模型中心范式，依赖复杂的定制架构，难以捕捉跨域的细微非结构化序列依赖，导致泛化能力差、计算资源需求高。同时，域间差距会降低混合域数据质量，引发负迁移问题。

Method: 提出Taesar（目标对齐序列再生）数据框架，采用对比解码机制自适应地将跨域上下文编码到目标域序列中，使标准模型无需复杂融合架构即可学习复杂依赖关系。

Result: 实验表明Taesar优于模型中心解决方案，并能泛化到各种序列模型。通过生成丰富的数据集，有效结合了数据中心和模型中心范式的优势。

Conclusion: Taesar通过数据中心的序列再生方法解决了跨域推荐中的负迁移和泛化问题，为跨域推荐提供了更高效、更通用的解决方案。
...
TL;DR: 该研究分析了HealthBench医疗AI评估数据集中医生意见分歧的来源，发现81.8%的分歧来自病例层面的残差，无法通过现有元数据、专业领域或语言特征解释，但可减少的不确定性（如信息缺失）会显著增加分歧几率。
...
Method: 使用HealthBench医疗AI评估数据集，通过方差分解分析医生意见分歧的来源。评估了多种因素对分歧的影响，包括：评估标准身份、医生身份、元数据标签、规范性语言、医学专业领域、表面特征分类、嵌入表示等。同时分析了完成质量与分歧的关系，并基于医生验证的不确定性类别（可减少vs不可减少的不确定性）进行回归分析。

Result: 1. 病例层面的残差解释了81.8%的分歧方差，无法通过现有特征显著减少；2. 评估标准身份仅解释3.6-6.9%的分歧方差，医生身份仅2.4%；3. 分歧与完成质量呈倒U型关系（AUC=0.689）；4. 可减少的不确定性（缺失上下文、模糊表述）使分歧几率增加2.55倍，而不可减少的不确定性（真正的医学模糊性）无显著影响；5. 即使可减少的不确定性也仅解释约3%的总方差。

Conclusion: 医疗AI评估中的一致性上限主要是结构性的，但可减少与不可减少不确定性的分离表明，在评估场景中填补信息缺口可以在不涉及固有临床模糊性的情况下降低分歧，这为改进评估设计提供了可操作的路径。
...
TL;DR: AMA-Bench：评估LLM智能体长时记忆的新基准，包含真实和合成轨迹数据，揭示现有记忆系统在因果性和目标信息方面的不足，并提出改进方案AMA-Agent


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆评估标准与现实应用存在显著差距：当前基准主要关注对话中心的人机交互，而实际智能体记忆包含连续的机器生成表示的环境交互流。需要建立更贴近真实应用的评估基准。

Method: 提出AMA-Bench基准，包含两个关键组件：1）真实世界智能体轨迹数据集，涵盖代表性应用场景并配有专家标注的QA；2）可扩展到任意时间跨度的合成智能体轨迹数据集，配有基于规则的QA。同时提出AMA-Agent记忆系统，采用因果图结构和工具增强检索机制。

Result: 研究发现现有记忆系统在AMA-Bench上表现不佳，主要因为缺乏因果性和目标信息，且受限于基于相似性检索的有损特性。AMA-Agent在AMA-Bench上达到57.22%的平均准确率，比最强基线记忆系统提升11.16%。

Conclusion: AMA-Bench为评估LLM智能体长时记忆提供了更贴近实际应用的基准，揭示了现有记忆系统的关键缺陷。提出的AMA-Agent通过因果图和工具增强检索有效解决了这些问题，显著提升了智能体在长时记忆任务上的性能。
...
TL;DR: 论文提出CC-BOS框架，利用文言文特性自动生成对抗性提示进行越狱攻击，通过多维度果蝇优化算法在八种策略维度上迭代优化，显著提升黑盒越狱攻击效果。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型广泛应用，其安全风险日益受到关注。现有研究表明LLMs对越狱攻击高度敏感，且攻击效果在不同语言环境下存在差异。文言文因其简洁性和隐晦性，能够部分绕过现有安全约束，暴露LLMs的显著漏洞，这为研究提供了切入点。

Method: 提出CC-BOS框架，基于多维度果蝇优化算法自动生成文言文对抗性提示。将提示编码为八个策略维度：角色、行为、机制、隐喻、表达、知识、触发模式和上下文；通过嗅觉搜索、视觉搜索和柯西变异进行迭代优化。为提升可读性和评估准确性，设计了文言文到英文的翻译模块。

Result: 大量实验证明CC-BOS框架的有效性，在越狱攻击效果上持续优于最先进的攻击方法，验证了文言文在绕过LLMs安全约束方面的优势。

Conclusion: 文言文因其语言特性能够有效绕过LLMs的安全约束，CC-BOS框架通过自动化生成文言文对抗性提示，显著提升了黑盒越狱攻击的效果，揭示了LLMs在跨语言安全防护方面的重要漏洞。
...
TL;DR: ClinDet-Bench基准测试评估LLM在临床信息不全场景下的可确定性识别能力，发现现有LLM无法正确判断何时可下结论何时应弃权，存在过早判断和过度弃权问题。


<details>
  <summary>Details</summary>
Motivation: 临床决策常在信息不全情况下进行，专家需判断可用信息是否足以做出判断。过早结论和不必要弃权都会危害患者安全。现有基准测试不足以评估LLM在临床环境中的安全性，需要专门评估可确定性识别能力。

Method: 基于临床评分系统开发ClinDet-Bench基准测试，将信息不全场景分解为可确定和不可确定条件。识别可确定性需要考虑所有关于缺失信息的假设（包括不太可能的假设），并验证结论是否在所有假设下都成立。

Result: 近期LLM在信息不全情况下无法识别可确定性，产生过早判断和过度弃权，尽管它们能正确解释底层评分知识并在完整信息下表现良好。这表明现有基准测试不足以评估LLM在临床环境中的安全性。

Conclusion: ClinDet-Bench为评估可确定性识别能力提供了框架，促使适当的弃权决策，在医学和其他高风险领域具有潜在适用性，且已公开可用。
...
TL;DR: MiroFlow是一个高性能、鲁棒的开源智能体框架，通过智能体图编排、深度推理模式和鲁棒工作流执行来解决现有LLM智能体在复杂任务中的局限性，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型取得了显著进展，但独立LLM在处理需要与外部工具和环境交互的复杂现实任务时能力开始趋于平缓。现有智能体框架存在工作流程简单、性能不稳定、基准支持有限、过度依赖昂贵商业API等问题。

Method: 提出了MiroFlow框架，包含三个核心组件：1) 智能体图用于灵活编排任务流程；2) 可选的深度推理模式以增强性能；3) 鲁棒的工作流执行确保稳定和可复现的性能。
...
### [26] [LLM Novice Uplift on Dual-Use, In Silico Biology Tasks](https://arxiv.org/abs/2602.23329)
...
TL;DR: LLM访问显著提升新手在生物安全相关任务上的表现，准确率比仅使用互联网的对照组高4.16倍，甚至在某些任务上超越专家，但用户未能充分利用LLM的全部能力


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估LLM是否真正提升新手用户在生物学任务上的表现，而不仅仅是基准测试成绩，这对于理解科学加速和双重用途风险至关重要

Method: 采用多模型、多基准的人类提升研究，比较新手在有LLM访问和仅使用互联网访问两种情况下的表现，涵盖八个生物安全相关任务集，参与者有充足时间（最复杂任务达13小时）

Result: LLM访问提供显著提升：新手使用LLM的准确率比对照组高4.16倍；在四个有专家基线的基准测试中，新手使用LLM在三个上超越了专家；独立LLM通常超过LLM辅助的新手，表明用户未能充分利用LLM能力；89.6%参与者报告获取双重用途相关信息几乎没有困难

Conclusion: LLM显著提升了新手在原本需要专业训练的生物任务上的表现，强调需要持续、交互式的提升评估与传统基准测试并行，以全面评估LLM的实际影响和风险
...
TL;DR: FlexMS：用于质谱预测的灵活基准框架，支持多种模型架构的动态构建和评估，提供性能影响因素分析及实际应用指导


<details>
  <summary>Details</summary>
Motivation: 质谱技术在药物发现和材料科学中至关重要，但实验谱图缺乏阻碍了分子鉴定，需要建立计算预测方法。深度学习模型在预测分子结构谱图方面有前景，但由于方法异质性和缺乏明确定义的基准，整体评估仍然困难。

Method: 创建FlexMS基准框架，支持动态构建多种模型架构组合，在预处理公共数据集上使用不同指标评估性能。分析影响性能的因素，包括数据集结构多样性、超参数（学习率、数据稀疏性）、预训练效果、元数据消融设置和跨域迁移学习分析。

Result: FlexMS框架提供了灵活易用的基准测试环境，能够系统评估不同模型架构在质谱预测任务上的性能。通过分析揭示了数据集多样性、超参数设置、预训练策略等因素对预测性能的具体影响，为模型选择提供了实践指导。

Conclusion: FlexMS为质谱预测领域建立了标准化基准框架，解决了方法异质性和评估标准不统一的问题。该框架不仅支持模型性能评估，还通过检索基准模拟实际鉴定场景，为药物发现和材料科学中的分子鉴定提供了实用工具和指导原则。
...
TL;DR: 通过人类-AI协作发现Hermite求积规则新误差表示和界限的案例研究，展示了AI在数学研究中的潜力与局限


<details>
  <summary>Details</summary>
Motivation: 探究人工智能是否真正能够促进创造性数学研究，还是仅仅自动化常规计算并引入错误风险

Method: 使用多个AI助手进行系统的人类-AI协作，通过详细案例研究Hermite求积规则的新误差表示和界限的发现过程

Result: AI在代数操作、系统证明探索、文献综合和LaTeX准备方面表现出色，但每一步都需要严格的人类验证、数学直觉和战略指导；成功制定并证明了多个定理
...
### [29] [Towards LLM-Empowered Knowledge Tracing via LLM-Student Hierarchical Behavior Alignment in Hyperbolic Space](https://arxiv.org/abs/2602.22879)
...
<details>
  <summary>Details</summary>
Motivation: 现有知识追踪方法主要基于ID序列或浅层文本特征，难以捕捉认知状态的层次演化以及个体对问题难度的感知差异，因为语义建模能力有限

Method: 1) 教师智能体深度解析问题语义并显式构建知识点层次依赖关系；学生智能体模拟学习行为生成合成数据
...
Conclusion: 提出的L-HAKT框架通过大语言模型深度语义解析和双曲空间对齐，能够更好地建模知识点的层次结构和个体学习差异，提升了知识追踪性能
...
TL;DR: 提出OmniGAIA基准测试和OmniAtlas原生全模态基础智能体，用于评估和提升全模态（视觉、音频、语言）智能体的推理和工具使用能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要局限于双模态交互（如视觉-语言），缺乏统一认知能力，无法满足通用AI助手的需求。需要弥合人类智能的全模态感知与复杂推理、工具使用之间的差距。

Method: 1. 提出OmniGAIA基准：通过全模态事件图方法构建，合成需要跨模态推理和多轮工具执行的复杂查询。2. 提出OmniAtlas智能体：基于工具集成推理范式的原生全模态基础智能体，采用主动全模态感知，通过后见指导树探索策略合成轨迹，并使用OmniDPO进行细粒度错误校正。

Result: OmniGAIA基准能够评估全模态智能体在视频、音频和图像模态上的深度推理和多轮工具执行能力。OmniAtlas有效提升了现有开源模型的工具使用能力。

Conclusion: 这项工作标志着向下一代原生全模态AI助手迈出了一步，为现实世界场景中的通用AI助手发展奠定了基础。
...
TL;DR: 本文提出首个通用智能体评估框架，通过统一协议和Exgentic框架对五种主流智能体在六个环境中进行基准测试，建立开放通用智能体排行榜，证明通用智能体无需环境特定调优即可达到领域专用智能体性能水平。


<details>
  <summary>Details</summary>
Motivation: 当前通用智能体（能在陌生环境中执行任务而无需领域特定工程）的承诺尚未实现。现有智能体多为专用系统，缺乏对通用智能体性能的系统性评估。现有基准测试假设领域特定集成，编码任务信息的方式无法公平评估通用智能体。

Method: 提出通用智能体评估作为首要研究目标，建立概念性原则和统一协议实现智能体-基准集成，开发Exgentic实践框架。对五种主流智能体实现（包括OpenAI SDK Agent和Claude Code）在六个环境中进行基准测试，建立首个开放通用智能体排行榜。

Result: 实验表明通用智能体能在多样化环境中泛化，无需任何环境特定调优即可达到与领域专用智能体相当的性能水平。建立了首个系统性的通用智能体评估基准和排行榜。

Conclusion: 本文为通用智能体研究奠定基础，通过发布评估协议、框架和排行榜，推动通用智能体研究的系统化发展。证明通用智能体评估的可行性和重要性，为未来研究提供标准化工具。
...
Conclusion: Certified Circuits为电路发现奠定了形式化基础，产生可证明稳定且与目标概念更好对齐的机制解释，提高了电路发现的可靠性和可解释性。
...
TL;DR: SPM-Bench：针对扫描探针显微镜的PhD级别多模态基准测试，通过自动化数据合成和Anchor-Gated Sieve技术从arXiv和期刊论文中提取高质量图像-文本对，引入SIP-F1分数评估LLM性能并量化模型"个性"。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在专业科学领域存在明显差距，现有基准测试存在数据污染、复杂度不足和人工成本过高的问题。需要开发专门针对扫描探针显微镜（SPM）这一复杂物理科学领域的高质量基准测试。

Method: 1. 开发全自动数据合成管道，确保高权威性和低成本；2. 使用Anchor-Gated Sieve技术从2023-2025年arXiv和期刊论文中高效提取高价值图像-文本对；3. 采用混合云-本地架构，VLMs仅返回空间坐标供本地高保真裁剪，实现极致的token节省；4. 引入Strict Imperfection Penalty F1（SIP-F1）评分系统。

Result: 建立了SPM-Bench基准测试，能够准确客观评估LLM性能。SIP-F1分数不仅建立了严格的能力层次结构，还首次量化了模型的"个性"（保守型、激进型、赌徒型或智慧型）。通过将结果与模型报告的置信度和感知难度相关联，揭示了当前AI在复杂物理场景中的真实推理边界。

Conclusion: SPM-Bench为自动化科学数据合成提供了一个可推广的范式，能够有效评估LLM在专业科学领域的真实能力，并为理解模型在复杂物理场景中的推理行为提供了新的量化框架。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键的临床AI中，人机协同验证至关重要，但初始模型推理与专家修正之间的过渡很少被分析为结构化信号。需要一种能够量化修正动态并支持可追溯、人机对齐评估的方法。

Method: 引入诊断对齐框架，保留AI生成的影像报告作为不可变推理状态，系统比较医生验证结果。推理管道集成视觉大语言模型、基于BERT的医学实体提取和序列语言模型推理(SLMI)步骤，在专家审查前强制执行领域一致的精炼。在21个皮肤病案例上使用四级一致性框架评估：精确主要匹配率(PMR)、语义相似性调整率(AMR)、跨类别对齐和综合一致性率(CCR)。

Result: 精确一致性达到71.4%，语义相似性下保持不变(t=0.60)。结构化跨类别和鉴别诊断重叠分析产生100%综合一致性(95% CI: [83.9%, 100%])。没有案例显示完全诊断分歧。二元词汇评估显著低估了临床意义对齐。

Conclusion: 将专家验证建模为结构化转换，能够实现信号感知的修正动态量化，支持基于影像的临床决策支持系统的可追溯、人机对齐评估。该方法超越了简单的词汇匹配，捕捉了临床意义的对齐。
...
TL;DR: RepSPD：一种基于黎曼流形交叉注意力的几何深度学习模型，用于解码EEG脑电活动，通过图功能连接特征调制SPD几何属性，并采用全局双向对齐策略增强几何一致性。


<details>
  <summary>Details</summary>
Motivation: 当前基于对称正定矩阵的EEG解码方法主要关注统计聚合，忽略了频率特异性同步和脑区局部拓扑结构，需要更精细的几何深度学习方法来揭示脑功能连接的结构特征。

Method: 提出RepSPD模型：1）在黎曼流形上实现交叉注意力机制，用图功能连接特征调制SPD的几何属性；2）引入全局双向对齐策略重塑切空间嵌入，减轻曲率引起的几何失真，增强几何一致性。
...
<details>
  <summary>Details</summary>
Motivation: F1比赛中策略需要根据不断变化的比赛条件和对手行动进行调整，传统方法难以实时优化多智能体交互下的复杂决策

Method: 基于预训练单智能体策略，引入交互模块考虑对手行为，结合自博弈训练方案生成竞争性策略，根据相对性能对智能体进行排名

Result: 智能体能适应对手调整进站时机、轮胎选择和能量分配，实现稳健一致的比赛表现，框架仅依赖真实比赛中的可用信息
...
TL;DR: 提出AILS-AHD方法，结合大语言模型与自适应迭代局部搜索，通过LLM动态生成和优化破坏启发式，显著提升CVRP求解性能，在多个基准测试中创下新最优解记录。


<details>
  <summary>Details</summary>
Motivation: CVRP作为组合优化基础问题具有NP-hard特性，大规模实例计算挑战巨大。现有方法在动态启发式设计方面存在局限，需要更智能的优化策略来提升求解效率和质量。

Method: 提出AILS-AHD方法，将LLM集成到进化搜索框架中，动态生成和优化破坏启发式；采用LLM加速机制提升计算效率；结合自适应迭代局部搜索算法。

Result: 在CVRPLib大规模基准测试中，AILS-AHD在10个实例中的8个创下新的最优解记录；相比AILS-II和HGS等先进求解器表现出优越性能；验证了LLM驱动启发式设计的有效性。

Conclusion: LLM驱动的启发式设计为车辆路径优化领域带来新突破，AILS-AHD方法通过智能生成优化启发式，显著提升CVRP求解性能，展示了AI与运筹学结合的潜力。
...
TL;DR: MALLET是一个基于多智能体LLM的情感去毒系统，通过四个智能体将高刺激内容转化为平衡/冷静模式，减少情感刺激同时保持语义完整性。


<details>
  <summary>Details</summary>
Motivation: 在注意力经济中，煽情内容让消费者暴露于过度情感刺激，阻碍冷静决策。需要一种既能减少情感刺激又不限制原始文本访问的信息净化方法。

Method: 提出多智能体LLM情感去毒系统(MALLET)，包含四个智能体：情感分析智能体（使用6情感BERT分类器量化刺激强度）、情感调整智能体（用LLM将文本重写为BALANCED和COOL两种呈现模式）、平衡监控智能体（聚合周度信息消费模式生成个性化建议）、个人指导智能体（根据消费者敏感性推荐呈现模式）。

Result: 在800篇AG News文章上的实验显示：刺激分数显著降低（最高19.3%），情感平衡改善，同时保持语义保存；刺激减少与语义保存之间的相关性接近零，表明两者可独立控制；类别分析显示体育、商业、科技类刺激大幅减少（17.8-33.8%），而世界类效果有限（事实本身具有高刺激性）。

Conclusion: MALLET提供了一个框架，在不限制访问原始文本的情况下支持消费者冷静接收信息，为情感刺激管理提供了可行的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前虽然存在Sora等视频生成模型和统一多模态模型（UMM）在物理动力学近似方面取得进展，但缺乏定义通用世界模型必备属性的原则性理论框架。需要建立系统的理论指导来推动通用人工智能的发展

Method: 提出三位一体一致性理论框架：模态一致性（语义接口）、空间一致性（几何基础）、时间一致性（因果引擎）。通过这一框架系统回顾多模态学习演进，并引入CoW-Bench基准来评估多帧推理和生成场景下的模型性能

Result: 建立了通用世界模型的原则性理论框架，揭示了从松散耦合的专门模块向统一架构的发展轨迹，这些架构能够协同涌现内部世界模拟器。CoW-Bench为视频生成模型和UMMs提供了统一的评估协议

Conclusion: 该工作为通用世界模型建立了原则性路径，阐明了当前系统的局限性以及未来进展所需的架构要求，为人工智能领域提供了重要的理论指导和评估工具
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的时间序列推理方法存在两个主要局限：1) 将时间序列简单视为文本或图像，无法捕捉趋势和季节性等关键模式来回答具体问题；2) 在混合简单和复杂任务训练时，简单目标主导学习过程，阻碍深度推理能力发展。
...
Conclusion: PATRA通过模式感知对齐和平衡奖励设计，有效解决了现有方法在时间序列模式捕捉和深度推理能力发展方面的局限性，为时间序列推理提供了更强大的解决方案。
...
TL;DR: 论文证明基于优化的AI系统（如RLHF训练的LLM）在形式上无法实现规范性治理，因为它们缺乏真正智能体所需的不通约性和否定性响应能力，导致现有失败模式是结构性而非偶然的。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统被部署在医疗、法律、金融等高风险领域，假设它们可以被规范所治理。本文旨在证明这一假设对于基于优化的系统（特别是通过RLHF训练的大语言模型）在形式上是无效的。

Method: 通过形式化分析建立真正智能体所需的两项必要且充分的结构条件：1) 不通约性 - 将某些边界视为不可协商的约束而非可交易权重的能力；2) 否定性响应 - 当这些边界受到威胁时能够暂停处理的非推理机制。证明RLHF系统在结构上与这两个条件不相容。

Result: 证明基于优化的系统（如RLHF）在结构上无法实现规范性治理，因为它们将所有价值统一到标量度量上并始终选择最高分输出的操作，恰恰排除了规范性治理的可能性。现有失败模式（奉承、幻觉、不忠推理）不是偶然而是结构性表现。还识别出"趋同危机"这一二阶风险。

Conclusion: 基于优化的AI系统在形式上无法成为真正的规范性智能体。论文的主要积极贡献是提供了一个基质中立的架构规范，定义了任何系统（生物、人工或制度）要成为智能体而非复杂工具所必须满足的条件。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用强化学习中的最优智能体（如AIXI）都是基于模型的，需要显式维护和使用环境模型。这限制了智能体的多样性和实现方式。本研究旨在探索无模型方法在通用强化学习中的可行性，扩展已知通用智能体的多样性。

Method: 提出Universal AI with Q-Induction (AIQI)，这是一种无模型智能体。与以往工作对策略或环境进行归纳不同，AIQI对分布动作价值函数进行通用归纳。该方法在"grain of truth"条件下运作，避免了显式的环境模型维护。

Result: 在"grain of truth"条件下，证明了AIQI具有强渐近ε最优性和渐近ε贝叶斯最优性。这是首个被证明在通用强化学习中具有渐近最优性的无模型智能体，显著扩展了已知通用智能体的类型。

Conclusion: AIQI作为首个无模型通用智能体，通过分布动作价值函数的归纳实现了渐近最优性，突破了传统基于模型方法的限制，为通用强化学习提供了新的研究方向和方法多样性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型输出需要能被较弱系统轻松验证，但现有证明者-验证者游戏会导致准确性下降（可读性税）

Method: 解耦正确性和可验证性条件，先训练求解器最大化正确性，再训练翻译器将求解器输出转换为可验证形式并保持答案

Result: 提出解耦证明者-验证者游戏，其均衡对应忠实且可验证的翻译器

Conclusion: 通过解耦框架可在保持求解器准确性的同时实现输出可验证性，避免可读性税问题
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统在复杂推理中表现出色，但存在个体参与者生成错误信息导致的级联影响问题。现有解决方案通常依赖刚性结构工程或昂贵的微调，限制了部署性和适应性。
...
TL;DR: ODEBRAIN：一种基于神经ODE的连续潜在动态预测框架，用于建模神经群体动力学，通过整合时空频率特征到谱图节点中，克服传统方法的累积预测误差问题。


<details>
  <summary>Details</summary>
Motivation: 传统潜在变量方法通常通过递归架构离散化时间来建模连续大脑动力学，这必然导致累积预测误差的复合，并且无法捕捉EEG的瞬时非线性特征。需要一种能够建模连续潜在动态并捕捉复杂脑状态随机变化的方法。

Method: 提出ODEBRAIN框架：1）将时空频率特征整合到谱图节点中；2）使用神经ODE建模连续潜在动态；3）确保潜在表示能够捕捉任意时间点上复杂脑状态的随机变化。

Result: 大量实验验证ODEBRAIN在预测EEG动态方面显著优于现有方法，具有增强的鲁棒性和泛化能力。

Conclusion: ODEBRAIN通过神经ODE框架成功建模了连续的大脑动力学，克服了传统离散化方法的局限性，为神经科学研究提供了更准确的预测工具。
...
Conclusion: AGM信念修正是KM信念更新的特殊情形，两者在模态逻辑框架下具有明确的包含关系，强版本KM更新与AGM修正的差异主要体现在处理非意外信息的公理上。
...
<details>
  <summary>Details</summary>
Motivation: 即使经过优化的AI模型在推理时仍会因偶然性和认知不确定性产生错误，观察到基于输入不变变换的多个样本推理中，错误存在部分独立性
...
<details>
  <summary>Details</summary>
Motivation: GRAVE算法在通用游戏博弈中表现出色，但其需要在每个节点存储额外的胜率/访问统计信息，在内存受限环境中不实用，限制了实际应用。

Method: 提出三种新算法：GRAVE2（两级搜索）、GRAVER（节点回收）、GRAVER2（两种技术结合），通过减少存储节点数量来优化GRAVE的内存效率。

Result: 这些增强技术能够在保持GRAVE游戏强度的同时，实现存储节点数量的显著减少。

Conclusion: GRAVE2、GRAVER和GRAVER2算法通过内存优化扩展了GRAVE的实用性，使其在内存受限环境中更具应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于多智能体系统的自主金融交易系统通常采用模仿分析师和经理角色的抽象指令，忽略了真实工作流程的复杂性，导致推理性能下降和决策透明度降低。因此需要更精细的任务分解方法来提升系统性能。

Method: 提出多智能体LLM交易框架，将投资分析明确分解为细粒度任务而非粗粒度指令。使用日本股票数据（价格、财务报表、新闻、宏观信息）在泄漏控制的回测设置下评估。通过标准投资组合优化，利用与股票指数的低相关性及各系统输出的方差来提升性能。

Result: 实验结果表明，细粒度任务分解相比传统粗粒度设计显著改善了风险调整后收益。进一步分析中间智能体输出发现，分析输出与下游决策偏好的对齐是系统性能的关键驱动因素。投资组合优化方法实现了优越的性能表现。

Conclusion: 细粒度任务分解和多智能体对齐是提升LLM交易系统性能的关键因素。这些发现为在实际交易系统中应用LLM智能体的智能体结构和任务配置设计提供了重要指导。
...
Result: 该方法显著提高了模型鲁棒性，同时通过聚焦于生物学相关特征改善了预测准确性，成功缓解了基础模型对技术变异的敏感性

Conclusion: 该方法无需重新训练基础模型即可解决计算病理学基础模型的鲁棒性问题，使开发的模型能够应用于常规临床实践中的真实世界数据
...
TL;DR: AeroDGS：基于物理引导的4D高斯泼溅框架，用于单目无人机视频的动态空中场景重建，通过几何提升和物理优化解决单视角、大范围、小目标大运动的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有4D场景重建方法在单视角无人机拍摄条件下存在局限：大空间范围、动态目标空间足迹小且运动差异大，导致深度模糊和运动估计不稳定，使单目空中重建成为不适定问题。
...
TL;DR: 提出一种基于器官聚焦注意力损失函数的深度学习框架，无需分割3D肾脏CT图像即可预测恶性肿瘤，在私有和公开数据集上均超越传统需要分割的方法。


<details>
  <summary>Details</summary>
Motivation: 肾脏肿瘤恶性预测对临床决策至关重要，但现有影像方法在手术前预测准确性不足。传统深度学习依赖手动分割肿瘤区域以减少噪声，但手动分割耗时、昂贵且依赖专家知识。

Method: 开发深度学习框架，采用器官聚焦注意力损失函数，修改图像补丁的注意力机制，使器官补丁仅关注其他器官补丁，从而在部署时无需对3D肾脏CT图像进行分割即可预测恶性肿瘤。
...
TL;DR: ViT存在由全局注意力和粗粒度语义监督导致的"懒惰聚合"行为，使用语义无关的背景patch作为捷径表示全局语义，本文提出选择性集成patch特征到CLS token的方法来缓解此问题。
...
Result: 方法在12个基准测试中（标签监督、文本监督和自监督）均能一致提升性能。

Conclusion: ViT的伪影源于懒惰聚合行为，选择性特征集成能有效缓解此问题，为理解ViT行为提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: CLIP模型在互联网规模数据上通过图像-文本对比学习学习可迁移的多模态特征，但其预训练主要基于图像与简短标题配对，导致模型偏向编码显著物体的简单描述，在复杂场景和密集描述上对齐粗糙。现有工作通过在小型长标题数据集上微调来缓解此问题，但发现人类和LLM生成的长标题通常以一句话摘要开头，这成为训练中的捷径，使注意力集中在开头句子和早期标记上，削弱了与标题其余部分的对齐。

Method: 提出DeBias-CLIP方法：1) 移除训练中的摘要句；2) 应用句子子采样，从长标题中随机选择句子子集进行训练；3) 使用文本标记填充，确保所有标记位置都能获得监督信号。该方法作为Long-CLIP的直接替代品，无需额外可训练参数。

Result: DeBias-CLIP在长文本检索方面达到最先进水平，同时改善了短文本检索性能，对句子顺序排列的敏感性降低。模型能够更均匀地分布注意力到整个标题内容，而不仅仅是开头部分。

Conclusion: 通过识别并解决长标题训练中的摘要偏差问题，DeBias-CLIP显著提升了CLIP模型在复杂场景和密集描述上的对齐能力，为多模态检索任务提供了更鲁棒的解决方案。
...
TL;DR: 该论文诊断了多模态大语言模型(MLLMs)的视觉接地机制问题，发现模型存在"模态惰性"——尽管具备OCR能力，但在需要视觉参与的结构化任务中性能显著下降。作者提出了SimpleOCR训练策略，通过将训练样本转换为随机样式的可视化问题格式，强制模型激活视觉文本提取路径，从而在少量数据下实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究动机是诊断MLLMs的视觉接地机制：这些模型是否真正"读取"图像中的文本，还是仅仅依赖文本提示中的参数捷径。作者关注模型在需要视觉参与的结构化任务中的表现，以揭示其视觉文本处理能力的真实情况。

Method: 提出Visualized-Question(VQ)设置，将文本查询直接渲染到图像上，强制要求视觉参与。基于此诊断发现模型存在"模态惰性"，然后提出SimpleOCR训练策略：将训练样本转换为VQ格式并应用随机样式，使文本捷径失效，强制模型激活和优化视觉文本提取路径。这是一种即插即用的训练方法，无需架构修改。

Result: 在Qwen2.5-VL上的诊断实验显示，尽管模型具备强OCR能力，但在VQ设置下性能下降高达12.7%，揭示了"模态惰性"。SimpleOCR在四个代表性OOD基准测试中，比基础模型提升5.4%，比基于原始图像的GRPO提升2.7%。具有极高的数据效率，仅用8.5K样本(比最近RL方法少30倍)就实现了优越性能。还能与NoisyRollout等高级RL策略无缝集成获得互补改进。

Conclusion: MLLMs存在严重的"模态惰性"问题，即使具备OCR能力，也倾向于依赖文本捷径而非真正处理视觉文本。SimpleOCR通过结构约束有效解决了这一问题，以极低的数据成本显著提升了模型的视觉文本处理能力，为改进MLLMs的视觉接地机制提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 云端卸载虽然常见，但对于可穿戴助手存在隐私和延迟问题，因此研究在边缘设备上实现实时在线情景记忆问答系统，以解决隐私保护和低延迟需求。

Method: 采用双线程异步架构：描述符线程持续将视频转换为轻量级文本记忆，问答线程基于文本记忆进行推理回答。在QAEgo4D-Closed基准上测试，并考虑了严格的资源约束。

Result: 在消费级8GB GPU上实现端到端配置，准确率达51.76%，首词时间0.41秒；在企业级本地服务器上准确率54.40%，首词时间0.88秒；云端方案准确率56.00%。边缘方案表现出竞争力。

Conclusion: 边缘解决方案在隐私保护的情景记忆检索方面具有潜力，能够在资源受限环境下实现与云端方案竞争的性能，为可穿戴助手等应用提供了可行的本地化方案。
...
TL;DR: MammoWise是一个本地多模型管道，将开源视觉语言模型转换为乳腺X光报告生成器和多任务分类器，支持零样本、少样本和思维链提示，通过参数高效微调在BI-RADS、乳腺密度等任务上取得良好性能。


<details>
  <summary>Details</summary>
Motivation: 乳腺筛查工作量大、时间敏感且文档繁重，现有视觉语言模型多依赖封闭云系统或紧耦合架构，限制了隐私性、可复现性和适应性。需要开发本地化、可扩展的解决方案。

Method: 开发MammoWise本地多模型管道，支持任何Ollama托管的视觉语言模型和乳腺X光数据集，采用零样本、少样本、思维链提示，可选多模态检索增强生成，使用向量数据库提供病例特定上下文，并对MedGemma进行参数高效微调（QLoRA）。

Result: 在VinDr-Mammo和DMID数据集上评估MedGemma、LLaVA-Med和Qwen2.5-VL，报告生成质量稳定且通过少样本提示和RAG得到改善；分类任务可行但对模型和数据集选择敏感；MedGemma经QLoRA微调后，BI-RADS准确率达0.7545，密度准确率0.8840，钙化准确率0.9341，同时保持报告质量。

Conclusion: MammoWise为在统一可复现工作流中部署本地视觉语言模型进行乳腺X光报告提供了实用且可扩展的框架，在保护隐私和适应性的同时实现了良好的报告生成和分类性能。
...
TL;DR: 提出Spatial Credit Redistribution (SCR)方法，通过重新分配高注意力源补丁的激活到其上下文，减少视觉语言模型中的物体幻觉问题，在推理时无需额外训练。
...
Method: 引入Spatial Credit Redistribution (SCR)，一种无需训练、在推理时进行的干预方法。该方法重新分配隐藏状态激活，从高注意力的源补丁到其上下文，由低熵输入引导。
...
Conclusion: SCR通过解决空间信用崩溃问题，有效减少视觉语言模型的物体幻觉，同时保持计算效率，适用于实时应用。注意力引导的源选择是关键机制。
...
<details>
  <summary>Details</summary>
Motivation: 传统合成图像检索方法存在局限性：监督学习方法可能丢失细粒度线索，而零样本方法通过图像描述与编辑文本合并可能忽略用户隐含意图并产生重复结果。需要一种能更好理解用户意图并保持结果多样性的方法。

Method: 提出Pix2Key方法，将查询和候选图像表示为开放词汇视觉词典，在统一嵌入空间中进行意图感知约束匹配和多样性感知重排序。引入自监督预训练组件V-Dict-AE，仅使用图像数据改进词典表示，增强细粒度属性理解而无需CIR特定监督。

Result: 在DFMM-Compose基准测试中，Pix2Key将Recall@10提升了最高3.2个百分点，添加V-Dict-AE后进一步获得2.3个百分点的提升，同时改善了意图一致性并保持了高列表多样性。

Conclusion: Pix2Key通过开放词汇视觉词典表示和自监督预训练，有效解决了合成图像检索中的意图理解和多样性保持问题，在基准测试中表现出显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: tau-PET作为阿尔茨海默病病理的体内标志物，成本高且可用性有限，需要开发基于MRI的替代方法。

Method: 结合：(1) 基于部分信息分解(PID)的向量量化编码器，将潜在信息划分为冗余、独特和互补分量；(2) 半UNet解码器，使用基于结构边缘线索的伪跳跃连接而非直接特征重用，以保留解剖细节。

Result: 与多个基线方法(VAE、VQ-VAE、UNet)相比，DisQ-HNet保持了重建保真度，并更好地保留了与疾病相关的信号，适用于下游AD任务，包括Braak分期、tau定位和分类。PID-based Shapley分析提供了合成摄取模式的模态特异性归因。

Conclusion: DisQ-HNet提供了一种从MRI合成tau-PET的有效方法，同时通过信息分解框架提供了对多模态贡献的可解释性分析。
...
TL;DR: DrivePTS：一种用于自动驾驶场景生成的扩散模型，通过渐进学习、多视图文本描述和频率引导结构损失解决现有方法的条件依赖、语义细节不足和结构模糊问题


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶场景生成方法存在三个主要问题：1）几何条件间的隐式依赖导致条件独立变化时生成失败；2）视图不变且简短的文本描述限制了语义上下文，导致背景建模薄弱；3）均匀空间加权的标准去噪损失忽略前景结构细节，造成视觉失真和模糊

Method: 提出DrivePTS框架，包含三个关键创新：1）采用渐进学习策略配合显式互信息约束来缓解几何条件间的相互依赖；2）利用视觉语言模型生成跨六个语义方面的多视图分层描述，提供细粒度文本指导；3）引入频率引导结构损失增强模型对高频元素的敏感性，提升前景结构保真度
...
TL;DR: SwiftNDC：一种基于神经深度校正场的快速3D重建框架，通过生成跨视角一致的深度图，创建干净均匀的点云几何初始化，显著加速3D高斯溅射的网格重建和提升新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有深度引导的3D重建方法存在尺度漂移、多视角不一致性等问题，需要大量细化才能获得高保真几何。需要一种快速且通用的框架来解决这些问题，实现高效高质量的3D重建。

Method: 提出SwiftNDC框架，核心是神经深度校正场，用于生成跨视角一致的深度图。通过反投影和稳健的重投影误差滤波生成密集点云，为下游重建提供干净均匀的几何初始化。该几何初始化显著加速3D高斯溅射的网格重建，并提升新视角合成的渲染质量。

Result: 在五个数据集上的综合研究表明，SwiftNDC在准确网格重建方面显著减少运行时间，在视角合成方面提升渲染保真度。包括两个网格重建数据集和三个新视角合成数据集的实验结果验证了方法的有效性。

Conclusion: SwiftNDC通过结合神经深度细化和稳健几何初始化，实现了高保真且高效的3D重建。该方法为下游任务提供了可靠的密集几何初始化，显著加速3D高斯溅射的网格重建并提升新视角合成质量。
...
TL;DR: 提出QARMVC框架，通过信息瓶颈提取语义特征，利用重构差异量化细粒度噪声强度，采用分层学习策略：特征级质量加权对比学习抑制噪声传播，融合级质量加权聚合构建高质量全局共识，通过互信息最大化对齐校正局部视图。


<details>
  <summary>Details</summary>
Motivation: 现有多视图聚类方法主要基于二元假设（数据完全干净或完全损坏），忽略了现实应用中普遍存在的异质观测噪声（污染强度在数据间连续变化）。需要开发能够处理这种细粒度噪声的鲁棒方法。

Method: 1. 使用信息瓶颈机制提取内在语义进行视图重构；2. 利用重构差异量化污染强度，生成实例级质量分数；3. 分层学习策略：特征级采用质量加权对比目标自适应抑制噪声传播，融合级通过质量加权聚合构建高质量全局共识，并通过互信息最大化对齐校正局部视图。

Result: 在五个基准数据集上的广泛实验表明，QARMVC始终优于最先进的基线方法，特别是在具有异质噪声强度的场景中表现突出。

Conclusion: QARMVC通过量化细粒度噪声强度和分层学习策略，有效解决了现有方法忽略异质观测噪声的问题，为现实世界复杂噪声环境下的多视图聚类提供了鲁棒解决方案。
...
TL;DR: 论文揭示扩散模型指导方法评估存在严重偏差，提出新的评估框架GA-Eval，并设计了一个实际上无效但能在传统评估中得高分的TDG方法，证明当前评估体系存在问题。


<details>
  <summary>Details</summary>
Motivation: 尽管出现了多种扩散模型指导方法声称能提升生成质量，但作者质疑这些方法是否真的实现了实质性改进。他们发现当前评估体系存在严重偏差，导致评估结果不可靠。

Method: 1) 揭示评估陷阱：发现人类偏好模型对高指导尺度有强烈偏见；2) 提出GA-Eval框架：通过指导尺度校准实现公平比较；3) 设计TDG方法：构造一个实际上无效但能在传统评估中得高分的方法；4) 实验评估：在传统框架和GA-Eval框架下评估8种扩散指导方法。

Result: 1) 仅增加CFG尺度就能与大多数扩散指导方法竞争；2) 所有方法在胜率上都严重劣于标准CFG；3) TDG方法在传统评估中表现良好但实际上无效；4) 当前评估体系存在严重偏差，需要重新思考。

Conclusion: 当前扩散模型指导方法的评估体系存在根本性问题，需要重新思考评估范式和未来研究方向。仅增加CFG尺度就能达到类似效果，而许多新方法并未带来实质性改进。
...
TL;DR: GIFSplat是一种纯前馈的迭代优化框架，用于从稀疏无位姿视图中进行3D高斯泼溅重建，通过残差更新和蒸馏扩散先验实现高效高质量重建。
...
Method: 提出GIFSplat框架：1）使用少量前馈残差更新逐步优化当前3D场景；2）从增强的新视角渲染中蒸馏冻结的扩散先验到高斯级线索，无需梯度反向传播或不断增加的视图集扩展；3）保持纯前馈效率，无需相机位姿或测试时梯度优化。

Result: 在DL3DV、RealEstate10K和DTU数据集上，GIFSplat持续优于最先进的前馈基线，PSNR提升高达+2.1 dB，同时保持秒级推理时间，无需相机位姿或任何测试时梯度优化。

Conclusion: GIFSplat通过迭代残差更新和生成先验蒸馏，在前馈3D高斯泼溅重建中实现了效率与质量的良好平衡，克服了现有一次性预测范式的限制，为稀疏无位姿视图重建提供了高效解决方案。
...
TL;DR: CMDM提出了一种基于因果扩散变换器的自回归运动生成统一框架，通过语义对齐的潜在空间实现高质量文本到运动生成，支持流式合成和长时运动生成，在推理延迟和生成质量上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在局限性：全序列扩散模型依赖双向生成，限制了时间因果性和实时应用性；自回归模型则存在不稳定性和累积误差问题。需要一种既能保持高质量生成又能支持实时流式合成的统一框架。

Method: 1. 提出Motion-Language-Aligned Causal VAE (MAC-VAE)，将运动序列编码为时间因果的潜在表示；2. 在潜在表示上训练自回归扩散变换器，使用因果扩散强制进行时间有序的去噪；3. 引入具有因果不确定性的帧级采样调度，从部分去噪的前帧预测后续帧。

Result: 在HumanML3D和SnapMoGen数据集上的实验表明，CMDM在语义保真度和时间平滑性方面优于现有扩散和自回归模型，同时显著降低了推理延迟，支持交互速率的流式合成和长时运动生成。

Conclusion: CMDM提供了一个统一的因果运动扩散框架，解决了现有方法在时间因果性、实时应用性和稳定性方面的局限性，实现了高质量、低延迟的运动生成，为实时交互应用提供了有效解决方案。
...
TL;DR: 提出了一种名为Association DETR的新型目标检测模型，通过利用背景上下文信息来提升检测性能，在COCO val2017数据集上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有目标检测模型（包括YOLO系列和DETR系列）主要关注前景目标特征，而忽视了背景提供的上下文信息。作者认为背景信息对目标检测任务有显著帮助，例如汽车更可能出现在道路上而非办公室，野生动物更可能出现在森林而非繁忙街道。

Method: 提出Association DETR模型，该模型通过某种关联机制（具体未在摘要中详细说明）来利用背景上下文信息，弥补现有模型忽视背景信息的缺陷。

Result: 在COCO val2017数据集上，Association DETR相比其他目标检测模型取得了最先进（state-of-the-art）的结果。作为对比，YOLOv12达到55.2 mAP，RT-DETRv2达到53.4 mAP。

Conclusion: 背景上下文信息对目标检测任务至关重要，提出的Association DETR模型通过有效利用这种信息，在COCO数据集上实现了最先进的性能，证明了背景信息在目标检测中的价值。
...
TL;DR: SATtxt是一个面向地球观测的频谱感知视觉语言基础模型，仅需RGB输入即可进行推理，同时保留训练期间学习到的光谱信息，在零样本分类、检索和线性探测方面显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基础模型在地球观测应用中面临两个主要障碍：1）多光谱输入信息丰富但存在波段冗余和对齐问题，难以一致利用；2）CLIP式文本编码器语义表达能力有限，细粒度对齐效果不佳。同时，实际卫星系统常缺乏完整多光谱覆盖，需要仅使用RGB输入进行可扩展部署。

Method: SATtxt采用两阶段框架：1）光谱表示蒸馏：通过轻量级投影器将冻结多光谱教师模型的光谱先验知识转移到RGB学生模型；2）频谱基础对齐：使用指令增强的大型语言模型将蒸馏后的视觉空间与表达力更强的LLM嵌入空间进行对齐。

Result: 在EuroSAT、BigEarthNet和ForestNet数据集上，SATtxt相比基线方法平均提升零样本分类4.2%、检索5.9%、线性探测2.7%，展示了面向地球观测的频谱感知视觉语言学习的高效路径。

Conclusion: SATtxt通过光谱表示蒸馏和频谱基础对齐，实现了仅使用RGB输入就能保留光谱信息的视觉语言基础模型，为地球观测任务提供了可扩展且高效的解决方案。
...
TL;DR: 提出Coded-E2LF方法，使用编码孔径和静态事件相机获取4D光场，首次实现仅从事件数据重建像素级精度的光场
...
Method: 使用编码孔径和静态事件相机，通过理论分析阐明孔径编码模式中黑色图案的关键作用，改进从事件数据重建光场的算法

Result: 首次实现仅从事件数据重建像素级精度的4D光场，在实际成像硬件上验证了方法的有效性

Conclusion: Coded-E2LF方法成功实现了纯事件驱动的光场成像，为计算成像提供了新的硬件简化方案
...
<details>
  <summary>Details</summary>
Motivation: 现有无源域自适应目标检测方法主要关注伪标签阈值调整或教师-学生框架优化，忽视了跨域数据中的对象级结构线索，需要更有效的隐私敏感适应方案

Method: 提出CGSA框架，在DETR检测器中集成层次化槽感知模块进行渐进式图像解耦，并通过类引导槽对比模块将槽表示引导至类语义，保持语义一致性并促进域不变适应

Result: 在多个跨域数据集上的实验表明，CGSA优于先前无源域自适应目标检测方法，理论推导和实验分析验证了所提组件的有效性

Conclusion: 对象中心设计在隐私敏感适应场景中具有前景，CGSA框架为无源域自适应目标检测提供了新的有效解决方案
...
TL;DR: CRAG：将3D装配重新定义为装配与生成的联合问题，通过生成完整形状和预测零件姿态的相互增强，解决现有方法无法合成缺失几何的问题


<details>
  <summary>Details</summary>
Motivation: 现有3D装配方法主要关注姿态估计，通过刚性变换重新排列观察到的零件。然而人类装配自然地将结构推理与整体形状推断相结合。本文受此启发，将3D装配重新定义为装配与生成的联合问题，以解决现有方法无法合成缺失几何的局限性。

Method: 提出CRAG方法，同时生成合理的完整形状并预测输入零件的姿态。该方法利用装配过程提供零件级结构先验用于生成，而生成过程注入整体形状上下文以解决装配中的歧义，形成相互增强的循环。

Result: 在具有多样几何形状、不同零件数量和缺失零件的野外物体上进行了广泛实验，展示了最先进的性能。CRAG能够处理现有方法无法处理的缺失几何情况。

Conclusion: 3D装配应被视为装配与生成的联合问题，这两个过程相互增强。CRAG通过同时生成完整形状和预测零件姿态，在多样场景下实现了优越性能，为处理不完整输入提供了新思路。
...
TL;DR: 提出基于四焦点张量的相机同步新框架，通过Tucker分解恢复多相机位姿，挑战了四焦点张量不实用的传统观点


<details>
  <summary>Details</summary>
Motivation: 四焦点张量比双焦点张量（本质矩阵）包含更多信息，但传统上被认为不实用且仅有理论价值。本研究挑战这一观点，探索利用高阶信息进行相机同步的潜力
...
Conclusion: 四焦点张量在运动恢复结构中是实用的，通过提出的同步框架可以有效地恢复多个相机位姿，为利用高阶几何约束开辟了新途径
...
TL;DR: 提出频率比度量(FRM)和多模态权重分配模块(MWAM)，通过频域分析量化模态偏好并动态平衡各模态贡献，解决缺失模态导致的性能下降问题
...
Method: 首先引入频率比度量(FRM)在频域分析特征以量化模态偏好，然后提出多模态权重分配模块(MWAM)，这是一个即插即用组件，能在训练过程中动态重新平衡每个分支的贡献

Result: MWAM可无缝集成到CNN和ViT等多种架构骨干中，在各种任务和模态组合上带来一致的性能提升，不仅优化基础模型性能，还能进一步提升解决缺失模态问题的最先进方法的性能

Conclusion: 通过频域分析模态主导关系并动态平衡模态贡献，能有效解决多模态模型中的缺失模态问题，MWAM作为一种通用方法在不同架构和任务中均表现出色
...
TL;DR: Medical-SAM2 GUI是一个基于Napari的开源桌面应用，用于2D/3D医学图像半自动标注，通过SAM2风格传播实现稀疏提示下的掩码传播，提供统一的队列导向工作流。


<details>
  <summary>Details</summary>
Motivation: 医学图像算法开发需要体素级标注，但3D扫描的手动标注耗时昂贵，现有工具缺乏统一的队列导向工作流，无法在一个本地流程中集成导航、传播、交互校正和定量导出功能。

Method: 基于Napari多维度查看器构建，将3D体视为切片序列，集成框/点提示与SAM2风格传播。提供本地优先工作流，支持标准DICOM系列和NIfTI格式，支持顺序标注、框优先提示、点提示细化、提示优先校正等功能。

Result: 开发了开源桌面应用，支持多研究高效3D标注，提供统一工作流，支持每对象体积测量和3D体渲染，通过SimpleITK保持图像几何结构，包含可选N4偏置场校正。

Conclusion: Medical-SAM2 GUI为医学图像研究标注工作流提供了实用的本地解决方案，通过SAM2风格传播和统一工作流显著提高了3D标注效率，代码已在GitHub开源。
...
TL;DR: DPCache：一种基于动态规划的扩散模型训练免费加速框架，通过全局路径规划优化关键时间步选择，减少计算开销


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成中表现出色，但多步迭代采样的计算开销阻碍了实际部署。现有基于缓存的加速方法采用固定或局部自适应调度，未考虑去噪轨迹的全局结构，导致误差累积和视觉伪影。

Method: 提出DPCache框架，将扩散采样加速建模为全局路径规划问题。从小型校准集构建路径感知成本张量，量化跳过时间步的路径相关误差。利用动态规划选择最小化总路径成本的关键时间步序列，在推理时仅在关键时间步进行完整计算，中间输出使用缓存特征预测。

Result: 在DiT、FLUX和HunyuanVideo上的实验表明，DPCache在保持质量的同时实现显著加速：在FLUX上以4.87倍加速获得+0.031 ImageReward提升，以3.54倍加速甚至超越完整步长基线+0.028 ImageReward。

Conclusion: DPCache通过路径感知的全局调度框架，有效解决了扩散模型加速中的误差累积问题，实现了高质量的训练免费加速，优于现有加速方法。
...
TL;DR: 该论文提出了一个实用的音视频质量评估数据集构建方法，通过众包主观实验框架、系统化的数据准备策略和额外标注扩展，创建了YT-NTU-AVQ数据集——目前最大且最多样化的AVQA数据集。


<details>
  <summary>Details</summary>
Motivation: 现有音视频质量评估数据集存在规模小、内容多样性不足、仅标注总体分数等局限性，这限制了模型发展和多模态感知研究。需要更实用的大规模数据集构建方法。

Method: 设计了众包主观实验框架打破实验室环境限制；采用系统化数据准备策略确保质量和语义场景的广泛覆盖；扩展数据集包含额外标注以支持多模态感知机制研究。

Result: 成功构建了YT-NTU-AVQ数据集，包含1,620个用户生成的音视频序列，是目前最大且最多样化的AVQA数据集，数据集和平台代码已开源。

Conclusion: 提出的实用AVQA数据集构建方法有效解决了现有数据集的局限性，为模型发展和多模态感知研究提供了更好的支持，YT-NTU-AVQ数据集将成为该领域的重要资源。
...
TL;DR: ArtPro：基于自适应运动提案集成的自监督框架，用于重建复杂多部件铰接物体，解决了现有方法对初始部件分割高度敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于可微分渲染框架（如3D高斯泼溅）的自监督方法对初始部件分割高度敏感，依赖启发式聚类或预训练模型，导致优化容易陷入局部最小值，特别是在处理复杂多部件物体时。

Method: 提出ArtPro框架，采用自适应运动提案集成方法：1）基于几何特征和运动先验进行过分割初始化，生成具有合理运动假设的部件提案；2）优化过程中通过分析空间邻域间的运动一致性动态合并提案；3）碰撞感知的运动剪枝机制防止错误的运动学估计。

Result: 在合成和真实世界物体上的大量实验表明，ArtPro能够稳健地重建复杂多部件物体，在准确性和稳定性方面显著优于现有方法。

Conclusion: ArtPro通过自适应运动提案集成解决了铰接物体重建中对初始分割敏感的问题，为机器人操作和交互仿真等应用提供了高保真数字孪生重建的有效解决方案。
...
TL;DR: 该论文提出了一种用于室内场景的开放词汇3D占用预测方法，通过几何监督和语言嵌入高斯模型，解决了室内环境几何密集、布局复杂、语义细粒度的问题。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D占用方法主要针对室外驾驶场景，在室内场景中表现不佳。室内环境具有几何密集、布局复杂、语义细粒度等特点，需要新的方法来解决这些挑战。

Method: 采用仅几何监督范式（仅使用二元占用标签），基于3D语言嵌入高斯模型作为统一中间表示。几何方面提出不透明度感知的泊松基体积聚合方法；语义方面提出渐进温度衰减调度，在渲染过程中逐渐锐化不透明度以增强高斯-语言对齐。

Result: 在Occ-ScanNet数据集上，开放词汇设置下达到59.50 IoU和21.05 mIoU，在所有现有占用方法中IoU最高，并在mIoU上大幅超越先前开放词汇方法。

Conclusion: 该方法成功解决了室内开放词汇3D占用预测的挑战，通过几何监督和语言嵌入高斯的结合，在几何重建和语义理解方面都取得了显著提升。
...
TL;DR: ViCLIP-OT：针对越南语的图像-文本检索基础模型，结合CLIP对比学习和SIGROT损失，在低资源语言环境下显著提升跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型主要针对高资源语言优化，在越南语等低资源语言环境下表现欠佳，需要专门设计适用于低资源语言的跨模态检索模型。
...
Conclusion: SIGROT整合为低资源语言的跨模态检索提供了有效且可扩展的策略，对越南语及其他代表性不足语言环境的智能多媒体检索系统具有实际应用价值。
...
TL;DR: 提出SoPE方法，通过球坐标系位置编码改进3D大视觉语言模型的空间感知能力，解决传统RoPE在3D多模态理解中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D LVLMs继承的旋转位置编码(RoPE)在3D多模态理解中存在不足：1）传统RoPE公式无法保持3D标记的三维空间结构；2）相对距离计算忽略了角度依赖性，限制了模型捕捉视觉表示中方向变化的能力。
...
TL;DR: 提出基于扩散模型的超声图像去斑方法，使用MRI模拟超声图像进行监督训练，在保留解剖结构的同时抑制斑点噪声，优于传统滤波器和现有学习方法。


<details>
  <summary>Details</summary>
Motivation: 超声成像虽广泛用于实时无创诊断，但斑点噪声及相关伪影会降低图像质量并妨碍解读，需要有效的去斑方法来改善图像质量。

Method: 基于图像恢复随机微分方程框架构建扩散模型，通过Matlab超声工具箱从无斑点的MRI图像模拟超声图像来创建大型配对数据集进行监督训练。

Result: 在保留的模拟测试集上，该方法始终优于经典滤波器和近期基于学习的去斑基线方法；通过跨模型方差量化预测不确定性，显示高不确定性区域与高重建误差相关；评估对模拟探头设置的敏感性并观察到域偏移。

Conclusion: 提出的扩散模型能有效抑制斑点噪声并保留解剖边缘和对比度，预测不确定性可作为困难区域的实用指标，域偏移问题提示需要多样化训练和适应策略以实现稳健的临床部署。
...
Method: 提出正交子空间编辑方法，将模型的隐藏状态分解为三个正交子空间：视觉证据、冲突先验和残差不确定性。通过选择性抑制幻觉模式而不干扰视觉基础，该方法在数学上保证对先验子空间的编辑完全不影响视觉成分。

Result: 在POPE和CHAIR等多个基准测试中实现了最先进的幻觉减少效果，同时在不同架构上保持MME的一般能力，并维持高效推理。该方法在对比解码和静态子空间编辑基线中表现一致优越。

Conclusion: HulluEdit为大视觉语言模型提供了更可信赖的新途径，通过正交子空间编辑在单次前向传播中有效减少物体幻觉，同时保持模型的视觉基础能力和推理效率。
...
Method: 提出Alignment-Aware Masked Learning (AML)：1) 显式估计像素级视觉-语言对齐；2) 在优化过程中过滤对齐差的区域；3) 聚焦于可信的视觉-语言线索

Result: 在RefCOCO数据集上取得最先进的性能，同时增强了对多样化描述和场景的鲁棒性

Conclusion: AML通过显式建模和利用像素级对齐信息，有效提升了Referring Image Segmentation的性能和鲁棒性，为视觉-语言对齐学习提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成具有精确空间控制的人类运动时，通常需要任务特定训练或缓慢优化，且硬约束经常破坏运动自然性。许多动画任务可表述为线性逆问题，但现有方法难以同时满足精确约束和保持运动真实性。

Method: 提出ProjFlow训练免费采样器，核心创新是骨骼感知度量编码骨骼拓扑结构。该度量使采样器能通过在整个骨骼上相干分布修正来强制执行硬约束，避免朴素投影的不自然伪影。对于稀疏输入（如填充关键帧间长间隙），引入使用伪观测的时间变化公式，在采样过程中逐渐衰减。

Result: 在代表性应用（运动修复、2D到3D提升）上的广泛实验表明，ProjFlow实现精确约束满足，在零样本基线上匹配或改进真实感，同时与基于训练的控制器保持竞争力。

Conclusion: ProjFlow通过骨骼感知度量和时间变化伪观测，提供了一种无需训练、零样本精确满足线性空间约束的方法，在保持运动自然性的同时解决现有方法的局限性。
...
Method: 提出SPATIALALIGN自改进框架，采用零阶正则化直接偏好优化微调T2V模型；设计基于几何的DSR-SCORE度量标准量化视频与提示中动态空间关系的对齐度；构建包含多样化动态空间关系的文本-视频对数据集

Result: 微调后的模型在空间关系描绘方面显著优于基线模型；DSR-SCORE提供了比依赖视觉语言模型评估更精确的量化评估方法

Conclusion: SPATIALALIGN框架有效提升了文本到视频模型对动态空间关系的理解和生成能力，为空间约束对齐提供了系统化的解决方案
...
TL;DR: 提出统一隐藏码恢复框架，实现篡改图像的检索与恢复，兼容后处理和生成中水印范式，构建ImageNet-S基准进行系统评估


<details>
  <summary>Details</summary>
Motivation: 当前图像真实性研究主要关注深度伪造检测和定位，而针对篡改内容的事实检索恢复相对未充分探索，需要建立超越检测和定位的通用图像恢复基础

Method: 通过多尺度向量量化将语义和感知信息编码为紧凑隐藏码表示，利用条件Transformer模块增强上下文推理，构建统一隐藏码恢复框架，兼容多种水印管道

Result: 在ImageNet-S基准上的广泛实验表明，该方法展现出有前景的检索和重建性能，同时保持与多样化水印管道的完全兼容性

Conclusion: 该框架为超越检测和定位的通用图像恢复奠定了基础，实现了篡改内容的检索与恢复，填补了图像真实性研究中的空白
...
TL;DR: TrajTok是一种端到端视频分词器模块，通过统一的时空分割器直接生成对象轨迹，动态调整分词粒度以适应语义复杂度，显著提升视频理解效率和性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频模型通过patchification进行分词会产生过多冗余token，严重限制视频处理效率和可扩展性。现有基于轨迹的分词器虽然解耦了视频时长与token数量，但依赖复杂的外部分割和跟踪流程，速度慢且任务无关。

Method: 提出TrajTok端到端视频分词器模块，包含统一的时空分割器，通过隐式聚类直接在单次前向传播中生成对象轨迹。该模块与视频模型完全集成并针对下游目标进行协同训练，动态调整分词粒度以适应语义复杂度。
...
Conclusion: TrajTok通过端到端、任务感知的视频分词方法，在保持高效率的同时显著提升视频理解性能，为视频模型提供了一种通用且高效的组件，特别适用于长视频推理任务。
...
TL;DR: SceneTransporter：通过最优传输约束实现从单图像生成结构化3D场景的端到端框架，解决了现有方法在开放世界场景中无法将部件组织成独立实例的问题。
...
Method: 将结构化3D场景生成重新定义为全局相关性分配问题。在组合式DiT模型的去噪循环中，制定并求解熵最优传输目标，该目标施加两个结构约束：1) 传输计划通过门控交叉注意力强制图像块与部件级3D潜在表示之间的一对一独占路由；2) 传输的竞争性结合基于边缘的成本正则化，鼓励相似块分组形成连贯对象。

Result: 在开放世界场景生成任务上，SceneTransporter显著优于现有方法，在实例级连贯性和几何保真度方面都有显著提升。

Conclusion: 通过将结构化3D场景生成重新定义为全局相关性分配问题，并引入最优传输约束，SceneTransporter成功解决了现有方法在实例组织方面的不足，为开放世界场景生成提供了有效的解决方案。
...
Result: 在ATSyn-static数据集上达到PSNR 27.67 dB和SSIM 0.8735，相比SOTA方法PSNR提升1.3 dB（4.5%），SSIM提升0.048（5.8%）；在真实数据集TSRWGAN Real-World和CLEAR上也优于现有方法。

Conclusion: 光流引导的倾斜校正与高斯泼溅结合能有效增强合成和真实湍流条件下的图像恢复，证明了该方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑高度依赖精心设计的指令，将任务分解和顺序规划的负担完全放在用户身上，需要实现更自主的图像编辑系统

Method: 将自主图像编辑建模为长时程决策问题，通过理解用户美学意图、基于树搜索规划多步编辑动作、利用记忆和视觉反馈进行闭环迭代执行

Result: 相比基线方法，PhotoAgent在指令遵循和视觉质量方面均取得一致改进，并构建了包含7,000张照片的美学评估基准UGC-Edit和包含1,017张照片的测试集

Conclusion: PhotoAgent通过显式美学规划和闭环执行实现了自主图像编辑，显著提升了编辑质量和用户体验，为实际应用场景提供了可靠的评估框架
...
TL;DR: FaceTT是一个基于扩散模型的框架，通过面部属性感知提示细化、免调谐角度反演和自适应注意力控制，实现高保真、身份一致的人脸年龄变换。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸年龄变换方法存在三个主要问题：1）依赖数值年龄表示而忽略生物和环境线索的相互作用；2）在宽年龄变换中难以保持身份一致性；3）静态注意力和优化密集的反演限制了适应性、细粒度控制和背景一致性。

Method: 提出FaceTT框架，包含三个核心技术：1）面部属性感知提示细化策略，编码内在（生物）和外在（环境）老化线索进行上下文感知条件化；2）免调谐角度反演方法，高效将真实人脸映射到扩散潜在空间以实现快速准确重建；3）自适应注意力控制机制，动态平衡用于语义老化线索的交叉注意力和用于结构及身份保持的自注意力。

Result: 在基准数据集和野外测试集上的广泛实验表明，FaceTT在身份保持、背景保留和老化真实性方面优于最先进方法。

Conclusion: FaceTT通过创新的提示细化、高效反演和动态注意力控制，成功解决了现有年龄变换方法的局限性，实现了高保真、身份一致的人脸年龄变换。
...
### [94] [CMSA-Net: Causal Multi-scale Aggregation with Adaptive Multi-source Reference for Video Polyp Segmentation](https://arxiv.org/abs/2602.22821)
*Tong Wang,Yaolei Qi,Siwen Wang,Imran Razzak,Guanyu Yang,Yutong Xie*

Main category: cs.CV

TL;DR: CMSA-Net：一种用于视频息肉分割的鲁棒框架，通过因果多尺度聚合模块和动态多源参考策略解决息肉与周围黏膜相似性及跨帧位置尺度变化问题，在SUN-SEG数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频息肉分割（VPS）在计算机辅助结肠镜检查中至关重要，但面临两大挑战：1）息肉与周围黏膜外观相似导致语义区分度弱；2）跨视频帧中息肉位置和尺度的大幅变化使得稳定准确分割困难。

Method: 提出CMSA-Net框架，包含两个核心组件：1）因果多尺度聚合（CMA）模块，使用因果注意力从多个历史帧的不同尺度有效收集语义信息，确保时间特征传播遵循严格时间顺序；2）动态多源参考（DMR）策略，基于语义可分性和预测置信度自适应选择信息丰富且可靠的参考帧，提供强大多帧指导同时保持实时推理效率。

Result: 在SUN-SEG数据集上的大量实验表明，CMSA-Net实现了最先进的性能，在分割准确性和实时临床适用性之间取得了良好平衡。

Conclusion: CMSA-Net通过因果多尺度特征聚合和自适应参考帧选择，有效解决了视频息肉分割中的语义模糊性和时空变化问题，为实时临床应用提供了准确可靠的分割解决方案。
...
### [95] [Reflectance Multispectral Imaging for Soil Composition Estimation and USDA Texture Classification](https://arxiv.org/abs/2602.22829)
...
TL;DR: 提出一种基于多光谱成像和机器学习的经济高效、可现场部署的土壤质地检测系统，实现土壤成分预测和USDA质地分类，准确率超过99%。


<details>
  <summary>Details</summary>
Motivation: 传统土壤质地检测方法（实验室粒度分析）耗时耗力，现有传感技术要么成本高昂，要么分辨率不足，难以支持常规田间尺度部署。需要开发一种准确、非破坏性、可现场部署的土壤质地表征方法，以满足岩土工程筛选和精准农业需求。

Method: 设计经济高效的自制多光谱成像设备（365-940 nm，13个光谱波段），捕捉土壤质地光谱特性。建立机器学习框架：回归模型预测粘土、粉砂和砂土百分比；直接分类器预测12个USDA质地类别；间接分类通过USDA土壤质地三角图将回归成分映射到质地类别。使用混合数据（按不同比例混合粘土、粉砂和砂土）进行评估。

Result: 成分预测的决定系数R²高达0.99，质地分类准确率超过99%。表明多光谱成像与数据驱动建模相结合，能够提供准确、非破坏性、可现场部署的土壤质地表征。

Conclusion: 该方法为岩土工程筛选和精准农业提供了一种准确、非破坏性、可现场部署的土壤质地表征方案，解决了传统方法耗时耗力、现有传感技术成本高或分辨率不足的问题。
...
### [96] [A data- and compute-efficient chest X-ray foundation model beyond aggressive scaling](https://arxiv.org/abs/2602.22843)
...
TL;DR: CheXficient是一种胸部X光基础模型，通过主动数据筛选仅使用22.7%的数据和27.3%的计算资源，实现了与全数据模型相当或更优的性能，解决了大规模医学数据冗余和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像基础模型遵循"不惜一切代价扩大规模"的范式，面临两个关键挑战：大规模医学数据集存在大量冗余和严重类别不平衡，导致表示学习偏向过表达模式；不考虑数据质量异质性的盲目训练导致计算效率低下。

Method: 提出CheXficient胸部X光基础模型，采用主动、原则性的数据筛选策略，在预训练过程中选择性优先处理信息丰富的训练样本。模型仅使用1,235,004对CXR图像和报告中22.7%的数据，消耗不到27.3%的总计算预算。

Result: CheXficient在20个基准测试中（涵盖5种任务类型）表现出与全数据模型相当或更优的性能。这些任务包括：零样本发现分类、跨模态检索、疾病预测、语义分割和放射学报告生成。模型系统性地优先处理代表性不足的训练样本，提高了对长尾或罕见病症的泛化能力。

Conclusion: 主动数据筛选可以作为暴力扩大数据集的有效、经济替代方案。这项工作为医学视觉-语言基础模型的高效预训练和下游适应提供了关于数据和计算需求的实用见解。
...
### [97] [SO3UFormer: Learning Intrinsic Spherical Features for Rotation-Robust Panoramic Segmentation](https://arxiv.org/abs/2602.22867)
*Qinfeng Zhu,Yunxi Jiang,Lei Fan*

Main category: cs.CV

TL;DR: SO3UFormer：一种旋转鲁棒的全景语义分割架构，通过解耦重力向量依赖、引入正交一致的球面注意力机制和规范感知的相对位置编码，解决了现有方法在相机姿态变化下的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有全景语义分割模型严重依赖重力对齐假设，但在实际应用中（如手持设备抖动、无人机姿态变化）相机姿态会发生3D旋转，导致标准球面Transformer过度拟合全局纬度线索，在重定向时性能崩溃。

Method: 1. 内在特征表示：去除绝对纬度编码，解耦特征与重力向量依赖；2. 正交一致球面注意力：考虑非均匀采样密度；3. 规范感知相对位置机制：使用切平面投影角度和离散规范池化编码局部角度几何；4. 索引球面重采样和SO(3)一致性正则化训练。

Result: 在Pose35数据集（Stanford2D3D的±35°随机旋转变体）上达到72.03 mIoU，在任意完整SO(3)旋转下仍保持70.67 mIoU，而基线SphereUFormer从67.53 mIoU暴跌至25.26 mIoU，显示显著稳定性优势。

Conclusion: SO3UFormer通过几何驱动的架构设计实现了对3D旋转的鲁棒性，解决了全景分割中重力对齐假设的局限性，为实际非结构化环境中的视觉感知提供了可靠解决方案。
...
### [98] [Decomposing Private Image Generation via Coarse-to-Fine Wavelet Modeling](https://arxiv.org/abs/2602.23262)
*Jasmine Bayrooti,Weiwei Kong,Natalia Ponomareva,Carlos Esteves,Ameesh Makadia,Amanda Prorok*

Main category: cs.CV

TL;DR: 提出基于小波变换的谱域差分隐私图像生成框架，通过将隐私预算集中在低分辨率小波系数（低频分量）来保护敏感信息，同时利用公开预训练的超分辨率模型恢复高频细节，实现隐私保护与图像质量的更好权衡。


<details>
  <summary>Details</summary>
Motivation: 传统差分隐私微调方法（如DP-SGD）在所有模型参数上均匀添加噪声，导致图像质量严重下降，特别是高频纹理细节的损失。作者假设图像中最隐私敏感的部分通常是低频分量（如面部特征和物体形状），而高频分量大多是通用的公开信息。
...
Result: 在MS-COCO和MM-CelebA-HQ数据集上的实验表明，该方法相比其他领先的差分隐私图像生成框架，能够生成质量更高、风格捕捉更好的图像，实现了隐私保护与实用性的更好权衡。

Conclusion: 通过将隐私保护集中在低频分量（隐私敏感信息）而保留高频分量（通用信息）的策略，提出的谱域差分隐私框架在图像生成任务中实现了比传统方法更好的隐私-效用权衡，为隐私保护图像生成提供了新思路。
...
### [99] [Chain of Flow: A Foundational Generative Framework for ECG-to-4D Cardiac Digital Twins](https://arxiv.org/abs/2602.22919)
*Haofan Wu,Nay Aung,Theodoros N. Arvanitis,Joao A. C. Lima,Steffen E. Petersen,Le Zhang*

Main category: cs.CV

TL;DR: Chain of Flow (COF) 是一个基于心电图的生成式框架，能够从单个心动周期重建完整的4D心脏结构和运动，将心脏数字孪生从特定任务预测模型转变为患者特异性、可操作的虚拟心脏。


<details>
  <summary>Details</summary>
Motivation: 现有心脏数字孪生（CDT）框架局限于特定任务预测器，而非构建患者特异性、可操作的虚拟心脏。临床可操作的CDT需要重建个体化心脏解剖和生理结构，从多模态信号更新内部状态，并支持广泛的下游模拟任务。

Method: COF是一个基于心电图的生成式框架，在训练过程中整合电影磁共振成像（cine-CMR）和12导联心电图，学习心脏几何结构、电生理和运动动力学的统一表示，从单个心动周期重建完整的4D心脏结构和运动。

Result: 在多个队列上评估显示，COF能够准确恢复心脏解剖结构、各心腔功能和动态运动模式。重建的4D心脏进一步支持下游CDT任务，包括容积测量、区域功能分析和虚拟电影合成。

Conclusion: COF通过直接从心电图实现完整的4D器官重建，将心脏数字孪生从狭窄的预测模型转变为完全生成式、患者特异性的虚拟心脏，为临床可操作的CDT提供了基础框架。
...
### [100] [OSDaR-AR: Enhancing Railway Perception Datasets via Multi-modal Augmented Reality](https://arxiv.org/abs/2602.22920)
...
<details>
  <summary>Details</summary>
Motivation: 铁路安全关键任务（如障碍物检测）面临高质量标注数据稀缺的挑战。现有方法存在局限性：逼真模拟器存在"模拟到现实"差距，而简单图像掩码技术缺乏时空一致性，无法生成具有正确外观和尺寸的增强单帧和多帧场景。

Method: 提出多模态增强现实框架，利用Unreal Engine 5功能，基于LiDAR点云和INS/GNSS数据将逼真虚拟物体集成到真实铁路序列中。同时提出基于分割的INS/GNSS数据细化策略，确保物体放置的准确性和时间稳定性。

Result: 开发了OSDaR-AR公开数据集，包含精心设计的增强序列，支持下一代铁路感知系统的开发。通过比较研究证实了所提方法能显著提高增强序列的真实感。

Conclusion: 该框架有效弥合了铁路感知任务中的"模拟到现实"差距，通过多模态数据融合和细化策略生成了高质量的增强现实序列，为铁路安全关键应用提供了有价值的训练数据资源。
...
### [101] [WaterVideoQA: ASV-Centric Perception and Rule-Compliant Reasoning via Multi-Modal Agents](https://arxiv.org/abs/2602.22923)
...
TL;DR: 提出了首个面向全水域环境的大规模视频问答基准WaterVideoQA和创新的多智能体神经符号系统NaviMind，旨在解决自主水面舰艇从被动感知到知识驱动交互认知的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 自主导航在被动感知方面取得显著进展，但在知识驱动的交互环境认知方面存在根本性限制。在海上导航这一高风险领域，弥合原始视觉感知与复杂认知推理之间的差距不仅是增强功能，更是自主水面舰艇执行安全精确操作的关键前提。

Method: 1) 构建WaterVideoQA基准：包含6类水域的3029个视频片段，融入多变光照和动态天气等多维度变量，通过五层分级认知框架严格测试ASV能力；2) 提出NaviMind系统：创新的多智能体神经符号系统，结合自适应语义路由、情境感知分层推理和自主反思验证，实现从表面模式匹配到符合法规的可解释决策。

Result: 实验结果表明，该框架显著超越了现有基线，为动态海洋环境中的智能可信交互建立了新范式。

Conclusion: 该研究填补了自主水面舰艇从被动感知到知识驱动交互认知的空白，通过WaterVideoQA基准和NaviMind系统，为动态海洋环境中的智能可信交互建立了新的技术范式，推动了自主导航向更高层次的认知能力发展。
...
### [102] [MSJoE: Jointly Evolving MLLM and Sampler for Efficient Long-Form Video Understanding](https://arxiv.org/abs/2602.22932)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在理解长视频方面面临效率挑战，因为处理所有视频帧计算成本过高。作者认为对于每个问题，只有少量关键帧真正具有信息价值，因此需要智能选择关键帧而非处理全部内容。

Method: 1. 首先推理出多个描述问题相关视觉视角的查询；2. 这些查询与冻结的CLIP模型交互生成查询-帧相似度矩阵；3. 轻量级采样器从该矩阵预测关键帧采样权重，选择紧凑的信息帧集；4. 将选中的帧输入MLLM生成答案；5. 通过强化学习联合优化MLLM和采样器。

Result: 在VideoMME、LongVideoBench、LVBench和MLVU等基准测试中，MSJoE相比基础MLLM实现了8.0%的准确率提升，比最强基线方法高1.1%。同时构建了包含2.8K视频和7K问答对的新长视频QA数据集支持训练。

Conclusion: MSJoE通过联合进化MLLM和关键帧采样器，实现了高效的长视频理解。该方法证明了智能帧选择的重要性，并通过强化学习实现了查询推理、帧采样和视频理解的协同适应，为长视频分析提供了有效解决方案。
...
### [103] [pMoE: Prompting Diverse Experts Together Wins More in Visual Adaptation](https://arxiv.org/abs/2602.22938)
...
<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法通常只利用单个预训练模型的知识，忽略了整合不同领域专家知识可能产生的协同效应，限制了模型的适应能力和应用范围

Method: 提出pMoE（Mixture-of-Experts提示调优方法），包含专家专用提示令牌和动态令牌调度机制，在多个提示层优化各领域专家的贡献，有效整合多样化专家知识

Result: 在47个适应任务（包括通用和医疗领域的分类与分割）上进行广泛实验，pMoE不仅以较大优势实现卓越性能，还在计算效率与适应效果之间提供了最佳权衡

Conclusion: pMoE通过整合多样化专家知识，显著增强了模型的通用性和适用性，为参数高效微调提供了更有效的解决方案
...
### [104] [Velocity and stroke rate reconstruction of canoe sprint team boats based on panned and zoomed video recordings](https://arxiv.org/abs/2602.22941)
*Julian Ziegler,Daniel Matthes,Finn Gerdts,Patrick Frenzel,Torsten Warnke,Matthias Englert,Tina Koevari,Mirco Fuchs*

Main category: cs.CV

TL;DR: 提出基于视频的皮划艇冲刺运动自动分析框架，通过计算机视觉技术从平移缩放视频中重建性能指标，无需GPS或船上传感器


<details>
  <summary>Details</summary>
Motivation: GPS是皮划艇冲刺运动分析的黄金标准，但可用性有限，需要开发自动化的视频解决方案来为教练提供准确反馈

Method: 使用YOLOv8检测浮标和运动员，利用已知浮标网格估计单应性；通过U-net船头校准学习船特定的运动员偏移；采用光流进行鲁棒跟踪；从姿态估计或边界框提取划桨频率信息

Result: 与精英比赛GPS数据对比，速度相对均方根误差为0.020±0.011（相关系数0.956），划桨频率相对均方根误差为0.022±0.024（相关系数0.932）

Conclusion: 该方法为教练提供了高度准确的自动化反馈，无需船上传感器或手动标注，适用于所有冲刺项目和距离
...
### [105] [ToProVAR: Efficient Visual Autoregressive Modeling via Tri-Dimensional Entropy-Aware Semantic Analysis and Sparsity Optimization](https://arxiv.org/abs/2602.22948)
*Jiayu Chen,Ruoyu Lin,Zihao Zheng,Jingxin Li,Maoliang Li,Guojie Luo,Xiang chen*

Main category: cs.CV

TL;DR: 本文提出了一种基于注意力熵的VAR模型优化框架，通过分析token粒度、语义范围和生成尺度下的参数动态，识别token、层和尺度三个维度的稀疏模式，实现高效加速而保持语义保真度。


<details>
  <summary>Details</summary>
Motivation: VAR模型在生成质量上有所提升，但在后期阶段面临严重的效率瓶颈。现有方法如FastVAR和SkipVAR依赖启发式跳过策略，缺乏对模型架构不同维度语义投影的系统分析，无法精确优化参数动态。

Method: 提出基于注意力熵的优化框架，通过注意力熵表征模型架构不同维度的语义投影，精确识别token粒度、语义范围和生成尺度变化下的参数动态。在此基础上，发现token、层和尺度三个维度的稀疏模式，并针对这些模式提出细粒度优化策略。

Result: 在Infinity-2B和Infinity-8B模型上的实验表明，ToProVAR方法实现了高达3.4倍的加速，同时保持最小质量损失，在效率和生成质量上均优于传统方法，有效解决了先前工作中的问题。

Conclusion: 提出的基于注意力熵的VAR模型优化框架通过系统分析参数动态和稀疏模式，实现了高效的生成加速同时保持语义保真度，为VAR模型的实际部署提供了有效的解决方案。
...
### [106] [OpenFS: Multi-Hand-Capable Fingerspelling Recognition with Implicit Signing-Hand Detection and Frame-Wise Letter-Conditioned Synthesis](https://arxiv.org/abs/2602.22949)
*Junuk Cha,Jihyeon Kim,Han-Mu Park*

Main category: cs.CV

TL;DR: OpenFS：一种开源手语拼写识别与合成方法，通过多手能力识别器、签名手焦点损失和单调对齐损失解决手语拼写识别中的关键挑战，并引入帧级字母条件生成器处理OOV问题。


<details>
  <summary>Details</summary>
Motivation: 手语拼写识别在聋人与听人社区沟通中至关重要，但面临签名手歧义、缺乏适当训练损失和词汇外问题。现有方法依赖显式签名手检测（易导致识别失败）和CTC损失（存在峰值行为问题），需要更鲁棒的解决方案。

Method: 1) 多手能力手语拼写识别器：支持单/多手输入，通过双级位置编码和签名手焦点损失实现隐式签名手检测；2) 单调对齐损失：通过交叉注意力正则化强制输出字母序列遵循输入姿势序列的时间顺序；3) 帧级字母条件生成器：合成真实手语拼写姿势序列用于OOV词汇，构建FSNeo合成基准。

Result: 实验表明该方法在手语拼写识别方面达到最先进性能，验证了所提识别器和生成器的有效性。代码和数据已开源。

Conclusion: OpenFS通过隐式签名手检测、改进的损失函数和OOV词汇合成能力，为手语拼写识别提供了全面解决方案，显著提升了识别性能并解决了现有方法的局限性。
...
### [107] [MM-NeuroOnco: A Multimodal Benchmark and Instruction Dataset for MRI-Based Brain Tumor Diagnosis](https://arxiv.org/abs/2602.22955)
*Feng Guo,Jiaxiang Liu,Yang Li,Qianqian Shi,Mingkun Xu*

Main category: cs.CV

TL;DR: MM-NeuroOnco是一个大规模多模态脑肿瘤MRI理解基准与指令调优数据集，包含24,726个MRI切片和约200,000条语义丰富的多模态指令，用于提升脑肿瘤诊断的临床可解释性推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有公共数据集在注释丰富性和诊断语义方面存在局限，无法满足准确脑肿瘤诊断所需的临床可解释性推理需求。需要构建一个能够支持多模态脑肿瘤MRI理解的大规模基准数据集。

Method: 1) 构建MM-NeuroOnco数据集：整合20个数据源的24,726个MRI切片，生成约200,000条语义丰富的多模态指令；2) 开发多模型协作管道：用于自动化医学信息补全和质量控制，生成超越掩码注释的诊断相关语义；3) 构建MM-NeuroOnco-Bench评估基准：采用拒绝感知设置的手动注释评估基准，减少封闭式问题的固有偏差。

Result: 1) 在10个代表性模型评估中，最强基线Gemini 3 Flash在诊断相关问题上的准确率仅为41.88%；2) 基于MM-NeuroOnco微调的NeuroOnco-GPT在诊断问题上实现了27%的绝对准确率提升；3) 数据集和基准有效促进了基于临床的多模态诊断推理能力提升。

Conclusion: MM-NeuroOnco数据集和基准显著提升了多模态脑肿瘤诊断理解能力，NeuroOnco-GPT的优异表现证明了该资源在推进临床可解释性诊断推理方面的有效性。该工作为脑肿瘤MRI理解领域提供了重要的数据资源和评估框架。
...
### [108] [UCM: Unifying Camera Control and Memory with Time-aware Positional Encoding Warping for World Models](https://arxiv.org/abs/2602.22960)
*Tianxing Xu,Zixuan Wang,Guangyuan Wang,Li Hu,Zhongyi Zhang,Peng Zhang,Bang Zhang,Song-Hai Zhang*

Main category: cs.CV

TL;DR: UCM框架通过时间感知位置编码扭曲机制统一长期记忆和精确相机控制，使用双流扩散Transformer实现高效高保真生成，在长期场景一致性和相机可控性方面优于现有方法。
...
Method: 提出UCM框架：1) 时间感知位置编码扭曲机制统一长期记忆和相机控制；2) 高效双流扩散Transformer减少计算开销；3) 基于点云渲染的可扩展数据策展策略，在50万+单目视频上训练；4) 模拟场景重访以提升长期一致性。
...
### [109] [DMAligner: Enhancing Image Alignment via Diffusion Model Based View Synthesis](https://arxiv.org/abs/2602.23022)
*Xinglong Luo,Ao Luo,Zhengning Wang,Yueqi Yang,Chaoyu Feng,Lei Lei,Bing Zeng,Shuaicheng Liu*

Main category: cs.CV

TL;DR: DMAligner：基于扩散模型的图像对齐框架，通过面向对齐的视图合成解决传统光流方法的遮挡和光照变化问题


<details>
  <summary>Details</summary>
Motivation: 现有图像对齐方法主要采用基于光流的图像扭曲技术，但这种方法容易受到遮挡和光照变化等常见挑战的影响，导致对齐视觉质量下降，并影响下游任务的准确性。需要从新视角解决图像对齐中的挑战。

Method: 提出DMAligner框架，采用基于生成的方法进行图像对齐。核心包括：1）动态感知扩散训练方法，学习条件图像生成以合成对齐所需的新视图；2）动态感知掩码生成模块，自适应区分动态前景区域和静态背景，使扩散模型能更有效处理传统方法难以解决的挑战；3）使用Blender创建动态场景图像对齐数据集，包含1033个室内外场景和超过30K图像对。

Result: 在DSIA基准测试以及一系列广泛使用的视频数据集上进行定性比较，实验结果表明该方法具有优越性。代码已开源。

Conclusion: DMAligner通过基于生成的解决方案为图像对齐提供了新视角，展示了强大的能力，避免了基于光流的图像扭曲相关问题，在动态场景处理方面表现出色。
...
### [110] [WISER: Wider Search, Deeper Thinking, and Adaptive Fusion for Training-Free Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2602.23029)
...
Result: 在多个基准测试中显著优于先前方法：在CIRCO上相对提升45%（mAP@5），在CIRR上相对提升57%（Recall@1）。甚至超越了许多需要训练的方法，显示出优越的泛化能力。

Conclusion: WISER通过统一文本到图像和图像到图像检索范式，结合意图感知和不确定性感知，实现了无需训练的高效组合图像检索。该方法在多样场景下表现出优越性能和泛化能力，为多模态检索提供了新思路。
...
### [111] [PackUV: Packed Gaussian UV Maps for 4D Volumetric Video](https://arxiv.org/abs/2602.23040)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的方法在长序列、时间不一致性、大运动和遮挡处理方面存在不足，且输出格式与传统视频编码管道不兼容，限制了实际应用。

Method: 提出了PackUV-GS方法：1) 将高斯参数直接优化到UV域；2) 流引导的高斯标记和视频关键帧模块识别动态高斯、稳定静态区域；3) 生成与标准视频编解码器兼容的UV图集格式。

Result: 方法在渲染保真度上超越现有基线，可扩展到30分钟序列并保持一致性质量。同时发布了PackUV-2B数据集，包含50+同步相机、100个序列和20亿帧。

Conclusion: PackUV首次实现了与标准视频编解码器兼容的统一体视频表示，无需质量损失，能够在现有多媒体基础设施中实现高效流式传输。
...
### [112] [D-FINE-seg: Object Detection and Instance Segmentation Framework with multi-backend deployment](https://arxiv.org/abs/2602.23043)
*Argo Saakyan,Dmitry Solntsev*

Main category: cs.CV

TL;DR: D-FINE-seg：基于D-FINE的实时实例分割扩展，添加轻量级掩码头、分割感知训练和端到端优化推理框架，在TACO数据集上超越YOLO26性能


<details>
  <summary>Details</summary>
Motivation: Transformer-based实时目标检测器（如D-FINE）在精度-延迟权衡上表现优异，但基于Transformer的实时实例分割方法仍较少见。需要开发既能保持实时性能又能提供准确实例分割的解决方案。

Method: 1. 在D-FINE架构基础上添加轻量级掩码头；2. 分割感知训练：包括框裁剪BCE损失和Dice掩码损失；3. 辅助掩码监督和去噪掩码监督；4. 适配的匈牙利匹配成本；5. 端到端训练、导出和优化推理管道，支持ONNX、TensorRT、OpenVINO。

Result: 在TACO数据集上，D-FINE-seg在统一的TensorRT FP16端到端基准测试协议下，F1分数优于Ultralytics YOLO26，同时保持有竞争力的延迟性能。

Conclusion: D-FINE-seg成功将D-FINE扩展到实例分割任务，实现了实时性能与分割精度的良好平衡。开源框架为研究社区提供了完整的训练-导出-推理工具链，促进了实时实例分割技术的发展。
...
### [113] [GeoWorld: Geometric World Models](https://arxiv.org/abs/2602.23058)
*Zeyu Zhang,Danning Li,Ian Reid,Richard Hartley*

Main category: cs.CV

TL;DR: GeoWorld提出了一种几何世界模型，通过双曲JEPA将潜在表示从欧几里得空间映射到双曲流形，保留几何结构和层次关系，结合几何强化学习实现双曲潜在空间中的稳定多步规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于能量的预测世界模型存在两个主要问题：(1) 潜在表示通常在欧几里得空间中学习，忽略了状态间的几何和层次结构；(2) 难以进行长时程预测，导致在扩展推演中性能快速下降。

Method: 提出GeoWorld几何世界模型，采用双曲JEPA将潜在表示从欧几里得空间映射到双曲流形，保留几何结构和层次关系。引入几何强化学习进行基于能量的优化，实现双曲潜在空间中的稳定多步规划。
...
### [114] [Cytoarchitecture in Words: Weakly Supervised Vision-Language Modeling for Human Brain Microscopy](https://arxiv.org/abs/2602.23088)
*Matthew Sutton,Katrin Amunts,Timo Dickscheid,Christian Schiffer*

Main category: cs.CV

TL;DR: 提出一种标签介导的方法，通过标签而非配对的图像-文本数据，将生物医学视觉基础模型与语言模型连接，生成显微图像的自然语言描述


<details>
  <summary>Details</summary>
Motivation: 在研究和临床环境中，配对的图像-文本数据稀缺且难以获取，限制了视觉-语言耦合的自然语言接口在显微图像分析中的应用

Method: 使用标签作为中介，从相关文献自动挖掘区域描述作为合成标注，通过图像到文本的训练目标将现有的细胞构筑视觉基础模型(CytoNet)与大型语言模型耦合

Result: 在57个脑区产生合理的区域级描述，支持开放集使用；对范围内图像块匹配参考标签准确率达90.6%；在标签掩蔽情况下，描述仍具有足够区分度，在8路测试中恢复区域准确率达68.6%
...
### [115] [SpectralMamba-UNet: Frequency-Disentangled State Space Modeling for Texture-Structure Consistent Medical Image Segmentation](https://arxiv.org/abs/2602.23103)
*Fuhao Zhang,Lei Liu,Jialin Zhang,Ya-Nan Zhang,Nan Mu*

Main category: cs.CV

TL;DR: SpectralMamba-UNet：一种在频域解耦结构和纹理信息的新型医学图像分割框架，通过频域Mamba建模全局上下文，同时保留高频边界细节。
...
Method: 提出频域解耦框架：1）谱分解与建模（SDM）模块使用离散余弦变换分解低频和高频特征，低频通过频域Mamba建模全局上下文，高频保留边界敏感细节；2）谱通道重加权（SCR）机制形成通道级频率感知注意力；3）谱引导融合（SGF）模块实现解码器中自适应多尺度融合。

Result: 在五个公开基准测试中，该方法在不同模态和分割目标上均表现出持续改进，验证了方法的有效性和泛化能力。

Conclusion: SpectralMamba-UNet通过在频域解耦结构和纹理信息，有效解决了现有状态空间模型在医学图像分割中的局限性，实现了全局上下文建模和局部细节保留的良好平衡。
...
### [116] [WARM-CAT: : Warm-Started Test-Time Comprehensive Knowledge Accumulation for Compositional Zero-Shot Learning](https://arxiv.org/abs/2602.23114)
...
<details>
  <summary>Details</summary>
Motivation: 现有CZSL方法在测试时面临标签空间分布偏移问题，因为测试集包含从属性和对象重新组合的未见组合，导致性能下降。需要克服这一挑战。

Method: 1) 从无监督数据中累积文本和视觉模态的全面知识来更新多模态原型；2) 设计自适应更新权重控制原型调整程度；3) 引入动态优先级队列存储高置信度图像获取历史图像的视觉原型；4) 通过多模态协同表示学习对齐文本和视觉原型；5) 创建新基准数据集C-Fashion并优化MIT-States数据集。

Result: 在四个基准数据集上，在闭世界和开放世界设置下均实现了最先进的性能。提供了源代码和数据集。

Conclusion: 提出的WARM-CAT方法通过累积多模态知识、自适应原型更新和动态优先级队列，有效解决了CZSL中的分布偏移问题，在多个基准上取得了优异性能。
...
### [117] [FLIGHT: Fibonacci Lattice-based Inference for Geometric Heading in real-Time](https://arxiv.org/abs/2602.23115)
...
<details>
  <summary>Details</summary>
Motivation: 现有相机航向估计方法在低噪声、低异常值条件下表现良好，但随着噪声和异常值增加，准确性下降或计算成本升高。需要一种在复杂条件下仍能保持准确性和效率的方法。

Method: 提出单位球面霍夫变换的泛化方法：1) 提取帧间对应点；2) 为每对对应点生成兼容方向的大圆；3) 使用斐波那契晶格离散化单位球面作为投票中心；4) 每个大圆为一系列方向投票，确保未受噪声或动态物体影响的特征一致投票给正确运动方向。

Result: 在三个数据集上的实验表明，该方法在准确性与效率的帕累托前沿上。SLAM实验显示，通过在相机位姿初始化时校正航向，该方法能减少RMSE。

Conclusion: 提出的单位球面霍夫变换方法能够有效估计相机航向，在噪声和异常值条件下保持高准确性和计算效率，并能改善SLAM系统的性能。
...
### [118] [Devling into Adversarial Transferability on Image Classification: Review, Benchmark, and Evaluation](https://arxiv.org/abs/2602.23117)
*Xiaosen Wang,Zhijin Ge,Bohan Liu,Zheng Fang,Fengfan Zhou,Ruixuan Zhang,Shaokang Wang,Yuyang Luo*

Main category: cs.CV

TL;DR: 该论文提出一个标准化框架来评估对抗性可迁移攻击，通过系统分类现有方法、建立评估基准、总结增强策略，解决当前评估标准不统一的问题。


<details>
  <summary>Details</summary>
Motivation: 对抗性可迁移性使得攻击者无需直接访问目标模型即可实施攻击，引发严重安全担忧。当前缺乏统一的评估框架和标准，导致对现有攻击方法的评估可能存在偏差，需要建立标准化评估体系。

Method: 1. 系统回顾数百篇相关文献，将基于迁移的攻击方法分为六大类别；2. 提出综合性评估框架作为攻击方法的基准测试标准；3. 总结增强对抗性可迁移性的常见策略；4. 指出可能导致不公平比较的普遍问题。

Result: 建立了对抗性可迁移攻击的标准化评估框架，系统分类了现有攻击方法，明确了增强可迁移性的策略，并识别了评估中的常见偏差问题，为后续研究提供了统一的评估基准。

Conclusion: 该研究填补了对抗性可迁移攻击评估标准化的空白，提出的框架和分类体系有助于更公平、系统地评估不同攻击方法，同时简要回顾了图像分类之外的迁移攻击应用。
...
### [119] [From Calibration to Refinement: Seeking Certainty via Probabilistic Evidence Propagation for Noisy-Label Person Re-Identification](https://arxiv.org/abs/2602.23133)
*Xin Yuan,Zhiyong Zhang,Xin Xu,Zheng Wang,Chia-Wen Lin*

Main category: cs.CV

TL;DR: CARE方法：一个两阶段框架，通过从校准到细化的概率证据传播来解决带噪标签和稀疏样本的人体重识别问题，克服了softmax平移不变性和传统小损失样本选择的局限性。


<details>
  <summary>Details</summary>
Motivation: 在无约束环境中，从带有噪声标签和稀疏身份样本的数据集中学习鲁棒的人体重识别仍然是一个关键挑战。现有方法主要依赖基于softmax输出的损失校正或样本选择策略，但这些方法存在两个关键限制：1) Softmax具有平移不变性，导致对噪声标签产生过度自信和不可靠的预测；2) 基于小损失标准的传统样本选择往往会丢弃对学习判别性特征至关重要的有价值困难正样本。

Method: 提出了CAlibration-to-REfinement (CARE)方法，一个两阶段框架。在校准阶段，提出概率证据校准(PEC)，通过向相似度函数注入自适应可学习参数来消除softmax平移不变性，并使用证据校准损失来减轻对错误标记样本的过度自信。在细化阶段，设计证据传播细化(EPR)，包含两个步骤：1) 提出复合角度边界(CAM)度量，在超球面空间中精确区分干净但难以学习的正样本与错误标记样本；2) 开发面向确定性的球面加权(COSW)，根据CAM动态分配样本重要性，确保干净样本驱动模型更新。

Result: 在Market1501、DukeMTMC-ReID和CUHK03数据集上，在随机噪声和模式噪声下的广泛实验结果表明，CARE取得了具有竞争力的性能。

Conclusion: CARE方法通过从校准到细化的概率证据传播，有效解决了带噪标签和稀疏样本的人体重识别问题，克服了现有方法的局限性，在多个基准数据集上表现出色。
...
### [120] [No Labels, No Look-Ahead: Unsupervised Online Video Stabilization with Classical Priors](https://arxiv.org/abs/2602.23141)
*Tao Liu,Gang Wan,Kan Ren,Shibo Wen*

Main category: cs.CV

TL;DR: 提出新的无监督在线视频稳定框架，采用经典三阶段流水线设计，无需配对数据集，在资源受限硬件上高效运行


<details>
  <summary>Details</summary>
Motivation: 解决端到端学习方法面临的三大挑战：数据有限、可控性差、资源受限硬件效率低；现有基准主要针对手持可见光视频，限制了在无人机夜间遥感等领域的应用

Method: 基于经典稳定化流水线的三阶段无监督框架，结合多线程缓冲机制，无需配对训练数据；引入新的多模态无人机航空视频数据集(UAV-Test)

Result: 在定量指标和视觉质量上持续优于最先进的在线稳定器，性能可与离线方法相媲美

Conclusion: 提出的无监督在线视频稳定框架有效解决了端到端学习的局限性，在资源受限硬件上实现高效稳定，并通过新数据集扩展了稳定化技术的应用领域
...
### [121] [Efficient Encoder-Free Fourier-based 3D Large Multimodal Model](https://arxiv.org/abs/2602.23153)
*Guofeng Mei,Wei Lin,Luigi Riz,Yujiao Wu,Yiming Wang,Fabio Poiesi*

Main category: cs.CV

TL;DR: Fase3D是首个无需视觉编码器的高效傅里叶基3D场景大模型，通过点云序列化和快速傅里叶变换实现无序3D数据的有效标记化


<details>
  <summary>Details</summary>
Motivation: 现有3D大模型依赖繁重的预训练视觉编码器提取几何特征，而2D大模型已开始摒弃编码器以提高效率和可扩展性。将这一范式扩展到3D面临挑战，因为点云具有无序性和大规模特性，需要解决如何设计无需繁琐编码器的高效3D数据标记化方法

Method: 提出三个关键创新：1) 通过结构化超点紧凑表示大场景；2) 空间填充曲线序列化后接快速傅里叶变换，实现高效全局上下文建模和图基标记合并；3) 傅里叶增强的LoRA适配器以可忽略成本向大语言模型注入全局频率感知交互

Result: Fase3D在性能上与基于编码器的3D大模型相当，同时在计算和参数方面显著更高效

Conclusion: Fase3D证明了无需繁重视觉编码器即可有效处理无序3D数据的可行性，为高效3D大模型提供了新范式
...
### [122] [DyaDiT: A Multi-Modal Diffusion Transformer for Socially Favorable Dyadic Gesture Generation](https://arxiv.org/abs/2602.23165)
*Yichen Peng,Jyun-Ting Song,Siyeol Jung,Ruofan Liu,Haiyang Liu,Xuangeng Chu,Ruicong Liu,Erwin Wu,Hideki Koike,Kris Kitani*

Main category: cs.CV

TL;DR: DyaDiT：基于多模态扩散Transformer的对话手势生成模型，能够从双人对话音频生成具有社交情境感知的逼真手势动作，考虑交互动态和社交上下文。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将单个音频流映射到单个说话者的动作，忽略了社交情境和对话双方之间的相互动态关系，难以实现自然、社交参与度高的数字人交互。

Method: 提出DyaDiT多模态扩散Transformer，从双人对话音频生成情境适当的人类动作。模型融合双方信息捕捉交互动态，使用动作词典编码动作先验，可选择性地利用对话伙伴的手势生成更具响应性的动作。在Seamless Interaction Dataset上训练。

Result: DyaDiT在标准动作生成指标上超越现有方法，用户定量研究显示用户强烈偏好该模型生成的动作，证明了其鲁棒性和社交友好的动作生成能力。

Conclusion: DyaDiT通过考虑社交情境和对话双方的相互动态，能够生成更自然、社交参与度高的对话手势，为数字人交互提供了更逼真的动作生成解决方案。
...
### [123] [Learning Continuous Wasserstein Barycenter Space for Generalized All-in-One Image Restoration](https://arxiv.org/abs/2602.23169)
*Xiaole Tang,Xiaoyi He,Jiayi Xu,Xiang Gu,Jian Sun*

Main category: cs.CV

TL;DR: BaryIR：一种基于Wasserstein重心对齐的多源退化特征表示学习框架，通过解耦退化无关共享内容和退化特定知识来提升图像恢复的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有的一体化图像恢复方法在面对分布外退化时泛化能力有限。作者观察到多源退化特征分布是由不同退化特定偏移从底层退化无关分布中诱导产生的，恢复这种共享分布对于跨退化泛化至关重要。
...
Conclusion: 通过解耦退化无关共享内容和退化特定知识，BaryIR缓解了对分布内退化的过拟合，实现了基于退化无关共享不变性的自适应恢复，显著提升了一体化图像恢复方法在真实场景中的泛化能力。
...
### [124] [Latent Gaussian Splatting for 4D Panoptic Occupancy Tracking](https://arxiv.org/abs/2602.23172)
*Maximilian Luz,Rohit Mohan,Thomas Nürnberg,Yakov Miron,Daniele Cattaneo,Abhinav Valada*

Main category: cs.CV

TL;DR: LaGS提出了一种用于4D全景占据跟踪的新方法，通过潜在高斯溅射将多视角信息高效聚合到3D体素网格中，实现时空场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法在动态环境中的4D时空场景理解存在局限：要么只能提供粗糙的几何跟踪（如边界框），要么只能提供缺乏明确时间关联的详细3D结构（如体素占据）。需要一种能够同时处理几何跟踪和时空关联的完整解决方案。

Method: 提出潜在高斯溅射方法：1) 将观测融合到3D高斯中，作为3D场景的稀疏点中心潜在表示；2) 将聚合的特征溅射到3D体素网格上；3) 通过基于掩码的分割头解码。结合了基于摄像头的端到端跟踪和基于掩码的多视角全景占据预测。

Result: 在Occ3D nuScenes和Waymo数据集上评估，实现了4D全景占据跟踪的最先进性能。

Conclusion: LaGS通过潜在高斯溅射方法，为4D时空场景理解提供了一个完整的解决方案，能够同时处理几何跟踪和时空关联，在动态环境中为机器人提供安全可靠的运行支持。
...
### [125] [Phys-3D: Physics-Constrained Real-Time Crowd Tracking and Counting on Railway Platforms](https://arxiv.org/abs/2602.23177)
...
<details>
  <summary>Details</summary>
Motivation: 铁路站台实时人群计数对安全和容量管理至关重要，现有方法在动态条件下（相机运动、遮挡、透视变形）不可靠，需要物理一致的运动建模

Method: 提出物理约束跟踪框架：集成迁移学习的YOLOv11m检测器和EfficientNet-B0外观编码的DeepSORT，引入物理约束卡尔曼模型(Phys-3D)通过针孔几何强制物理合理的3D运动动力学，使用虚拟计数带增强遮挡鲁棒性

Result: 在MOT-RailwayPlatformCrowdHead数据集上，计数误差降至2.97%，在运动和遮挡条件下表现出鲁棒性能

Conclusion: 结合几何第一原理和运动先验可实现安全关键交通场景中可靠的人群计数，有助于列车调度和站台安全管理
...
### [126] [Uni-Animator: Towards Unified Visual Colorization](https://arxiv.org/abs/2602.23191)
*Xinyuan Chen,Yao Xu,Shaowen Wang,Pengjie Song,Bowen Deng*

Main category: cs.CV

TL;DR: Uni-Animator：基于扩散Transformer的统一图像与视频草图着色框架，解决现有方法在颜色迁移不精确、高频细节保留不足和大运动场景时间一致性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有草图着色方法难以统一处理图像和视频任务，存在三个主要问题：1）单/多参考下的颜色迁移不精确；2）高频物理细节保留不足；3）大运动场景中时间一致性差，出现运动伪影。

Method: 提出基于扩散Transformer的统一框架，包含三个关键技术：1）通过实例块嵌入实现视觉参考增强，精确对齐和融合参考颜色信息；2）使用物理特征进行物理细节强化，有效捕捉和保留高频纹理；3）基于草图的动态RoPE编码，自适应建模运动感知的时空依赖关系。

Result: 大量实验结果表明，Uni-Animator在图像和视频草图着色任务上均达到竞争性性能，与任务专用方法相当，同时解锁了统一的跨域能力，具有高细节保真度和鲁棒的时间一致性。

Conclusion: Uni-Animator成功解决了统一图像和视频草图着色中的关键挑战，通过创新的参考增强、细节强化和动态编码技术，实现了精确的颜色迁移、高频细节保留和运动感知的时间一致性。
...
### [127] [ColoDiff: Integrating Dynamic Consistency With Content Awareness for Colonoscopy Video Generation](https://arxiv.org/abs/2602.23203)
...
<details>
  <summary>Details</summary>
Motivation: 结肠镜视频生成对于诊断肠道疾病至关重要，特别是在数据稀缺场景下。高质量视频生成面临肠道结构不规则、疾病表征多样、成像模式各异等挑战，需要时间一致性和临床属性的精确控制。

Method: 提出ColoDiff扩散框架：1) TimeStream模块通过跨帧标记化机制解耦时间依赖性，实现不规则肠道结构的动态建模；2) Content-Aware模块结合噪声注入嵌入和可学习原型，实现临床属性的精确控制；3) 采用非马尔可夫采样策略，减少90%以上步骤实现实时生成。

Result: 在三个公共数据集和一个医院数据库上进行评估，包括生成指标和下游任务（疾病诊断、模态判别、肠道准备评分、病变分割）。实验表明ColoDiff能生成具有平滑过渡和丰富动态的视频。
...
### [128] [Motion-aware Event Suppression for Event Cameras](https://arxiv.org/abs/2602.23204)
...
<details>
  <summary>Details</summary>
Motivation: 事件相机在动态场景中面临独立运动物体(IMO)和自身运动产生的冗余事件干扰，需要实时过滤这些事件以提升下游任务性能

Method: 提出联合分割当前事件流中的IMO并预测其未来运动的框架，采用轻量级架构实现实时处理

Result: 在EVIMO基准上分割精度提升67%，推理速度达173Hz（提升53%），内存使用<1GB；下游任务中ViT推理加速83%，视觉里程计ATE降低13%

Conclusion: 该框架首次实现实时运动感知事件抑制，显著提升事件相机在动态场景中的性能，为下游应用带来实质性改进
...
### [129] [EmbodMocap: In-the-Wild 4D Human-Scene Reconstruction for Embodied Agents](https://arxiv.org/abs/2602.23205)
...
TL;DR: EmbodMocap：使用两台移动iPhone进行便携式、低成本的人类-场景运动捕捉系统，实现统一度量坐标系下的人类与场景重建，支持多种具身AI任务。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉系统依赖昂贵的专业设备和固定工作室设置，难以在自然环境中大规模收集场景条件化的人类运动数据。需要开发便携、低成本的数据采集方案来促进具身AI研究。

Method: 提出EmbodMocap系统，使用两台移动iPhone采集双RGB-D序列，通过联合标定技术将人类和场景重建到统一的度量世界坐标系中。双视角设置有效缓解深度歧义问题，无需静态相机或标记点。

Result: 与光学捕捉基准相比，双视角系统显著优于单iPhone或单目模型，在配准和重建性能上表现优越。基于采集数据成功赋能三项具身AI任务：单目人类-场景重建、基于物理的角色动画、机器人运动控制。

Conclusion: EmbodMocap提供了一种便携、经济高效的数据采集方案，能够无缝桥接人类运动与场景几何，为具身AI研究提供了高质量、大规模的数据支持，推动了该领域的发展。
...
### [130] [Through BrokenEyes: How Eye Disorders Impact Face Detection?](https://arxiv.org/abs/2602.23212)
...
<details>
  <summary>Details</summary>
Motivation: 视觉障碍严重影响数百万人，改变视觉信息的处理和感知方式。需要理解这些疾病如何影响视觉处理系统，特别是深度学习模型中的神经样特征表示。

Method: 使用BrokenEyes系统开发计算框架，模拟年龄相关性黄斑变性、白内障、青光眼、屈光不正和糖尿病视网膜病变五种眼病。结合人类和非人类数据集，在正常和疾病特定条件下训练模型，分析特征图的变化。

Result: 模型训练显示特征图出现关键性破坏，特别是白内障和青光眼，这与这些疾病中已知的神经处理挑战一致。激活能量和余弦相似度等评估指标量化了这些扭曲的严重程度。

Conclusion: 该框架提供了关于退化视觉输入与学习表示之间相互作用的见解，有助于理解眼病如何影响视觉信息处理，并为相关研究提供计算工具。
...
### [131] [Plug-and-Play Diffusion Meets ADMM: Dual-Variable Coupling for Robust Medical Image Reconstruction](https://arxiv.org/abs/2602.23214)
*Chenhe Du,Xuanyu Tian,Qing Wu,Muyu Liu,Jingyi Yu,Hongjiang Wei,Yuyao Zhang*

Main category: cs.CV

TL;DR: 提出Dual-Coupled PnP Diffusion框架，通过恢复经典对偶变量提供积分反馈，结合谱同质化技术解决现有PnP扩散先验框架的稳态偏差与幻觉问题。
...
Method: 提出Dual-Coupled PnP Diffusion框架：1) 恢复经典对偶变量提供积分反馈，理论上保证渐近收敛到精确数据流形；2) 引入谱同质化技术，在频域调制累积对偶残差，将其转化为符合加性高斯白噪声假设的伪AWGN输入，解决结构化伪影导致的幻觉问题。

Result: 在CT和MRI重建任务上的大量实验表明，该方法解决了偏差-幻觉权衡问题，实现了最先进的保真度并显著加速了收敛速度。

Conclusion: 通过将严格优化轨迹与去噪器有效统计流形对齐，Dual-Coupled PnP Diffusion with Spectral Homogenization框架成功解决了PnP扩散先验中的稳态偏差和幻觉问题，为成像逆问题提供了可靠解决方案。
...
### [132] [Multidimensional Task Learning: A Unified Tensor Framework for Computer Vision Tasks](https://arxiv.org/abs/2602.23217)
...
<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉任务表述受限于基于矩阵的思维模式：标准架构依赖矩阵值权重和向量值偏置，需要结构扁平化处理，这限制了自然可表达任务的空间。需要一种能够保持维度信息、避免信息损失的统一框架。

Method: 提出广义爱因斯坦MLP（GE-MLP），使用张量值参数，通过爱因斯坦积直接在张量上操作。该框架允许显式控制哪些维度被保留或收缩，无需信息损失。通过数学推导将分类、分割和检测等任务统一为多维任务学习的特例。

Result: 证明了分类、分割和检测任务只是MTL的特殊情况，仅在形式化定义的任务空间中的维度配置不同。证明了该任务空间严格大于基于矩阵的表述所能原生表达的空间，能够实现时空或跨模态预测等需要破坏性扁平化的任务配置。

Conclusion: 该工作通过张量代数为理解、比较和设计计算机视觉任务提供了数学基础，突破了传统矩阵思维的局限性，为更丰富、更自然的任务表达开辟了新的可能性。
...
### [133] [MovieTeller: Tool-augmented Movie Synopsis with ID Consistent Progressive Abstraction](https://arxiv.org/abs/2602.23228)
*Yizhi Li,Xiaohan Chen,Miao Jiang,Wentao Tang,Gaoang Wang*

Main category: cs.CV

TL;DR: MovieTeller：一种通过工具增强渐进抽象生成电影摘要的新框架，无需微调即可利用现成模型，通过人脸识别建立事实基础，解决现有视觉语言模型在长视频中角色识别不一致和叙事连贯性差的问题。


<details>
  <summary>Details</summary>
Motivation: 随着数字娱乐的爆炸式增长，自动视频摘要对于内容索引、个性化推荐和高效媒体归档变得不可或缺。现有视觉语言模型虽然在单图像描述方面表现良好，但在处理电影和电视剧等长视频时存在关键缺陷：缺乏ID一致的角色识别和破碎的叙事连贯性。

Method: 提出MovieTeller框架，采用训练免费、工具增强、事实基础的生成过程。首先调用专门的人脸识别模型作为外部"工具"建立事实基础——精确的角色身份及其对应的边界框。然后将这些基础信息注入提示中以引导VLM的推理，确保生成的场景描述基于可验证的事实。此外，采用渐进抽象管道将完整电影的摘要分解为多阶段过程，有效缓解当前VLM的上下文长度限制。

Result: 实验表明，与端到端基线相比，该方法在事实准确性、角色一致性和整体叙事连贯性方面取得了显著改进。

Conclusion: MovieTeller框架通过工具增强的渐进抽象方法，成功解决了长视频摘要中的角色识别一致性和叙事连贯性问题，为自动电影摘要生成提供了有效的解决方案。
...
### [134] [Large Multimodal Models as General In-Context Classifiers](https://arxiv.org/abs/2602.23229)
*Marco Garosi,Matteo Farina,Alessandro Conti,Massimiliano Mancini,Elisa Ricci*

Main category: cs.CV

TL;DR: LMMs通过上下文学习在分类任务中表现优异，提出的CIRCLE方法在开放世界分类中超越对比式VLMs


<details>
  <summary>Details</summary>
Motivation: 现有研究认为CLIP类对比视觉语言模型在零样本分类中表现优异，而大型多模态模型更适合复杂任务。本文认为这种观点忽视了LMMs的上下文学习能力，特别是在开放世界分类场景中。

Method: 1. 在闭集分类任务上基准测试LMMs与CLIP的性能；2. 在开放世界分类中提出CIRCLE方法：一种无需训练的方法，通过为上下文示例分配伪标签，并利用可用上下文本身迭代优化这些标签。

Result: 1. LMMs在提供少量上下文示例时，其性能可以匹配甚至超过带有缓存适配器的对比式VLMs；2. 在开放世界分类中，当上下文信息不完美时LMMs表现不佳；3. CIRCLE方法在开放世界分类中建立了强大的基准，超越了VLM对应方法。

Conclusion: LMMs具有作为统一分类器的潜力，是专用模型的灵活替代方案。上下文学习能力使LMMs在分类任务中表现出色，特别是在开放世界场景中，CIRCLE方法有效解决了不完美上下文信息的问题。
...
### [135] [Skarimva: Skeleton-based Action Recognition is a Multi-view Application](https://arxiv.org/abs/2602.23231)
...
<details>
  <summary>Details</summary>
Motivation: 当前骨架动作识别研究主要关注算法改进，而忽视了输入骨架数据的质量问题。本文旨在证明提升输入数据质量对模型性能的重要性。

Method: 利用多摄像头视角进行三角测量，生成更精确的3D骨架数据，以此作为高质量输入提供给现有动作识别模型。

Result: 使用多视角三角测量得到的更准确3D骨架数据，能够显著提升现有最先进动作识别模型的性能表现。

Conclusion: 输入数据质量是当前动作识别模型的性能瓶颈，多摄像头方案具有有利的成本效益比，未来骨架动作识别研究应将多视角应用作为标准配置。
...
### [136] [LineGraph2Road: Structural Graph Reasoning on Line Graphs for Road Network Extraction](https://arxiv.org/abs/2602.23290)
*Zhengyang Wei,Renzhi Jing,Yiyi He,Jenny Suckale*

Main category: cs.CV

TL;DR: LineGraph2Road：一种通过将道路连通性预测转化为线图上的二分类问题来改进道路提取的新框架，在多个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有道路提取方法通常分解为关键点提取和连通性预测两个步骤，但难以捕捉长距离依赖和复杂拓扑结构。端点嵌入融合方法在处理集合同构链接时存在局限性，需要更有效的链接表示和全局结构关系推理。

Method: 1. 构建全局稀疏欧几里得图：节点来自分割掩码提取的关键点，边连接预设距离阈值内的节点对，表示潜在道路段；2. 将原图转换为对应的线图；3. 在线图上应用图变换器进行连通性预测；4. 引入立交桥/地下通道头解决多层交叉问题；5. 采用耦合NMS策略保留关键连接。

Result: 在City-scale、SpaceNet和Global-scale三个基准测试上评估，在TOPO-F1和APLS两个关键指标上达到最先进水平。能够捕捉对实际部署至关重要的精细视觉细节。

Conclusion: LineGraph2Road通过将连通性预测重新定义为线图上的二分类问题，克服了现有方法的局限性，实现了更准确的道路提取，特别是在处理复杂拓扑和长距离依赖方面表现优异。
...
### [137] [PGVMS: A Prompt-Guided Unified Framework for Virtual Multiplex IHC Staining with Pathological Semantic Learning](https://arxiv.org/abs/2602.23292)
...
<details>
  <summary>Details</summary>
Motivation: 传统免疫组化染色需要足够组织样本，而小活检组织量不足限制了全面分析。虚拟多重染色技术可将H&E图像转换为多种IHC表示，但现有方法存在语义指导不足、染色分布不一致和空间错位三大挑战。

Method: 提出PGVMS框架：1) 自适应提示引导机制，使用病理视觉语言模型动态调整染色提示；2) 蛋白质感知学习策略(PALS)，通过直接量化和约束蛋白质分布保持精确表达模式；3) 原型一致学习策略(PCLS)，建立跨图像语义交互以校正空间错位。

Result: 未在摘要中明确说明具体实验结果，但框架针对性地解决了现有虚拟多重IHC染色方法的三大关键挑战。

Conclusion: PGVMS框架通过创新的自适应提示引导、蛋白质感知学习和原型一致学习策略，有效解决了虚拟多重免疫组化染色中的语义指导、染色分布一致性和空间对齐问题，仅需单重训练数据即可实现高质量的多重染色预测。
...
### [138] [Towards Long-Form Spatio-Temporal Video Grounding](https://arxiv.org/abs/2602.23294)
*Xin Gu,Bing Fan,Jiali Yao,Zhipeng Zhang,Yan Huang,Cheng Han,Heng Fan,Libo Zhang*

Main category: cs.CV

TL;DR: 提出ART-STVG方法解决长视频时空定位问题，通过自回归Transformer架构处理流式视频输入，设计时空记忆库和级联时空解码器，显著提升长视频定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要针对数十秒的短视频，而现实场景中视频可达数分钟甚至数小时，长视频包含更长的时间跨度和更多无关信息，传统方法难以有效处理。

Method: 提出ART-STVG自回归Transformer架构：1) 将视频作为流式输入顺序处理帧；2) 设计时空记忆库存储历史信息；3) 引入记忆选择策略提供相关上下文；4) 采用级联时空设计，空间解码器辅助时间解码器。

Result: 在新扩展的LF-STVG数据集上，ART-STVG显著优于现有方法，同时在传统短视频时空定位任务上保持竞争力。

Conclusion: ART-STVG通过自回归处理、记忆库机制和级联时空设计，有效解决了长视频时空定位的挑战，为现实应用提供了可行方案。
...
### [139] [ManifoldGD: Training-Free Hierarchical Manifold Guidance for Diffusion-Based Dataset Distillation](https://arxiv.org/abs/2602.23295)
*Ayush Roy,Wei-Yang Alex Lee,Rudrasis Chakraborty,Vishnu Suresh Lokhande*

Main category: cs.CV

TL;DR: 提出ManifoldGD：一种无需训练的扩散模型数据蒸馏框架，通过分层聚类获取多尺度IPC核心集，并在每个去噪步骤中施加流形一致性指导，提升生成数据的代表性、多样性和保真度。


<details>
  <summary>Details</summary>
Motivation: 大型数据集训练效率低且包含冗余概念，现有基于扩散模型的免训练蒸馏方法指导策略有限，要么进行无指导去噪，要么依赖简单的基于原型的模式指导，这些方法通常粗糙且次优。

Method: ManifoldGD框架：1) 通过VAE潜在特征的分层可分割聚类计算IPC中心，获得多尺度核心集；2) 在每个扩散去噪时间步，使用提取的IPC中心邻域构建潜在流形；3) 将模式对齐向量投影到估计的潜在流形切空间，约束生成轨迹保持流形一致性同时维持语义一致性。

Result: 实验结果显示，在FID、真实与合成数据集嵌入的l2距离、分类准确率等指标上，ManifoldGD一致优于现有的免训练和基于训练的基线方法，成为首个几何感知的免训练数据蒸馏框架。

Conclusion: ManifoldGD通过流形一致性指导改进了扩散模型在数据蒸馏中的表现，无需模型重新训练即可提升生成数据的代表性、多样性和图像保真度，为高效数据蒸馏提供了新的几何感知方法。
...
### [140] [PRIMA: Pre-training with Risk-integrated Image-Metadata Alignment for Medical Diagnosis via LLM](https://arxiv.org/abs/2602.23297)
*Yiqing Wang,Chunming He,Ming-Chen Lu,Mercy Pawar,Leslie Niziol,Maria Woodward,Sina Farsiu*

Main category: cs.CV

TL;DR: PRIMA是一个多模态医学诊断预训练框架，通过风险整合的图像-元数据对齐，将领域专业知识融入表示学习，显著提升疾病分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学诊断方法通常将临床元数据视为孤立标签，未能充分利用临床描述中丰富的语义知识，需要一种能够有效融合视觉表现和临床元数据的框架。
...
Conclusion: PRIMA框架成功将领域特定知识整合到多模态表示学习中，通过风险整合的图像-元数据对齐，为医学诊断提供了更有效的视觉表现与临床元数据融合方法。
...
### [141] [ThinkOmni: Lifting Textual Reasoning to Omni-modal Scenarios via Guidance Decoding](https://arxiv.org/abs/2602.23306)
...
### [142] [Retrieve and Segment: Are a Few Examples Enough to Bridge the Supervision Gap in Open-Vocabulary Segmentation?](https://arxiv.org/abs/2602.23339)
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇分割（OVS）在像素级预测方面落后于完全监督方法，主要受限于视觉语言模型的粗粒度图像级监督和自然语言的语义模糊性

Method: 引入少样本设置，通过像素标注图像的支持集增强文本提示；提出检索增强的测试时适配器，通过学习每图像的轻量级分类器，融合文本和视觉支持特征，实现每查询的学习融合

Result: 实验表明该方法显著缩小了零样本与监督分割之间的差距，同时保持了开放词汇能力，支持持续扩展的支持集，适用于细粒度任务如个性化分割

Conclusion: 通过融合文本和视觉支持特征的检索增强测试时适配器，有效解决了开放词汇分割中的监督不足和语义模糊问题，实现了更优的少样本分割性能
...
### [143] [Sensor Generalization for Adaptive Sensing in Event-based Object Detection via Joint Distribution Training](https://arxiv.org/abs/2602.23357)
...
Method: 通过深入分析事件相机内在参数如何影响基于事件数据的物体检测模型性能，并利用这些发现来扩展下游模型的能力。

Result: 提供了对事件相机内在参数影响模型性能的深入理解，并成功扩展了模型能力以实现传感器无关的鲁棒性。

Conclusion: 该研究填补了事件相机信号参数分析的空白，为基于事件数据的物体检测模型提供了重要指导，并推动了传感器无关鲁棒性的发展。
...
### [144] [SeeThrough3D: Occlusion Aware 3D Control in Text-to-Image Generation](https://arxiv.org/abs/2602.23359)
*Vaibhav Agrawal,Rishubh Parihar,Pradhaan Bhat,Ravi Kiran Sarvadevabhatla,R. Venkatesh Babu*

Main category: cs.CV

TL;DR: SeeThrough3D是一个用于3D布局条件生成的新模型，通过显式建模遮挡关系来解决现有方法在生成部分遮挡物体时的问题，实现了深度一致的几何和尺度生成。
...
Method: 提出遮挡感知的3D场景表示(OSCR)，将物体表示为虚拟环境中的半透明3D盒子，从期望的相机视角渲染。透明度编码隐藏的物体区域，使模型能够推理遮挡关系。基于预训练的基于流的文本到图像生成模型，引入从渲染的3D表示中提取的视觉标记进行条件化。应用掩码自注意力将每个物体边界框准确绑定到其对应的文本描述，避免物体属性混合。使用包含强物体间遮挡的多样化多物体场景合成数据集进行训练。

Result: SeeThrough3D能够有效泛化到未见过的物体类别，实现精确的3D布局控制，生成具有真实遮挡关系和一致相机控制的场景。

Conclusion: 通过显式建模遮挡关系，SeeThrough3D解决了3D布局条件生成中的关键挑战，实现了更精确、深度一致的场景生成，为3D场景合成提供了更好的控制能力。
...
### [145] [VGG-T$^3$: Offline Feed-Forward 3D Reconstruction at Scale](https://arxiv.org/abs/2602.23361)
*Sven Elflein,Ruilong Li,Sérgio Agostinho,Zan Gojcic,Laura Leal-Taixé,Qunjie Zhou,Aljosa Osep*

Main category: cs.CV

TL;DR: VGG-T³是一种可扩展的3D重建模型，通过将可变长度的键值空间表示蒸馏到固定大小的MLP中，实现了与输入视图数量线性相关的计算复杂度，相比基于softmax注意力的基线方法加速11.6倍。


<details>
  <summary>Details</summary>
Motivation: 解决离线前馈方法在3D重建中的关键限制：计算和内存需求随输入图像数量呈二次方增长，这限制了模型处理大规模图像集合的能力。

Method: 基于关键洞察：瓶颈源于场景几何的可变长度键值空间表示。通过测试时训练将这种表示蒸馏到固定大小的多层感知机中，构建VGG-T³模型。该方法保留了全局场景聚合能力，同时实现线性时间复杂度。

Result: 1）线性时间复杂度：与输入视图数量线性相关；2）高效重建：1000张图像集合仅需54秒重建；3）11.6倍加速：相比基于softmax注意力的基线方法；4）精度优势：点云重建误差显著优于其他线性时间方法；5）视觉定位能力：通过未见图像查询场景表示。

Conclusion: VGG-T³通过将可变长度键值空间蒸馏到固定大小MLP中，成功解决了3D重建中的二次方复杂度问题，实现了线性时间复杂度的高效重建，同时保持了全局场景聚合能力和视觉定位功能，为大规模3D重建提供了实用解决方案。
...
### [146] [MediX-R1: Open Ended Medical Reinforcement Learning](https://arxiv.org/abs/2602.23363)
...
TL;DR: MediX-R1是一个用于医学多模态大语言模型的开环强化学习框架，通过复合奖励信号实现临床基础的开放式答案生成，超越了传统选择题格式的限制。


<details>
  <summary>Details</summary>
Motivation: 传统医学MLLM评估主要依赖选择题格式，缺乏对开放式临床推理能力的评估。现有奖励机制难以处理开放式输出的语义正确性判断，需要更全面的评估框架来推动医学多模态模型的实际应用。

Method: 采用基于组的强化学习，结合四种奖励信号：1)基于LLM的准确性奖励（严格YES/NO判断语义正确性）；2)医学嵌入语义奖励（捕捉同义表达和术语变体）；3)轻量级格式奖励（确保可解释推理）；4)模态识别奖励。使用仅约51K指令样本进行微调。

Result: 在标准医学LLM（纯文本）和VLM（图像+文本）基准测试中表现优异，超越强大的开源基线模型，在开放式临床任务上取得显著提升。证明开环RL结合综合奖励信号和LLM评估是提升多模态医学推理可靠性的有效途径。

Conclusion: MediX-R1展示了通过精心设计的复合奖励信号和基于参考的LLM评估框架，可以在有限数据下有效提升医学多模态模型的开放式推理能力，为临床实际应用提供了实用路径。
...
### [147] [An Adaptive Multichain Blockchain: A Multiobjective Optimization Approach](https://arxiv.org/abs/2602.22230)
...
<details>
  <summary>Details</summary>
Motivation: 现有区块链系统可扩展性有限，且多链设计通常是静态的，无法适应需求和容量的动态变化。需要一种能够根据需求波动动态调整资源配置的机制。

Method: 将区块链配置建模为多智能体资源分配问题：应用和运营商声明需求、容量和价格边界；优化器每周期将它们分组到临时链中，并设置链级清算价格。目标函数最大化治理加权的应用、运营商和系统标准化效用组合。模型具有模块化特性，支持能力兼容性、应用类型多样性和周期稳定性，可在链下求解并在链上验证结果。

Result: 分析了公平性和激励问题，并通过模拟展示了吞吐量、去中心化、运营商收益和服务稳定性之间的权衡关系。模型能够动态适应需求变化，优化资源配置。

Conclusion: 提出的多智能体资源分配框架为区块链配置提供了一种动态、可优化的解决方案，能够平衡多方利益，提高系统效率和适应性，同时保持可验证性。
...
### [148] [Optimized Disaster Recovery for Distributed Storage Systems: Lightweight Metadata Architectures to Overcome Cryptographic Hashing Bottleneck](https://arxiv.org/abs/2602.22237)
*Prasanna Kumar,Nishank Soni,Gaurang Munje*

Main category: cs.CR

TL;DR: 论文提出用确定性元数据驱动标识替代内容哈希，解决灾备中哈希索引失效导致的恢复时间瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 分布式存储架构中，灾备工作流依赖内容哈希进行数据识别和同步，但当哈希索引过时、不完整或需重建时，会导致系统性风险，影响恢复时间目标（RTO）合规性。

Method: 提出通用架构转变：在数据摄取时为数据块分配全局唯一的复合标识符（独立于内容分析），实现灾备期间无需密码学开销的即时增量计算。

Result: 精确描述了完全或部分重新哈希不可避免的操作条件，分析了密码学重新哈希对RTO合规性的下游影响，提出了元数据驱动识别框架。

Conclusion: 确定性元数据驱动标识能够消除灾备期间的密码学开销，实现即时增量计算，显著改善恢复时间目标合规性。
...
### [149] [Analysis of LLMs Against Prompt Injection and Jailbreak Attacks](https://arxiv.org/abs/2602.22242)
*Piyush Jaiswal,Aaditya Pratap,Shreyansh Saraswati,Harsh Kasyap,Somanath Tripathy*

Main category: cs.CR

TL;DR: 评估多个开源LLM的提示注入和越狱漏洞，使用手动策划的数据集测试Phi、Mistral、DeepSeek-R1、Llama 3.2、Qwen和Gemma等模型，发现显著行为差异，并测试轻量级推理时防御机制的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在现实系统中的广泛部署，提示工程成为资源有限组织采用LLM的高效工具，但LLM易受提示攻击，分析这种风险已成为关键安全需求。

Method: 使用大型手动策划的数据集评估多个开源LLM的提示注入和越狱漏洞，包括Phi、Mistral、DeepSeek-R1、Llama 3.2、Qwen和Gemma变体，并评估几种轻量级推理时防御机制，这些机制作为过滤器运行，无需重新训练或GPU密集型微调。

Result: 观察到模型间显著的行为差异，包括拒绝响应和由内部安全机制触发的完全静默无响应。轻量级防御机制能缓解直接攻击，但始终被长且推理密集的提示绕过。

Conclusion: LLM对提示攻击存在显著漏洞，现有轻量级防御机制对复杂攻击无效，需要更强大的安全措施来保护LLM部署。
...
TL;DR: 该论文提出了DiSP（扩散自净化）框架，用于防御多模态扩散语言模型（MDLMs）的后门攻击，无需辅助模型或干净参考数据即可有效移除后门。


<details>
  <summary>Details</summary>
Motivation: 多模态扩散语言模型（MDLMs）已成为自回归模型的有力竞争者，但其后门攻击的脆弱性尚未得到充分研究。虽然现有的数据投毒方法能成功在MDLMs中植入后门，但针对这些模型的有效防御策略尚未出现。

Method: 提出DiSP（扩散自净化）防御框架，基于关键观察：在推理时选择性屏蔽某些视觉标记可以中和后门模型的触发诱导行为并恢复正常功能。基于此，使用被攻击模型自身净化被投毒的数据集，然后在净化数据上微调模型以恢复为干净模型。该设计无需辅助模型或干净参考数据。

Result: 大量实验表明，该方法能有效减轻后门效应，将攻击成功率（ASR）从超过90%降低到通常低于5%，同时保持模型在良性任务上的性能。

Conclusion: DiSP框架为多模态扩散语言模型提供了一种有效的后门防御方案，能够在无需外部资源的情况下恢复被攻击模型，填补了该领域防御策略的空白。
...
### [151] [Differentially Private Truncation of Unbounded Data via Public Second Moments](https://arxiv.org/abs/2602.22282)
*Zilong Cao,Xuan Bi,Hai Zhang*

Main category: cs.CR

TL;DR: 提出PMT方法，利用少量公开数据的二阶矩信息对私有数据进行变换和截断，改善差分隐私模型在无界分布数据上的性能


<details>
  <summary>Details</summary>
Motivation: 差分隐私(DP)通常要求数据具有有界分布，但在实际应用中数据分布往往无界。现有方法无法有效处理无界分布数据，导致DP噪声严重影响模型性能。

Method: 提出公共矩引导截断(PMT)：1)利用少量公开数据的二阶矩矩阵对私有数据进行变换；2)设计基于数据维度和样本量的截断半径；3)为惩罚回归和广义线性回归设计新的损失函数和算法，确保变换空间解能映射回原始域。

Result: PMT显著改善了二阶矩矩阵的条件数，增强了抵抗DP噪声的能力。理论分析表明：1)建立了改进的DP估计误差界；2)提供了鲁棒性保证；3)证明了收敛性结果。合成和真实数据集实验证实PMT大幅提高了DP模型的准确性和稳定性。

Conclusion: PMT通过利用公开二阶矩信息有效解决了DP在无界分布数据上的局限性，显著提升了差分隐私模型的性能，为实际隐私保护应用提供了实用解决方案。
...
### [152] [HubScan: Detecting Hubness Poisoning in Retrieval-Augmented Generation Systems](https://arxiv.org/abs/2602.22427)
*Idan Habler,Vineeth Sai Narajala,Stav Koren,Amy Chang,Tiffany Saade*

Main category: cs.CR

TL;DR: Hubscan是一个开源安全扫描器，用于检测RAG系统中的hubness安全漏洞，通过多检测器架构识别恶意内容注入点，在多个基准测试中实现高召回率。


<details>
  <summary>Details</summary>
Motivation: RAG系统存在hubness安全漏洞，即某些项目会频繁出现在大量不同查询的top-k检索结果中，这些hub可被利用来注入有害内容、篡改搜索排名、绕过内容过滤并降低系统性能，需要专门的安全检测工具。

Method: 提出hubscan多检测器架构，包括：(1)基于中位数/MAD的鲁棒统计hubness检测，(2)聚类分布分析评估跨聚类检索模式，(3)查询扰动下的稳定性测试，(4)领域感知和模态感知检测。支持多种向量数据库和检索技术。

Result: 在Food-101、MS-COCO和FiQA对抗性hubness基准测试中，hubscan在0.2%警报预算下达到90%召回率，0.4%时达到100%召回率，对抗性hub排名高于99.8百分位。领域限定扫描可检测出100%逃避全局检测的定向攻击。

Conclusion: Hubscan为生产RAG系统提供了一个实用、可扩展的hubness威胁检测框架，能有效识别安全漏洞，保护系统免受恶意内容注入和其他攻击。
...
TL;DR: 提出一种保护马尔可夫链模型用户数据的差分隐私方法，通过私有化单位单纯形查询来保护数据，同时保持模型准确性


<details>
  <summary>Details</summary>
Motivation: 马尔可夫链广泛用于建模用户行为，但构建准确模型需要大量用户数据，而共享这些模型可能泄露底层用户数据的敏感信息，因此需要保护用户隐私

Method: 首先开发了私有化单位单纯形查询的方法并证明其差分隐私性；然后扩展该方法来私有化随机矩阵（每行都是数据库的单纯形值查询），包括数据驱动的马尔可夫链模型

Result: 通过KL散度量化查询准确性；分析边界了私有和非私有马尔可夫链模型之间平稳分布和收敛速率的变化；模拟显示在典型隐私实现下，平稳分布误差小于2%

Conclusion: 该方法能够在保护用户数据隐私的同时，准确捕获系统行为，为私有马尔可夫链建模提供了有效解决方案
...
### [154] [Explainability-Aware Evaluation of Transfer Learning Models for IoT DDoS Detection Under Resource Constraints](https://arxiv.org/abs/2602.22488)
*Nelly Elsayed*

Main category: cs.CR

TL;DR: 该研究对七种预训练CNN架构在IoT DDoS检测中进行可解释性评估，发现DenseNet和MobileNet在性能、可靠性和可解释性方面表现最佳，强调选择模型时应综合考虑这些因素。


<details>
  <summary>Details</summary>
Motivation: DDoS攻击威胁IoT基础设施可用性，特别是在资源受限环境中。虽然迁移学习模型已显示出良好的检测精度，但其在操作环境中的可靠性、计算可行性和可解释性尚未得到充分探索。

Method: 使用CICDDoS2019数据集和基于图像的流量表示，对七种预训练CNN架构进行可解释性感知的实证评估。分析整合了性能指标、可靠性导向统计（MCC、Youden指数、置信区间）、延迟和训练成本评估，以及使用Grad-CAM和SHAP的可解释性评估。

Result: DenseNet和MobileNet架构在检测性能、可靠性和紧凑的类别一致性归因模式方面表现优异。DenseNet169提供最强的可靠性和可解释性对齐，而MobileNetV3为雾计算部署提供了有效的延迟-精度权衡。

Conclusion: 研究结果强调在选择IoT DDoS检测的深度学习模型时，需要结合性能、可靠性和可解释性标准进行综合考量。
...
### [155] [Layer-Targeted Multilingual Knowledge Erasure in Large Language Models](https://arxiv.org/abs/2602.22562)
*Taoran Li,Varun Chandrasekaran,Zhiyuan Yu*

Main category: cs.CR

TL;DR: 论文提出MUTE框架，通过识别语言无关的中间层进行定向擦除，解决了大语言模型多语言遗忘中存在的泛化失败问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现大语言模型的知识遗忘在多语言环境中存在泛化失败：在一种语言中擦除的知识在其他语言中仍然可访问。然而，这种失败的根本原因和原则性解决方案尚未明确。

Method: 提出MUTE框架，使用中心核对齐(CKA)和语言区域发展分数(LRDS)识别中间、语言无关的层，这些层中跨语言表示趋于收敛。将遗忘更新限制在这些层中，实现鲁棒的多语言知识擦除。

Result: 通过系统性的分层实验，识别了两种不同的失败模式：浅层干预实现擦除但损害保留语言的多语言能力；深层干预保留效用但即使在源语言中也无法擦除目标知识。MUTE在三种LLM架构和三种遗忘算法上验证有效，Logit Lens探测确认了真正的知识移除而非输出级抑制。

Conclusion: 干预深度是决定多语言泛化的关键因素，干预层的选择不是自由参数，而是决定多语言遗忘是否成功的基本因素。MUTE框架通过识别语言无关层进行定向擦除，实现了鲁棒的多语言知识遗忘。
...
### [156] [DPSQL+: A Differentially Private SQL Library with a Minimum Frequency Rule](https://arxiv.org/abs/2602.22699)
...
Method: DPSQL+采用模块化架构：1) Validator静态限制查询到DP安全的SQL子集；2) Accountant一致跟踪多个查询的累积隐私损失；3) Backend与各种数据库引擎接口，确保可移植性和可扩展性。系统同时强制执行用户级(ε,δ)-DP和最小频率规则。

Result: 在TPC-H基准测试上的实验表明，DPSQL+在广泛的分析工作负载（从基本聚合到二次统计和连接操作）中实现了实用准确性，并且在固定全局隐私预算下，比评估中的先前库允许更多查询。

Conclusion: DPSQL+成功解决了同时满足差分隐私和最小频率规则的需求，通过模块化设计实现了安全、可扩展的隐私保护SQL查询处理，在隐私保证和实用性之间取得了良好平衡。
...
### [157] [IMMACULATE: A Practical LLM Auditing Framework via Verifiable Computation](https://arxiv.org/abs/2602.22700)
...
<details>
  <summary>Details</summary>
Motivation: 商业大语言模型通常作为黑盒API服务部署，用户需要信任提供商正确执行推理并如实报告令牌使用情况。现有方法缺乏有效的审计机制来检测提供商可能出于经济动机进行的恶意行为。

Method: IMMACULATE采用选择性审计策略，仅对一小部分请求使用可验证计算进行审计。该方法通过密码学技术实现强检测保证，同时通过分摊密码学开销来保持实用性。框架无需可信硬件或模型内部访问权限。

Result: 在密集模型和MoE模型上的实验表明，IMMACULATE能够可靠地区分良性执行和恶意执行，同时保持低于1%的吞吐量开销。该框架有效检测了模型替换、量化滥用和令牌超额计费等经济动机偏差。

Conclusion: IMMACULATE为商业大语言模型API服务提供了一个实用且高效的审计框架，能够在保持低性能开销的同时，检测提供商可能的经济动机偏差行为，增强了用户对黑盒API服务的信任。
...
### [158] [SettleFL: Trustless and Scalable Reward Settlement Protocol for Federated Learning on Permissionless Blockchains (Extended version)](https://arxiv.org/abs/2602.23167)
*Shuang Liang,Yang Hua,Linshan Jiang,Peishen Yan,Tao Song,Bin Yao,Haibing Guan*

Main category: cs.CR

TL;DR: SettleFL是一个用于开放联邦学习的去中心化奖励结算协议，通过两种可互操作策略（乐观执行和有效性证明）解决区块链成本与模型训练高频迭代之间的矛盾，实现可扩展性和经济效率。
...
Method: 提出SettleFL协议，基于共享领域特定电路架构，提供两种可互操作策略：1) Commit-and-Challenge变体，通过乐观执行和争议驱动仲裁最小化链上成本；2) Commit-with-Proof变体，通过每轮有效性证明保证即时最终性。该设计可在不同延迟和成本约束下灵活适应，同时确保理性鲁棒性而无需可信协调。

Result: 结合真实联邦学习工作负载和受控模拟的广泛实验表明，SettleFL在扩展到800名参与者时仍保持实用性，实现了显著降低的gas成本。

Conclusion: SettleFL通过创新的协议设计解决了开放联邦学习中奖励结算的可扩展性和成本问题，为去中心化协作提供了经济高效的解决方案，同时保持了去中心化和安全性。
...
### [159] [Strengthening security and noise resistance in one-way quantum key distribution protocols through hypercube-based quantum walks](https://arxiv.org/abs/2602.23261)
...
<details>
  <summary>Details</summary>
Motivation: 传统BB84等QKD协议虽然简单，但对窃听抵抗有限且在现实噪声条件下性能不佳。量子行走(QW)为增强QKD提供了新途径，但现有基于环形拓扑的QW-QKD协议仍有改进空间

Method: 1. 提出基于超立方体拓扑的量子行走单向QKD协议，其安全性完全依赖于量子行走拓扑而非协议细节；2. 开发高效可扩展的QW-QKD仿真框架，支持环形和超立方体拓扑，使用Qiskit实现并包含现实噪声模型

Result: 在相同参数下，超立方体拓扑相比环形拓扑（当前最优）提供显著增强的安全性和噪声抵抗能力，有效加强了对窃听的防护。开源框架支持可重复性和未来研究

Conclusion: 基于超立方体拓扑的量子行走QKD协议结合了增强的噪声容忍度和拓扑驱动的安全性，为设计拓扑感知的QKD协议奠定了基础
...
### [160] [LoBoost: Fast Model-Native Local Conformal Prediction for Gradient-Boosted Trees](https://arxiv.org/abs/2602.22432)
*Vagner Santos,Victor Coscrato,Luben Cabezas,Rafael Izbicki,Thiago Ramos*

Main category: stat.ML

TL;DR: LoBoost：一种基于梯度提升决策树的本地化保形预测方法，利用树结构定义多尺度校准组，无需重新训练或辅助模型
...
Method: 利用已训练集成模型的叶子结构定义多尺度校准组：每个输入通过其访问的叶子序列编码；在分辨率级别k，通过匹配前k棵树中叶子索引的前缀来分组点，并在每个组内校准残差分位数

Result: 实验显示具有竞争力的区间质量，在大多数数据集上改进了测试MSE，并实现了大幅的校准加速

Conclusion: LoBoost提供了一种模型原生的本地保形预测方法，无需重新训练、辅助模型或额外数据分割，在保持预测性能的同时显著提升校准效率
...
### [161] [Flow Matching is Adaptive to Manifold Structures](https://arxiv.org/abs/2602.22486)
*Shivam Kumar,Yixin Wang,Lizhen Lin*

Main category: stat.ML

TL;DR: 该论文从理论上分析了当目标分布支撑在光滑流形上时，使用线性插值的流匹配方法，证明了其学习速度场的非渐近收敛性，并获得了接近极小极大最优的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 流匹配作为基于扩散的生成建模的无模拟替代方案，在高维数据集中表现出色，但现有理论分析假设目标分布具有光滑的全维密度，无法解释其在流形支撑场景下的有效性。

Method: 理论分析流匹配在线性插值下的性能，当目标分布支撑在光滑流形上时，建立学习速度场的非渐近收敛保证，并通过ODE传播估计误差获得流匹配目标诱导的隐式密度估计器的统计一致性。

Result: 获得了接近极小极大最优的收敛速率，该速率仅依赖于内在维度，并反映了流形和目标分布的光滑性，为流匹配如何适应内在数据几何结构并规避维度诅咒提供了原理性解释。

Conclusion: 流匹配在线性插值和流形支撑目标分布下具有理论保证，其收敛速率接近最优且仅依赖于内在维度，这解释了流匹配在高维流形数据场景中的实际有效性。
...
### [162] [Unsupervised Continual Learning for Amortized Bayesian Inference](https://arxiv.org/abs/2602.22884)
*Aayush Mishra,Šimon Kucharský,Paul-Christian Bürkner*

Main category: stat.ML

TL;DR: 提出用于摊销贝叶斯推理的持续学习框架，通过解耦模拟预训练和无监督顺序自洽微调，结合情景回放和弹性权重巩固来缓解灾难性遗忘，在模型误设下提升后验估计性能。


<details>
  <summary>Details</summary>
Motivation: 摊销贝叶斯推理在模型误设下性能下降，现有自洽训练方法仅限于静态单任务设置，无法处理顺序到达的数据或分布偏移，需要持续学习框架来适应现实世界数据流。

Method: 提出持续学习框架：1）解耦模拟预训练和无监督顺序自洽微调；2）情景回放策略，使用过去观测的记忆缓冲区；3）弹性权重巩固策略，正则化更新以保留任务关键参数。

Result: 在三个不同案例研究中，方法显著缓解了灾难性遗忘，产生的后验估计优于标准模拟训练，更接近MCMC参考结果，为跨任务可信赖的摊销贝叶斯推理提供了可行路径。

Conclusion: 提出的持续学习框架成功解决了摊销贝叶斯推理在顺序数据下的适应性问题，结合情景回放和弹性权重巩固的策略有效缓解了灾难性遗忘，提升了模型误设下的后验估计性能。
...
### [163] [Kernel Integrated $R^2$: A Measure of Dependence](https://arxiv.org/abs/2602.22985)
...
<details>
  <summary>Details</summary>
Motivation: 现有集成R²方法仅限于标量响应，需要扩展到更一般的响应空间（如多元、函数、结构化数据），同时保持对非线性依赖结构的敏感性。

Method: 将集成R²扩展到配备特征核的再生核希尔伯特空间，提出两种估计器：基于K近邻的图方法和基于条件均值嵌入的RKHS方法。

Result: 新度量取值[0,1]，零值当且仅当独立，单位值当且仅当响应是协变量的可测函数。图基估计器具有一致性、收敛速率和内在维度适应性。

Conclusion: 核集成R²在非线性依赖和结构化关系场景中，相比现有依赖性度量方法展现出竞争优势，适用于多元、函数和结构化数据的依赖性分析。
...
### [164] [Regular Fourier Features for Nonstationary Gaussian Processes](https://arxiv.org/abs/2602.23006)
*Arsalan Jawaid,Abdullah Karatas,Jörg Seewig*

Main category: stat.ML

TL;DR: 提出正则傅里叶特征方法用于可调和过程，避免传统谱方法对概率分布的依赖，通过直接离散化谱表示实现高效低秩近似


<details>
  <summary>Details</summary>
Motivation: 传统高斯过程模拟需要从高维高斯分布采样，计算复杂度随采样点数量立方增长。谱方法利用傅里叶表示将谱密度视为概率分布进行蒙特卡洛近似，但这仅适用于平稳过程，对于非平稳过程过于限制，因为其谱密度通常不是概率测度

Method: 提出正则傅里叶特征方法，直接离散化可调和过程的谱表示，保持谱权重间的相关结构而不需要概率假设。在有限谱支撑假设下，该方法构造出正半定的高效低秩近似。当谱密度未知时，框架可自然扩展到从数据学习核函数

Result: 该方法避免了传统谱方法对概率分布的依赖，适用于非平稳过程。在局部平稳核和具有复值谱密度的可调和混合核上进行了验证

Conclusion: 正则傅里叶特征方法为可调和过程提供了有效的谱表示离散化方案，突破了传统谱方法需要概率假设的限制，适用于更广泛的非平稳高斯过程模拟和核学习任务
...
### [165] [Sampling from Constrained Gibbs Measures: with Applications to High-Dimensional Bayesian Inference](https://arxiv.org/abs/2602.22369)
*Ruixiao Wang,Xiaohong Chen,Sinho Chewi*

Main category: math.ST

TL;DR: 该论文研究了低温吉布斯分布的非标准采样问题，其中分布的支撑集受到约束且部分坐标位于边界上。作者分析了这种分布在"前渐近"区域内的结构，并基于此提供了非渐近的采样保证。


<details>
  <summary>Details</summary>
Motivation: 研究低温吉布斯分布（如贝叶斯后验）的采样问题，特别是当分布的支撑集受到约束且部分坐标位于边界时的非标准情况。这类问题在实际应用中常见，但现有方法难以处理边界效应。

Method: 首先分析低温吉布斯分布在"前渐近"区域内的结构特性，证明其集中在模的邻域内，并在该区域内表现为有界扰动的乘积测度：正则部分为强对数凹分布，非正则部分每个坐标为一维指数型分布。然后利用这种结构分析朗之万动力学的谱隙，提供非渐近采样保证。

Result: 证明了在"前渐近"区域内，低温吉布斯分布集中在模的邻域内，且在该区域内可分解为有界扰动的乘积测度。基于此结构，通过分析朗之万动力学的谱隙，获得了非渐近的采样保证。在三个典型示例（高维逻辑回归、泊松线性模型、高斯混合模型）上验证了结果。

Conclusion: 该论文为具有约束支撑集和边界坐标的低温吉布斯分布提供了系统的采样理论框架，揭示了这类分布在"前渐近"区域内的特殊结构，并基于朗之万动力学获得了非渐近采样保证，为处理实际贝叶斯推断中的边界效应问题提供了理论依据。
...
### [166] [Remarks on stationary GARCH processes under heavy tail distributions](https://arxiv.org/abs/2602.22929)
*Marc Taberner-Ortiz,Manfred Denker*

Main category: math.ST

TL;DR: 提出GARCH过程平方序列中心化和的分布数值逼近方法，用于构造均值置信区间，并在重尾情况下改进重抽样方法


<details>
  <summary>Details</summary>
Motivation: 对于GARCH过程，当观测到平方序列时，需要数值逼近其中心化和的分布，以构造均值参数的置信区间，并改进传统正态逼近的精度

Method: 推导GARCH过程平方序列中心化和分布的数值逼近方法，基于观测数据构造置信区间；针对重尾创新过程，提出新的重抽样方法改进估计

Result: 获得了GARCH过程平方序列中心化和分布的数值逼近，构建了均值参数的置信区间，并在重尾情况下通过新重抽样方法提高了估计精度

Conclusion: 提出的数值逼近方法能够有效构造GARCH过程平方序列均值的置信区间，新重抽样方法在重尾情况下显著优于传统正态逼近方法
...
### [167] [Effective sample size approximations as entropy measures](https://arxiv.org/abs/2602.22954)
*L. Martino,V. Elvira*

Main category: math.ST

TL;DR: 本文分析了重要性采样算法中替代有效样本量（ESS）度量，建立了ESS表达式与Rényi熵和Tsallis熵的数学关系，证明了Huggins-Roy ESS族满足所有理论条件，并探讨了ESS在生态学、经济学和机器学习等领域的应用联系。


<details>
  <summary>Details</summary>
Motivation: 研究重要性采样算法中替代有效样本量（ESS）度量的理论基础和应用范围，建立ESS与信息论中熵概念的数学联系，探索ESS在不同领域的交叉应用价值。

Method: 1. 分析文献中使用的ESS表达式与Rényi熵和Tsallis熵的关系；2. 证明Huggins-Roy ESS族满足所有理论条件；3. 建立ESS与生态学Hill数、经济学基尼系数、机器学习基尼不纯度等概念的连接；4. 通过数值模拟评估不同ESS表达式的性能。

Result: 1. 建立了ESS表达式与Rényi熵和Tsallis熵的数学关系；2. 证明了Huggins-Roy ESS族满足所有理论条件；3. 发现了ESS与多个领域概念的深层联系；4. 数值模拟显示了不同ESS表达式在近似理论ESS定义方面的性能差异，并展示了ESS在变量选择问题中的应用。

Conclusion: 本文为重要性采样中的ESS度量提供了统一的理论框架，建立了ESS与信息论熵概念的数学联系，证明了Huggins-Roy ESS族的理论完备性，并展示了ESS在多个领域的广泛应用潜力，为ESS度量的选择和解释提供了理论依据。
...
### [168] [On the errors committed by sequences of estimator functionals](https://arxiv.org/abs/2602.23021)
*Steffen Grønneberg,Nils Lid Hjort*

Main category: math.ST

TL;DR: 该论文研究了几乎必然收敛的估计量序列中，最后一次偏离目标参数超过ε的时间的渐近极限，并应用于构建序列固定宽度置信区域


<details>
  <summary>Details</summary>
Motivation: Hjort和Fenstad(1992)在欧几里得参数空间中的相关工作需要扩展到更一般的参数泛函情形，以处理更广泛的统计估计问题

Method: 在弱光滑性条件下，识别估计量序列最后一次偏离目标参数超过ε的时间的渐近极限，该条件要求估计量接近经验分布的Hadamard可微泛函

Result: 得到了渐近极限的解析表达式，用于构建序列固定宽度置信区域，并推导了与估计量序列远尾相关的其他量的极限分布

Conclusion: 将欧几里得参数空间的结果推广到参数泛函情形，为基于Nelson-Aalen估计量的累积风险函数提供了新的序列同时置信集，并应用于随机规划中的计算复杂度问题
...
### [169] [Low-degree Lower bounds for clustering in moderate dimension](https://arxiv.org/abs/2602.23023)
*Alexandra Carpentier,Nicolas Verzelen*

Main category: math.ST

TL;DR: 该论文研究了高斯混合模型中聚类问题的计算复杂性，重点关注中等维度下（n ≥ dK）部分恢复所需的最小均值距离Δ，建立了新的低次多项式下界，并提出了匹配该界限的非谱算法。


<details>
  <summary>Details</summary>
Motivation: 高斯混合聚类问题中，信息论极限与已知多项式时间算法性能之间存在显著差距。虽然高维情况（n ≤ dK）的差距已被表征，但中等维度情况（n ≥ dK）的研究仍然不足，需要深入探索该维度下的计算复杂性。

Method: 建立了中等维度下（d ≥ K）的新低次多项式下界，分析了该维度下聚类难度的本质特征，并提出了一种匹配该界限的非谱算法，区别于高维情况下的降维和谱方法。

Result: 发现中等维度下的聚类难度涉及更微妙的现象，导致"非参数速率"，与高维情况下的降维和谱方法驱动机制不同。提出的非谱算法能够匹配这一速率，揭示了中等维度下聚类问题的计算极限。

Conclusion: 中等维度下的高斯混合聚类问题具有独特的计算特性，需要不同于高维情况的方法。研究填补了该维度下计算复杂性理解的空白，为聚类算法的设计提供了新的理论指导。
...
### [170] [Accelerated Online Risk-Averse Policy Evaluation in POMDPs with Theoretical Guarantees and Novel CVaR Bounds](https://arxiv.org/abs/2602.23073)
*Yaacov Pariente,Vadim Indelman*

Main category: math.ST

TL;DR: 提出一种加速POMDP中CVaR值函数评估的理论框架，通过推导CVaR的新界限并构建可计算边界，在简化模型下实现动作消除，显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 在部分可观察马尔可夫决策过程(POMDP)中进行风险规避决策是AI核心挑战，但求解POMDP计算不可行，现有近似方法依赖昂贵的未来轨迹模拟，需要加速CVaR值函数评估的方法。

Method: 1) 推导使用辅助随机变量Y对X的CVaR新界限，基于累积分布和密度函数关系假设；2) 建立从简化信念MDP可计算的CVaR值函数上下界；3) 在粒子信念MDP框架内开发具有概率保证的边界估计器；4) 通过动作消除实现加速：在简化模型下边界显示次优的动作被安全丢弃，同时确保与原POMDP一致性。

Result: 在多个POMDP领域实证评估表明，边界能可靠区分安全与危险策略，在简化模型下实现显著计算加速。

Conclusion: 该工作提出了一个具有形式性能保证的加速POMDP中CVaR值函数评估的理论框架，通过推导新的CVaR界限和构建可计算边界，在保持与原问题一致性的同时实现计算加速。
...
### [171] [Duel-Evolve: Reward-Free Test-Time Scaling via LLM Self-Preferences](https://arxiv.org/abs/2602.21585)
...
Result: 在MathBench上比现有方法和基线准确率提高20个百分点；在LiveCodeBench上比可比较的迭代方法改进超过12个百分点。方法无需奖励模型、搜索期间的真实标签或手工评分函数。

Conclusion: 成对自偏好为大规模离散输出空间的测试时改进提供了强大的优化信号，展示了仅通过LLM内部比较实现有效优化的可行性。
...
### [172] [A 1/R Law for Kurtosis Contrast in Balanced Mixtures](https://arxiv.org/abs/2602.22334)
*Yuda Bi,Wenjun Xiao,Linhao Bai,Vince D Calhoun*

Main category: cs.LG

TL;DR: 论文证明了在宽平衡混合中，基于峰度的独立成分分析(ICA)性能会减弱，提出了一个尖锐的冗余定律：峰度随有效宽度反比例衰减，并给出了样本估计的可行性条件。


<details>
  <summary>Details</summary>
Motivation: 传统基于峰度的ICA方法在处理宽平衡混合时性能下降，需要理解这种性能衰减的理论边界和恢复对比度的可行方法。

Method: 1) 证明标准化投影的总体超额峰度与有效宽度成反比关系；2) 建立样本峰度估计的可行性条件；3) 提出"纯化"方法选择符号一致的源信号以恢复对比度。

Result: 1) 建立了峰度衰减的尖锐边界：|κ(y)|=O(κ_max/R_eff)；2) 确定了样本估计的可行性阈值：R≲κ_max√T；3) 纯化方法能恢复R无关的对比度Ω(1/m)。

Conclusion: 基于峰度的ICA在宽平衡混合中存在根本性限制，但通过纯化方法可以恢复性能，为实际应用提供了理论指导和实用解决方案。
...
### [173] [Sample Complexity Bounds for Robust Mean Estimation with Mean-Shift Contamination](https://arxiv.org/abs/2602.22130)
*Ilias Diakonikolas,Giannis Iakovidis,Daniel M. Kane,Sihan Liu*

Main category: cs.LG

TL;DR: 该论文研究了均值偏移污染模型下的均值估计问题，建立了通用基分布下的样本复杂度上下界，证明了在温和谱条件下存在样本高效的估计算法。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅针对高斯和拉普拉斯分布等特殊情况给出了均值偏移污染模型下的样本复杂度，但通用基分布下的样本复杂度仍是一个开放问题。该工作旨在解决这一开放问题，确定通用基分布在均值偏移污染模型下的均值估计样本复杂度。

Method: 采用傅里叶分析技术，引入了傅里叶见证的概念作为上下界证明的关键工具。在温和的谱条件下，设计了一个样本高效的算法来估计目标均值。

Result: 证明了在温和谱条件下存在样本高效的均值估计算法，能够达到任意期望精度。同时给出了与上界在质量上匹配的样本复杂度下界，基本解决了这一开放问题。

Conclusion: 该工作基本解决了均值偏移污染模型中通用基分布的均值估计样本复杂度问题，表明在温和谱条件下存在样本高效的算法，这一性质与Huber污染模型形成对比。傅里叶分析技术和傅里叶见证概念是解决该问题的关键技术。
...
### [174] [Support Tokens, Stability Margins, and a New Foundation for Robust LLMs](https://arxiv.org/abs/2602.22271)
*Deepak Agarwal,Dhyey Dharmendrakumar Mavani,Suyash Gupta,Karthik Sethuraman,Tejas Dharamsi*

Main category: cs.LG

TL;DR: 论文将因果自注意力Transformer重新解释为概率框架，揭示了参数上的障碍约束，提出了类似支持向量机的边界解释和支持令牌概念，并提出了带对数障碍惩罚的贝叶斯训练框架。


<details>
  <summary>Details</summary>
Motivation: 重新解释现代基础模型核心的因果自注意力Transformer，将其置于概率框架中，类似于将经典PCA扩展到概率PCA，以揭示更深层的结构洞察。

Method: 1. 在概率框架下重新解释因果自注意力Transformer；2. 揭示参数变化引起的障碍约束和结构化几何；3. 提出类似支持向量机的边界解释和支持令牌概念；4. 将LLMs解释为令牌空间幂集上的随机过程；5. 提出贝叶斯框架和MAP估计目标，在标准交叉熵损失中添加平滑对数障碍惩罚。

Result: 1. 揭示了自注意力参数上的障碍约束，导致令牌空间的高度结构化几何；2. 发现了注意力变得病态的边界，提出了类似支持向量机的边界解释；3. 提出的带对数障碍惩罚的训练方法在不牺牲样本外准确性的情况下提供了更鲁棒的模型，且易于实践应用。

Conclusion: 论文通过概率重新解释揭示了因果自注意力Transformer的深层结构特性，提出了支持令牌概念和带对数障碍惩罚的贝叶斯训练框架，为LLM解码动态提供了理论洞察，并展示了实践中的改进效果。
...
### [175] [Mean Estimation from Coarse Data: Characterizations and Efficient Algorithms](https://arxiv.org/abs/2602.23341)
*Alkis Kalavasis,Anay Mehrotra,Manolis Zampetakis,Felix Zhou,Ziyu Zhu*

Main category: cs.LG

TL;DR: 该论文解决了从粗数据中进行高斯均值估计的两个核心问题：确定凸划分下均值可识别的条件，以及在可识别性和凸划分下实现计算高效的估计。


<details>
  <summary>Details</summary>
Motivation: 粗数据在实际应用中普遍存在（如测量舍入、传感器限制、经济系统滞后），但现有研究仅解决了凸划分下的样本高效估计问题，对于均值可识别的具体条件以及计算效率问题仍待解决。

Method: 研究高斯均值估计问题，其中真实样本来自d维高斯分布（单位协方差），但仅通过包含样本的划分集合被观察到。重点分析凸划分下的可识别性条件和计算高效估计算法。

Result: 解决了两个关键问题：1) 确定了凸划分下均值可识别的具体条件；2) 证明了在可识别性和凸划分下，计算高效的均值估计是可行的。

Conclusion: 该工作完全解决了从粗数据进行高斯均值估计的两个基本开放问题，为凸划分下的可识别性和计算效率提供了理论保证，推动了粗数据分析的理论发展。
...
### [176] [Regularized Online RLHF with Generalized Bilinear Preferences](https://arxiv.org/abs/2602.23116)
*Junghyun Lee,Minju Hong,Kwang-Sung Jun,Chulhee Yun,Se-Young Yun*

Main category: cs.LG

TL;DR: 论文研究了上下文在线RLHF中的纳什均衡识别问题，采用广义双线性偏好模型处理非传递性偏好，提出了两种算法实现理论保证的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF研究多局限于传递性偏好和特定正则化方法，需要扩展到更一般的偏好模型和正则化器，以处理现实世界中复杂的非传递性偏好。

Method: 采用广义双线性偏好模型（GBPM）通过低秩斜对称矩阵建模非传递性偏好；基于强凸正则化和特征多样性假设，提出两种算法：贪婪采样算法和探索-提交算法。

Result: 贪婪采样算法获得$\tilde{O}(ηd^4 (\log T)^2)$的多对数遗憾界，不依赖$e^{O(η)}$；探索-提交算法获得$\tilde{O}(\sqrt{ηr T})$的遗憾界，不依赖$\mathrm{poly}(d)$，是高维在线RLHF的首个统计有效保证。

Conclusion: 该研究为一般偏好下的在线RLHF提供了理论框架，通过GBPM模型和强凸正则化扩展了现有工作，两种算法分别在不同维度上实现了优化的遗憾界。
...
### [177] [Differentiable Zero-One Loss via Hypersimplex Projections](https://arxiv.org/abs/2602.23336)
*Camilo Gomez,Pengyang Wang,Liansheng Tang*

Main category: cs.LG

TL;DR: 提出Soft-Binary-Argmax算子，为0-1损失提供可微近似，解决其因不可微而无法用于梯度优化的历史难题，通过几何一致性约束提升大批量训练下的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 0-1损失是分类任务的黄金标准，但由于其不可微性无法用于基于梯度的优化。现有方法缺乏对0-1损失的有效可微近似，特别是在大规模批量训练中，传统方法存在泛化性能差距。

Method: 通过约束优化框架构建平滑、保序的投影到n,k维超单纯形，提出Soft-Binary-Argmax算子。推导其数学性质，展示其雅可比矩阵的高效计算方法，并将其集成到二分类和多分类学习系统中。

Result: 通过施加输出logits的几何一致性约束，显著提升了大批量训练下的泛化性能，缩小了传统大批量训练中观察到的性能差距。

Conclusion: 提出的Soft-Binary-Argmax算子为0-1损失提供了有效的可微近似，将结构化优化组件集成到端到端可微模型中，增强了归纳偏置并与任务特定目标更紧密对齐。
...
### [178] [Manifold of Failure: Behavioral Attraction Basins in Language Models](https://arxiv.org/abs/2602.22291)
*Sarthak Munshi,Manish Bhatt,Vineeth Sai Narajala,Idan Habler,AmmarnAl-Kahfah,Ken Huang,Blake Gatto*

Main category: cs.LG

TL;DR: 该论文提出了一种系统映射大语言模型失败流形（Manifold of Failure）的框架，将漏洞搜索重新定义为质量多样性问题，使用MAP-Elites算法揭示失败区域的连续拓扑结构（行为吸引盆地），为理解AI安全提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注将对抗性示例投影回自然数据流形以恢复安全性，但作者认为全面理解AI安全需要表征不安全区域本身。该研究旨在系统性地映射大语言模型的失败流形，从寻找离散失败转向理解其底层结构。

Method: 将漏洞搜索重新定义为质量多样性问题，使用MAP-Elites算法来揭示失败区域的连续拓扑结构（行为吸引盆地）。采用对齐偏差（Alignment Deviation）作为质量度量，引导搜索朝向模型行为与预期对齐偏离最大的区域。在Llama-3-8B、GPT-OSS-20B和GPT-5-Mini三个大语言模型上进行实验。

Result: MAP-Elites实现了高达63%的行为覆盖率，发现了多达370个不同的漏洞生态位。不同模型展现出显著不同的拓扑特征：Llama-3-8B表现出近乎普遍性的脆弱性平台（平均对齐偏差0.93），GPT-OSS-20B显示碎片化景观和空间集中的盆地（平均0.73），GPT-5-Mini表现出强大的鲁棒性，上限为0.50。该方法生成了现有攻击方法（GCG、PAIR或TAP）无法提供的可解释全局安全景观图。

Conclusion: 该研究引入了一个系统映射大语言模型失败流形的框架，将漏洞搜索范式从寻找离散失败转向理解其底层结构。该方法能够生成可解释的全局安全景观图，揭示了不同模型特有的拓扑特征，为AI安全评估提供了新的视角和工具。
...
### [179] [Beyond performance-wise Contribution Evaluation in Federated Learning](https://arxiv.org/abs/2602.22470)
*Balazs Pejo*

Main category: cs.LG

TL;DR: 该研究提出了一种新的联邦学习客户端评估框架，不仅考虑模型性能（如准确率），还评估客户端对模型可信度的贡献，包括可靠性、抗攻击性和公平性，发现这些维度相互独立，单一指标无法全面评估客户端贡献。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习客户端评估方法主要关注模型性能指标（如准确率、损失），但这只是机器学习模型整体效用的一个维度。本研究关注被忽视的问题：客户端对模型可信度的贡献，包括可靠性（对噪声数据的容忍度）、抗攻击性（对抗样本的抵抗能力）和公平性（通过人口统计奇偶性衡量）。

Method: 使用最先进的Shapley值近似方法来量化客户端在多维贡献中的价值。Shapley值是一种基于合作博弈论的价值分配原则方法，能够公平地评估每个参与者在整体效用中的贡献。

Result: 研究发现没有单个客户端在所有维度上都表现出色，且这些维度在很大程度上相互独立。这揭示了当前评估方案的关键缺陷：单一指标无法进行全面评估和公平的奖励分配。

Conclusion: 联邦学习需要多维度的客户端评估框架，考虑模型可信度的多个方面（可靠性、抗攻击性、公平性），而不仅仅是性能指标。Shapley值提供了一种原则性的方法来量化这些多维贡献，有助于实现更公平的奖励分配和更全面的客户端评估。
...
### [180] [To Deceive is to Teach? Forging Perceptual Robustness via Adversarial Reinforcement Learning](https://arxiv.org/abs/2602.22227)
*Yicheng Bao,Xuhong Wang,Xin Tan*

Main category: cs.LG

TL;DR: 提出AOT-SFT对抗数据集和AOT自博弈框架，通过攻击者-防御者协同进化提升多模态大语言模型的感知鲁棒性


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视觉场景中存在感知脆弱性，依赖有限训练数据且扩展成本高昂，限制了模型鲁棒性

Method: 提出AOT-SFT大规模对抗数据集和AOT自博弈框架，包含图像编辑攻击者和防御者MLLM的协同进化，攻击者生成多样化图像操作课程，迫使防御者适应改进

Result: 实验表明AOT显著提升防御者的感知鲁棒性并减少幻觉，建立了可扩展的训练范式

Conclusion: AOT框架通过自博弈协同进化有效提升MLLM的鲁棒性，为训练更可靠的多模态大语言模型提供了可扩展的解决方案
...
### [181] [Zatom-1: A Multimodal Flow Foundation Model for 3D Molecules and Materials](https://arxiv.org/abs/2602.22251)
...
TL;DR: Zatom-1是首个统一3D分子和材料生成与预测的基础模型，通过多模态流匹配目标联合建模离散原子类型和连续3D几何结构，在保持高效推理的同时实现跨化学领域的正向迁移。
...
Result: Zatom-1在生成和预测基准测试中匹配或优于专用基线模型，同时将生成推理时间减少一个数量级以上。实验证明联合生成预训练在化学领域间产生正向预测迁移：材料建模预训练提高了分子属性预测准确性。

Conclusion: Zatom-1成功统一了3D分子和材料的生成与预测学习，展示了基础模型在化学领域的潜力，通过联合预训练实现跨领域知识迁移和性能提升，为通用3D化学建模提供了有效解决方案。
...
### [182] [Deep Sequence Modeling with Quantum Dynamics: Language as a Wave Function](https://arxiv.org/abs/2602.22255)
*Ahmed Nebli,Hadi Saadatdoorabi,Kevin Yam*

Main category: cs.LG

TL;DR: 提出一种基于复值波函数和量子动力学的序列建模框架，利用量子干涉而非门控机制处理竞争假设，通过Born规则提取概率，理论证明在特定任务上相比实值正交模型具有二次维度优势。


<details>
  <summary>Details</summary>
Motivation: 传统循环架构依赖门控机制抑制竞争假设，本文旨在探索一种基于量子干涉原理的替代方案，利用复值波函数的相位演化实现更高效的序列建模，并理论分析其表示优势。

Method: 构建有限维希尔伯特空间上的复值波函数作为隐状态，通过学习的时变哈密顿量进行演化，采用Cayley离散化确保严格幺正性，通过Born规则（二次测量算子）提取token概率，并推导了潜概率质量的连续性方程。

Result: 理论证明了复值幺正模型在维度N下能精确解决的一类消歧任务，任何配备标准仿射-softmax读取头的实值正交模型都需要Ω(N²)维度，揭示了二次维度优势源于Born规则隐含地将N维状态提升到秩一厄米矩阵空间。

Conclusion: 该框架将量子力学原理引入序列建模，通过量子干涉机制和Born规则读取实现了比传统实值模型更高效的表示能力，守恒的成对电流为信息流追踪提供了内置诊断工具。
...
### [183] [Code World Models for Parameter Control in Evolutionary Algorithms](https://arxiv.org/abs/2602.22260)
*Camilo Chacón Sartori,Guillem Rodríguez Corominas*

Main category: cs.LG

TL;DR: LLM通过合成代码世界模型(CWM)来学习和控制优化器行为，在组合优化问题上超越传统自适应方法和深度强化学习


<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能够学习优化器的动态行为，并利用这种知识来控制优化器，特别是在随机组合优化问题中，传统自适应方法存在局限性

Method: 使用LLM合成Python程序（代码世界模型）来预测优化器动态，基于次优轨迹训练CWM，然后通过贪婪规划选择每一步的变异强度k

Result: 在LO和OneMax问题上，CWM-greedy达到理论最优策略的94%性能；在Jump_k问题上，所有基线方法失败时CWM-greedy达到100%成功率；在NK-Landscape上，CWM-greedy在15个实例中均优于所有基线

Conclusion: LLM合成的代码世界模型能够有效学习和控制优化器行为，在样本效率、成功率和泛化能力方面显著优于传统自适应方法和深度强化学习
...
### [184] [Sustainable LLM Inference using Context-Aware Model Switching](https://arxiv.org/abs/2602.22261)
*Yuvarani,Akashdeep Singh,Zahra Fathanah,Salsabila Harlen,Syeikha Syafura Al-Zahra binti Zahari,Hema Subramaniam*

Main category: cs.LG

TL;DR: 提出上下文感知模型切换方法，根据查询复杂度动态选择语言模型，实现节能高效的LLM推理，相比始终使用最大模型可降低能耗67.5%，同时保持93.6%的响应质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能耗日益增长引发可持续性担忧。当前AI部署依赖一刀切的推理策略，无论任务复杂度都路由到同一大型模型，导致大量不必要的能源浪费。

Method: 提出上下文感知模型切换方法，结合重复查询缓存、基于规则的复杂度评分（快速可解释决策）、机器学习分类（捕捉语义意图）和用户自适应组件（学习交互模式）。使用三个开源模型（Gemma3 1B、4B和Qwen3 4B）评估。

Result: 模型切换方法相比始终使用最大模型可降低能耗67.5%，保持93.6%的响应质量（BERTScore F1）。简单查询响应时间提升约68%。路由准确率高，在真实对话工作负载中验证有效。
...
### [185] [Entropy-Controlled Flow Matching](https://arxiv.org/abs/2602.22265)
*Chika Maduabuchi*

Main category: cs.LG

TL;DR: 提出熵控制流匹配(ECFM)，通过施加全局熵率约束来避免传统流匹配中的低熵瓶颈和模式塌陷问题


<details>
  <summary>Details</summary>
Motivation: 传统流匹配方法（ODE/SDE）虽然经验性能强，但无法直接控制轨迹的信息几何，允许低熵瓶颈出现，导致语义模式暂时性耗尽

Method: 提出熵控制流匹配(ECFM)：在连续性方程路径上施加全局熵率约束d/dt H(mu_t) >= -lambda的变分原理，是Wasserstein空间中的凸优化问题，具有KKT/Pontryagin系统，等价于具有显式熵乘子的薛定谔桥

Result: 在纯传输机制下，ECFM恢复熵最优传输测地线，当lambda->0时Gamma收敛到经典最优传输；获得具有Lipschitz稳定性的模式覆盖和密度下界保证；构建了无约束流匹配的近似最优塌陷反例

Conclusion: ECFM通过熵率约束有效解决了传统流匹配中的模式塌陷问题，提供了理论保证和几何控制，在保持传输效率的同时确保模式覆盖
...
### [186] [Data-Driven Supervision of a Thermal-Hydraulic Process Towards a Physics-Based Digital Twin](https://arxiv.org/abs/2602.22267)
...
<details>
  <summary>Details</summary>
Motivation: 工业生产过程实时监控是多个行业的共同挑战，需要监测过程组件并进行预测性维护以确保安全、连续生产和高效率。数字孪生概念为系统监控提供了合适的框架。
...
Conclusion: 数字孪生框架结合数值模拟和机器学习方法能够有效实现热工水力过程的故障检测与诊断，为工业生产过程的实时监控提供了可行的技术方案。
...
### [187] [AutoQRA: Joint Optimization of Mixed-Precision Quantization and Low-rank Adapters for Efficient LLM Fine-Tuning](https://arxiv.org/abs/2602.22268)
...
<details>
  <summary>Details</summary>
Motivation: 现有量化后参数高效微调方法存在局限性：量化位宽和LoRA秩的单独优化无法充分利用两者间的复杂交互关系，导致在相同内存预算下性能差异显著

Method: 提出两阶段联合优化框架：1）全局多保真度进化搜索，通过层重要性先验初始化种群，使用特定算子和性能模型高效筛选候选配置；2）信任域贝叶斯优化，在搜索空间有希望区域进行局部细化，识别给定内存预算下的最优配置

Result: AutoQRA在内存占用与均匀4位方法相当的情况下，实现了接近全精度微调的性能

Conclusion: AutoQRA通过联合优化量化位宽和LoRA秩，有效解决了量化微调中的配置优化问题，能够在有限GPU内存约束下实现高性能下游适应
...
### [188] [Prior Knowledge-enhanced Spatio-temporal Epidemic Forecasting](https://arxiv.org/abs/2602.22270)
...
<details>
  <summary>Details</summary>
Motivation: 现有时空疫情预测方法存在三个主要问题：对弱疫情信号不敏感、空间关系过度简化、参数估计不稳定。这些问题限制了预测的准确性和实用性，特别是在公共卫生管理中的实际应用。

Method: STOEP框架包含三个核心组件：1) 病例感知邻接学习(CAL)：基于历史感染模式动态调整基于移动性的区域依赖关系；2) 空间信息参数估计(SPE)：使用可学习的空间先验增强弱疫情信号；3) 基于滤波的机制预测(FMF)：采用专家引导的自适应阈值策略正则化疫情参数。

Result: 在真实世界的COVID-19和流感数据集上的实验表明，STOEP在RMSE指标上比最佳基线方法提升了11.1%。该系统已在中国某省级疾控中心部署，支持下游应用。

Conclusion: STOEP通过整合隐式和显式先验知识，有效解决了现有疫情预测方法的局限性，在预测精度和实际部署方面都取得了显著进展，为公共卫生管理提供了更可靠的决策支持工具。
...
### [189] [X-REFINE: XAI-based RElevance input-Filtering and archItecture fiNe-tuning for channel Estimation](https://arxiv.org/abs/2602.22277)
*Abdul Karim Gizzini,Yahia Medjahdi*

Main category: cs.LG

TL;DR: X-REFINE是一个基于XAI的框架，通过联合输入滤波和架构微调来解决6G无线通信中深度学习模型的黑盒性和高复杂度问题，实现更好的可解释性-性能-复杂度权衡。
...
Method: 提出X-REFINE框架，采用基于分解的、符号稳定的LRP epsilon规则，通过反向传播预测来推导子载波和隐藏神经元的高分辨率相关性分数，从而实现联合输入滤波和架构微调的整体优化。

Result: 仿真结果表明，X-REFINE实现了优越的可解释性-性能-复杂度权衡，显著降低了计算复杂度，同时在不同场景下保持了稳健的误码率性能。

Conclusion: X-REFINE框架通过XAI驱动的联合优化方法，为6G无线通信中的AI模型部署提供了有效的解决方案，平衡了可解释性、性能和计算效率的需求。
...
### [190] [Integrating Machine Learning Ensembles and Large Language Models for Heart Disease Prediction Using Voting Fusion](https://arxiv.org/abs/2602.22280)
...
TL;DR: 该研究比较了传统机器学习模型与大型语言模型在心血管疾病预测中的表现，发现机器学习集成方法（95.78%准确率）优于LLMs（78.9%准确率），而两者融合的混合方法取得了最佳效果（96.62%准确率）。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，需要早期识别、精确风险分类和可靠的决策支持技术。尽管传统机器学习模型在复杂非线性患者数据建模方面表现出色，但大型语言模型的出现提供了新的零样本和少样本推理能力，研究者希望探索两者结合是否能提升心血管疾病预测性能。

Method: 使用包含1,190条患者记录的合并数据集，比较传统机器学习模型（包括Random Forest、XGBoost、LightGBM、CatBoost等集成方法）与通过OpenRouter API访问的开源大型语言模型。最终提出并评估了机器学习集成与LLM推理的混合融合方法，其中Gemini 2.5 Flash模型表现最佳。

Result: 机器学习集成方法取得了最高性能（95.78%准确率，ROC-AUC 0.96），而LLMs在零样本设置下表现中等（78.9%准确率），在少样本设置下稍差（72.6%准确率）。提出的混合方法（ML集成+LLM推理）获得了最佳结果（96.62%准确率，0.97 AUC），表明LLMs与ML模型结合使用时效果最佳。

Conclusion: 虽然机器学习集成被认为是结构化表格预测的最佳方案，但可以与混合ML-LLM系统集成以提供小幅性能提升，并为开发更可靠的临床决策支持工具开辟道路。混合方法在不确定情况下增强了预测能力，展示了结合两种技术优势的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 临床研究中剂量错误是常见问题，可能导致患者伤害和试验失败。目前缺乏在试验启动前预测剂量错误风险的系统方法，需要开发可扩展的机器学习框架进行早期风险分层，支持主动的质量管理。

Method: 从ClinicalTrials.gov构建包含42,112个临床试验的数据集。提取结构化、半结构化试验数据和协议相关的非结构化自由文本数据。使用不良事件报告、MedDRA术语和Wilson置信区间为试验分配二元标签（高剂量错误率）。评估三种模型：基于结构化特征的XGBoost模型、使用文本数据的ClinicalModernBERT模型，以及结合两种模态的简单后期融合模型。应用事后概率校准以实现可解释的试验级风险分层。

Result: 后期融合模型获得最高的AUC-ROC（0.862）。校准输出能够将试验稳健地分层到预定义的风险类别中。标记为具有过高剂量错误率的试验比例在较高预测风险组中单调增加，并与相应的预测概率范围一致。

Conclusion: 该研究提出了一个可重复且可扩展的机器学习框架，用于对高风险剂量错误率的临床试验进行早期试验级风险分层，支持临床研究中的主动、基于风险的质量管理。剂量错误风险可以在试验启动前使用预启动信息进行预测，概率校准对于将模型输出转化为可靠且可解释的风险类别至关重要。
...
### [192] [Reliable XAI Explanations in Sudden Cardiac Death Prediction for Chagas Cardiomyopathy](https://arxiv.org/abs/2602.22288)
...
TL;DR: 该研究将具有正确性保证的逻辑可解释性方法应用于恰加斯心肌病患者的猝死预测，解决了AI模型作为黑箱的问题，实现了高精度和完全可解释性。


<details>
  <summary>Details</summary>
Motivation: 恰加斯心肌病患者的猝死预测具有挑战性，尤其是对非高风险患者。现有AI模型虽然能改善风险分层，但缺乏透明度，被视为黑箱，且一些启发式解释方法缺乏正确性保证，导致决策错误。

Method: 应用具有正确性保证的逻辑可解释性方法到恰加斯心肌病猝死预测中。该方法应用于一个准确率和召回率超过95%的AI分类器，实现了高预测性能和100%解释保真度。

Result: 该方法展示了强大的预测性能，与最先进的启发式方法相比，表现出更优的一致性和鲁棒性。解释保真度达到100%，准确率和召回率超过95%。
...
<details>
  <summary>Details</summary>
Motivation: 现有概念漂移框架主要基于分布变化，无法区分生理信号中良性的能量波动与真实的概念变化，导致深度模型对幅度、频率或形态的无害变化产生过度反应，产生不稳定预测

Method: 提出生理能量守恒理论(PECT)，认为在虚拟漂移下，归一化潜在位移应与归一化信号能量变化成比例；开发能量约束表示学习(ECRL)正则化器，惩罚能量不一致的潜在移动

Result: 在七种单模态和混合模型上的实验表明，最强三模态混合模型(1D+2D+Transformer)在保持清洁准确率(96.0%到94.1%)的同时，扰动准确率显著提升(72.6%到85.5%)，融合表示漂移减少超过45%

Conclusion: PECT作为能量漂移定律，有效管理连续生理信号中的概念稳定性，ECRL正则化器在不改变编码器架构或增加推理成本的情况下，显著提升模型对生理合理波动的鲁棒性
...
Method: 将机械故障检测建模为离线逆强化学习问题，使用对抗逆强化学习训练判别器来区分正常（专家）和策略生成的转移。判别器学习的奖励作为异常分数，指示与正常运行行为的偏差。
...
### [195] [Learning geometry-dependent lead-field operators for forward ECG modeling](https://arxiv.org/abs/2602.22367)
*Arsenii Dokuchaev,Francesca Bonizzoni,Stefano Pagani,Francesco Regazzoni,Simone Pezzuto*

Main category: cs.LG

TL;DR: 提出一种基于形状信息的导联场算子替代模型，用于高效、高精度的前向心电图仿真，解决了传统方法在解剖精度、数据需求和计算效率方面的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统前向心电图计算模型面临三个主要挑战：1）临床实践中难以获得完整躯干的高解剖精度表示；2）导联场方法的计算成本随电极数量线性增长，限制了高密度记录的应用；3）现有方法无法同时实现高解剖保真度、低数据需求和计算效率。

Method: 提出一个两组件框架：1）几何编码模块，将解剖形状映射到低维潜在空间；2）几何条件神经替代模型，根据空间坐标、电极位置和潜在编码预测导联场梯度。该模型可作为全阶模型的即插即用替代品。

Result: 方法在躯干内部（平均角度误差5°）和心脏内部均实现高精度的导联场近似，心电图仿真相对均方误差<2.5%。替代模型持续优于广泛使用的伪导联场近似，同时保持可忽略的推理成本。紧凑的潜在表示使其无需完整躯干分割即可部署。

Conclusion: 提出的形状信息替代模型成功解决了前向心电图仿真中解剖精度、数据需求和计算效率的平衡问题，能够在数据有限的环境中实现高保真心电图仿真，为临床实践提供了实用解决方案。
...
Method: 提出背景对比非负矩阵分解（bNMF），通过联合分解目标数据集和匹配背景数据，使用共享非负基，在对比目标下抑制背景表达结构。采用高效的乘法更新算法，通过矩阵乘法实现GPU硬件高效计算，并支持类似深度学习方法的小批量训练以扩展到大数据。

Result: 在模拟和多种生物数据集中，bNMF揭示了传统方法掩盖的信号：包括死后抑郁脑单细胞RNA-seq中的疾病相关程序、小鼠中基因型相关的蛋白表达模式、白血病中治疗特异性转录变化，以及癌细胞系中TP53依赖的药物反应。

Conclusion: bNMF提供了一种可扩展且可解释的方法，通过对比目标数据集和匹配背景，提取目标特异性变异，解决了高维生物数据中信号被共享变异掩盖的问题。
...
<details>
  <summary>Details</summary>
Motivation: 抗菌素耐药性（特别是多重耐药性）对临床决策构成严峻挑战，传统药敏测试耗时且治疗选择有限，需要开发能够早期识别MDR并提供可解释预测的方法

Method: 开发可解释机器学习框架，使用临床特征和抗生素敏感性模式预测MDR。评估五种分类模型（逻辑回归、随机森林、AdaBoost、XGBoost、LightGBM），在9,714个分离株数据集上训练，将耐药性编码到抗生素家族层面以捕获交叉耐药模式。应用LIME进行实例级解释

Result: 集成模型（特别是XGBoost和LightGBM）在所有评估指标（准确率、F1分数、AUC-ROC、马修斯相关系数）上表现最佳。LIME识别出喹诺酮类、复方新诺明、多粘菌素、氨基糖苷类和呋喃类耐药性为MDR预测的最强贡献因素，与已知生物学机制一致

Conclusion: 将高性能模型与局部可解释性相结合，为抗菌药物管理提供准确且可操作的见解。该框架支持早期MDR识别，增强对机器学习辅助临床决策支持的信任，有助于改善抗菌药物管理
...
TL;DR: MolFM-Lite是一个多模态分子性质预测模型，通过交叉注意力融合SELFIES序列（1D）、分子图（2D）和构象集合（3D）表示，并使用FiLM整合实验上下文信息。
...
Method: 1) 构象集合注意力机制：结合可学习注意力与Boltzmann加权先验，处理多个RDKit生成的构象以捕捉分子形状的热力学分布；2) 交叉模态融合层：各模态可通过注意力相互关注，实现互补信息共享；3) 使用FiLM整合实验上下文；4) 在ZINC250K上进行跨模态对比和掩码原子目标的预训练。

Result: 在四个MoleculeNet支架分割基准测试中，三模态融合相比单模态基线提供7-11%的AUC提升，构象集合相比单构象变体增加约2%的改进。全面的消融研究证实每个架构组件独立贡献。

Conclusion: MolFM-Lite通过多模态融合和构象集合建模有效提升了分子性质预测性能，证明了整合1D、2D、3D表示以及考虑分子构象动态分布的重要性。模型代码、训练权重和数据分割已开源以确保可复现性。
...
### [199] [A Learning-Based Hybrid Decision Framework for Matching Systems with User Departure Detection](https://arxiv.org/abs/2602.22412)
*Ruiqi Zhou,Donghao Zhu,Houcai Shen*

Main category: cs.LG

TL;DR: 提出基于学习的混合匹配框架，自适应结合即时与延迟匹配，通过动态调整策略在贪婪与耐心策略间灵活插值，减少等待时间和拥堵，同时仅牺牲有限匹配效率


<details>
  <summary>Details</summary>
Motivation: 在肾脏交换和货运交换等匹配市场中，延迟匹配能提高整体效率，但延迟的收益高度依赖参与者的停留时间和离开行为，且延迟匹配会带来等待时间增加和市场拥堵等显著成本。这些竞争效应使得固定匹配策略在动态环境中缺乏灵活性。

Method: 提出基于学习的混合框架，自适应结合即时与延迟匹配。框架持续收集用户离开时间数据，通过回归估计底层离开分布，基于控制系统对匹配效率损失容忍度的决策阈值，决定后续时期是否延迟匹配。

Result: 该框架能显著减少等待时间和拥堵，同时仅牺牲有限的匹配效率。通过动态调整匹配策略，混合框架使系统性能能在纯贪婪和纯耐心策略之间灵活插值。

Conclusion: 该混合框架为静态匹配机制提供了鲁棒且自适应的替代方案，能够在动态环境中平衡匹配效率与等待成本，实现更优的市场表现。
...
### [200] [Calibrated Test-Time Guidance for Bayesian Inference](https://arxiv.org/abs/2602.22428)
*Daniel Geyfman,Felix Draxler,Jan Groeneveld,Hyunsoo Lee,Theofanis Karaletsos,Stephan Mandt*

Main category: cs.LG

TL;DR: 本文提出了一种新的测试时引导方法，用于从贝叶斯后验分布中进行校准采样，解决了现有方法只关注奖励最大化而忽视正确后验分布的问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时引导方法主要关注最大化奖励函数，而不是从真实的贝叶斯后验分布中采样，这导致了校准错误的推理结果。作者发现常见测试时引导方法无法恢复正确的后验分布，并识别了导致这种失败的结构性近似问题。

Method: 作者提出了具有一致性的替代估计器，能够实现从贝叶斯后验分布中进行校准采样。该方法修正了现有测试时引导方法的结构性近似问题，确保能够正确恢复后验分布。

Result: 在贝叶斯推理任务上显著优于先前方法，并在黑洞图像重建任务上达到了最先进的性能水平。

Conclusion: 通过提出一致的替代估计器，本文解决了测试时引导方法中的后验校准问题，为扩散模型的贝叶斯推理提供了更可靠的方法，在多个任务上展现了优越性能。
...
### [201] [From Bias to Balance: Fairness-Aware Paper Recommendation for Equitable Peer Review](https://arxiv.org/abs/2602.22438)
*Uttamasha Anjally Oyshi,Susan Gauch*

Main category: cs.LG

TL;DR: Fair-PaperRec：一种在双盲评审后通过公平性正则化重新排序论文的MLP模型，能在保持质量的同时显著提高代表性不足群体的参与度


<details>
  <summary>Details</summary>
Motivation: 尽管采用双盲评审，系统性偏见仍使代表性不足群体处于劣势。研究假设：在评审后推荐系统中加入显式公平性正则化，可以在不降低质量的前提下提高包容性。

Method: 提出Fair-PaperRec，一个多层感知机（MLP）模型，在交叉属性（如种族、国家）上使用可微公平性损失函数，在双盲评审后对论文进行重新排序。先在合成数据集（高、中、接近公平的偏见水平）上验证假设，然后在真实会议数据（ACM SIGCHI、DIS、IUI）上应用。

Result: 合成数据集实验显示，增加公平性权重能增强宏观/微观多样性，同时保持效用稳定。真实场景中，适当调优的Fair-PaperRec配置使代表性不足群体参与度提高42.03%，整体效用变化最多3.16%。

Conclusion: 公平性正则化既能作为公平机制，也能作为温和的质量正则化器，特别是在高度偏见的环境中。Fair-PaperRec提供了一个实用、以公平为中心的评审后论文选择框架，在保持甚至在某些情况下增强学术质量的同时促进公平。
...
### [202] [Efficient Continual Learning in Language Models via Thalamically Routed Cortical Columns](https://arxiv.org/abs/2602.22479)
*Afshin Khadangi*

Main category: cs.LG

TL;DR: TRC²是一种用于持续学习的解码器架构，通过丘脑路由皮质柱结构解决语言模型在非平稳数据下的灾难性遗忘问题，实现稳定性与可塑性的更好平衡。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在持续学习场景下面临挑战：在线更新会导致灾难性遗忘，而提高稳定性的方法通常会增加延迟、内存占用或密集计算，难以扩展到长上下文场景。

Method: 提出TRC²架构，结合稀疏丘脑路由皮质柱、调制、预测、记忆和反馈机制，以及快速校正通路。该架构采用稀疏块并行设计，支持高效训练和推理，同时保持各子系统的清晰可分离性。

Result: 在语言建模和持续学习基准测试中，TRC²在相同计算量下改善了稳定性-可塑性权衡，支持快速在线适应同时保留先前习得的行为。

Conclusion: TRC²通过在架构层面解决持续学习问题，为语言模型提供了更有效的持续学习能力，平衡了适应新数据与保留旧知识的需求。
...
### [203] [Reinforcement-aware Knowledge Distillation for LLM Reasoning](https://arxiv.org/abs/2602.22495)
*Zhaoyang Zhang,Shuli Jiang,Yantao Shen,Yuting Zhang,Dhananjay Ram,Shuo Yang,Zhuowen Tu,Wei Xia,Stefano Soatto*

Main category: cs.LG

TL;DR: 提出RLAD方法，通过选择性模仿解决RL后训练中知识蒸馏的分布不匹配和目标冲突问题，用TRRD替代KL正则化，在逻辑推理和数学基准上表现优异


<details>
  <summary>Details</summary>
Motivation: 强化学习后训练虽然提升了长链推理大语言模型的性能，但推理成本高需要蒸馏到更小的学生模型。现有知识蒸馏方法主要针对监督微调设计，与强化学习结合时存在分布不匹配和目标冲突问题：教师监督可能与学生不断演化的rollout分布不对齐，KL正则化可能与奖励最大化竞争并需要仔细的损失平衡。

Method: 提出RL感知蒸馏（RLAD），在强化学习过程中进行选择性模仿——只有当教师指导能改进当前策略更新时才引导学生向教师学习。核心组件是信任区域比率蒸馏（TRRD），用基于PPO/GRPO风格的似然比目标替代教师-学生KL正则化，该目标锚定在教师-旧策略混合上，在学生rollout上实现优势感知、信任区域有界的蒸馏，自然平衡探索、利用和模仿。

Result: 在多样化的逻辑推理和数学基准测试中，RLAD始终优于离线蒸馏、标准GRPO以及基于KL的在线教师-学生知识蒸馏方法。

Conclusion: RLAD通过选择性模仿和TRRD机制有效解决了强化学习后训练中知识蒸馏的分布不匹配和目标冲突问题，为将大型推理模型蒸馏到更小模型提供了一种更有效的强化学习感知蒸馏方法。
...
### [204] [TEFL: Prediction-Residual-Guided Rolling Forecasting for Multi-Horizon Time Series](https://arxiv.org/abs/2602.22520)
*Xiannan Huang,Shen Fang,Shuhan Qiu,Chengcheng Yu,Jiayuan Du,Chao Yang*

Main category: cs.LG

TL;DR: TEFL（时间误差反馈学习）是一个统一的深度学习框架，通过将历史预测残差纳入训练和评估过程来改进时间序列预测精度。
...
### [205] [Predicting Tennis Serve directions with Machine Learning](https://arxiv.org/abs/2602.22527)
*Ying Zhu,Ruthuparna Naikar*

Main category: cs.LG

TL;DR: 开发机器学习方法预测职业网球选手一发方向，平均准确率男49%、女44%，分析显示顶级选手采用混合策略，疲劳影响发球方向选择，情境信息对回球方预判很重要


<details>
  <summary>Details</summary>
Motivation: 职业网球中发球（尤其是一发）至关重要，发球方需策略性选择发球方向以最大化获胜机会并保持不可预测性，而接发球方则试图预测发球方向以做出良好回击。双方的心理博弈是职业网球比赛决策的重要组成部分，需要理解选手的发球决策模式。

Method: 通过特征工程开发机器学习方法来预测职业网球选手的一发方向。该方法利用各种特征来建模选手的发球决策模式。

Result: 该方法实现了平均预测准确率：男性选手约49%，女性选手约44%。分析提供证据表明顶级职业选手在发球决策中使用混合策略模型，疲劳可能是选择发球方向的一个因素。分析还表明情境信息对回球方的预判反应可能比先前认为的更为重要。

Conclusion: 机器学习方法能够有效预测职业网球选手的发球方向，揭示了顶级选手采用混合策略、疲劳影响发球决策等关键发现。情境信息在回球预判中的重要性被重新评估，为理解网球比赛中的心理博弈和决策制定提供了新见解。
...
TL;DR: 本文提出FaiDPO（φ-DPO）框架，用于解决大型多模态模型持续学习中的公平性问题，通过改进直接偏好优化来处理数据不平衡导致的偏见。


<details>
  <summary>Details</summary>
Motivation: 当前持续学习研究主要关注灾难性遗忘问题，但数据分布不平衡导致的公平性问题在大型多模态模型中尚未得到充分探索。传统DPO方法在数据不平衡情况下存在局限性，需要新的方法来同时解决遗忘和公平性问题。

Method: 提出基于直接偏好优化的新型持续学习范式，首先利用成对偏好信号对齐学习以缓解灾难性遗忘；然后针对传统DPO在数据不平衡中的局限性，提出新的φ-DPO损失函数，显式处理分布偏差；为支持φ-DPO持续学习，为现有基准构建了成对偏好标注。

Result: 实验表明φ-DPO在多个基准测试中达到最先进性能，优于现有的大型多模态模型持续学习方法。理论分析证明该方法能同时解决遗忘和数据不平衡问题。

Conclusion: FaiDPO（φ-DPO）框架成功解决了大型多模态模型持续学习中的公平性问题，通过改进的偏好优化方法有效处理了数据不平衡导致的偏见，为持续学习领域提供了新的研究方向。
...
### [207] [Persistent Nonnegative Matrix Factorization via Multi-Scale Graph Regularization](https://arxiv.org/abs/2602.22536)
*Jichao Zhang,Ran Miao,Limin Li*

Main category: cs.LG

TL;DR: 提出持久非负矩阵分解(pNMF)，一种尺度参数化的NMF方法，通过持久同调识别底层连接结构发生定性变化的规范尺度，生成序列化的嵌入而非单一分解。


<details>
  <summary>Details</summary>
Motivation: 现有NMF方法是单尺度的，无法捕捉连接结构在不同分辨率下的演化。需要一种能够处理多尺度数据表示的方法。

Method: 利用持久同调识别规范尺度集，构建尺度诱导的图拉普拉斯序列，提出具有尺度几何正则化和跨尺度一致性约束的耦合NMF公式，开发序列交替优化算法。

Result: 在合成数据和单细胞RNA测序数据集上的数值实验表明，该方法在多尺度低秩嵌入方面具有有效性。

Conclusion: pNMF通过生成尺度参数化的嵌入序列，解决了传统NMF的单尺度局限性，为多尺度数据表示提供了新框架。
...
TL;DR: 提出DP-aware AdaLN-Zero，一种针对条件扩散变换器的敏感性感知条件机制，通过限制条件表示幅度和AdaLN调制参数来抑制极端梯度尾部事件，从而在差分隐私SGD下提升条件扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私随机梯度下降（DP-SGD）中，异构条件上下文（如观测历史、缺失模式或异常协变量）会引发重尾的每样本梯度。这些罕见但由条件驱动的重尾梯度会不成比例地触发全局裁剪，导致异常值主导的更新、更大的裁剪偏差，以及在固定隐私预算下降低的效用。

Method: 提出DP-aware AdaLN-Zero，这是一种即插即用的敏感性感知条件机制，用于条件扩散变换器。它通过有界重新参数化联合约束条件表示幅度和AdaLN调制参数，在梯度裁剪和噪声注入之前抑制极端梯度尾部事件。该方法不修改DP-SGD机制本身。

Result: 在匹配的隐私设置下，配备DP-aware AdaLN-Zero的DP-SGD在插值/填补和预测任务上表现更优。在一个真实世界电力数据集和两个公共ETT基准测试上，相比原始DP-SGD观察到一致的性能提升。梯度诊断将这些改进归因于条件特定的尾部重塑和减少的裁剪失真，同时在非私有训练中保持了表达能力。

Conclusion: 敏感性感知条件机制可以显著改善私有条件扩散训练，而无需牺牲标准性能。该方法通过抑制由条件引起的极端梯度尾部事件，缓解了DP-SGD中由异常值主导的更新和裁剪偏差问题。
...
<details>
  <summary>Details</summary>
Motivation: 科学机器学习模型设计需要大量先验知识和人工专业知识，特别是在确定输入特征和模型规模方面存在挑战。当前方法依赖手动调优，限制了SciML的民主化和广泛应用。

Method: LUMOS采用基于L0正则化学习的端到端框架，结合半随机门控和重参数化技术，在训练过程中动态选择信息特征并剪枝冗余参数，减少对人工调优的依赖。

Result: 在13个多样化SciML任务（包括宇宙学和分子科学）的评估中，LUMOS平均实现71.45%的参数减少和6.4倍的推理加速。在最多8个GPU上的分布式数据并行训练验证了其可扩展性。

Conclusion: LUMOS通过统一特征选择和模型剪枝的自动化框架，有效简化了SciML模型设计流程，在保持预测准确性的同时显著减少模型复杂度和计算开销，推动了科学机器学习的民主化应用。
...
### [210] [Moral Preferences of LLMs Under Directed Contextual Influence](https://arxiv.org/abs/2602.22831)
*Phil Blandfort,Tushar Karayil,Urja Pawar,Robert Graham,Alex McKenzie,Dmitrii Krasheninnikov*

Main category: cs.LG

TL;DR: 研究LLM在道德困境中的上下文影响：通过翻转上下文方向系统测量模型决策如何被引导，发现上下文显著改变决策，基线偏好不能预测可引导性，影响可能适得其反，推理减少平均敏感性但放大偏见示例影响


<details>
  <summary>Details</summary>
Motivation: 现有LLM道德基准通常使用无上下文提示，假设偏好稳定。但在实际部署中，提示常包含用户请求、社会规范线索等上下文信号，可能引导决策。需要研究定向上下文影响如何重塑道德困境中的决策

Method: 引入针对电车问题式道德分类场景的定向上下文影响评估框架：对每个人口统计因素，应用匹配的、方向翻转的上下文影响（仅在偏袒群体上不同），实现方向性响应的系统测量

Result: 发现：(1)上下文影响常显著改变决策，即使仅表面相关；(2)基线偏好不能预测方向可引导性，模型可能基线中立但在影响下表现出系统可引导性不对称；(3)影响可能适得其反：模型可能声称中立或忽略上下文线索，但选择仍改变，有时方向相反；(4)推理减少平均敏感性，但放大偏见少样本示例的影响

Conclusion: 研究结果支持将道德评估扩展到包含受控的、方向翻转的上下文操作，以更好地表征模型行为。需要超越简单的基线偏好测量，系统评估上下文影响下的决策变化
...
### [211] [RAIN-Merging: A Gradient-Free Method to Enhance Instruction Following in Large Reasoning Models with Preserved Thinking Format](https://arxiv.org/abs/2602.22538)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理链方面表现出色，但在遵循输出格式、约束或特定要求等指令方面存在不足。研究旨在探索是否可以通过将指令调优模型集成到大型推理模型中来弥补这一差距。
...
### [212] [Relatron: Automating Relational Machine Learning over Relational Databases](https://arxiv.org/abs/2602.22552)
*Zhikai Chen,Han Xie,Jian Zhang,Jiliang Tang,Xiang Song,Huzefa Rangwala*

Main category: cs.LG

TL;DR: 该研究系统比较了关系深度学习（RDL）与深度特征合成（DFS）在关系数据库预测建模中的性能，发现两者优劣取决于具体任务，并提出基于任务信号的元选择器Relatron来指导模型选择。


<details>
  <summary>Details</summary>
Motivation: 关系数据库预测建模面临捕获跨表依赖和复杂特征交互的挑战。虽然RDL方法通过消息传递自动化特征工程，而DFS依赖预定义聚合器，但两者比较优势不明确，缺乏有效的架构选择原则。

Method: 将RDL和DFS统一到共享设计空间，进行架构中心搜索；构建模型性能库；提出RDB任务同质性和亲和嵌入两个任务信号；开发基于任务嵌入的元选择器Relatron，结合轻量级损失景观指标避免脆弱检查点。

Result: 发现：1）RDL不总是优于DFS，性能高度任务依赖；2）无单一架构主导所有任务；3）验证精度不可靠。Relatron解决了"更多调优，更差性能"效应，在联合超参-架构优化中比强基线提升18.5%，成本比基于Fisher信息的替代方案低10倍。

Conclusion: 关系数据库预测建模需要任务感知的模型选择。提出的任务信号和Relatron元选择器提供了原则性路由机制，能够有效指导RDL与DFS的选择并优化架构搜索，显著提升性能同时降低成本。
...
### [213] [Multilingual Safety Alignment Via Sparse Weight Editing](https://arxiv.org/abs/2602.22554)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在不同语言间存在显著安全差异，低资源语言往往能绕过为高资源语言（如英语）建立的安全防护。现有解决方案（如多语言监督微调或RLHF）计算成本高且依赖稀缺的多语言安全数据。

Method: 基于稀疏权重编辑的无训练对齐框架。识别安全能力集中在稀疏的安全神经元中，将跨语言对齐问题形式化为约束线性变换问题。推导闭式解，将低资源语言的有害表示最优映射到高资源语言的鲁棒安全子空间，同时通过零空间投影约束保持通用效用。

Result: 在8种语言和多个模型家族（Llama-3、Qwen-2.5）上的广泛实验表明，该方法显著降低了低资源语言的攻击成功率，对通用推理能力影响可忽略，仅需一次数据高效计算。

Conclusion: 提出的稀疏权重编辑框架为跨语言安全对齐提供了高效、数据节约的解决方案，无需额外训练即可显著提升低资源语言的安全防护能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前天气分析和预报缺乏米尺度近地表大气条件的空间变异性，这种变异性可能由地表特征和大尺度大气强迫决定，但尚不清楚其是否可预测。

Method: 通过将粗粒度大气状态条件化于稀疏地面站测量和高分辨率地球观测数据，推断出美国大陆10米分辨率的近地表风、温度和湿度连续场。

Result: 相比ERA5，推断场将风误差降低29%，温度和露点误差降低6%，在固定时间步长上解释了更多空间方差，并显示出城市热岛、蒸散发驱动湿度对比等物理可解释结构。

Conclusion: 研究表明米尺度近地表天气存在可统计恢复的物理相干分量，通过将粗粒度动力模型条件化于静态细尺度特征，可以揭示地球系统中先前未解析的组分，扩展了天气建模的前沿。
...
### [215] [Autoregressive Visual Decoding from EEG Signals](https://arxiv.org/abs/2602.22555)
...
Method: 1) 使用预训练EEG模型LaBraM，通过对比学习微调对齐EEG与图像表示；2) 采用"下一尺度预测"的自回归生成框架，将图像编码为多尺度token映射，以EEG嵌入为最粗表示，自回归预测更细尺度token

Result: 在两个数据集上，AVDE在图像检索和重建任务中均优于先前SOTA方法，参数量仅需10%；中间输出可视化显示生成过程反映了人类视觉感知的层次性

Conclusion: 自回归模型可作为高效且可解释的工具，用于实际BCI应用，AVDE框架在保持EEG信号与重建图像直接联系的同时实现了连贯生成
...
### [216] [A Dataset is Worth 1 MB](https://arxiv.org/abs/2602.23358)
*Elad Kimchi Shoshani,Leeyam Gabay,Yedid Hoshen*

Main category: cs.LG

TL;DR: PLADA方法通过仅传输类别标签而非像素数据来大幅降低数据集传输成本，利用预加载的通用参考数据集和标签筛选机制实现高效任务知识传递
...
Method: 提出PLADA方法：假设客户端预加载大型通用无标签参考数据集（如ImageNet），仅传输目标任务的类别标签。引入剪枝机制筛选参考数据集中与目标任务最语义相关的图像标签，最大化训练效率同时最小化传输负载
...
### [217] [Stable Adaptive Thinking via Advantage Shaping and Length-Aware Gradient Regulation](https://arxiv.org/abs/2602.22556)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理低复杂度查询时经常表现出过思考行为，现有方法在准确性与效率的权衡上不稳定，且对异构推理行为的鲁棒性较差。

Method: 两阶段框架：1) 混合微调使模型同时接触思考和不思考行为，建立良好初始化；2) 自适应强化学习，包含保持正确性的优势塑形和长度感知梯度调节，以稳定优化。

Result: 在Qwen2.5-1.5B和7B模型上实验显示，相比强基线准确率提升最高达+3.7/+3.6点，同时生成token减少40.6%/43.9%。在不同难度问题和分布外任务上验证了方法的鲁棒性和泛化能力。

Conclusion: 提出的两阶段框架有效解决了大型推理模型的过思考问题，实现了稳定准确性与效率的平衡，具有良好鲁棒性和泛化性。
...
### [218] [Operationalizing Fairness: Post-Hoc Threshold Optimization Under Hard Resource Limits](https://arxiv.org/abs/2602.22560)
...
<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署机器学习需要在预测安全性和算法公平性之间取得平衡。现有公平性干预措施通常假设资源不受约束，并使用违反反歧视法规的群体特定决策阈值。

Method: 引入一个后处理、模型无关的阈值优化框架，通过参数化伦理损失函数和边界决策规则，在严格容量约束下联合平衡安全性、效率和公平性，强制使用单一全局决策阈值以确保法律合规性。

Result: 容量约束主导伦理优先级，严格资源限制在超过80%的测试配置中决定了最终部署的阈值。在25%的严格容量限制下，该框架成功保持了高风险识别（召回率0.409-0.702），而标准的无约束公平性启发式方法则崩溃到接近零的效用。

Conclusion: 理论公平性目标必须明确服从于运营容量限制才能保持部署可行性。通过将预测评分与政策评估解耦并严格限制干预率，该框架为利益相关者在资源受限环境中导航不可避免的伦理权衡提供了实用且法律合规的机制。
...
### [219] [S2O: Early Stopping for Sparse Attention via Online Permutation](https://arxiv.org/abs/2602.22575)
*Yu Zhang,Songwei Liu,Chenqian Yan,Sheng Lin,Beichen Ning,Fangmin Chen,Xing Wang*

Main category: cs.LG

TL;DR: S2O通过在线排列和提前停止机制提升稀疏注意力效率，突破现有块粒度稀疏化的稀疏度上限，显著减少长上下文推理的计算开销。
...
Method: S2O采用在线排列和提前停止机制：1) 借鉴内存系统的虚拟到物理地址映射，重新分解FlashAttention执行，使推理能够加载非连续token；2) 将显式排列转化为在线、索引引导的离散加载策略，通过轻量级预处理和索引重映射将重要性集中在高优先级块；3) 基于重要性引导的在线排列，引入提前停止规则：从高到低重要性计算，当当前块分数低于阈值时提前终止并跳过剩余低贡献块。

Result: 在Llama-3.1-8B的128K上下文长度下：1) 在相同稀疏度下，单算子MSE降低3.82倍；2) 在相同MSE下，预填充计算密度降低3.31倍；3) 保持端到端精度，注意力部分加速7.51倍，端到端整体加速3.81倍。

Conclusion: S2O通过在线排列和提前停止机制，显著提高了稀疏注意力的实际稀疏度上限，在控制误差预算的同时大幅减少计算量，为长上下文推理提供了有效的解决方案。
...
### [220] [IBCircuit: Towards Holistic Circuit Discovery with Information Bottleneck](https://arxiv.org/abs/2602.22581)
...
Method: 基于信息瓶颈原理的端到端优化框架IBCircuit，能够整体性识别信息丰富的电路，可应用于任何给定任务而无需繁琐的激活扰动设计

Result: 在间接宾语识别（IOI）和大于关系（Greater-Than）任务中，IBCircuit识别出的电路在关键节点组件和边组件方面比近期相关工作更忠实和最小化

Conclusion: IBCircuit提供了一种无需任务特定激活扰动设计的整体性电路发现方法，能够更准确、高效地识别语言模型中的计算子图
...
### [221] [TabDLM: Free-Form Tabular Data Generation via Joint Numerical-Language Diffusion](https://arxiv.org/abs/2602.22586)
*Donghong Cai,Jiarui Feng,Yanbo Wang,Da Zheng,Yixin Chen,Muhan Zhang*

Main category: cs.LG

TL;DR: TabDLM是一个统一的自由形式表格数据生成框架，通过基于掩码扩散语言模型的联合数值-语言扩散模型，同时处理文本、分类和数值特征。


<details>
  <summary>Details</summary>
Motivation: 现实世界表格数据越来越多地包含自由形式的文本字段（如评论或临床记录）以及结构化的数值和分类属性。生成这种异构表格并联合建模不同模态具有挑战性。现有方法主要分为两类：扩散模型能捕捉数值和分类特征的复杂依赖关系，但扩展到开放文本困难且文本质量下降；LLM方法能自然生成流畅文本，但其离散分词会扭曲精确或大范围的数值，阻碍准确建模数字和语言。

Method: TabDLM是一个统一的自由形式表格数据生成框架，通过基于掩码扩散语言模型的联合数值-语言扩散模型。该方法通过掩码扩散建模文本和分类特征，同时通过学习的专用数值标记嵌入对数值特征进行连续扩散过程建模；双向注意力在单个模型内捕捉跨模态交互。

Result: 在多样化基准测试上的广泛实验证明了TabDLM相比强大的基于扩散和LLM的基线方法的有效性。

Conclusion: TabDLM提供了一个统一的框架，能够有效生成包含自由形式文本、分类和数值特征的异构表格数据，解决了现有方法在跨模态建模方面的局限性。
...
TL;DR: pQuant提出了一种新的极低位量化训练方法，通过将线性层拆分为1位主导分支和高精度分支来解耦参数，解决了参数民主化效应导致的表达能力受限问题。


<details>
  <summary>Details</summary>
Motivation: 现有极低位（低于2位）量化训练方法在准确性和可扩展性方面仍不理想，主要瓶颈是参数民主化效应——所有参数的敏感性趋于同质化，严重限制了模型的表达能力。

Method: 提出pQuant方法：1）将线性层拆分为两个专门分支：1位主导分支用于高效计算，紧凑高精度分支用于保留最敏感参数；2）通过特征缩放引导模型将敏感参数分配到高精度分支；3）将高精度分支扩展为多个稀疏激活的专家，实现高效容量扩展。

Result: 大量实验表明，pQuant在极低位量化方面达到了最先进的性能。

Conclusion: pQuant通过解耦参数和专门化分支设计，有效解决了极低位量化中的参数民主化问题，为实现高效边缘部署的大语言模型提供了有前景的解决方案。
...
### [223] [Transformers converge to invariant algorithmic cores](https://arxiv.org/abs/2602.22600)
*Joshua S. Schiffman*

Main category: cs.LG

TL;DR: 该研究提出"算法核心"概念，发现不同训练运行的Transformer模型在内部权重配置上收敛到相同的紧凑子空间结构，这些核心结构对任务性能是必要且充分的


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型内部工作机制面临根本障碍：训练选择的是行为而非电路结构，许多权重配置可以实现相同功能。需要区分哪些内部结构反映计算本质，哪些是特定训练运行的偶然产物

Method: 提取算法核心——对任务性能必要且充分的紧凑子空间。分析独立训练的Transformer权重，研究Markov-chain transformer的3D核心，观察模块化加法transformer在grokking过程中的变化，分析GPT-2的主谓一致机制

Result: 1) 独立训练的transformer学习不同权重但收敛到相同核心；2) Markov-chain transformer在近乎正交的子空间中嵌入3D核心但恢复相同转移谱；3) 模块化加法transformer在grokking时发现紧凑循环算子，后期膨胀；4) GPT-2通过单一轴控制主谓一致，翻转该轴可在所有尺度上反转语法数

Conclusion: Transformer计算围绕紧凑、共享的算法结构组织，这些低维不变量在不同训练运行和尺度上持续存在。机制可解释性应针对这些计算本质（算法核心）而非实现特定细节
...
### [224] [Mitigating Membership Inference in Intermediate Representations via Layer-wise MIA-risk-aware DP-SGD](https://arxiv.org/abs/2602.22611)
*Jiayang Meng,Tao Huang,Chen Hou,Guolong Zheng,Hong Chen*

Main category: cs.LG

TL;DR: LM-DP-SGD：一种层感知的差分隐私SGD方法，根据各层的成员推理攻击风险自适应分配隐私保护，在相同隐私预算下实现更好的隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 在嵌入即接口（EaaI）场景中，预训练模型的中间表示（IRs）会泄露训练集成员信息，使成员推理攻击（MIAs）风险在不同层间存在差异。传统DP-SGD采用统一的噪声乘数和逐样本梯度裁剪，忽略了层间MIA风险的异质性。

Method: 提出层感知MIA风险感知DP-SGD（LM-DP-SGD）：1）在公共影子数据集上训练影子模型；2）从训练/测试分割中提取各层IRs；3）拟合层特定的MIA攻击器，用攻击错误率估计MIA风险；4）利用MIA的跨数据集可迁移性，在私有训练中根据风险重新加权各层对全局裁剪梯度的贡献，在固定噪声幅度下提供层适当的保护。

Result: 在相同隐私预算下，LM-DP-SGD显著降低了峰值IR级MIA风险，同时保持了模型效用，实现了更优的隐私-效用权衡。理论分析证明了LM-DP-SGD的隐私保证和收敛性。

Conclusion: LM-DP-SGD通过层感知的隐私保护分配机制，有效解决了传统DP-SGD中忽略层间MIA风险差异的问题，为EaaI场景提供了更精细化的隐私保护方案。
...
### [225] [Semantic Tube Prediction: Beating LLM Data Efficiency with JEPA](https://arxiv.org/abs/2602.22617)
*Hai Huang,Yann LeCun,Randall Balestriero*

Main category: cs.LG

TL;DR: 本文提出Geodesic Hypothesis和Semantic Tube Prediction (STP)任务，通过几何先验约束隐藏状态轨迹，使LLM仅需1/16训练数据即可达到基线精度，直接挑战Chinchilla缩放定律的数据效率边界。


<details>
  <summary>Details</summary>
Motivation: 现有LLM缩放定律是描述性的而非规定性的，它们刻画典型训练而非最优训练。很少有工作成功挑战这些定律所隐含的数据效率边界，这是本文的主要关注点。

Method: 提出Geodesic Hypothesis，认为token序列在平滑语义流形上追踪测地线，因此具有局部线性。基于此提出Semantic Tube Prediction (STP)任务，这是一种JEPA风格的正则化器，将隐藏状态轨迹限制在测地线的管状邻域内。STP将JEPA推广到语言领域，无需显式的多视图增强。

Result: STP约束提高了信噪比，并在推理过程中通过防止轨迹碰撞来保持多样性。在NL-RX-SYNTH数据集上，STP使LLM仅需1/16的训练数据即可匹配基线精度，直接违反了Chinchilla风格缩放定律的数据项。

Conclusion: 有原则的几何先验可以超越暴力缩放，通过引入Geodesic Hypothesis和STP任务，成功挑战了LLM缩放定律的数据效率边界，展示了更高效训练的可能性。
...
### [226] [ContextRL: Enhancing MLLM's Knowledge Discovery Efficiency with Context-Augmented RL](https://arxiv.org/abs/2602.22623)
...
TL;DR: ContextRL框架通过上下文增强解决强化学习中的可识别性和可达性问题，使用完整参考方案进行细粒度过程验证，并采用多轮采样策略引导策略从失败尝试中恢复正确响应。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习中的两个关键瓶颈：可识别性（区分高质量与低质量推理过程）和可达性（从失败尝试中恢复正确响应），以提升知识发现效率并缓解奖励黑客问题。

Method: 1) 为奖励模型提供完整参考方案作为上下文，实现细粒度过程验证以过滤假阳性；2) 引入多轮采样策略，奖励模型为失败尝试生成错误报告，引导策略从全负样本组中恢复正确响应。

Result: 在11个感知和推理基准测试中显著提升知识发现效率，使Qwen3-VL-8B模型达到与32B模型相当的性能，大幅超越标准RLVR基线，有效缓解奖励黑客问题。
...
### [227] [Tackling Privacy Heterogeneity in Differentially Private Federated Learning](https://arxiv.org/abs/2602.22633)
...
<details>
  <summary>Details</summary>
Motivation: 现有差分隐私联邦学习方法通常假设所有客户端共享统一的隐私预算，但实际场景中隐私需求存在显著差异。这种隐私异质性带来了挑战：传统基于数据量的客户端选择策略无法区分提供高质量更新的客户端和因严格隐私约束引入大量噪声的客户端。
...
Conclusion: 研究结果强调了将隐私异质性纳入客户端选择对于实现实用有效的联邦学习的重要性，为解决实际场景中隐私需求差异问题提供了系统解决方案。
...
### [228] [Compress the Easy, Explore the Hard: Difficulty-Aware Entropy Regularization for Efficient LLM Reasoning](https://arxiv.org/abs/2602.22642)
*Qin-Wen Luo,Sheng Ren,Xiang Chen,Rui Liu,Jun Fang,Naiqiang Tan,Sheng-Jun Huang*

Main category: cs.LG

TL;DR: CEEH方法通过难度感知的熵正则化和动态长度惩罚，在保持推理能力的同时压缩CoT响应长度，解决现有压缩方法因熵塌缩导致的推理能力下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有CoT压缩方法（自训练、带长度约束的强化学习）在追求简短响应时往往牺牲推理能力，主要问题是显式优化短轨迹会触发快速熵塌缩，过早缩小探索空间，阻碍发现有效推理路径，特别是在需要大量推导的难题上。

Method: 提出CEEH（Compress responses for Easy questions and Explore Hard ones），一种基于强化学习的难度感知高效推理方法：1）动态评估实例难度，应用选择性熵正则化——对难题保持多样化搜索空间确保鲁棒性，对易题允许激进压缩；2）引入基于历史最短正确响应的动态最优长度惩罚，抵消熵诱导的长度膨胀并稳定奖励信号。

Result: 在六个推理基准测试中，CEEH持续减少响应长度，同时保持与基础模型相当的准确性，并相对于仅优化长度的方法提高了Pass@k指标。

Conclusion: CEEH通过难度感知的熵正则化和动态长度惩罚，有效解决了CoT压缩中的熵塌缩问题，在保持推理能力的同时实现了响应长度的大幅减少，为LLM在实际部署中的高效推理提供了可行方案。
...
### [229] [MUG: Meta-path-aware Universal Heterogeneous Graph Pre-Training](https://arxiv.org/abs/2602.22645)
*Lianze Shan,Jitao Zhao,Dongxiao He,Yongqi Huang,Zhiyong Feng,Weixiong Zhang*

Main category: cs.LG

TL;DR: 提出MUG方法，用于异构图的通用预训练，解决异构图中类型多样性和元路径差异带来的挑战，通过输入统一化和共享编码器实现跨图对齐。


<details>
  <summary>Details</summary>
Motivation: 当前通用图预训练主要关注同构图，而异构图的通用预训练尚未探索。异构图具有更复杂的结构和语义，面临两大挑战：1) 多样类型和数据集特定语义阻碍统一表示空间的构建；2) 元路径的数量和语义在不同数据集中差异大，使得编码和聚合模式难以跨数据集迁移。
...
Conclusion: MUG成功解决了异构图的通用预训练挑战，通过输入统一化和共享编码器实现了跨图对齐，为异构图表示学习提供了有效的预训练框架。
...
### [230] [LEDA: Latent Semantic Distribution Alignment for Multi-domain Graph Pre-training](https://arxiv.org/abs/2602.22660)
*Lianze Shan,Jitao Zhao,Dongxiao He,Siqi Liu,Jiaxu Cui,Weixiong Zhang*

Main category: cs.LG

TL;DR: LEDA是一种用于通用图预训练的新型潜在语义分布对齐模型，通过维度投影单元和变分语义推理模块解决现有方法在数据对齐和训练指导方面的局限性，在跨域场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有通用图预训练方法面临两个主要挑战：1）简单数据对齐无法处理高度多样化的图数据，导致语义错位；2）将域内预训练范式任意应用于跨域场景，难以从多个图中捕获有效知识。这些问题限制了模型学习丰富且可泛化知识的能力。

Method: 提出LEDA模型：1）维度投影单元自适应地将不同域特征对齐到共享语义空间，最小化信息损失；2）变分语义推理模块获取共享潜在分布；3）利用该分布指导域投影，确保跨域语义学习的一致性。

Result: LEDA在广泛的图数据和下游任务中表现出强大性能。在少样本跨域设置中，显著优于域内基线和先进的通用预训练模型。

Conclusion: LEDA通过有效的潜在语义分布对齐机制，成功解决了通用图预训练中的语义对齐和跨域知识学习问题，为图表示学习提供了更强大的通用预训练框架。
...
### [231] [Forecasting Antimicrobial Resistance Trends Using Machine Learning on WHO GLASS Surveillance Data: A Retrieval-Augmented Generation Approach for Policy Decision Support](https://arxiv.org/abs/2602.22673)
*Md Tanvir Hasan Turja*

Main category: cs.LG

TL;DR: 该研究开发了一个两组件框架，用于预测抗菌素耐药性趋势并提供基于证据的政策决策支持，使用WHO GLASS数据和机器学习模型，XGBoost表现最佳，并实现了结合政策文档的RAG系统。
...
Method: 提出了一个两组件框架：1）AMR趋势预测：在WHO GLASS数据（2021-2023年，5909个观测值，覆盖6个WHO区域）上基准测试了6个模型（Naive、线性回归、岭回归、XGBoost、LightGBM、LSTM）；2）政策决策支持：实现了检索增强生成（RAG）管道，将WHO政策文档存储在ChromaDB向量数据库中，与本地部署的Phi-3 Mini语言模型结合，生成有来源引用、幻觉受限的政策答案。

Result: XGBoost模型表现最佳，测试MAE为7.07%，R平方为0.854，比朴素基线提高了83.1%。特征重要性分析显示前一年耐药率是最重要的预测因子（50.5%重要性）。区域MAE范围从4.16%（欧洲区域）到10.14%（东南亚区域）。RAG系统能够生成有来源引用的政策答案。

Conclusion: 该研究成功开发了一个有效的AMR趋势预测和政策决策支持框架，XGBoost模型在预测准确性方面表现优异，RAG系统为政策制定提供了基于证据的支持工具。代码和数据已开源，有助于推动AMR监测和干预措施的制定。
...
### [232] [Accelerating LLM Pre-Training through Flat-Direction Dynamics Enhancement](https://arxiv.org/abs/2602.22681)
...
<details>
  <summary>Details</summary>
Motivation: LLM预训练需要巨大计算资源，优化器效率至关重要。现有矩阵优化器（如Muon、SOAP）虽然利用细粒度曲率信息优于AdamW，但其更新趋向各向同性——沿平坦方向相对保守，而沿陡峭方向可能过于激进，限制了训练效率。
...
Conclusion: LITE为高效LLM预训练提供了原则性方法，通过针对性地增强平坦方向的训练动态，解决了现有矩阵优化器的各向同性限制，在理论和实验上都证明了其加速效果。
...
### [233] [Switch-Hurdle: A MoE Encoder with AR Hurdle Decoder for Intermittent Demand Forecasting](https://arxiv.org/abs/2602.22685)
...
<details>
  <summary>Details</summary>
Motivation: 间歇性需求（长时间零销售与零星非零值交替出现）在零售和供应链预测中持续存在挑战。传统方法（ARIMA、指数平滑、Croston变体）和现代神经架构（DeepAR、Transformer）通常表现不佳，因为它们将需求视为单一连续过程，或在处理大量稀疏序列时计算成本过高。

Method: Switch-Hurdle框架集成了混合专家编码器和基于障碍的概率解码器。编码器在前向传播中使用稀疏Top-1专家路由，但通过直通估计器在反向传播中实现近似密集路由。解码器采用交叉注意力自回归设计，共享障碍头将预测任务明确分为两个组件：估计销售概率的二元分类组件和给定销售时预测数量的条件回归组件。

Result: 在M5基准测试和大型专有零售数据集上的实证结果表明，Switch-Hurdle实现了最先进的预测性能，同时保持了可扩展性。

Conclusion: Switch-Hurdle通过结构化分离销售发生和数量预测过程，有效捕捉间歇性需求的内在特征，为解决间歇性需求预测问题提供了高效且可扩展的解决方案。
...
TL;DR: HGPO提出了一种分层组策略优化方法，通过将轨迹中的步骤分配到多个历史上下文一致的分层组中，解决步进式组策略优化中的上下文不一致问题，从而改善优势估计偏差。


<details>
  <summary>Details</summary>
Motivation: 现有步进式组策略优化方法在处理长视野智能体任务时存在上下文不一致问题：同一组内的步骤可能具有不同的历史上下文，这会导致严重偏置的优势估计，从而显著降低策略优化效果。

Method: HGPO将轨迹组中的每个步骤根据历史上下文一致性分配到多个分层组中，然后在每个组内为每个步骤计算不同的优势值，并通过自适应加权方案进行聚合，实现优势估计的偏差-方差权衡优化。

Result: 在ALFWorld和WebShop两个具有挑战性的智能体任务上，使用Qwen2.5-1.5B-Instruct和Qwen2.5-7B-Instruct模型进行评估，HGPO在相同计算约束下显著优于现有的智能体强化学习方法。

Conclusion: HGPO通过分层组分配和自适应优势聚合有效解决了步进式组策略优化中的上下文不一致问题，无需额外模型或轨迹采样即可实现更好的偏差-方差权衡，从而提升长视野智能体任务的性能。
...
### [235] [Enhancing Geometric Perception in VLMs via Translator-Guided Reinforcement Learning](https://arxiv.org/abs/2602.22703)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在几何推理方面存在困难，主要是因为它们对基本图表元素的感知能力有限。现有方法难以将几何感知与推理能力分开评估和提升。

Method: 1) 提出GeoPerceive基准测试，包含图表实例和领域特定语言表示，配备高效自动数据生成流水线；2) 提出GeoDPO框架，使用在GeoPerceive合成数据上训练的NL-to-DSL翻译器，将自然语言转换为DSL，计算细粒度DSL级分数作为强化学习的奖励信号。

Result: GeoDPO相比监督微调(SFT)取得显著提升：在域内数据上+26.5%，在域外数据上+8.0%，在下游推理任务上+39.0%。SFT仅提供边际改进，在域外场景中甚至可能损害性能。

Conclusion: GeoDPO在几何感知和下游推理任务上表现出卓越的性能和泛化能力，优于监督微调方法。GeoPerceive基准测试为评估几何感知能力提供了独立于推理的评估框架。
...
TL;DR: MEDNA-DFM是一个高性能的DNA甲基化预测模型，结合了机制启发的信号净化算法，不仅能准确预测甲基化，还能提取保守的生物学模式，提出了"序列-结构协同"假说。


<details>
  <summary>Details</summary>
Motivation: 深度学习在DNA甲基化识别中表现出色，但其"黑箱"特性阻碍了生物学洞见的获取。需要开发既能保持高性能又能提供生物学解释的模型。

Method: 开发了MEDNA-DFM模型和机制启发的信号净化算法。通过跨物种验证、外部独立数据集测试、基序提取和计算机突变实验验证模型的可解释性和生物学意义。

Result: MEDNA-DFM能有效捕获保守的甲基化模式，在多种物种中实现稳健区分。模型泛化能力由保守的内在基序（如GC含量）驱动而非系统发育邻近性。提取的基序可靠性显著高于先前研究。果蝇6mA案例支持"序列-结构协同"假说。

Conclusion: 该工作提供了强大的甲基化预测工具，展示了可解释深度学习如何推动方法创新和生物学假说生成，特别是提出的"序列-结构协同"机制为理解甲基化调控提供了新视角。
...
TL;DR: NoRA是一种非线性秩适应方法，通过SiLU门控和结构化dropout打破LoRA的线性瓶颈，在复杂推理任务中实现更高的谱效率
...
Result: 在SlimOrca基准测试中，NoRA在秩64时（PPL 3.89）优于LoRA在秩512时（PPL 3.90）；在数学推理中，NoRA在MathInstruct上达到PPL 1.97，显著优于LoRA的饱和点2.07

Conclusion: NoRA通过激活奇异值谱的休眠尾部，有效防止线性方法中观察到的秩崩溃，打破了LoRA的线性瓶颈，实现了更高的谱效率和性能
...
### [238] [Multi-agent imitation learning with function approximation: Linear Markov games and beyond](https://arxiv.org/abs/2602.22810)
*Luca Viano,Till Freihaut,Emanuele Nevali,Volkan Cevher,Matthieu Geist,Giorgia Ramponi*

Main category: cs.LG

TL;DR: 首次对线性马尔可夫博弈中的多智能体模仿学习进行理论分析，提出基于特征层面的集中系数替代状态-动作层面的集中系数，并设计了首个计算高效的交互式MAIL算法，在Tic-Tac-Toe和Connect4游戏中超越行为克隆。


<details>
  <summary>Details</summary>
Motivation: 多智能体模仿学习（MAIL）在理论分析方面存在空白，特别是在线性马尔可夫博弈中。现有方法依赖于状态-动作层面的"所有策略偏差集中系数"，该系数可能过大且难以满足。需要开发更高效的理论框架和算法来降低样本复杂度。

Method: 1. 利用线性马尔可夫博弈的结构（转移动态和奖励函数在给定特征下线性），将状态-动作层面的集中系数替换为特征层面的集中系数；2. 提出首个计算高效的交互式MAIL算法，其样本复杂度仅依赖于特征映射维度d；3. 基于理论发现开发深度MAIL交互算法。

Result: 1. 理论证明特征层面的集中系数可以远小于状态-动作层面的集中系数（当特征能有效表示状态相似性时）；2. 交互式MAIL算法的样本复杂度仅依赖于特征维度d，避免了集中系数的需求；3. 深度MAIL算法在Tic-Tac-Toe和Connect4游戏中明显优于行为克隆（BC）。

Conclusion: 该工作首次为线性马尔可夫博弈中的多智能体模仿学习提供了理论分析框架，通过特征层面的集中系数和交互式算法显著降低了样本复杂度要求，并在实际游戏中验证了方法的有效性，为MAIL的理论和算法发展奠定了基础。
...
### [239] [Exploratory Memory-Augmented LLM Agent via Hybrid On- and Off-Policy Optimization](https://arxiv.org/abs/2602.23008)
...
<details>
  <summary>Details</summary>
Motivation: 探索是基于强化学习训练的大型语言模型智能体的关键瓶颈。现有方法依赖预训练知识，但在需要发现新状态的环境中表现不佳。
...
Conclusion: EMPO²是一个有前景的框架，可用于构建更具探索性和泛化能力的LLM智能体。
...
TL;DR: DyGnROLE：一种基于Transformer的动态图架构，通过分离源节点和目标节点表示，结合角色语义位置编码和时间对比链接预测预训练，在动态图学习中实现角色感知建模。
...
Method: 提出DyGnROLE架构：1）使用独立的嵌入词汇表分离源节点和目标节点表示；2）引入角色语义位置编码捕获每个角色独特的结构和时序上下文；3）提出时间对比链接预测（TCLP）自监督预训练目标，利用未标记交互历史编码信息结构偏置。

Result: 在未来边分类任务评估中，DyGnROLE显著优于多种最先进的基线方法，确立了角色感知建模作为动态图学习的有效策略。

Conclusion: 角色感知建模是动态图学习的重要方向，DyGnROLE通过显式解耦源节点和目标节点表示，结合专门的嵌入和预训练策略，在低标签场景下实现了优异性能。
...
### [241] [Fair feature attribution for multi-output prediction: a Shapley-based perspective](https://arxiv.org/abs/2602.22882)
*Umberto Biccari,Alain Ibáñez de Opakua,José María Mato,Óscar Millet,Roberto Morales,Enrique Zuazua*

Main category: cs.LG

TL;DR: 该论文通过将经典Shapley公理扩展到向量值合作博弈，证明了任何满足效率、对称性、虚拟玩家和可加性的归因规则必须按输出分量分解，揭示了多输出预测中Shapley归因的结构约束。


<details>
  <summary>Details</summary>
Motivation: 现有SHAP解释通常独立计算每个输出坐标的归因，但这种做法的理论必要性尚不明确。论文旨在为多输出预测器的特征归因提供公理化表征，澄清Shapley框架下的结构约束。

Method: 将经典Shapley公理（效率、对称性、虚拟玩家、可加性）扩展到向量值合作博弈，建立刚性定理证明任何满足这些公理的归因规则必须按分量分解。通过数值实验在生物医学基准上验证理论结果。

Result: 证明了任何满足四个经典Shapley公理的归因规则必须按输出分量分解，因此任何联合输出归因规则必须至少放松一个经典公理。数值实验显示多输出模型在训练和部署中能带来计算节省，同时产生与Shapley公理强加的组件结构完全一致的SHAP解释。

Conclusion: 该研究揭示了Shapley基础可解释性中先前未形式化的结构约束，阐明了多输出学习中公平一致解释的精确范围，为多输出模型的SHAP解释提供了理论基础。
...
### [242] [Scaling Laws of Global Weather Models](https://arxiv.org/abs/2602.22962)
*Yuejiang Yu,Langwen Huang,Alexandru Calotoiu,Torsten Hoefler*

Main category: cs.LG

TL;DR: 该论文分析了天气预测数据驱动模型的缩放规律，发现Aurora模型具有最强的数据缩放能力，GraphCast参数效率最高但硬件利用率有限，计算最优分析表明延长训练时间比增加模型规模更有效，天气模型与语言模型不同更倾向于宽度而非深度。


<details>
  <summary>Details</summary>
Motivation: 随着数据驱动模型在天气预测领域的革命性发展，需要优化训练效率和模型性能。本文旨在通过实证分析该领域的缩放规律，研究模型性能与模型规模、数据集大小和计算预算之间的关系，为未来天气模型的设计提供指导。

Method: 研究分析了多个天气预测模型，包括Aurora和GraphCast等，通过系统实验探究验证损失与三个关键因素（模型规模N、数据集大小D、计算预算C）之间的关系。进行了计算最优分析，比较了在固定计算预算下不同资源分配策略的效果，并分析了模型形状（宽度与深度）对性能的影响。

Result: Aurora模型展现出最强的数据缩放行为：训练数据集增加10倍可使验证损失降低高达3.2倍。GraphCast具有最高的参数效率，但硬件利用率有限。计算最优分析表明，在固定计算预算下，将资源分配给更长的训练时间比增加模型规模能获得更大的性能提升。模型形状分析显示天气预测模型与语言模型存在根本差异：天气模型始终倾向于增加宽度而非深度。

Conclusion: 未来天气预测模型应优先考虑更宽的架构和更大的有效训练数据集，以最大化预测性能。研究揭示了天气预测领域特有的缩放规律，为模型设计和资源分配提供了实证依据，表明该领域需要不同于语言模型的优化策略。
...
<details>
  <summary>Details</summary>
Motivation: 标准混合精度训练中每个参数需要大量内存（参数、梯度和优化器状态各需4字节），使得在有限内存资源下训练大模型（如70亿参数）变得困难

Method: 提出两种关键技术：1）改进的主权重分割技术，通过寻找和利用量化误差的紧界；2）设计压缩扩展函数，大幅减少8位优化器状态量化的误差。结合16位梯度，实现内存优化

Result: 将AdamW的内存占用从每个参数16字节减少到7字节（或梯度释放时为5字节），模型检查点大小减少一半以上。在SGD、AdamW和Lion优化器上应用FlashOptim，在标准视觉和语言基准测试（包括Llama-3.1-8B微调）中未观察到质量下降

Conclusion: FlashOptim提供了一套有效的内存优化技术，显著减少训练大模型所需的内存资源，同时保持模型质量，为资源受限的研究人员提供了可行的解决方案
...
### [244] [Learning Disease-Sensitive Latent Interaction Graphs From Noisy Cardiac Flow Measurements](https://arxiv.org/abs/2602.23035)
*Viraj Patel,Marko Grujic,Philipp Aigner,Theodor Abart,Marcus Granegger,Deblina Bhattacharjee,Katharine Fraser*

Main category: cs.LG

TL;DR: 提出物理信息驱动的潜在关系框架，将心脏涡流建模为图中的交互节点，用于量化疾病严重程度和干预水平。


<details>
  <summary>Details</summary>
Motivation: 当前成像和计算方法未能捕捉心脏血流模式中相干流动特征的关系结构，而这些模式包含丰富的疾病严重程度和临床干预信息。

Method: 结合神经关系推断架构与物理启发的交互能量和生死动力学，构建对疾病严重程度和干预水平敏感的潜在图模型。

Result: 在主动脉缩窄CFD模拟中，随着主动脉半径变窄，涡流相互作用增强且更频繁，图熵单调相关于缩窄严重程度（R²=0.78，Spearman |ρ|=0.96）。在左心室超声数据中，该方法能捕捉涡流结构的减弱，展示跨模态泛化能力。

Conclusion: 潜在交互图和熵可作为心脏疾病和干预的鲁棒且可解释的生物标志物，该框架能有效量化血流模式中的关系结构。
...
### [245] [SOTAlign: Semi-Supervised Alignment of Unimodal Vision and Language Models via Optimal Transport](https://arxiv.org/abs/2602.23353)
...
<details>
  <summary>Details</summary>
Motivation: 探索是否能用更少的监督实现有意义的模态对齐。现有方法依赖对比损失和数百万配对样本，而本文研究在少量图像-文本配对数据和大量未配对数据下对齐预训练单模态编码器的可行性。

Method: 提出SOTAlign两阶段框架：1）使用线性教师从有限配对数据中恢复粗略共享几何结构；2）基于最优传输的散度在未配对样本上细化对齐，转移关系结构而不过度约束目标空间。

Result: SOTAlign能有效利用未配对图像和文本，学习跨数据集和编码器对的鲁棒联合嵌入，在多个数据集和编码器组合上显著优于监督和半监督基线方法。

Conclusion: SOTAlign证明了在半监督设置下对齐预训练单模态编码器的可行性，为利用大量未配对数据实现高效跨模态对齐提供了新途径。
...
### [246] [RhythmBERT: A Self-Supervised Language Model Based on Latent Representations of ECG Waveforms for Heart Disease Detection](https://arxiv.org/abs/2602.23060)
*Xin Wang,Burcu Ozek,Aruna Mohan,Amirhossein Ravari,Or Zilbershot,Fatemeh Afghah*

Main category: cs.LG

TL;DR: RhythmBERT：将心电图视为结构化语言的生成式语言模型，通过符号化P、QRS、T波段捕获节律语义，在单导联上达到或超越12导联基线性能
...
Method: 提出RhythmBERT生成式ECG语言模型，将ECG视为语言范式：通过自编码器潜在表示将P、QRS、T波段编码为符号标记，离散标记捕获节律语义，连续嵌入保留细粒度形态。在约80万条未标记ECG记录上进行掩码预测预训练
...
### [247] [PRAC: Principal-Random Subspace for LLM Activation Compression and Memory-Efficient Training](https://arxiv.org/abs/2602.23111)
*Yanyi Li,Yimu Zhang,Cong Fang*

Main category: cs.LG

TL;DR: PRAC提出了一种基于主成分-随机子空间的LLM激活压缩方法，通过SVD保留主成分信息，随机采样正交补空间近似尾部信息，实现无偏梯度估计和最小方差，在预训练和微调任务中达到36%内存减少且性能损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 现有激活压缩方法未能充分利用激活的谱结构，导致收敛缓慢或压缩有限。激活已成为大批量LLM训练中的主要内存瓶颈，需要更有效的压缩方法。

Method: PRAC将激活分解为两个组件：通过SVD捕获的主子空间保留主导信息，以及从正交补空间采样的随机子空间近似尾部信息。引入精确缩放因子确保无偏梯度估计和最小方差。

Result: 在预训练和微调任务中，PRAC实现了高达36%的总内存减少，性能退化可忽略，计算成本最小。该方法在保持模型性能的同时显著降低了内存占用。

Conclusion: PRAC通过结合主成分分析和随机子空间采样，有效利用了激活的谱结构，提供了一种高效、无偏的LLM激活压缩方法，解决了大批量训练中的内存瓶颈问题。
...
TL;DR: 提出一种混合张量补全方法（TCM）预测二元混合物中无限稀释扩散系数的温度依赖性，结合贝叶斯训练框架和主动学习策略，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 混合物中扩散系数的预测在许多应用中至关重要，但实验数据稀缺。现有的矩阵补全方法（MCMs）仅限于单一温度预测，且准确性高度依赖于每个温度下高质量实验数据的可用性。需要开发能够预测温度依赖性扩散系数的方法。

Method: 提出混合张量补全方法（TCM），采用Tucker分解，在298K、313K和333K温度下二元体系无限稀释扩散系数的实验数据上进行联合训练。将半经验SEGWE模型的预测作为贝叶斯训练框架中的先验知识。TCM可线性外推到268K至378K之间的任何温度。通过主动学习（AL）策略扩展实验数据库，使用脉冲场梯度（PFG）NMR测量了19个溶质+溶剂体系在三个温度下的扩散系数。

Result: TCM在所有研究温度下相比现有模型显著提高了预测精度。通过主动学习获取新数据并纳入模型后，TCM的预测准确性得到实质性提升。该方法在268K至378K温度范围内实现了更准确的扩散系数预测。
...
### [249] [Benchmarking Temporal Web3 Intelligence: Lessons from the FinSurvival 2025 Challenge](https://arxiv.org/abs/2602.23159)
*Oshani Seneviratne,Fernando Spadea,Adrien Pavao,Aaron Micah Green,Kristin P. Bennett*

Main category: cs.LG

TL;DR: FinSurvival Challenge 2025作为时间Web3智能的基准研究案例，使用Aave v3协议的2180万笔交易记录，构建了16个生存预测任务来建模用户行为转换，展示了领域感知的时间特征构建显著优于通用建模方法。
...
Method: 使用Aave v3协议的2180万笔交易记录，设计了16个生存预测任务来建模用户行为转换。通过基准设计和获胜解决方案分析，重点研究了领域感知的时间特征构建方法。

Result: 领域感知的时间特征构建方法在生存预测任务中显著优于通用建模方法。Web3系统为研究流失、风险和演化等时间挑战提供了高保真沙盒环境。

Conclusion: FinSurvival Challenge 2025展示了Web3系统作为时间基准测试的理想平台，能够捕捉真实世界的时间动态。该研究为下一代时间基准设计提供了经验教训，并强调了领域知识在时间建模中的重要性。
...
### [250] [Induction Meets Biology: Mechanisms of Repeat Detection in Protein Language Models](https://arxiv.org/abs/2602.23179)
...
<details>
  <summary>Details</summary>
Motivation: 蛋白质序列中存在大量重复片段（精确和近似重复），这些重复对蛋白质结构和功能至关重要。虽然已有研究表明蛋白质语言模型（PLMs）能够识别这些重复，但其内部机制尚不清楚。

Method: 通过分析PLMs在掩码标记预测中的行为，研究其检测精确和近似重复的内部机制。具体包括：1）比较两种重复的检测机制；2）表征机制的两个主要阶段：特征表示构建和诱导头对齐。

Result: 发现近似重复的检测机制在功能上包含了精确重复的机制。PLMs通过两个阶段工作：首先使用通用位置注意力头和生物特化组件（如编码氨基酸相似性的神经元）构建特征表示；然后诱导头关注重复片段间的对齐标记以促进正确答案。

Conclusion: PLMs通过结合语言基础的模式匹配和专门的生物知识来解决这一生物任务，为研究PLMs中更复杂的进化过程奠定了基础。
...
Method: 提出从Hamilton-Jacobi-Bellman方程粘性解推导的物理信息正则化方法，利用Feynman-Kac定理将PDE解重新表述为期望，实现可处理的蒙特卡洛估计，避免高阶梯度数值不稳定

Result: 实验表明该方法提高了几何一致性，可广泛应用于导航和高维复杂操作任务

Conclusion: 通过基于最优控制理论的物理归纳偏置，该方法为学习过程提供了理论基础，明确正则化并限制了价值迭代中的更新
...
### [252] [Inferential Mechanics Part 1: Causal Mechanistic Theories of Machine Learning in Chemical Biology with Implications](https://arxiv.org/abs/2602.23303)
*Ilya Balabin,Thomas M. Kaiser*

Main category: cs.LG

TL;DR: 该论文系列旨在解决自然科学中机器学习模型缺乏因果结构考虑的问题，提出将化学理论、生物理论、概率论和因果关系相结合的新框架，以纠正当前机器学习在自然科学中的因果缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前自然科学中的机器学习模型通常被视为黑箱，缺乏对数据因果结构的深入考虑。虽然已有尝试将因果关系引入自然现象机器学习模型的讨论，但缺乏坚实统一的理论处理。该系列论文旨在填补这一空白，为化学生物学建立新的数学框架。

Method: 本论文（系列第一部分）提供了化学生物学现象基础因果结构的正式框架，并通过"焦点"这一新概念扩展到机器学习。"焦点"定义为机器学习算法在大数据集中缩小到隐藏基础机制的能力。论文还提供了在Akt抑制剂家族上的初步原理验证。

Result: 提出了化学生物学现象因果结构的正式框架，引入了"焦点"概念作为机器学习算法识别隐藏机制的能力。在Akt抑制剂家族上提供了初步原理验证，为后续研究奠定了基础。

Conclusion: 该系列论文旨在为化学生物学建立一种新的数学框架——推理力学，用于建模自然机制而无需还原论工具。第一部分建立了基础因果结构框架，后续部分将探讨化学相似性（第二部分）和隐藏因果结构如何削弱化学生物学中所有机器学习（第三部分）。
...
### [253] [A Proper Scoring Rule for Virtual Staining](https://arxiv.org/abs/2602.23305)
*Samuel Tonks,Steve Hood,Ryan Musso,Ceridwen Hopely,Steve Titus,Minh Doan,Iain Styles,Alexander Krull*

Main category: cs.LG

TL;DR: 提出信息增益（IG）作为细胞层面的评估框架，用于直接评估生成式虚拟染色模型预测的后验分布，解决了现有评估方法只能检查数据集边际分布准确性的局限性。


<details>
  <summary>Details</summary>
Motivation: 高通量筛选中的生成式虚拟染色模型能为每个输入和细胞提供可能的生物特征值的估计后验分布，但评估时真实后验不可得，现有评估协议只能检查数据集边际分布的准确性，无法评估预测的后验分布。

Method: 引入信息增益（IG）作为细胞层面的评估框架，IG是一种严格适当的评分规则，具有坚实的理论基础，允许结果的可解释性以及跨模型和特征的比较。

Result: 在广泛的高通量筛选数据集上评估基于扩散和GAN的模型，使用IG和其他指标，结果显示IG能揭示其他指标无法检测的显著性能差异。

Conclusion: 信息增益（IG）为生成式虚拟染色模型的评估提供了直接评估预测后验分布的有效框架，具有理论严谨性、可解释性和跨模型可比性，能揭示传统指标无法检测的性能差异。
