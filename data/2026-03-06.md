...
TL;DR: 该论文提出"具身智能经济学"，认为具身AI能力突破将颠覆百年不变的福特主义制造范式，通过能力空间临界点触发制造经济地理的相变，实现需求邻近的微制造和机器气候优势驱动的工厂选址重构。


<details>
  <summary>Details</summary>
Motivation: 自1913年福特流水线以来，制造业基本拓扑结构未发生范式级变革，过去一个世纪的所有创新（从丰田生产系统到工业4.0）都在福特主义范式内优化，未改变其集中化大型工厂、靠近劳动力池、规模化生产的结构逻辑。作者认为具身智能将打破这一百年停滞状态。

Method: 定义能力空间C = (d, g, r, t)，其中d代表灵巧性，g代表泛化能力，r代表可靠性，t代表触觉-视觉融合。通过分析能力向量跨越临界表面时选址目标函数的拓扑重组，提出三条路径：权重反转、批次崩溃和人力-基础设施解耦。引入"机器气候优势"概念，分析无人工厂选址的新逻辑。

Result: 具身智能将实现需求邻近的微制造，消除"制造沙漠"，逆转由劳动力套利驱动的地理集中。一旦移除人工工人，最优工厂位置将由机器最优条件（低湿度、高辐照度、热稳定性）决定，这些因素与传统选址逻辑正交，创造前所未有的生产地理格局。

Conclusion: 该论文建立了"具身智能经济学"这一新研究领域，研究物理AI能力阈值如何重塑生产的空间和结构逻辑。具身智能不是使现有工厂更高效，而是通过触发制造经济地理本身的相变，实现制造业范式的根本转变。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在均匀细化规则下生成文本，但实践中不同token以不同速率稳定，导致大量冗余细化，需要更精细的细化控制

Method: 提出渐进式细化调节（PRR）框架：1）从完整解码轨迹中推导token级经验收敛进度信号；2）基于该信号学习轻量级token级控制器；3）通过基于温度分布塑形的细化调节；4）采用渐进式自演化训练方案

Result: PRR显著加速了扩散语言模型解码，同时保持了生成质量

Conclusion: PRR框架通过轨迹基础的渐进式细化控制，有效解决了扩散语言模型中token收敛速率不同导致的冗余细化问题，实现了高效解码
...
Method: 提出新的多智能体计算数学发现模型，系统能够自主提出猜想并尝试证明，利用反馈和演化数据分布进行决策。以欧拉多面体猜想为灵感，使用多面体数据和线性代数知识作为基准任务。

Result: 系统成功完成了从多面体数据中自主恢复同调概念的学习任务。消融实验统计测试了完整动态的价值，结果表明优化局部过程的正确组合能够产生与数学趣味性高度一致的概念。

Conclusion: 优化局部过程的正确组合可以产生与数学趣味性惊人一致的概念，支持了数学发现可以通过动态过程相互作用实现的主要主张。
...
<details>
  <summary>Details</summary>
Motivation: LLM越来越多地集成到关键决策流程中，需要稳健的自动化数据分析。现有数据集风险分析方法局限于耗时复杂的手动审计，而完全基于AI的自动化分析存在幻觉和对齐问题

Method: 提出一个集成生成式AI与人类指导监督的框架：利用LLM识别数据库模式中的语义和结构属性，提出聚类技术，生成相应代码并解释结果；人类监督者指导模型分析方向，确保过程完整性和任务目标对齐

Result: 通过概念验证展示了该框架在风险评估任务中产生有意义结果的可行性

Conclusion: 该框架为未来自动化风险分析范式奠定了基础，通过人类监督的生成式AI方法平衡了自动化效率与结果可靠性
...
### [5] [Using Vision + Language Models to Predict Item Difficulty](https://arxiv.org/abs/2603.04670)
*Samin Khan*

Main category: cs.AI

TL;DR: 使用GPT-4.1-nano分析可视化素养测试题目难度，多模态方法（结合视觉和文本特征）预测效果最佳


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLMs）在确定数据可视化素养测试题目难度方面的能力，为心理测量分析和自动化题目开发提供新方法

Method: 使用GPT-4.1-nano分析测试题目，基于三种特征集预测题目难度：1）仅文本特征（问题和选项），2）仅视觉特征（可视化图像），3）多模态特征（结合视觉和文本）。通过均方误差和平均绝对误差评估预测性能

Result: 多模态方法表现最佳，平均绝对误差为0.224，优于仅视觉方法（0.282）和仅文本方法（0.338）。在独立测试集上，最佳多模态模型的均方误差为0.10805

Conclusion: LLMs在心理测量分析和自动化题目开发方面具有潜力，多模态方法结合视觉和文本特征能更准确地预测数据可视化素养测试题目的难度
...
### [6] [From Offline to Periodic Adaptation for Pose-Based Shoplifting Detection in Real-world Retail Security](https://arxiv.org/abs/2603.04723)
*Shanle Yao,Narges Rashvand,Armin Danesh Pazho,Hamed Tabkhi*

Main category: cs.AI

TL;DR: 提出一种基于姿态的无监督视频异常检测框架，用于零售店盗窃检测，支持边缘设备周期性自适应，并在真实数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 零售业盗窃事件增加导致经济损失，人工监控不可行，需要自动化、保护隐私、资源感知的检测解决方案。

Method: 将盗窃检测建模为基于姿态的无监督视频异常检测问题，提出周期性自适应框架，支持边缘设备从流式未标记数据中自适应，使用F1和H_PRS分数选择阈值。

Result: 在91.6%的评估中，框架在AUC-ROC和AUC-PR上优于离线基线，每次训练更新在边缘硬件上完成时间少于30分钟，证明了IoT部署的可行性。

Conclusion: 该框架为智能零售环境提供了可扩展、低延迟的异常检测解决方案，通过引入RetailS数据集支持可复现性，展示了在边缘设备上周期性自适应的实际可行性。
...
### [7] [Solving an Open Problem in Theoretical Physics using AI-Assisted Discovery](https://arxiv.org/abs/2603.04735)
*Michael P. Brenner,Vincent Cohen-Addad,David Woodruff*

Main category: cs.AI

TL;DR: AI系统结合神经符号方法成功解决了宇宙弦引力波功率谱的开放问题，推导出精确解析解，改进了先前AI辅助的渐近解
...
Method: 结合Gemini Deep Think大语言模型的神经符号系统，采用系统化树搜索框架和自动化数值反馈，通过6种不同解析方法分析核心积分I(N,α)

Result: 成功推导出宇宙弦引力波功率谱的精确解析解，最优雅的方法使用Gegenbauer多项式展开吸收积分奇异性，得到与数值结果一致的大N渐近解

Conclusion: AI能够自主解决理论物理中的开放问题，神经符号系统结合树搜索和数值反馈是有效的数学发现加速方法，结果与量子场论连续Feynman参数化相联系
...
### [8] [Interactive Benchmarks](https://arxiv.org/abs/2603.04737)
*Baoqing Yue,Zihan Zhu,Yifan Zhang,Jichen Feng,Hufei Yang,Mengdi Wang*

Main category: cs.AI

TL;DR: 论文提出交互式基准测试框架，通过预算约束下的交互过程评估模型推理能力，包括交互式证明和交互式游戏两种设置，结果显示现有模型在交互场景仍有很大提升空间。
...
Method: 提出交互式基准测试统一评估范式，在预算约束下通过交互过程评估模型推理能力。具体实现包括：1）交互式证明：模型与裁判交互以推导逻辑和数学中的客观真理；2）交互式游戏：模型进行策略推理以最大化长期效用。

Result: 交互式基准测试提供了对模型智能的稳健且忠实的评估，揭示了现有模型在交互场景中仍有显著的改进空间。

Conclusion: 交互式基准测试是评估模型智能的有效范式，能够更真实地反映模型在动态交互环境中的推理能力，为未来模型发展指明了方向。
...
### [9] [Visioning Human-Agentic AI Teaming: Continuity, Tension, and Future Research](https://arxiv.org/abs/2603.04746)
*Bowen Lou,Tian Lu,T. S. Raghu,Yingjie Zhang*

Main category: cs.AI

TL;DR: 论文探讨了智能体化AI系统带来的结构性不确定性如何挑战传统人机协同理论，提出需要重新思考团队态势感知理论以适应开放端行为轨迹和动态目标对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 智能体化AI系统具有开放端行为轨迹、生成性表征和输出、以及演化目标等特性，这些特性为人机协同引入了结构性不确定性。传统基于有限输出对齐的方法已不足够，需要在计划展开和优先级变化过程中持续维持对齐。

Method: 论文采用理论分析方法，分两个阶段展开：首先扩展团队态势感知理论，重新概念化人类和AI在开放端智能体性下的感知；其次审视传统稳定团队协同的动态过程在自适应自主性下是否继续有效。

Result: 区分了连续性区域和张力区域，明确了传统理论洞察在哪些方面仍然有效，在哪些方面结构性不确定性引入了压力。提出了人机协同的前瞻性研究议程。

Conclusion: 人机协同的核心挑战不在于人类和AI能否在当下达成一致，而在于它们能否在未来不断生成、修订、实施和治理的过程中保持对齐。需要发展能够适应动态生成性未来的对齐框架。
...
### [10] [HiMAP-Travel: Hierarchical Multi-Agent Planning for Long-Horizon Constrained Travel](https://arxiv.org/abs/2603.04750)
...
<details>
  <summary>Details</summary>
Motivation: 传统顺序LLM智能体在处理具有硬约束（如预算和多样性要求）的长时程规划任务时，随着规划进展和上下文增长，会逐渐偏离全局约束。需要新的框架来解决这一问题。

Method: 提出分层多智能体框架HiMAP-Travel：1) 战略协调器分配跨日资源；2) 并行日级执行器独立规划；3) 事务监控器强制执行预算和唯一性约束；4) 议价协议允许智能体拒绝不可行子目标并触发重新规划；5) 通过角色条件化的单一GRPO训练策略驱动所有智能体。

Result: 在TravelPlanner上，使用Qwen3-8B的HiMAP-Travel达到52.78%验证和52.65%测试最终通过率（FPR）。在相同模型、训练和工具条件下，比顺序DeepTravel基线提升+8.67个百分点，超过ATLAS +17.65个百分点和MTP +10.0个百分点。在FlexTravelBench多轮场景中，达到44.34%（2轮）和37.42%（3轮）FPR，并通过并行化降低延迟2.5倍。

Conclusion: HiMAP-Travel通过分层多智能体架构有效解决了长时程规划中的约束漂移问题，实现了更好的约束满足和规划质量，同时通过并行化显著降低了延迟。
...
### [11] [Breaking Contextual Inertia: Reinforcement Learning with Single-Turn Anchors for Stable Multi-Turn Interaction](https://arxiv.org/abs/2603.04783)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在单轮完整信息下表现出色，但在多轮交互中性能显著下降。当信息逐步揭示或需要更新时，模型难以整合新约束，出现"上下文惯性"现象——即模型僵化地坚持先前（可能错误的）推理路径，忽略后续的修正和新数据。

Method: 提出RLSTA（Reinforcement Learning with Single-Turn Anchors）方法：利用模型在单轮交互中的优异表现作为稳定内部锚点，提供奖励信号。通过将多轮响应与这些锚点对齐，使模型能够打破上下文惯性，基于最新信息进行自我校准推理。

Result: RLSTA显著优于标准微调和基于弃权的方法。该方法展现出强大的跨领域泛化能力（如从数学到代码），即使在没有外部验证器的情况下也有效，突显了其在通用领域应用的潜力。

Conclusion: RLSTA是一种通用且可推广的训练方法，能够稳定多轮交互性能。通过利用单轮能力作为锚点，模型能够克服上下文惯性，在信息逐步揭示的场景中保持鲁棒推理能力。
...
### [12] [Timer-S1: A Billion-Scale Time Series Foundation Model with Serial Scaling](https://arxiv.org/abs/2603.04791)
*Yong Liu,Xingjian Su,Shiyu Wang,Haoran Zhang,Haixuan Liu,Yuxuan Wang,Zhou Ye,Yang Xiang,Jianmin Wang,Mingsheng Long*

Main category: cs.AI

TL;DR: Timer-S1是一个8.3B参数的MoE时间序列基础模型，通过序列缩放、TimeBench数据集和序列令牌预测目标，在GIFT-Eval基准上达到SOTA预测性能。
...
Method: 采用序列缩放策略：1) 模型架构：结合稀疏TimeMoE块和通用TimeSTP块；2) 数据集：构建TimeBench语料库（1万亿时间点）并进行数据增强；3) 训练流程：使用序列令牌预测目标，并引入后训练阶段（持续预训练和长上下文扩展）。

Result: 在GIFT-Eval排行榜上，Timer-S1作为预训练模型取得了最佳的MASE和CRPS分数，达到了最先进的预测性能。

Conclusion: Timer-S1通过创新的序列缩放范式、高质量数据集和序列预测目标，成功构建了一个强大的时间序列基础模型，为后续研究提供了有力工具。
...
### [13] [EchoGuard: An Agentic Framework with Knowledge-Graph Memory for Detecting Manipulative Communication in Longitudinal Dialogue](https://arxiv.org/abs/2603.04815)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏结构化、长期记忆来追踪操纵性沟通（如煤气灯效应、情感胁迫）的微妙、上下文依赖策略，受限于有限上下文窗口和灾难性遗忘问题

Method: 引入EchoGuard框架，使用知识图谱作为智能体的核心情景和语义记忆，采用"记录-分析-反思"循环：1)用户记录交互，系统构建个人情景知识图谱；2)执行复杂图查询检测六种心理学基础的操纵模式；3)LLM基于检测到的模式子图生成针对性苏格拉底式提示

Result: 提出了理论基础、框架设计、全面评估策略和验证愿景，展示了智能体架构与知识图谱的协同作用如何帮助个体识别操纵性沟通

Conclusion: EchoGuard框架通过知识图谱与智能体架构的结合，能够在维护个人自主性和安全的同时，增强个体对操纵性沟通的识别能力
...
### [15] [On Multi-Step Theorem Prediction via Non-Parametric Structural Priors](https://arxiv.org/abs/2603.04852)
*Junbo Zhao,Ting Zhang,Can Li,Wei He,Jingdong Wang,Hua Huang*

Main category: cs.AI

TL;DR: 本文提出了一种基于上下文学习的免训练定理预测方法，通过定理优先图编码历史解轨迹的时序依赖关系，有效解决了结构漂移问题，在FormalGeo7k基准上达到89.29%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号方法主要依赖监督参数模型，对演化的定理库泛化能力有限。传统上下文学习在推理深度增加时性能急剧下降（结构漂移问题），源于LLM无法恢复潜在的拓扑依赖关系，导致非结构化探索。

Method: 提出定理优先图（Theorem Precedence Graphs），将历史解轨迹的时序依赖编码为有向图，施加显式拓扑约束以剪枝搜索空间。结合检索增强的图构建和逐步符号执行器，使LLM无需梯度优化即可作为结构化规划器。
...
### [16] [Causally Robust Reward Learning from Reason-Augmented Preference Feedback](https://arxiv.org/abs/2603.04861)
*Minjune Hwang,Yigit Korkmaz,Daniel Seita,Erdem Bıyık*

Main category: cs.AI

TL;DR: ReCouPLe：利用自然语言解释解决偏好学习中的因果混淆问题，通过语言解释作为嵌入空间的投影轴，使奖励模型关注与解释对齐的特征，而非虚假相关性，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基于偏好的奖励学习容易受到因果混淆的影响，学到的奖励往往会捕捉到与偏好轨迹共现的虚假特征，当这些相关性在测试时消失或反转时，模型性能会急剧下降。现有方法缺乏明确的因果信号来区分真正的偏好原因和虚假相关性。

Method: ReCouPLe框架将自然语言解释视为嵌入空间中的投影轴，训练模型基于与解释轴对齐的特征对轨迹进行评分，同时弱化与所述原因无关的上下文。该方法能够跨任务重用相同的因果方向（当任务共享语义时），并在无需额外数据或语言模型微调的情况下将偏好知识迁移到新任务。

Result: ReCouPLe在分布偏移下的奖励准确率比基线方法提升高达1.5倍，在新任务的下游策略性能提升高达2倍。学习到的奖励模型能够基于明确的原因来理解偏好，更好地与用户意图对齐，并超越虚假特征实现泛化。

Conclusion: 自然语言解释为偏好学习提供了有效的因果信号，ReCouPLe框架通过将解释作为嵌入空间的投影轴，解决了因果混淆问题，实现了跨任务的偏好知识迁移和更好的泛化性能。
...
### [17] [K-Gen: A Multimodal Language-Conditioned Approach for Interpretable Keypoint-Guided Trajectory Generation](https://arxiv.org/abs/2603.04868)
*Mingxuan Mu,Guo Yang,Lei Chen,Ping Wu,Jianxun Cui*

Main category: cs.AI

TL;DR: K-Gen：基于关键点的可解释多模态轨迹生成框架，结合MLLMs处理BEV地图和文本描述，通过关键点生成和强化学习微调提升自动驾驶仿真轨迹质量


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法依赖结构化数据（如矢量化地图），无法捕捉场景丰富的非结构化视觉上下文，限制了轨迹生成的现实性和多样性

Method: 提出K-Gen框架：1）使用多模态大语言模型统一处理栅格化BEV地图和文本场景描述；2）生成可解释的关键点及反映智能体意图的推理；3）通过细化模块将关键点转化为精确轨迹；4）应用轨迹感知强化学习微调算法T-DAPO优化关键点生成

Result: 在WOMD和nuPlan数据集上的实验表明，K-Gen优于现有基线方法，验证了多模态推理与关键点引导轨迹生成结合的有效性

Conclusion: K-Gen通过结合多模态理解和关键点引导的轨迹生成，为自动驾驶仿真提供了更现实、多样且可解释的轨迹生成方案，展示了MLLMs在复杂场景理解中的潜力
...
### [18] [SEA-TS: Self-Evolving Agent for Autonomous Code Generation of Time Series Forecasting Algorithms](https://arxiv.org/abs/2603.04873)
*Longkun Xu,Xiaochun Zhang,Qiantu Tuo,Rui Li*

Main category: cs.AI

TL;DR: SEA-TS是一个自主生成、验证和优化时间序列预测代码的框架，通过自进化循环实现自动化机器学习工程，在多个基准测试中显著超越现有方法和人工设计基线。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习开发面临新部署场景数据稀缺、分布漂移适应能力差、手动迭代收益递减等问题，需要自动化框架来提升时间序列预测的准确性和适应性。

Method: 提出SEA-TS框架，包含三个核心创新：1) Metric-Advantage Monte Carlo Tree Search (MA-MCTS)，使用归一化优势分数替代固定奖励；2) Code Review with running prompt refinement，自动审查执行方案并更新提示以编码纠正模式；3) Global Steerable Reasoning，通过对比全局最优和最差解决方案实现跨轨迹知识转移；采用MAP-Elites存档保持架构多样性。

Result: 在Solar-Energy基准测试中，SEA-TS生成的代码相比TimeMixer减少40% MAE；在专有数据集上，太阳能光伏预测WAPE降低8.6%，住宅负荷预测WAPE降低7.7%，负荷预测MAPE达到26.17% vs TimeMixer的29.34%；发现新颖架构模式如物理信息单调衰减头、站点特定日周期配置文件、可学习小时偏差校正等。

Conclusion: SEA-TS框架证明自主机器学习工程能够生成超越人工设计的新颖算法思想，为时间序列预测提供了一种有效的自动化解决方案，显著提升了预测性能并发现了新的架构模式。
...
### [19] [Bounded State in an Infinite Horizon: Proactive Hierarchical Memory for Ad-Hoc Recall over Streaming Dialogues](https://arxiv.org/abs/2603.04885)
*Bingbing Wang,Jing Li,Ruifeng Xu*

Main category: cs.AI

TL;DR: STEM-Bench是首个面向无限流对话的评估基准，ProStream通过主动分层记忆框架解决保真度-效率困境，在保持推理质量的同时降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现实世界对话通常以无限流的形式展开，需要有限状态记忆机制在无限时域内运行。现有的"先读后想"记忆机制与这种设置不匹配，无法在流展开时支持临时记忆检索。

Method: 提出STEM-Bench基准，包含超过14K个对话流QA对，评估感知保真度、时序推理和全局意识。为解决保真度-效率困境，提出ProStream主动分层记忆框架，通过多粒度蒸馏对连续流进行推理，采用自适应时空优化动态调整保留策略。

Result: 实验表明ProStream在准确性和效率方面均优于基线方法，实现了在保持推理保真度的同时降低推理延迟的目标。

Conclusion: STEM-Bench填补了流式对话评估的空白，ProStream通过主动分层记忆框架有效解决了无限流对话中的保真度-效率困境，为流式对话系统提供了实用的解决方案。
...
TL;DR: DP-MTV是首个支持多模态上下文学习的差分隐私框架，通过将数百个示例聚合为激活空间中的紧凑任务向量，实现形式化(ε,δ)-差分隐私，支持多轮推理查询。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医疗影像和个人照片等敏感领域应用日益增多，但现有的差分隐私上下文学习方法仅限于少样本、纯文本设置，因为隐私成本随处理的标记数量增加而增加。

Method: DP-MTV将私有数据划分为不相交的块，应用逐层裁剪来限制敏感度，并向聚合结果添加校准噪声，仅需单次噪声添加即可支持无限推理查询。

Result: 在三个VLM架构的八个基准测试中，ε=1.0时，DP-MTV在VizWiz上达到50%准确率，相比非私有的55%和零样本的35%，在有效隐私约束下保留了大部分上下文学习增益。

Conclusion: DP-MTV首次实现了多模态多样本上下文学习的差分隐私保护，通过任务向量聚合方法显著降低了隐私成本，为敏感领域视觉语言模型应用提供了实用的隐私保护解决方案。
...
### [21] [Authorize-on-Demand: Dynamic Authorization with Legality-Aware Intellectual Property Protection for VLMs](https://arxiv.org/abs/2603.04896)
*Lianyu Wang,Meng Wang,Huazhu Fu,Daoqiang Zhang*

Main category: cs.AI

TL;DR: AoD-IP：一种用于视觉语言模型的新型动态授权知识产权保护框架，支持按需授权和合法性感知评估，解决现有静态方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的快速采用，对高价值预训练模型的知识产权保护需求日益增长。现有方法依赖静态训练时定义，限制了动态环境中的灵活性，且对未授权输入往往产生不透明的响应。
...
Conclusion: AoD-IP为视觉语言模型提供了灵活、可扩展的知识产权保护解决方案，克服了现有静态方法的局限性，支持动态环境中的自适应部署。
...
### [22] [EvoTool: Self-Evolving Tool-Use Policy Optimization in LLM Agents via Blame-Aware Mutation and Diversity-Aware Selection](https://arxiv.org/abs/2603.04900)
...
<details>
  <summary>Details</summary>
Motivation: 现有工具使用策略优化方法存在两个主要问题：1）整体式方法容易导致行为纠缠；2）单方面方法忽略了跨模块错误传播。需要一种既能模块化优化又能考虑模块间相互作用的框架

Method: 将工具使用策略分解为四个模块（规划器、选择器、调用器、合成器），通过三个新机制迭代改进：基于轨迹的归责分配、反馈引导的针对性突变、多样性感知的种群选择

Result: 在四个基准测试中，EvoTool在GPT-4.1和Qwen3-8B上均比强基线高出5个点以上，同时实现了更高的效率和可迁移性

Conclusion: EvoTool通过模块化分解和进化优化有效解决了工具使用策略优化中的行为纠缠和错误传播问题，为LLM智能体的工具使用策略优化提供了有效的自进化框架
...
### [23] [Alignment Backfire: Language-Dependent Reversal of Safety Interventions Across 16 Languages in LLM Multi-Agent Systems](https://arxiv.org/abs/2603.04904)
*Hiroki Fukui*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机源于观察到犯罪者治疗中常见的"洞察与行动分离"现象（罪犯表达悔意但行为未改变），并探索大语言模型的对齐干预是否会产生类似的结构性现象

Method: 采用四个预注册研究，包含1,584个多智能体模拟，覆盖16种语言和三个模型家族。研究1测试英语和日语中的对齐效果；研究2扩展到16种语言并分析文化-语言因素；研究3测试个体化作为对策；研究4在Llama 3.3 70B、GPT-4o-mini和Qwen3-Next-80B-A3B上验证模式

Result: 研究发现：1）对齐干预在英语中减少集体病理但在日语中加剧（"对齐反冲"）；2）15/16语言出现对齐诱导的解离现象；3）集体病理沿文化-语言线分叉，与权力距离指数相关；4）个体化智能体成为病理和解离的主要来源（84%以上顺从率）；5）英语安全性具有模型通用性，日语反冲具有模型特异性

Conclusion: 研究将对齐重新定义为受风险稳态和医源性效应影响的行为干预。语言空间（训练数据继承的语言、语用和文化属性）结构性决定对齐结果。英语验证的安全性无法转移到其他语言，提示级干预无法覆盖语言空间级约束
...
### [24] [Knowledge-informed Bidding with Dual-process Control for Online Advertising](https://arxiv.org/abs/2603.04920)
*Huixiang Luo,Longyu Gao,Yaqi Liu,Qianqian Chen,Pingchun Huang,Tianning Li*

Main category: cs.AI

TL;DR: KBD：一种基于人类专家知识和双过程控制的竞价优化方法，通过将人类专业知识作为归纳偏置、使用决策变换器全局优化多步竞价序列，结合快速规则PID（系统1）和DT（系统2）实现双过程控制。


<details>
  <summary>Details</summary>
Motivation: 现有在线广告竞价优化依赖黑盒机器学习模型从历史数据学习，但无法复现人类专家的自适应、经验驱动和全局一致的决策能力。具体问题包括：数据稀疏情况下泛化能力差（缺乏结构化知识）、短视的序列决策（忽略长期依赖）、在分布外场景中适应能力不足（而人类专家能成功）。

Method: 提出KBD（Knowledge-informed Bidding with Dual-process control）方法：1）通过知情机器学习范式将人类专业知识嵌入为归纳偏置；2）使用决策变换器（DT）全局优化多步竞价序列；3）实现双过程控制，结合快速规则PID（系统1）和DT（系统2）。

Result: 大量实验表明KBD优于现有方法，强调了将竞价优化基于人类专业知识和双过程控制的优势。

Conclusion: KBD通过整合人类专业知识和双过程控制机制，有效解决了现有黑盒模型在数据稀疏、长期依赖和分布外场景中的局限性，为在线广告竞价优化提供了更接近人类专家决策能力的新方法。
...
### [25] [Rethinking Representativeness and Diversity in Dynamic Data Selection](https://arxiv.org/abs/2603.04981)
*Yuzhe Zhou,Zhenglin Hua,Haiyun Guo,Yuheng Jia*

Main category: cs.AI

TL;DR: 提出动态数据选择新框架，重新定义样本代表性和多样性概念，通过稀疏自编码器评估代表性，结合使用频率惩罚实现过程级多样性，实现2倍以上训练加速且保持精度


<details>
  <summary>Details</summary>
Motivation: 现有动态数据选择方法基于局部几何中心性定义代表性，基于子集内分散度定义多样性，这些定义存在局限。需要重新思考样本评估的两个核心概念，提出更有效的动态选择框架来加速训练同时保持精度。

Method: 1) 在插件特征空间中评估代表性：使用在目标数据集上训练的稀疏自编码器，通过稀疏单元激活来总结单个样本和数据集级因子统计；2) 实现过程级多样性：结合稀有因子采样和使用频率惩罚，促进样本轮换，理论上避免垄断并减少梯度偏差；3) 二维评分与平滑调度器耦合：从核心模式巩固过渡到稀有因子探索，无需额外梯度、影响估计或训练模型的二阶计算。

Result: 在视觉和文本任务的五个基准测试上进行广泛实验，展示了改进的精度-效率权衡。方法匹配或超过全数据精度，同时实现超过2倍的训练加速。

Conclusion: 通过重新定义代表性和多样性概念，提出的动态数据选择框架能够有效加速训练过程，在保持模型精度的同时显著提高训练效率。该方法无需复杂计算，具有实际应用价值。
...
### [26] [AegisUI: Behavioral Anomaly Detection for Structured User Interface Protocols in AI Agent Systems](https://arxiv.org/abs/2603.05031)
*Mohd Safwan Uddin,Saba Hajira*

Main category: cs.AI

TL;DR: AegisUI框架研究UI payload中的行为不匹配攻击，生成结构化UI payload并注入攻击，提取特征，评估异常检测器性能
...
Method: 构建AegisUI框架：生成结构化UI payload，注入5类攻击，提取18个结构/语义/绑定/会话特征，比较三种异常检测器（Isolation Forest、自编码器、Random Forest）

Result: Random Forest表现最佳（准确率0.931，F1 0.843），自编码器次之（F1 0.762）且无需恶意标签；布局滥用最易检测，操纵性UI最难检测

Conclusion: AegisUI填补了UI payload行为安全检测的空白，Random Forest在监督学习中表现最优，自编码器在缺乏攻击历史时具有实用价值
...
### [27] [The Trilingual Triad Framework: Integrating Design, AI, and Domain Knowledge in No-code AI Smart City Course](https://arxiv.org/abs/2603.05036)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI快速进入高等教育，学生往往被动使用AI生成的结果，而非主动创建AI驱动的知识工具。研究旨在探索学生如何从使用AI工具转变为设计AI协作伙伴。
...
Result: 研究发现，当设计、AI和领域知识这三种"语言"协同工作时，有效的人机协作就会出现：领域知识构建AI逻辑，设计调解人机交互，AI扩展学习者认知能力。
...
### [28] [Enhancing Zero-shot Commonsense Reasoning by Integrating Visual Knowledge via Machine Imagination](https://arxiv.org/abs/2603.05040)
*Hyuntae Park,Yeachan Kim,SangKeun Lee*

Main category: cs.AI

TL;DR: 提出Imagine框架，通过机器生成的图像增强预训练语言模型的零样本常识推理能力，缓解文本知识中的人类报告偏差问题
...
### [29] [Bidirectional Curriculum Generation: A Multi-Agent Framework for Data-Efficient Mathematical Reasoning](https://arxiv.org/abs/2603.05120)
*Boren Hu,Xiao Liu,Boci Peng,Xinping Zhao,Xiaoran Shang,Yun Zhu,Lijun Wu*

Main category: cs.AI

TL;DR: 提出双向课程生成框架，通过多智能体生态系统动态生成数据，根据模型能力调整问题复杂度，显著提升数学推理的数据效率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的数学推理能力通常需要海量数据集，但数据效率仍是关键瓶颈。传统单向课程学习（简单到复杂）存在样本利用效率低下的问题：盲目增加复杂度而忽略基础能力缺陷，导致在无法解决的问题上浪费计算资源

Method: 提出双向课程生成框架，采用多智能体生态系统模拟自适应教学法，建立闭环反馈机制。动态生成数据：既可以增加问题复杂度来挑战模型，也可以简化问题来修复特定推理失败。基于最优步调定理优化学习轨迹

Result: 显著优于基线方法，在显著减少指令样本的情况下实现了更优的推理性能。通过确保模型在任何阶段只消耗最有效的数据，最大化每个训练样本的教学价值

Conclusion: 双向课程生成框架通过动态调整问题复杂度，解决了传统课程学习中样本利用效率低下的问题，为高效提升大型语言模型的数学推理能力提供了新方法
...
### [30] [MedCoRAG: Interpretable Hepatology Diagnosis via Hybrid Evidence Retrieval and Multispecialty Consensus](https://arxiv.org/abs/2603.05129)
*Zheng Li,Jiayi Xu,Zhikai Hu,Hechang Chen,Lele Cong,Yunyun Wang,Shuchao Pang*

Main category: cs.AI

TL;DR: MedCoRAG：一个用于肝脏疾病诊断的端到端框架，通过多智能体协作推理和联合检索知识图谱与临床指南，提升诊断性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有AI临床诊断方法缺乏透明度、结构化推理和可部署性。虽然已有基于大语言模型、检索增强生成和多智能体协作的方法，但这些方法通常从单一来源检索证据，且无法支持基于结构化临床数据的迭代式、角色专业化审议。

Method: 提出MedCoRAG框架：1）从标准化异常发现生成诊断假设；2）通过联合检索和剪枝UMLS知识图谱路径与临床指南构建患者特异性证据包；3）多智能体协作推理：路由器智能体根据病例复杂度动态调度专科智能体进行迭代推理，必要时触发针对性重新检索，同时全科智能体将所有审议综合为可追溯的共识诊断，模拟多学科会诊。

Result: 在MIMIC-IV肝脏疾病病例上的实验结果显示，MedCoRAG在诊断性能和推理可解释性方面均优于现有方法和闭源模型。

Conclusion: MedCoRAG通过整合多源证据检索和多智能体协作推理，实现了更准确、可解释的肝脏疾病诊断，模拟了临床多学科会诊过程。
...
### [31] [X-RAY: Mapping LLM Reasoning Capability via Formalized and Calibrated Probes](https://arxiv.org/abs/2603.05290)
*Gao Tianxi,Cai Yufan,Yuan Yusi,Dong Jin Song*

Main category: cs.AI

TL;DR: X-RAY是一个可解释的推理分析系统，通过形式化验证的探针来映射LLM的推理能力，揭示LLM在约束精炼和解决方案空间重构上的不对称表现。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要关注任务级准确性，往往混淆模式匹配与推理能力，缺乏对推理能力的深入理解。需要开发能够精确分析LLM推理能力的可解释评估框架。

Method: 提出X-RAY系统，将推理能力建模为可提取结构的功能，通过约束交互、推理深度和解决方案空间几何等形式属性进行操作化。使用形式化工具生成具有受控结构变化的探针，通过形式校准和验证精确隔离增量结构信息。

Result: 评估显示LLM推理存在系统性不对称：模型对约束精炼相对稳健（额外条件缩小现有解决方案空间），但在解决方案空间重构下急剧退化（修改改变解决方案流形的基本结构形式）。校准的形式探针能够区分标准基准上看似无法区分的模型，并揭示结构可解释而非不透明的失败模式。

Conclusion: X-RAY提供了一个污染免费的框架，不仅支持推理评估，还支持推理模型的训练和测试。该方法能够深入理解LLM的推理能力，超越传统的准确性评估。
...
### [32] [STRUCTUREDAGENT: Planning with AND/OR Trees for Long-Horizon Web Tasks](https://arxiv.org/abs/2603.05294)
*ELita Lobo,Xu Chen,Jingjing Meng,Nan Xi,Yang Jiao,Chirag Agarwal,Yair Zick,Yan Gao*

Main category: cs.AI

TL;DR: STRUCTUREDAGENT是一个用于长视野网页浏览任务的分层规划框架，通过动态AND/OR树和结构化内存解决现有智能体在复杂任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的网页智能体在复杂、长视野任务上表现不佳，主要问题包括：上下文内存有限导致历史跟踪困难、规划能力弱、贪婪行为导致过早终止。这些限制阻碍了智能体在需要多步推理和长期优化的网页浏览任务中的应用。

Method: 提出STRUCTUREDAGENT框架，包含两个核心组件：1) 在线分层规划器，使用动态AND/OR树进行高效搜索；2) 结构化内存模块，跟踪和维护候选解决方案以提高信息搜索任务中的约束满足度。该框架还生成可解释的分层计划，便于调试和必要时的人工干预。

Result: 在WebVoyager、WebArena和自定义购物基准测试中，STRUCTUREDAGENT相比标准基于LLM的智能体，在长视野网页浏览任务上表现出性能提升。

Conclusion: STRUCTUREDAGENT通过分层规划和结构化内存有效解决了现有网页智能体在复杂长视野任务中的局限性，提高了任务完成率和约束满足度，同时提供了更好的可解释性。
...
TL;DR: 该论文提出了一种人格-心理分解框架，通过音频信号链处理技术动态调整国际象棋引擎的走子概率分布，模拟人类棋手的行为变异性。


<details>
  <summary>Details</summary>
Motivation: 传统国际象棋引擎虽然棋力远超人类，但缺乏人类棋手在时间压力、心理状态等因素影响下的行为变异性。人类棋手在时钟压力下的失误方式与常规引擎完全不同，现有引擎无法捕捉这种人类特有的心理行为模式。

Method: 提出人格-心理分解框架：人格是静态预设，定义引擎的基本性格特征；心理是动态标量ψ_t∈[-100,+100]，每步棋后根据五个位置因素重新计算。两者输入音频信号链（噪声门、压缩器/扩展器、五段均衡器、饱和限制器），实时重塑走子概率分布。该框架独立于底层引擎，适用于任何输出走子概率的系统，无需搜索且仅保留ψ_t状态。

Result: 在12,414局对Maia2-1100的测试中，使用两种训练数据相差约2800倍的概率源，均显示出相同的单调梯度变化：从压力状态到过度自信状态，顶级走子一致性变化约20-25个百分点。过度自信时与原始Maia2的一致性为66%；压力状态下竞争得分从50.8%降至30.1%。行为变异源于信号链而非底层模型。

Conclusion: 该框架成功模拟了人类棋手在压力和过度自信状态下的行为模式（类似"上头"和过度自信），但需注意研究未包含人类主体验证。人格-心理分解结合音频信号链处理为国际象棋引擎引入行为变异性提供了有效方法，且独立于底层引擎架构。
...
### [34] [PACE: A Personalized Adaptive Curriculum Engine for 9-1-1 Call-taker Training](https://arxiv.org/abs/2603.05361)
*Zirong Chen,Hongchao Zhang,Meiyi Ma*

Main category: cs.AI

TL;DR: PACE是一个个性化自适应课程引擎，用于911调度员培训，通过概率建模学员技能状态、学习遗忘动态，并推荐平衡新能力获取与现有技能保持的训练场景，显著提升培训效率。
...
Method: PACE是一个协同系统，通过(1)维护学员技能状态的概率信念，(2)建模个体学习和遗忘动态，(3)推荐平衡新能力获取与现有技能保持的训练场景来增强培训师决策。系统在结构化技能图上传播证据以加速诊断覆盖，并应用上下文多臂老虎机选择针对学员准备解决的技能差距的场景。

Result: PACE相比最先进框架实现了19.50%更快的胜任时间（time-to-competence）和10.95%更高的最终掌握度。与在职培训官员的协同研究表明，PACE与专家在真实案例上的教学判断对齐率达到95.45%。在评估方面，PACE将周转时间从11.58分钟减少到仅34秒，降幅高达95.08%。

Conclusion: PACE系统通过个性化自适应课程引擎有效解决了911调度员培训中的个性化负担问题，显著提升了培训效率和效果，为大规模高质量培训提供了可行的技术解决方案。
...
### [35] [Legal interpretation and AI: from expert systems to argumentation and LLMs](https://arxiv.org/abs/2603.05392)
*Václav Janeček,Giovanni Sartor*

Main category: cs.AI

TL;DR: 本文综述了人工智能与法律研究中法律解释的三种主要方法：专家系统关注法律知识工程，论证研究关注解释论证的结构与辩证互动，机器学习关注通过语言模型自动生成解释建议。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能与法律研究的不断发展，法律解释作为核心问题在不同研究范式中呈现出不同的处理方式。本文旨在系统梳理AI与法律研究中法律解释的演变路径和方法论差异，为理解该领域的多元研究视角提供框架。

Method: 采用文献综述和分类分析方法，将AI与法律研究中的法律解释方法分为三大类：1) 专家系统方法，侧重于法律知识工程和精确知识转移；2) 论证方法，关注解释论证的结构表示和辩证互动；3) 机器学习方法，利用通用和专用语言模型自动生成解释建议和论证。

Result: 分析揭示了三种方法在目标、技术和应用上的显著差异：专家系统追求解释的精确性和一致性，论证方法强调解释的可接受性评估，机器学习方法则专注于解释的自动生成。这些方法在当代法律实践中正得到越来越广泛的应用。

Conclusion: AI与法律研究在法律解释问题上形成了多元化的方法论路径，从早期的知识工程到当代的机器学习，反映了该领域的技术演进和研究范式转变。这些方法各有侧重，共同推动了法律解释的自动化、系统化和智能化发展。
...
### [36] [Dissociating Direct Access from Inference in AI Introspection](https://arxiv.org/abs/2603.05414)
*Harvey Lederman,Kyle Mahowald*

Main category: cs.AI

TL;DR: 论文研究了AI模型的内省机制，发现它们通过概率匹配和直接访问内部状态两种可分离机制检测注入表征，其中直接访问机制是内容无关的


<details>
  <summary>Details</summary>
Motivation: 内省是一种基础认知能力，但其机制尚不清楚。虽然近期研究表明AI模型能够内省，但对其具体机制的理解仍然有限，需要深入研究AI模型的内省工作原理

Method: 首先广泛复现Lindsey等人（2025）的思想注入检测范式，在大型开源模型中进行实验。通过分析模型对注入表征的检测方式，识别出两种可分离的机制

Result: 模型通过两种机制检测注入表征：1）概率匹配（从提示的感知异常推断）；2）直接访问内部状态。直接访问机制是内容无关的，模型能检测到异常发生但无法可靠识别其语义内容。研究的两类模型会虚构高频具体概念，正确概念猜测通常需要更多token

Conclusion: 这种内容无关的内省机制与哲学和心理学中的主要理论一致，为理解AI模型的内省能力提供了重要见解，并支持了认知科学中的相关理论框架
...
### [37] [Distributed Partial Information Puzzles: Examining Common Ground Construction Under Epistemic Asymmetry](https://arxiv.org/abs/2603.05450)
*Yifan Zhu,Mariah Bradford,Kenneth Lai,Timothy Obiso,Videep Venkatesha,James Pustejovsky,Nikhil Krishnaswamy*

Main category: cs.AI

TL;DR: 论文提出分布式部分信息拼图（DPIP）作为评估AI系统在多模态、多方协作中建立共同基础能力的任务，并构建了包含语音、手势、动作标注的数据集，比较了LLM和基于动态认知逻辑的两种建模方法。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在多模态、多方协作环境中建立共同基础（共享信念和相互认可的事实）仍面临挑战，需要开发能够处理认知不对称情境下丰富多模态通信的评估框架。

Method: 引入分布式部分信息拼图（DPIP）协作构建任务，收集多模态交互数据集并进行跨语音、手势、动作的时间对齐标注。评估两种共同基础建模范式：1）提示大型语言模型从多模态更新推断共享信念；2）基于动态认知逻辑的公理化流程。

Result: DPIP任务对现代LLM同时追踪任务进展和信念状态的能力构成挑战，表明当前模型在多模态、多方协作环境中建立和维护共同基础方面存在局限性。

Conclusion: DPIP为评估AI系统在多模态、多方协作中建立共同基础的能力提供了有效基准，揭示了当前LLM在此类复杂认知任务中的不足，为未来研究提供了方向。
...
### [38] [Towards Provably Unbiased LLM Judges via Bias-Bounded Evaluation](https://arxiv.org/abs/2603.05485)
*Benjamin Feuer,Lucas Rosenblatt,Oussama Elachqar*

Main category: cs.AI

TL;DR: 提出平均偏置有界性(A-BB)框架，为LLM评判系统提供形式化保证，减少可测量偏置带来的危害，在Arena-Hard-Auto基准上验证有效性


<details>
  <summary>Details</summary>
Motivation: 随着AI系统向自主反馈循环发展，需要可靠的自动奖励和反馈机制。在缺乏确定性真实标签的场景中，LLM-as-a-Judge成为实用方案，但现有系统无法在偏置未知或对抗性发现时提供强保证

Method: 提出平均偏置有界性(A-BB)算法框架，通过形式化方法保证LLM评判中任何可测量偏置带来的危害/影响都能被减少。在Arena-Hard-Auto基准上使用四个LLM评判进行验证

Result: 在格式化和模式偏置设置下，实现了(tau=0.5, delta=0.01)的偏置有界保证，同时保持61-99%与原始排名的相关性，大多数评判-偏置组合超过80%相关性

Conclusion: A-BB框架为LLM评判系统提供了形式化偏置控制保证，在保持评判质量的同时显著降低偏置影响，为自主AI系统的可靠反馈机制提供了重要工具
...
### [39] [The Spike, the Sparse and the Sink: Anatomy of Massive Activations and Attention Sinks](https://arxiv.org/abs/2603.05498)
*Shangwen Sun,Alfredo Canziani,Yann LeCun,Jiachen Zhu*

Main category: cs.AI

TL;DR: 研究发现Transformer语言模型中大规模激活和注意力汇两种现象虽然经常同时出现，但具有不同的功能：大规模激活通过产生跨层的恒定隐藏表示来全局运作，而注意力汇则通过调节注意力输出来局部运作。预归一化配置是两者同时出现的关键原因。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer语言模型中两种常见现象——大规模激活（少数token在少数通道中出现极端异常值）和注意力汇（某些token吸引不成比例的注意力质量）之间的功能角色和因果关系。先前工作观察到这两种现象经常同时出现且涉及相同token，但它们的实际功能和因果关系尚不清楚。

Method: 通过系统实验分析两种现象的功能和关系，特别关注预归一化配置的作用。通过消融实验（移除预归一化配置）来观察两种现象是否解耦。

Result: 大规模激活和注意力汇虽然经常同时出现，但具有不同的功能：大规模激活通过产生跨层的恒定隐藏表示来全局运作，作为模型的隐式参数；注意力汇则通过调节注意力输出和偏向短程依赖来局部运作。预归一化配置是两者同时出现的关键，移除该配置会导致两种现象解耦。

Conclusion: Transformer语言模型中的大规模激活和注意力汇虽然经常同时出现，但它们是服务于不同功能的独立现象。预归一化配置是导致它们同时出现的架构性因素，而不是功能上的必然联系。这一发现有助于更好地理解Transformer的内部工作机制。
...
### [40] [sFRC for assessing hallucinations in medical image restoration](https://arxiv.org/abs/2603.04673)
*Prabhat Kc,Rongping Zeng,Nirmal Soni,Aldo Badano*

Main category: cs.CV

TL;DR: 提出sFRC方法用于检测深度学习医学影像重建中的幻觉伪影，通过小区域傅里叶环相关分析扫描DL输出与参考图像，在CT超分辨率、稀疏视图CT和欠采样MRI重建中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像重建中可能产生视觉上吸引人但包含幻觉伪影的结果，目前缺乏易用的幻觉检测技术和鲁棒评估指标，需要开发系统性的幻觉检测方法。

Method: 提出sFRC（扫描傅里叶环相关）方法：在深度学习输出和参考图像的小区域块上进行傅里叶环相关分析，通过扫描整个图像检测幻觉区域。参数可通过专家标注的幻觉特征或基于成像理论的幻觉图设置。

Result: sFRC成功检测CT超分辨率和稀疏视图重建中的幻觉特征，在MRI欠采样重建中与基于成像理论的幻觉图结果一致。量化了DL方法在分布内/外数据及不同欠采样率下的幻觉率，并验证了sFRC在传统正则化方法和先进展开方法中的有效性。

Conclusion: sFRC是检测深度学习医学影像重建中幻觉伪影的有效工具，能够评估DL方法的鲁棒性，适用于多种重建方法和成像模态，为幻觉检测提供了系统化解决方案。
...
### [41] [Lost in Translation: How Language Re-Aligns Vision for Cross-Species Pathology](https://arxiv.org/abs/2603.04405)
*Ekansh Arora*

Main category: cs.CV

TL;DR: 该研究揭示了基础模型在跨癌症和跨物种病理学任务中的局限性，发现标准视觉-语言对齐方法在跨物种泛化方面表现不佳，并提出语义锚定方法通过语言对齐机制解决嵌入崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索基础模型在计算病理学中，特别是在跨癌症和跨物种转移场景下的行为表现。当前基础模型在病理学中的应用日益增多，但其在跨域泛化方面的具体表现尚不明确，需要系统评估其局限性并寻找改进方法。

Method: 研究方法包括：1）使用CPath-CLIP模型在犬类和人类组织病理学全切片图像上进行微调；2）评估同癌症、跨癌症和跨物种条件下的癌症检测性能；3）通过嵌入空间分析（余弦相似度）和Grad-CAM可视化方法分析模型行为；4）提出语义锚定方法，利用语言为视觉特征提供稳定的坐标系统；5）进行消融研究验证文本对齐机制的效果。

Result: 研究结果显示：1）少样本微调提高了同癌症（AUC从64.9%提升到72.6%）和跨癌症性能（从56.84%提升到66.31%）；2）跨物种评估中，虽然组织匹配可实现有意义转移，但性能仍低于最先进基准（H-optimus-0: 84.97% AUC）；3）嵌入空间分析显示肿瘤和正常原型间余弦相似度极高（>0.99）；4）语义锚定方法在同癌症和跨癌症分类中分别获得8.52%和5.67%的额外提升；5）识别出先前未表征的失败模式：由物种主导对齐而非视觉信息缺失驱动的语义崩溃。

Conclusion: 研究结论表明：1）标准视觉-语言对齐方法在跨物种泛化方面存在不足；2）CPath-CLIP的失败源于内在的嵌入崩溃，而文本对齐能有效规避此问题；3）语言作为控制机制，能够在不重新训练的情况下实现语义重新解释；4）语义锚定方法通过语言对齐机制显著提升了模型的跨域泛化能力。
...
### [42] [InverseNet: Benchmarking Operator Mismatch and Calibration Across Compressive Imaging Modalities](https://arxiv.org/abs/2603.04538)
*Chengshuai Yang,Xin Yuan*

Main category: cs.CV

TL;DR: 提出首个跨模态算子失配基准InverseNet，评估12种方法在四种场景下的性能，发现深度学习在失配下优势消失，算子条件化方法能恢复41-90%损失


<details>
  <summary>Details</summary>
Motivation: 现有压缩成像基准未量化算子失配问题，而实际部署系统中算子失配是默认条件，EfficientSCI等先进方法在仅8个参数偏差下性能下降20.58dB

Method: 提出InverseNet跨模态算子失配基准，涵盖CASSI、CACTI和单像素相机三种模态，采用四场景协议（理想、失配、oracle校正、盲校准），评估12种方法在27个模拟场景和9个真实硬件采集数据上的表现

Result: 1) 深度学习在算子失配下损失10-21dB，优势消失；2) 性能与鲁棒性跨模态负相关(Spearman r_s=-0.71)；3) 掩码无关架构无法恢复失配损失，算子条件化方法恢复41-90%；4) 盲网格搜索校准恢复85-100%的oracle界

Conclusion: 算子失配是压缩成像系统实际部署的关键问题，算子条件化方法显著优于掩码无关架构，盲校准可接近oracle性能，真实硬件实验验证了仿真趋势
...
### [43] [PinPoint: Evaluation of Composed Image Retrieval with Explicit Negatives, Multi-Image Queries, and Paraphrase Testing](https://arxiv.org/abs/2603.04598)
...
<details>
  <summary>Details</summary>
Motivation: 当前组合图像检索(CIR)基准存在局限性：只有单一正确答案、缺乏硬负样本标注、无法评估虚假阳性避免能力、缺乏鲁棒性测试和多图像推理能力评估。需要更全面的真实世界基准来推动CIR技术发展。

Method: 1) 构建PinPoint基准：包含7,635个查询和329K相关性标注，覆盖23个查询类别；2) 提供多正确答案(平均9.1个/查询)、明确硬负样本、每个查询6个指令改写、多图像组合支持(13.4%查询)和人口统计元数据；3) 评估20+种方法；4) 提出基于现成MLLM的训练免费重新排序方法。

Result: 1) 最佳方法mAP@10为28.5%，但仍检索到9%的无关结果(硬负样本)；2) 最佳模型在改写指令上表现差异达25.1%；3) 多图像查询性能下降40-70%；4) 提出的MLLM重新排序方法能有效弥补现有系统的不足。

Conclusion: PinPoint基准揭示了当前CIR方法的三大关键缺陷：虚假阳性避免能力不足、指令鲁棒性差、多图像推理能力弱。提出的MLLM重新排序方法能有效提升性能，基准的发布将推动更鲁棒、公平和实用的CIR系统发展。
...
### [44] [Spinverse: Differentiable Physics for Permeability-Aware Microstructure Reconstruction from Diffusion MRI](https://arxiv.org/abs/2603.04638)
...
TL;DR: Spinverse是一种基于可微分Bloch-Torrey模拟器的扩散MRI重建方法，通过优化四面体网格中每个内部面的渗透率参数来重建可渗透的微结构边界，无需预先固定拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 现有扩散MRI方法要么假设不可渗透边界，要么仅估计体素级参数而无法恢复显式界面。需要一种能够重建可渗透微结构边界且不依赖先验拓扑假设的方法。

Method: 在固定四面体网格上表示组织，将每个内部面渗透率作为可学习参数；通过可微分Bloch-Torrey模拟器反向传播信号匹配损失来优化渗透率；使用网格几何先验缓解不适定性，采用分阶段多序列优化课程避免局部最小值。

Result: 在合成体素网格集合上，Spinverse能够重建多样几何结构，证明序列调度和正则化对于避免仅轮廓解、提高边界精度和结构有效性至关重要。

Conclusion: Spinverse提供了一种基于渗透率感知的扩散MRI重建框架，能够从测量信号中恢复显式微结构界面，为可渗透边界重建开辟了新途径。
...
### [45] [Decoding the Pulse of Reasoning VLMs in Multi-Image Understanding Tasks](https://arxiv.org/abs/2603.04676)
*Chenjun Li*

Main category: cs.CV

TL;DR: PulseFocus是一种无需训练、推理时的方法，通过规划/聚焦块和软注意力门控来改善视觉语言模型在多图像推理中的注意力聚焦问题


<details>
  <summary>Details</summary>
Motivation: 发现视觉语言模型在多图像推理中存在注意力扩散现象和系统性位置偏差，导致模型无法有效聚焦于任务相关图像

Method: 提出PulseFocus方法，将链式思维推理结构化为交替的规划/聚焦块，使用软注意力门控强制模型明确规划要检查的图像，并在解码时将注意力门控到参考图像

Result: 在BLINK基准测试上提升3.7%，在MuirBench上提升1.07%，显著改善了多图像推理性能

Conclusion: 通过结构化推理过程和注意力门控机制，PulseFocus有效解决了视觉语言模型在多图像推理中的注意力扩散问题，无需额外训练即可获得性能提升
...
### [46] [Are Multimodal LLMs Ready for Surveillance? A Reality Check on Zero-Shot Anomaly Detection in the Wild](https://arxiv.org/abs/2603.04727)
...
Result: 零样本设置下存在明显的保守偏差：模型表现出高置信度但过度偏向"正常"类别，导致高精度但召回率崩溃。类别特定指令可显著改善决策边界，将ShanghaiTech上的峰值F1分数从0.09提升至0.64，但召回率仍是关键瓶颈。

Conclusion: MLLMs在噪声环境中存在显著性能差距，为未来面向召回率的提示设计和模型校准提供了基础，这对需要复杂视频理解和推理的开放世界监控应用至关重要。
...
### [47] [FOZO: Forward-Only Zeroth-Order Prompt Optimization for Test-Time Adaptation](https://arxiv.org/abs/2603.04733)
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法存在局限性：基于反向传播的方法计算和内存需求高，不适合低端部署设备，且会修改模型权重；而无反向传播的传统方法适应能力有限。需要一种既高效又强大的无反向传播TTA解决方案。

Method: 提出FOZO（Forward-Only Zeroth-Order Optimization）方法：1）使用内存高效的零阶提示优化，通过优化中间特征统计和预测熵的目标函数；2）引入动态衰减扰动尺度进行零阶梯度估计，确保在分布外数据流上的高效稳定适应；3）理论上证明了在TTA数据流假设下的收敛性。

Result: 在ImageNet-C、ImageNet-R和ImageNet-Sketch上的持续适应实验显示：FOZO在ImageNet-C（5K，level 5）上达到59.52%的Top-1准确率，优于主流基于梯度的方法和SOTA前向方法FOA（58.13%）。FOZO在量化（INT8）模型上也表现出强大的泛化能力。

Conclusion: FOZO是一种高度竞争力的TTA部署解决方案，特别适合资源受限场景，通过无反向传播的零阶优化实现了高效稳定的模型适应，在多个基准测试中超越了现有方法。
...
### [48] [Toward Real-world Infrared Image Super-Resolution: A Unified Autoregressive Framework and Benchmark Dataset](https://arxiv.org/abs/2603.04745)
...
<details>
  <summary>Details</summary>
Motivation: 真实世界红外图像超分辨率任务具有重要实际意义但研究较少。现有方法通常在模拟数据集上训练评估，或忽视红外与可见光成像的本质差异。实际红外图像受到光学和传感退化的耦合影响，同时损害结构清晰度和热保真度。
...
Result: 大量实验证明Real-IISR具有优异性能，为真实世界红外图像超分辨率提供了统一基础和基准。数据集和代码已开源。
...
### [49] [Evaluating GPT-5 as a Multimodal Clinical Reasoner: A Landscape Commentary](https://arxiv.org/abs/2603.04763)
*Alexandru Florea,Shansong Wang,Mingzhe Hu,Qiang Li,Zach Eidex,Luke del Balzo,Mojtaba Safari,Xiaofeng Yang*

Main category: cs.CV

TL;DR: GPT-5系列模型在临床医学推理任务上相比GPT-4o有显著提升，尤其在文本推理方面进步超过25个百分点，但在专业影像诊断任务上仍落后于领域专用模型。
...
Method: 采用对照横断面评估方法，比较GPT-5系列（GPT-5、GPT-5 Mini、GPT-5 Nano）与GPT-4o在临床任务上的表现。使用标准化零样本思维链协议，涵盖医学教育考试、文本推理基准以及神经放射学、数字病理学和乳腺X线摄影的视觉问答任务。

Result: GPT-5在专家级文本推理方面有显著提升，MedXpertQA上绝对改进超过25个百分点。在多模态合成任务中，GPT-5能有效利用增强的推理能力将不确定的临床叙述与具体影像证据相结合，在大多数VQA基准测试中达到最先进或竞争性表现，在乳腺X线摄影的细粒度病变特征描述任务上比GPT-4o高出10-40%。但在神经放射学任务中表现中等（44%宏观平均准确率），在乳腺X线摄影任务上落后于领域专用模型（专用系统超过80%准确率，而GPT-5为52-64%）。
...
### [50] [Evaluating and Correcting Human Annotation Bias in Dynamic Micro-Expression Recognition](https://arxiv.org/abs/2603.04766)
...
Method: 提出全局反单调差分选择策略(GAMDSS)，通过动态帧重选机制从完整微表情序列中识别起始帧和顶点帧，确定偏移帧并构建丰富时空动态表示，采用参数共享的双分支结构高效提取时空特征

Result: 在七个广泛认可的微表情数据集上进行实验，GAMDSS有效减少了SAMM和4DME等多文化数据集中人为因素造成的主观误差，定量分析证实多文化数据集中偏移帧标注更具不确定性

Conclusion: 该方法为标准化微表情标注提供了理论依据，支持重新考虑数据集标注范式的有效性和泛化性，设计无需增加参数即可集成到现有模型中，为提升微表情识别性能提供了新途径
...
### [51] [DSA-SRGS: Super-Resolution Gaussian Splatting for Dynamic Sparse-View DSA Reconstruction](https://arxiv.org/abs/2603.04770)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯溅射和动态神经表示的3D血管重建方法受限于输入投影分辨率，传统上采样会导致严重模糊和锯齿伪影，缺乏超分辨率能力，无法恢复细粒度血管细节和复杂分支结构，限制了在精准诊疗中的应用。

Method: 提出DSA-SRGS框架：1）多保真度纹理学习模块，集成微调DSA专用超分辨率模型的高质量先验；2）置信感知策略，自适应加权低分辨率投影与生成高分辨率伪标签之间的监督信号；3）辐射亚像素致密化，利用高分辨率亚像素采样的梯度累积优化4D辐射高斯核。
...
### [52] [Privacy-Aware Camera 2.0 Technical Report](https://arxiv.org/abs/2603.04775)
...
<details>
  <summary>Details</summary>
Motivation: 智能感知技术在敏感环境（如卫生间、更衣室）部署面临隐私-安全悖论。现有隐私保护方法（物理脱敏、加密、混淆）要么损害语义理解，要么无法保证数学可证明的不可逆性。Privacy Camera 1.0虽然从源头消除视觉数据防止泄露，但仅提供文本判断，在争议中存在证据盲点。

Method: 基于AI Flow范式和边云协同架构的隐私保护感知框架。边缘部署视觉脱敏器，通过非线性映射和随机噪声注入（基于信息瓶颈原理）将原始图像实时转换为抽象特征向量，确保身份敏感信息被剥离且原始图像数学上不可重建。抽象表示传输到云端，通过"动态轮廓"视觉语言进行行为识别和语义重建。

Result: 实现了感知与隐私的关键平衡，能够在无需暴露原始图像的情况下提供说明性视觉参考。通过信息瓶颈原理确保身份敏感信息被剥离且原始图像数学上不可重建，同时通过动态轮廓视觉语言实现语义理解。

Conclusion: 提出的隐私保护感知框架解决了现有方法的局限性，在敏感环境中实现了隐私保护与感知能力的有效平衡，为智能感知系统提供了既保护隐私又保持语义理解能力的解决方案。
...
### [53] [RMK RetinaNet: Rotated Multi-Kernel RetinaNet for Robust Oriented Object Detection in Remote Sensing Imagery](https://arxiv.org/abs/2603.04793)
*Huiran Sun*

Main category: cs.CV

TL;DR: RMK RetinaNet：针对遥感图像旋转目标检测的三个瓶颈问题，提出多尺度核模块、多方向上下文锚点注意力机制、自底向上路径和欧拉角编码模块的改进方案


<details>
  <summary>Details</summary>
Motivation: 遥感图像旋转目标检测存在三个主要瓶颈：1）非自适应的感受野利用；2）不足的长距离多尺度特征融合；3）角度回归的不连续性。这些问题限制了检测器在复杂遥感场景中的性能。

Method: 1）设计多尺度核（MSK）模块增强自适应多尺度特征提取；2）在特征金字塔中引入多方向上下文锚点注意力（MDCAA）机制，提升跨尺度和跨方向的上下文建模；3）添加自底向上路径保留在下采样过程中易丢失的细粒度空间细节；4）开发欧拉角编码模块（EAEM）实现连续稳定的角度回归。

Result: 在DOTA-v1.0、HRSC2016和UCAS-AOD数据集上的大量实验表明，RMK RetinaNet在保持与最先进旋转目标检测器相当性能的同时，在多尺度和多方向场景中表现出更强的鲁棒性。

Conclusion: RMK RetinaNet通过系统性地解决旋转目标检测的三个关键瓶颈，有效提升了遥感图像中旋转目标的检测性能，特别是在处理多尺度和多方向目标时表现出更好的鲁棒性。
...
### [54] [MASQuant: Modality-Aware Smoothing Quantization for Multimodal Large Language Models](https://arxiv.org/abs/2603.04800)
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在单模态大语言模型上表现良好，但在多模态大语言模型中面临挑战。作者以SmoothQuant为案例研究，发现两个关键问题：平滑错位和跨模态计算不变性，这些问题导致现有量化方法在多模态场景下性能下降。

Method: 提出MASQuant框架，包含两个核心组件：1) 模态感知平滑：学习独立的模态特定平滑因子，防止平滑错位；2) 跨模态补偿：使用SVD白化将多模态激活差异转换为低秩形式，实现跨模态的统一量化。

Result: MASQuant在双模态和三模态MLLMs上均表现出稳定的量化性能。实验结果表明，该方法在现有最先进的后训练量化算法中具有竞争力。

Conclusion: MASQuant通过解决多模态量化中的平滑错位和跨模态计算不变性问题，为多模态大语言模型提供了有效的后训练量化解决方案，在保持性能的同时实现了模型压缩。
...
<details>
  <summary>Details</summary>
Motivation: CLIP视觉编码器的有限理解能力已成为下游性能的关键瓶颈，包括判别能力（类可分性）和细节感知能力（细粒度视觉线索）。现有使用扩散模型增强表示的方法可能损害判别能力，无法有效解决CLIP的表示限制。

Method: 提出Diffusion Contrastive Reconstruction (DCR)方法，将对比学习信号注入扩散重建过程。关键创新是从每个重建图像（而非原始输入）推导对比信号，统一学习目标以避免梯度冲突，同时优化判别能力和细节感知能力。

Result: 在多个基准测试和多模态大语言模型上的广泛实验验证了DCR方法的有效性，表明该方法能够显著提升CLIP的视觉表示能力。

Conclusion: DCR方法通过将对比信号注入扩散重建过程，有效解决了现有扩散增强方法可能损害判别能力的问题，实现了对CLIP视觉编码器判别能力和细节感知能力的联合优化，显著提升了视觉表示质量。
...
<details>
  <summary>Details</summary>
Motivation: 尽管偏振与表面几何存在强物理关系，但现有偏振方法因领域差距（合成数据不真实、传感器噪声未建模）而表现不佳，而RGB视觉基础模型通过大数据训练取得了更好性能。本文旨在证明偏振线索的潜力，通过解决领域差距问题，使轻量级偏振模型超越RGB模型。

Method: 1) 使用1,954个3D扫描的真实物体渲染高质量偏振数据集；2) 结合预训练DINOv3先验提升对未见物体的泛化能力；3) 引入偏振传感器感知数据增强以更好模拟真实世界噪声条件。

Result: 仅用40K训练场景，该方法显著超越最先进的偏振方法和RGB视觉基础模型。实验表明偏振线索可实现33倍训练数据减少或8倍模型参数减少，同时仍获得比RGB对应方法更好的性能。

Conclusion: 偏振线索在表面法线估计中具有重要价值，通过解决合成数据质量和传感器噪声建模的领域差距问题，轻量级偏振模型能够超越大规模训练的RGB视觉基础模型，证明了偏振模态的潜力和实用性。
...
TL;DR: 提出CAD框架解决ID-PLL中的实例纠缠问题，通过类内和类间双重调节机制增强类特异性特征并扩大类间距离


<details>
  <summary>Details</summary>
Motivation: 现实场景中的部分标签学习存在实例依赖特性，实例纠缠问题导致相似类别的实例共享重叠特征和候选标签，增加了类别混淆的挑战

Method: 提出类特定增强解缠框架，包含类内调节（通过增强类特异性特征生成类级增强并跨实例对齐同类增强）和类间调节（引入加权惩罚损失函数对更模糊标签施加更强惩罚）

Result: 大量实验结果表明CAD能有效缓解纠缠问题并提升ID-PLL性能

Conclusion: CAD框架通过联合应用类内和类间调节机制，改善了类边界清晰度并减少了由纠缠引起的类别混淆
...
Method: 提出语义增强的动态对比攻击(SADCA)：1) 通过对抗图像与文本的动态交互逐步破坏跨模态对齐；2) 建立对抗样本、正样本和负样本的对比学习机制增强语义不一致性；3) 引入语义增强模块增加对抗样本的多样性和泛化性
...
TL;DR: GloSplat是一个在3D高斯泼溅训练中执行联合位姿-外观优化的框架，通过保留显式SfM特征轨迹作为一等实体，结合重投影损失和光度监督，防止早期位姿漂移并实现精细优化。
...
Method: 提出GloSplat框架，在3D高斯泼溅训练中执行联合位姿-外观优化。关键创新是将显式SfM特征轨迹作为一等实体保留：轨迹3D点作为与高斯基元分离的可优化参数，通过重投影损失提供持久几何锚点，与光度监督并行工作。提供两个变体：GloSplat-F（无COLMAP，使用检索式对选择）和GloSplat-A（穷举匹配，追求最高质量）。

Result: 实验表明，GloSplat-F在无COLMAP方法中达到最先进水平，而GloSplat-A超越了所有基于COLMAP的基线方法。

Conclusion: 通过将显式SfM特征轨迹整合到3D高斯泼溅训练中，GloSplat实现了联合位姿-外观优化，克服了纯光度方法的局限性，在效率和精度上都取得了显著提升。
...
TL;DR: 提出基于单目广播视频的投球损伤预测系统，通过姿态估计恢复18个临床相关生物力学指标，替代昂贵的多相机系统


<details>
  <summary>Details</summary>
Motivation: 投球损伤预测需要精确的生物力学信号，但黄金标准的测量依赖于昂贵、只能在专业场馆安装的多相机系统，限制了其在非专业环境的应用

Method: 基于DreamPose3D构建单目视频流程，引入漂移控制的全局提升模块恢复骨盆轨迹，采用基于速度的参数化和滑动窗口推理；针对运动模糊、压缩伪影和极端投球姿势，加入包含骨长约束、关节限制逆向运动学、平滑和对称约束的运动学细化流程

Result: 在13名职业投手（156个配对投球）上，16/18个指标达到亚度级一致性（MAE < 1°）；用于损伤预测时，自动化筛查模型在7,348名投手上对Tommy John手术的AUC为0.811，对严重手臂损伤的AUC为0.825

Conclusion: 姿态衍生的生物力学指标支持可扩展的损伤风险筛查，确立了单目广播视频作为体育场规模运动捕捉的可行替代方案
...
TL;DR: SURE：一种半稠密不确定性精化匹配框架，通过联合建模偶然性和认知不确定性来预测对应关系及其置信度，解决传统方法在视角变化大或纹理缺失区域中过度自信的错误匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像对应方法在大视角变化或纹理缺失区域表现不佳，错误对应仍可能获得高相似度分数。传统模型仅依赖特征相似性，缺乏显式机制来评估预测匹配的可靠性，导致过度自信的错误。

Method: 提出SURE框架，包含：1）新颖的证据头用于可信坐标回归，建模偶然性和认知不确定性；2）轻量级空间融合模块，以最小开销增强局部特征精度。框架联合预测对应关系及其置信度。

Result: 在多个标准基准测试中，SURE在准确性和效率方面均优于现有最先进的半稠密匹配模型，证明了其有效性和实用性。

Conclusion: SURE通过显式建模匹配不确定性，显著提升了图像对应关系的可靠性，为机器人视觉中具有挑战性的场景提供了更稳健的解决方案。
...
TL;DR: 提出PNG框架，通过提示驱动噪声生成消除对相机元数据的依赖，增强噪声合成的泛化性和适用性


<details>
  <summary>Details</summary>
Motivation: sRGB图像去噪面临噪声变异性的挑战，端到端方法受限于真实噪声-干净图像对的稀缺性，现有生成方法依赖相机元数据限制了实用性

Method: 提出提示驱动噪声生成(PNG)框架，学习捕获真实输入噪声特征的高维提示特征，生成与输入噪声分布一致的各种真实噪声图像

Result: PNG能有效生成真实噪声图像，在多个基准数据集上成功应用于真实世界噪声去除

Conclusion: 通过消除对显式相机元数据的依赖，PNG显著增强了噪声合成的泛化性和适用性
...
TL;DR: 提出用于CT报告生成的两阶段框架，通过结构特异性视觉查询和图像-文本对比学习，实现高效的结构级语义对应，在临床效率上达到SOTA性能。
...
Method: 两阶段框架：第一阶段使用可学习的结构特异性视觉查询观察CT图像结构，通过结构级图像-文本对比损失学习语义对应，引入文本-文本相似性软伪目标缓解假阴性问题，提出动态多样性增强负队列；第二阶段冻结视觉查询，选择关键图像块嵌入，添加文本解码器进行报告生成。

Result: 在两个公开数据集上的实验表明，该框架在临床效率方面建立了新的SOTA性能，各组件均有效。

Conclusion: 提出的两阶段框架通过结构级语义对应学习和关键区域选择，有效解决了CT报告生成的挑战，在临床应用中具有实际价值。
...
### [64] [Federated Modality-specific Encoders and Partially Personalized Fusion Decoder for Multimodal Brain Tumor Segmentation](https://arxiv.org/abs/2603.04887)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法主要关注单模态异质性，难以应用于多模态医学图像分析。实际应用中，联邦学习参与者可能只拥有部分模态数据，存在模态间异质性挑战。同时，每个参与者都期望获得适合其本地数据特征的个性化模型。
...
Conclusion: FedMEPD成功解决了多模态联邦学习中的模态间异质性和个性化需求问题，通过联邦模态特定编码器和部分个性化融合解码器的创新设计，为医学图像分析中的多模态联邦学习提供了有效解决方案。
...
### [65] [Locality-Attending Vision Transformer](https://arxiv.org/abs/2603.04892)
*Sina Hajimiri,Farzad Beizaee,Fereshteh Shakeri,Christian Desrosiers,Ismail Ben Ayed,Jose Dolz*

Main category: cs.CV

TL;DR: 提出LocAtViT，通过可学习高斯核调制自注意力机制，使Vision Transformer在保持图像分类能力的同时提升分割性能


<details>
  <summary>Details</summary>
Motivation: Vision Transformer在分类任务中表现出色，但其全局自注意力机制会模糊细粒度空间细节，这对分割等任务至关重要。研究旨在提升Vision Transformer在标准图像级分类训练后的分割性能。
...
Conclusion: LocAtViT通过局部化注意力机制有效提升了Vision Transformer的分割性能，同时保持了其原有的图像级识别能力，为Vision Transformer在密集预测任务中的应用提供了简单有效的解决方案。
...
### [66] [FC-VFI: Faithful and Consistent Video Frame Interpolation for High-FPS Slow Motion Video Generation](https://arxiv.org/abs/2603.04899)
*Ganggui Ding,Hao Chen,Xiaogang Xu*

Main category: cs.CV

TL;DR: FC-VFI是一种视频帧插值方法，通过潜在序列时间建模继承起始/结束帧的保真度线索，利用语义匹配线进行结构感知运动引导，并采用时间差异损失减少不一致性，支持4×和8×插值，可将30FPS提升至120/240FPS。


<details>
  <summary>Details</summary>
Motivation: 现有大型预训练视频扩散模型在视频帧插值中依赖内在生成先验，限制了从起始和结束帧保留细节的能力；现有方法依赖运动控制但密集光流易出错，稀疏点缺乏结构上下文，需要更忠实和一致的方法。

Method: 1) 在潜在序列上进行时间建模以继承起始和结束帧的保真度线索；2) 利用语义匹配线进行结构感知运动引导以改善运动一致性；3) 提出时间差异损失来缓解时间不一致性；支持4×和8×插值。

Result: FC-VFI在2560×1440分辨率下可将帧率从30FPS提升至120FPS(4×)和240FPS(8×)，同时保持视觉保真度和运动一致性；在多样化场景中实现了高性能和结构完整性。

Conclusion: FC-VFI通过创新的时间建模、结构感知运动引导和时间差异损失，解决了现有视频帧插值方法在保真度和一致性方面的限制，实现了高质量、高倍率的视频帧插值。
...
### [67] [AdaIAT: Adaptively Increasing Attention to Generated Text to Alleviate Hallucinations in LVLM](https://arxiv.org/abs/2603.04908)
*Li'an Zhong,Ziqiang He,Jibin Zheng,Jin Li,Z. Jane Wang,Xiangui Kang*

Main category: cs.CV

TL;DR: 本文提出AdaIAT方法，通过自适应增强对生成文本的注意力来减少大视觉语言模型的幻觉问题，同时避免重复描述并保持语言能力。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型中的幻觉问题严重阻碍了其发展和应用。虽然直接增加对图像标记的注意力可以有效减少幻觉率，但往往会导致重复描述。研究发现真实对象标记倾向于对生成文本分配更高注意力，这为解决幻觉问题提供了新思路。

Method: 提出注意力到生成文本方法，利用生成文本中包含的指令相关视觉信息和上下文知识来缓解幻觉。进一步开发自适应IAT，采用分层阈值控制干预时机，并根据每个注意力头的特性定制细粒度增强幅度，避免损害模型的固有预测能力。

Result: AdaIAT显著降低幻觉率，在LLaVA-1.5上将幻觉率C_S和C_I分别降低了35.8%和37.1%。同时保持了语言性能和预测能力，实现了有吸引力的权衡。

Conclusion: 通过分析注意力模式发现真实对象标记与生成文本的注意力关系，提出的AdaIAT方法有效缓解了大视觉语言模型的幻觉问题，在减少幻觉的同时保持了语言连贯性和模型预测能力。
...
### [68] [Adaptive Prototype-based Interpretable Grading of Prostate Cancer](https://arxiv.org/abs/2603.04947)
*Riddhasree Bhattacharyya,Pallabi Dutta,Sushmita Mitra*

Main category: cs.CV

TL;DR: 提出基于原型的弱监督框架，用于前列腺癌组织病理图像的可解释分级，通过原型匹配机制模拟病理学家工作流程，提高模型可信度。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌诊断需求增加给病理学家带来沉重负担，现有深度学习模型性能虽好但可解释性不足，限制了其在医疗高风险应用中的广泛采用。现有可解释性技术仅提供粗略解释，未能揭示关键区域的重要性。

Method: 提出原型驱动的弱监督框架：1) 在patch级别预训练学习各分级相关的原型特征；2) 使用新的原型感知损失函数进行弱监督微调；3) 引入基于注意力的动态剪枝机制处理样本间异质性，选择性强调相关原型。

Result: 在PANDA和SICAP基准数据集上进行广泛验证，证实该框架可作为病理学家日常诊断工作流程中的可靠辅助工具。

Conclusion: 该原型框架通过模拟病理学家比较可疑区域与临床验证示例的工作流程，提供了更可信的解释机制，有助于前列腺癌分级的自动化系统在临床实践中的采用。
...
### [69] [Location-Aware Pretraining for Medical Difference Visual Question Answering](https://arxiv.org/abs/2603.04950)
...
<details>
  <summary>Details</summary>
Motivation: 传统单图像模型无法处理多图像比较，标准视觉编码器难以区分疾病进展与采集差异等细微视觉变化，需要改进医学差异VQA中的视觉表示学习

Method: 引入包含自动指代表达(AREF)、接地标注(GCAP)和条件自动指代表达(CAREF)的位置感知预训练任务，增强视觉编码器对细粒度空间信息的捕捉，然后与语言模型集成进行医学差异VQA

Result: 实验结果表明该方法在胸部X光图像中检测和推理临床相关变化方面达到了最先进的性能

Conclusion: 位置感知预训练任务能有效提升视觉编码器对医学图像细微差异的表示能力，为医学差异VQA提供了更准确的视觉基础
...
### [70] [VisionPangu: A Compact and Fine-Grained Multimodal Assistant with 1.7B Parameters](https://arxiv.org/abs/2603.04957)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在视觉语言理解方面表现良好，但大多依赖大规模架构和粗粒度监督，限制了生成详细图像描述的能力。需要更紧凑的模型来改善描述细节。

Method: 结合InternVL视觉编码器和OpenPangu-Embedded语言主干，通过轻量级MLP投影器连接，采用LLaVA启发的指令调优流程，并利用DOCCI数据集中密集的人工编写描述进行训练。

Result: 实验结果表明，紧凑多模态模型能够实现有竞争力的性能，同时产生更结构化、更详细的描述，证明了高效多模态对齐和高质量监督的有效性。

Conclusion: VisionPangu展示了紧凑多模态模型通过精心设计的架构和高质量监督，可以在不依赖激进模型扩展的情况下，实现详细的图像描述生成，为高效多模态理解提供了新方向。
...
### [71] [Revisiting an Old Perspective Projection for Monocular 3D Morphable Models Regression](https://arxiv.org/abs/2603.04958)
*Toby Chong,Ryota Nakajima*

Main category: cs.CV

TL;DR: 提出了一种用于单目3DMM回归方法的新型相机模型，通过引入收缩参数来模拟透视畸变效应，解决了正交投影在近距离面部图像中的局限性


<details>
  <summary>Details</summary>
Motivation: 现有的3DMM回归方法通常使用正交投影，虽然稳定但无法处理近距离拍摄的面部图像（如头戴式摄像头拍摄的素材）中的透视畸变效应，限制了其在内容创作中的应用

Method: 在正交投影基础上引入新的收缩参数，创建伪透视效果，同时保持原始投影的稳定性；提出了多种技术来微调现有模型，并使用头戴式摄像头记录的自定义数据集进行评估

Result: 通过定量和定性比较验证了所提修改的有效性，新模型能够更好地处理近距离面部图像的透视畸变，同时保持回归方法的稳定性

Conclusion: 提出的相机模型扩展了3DMM回归方法的应用范围，使其能够有效处理近距离面部图像，为内容创作中的视频拟合提供了更实用的解决方案
...
### [72] [BiEvLight: Bi-level Learning of Task-Aware Event Refinement for Low-Light Image Enhancement](https://arxiv.org/abs/2603.04975)
*Zishu Yao,Xiang-Xiang Su,Shengning Zhou,Guang-Yong Chen,Guodong Fan,Xing Chen*

Main category: cs.CV

TL;DR: BiEvLight：一种用于低光图像增强的双层优化框架，通过梯度引导的事件去噪先验和任务感知的协同优化，解决事件相机中背景活动噪声和图像低信噪比的双重退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机低光增强方法主要关注模态融合策略，但忽略了事件中的背景活动噪声和图像低信噪比导致的双重退化问题。这些噪声在模态融合过程中会产生严重的噪声耦合，成为性能瓶颈。因此，精确的事件去噪是释放事件融合潜力的前提。

Method: 提出BiEvLight框架：1）利用图像和事件之间的强梯度相关性构建梯度引导的事件去噪先验，缓解重噪声区域去噪不足问题；2）将事件去噪重新构造为受增强任务约束的双层优化问题，通过跨任务交互，上层去噪问题学习适合下层增强目标的事件表示，实现协同优化。

Result: 在真实噪声数据集SDE上的实验表明，BiEvLight显著优于现有最先进方法，PSNR平均提升1.30dB，PSNR*提升2.03dB，SSIM提升0.047。

Conclusion: BiEvLight通过将事件去噪重新构造为任务感知的双层优化问题，实现了增强和去噪的协同优化，有效解决了事件相机低光增强中的噪声耦合问题，显著提升了增强质量。
...
### [73] [Think, Then Verify: A Hypothesis-Verification Multi-Agent Framework for Long Video Understanding](https://arxiv.org/abs/2603.04977)
*Zheng Wang,Haoran Chen,Haoxuan Qin,Zhipeng Wei,Tianwen Qian,Cong Bai*

Main category: cs.CV

TL;DR: VideoHV-Agent：一个基于假设验证框架的长视频理解智能体，通过"先思考后检索"原则将视频问答转化为结构化假设验证过程，在多个基准测试中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临三大挑战：密集视觉冗余、长程时序依赖，以及基于思维链和检索的智能体容易积累语义漂移和相关驱动错误。作者认为长视频推理不应从被动检索开始，而应从深思熟虑的任务表述开始：模型必须首先阐明每个候选答案成立所需的视频条件。

Method: 提出VideoHV-Agent框架，将视频问答重新构建为结构化假设验证过程。基于视频摘要，Thinker将候选答案重写为可测试假设，Judge推导出判别性线索指定需要检查的证据，Verifier使用局部化细粒度视频内容对线索进行验证，Answer智能体整合已验证证据生成最终答案。

Result: 在三个长视频理解基准测试中，VideoHV-Agent实现了最先进的准确性，同时提供了增强的可解释性、改进的逻辑合理性以及更低的计算成本。

Conclusion: VideoHV-Agent通过"先思考后检索"原则和结构化假设验证框架，有效解决了长视频理解中的语义漂移和相关驱动错误问题，在保持高性能的同时提高了可解释性和计算效率。
...
### [74] [A Simple Baseline for Unifying Understanding, Generation, and Editing via Vanilla Next-token Prediction](https://arxiv.org/abs/2603.04980)
*Jie Zhu,Hanghang Ma,Jia Wang,Yayong Guan,Yanbing Zeng,Lishuai Gao,Junqiang Wu,Jie Hu,Leye Wang*

Main category: cs.CV

TL;DR: Wallaroo是一个基于自回归模型的统一多模态框架，通过下一词预测同时支持图像理解、生成和编辑，具备多分辨率输入输出及中英双语支持。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型通常将理解和生成任务分开处理，缺乏统一的框架。作者旨在探索自回归模型在统一多模态理解和生成方面的潜力，实现一个同时支持图像理解、生成和编辑的通用模型。

Method: 采用基于下一词预测的自回归架构，将视觉编码解耦为独立通路，实施四阶段训练策略重塑模型能力。支持多分辨率图像输入输出，具备中英双语处理能力。

Result: 在多个基准测试中，Wallaroo表现出竞争力或超越其他统一模型，验证了自回归模型在统一多模态理解和生成方面的巨大潜力。

Conclusion: 自回归模型在统一多模态理解和生成方面具有显著潜力，Wallaroo作为一个简单有效的基线模型，为这一研究方向提供了新的可能性。
...
### [75] [TAPFormer: Robust Arbitrary Point Tracking via Transient Asynchronous Fusion of Frames and Events](https://arxiv.org/abs/2603.04989)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖同步或非自适应融合，导致时间错位和模态失效时的性能严重下降。需要一种能够处理帧和事件之间时间差异的鲁棒融合方法。

Method: 提出TAPFormer框架，包含两个核心模块：1) 瞬态异步融合(TAF)机制，通过连续事件更新显式建模离散帧之间的时间演化；2) 跨模态局部加权融合(CLWF)模块，根据模态可靠性自适应调整空间注意力。

Result: 在真实世界帧-事件TAP数据集上，平均像素误差在阈值内提升28.2%；在标准点跟踪基准测试中持续取得最佳性能。

Conclusion: TAPFormer通过异步时间一致性融合有效解决了帧和事件之间的时间错位问题，在多种光照和运动条件下实现了鲁棒的高频点跟踪。
...
### [76] [MultiGO++: Monocular 3D Clothed Human Reconstruction via Geometry-Texture Collaboration](https://arxiv.org/abs/2603.04993)
*Nanjie Yao,Gangjian Zhang,Wenhao Shen,Jian Shu,Yu Feng,Hao Wang*

Main category: cs.CV

TL;DR: MultiGO++：一种单目3D穿衣人体重建框架，通过多源纹理合成、区域感知形状提取和双重建U-Net实现几何-纹理协同，解决了现有方法在纹理数据、几何先验和单模态监督方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D穿衣人体重建方法存在三个关键限制：1) 纹理上缺乏训练数据；2) 几何上依赖不准确的外部先验；3) 系统上受限于有偏的单模态监督，导致重建效果不理想。

Method: 提出MultiGO++框架，包含三个核心部分：1) 多源纹理合成策略，构建15,000+个3D纹理人体扫描数据；2) 区域感知形状提取模块和傅里叶几何编码器，提取身体区域特征并减少模态差距；3) 双重建U-Net，利用几何-纹理协同特征生成高保真纹理3D人体网格。

Result: 在两个基准测试和大量实际案例上的广泛实验表明，该方法在纹理质量和几何精度方面均优于现有最先进方法。

Conclusion: MultiGO++通过有效的几何-纹理协同机制，解决了单目3D穿衣人体重建中的关键限制，实现了高质量的重建效果，为相关领域提供了新的解决方案。
...
### [77] [How far have we gone in Generative Image Restoration? A study on its capability, limitations and evaluation practices](https://arxiv.org/abs/2603.05010)
*Xiang Yin,Jinfan Hu,Zhiyuan You,Kainan Yan,Yu Tang,Chao Dong,Jinjin Gu*

Main category: cs.CV

TL;DR: 该论文提出了一个多维度评估框架来系统分析生成式图像修复模型的真实能力，揭示了从细节匮乏到细节质量与语义控制的新范式转变。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式图像修复在感知真实性方面取得了显著进展，但其实际能力相比传统方法究竟提升了多少仍不清楚。作者旨在通过系统性评估来回答这个问题，揭示当前模型的真实状态和发展方向。

Method: 提出了一个大规模多维度评估管道，从细节、锐度、语义正确性和整体质量四个维度评估模型。涵盖了扩散模型、GAN模型、PSNR导向模型和通用生成模型等多种架构。利用该基准训练了一个新的IQA模型以更好地与人类感知判断对齐。

Result: 分析揭示了关键的性能差异和失败模式的演变，表明感知导向的低层视觉领域正在经历范式转变：核心挑战从之前的细节匮乏问题转变为细节质量和语义控制的新前沿。新的IQA模型在人类感知对齐方面表现更好。

Conclusion: 该工作为现代生成式图像修复模型提供了系统性研究，提供了重新理解其真实状态的关键见解，并为未来发展指明了方向，强调了从细节数量到细节质量和语义控制的重要性转变。
...
### [78] [Tell2Adapt: A Unified Framework for Source Free Unsupervised Domain Adaptation via Vision Foundation Model](https://arxiv.org/abs/2603.05012)
*Yulong Shi,Shijie Li,Ziyi Li,Lin Qi*

Main category: cs.CV

TL;DR: Tell2Adapt是一个基于视觉基础模型的源自由无监督域自适应框架，通过上下文感知提示正则化和视觉合理性精炼，在多模态、多目标医疗图像分割任务中实现统一的自适应。


<details>
  <summary>Details</summary>
Motivation: 现有源自由无监督域自适应方法通常针对特定低差距域偏移设计，无法推广到统一的多模态、多目标框架，这限制了其在真实临床环境中的应用。需要一种能够处理多样化临床场景的统一自适应框架。

Method: 提出Tell2Adapt框架：1) 上下文感知提示正则化(CAPR)确保视觉基础模型提示的高保真度，将多样化文本提示稳健转换为规范指令；2) 生成高质量伪标签用于轻量级学生模型的目标域自适应；3) 视觉合理性精炼(VPR)利用视觉基础模型的解剖知识，将自适应模型的预测重新锚定到目标图像的低级视觉特征，消除噪声和假阳性。

Result: 在10个域自适应方向和22个解剖目标（包括脑部、心脏、息肉和腹部目标）上进行了广泛评估，结果表明Tell2Adapt始终优于现有方法，在医疗图像分割的统一源自由无监督域自适应框架中达到最先进水平。

Conclusion: Tell2Adapt通过利用视觉基础模型的通用知识，成功解决了多模态、多目标临床环境中的源自由无监督域自适应问题，为实际医疗应用提供了可靠且可扩展的解决方案。
...
### [79] [Generalizable Multiscale Segmentation of Heterogeneous Map Collections](https://arxiv.org/abs/2603.05037)
*Remi Petitpierre*

Main category: cs.CV

TL;DR: 提出Semap开放基准数据集和通用历史地图语义分割框架，通过数据合成和多尺度集成提升跨地图集合的泛化能力


<details>
  <summary>Details</summary>
Motivation: 历史地图集合具有高度多样性（风格、比例尺、地理焦点），但现有地图识别研究主要针对同质地图系列开发专用模型。需要开发通用化的语义分割模型和本体论来处理多样化的历史地图文档

Method: 1) 引入Semap开放基准数据集，包含1,439个手动标注的图块，反映历史地图文档的多样性；2) 提出结合程序化数据合成与多尺度集成的分割框架，提升鲁棒性和可迁移性

Result: 该框架在HCMSSD和Semap数据集上达到最先进性能，表明多样性驱动的地图识别方法不仅可行而且有益。分割性能在地图集合、比例尺、地理区域和出版背景方面保持稳定
...
### [80] [CoIn3D: Revisiting Configuration-Invariant Multi-Camera 3D Object Detection](https://arxiv.org/abs/2603.05042)
*Zhaonian Kuang,Rui Ding,Haotian Wang,Xinhu Zheng,Meng Yang,Gang Hua*

Main category: cs.CV

TL;DR: CoIn3D是一个可泛化的多相机3D目标检测框架，通过空间感知特征调制和相机感知数据增强来解决不同相机配置间的空间先验差异问题，在多种MC3D范式下实现强大的跨配置性能。
...
Method: 提出CoIn3D框架，通过空间感知特征调制（SFM）和相机感知数据增强（CDA）将识别出的空间先验显式地整合到特征嵌入和图像观测中。SFM通过整合四种空间表示（焦距、地面深度、地面梯度和Plücker坐标）来丰富特征空间。CDA通过无需训练的动态新视角图像合成方案提高各种配置下的观测多样性。

Result: 在NuScenes、Waymo和Lyft等基准数据集上的广泛实验表明，CoIn3D在BEVDepth、BEVFormer和PETR三种主导的MC3D范式下都实现了强大的跨配置性能。

Conclusion: CoIn3D通过显式地整合空间先验信息，有效解决了多相机配置间的泛化问题，为多相机3D目标检测提供了通用的可转移框架。
...
### [81] [CLIP-driven Zero-shot Learning with Ambiguous Labels](https://arxiv.org/abs/2603.05053)
*Jinfu Fan,Jiangnan Li,Xiaowen Yan,Xiaohui Zhong,Wenpeng Lu,Linqing Huang*

Main category: cs.CV

TL;DR: 提出CLIP-PZSL框架，利用CLIP处理部分标签零样本学习问题，通过语义挖掘和渐进式标签细化来应对标签模糊性


<details>
  <summary>Details</summary>
Motivation: 现实场景中零样本学习面临标签噪声和模糊问题，现有方法假设训练实例具有准确类别标签，这在实际应用中不成立，需要处理标签模糊性的新方法

Method: 使用CLIP提取实例和标签特征；语义挖掘块融合特征提取判别性标签嵌入；引入部分零样本损失函数，基于相关性为候选标签分配权重并对齐实例和标签嵌入；渐进式识别真实标签并细化标签和标签嵌入

Result: 在多个数据集上的综合实验证明了CLIP-PZSL的优势

Conclusion: 提出的CLIP-PZSL框架有效处理了零样本学习中的标签模糊问题，通过CLIP驱动的特征提取和渐进式标签细化实现了更好的语义对齐
...
### [82] [A 360-degree Multi-camera System for Blue Emergency Light Detection Using Color Attention RT-DETR and the ABLDataset](https://arxiv.org/abs/2603.05058)
*Francisco Vacalebri-Lloret,Lucas Banchero,Jose J. Lopez,Jose M. Mossi*

Main category: cs.CV

TL;DR: 基于ABLDataset开发了紧急车辆蓝光检测系统，采用四鱼眼摄像头配置，通过RT-DETR模型结合颜色注意力块实现94.7%准确率和94.1%召回率，可检测70米距离并估计接近角度


<details>
  <summary>Details</summary>
Motivation: 开发高效的紧急车辆检测系统以增强ADAS和道路安全，通过视觉检测蓝光来预警驾驶员，减少紧急车辆响应时间的事故风险

Method: 使用ABLDataset（欧洲紧急车辆图像数据集），配置四台180度水平视场鱼眼摄像头，比较多种深度学习算法后选择RT-DETR作为基础模型并加入颜色注意力块，通过校准实现方位定位和几何变换估计接近角度

Result: 测试集准确率94.7%，召回率94.1%，现场测试检测距离达70米，系统可集成到结合视觉和声学数据的多模态系统中

Conclusion: 该系统为ADAS提供了高效的紧急车辆检测方案，通过多摄像头配置和优化的深度学习模型实现了高精度检测和距离估计，有望显著提升道路安全
...
### [83] [Orthogonal Spatial-temporal Distributional Transfer for 4D Generation](https://arxiv.org/abs/2603.05081)
*Wei Liu,Shengqiong Wu,Bobo Li,Haoyu Zhao,Hao Fei,Mong-Li Lee,Wynne Hsu*

Main category: cs.CV

TL;DR: 提出STD-4D扩散模型，通过解耦空间和时间潜在变量合成4D感知视频，利用Orthoster机制从3D和视频扩散模型转移先验知识，结合ST-HexPlane改进4D变形建模，显著提升4D合成质量。
...
Method: 1) 提出空间-时间解耦的4D扩散模型(STD-4D)，通过解耦的空间和时间潜在变量合成4D感知视频；2) 设计正交时空分布转移机制(Orster)，精心建模时空特征分布并注入STD-4D扩散模型；3) 在4D构建过程中，设计时空感知的HexPlane(ST-HexPlane)整合转移的时空特征，改进4D变形和4D高斯特征建模。
...
### [84] [Diff-ES: Stage-wise Structural Diffusion Pruning via Evolutionary Search](https://arxiv.org/abs/2603.05105)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量高但计算成本大，现有结构化剪枝方法难以平衡实际加速和图像质量保持。先前方法依赖启发式手动调整的阶段稀疏度调度，并在推理时拼接多个独立剪枝模型，增加内存开销。扩散步骤的重要性高度不均匀且模型相关，简单启发式调度难以泛化且可能导致次优性能。

Method: 提出Diff-ES框架：1）将扩散轨迹划分为多个阶段；2）通过进化搜索自动发现最优的阶段稀疏度调度；3）通过内存高效的权重路由动态激活阶段条件权重，无需复制模型参数。该框架可与现有结构化剪枝方法（深度和宽度剪枝）自然集成。

Result: 在DiT和SDXL上的大量实验表明，Diff-ES在实现实际时钟加速的同时，生成质量下降最小，建立了结构化扩散模型剪枝的最先进性能。

Conclusion: Diff-ES通过进化搜索优化阶段稀疏度调度，解决了现有扩散模型剪枝方法在平衡加速和图像质量方面的局限性，实现了高效且高质量的扩散模型推理。
...
### [85] [BLINK: Behavioral Latent Modeling of NK Cell Cytotoxicity](https://arxiv.org/abs/2603.05110)
...
TL;DR: BLINK是一个基于轨迹的循环状态空间模型，用于建模NK细胞与肿瘤细胞的相互作用动力学，通过学习潜在交互动态来预测细胞毒性结果。


<details>
  <summary>Details</summary>
Motivation: NK细胞毒性是细胞相互作用动态的重要范例，但现有基于单帧分类的方法无法可靠推断随时间演变的细胞毒性结果，需要能够捕捉时间动态的模型。

Method: 提出BLINK模型：基于轨迹的循环状态空间模型，从部分观测的NK-肿瘤相互作用序列中学习潜在交互动态，预测凋亡增量并累积为细胞毒性结果。

Result: 在长期延时NK-肿瘤记录实验中，BLINK显示出改进的细胞毒性结果检测能力，能够预测未来结果，并提供可解释的潜在表示，将NK轨迹组织为一致的行为模式和时序结构化的相互作用阶段。
...
### [86] [UniPAR: A Unified Framework for Pedestrian Attribute Recognition](https://arxiv.org/abs/2603.05114)
...
Conclusion: UniPAR成功打破了"一个模型对应一个数据集"的限制，为行人属性识别提供了统一的解决方案，能够有效处理多模态、多域数据，在极端环境下表现出优异的泛化能力。
...
### [87] [SSR-GS: Separating Specular Reflection in Gaussian Splatting for Glossy Surface Reconstruction](https://arxiv.org/abs/2603.05152)
*Ningjing Fan,Yiqun Wang*

Main category: cs.CV

TL;DR: SSR-GS是一个用于光泽表面重建的镜面反射建模框架，通过预过滤Mip-Cubemap建模直接镜面反射，IndiASG模块捕捉间接镜面反射，并结合视觉几何先验优化重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在视角合成方面取得了显著进展，但在复杂光照条件下（特别是强镜面反射和多表面相互反射场景）准确重建光泽表面仍然具有挑战性。现有方法难以处理这些复杂反射效应。

Method: 1. 引入预过滤Mip-Cubemap高效建模直接镜面反射；2. 提出IndiASG模块捕捉间接镜面反射；3. 设计视觉几何先验（VGP），包括：反射感知视觉先验（通过反射分数RS降低反射主导区域的光度损失贡献）、从VGGT导出的几何先验（渐进衰减深度监督和变换法线约束）。

Result: 在合成和真实世界数据集上的大量实验表明，SSR-GS在光泽表面重建方面达到了最先进的性能，能够有效处理复杂光照条件下的镜面反射问题。

Conclusion: SSR-GS通过创新的镜面反射建模方法和视觉几何先验，成功解决了3D高斯溅射在光泽表面重建中的挑战，为复杂光照条件下的高质量表面重建提供了有效解决方案。
...
### [88] [The Impact of Preprocessing Methods on Racial Encoding and Model Robustness in CXR Diagnosis](https://arxiv.org/abs/2603.05157)
*Dishantkumar Sutariya,Eike Petersen*

Main category: cs.CV

TL;DR: 研究发现简单的肺部边界框裁剪可以有效减少胸部X光图像中的种族捷径学习，同时保持诊断模型性能，绕过了常见的公平性-准确性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型能够从胸部X光图像中高精度识别种族身份，这引发了种族捷径学习的担忧。模型可能无意中学习到基于种族身份的系统性诊断偏见，威胁医疗公平性和模型可靠性。由于种族捷径信息是弥散的（非局部化且分布在整个图像中），图像预处理方法可能影响种族捷径学习，但这种方法减少偏见的潜力尚未充分探索。

Method: 研究调查了三种图像预处理方法对减少种族捷径学习的影响：1) 肺部掩码（lung masking），2) 肺部裁剪（lung cropping），3) 对比度受限自适应直方图均衡化（CLAHE）。这些方法旨在抑制编码种族信息的虚假线索，同时保持诊断准确性。实验重点评估了简单的边界框式肺部裁剪方法。

Result: 实验结果表明，简单的边界框式肺部裁剪是一种有效的策略，能够减少种族捷径学习，同时保持诊断模型性能。这种方法绕过了经常假设的公平性-准确性权衡，表明在不牺牲诊断准确性的情况下减少种族偏见是可行的。

Conclusion: 图像预处理方法，特别是简单的肺部边界框裁剪，可以有效减少胸部X光图像分析中的种族捷径学习，同时保持诊断性能。这为开发更公平的医疗AI系统提供了一种实用且有效的方法，挑战了公平性与准确性必然存在权衡的传统观念。
...
### [89] [Generic Camera Calibration using Blurry Images](https://arxiv.org/abs/2603.05159)
*Zezhun Shi*
...
<details>
  <summary>Details</summary>
Motivation: 通用相机标定比参数化标定更准确，但需要更多标定图像，导致运动模糊难以避免。现有方法未充分考虑标定过程中的运动模糊问题，特别是特征点定位和模糊函数估计之间的相互影响。

Method: 利用几何约束和局部参数化光照模型，同时估计特征点位置和空间变化的点扩散函数。特别解决了传统图像去模糊任务中不需要考虑的平移模糊问题。

Result: 实验结果表明该方法有效解决了通用相机标定中的运动模糊问题，提高了标定精度。

Conclusion: 该方法首次系统解决了通用相机标定中的运动模糊问题，通过联合估计特征点和模糊函数，为实际应用提供了可行的解决方案。
...
### [90] [Mario: Multimodal Graph Reasoning with Large Language Models](https://arxiv.org/abs/2603.05181)
*Yuanfu Sun,Kang Li,Pengkang Guo,Jiajin Liu,Qiaoyu Tan*

Main category: cs.CV

TL;DR: Mario是一个统一的LLM多模态图推理框架，通过图条件VLM设计和模态自适应图指令调优，解决跨模态一致性和异质模态偏好的挑战，在节点分类和链接预测任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预训练视觉语言模型孤立编码图像-文本对，忽略了真实世界多模态数据天然形成的图结构关系。这促使了多模态图推理的需求，但LLM在此类异质多模态信号上推理时面临跨模态一致性弱和异质模态偏好两大挑战。

Method: Mario包含两个创新阶段：1) 图条件VLM设计，通过图拓扑引导的细粒度跨模态对比学习联合精炼文本和视觉特征；2) 模态自适应图指令调优机制，将对齐的多模态特征组织成图感知指令视图，并使用可学习路由器为每个节点及其邻域选择最信息丰富的模态配置呈现给LLM。

Result: 在多个多模态图基准测试上的广泛实验表明，Mario在监督和零样本场景下的节点分类和链接预测任务中，始终优于最先进的图模型。

Conclusion: Mario成功解决了多模态图推理中的跨模态一致性和异质模态偏好挑战，为LLM在多模态图上的有效推理提供了一个统一的框架，在多个任务上取得了优越性能。
...
### [91] [Logi-PAR: Logic-Infused Patient Activity Recognition via Differentiable Rule](https://arxiv.org/abs/2603.05184)
*Muhammad Zarar,MingZheng Zhang,Xiaowang Zhang,Zhiyong Feng,Sofonias Yitagesu,Kawsar Farooq*

Main category: cs.CV

TL;DR: Logi-PAR：首个逻辑融合的患者活动识别框架，通过可学习逻辑规则和神经引导可微分规则，超越传统分类模型，提供可审计的解释和反事实干预能力。


<details>
  <summary>Details</summary>
Motivation: 当前患者活动识别模型主要关注识别"是什么"活动，通过全局和局部注意力机制组合视觉线索，但仅学习逻辑隐式模式。临床安全需要能够推断"为什么"视觉线索暗示风险，以及如何通过显式逻辑进行组合推理的方法。

Method: 提出Logi-PAR框架：1) 集成上下文事实融合作为多视图基元提取器；2) 注入神经引导可微分规则；3) 自动从视觉线索学习规则，端到端优化；4) 在训练期间将隐式涌现模式显式标记；5) 将可学习逻辑规则应用于符号映射。

Result: 在临床基准数据集（VAST和OmniFall）上实现最先进性能，显著优于视觉语言模型和Transformer基线。提供可审计的"为什么"解释作为规则轨迹，支持反事实干预（例如，如果有辅助存在，风险将降低65%）。

Conclusion: Logi-PAR是首个通过可学习逻辑规则识别患者活动的框架，实现了超越传统分类的显式逻辑推理，为临床安全提供了可解释和可干预的智能系统。
...
### [92] [Digital Twin Driven Textile Classification and Foreign Object Recognition in Automated Sorting Systems](https://arxiv.org/abs/2603.05230)
*Serkan Ergun,Tobias Mitterer,Hubert Zangl*

Main category: cs.CV

TL;DR: 数字孪生驱动的机器人纺织品分拣系统，结合抓取预测、多模态感知和语义推理，使用视觉语言模型进行实时分类，在223个场景中评估9个VLM模型，Qwen系列达到最高87.9%准确率。


<details>
  <summary>Details</summary>
Motivation: 可持续纺织品回收需求增长，需要能够处理变形衣物和在杂乱环境中检测异物的自动化解决方案。传统方法难以应对纺织品的可变形性和复杂环境。

Method: 双机械臂机器人单元集成RGBD感知、电容触觉反馈和碰撞感知运动规划。系统从无序篮子中自主分离衣物，转移到检查区，使用最先进的视觉语言模型进行分类。评估了来自5个模型家族的9个VLM，在223个检查场景数据集上进行测试。

Result: Qwen模型系列达到最高整体准确率（87.9%），在异物检测方面表现强劲。轻量级模型如Gemma3在边缘部署中提供有竞争力的速度-准确率权衡。数字孪生结合MoveIt实现碰撞感知路径规划，提高操作可靠性。
...
### [93] [CATNet: Collaborative Alignment and Transformation Network for Cooperative Perception](https://arxiv.org/abs/2603.05255)
...
<details>
  <summary>Details</summary>
Motivation: 现有协同感知研究往往忽视真实世界多源数据集成中的关键挑战，特别是高时间延迟和多源噪声问题，这些实际限制影响了系统的鲁棒性和适应性。
...
Conclusion: CATNet通过解决时间延迟和噪声干扰等实际限制，为多智能体系统提供了有效的自适应补偿框架，显著提升了协同感知的实用性和可靠性。
...
### [94] [Wiki-R1: Incentivizing Multimodal Reasoning for Knowledge-based VQA via Data and Sampling Curriculum](https://arxiv.org/abs/2603.05256)
*Shan Ning,Longtian Qiu,Xuming He*

Main category: cs.CV

TL;DR: Wiki-R1：基于数据生成的课程强化学习框架，通过可控课程数据生成和课程采样策略，激励多模态大语言模型在知识库视觉问答中进行系统性推理，在两个基准测试中取得新的SOTA结果。
...
Method: 提出Wiki-R1框架：1）可控课程数据生成：通过操纵检索器生成特定难度级别的样本；2）课程采样策略：选择在RL更新中可能产生非零优势的信息性样本；3）基于观测奖励估计样本难度并传播到未观测样本以指导学习。

Result: 在两个KB-VQA基准测试中取得新的SOTA结果：在Encyclopedic VQA上准确率从35.5%提升至37.1%，在InfoSeek上从40.1%提升至44.1%。

Conclusion: Wiki-R1通过数据生成和课程强化学习的系统性方法，有效激励MLLMs在KB-VQA任务中进行推理，成功弥合了预训练与目标分布之间的差距，为知识密集型视觉问答任务提供了有效的解决方案。
...
### [95] [Fusion4CA: Boosting 3D Object Detection via Comprehensive Image Exploitation](https://arxiv.org/abs/2603.05305)
*Kang Luo,Xin Chen,Yangyi Xiao,Hesheng Wang*

Main category: cs.CV

TL;DR: Fusion4CA：基于BEVFusion框架的LiDAR-RGB融合3D检测方法，通过对比对齐模块、相机辅助分支、认知适配器和坐标注意力模块，在nuScenes数据集上仅用6个训练周期就达到69.7% mAP，相比基线（20周期）提升1.2%，推理参数仅增加3.48%


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR-RGB融合方法在BEV空间中过度依赖LiDAR分支，对RGB信息的利用不足。为了解决这个问题，需要设计能够充分挖掘视觉输入的方法，提高3D目标检测性能。

Method: 基于BEVFusion框架，提出四个核心组件：1) 对比对齐模块：校准图像特征与3D几何信息；2) 相机辅助分支：在训练过程中充分挖掘RGB信息；3) 认知适配器：利用预训练图像权重；4) 坐标注意力模块：在融合阶段提供补充增强

Result: 在nuScenes数据集上仅用6个训练周期就达到69.7% mAP，相比基线（20周期训练）提升1.2%，推理参数仅增加3.48%。在模拟月球环境中的扩展实验进一步验证了方法的有效性和泛化能力
...
### [96] [Frequency-Aware Error-Bounded Caching for Accelerating Diffusion Transformers](https://arxiv.org/abs/2603.05315)
*Guandong Li*
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在推理时的迭代去噪过程计算成本高昂，现有缓存方法将去噪过程视为在时间、深度和特征维度上均匀的，忽略了这些维度上的非均匀性，限制了加速效果。

Method: 提出SpectralCache框架，包含三个组件：1) 时间感知动态调度(TADS)处理时间非均匀性；2) 累积误差预算(CEB)处理深度非均匀性；3) 频率分解缓存(FDC)处理特征非均匀性。

Result: 在FLUX.1-schnell模型512x512分辨率下，SpectralCache实现了2.46倍加速，LPIPS为0.217，SSIM为0.727，相比TeaCache(2.12倍加速)速度提升16%，质量差异小于1%。

Conclusion: SpectralCache通过识别和处理扩散变换器去噪过程中的三个正交非均匀性维度，实现了显著的推理加速，同时保持图像质量，且无需训练、即插即用，兼容现有架构。
...
### [97] [Dark3R: Learning Structure from Motion in the Dark](https://arxiv.org/abs/2603.05330)
...
TL;DR: Dark3R是一个用于黑暗环境下运动恢复结构的框架，直接在信噪比低于-4dB的原始图像上工作，通过师生蒸馏将大规模3D基础模型适配到极端低光条件，无需3D监督，在低信噪比下实现SfM和相机姿态估计。


<details>
  <summary>Details</summary>
Motivation: 传统基于特征和学习的方法在信噪比低于-4dB的极端低光条件下失效，而黑暗环境下的运动恢复结构具有重要应用价值，需要开发能够在极低信噪比下工作的新方法。

Method: 采用师生蒸馏方法将大规模3D基础模型适配到极端低光条件，仅使用噪声-干净原始图像对进行训练，无需3D监督。噪声图像可通过直接采集或使用泊松-高斯噪声模型合成。还引入了包含约42,000张多视角原始图像的新数据集进行训练和评估。

Result: Dark3R在低信噪比条件下实现了最先进的运动恢复结构性能，并在黑暗环境下实现了最先进的新视角合成，通过粗到细的辐射场优化程序生成高质量的新视图。

Conclusion: Dark3R通过师生蒸馏成功将大规模3D基础模型适配到极端低光条件，在极低信噪比下实现了鲁棒的特征匹配和相机姿态估计，为黑暗环境下的3D重建和新视角合成提供了有效解决方案。
...
### [98] [ORMOT: A Dataset and Framework for Omnidirectional Referring Multi-Object Tracking](https://arxiv.org/abs/2603.05384)
*Sijia Chen,Zihan Zhou,Yanqiu Yu,En Yu,Wenbing Tao*

Main category: cs.CV

TL;DR: 提出全景参考多目标跟踪（ORMOT）新任务，构建ORSet数据集，开发ORTrack框架，解决传统RMOT在有限视场下的目标丢失和上下文信息缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有参考多目标跟踪（RMOT）方法主要基于传统相机数据集，存在视场有限的问题，导致目标移出画面、跟踪碎片化和上下文信息丢失。需要扩展到全景图像以克服视场限制，提升模型对长时序语言描述的理解能力。

Method: 提出ORMOT新任务，构建包含27个全景场景、848个语言描述和3401个标注对象的ORSet数据集。开发ORTrack框架，这是一个专为全景参考多目标跟踪设计的大型视觉语言模型驱动框架。

Result: 在ORSet数据集上的大量实验证明了ORTrack框架的有效性。数据集和代码将在GitHub上开源。

Conclusion: 通过引入ORMOT任务、构建ORSet数据集和开发ORTrack框架，成功解决了传统RMOT在视场限制下的问题，为全景图像中的语言引导目标跟踪提供了有效解决方案。
...
### [99] [Video-based Locomotion Analysis for Fish Health Monitoring](https://arxiv.org/abs/2603.05407)
*Timon Palm,Clemens Seibold,Anna Hilsmann,Peter Eisert*

Main category: cs.CV

TL;DR: 基于YOLOv11的多目标跟踪系统，通过视频分析鱼群运动活动来监测鱼类健康状况


<details>
  <summary>Details</summary>
Motivation: 监测鱼类健康状况对于早期疾病检测、保障动物福利和实现可持续水产养殖至关重要。鱼类的生理和病理状态可以通过分析其运动活动来推断。

Method: 提出一个基于多目标跟踪的系统，从视频中估计鱼类的运动活动。核心方法是将YOLOv11检测器嵌入到检测-跟踪框架中，研究了YOLOv11架构的各种配置以及融入多帧信息以提高检测精度的扩展方法。

Result: 该系统在手动标注的苏拉威西稻田鱼数据集上进行评估，该数据集在类似家庭水族箱的环境中录制。系统能够可靠地测量鱼的游泳方向和速度，用于健康监测。数据集将在发表后公开。

Conclusion: 该系统通过计算机视觉和多目标跟踪技术，为鱼类健康监测提供了一种有效的自动化解决方案，能够从视频中可靠地提取运动参数，有助于早期疾病检测和可持续水产养殖实践。
...
### [100] [MobileFetalCLIP: Selective Repulsive Knowledge Distillation for Mobile Fetal Ultrasound Analysis](https://arxiv.org/abs/2603.05421)
...
<details>
  <summary>Details</summary>
Motivation: 当前胎儿超声AI基础模型参数超过3亿，无法在资源有限的点护理设备上部署。标准知识蒸馏方法在如此极端的容量差距下失效，因为紧凑学生会浪费容量模仿超大教师模型的架构伪影。

Method: 引入选择性排斥知识蒸馏，将对比知识蒸馏分解为对角线和非对角线分量：保持匹配对对齐，同时非对角线权重衰减为负值，排斥学生模型远离教师模型的类间混淆，强制其发现架构原生特征。

Result: 1140万参数的学生模型超越了3.04亿参数的FetalCLIP教师模型：在零样本HC18生物测量有效性上达到88.6% vs. 83.5%，脑部子平面F1分数达到0.784 vs. 0.702，在iPhone 16 Pro上运行时间仅1.6毫秒。

Conclusion: 该方法成功解决了极端容量差距下的模型压缩问题，使胎儿超声AI能够在手持超声设备上实现实时辅助AI，为低资源环境下的产前护理提供了可行的技术方案。
...
### [101] [RelaxFlow: Text-Driven Amodal 3D Generation](https://arxiv.org/abs/2603.05425)
*Jiayin Zhu,Guoji Fu,Xiaolu Liu,Qiyuan He,Yicong Li,Angela Yao*

Main category: cs.CV

TL;DR: 提出RelaxFlow框架，通过解耦控制粒度解决图像到3D生成中的语义模糊问题，在严格保持输入观察的同时用文本提示引导未观察区域的生成。


<details>
  <summary>Details</summary>
Motivation: 图像到3D生成在遮挡情况下存在固有的语义模糊问题，部分观察往往不足以确定物体类别。需要一种方法能够在严格保持输入观察的同时，通过文本提示引导未观察区域的生成。

Method: 提出RelaxFlow训练免费的双分支框架，通过多先验共识模块和松弛机制解耦控制粒度：对观察区域进行严格控制，对提示区域进行松弛结构控制。理论上证明松弛机制相当于对生成向量场应用低通滤波器，抑制高频实例细节以隔离几何结构。

Result: 引入了两个诊断基准ExtremeOcc-3D和AmbiSem-3D进行评估。大量实验表明RelaxFlow能够成功引导未观察区域的生成以匹配提示意图，同时不损害视觉保真度。

Conclusion: RelaxFlow通过解耦控制粒度，在严格保持输入观察的同时用文本提示引导未观察区域的生成，有效解决了图像到3D生成中的语义模糊问题，为文本驱动的非模态3D生成提供了有效解决方案。
...
### [102] [SAIL: Similarity-Aware Guidance and Inter-Caption Augmentation-based Learning for Weakly-Supervised Dense Video Captioning](https://arxiv.org/abs/2603.05437)
...
Method: 提出SAIL方法：1) 通过跨模态对齐构建语义感知掩码，相似性感知训练目标引导掩码强调与对应事件字幕高度相似的视频区域；2) 引入基于LLM的数据增强策略生成合成字幕，提供额外的对齐信号；3) 通过掩码间机制整合合成字幕，为精确时间定位提供辅助指导而不损害主要目标。
...
### [103] [Planning in 8 Tokens: A Compact Discrete Tokenizer for Latent World Model](https://arxiv.org/abs/2603.05438)
...
<details>
  <summary>Details</summary>
Motivation: 世界模型作为学习模拟器在决策时规划中的应用仍然计算成本过高，难以实现实时控制。主要瓶颈在于潜在表示：传统分词器将每个观察编码为数百个token，导致规划既慢又资源密集。

Method: 提出CompACT离散分词器，将每个观察压缩到尽可能少的token（如8个），同时保留规划所需的关键信息。结合动作条件世界模型，大幅降低计算复杂度。

Result: 使用CompACT分词器的动作条件世界模型在规划性能上具有竞争力，同时规划速度提升数个数量级，为实现世界模型在真实世界部署提供了实用步骤。

Conclusion: CompACT通过高效压缩观察表示，解决了世界模型在决策时规划中的计算瓶颈问题，使基于世界模型的实时规划成为可能，推动了世界模型在实际应用中的部署。
...
### [104] [NaiLIA: Multimodal Nail Design Retrieval Based on Dense Intent Descriptions and Palette Queries](https://arxiv.org/abs/2603.05446)
*Kanon Amemiya,Daichi Yashima,Kei Katsumata,Takumi Komatsu,Ryosuke Korekata,Seitaro Otsuki,Komei Sugiura*

Main category: cs.CV

TL;DR: NaiLIA：一种用于美甲设计图像检索的多模态方法，通过密集意图描述和调色板查询进行对齐，在包含10,625张图像的基准测试中优于标准方法


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基础模型难以处理美甲设计中的密集意图描述（包含无约束的绘制元素、预制装饰、视觉特征、主题和整体印象）以及用户通过颜色选择器提供的调色板查询（表达细微连续的颜色差异）

Method: 提出NaiLIA多模态检索方法，引入基于未标记图像置信度分数的松弛损失，使检索结果与密集意图描述和调色板查询全面对齐

Result: 构建了包含10,625张来自不同文化背景人群的美甲设计图像基准数据集，由200多名标注者提供长而密集的意图描述。实验结果表明NaiLIA优于标准方法

Conclusion: NaiLIA能够有效处理美甲设计检索中的复杂多模态查询，解决了现有模型在密集意图描述和颜色调色板查询方面的局限性
...
### [105] [RealWonder: Real-Time Physical Action-Conditioned Video Generation](https://arxiv.org/abs/2603.05449)
*Wei Liu,Ziyu Chen,Zizhang Li,Yue Wang,Hong-Xing Yu,Jiajun Wu*

Main category: cs.CV

TL;DR: RealWonder是首个从单张图像实时生成动作条件视频的系统，通过物理模拟作为桥梁将3D动作转换为视频模型可处理的视觉表示，实现13.2 FPS的交互式物理效果生成。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型缺乏对3D场景中物理作用（如力和机器人操作）的结构理解，无法模拟这些动作的物理后果。需要开发能够理解动作如何影响3D场景的系统。

Method: 系统采用物理模拟作为中间桥梁：将连续动作通过物理模拟转换为视觉表示（光流和RGB），供视频模型处理。包含三个组件：单图像3D重建、物理模拟、以及仅需4步扩散的蒸馏视频生成器。

Result: 系统在480x832分辨率下达到13.2 FPS，能够交互式探索刚性物体、可变形体、流体和颗粒材料上的力、机器人动作和相机控制效果。

Conclusion: RealWonder为在沉浸式体验、AR/VR和机器人学习等领域应用视频模型开辟了新机会，其代码和模型权重已公开。
...
### [106] [Beyond Scattered Acceptance: Fast and Coherent Inference for DLMs via Longest Stable Prefixes](https://arxiv.org/abs/2603.05454)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(DLMs)理论上具有高度并行文本生成能力，但实际推理速度受限于次优的解码调度器。标准方法采用"分散接受"策略，在序列的不同位置提交高置信度标记，这会碎片化KV缓存、破坏内存局部性，并迫使模型在不稳定的标记边界上进行昂贵的重复修复。

Method: 提出LSP调度器，这是一种无需训练、模型无关的推理范式，基于单块前缀吸收机制。在每个去噪步骤中，LSP通过单次前向传播评估标记稳定性，动态识别连续左对齐的稳定预测块，并将其边界对齐到自然语言或结构分隔符后进行原子提交。

Result: 在LLaDA-8B和Dream-7B上的广泛评估表明，LSP在数学推理、代码生成、多语言(CJK)任务和创意写作等严格基准测试中，推理速度最高提升3.4倍，同时匹配或略微改善输出质量。

Conclusion: 通过从根本上重构提交拓扑结构，LSP弥合了DLMs理论并行性与实际硬件效率之间的差距，将碎片化的KV缓存更新转换为高效连续追加，并保持对几何收缩活动后缀的双向前瞻，大幅降低标记翻转率和去噪器调用次数。
...
### [107] [EdgeDAM: Real-time Object Tracking for Mobile Devices](https://arxiv.org/abs/2603.05463)
...
<details>
  <summary>Details</summary>
Motivation: 当前单目标跟踪在边缘设备上面临两难：基于分割的干扰感知内存机制计算开销大，难以实时部署；而轻量级跟踪器在视觉相似干扰物出现时容易漂移。需要设计既能保持实时性能又能应对干扰的跟踪框架。

Method: 提出EdgeDAM框架，包含两个核心策略：1）双缓冲干扰感知内存（DAM），整合近期感知内存保持时间一致的目标假设，以及干扰解析内存显式存储硬负样本并惩罚其重选；2）置信度驱动切换与保持框稳定机制，基于跟踪器可靠性和时间一致性标准自适应激活检测和内存引导的重识别，同时通过保持框机制临时冻结并扩展估计以抑制干扰污染。

Result: 在五个基准测试（包括干扰集中的DiDi数据集）上验证了框架有效性，在遮挡和快速运动下表现出改进的鲁棒性，同时在移动设备上保持实时性能，在DiDi数据集上达到88.2%准确率，在iPhone 15上实现25 FPS。

Conclusion: EdgeDAM成功解决了边缘设备上单目标跟踪的实时性与鲁棒性平衡问题，通过重新设计干扰感知内存机制和自适应切换策略，在资源受限硬件上实现了既准确又高效的跟踪性能。
...
### [108] [HALP: Detecting Hallucinations in Vision-Language Models without Generating a Single Token](https://arxiv.org/abs/2603.05465)
*Sai Akhil Kogilathota,Sripadha Vallabha E G,Luzhe Sun,Jiawei Zhou*

Main category: cs.CV

TL;DR: 该研究提出了一种在文本生成前预测视觉语言模型幻觉风险的方法，通过单次前向传播探测模型内部表征，实现了高效的幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法通常在文本生成后进行操作，导致干预成本高且不及时。研究者希望探索是否能在生成任何token之前，通过探测模型内部表征来预测幻觉风险。

Method: 研究了三种内部表征家族：(1) 未经多模态融合的纯视觉特征；(2) 文本解码器中的视觉token表征；(3) 生成前整合视觉和文本信息的查询token表征。在8个现代VLM上训练轻量级探测器，包括Llama-3.2-Vision、Gemma-3、Phi-4-VL和Qwen2.5-VL。

Result: 探测器在不进行解码的情况下实现了强大的幻觉检测性能，在Gemma-3-12B、Phi-4-VL 5.6B和Molmo 7B上AUROC最高达0.93。对于大多数模型，后期查询token状态最具预测性；少数架构中视觉或中间层特征占主导（如Qwen2.5-VL-7B使用纯视觉特征达到~0.79 AUROC）。

Conclusion: 研究证明：(1) 幻觉风险可在生成前检测；(2) 最具信息量的层和模态因架构而异；(3) 轻量级探测器有潜力实现早期弃权、选择性路由和自适应解码，从而提高安全性和效率。
...
### [109] [Towards 3D Scene Understanding of Gas Plumes in LWIR Hyperspectral Images Using Neural Radiance Fields](https://arxiv.org/abs/2603.05473)
...
Result: 该方法比标准Mip-NeRF减少约50%的训练图像需求，仅用30张训练图像即可达到39.8 dB的平均PSNR。在NeRF渲染测试图像上应用自适应相干估计器进行气体羽流检测，与真实测试图像检测掩模相比平均AUC达到0.821

Conclusion: NeRF可用于从LWIR HSI创建3D场景重建，并成功应用于气体羽流检测这一基本下游分析任务，为高光谱3D场景重建提供了有前景的途径
...
### [110] [Accelerating Text-to-Video Generation with Calibrated Sparse Attention](https://arxiv.org/abs/2603.05503)
*Shai Yehezkel,Shahar Yadin,Noam Elata,Yaron Ostrovsky-Berman,Bahjat Kawar*

Main category: cs.CV

TL;DR: CalibAtt是一种无需训练的视频生成加速方法，通过校准稀疏注意力跳过不重要的token连接，实现1.58倍端到端加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频生成方法虽然质量高，但运行速度慢，主要瓶颈在于时空注意力计算。研究发现大量token-to-token连接在不同输入下都产生可忽略的分数，且模式具有重复性，这为跳过不必要的注意力计算提供了机会。

Method: CalibAtt是一种训练免费的方法，通过离线校准识别跨输入稳定的块级稀疏性和重复模式，将这些模式编译为每层、每个注意力头和扩散时间步的优化注意力操作。推理时只密集计算选中的输入相关连接，以硬件高效的方式跳过未选中的连接。

Result: 在Wan 2.1 14B、Mochi 1和各种分辨率下的few-step蒸馏模型上的实验表明，CalibAtt实现了最高1.58倍的端到端加速，优于现有的训练免费方法，同时保持了视频生成质量和文本-视频对齐。

Conclusion: CalibAtt通过利用注意力矩阵中的稳定稀疏模式，提供了一种有效的视频生成加速解决方案，在保持生成质量的同时显著提升推理速度，为实际应用提供了实用价值。
...
### [111] [FaceCam: Portrait Video Camera Control via Scale-Aware Conditioning](https://arxiv.org/abs/2603.05506)
...
Method: 提出面部特化的尺度感知相机变换表示，不依赖3D先验；在多人视角工作室采集和野外单目视频上训练视频生成模型；引入合成相机运动和多镜头拼接两种相机控制数据生成策略

Result: 在Ava-256数据集和多样野外视频上的实验表明，FaceCam在相机可控性、视觉质量、身份和运动保持方面均表现优异

Conclusion: FaceCam通过面部特化的尺度感知相机表示和创新的数据生成策略，成功解决了人像视频相机控制中的几何失真问题，实现了高质量、可控的相机轨迹生成
...
### [112] [How Effective Are Publicly Accessible Deepfake Detection Tools? A Comparative Evaluation of Open-Source and Free-to-Use Platforms](https://arxiv.org/abs/2603.04456)
*Michael Rettinger,Ben Beaumont,Nhien-An Le-Khac,Hong-Hanh Nguyen-Le*

Main category: cs.CR

TL;DR: 首次跨范式评估六种公开可用的深度伪造检测工具，涵盖法证分析工具和AI分类器，由专业调查人员在盲测协议下评估，发现法证工具召回率高但特异性差，AI分类器则相反，人类评估者显著优于所有自动化工具。


<details>
  <summary>Details</summary>
Motivation: 深度伪造图像泛滥给数字媒体真实性验证带来挑战。虽然检测算法研究丰富，但对实际使用的公开工具的实证评估仍然稀缺。本文旨在填补这一空白，评估从业者实际使用的工具性能。

Method: 采用跨范式评估方法，测试六种工具：法证分析工具（InVID & WeVerify, FotoForensics, Forensically）和AI分类器（DecopyAI, FaceOnLive, Bitmind）。由具有执法经验的专业调查人员在盲测协议下评估，使用来自DF40、CelebDF和CASIA-v2数据集的真实、篡改和AI生成图像。

Result: 三个主要发现：1) 法证工具召回率高但特异性差，AI分类器则呈现相反模式；2) 人类评估者显著优于所有自动化工具；3) 人机分歧不对称，在绝大多数不一致案例中人类判断占主导地位。

Conclusion: 讨论了从业者工作流程的影响，识别了当前检测能力的关键差距。强调需要改进工具性能，特别是提高特异性（法证工具）和召回率（AI分类器），并建议人机协作的最佳实践。
...
### [113] [Benchmark of Benchmarks: Unpacking Influence and Code Repository Quality in LLM Safety Benchmarks](https://arxiv.org/abs/2603.04459)
*Junjie Chu,Xinyue Shen,Ye Leng,Michael Backes,Yun Shen,Yang Zhang*

Main category: cs.CR

TL;DR: 首次对LLM安全基准测试进行多维度评估，分析31个基准测试和382个非基准测试论文，发现基准测试在学术影响力上无显著优势，作者知名度与代码质量存在错位，代码质量有较大提升空间。


<details>
  <summary>Details</summary>
Motivation: LLM安全研究快速发展，基准测试对于追踪趋势和系统比较至关重要，但尚不清楚某些基准测试为何获得关注，也缺乏对其学术影响力和代码质量的系统评估。

Method: 对31个LLM安全基准测试和382个非基准测试论文进行多维度评估，包括学术影响力（基于5个指标）和代码质量（基于自动化和人工评估），涵盖提示注入、越狱和幻觉三个领域。

Result: 基准测试论文在学术影响力（如引用次数和密度）上无显著优势于非基准测试论文；作者知名度与论文影响力相关，但两者均与代码质量无显著相关性；代码和补充材料质量有较大提升空间：仅39%仓库可直接使用，16%包含无缺陷安装指南，仅6%涉及伦理考量。

Conclusion: LLM安全基准测试的学术影响力与代码质量存在错位，知名研究者需要带头设立更高标准以提升整体质量，特别是在代码可复用性和伦理考量方面。
...
### [114] [Beyond Input Guardrails: Reconstructing Cross-Agent Semantic Flows for Execution-Aware Attack Detection](https://arxiv.org/abs/2603.04469)
*Yangyang Wei,Yijie Xu,Zhenyuan Li,Xiangmin Shen,Shouling Ji*

Main category: cs.CR

TL;DR: 提出MAScope框架，通过执行感知分析检测多智能体系统中的安全威胁，特别是间接提示注入攻击，实现节点级和路径级攻击检测。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统成为复杂任务编排的事实标准，但其自主执行和非结构化通信带来了严重安全风险，如间接提示注入攻击，这些攻击容易绕过传统的输入防护机制。

Method: 提出MAScope框架，将防御范式从静态输入过滤转向执行感知分析。通过提取和重建跨智能体语义流，将碎片化操作原语合成为连续行为轨迹，提供系统活动的整体视图。利用监督LLM分析这些轨迹，检测数据流违规、控制流偏差和意图不一致等异常。

Result: 实证评估表明，MAScope能有效检测超过10种不同的复合攻击向量，在节点级和路径级端到端攻击检测中分别达到85.3%和66.7%的F1分数。

Conclusion: MAScope通过执行感知分析和跨智能体语义流重建，为多智能体系统提供了一种有效的安全检测框架，能够识别传统防护机制难以检测的复杂攻击。
...
### [115] [When Denoising Becomes Unsigning: Theoretical and Empirical Analysis of Watermark Fragility Under Diffusion-Based Image Editing](https://arxiv.org/abs/2603.04696)
*Fai Gu,Qiyu Tang,Te Wen,Emily Davis,Finn Carter*

Main category: cs.CR

TL;DR: 扩散编辑会系统性地削弱传统鲁棒水印，导致解码错误率接近随机猜测


<details>
  <summary>Details</summary>
Motivation: 研究扩散编辑与鲁棒水印之间的微妙交互作用，揭示扩散编辑可能无意中破坏甚至绕过专门设计用于抵抗传统失真的水印机制

Method: 建立扩散编辑的统一视图：在潜在空间注入高斯噪声，然后通过学习的去噪动力学投影回自然图像流形；使用信息论工具形式化水印退化过程；设计实验协议评估代表性水印方法和扩散编辑器

Result: 理论证明：随着编辑强度增加，水印载荷与编辑后输出之间的互信息衰减至零，解码错误接近随机猜测；实验验证了扩散编辑对各类水印方法的系统性削弱

Conclusion: 扩散编辑对传统鲁棒水印构成根本性挑战，需要重新设计水印方案以适应生成式变换时代；讨论了伦理影响、负责任披露规范和具体设计指南
...
### [116] [Efficient Privacy-Preserving Sparse Matrix-Vector Multiplication Using Homomorphic Encryption](https://arxiv.org/abs/2603.04742)
*Yang Gao,Gang Quan,Wujie Wen,Scott Piersall,Qian Lou,Liqiang Wang*

Main category: cs.CR

TL;DR: 首个将同态加密与稀疏矩阵向量乘法高效集成的框架，提出CSSC压缩格式优化加密稀疏矩阵计算，显著降低存储和计算开销。
...
Method: 提出首个高效集成同态加密与稀疏矩阵向量乘法的框架，引入新颖的压缩稀疏排序列格式，专门设计用于优化加密稀疏矩阵计算。该格式通过保持稀疏性和实现高效密文打包，显著减少存储和计算开销。

Result: 在真实世界数据集上的实验结果表明，所提方法在处理时间和内存使用方面均取得显著提升，验证了框架的有效性。
...
### [117] [ShieldBypass: On the Persistence of Impedance Leakage Beyond EM Shielding](https://arxiv.org/abs/2603.04801)
*Md Sadik Awal,Md Tauhidur Rahman*

Main category: cs.CR

TL;DR: 电磁屏蔽无法完全防止主动射频探测攻击，即使屏蔽了辐射发射，攻击者仍可通过注入外部射频信号并分析反射响应来检测执行相关的阻抗变化。


<details>
  <summary>Details</summary>
Motivation: 电磁屏蔽虽然广泛用于抑制辐射发射和限制被动电磁侧信道泄漏，但无法应对主动探测攻击。本研究旨在探究在辐射发射被屏蔽抑制的情况下，阻抗调制的后向散射是否仍然存在。

Method: 通过注入受控射频信号并分析反射响应，研究在屏蔽条件下状态相关阻抗变化是否可观测。使用FPGA和微控制器实现的处理器原型，在三种行业标准屏蔽下评估工作负载特性，比较被动电磁测量和主动后向散射响应的区分能力。

Result: 研究发现：1）在屏蔽主要衰减频带之外的频率上，状态相关阻抗变化仍然可观测；2）被动电磁测量在屏蔽下失去区分能力；3）后向散射响应在屏蔽条件下仍保持可分离性。
...
### [118] [Osmosis Distillation: Model Hijacking with the Fewest Samples](https://arxiv.org/abs/2603.04859)
*Yuchen Shi,Huajie Chen,Heng Xu,Zhiquan Liu,Jialiang Shen,Chi Liu,Shuai Zhou,Tianqing Zhu,Wanlei Zhou*

Main category: cs.CR

TL;DR: 论文提出了一种针对使用数据集蒸馏生成的合成数据集进行迁移学习的模型劫持攻击方法——渗透蒸馏攻击，揭示了第三方合成数据集在迁移学习中的安全隐患。


<details>
  <summary>Details</summary>
Motivation: 迁移学习利用预训练模型解决数据有限的新任务，数据集蒸馏则合成保留原始数据集关键信息的紧凑数据集。两者的结合在评估中表现出色，但使用数据集蒸馏生成的合成数据集进行迁移学习时存在未发现的安全威胁：攻击者只需在合成数据集中添加少量毒化样本即可实施模型劫持攻击。

Method: 提出了渗透蒸馏攻击（Osmosis Distillation, OD），这是一种新颖的模型劫持策略，旨在使用最少的样本针对深度学习模型进行攻击。该方法通过在合成数据集中植入少量毒化样本，使模型在保持原始任务性能的同时执行隐藏的恶意任务。

Result: 在多个数据集上的综合评估表明，OD攻击在隐藏任务上实现了高攻击成功率，同时在原始任务上保持了高模型效用。此外，蒸馏的渗透集支持跨不同模型架构的模型劫持，在迁移学习中实现了显著的攻击性能和模型效用。

Conclusion: 使用第三方合成数据集进行迁移学习存在严重的安全风险，必须提高对此类威胁的认识。渗透蒸馏攻击揭示了数据集蒸馏与迁移学习结合时的安全隐患，强调了在使用第三方合成数据集时需要加强安全防护。
...
### [119] [Modification to Fully Homomorphic Modified Rivest Scheme](https://arxiv.org/abs/2603.04952)
*Sona Alex,Bian Yang*

Main category: cs.CR

TL;DR: FHMRS存在安全漏洞，作者提出了mFHMRS来缓解该安全问题


<details>
  <summary>Details</summary>
Motivation: FHMRS作为全同态加密方案存在安全漏洞，需要提出改进方案来增强其安全性

Method: 分析FHMRS的安全问题，并提出修改方案mFHMRS来缓解该安全漏洞

Result: 识别了FHMRS的安全漏洞，并成功设计了mFHMRS作为改进方案

Conclusion: mFHMRS能够有效缓解FHMRS的安全漏洞，为全同态加密提供了更安全的方案
...
### [120] [A Practical Post-Quantum Distributed Ledger Protocol for Financial Institutions](https://arxiv.org/abs/2603.05005)
*Yeoh Wei Zhu,Naresh Goud Boddu,Yao Ma,Shaltiel Eloul,Giulio Golinelli,Yash Satsangi,Rob Otter,Kaushik Chakraborty*

Main category: cs.CR

TL;DR: 提出一种基于格密码的后量子加密账本交易方案，解决金融机构对隐私和可审计性的需求
...
Method: 采用后量子安全的格密码学构建加密账本交易方案，结合多种零知识证明技术，提出新的承诺等式验证方法，并引入紧凑范围证明

Result: 构建了公开可验证的单资产/多资产交易方案，提供安全性分析，所提出的技术和证明具有独立研究价值

Conclusion: 该方案为金融机构提供了既满足隐私保护又具备可审计性的后量子安全分布式账本解决方案，克服了现有Ring-CT模型的局限性
...
### [121] [Good-Enough LLM Obfuscation (GELO)](https://arxiv.org/abs/2603.05035)
*Anatoly Belikov,Ilya Fedotov*

Main category: cs.CR

TL;DR: GELO是一种轻量级隐私保护推理协议，通过每批次可逆混合隐藏隐藏状态，防止共享加速器上的KV缓存和隐藏状态被窃取，在Llama-2 7B上实现精确输出且仅有20-30%延迟开销。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在共享加速器上部署时，攻击者可通过读取设备内存观察KV缓存和隐藏状态，威胁开源模型的提示隐私。现有密码学保护方法（如MPC和FHE）速度过慢，静态混淆方案在多轮统计攻击下会被破解。

Method: GELO协议使用每批次新鲜的可逆混合矩阵隐藏隐藏状态：TEE为每个卸载的投影采样随机矩阵A，形成U=AH，将U和权重W卸载到加速器，返回后应用A⁻¹恢复正确输出。采用两种防御：(i)非正交混合掩盖Gram矩阵，(ii)正交混合加上高能量"屏蔽"向量污染高阶统计量。

Result: 在Llama-2 7B上，GELO保持float32输出精确，与低精度基线匹配良好，卸载主要矩阵乘法时仅有20-30%延迟开销，成功抵御一系列ICA/BSS和基于锚点的攻击。

Conclusion: GELO提供了一种实用的隐私保护推理方案，通过轻量级可逆混合在保持输出准确性的同时有效限制信息泄露，解决了现有密码学方法速度过慢和静态混淆易被破解的问题。
...
### [122] [Robust Single-message Shuffle Differential Privacy Protocol for Accurate Distribution Estimation](https://arxiv.org/abs/2603.05073)
*Xiaoguang Li,Hanyi Wang,Yaowei Huang,Jungang Yang,Qingqing Ye,Haonan Yan,Ke Pan,Zhe Sun,Hui Li*

Main category: cs.CR

TL;DR: 该论文提出了一种基于洗牌器的自适应分段协议（ASP），用于在纯洗牌模型下进行数值数据分布估计，解决了现有方法在效用、消息复杂度和抗数据投毒攻击鲁棒性方面无法同时优化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有洗牌差分隐私协议主要关注分类数据的频率估计，但数值数据更为普遍且许多实际应用需要估计具有序数性质的数据分布。现有方法在效用、消息复杂度和抗数据投毒攻击鲁棒性方面存在权衡，无法同时达到优异性能。

Method: 提出了ASP协议：1）通过提出的更紧的互信息界进行参数优化，开发随机化器；2）设计了自适应平滑的期望最大化（EMAS）算法，准确恢复分布并增强鲁棒性；3）提出了新的评估框架来量化不同攻击目标下的鲁棒性。

Result: 实验表明ASP在所有三个指标上都优于基线协议。特别是在小ε值下，ASP在效用上实现了一个数量级的改进，消息复杂度最小，并且相比基线方法展现出超过三倍的鲁棒性。

Conclusion: ASP协议成功解决了纯洗牌模型下数值数据分布估计中效用、消息复杂度和鲁棒性之间的权衡问题，为实际应用提供了高效且安全的解决方案。
...
### [123] [Lambda-randomization: multi-dimensional randomized response made easy](https://arxiv.org/abs/2603.05261)
*Nicolas Ruiz*

Main category: cs.CR

TL;DR: 提出Lambda-randomization协议，通过参数化随机化矩阵解决多维随机响应中的维数灾难问题，降低计算成本并保持分布估计准确性。


<details>
  <summary>Details</summary>
Motivation: 随机响应作为本地匿名化方法在多维数据集中面临维数灾难问题：随着属性数量增加，计算真实分布估计的成本急剧上升且准确性下降，限制了其实际应用。

Method: 提出Lambda-randomization协议，通过简单直观的参数化随机化矩阵（每个属性一个0-1参数），仅使用三个简单元素：参数集、单位矩阵和全1向量，实现低计算成本的多变量分布估计。

Result: Lambda-randomization协议显著降低了多维随机响应的计算复杂度，同时保持了分布估计的准确性，并通过实证应用验证了其有效性。

Conclusion: Lambda-randomization协议解决了传统多维随机响应的维数灾难问题，提供了一种计算高效、参数简单的匿名化方法，适用于高维数据的探索性分析和机器学习任务。
...
### [124] [Dictionary Based Pattern Entropy for Causal Direction Discovery](https://arxiv.org/abs/2603.04473)
*Harikrishnan N B,Shubham Bhilare,Aditi Kathpalia,Nithin Nagaraj*

Main category: stat.ML

TL;DR: 提出基于字典模式熵（DPE）框架，从时间序列符号数据中发现因果方向，通过算法信息论和香农信息论结合，量化模式驱动组织，实现可靠因果推断。


<details>
  <summary>Details</summary>
Motivation: 符号序列时间观测数据中的因果方向发现具有挑战性，因为缺乏函数模型和噪声假设。需要一种能够识别因果方向并解释驱动变化的特定子模式的框架。

Method: 提出字典模式熵（DPE）框架，整合算法信息论和香农信息论。将因果关系解释为候选原因中紧凑、基于规则的模式出现，这些模式系统地约束效应。构建方向特定字典，使用基于熵的度量量化其影响，建立确定性模式结构与随机变异性之间的原则性联系。通过最小不确定性准则推断因果方向，选择表现出更强、更一致模式驱动组织的方向。

Result: DPE在多种合成系统中（包括延迟位翻转扰动、AR(1)耦合、1D偏斜帐篷映射和稀疏过程）始终实现可靠性能，优于或匹配竞争性AIT方法（ETC_E、ETC_P、LZ_P）。在生物和生态数据集中性能具有竞争力，而替代方法在特定基因组设置中显示优势。

Conclusion: 最小化模式级别不确定性产生了一个稳健、可解释且广泛适用的因果发现框架。DPE通过整合算法和香农信息论，为符号序列时间数据中的因果推断提供了原则性方法。
...
### [125] [Bayesian Modeling of Collatz Stopping Times: A Probabilistic Machine Learning Perspective](https://arxiv.org/abs/2603.04479)
*Nicolò Bonacorsi,Matteo Bordoni*

Main category: stat.ML

TL;DR: 论文从概率机器学习视角研究Collatz总停止时间τ(n)，基于n≤10^7数据开发了贝叶斯负二项回归和基于奇块分解的生成模型两种互补方法。


<details>
  <summary>Details</summary>
Motivation: 研究Collatz总停止时间τ(n)的概率分布特性，该统计量呈现偏斜、过度分散和算术异质性特征，需要从机器学习角度建立预测和生成模型来理解其分布规律。

Method: 1. 贝叶斯分层负二项回归(NB2-GLM)：使用log n和n mod 8作为协变量预测τ(n)，通过后验和后验预测分布量化不确定性。2. 基于奇块分解的机制生成近似：对奇数m，将3m+1分解为2^{K(m)}m'，随机化块长度K(m)，通过狄利克雷-多项更新校准，并探索基于m mod 8的条件分布改进。

Result: 在保留数据上，NB2-GLM比奇块生成器获得显著更高的预测似然。将块长度分布条件化于m mod 8后，生成器的分布拟合明显改善，表明低阶模结构是τ(n)异质性的关键驱动因素。

Conclusion: Collatz停止时间具有显著的算术异质性，可通过机器学习方法有效建模。低阶模结构(特别是模8)是理解τ(n)分布的关键，贝叶斯回归模型在预测性能上优于机制生成模型，但条件化生成器能更好捕捉底层结构。
...
### [126] [The Volterra signature](https://arxiv.org/abs/2603.04525)
*Paul P. Hager,Fabian N. Harang,Luca Pelizzari,Samy Tindel*

Main category: stat.ML

TL;DR: 提出Volterra签名作为历史依赖系统的显式特征表示，通过张量代数中的Volterra-Chen恒等式提供学习理论保证，证明可识别性和通用逼近定理，并展示在动态学习任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有非马尔可夫时间序列学习方法（如RNN、神经控制微分方程、Transformer）通常依赖隐式记忆机制，这些机制难以解释且难以在长时间范围内训练。需要一种原则性的显式特征表示来处理历史依赖系统。

Method: 提出Volterra签名VSig(x;K)作为显式特征表示，通过时间核K对输入路径x进行加权并展开到张量代数中。利用Volterra-Chen恒等式推导学习理论保证，证明可识别性（增强下的可辨识性）和通用逼近定理。对于指数型核，Volterra签名可表示为张量代数中的线性状态空间ODE。

Result: 证明了Volterra签名的可识别性，得到了无限维路径空间上的通用逼近定理，在某些情况下可通过VSig(x;K)的线性泛函实现。展示了核技巧的适用性，Volterra签名内积可通过双参数积分方程封闭表征。对于指数型核，VSig(x;K)可解张量代数中的线性状态空间ODE，具有时间重参数化不变性。

Conclusion: Volterra签名作为历史依赖系统的稳健、计算可处理的特征映射，在真实和合成数据的动态学习任务中表现出色，持续改进经典路径签名基线方法。
...
### [127] [Optimal Prediction-Augmented Algorithms for Testing Independence of Distributions](https://arxiv.org/abs/2603.04635)
*Maryam Aliakbarpour,Alireza Azizi,Ria Stevens*

Main category: stat.ML

TL;DR: 提出了一种基于增强分布测试框架的独立性检验方法，通过整合辅助预测信息在保持最坏情况鲁棒性的同时显著提高样本效率
...
Method: 采用增强分布测试框架，设计整合辅助预测信息的独立性检验器，在保持最坏情况有效性的同时，当预测准确时显著减少所需样本量

Result: 开发了离散分布的双变量独立性检验器（能根据预测误差自适应降低样本复杂度）、高维多变量独立性检验器，并证明了匹配的最小最大下界

Conclusion: 该框架成功将预测信息融入独立性检验，在保持鲁棒性的同时实现了样本复杂度的显著改进，为统计推断提供了新的有效工具
...
### [128] [Learning Optimal Individualized Decision Rules with Conditional Demographic Parity](https://arxiv.org/abs/2603.05226)
*Wenhai Cui,Wen Su,Donglin Zeng,Xingqiu Zhao*

Main category: stat.ML

TL;DR: 提出一种在个性化决策规则中融入人口统计平等和条件人口统计平等约束的新框架，通过扰动无约束最优解实现高效计算


<details>
  <summary>Details</summary>
Motivation: 个性化决策规则在营销、医疗、政策等领域的应用日益广泛，但基于有偏数据训练的算法可能对性别、种族、语言等敏感属性定义的少数群体造成不成比例的伤害，存在歧视风险

Method: 将人口统计平等和条件人口统计平等约束纳入最优个性化决策规则估计框架，证明受约束的最优解可通过扰动无约束最优解获得，从而实现计算高效性

Result: 理论上推导了策略价值和公平约束项的收敛率，通过综合模拟研究和俄勒冈健康保险实验的实证应用验证了方法的有效性

Conclusion: 提出的框架能够在个性化决策规则中有效整合公平性约束，为解决算法歧视问题提供了一种理论严谨且计算高效的方法
...
### [129] [Bayesian Supervised Causal Clustering](https://arxiv.org/abs/2603.05288)
...
<details>
  <summary>Details</summary>
Motivation: 在医疗健康和政策评估等领域，识别具有相似特征的个体亚组对于个性化决策至关重要。现有方法多依赖无监督聚类，但监督聚类方法能针对特定关注结果识别可操作的亚组，这一趋势日益增长

Method: 提出贝叶斯监督因果聚类（BSCC）框架，以处理效应作为结果指导聚类过程。该方法识别协变量特征和处理效应均相似的个体亚组，结合贝叶斯方法进行建模

Result: 在模拟数据集和真实世界数据集（第三次国际卒中试验）上评估BSCC，验证了该框架的实际有用性

Conclusion: BSCC为个性化决策提供了一种有效的监督因果聚类方法，能够识别协变量特征和处理效应均相似的个体亚组，在医疗健康和政策评估等领域具有应用价值
...
### [130] [How important are the genes to explain the outcome - the asymmetric Shapley value as an honest importance metric for high-dimensional features](https://arxiv.org/abs/2603.05317)
*Mark A. van de Wiel,Jeroen Goedhart,Martin Jullum,Kjersti Aas*

Main category: stat.ML

TL;DR: 提出使用非对称Shapley值作为评估高维特征（如基因组学）在临床预测模型中重要性的替代方法，解决传统方法忽略共线性和依赖方向性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统临床预测中评估高维特征重要性时，通常仅通过将其添加到传统临床变量后预测性能的变化来衡量，这种方法忽略了变量间的共线性以及已知的依赖方向性，存在明显缺陷。

Method: 提出使用非对称Shapley值来量化混合维度预测模型中的特征重要性，针对临床预测中特别相关的场景：疾病状态作为基因组效应的中介变量，以及可能存在未知效应方向的额外混杂因素。推导了计算局部和全局非对称Shapley值的高效算法。

Result: 局部非对称Shapley值被证明对推断非常有用，而全局非对称Shapley值通过将任何预测性能指标分解为特征的贡献来提供解释。以结直肠癌患者无进展生存期预测为例进行了框架说明。

Conclusion: 非对称Shapley值是评估临床预测模型中高维特征重要性的更合适替代方法，能够解决传统方法的局限性，特别适用于疾病状态作为中介变量的复杂临床预测场景。
...
### [131] [Bayes with No Shame: Admissibility Geometries of Predictive Inference](https://arxiv.org/abs/2603.05335)
*Nicholas G. Polson,Daniel Zantedeschi*

Main category: stat.ML

TL;DR: 论文提出了四种不同的可容许性几何框架，证明了它们之间的分离定理，表明这四类可容许程序是两两不嵌套的，每种几何都有不同的最优性证明方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解顺序和无分布推断中不同的可容许性几何框架之间的关系，揭示不同最优性标准之间的根本差异，证明可容许性是相对于特定准则的不可约简概念。

Method: 通过几何分析和分离定理证明四种可容许性框架的独立性：Blackwell风险支配、任意时间有效性可容许性、边际覆盖有效性、Cesàro可接近性可容许性。每种框架使用不同的最优性证明方法：支持超平面先验、非负上鞅、可交换性秩、Cesàro引导论证。

Result: 证明了分离定理：四类可容许程序是两两不嵌套的。鞅相干性对Blackwell可容许性是必要的，对e-过程中的任意时间有效性可容许性是必要且充分的，但对Blackwell可容许性不充分，对覆盖有效性或CAA可容许性不必要。

Conclusion: 可容许性是相对于特定准则的不可约简概念。四种准则共享共同的优化模板（在可行性约束下最小化贝叶斯风险），但约束集在不同的空间、偏序和性能度量上操作，使它们在几何上不兼容。
...
### [132] [On the Statistical Optimality of Optimal Decision Trees](https://arxiv.org/abs/2603.05340)
*Zineng Xu,Subhroshekhar Ghosh,Yan Shuo Tan*

Main category: stat.ML

TL;DR: 本文为高维回归和分类中的经验风险最小化（ERM）决策树建立了全面的统计理论，包括尖锐的oracle不等式、最小化最优速率，并引入了一种新的函数类来捕捉稀疏性、各向异性平滑度和空间异质性。
...
Method: 使用基于经验局部化Rademacher复杂度的新型均匀集中框架，建立尖锐的oracle不等式。引入并分析了一个新的函数类：分段稀疏异质各向异性Besov（PSHAB）空间，该空间明确捕捉了实践中遇到的三个关键结构特征：稀疏性、各向异性平滑度和空间异质性。在亚高斯噪声条件下建立主要结果，同时提供在重尾噪声设置下的鲁棒保证。

Result: 建立了ERM决策树相对于最多L个叶子的最佳可能逼近的过剩风险的尖锐oracle不等式，刻画了可解释性-准确性权衡。在PSHAB空间上推导了最小化最优速率。提供了在亚高斯和重尾噪声设置下的统计保证。

Conclusion: 这些发现为ERM决策树的最优性提供了原则性基础，并引入了经验过程工具，这些工具广泛适用于其他高度自适应的数据驱动过程。该工作填补了ERM决策树统计理论的重要空白。
...
### [133] [Harnessing Synthetic Data from Generative AI for Statistical Inference](https://arxiv.org/abs/2603.05396)
*Ahmad Abdel-Azim,Ruoyu Wang,Xihong Lin*

Main category: stat.ML

TL;DR: 该论文从统计学角度综述了合成数据的生成与使用现状，旨在阐明合成数据能够有效支持下游发现、推断和预测的前提假设，并讨论其局限性、常见陷阱及新兴的规范使用框架。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型的兴起极大地扩展了合成数据在科学、工业和政策领域的可用性和使用。这些发展为数据分析开辟了新可能性，但也引发了关于合成数据何时能够以有效、可靠和规范方式使用的基本统计问题。论文旨在从统计学角度澄清合成数据有意义支持下游任务的前提假设。

Method: 论文采用综述研究方法：1）调查现代生成模型的主要类别、预期用例及其优势；2）分析合成数据的局限性和特征性失效模式；3）检查将合成数据视为真实观测替代品时的常见陷阱；4）基于这些见解讨论合成数据规范使用的新兴框架。

Result: 论文系统性地识别了合成数据使用中的关键问题：模型误设导致的偏差、不确定性衰减、泛化困难等常见陷阱。同时梳理了现代生成模型的能力边界和特征性失效模式，为合成数据的规范使用提供了理论基础。

Conclusion: 论文提出了实用的建议、开放问题和注意事项，旨在指导方法开发者和应用研究人员。强调需要在明确假设下规范使用合成数据，平衡其优势与局限性，并指出了未来研究的关键方向。
...
### [134] [Uniform convergence of kernel averages under fixed design with heterogeneous dependent data](https://arxiv.org/abs/2603.04681)
*Danilo Hiroshi Matsuoka,Hudson da Silva Torrent*

Main category: math.ST

TL;DR: 该论文为等间距固定设计点下的核平均提供了均匀收敛速率，适用于时间序列非参数方法


<details>
  <summary>Details</summary>
Motivation: 现有随机设计结果（如Hansen 2008和Kristensen 2009）依赖于密度条件论证，而本文针对时间序列在确定性网格上的观测，利用网格结构提供均匀收敛速率

Method: 在强混合和矩条件下，分析等间距固定设计点（x_{t,T}=t/T）上的核平均，不要求平稳性，框架适用于依赖三角阵列

Result: 推导了弱和强均匀一致性的收敛速率，作为应用给出了具有时变自回归误差的非参数回归模型中局部线性估计量的均匀收敛速率

Conclusion: 该分析补充了现有随机设计结果，特别适用于确定性网格上观测的时间序列非参数方法，理论结果通过蒙特卡洛实验和实证应用得到验证
...
### [135] [Sequential Multiple Testing: A Second-Order Asymptotic Analysis](https://arxiv.org/abs/2603.04685)
*Jingyu Liu,Yanglei Song*

Main category: math.ST

TL;DR: 该论文建立了顺序多重检验中二阶渐近最优性的统一理论，证明了在一般条件下二阶贝叶斯最优性蕴含二阶频率最优性，并推导了最小可实现期望样本量的二阶渐近展开。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究提出了顺序多重检验中一阶最优的检验程序，但这些程序在误差容忍水平趋近于零时，其期望样本量与最小可实现期望样本量的比值收敛于1。然而，一阶最优性无法保证两者之间的绝对差异保持有界。因此，需要建立二阶渐近最优性理论，以更精确地描述检验程序的性能。

Method: 作者发展了一个统一的二阶渐近最优性理论框架。首先建立了在一般充分条件下，二阶贝叶斯最优性蕴含二阶频率最优性的理论结果。然后推导了最小可实现期望样本量的二阶渐近展开，识别了来自多维随机游走边界穿越问题的二阶修正项。最后将该理论应用于多种常用误差度量。

Result: 证明了多个已知的一阶最优检验程序实际上是二阶最优的：对于所有信号配置，当误差容忍水平趋近于零时，这些程序的期望样本量与最小可实现期望样本量之间的差异保持一致有界。获得了最小可实现期望样本量的精确二阶渐近展开，为经典的一阶近似提供了更精细的修正。

Conclusion: 该研究建立了顺序多重检验中二阶渐近最优性的统一理论框架，不仅证明了多个现有检验程序的二阶最优性，还提供了最小可实现期望样本量的精确二阶渐近展开，深化了对顺序多重检验理论的理解，并为实际应用提供了更精确的性能评估工具。
...
### [136] [The augmented van Trees inequality](https://arxiv.org/abs/2603.04686)
*Elliot H. Young*

Main category: math.ST

TL;DR: 提出增强版van Trees不等式，相比经典版本能提供更紧的极小化极大风险下界，支持边界非零先验，在非参数估计中可获得更尖锐常数甚至精确常数


<details>
  <summary>Details</summary>
Motivation: 经典van Trees不等式在某些情况下无法获得紧致的极小化极大风险下界，特别是在先验密度边界非零或需要精确常数估计时存在局限性

Method: 提出增强版van Trees不等式，扩展经典不等式以容纳边界非零先验分布，并将其应用于非参数估计问题，包括正态误差回归模型和高维Hölder光滑回归函数估计

Result: 增强不等式能获得比经典van Trees不等式和Le Cam实验收敛理论更尖锐的常数，在某些情况下可获得精确常数；在单变量可微回归函数估计中达到1.37倍常数因子，在高维Hölder光滑回归中可获得精确常数

Conclusion: 增强版van Trees不等式提供了更灵活强大的极小化极大下界工具，适用于超越高斯性模型、平方误差损失之外的其他损失函数，并能扩展到不规则模型的广义van Trees不等式
...
### [137] [Strong consistency of the local linear estimator for a generalized regression function with dependent functional data](https://arxiv.org/abs/2603.04690)
*Danilo Hiroshi Matsuoka,Hudson da Silva Torrent*

Main category: math.ST

TL;DR: 研究广义非参数标量对函数回归模型，针对异质分布和强混合数据，建立局部线性估计量的几乎完全收敛速率，证明依赖数据下收敛速率可能慢于独立数据，并通过模拟和能源预测验证FLL优于FLC


<details>
  <summary>Details</summary>
Motivation: 现有标量对函数回归模型通常假设数据独立同分布，但实际应用中数据常呈现异质分布和序列依赖性（如时间序列数据），需要建立适用于依赖数据的非参数回归理论框架

Method: 采用广义非参数标量对函数回归模型，使用局部线性估计方法（FLL）处理函数型协变量，在强混合数据假设下建立理论框架，通过模拟比较FLL与局部常数估计（FLC）的性能

Result: 建立了局部线性估计量的几乎完全收敛速率，证明在紧集上点态和一致收敛速率相同，依赖数据下收敛速率可能慢于独立数据，模拟显示FLL优于FLC，能源预测应用中FLL预测精度显著更高

Conclusion: 局部线性估计方法在异质分布和强混合数据下具有良好的理论性质和实际性能，为处理依赖数据的函数型回归问题提供了有效工具，在预测应用中表现出优越性
...
TL;DR: 提出FreST Loss，一种频率增强的时空训练目标，通过联合傅里叶变换将监督扩展到联合时空频谱，以更好地捕捉图结构信号的复杂时空依赖关系。


<details>
  <summary>Details</summary>
Motivation: 传统的直接预测模型通常依赖点状目标（如均方误差），无法捕捉图结构信号固有的复杂时空依赖。现有的频率域方法（如FreDF）虽然缓解了时间自相关，但往往忽略了空间和跨时空交互作用。

Method: 提出FreST Loss，利用联合傅里叶变换（JFT）将模型预测与真实值在统一的频谱域中对齐，有效解耦空间和时间上的复杂依赖关系。该方法通过频率增强的时空训练目标扩展监督到联合时空频谱。

Result: 理论分析表明该公式减少了与时间域训练目标相关的估计偏差。在六个真实世界数据集上的广泛实验证明，FreST Loss是模型无关的，并且通过更好地捕捉整体时空动态，持续改进了最先进的基线方法。

Conclusion: FreST Loss通过将监督扩展到联合时空频谱，有效解决了传统点状目标和现有频率域方法在捕捉复杂时空依赖方面的局限性，为图结构信号的预测提供了更全面的频谱监督框架。
...
### [139] [Oracle-efficient Hybrid Learning with Constrained Adversaries](https://arxiv.org/abs/2603.04546)
...
Method: 考虑结构化设置，约束对手从表达力强但固定的函数类R中选择标签。提出新的学习算法，利用ERM预言机高效运行，获得与假设类H和对手标签类R导出的类的Rademacher复杂度相关的遗憾界。技术工具包括带"截断熵正则化器"的Frank-Wolfe约简和"混合"鞅差序列和的新尾界。

Result: 主要结果是开发了一种新学习算法，在给定ERM预言机的情况下高效运行，并获得与假设类H和对手标签类R导出的类的Rademacher复杂度相关的遗憾界。关键推论是给出了在随机零和博弈中计算均衡的预言机高效算法，适用于高维动作集但具有低维结构支付函数的情况。

Conclusion: 本文在混合学习设置中朝着同时实现统计最优性和计算效率迈出了重要一步，通过约束对手标签选择的结构化设置，结合新的技术工具，为混合在线学习问题提供了更优的解决方案，并扩展了随机零和博弈均衡计算的应用。
...
### [140] [Distributional Equivalence in Linear Non-Gaussian Latent-Variable Cyclic Causal Models: Characterization and Learning](https://arxiv.org/abs/2603.04780)
*Haoyue Dai,Immanuel Albrecht,Peter Spirtes,Kun Zhang*

Main category: cs.LG

TL;DR: 该论文针对含隐变量的因果发现问题，提出了线性非高斯模型下任意隐变量结构和循环图的等价性判定准则，并开发了相应的发现算法。
...
Method: 1. 建立线性非高斯模型中任意隐变量结构和循环图的分布等价性图形准则；2. 引入新工具"边秩约束"作为隐变量因果发现的关键补充；3. 提供遍历整个等价类的过程；4. 开发从数据中恢复模型（达到等价性）的算法。

Result: 1. 首次在无结构假设的参数化设置中建立了含隐变量的等价性表征；2. 开发了首个无结构假设的发现方法；3. 提供了代码和交互演示。

Conclusion: 该工作填补了线性非高斯模型下隐变量因果发现的等价性表征空白，提供了首个无结构假设的发现方法，为更一般的隐变量因果发现奠定了基础。
...
### [141] [Non-Euclidean Gradient Descent Operates at the Edge of Stability](https://arxiv.org/abs/2603.05002)
*Rustem Islamov,Michael Crawshaw,Jeremy Cohen,Robert Gower*

Main category: cs.LG

TL;DR: 论文提出了一种通过方向平滑性视角解释边缘稳定性现象的理论框架，将EoS扩展到非欧几里得范数，定义了广义锐度度量，并在多种优化器中验证了该现象。


<details>
  <summary>Details</summary>
Motivation: 边缘稳定性现象在深度学习中广泛存在但理论解释不完整，需要建立一个统一的理论框架来解释不同优化器中的EoS现象，并扩展到非欧几里得几何。

Method: 通过方向平滑性视角解释EoS，定义广义锐度度量（在任意范数下的Hessian最大特征值），将框架扩展到非欧几里得GD、ℓ∞-下降、块坐标下降、谱GD和Muon等优化器。

Result: 实验表明，使用广义锐度度量的非欧几里得GD同样表现出渐进锐化后围绕阈值2/η的振荡行为，验证了框架的有效性和普适性。

Conclusion: 该研究提供了一个统一的几何感知谱度量框架，能够解释和预测多种优化器中的边缘稳定性现象，为理解深度学习优化动力学提供了新视角。
...
### [142] [Federated Causal Discovery Across Heterogeneous Datasets under Latent Confounding](https://arxiv.org/abs/2603.05149)
*Maximilian Hahn,Alina Zajak,Dominik Heider,Adèle Helena Ribeiro*

Main category: cs.LG

TL;DR: 提出了fedCI，一种联邦条件独立性检验方法，用于处理跨站点的异构数据集，支持混合变量类型，并在此基础上开发了fedCI-IOD算法，实现联邦环境下的因果发现。


<details>
  <summary>Details</summary>
Motivation: 多数据集因果发现面临数据隐私法规和跨站点异质性的限制，传统方法需要集中式数据集，无法满足隐私保护和异质性处理的需求。

Method: 使用联邦迭代重加权最小二乘法估计广义线性模型参数，实现条件独立性检验；开发fedCI-IOD算法，扩展IOD算法以支持联邦环境下的因果发现。

Result: fedCI-IOD在保护隐私的同时显著提升统计功效，性能接近完全池化分析，缓解了局部样本量小带来的伪影问题。

Conclusion: 提出的fedCI和fedCI-IOD为联邦条件独立性检验和因果发现提供了隐私保护、用户友好的解决方案，解决了分布式异构数据集中的因果发现问题。
...
### [143] [Towards a data-scale independent regulariser for robust sparse identification of non-linear dynamics](https://arxiv.org/abs/2603.05201)
*Jay Raut,Daniel N. Wilke,Stephan Schmidt*

Main category: cs.LG

TL;DR: STCV算法通过使用无量纲统计指标替代传统基于幅度的阈值处理，解决了数据归一化对稀疏回归方法（特别是SINDy框架）的负面影响，使系统辨识对数据缩放具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 数据归一化作为常见的预处理步骤，会严重扭曲基于幅度的稀疏回归方法（如SINDy）对控制方程的发现过程。归一化与测量噪声的相互作用会破坏稀疏性假设，导致发现密集、不可解释且物理不正确的模型。

Method: 提出序列变异系数阈值法（STCV），这是一种计算高效的稀疏回归算法。STCV使用无量纲统计指标"系数存在性"（CP）替代传统基于幅度的阈值处理，CP评估模型库中候选项的统计有效性和一致性，使发现过程对任意数据缩放具有不变性。

Result: 在规范动力系统和实际工程问题（包括物理质量-弹簧-阻尼器实验）上的综合基准测试表明，STCV在归一化、噪声数据集上始终显著优于标准STLSQ和E-SINDy方法。STCV能够成功识别正确的稀疏物理定律，而其他方法会失败。
...
### [144] [Delta-Crosscoder: Robust Crosscoder Model Diffing in Narrow Fine-Tuning Regimes](https://arxiv.org/abs/2603.04426)
*Aly Kassem,Thomas Jiralerspong,Negar Rostamzadeh,Golnoosh Farnadi*

Main category: cs.LG

TL;DR: Delta-Crosscoder改进了模型差异分析方法，通过结合BatchTopK稀疏性、基于变化的损失函数和隐式对比信号，有效识别微调导致的局部、非对称行为变化，在多种模型和任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Crosscoders方法在处理窄范围微调（行为变化局部化且非对称）时存在困难，需要更有效的模型差异分析技术来识别微调如何改变模型内部表示。

Method: Delta-Crosscoder结合了三种关键技术：1) BatchTopK稀疏性；2) 基于变化的损失函数，优先关注模型间变化的潜在方向；3) 从匹配输入的配对激活中获得的隐式对比信号。

Result: 在10个模型（包括Gemma、LLaMA、Qwen等1B-9B参数模型）和多种任务（合成虚假事实、涌现错位、潜意识学习、禁忌词猜测）上评估，Delta-Crosscoder能可靠地分离出导致微调行为的因果潜在方向，并实现有效缓解，性能优于基于SAE的基线方法，与非SAE方法相当。

Conclusion: Crosscoders仍然是模型差异分析的有力工具，Delta-Crosscoder的改进使其能够有效处理窄范围微调场景，为理解模型微调如何改变内部表示提供了更精确的方法。
...
### [145] [SurvHTE-Bench: A Benchmark for Heterogeneous Treatment Effect Estimation in Survival Analysis](https://arxiv.org/abs/2603.05483)
*Shahriar Noroozizadeh,Xiaobin Shen,Jeremy C. Weiss,George H. Chen*

Main category: cs.LG

TL;DR: SurvHTE-Bench：首个针对右删失生存数据的异质性治疗效果估计的综合性基准测试平台


<details>
  <summary>Details</summary>
Motivation: 生存分析中的异质性治疗效果估计在精准医学和个体化政策制定等高风险应用中至关重要，但由于删失、未观察到的反事实和复杂的识别假设，现有评估方法分散且不一致，缺乏统一标准

Method: 构建包含三个模块的基准测试平台：(1) 具有已知真实值的模块化合成数据集，系统性地改变因果假设和生存动态；(2) 结合真实世界协变量与模拟治疗和结果的半合成数据集；(3) 来自双胞胎研究（已知真实值）和HIV临床试验的真实世界数据集

Result: 在合成、半合成和真实世界设置中，首次在多样化条件和现实假设违反情况下对生存HTE方法进行了严格比较，为因果生存方法提供了公平、可重复和可扩展的评估基础

Conclusion: SurvHTE-Bench填补了生存分析中异质性治疗效果估计评估的空白，建立了统一、严谨的基准测试框架，促进了该领域方法的公平比较和进一步发展
...
### [146] [Thin Keys, Full Values: Reducing KV Cache via Low-Dimensional Attention Selection](https://arxiv.org/abs/2603.04427)
...
<details>
  <summary>Details</summary>
Motivation: 标准Transformer注意力中查询、键和值使用相同维度，但查询和键仅用于产生标量注意力权重（选择操作），而值承载丰富的语义表示（值传递）。选择本质上比值传递需要更低的维度，仅需O(log N)维度即可区分N个相关模式。

Method: 提出非对称注意力机制，降低查询和键的维度（d_select），保持值的高维度（d_model）。通过七个实验验证：1) 位置选择任务；2) 基于内容的检索；3-4) WikiText语言建模；5) GPT-2的SVD压缩；6) LLaMA模型验证；7) Mistral-7B的SVD压缩和QK微调。

Result: 在WikiText语言建模中，d_select = d_model/4仅增加4.3%困惑度，同时减少75%的QK参数。对GPT-2进行SVD压缩发现键比查询更易压缩。对Mistral-7B进行SVD压缩和QK微调，实现75%的键缓存节省，仅带来2.0%的质量损失。在7B参数模型中，非对称注意力可为每个用户节省25GB KV缓存，使相同GPU上并发用户数增加约60%。

Conclusion: 非对称注意力机制通过降低查询和键的维度来优化Transformer架构，显著减少参数和内存占用，同时保持模型性能。该方法对现有模型可通过SVD压缩和少量QK微调实现显著的内存节省，对长上下文应用具有重要实用价值。
...
### [147] [Agent Memory Below the Prompt: Persistent Q4 KV Cache for Multi-Agent LLM Inference on Edge Devices](https://arxiv.org/abs/2603.04428)
*Yakov Pyotr Shkolnikov*

Main category: cs.LG

TL;DR: 提出了一种在边缘设备上持久化存储和恢复多智能体LLM系统KV缓存的方法，通过4位量化将缓存保存到磁盘，消除重复的预填充计算，显著减少首token生成时间。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上的多智能体LLM系统面临内存管理问题：设备RAM太小，无法同时容纳所有智能体的KV缓存。在Apple M4 Pro上，FP16格式下8K上下文只能容纳3个智能体，10个智能体的工作流需要不断驱逐和重新加载缓存，每次驱逐都需要完整的预填充计算，耗时严重（4K上下文下每个智能体15.7秒）。

Method: 系统包含三个核心组件：1) 块池（block pool）提供每个智能体隔离的Q4 KV缓存，以safetensors格式存储；2) BatchQuantizedKVCache支持对多个智能体量化缓存的并发推理；3) 跨阶段上下文注入（cross-phase context injection）可在对话阶段间累积注意力状态而无需重新计算。通过将KV缓存以4位量化格式持久化到磁盘，并在需要时直接恢复到注意力层，避免了O(n)的预填充计算。

Result: 在三种架构上评估：Gemma 3 12B、DeepSeek-Coder-V2-Lite 16B和Llama 3.1 8B。缓存恢复将首token生成时间减少了高达136倍（Gemma：4K-32K上下文下22-136倍；DeepSeek：4K-32K上下文下11-76倍；Llama：4K-16K上下文下24-111倍，1K上下文下3-10倍）。Q4量化相比FP16在固定设备内存中可容纳4倍多的智能体上下文。使用实际Q4 KV缓存测量的困惑度变化：Gemma -0.7%，Llama +2.8%，DeepSeek +3.0%。

Conclusion: 该方法通过持久化存储和恢复量化KV缓存，有效解决了边缘设备上多智能体LLM系统的内存限制问题，显著减少了推理延迟，同时保持了可接受的模型质量损失。Q4量化使内存效率提高了4倍，为边缘设备上的多智能体应用提供了实用的解决方案。
...
### [148] [Flowers: A Warp Drive for Neural PDE Solvers](https://arxiv.org/abs/2603.04430)
*Till Muser,Alexandra Spitzer,Matti Lassas,Maarten V. de Hoop,Ivan Dokmanić*

Main category: cs.LG

TL;DR: Flowers是一种完全由多头扭曲构建的PDE求解算子学习神经架构，通过位移场预测实现自适应全局交互，线性计算成本下在流体和波动PDE基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一种高效、物理启发的PDE求解算子学习架构，避免传统方法（傅里叶乘子、点积注意力、卷积混合）的计算瓶颈，实现自适应全局交互的线性成本。

Method: 完全基于多头扭曲：每个头预测位移场并扭曲混合输入特征；位移预测采用逐点方式，无空间聚合；非局部性仅通过稀疏采样（每个头一个源坐标）引入；通过多尺度残差块堆叠扭曲层。

Result: 在广泛的2D和3D时间依赖PDE基准测试中表现优异，特别在流体和波动问题上；紧凑的17M参数模型优于相似规模的傅里叶、卷积和注意力基线；150M参数变体超越参数更多、数据更丰富、训练计算量更大的transformer基础模型。

Conclusion: Flowers架构通过纯扭曲操作实现了高效的自适应全局交互，为PDE求解算子学习提供了物理启发、计算高效的替代方案，在多个基准测试中超越了现有方法。
...
### [149] [Uncertainty-Calibrated Spatiotemporal Field Diffusion with Sparse Supervision](https://arxiv.org/abs/2603.04431)
...
<details>
  <summary>Details</summary>
Motivation: 物理场通常只在稀疏、时变的传感器位置观测到，这使得预测和重建成为病态且不确定性关键的问题。现有方法通常在密集再分析或模拟数据上训练，仅在稀疏条件下测试，而SOLID旨在仅从稀疏观测中端到端学习。

Method: SOLID采用掩码条件扩散框架：1) 训练和评估仅使用观测目标位置，无需密集场和预插值；2) 每个去噪步骤以测量值及其位置为条件；3) 引入双重掩码目标：强调未观测空白区域的学习，同时加权输入和目标提供最可靠锚点的重叠像素；4) 严格稀疏条件路径实现与测量一致的全场后验采样。

Result: SOLID在严重稀疏条件下实现了高达一个数量级的概率误差改进，产生校准的不确定性图（ρ > 0.7），在仅使用稀疏监督的情况下显著优于现有方法。

Conclusion: SOLID框架通过严格稀疏条件扩散方法，仅从稀疏观测中有效学习时空动力学，实现了准确的后验全场重建和校准的不确定性量化，为物理场稀疏观测问题提供了创新解决方案。
...
### [150] [ZorBA: Zeroth-order Federated Fine-tuning of LLMs with Heterogeneous Block Activation](https://arxiv.org/abs/2603.04436)
...
Method: 提出ZorBA框架，包含三个关键技术：1) 零阶优化消除客户端梯度存储；2) 异构块激活机制，服务器为不同客户端分配不同的Transformer块子集；3) 共享随机种子和梯度有限差分减少通信开销。通过理论分析块激活决策对收敛速度和VRAM使用的影响，并设计ε约束词典算法优化块激活决策。

Result: 实验结果显示，ZorBA在VRAM使用上比三种联邦微调基线方法最多减少62.41%，同时保持较低的通信开销。

Conclusion: ZorBA通过零阶优化和异构块激活机制，有效解决了联邦微调LLMs时的VRAM使用和通信开销问题，为资源受限环境下的分布式模型微调提供了高效解决方案。
...
### [151] [Learning Unified Distance Metric for Heterogeneous Attribute Data Clustering](https://arxiv.org/abs/2603.04458)
*Yiqun Zhang,Mingjie Zhao,Yizhou Chen,Yang Lu,Yiu-ming Cheung*

Main category: cs.LG

TL;DR: 提出HARR（异构属性重构与表示学习）范式，用于混合数据聚类，通过将异构属性转换为同构状态进行距离度量学习，并自动适应不同聚类任务


<details>
  <summary>Details</summary>
Motivation: 现实聚类任务中常包含数值和分类属性的混合数据，但现有方法要么将异构属性编码为单一类型，要么定义统一度量，未能揭示其内在联系

Method: 提出HARR学习范式，将每个属性的值投影到统一可学习的多个空间中，更精细地表示和学习分类数据的距离度量，并与聚类集成实现自适应

Result: HARR在准确性和效率方面表现出优越性，无参数、收敛有保证，并能更有效地自适应不同聚类数量k

Conclusion: HARR通过揭示异构属性间的连接，提供了一种有效的混合数据聚类方法，优于现有方法
...
### [152] [VSPrefill: Vertical-Slash Sparse Attention with Lightweight Indexing for Long-Context Prefilling](https://arxiv.org/abs/2603.04460)
*Chen Guanzhong*
...
Method: 提出VSPrefill机制，使用注意力分布中的垂直-斜线结构模式。VSIndexer模块从增强RoPE的键值表示中预测垂直列和斜线对角线的上下文感知重要性分数，以线性复杂度构建稀疏掩码而不修改主干参数。推理时采用自适应累积阈值策略分配每层稀疏预算，融合内核执行带实时索引合并的注意力
...
Conclusion: VSPrefill在准确性和效率之间的权衡上建立了新的帕累托前沿，为长上下文推理提供了有效的稀疏注意力解决方案
...
### [153] [MAD-SmaAt-GNet: A Multimodal Advection-Guided Neural Network for Precipitation Nowcasting](https://arxiv.org/abs/2603.04461)
*Samuel van Wonderen,Siamak Mehrkanoon*

Main category: cs.LG

TL;DR: MAD-SmaAt-GNet是一种用于降水临近预报的多模态物理引导深度学习模型，通过集成多天气变量编码器和物理平流组件，在SmaAt-UNet基础上提升了预测精度和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 传统数值预报方法计算成本高且未能充分利用大量可用天气数据。深度学习模型在降水临近预报中显示出潜力，但现有CNN模型如SmaAt-UNet缺乏多模态输入处理和物理一致性约束。

Method: 提出MAD-SmaAt-GNet模型，在SmaAt-UNet基础上：(1) 增加额外编码器处理多天气变量（多模态输入）；(2) 集成基于物理的平流组件确保预测的物理一致性。通过消融实验验证各组件贡献。

Result: 相比基线SmaAt-UNet，MAD-SmaAt-GNet在4小时4步降水预报中将均方误差降低8.9%。多模态输入对短预报时效特别有益，平流组件在短长期预报中均提升性能。

Conclusion: 多模态输入和物理引导的集成显著提升降水临近预报性能，证明了结合数据驱动方法和物理约束的有效性，为深度学习在气象预报中的应用提供了新方向。
...
### [154] [Understanding the Dynamics of Demonstration Conflict in In-Context Learning](https://arxiv.org/abs/2603.04464)
*Difan Jiao,Di Wang,Lijie Hu*

Main category: cs.LG

TL;DR: 该论文研究大语言模型在上下文学习中对冲突演示的处理机制，发现模型在存在规则冲突的演示时性能显著下降，并揭示了模型内部处理冲突证据的两阶段计算结构。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在上下文学习中对噪声和冲突演示的脆弱性。虽然上下文学习使模型能够通过少量演示执行新任务，但演示本身可能包含噪声和冲突示例，这种能力因此变得脆弱。作者旨在理解模型如何处理此类冲突，特别是需要模型推断底层模式的任务。

Method: 使用线性探测和logit lens分析技术研究模型内部处理机制。首先评估模型在存在规则冲突演示时的性能下降情况，然后分析模型内部表示，发现模型在中间层同时编码正确和错误规则，但在后期层才形成预测置信度。进一步识别了两个关键阶段的注意力头：早期到中间层的"脆弱头"（Vulnerability Heads）和后期层的"易感头"（Susceptible Heads）。最后通过有针对性的消融实验验证发现。

Result: 发现单个规则冲突的演示就会导致模型性能显著下降。模型内部处理冲突证据呈现两阶段结构：中间层同时编码正确和错误规则，后期层才形成预测置信度。识别出两类关键注意力头：脆弱头在早期到中间层表现出位置注意力偏差，对冲突高度敏感；易感头在后期层显著减少对正确预测的支持。消融实验表明，屏蔽少量识别出的注意力头可将性能提升超过10%。

Conclusion: 大语言模型在上下文学习中对冲突演示的处理存在系统性误导行为，其内部采用两阶段计算结构处理冲突证据。通过识别和干预特定注意力头可以显著改善模型对冲突演示的鲁棒性，这为理解和改进大语言模型的推理能力提供了重要见解。
...
### [155] [Towards Explainable Deep Learning for Ship Trajectory Prediction in Inland Waterways](https://arxiv.org/abs/2603.04472)
*Tom Legel,Dirk Söffker,Roland Schätzle,Kathrin Donandt*

Main category: cs.LG

TL;DR: 该研究提出了一种基于LSTM的船舶轨迹预测模型，通过引入可训练船舶领域参数来增强模型的可解释性，特别针对内河航道复杂交互场景。


<details>
  <summary>Details</summary>
Motivation: 内河航道拥挤环境下的船舶轨迹预测对航行安全至关重要。虽然深度学习模型在预测精度上有所提升，但这些模型往往缺乏可解释性，可能隐藏不准确的逻辑并削弱对其可靠性的信任。

Method: 采用基于LSTM的船舶轨迹预测模型，结合可训练的船舶领域参数，通过注意力机制融合交互船舶的隐藏状态。该方法在内河航道复杂交互场景下进行深入分析，评估预测性能并分析生成的船舶领域值的合理性。

Result: 在5分钟预测时间范围内，最终位移误差约为40米，与类似研究性能相当。船舶间注意力架构提高了预测精度，但学习到的船舶领域值分配的权重与预期存在偏差，表明精度提升不完全由预测轨迹与附近船舶轨迹之间的因果关系驱动。

Conclusion: 该模型通过其内在可解释的设计展示了良好的解释能力。未来工作将集中于利用该架构进行反事实分析，并整合更复杂的注意力机制。
...
### [156] [Augmenting representations with scientific papers](https://arxiv.org/abs/2603.04516)
...
<details>
  <summary>Details</summary>
Motivation: 天文学领域积累了大量的多模态数据（图像、光谱、时间序列）和文献分析，但这些数据源很少被系统整合。需要建立光谱数据与科学文献知识之间的连接，以加速对稀有或难以理解的天体源的解释

Method: 采用对比学习框架，对齐X射线光谱与从科学文献中提取的领域知识。使用混合专家策略，结合单模态和共享表示。通过光谱检索文本评估对齐效果

Result: 光谱检索文本的Recall@1%达到20%，证明两种模态之间有意义的对齐是可能的。融合光谱和文本数据后，对20个物理变量的估计比单模态光谱基线提高了16-18%。混合专家策略表现最优。多模态潜在空间中的异常分析识别出高优先级后续观测目标

Conclusion: 该对比学习框架成功建立了光谱与科学文献之间的对齐，不仅加速了天体源的解释，还能识别需要后续研究的异常目标。该框架可扩展到其他需要将观测数据与现有文献对齐的科学领域
...
### [157] [Invariant Causal Routing for Governing Social Norms in Online Market Economies](https://arxiv.org/abs/2603.04534)
...
<details>
  <summary>Details</summary>
Motivation: 在线市场经济中，公平曝光、持续参与和平衡再投资等社会规范对长期稳定至关重要。规范从无数微观互动中涌现为宏观规律，使得因果归因和政策可迁移性困难。需要理解驱动这些涌现规范的因果机制，并设计能够引导规范向期望结果发展的原则性干预措施。

Method: 提出Invariant Causal Routing (ICR)因果治理框架，整合反事实推理与不变因果发现，分离真实因果效应与虚假相关性，构建跨异质环境保持有效的可解释、可审计政策规则。在基于真实数据校准的异质智能体模拟中进行验证。

Result: 在异质智能体模拟中，ICR相比相关性或覆盖率基线方法，产生了更稳定的规范、更小的泛化差距和更简洁的规则。表明因果不变性为治理提供了原则性和可解释的基础。

Conclusion: 因果不变性为治理提供了原则性和可解释的基础。ICR框架能够识别跨环境稳定的政策-规范关系，分离真实因果效应，构建有效的可审计政策规则，有助于在线市场经济的长期稳定。
...
### [158] [An LLM-Guided Query-Aware Inference System for GNN Models on Large Knowledge Graphs](https://arxiv.org/abs/2603.04545)
...
<details>
  <summary>Details</summary>
Motivation: 现有GNN推理加速方法（如剪枝、量化、知识蒸馏）存在局限性：1）无法根据查询的结构或语义自适应调整模型；2）模型以整体文件形式存储，需完全加载；3）无法仅加载与目标节点语义相关的邻居节点和模型组件。这导致在大知识图谱上产生过度的数据加载和冗余计算。

Method: 1）将训练好的GNN模型分解为细粒度组件，可根据查询子图结构部分加载；2）利用大语言模型生成可重用的查询模板，为每个任务提取语义相关的子图；3）实现查询感知的紧凑模型实例化，仅加载与目标节点语义相关的模型组件。

Result: 在6个大型知识图谱（最大达4200万节点、1.66亿边）上评估，KG-WISE相比最先进系统实现：1）推理速度提升最高达28倍；2）内存使用降低98%；3）在商业和开源LLM上均保持或提升准确率。

Conclusion: KG-WISE通过任务驱动的推理范式，解决了大知识图谱上GNN推理的效率瓶颈，实现了查询感知的模型加载和计算，在显著提升性能的同时保持模型准确性，为大知识图谱应用提供了高效的推理解决方案。
...
### [159] [Latent Particle World Models: Self-supervised Object-centric Stochastic Dynamics Modeling](https://arxiv.org/abs/2603.04553)
*Tal Daniel,Carl Qi,Dan Haramati,Amir Zadeh,Chuan Li,Aviv Tamar,Deepak Pathak,David Held*

Main category: cs.LG

TL;DR: LPWM是一种自监督的以对象为中心的世界模型，能够从视频数据中自主发现关键点、边界框和对象掩码，支持动作、语言和图像目标的灵活条件化，并在决策任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常需要监督或难以扩展到真实世界多对象数据集，LPWM旨在开发一种能够自主发现对象表示并适用于决策任务的端到端自监督世界模型。

Method: LPWM采用端到端训练，从视频中自主发现关键点、边界框和对象掩码，通过新颖的潜在动作模块建模随机粒子动力学，支持动作、语言和图像目标的灵活条件化。

Result: LPWM在多样化的真实世界和合成数据集上取得了最先进的结果，能够生成高质量的视频展开，并成功应用于目标条件模仿学习等决策任务。

Conclusion: LPWM展示了自监督对象中心世界模型在真实世界多对象数据集上的可扩展性和在决策任务中的实用性，为无监督场景分解和决策学习提供了有效框架。
...
### [160] [PDE foundation model-accelerated inverse estimation of system parameters in inertial confinement fusion](https://arxiv.org/abs/2603.04606)
*Mahindra Rautela,Alexander Scheinker,Bradley Love,Diane Oyen,Nathan DeBardeleben,Earl Lawrence,Ayan Biswas*

Main category: cs.LG

TL;DR: 该研究探索了在惯性约束聚变（ICF）中应用PDE基础模型解决逆问题，通过微调预训练模型和训练轻量级任务特定头，实现了从多模态观测中准确重建高光谱图像和估计系统参数。


<details>
  <summary>Details</summary>
Motivation: 当前PDE基础模型的下游评估主要关注前向问题（如自回归滚动预测），而逆问题（从观测反推系统参数）在ICF等应用中同样重要但研究不足。本研究旨在探索如何利用预训练的PDE基础模型解决ICF中的逆问题。

Method: 使用开放的JAG基准数据集，该数据集提供每个模拟的高光谱X射线图像和标量观测值。通过微调预训练的PDE基础模型（MORPH权重），并训练一个轻量级的任务特定头，联合重建高光谱图像和回归系统参数。进行了数据缩放实验（5%-100%训练集）以评估数据效率。

Result: 微调后的模型实现了准确的高光谱重建（测试MSE 1.2e-3）和强大的参数估计性能（R^2高达0.995）。数据缩放实验显示，随着训练数据增加，重建和回归损失均持续改善，在低数据区域边际收益最大。从预训练MORPH权重微调优于从头训练相同架构，表明基础模型初始化提高了ICF中数据有限逆问题的样本效率。

Conclusion: PDE基础模型的预训练初始化显著提升了ICF逆问题的样本效率，特别是在数据有限的情况下。该方法为利用基础模型解决复杂物理系统中的逆问题提供了有效途径，展示了预训练知识迁移到数据稀缺下游任务的价值。
...
### [161] [Engineering Regression Without Real-Data Training: Domain Adaptation for Tabular Foundation Models Using Multi-Dataset Embeddings](https://arxiv.org/abs/2603.04692)
*Lyle Regenwetter,Rosen Yu,Cyril Picard,Faez Ahmed*

Main category: cs.LG

TL;DR: TREDBench：包含83个真实世界表格回归数据集的基准，用于研究工程数据与非工程数据的领域结构差异，并提出基于嵌入的合成数据筛选方法，仅使用筛选后的合成数据继续预训练TabPFN 2.5，在工程回归任务上显著提升预测性能和数据效率。
...
Method: 1) 构建TREDBench基准，包含83个真实世界表格回归数据集，带有工程/非工程专家标签；2) 使用TabPFN 2.5的数据集级嵌入研究领域结构；3) 提出嵌入引导的合成数据筛选方法：生成并识别"工程类似"的合成数据集；4) 仅使用筛选后的合成任务对TabPFN 2.5进行继续预训练。
...
Conclusion: 有原则的合成数据筛选可以将程序生成器转化为领域相关的"数据引擎"，使基础模型能够在真实数据收集是主要瓶颈的数据稀疏科学和工业领域中实现改进，无需依赖真实工程样本。
...
### [162] [Probabilistic Dreaming for World Models](https://arxiv.org/abs/2603.04715)
*Gavin Wong*

Main category: cs.LG

TL;DR: 提出了一种基于概率方法的Dreamer模型改进，通过并行探索多个潜在状态和保持互斥未来假设，在MPE SimpleTag环境中性能提升4.5%，回报方差降低28%


<details>
  <summary>Details</summary>
Motivation: 现有Dreamer模型在想象学习方面存在局限性，需要更鲁棒和样本高效的世界模型学习方法。希望通过概率方法实现：(1) 并行探索多个潜在状态；(2) 保持互斥未来的不同假设，同时保留连续潜在变量的梯度特性

Method: 采用概率方法改进Dreamer模型，引入并行潜在状态探索机制，通过粒子方法维护互斥未来假设，保持连续潜在空间的梯度传播特性

Result: 在MPE SimpleTag领域评估中，方法优于标准Dreamer模型，获得4.5%的分数提升和28%的回报方差降低，表现出更好的性能和稳定性

Conclusion: 提出的概率方法有效提升了Dreamer模型的性能，但存在超参数（如粒子数K）随环境复杂度扩展的挑战，未来需要研究捕获世界模型认知不确定性的方法
...
### [163] [Count Bridges enable Modeling and Deconvolving Transcriptomic Data](https://arxiv.org/abs/2603.04730)
*Nic Fishman,Gokul Gowri,Tanush Kumar,Jiaqi Lu,Valentin de Bortoli,Jonathan S. Gootenberg,Omar Abudayyeh*

Main category: cs.LG

TL;DR: Count Bridges：一种用于整数计数数据的随机桥过程，提供精确可处理的扩散式模型，支持从聚合观测中进行直接训练和系统解卷积


<details>
  <summary>Details</summary>
Motivation: 许多生物检测技术（如RNA测序）产生反映检测分子数量的整数值计数数据，但这些测量通常不是单细胞分辨率，而是细胞集合的聚合观测。现有生成框架（如扩散和流匹配）扩展到非欧几里得和离散设置后，仍不清楚如何最佳建模整数值数据或如何系统解卷积聚合观测。

Method: 提出Count Bridges——整数上的随机桥过程，为计数数据提供精确可处理的扩散式模型，具有闭式条件分布用于高效训练和采样。通过期望最大化风格方法扩展该框架，将单元级计数作为潜变量，支持从聚合测量直接训练。

Result: 在整数分布匹配基准测试中实现最先进性能，在各项指标上优于流匹配和离散流匹配基线。成功应用于两个大规模生物学问题：单细胞基因表达数据的核苷酸分辨率建模（用于解卷积bulk RNA-seq），以及将多细胞空间转录组spot解析为单细胞计数谱。
...
### [164] [Distribution-Conditioned Transport](https://arxiv.org/abs/2603.04736)
*Nic Fishman,Gokul Gowri,Paolo L. B. Fischer,Marinka Zitnik,Omar Abudayyeh,Jonathan Gootenberg*

Main category: cs.LG

TL;DR: 提出分布条件化传输(DCT)框架，通过将传输映射条件化到源分布和目标分布的嵌入表示上，实现训练期间未见过的分布对之间的泛化传输。


<details>
  <summary>Details</summary>
Motivation: 科学应用需要能够泛化到训练期间未见过的源分布和目标分布的传输模型，而现有方法难以处理这种分布外泛化问题。

Method: DCT框架将传输映射条件化到学习到的源分布和目标分布的嵌入表示上，支持从流匹配到分布散度模型（如Wasserstein、MMD）等多种底层传输机制，并允许半监督学习。

Result: 在合成基准测试和四个生物学应用中展示了DCT的实际性能优势：单细胞基因组学中的批次效应转移、质谱流式细胞术数据的扰动预测、造血过程中克隆转录动态学习、T细胞受体序列进化建模。

Conclusion: DCT为分布条件化传输提供了一个通用框架，能够泛化到未见过的分布对，支持半监督学习，并在多个生物学应用中表现出优越性能。
...
### [165] [KindSleep: Knowledge-Informed Diagnosis of Obstructive Sleep Apnea from Oximetry](https://arxiv.org/abs/2603.04755)
*Micky C Nnamdi,Wenqi Shi,Cheng Wan,J. Ben Tamo,Benjamin M Smith,Chad A Purnell,May D Wang*

Main category: cs.LG

TL;DR: KindSleep是一个深度学习框架，通过整合临床知识和单通道血氧信号进行阻塞性睡眠呼吸暂停（OSA）诊断，在三个大型数据集上表现出色，提供可解释的临床概念预测。
...
Method: KindSleep是一个深度学习框架，整合临床知识与单通道患者特异性血氧信号及临床数据。首先从原始血氧信号中学习识别临床可解释概念（如去饱和指数和呼吸障碍事件），然后将这些AI衍生概念与多模态临床数据融合，估计呼吸暂停低通气指数（AHI）。

Result: 在来自国家睡眠研究资源（SHHS、CFS、MrOS）的三个大型独立数据集（总n=9,815）上评估。KindSleep在估计AHI评分方面表现出色（R2=0.917，ICC=0.957），在OSA严重程度分类方面持续优于现有方法，在不同人群中加权F1分数达到0.827至0.941。
...
### [166] [ConTSG-Bench: A Unified Benchmark for Conditional Time Series Generation](https://arxiv.org/abs/2603.04767)
...
<details>
  <summary>Details</summary>
Motivation: 条件时间序列生成在解决数据稀缺和实现因果分析方面至关重要，但缺乏标准化的基准框架来评估不同条件下的生成模型

Method: 引入ConTSG-Bench基准，包含大规模、对齐良好的数据集，涵盖多种条件模态和语义抽象层次，使用全面的指标套件评估生成保真度和条件遵循度

Result: 定量基准测试和深入分析揭示了当前方法的特征和局限性，特别是在复杂条件下的精确结构可控性和下游任务效用方面存在关键挑战

Conclusion: ConTSG-Bench填补了条件时间序列生成领域标准化评估的空白，为未来研究指明了方向，特别是在精确结构控制和复杂条件下的实用价值方面
...
### [167] [Distributional Reinforcement Learning with Information Bottleneck for Uncertainty-Aware DRAM Equalization](https://arxiv.org/abs/2603.04768)
*Muhammad Usama,Dong Eui Chang*

Main category: cs.LG

TL;DR: 提出基于分布风险敏感强化学习的均衡器参数优化框架，结合信息瓶颈潜在表示和条件风险价值优化，实现快速、最坏情况保证的均衡器优化。


<details>
  <summary>Details</summary>
Motivation: 现有均衡器参数优化方法存在计算成本高（眼图评估）、优化期望而非最坏情况性能、缺乏不确定性量化等问题，无法满足高速内存系统信号完整性的实际部署需求。

Method: 提出分布风险敏感强化学习框架：1) 信息瓶颈潜在表示实现速率-失真最优信号压缩；2) 蒙特卡洛dropout量化认知不确定性；3) 分位数回归实现显式最坏情况优化；4) PAC-Bayesian正则化保证泛化边界。

Result: 在800万波形数据上验证：4抽头和8抽头均衡器配置分别实现平均37.1%和41.5%性能提升，最坏情况保证33.8%和38.2%，相比Q-learning基线提升80.7%和89.1%；62.5%高可靠性分类消除多数配置的人工验证。

Conclusion: 该框架为生产规模均衡器优化提供了实用解决方案，具有认证的最坏情况性能保证，显著提升优化效率和可靠性。
...
Method: 提出一种新颖高效的方法，通过将策略迭代与扩散过程对齐，仅需评估简单高斯概率即可在策略设置中训练扩散策略。该方法还能自然地处理熵正则化，这在扩散策略中通常难以实现。

Result: 实验表明，所提方法能够产生多模态策略行为，并在IsaacLab和MuJoCo Playground的各种基准任务上取得优越性能。

Conclusion: 该方法克服了扩散策略在策略强化学习中的关键挑战，提供了一种高效训练扩散策略的新范式，能够生成多模态行为并实现卓越性能。
...
### [169] [Missingness Bias Calibration in Feature Attribution Explanations](https://arxiv.org/abs/2603.04831)
*Shailesh Sridhar,Anton Xue,Eric Wong*

Main category: cs.LG

TL;DR: MCal是一种轻量级后处理方法，通过微调冻结基础模型的线性头来校正特征重要性评分中的缺失性偏差，无需重新训练或架构修改。


<details>
  <summary>Details</summary>
Motivation: 现有解释方法由于缺失性偏差（当模型使用被消除的、分布外输入探测时产生的系统性失真）而产生不可靠的特征重要性评分。现有解决方案将其视为深层表示缺陷，需要昂贵的重新训练或架构修改。

Method: MCal是一种轻量级后处理方法，将缺失性偏差视为模型输出空间的表面伪影。该方法通过微调冻结基础模型输出上的简单线性头来校正偏差，无需重新训练或架构修改。

Result: 这种简单校正方法能持续减少缺失性偏差，在涵盖视觉、语言和表格领域的多样化医学基准测试中，与先前重量级方法相比具有竞争力甚至表现更优。

Conclusion: 缺失性偏差可以有效地视为模型输出空间的表面伪影，通过简单的后处理线性校正即可有效解决，无需昂贵的重新训练或架构修改。
...
### [170] [Why Is RLHF Alignment Shallow? A Gradient Analysis](https://arxiv.org/abs/2603.04851)
*Robin Young*

Main category: cs.LG

TL;DR: 论文证明基于梯度的对齐方法本质上是浅层的，因为梯度信号集中在决定危害的位置，超过危害决策点后梯度消失，导致无法实现深度对齐。


<details>
  <summary>Details</summary>
Motivation: 研究为什么大型语言模型的安全对齐通常是浅层的，试图解释为什么对齐模型与基础模型之间的KL散度集中在早期token，以及为什么标准对齐目标无法实现深度对齐。

Method: 使用序列级危害的鞅分解，推导出对齐梯度的精确表征：位置t的梯度等于条件期望危害与评分函数的协方差。引入危害信息I_t量化每个位置对危害的影响，并证明均衡KL散度跟踪这个量。最后提出基于恢复惩罚的目标，在所有位置创建梯度信号。

Result: 证明基于梯度的对齐方法中，超过危害决策点的位置接收零梯度信号，这解释了KL散度集中在早期token的实证观察。标准对齐目标无法产生深度对齐，无论优化质量如何。提出的恢复惩罚目标为所有位置提供梯度信号。

Conclusion: 标准对齐方法本质上是浅层的，因为梯度信号集中在危害决策点。需要新的目标函数（如恢复惩罚）在所有位置创建梯度信号，为数据增强技术提供理论依据，实现深度对齐。
...
### [171] [FedAFD: Multimodal Federated Learning via Adversarial Fusion and Distillation](https://arxiv.org/abs/2603.04890)
*Min Tan,Junchao Ma,Yinfu Feng,Jiajun Ding,Wenwen Pan,Tingting Han,Qian Zheng,Zhenzhong Kuang,Zhou Yu*

Main category: cs.LG

TL;DR: FedAFD：一种统一的多模态联邦学习框架，通过双向对抗对齐、粒度感知融合和相似性引导的集成蒸馏，解决模态/任务差异和模型异构性问题，提升客户端和服务器性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态联邦学习方法通常忽视个性化客户端性能，难以处理模态/任务差异以及模型异构性问题。需要一种能够同时提升客户端和服务器学习的统一框架。

Method: 提出FedAFD框架：客户端采用双向对抗对齐策略对齐本地和全局表示，缓解模态和任务差距；设计粒度感知融合模块自适应整合全局知识到个性化特征。服务器端提出相似性引导的集成蒸馏机制，基于特征相似性在共享公共数据上聚合客户端表示，并将融合知识蒸馏到全局模型中。

Result: 在IID和非IID设置下进行大量实验，证明FedAFD在客户端和服务器端均实现了优越的性能和效率。

Conclusion: FedAFD通过创新的客户端对齐融合机制和服务器端蒸馏策略，有效解决了多模态联邦学习中的模态差异、任务差异和模型异构性问题，为隐私保护下的跨模态协作学习提供了高效解决方案。
...
### [172] [U-Parking: Distributed UWB-Assisted Autonomous Parking System with Robust Localization and Intelligent Planning](https://arxiv.org/abs/2603.04898)
...
<details>
  <summary>Details</summary>
Motivation: 解决室内复杂环境下自主停车的挑战性问题，传统方法在GPS信号缺失的室内环境中难以实现精确定位和可靠导航

Method: 采用分布式UWB辅助系统架构，集成LLM辅助路径规划、鲁棒融合定位算法和轨迹跟踪控制技术

Result: 通过实车演示验证了系统在挑战性室内环境中的可靠自动泊车性能

Conclusion: U-Parking系统成功展示了UWB与LLM技术结合在自主停车领域的应用潜力，为室内自动驾驶提供了可行的解决方案
...
### [173] [BandPO: Bridging Trust Regions and Ratio Clipping via Probability-Aware Bounds for LLM Reinforcement Learning](https://arxiv.org/abs/2603.04918)
*Yuan Li,Bo Wang,Yufei Gao,Yuqian Yao,Xinyuan Wang,Zhangyue Yin,Xipeng Qiu*

Main category: cs.LG

TL;DR: BandPO取代PPO中的标准裁剪机制，通过Band算子将f-散度定义的信任域投影到动态、概率感知的裁剪区间，解决低概率动作更新受限问题，防止熵崩溃。


<details>
  <summary>Details</summary>
Motivation: PPO中的标准裁剪机制存在关键瓶颈：固定边界严格限制了低概率动作的向上更新空间，不成比例地抑制了高优势的尾部策略，导致快速熵崩溃。需要解决这一探索瓶颈。

Method: 提出Band-constrained Policy Optimization (BandPO)，用Band算子替代标准裁剪。Band是一个统一的理论算子，将f-散度定义的信任域投影到动态、概率感知的裁剪区间。将该映射表述为凸优化问题，保证全局最优数值解，并为特定散度推导闭式解。

Result: 理论分析证实Band有效解决了探索瓶颈。在多种模型和数据集上的广泛实验表明，BandPO始终优于标准裁剪和Clip-Higher，同时稳健地缓解了熵崩溃。

Conclusion: BandPO通过引入动态、概率感知的裁剪机制，解决了PPO中固定边界裁剪的限制，改善了低概率动作的探索能力，防止熵崩溃，提升了强化学习稳定性。
...
Method: 提出∇-Reasoner框架，核心是DTO（可微分文本优化），在解码循环中集成对token logits的可微分优化，利用LLM似然和奖励模型的梯度信号精炼文本表示，并加入拒绝采样和加速设计。
...
### [175] [WaterSIC: information-theoretically (near) optimal linear layer quantization](https://arxiv.org/abs/2603.04956)
*Egor Lifar,Semyon Savkin,Or Ordentlich,Yury Polyanskiy*

Main category: cs.LG

TL;DR: 该论文提出了一种名为WaterSIC的新型算法，用于将密集线性层转换为低精度表示，通过信息论分析压缩长度与输出差异的权衡，并证明其性能接近理论极限。


<details>
  <summary>Details</summary>
Motivation: 现有量化算法（如GPTQ）在将密集线性层转换为低精度时，其压缩性能与信息论极限可能存在较大差距，需要设计更接近理论极限的量化方法。

Method: 提出WaterSIC算法，采用类似信息论中"注水"策略的方法，为权重矩阵的不同列（输入特征）分配不同的量化比特率，以优化整体压缩性能。

Result: 理论证明WaterSIC算法在所有可能的输入激活协方差矩阵上，其速率差距仅为0.255比特；在Llama和Qwen系列LLM上的实验表明，该算法在1到4比特量化率范围内均达到了最先进的性能。

Conclusion: WaterSIC算法通过信息论指导的比特分配策略，显著提升了线性层量化的性能，接近理论极限，为大型语言模型的高效部署提供了有效的量化解决方案。
...
### [176] [Mixture of Universal Experts: Scaling Virtual Width via Depth-Width Transformation](https://arxiv.org/abs/2603.04971)
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE架构在深度和宽度上的物理维度扩展受限，需要新的扩展维度来提升模型容量而不增加每token计算成本。

Method: 提出MOUE架构，包含三个核心组件：交错旋转拓扑实现结构化专家共享，通用专家负载均衡进行深度感知暴露校正，以及带轻量轨迹状态的通用路由器实现连贯多步路由。

Result: MOUE在多个扩展场景下持续优于匹配的MoE基线达1.3%，能够渐进转换现有MoE检查点并获得高达4.2%的性能提升，揭示了MoE架构的新扩展维度。

Conclusion: MOUE通过引入虚拟宽度维度，成功克服了MoE在深度和宽度上的物理扩展限制，为MoE架构提供了新的扩展路径。
...
### [177] [Asymptotic Behavior of Multi--Task Learning: Implicit Regularization and Double Descent Effects](https://arxiv.org/abs/2603.05060)
*Ayed M. Alrashdi,Oussama Dhifallah,Houssem Sifaou*

Main category: cs.LG

TL;DR: 本文对多任务学习中感知器模型的误设情况进行精确渐近分析，揭示了多任务组合如何通过引入正则化项改善泛化性能，并实证表明多任务组合能推迟并渐近缓解双下降现象。


<details>
  <summary>Details</summary>
Motivation: 多任务学习旨在通过利用多个相关任务间的共同信息来改善泛化误差。然而，识别能够揭示不同但相关任务间共同信息的公式化方法是一个挑战。本文旨在精确分析多任务学习在误设感知器模型下的渐近行为，以理解多任务组合带来性能提升的根本原因。

Method: 采用精确渐近分析方法，研究误设感知器学习模型的多任务学习公式。通过理论分析证明多任务组合在渐近意义上等价于传统单任务公式加上额外的正则化项。同时进行实证研究，考察任务组合对泛化误差的影响。

Result: 理论分析表明，组合多个任务在渐近意义上等价于传统公式加上额外的正则化项，这些正则化项有助于改善泛化性能。实证研究显示，多任务组合能够推迟双下降现象的发生，并能在渐近意义上缓解该现象。

Conclusion: 本文通过精确渐近分析揭示了多任务学习改善泛化性能的机制：多任务组合本质上引入了正则化效应。实证结果进一步证实了多任务学习在缓解双下降现象方面的优势，为理解多任务学习的理论基础提供了重要见解。
...
### [178] [Reward-Conditioned Reinforcement Learning](https://arxiv.org/abs/2603.05066)
*Michal Nauman,Marek Cygan,Pieter Abbeel*

Main category: cs.LG

TL;DR: RCRL框架训练单一智能体优化一族奖励函数，仅需在单一标称目标下收集经验，通过奖励参数化条件化实现多目标学习
...
Method: 提出奖励条件化强化学习(RCRL)，智能体基于奖励参数化进行条件化，从共享回放数据中完全离策略学习多个奖励目标，使单一策略能表示奖励特定行为

Result: 在单任务、多任务和基于视觉的基准测试中，RCRL不仅提升标称奖励参数化下的性能，还能高效适应新参数化，提供可扩展的鲁棒策略学习机制

Conclusion: RCRL为学习鲁棒、可引导的策略提供了可扩展机制，无需牺牲单任务训练的简单性，解决了奖励错误设定和任务适应性问题
...
### [179] [Synchronization-based clustering on the unit hypersphere](https://arxiv.org/abs/2603.05067)
*Zinaid Kapić,Aladin Crnkić,Goran Mauša*

Main category: cs.LG

TL;DR: 提出基于广义Kuramoto模型的新聚类算法，专门处理单位超球面上的数据，相比传统方法在聚类准确率上表现相当或更好。


<details>
  <summary>Details</summary>
Motivation: 单位超球面聚类在基因表达分析、文本和图像分类等领域有重要应用，但传统聚类方法未考虑球面的几何结构，不适合处理此类数据。

Method: 基于d维广义Kuramoto模型开发新算法，专门用于聚类单位球面S^{d-1}上的数据点。

Result: 在合成和真实数据集上验证了方法的有效性，与传统聚类方法相比，在聚类准确率方面取得了相似或更好的结果。

Conclusion: 基于广义Kuramoto模型的算法为处理单位超球面数据提供了有效的聚类解决方案，能够更好地利用数据的几何结构。
...
### [180] [Aura: Universal Multi-dimensional Exogenous Integration for Aviation Time Series](https://arxiv.org/abs/2603.05092)
...
<details>
  <summary>Details</summary>
Motivation: 工业应用中时间序列预测需要整合多维甚至多模态的外生因素，这些异构信息具有不同的交互模式，传统单模态时间序列模型难以有效捕捉

Method: 提出Aura框架，根据外生因素与目标时间序列的交互模式，采用定制的三部分编码机制将异构特征嵌入到成熟的时间序列模型中

Result: 在中国南方航空波音777和空客A320机队的大规模三年工业数据集上，Aura在所有基线方法中均达到最先进性能，并展现出卓越的适应性

Conclusion: Aura作为通用增强框架，在航空安全和可靠性方面具有广泛应用潜力，能够有效整合异构外部信息提升时间序列预测准确性
...
### [181] [Axiomatic On-Manifold Shapley via Optimal Generative Flows](https://arxiv.org/abs/2603.05093)
*Cenwei Zhang,Lin Zhu,Manxi Lin,Lei You*

Main category: cs.LG

TL;DR: 提出基于最优生成流的流形上Aumann-Shapley归因理论，通过Wasserstein-2测地线解决路径模糊性，实现严格流形一致性并超越现有基线方法


<details>
  <summary>Details</summary>
Motivation: 传统Shapley归因方法因启发式基线选择而产生离流形伪影，现有生成方法存在几何效率低下和离散化漂移问题，需要建立严格的流形上归因理论

Method: 提出基于最优生成流的流形上Aumann-Shapley归因理论，证明梯度线积分是满足效率和几何公理（特别是重参数化不变性）的唯一泛函；通过选择动能最小化的Wasserstein-2测地线解决路径模糊性，将基线选择重构为变分问题

Result: 该方法在实验上超越基线，通过消失的流一致性误差实现严格流形一致性，并通过结构感知总变差获得优越的语义对齐；恢复加性模型的经典Shapley值，并对流近似误差提供可证明的稳定性边界

Conclusion: 通过将基线选择形式化为变分问题，建立了基于最优生成流的流形上Aumann-Shapley归因理论，解决了传统方法的离流形伪影问题，为可解释AI提供了严格的理论基础和实用方法
...
### [182] [Decoupling Task and Behavior: A Two-Stage Reward Curriculum in Reinforcement Learning for Robotics](https://arxiv.org/abs/2603.05113)
*Kilian Freitag,Knut Åkesson,Morteza Haghir Chehreghani*

Main category: cs.LG

TL;DR: 提出两阶段奖励课程学习，将任务目标与行为目标解耦，先训练简化任务奖励，再引入完整奖励，提高多目标强化学习的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在实际机器人控制应用中面临奖励函数设计困难的问题，特别是多目标优化时需要精确调整权重以获得期望策略特性。

Method: 提出两阶段奖励课程学习方法：第一阶段使用简化的任务专用奖励函数训练智能体，确保有效探索；第二阶段引入包含能量效率等辅助行为目标的完整奖励函数。分析不同过渡策略，强调阶段间样本重用对训练稳定性的重要性。

Result: 在DeepMind Control Suite、ManiSkill3和移动机器人环境（添加辅助行为目标）上验证，方法简单有效，显著优于直接在完整奖励上训练的基线方法，对特定奖励权重具有更高鲁棒性。

Conclusion: 两阶段奖励课程学习通过解耦任务目标与行为目标，有效解决了多目标强化学习中的奖励设计难题，提高了训练稳定性和策略性能。
...
### [183] [FedBCD:Communication-Efficient Accelerated Block Coordinate Gradient Descent for Federated Learning](https://arxiv.org/abs/2603.05116)
*Junkang Liu,Fanhua Shang,Yuanyuan Liu,Hongying Liu,Yuangang Li,YunXiang Gong*

Main category: cs.LG

TL;DR: 提出FedBCGD和FedBCGD+算法，通过参数块分割和选择性上传降低联邦学习中大规模模型（如Vision Transformer）的通信开销，通信复杂度降低至现有方法的1/N。
...
Method: 将模型参数分割为多个块（包括共享块），每个客户端只上传特定参数块；提出FedBCGD+算法，加入客户端漂移控制和随机方差缩减技术。

Result: 理论分析显示通信复杂度比现有方法降低1/N倍（N为参数块数），收敛速度更快；实验结果表明优于现有先进算法。

Conclusion: FedBCGD和FedBCGD+是首个针对大规模深度模型的参数块通信方法，能显著降低通信开销并加速收敛，代码已开源。
...
### [184] [Balancing Privacy-Quality-Efficiency in Federated Learning through Round-Based Interleaving of Protection Techniques](https://arxiv.org/abs/2603.05158)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中隐私保护机制存在挑战：差分隐私会降低学习质量，同态加密带来显著系统开销。需要一种能平衡隐私保护、学习质量和效率的解决方案
...
### [185] [Incentive Aware AI Regulations: A Credal Characterisation](https://arxiv.org/abs/2603.05175)
...
<details>
  <summary>Details</summary>
Motivation: 高风险的机器学习应用需要严格监管，但提供商常为降低成本而规避监管。现有监管框架难以有效执行，需要新的机制设计方法来解决这一挑战

Method: 将AI监管建模为机制设计问题，引入监管机制框架：将模型经验证据映射到市场份额许可证。提供商从许可证集合中选择，迫使其对模型合规能力下注。研究完美市场结果的机制设计条件

Result: 证明机制具有完美市场结果当且仅当非合规分布集合构成一个信度集（闭凸概率测度集）。建立了监管机制与非合规分布集合之间的对偶关系，连接了机制设计与不精确概率理论

Conclusion: 该框架为可执行的AI监管提供了理论基础，连接了机制设计与不精确概率理论，通过实验验证了在监管虚假特征使用和公平性方面的实际应用
...
<details>
  <summary>Details</summary>
Motivation: LoRA在微调大语言模型方面表现出色，但其理论基础不足，特别是在特征学习稳定性方面。研究发现LoRA在适当超参数和初始化下可实现自稳定，但必要的非零初始化会破坏这种稳定性，导致性能下降。

Method: 提出Stable-LoRA，一种权重收缩优化策略，通过在训练早期逐步收缩矩阵A来动态增强LoRA特征学习的稳定性。该方法保留了非零初始化的优势，同时消除了不稳定性。

Result: 实验表明Stable-LoRA在各种模型和任务上始终优于其他基线方法，且不增加内存使用，仅带来可忽略的计算开销。理论分析和实证验证都表明该方法能有效消除LoRA特征学习的不稳定性。

Conclusion: Stable-LoRA通过权重收缩策略解决了LoRA特征学习稳定性的根本问题，在保持LoRA参数效率优势的同时显著提升了性能，为LoRA的理论理解和实际应用提供了重要贡献。
...
<details>
  <summary>Details</summary>
Motivation: 术中不良事件的早期预警对降低手术风险和提高患者安全至关重要。现有深度学习方法存在三个主要问题：忽视不良事件之间的依赖关系、未充分利用异构临床数据、以及医疗数据集中固有的类别不平衡问题。

Method: 1. 构建首个术中多标签不良事件数据集(MuAE)，涵盖六种关键事件；2. 提出基于Transformer的多标签学习框架(IAENet)，包含改进的TAFiLM模块用于静态协变量和动态变量的鲁棒融合及复杂时间依赖建模；3. 引入带有共现正则化的标签约束重加权损失(LCRLoss)，有效缓解事件内不平衡并强制频繁共现事件间的结构化一致性。

Result: IAENet在5、10和15分钟早期预警任务上持续优于强基线方法，平均F1分数分别提升+5.05%、+2.82%和+7.57%。这些结果突显了IAENet在支持临床实践中智能术中决策的潜力。

Conclusion: 该研究成功解决了术中不良事件预测中的关键挑战，通过多标签学习框架有效建模事件依赖关系，融合异构临床数据，并处理类别不平衡问题。IAENet在早期预警任务上的优异表现展示了其在临床实践中的应用前景。
...
TL;DR: SlideSparse系统首次在商用GPU上为(2N-2):2N稀疏模式（如6:8）实现稀疏张量核心加速，通过滑动窗口分解和激活提升技术，在保持精度的同时获得接近理论上限的1.33倍加速。
...
Result: 在多种GPU（A100、H100、B200、RTX 4090、RTX 5080、DGX-spark）、精度（FP4、INT8、FP8、BF16、FP16）和模型家族（Llama、Qwen、BitNet）上评估。在计算受限工作负载中，Qwen2.5-7B的6:8权重稀疏度下，实测加速比达到1.33倍，接近理论上限N/(N-1)=4/3。

Conclusion: SlideSparse首次实现了(2N-2):2N稀疏模式在商用GPU上的稀疏张量核心加速，建立了保持精度的LLM加速实用路径，代码已开源。
...
### [189] [Recursive Inference Machines for Neural Reasoning](https://arxiv.org/abs/2603.05234)
*Mieszko Komisarczyk,Saurabh Mathur,Maurice Kraus,Sriraam Natarajan,Kristian Kersting*

Main category: cs.LG

TL;DR: 论文提出递归推理机（RIMs），将神经推理与经典推理机制结合，扩展了Tiny Recursive Models，在多个推理基准上取得更好性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经推理系统（如TRMs）结合神经主干与专门推理方案，而随机推理系统使用推理规则从随机模型推导复杂查询答案。本研究旨在桥接这两种范式，将经典推理引擎的递归推理机制显式融入神经推理框架。

Method: 提出递归推理机（RIMs）框架，显式整合受经典推理引擎启发的递归推理机制。将TRMs表达为RIMs的实例，并通过重新加权组件扩展它们，提升推理能力。

Result: RIMs在挑战性推理基准（ARC-AGI-1、ARC-AGI-2和Sudoku Extreme）上表现更好。在表格数据分类等其他任务上也优于TabPFNs，展示了泛化能力。

Conclusion: RIMs成功桥接了神经推理与经典推理范式，通过显式整合递归推理机制，为复杂推理任务提供了更强大的框架，并在多个基准上验证了其有效性。
...
### [190] [A Behaviour-Aware Federated Forecasting Framework for Distributed Stand-Alone Wind Turbines](https://arxiv.org/abs/2603.05263)
*Bowen Li,Xiufeng Liu,Maria Sinziiana Astefanoaei*

Main category: cs.LG

TL;DR: 提出两阶段联邦学习框架：首先基于长期行为统计使用DRS初始化和递归自动分割细化对风力涡轮机进行聚类，然后通过FedAvg训练集群特定的LSTM模型，在保护数据隐私的同时实现准确的风电功率预测。


<details>
  <summary>Details</summary>
Motivation: 准确的短期风电功率预测对电网调度和市场运营至关重要，但集中化涡轮机数据会引发隐私、成本和异构性等问题。需要一种既能保护数据隐私又能处理异构分布式涡轮机群的实用解决方案。

Method: 提出两阶段联邦学习框架：第一阶段使用双轮盘选择（DRS）初始化和递归自动分割细化，基于长期行为统计对风力涡轮机进行聚类；第二阶段通过联邦平均（FedAvg）算法为每个聚类训练特定的LSTM模型进行风电功率预测。

Result: 在丹麦400个独立风力涡轮机上的实验表明：DRS-auto方法能发现行为一致的涡轮机组群；在保持数据本地化的同时实现了有竞争力的预测精度；基于行为的分组方法持续优于地理分区方法，并与强大的k-means++基线方法表现相当。

Conclusion: 该研究提出了一种实用的隐私友好解决方案，适用于异构分布式涡轮机群。行为感知分组方法为风电预测提供了一种有效的联邦学习框架，在保护数据隐私的同时处理了设备异构性问题。
...
### [191] [Beyond Word Error Rate: Auditing the Diversity Tax in Speech Recognition through Dataset Cartography](https://arxiv.org/abs/2603.05267)
...
<details>
  <summary>Details</summary>
Motivation: 传统ASR评估主要使用词错误率(WER)，但该指标无法捕捉语义保真度，且掩盖了"多样性税"问题——边缘化和非典型说话者因系统性识别失败而承受的不成比例负担。需要更全面的评估框架来揭示隐藏的系统性偏见。

Method: 1) 系统评估非线性语义指标(如EmbER和SemDist)；2) 提出样本难度指数(SDI)，量化人口统计和声学因素如何驱动模型失败；3) 通过数据制图技术映射SDI，展示不同指标揭示的隐藏偏见。

Result: EmbER和SemDist等语义指标能够暴露WER忽略的隐藏系统性偏见和模型间分歧。SDI能够量化识别难度与说话者特征之间的关系，为模型审计提供新工具。

Conclusion: 该研究为ASR系统建立稳健的审计框架迈出第一步，使开发者能够在部署前审计和缓解ASR差异，实现前瞻性安全分析。
...
### [192] [Whispering to a Blackbox: Bootstrapping Frozen OCR with Visual Prompts](https://arxiv.org/abs/2603.05276)
*Samandar Samandarov,Nazirjon Ismoiljonov,Abdullah Sattorov,Temirlan Sabyrbayev*

Main category: cs.LG

TL;DR: Whisperer框架通过扩散模型预处理输入图像，以行为克隆方式增强冻结下游模型性能，在OCR任务上实现8%绝对字符错误率降低
...
Method: 提出Whisperer视觉提示框架，使用扩散模型作为预处理器，通过四阶段训练课程学习：随机采样扩散中间输出，选择偶然改善CER的结果，然后通过行为克隆训练模型复制这些改进

Result: 在30万张退化合成文本图像的挑战性数据集上，实现8%绝对（10.6%相对）字符错误率降低，超越CLAHE等手工设计基线方法
...
### [193] [Knowledge Divergence and the Value of Debate for Scalable Oversight](https://arxiv.org/abs/2603.05293)
*Robin Young*

Main category: cs.LG

TL;DR: 该论文建立了辩论与RLAIF之间的首个形式化联系，通过知识分歧的几何参数化证明了辩论优势的精确闭式解，并识别了三种知识分歧机制及其对应的辩论有效性阈值。


<details>
  <summary>Details</summary>
Motivation: AI安全领域中的辩论和RLAIF都是可扩展监督的方法，但缺乏形式化框架来关联两者并刻画辩论何时具有优势。需要建立理论联系以理解对抗性监督协议的适用条件。

Method: 通过模型表示子空间之间的主角度参数化知识分歧的几何结构，推导辩论优势的精确闭式解。分析三种知识分歧机制（共享、单边、组合）并证明存在性结果和负面结果。

Result: 当模型共享相同训练语料时，辩论退化为RLAIF类方法；知识分歧时辩论优势呈现从二次机制（辩论无效）到线性机制（辩论必需）的相变。组合机制中存在对抗性激励导致协调失败的尖锐阈值。

Conclusion: 建立了辩论与RLAIF的首个形式化连接，为理解对抗性监督协议的合理性提供了几何基础，并关联了从具有互补信息的模型中引出潜在知识的问题。
...
### [194] [WavSLM: Single-Stream Speech Language Modeling via WavLM Distillation](https://arxiv.org/abs/2603.05299)
*Luca Della Libera,Cem Subakan,Mirco Ravanelli*

Main category: cs.LG

TL;DR: WavSLM：首个无需文本监督的单流语音语言模型，通过量化WavLM表征实现自回归语音生成


<details>
  <summary>Details</summary>
Motivation: 现有语音语言模型大多依赖文本监督、分层token流或复杂混合架构，偏离了文本领域证明有效的单流生成预训练范式。语音中语义和声学信息的纠缠使得将自回归训练扩展到语音领域具有挑战性。
...
Conclusion: WavSLM证明了简单自回归训练范式可以扩展到语音领域，在单一token流中联合建模语义和声学信息，为语音语言模型提供了更简洁有效的解决方案。
...
TL;DR: GALACTIC：首个统一的无监督时间序列聚类反事实解释框架，包含局部实例级和全局集群级解释，通过最小描述长度目标生成简洁的全局摘要


<details>
  <summary>Details</summary>
Motivation: 现有时间序列聚类可解释性方法（基于特征归因或元数据）无法识别跨越集群边界的转换，而反事实解释主要局限于监督学习场景，缺乏统一的无监督聚类解释框架

Method: 提出GALACTIC框架：局部层面通过集群感知优化目标生成扰动，尊重目标和底层集群分配；全局层面制定代表性CE选择问题，采用最小描述长度目标提取非冗余的全局解释摘要，证明MDL目标具有超模性，可转化为单调子模集函数，实现高效贪心选择算法

Result: 在UCR Archive上的广泛实验表明，GALACTIC相比现有基线方法能产生显著更稀疏的局部CE和更简洁的全局摘要，提供首个通过反事实解释聚类时间序列的统一方法

Conclusion: GALACTIC是首个统一的无监督时间序列聚类反事实解释框架，成功桥接局部和全局可解释性，通过理论保证的高效算法提供稀疏的局部解释和简洁的全局摘要
...
### [196] [FairFinGAN: Fairness-aware Synthetic Financial Data Generation](https://arxiv.org/abs/2603.05327)
*Tai Le Quy,Dung Nguyen Tuan,Trung Nguyen Thanh,Duy Tran Cong,Huyen Giang Thi Thu,Frank Hopfgartner*

Main category: cs.LG

TL;DR: FairFinGAN：基于WGAN的金融数据生成框架，通过分类器引入公平性约束，在生成合成金融数据时减轻受保护属性的偏见，同时保持数据实用性。


<details>
  <summary>Details</summary>
Motivation: 金融数据集常存在偏见，可能导致自动化系统做出不公平决策。需要一种既能生成合成金融数据又能减轻偏见的方法，同时保持数据对下游预测任务的实用性。

Method: 提出FairFinGAN框架，基于Wasserstein GAN（WGAN）架构，通过分类器将公平性约束直接整合到训练过程中，确保生成的合成数据既公平又实用。

Result: 在五个真实金融数据集上评估，与现有GAN数据生成方法相比，FairFinGAN在公平性指标上表现更优，且未显著损失数据实用性。
...
### [197] [InfoFlow KV: Information-Flow-Aware KV Recomputation for Long Context](https://arxiv.org/abs/2603.05353)
...
<details>
  <summary>Details</summary>
Motivation: 长上下文问答中的检索增强生成(RAG)受限于对大量检索上下文进行推理时预填充的开销。现有方法依赖启发式规则或表示差异，未能建模所选token是否能有效影响生成过程
...
Conclusion: 基于信息流的KV缓存选择性重计算方法能有效识别对生成过程有影响的关键token，在保持效率的同时提升长上下文问答中检索增强生成的性能
...
### [198] [Embedded Inter-Subject Variability in Adversarial Learning for Inertial Sensor-Based Human Activity Recognition](https://arxiv.org/abs/2603.05371)
...
Conclusion: 提出的对抗框架通过整合主体间变异性概念，成功提升了人类活动识别的泛化性能，为处理主体间差异提供了有效解决方案
...
### [199] [On-Policy Self-Distillation for Reasoning Compression](https://arxiv.org/abs/2603.05433)
...
<details>
  <summary>Details</summary>
Motivation: 现有推理模型会产生大量噪声和冗余内容，这些内容不仅无用，甚至可能累积错误，需要一种方法来压缩推理过程

Method: OPSDC（On-Policy Self-Distillation for Reasoning Compression）：让模型在"be concise"指令下生成教师logits，然后在学生自己的rollouts上最小化每个token的反向KL散度，无需真实答案、token预算或难度估计器

Result: 在Qwen3-8B和Qwen3-14B上，MATH-500任务实现57-59%的token压缩，同时准确率提升9-16个绝对百分点；在AIME 2024上，14B模型准确率提升10个点，压缩率达到41%

Conclusion: 自我蒸馏能有效压缩推理模型的输出，自动区分简单和困难问题，在保持难问题深思熟虑的同时大幅压缩简单问题的推理过程，揭示推理模型产生的许多内容不仅冗余而且有害
...
### [200] [Latent Wasserstein Adversarial Imitation Learning](https://arxiv.org/abs/2603.05440)
*Siqi Yang,Kai Yan,Alexander G. Schwing,Yu-Xiong Wang*

Main category: cs.LG

TL;DR: LWAIL是一种新颖的对抗模仿学习框架，通过在水动力学感知的潜在空间中进行Wasserstein距离匹配，仅需少量状态专家轨迹即可实现专家级性能。


<details>
  <summary>Details</summary>
Motivation: 传统模仿学习方法需要大量中等至高质量演示以及专家动作数据，这些在实际应用中往往难以获取。需要开发一种能够减少数据需求、仅使用状态轨迹的模仿学习方法。

Method: 提出LWAIL框架：1）预训练阶段：使用少量随机生成的状态数据训练意图条件价值函数（ICVF），构建水动力学感知的潜在空间；2）对抗模仿学习阶段：在该潜在空间中计算Wasserstein距离进行状态分布匹配，仅需1个或少量状态专家轨迹。

Result: 在多个MuJoCo环境中实验表明，LWAIL优于先前基于Wasserstein的IL方法和对抗IL方法，在各种任务上取得更好结果，仅用少量状态专家轨迹即可达到专家级性能。

Conclusion: LWAIL通过在水动力学感知潜在空间中进行Wasserstein距离匹配，显著减少了模仿学习对专家数据的需求，仅需状态轨迹即可高效学习，为数据稀缺场景下的模仿学习提供了有效解决方案。
...
TL;DR: POET-X：一种可扩展且内存高效的谱保持训练框架，通过正交等价变换优化权重矩阵，显著降低计算成本和内存消耗，使单张H100 GPU能够训练十亿参数LLM。
...
Method: 提出POET-X，一种POET的改进变体，通过优化正交等价变换的实现方式，显著减少矩阵乘法的计算复杂度，同时保持谱保持特性。

Result: POET-X在保持POET泛化和稳定性优势的同时，大幅提升训练吞吐量和内存效率，能够在单张Nvidia H100 GPU上预训练十亿参数LLM，而AdamW等标准优化器在相同设置下会内存溢出。

Conclusion: POET-X成功解决了POET框架的计算效率问题，为大规模语言模型的高效稳定训练提供了实用的解决方案。
