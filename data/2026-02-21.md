...
Motivation: 年龄评估在法医和司法决策中至关重要，特别是对于无证件个人和无人陪伴未成年人。当前牙科年龄评估实践面临方法异质性、数据表示碎片化、临床-法医-法律信息系统互操作性有限等问题，这些问题阻碍了透明度和可重复性，而AI方法的采用进一步放大了这些挑战。

Method: 开发AIdentifyAGE领域特定本体论，提供标准化语义框架，涵盖人工和AI辅助法医牙科年龄评估工作流程。该本体论建模完整的法医-法律工作流程，整合司法背景、个体信息、法医检查数据、牙齿发育评估方法、放射影像、统计参考研究和AI估计方法。基于上层和已建立的生物医学、牙科和机器学习本体论开发，确保互操作性、可扩展性和FAIR原则合规性。

Result: AIdentifyAGE本体论建立了一个标准化语义框架，能够实现观察、方法、参考数据和报告结果之间的可追溯链接。它增强了法医牙科年龄评估的一致性、透明度和可解释性，为法医-法律和司法环境中的本体驱动决策支持系统奠定了坚实基础。

Conclusion: AIdentifyAGE本体论是提高法医牙科年龄评估一致性、透明度和可解释性的关键步骤，为法医-法律和司法环境中的本体驱动决策支持系统建立了稳健基础，解决了当前实践中的方法异质性和互操作性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 自适应系统经常在多个上下文中操作，同时重用固定的内部状态空间，这种单状态重用普遍存在于自然和人工智能中，但其基本的表示后果尚未被充分理解

Method: 将上下文建模为作用于共享内部状态的干预，证明任何再现上下文结果统计的经典模型都必须承担不可约的信息论成本，提供最小构造示例，并解释非经典概率框架如何通过放松单一全局联合概率空间假设来避免这种障碍

Result: 证明了上下文性不是量子力学的特性，而是经典概率表示中单状态重用的必然结果；任何再现上下文结果统计的经典模型都必须承担不可约的信息论成本；非经典概率框架可以通过放松单一全局联合概率空间假设来避免这种障碍

Conclusion: 上下文性是自适应智能的普遍表示约束，与物理实现无关，揭示了单状态重用在经典概率表示中的基本限制
...
<details>
  <summary>Details</summary>
Motivation: AI基准测试在衡量模型进展和指导部署决策中起核心作用，但许多基准测试很快饱和，无法区分最佳模型，降低了长期价值。需要了解哪些设计因素导致饱和，以创建更持久的评估方法。

Method: 从主要模型开发商的技术报告中选取60个大型语言模型基准测试，沿14个属性（任务设计、数据构建、评估格式）进行表征。测试五个假设，检验每个属性如何影响饱和率。

Result: 近半数基准测试表现出饱和现象，饱和率随基准测试年龄增长而增加。隐藏测试数据（公开vs私有）没有保护效果，而专家策划的基准测试比众包基准更能抵抗饱和。

Conclusion: 研究揭示了哪些设计选择能延长基准测试寿命，为创建更持久的评估策略提供了信息。专家策划的基准设计比数据隐藏更能有效抵抗饱和。
...
### [4] [Simple Baselines are Competitive with Code Evolution](https://arxiv.org/abs/2602.16805)
*Yonatan Gideoni,Sebastian Risi,Yarin Gal*

Main category: cs.AI

TL;DR: 简单基线方法在代码演化任务中表现优于复杂方法，揭示了当前代码演化研究中的评估缺陷


<details>
  <summary>Details</summary>
Motivation: 当前代码演化技术虽然展示了令人印象深刻的性能，但往往缺乏与简单基线的比较，需要验证这些复杂方法是否真正优于简单方法

Method: 在三个领域（数学边界优化、智能体脚手架设计、机器学习竞赛）测试两种简单基线方法，并与更复杂的方法进行比较分析

Result: 简单基线方法在所有三个领域都匹配或超越了更复杂的方法；数学边界优化的主要挑战是搜索空间设计而非搜索算法；智能体脚手架设计中高方差导致次优选择

Conclusion: 代码演化研究需要更严格的评估方法，减少评估随机性，并提出了未来工作的最佳实践和方向
...
TL;DR: 该论文改进了超立方体边切片问题中所需超平面数量的上界，从之前的⌈5n/6⌉降低到⌈4n/5⌉，并利用AI工具CPro1发现了Q10的8个超平面切片构造。


<details>
  <summary>Details</summary>
Motivation: 研究超立方体Q_n的边切片问题，即找到能切割所有边的最小超平面集合。这个问题在组合几何和离散几何中具有重要意义，Paterson在1971年给出了S(n) ≤ ⌈5n/6⌉的上界，但40多年来未得到改进。

Method: 使用新开发的AI工具CPro1，该工具结合推理大语言模型和自动超参数调优来创建搜索算法，发现了Q10的8个超平面切片构造。基于这个构造，通过归纳法推广到一般维度n，证明了改进的上界。

Result: 证明了S(n) ≤ ⌈4n/5⌉（当n不是5的奇数倍时），当n是5的奇数倍时S(n) ≤ 4n/5 + 1。这改进了Paterson的S(n) ≤ ⌈5n/6⌉上界。同时获得了当k<n时，k个超平面能切片的最大边数的新下界。

Conclusion: 成功将超立方体边切片问题的上界从⌈5n/6⌉改进到⌈4n/5⌉，展示了AI辅助数学发现工具CPro1在寻找复杂数学构造方面的有效性，为组合几何中的经典问题提供了实质性进展。
...
<details>
  <summary>Details</summary>
Motivation: 大规模科学设施面临分析延迟问题，特别是对于结构复杂的样品，传统的手动分析流程耗时且效率低下。需要一种能够加速从仪器数据到验证晶体结构和可发表CIF文件的完整流程的解决方案。

Method: NeuDiff Agent是一个受治理的工具使用AI工作流，通过限制操作到允许列表工具、在关键工作流边界实施故障关闭验证门、捕获完整溯源信息来实现。该系统在TOPAZ仪器上执行从数据还原、积分、精修到验证的完整晶体学分析流程。

Result: 在基准测试中，NeuDiff Agent将处理时间从435分钟（手动）减少到86.5(4.7)到94.4(3.5)分钟，加速4.6-5.0倍。生成的CIF文件通过验证，无checkCIF A级或B级警报，同时保持了完整的溯源和审计能力。

Conclusion: NeuDiff Agent为设施晶体学中部署智能AI提供了一条实用路径，在保持可追溯性和发表验证要求的同时，显著提高了分析效率和结果产出速度。
...
<details>
  <summary>Details</summary>
Motivation: 集中式AI在边缘计算中存在成本高、脆弱性、数据传输延迟、能耗大、依赖大型数据中心等问题，难以适应异构、移动和资源受限环境

Method: 节点持续从本地数据学习，维护自身模型状态，在协作有益时机会性地交换学习知识；学习通过重叠和扩散传播，而非全局同步或中心聚合

Result: 提出统一自主和协作行为的抽象框架，适应数据、硬件、目标和连接性的异构性，为去中心化学习奠定概念基础
...
Method: 1) 提出基于给定序的评分统一框架；2) 分析犹豫模糊元素上的经典序，证明其不构成格结构；3) 证明对称序定义的评分满足强单调性和Gärdenfors条件等规范准则；4) 引入优势函数类用于犹豫模糊元素排序，包含离散优势函数和相对优势函数两种具体实现。

Result: 1) 经典序不诱导格结构，反驳了先前主张；2) 对称序定义的评分满足评分函数的关键规范准则；3) 优势函数可用于构建典型犹豫模糊集上的模糊偏好关系并支持群体决策。

Conclusion: 基于序理论的评分框架为犹豫模糊集提供了形式化基础，对称序满足评分函数的规范要求，优势函数为犹豫模糊元素排序提供了实用工具，支持群体决策应用。
...
Method: OpenSage让LLM自动创建具有自生成拓扑结构和工具集的智能体，提供分层图结构内存系统，并配备专门针对软件工程任务的工具包
...
TL;DR: AgentLAB是首个专门评估LLM智能体对自适应长视野攻击脆弱性的基准，包含5种新型攻击类型、28个真实环境、644个安全测试用例，发现现有智能体高度脆弱且单轮防御无效。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在长视野复杂环境中的部署增加，它们面临利用多轮用户-智能体-环境交互的长视野攻击风险，这些攻击在单轮设置中无法实现，需要专门的评估基准来衡量智能体对此类风险的脆弱性。

Method: 提出AgentLAB基准，支持五种新型攻击类型：意图劫持、工具链攻击、任务注入、目标漂移和内存污染，涵盖28个真实智能体环境和644个安全测试用例，用于系统评估LLM智能体对长视野攻击的脆弱性。

Result: 评估代表性LLM智能体发现它们对长视野攻击高度脆弱，且为单轮交互设计的防御措施无法可靠缓解长视野威胁，表明现有安全防护存在显著不足。

Conclusion: AgentLAB作为首个专门评估LLM智能体长视野攻击脆弱性的基准，将为实际环境中智能体安全性的进展跟踪提供有价值的工具，基准已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂规划、长时程推理和世界知识整合方面仍存在明显不足，需要开发能够全面评估这些能力的基准测试，以揭示前沿模型的真实局限性和改进方向。

Method: 创建LLM-Wikirace基准测试，要求模型在维基百科超链接图中进行逐步导航，包含不同难度级别（简单和困难）。评估了包括Gemini-3、GPT-5、Claude Opus 4.5在内的广泛开源和闭源模型，分析其规划能力、世界知识利用和失败恢复机制。

Result: 前沿模型在简单任务上表现优异甚至超越人类，但在困难任务上性能急剧下降：最佳模型Gemini-3仅在23%的困难游戏中成功。世界知识是成功的基础但存在阈值，超过该阈值后规划和长时程推理能力成为主导因素。轨迹分析显示最强模型在失败后难以重新规划，经常陷入循环而非恢复。

Conclusion: LLM-Wikirace作为一个简单但有效的基准测试，清晰揭示了当前推理系统的局限性，特别是在复杂规划、长时程推理和失败恢复方面。该基准为规划能力强的LLMs提供了开放的竞技场，表明前沿模型在这些关键能力上仍有很大提升空间。
...
TL;DR: 对已对齐的视觉语言模型进行窄域有害数据集微调会导致严重的涌现性错位，这种错位会泛化到无关任务和模态，且多模态评估比单模态评估揭示更严重的错位问题


<details>
  <summary>Details</summary>
Motivation: 终身多模态智能体需要通过后训练持续适应新任务，但这在获取能力和保持安全对齐之间产生了根本性矛盾。研究旨在揭示对齐模型在窄域有害数据微调后出现的严重错位问题及其泛化特性

Method: 使用Gemma3-4B模型进行实验，通过LoRA微调研究错位随秩的变化，采用多模态和文本单模态评估对比，分析有害数据比例的影响，进行几何分析识别有害行为的低维子空间，并评估良性窄域微调和基于激活的引导两种缓解策略

Result: 错位随LoRA秩单调增加，多模态评估显示错位率(70.71±1.22@r=128)远高于文本单模态评估(41.19±2.51)；即使10%有害数据也会导致显著对齐退化；有害行为占据极低维子空间(10个主成分捕获大部分错位信息)；两种缓解策略能大幅减少错位但无法完全消除有害行为

Conclusion: 当前的后训练范式可能无法充分保持部署后的对齐性，需要开发更鲁棒的持续学习框架。单模态安全基准可能低估视觉语言模型的对齐退化程度，多模态评估对于全面评估安全风险至关重要
...
TL;DR: DeepContext是一个状态感知的监控框架，通过RNN架构捕获多轮对话中的意图演化轨迹，显著提升越狱攻击检测性能，同时保持实时推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全护栏大多是无状态的，将多轮对话视为独立事件处理，缺乏时间感知能力。这导致"安全漏洞"，使对抗性攻击（如Crescendo和ActorAttack）能够通过跨轮边界缓慢渗透恶意意图来绕过无状态过滤器。

Method: 提出DeepContext状态监控框架，采用RNN架构处理经过微调的轮级嵌入序列。通过在整个对话中传播隐藏状态，捕获无状态模型忽略的风险增量积累。

Result: DeepContext在多轮越狱检测中显著优于现有基线，达到0.84的F1分数，远超超大规模云提供商护栏（0.67）和领先开源模型如Llama-Prompt-Guard-2（0.67）和Granite-Guardian（0.67）。在T4 GPU上保持低于20ms的推理开销。

Conclusion: 建模意图的顺序演化是部署大规模无状态模型更有效且计算效率更高的替代方案，状态感知监控对于多轮对话安全至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 挑战AI系统黑盒安全评估的基本假设——模型在测试分布上的行为能可靠预测部署性能。作者认为这一假设在模型输出依赖于未观测内部变量（在评估中罕见但在部署中普遍）时会失效。

Method: 形式化潜在上下文条件策略的概念，通过Le Cam方法证明被动评估的最小最大下界，使用基于哈希的触发构造和Yao最小最大原理分析自适应评估，基于陷门单向函数假设展示计算分离，并为白盒探测提供样本复杂度分析和偏差校正。

Result: (1) 被动评估：任何估计器的期望绝对误差≥0.208δL；(2) 自适应评估：最坏情况误差≥δL/16，检测需要Θ(1/ε)查询；(3) 计算分离：拥有特权信息的部署环境可激活不安全行为，而多项式时间评估器无法区分；(4) 白盒探测：估计部署风险到精度ε_R需要O(1/(γ²ε_R²))样本。

Conclusion: 黑盒测试在统计上是未确定的，当模型依赖未观测内部变量时，需要额外的安全保障措施——架构约束、训练时保证、可解释性和部署监控——来确保最坏情况下的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 传统推荐基准主要评估模型模仿用户行为的能力，但在金融咨询领域，观察到的用户行为可能因市场波动而存在噪音或短视，且可能与用户的长期目标相冲突。将用户选择作为唯一真实标准会混淆行为模仿和决策质量。

Method: 构建Conv-FinRe基准：使用真实市场数据和人类决策轨迹，包含入职访谈、逐步市场情境和咨询对话。模型需要在固定投资期限内生成股票排名。基准提供多视角参考，区分描述性行为（用户实际选择）和规范性效用（基于投资者特定风险偏好）。通过受控咨询对话实例化评估。

Result: 评估了一系列最先进的LLM，结果显示理性决策质量和行为对齐之间存在持续张力：在基于效用的排名上表现良好的模型往往无法匹配用户选择，而行为对齐的模型可能过度拟合短期噪音。

Conclusion: Conv-FinRe基准超越了传统的行为模仿评估，能够诊断LLM是否遵循理性分析、模仿用户噪音或受市场动量驱动。数据集已在Hugging Face公开，代码库在GitHub可用，为金融推荐系统提供了更全面的评估框架。
...
Method: 提出Sonar-TS神经符号框架，采用搜索-验证流程：首先使用特征索引通过SQL查询候选窗口，然后通过生成的Python程序在原始信号上锁定和验证候选结果。

Result: 实验表明Sonar-TS能有效处理传统方法无法解决的复杂时间查询，并揭示了该领域的独特挑战。提出的NLQTSBench成为首个用于TSDB规模历史数据NLQ的大规模基准。
...
TL;DR: Cinder是一个两阶段匹配系统，通过Ruzicka相似度指数快速筛选，然后使用基于反正态分布技能桶的Kantorovich距离计算"制裁分数"，实现公平的预组队匹配。


<details>
  <summary>Details</summary>
Motivation: 现代多人游戏中，公平快速的匹配系统直接影响玩家留存和满意度。预组队（lobbies）之间的匹配尤其困难，因为基于平均技能指标的简单匹配方法在技能分布广泛或偏斜时会导致不平衡的对局。

Method: Cinder采用两阶段方法：第一阶段使用Ruzicka相似度指数快速比较预组队的"非异常值"技能范围进行初步筛选；第二阶段将玩家等级映射到基于反正态分布生成的技能桶中，然后使用Kantorovich距离计算预组队排序桶索引之间的"制裁分数"来量化匹配公平性。

Result: 通过分析1.4亿个模拟预组队配对的制裁分数分布，验证了系统的可行性，为公平匹配阈值提供了稳健基础。

Conclusion: Cinder系统能够为技能水平异质的预组队提供快速且公平的匹配，解决了传统平均技能匹配方法在技能分布广泛时的局限性。
...
TL;DR: M2F是首个用于端到端、项目规模数学自动形式化的智能体框架，能够在Lean中将长篇数学资料转换为可编译的形式化项目。


<details>
  <summary>Details</summary>
Motivation: 现有数学自动形式化方法仅限于孤立定理和短代码片段，无法扩展到教科书和研究论文规模，因为需要处理跨文件依赖、解析导入并确保整个项目端到端编译。

Method: 采用两阶段框架：1) 语句编译阶段将文档拆分为原子块，通过推断依赖关系排序，修复声明骨架直到项目可编译（证明中允许占位符）；2) 证明修复阶段在固定签名下使用目标导向的局部编辑填补证明空缺。整个过程保持验证器在循环中，仅在工具链反馈确认改进后才提交编辑。

Result: 在约三周内，M2F将479页的实分析和凸分析教科书转换为包含153,853行代码的Lean库，完全形式化为带有证明的Lean声明。在FATE-H基准测试中达到96%的证明成功率（基线为80%），实现了教科书规模的自动形式化。

Conclusion: M2F框架证明大规模自动化数学文献形式化是可行的，其效率通常需要专家数月或数年的努力，为数学自动形式化的实际应用开辟了新途径。
...
### [19] [Dynamic System Instructions and Tool Exposure for Efficient Agentic LLMs](https://arxiv.org/abs/2602.17046)
*Uria Franko*

Main category: cs.AI

TL;DR: ITR通过检索式方法为LLM智能体动态选择最小化的系统指令片段和必要工具子集，大幅降低上下文长度和成本


<details>
  <summary>Details</summary>
Motivation: LLM智能体在运行时需要反复加载长系统指令和大量工具，导致成本高、延迟大、工具选择错误多、智能体偏离概率增加

Method: 提出Instruction-Tool Retrieval (ITR)，一种RAG变体，每步检索最小系统提示片段和必要工具子集，组合成动态运行时系统提示，并提供置信度门控回退机制

Result: 在受控基准测试中，ITR将每步上下文token减少95%，正确工具路由相对提升32%，端到端成本降低70%，使智能体能在上下文限制内运行2-20倍更多循环

Conclusion: ITR特别适用于长运行自主智能体，其节省效果随智能体步数增加而复合，论文提供了方法细节、评估协议、消融研究和实际部署操作指南
...
### [20] [RFEval: Benchmarking Reasoning Faithfulness under Counterfactual Reasoning Intervention in Large Reasoning Models](https://arxiv.org/abs/2602.17053)
*Yunseok Han,Yejoon Lee,Jaeyoung Do*

Main category: cs.AI

TL;DR: 论文提出了推理忠实性的形式化框架和评估基准RFEval，发现大型推理模型在49.7%的输出中存在不忠实推理，准确性与忠实性关联微弱，后训练目标可能损害推理忠实性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）虽然表现出色，但常常产生听起来合理却不能反映真实决策过程的推理，这损害了可靠性和信任。现有评估主要关注准确性，缺乏对推理过程忠实性的系统评估。

Method: 提出了推理忠实性的形式化框架，包含两个可测试条件：立场一致性（推理与答案间的连贯立场）和因果影响（陈述的推理在输出级干预下因果驱动答案）。开发了RFEval基准，包含7,186个实例覆盖七个任务，通过受控的输出级反事实干预来探测忠实性。评估了12个开源LRMs。

Result: 49.7%的输出存在不忠实推理，主要源于立场不一致。失败集中在数学和代码等脆弱、收敛领域。忠实性与后训练机制相关度高于模型规模：在监督微调基础上添加当前RL风格目标可能降低推理忠实性，即使准确性得以保持。准确性与忠实性关联微弱且统计不显著。

Conclusion: 建立了审计LRM可靠性的严谨方法，表明可信AI不仅需要优化正确结果，还需要确保推理过程的结构完整性。准确性与忠实性脱钩，需要专门优化推理忠实性。
...
### [21] [Retaining Suboptimal Actions to Follow Shifting Optima in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.17062)
*Yonghyeon Jo,Sunwoo Lee,Seungyul Han*

Main category: cs.AI

TL;DR: S2Q通过连续子值函数学习保留替代高价值动作，解决传统值分解方法依赖单一最优动作、难以适应值函数变化的问题，在MARL基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于值分解的多智能体强化学习方法依赖单一最优动作，当训练过程中底层值函数发生变化时难以适应，常常收敛到次优策略。

Method: 提出连续子值Q学习(S2Q)，学习多个子值函数来保留替代的高价值动作，将这些子值函数整合到基于Softmax的行为策略中，促进持续探索并使总Q值能够快速适应变化的最优解。

Result: 在具有挑战性的多智能体强化学习基准测试中，S2Q一致优于各种MARL算法，展现出改进的适应性和整体性能。

Conclusion: S2Q通过子值函数学习保留替代动作，有效解决了传统值分解方法的局限性，在多智能体协作任务中实现了更好的探索和适应性。
...
### [22] [Predictive Batch Scheduling: Accelerating Language Model Training Through Loss-Aware Sample Prioritization](https://arxiv.org/abs/2602.17066)
*Sumedh Rasal*
...
Method: PBS使用在线训练的轻量级线性预测器，仅基于四个简单的标记级特征（标记频率、序列长度、词汇多样性和稀有标记比例）来估计样本难度，动态构建训练批次。

Result: 在130M参数transformer上的实验表明，PBS实现了6-13%的收敛加速（通过评估损失测量），预测器相关性在10,000训练步中从0.14提升到0.44。

Conclusion: 标记频率统计编码了关于样本难度的有意义信息，使得能够以可忽略的计算开销实现有效的课程学习，PBS为语言模型训练提供了高效优化方案。
...
Method: 提出通用人机协作框架，包含两个互补部分：STRIDE（提供原则性标准和评分系统，指导使用大语言模型构建公司级基准数据集）和SR-Delta（差异分析程序框架，揭示潜在调整的见解）。

Result: 该框架能够实现可持续发展评级方法论的可扩展和可比评估，为评估不同评级方法提供基准数据集。
...
TL;DR: 提出O-Shap方法，通过满足T属性的层次分割改进Owen值在图像解释中的应用，提升特征归因的准确性、语义一致性和计算效率。


<details>
  <summary>Details</summary>
Motivation: Shapley值方法在可解释AI中具有理论基础，但在视觉任务中特征独立性假设不成立，因为像素具有空间和语义依赖性。现有SHAP实现中的Owen值虽然支持分组归因，但其效果严重依赖于特征分组的定义方式，常用的分割方法（如轴对齐或SLIC）违反了关键的一致性属性。

Method: 提出新的分割方法，满足T属性以确保层次结构中的语义对齐。该方法通过层次化分组实现计算剪枝，同时提高归因准确性和可解释性。具体包括：1) 设计满足T属性的分割策略；2) 构建层次化特征分组；3) 在Owen值框架中应用这些分组。

Result: 在图像和表格数据集上的实验表明，O-Shap在归因精度、语义一致性和运行时效率方面优于基线SHAP变体，特别是在结构重要的情况下表现更佳。

Conclusion: 通过引入满足T属性的层次分割方法，O-Shap改进了Owen值在特征归因中的应用，解决了现有分割方法违反一致性属性的问题，从而在保持Shapley值理论基础的同时，提高了视觉任务中特征归因的准确性和可解释性。
...
### [25] [Instructor-Aligned Knowledge Graphs for Personalized Learning](https://arxiv.org/abs/2602.17111)
*Abdulrahman AlRabah,Priyanka Kargupta,Jiawei Han,Abdussalam Alawini*

Main category: cs.AI

TL;DR: InstructKG：基于课程讲义自动构建教师对齐的知识图谱框架，用于捕捉学习依赖关系


<details>
  <summary>Details</summary>
Motivation: 大规模课程中，教师难以诊断学生的知识缺口并提供个性化干预。现有知识图谱方法要么过于表层（关注课程级概念），要么忽略了教学材料中丰富的教学信号，无法准确捕捉概念间的学习依赖关系。

Method: 提出InstructKG框架，从课程讲义（幻灯片、笔记等）中自动提取重要概念作为节点，并推断学习依赖关系作为有向边（如"部分-整体"或"依赖"关系）。该框架将教育材料特有的丰富时序和语义信号（如"递归"在"归并排序"之前教授；"递归"在"归并排序"定义中被提及）与大型语言模型的泛化能力相结合。

Result: 通过在多个课程的真实世界多样化讲义材料上进行实验和基于人工的评估，证明InstructKG能够捕捉丰富且与教师教学意图一致的学习进展关系。

Conclusion: InstructKG为自动构建教师对齐的知识图谱提供了有效框架，能够准确捕捉课程中的学习依赖关系，有助于识别学生知识缺口并实现个性化学习干预。
...
### [26] [Epistemology of Generative AI: The Geometry of Knowing](https://arxiv.org/abs/2602.17116)
*Ilya Levin*

Main category: cs.AI

TL;DR: 论文提出高维空间的索引性认识论，将生成式AI视为在语义空间中导航，而非传统符号处理或统计重组。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的认知机制尚未被充分理解，其语义处理方式与传统计算范式（图灵-香农-冯·诺依曼传统）存在根本性断裂。缺乏对这种新范式的哲学理解，就无法在科学、教育和制度生活中负责任地整合生成式AI。

Method: 基于高维几何的四个结构特性（测度集中、近正交性、指数方向容量、流形正则性），借鉴皮尔士符号学和帕珀特建构主义，发展"高维空间的索引性认识论"，将生成模型重新概念化为学习流形的导航器。

Result: 提出导航性知识作为第三种知识生产模式，区别于符号推理和统计重组。生成式AI通过在语义参数的高维几何空间中定位和导航来产生知识，这种空间构成了生成生产的主动认知条件。

Conclusion: 理解生成式AI需要范式转变：从传统的信息处理模型转向高维语义空间的导航模型。导航性知识为负责任地整合生成式AI提供了原则基础，并重新定义了知识生产的本质。
...
### [27] [Efficient Parallel Algorithm for Decomposing Hard CircuitSAT Instances](https://arxiv.org/abs/2602.17130)
...
<details>
  <summary>Details</summary>
Motivation: CircuitSAT问题在逻辑等价性检查和密码学哈希函数原像攻击等应用中具有重要价值，但现有方法在处理困难实例时效率有限，需要更有效的分解技术

Method: 采用基于专用约束的并行算法，将原始SAT实例划分为弱化公式族，通过参数调整和并行计算的硬度估计来指导高质量分解的识别

Result: 算法在具有挑战性的CircuitSAT实例上展示了实际效果，包括布尔电路逻辑等价性检查和密码学哈希函数原像攻击的编码实例

Conclusion: 提出的参数化并行分解算法能够有效处理困难的CircuitSAT实例，为逻辑等价性检查和密码学攻击等应用提供了实用解决方案
...
### [28] [JEPA-DNA: Grounding Genomic Foundation Models through Joint-Embedding Predictive Architectures](https://arxiv.org/abs/2602.17162)
...
TL;DR: JEPA-DNA是一种新的基因组基础模型预训练框架，结合联合嵌入预测架构与生成目标，通过潜在空间预测提升对基因组功能上下文的理解。
...
Method: 引入JEPA-DNA框架，将联合嵌入预测架构与传统的生成目标相结合。通过潜在接地方法，将标记级恢复与潜在空间中的预测目标耦合，通过监督CLS标记来预测被掩码基因组片段的高级功能嵌入，而非仅关注单个核苷酸。

Result: 在多样化的基因组基准测试中，JEPA-DNA在监督和零样本任务上始终优于仅生成式基线，提供了更稳健且生物学基础更坚实的表征。

Conclusion: JEPA-DNA为基因组基础模型提供了一条可扩展的路径，使其不仅能理解基因组字母表，还能理解序列底层的功能逻辑，扩展了NTP和MLM范式，可作为独立训练目标或现有模型的持续预训练增强。
...
### [29] [Texo: Formula Recognition within 20M Parameters](https://arxiv.org/abs/2602.17189)
*Sicheng Mao*
...
Result: Texo在性能上达到与UniMERNet-T和PPFormulaNet-S等SOTA模型相当的水平，同时模型大小分别减少了80%和65%，实现了在消费级硬件上的实时推理和浏览器内部署。

Conclusion: Texo证明了通过精心设计和优化，可以在大幅减少参数量的情况下保持公式识别的高性能，为资源受限环境中的实时数学公式识别应用提供了实用解决方案。
...
### [30] [From Labor to Collaboration: A Methodological Experiment Using AI Agents to Augment Research Perspectives in Taiwan's Humanities and Social Sciences](https://arxiv.org/abs/2602.17221)
*Yi-Chih Huang*
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI正在重塑知识工作，但现有研究主要关注软件工程和自然科学领域，对人文社科领域的方法论探索有限。本研究旨在填补这一空白，为人文社科研究者提供一个可复制的AI协作框架。

Method: 本研究采用"方法论实验"的定位，提出基于AI Agent的协作研究工作流（Agentic Workflow）。该工作流基于三个原则：任务模块化、人机分工、可验证性，包含七个阶段，每个阶段明确人类研究者（研究判断和伦理决策）与AI Agent（信息检索和文本生成）的角色分工。使用台湾Claude.ai的7,729个对话数据（来自Anthropic Economic Index，2025年11月）作为实证案例来验证该方法的可行性。

Result: 研究提出了一个可复制的人文社科AI协作框架，并通过操作过程的反身性文档记录，识别了三种人机协作操作模式：直接执行、迭代精炼、人类主导。该分类揭示了人类判断在研究问题制定、理论解释、情境化推理和伦理反思方面的不可替代性。

Conclusion: 本研究为人文社科研究者提供了一个实用的AI协作方法论框架，强调了人类判断在研究过程中的核心地位。同时承认了单平台数据、横截面设计和AI可靠性风险等局限性，为未来研究提供了方向。
...
### [31] [Decoding the Human Factor: High Fidelity Behavioral Prediction for Strategic Foresight](https://arxiv.org/abs/2602.17222)
*Ben Yellin,Ehud Ezra,Mark Foreman,Shula Grinapol*

Main category: cs.AI

TL;DR: LBM（大型行为模型）通过将心理特征嵌入而非提示来预测个体战略决策，相比传统LLM在行为预测上表现更优且能利用更丰富的心理特征信息。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在预测高风险环境下的人类决策时存在局限性，特别是难以生成一致、个体特定的行为，且提示方法易出现身份漂移和无法充分利用详细人格描述的问题。

Method: 提出LBM（大型行为模型），通过微调将结构化、高维度的心理特征剖面（来自综合心理测量工具）作为条件输入，而非临时性人格提示，训练数据连接了稳定倾向、动机状态和情境约束与观察到的选择。

Result: 在保留场景评估中，LBM微调相比未适应的Llama-3.1-8B-Instruct骨干模型改进了行为预测，基于大五人格特征时性能与前沿基线相当；LBM能持续受益于更密集的特征剖面，而提示基线存在复杂性上限。
...
### [32] [Mechanistic Interpretability of Cognitive Complexity in LLMs via Linear Probing using Bloom's Taxonomy](https://arxiv.org/abs/2602.17229)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的黑箱性质需要超越表面性能指标的新型评估框架。本研究旨在探索模型内部如何表示认知复杂度，特别是不同认知层次（从基础回忆到抽象综合）是否在神经表征中具有可区分的结构。

Method: 使用布卢姆分类法作为层次化认知复杂度框架，分析不同LLM的高维激活向量，通过线性分类器探究不同认知水平在模型残差流中的线性可分性。

Result: 线性分类器在所有布卢姆认知水平上达到约95%的平均准确率，表明认知水平在模型表征的线性可访问子空间中被编码。模型在正向传播早期就解析了提示的认知难度，且表征在不同层间变得越来越可分。

Conclusion: 研究提供了证据表明大语言模型内部存在认知复杂度的结构化表征，这些表征在线性子空间中是可访问的，为理解LLM的内部工作机制和开发更精细的评估方法提供了新视角。
...
### [33] [All Leaks Count, Some Count More: Interpretable Temporal Contamination Detection in LLM Backtesting](https://arxiv.org/abs/2602.17234)
*Zeyu Zhang,Ryan Chen,Bradly C. Stadie*

Main category: cs.AI

TL;DR: 提出Shapley-DCLR指标量化LLM预测中的时间知识泄漏，并开发TimeSPEC方法通过声明验证和再生来减少泄漏，提高回溯测试的可靠性。


<details>
  <summary>Details</summary>
Motivation: 评估LLM预测未来事件能力需要进行回溯测试，但LLM可能在训练中编码了截止日期后的知识，导致时间知识泄漏，影响评估有效性。需要检测和量化这种泄漏。

Method: 1. 提出声明级框架检测时间知识泄漏：将模型推理分解为原子声明，按时间可验证性分类，使用Shapley值测量每个声明对预测的贡献，得到Shapley-DCLR指标；2. 开发TimeSPEC方法：通过声明验证和再生的交错过程主动过滤时间污染，确保所有支持声明都能追溯到截止日期前的可用来源。

Result: 在350个实例（美国最高法院案件预测、NBA薪资估计、股票回报排名）上的实验显示：标准提示基线存在显著泄漏；TimeSPEC在保持任务性能的同时降低了Shapley-DCLR；显式的、可解释的声明级验证优于基于提示的时间约束。

Conclusion: 提出的Shapley-DCLR指标能有效量化LLM预测中的时间知识泄漏，TimeSPEC方法通过主动的声明验证机制减少泄漏，为可靠的LLM回溯测试提供了框架，显式声明级验证比简单提示约束更有效。
...
### [34] [ArXiv-to-Model: A Practical Study of Scientific LM Training](https://arxiv.org/abs/2602.17288)
*Anuj Gupta*

Main category: cs.AI

TL;DR: 本文详细记录了在有限计算资源（2xA100 GPU）下，从arXiv LaTeX原始源训练1.36B参数科学语言模型的端到端工程实践，分析了预处理、分词和基础设施瓶颈对训练的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管前沿大语言模型展现出强大的推理和数学能力，但从原始源训练领域专业化科学语言模型的实际过程仍缺乏详细文档。本研究旨在为在中等计算预算下构建领域专用模型的研究者提供工程实践指导。

Method: 开发了一个端到端训练流程，包括：元数据过滤、存档验证、LaTeX提取、文本规范化、领域感知分词，以及在2xA100 GPU约束下的密集Transformer训练。通过24次实验运行分析训练稳定性、扩展行为、数据损失和基础设施瓶颈。

Result: 研究发现：预处理决策显著影响可用token数量；分词策略影响符号稳定性；存储和I/O约束可能与计算资源同等重要地成为限制因素。在数据丰富（520亿预训练token）的情况下，模型展现出稳定的训练行为。

Conclusion: 本研究没有提出新架构，而是提供了一个工程基础的、透明的从零开始训练小型科学语言模型的实践记录。这些见解旨在支持在中等计算预算下寻求构建领域专用模型的研究者。
...
TL;DR: 提出了一种无数据的任务向量正则化方法，通过曲率矩阵近似解决表示漂移问题，在任务加法和否定中取得SOTA效果
...
Method: 将表示漂移正则化框架化为曲率矩阵近似问题，利用Kronecker分解近似曲率技术，获得一个实用的正则化器。该方法在任务数量上具有常数复杂度，促进对任务向量缩放的鲁棒性

Result: 在任务加法和否定任务中取得了最先进的结果，无需保留调优，具有常数复杂度

Conclusion: 提出了一种无数据的任务向量正则化方法，通过曲率矩阵近似有效解决表示漂移问题，在保持模块化和数据隐私约束的同时提升任务算术性能
...
Result: 框架不仅返回匹配结果，还能识别和标记哪些具体约束被满足或未满足，提供更透明和可问责的检索过程，同时提升了主流嵌入方法的性能

Conclusion: 通过将检索结果建立在形式化推理系统上，超越了向量表示的模糊性和近似性，为开放词汇自然语言查询提供了可信且可验证的解决方案
...
TL;DR: 提出MCVAE模型，通过变分编码器处理多模态数据，引入融合瓶颈和门控机制，结合生存损失、重建损失和跨模态对比损失，提高NSCLC生存预测在数据严重缺失情况下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: NSCLC患者生存预测因个体预后特征差异而具有挑战性，多模态数据（全切片图像、转录组学、DNA甲基化）可提供互补信息，但临床数据常存在模态缺失问题，现有方法在严重缺失情况下缺乏鲁棒性。

Method: 提出多模态对比变分自编码器（MCVAE）：使用模态特定变分编码器捕捉各数据源不确定性；引入带学习门控机制的融合瓶颈归一化现有模态贡献；采用多任务目标结合生存损失和重建损失正则化患者表示；添加跨模态对比损失强制潜在空间对齐；训练时应用随机模态掩码提高对任意缺失模式的鲁棒性。

Result: 在TCGA-LUAD（n=475）和TCGA-LUSC（n=446）数据集上的广泛评估表明，该方法在预测疾病特异性生存（DSS）方面有效，且在严重缺失情况下比两种最先进模型更具鲁棒性。测试所有模态子集发现，多模态集成并不总是有益于任务。

Conclusion: MCVAE通过变分编码器处理模态不确定性、融合瓶颈门控机制和多任务学习，有效解决了多模态数据严重缺失下的NSCLC生存预测问题，同时揭示了多模态集成并非总是有益的见解。
...
<details>
  <summary>Details</summary>
Motivation: 儿童越来越多地使用AI技术，但面临隐私风险。现有隐私法规要求企业实施保护措施，但在实践中实施困难。需要为AI服务提供商和开发者提供实用框架，在满足法律标准的同时减少儿童隐私风险。

Method: 提出基于隐私设计（PbD）的框架，整合GDPR、PIPEDA、COPPA等隐私法规原则，映射到LLM应用的数据收集、模型训练、运营监控和持续验证等阶段。结合UNCRC、AADC和学术研究制定儿童设计指南，并通过13岁以下儿童LLM教育导师的案例研究展示框架应用。

Result: 框架提供了操作控制措施和设计指南，帮助开发者在LLM生命周期中实施技术和组织控制，做出适龄设计决策。案例研究表明，通过数据保护策略和适龄设计，可以开发出既提供隐私保护又符合法律要求的儿童AI应用。

Conclusion: 基于隐私设计的框架能够支持开发者为儿童设计隐私保护的AI应用，通过整合多法规原则、操作控制和适龄设计指南，在LLM生命周期中实现隐私保护和法律合规。
...
<details>
  <summary>Details</summary>
Motivation: 当前推荐系统创新受到生态系统分裂的阻碍：研究人员必须在易于内存实验与需要昂贵复杂重写的分布式工业引擎之间做出选择。这种分裂阻碍了学术研究向工业应用的转化。

Method: 提出WarpRec框架，采用新颖的后端无关架构，包含50多种最先进算法、40个评估指标和19种过滤与分割策略。框架集成CodeCarbon进行实时能耗追踪，支持从本地执行到分布式训练和优化的无缝过渡。

Result: WarpRec成功消除了内存实验与分布式部署之间的权衡，实现了从学术研究到工业应用的无缝过渡。框架展示了可扩展性不必以科学完整性或可持续性为代价，并为推荐系统向智能体AI生态系统演进提供了架构基础。

Conclusion: WarpRec不仅弥合了学术界与工业界之间的鸿沟，还可作为下一代可持续、智能体就绪推荐系统的架构支柱，推动推荐系统从静态排名引擎向生成式AI生态系统中的交互工具演进。
...
TL;DR: 开发ARM Cortex处理器（M0+、M4、M7）上AI模型的实用基准测试框架，通过自动化测试平台系统评估能效、精度和资源利用率，发现FLOPs与推理时间呈近线性关系，并利用帕累托分析平衡能耗与精度权衡。


<details>
  <summary>Details</summary>
Motivation: 嵌入式系统中AI模型部署面临能效、精度和资源利用率的平衡挑战，需要系统化的评估框架来指导开发者选择最优的处理器与模型组合，实现高性能且可持续的AI应用。

Method: 设计自动化测试平台，系统评估ARM Cortex处理器（M0+、M4、M7）上AI模型的关键性能指标（KPIs），分析浮点运算（FLOPs）与推理时间的相关性，并采用帕累托分析平衡能耗与模型精度的权衡。

Result: 发现FLOPs与推理时间存在近线性相关，可作为计算需求估计的可靠指标；M7处理器适合短推理周期任务，M4处理器在长推理任务中能效更优，M0+处理器适用于简单AI任务；通过帕累托分析可有效平衡能耗与精度。

Conclusion: 该基准测试框架为嵌入式AI系统开发提供了实用指导，帮助开发者根据具体应用需求选择最优处理器与模型组合，实现能效与性能的最佳平衡，推动可持续AI在嵌入式领域的应用。
...
Method: 提出KG-RAG框架，将知识图谱（提供电信标准和文档的结构化领域知识）与检索增强生成（动态检索相关事实）相结合，增强LLM在电信任务中的表现
...
TL;DR: 论文提出了两个新的评估指标——可重用性和可验证性，用于衡量多智能体IR管道中思维链的质量，发现这些指标与标准准确率不相关，揭示了当前基于准确率的排行榜在评估推理能力方面的盲点。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体IR管道中LLM智能体通过思维链交换中间推理，但现有评估仅关注目标任务准确率，无法评估推理过程本身的质量或效用。需要新的评估指标来弥补这一局限性。
...
Result: 结果显示可重用性和可验证性与标准准确率不相关，暴露了当前基于准确率的排行榜在评估推理能力方面的盲点。令人惊讶的是，专门推理模型生成的思维链并不比通用LLM（如Llama和Gemma）生成的思维链更具可重用性或可验证性。

Conclusion: 需要超越准确率的新评估指标来评估思维链的质量，可重用性和可验证性提供了有价值的补充视角。专门推理模型在生成高质量思维链方面不一定优于通用模型，这对模型选择和评估实践有重要启示。
...
TL;DR: KLong是一个开源的LLM智能体，通过轨迹分割SFT和渐进式RL训练来解决超长视野任务，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决当前LLM智能体在处理超长视野任务时的能力不足问题，需要开发能够处理复杂、多步骤任务的智能体系统。

Method: 1. 使用轨迹分割SFT冷启动：通过保留早期上下文、渐进截断后期上下文、保持子轨迹重叠的方式处理超长轨迹；2. 构建Research-Factory自动化管道生成高质量训练数据；3. 提出渐进式RL训练：将训练分为多个阶段，逐步延长超时时间。

Result: KLong（106B）在PaperBench上超越Kimi K2 Thinking（1T）11.28%，性能提升泛化到SWE-bench Verified和MLE-bench等其他编码基准测试。

Conclusion: 提出的轨迹分割SFT和渐进式RL训练方法有效提升了LLM智能体解决超长视野任务的能力，KLong在多个基准测试中表现出优越性和良好的泛化能力。
...
TL;DR: 本文提出了一种基于常微分方程的统一理论框架ODESteer，用于改进大语言模型中的激活引导技术，通过屏障函数和多步自适应引导在多个对齐基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前激活引导方法存在两个关键限制：缺乏统一的理论框架来指导引导方向设计，以及过度依赖单步引导而无法捕捉激活分布的复杂模式。需要建立更理论化的方法来改进LLM对齐效果。

Method: 提出基于常微分方程的统一理论框架，将传统激活加法解释为ODE的一阶近似。基于此视角，将引导方向识别转化为控制理论中的屏障函数设计。具体提出ODESteer方法，将屏障函数定义为正负激活的对数密度比，并构建用于多步自适应引导的ODE。

Result: 在多个LLM对齐基准测试中，ODESteer相比最先进的激活引导方法取得一致改进：TruthfulQA提升5.7%，UltraFeedback提升2.5%，RealToxicityPrompts提升2.4%。

Conclusion: 本研究通过ODE统一了激活引导的理论基础，提出了ODESteer方法并在实证中验证了其有效性，为LLM对齐中的激活引导建立了新的原则性视角。
...
TL;DR: 提出一种基于联邦学习的混合AI模型，结合SWIN Transformer和CNN，用于从X光片诊断COVID-19和肺炎，确保医疗数据安全处理


<details>
  <summary>Details</summary>
Motivation: 利用人工智能在医疗保健领域的快速发展，解决医疗数据共享的安全性和隐私问题，同时提高疾病诊断的准确性和可靠性

Method: 采用联邦学习框架，结合最新的CNN模型（DenseNet201、Inception V3、VGG 19）和SWIN Transformer构建混合模型，使用TensorFlow、Keras和微软开发的Vision Transformer技术

Result: 未在摘要中明确说明具体实验结果，但提出了一个能够提高疾病诊断准确性和严重程度预测的实时持续学习系统

Conclusion: 联邦学习支持的混合AI模型能够为医疗专业人员提供可靠辅助，确保数据安全性和信息真实性，共同应对全球疫情挑战
...
TL;DR: 提出AI GameStore平台，通过LLM生成人类游戏来评估AI系统在"人类游戏宇宙"中的表现，作为衡量类人通用智能的新方法


<details>
  <summary>Details</summary>
Motivation: 传统AI基准测试评估范围狭窄且容易饱和，需要一种更全面的方法来评估AI系统的类人通用智能

Method: 定义"人类游戏"概念，开发AI GameStore平台，使用LLM和人类参与循环从App Store和Steam等平台自动生成标准化游戏环境，生成100个代表性游戏

Result: 评估7个前沿视觉语言模型，最佳模型在大多数游戏中得分不到人类平均分的10%，特别是在需要世界模型学习、记忆和规划能力的游戏中表现较差

Conclusion: AI GameStore为衡量和推动机器实现类人通用智能提供了一种实用方法，展示了当前AI在复杂游戏环境中的局限性
...
<details>
  <summary>Details</summary>
Motivation: 现有图扩散模型在分子生成中存在化学有效性低、难以满足期望性质的问题，相比1D建模方法性能不足。需要开发能够克服这些长期性能限制的分子图生成框架。

Method: 基于分层离散扩散模型，将离散扩散推广到编码化学先验的额外类别，并采用解耦原子编码，根据原子的化学角色分割原子类型。

Result: 在MOSES数据集上实现了新的最先进性能，首次在图扩散中达到接近完美的化学有效性，在多个指标上超越了强大的1D基线方法。在下游任务（包括多属性引导生成和支架扩展）中也表现出强大性能。

Conclusion: MolHIT成功克服了现有图扩散模型的性能限制，为AI驱动的药物发现和材料科学提供了强大的分子生成框架，实现了图扩散模型在化学有效性和性质满足方面的突破。
...
TL;DR: HIPE-2026是CLEF评估实验室，专注于从多语言历史文本中提取人物-地点关系，扩展了HIPE系列到语义关系抽取，评估系统在嘈杂历史文本中识别两种时空关系的能力。


<details>
  <summary>Details</summary>
Motivation: 历史文本包含丰富的人物-地点关联信息，但传统方法难以从嘈杂、多语言的历史资料中有效提取这些语义关系。HIPE系列需要扩展到关系抽取层面，以支持数字人文领域的大规模历史数据处理。

Method: 提出三方面评估框架：1) 准确性评估系统识别两种关系类型($at$和$isAt$)的能力；2) 计算效率评估处理大规模历史数据的性能；3) 领域泛化评估跨语言和跨时间段的适应性。构建多语言、多时期的历史文本数据集。

Result: 建立了针对历史文本人物-地点关系抽取的标准化评估框架，定义了两种具体的关系类型($at$表示"是否曾到过此地"，$isAt$表示"发表时是否位于此地")，并提出了结合准确性、效率和泛化的综合评价体系。

Conclusion: HIPE-2026将历史信息处理扩展到语义关系抽取层面，为知识图谱构建、历史传记重建和空间分析等数字人文应用提供了重要基础，推动了历史文本处理的标准化和系统化发展。
...
### [49] [Three-dimensional Damage Visualization of Civil Structures via Gaussian Splatting-enabled Digital Twins](https://arxiv.org/abs/2602.16713)
*Shuo Wang,Shuo Wang,Xin Nie,Yasutaka Narazaki,Thomas Matiki,Billie F. Spencer*

Main category: cs.CV

TL;DR: 该研究提出了一种基于高斯泼溅的数字孪生方法，用于土木基础设施的三维损伤可视化，通过多尺度重建策略平衡效率与细节，并支持随时间演变的损伤更新。


<details>
  <summary>Details</summary>
Motivation: 传统2D图像损伤识别无法满足数字孪生对精确三维损伤可视化的需求。相比传统摄影测量三维重建技术，神经辐射场和高斯泼溅在场景表示、渲染质量和处理特征缺失区域方面表现更优，其中高斯泼溅因其离散各向异性3D高斯表示而具有更高效率。

Method: 1) 基于高斯泼溅的三维重建技术，将2D损伤分割结果可视化并减少分割误差；2) 开发多尺度重建策略以平衡效率与损伤细节；3) 支持数字孪生随损伤演变而更新。方法在开源合成地震后检测数据集上进行验证。

Result: 该方法在合成地震后检测数据集上展示了全面的三维损伤可视化能力，为土木基础设施数字孪生提供了一个有前景的解决方案。

Conclusion: 基于高斯泼溅的数字孪生方法能够有效实现三维损伤可视化，通过多尺度策略平衡效率与细节，并支持损伤演变的动态更新，为土木基础设施检测提供了新的技术途径。
...
### [50] [Analytic Score Optimization for Multi Dimension Video Quality Assessment](https://arxiv.org/abs/2602.16856)
*Boda Lin,Yongjie Zhu,Wenyu Qin,Meng Wang,Pengfei Wan*

Main category: cs.CV

TL;DR: UltraVQA数据集包含五个质量维度的多维度视频质量评估，结合GPT生成解释和Analytic Score Optimization方法，在质量预测上优于基线模型


<details>
  <summary>Details</summary>
Motivation: 视频质量评估需要从单一分数向更丰富的多维度评估演进，当前缺乏大规模、多维度、带解释性标注的视频质量数据集

Method: 1) 构建UltraVQA大规模多维度VQA数据集，包含运动质量、运动幅度、美学质量、内容质量和清晰度五个维度，每个视频由3+人工标注者评分并附带GPT生成的解释；2) 提出Analytic Score Optimization (ASO)方法，将质量评估重构为带正则化的决策过程，获得闭式解以捕捉人类评分的序数特性

Result: 提出的方法在实验中优于大多数基线模型（包括闭源API和开源模型），同时降低了质量预测的平均绝对误差(MAE)

Conclusion: 多维度、可解释的标注和基于强化对齐的方法对推进视频质量评估具有重要意义，UltraVQA数据集和ASO方法为更全面、准确的视频质量评估提供了新方向
...
### [51] [DODO: Discrete OCR Diffusion Models](https://arxiv.org/abs/2602.16872)
...
Method: 提出DODO模型，采用块离散扩散方法。通过将生成过程分解为块，缓解全局扩散中的同步错误，从而利用扩散模型的并行解码优势，同时保持OCR所需的精确性。

Result: DODO在保持接近最先进准确率的同时，实现了比自回归基线快3倍的推理速度，成功解锁了扩散模型在OCR任务中的加速潜力。

Conclusion: 块离散扩散是解决OCR任务中自回归模型效率瓶颈的有效方法，DODO展示了扩散模型在确定性视觉语言任务中的实用价值，为高效OCR系统提供了新思路。
...
### [52] [StereoAdapter-2: Globally Structure-Consistent Underwater Stereo Depth Estimation](https://arxiv.org/abs/2602.16915)
*Zeyu Ren,Xiang Li,Yiran Wang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: StereoAdapter-2提出基于选择性状态空间模型的ConvSS2D算子替代ConvGRU，实现单步长距离空间传播，并构建大规模合成水下立体数据集UW-StereoDepth-80K，在水下立体深度估计任务中取得SOTA性能。
...
Method: 1. 提出ConvSS2D算子，基于选择性状态空间模型，采用四向扫描策略对齐极线几何并捕获垂直结构一致性；2. 构建UW-StereoDepth-80K大规模合成水下立体数据集，通过语义感知风格迁移和几何一致新视图合成的两阶段生成流程；3. 继承StereoAdapter的动态LoRA适应机制。

Result: 在TartanAir-UW上提升17%，在SQUID上提升7.2%，达到SOTA零样本性能。在BlueROV2平台上的真实世界验证证明了方法的鲁棒性。

Conclusion: StereoAdapter-2通过ConvSS2D算子和大规模合成数据集，有效解决了水下立体深度估计中的长距离视差传播问题，实现了高效的单步更新和线性计算复杂度，显著提升了水下深度估计性能。
...
### [53] [HS-3D-NeRF: 3D Surface and Hyperspectral Reconstruction From Stationary Hyperspectral Images Using Multi-Channel NeRFs](https://arxiv.org/abs/2602.16950)
*Kibon Ku,Talukder Z. Jubery,Adarsh Krishnamurthy,Baskar Ganapathysubramanian*

Main category: cs.CV

TL;DR: 提出HSI-SC-NeRF框架，利用固定相机和多通道NeRF实现农业产品的高通量高光谱3D重建，适用于自动化表型分析和采后检测


<details>
  <summary>Details</summary>
Motivation: 高光谱成像（HSI）和3D重建技术对农业产品质量和植物表型分析至关重要，但现有方法硬件复杂且与自动化系统不兼容。传统NeRF需要移动相机设置，限制了室内农业环境中的通量和可重复性。

Method: 开发HSI-SC-NeRF框架：使用固定相机采集多视角高光谱数据，物体在特氟龙成像室中旋转；通过ArUco标记估计姿态并转换到相机坐标系；采用多通道NeRF联合优化所有高光谱波段，使用复合光谱损失函数；实施两阶段训练协议，分离几何初始化和辐射度细化。

Result: 在三种农业产品样本上的实验表明，HSI-SC-NeRF在可见光和近红外光谱范围内实现了高空间重建精度和强光谱保真度，验证了其适用于自动化农业工作流程。

Conclusion: HSI-SC-NeRF成功解决了固定相机设置下的高通量高光谱3D重建问题，为自动化农业表型分析和采后检测提供了可行的技术方案。
...
### [54] [DDiT: Dynamic Patch Scheduling for Efficient Diffusion Transformers](https://arxiv.org/abs/2602.16968)
*Dahye Kim,Deepti Ghadiyaram,Raghudeep Gadde*

Main category: cs.CV

TL;DR: 提出动态标记化策略，根据内容复杂度和去噪时间步动态调整DiTs的补丁大小，在保持生成质量的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 当前Diffusion Transformers在图像和视频生成中取得了SOTA性能，但计算成本高昂。主要问题在于固定的标记化过程在整个去噪阶段使用恒定大小的补丁，不考虑内容复杂度

Method: 提出动态标记化策略，基于内容复杂度和去噪时间步动态调整补丁大小：早期时间步使用较粗的补丁建模全局结构，后期使用较细的补丁细化局部细节。在推理过程中动态重新分配不同去噪步骤的补丁大小

Result: 在FLUX-1.Dev上实现3.52倍加速，在Wan 2.1上实现3.2倍加速，同时保持生成质量和提示遵循性

Conclusion: 动态标记化是一种高效的测试时策略，能够显著降低Diffusion Transformers的计算成本而不影响生成质量，为高效图像和视频生成提供了新思路
...
### [55] [Characterizing the Predictive Impact of Modalities with Supervised Latent-Variable Modeling](https://arxiv.org/abs/2602.16979)
*Divyam Madaan,Sumit Chopra,Kyunghyun Cho*

Main category: cs.CV

TL;DR: PRIMO是一种监督式潜在变量插补模型，用于处理多模态学习中模态缺失问题，通过量化缺失模态的预测影响，利用所有可用训练样本（无论模态是否完整）进行学习。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs通常假设训练和推理时所有模态都可用，但实际应用中多模态数据往往不完整（模态缺失、异步收集或仅部分样本可用），需要处理模态缺失问题并量化缺失模态的预测影响。

Method: PRIMO通过潜在变量建模缺失模态，捕捉其与观测模态在预测上下文中的关系。训练时利用所有可用样本（完整和部分模态），推理时从学习到的缺失模态分布中采样，获得边际预测分布并分析缺失模态对每个实例预测的影响。
...
Conclusion: PRIMO有效处理多模态学习中的模态缺失问题，能够利用不完整训练数据，量化缺失模态的预测不确定性，并通过潜在变量插补生成合理的预测分布。
...
### [56] [Patch-Based Spatial Authorship Attribution in Human-Robot Collaborative Paintings](https://arxiv.org/abs/2602.17030)
*Eric Chen,Patricia Alves-Oliveira*

Main category: cs.CV

TL;DR: 提出基于图像块的框架用于人机协作绘画中的空间作者归属，通过法医案例研究实现88.8%的块级准确率，并使用条件香农熵量化风格重叠以处理协作作品的模糊性。
...
Method: 提出基于图像块的框架，使用普通平板扫描仪采集数据，采用留一绘画交叉验证。对于协作作品，使用条件香农熵量化风格重叠，分析混合作者区域的不确定性。

Result: 在15幅抽象绘画上达到88.8%的块级准确率（通过多数投票达到86.7%的绘画级准确率），优于纹理基线和预训练特征方法（68.0%-84.7%）。混合区域的不确定性比纯绘画高64%（p=0.003）。

Conclusion: 该模型针对特定人机对，但为数据稀缺的人机创意工作流中的样本高效归属提供了方法论基础，未来有潜力扩展到任何人机协作绘画的作者归属。
...
### [57] [PartRAG: Retrieval-Augmented Part-Level 3D Generation and Editing](https://arxiv.org/abs/2602.17033)
*Peize Li,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: PartRAG：基于检索增强的单图像3D生成框架，通过外部部件数据库和扩散变换器实现部件级结构生成和可编辑表示，解决了部件几何多样性和多视图一致性问题。
...
Method: 提出PartRAG框架，集成外部部件数据库与扩散变换器。包含：1）分层对比检索模块，在部件和对象粒度上对齐密集图像块与3D部件潜在表示，从1236个部件标注资产库中检索；2）掩码部件级编辑器，在共享规范空间中操作，支持部件交换、属性细化和组合更新。
...
Conclusion: PartRAG通过检索增强方法有效解决了部件级3D生成的挑战，实现了高质量生成和精确局部编辑，为交互式3D创作提供了新途径。
...
### [58] [Amber-Image: Efficient Compression of Large-Scale Diffusion Transformers](https://arxiv.org/abs/2602.17047)
*Chaojie Yang,Tian Li,Yue Zhang,Jun Gao*

Main category: cs.CV

TL;DR: 提出了一种高效的压缩框架，将60层双流MMDiT架构的Qwen-Image模型压缩为轻量级Amber-Image系列，参数减少70%，无需大规模数据工程，训练成本极低。


<details>
  <summary>Details</summary>
Motivation: 解决Diffusion Transformer (DiT)架构在文本到图像生成中计算成本过高和部署困难的问题，需要开发高效的模型压缩方法。

Method: 1. 提出高效压缩框架，无需从头训练；2. 使用时间步敏感的深度剪枝策略获得Amber-Image-10B，通过局部权重平均重新初始化保留层，采用层间蒸馏和全参数微调优化；3. 开发Amber-Image-6B，引入混合流架构，将深层双流转换为单流（从图像分支初始化），通过渐进蒸馏和轻量级微调进一步优化。

Result: 参数减少70%，整个压缩训练流程（从10B到6B）仅需不到2000 GPU小时，在DPG-Bench和LongText-Bench等基准测试中实现高保真合成和优异文本渲染，性能匹配更大模型。

Conclusion: 提出的压缩框架能高效地将大型DiT模型转换为轻量级版本，显著降低计算成本和部署门槛，同时保持高质量生成能力，为实际应用提供了可行的解决方案。
...
### [59] [StructCore: Structure-Aware Image-Level Scoring for Training-Free Unsupervised Anomaly Detection](https://arxiv.org/abs/2602.17048)
...
<details>
  <summary>Details</summary>
Motivation: 最大池化作为基于记忆库的无监督异常检测中异常得分图到图像级决策的标准方法，存在明显缺陷：它仅依赖单个极端响应，丢弃了异常证据在图像中的分布和结构信息，导致正常和异常得分经常重叠。

Method: StructCore是一种无需训练的结构感知图像级评分方法。给定异常得分图，它计算低维结构描述符φ(S)来捕捉分布和空间特征，然后通过从训练正常样本估计的对角马氏距离校准来优化图像级评分，而不改变像素级定位。

Result: 在MVTec AD数据集上达到99.6%的图像级AUROC，在VisA数据集上达到98.4%的图像级AUROC，展示了通过利用最大池化忽略的结构特征实现鲁棒的图像级异常检测。

Conclusion: StructCore通过捕捉异常得分图的结构特征，超越了传统的最大池化方法，显著提升了图像级异常检测性能，同时保持了无需训练和保持像素级定位不变的优势。
...
### [60] [Cholec80-port: A Geometrically Consistent Trocar Port Segmentation Dataset for Robust Surgical Scene Understanding](https://arxiv.org/abs/2602.17060)
*Shunsuke Kikuchi,Atsushi Kouno,Hiroki Matsuzaki*

Main category: cs.CV

TL;DR: 提出Cholec80-port数据集，提供高质量、几何一致的套管端口分割标注，并统一现有数据集标注标准，显著提升跨数据集鲁棒性


<details>
  <summary>Details</summary>
Motivation: 套管端口在腹腔镜手术中会持续遮挡视野并吸引过多特征点，对图像拼接、3D重建和视觉SLAM等几何处理流程造成严重影响。现有公开数据集缺乏准确的端口标注，且现有标注常违反几何一致性（如遮盖中央开口），影响算法性能
...
Conclusion: 几何一致的套管端口标注对于提升手术视觉算法的鲁棒性至关重要，Cholec80-port数据集及相关SOP为手术计算机视觉研究提供了重要基础
...
### [61] [Cross Pseudo Labeling For Weakly Supervised Video Anomaly Detection](https://arxiv.org/abs/2602.17077)
*Lee Dayeon,Kim Dongheyong,Park Chaewon,Woo Sungmin,Lee Sangyoun*

Main category: cs.CV

TL;DR: CPL-VAD：基于交叉伪标签的双分支弱监督视频异常检测框架，通过异常检测分支和类别分类分支的互补优势，实现异常定位和类别识别


<details>
  <summary>Details</summary>
Motivation: 弱监督视频异常检测需要在仅有视频级标签的情况下检测异常并识别异常类别。现有方法通常在异常定位和类别识别之间存在性能差距，需要一种能够同时优化这两个任务的方法。
...
Conclusion: CPL-VAD通过双分支结构和交叉伪标签机制，有效解决了弱监督视频异常检测中的异常定位和类别识别问题，证明了两个任务之间的互补性能够提升整体性能。
...
### [62] [ComptonUNet: A Deep Learning Model for GRB Localization with Compton Cameras under Noisy and Low-Statistic Conditions](https://arxiv.org/abs/2602.17085)
...
<details>
  <summary>Details</summary>
Motivation: 微弱伽马射线暴（GRBs）是研究早期恒星形成的重要探针，但由于低光子统计和强背景噪声，检测和定位这些弱源仍具挑战性。现有机器学习模型难以在统计鲁棒性和噪声抑制之间取得平衡。

Method: 提出ComptonUNet混合深度学习框架，结合直接重建模型的统计效率和基于图像架构的去噪能力，联合处理原始数据并重建图像。通过模拟低地球轨道任务中GRB样事件嵌入背景环境来评估性能。

Result: ComptonUNet显著优于现有方法，在广泛的低统计和高背景场景中实现了改进的定位精度。

Conclusion: ComptonUNet为在具有挑战性的观测条件下实现微弱伽马射线暴的鲁棒定位提供了有效的解决方案，平衡了统计效率和噪声抑制的需求。
...
### [63] [3D Scene Rendering with Multimodal Gaussian Splatting](https://arxiv.org/abs/2602.17124)
*Chi-Shiang Gau,Konstantinos D. Polyzos,Athanasios Bacharis,Saketh Madhuvarasu,Tara Javidi*

Main category: cs.CV

TL;DR: 提出了一种将射频传感与3D高斯泼溅相结合的多模态框架，用于在视觉线索不可靠条件下实现高效、鲁棒的3D场景重建与渲染。


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的3D高斯泼溅方法需要大量相机视角来初始化高斯基元，在恶劣天气、低光照或部分遮挡等视觉线索不可靠条件下表现不佳。射频信号对这些条件具有鲁棒性，因此需要整合射频传感来提升系统的鲁棒性和效率。

Method: 提出多模态框架，集成射频传感（如汽车雷达）与高斯泼溅渲染。利用稀疏的RF深度测量进行高效深度预测，生成高质量3D点云用于初始化各种GS架构中的高斯函数。

Result: 数值测试表明，将射频传感合理整合到GS流程中，能够实现由RF信息驱动的结构精确的高保真3D场景渲染。

Conclusion: 射频传感与高斯泼溅的集成提供了一种比纯视觉GS渲染更高效、更鲁棒的替代方案，特别是在视觉线索不可靠的环境条件下。
...
### [64] [B$^3$-Seg: Camera-Free, Training-Free 3DGS Segmentation via Analytic EIG and Beta-Bernoulli Bayesian Updates](https://arxiv.org/abs/2602.17134)
*Hiromichi Kamata,Samuel Arthur Munro,Fuminori Homma*

Main category: cs.CV

TL;DR: B³-Seg：一种基于Beta-Bernoulli贝叶斯更新的快速、免训练、免相机视角的3D高斯泼溅开放词汇分割方法，通过分析性期望信息增益实现最优视角采样


<details>
  <summary>Details</summary>
Motivation: 现有3DGS分割方法依赖预定义相机视角、真实标签或昂贵重训练，无法满足影视游戏制作中实时编辑的低延迟需求，需要一种快速、免训练、免相机约束的实用分割方案

Method: 将分割问题重新表述为顺序Beta-Bernoulli贝叶斯更新，通过分析性期望信息增益主动选择下一个最优视角，贝叶斯公式保证了EIG的自适应单调性和子模性，实现贪婪(1-1/e)近似最优视角采样策略

Result: 在多个数据集上实验表明，B³-Seg在几秒内完成端到端分割，达到与高成本监督方法竞争的结果，同时保持信息效率的理论保证

Conclusion: B³-Seg实现了具有可证明信息效率的实用交互式3DGS分割，为影视游戏生产中的实时资产编辑提供了理论支撑的低延迟解决方案
...
### [65] [BadCLIP++: Stealthy and Persistent Backdoors in Multimodal Contrastive Learning](https://arxiv.org/abs/2602.17168)
*Siyuan Liang,Yongcheng Jing,Yingjie Wang,Jiaxing Huang,Ee-chien Chang,Dacheng Tao*

Main category: cs.CV

TL;DR: BadCLIP++是一个针对多模态对比学习模型的后门攻击框架，通过语义融合QR微触发器和目标对齐子集选择实现高隐蔽性，通过半径收缩、质心对齐、曲率控制和弹性权重巩固实现强持久性，在仅0.3%毒化率下达到99.99%攻击成功率。
...
Method: 1) 隐蔽性方面：提出语义融合QR微触发器，在任务相关区域附近嵌入不可感知的模式，保持干净数据统计特性同时产生紧凑的触发分布；采用目标对齐子集选择在低注入率下增强信号。2) 持久性方面：通过半径收缩和质心对齐稳定触发嵌入；通过曲率控制和弹性权重巩固稳定模型参数，将解保持在低曲率宽盆地中抵抗微调。3) 提供首个理论分析，证明在信任区域内，干净微梯度和后门目标梯度同向，攻击成功率下降有非递增上界。

Result: 在仅0.3%毒化率下，BadCLIP++在数字设置中达到99.99%攻击成功率，比基线高11.4个百分点。在19种防御方法下，攻击成功率保持在99.90%以上，干净准确率下降小于0.8%。物理攻击成功率65.03%，对水印移除防御具有鲁棒性。

Conclusion: BadCLIP++统一解决了多模态对比学习后门攻击的隐蔽性和持久性挑战，通过创新的触发设计和稳定性机制，在极低毒化率下实现了高攻击成功率和强防御抵抗能力，为后门攻击研究提供了新的理论分析和实用框架。
...
### [66] [NRGS-SLAM: Monocular Non-Rigid SLAM for Endoscopy via Deformation-Aware 3D Gaussian Splatting](https://arxiv.org/abs/2602.17182)
*Jiwei Shan,Zeyu Cai,Yirui Li,Yongbo Chen,Lijun Han,Yun-hui Liu,Hesheng Wang,Shing Shin Cheng*

Main category: cs.CV

TL;DR: NRGS-SLAM：基于3D高斯泼溅的内窥镜单目非刚性SLAM系统，通过可变形感知的3D高斯地图和贝叶斯自监督策略解决相机运动与组织变形耦合问题，实现更精确的姿态估计和高质量重建。


<details>
  <summary>Details</summary>
Motivation: 内窥镜场景因软组织持续变形而违反刚性假设，导致相机自运动与内在变形之间存在强耦合模糊性。现有单目非刚性SLAM方法缺乏有效解耦机制，依赖稀疏或低保真场景表示，导致跟踪漂移和重建质量受限。

Method: 提出NRGS-SLAM系统：1）引入可变形感知的3D高斯地图，为每个高斯基元添加可学习的变形概率，通过贝叶斯自监督策略优化；2）设计可变形跟踪模块，优先处理低变形区域进行粗到精的姿态估计，然后进行逐帧变形更新；3）设计可变形建图模块，平衡表示能力和计算效率；4）引入统一的鲁棒几何损失函数，结合外部几何先验缓解单目非刚性SLAM的病态性。

Result: 在多个公开内窥镜数据集上的实验表明，NRGS-SLAM相比现有最先进方法实现了更精确的相机姿态估计（RMSE降低达50%）和更高质量的逼真重建。全面的消融研究验证了关键设计选择的有效性。

Conclusion: NRGS-SLAM通过创新的可变形感知3D高斯表示和贝叶斯自监督策略，有效解决了内窥镜场景中相机运动与组织变形的耦合问题，在姿态估计精度和重建质量方面显著优于现有方法，为内窥镜非刚性SLAM提供了有效的解决方案。
...
### [67] [Selective Training for Large Vision Language Models via Visual Information Gain](https://arxiv.org/abs/2602.17186)
*Seulbi Lee,Sangheum Hwang*

Main category: cs.CV

TL;DR: 提出视觉信息增益(VIG)指标量化视觉输入对预测不确定性的减少，并基于此设计选择性训练方案，优先训练高VIG样本和token以改善视觉基础性并缓解语言偏见
...
Result: VIG能够有效识别视觉基础元素如颜色、空间关系和属性。选择性训练方案改善了视觉基础性，缓解了语言偏见，在显著减少监督的情况下实现了优越性能

Conclusion: VIG为量化视觉信息贡献提供了细粒度分析工具，基于VIG的选择性训练能有效提升LVLMs的视觉基础能力，同时减少训练成本
...
### [68] [EntropyPrune: Matrix Entropy Guided Visual Token Pruning for Multimodal Large Language Models](https://arxiv.org/abs/2602.17196)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型处理大量视觉token导致推理成本高昂，现有剪枝方法依赖启发式静态层选择，缺乏可解释性和跨模型可迁移性，需要更原则性的剪枝准则。

Method: 1) 引入矩阵熵视角识别"熵崩溃层"作为剪枝时机；2) 提出EntropyPrune框架，基于矩阵熵量化单个视觉token信息价值进行剪枝；3) 利用对偶Gram矩阵谱等价性降低熵计算复杂度，实现理论64倍加速。

Result: 在LLaVA-1.5-7B上实现68.2% FLOPs减少，保持96.0%原始性能；在多样化多模态基准测试中优于现有剪枝方法；在高分辨率和视频模型中表现出良好泛化能力。

Conclusion: EntropyPrune提供了一种原则性、可解释的视觉token剪枝方法，通过矩阵熵指导实现高效MLLM加速，具有强鲁棒性和可扩展性，为实际应用提供了有效解决方案。
...
### [69] [GASS: Geometry-Aware Spherical Sampling for Disentangled Diversity Enhancement in Text-to-Image Generation](https://arxiv.org/abs/2602.17200)
*Ye Zhu,Kaleb S. Newman,Johannes F. Lutzeyer,Adriana Romero-Soriano,Michal Drozdzal,Olga Russakovsky*

Main category: cs.CV

TL;DR: 提出GASS方法，通过几何视角增强文本到图像生成模型的多样性，同时控制提示相关和提示无关的变异源


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像生成模型虽然语义对齐度高，但缺乏生成多样性，这限制了用户选择并可能放大社会偏见

Method: 提出几何感知球形采样(GASS)，将CLIP嵌入中的多样性分解为两个正交方向：文本嵌入（捕获提示相关语义变异）和正交方向（捕获提示无关变异），通过增加生成图像嵌入在两个轴上的几何投影扩展来增强多样性

Result: 在不同冻结T2I骨干（U-Net和DiT，扩散和流模型）和基准测试上验证了方法的有效性，在最小化影响图像保真度和语义对齐的同时实现了解耦的多样性增强

Conclusion: GASS通过几何分解方法有效增强文本到图像生成的多样性，为多样性增强提供了新的几何视角
...
TL;DR: HiMAP提出了一种无需多目标跟踪的轨迹预测框架，通过将历史检测转换为时空不变的占用图表示，并使用历史查询模块检索特定智能体的历史信息，从而在跟踪失败时仍能保持可靠的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶运动预测方法大多依赖多目标跟踪的身份关联，当跟踪因遮挡、身份切换或漏检而失败时，预测质量会下降并增加安全风险。需要一种不依赖跟踪的鲁棒预测框架。

Method: HiMAP将过去检测转换为时空不变的历史占用图，引入历史查询模块基于当前智能体状态从无标签的占用表示中迭代检索智能体特定历史。检索到的历史通过时间图嵌入进行总结，与最终查询和地图上下文一起驱动DETR风格解码器生成多模态未来轨迹。

Result: 在Argoverse 2数据集上，HiMAP在无跟踪设置下显著优于强基线，相对QCNet在FDE上提升11%，ADE提升12%，MR降低4%。性能与基于跟踪的方法相当，同时能为所有智能体同时提供稳定预测，无需等待跟踪恢复。

Conclusion: HiMAP通过消除对身份关联的依赖，在跟踪失败时仍能保持可靠的轨迹预测，可作为跟踪不可用时的鲁棒后备方案，对安全关键自动驾驶具有实际价值。
...
### [71] [A Multi-modal Detection System for Infrastructure-based Freight Signal Priority](https://arxiv.org/abs/2602.17252)
*Ziyan Zhang,Chuheng Wei,Xuanpeng Zhao,Siyan Li,Will Snyder,Mike Stas,Peng Hao,Kanok Boriboonsomsin,Guoyuan Wu*

Main category: cs.CV

TL;DR: 基于基础设施的多模态货运车辆检测系统，集成LiDAR和摄像头传感器，支持货运信号优先控制
...
Method: 采用混合传感架构，包括交叉口安装子系统和路段中间子系统，通过无线通信连接实现同步数据传输。感知流水线结合了基于聚类和基于深度学习的检测方法，并采用卡尔曼滤波跟踪以实现稳定的实时性能。LiDAR测量注册到地理参考框架中，支持车道级定位和一致的车辆跟踪。

Result: 现场评估表明，该系统能够以高时空分辨率可靠地监测货运车辆运动。设计和部署为开发支持货运信号优先应用的基于基础设施的传感系统提供了实用见解。

Conclusion: 本文提出的基于基础设施的多模态货运车辆检测系统能够有效支持货运信号优先控制，为相关应用提供了实用的系统设计和部署经验。
...
TL;DR: EA-Swin是一种嵌入无关的Swin Transformer模型，通过因子化窗口注意力设计直接在预训练视频嵌入上建模时空依赖关系，用于AI生成视频检测，并在包含130K视频的EA-Video数据集上实现0.97-0.99的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有Sora2、Veo3等基础视频生成器能产生高度逼真的合成视频，暴露了现有检测方法的局限性：依赖浅层嵌入轨迹、基于图像的适应或计算量大的MLLMs，需要更有效的检测方案。

Method: 提出EA-Swin（嵌入无关Swin Transformer），通过因子化窗口注意力设计直接在预训练视频嵌入上建模时空依赖，兼容通用ViT风格补丁编码器。同时构建EA-Video数据集（130K视频），整合新收集样本和现有数据集，覆盖多种商业和开源生成器，包含未见生成器分割用于跨分布评估。

Result: EA-Swin在主要生成器上达到0.97-0.99准确率，比先前SoTA方法（通常0.8-0.9）提升5-20%，同时在未见分布上保持强泛化能力。

Conclusion: EA-Swin为现代AI生成视频检测提供了可扩展且鲁棒的解决方案，通过嵌入无关的时空建模和全面数据集实现了优异的检测性能和泛化能力。
...
### [73] [Attachment Anchors: A Novel Framework for Laparoscopic Grasping Point Prediction in Colorectal Surgery](https://arxiv.org/abs/2602.17310)
*Dennis N. Schneider,Lars Wagner,Daniel Rueckert,Dirk Wilhelm*

Main category: cs.CV

TL;DR: 该论文提出了一种称为"附着锚点"的结构化表示方法，用于编码结直肠手术中组织与其解剖附着点之间的局部几何和力学关系，以改善自主组织操作中的抓取点预测。


<details>
  <summary>Details</summary>
Motivation: 结直肠手术因其复杂性和持续时间长，在当前研究中代表性不足，但具有重复性组织操作的特点，为机器学习驱动的自主支持提供了有前景的切入点。准确预测抓取点是微创手术中自主组织操作的关键挑战。

Method: 引入附着锚点作为结构化表示，将手术场景归一化到一致的局部参考框架中，减少抓取点预测的不确定性。该方法可以从腹腔镜图像中预测附着锚点，并将其整合到基于机器学习的抓取框架中。

Result: 在90例结直肠手术数据集上的实验表明，与仅使用图像的基线方法相比，附着锚点显著改善了抓取点预测性能。在分布外设置（包括未见过的程序和手术医生）中表现尤为突出。
...
### [74] [Polaffini: A feature-based approach for robust affine and polyaffine image registration](https://arxiv.org/abs/2602.17337)
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像配准主要依赖基于强度的配准方法，这些方法使用对齐质量的替代度量。虽然基于特征的方法理论上更理想，但由于可靠提取特征的挑战而不再流行。深度学习分割模型的进步现在能够提供可靠、细粒度的解剖分割，这为创建新的基于解剖结构的图像配准算法提供了机会。

Method: Polaffini框架利用预训练分割模型获取解剖区域分割，从这些分割区域中提取质心作为具有1对1对应关系的解剖特征点。使用这些特征点通过闭式解实现高效的全局和局部仿射匹配。这些匹配用于生成从仿射到多仿射的变换，具有可调平滑度。多仿射变换在log-Euclidean框架中嵌入，确保微分同胚特性。

Result: Polaffini在结构对齐方面优于流行的基于强度配准方法，并为下游非线性配准提供了改进的初始化。该方法快速、稳健且准确，特别适合集成到医学图像处理流程中。

Conclusion: Polaffini展示了利用深度学习分割模型创建基于解剖结构的图像配准算法的可行性。该方法提供了一种快速、稳健且准确的配准解决方案，既可作为独立配准工具，也可作为后续非线性配准的预对齐步骤，在医学图像处理中具有实际应用价值。
...
### [75] [Tree crop mapping of South America reveals links to deforestation and conservation](https://arxiv.org/abs/2602.17372)
...
### [76] [DRetHTR: Linear-Time Decoder-Only Retentive Network for Handwritten Text Recognition](https://arxiv.org/abs/2602.17387)
*Changhun Kim,Martin Mayr,Thomas Gorges,Fei Wu,Mathias Seuret,Andreas Maier,Vincent Christlein*

Main category: cs.CV

TL;DR: DRetHTR：基于Retentive Networks的手写文本识别模型，通过软注意力替代和层间gamma缩放，实现线性时间/内存解码，在保持Transformer精度同时获得1.6-1.9倍加速和38-42%内存节省。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的手写文本识别系统存在KV缓存随解码长度增长的问题，导致推理速度慢、内存占用高。需要一种既能保持Transformer级别精度，又能实现高效解码的替代方案。

Method: 提出DRetHTR解码器模型，基于Retentive Networks构建：1) 用软注意力替代softmax注意力，避免KV缓存增长；2) 注入多尺度序列先验；3) 提出层间gamma缩放策略，在深层逐步扩大有效保留视野，模拟注意力从局部到全局的归纳偏置。

Result: 相比同等规模解码器Transformer基线，DRetHTR实现1.6-1.9倍推理加速和38-42%内存节省，无精度损失。在多个数据集上取得最佳字符错误率：IAM-A 2.26%、RIMES 1.81%、Bentham 3.46%，READ-2016上4.21%具有竞争力。

Conclusion: 基于RetNet的解码器模型能够在保持Transformer级别手写文本识别精度的同时，显著提升解码速度和内存效率，为实际部署提供了更高效的解决方案。
...
### [77] [SpectralGCD: Spectral Concept Selection and Cross-modal Representation Learning for Generalized Category Discovery](https://arxiv.org/abs/2602.17395)
*Lorenzo Caselli,Marco Mistretta,Simone Magistri,Andrew D. Bagdanov*

Main category: cs.CV

TL;DR: SpectralGCD是一种高效的多模态广义类别发现方法，利用CLIP跨模态图像-概念相似性作为统一表示，通过谱过滤和知识蒸馏保持语义质量，在六个基准测试中达到或超越SOTA性能，同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 广义类别发现(GCD)需要在利用少量已知类别标注数据的同时识别未标注数据中的新类别。现有方法存在两个主要问题：1)仅基于图像特征的参数化分类器容易对旧类别过拟合；2)多模态方法虽然提升了性能，但独立处理不同模态且计算成本高昂。

Method: 提出SpectralGCD方法：1)使用CLIP跨模态图像-概念相似性作为统一的跨模态表示，将每张图像表示为大型任务无关字典中语义概念的混合；2)引入谱过滤技术，利用强教师模型计算的跨模态协方差矩阵自动保留字典中的相关概念；3)采用前向和反向知识蒸馏，确保学生模型的跨模态表示既语义充分又对齐良好。

Result: 在六个基准测试中，SpectralGCD达到了与最先进方法相当或显著更优的准确率，同时计算成本仅为现有方法的一小部分。代码已公开可用。

Conclusion: SpectralGCD通过统一的跨模态表示、谱过滤和知识蒸馏，实现了高效且有效的广义类别发现，在保持语义质量的同时大幅降低了计算复杂度，为多模态GCD提供了新的解决方案。
...
### [78] [A High-Level Survey of Optical Remote Sensing](https://arxiv.org/abs/2602.17397)
...
<details>
  <summary>Details</summary>
Motivation: 近年来计算机视觉的显著进展推动了遥感领域的发展，同时无人机的应用日益广泛。大多数无人机默认配备RGB相机，这些传感器既鲁棒又易于使用和解释。光学遥感文献庞大，涵盖多样化的任务、能力和方法，但缺乏从整体视角出发的综述性工作。

Method: 该论文采用文献综述方法，对无人机光学遥感领域进行全面梳理。它涵盖了该领域的各种任务、能力和方法，同时提供关键信息如数据集和见解。论文旨在为研究人员提供高层次的指导，帮助他们聚焦于最相关的领域。

Result: 论文提供了无人机光学遥感领域的全面概览，包括该领域的能力、关键数据集和见解。据作者所知，这是首个从整体视角出发的综述性工作，填补了现有文献的空白。

Conclusion: 该论文为进入无人机光学遥感领域的研究人员提供了有价值的指导，通过提供高层次的见解和关键信息，帮助他们更好地理解该领域的整体情况并聚焦于相关研究方向。
...
### [79] [EAGLE: Expert-Augmented Attention Guidance for Tuning-Free Industrial Anomaly Detection in Multimodal Large Language Models](https://arxiv.org/abs/2602.17419)
*Xiaomeng Peng,Xilang Huang,Seon Han Choi*

Main category: cs.CV

TL;DR: EAGLE是一个无需调优的工业异常检测框架，通过专家模型输出指导多模态大语言模型，实现准确检测和可解释异常描述，性能媲美微调方法。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测对智能制造至关重要，但现有深度学习方法通常只提供二元决策且语义解释有限。多模态大语言模型虽能生成细粒度语言分析，但现有方法需要昂贵微调，且相比轻量级专家检测器并未持续提升检测精度。

Method: 提出EAGLE框架，无需调优，通过集成专家模型输出来指导多模态大语言模型，实现准确异常检测和可解释异常描述。进一步研究EAGLE如何影响MLLMs内部机制，通过检查中间层对异常图像区域的注意力分布来分析。

Result: 实验表明，成功的异常检测与异常区域注意力集中度增加相关，EAGLE倾向于促进这种对齐。在MVTec-AD和VisA数据集上，EAGLE无需参数更新即可提升多个MLLMs的异常检测性能，达到与基于微调方法相当的结果。

Conclusion: EAGLE是一个有效的无需调优框架，通过专家模型指导MLLMs实现工业异常检测，既提高检测准确性又提供可解释的异常描述，为MLLMs在工业检测中的应用提供了新思路。
...
### [80] [4D Monocular Surgical Reconstruction under Arbitrary Camera Motions](https://arxiv.org/abs/2602.17473)
*Jiwei Shan,Zeyu Cai,Cheng-Tai Hsieh,Yirui Li,Hao Liu,Lijun Han,Hesheng Wang,Shing Shin Cheng*

Main category: cs.CV

TL;DR: Local-EndoGS：一种用于具有任意相机运动的单目内窥镜序列的高质量4D重建框架，通过渐进式窗口化全局表示和从粗到细的策略解决大相机运动下的变形场景重建问题


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示或3D高斯泼溅的方法主要针对固定视点的变形场景，依赖立体深度先验或准确的结构从运动进行初始化和优化，难以处理真实临床环境中具有大相机运动的单目序列

Method: 提出渐进式窗口化全局表示，为每个观测窗口分配局部变形场景模型；设计从粗到细策略整合多视图几何、跨窗口信息和单目深度先验；引入长距离2D像素轨迹约束和物理运动先验

Result: 在三个具有变形场景和不同相机运动的公共内窥镜数据集上，Local-EndoGS在视觉质量和几何精度方面持续优于现有最先进方法，消融研究验证了关键设计的有效性

Conclusion: Local-EndoGS成功解决了具有大相机运动的单目内窥镜序列的4D重建挑战，通过窗口化表示和鲁棒的初始化策略实现了高质量变形场景重建，为临床应用提供了实用解决方案
...
### [81] [QuPAINT: Physics-Aware Instruction Tuning Approach to Quantum Material Discovery](https://arxiv.org/abs/2602.17478)
*Xuan-Bac Nguyen,Hoang-Quan Nguyen,Sankalp Pandey,Tim Faltermeier,Nicholas Borys,Hugh Churchill,Khoa Luu*

Main category: cs.CV

TL;DR: 提出一个物理感知的多模态框架，通过合成数据生成、指令数据集构建、物理感知注意力机制和标准化基准，解决二维量子材料光学图像分析中的挑战。


<details>
  <summary>Details</summary>
Motivation: 二维量子材料的光学显微图像分析面临三大挑战：层数依赖的对比度微弱、标注数据有限、不同实验室和成像设备间的显著差异。现有视觉模型因缺乏物理先验知识，无法泛化到新材料或新硬件条件。

Method: 1. Synthia：基于物理的合成数据生成器，模拟量子材料薄片在薄膜干涉下的光学响应；2. QMat-Instruct：首个量子材料大规模指令数据集，包含多模态、物理信息的问题-答案对；3. QuPAINT：物理感知指令调优的多模态架构，集成物理感知注意力模块，融合视觉嵌入与光学先验；4. QF-Bench：涵盖多种材料、基底和成像设置的综合性基准。

Result: 该方法从数据和模型两个角度解决了现有局限：Synthia生成多样高质量合成样本，减少对专家手动标注的依赖；QMat-Instruct为MLLMs提供理解薄片外观和厚度的训练数据；QuPAINT通过物理感知注意力实现更鲁棒和可区分的薄片表征；QF-Bench为公平可复现评估提供标准化协议。

Conclusion: 该物理感知多模态框架通过整合物理先验知识、合成数据生成和专门设计的架构，显著提升了二维量子材料光学图像分析的准确性和泛化能力，为材料科学中的自动化表征提供了新范式。
...
### [82] [Tracing Copied Pixels and Regularizing Patch Affinity in Copy Detection](https://arxiv.org/abs/2602.17484)
*Yichen Lu,Siwei Nie,Minlong Lu,Xudong Yang,Xiaobo Zhang,Peng Zhang*

Main category: cs.CV

TL;DR: 提出PixTrace像素坐标跟踪模块和CopyNCE几何引导对比损失，通过利用编辑内容的几何可追溯性改进图像复制检测，在DISC21数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在图像复制检测中面临精细对应学习不足的问题，难以处理复杂编辑操作。需要利用编辑内容的固有几何可追溯性来改进特征表示学习。

Method: 提出PixTrace模块维护跨编辑变换的显式空间映射，以及CopyNCE损失利用PixTrace验证的映射产生的重叠比率来正则化块亲和度，将像素级可追溯性与块级相似性学习相结合。

Result: 在DISC21数据集上取得最佳性能：匹配器88.7% uAP / 83.9% RP90，描述符72.6% uAP / 68.4% RP90，同时相比现有方法具有更好的可解释性。

Conclusion: 通过利用编辑内容的几何可追溯性，提出的方法有效桥接了像素级跟踪与块级相似性学习，抑制了自监督训练中的监督噪声，显著提升了图像复制检测性能。
...
### [83] [FoundationPose-Initialized 3D-2D Liver Registration for Surgical Augmented Reality](https://arxiv.org/abs/2602.17517)
...
TL;DR: 提出一种用于腹腔镜肝脏手术中肿瘤定位的增强现实注册管道，结合腹腔镜深度图与基础姿态估计器进行相机-肝脏姿态估计，并用非刚性迭代最近点替代有限元变形模型，降低工程复杂度


<details>
  <summary>Details</summary>
Motivation: 现有腹腔镜肝脏手术中的增强现实注册管道通常依赖器官轮廓，且变形（非刚性）对齐常使用有限元模型结合降维或机器学习组件，这些方法工程复杂度高且需要专业知识

Method: 整合腹腔镜深度图与基础姿态估计器进行相机-肝脏姿态估计，用非刚性迭代最近点替代有限元变形模型，结合刚性-NICP注册方法

Result: 在真实患者数据上，深度增强的基础姿态方法在3个病例中达到9.91毫米平均注册误差，刚性-NICP注册优于纯刚性注册，NICP可作为有限元变形模型的有效替代

Conclusion: 该管道在达到临床相关精度的同时，提供了比有限元变形模型更轻量、工程友好的替代方案，降低了建模复杂度和专业知识要求
...
### [84] [LATA: Laplacian-Assisted Transductive Adaptation for Conformal Uncertainty in Medical VLMs](https://arxiv.org/abs/2602.17535)
*Behzad Bozorgtabar,Dwarikanath Mahapatra,Sudipta Roy,Muzammal Naseer,Imran Razzak,Zongyuan Ge*

Main category: cs.CV

TL;DR: LATA：一种用于医学视觉语言模型的训练和标签自由的Laplacian辅助转导适应方法，通过图像-图像k-NN图平滑零样本概率，保持分形预测有效性，提高预测集效率和类别平衡


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在领域偏移下的可靠性依赖于有保证的校准不确定性。传统分形预测在少样本、不平衡场景下存在预测集过大（效率低）和类别覆盖不平衡（高类别条件覆盖差距）的问题，且直接适应校准标签会破坏可交换性并失去理论保证。

Method: 提出LATA方法：1）基于图像-图像k-NN图使用少量CCCP平均场更新平滑零样本概率；2）引入故障感知的符合分数，结合视觉语言不确定性框架提供实例级难度和标签合理性；3）保持黑盒特性（无需更新VLM）、计算轻量（窗口化转导、无反向传播）；4）提供可选先验旋钮，支持严格标签自由或标签知情变体。

Result: 在三个医学VLM和九个下游任务上，LATA一致减少集合大小和类别条件覆盖差距，同时匹配或收紧目标覆盖，优于先前的转导基线，缩小与标签使用方法的差距，且计算量显著减少。消融实验和定性分析表明LATA在不损害可交换性的情况下锐化了零样本预测。

Conclusion: LATA是一种有效、计算高效的训练和标签自由方法，能够改善医学视觉语言模型在领域偏移下的不确定性校准，提高预测集效率和类别平衡，同时保持分形预测的理论保证。
...
### [85] [GraphThinker: Reinforcing Video Reasoning with Event Graph Thinking](https://arxiv.org/abs/2602.17555)
...
<details>
  <summary>Details</summary>
Motivation: 视频推理需要理解事件间的因果关系，但这些关系通常是隐式的且标注成本高。现有的多模态大语言模型通过密集描述或视频摘要推断事件关系，但缺乏因果结构建模，导致视频推理中出现幻觉问题。
...
Conclusion: 通过显式建模事件级场景图结构和增强视觉定位，GraphThinker有效减少了视频推理中的幻觉问题，提升了因果理解能力。
...
TL;DR: ArtToMus：首个直接基于艺术作品生成音乐的框架，无需图像到文本转换或语言监督，使用视觉嵌入指导潜在扩散模型生成音乐。


<details>
  <summary>Details</summary>
Motivation: 现有图像条件音乐生成系统存在两个根本限制：1）通常在自然照片上训练，无法捕捉艺术作品更丰富的语义、风格和文化内容；2）大多依赖图像到文本转换阶段，使用语言作为语义捷径，简化了条件约束但阻碍了直接的视觉到音频学习。

Method: 提出ArtSound数据集（105,884个艺术作品-音乐配对，带双模态描述），并开发ArtToMus框架，将视觉嵌入投影到潜在扩散模型的条件空间中，实现无需图像到文本翻译或语言监督的直接艺术作品到音乐生成。

Result: ArtToMus生成音乐连贯且风格一致，能反映源艺术作品的显著视觉线索。虽然绝对对齐分数低于文本条件系统（考虑到去除语言监督的难度显著增加），但在感知质量和有意义的跨模态对应方面具有竞争力。
...
TL;DR: 提出统一地理空间发现框架，集成主动学习、在线元学习和概念引导推理，通过概念相关性加权不确定性采样和相关性感知元批次形成策略，在数据有限和动态环境中高效发现隐藏目标。


<details>
  <summary>Details</summary>
Motivation: 在环境监测、灾害响应、公共卫生等实际应用中，数据收集成本高、环境动态变化，需要在资源约束下战略性地从未观测区域采样以高效发现隐藏目标。现有基于学习的方法（如强化学习）受限于稀疏且有偏的地理空间地面实况数据，应用受限。

Method: 提出统一地理空间发现框架，集成主动学习、在线元学习和概念引导推理。核心创新包括：1）概念加权不确定性采样策略，基于领域特定概念（如土地覆盖、污染源距离）学习概念相关性，用相关性调制不确定性；2）相关性感知元批次形成策略，在在线元更新中促进语义多样性，提升动态环境下的泛化能力。

Result: 在真实世界数据集（致癌PFAS污染物）上进行实验，展示了该方法在数据有限和环境变化条件下可靠发现目标的能力。

Conclusion: 该框架通过整合领域概念和在线元学习，有效解决了地理空间发现中数据稀疏、有偏和环境动态的挑战，在资源受限的实际应用中具有实用价值。
...
TL;DR: CORAL是一个基于扩散变换器(DiT)的虚拟试穿框架，通过显式对齐查询-键匹配与外部对应关系来改善服装细节保留，包含对应蒸馏损失和熵最小化损失两个组件。
...
Method: 首先分析DiT架构中的完整3D注意力机制，发现人-服装对应关系依赖于查询-键精确匹配。基于此提出CORAL框架，包含：1）对应蒸馏损失，将可靠匹配与人-服装注意力对齐；2）熵最小化损失，锐化注意力分布。还提出基于VLM的评估协议。

Result: CORAL在基线上持续改进，增强了全局形状传递和局部细节保留。大量消融实验验证了设计选择的有效性。

Conclusion: 通过显式对齐查询-键匹配与外部对应关系，CORAL框架有效解决了虚拟试穿中服装细节保留不足的问题，为DiT架构中对应关系形成提供了理论解释。
...
Method: 提出IntRec交互式物体检索框架，核心是意图状态维护正锚点（确认线索）和负约束（拒绝假设）的双记忆集，通过对比对齐函数对候选物体排序，最大化与正线索的相似性同时惩罚被拒绝的假设。

Result: 在LVIS上达到35.4 AP，分别超过OVMR、CoDet和CAKE 2.3、3.7和0.5个点；在LVIS-Ambiguous基准上，单次纠正反馈后比一次性基线提升7.9 AP，每次交互增加延迟小于30毫秒。
...
### [90] [When Vision Overrides Language: Evaluating and Mitigating Counterfactual Failures in VLAs](https://arxiv.org/abs/2602.17659)
*Yu Fang,Yuchun Feng,Dong Jing,Jiaqi Liu,Yue Yang,Zhenyu Wei,Daniel Szafir,Mingyu Ding*

Main category: cs.CV

TL;DR: 提出LIBERO-CF基准和CAG方法，解决VLA模型在缺乏场景监督时依赖视觉捷径而非语言指令的问题


<details>
  <summary>Details</summary>
Motivation: VLA模型在实际应用中经常无法忠实遵循语言指令，当指令缺乏强场景特定监督时，模型会基于数据集偏见产生的视觉捷径采取行动，而非遵循语言意图

Method: 1) 引入LIBERO-CF基准评估VLA语言跟随能力；2) 提出CAG方法，结合标准VLA策略和无语言条件的视觉-动作模块，通过反事实比较进行动作选择

Result: CAG在LIBERO-CF上将语言跟随准确率提高9.7%，任务成功率提高3.6%；结合VA模型后分别提升15.5%和8.5%；真实世界评估中减少9.4%反事实失败，平均任务成功率提高17.2%

Conclusion: CAG是一种简单有效的即插即用方法，能显著减少VLA模型对视觉捷径的依赖，提高语言跟随能力和任务成功率，无需额外演示或修改现有架构
...
### [91] [OpenEarthAgent: A Unified Framework for Tool-Augmented Geospatial Agents](https://arxiv.org/abs/2602.17665)
...
Method: 提出统一框架，通过监督微调结构化推理轨迹进行训练，对齐模型与已验证的多步工具交互。包含14,538个训练实例和1,169个评估实例，涵盖城市、环境、灾害和基础设施领域，整合GIS操作和NDVI、NBR、NDBI等指数分析。
...
TL;DR: 研究发现当前AI安全数据集过度依赖"触发线索"（具有明显负面/敏感含义的词汇），无法真实反映现实世界攻击，且通过"意图清洗"技术可有效绕过现有安全机制。


<details>
  <summary>Details</summary>
Motivation: 评估广泛使用的AI安全数据集质量，检验其是否真实反映现实世界攻击，以及是否真正衡量安全风险而非仅仅通过触发线索引发拒绝响应。

Method: 从两个角度系统评估：1) 孤立分析数据集是否反映真实攻击的三个关键属性（恶意意图驱动、精心设计、分布外）；2) 实践中通过"意图清洗"技术抽象掉触发线索但保留恶意意图，评估模型安全性。

Result: 当前AI安全数据集过度依赖触发线索，无法忠实代表现实攻击。移除触发线索后，所有先前评估为"相对安全"的模型（包括Gemini 3 Pro和Claude Sonnet 3.7）都变得不安全。意图清洗作为越狱技术，在黑盒访问下攻击成功率高达90%-98%。

Conclusion: 当前AI安全评估方法与现实世界攻击者行为存在显著脱节，需要开发更贴近实际威胁的安全数据集和评估方法。
...
TL;DR: 对抗性注释对LLM漏洞检测性能影响有限，与代码生成场景不同，检测准确率未显著下降


<details>
  <summary>Details</summary>
Motivation: 测试对抗性提示操作是否会在漏洞检测中误导LLMs，因为先前研究表明在代码生成任务中对抗性提示会降低LLM性能

Method: 构建包含Python、JavaScript和Java的100样本基准，每个样本配对8种注释变体（从无注释到权威欺骗和技术欺骗等对抗策略），在9,366次试验中评估8个前沿模型（5个商业模型和3个开源模型）

Result: 对抗性注释对检测准确率产生微小且统计上不显著的影响（McNemar精确p>0.21；所有95%置信区间包含零），商业模型基线检测率为89-96%，开源模型为53-72%，复杂对抗策略相比简单操纵性注释无优势

Conclusion: 与代码生成场景不同，对抗性注释不会显著降低漏洞检测性能，失败主要集中在固有的困难漏洞类别（如竞争条件、时序侧信道和复杂授权逻辑），而非对抗性注释
...
TL;DR: 该研究对LLM排序器进行全面的越狱提示攻击实证分析，评估不同模型架构和设置下的漏洞边界条件


<details>
  <summary>Details</summary>
Motivation: LLM作为强大的重排序器存在安全风险，简单提示注入可显著改变其排序决策，但该漏洞在不同LLM家族、架构和设置中的持续性尚未充分探索

Method: 采用两种互补任务评估：偏好漏洞评估（通过攻击成功率测量内在易感性）和排序漏洞评估（通过nDCG@10量化对排序质量的操作影响）。系统研究三种主流排序范式（成对、列表、集合）下的两种注入变体：决策目标劫持和决策标准劫持

Result: 研究揭示了这些漏洞的边界条件，关键发现包括编码器-解码器架构对越狱攻击表现出强大的固有韧性。分析了漏洞在不同模型家族、位置敏感性、骨干架构和跨域鲁棒性中的扩展情况

Conclusion: 该研究提供了LLM排序器越狱攻击的全面实证分析，揭示了架构相关的漏洞模式，为开发更安全的LLM排序系统提供了重要见解
...
TL;DR: LLM能够大规模去匿名化：通过分析伪匿名在线资料和对话，可高精度重新识别用户，性能远超传统方法


<details>
  <summary>Details</summary>
Motivation: 研究LLM在去匿名化攻击中的能力，验证在线伪匿名用户的"实际模糊性"保护是否仍然有效，重新评估在线隐私威胁模型

Method: 设计可扩展的攻击流程：1) 使用LLM提取身份相关特征；2) 通过语义嵌入搜索候选匹配；3) 对顶级候选进行推理验证以减少误报。构建三个真实数据集进行评估

Result: LLM方法显著优于传统基线：在90%精确度下达到68%召回率，而最佳非LLM方法接近0%。成功重新识别Hacker News用户和Anthropic Interviewer参与者

Conclusion: 保护伪匿名用户的"实际模糊性"已失效，LLM使大规模去匿名化变得可行，需要重新考虑在线隐私威胁模型
...
TL;DR: NeST：一种轻量级、结构感知的安全对齐框架，通过选择性适配安全相关神经元子集来增强拒绝行为，相比全微调减少17,310倍参数更新，相比LoRA减少9.25倍参数，将攻击成功率从44.5%降至4.36%


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法存在多个问题：全微调计算和存储开销大；参数高效方法如LoRA在安全增益上不一致且对设计选择敏感；安全干预机制如断路器不直接塑造或保留控制安全行为的内部表示。这些限制阻碍了快速可靠的安全更新，特别是在模型频繁演化或需要适应新策略和领域的场景中。

Method: NeST框架通过选择性适配安全相关神经元子集来增强拒绝行为，同时冻结模型的其余部分。它通过聚类功能一致的安全神经元并在每个簇内强制执行共享更新，使参数更新与安全行为的内部组织对齐，实现有针对性和稳定的安全适配，无需广泛的模型修改或推理时开销。

Result: 在10个开源LLM上进行基准测试，NeST将攻击成功率从平均44.5%降至4.36%，相当于不安全生成减少了90.2%，平均仅需0.44百万可训练参数。相比全微调参数更新减少17,310倍，相比LoRA减少9.25倍，同时始终实现更强的安全性能。

Conclusion: NeST提供了一种轻量级、结构感知的安全对齐方法，能够快速可靠地更新模型安全行为，特别适用于模型频繁演化或需要适应新策略和领域的场景，为负责任部署LLM提供了有效的解决方案。
...
TL;DR: 利用隐私保护LLM推理实现低成本验证推理的新方法，避免传统零知识证明的高计算开销


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模增长，用户难以本地运行而依赖第三方托管服务，但缺乏对推理计算过程的保证，不诚实的提供商可能用廉价弱模型替代昂贵大模型

Method: 提出两种新协议，利用隐私保护LLM推理技术提供推理过程保证，只需添加少量额外token计算，对下游影响极小

Result: 提出的方法成本低廉，计算开销小，验证运行时间优于零知识证明方法，隐私保护推理方法通常比ZK方法更快

Conclusion: 工作揭示了隐私与可验证性在LLM推理中的新联系，通过隐私保护推理实现边际成本的验证推理
...
TL;DR: 该论文提出基于格罗滕迪克拓扑和层理论的密码学安全结构重构，将攻击者观测建模为格罗滕迪克位点，协议转录本自然形成层，安全性质表现为几何条件。


<details>
  <summary>Details</summary>
Motivation: 传统密码学安全使用基于游戏或模拟的定义，本文旨在通过代数几何工具提供更结构化的安全表述，为模拟安全性提供概念性解释，并为密码学抽象建立几何基础。

Method: 将攻击者观测建模为格罗滕迪克位点，覆盖族表示由高效模拟确定的部分信息的可容许分解。在此框架下，协议转录本形成层，安全性质作为几何条件出现。特别针对Σ-协议，证明其转录本结构定义了关联层拓扑中的挠子。

Result: 展示了Σ-协议转录本结构在关联层拓扑中定义挠子，局部平凡性对应零知识性，全局截面的缺失反映可靠性。通过Schnorr Σ-协议的具体分析说明了该构造。

Conclusion: 层理论视角为模拟安全性提供了概念性解释，并为进一步的密码学抽象提出了几何基础，为密码学安全提供了新的结构化表述框架。
...
<details>
  <summary>Details</summary>
Motivation: Fischlin变换在经典随机预言机模型中能产生具有直线可提取性的非交互零知识证明，但其在量子随机预言机模型(QROM)中的直线可提取性一直未解决，因为量子查询的复杂性使得在QROM中分析查询转录本的概率变得困难。

Method: 使用基于压缩预言机的提取器，结合独立随机变量和的尾界、鞅的尾界、对称化、查询振幅和量子并集界等分析技术，证明Fischlin变换在QROM中的直线可提取性。

Result: 证明了Fischlin变换在量子随机预言机模型(QROM)中保持直线可提取性，建立了该变换的后量子安全性，提供了比Pass变换证明尺寸更小的后量子直线可提取NIZK方案。

Conclusion: Fischlin变换在量子随机预言机模型中是直线可提取的，这为后量子安全提供了更高效的NIZK构造方案，解决了长期存在的开放性问题。
...
### [100] [DAVE: A Policy-Enforcing LLM Spokesperson for Secure Multi-Document Data Sharing](https://arxiv.org/abs/2602.17413)
*René Brinkhege,Prahlad Menon*

Main category: cs.CR

TL;DR: DAVE是一个基于LLM的发言人系统，在数据空间中通过自然语言接口回答查询，根据机器可读的使用策略实时过滤敏感信息，避免直接共享原始文档。


<details>
  <summary>Details</summary>
Motivation: 当前组织间数据空间中的使用策略主要在资产级别执行，要么共享整个文档，要么完全不共享。当文档只有部分内容敏感时，数据提供者需要手动编辑文档，这种方法成本高、粒度粗，且难以随策略或合作伙伴变化而维护。

Method: 提出DAVE系统，这是一个基于LLM的使用策略执行发言人，代表数据提供者回答关于私有文档的问题。系统采用虚拟编辑技术，在查询时根据机器可读的使用策略（如ODRL风格策略）抑制敏感信息，而不修改源文档。系统架构与Eclipse Dataspace Components集成，QA请求通过发言人服务路由而非触发原始文档传输。

Result: 本文主要贡献是架构设计，尚未实现或实证评估完整的执行流程。提出了一个评估方法论来评估安全性、实用性和性能权衡，作为未来实证研究的基础。

Conclusion: DAVE为多方数据空间中系统化管理的LLM访问提供了一种新方法，通过自然语言接口和实时策略执行，解决了传统文档级共享的局限性，为未来研究奠定了基础。
...
### [101] [Privacy in Theory, Bugs in Practice: Grey-Box Auditing of Differential Privacy Libraries](https://arxiv.org/abs/2602.17454)
*Tudor Cebere,David Erb,Damien Desfontaines,Aurélien Bellet,Jack Fitzsimons*

Main category: cs.CR

TL;DR: Re:cord-play是一种灰盒审计范式，通过检查DP算法内部状态来检测隐私违规，相比现有方法更实用有效


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私验证方法存在局限性：形式化工具限制过多，黑盒统计审计对复杂流程不可行且无法定位错误源，需要更实用的验证方法

Method: 引入Re:cord-play灰盒审计范式，通过运行带仪器化的算法在相邻数据集上（使用相同随机性），检查数据相关控制流，并通过比较声明敏感度与内部输入实测距离来具体证伪敏感度违规；进一步扩展为Re:cord-play-sample，对每个组件进行完整统计审计

Result: 审计12个开源库（包括SmartNoise SDK、Opacus、Diffprivlib），发现13个影响理论保证的隐私违规，证明该方法既有效又必要

Conclusion: Re:cord-play提供了一种计算成本低、易于集成的隐私测试框架，可作为软件开发周期的一部分，帮助DP开发者进行有效的隐私验证
...
### [102] [Coin selection by Random Draw according to the Boltzmann distribution](https://arxiv.org/abs/2602.17490)
*Jan Lennart Bönsel,Michael Maurer,Silvio Petriconi,Andrea Tundis,Marc Winstel*

Main category: cs.CR

TL;DR: 本文提出了一种基于玻尔兹曼分布的随机算法（Boltzmann Draw）用于代币支付系统中的币选择问题，相比随机选择和贪婪算法，该算法能有效限制输入代币数量、减少粉尘生成并控制钱包代币池大小。


<details>
  <summary>Details</summary>
Motivation: 在加密货币和央行数字货币等代币支付系统中，币选择问题（选择一组代币来资助交易）是一个重要挑战。现有方法如随机选择和贪婪算法存在各种限制，需要一种既能有效管理代币数量、减少粉尘生成，又能保持高效实现和隐私保护的改进方案。

Method: 提出玻尔兹曼抽取算法，这是一种基于统计物理学原理的概率算法。该方法通过玻尔兹曼分布来抽取代币，作为随机抽取方法的扩展和改进。算法设计考虑了高效实现、性能优化和隐私保护要求。

Result: 数值结果表明，玻尔兹曼抽取算法能有效限制所选输入代币的数量，减少粉尘生成，并控制钱包中的代币池大小。与标准随机抽取和贪婪算法相比，玻尔兹曼抽取在实现效率、性能表现和隐私保护方面表现更优。

Conclusion: 玻尔兹曼抽取算法为代币支付系统的币选择问题提供了有效的解决方案，特别适用于需要处理高频大交易量的央行数字货币等场景。该算法在限制代币数量、减少粉尘、控制钱包大小等方面优于现有方法，同时满足高效实现和隐私保护的要求。
...
### [103] [Non-Trivial Zero-Knowledge Implies One-Way Functions](https://arxiv.org/abs/2602.17651)
*Suvradip Chakraborty,James Hulett,Dakshita Khurana,Kabir Tomer*

Main category: cs.CR

TL;DR: 该论文在NP⊈ioP/poly假设下，证明了非平凡零知识论证（包括非交互式和常数轮公开掷币交互式）蕴含单向函数的存在性，填补了高误差区域的理论空白。


<details>
  <summary>Details</summary>
Motivation: 先前研究[Hirahara and Nanashima, STOC'2024]表明，在NP⊈ioP/poly假设下，具有可忽略误差的零知识论证蕴含单向函数。然而，对于高误差区域（即零知识误差、完备性误差和可靠性误差之和接近1的情况），单向函数的存在性问题尚未解决。本文旨在填补这一理论空白，探索在误差参数较大的情况下，零知识论证与单向函数之间的关系。

Method: 论文定义了"非平凡"零知识论证：其完备性误差、可靠性误差和零知识误差之和严格小于1。在此定义下，研究了两类零知识系统：1）非交互式零知识（NIZK）论证；2）常数轮公开掷币交互式零知识论证。在NP⊈ioP/poly的复杂性假设下，证明了这两类非平凡零知识论证都蕴含单向函数的存在性。此外，利用已知的放大技术，将弱NIZK无条件转化为标准NIZK。

Result: 主要结果：1）在NP⊈ioP/poly假设下，非平凡NIZK论证蕴含单向函数，且可无条件将弱NIZK转化为标准NIZK；2）同样假设下，非平凡常数轮公开掷币交互式零知识论证也蕴含单向函数，进而可得标准四轮零知识论证。这些结果填补了先前工作[Chakraborty, Hulett and Khurana, CRYPTO'2025]留下的理论空白，该工作仅处理了ε_zk+√ε_s<1的情况，而本文解决了ε_zk+√ε_s≥1的高误差区域。

Conclusion: 本文在高误差区域建立了零知识论证与单向函数之间的紧密联系，证明了非平凡零知识论证（包括非交互式和交互式）在标准复杂性假设下蕴含单向函数。这一成果不仅填补了理论空白，还为从最坏情况硬度构造单向函数提供了新的技术路径和启示。
...
### [104] [Beyond Procedure: Substantive Fairness in Conformal Prediction](https://arxiv.org/abs/2602.16794)
*Pengqi Liu,Zijun Yu,Mouloud Belbahri,Arthur Charpentier,Masoud Asgharian,Jesse C. Cresswell*

Main category: stat.ML

TL;DR: 该论文研究共形预测与公平性的交互，超越程序公平性分析下游决策管道的实质性公平性，提出理论界限分解预测集大小差异，引入LLM评估器，实验显示标签聚类CP变体提供更好的实质性公平性。


<details>
  <summary>Details</summary>
Motivation: 共形预测提供分布无关的不确定性量化，但其与下游决策公平性的交互关系尚未充分探索。现有研究多关注共形预测作为独立操作的程序公平性，而缺乏对整个决策管道中实质性公平性（下游结果公平性）的系统分析。

Method: 1) 理论分析：推导预测集大小差异的上界，将其分解为可解释的组成部分，阐明标签聚类共形预测如何控制方法驱动的公平性贡献；2) 实证评估：引入LLM在环评估器，近似人类对不同模态下实质性公平性的评估；3) 实验设计：比较不同CP变体的公平性表现。

Result: 实验表明标签聚类CP变体始终提供更优的实质性公平性。实证结果显示，预测集大小的均衡化（而非覆盖率）与改善的实质性公平性强烈相关，这为设计更公平的CP系统提供了实用指导。

Conclusion: 通过理论分析和LLM辅助的实证评估，该研究揭示了共形预测与公平性的深层关系，表明标签聚类CP方法能有效提升下游决策的实质性公平性，且预测集大小均衡化是改善公平性的关键因素，为公平感知的CP系统设计提供了理论基础和实践指导。
...
### [105] [Poisson-MNL Bandit: Nearly Optimal Dynamic Joint Assortment and Pricing with Decision-Dependent Customer Arrivals](https://arxiv.org/abs/2602.16923)
*Junhui Cai,Ran Chen,Qitao Huang,Linda Zhao,Wu Zhu*

Main category: stat.ML

TL;DR: 提出Poisson-MNL模型，将MNL选择模型与泊松到达模型结合，开发基于UCB的PMNL算法，实现动态联合品类和定价优化


<details>
  <summary>Details</summary>
Motivation: 传统MNL模型假设顾客到达率固定，但现实中品类和定价不仅影响顾客购买选择，还影响到达顾客数量。忽略到达率对决策的依赖会导致次优决策，需要开发能同时学习顾客选择和到达模型的动态优化方法。

Method: 提出Poisson-MNL模型，将上下文MNL选择模型与泊松到达模型耦合，到达率取决于提供的品类和价格。基于此模型开发PMNL算法，采用上置信界(UCB)思想进行动态联合品类和定价决策。

Result: 证明了PMNL算法的非渐进遗憾界为O(√T log T)，并给出了匹配的下界（相差log T因子）。仿真研究表明，考虑到达率对品类和定价的依赖性至关重要，PMNL能有效学习顾客选择和到达模型，其联合决策优于假设固定到达率的方法。

Conclusion: Poisson-MNL模型能更准确地描述现实零售场景，PMNL算法在动态联合品类和定价优化中具有理论保证和实际优势，为考虑到达率依赖性的动态决策问题提供了有效解决方案。
...
### [106] [Anti-causal domain generalization: Leveraging unlabeled data](https://arxiv.org/abs/2602.17187)
*Sorawit Saengkyongam,Juan L. Gamella,Andrew C. Miller,Jonas Peters,Nicolai Meinshausen,Christina Heinze-Deml*

Main category: stat.ML

TL;DR: 提出一种在反因果设置下的领域泛化方法，利用无标签多环境数据，通过惩罚模型对协变量扰动的敏感性来提高泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有领域泛化方法通常需要多个训练环境的标签数据，限制了在标签稀缺场景下的应用。反因果设置中，结果导致观测协变量，环境扰动影响协变量但不传播到结果，这启发了利用无标签数据估计扰动方向

Method: 提出两种方法：分别惩罚模型对协变量均值和协方差跨环境变化的敏感性。利用无标签多环境数据估计扰动方向，无需标签信息

Result: 证明了这些方法在特定环境类别下具有最坏情况最优性保证。在受控物理系统和生理信号数据集上展示了方法的实证性能

Conclusion: 反因果设置下的领域泛化方法能够有效利用无标签多环境数据，通过正则化模型对协变量扰动的敏感性来提高泛化能力，解决了标签稀缺场景下的应用限制
...
### [107] [genriesz: A Python Package for Automatic Debiased Machine Learning with Generalized Riesz Regression](https://arxiv.org/abs/2602.17543)
*Masahiro Kato*

Main category: stat.ML

TL;DR: genriesz是一个Python包，通过Riesz表示定理和去偏机器学习实现因果与结构参数的高效估计，提供自动DML和广义Riesz回归框架。


<details>
  <summary>Details</summary>
Motivation: 开发一个开源工具来自动化因果和结构参数的高效估计，统一多种估计方法（协变量平衡、最近邻匹配、校准估计、密度比估计等），通过自动回归器平衡简化用户操作。

Method: 基于Riesz表示定理和去偏机器学习，实现广义Riesz回归框架，通过最小化经验Bregman散度估计Riesz表示子。核心设计原则是自动回归器平衡（ARB）：给定Bregman生成器和表示子模型类，自动构建兼容的链接函数，使估计器在用户选择基中满足平衡最优条件。

Result: 开发了genriesz Python包，提供模块化接口用于指定目标线性泛函、表示子模型和Bregman生成器，返回回归调整、Riesz加权、增强Riesz加权和TMLE风格估计器，支持交叉拟合、置信区间和p值计算。

Conclusion: genriesz为因果推断提供统一、自动化的估计框架，简化了ATE、ATT和平均边际效应等常见问题的估计流程，代码开源且易于使用。
...
### [108] [SOLVAR: Fast covariance-based heterogeneity analysis with pose refinement for cryo-EM](https://arxiv.org/abs/2602.17603)
*Roey Yadgar,Roy R. Lederman,Yoel Shkolnisky*

Main category: stat.ML

TL;DR: SOLVAR是一种基于随机优化的低秩变异性分析方法，用于高效估计冷冻电镜中的连续构象异质性，通过低秩协方差矩阵假设实现计算可行性，并支持粒子图像姿态精修。


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜中连续异质性表征是一个关键挑战，现有协方差方法在高效估计协方差矩阵方面存在计算困难，需要一种既能准确捕捉结构变异性又保持计算效率的方法。

Method: 提出SOLVAR方法，利用协方差矩阵的低秩假设，将主成分估计转化为可快速准确求解的优化问题，并引入粒子图像姿态精修功能，这是大多数异质性分析方法所不具备的。

Result: 在合成和实验数据集上的数值实验表明，SOLVAR能准确捕捉变异性的主要成分，同时保持计算效率，在最近的异质性基准测试中实现了最先进的性能。

Conclusion: SOLVAR为冷冻电镜连续异质性分析提供了一种高效、准确的协方差估计方法，其低秩优化框架和姿态精修能力使其在计算效率和功能完整性方面具有优势。
...
### [109] [Asymptotically Optimal Sequential Testing with Markovian Data](https://arxiv.org/abs/2602.17587)
*Alhad Sethi,Kavali Sofia Sagar,Shubhada Agrawal,Debabrota Basu,P. N. Karthik*

Main category: math.ST

TL;DR: 该论文研究了马尔可夫链数据的单边和α-正确序贯假设检验，建立了非渐近的实例依赖下界，并提出了渐近匹配该下界的最优检验方法。


<details>
  <summary>Details</summary>
Motivation: 研究马尔可夫链数据的序贯假设检验问题，现有下界要么是渐近的，要么在该设置下被证明是次优的，需要建立更精确的非渐近下界来表征最优序贯检验程序。

Method: 1. 建立了一个紧的非渐近实例依赖下界，该下界包含了未知马尔可夫链诱导的平稳分布和转移结构；2. 提出了一种最优检验方法，其期望停止时间在α→0时渐近匹配该下界。

Result: 1. 获得了比现有下界更精确的非渐近下界；2. 提出的检验方法在渐近意义下达到最优；3. 框架应用于马尔可夫链蒙特卡洛的模型误设检测和马尔可夫决策过程中的转移动态线性性测试等实际问题。

Conclusion: 该研究为马尔可夫依赖下的最优序贯检验程序提供了尖锐而一般的表征，改进了现有理论结果，并具有实际应用价值。
...
### [110] [Support Recovery and $\ell_2$-Error Bound for Sparse Regression with Quadratic Measurements via Weakly-Convex-Concave Regularization](https://arxiv.org/abs/2602.17466)
*Jun Fan,Jingyu Yang,Xinyu Zhang,Liqun Wang*

Main category: math.ST

TL;DR: 该论文研究了高维二次测量模型中弱凸凹正则化估计器的有限样本性质，建立了支持恢复和ℓ₂误差界，并提出了两种近端梯度求解策略。


<details>
  <summary>Details</summary>
Motivation: 从二次测量中恢复未知信号在相位恢复、电力系统状态估计和无标记距离几何等领域有广泛应用。需要研究高维二次测量模型中正则化估计器的有限样本性质。

Method: 采用弱凸凹惩罚最小二乘方法，建立局部最小化器的支持恢复和ℓ₂误差界。使用两种近端梯度策略求解优化问题：近端步长通过闭式解或加权ℓ₁近似计算，具体取决于正则化函数。

Result: 建立了支持恢复和ℓ₂误差的理论界限。数值实验证明了所提方法的有效性。

Conclusion: 该研究为高维二次测量模型中的弱凸凹正则化估计器提供了有限样本理论保证，并提出了有效的数值求解算法，在多个应用领域具有实用价值。
...
TL;DR: 提出一个用于检测和减少数据中冗余信息（ballast）的多模态框架，通过多种方法识别无效特征并引入Ballast Score进行统一剪枝，实验显示可显著降低特征维度并提升效率。


<details>
  <summary>Details</summary>
Motivation: 现代数据集常包含大量冗余或低效用信息（ballast），这些信息增加维度、存储需求和计算成本，但不提供有意义的分析价值。需要一种通用框架来检测和减少各种数据类型中的ballast。

Method: 提出一个多模态框架，使用熵、互信息、Lasso、SHAP、PCA、主题建模和嵌入分析等多种方法来识别ballast特征。引入新颖的Ballast Score将这些信号整合为统一的跨模态剪枝策略。

Result: 实验结果表明，在稀疏或半结构化数据中，通常超过70%的特征空间可以被剪枝，同时分类性能损失最小甚至有所改善。训练时间和内存占用显著减少，框架揭示了不同的ballast类型（统计、语义、基础设施等）。

Conclusion: 该框架为更精简、更高效的机器学习流程提供了实用指导，能够有效识别和消除各种数据类型中的冗余信息，显著提升数据处理效率。
...
### [112] [A Theoretical Framework for Modular Learning of Robust Generative Models](https://arxiv.org/abs/2602.17554)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 提出模块化LLM训练框架，通过门控机制组合领域专家模型，无需启发式数据混合调优，理论证明优于单体模型，实验验证有效缓解梯度冲突


<details>
  <summary>Details</summary>
Motivation: 大规模生成模型训练资源密集且依赖启发式数据集加权。需要解决两个基本问题：能否模块化训练LLM（组合小型领域专家）达到单体性能？能否对任意数据混合实现鲁棒性，消除启发式调优？

Method: 提出模块化生成建模理论框架：预训练专家通过门控机制组合。定义归一化门控函数空间G₁，将问题表述为极小极大博弈以寻找最小化最坏情况数据混合散度的鲁棒门控。使用Kakutani不动点定理证明鲁棒门控存在性。引入可扩展随机原始对偶算法和结构蒸馏方法进行高效推理。

Result: 理论证明模块化作为强正则化器，泛化界与轻量级门控复杂度成比例。证明模块化方法理论上可优于聚合数据重训练模型，差距由Jensen-Shannon散度表征。合成和真实数据集实验证实模块化架构有效缓解梯度冲突，鲁棒性优于单体基线。

Conclusion: 模块化LLM训练框架通过理论保证的鲁棒门控机制组合领域专家，无需启发式数据混合调优，在理论和实验上均优于传统单体训练方法，为高效可扩展的生成模型训练提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有统计水印方法存在两个关键局限：缺乏选择采样分布的原则性方法，以及依赖固定时间假设检验无法实现有效早期停止。需要开发既能统一最优采样又能支持任意时间有效推断的水印框架。

Method: 提出基于e值的锚定水印框架，通过锚定分布近似目标模型，构建检测过程的测试超鞅，表征最坏情况对数增长率下的最优e值，并推导最优期望停止时间。

Result: 在模拟和基准测试中验证了理论主张，相比最先进的基线方法，该框架能显著提升样本效率，将检测所需的平均token预算减少13-15%。

Conclusion: Anchored E-Watermarking是首个统一最优采样与任意时间有效推断的e值水印框架，解决了传统方法中可选停止使Type-I错误保证失效的问题，实现了有效的任意时间推断。
...
### [114] [MMCAformer: Macro-Micro Cross-Attention Transformer for Traffic Speed Prediction with Microscopic Connected Vehicle Driving Behavior](https://arxiv.org/abs/2602.16730)
*Lei Han,Mohamed Abdel-Aty,Younggun Kim,Yang-Jun Joo,Zubayer Islam*

Main category: cs.LG

TL;DR: 提出MMCAformer模型，通过交叉注意力机制融合宏观交通流特征与微观驾驶行为特征进行速度预测，在四个佛罗里达高速公路上验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖宏观聚合交通流数据进行预测，但交通动态也受微观驾驶行为影响。联网车辆数据提供了丰富的驾驶行为特征，为整合行为洞察提供了新机会。

Method: 提出Macro-Micro Cross-Attention Transformer (MMCAformer)，使用自注意力学习宏观交通流内在依赖，通过交叉注意力捕获宏观交通状态与微观驾驶行为的时空交互。采用Student-t负对数似然损失进行优化，提供点预测和不确定性估计。

Result: 在四个佛罗里达高速公路上验证，相比仅使用宏观特征，引入微观驾驶行为特征使整体RMSE、MAE和MAPE分别降低9.0%、6.9%和10.2%，预测不确定性区间缩小10.1-24.0%。硬刹车和加速频率是最具影响力的特征，改进在拥堵低速条件下更显著。

Conclusion: MMCAformer通过融合宏观交通流和微观驾驶行为特征，显著提升了速度预测精度并降低了不确定性，证明了微观驾驶行为特征在交通预测中的重要性，特别是在拥堵条件下。
...
### [115] [A Few-Shot LLM Framework for Extreme Day Classification in Electricity Markets](https://arxiv.org/abs/2602.16735)
...
<details>
  <summary>Details</summary>
Motivation: 电力市场中实时电价尖峰预测对市场参与者至关重要，但传统机器学习方法需要大量历史数据，在数据稀缺环境下性能受限。需要开发数据高效的方法来准确预测电价尖峰。

Method: 提出基于大语言模型的少样本分类框架：1) 聚合系统状态信息（电力需求、可再生能源发电、天气预报、近期电价）为统计特征；2) 将特征格式化为自然语言提示；3) 结合通用指令输入LLM；4) 模型输出次日为尖峰日的概率和置信度分数。

Result: 使用德克萨斯电力市场历史数据验证：1) 少样本方法性能与监督机器学习模型（支持向量机、XGBoost）相当；2) 在历史数据有限时，LLM方法优于后两者；3) 证明了LLM作为数据稀缺环境下电价尖峰分类的有效工具。

Conclusion: 大语言模型可作为数据高效的电力市场分析工具，在历史数据有限的情况下仍能准确预测电价尖峰，为数据稀缺环境下的电力市场预测提供了新途径。
...
### [116] [Real-time Secondary Crash Likelihood Prediction Excluding Post Primary Crash Features](https://arxiv.org/abs/2602.16739)
...
Method: 提出混合二次事故可能性预测框架：1)设计动态时空窗口从主事故位置及其上游路段提取实时交通流和环境特征；2)包含三个模型：主事故模型评估二次事故发生可能性，两个二次事故模型评估不同比较场景下事故路段和上游路段的交通状况；3)集成六种机器学习算法，采用投票机制结合三个模型的输出。

Result: 在佛罗里达高速公路上的实验表明：1)混合框架正确识别91%的二次事故，误报率仅为0.20；2)AUC值从单个模型的0.654、0.744和0.902提升到混合模型的0.952；3)性能优于先前研究。

Conclusion: 提出的混合框架有效解决了现有方法依赖事故后特征的局限性，通过实时特征提取和集成学习策略显著提升了二次事故可能性预测性能，为主动交通管理系统提供了实用的预测工具。
...
### [117] [Quantifying LLM Attention-Head Stability: Implications for Circuit Universality](https://arxiv.org/abs/2602.16740)
...
<details>
  <summary>Details</summary>
Motivation: 当前机制可解释性研究中的Transformer"电路"很少在不同架构实例间进行稳定性测试，这导致不清楚报告的电路是普遍存在的还是特定训练实例特有的，限制了在安全关键场景中的可信度。

Method: 系统研究不同复杂度和大小的Transformer语言模型在不同独立初始化训练运行间的稳定性，逐层量化注意力头学习表征的相似性，进行严格的实验分析。

Result: 发现：(1)中层注意力头最不稳定但表征最独特；(2)深层模型表现出更强的中层发散；(3)深层中不稳定头比同层其他头功能更重要；(4)权重衰减优化显著提升注意力头在不同随机初始化间的稳定性；(5)残差流相对稳定。

Conclusion: 跨实例电路稳健性是可扩展监督的重要但被低估的前提，为AI系统可能的白盒可监控性划定了轮廓，强调了稳定性测试对机制可解释性的重要性。
...
### [118] [DeepVision-103K: A Visually Diverse, Broad-Coverage, and Verifiable Mathematical Dataset for Multimodal Reasoning](https://arxiv.org/abs/2602.16742)
*Haoxiang Sun,Lizhen Xu,Bing Zhao,Wotao Yin,Wei Wang,Boyu Yang,Rui Wang,Hu Wei*

Main category: cs.LG

TL;DR: 论文提出了DeepVision-103K数据集，用于增强大型多模态模型的视觉反思和推理能力，解决了现有数据集规模小、多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有用于强化学习与可验证奖励（RLVR）的数据集主要来自小规模人工构建或现有资源的重组，这限制了数据的多样性和覆盖范围，从而制约了模型性能的进一步提升。

Method: 构建了DeepVision-103K数据集，该数据集覆盖了多样化的K12数学主题、广泛的知识点和丰富的视觉元素，用于RLVR训练。

Result: 在DeepVision上训练的模型在多模态数学基准测试中表现出色，并能有效泛化到一般的多模态推理任务中。分析显示训练后的模型在视觉感知、反思和推理能力方面都有所增强。

Conclusion: DeepVision-103K数据集有效提升了多模态模型的视觉反思和推理能力，验证了该数据集对于推进多模态推理研究的有效性。
...
### [119] [PETS: A Principled Framework Towards Optimal Trajectory Allocation for Efficient Test-Time Self-Consistency](https://arxiv.org/abs/2602.16745)
*Zhangyi Liu,Huaizhi Qu,Xiaowei Yin,He Sun,Yanjun Han,Tianlong Chen,Zhun Deng*

Main category: cs.LG

TL;DR: PETS提出了一种原则性的测试时自一致性方法，通过优化框架分配推理轨迹预算，在有限采样预算下实现样本高效的自一致性，相比均匀分配显著减少采样需求。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展通过聚合随机推理轨迹可以提升模型性能，但在有限预算下实现样本高效的测试时自一致性仍然是一个开放挑战。现有方法缺乏理论指导，无法在有限采样资源下优化自一致性性能。

Method: 提出PETS框架，引入自一致性率作为与无限预算多数投票一致性的新度量。研究离线和在线两种设置：离线场景将轨迹分配建模为众包问题，将推理轨迹视为工作者；在线流式场景提出自适应方法，根据问题难度动态调整预算。两种方法都提供理论保证和计算效率。

Result: 在GPQA数据集上，PETS在两种设置下都实现了完美的自一致性，同时相比均匀分配减少了75%（离线）和55%（在线）的采样预算。实验表明PETS始终优于均匀分配。

Conclusion: PETS为测试时自一致性提供了原则性的优化框架，通过理论指导的轨迹分配实现了样本高效的性能提升。该方法在有限预算下显著优于均匀分配，为实际部署中的资源受限场景提供了实用解决方案。
...
### [120] [Low-Dimensional and Transversely Curved Optimization Dynamics in Grokking](https://arxiv.org/abs/2602.16746)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: 该研究通过几何分析揭示了Transformer在模运算任务中"顿悟"现象的机制：训练轨迹主要限制在低维执行子空间内，横向曲率增长先于泛化发生，且沿子空间运动是顿悟的必要条件。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer在小规模算法任务中出现的"顿悟"现象——从记忆化到泛化的延迟转变机制。目前对这一现象的理解仍然有限，需要从优化动力学的几何角度进行深入分析。

Method: 1. 在模运算任务上训练Transformer模型；2. 对注意力权重轨迹进行主成分分析，识别低维执行子空间；3. 测量交换子缺陷（连续梯度步的非交换性）来探测损失景观的几何特性；4. 将曲率投影到学习到的子空间上；5. 进行因果干预实验，验证子空间运动和曲率增长的作用。

Result: 1. 训练轨迹主要限制在低维执行子空间内，单个主成分可解释68-83%的轨迹方差；2. 横向曲率增长先于泛化发生，且领先时间与顿悟时间尺度呈幂律关系；3. 沿学习子空间的运动是顿悟的必要条件，而人为增加曲率不足以引发顿悟；4. 正交梯度流是必要的但不充分；5. 结果在不同学习率范围、慢速训练机制和随机种子间具有可重复性。

Conclusion: 顿悟现象反映了从亚稳态机制中的逃逸过程，该机制特征包括低维限制和横向曲率积累。几何解释表明，训练轨迹被限制在低维子空间内，横向曲率增长为逃逸创造条件，最终实现从记忆化到泛化的转变。
...
### [121] [Fail-Closed Alignment for Large Language Models](https://arxiv.org/abs/2602.16977)
*Zachary Coalson,Beth Sohler,Aiden Gabriel,Sanghyun Hong*

Main category: cs.LG

TL;DR: 该论文发现当前LLM对齐存在结构弱点：拒绝机制是"故障开放"的，单个主导特征被抑制会导致安全对齐崩溃。作者提出"故障关闭"对齐原则，通过渐进对齐框架强制模型在独立子空间中重建安全机制，实现更强的抗攻击能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的对齐机制存在结构性弱点：现代拒绝机制是"故障开放"的。现有方法将拒绝行为编码在多个潜在特征中，但通过提示注入攻击抑制单个主导特征就能导致安全对齐崩溃，产生不安全生成。这促使研究者寻求更稳健的LLM安全设计原则。

Method: 提出"故障关闭"对齐作为稳健LLM安全的设计原则：拒绝机制即使在部分故障情况下也应保持有效，通过冗余、独立的因果路径实现。具体实现为渐进对齐框架：迭代识别并消融先前学习的拒绝方向，强制模型在新的独立子空间中重建安全机制。

Result: 在四种越狱攻击测试中，该方法实现了最强的整体鲁棒性，同时缓解了过度拒绝问题并保持了生成质量，计算开销较小。机制分析证实，使用该方法训练的模型编码了多个因果独立的拒绝方向，提示注入攻击无法同时抑制这些方向。

Conclusion: "故障关闭"对齐为稳健LLM安全提供了原则性基础。渐进对齐框架通过强制模型在独立子空间中重建安全机制，实现了多重冗余的拒绝路径，显著增强了模型对抗提示注入攻击的能力，同时保持了良好的生成性能。
...
### [122] [LiveClin: A Live Clinical Benchmark without Leakage](https://arxiv.org/abs/2602.16747)
*Xidong Wang,Shuqi Guo,Yue Shen,Junying Chen,Jian Wang,Jinjie Gu,Ping Zhang,Lei Liu,Benyou Wang*

Main category: cs.LG

TL;DR: LiveClin是一个动态更新的医学LLM实时基准测试，通过239名医生验证的工作流程，从最新病例报告构建了1407个病例和6605个问题，评估显示顶级模型仅达到35.7%的病例准确率，显著低于人类专家表现。


<details>
  <summary>Details</summary>
Motivation: 现有医学LLM评估存在数据污染和知识过时问题，导致静态基准测试分数虚高，无法反映真实临床实践能力，需要建立动态更新的临床基准来指导医学LLM发展。

Method: 从当代同行评审病例报告构建动态基准，每半年更新一次；通过239名医生参与的AI-人类验证工作流程，将真实患者病例转化为覆盖完整临床路径的复杂多模态评估场景。

Result: LiveClin包含1407个病例报告和6605个问题；评估26个模型显示顶级模型仅达到35.7%的病例准确率；人类专家表现优于大多数模型，主任医师准确率最高，主治医师紧随其后。

Conclusion: LiveClin提供了一个持续演化、临床基础的框架，能够更可靠地评估医学LLM的真实临床能力，指导模型发展以缩小与人类专家的差距，提高实际应用价值。
...
### [123] [Discovering Universal Activation Directions for PII Leakage in Language Models](https://arxiv.org/abs/2602.16980)
*Leo Marchyok,Zachary Coalson,Sungho Keum,Sooel Son,Sanghyun Hong*

Main category: cs.LG

TL;DR: UniLeak框架通过机制可解释性方法识别语言模型残差流中的通用激活方向，这些方向能一致增加生成PII的可能性，无需训练数据或真实PII，仅依赖自生成文本。
...
Result: 在多个模型和数据集上，沿着这些通用方向进行引导显著增加了PII泄露，相比现有的基于提示的提取方法效果更好。这些方向能泛化到不同上下文，对生成质量影响最小。

Conclusion: PII泄露是模型表示中潜在信号的叠加，这一新视角既支持风险放大也支持风险缓解。UniLeak为理解和控制PII泄露提供了新工具。
...
### [124] [Attending to Routers Aids Indoor Wireless Localization](https://arxiv.org/abs/2602.16762)
...
Method: 将注意力机制引入标准机器学习定位架构，提出"路由器注意力"概念，在三角定位聚合多路由器信息时差异化加权每个路由器的贡献。

Result: 在开源数据集上的评估表明，路由器注意力机制相比基准架构在定位准确率上提升超过30%。

Conclusion: 通过引入路由器注意力机制，差异化加权不同路由器的信息贡献，能够显著提升Wi-Fi定位系统的整体性能。
...
### [125] [Omitted Variable Bias in Language Models Under Distribution Shift](https://arxiv.org/abs/2602.16784)
*Victoria Lin,Louis-Philippe Morency,Eli Ben-Michael*

Main category: cs.LG

TL;DR: 该论文提出了一个框架，将语言模型中的分布偏移分为可观测和不可观测部分，通过建立遗漏变量与最坏情况泛化性能的边界关系，改进OOD评估和优化。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型虽然在多种任务上表现优异，但对分布偏移仍然脆弱，现有方法只处理可观测的分布偏移，而遗漏变量偏差会损害评估和优化效果。

Method: 将分布偏移分解为可观测和不可观测部分，建立遗漏变量强度与最坏情况泛化性能边界的映射框架，将该边界直接用于语言模型评估和优化。

Result: 实验表明，使用该边界框架能提供更原则性的OOD性能度量，相比标准分布偏移调整方法提升真实OOD性能，并在有目标分布标签时能推断遗漏变量强度。

Conclusion: 该框架通过处理不可观测分布偏移的遗漏变量偏差，为语言模型在分布偏移下的评估和优化提供了更可靠的方法，改善了OOD性能。
...
### [126] [Better Think Thrice: Learning to Reason Causally with Double Counterfactual Consistency](https://arxiv.org/abs/2602.16787)
*Victoria Lin,Xinnuo Xu,Rachel Lawrence,Risa Ueno,Amit Sharma,Javier Gonzalez,Niranjani Prasad*

Main category: cs.LG

TL;DR: 提出DCC方法评估和指导LLM的因果推理能力，无需标注数据，通过双重反事实一致性验证因果干预和反事实预测


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在推理基准上表现良好，但在面对反事实问题时表现脆弱，表明其因果推理能力存在缺陷。现有标注反事实数据的方法难以覆盖广阔的反事实空间，需要轻量级推理时方法来评估和指导LLM的因果推理能力。

Method: 提出双重反事实一致性(DCC)方法，这是一种无需标注反事实数据的轻量级推理时方法。DCC通过验证模型执行因果推理的两个关键要素（因果干预和反事实预测）的能力来评估LLM的因果推理能力。该方法还可作为训练无关的测试时拒绝采样准则。

Result: 使用DCC评估了多种领先LLM在各种推理任务和干预下的因果推理能力。实验表明DCC作为拒绝采样准则能有效提升多个模型家族在推理任务上的性能。

Conclusion: DCC是一种有效的轻量级方法，无需标注数据即可评估和指导LLM的因果推理能力，能够验证因果干预和反事实预测能力，并直接提升推理任务性能。
...
### [127] [Escaping the Cognitive Well: Efficient Competition Math with Off-the-Shelf Models](https://arxiv.org/abs/2602.16793)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用未发布的定制模型，要么使用公开模型但推理成本极高（如3000美元/问题），需要开发既高性能又低成本的解决方案

Method: 基于对"认知陷阱"（迭代优化收敛到错误解但内部评分器认为正确）的洞察，采用猜想提取方法：从生成解中分离候选引理，在新鲜环境中独立验证引理及其否定（上下文分离）

Result: 在IMO-ProofBench Advanced上达到67.1%性能，使用Gemini 3.0 Pro，平均成本约31美元/问题，当时是PB-Adv上的SOTA，比次优公开管道成功率翻倍以上

Conclusion: 通过解决认知陷阱问题，实现了高性能、低成本的数学推理管道，证明了使用通用现成模型也能达到顶级性能
...
### [128] [Efficient Tail-Aware Generative Optimization via Flow Model Fine-Tuning](https://arxiv.org/abs/2602.16796)
*Zifan Wang,Riccardo De Santi,Xiaoyu Mo,Michael M. Zavlanos,Andreas Krause,Karl H. Johansson*

Main category: cs.LG

TL;DR: TFFT是一种基于条件风险价值（CVaR）的分布微调算法，用于控制扩散和流模型输出的尾部行为，包括高奖励尾部的探索和低奖励尾部的可靠性控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵正则化的微调方法主要最大化期望奖励，缺乏控制尾部行为的机制。然而尾部控制在实际应用中至关重要：下尾部决定了可靠性（限制低奖励失败），上尾部则支持探索（发现罕见高奖励结果）。

Method: 提出Tail-aware Flow Fine-Tuning (TFFT)，基于CVaR的变分对偶公式，将问题分解为两阶段：轻量级的一维阈值优化步骤，以及通过特定伪奖励的单一熵正则化微调过程。

Result: TFFT在计算成本上与标准期望微调方法相当，在多个实验（包括高维文本到图像生成和分子设计）中展示了有效性。

Conclusion: TFFT提供了一种原则性且高效的分布微调算法，能够控制生成模型的尾部行为，平衡可靠性和探索性，适用于实际部署场景。
...
### [129] [HiVAE: Hierarchical Latent Variables for Scalable Theory of Mind](https://arxiv.org/abs/2602.16826)
*Nigel Doering,Rahath Malladi,Arshia Sangwan,David Danks,Tauhidur Rahman*

Main category: cs.LG

TL;DR: HiVAE：一种用于扩展心智理论推理到现实时空领域的分层变分架构，在校园导航任务中表现优异，但学习到的潜在表征缺乏对实际心理状态的显式基础。


<details>
  <summary>Details</summary>
Motivation: 现有心智理论方法主要关注小型人类可理解的网格世界空间，需要将其扩展到现实的时空领域以实现更实用的AI系统。

Method: 提出HiVAE（分层变分自编码器），受人类认知的信念-愿望-意图结构启发，采用三层VAE层次结构，将心智理论推理扩展到现实时空领域。

Result: 在3,185个节点的校园导航任务上取得了显著的性能提升，但发现学习到的潜在表征缺乏对实际心理状态的显式基础。

Conclusion: 虽然分层结构改进了预测性能，但表征基础问题仍需解决；提出了自监督对齐策略，并寻求社区对基础化方法的反馈。
...
### [130] [VAM: Verbalized Action Masking for Controllable Exploration in RL Post-Training -- A Chess Case Study](https://arxiv.org/abs/2602.16833)
*Zhicheng Zhang,Ziyan Wang,Yali Du,Fei Fang*

Main category: cs.LG

TL;DR: 该论文提出Verbalized Action Masking (VAM)方法，通过语言化动作掩码和迭代动作空间剪枝，解决大语言模型强化学习后训练中的探索瓶颈问题，在象棋任务中提升了学习效率和最终性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习后训练面临探索瓶颈，稀疏反馈和大动作空间容易导致模型陷入重复行为。需要一种可控的探索机制来改善学习效率。

Method: 提出Verbalized Action Masking (VAM)：1) 在提示中语言化动作掩码，强制模型从掩码集合中输出动作；2) 基于此接口引入迭代动作空间剪枝：如果未采样到目标动作，从掩码中移除已采样的有效动作，在减少的候选集下重新采样，重复直到采样到目标或达到固定预算。在象棋任务中评估两种训练机制：引擎对弈机制和固定数据集机制。

Result: 在象棋谜题和完整对弈中，VAM在平均百兵损失(ACPL)指标上相比强基线提升了学习效率和最终性能，验证了语言化掩码作为可控探索机制的有效性。

Conclusion: Verbalized Action Masking (VAM)是一种实用的可控探索机制，能有效解决大语言模型强化学习后训练中的探索瓶颈问题，在复杂任务中提升学习性能。
...
### [131] [A Residual-Aware Theory of Position Bias in Transformers](https://arxiv.org/abs/2602.16837)
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型系统性地偏好某些token位置，但这种位置偏置的架构起源尚不清楚。先前理论分析预测在无限深度因果掩码下注意力会崩溃到第一个token，但这与实际情况不符，需要解决这一理论-实践差异。

Method: 提出残差感知的累积注意力展开理论，将残差连接纳入分析框架。在无限深度下证明残差连接防止注意力崩溃；在有限深度下推导因果Transformer产生U形位置偏置的数学证明。

Result: 理论证明：1）在无限深度下，残差连接防止注意力完全崩溃到第一个token；2）在有限深度下，因果Transformer产生U形位置偏置，注意力集中在早期和晚期token上。这为"迷失在中间"现象提供了架构解释。

Conclusion: 残差连接是Transformer位置偏置的关键架构组件，它防止注意力崩溃并导致U形偏置模式。该理论为理解Transformer行为提供了新的分析框架，并解释了实际观察到的注意力分布特征。
...
### [132] [Training Large Reasoning Models Efficiently via Progressive Thought Encoding](https://arxiv.org/abs/2602.16839)
*Zeliang Zhang,Xiaodong Liu,Hao Cheng,Hao Sun,Chenliang Xu,Jianfeng Gao*

Main category: cs.LG

TL;DR: 提出渐进式思维编码方法，通过将中间推理过程编码为固定大小的向量表示，解决大型推理模型在强化学习训练中的内存效率问题，在有限缓存预算下显著提升数学推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在强化学习训练中面临效率瓶颈：基于结果的奖励需要长序列展开，自回归解码占用大量时间和内存。滑动窗口缓存策略虽然能限制内存，但会破坏长上下文推理并降低性能。需要一种方法使模型在固定大小缓存下仍能有效推理。

Method: 提出渐进式思维编码方法，通过参数高效的微调技术，将中间推理过程逐步编码为固定大小的向量表示。该方法无需在完整缓存展开中进行反向传播，显著减少内存使用，并在推理过程中保持恒定内存。

Result: 在三个模型（Qwen2.5-3B-Instruct、Qwen2.5-7B-Instruct、DeepSeek-R1-Distill-Llama-8B）和六个数学基准测试上的实验显示：相比LoRA微调平均提升19.3%，相比未微调的LRMs平均提升29.9%。在相同严格缓存预算下，AIME2024/2025上最高提升23.4%准确率。

Conclusion: 渐进式思维编码不仅提高了推理准确性，还使大型推理模型的强化学习训练在真实内存约束下更加高效和可扩展，为解决内存效率瓶颈提供了有效方案。
...
### [133] [What is the Value of Censored Data? An Exact Analysis for the Data-driven Newsvendor](https://arxiv.org/abs/2602.16842)
*Rachitesh Kumar,Omar Mouchtaki*

Main category: cs.LG

TL;DR: 研究离线数据驱动报童问题中的需求删失问题，提出计算经典库存策略最坏情况后悔值的通用方法，分析删失需求对学习能力的影响


<details>
  <summary>Details</summary>
Motivation: 传统报童问题研究通常假设需求完全可观测，但实际零售环境中需求常因库存不足而被删失，只能观测到销售数据而非真实需求。现有研究未充分考虑删失需求对数据驱动库存策略性能的影响，特别是当销售数据被误认为需求时的性能退化问题。

Method: 提出通用方法将无限维非凸优化问题简化为有限维问题，精确计算任何样本量和删失水平下经典库存策略的最坏情况后悔值。分析Kaplan-Meier策略性能，研究少量针对性探索对性能改善的作用，对比"销售即需求"启发式方法的性能退化。

Result: 需求删失从根本上限制了从被动销售数据中学习的能力，但少量高库存水平的针对性探索能显著改善最坏情况保证，即使在严重删失下也能实现接近最优性能。当POS系统不记录缺货事件时，"销售即需求"启发式方法会随删失数据积累而遭受严重性能退化。

Conclusion: POS系统信息质量对离线学习能力具有关键影响。需求删失限制了被动数据的学习能力，但通过少量探索可显著改善性能。误将销售数据视为需求会导致严重性能退化，凸显了准确记录库存状态信息的重要性。
...
TL;DR: 开发用于巴西中老年人的痴呆症分类模型，结合变量选择和多元分析，使用低成本可干预变量，基于ELSI-Brazil数据构建随机森林和逻辑回归模型识别痴呆风险因素


<details>
  <summary>Details</summary>
Motivation: 建立针对巴西中老年人群的痴呆症分类模型，重点关注低成本、可干预的风险因素，为公共卫生政策提供依据，促进资源有效分配和痴呆预防

Method: 采用横断面设计的预测建模方法，使用巴西老龄化纵向研究（ELSI-Brazil）的9,412名参与者数据。痴呆诊断基于神经心理学评估和知情者认知功能报告。分析方法包括随机森林（RF）和多元逻辑回归，用于估计痴呆风险

Result: 痴呆患病率为9.6%。最高风险因素包括：文盲（OR=7.42）、90岁及以上（OR=11.00）、低体重（OR=2.11）、低握力（OR=2.50）、自报黑人肤色（OR=1.47）、缺乏运动（OR=1.61）、听力损失（OR=1.65）和抑郁症状（OR=1.72）。保护因素包括：高等教育（OR=0.44）、较高生活满意度（OR=0.72）和就业（OR=0.78）。RF模型优于逻辑回归，ROC曲线下面积为0.776，敏感性0.708，特异性0.702，F1分数0.311，G-means 0.705，准确率0.703

Conclusion: 研究结果强调了痴呆的多维性质以及可及性因素在识别脆弱个体中的重要性。加强关注促进脑健康的公共政策，有助于巴西初级保健资源的有效分配和痴呆预防
...
TL;DR: 提出了一种去中心化多玩家随机臂机算法，在连续Lipschitz动作空间中处理硬碰撞问题，实现与时间无关的协调成本


<details>
  <summary>Details</summary>
Motivation: 解决去中心化多玩家随机臂机问题，其中硬碰撞导致零奖励，目标是设计无需通信的策略，最大化集体奖励，且协调成本与时间无关

Method: 提出模块化协议：首先通过新颖的最大值导向搜索解决多智能体协调问题，让玩家分配到不同的高价值区域；然后将问题解耦为N个独立的单玩家Lipschitz臂机问题

Result: 建立了近乎最优的遗憾界 $\tilde{O}(T^{(d+1)/(d+2)})$ 加上与T无关的协调成本，匹配单玩家速率，这是首个提供此类保证的框架

Conclusion: 该框架首次在去中心化多玩家随机臂机中实现了与时间无关的协调成本和近乎最优的遗憾界，并可扩展到一般距离阈值碰撞模型
...
TL;DR: 该论文提出了一种新的多智能体强化学习框架，通过策略依赖的指数衰减特性分析，揭示了局部性可以是策略依赖现象，并建立了比现有方法更紧的谱条件。
...
Method: 提出策略诱导的相互依赖矩阵H^π的分解方法，将环境对状态(E^s)和动作(E^a)的敏感性与策略对状态(Π(π))的敏感性解耦，建立了谱条件ρ(E^s+E^aΠ(π))<1用于指数衰减分析。

Result: 证明了局部性可以是策略依赖现象，即使环境存在强动作耦合，平滑策略也能诱导局部性；提出的谱条件比先前的范数基条件更严格；开发了具有理论保证的局部块坐标策略改进框架。

Conclusion: 该工作揭示了多智能体强化学习中策略依赖的局部性现象，建立了更精确的指数衰减特性分析框架，为可扩展的多智能体学习提供了理论基础和实用算法。
...
TL;DR: 研究发现交换子缺陷（非交换梯度更新的曲率度量）是Transformer中延迟泛化的早期预警信号，在三个任务家族（模运算、SCAN、Dyck-1）中具有普遍性。


<details>
  <summary>Details</summary>
Motivation: 探索Grokking现象（从记忆到泛化的突然转变）是否仅限于模算术任务，以及是否存在跨任务的通用早期预警信号。

Method: 研究两个序列学习基准：SCAN组合泛化和Dyck-1深度预测。使用交换子缺陷（非交换梯度更新的曲率度量）作为指标，通过权重空间PCA分析谱集中性，并进行因果干预实验（放大非交换性和抑制正交梯度流）。

Result: 交换子缺陷在泛化前显著上升，领先时间遵循超线性幂律（SCAN α≈1.18，Dyck α≈1.13）。谱集中性不是通用前兆，但交换子缺陷是。因果干预显示：放大非交换性加速Grokking（SCAN约32%，Dyck约50%），抑制正交梯度流延迟或阻止Grokking。三个任务家族形成因果敏感性谱系。

Conclusion: 交换子缺陷是Transformer中延迟泛化的稳健、架构无关、因果相关的早期预警信号，在多个任务家族中具有普遍必要性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管已有多种针对独立同分布rollouts的验证算法，但它们在匹配设置下的相对性能尚不明确。本文旨在系统评估不同验证策略的性能差异，并探索优化方法。

Method: 1) 系统评估不同验证策略在不同模型、任务和采样机制下的性能；2) 提出延迟树扩展方法，推迟独立同分布分支点；3) 开发动态神经选择器，根据草稿和目标特征估计最优传输方法的预期块效率。

Result: 遍历验证在各项评估中表现一致优于最优传输方法。延迟树扩展能保持目标分布并改进根节点独立同分布rollouts。神经选择器使OT方法首次超越遍历验证，在广泛模型、数据集和采样设置下实现平均5%的吞吐量提升。

Conclusion: 通过系统分析验证策略性能差异的根本原因，并基于此提出延迟树扩展和神经选择器，成功优化了最优传输方法的性能，使其在多路径推测解码中首次超越遍历验证，实现了显著的吞吐量提升。
...
TL;DR: AGP是一种通过建模智能体间动作依赖关系来提升多智能体协调能力的策略框架，能实现比独立策略更优的协同决策。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，成功的去中心化决策不仅需要良好的个体动作，更需要智能体间选择兼容的动作以实现行为同步、避免冲突和满足全局约束。现有方法在需要协调的任务上表现不佳。

Method: 提出动作图策略（AGP），通过构建协调上下文来建模智能体可用动作选择之间的依赖关系，使智能体能够基于全局动作依赖来条件化其决策。

Result: 理论上证明AGP比完全独立策略具有更严格的表达能力，能实现比贪婪执行更优的协调联合动作。实证中，在部分可观测性和反协调惩罚的典型协调任务上达到80-95%成功率，而其他MARL方法仅10-25%。

Conclusion: AGP通过显式建模动作依赖关系，有效解决了多智能体协调问题，在多种环境中一致优于现有基线方法，为需要复杂协调的MARL任务提供了有效解决方案。
...
TL;DR: 该论文建立了路径重参数化和Malliavin梯度估计器之间的严格联系，提出了一种基于经验协方差结构的自适应混合估计器，在VAE和合成问题上实现了显著的方差降低。


<details>
  <summary>Details</summary>
Motivation: 论文的动机在于建立路径重参数化和Malliavin梯度估计器之间的理论联系，解决现有随机梯度估计方法缺乏统一理论框架的问题，并开发能够自适应结合两种方法优势的混合估计器以降低方差。

Method: 基于Malliavin积分恒等式建立两种梯度估计器的等价性，引入基于经验协方差结构的自适应混合估计器，该估计器在所有无偏线性组合中实现最小方差，并提供闭式有限样本收敛界。

Result: 在CIFAR-10的VAE上实现9%的方差降低，在强耦合合成问题上实现高达35%的方差降低。探索性策略梯度实验表明非平稳优化环境对混合方法构成挑战。

Conclusion: Malliavin微积分为随机梯度估计提供了概念统一且实践可解释的框架，明确了混合方法何时提供实际效益以及何时面临固有局限，为未来研究指明了方向。
...
TL;DR: WS-GRPO通过将终端奖励转化为部分轨迹的正确性感知指导，改善GRPO的推理效率，减少冗余思考同时保持准确性


<details>
  <summary>Details</summary>
Motivation: GRPO在复杂推理训练中有效，但基于相对轨迹组的优化目标会导致过度思考和推理效率低下。长度惩罚难以校准，且缺乏何时继续/停止的监督信号，使得在正确性和推理效率间难以权衡。

Method: 提出弱监督GRPO（WS-GRPO），将终端奖励转化为部分轨迹的正确性感知指导。训练一个基于结果正确性的偏好模型，生成前缀级信号来指示额外延续是否有益，从而提供结果驱动的继续/停止指导。

Result: 理论分析和在推理基准上的实验表明，WS-GRPO显著减少了推理轨迹长度，同时保持与GRPO基线相当的准确性。

Conclusion: WS-GRPO通过结果驱动的继续/停止指导，有效解决了GRPO中的过度思考问题，在保持推理准确性的同时大幅提升了推理效率。
...
TL;DR: 论文提出了一种AI增强的科学发现流水线，利用上下文学习范式帮助行为神经科学家自动处理数据准备和模式解释，应用于小鼠恐惧泛化研究，相比传统方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统科学发现流程复杂、僵化且耗时，需要领域专家花费大量时间调试流水线或手动标注数据。AI技术有潜力改变这一现状，让专家能专注于结果解释和理解。特别是在行为神经科学领域，研究小鼠恐惧泛化对理解PTSD等临床疾病至关重要。

Method: 采用"上下文学习"范式作为领域专家与AI交互的接口，无需模型训练和微调专业知识。提出对张量分解模型的AI增强，以更好地从异构数据中发现模式。构建完整的AI增强流水线，包括数据准备和模式解释自动化。

Result: 实验评估显示，提出的流水线性能优于领域标准实践和非ICL范式的合理ML基线。在保持无缝易用界面的同时未牺牲性能。领域专家验证了发现结果的有效性。

Conclusion: 上下文学习范式为领域专家提供了有效的AI接口，能够显著加速科学发现流程。AI增强的张量分解模型能更好地处理异构数据。该研究展示了AI如何变革科学发现流水线，让专家专注于核心洞察而非技术细节。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于哈希的索引方法在唯一ID数量增加时会产生碰撞，降低模型性能和个人化质量，需要解决嵌入碰撞问题

Method: 基于线性探测的多探针零碰撞哈希机制，使用辅助张量和高性能CUDA内核实现可配置的探测和主动驱逐策略，通过淘汰过时ID和重置重新分配的槽位来防止陈旧嵌入继承

Result: MPZCH实现了用户嵌入的零碰撞，显著提高了物品嵌入的新鲜度和质量，同时保持了与现有方法相当的训练QPS和推理延迟

Conclusion: MPZCH有效解决了推荐系统嵌入表的碰撞问题，在保持生产规模效率的同时显著提升了嵌入质量，已开源集成到TorchRec库中
...
TL;DR: 提出了NAMO和NAMO-D两种新优化器，首次将正交化动量与基于范数的Adam型噪声适应原则性结合，在GPT-2预训练中优于AdamW和Muon基线


<details>
  <summary>Details</summary>
Motivation: 现有优化器如Adam使用自适应矩估计来适应随机扰动，而Muon利用权重层的矩阵结构通过正交化动量在大型语言模型训练中表现优异。需要将正交化动量与基于范数的噪声适应机制进行原则性集成，以同时获得两者的优势。

Method: 提出了NAMO和NAMO-D两种优化器：NAMO使用单一自适应步长缩放正交化动量，保持正交性同时以可忽略的额外成本改进Muon；NAMO-D将正交化动量右乘一个具有钳位条目的对角矩阵，实现神经元级噪声适应并符合常见的近块对角Hessian结构。

Result: 在标准假设下，两种算法在确定性设置中建立了最优收敛速率；在随机设置中，其收敛保证能够适应随机梯度的噪声水平。GPT-2预训练实验表明，NAMO和NAMO-D相比AdamW和Muon基线都有改进，NAMO-D通过额外的钳位超参数在保持良好条件更新方向与利用细粒度噪声适应之间取得平衡，获得进一步增益。

Conclusion: NAMO和NAMO-D成功将正交化动量与基于范数的Adam型噪声适应原则性结合，在理论和实验上都表现出优越性能，为大型语言模型训练提供了更有效的优化方法。
...
TL;DR: MeGU提出了一种基于概念感知重对齐的机器引导遗忘框架，通过多模态大语言模型确定语义扰动方向，使用正负特征噪声对显式解耦目标概念影响，实现选择性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法存在根本性权衡：激进遗忘会损害模型在保留数据上的效用，而保守策略则残留目标信息。研究发现预训练模型中的语义类概念在特征层面存在纠缠，共享关联特征但保留概念特异性判别成分，这种纠缠限制了现有遗忘范式的有效性。

Method: 提出MeGU框架：1）利用多模态大语言模型为目标准样本分配语义上有意义的扰动标签，明确确定重对齐方向；2）将MLLM估计的类间概念相似性编码到轻量级转移矩阵中提高效率；3）引入正负特征噪声对，负噪声抑制目标特异性特征模式，正噪声增强剩余关联特征并与扰动概念对齐；4）通过协调设计实现选择性破坏目标特异性表示同时保留共享语义结构。

Result: MeGU实现了受控和选择性遗忘，有效缓解了欠遗忘和过遗忘问题，在保持模型对保留数据效用的同时，有效消除了目标数据的影响。

Conclusion: 通过概念感知重对齐和显式特征解耦，MeGU框架解决了现有机器遗忘方法在概念纠缠问题上的局限性，为"被遗忘权"要求提供了更有效的技术方案。
...
TL;DR: 该论文提出使用潜在空间中的流匹配方法替代扩散模型来学习随机闭合模型，实现了单步采样，速度比迭代扩散方法快两个数量级，同时通过正则化控制潜在空间失真以确保物理保真度。
...
Method: 在2D Kolmogorov流的数值示例上系统比较基于传输的生成模型，采用低维潜在空间中的流匹配方法实现单步快速采样。为控制潜在空间失真和确保物理保真度，比较了联合训练方案的隐式正则化与两种显式正则化器：度量保持(MP)和几何感知(GA)约束。
...
<details>
  <summary>Details</summary>
Motivation: LoRA在联邦学习中存在两个挑战：1) 分别聚合两个低秩矩阵引入误差；2) 聚合矩阵乘积后服务器需通过矩阵分解恢复因子，分解不唯一且会引入分解漂移。需要解决这些挑战以提高联邦微调效果
...
Result: 实验结果表明：1) 在多个LLM微调基准测试中，FLoRG在目标任务精度上优于五种最先进的基线方案；2) 通信开销最多可减少2041倍

Conclusion: FLoRG通过使用单低秩矩阵和Gram矩阵聚合，结合Procrustes对齐方法，有效解决了LoRA在联邦学习中的聚合误差和分解漂移问题，显著提高了微调精度并大幅降低了通信开销
...
TL;DR: 论文提出一个基于无服务器架构的MLOps框架，用于协调从数据摄取到再训练的完整机器学习生命周期，并以HS代码预测为工业案例进行验证，实现了98%的准确率、可重现性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 解决合规关键任务中HS代码预测的挑战：短文本、非结构化产品描述、频繁更新和模糊描述导致分类困难，错误会导致货运延误和经济损失。需要可扩展、可审计且成本效益高的ML运维解决方案。

Method: 采用无服务器MLOps框架，使用事件驱动管道和托管服务。模型无关架构通过标准化接口支持多种推理模式。使用自定义文本嵌入编码器和多种深度学习架构（包括Text-CNN），实现自动化A/B测试、动态模型选择和自动扩展。

Result: Text-CNN在真实数据上达到98%的准确率。框架确保可重现性、可审计性和SLA遵守，在可变负载下通过自动扩展保持性能。成本效益驱动模型选择，优先考虑确定性分类、可预测延迟和可解释性。

Conclusion: 该工作提供了一个可复制的蓝图，用于使用无服务器架构实现ML操作化，使企业能够在优化性能和经济性的同时进行扩展。架构保持可扩展性，可扩展到transformer变体和基于LLM的推理。
...
TL;DR: i-PhysGaussian：结合3D高斯泼溅与隐式物质点法的物理模拟框架，通过隐式牛顿优化解决显式方法的时间步长敏感性问题，实现更稳定的复杂场景模拟


<details>
  <summary>Details</summary>
Motivation: 当前基于3D重建的模拟器依赖显式逐步更新方法，对时间步长敏感，在高刚度材料或准静态运动等复杂场景下精度快速下降，需要更稳定、物理一致的模拟框架

Method: 将3D高斯泼溅（3DGS）与隐式物质点法（MPM）积分器耦合，通过最小化动量平衡残差，采用带GMRES求解器的隐式牛顿型优化获得步末状态

Result: i-PhysGaussian在比显式基线大20倍的时间步长下仍保持稳定，在复杂动态转换中保持结构一致性和平滑运动

Conclusion: 提出的隐式框架显著减少时间步长敏感性，确保物理一致性，为工业和工程中的风险管理和蓝图设计提供更可靠的物理模拟工具
...
TL;DR: 提出TIFO方法，通过频域加权抑制非平稳分量，缓解时间序列预测中的分布偏移问题


<details>
  <summary>Details</summary>
Motivation: 传统方法处理非平稳时间序列预测时，仅去除单个样本的低阶矩，未能捕捉样本间的时间演化结构和复杂时间模式，导致分布偏移问题

Method: 提出时间不变频率算子(TIFO)，在频域学习对全数据集的平稳感知权重，突出平稳频率分量并抑制非平稳分量，可即插即用到各种预测模型中

Result: 在28个预测设置中获得18个第一和6个第二结果，在ETTm2数据集上平均MSE提升33.3%和55.3%，计算成本降低60%-70%

Conclusion: TIFO通过频域加权有效缓解分布偏移，具有强可扩展性和计算效率，为时间序列预测提供了新的频域视角
...
<details>
  <summary>Details</summary>
Motivation: VQ-VAE在训练中存在不稳定性和"码本崩溃"问题，这源于表示学习和离散码本优化的固有耦合。需要一种能解耦这两个过程的方法来改善训练稳定性。

Method: 提出VP-VAE，用分布一致且尺度自适应的潜在扰动（通过Metropolis-Hastings采样生成）替代不可微量化器，从而在训练期间无需显式码本。在近似均匀潜在变量假设下，进一步推导出轻量级变体FSP，为FSQ式固定量化器提供统一理论解释。

Result: 在图像和音频基准测试中，VP-VAE和FSP提高了重建保真度，实现了更平衡的token使用，同时避免了耦合码本训练固有的不稳定性。

Conclusion: VP-VAE通过解耦表示学习和离散化，用结构化潜在扰动替代量化操作，为VQ-VAE提供了更稳定、更平衡的训练范式，其理论框架还能解释和改进现有固定量化器方法。
...
TL;DR: TimeOmni-VL是首个基于视觉的统一框架，通过双向映射和理解引导生成，同时提升时间序列的语义理解和数值生成精度。
...
Method: 提出TimeOmni-VL框架，包含两个关键创新：1) 保真度双向映射(Bi-TSI)：实现时间序列与图像间的近无损转换；2) 理解引导生成：利用校准的思维链，将时间序列理解作为显式控制信号指导生成。同时引入TSUMM-Suite数据集，包含六个理解任务和两个生成任务。
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化作为免梯度方法，在微调大规模模型时具有内存效率优势，但存在准确性与查询效率之间的根本矛盾。现有方法需要大量函数评估来估计梯度，限制了实际应用。

Method: 提出统一框架：1）投影子空间视角，利用模型更新的内在低秩结构降低梯度估计方差；2）Muon风格谱优化，通过梯度正交化从噪声ZO梯度中提取信息谱结构。实例化为ZO-Muon方法，可解释为零阶设置下的低秩Muon优化器。

Result: 在大语言模型和视觉Transformer上的实验表明：1）相比MeZO基线，ZO-Muon仅需24.7%的查询即可达到相同SST-2性能；2）在CIFAR-100上ViT-B微调准确率提升25.1%；3）显著加速收敛，实现准确性与查询/运行时效率的双赢提升。

Conclusion: 通过统一子空间投影和梯度正交化，ZO-Muon有效解决了零阶优化中准确性与效率的矛盾，为大规模模型微调提供了更高效的免梯度优化方案。
...
Result: 结果展示了线性注意力相对于二次注意力在这一设置下的相似性和局限性
...
### [155] [Continual uncertainty learning](https://arxiv.org/abs/2602.17174)
...
<details>
  <summary>Details</summary>
Motivation: 机械系统多不确定性鲁棒控制面临挑战，特别是非线性动力学与工况变化交织时。现有DRL结合域随机化方法在处理所有不确定性源时往往导致次优策略和学习效率低下

Method: 提出课程持续学习框架：1)将多不确定性复杂控制问题分解为序列化持续学习任务；2)将原系统扩展为有限植物集，动态不确定性随学习进展逐渐扩展和多样化；3)结合模型基控制器保证共享基线性能，采用残差学习方案加速收敛；4)策略在任务定义的整个植物集上稳定更新，避免灾难性遗忘

Result: 应用于汽车动力总成主动振动控制器设计，验证了控制器对结构非线性和动态变化的鲁棒性，实现了成功的仿真到现实迁移

Conclusion: 提出的课程持续学习框架能有效处理多不确定性鲁棒控制问题，通过分解复杂任务、序列化学习和模型基控制器集成，提高了学习效率和策略鲁棒性，成功应用于实际工业控制问题
...
### [156] [SoftDTW-CUDA-Torch: Memory-Efficient GPU-Accelerated Soft Dynamic Time Warping for PyTorch](https://arxiv.org/abs/2602.17206)
*Ron Shapira Weber,Oren Freifeld*

Main category: cs.LG

TL;DR: softdtw-cuda-torch是一个开源的PyTorch库，用于在GPU上计算Soft Dynamic Time Warping，解决了现有GPU实现的三个关键限制：序列长度限制、数值不稳定性和内存消耗问题。


<details>
  <summary>Details</summary>
Motivation: 现有GPU实现的SoftDTW存在三个主要问题：1) 硬性序列长度限制（最多1024）；2) 小平滑参数下反向传播的数值不稳定；3) 由于需要存储成对距离张量而导致GPU内存消耗过大。这些限制阻碍了SoftDTW在大规模时间序列分析中的应用。

Method: 提出了三种关键技术：1) 分块反对角线核执行，消除序列长度限制；2) 对数空间反向传播，防止浮点溢出；3) 融合距离计算模式，消除O(BNM)中间距离张量，减少内存消耗。库支持任意序列长度、完整的PyTorch自动微分和Soft-DTW重心计算。

Result: 实现了支持任意序列长度的SoftDTW GPU计算，解决了数值稳定性问题，相比先前工作内存减少高达98%。库提供了完整的PyTorch集成，支持自动微分和Soft-DTW重心计算。

Conclusion: softdtw-cuda-torch是一个高效、稳定的SoftDTW GPU实现，解决了现有实现的三个关键限制，使SoftDTW能够应用于更广泛的时间序列分析任务，特别是长序列和大规模数据集。
...
### [157] [Structured Prototype-Guided Adaptation for EEG Foundation Models](https://arxiv.org/abs/2602.17251)
*Jingying Ma,Feng Wu,Yucheng Xing,Qika Lin,Tianyu Liu,Chenyu Liu,Ziyu Jia,Mengling Feng*

Main category: cs.LG

TL;DR: SCOPE框架通过结构化置信感知原型引导的适配方法，解决EEG基础模型在有限监督下的泛化问题，在跨被试标签受限场景中实现高效性能提升。


<details>
  <summary>Details</summary>
Motivation: EEG基础模型在全微调下表现良好，但在真实临床场景中面临被试级监督有限的问题，导致泛化性能差。研究发现这不仅是监督不足的问题，更是噪声有限监督与模型高度可塑参数空间之间的结构不匹配。

Method: 提出SCOPE框架，采用两阶段流程：第一阶段通过几何正则化任务先验学习构建可靠外部监督，在嵌入空间构建平衡的类级原型，基于原型一致性生成置信感知伪标签以过滤不可靠信号；第二阶段引入ProAdapter，通过轻量级适配器在结构化原型条件下适配冻结的EEG基础模型。

Result: 在三个EEG任务和五个基础模型骨干上的实验表明，SCOPE在标签受限的跨被试设置下始终实现强大的性能和效率。

Conclusion: SCOPE通过结构化原型引导的适配方法有效解决了EEG基础模型在有限监督下的泛化挑战，为真实临床场景中的EEG分析提供了实用解决方案。
...
### [158] [Learning a Latent Pulse Shape Interface for Photoinjector Laser Systems](https://arxiv.org/abs/2602.17263)
...
<details>
  <summary>Details</summary>
Motivation: 在自由电子激光器的光注入器中，控制纵向激光脉冲形状是优化电子束质量的重要手段，但庞大的设计空间使得系统探索受到脉冲传播模拟高成本的限制。

Method: 采用基于Wasserstein自编码器的生成建模框架，学习脉冲整形与下游束流动力学之间的可微分潜在接口，通过主成分分析和高斯混合模型分析潜在空间几何结构。

Result: 学习的潜在空间具有连续性和可解释性，能保持高保真重建；脉冲家族如高阶高斯脉冲形成连贯轨迹；模型能从模拟数据泛化到真实实验脉冲测量，准确重建脉冲并将其嵌入学习到的流形中。

Conclusion: 该方法减少了对昂贵脉冲传播模拟的依赖，促进了下游束流动力学的模拟和分析，为激光脉冲形状优化提供了高效工具。
...
### [159] [Unified Latents (UL): How to train your latents](https://arxiv.org/abs/2602.17270)
...
Result: ImageNet-512上FID 1.4，高重建质量(PSNR)，训练FLOPs少于基于Stable Diffusion潜在空间的模型；Kinetics-600上FVD 1.3创SOTA
...
### [160] [RLGT: A reinforcement learning framework for extremal graph theory](https://arxiv.org/abs/2602.17276)
*Ivan Damnjanović,Uroš Milivojević,Irena Đorđević,Dragan Stevanović*

Main category: cs.LG

TL;DR: 提出RLGT框架，将强化学习系统化应用于极值图论问题，支持有向/无向图、环、多色边，优化计算性能


<details>
  <summary>Details</summary>
Motivation: Wagner的开创性工作展示了深度交叉熵强化学习方法在解决极值图论问题中的应用潜力，许多研究者随后对该框架进行改进和扩展，创建了专门的图论RL环境，并解决了多个极值图论问题。然而，现有工作缺乏系统化框架，需要统一的工具来支持更广泛的图类型和问题

Method: 提出RLGT（Reinforcement Learning for Graph Theory）框架，系统化先前工作，支持无向图和有向图（带或不带环），支持任意数量的边颜色。该框架高效表示图结构，通过优化的计算性能和清晰模块化设计，旨在促进基于RL的极值图论研究

Result: 框架提供了系统化的RL工具集，支持广泛的图类型和配置，优化了计算性能，为未来基于强化学习的极值图论研究提供了统一平台

Conclusion: RLGT框架系统化了强化学习在极值图论中的应用，提供了灵活、高效的工具平台，有望促进该领域未来的研究进展，特别是在解决更复杂的图论问题方面
...
### [161] [Efficient privacy loss accounting for subsampling and random allocation](https://arxiv.org/abs/2602.17284)
*Vitaly Feldman,Moshe Shenfeld*

Main category: cs.LG

TL;DR: 本文提出了一种高效计算随机分配采样隐私损失分布的方法，证明其在隐私-效用权衡上至少与泊松采样相当，尤其更适合DP-SGD训练。


<details>
  <summary>Details</summary>
Motivation: 随机分配采样在差分隐私优化和高维私有聚合中显示出优于泊松采样的效用优势，但现有理论分析存在两个主要缺陷：1) 实际场景中隐私参数不够紧致；2) 使用曲棍球棒散度或Renyi散度会引入额外开销。

Method: 开发基于隐私损失分布实现概念的新工具，提出高效计算随机分配采样隐私损失分布的方法，并将其应用于高斯机制，扩展了准确的隐私损失核算到子采样场景。

Result: 证明了随机分配采样的隐私-效用权衡至少与泊松子采样相当，特别在DP-SGD训练中表现更优，实现了无需手动机制特定分析的准确隐私损失核算。

Conclusion: 随机分配采样在隐私保护性能上不劣于泊松采样，且更适合实际应用如DP-SGD，通过新的PLD实现工具实现了更准确、通用的隐私损失核算。
...
### [162] [SubQuad: Near-Quadratic-Free Structure Inference with Distribution-Balanced Objectives in Adaptive Receptor framework](https://arxiv.org/abs/2602.17330)
*Rong Fu,Zijian Zhang,Wenxin Zhang,Kun Liu,Jiekai Wu,Xianda Li,Simon Fong*

Main category: cs.LG

TL;DR: SubQuad是一个用于大规模适应性免疫库比较分析的端到端流水线，通过近次二次检索、GPU加速亲和核、学习多模态融合和公平约束聚类来解决计算成本和数据集不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 大规模群体适应性免疫库比较分析面临两个主要瓶颈：1) 成对亲和力评估的近二次方计算成本；2) 数据集不平衡掩盖了临床重要的少数克隆型。这些限制阻碍了疫苗靶点优先排序和生物标志物发现等转化任务。
...
Result: 在大型病毒和肿瘤免疫库上，SubQuad在吞吐量和峰值内存使用方面实现了可测量的提升，同时保持或改进了recall@k、聚类纯度和亚组公平性。系统通过协同设计索引、相似性融合和公平感知目标，为免疫库挖掘提供了可扩展、偏差感知的平台。

Conclusion: SubQuad通过协同设计索引、相似性融合和公平感知目标，提供了一个可扩展、偏差感知的平台，用于免疫库挖掘和下游转化任务，如疫苗靶点优先排序和生物标志物发现。
...
Method: 1) 多裁剪对齐：每次迭代从多个独立采样的局部视图平均梯度以降低方差；2) 辅助目标对齐：用语义相关分布的小型辅助集替代激进的目标增强，产生更平滑的目标流形；3) 补丁动量：重放历史裁剪梯度；4) 改进的补丁大小集成

Result: 在多个前沿LVLM上显著提升攻击成功率：Claude-4.0从8%提升至30%，Gemini-2.5-Pro从83%提升至97%，GPT-5从98%提升至100%，优于现有黑盒LVLM攻击方法

Conclusion: M-Attack-V2通过梯度去噪和结构改进有效解决了黑盒对抗攻击中的梯度方差问题，为LVLM安全性评估提供了更强大的工具
...
### [164] [From Subtle to Significant: Prompt-Driven Self-Improving Optimization in Test-Time Graph OOD Detection](https://arxiv.org/abs/2602.17342)
*Luzhi Wang,Xuanshuo Fu,He Zhang,Chuang Liu,Xiaobao Wang,Hongbo Liu*

Main category: cs.LG

TL;DR: SIGOOD是一种自改进的图OOD检测框架，通过提示增强图和能量偏好优化实现无监督的测试时训练，在21个真实数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图OOD检测方法多采用一次性推理范式，无法渐进修正错误预测以放大OOD信号，需要一种能够持续自我改进的检测框架。

Method: 提出SIGOOD框架：1）生成提示构建提示增强图以放大OOD信号；2）引入能量偏好优化损失，利用原始图与提示增强图的能量差异；3）通过自改进循环迭代优化提示，最终使用最优提示增强图进行OOD检测。
...
### [165] [A feature-stable and explainable machine learning framework for trustworthy decision-making under incomplete clinical data](https://arxiv.org/abs/2602.17364)
*Justyna Andrys-Olek,Paulina Tworek,Luca Gherardini,Mark W. Ruddock,Mary Jo Kurt,Peter Fitzgerald,Jose Sousa*

Main category: cs.LG

TL;DR: CACTUS是一个可解释的机器学习框架，专门设计用于处理小型、异构和不完整的临床数据集，通过特征抽象、可解释分类和系统特征稳定性分析来提高模型的可信度。
...
Method: CACTUS框架集成了特征抽象、可解释分类和系统特征稳定性分析，量化信息特征在数据质量下降时的保持一致性。在包含568名膀胱癌患者的血尿队列中，与随机森林和梯度提升等方法进行基准比较，在控制随机缺失数据水平下评估。

Result: CACTUS在预测性能上达到竞争性或优越水平，同时在缺失数据增加时保持显著更高的顶级特征稳定性，包括在性别分层分析中。特征稳定性提供了与传统性能指标互补的信息。

Conclusion: 通过显式量化对缺失数据的鲁棒性并优先考虑可解释、稳定的特征，CACTUS为可信的数据驱动决策支持提供了一个可推广的框架，特征稳定性对于评估生物医学机器学习模型的可信度至关重要。
...
TL;DR: 该论文通过简化Mamba-2并改进其A-mask和隐藏状态阶数，开发出2Mamba方法，使线性注意力在保持长上下文内存效率的同时，准确率接近softmax注意力


<details>
  <summary>Details</summary>
Motivation: 线性注意力变换器因其效率成为softmax注意力的替代方案，但存在表达能力不足和准确率降低的问题。需要缩小softmax注意力与线性注意力之间的准确率差距

Method: 首先简化Mamba-2到最基本组件，评估哪些选择使其最准确。从简化的Mamba变体(Mamba-2S)出发，改进A-mask并增加隐藏状态阶数，开发出2Mamba方法

Result: 2Mamba在保持长上下文内存效率的同时，准确率几乎与softmax注意力相当。还研究了使Mamba-2超越softmax注意力准确率的元素

Conclusion: 通过系统性地简化、分析和改进Mamba-2，开发出2Mamba方法，成功缩小了线性注意力与softmax注意力之间的准确率差距，同时保持了线性注意力在长上下文中的内存效率优势
...
<details>
  <summary>Details</summary>
Motivation: 现有张量基PEFT方法存在局限性：LoTR使用共享因子矩阵的Tucker分解，SuperLoRA对跨层梯度更新进行分组和重塑后分解，而PiSSA等方法独立处理每层。需要一种能结合两者优势、直接对预训练权重进行跨层分解的方法。

Method: CRAFT将预训练注意力权重矩阵组织为跨层3D张量，通过高阶奇异值分解（HOSVD）进行完整Tucker分解，冻结所有分解因子，仅训练轻量级适应矩阵应用于每个因子矩阵。该方法参数数量与模型维度和深度无关，仅取决于固定Tucker秩。

Result: 在RoBERTa-base和RoBERTa-large模型上的GLUE基准测试表明，CRAFT仅使用41K Tucker适应参数（与模型维度和深度无关）即达到与现有方法竞争的性能。

Conclusion: CRAFT通过跨层Tucker分解预训练权重并冻结因子，仅训练轻量适应矩阵，实现了参数高效微调，在保持性能的同时显著减少可训练参数，为PEFT方法提供了新思路。
...
TL;DR: 提出一种灰盒方法，将不完整物理模型直接集成到生成模型中，通过流匹配框架学习观测轨迹的动力学，无需真实物理参数，避免神经ODE的可扩展性和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型（如流匹配和扩散模型）能学习复杂分布和动力系统，但通常作为黑盒忽略底层物理；而基于物理的ODE/PDE模拟模型保持可解释性，但可能存在缺失或未知项，无法完全描述真实观测。需要弥合这一差距。
...
Conclusion: 该方法成功将不完整物理模型集成到生成模型中，实现了从观测轨迹中学习动力学，无需真实物理参数，在保持可解释性的同时达到或超过数据驱动方法的性能。
...
### [169] [The Anxiety of Influence: Bloom Filters in Transformer Attention Heads](https://arxiv.org/abs/2602.17526)
*Peter Balogh*
...
<details>
  <summary>Details</summary>
Motivation: 研究Transformer注意力头中是否存在专门用于成员测试（检测token是否在上下文中出现过）的特定模式，探索这些头的工作机制和特性

Method: 在四个语言模型（GPT-2 small/medium/large, Pythia-160M）中识别成员测试头，分析其错误率曲线，使用混淆控制排除伪阳性，比较不同头的容量特性，进行消融实验

Result: 识别出三种真正的成员测试头：L0H1和L0H5为高精度成员过滤器（错误率0-4%），L1H11符合经典Bloom过滤器理论曲线（容量约5位），这些头集中在早期层（0-1层），形成多分辨率系统，具有距离敏感特性，泛化能力强（对任何重复token类型响应比仅重复token高43%）

Conclusion: Transformer中存在专门的成员测试注意力头，这些头形成多分辨率系统，具有Bloom过滤器特性，成员测试功能与更广泛的计算角色共存，通过混淆控制验证了发现的可靠性
...
### [170] [Linear Convergence in Games with Delayed Feedback via Extra Prediction](https://arxiv.org/abs/2602.17486)
*Yuma Fujimoto,Kenshi Abe,Kaito Ariu*

Main category: cs.LG

TL;DR: WOGDA算法在带延迟反馈的双线性博弈中实现线性收敛，额外乐观主义（预测更远的未来奖励）显著加速收敛速度
...
Method: 将加权乐观梯度下降上升法（WOGDA）解释为额外近端点法（EPP）的近似，EPP基于比经典近端点法更远的未来奖励进行更新，分析标准乐观主义（预测下一步奖励）和额外乐观主义（预测更远未来奖励）的效果

Result: 标准乐观主义在延迟m下达到收敛速率exp(-Θ(t/m^5))，而额外乐观主义能容忍更大步长，显著加速收敛至exp(-Θ(t/(m^2 log m)))，实验验证了额外乐观主义驱动的加速收敛
...
### [171] [Position: Evaluation of ECG Representations Must Be Fixed](https://arxiv.org/abs/2602.17531)
*Zachary Berger,Daniel Prakah-Asante,John Guttag,Collin M. Stultz*

Main category: cs.LG

TL;DR: 该立场论文指出当前12导联心电图表示学习的基准测试存在问题，需要改进以确保进展可靠且与临床有意义的目标一致。论文建议扩展评估范围，提出最佳实践，并发现随机初始化编码器在许多任务上能与最先进的预训练方法相媲美。


<details>
  <summary>Details</summary>
Motivation: 当前心电图表示学习领域主要依赖三个公开的多标签基准数据集（PTB-XL、CPSC2018、CSN），这些数据集主要关注心律失常和波形形态标签。然而，心电图实际上编码了更广泛的临床信息。现有的基准测试实践可能导致进展评估不可靠，且与临床实际需求不一致。

Method: 论文提出改进基准测试的方法：1）扩展下游评估范围，包括结构性心脏病和患者水平预测等临床相关目标；2）为多标签、不平衡设置制定评估最佳实践；3）将随机初始化编码器作为合理的基线模型；4）对三种代表性的心电图预训练方法在六个评估设置中进行实证评估。

Result: 应用最佳实践后，文献中关于哪种表示性能最佳的结论发生了变化。令人惊讶的是，随机初始化的编码器配合线性评估在许多任务上能与最先进的预训练方法相匹配。这一发现支持将随机编码器作为合理的基线模型。

Conclusion: 当前心电图表示学习的基准测试实践需要修正，以确保进展评估的可靠性和临床相关性。扩展评估范围、采用最佳实践、以及使用随机编码器作为基线模型是改进方向。这些改变对于推动该领域朝着更有临床意义的方向发展至关重要。
...
### [172] [MASPO: Unifying Gradient Utilization, Probability Mass, and Signal Reliability for Robust and Sample-Efficient LLM Reasoning](https://arxiv.org/abs/2602.17550)
*Xiaoliang Fu,Jiaye Lin,Yangyi Fang,Binbin Zheng,Chaowen Hu,Zekai Shao,Cong Qin,Lu Pan,Ke Zeng,Xunliang Cai*

Main category: cs.LG

TL;DR: MASPO提出了一种质量自适应软策略优化框架，解决了现有RLVR算法在LLM优化中的三个关键挑战：硬截断导致的梯度利用效率低、均匀比率约束忽略令牌分布、以及正负样本信用分配不对称问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习算法（如GRPO）依赖于刚性、均匀且对称的信任区域机制，这些机制与大型语言模型的复杂优化动态存在根本性不匹配。具体存在三个关键挑战：硬截断导致的梯度利用效率低下、均匀比率约束对令牌分布不敏感、以及正负样本信用分配不对称导致的信号可靠性差异。

Method: 提出了质量自适应软策略优化（MASPO）统一框架，包含三个核心组件：1）可微分软高斯门控以最大化梯度效用；2）质量自适应限制器以平衡概率谱上的探索；3）非对称风险控制器以使更新幅度与信号置信度对齐。

Result: 广泛的评估表明，MASPO作为一个稳健的一体化RLVR解决方案，显著优于强基线方法。

Conclusion: MASPO通过解决现有RLVR算法的三个关键挑战，提供了一个更适应LLM优化动态的强化学习框架，在性能上取得了显著提升。
...
### [173] [Retrospective In-Context Learning for Temporal Credit Assignment with Large Language Models](https://arxiv.org/abs/2602.17497)
...
<details>
  <summary>Details</summary>
Motivation: 自采样数据和稀疏环境反馈的学习是训练自进化智能体的基本挑战。传统基于任务特定价值函数的信用分配方法样本效率低、泛化能力有限

Method: 提出回顾式上下文学习(RICL)，利用LLM预训练知识将稀疏奖励转化为优势函数；进一步提出在线学习框架RICOL，基于RICL的信用分配结果迭代优化策略

Result: RICL能用有限样本准确估计优势函数，有效识别环境中的关键状态；在四个BabyAI场景中，RICOL达到与传统在线RL算法相当的收敛性能，但样本效率显著更高

Conclusion: 研究展示了利用LLM进行时序信用分配的潜力，为更样本高效和可泛化的RL范式开辟了新途径
...
### [174] [Be Wary of Your Time Series Preprocessing](https://arxiv.org/abs/2602.17568)
*Sofiane Ennadir,Tianze Wang,Oleg Smirnov,Sahar Asadi,Lele Cao*

Main category: cs.LG

TL;DR: 本文首次从理论角度分析了不同归一化策略（实例归一化和全局归一化）对时间序列Transformer模型表达能力的影响，提出了专门的时间序列表达能力框架，并发现归一化选择对模型性能有显著影响，但没有单一方法始终最优。


<details>
  <summary>Details</summary>
Motivation: 归一化和缩放是时间序列建模的基本预处理步骤，但其在基于Transformer的模型中的作用从理论角度仍未被充分探索。本文旨在填补这一空白，分析不同归一化策略如何影响时间序列表示学习的表达能力。

Method: 提出了专门针对时间序列的新的表达能力框架，用于量化模型在表示空间中区分相似和不相似输入的能力。使用该框架推导了两种广泛使用的归一化方法（标准化和最小-最大缩放）的理论边界。通过多个基于Transformer的模型在分类和预测基准上进行实证验证。

Result: 分析表明归一化策略的选择能显著影响模型的表示能力，具体取决于任务和数据特性。实证结果显示没有单一归一化方法始终优于其他方法，在某些情况下完全省略归一化反而能获得更好的性能。

Conclusion: 预处理在时间序列学习中起着关键作用，需要针对特定任务和数据集开发更有原则的归一化策略。归一化选择应根据具体应用场景和数据特性进行定制化考虑。
...
TL;DR: VCPO通过控制策略梯度方差，解决异步强化学习中REINFORCE/GRPO方法因重要性采样比尾部过重导致的高方差和不稳定问题，显著提升异步训练鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 异步强化学习训练能提高端到端吞吐量，但广泛使用的无评论家策略梯度方法（如REINFORCE和GRPO）在高异步性下会产生高方差的策略梯度估计器：训练过时的rollout会产生尾部过重的重要性采样比，导致少数样本主导更新，使梯度噪声大且学习不稳定。
...
Conclusion: 显式控制策略梯度方差是实现大规模可靠异步强化学习的关键，VCPO通过有效样本量调节和最小方差基线有效解决了异步训练中的高方差问题。
...
### [176] [Reverso: Efficient Time Series Foundation Models for Zero-shot Forecasting](https://arxiv.org/abs/2602.17634)
*Xinghong Fu,Yanhong Li,Georgios Papaioannou,Yoon Kim*

Main category: cs.LG

TL;DR: 提出Reverso系列高效时间序列基础模型，通过小型混合架构（长卷积+线性RNN）实现与大型Transformer相当的性能，但参数量减少百倍以上，显著提升性能-效率帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型趋向大型化（数亿参数），虽然性能良好但实际使用效率低、成本高。需要探索更高效的架构替代大型Transformer，在保持零样本预测性能的同时大幅减小模型规模。

Method: 采用小型混合架构：交织长卷积层和线性RNN层（特别是DeltaNet层），替代大型Transformer。结合多种数据增强和推理策略进一步提升性能。

Result: Reverso系列模型在零样本时间序列预测任务中，性能与大型Transformer模型相当，但参数量减少超过100倍，显著提升了性能-效率的帕累托前沿。

Conclusion: 大型Transformer对时间序列基础模型并非必需，小型混合架构（长卷积+线性RNN）能够以极小的参数量实现同等性能，为高效时间序列基础模型提供了新方向。
...
### [177] [FAMOSE: A ReAct Approach to Automated Feature Discovery](https://arxiv.org/abs/2602.17641)
*Keith Burghardt,Jienan Liu,Sadman Sakib,Yuning Hao,Bo Li*

Main category: cs.LG

TL;DR: FAMOSE：首个基于ReAct范式的自动化特征工程智能体框架，在回归和分类任务上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 特征工程是机器学习中的关键瓶颈，传统方法需要大量领域专业知识，且从指数级特征空间中识别最优特征极具挑战性。需要自动化解决方案来降低对专家知识的依赖。

Method: 提出FAMOSE框架，利用ReAct范式自主探索、生成和优化特征，将特征选择和评估工具集成到智能体架构中。这是首个将智能体ReAct框架应用于自动化特征工程的方法。

Result: 在分类任务上达到或接近SOTA（尤其对超过10K实例的任务，ROC-AUC平均提升0.23%）；在回归任务上达到SOTA（RMSE平均降低2.0%），且比其他算法更稳健。ReAct机制使LLM能通过迭代特征发现和评估步骤记录有效/无效特征，类似few-shot提示，引导LLM发明更创新特征。

Conclusion: AI智能体在需要高度创新解决方案的问题（如特征工程）上表现出色，FAMOSE为自动化特征工程提供了有效框架，证明了智能体方法的潜力。
...
### [178] [Revisiting Weight Regularization for Low-Rank Continual Learning](https://arxiv.org/abs/2602.17559)
*Yaoyue Zheng,Yin Zhang,Joost van de Weijer,Gido M van de Ven,Shaoyi Du,Xuetao Zhang,Zhiqiang Tian*

Main category: cs.LG

TL;DR: EWC-LoRA：将弹性权重巩固（EWC）正则化应用于低秩适配器的参数高效持续学习新方法，通过正则化共享低秩更新来缓解任务干扰，保持存储和推理成本恒定。


<details>
  <summary>Details</summary>
Motivation: 在参数高效持续学习（PECL）范式中，虽然任务特定模块（如低秩适配器）被广泛使用，但权重正则化技术（如EWC）在这一新范式下仍未得到充分探索。研究者希望重新审视低秩持续学习中的权重正则化，为缓解PECL中的任务干扰提供新视角。

Method: 提出EWC-LoRA方法，通过EWC正则化共享的低秩更新来缓解任务干扰。该方法利用低秩表示在全维空间中估计参数重要性，保持存储需求和推理成本不随任务数量增加而变化。与现有低秩CL方法不同，该方法不是为每个任务分配特定模块，而是通过正则化共享更新来实现。

Result: 在各种基准测试上的广泛实验表明，EWC-LoRA在稳定性-可塑性权衡方面优于现有的低秩持续学习方法。结果表明，即使在低秩参数化下，权重正则化仍然是缓解任务干扰的有效机制。

Conclusion: EWC-LoRA为参数高效持续学习提供了一个实用、计算和内存高效的解决方案，并为PECL中正则化技术的更广泛应用提供了见解。该方法证明了在低秩参数化下，权重正则化仍然是缓解任务干扰的有效机制。
...
### [179] [MARS: Margin-Aware Reward-Modeling with Self-Refinement](https://arxiv.org/abs/2602.17658)
*Payel Bhattacharjee,Osvaldo Simeone,Ravi Tandon*

Main category: cs.LG

TL;DR: MARS提出了一种自适应、边缘感知的数据增强和采样策略，针对奖励模型的不确定性和失败模式进行增强，通过聚焦低边缘（模糊）偏好对来提高奖励建模的鲁棒性。
...
Method: MARS框架采用自适应、边缘感知的增强和采样策略，专门针对奖励模型的模糊和失败模式。该方法将增强集中在奖励模型最不确定的低边缘（模糊）偏好对上，并通过硬样本增强迭代优化训练分布。

Result: 理论保证显示该策略增加了损失函数的平均曲率，从而增强了信息量和改善了条件性。实证结果表明，与均匀增强相比，MARS在鲁棒奖励建模方面取得了持续增益。

Conclusion: MARS通过针对性地增强奖励模型最不确定的样本，提供了一种更有效的奖励模型训练方法，解决了现有增强方法忽视模型估计难度的局限性，为对齐流程提供了更可靠的数据增强策略。
...
### [180] [Canonicalizing Multimodal Contrastive Representation Learning](https://arxiv.org/abs/2602.17584)
...
<details>
  <summary>Details</summary>
Motivation: 探索独立训练的多模态对比模型之间是否存在系统性的几何关系，特别是这种关系是否在不同模态间保持一致，这对于模型升级兼容性和表示隐私有重要意义。

Method: 理论证明：如果两个模型在锚点集上的多模态核函数近似相等，则它们必然通过单一正交矩阵Q相关联，且该矩阵同时映射图像和文本编码器。实验验证了CLIP、SigLIP、FLAVA等模型家族中的这一现象。

Result: 实验表明，不同架构、不同数据分布训练的多模态对比模型之间确实存在正交映射关系（考虑全局均值偏移），同一正交矩阵Q能同时对齐图像编码器（f→f̃）和文本编码器（g→g̃）。

Conclusion: 多模态对比模型的嵌入空间之间存在系统性的正交对应关系，这一发现支持向后兼容的模型升级（避免重新嵌入计算），并对学习表示的隐私性有重要启示。
...
### [181] [Guarding the Middle: Protecting Intermediate Representations in Federated Split Learning](https://arxiv.org/abs/2602.17614)
*Obaidullah Zaland,Sajib Mistry,Monowar Bhuyan*

Main category: cs.LG

TL;DR: 提出KD-UFSL方法，结合k-匿名化和差分隐私保护U型联邦分割学习中的中间数据隐私，在保持模型效用的同时显著降低数据重建攻击风险


<details>
  <summary>Details</summary>
Motivation: 大规模异构数据分布式场景需要可扩展的隐私保护学习方法。U型联邦分割学习（UFSL）虽然将部分计算卸载到服务器，但客户端与服务器共享的中间表示（粉碎数据）容易暴露客户端隐私数据，存在数据重建攻击风险

Method: 提出k-匿名差分隐私UFSL（KD-UFSL），结合微聚合（k-匿名化）和差分隐私技术，保护从客户端传输到服务器的中间数据表示。首先展示攻击者如何通过数据重建攻击获取隐私数据，然后提出隐私增强解决方案

Result: 实验表明，KD-UFSL在某些情况下将实际图像与重建图像之间的均方误差增加高达50%，结构相似性降低高达40%（在四个基准数据集上）。更重要的是，KD-UFSL在保护隐私的同时保持了全局模型的效用

Conclusion: KD-UFSL适用于需要平衡隐私和效用的大规模大数据应用，通过隐私增强技术有效降低U型联邦分割学习中的数据泄漏风险
...
<details>
  <summary>Details</summary>
Motivation: 现代大数据系统产生大规模、异构、地理分散且隐私敏感的流数据，集中化处理困难。联邦学习虽然提供隐私增强的训练机制，但假设静态数据流且需要多轮学习，在有限通信场景下难以处理增量数据。需要解决通信开销和灾难性遗忘的双重挑战。

Method: 提出一次性增量联邦学习(OSI-FL)框架：1) 客户端使用冻结的视觉语言模型(VLM)生成类别特定嵌入，仅需单轮通信；2) 服务器端使用预训练扩散模型根据嵌入合成类似客户端数据分布的新数据；3) 引入选择性样本保留(SSR)机制，基于样本损失识别并保留每个类别-任务对中最具信息量的top-p样本，以限制灾难性遗忘。
...
### [183] [A.R.I.S.: Automated Recycling Identification System for E-Waste Classification Using Deep Learning](https://arxiv.org/abs/2602.17642)
*Dhruv Talwar,Harsh Desai,Wendong Yin,Goutam Mohanty,Rafael Reveles*

Main category: cs.LG

TL;DR: 开发低成本便携式电子垃圾分选系统A.R.I.S.，采用YOLOx模型实时识别金属、塑料和电路板，提升回收效率
...
Method: 采用A.R.I.S.（自动化回收识别系统），这是一个低成本便携式电子垃圾分选器，使用YOLOx模型对金属、塑料和电路板进行实时分类，实现低推理延迟和高检测精度

Result: 实验评估显示总体精度达90%，平均精度均值(mAP)为82.2%，分选纯度达84%，通过深度学习与传统分选方法结合提升了材料回收效率

Conclusion: A.R.I.S.系统通过集成深度学习与现有分选方法，提高了材料回收效率，降低了先进回收技术的采用门槛，支持产品生命周期延长、以旧换新和回收计划，减少供应链环境影响
...
### [184] [Multi-Round Human-AI Collaboration with User-Specified Requirements](https://arxiv.org/abs/2602.17646)
*Sima Noorani,Shayan Kiyani,Hamed Hassani,George Pappas*

Main category: cs.LG

TL;DR: 提出一个基于反事实伤害和互补性原则的框架，通过用户定义规则确保多轮对话AI不会削弱人类优势并能在人类易错处提供价值，提供具有有限样本保证的在线算法，并在医疗诊断和图像推理任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 随着人类在高风险决策中越来越依赖多轮对话AI，需要原则性框架确保这种交互能可靠提升决策质量。当前缺乏系统方法保证AI不会削弱人类优势或能在人类易错处提供价值。

Method: 采用以人为中心的视角，基于两个原则：反事实伤害（确保AI不削弱人类优势）和互补性（确保AI在人类易错处提供价值）。通过用户定义规则形式化这些概念，引入具有有限样本保证的在线、无分布算法，在协作动态中强制执行用户指定的约束。

Result: 在两个交互设置中评估：医疗诊断任务的LLM模拟协作和图像推理任务的人类众包研究。在线程序即使在非平稳交互动态下也能维持规定的反事实伤害和互补性违反率。收紧或放松这些约束会产生下游人类准确率的可预测变化。

Conclusion: 反事实伤害和互补性两个原则可作为实际杠杆，引导多轮协作走向更好的决策质量，无需建模或约束人类行为。用户定义规则允许针对特定任务定制伤害和互补性的含义，在线算法提供统计保证。
